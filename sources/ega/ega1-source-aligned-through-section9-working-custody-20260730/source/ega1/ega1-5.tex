...
But, for every $y\in Y$, there is a neighbourhood $V$ of $y$ in $Y$ such that $\sh{G}|V$ is isomorphic to $\sh{F}|V\oplus\sh{O}_Y|V$ \sref[0]{0.5.4.9};
...
Since $Y(Z)_Y$ consists of a single element, this is equivalent to saying that $X(Z)_Y$ has at most one element, or, equivalently, that the diagonal map from $X(Z)_Y$ to $X(Z)_Y\times X(Z)_Y$ is a bijection.
...
\label{I.5.5.9}
For a morphism $f:X\to Y$ to be separated, it is necessary and sufficient for the induced prescheme $f^{-1}(U)$ to be separated, for every open subset $U$ on which $Y$ induces a separated prescheme, and it is sufficient for it to be the case for every affine open subset $U\subset Y$.
\end{corollary}
