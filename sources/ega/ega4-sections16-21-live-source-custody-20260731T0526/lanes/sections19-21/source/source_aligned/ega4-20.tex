...
When $\sh{F}$ is strictly torsion-free, so that $\sh{F}$ is identified by \sref{IV.20.1.5.1} with a subsheaf of $\sh{M}_X(\sh{F})$, saying that $u$ is defined on $U$ means that $u|U$ is a \emph{section of $\sh{F}$ over $U$}.
\end{env}
...
indeed, the injection $j$ is then a quasi-compact and quasi-separated morphism \sref[I]{I.2.2,I.7.4}.
...
then $\Gamma_\omega$ exists and is none other than the \emph{reduced} closed sub-prescheme of $X\times_S Y$ having for underlying space the closure in $X\times_S Y$ of the underlying space of $\Gamma_u$ \sref[I]{I.5.2.1,I.5.2.2}.
