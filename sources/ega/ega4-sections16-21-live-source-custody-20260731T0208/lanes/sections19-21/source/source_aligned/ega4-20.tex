...
In the n\textsuperscript{os}\sref{IV.21.13} and \sref{IV.21.14}, also independent of the previous three, we study a useful variant of the notion of a local factorial ring, that of a local \emph{parafactorial} ring, which is introduced in particular~\cite{IV-41} in the development of the comparison theorem of the Picard group of a projective prescheme $X$ over a field $k$ and a ``hyperplane section''.
...
it is immediate that it is a sheaf (and not only a presheaf), the regularity of a section of $\sh{O}_X$ over $U$ being checked ``fibre by fibre'' (i.e. meaning that the germ of the section in $x$ is regular in $\sh{O}_X$ for all $x\in U$), in other words $\sh{S}(\sh{O}_X)_x$ is none other than the set of regular elements of $\sh{O}_{X,x}$.
...
their class is called the \emph{restriction} to $V$ of the pseudo-morphism $\omega$, class of $u$, and we denote this pseudo-morphism $\omega|U$.
...
If $V$ is a \emph{schematically dense} open in $X$, then for every open $U$ of $X$, $U\cap V$ is also schematically dense in $X$, so the map $\omega\mapsto\omega|V$ is a \emph{bijection} of $\operatorname{Ps.hom}(X,Y)$ (resp.\ $\operatorname{Ps.hom}_S(X,Y)$) onto $\operatorname{Ps.hom}(V,Y)$ (resp.\ $\operatorname{Ps.hom}_S(V,Y)$).
\end{env}
...
\label{IV.20.2.5}
Let $S$ be an $S_0$-schema, $X$, $Y$ two $S$-preschemes such that the composed morphism $Y\to S\to S_0$ is separated (which implies \emph{(\sref[I]{I.5.5.1})} that $Y\to S$ is also).
For every pseudo-$S$-morphism $\omega$ of $X$ in $Y$, one then has $\operatorname{dom}_S(\omega)=\operatorname{dom}_{S_0}(\omega)$.
In particular, if $Y$ is a schema, one has $\operatorname{dom}_S(\omega)=\operatorname{dom}(\omega)$.
\end{corollary}
...
\]
\oldpage[IV-4]{235}
We can write $S^{-1}A=\varinjlim A_t$, where $t$ runs through the set of \emph{regular} elements of $A$, ordered by the relation ``$t$ is a divisor of $t'$'' $(\mathbf{0_I}$, 1.4.5);
...
\begin{enumerate}[label=(\roman*)]
  ...
  For example, if $X=\Spec(A)$, where $A$ is a Noetherian ring such that $\operatorname{Ass}(A)$ contains embedded prime ideals (which implies that $A$ is not reduced),
  \oldpage[IV-4]{236}
  where $S$ is the set of regular elements of $A$, complementary of the union of the ideals $\mathfrak{p}\in\operatorname{Ass}(A)$;
  on the other hand, $R(X)$ is the complementary of the union of the \emph{minimal} ideals of $A$ \sref[I]{I.7.1.9}, and the canonical homomorphism $A\to Q^{-1}A$ (and \emph{a fortiori} $S^{-1}A\to Q^{-1}A$) is therefore not injective, since there exists in $A-Q$ elements $\neq 0$ of $A$ annihilated by elements of $Q$ (Bourbaki, \emph{Alg.~comm.}, chap.~IV, \textsection1, n\textsuperscript{o}~1, cor.~2 of prop.~1).
  ...
  but as $U$ is an integral scheme, $\sh{M}_X|U=R(U)$ is a simple sheaf, so $\sh{O}_U$ is not a simple sheaf since $\dim(A)\geq 2$.

  It remains to give an example of a ring $A$ having the above properties.
  It suffices to consider an integral local ring $B$ of dimension $\geq 2$ and residue field $k$, and to take $A=B\otimes k$ with the law of multiplication $(b,x)(b',x')=(bb',bx'+b'x)$.
...
\begin{env}[20.2.14]
\label{IV.20.2.14}
Let $X$ be a prescheme, $\sh{F}$ an $\sh{O}_X$-Module quasi-coherent and \emph{strictly torsion-free} \sref{IV.20.1.5}, so that $\sh{F}$ identifies via \sref{IV.20.1.5.1} with a sub-$\sh{O}_X$-Module of $\sh{M}_X(\sh{F})$.
...
to say that a section $f\in\Gamma(U,\sh{O}_X)$ belongs to $\Gamma(U,\sh{J})$ means that $f(u|U)\in\Gamma(U,\sh{F})$, and since $s|U$ is a regular element of $\Gamma(U,\sh{O}_X)$ and $\sh{F}$ is strictly torsion-free, the preceding relation is equivalent to $f((su)|U)\in\Gamma(U,s\sh{F})$;
...
Note that the only schematically dense opens in $X$ are those containing $x'$ and $x''$ \sref{IV.20.2.13}[(iv)], and that $U=U'\cap U''$.
...
As $V'$ and $V''$ contain the generic point of $Z$, and $V'\cap V''$ are distinct, but the restrictions of $u'$ and $u''$ to $U-(U\cap Z)$ are equal, and this is a dense open in $U$, hence schematically dense since $U$ is reduced.
...
then we say that the class (for the equivalence relation of \sref{IV.20.2.1}) of the $S$-morphism $u\circ(f|U')$ (class which is defined by virtue of the preceding hypothesis) is the pseudo-$S$-morphism \emph{composed} of $\omega$ and $f$, and we denote it $\omega\circ f$;
...
the conclusion therefore results from the hypothesis that the image by $f$ of every maximal point of $X'$ belonging to $f^{-1}(V)$ is a maximal point of $X$ belonging to $U$, hence to $V$ since the set of irreducible components of $X$ is locally finite.
\end{proof}
...
For every pseudo-$S$-morphism $\omega$ of $X$ in $Y$, the composed is therefore defined and is a pseudo-$S$-morphism of $\Spec(\sh{O}_{X,x})$ in $Y$, called the \emph{restriction} of $\omega$ to $\Spec(\sh{O}_{X,x})$.
...
\label{IV.20.3.6.1}
  ...
  \tag{20.3.6.1}
\]
where the left-hand side is the fibre at the point $x$ of the sheaf $\mathscr{P}\!s.\!\operatorname{hom}_S(X,Y)$, the set of germs at $x$ of pseudo-$S$-morphisms of open neighbourhoods of $x$ in $Y$.
\end{env}
...
\begin{enumerate}[label=\emph{(\roman*)}]
  ...
  \item The $X_\lambda$ are reduced, the set of irreducible components of $X$ is locally finite, and, for $\lambda\leq\mu$, every irreducible component of $X_\mu$ dominates an irreducible component of $X_\lambda$.
\end{enumerate}
...
\]
are defined (without hypothesis of separation on the $Y_\lambda$ or $Y$);
the same is true in case~(ii) by the same reasoning.
The proposition will result from the following lemmas.
...
Then, for $U$ to be schematically dense in $X$, it is necessary and sufficient that there exist $\lambda\geq\alpha$ such that $U_\lambda$ is schematically dense in $X_\lambda$, and in this case $U_\mu$ is schematically dense in $X_\mu$ for $\mu\geq 1$.
\end{lemma}

We first verify condition~(i);
denote by $j_\lambda:U_\lambda\to X_\lambda$ and $j:U\to X$ the canonical injections, by $\sh{J}_\lambda$ the kernel of the canonical homomorphism $\sh{O}_{X_\lambda}\to(j_\lambda)_*(\sh{O}_{U_\lambda})$, and $\sh{J}_\lambda$ is coherent, of finite type, hence it results from \sref{IV.2.3.1} that one can identify $(j_\lambda)_*(\sh{O}_{U_\lambda})$ to $p^*_\lambda((j_\lambda)_*(\sh{O}_{U_\lambda}))$ (resp.\ $j_*(\sh{O}_U)$ to $(j_\lambda)_*(\sh{O}_{U_\lambda}))$.
The assertion results then from the definition of a schematically dense open, and of \sref{IV.8.5.8}[(ii)].

To establish \sref{IV.20.3.8.2} when condition~(ii) is verified, and noting that $X$ is then \emph{reduced} by virtue of \sref{IV.8.7.1};
it amounts to the same to say that $U_\lambda$ (resp.\ $U$) is schematically dense in $X_\lambda$ (resp.\ $X$) or that it is dense in $X_\lambda$ (resp.\ $X$), and the conclusion results from \sref{IV.20.3.8.4} applied to $X_\lambda$ and~$X$.
...
\label{IV.20.3.8.4}
The hypotheses being those of \sref{IV.20.3.8.3}, suppose moreover $S_\alpha$ quasi-compact, and let $V_\alpha$ be an open of $S_\alpha$, and $V$ its inverse images in $S_\lambda$ and $S$.
...
\oldpage[IV-4]{241}
\emph{Proof of \sref{IV.20.3.8.5} (end of proof of \sref{IV.20.3.8})}.
...
\emph{Proof of \sref{IV.20.3.8.6} (injectivity of \sref{IV.20.3.8.1})}.
...
Then there exists $\lambda\geq\alpha$ tel such that $V$ is the inverse image of a quasi-compact open $V_\lambda$ of $X_\lambda$ \sref{IV.8.2.11}, and by \sref{IV.20.3.8.2} we can suppose that $V_\lambda$ is schematically dense in $X_\lambda$.
It results from \sref{IV.8.8.2}[(i)] that taking $\lambda$ sufficiently large, $u'_\lambda$ and $u''_\lambda$ coincide in $V_\lambda$, hence belong to the same pseudo-homomorphism.
...
\begin{enumerate}[label=(\roman*)]
  ...
  it results from the definition \sref{IV.17.4.5} and from $(\mathbf{I}$, 3.3.10.1) that $Z$ is the projective limit of the $Z_\mu$ for $\mu\geq\lambda$.
...
\end{lemma}
  ...
  This lemma being established, we conclude that for $\mu\geq\lambda$ sufficiently large, $Z_\mu$ is induced on an open of $X_\mu$ by virtue of \sref{IV.8.6.3}, and $Z_\lambda$ en tant que sub-prescheme of a Noetherian prescheme, is of finite presentation \sref{IV.1.6.2}, and so it is the same for $Z_\mu$ for $\mu\geq\lambda$ on $X_\mu$ and of $Z$ on $X$.
  One can now apply \sref{IV.20.3.8.2} which shows that for $\mu\geq\lambda$ sufficiently large, $Z$ is schematically dense in $X$, whence the conclusion.
...
\label{IV.20.3.9}
Suppose verified either hypothesis~a), b) of \sref{IV.20.3.6} on $X$, and let $Y$ locally of finite presentation over $S$.
...
\begin{lemma}[20.3.10]
\label{IV.20.3.10}
Let $f:X'\to X$ be an $S$-morphism faithfully flat and quasi-compact, $X''=X'\times_X X'$, $p_1$ and $p_2$ the canonical projections of $X''$ in $X'$, $Y$ an $S$-prescheme separated on $S$.
Let $U$ be an open of $X$, $\mathrm{U}'=f^{-1}(\mathrm{U})$, $\mathrm{U}''=p_1^{-1}(\mathrm{U}')=p_2^{-1}(\mathrm{U}')$, and suppose that $\mathrm{U}''$ is schematically dense in $X''$.
Let $g:U\to Y$ be an $S$-morphism;
then, if $g\circ(f|\mathrm{U})$ extends to an $S$-morphism $X'\to Y$, $g$ extends to an $S$-morphism $X\to Y$.
\end{lemma}

\begin{proof}
To prove \sref{IV.20.3.10}, denote by $g'$ an $S$-morphism $X'\to Y$ which extends $g\circ(f|\mathrm{U})$, and set $g''_i=g'\circ p_i:X''\to Y$ ($i=1,2$).
If one sets $f''=f\circ p_1=f\circ p_2:X''\to X$, it is clear that $g''_1$ and $g''_2$ coincide in $\mathrm{U}''$ with $g\circ(f''|\mathrm{U}'')$.
But as $Y$ is separated on $S$ and $\mathrm{U}''$ schematically dense in $X''$, we have $g''_1=g''_2$ \sref{IV.11.10.1}[, d)].
...
as the restriction $\mathrm{U}'\to\mathrm{U}$ of $f$ is a faithfully flat and quasi-compact morphism and $\mathrm{U}''=\mathrm{U}'\times_\mathrm{U}\mathrm{U}'$, the result of uniqueness applied to $\mathrm{U}$ instead of $X$, shows that $h|\mathrm{U}=g$, which proves the lemma.
\end{proof}
...
This is the hypothesis in case~(i), and of \sref{IV.20.2.12} in case~(iii), since in an affine scheme every open of the form $\mathrm{D}(t)$ is retrocompact;
in case~(ii), we can directly apply \sref{IV.20.3.5}[(i)].

Let us now prove \sref{IV.20.3.11.1}, i.e., that, for every point $x'\in\operatorname{dom}_S(\omega\circ f)$, on has $x=f(x')\in\operatorname{dom}_S(\omega)$.
Note first that we can restrict to the case where $f$ is \emph{faithfully flat and quasi-compact}.
This is indeed the second case of~(i) and in case~(ii) $f$ is an open morphism \sref{IV.11.3.1}, so replacing $X$ by the open $f(X')$, we may suppose $f$ surjective, hence faithfully flat.
...
Then, for every morphism flat and locally of finite presentation $f:X'\to X$, the pseudo-function $\varphi\circ f$ is defined and one has $\operatorname{dom}(\varphi\circ f)=f^{-1}(\operatorname{dom}(\varphi))$.
\end{corollary}
...
We note that $\Gamma_\omega$ is defined if $X$ is locally Noetherian;
it will also be defined when $X$ is \emph{reduced}, and it is then none other than the \emph{reduced} closed sub-prescheme of $X\times_S Y$ having for underlying space the adherence in $X\times_S Y$ \sref[I]{I.5.2.1} and \sref{IV.5.2.2}.
Note finally that if $Y$ is \emph{separated} on $S$, then $\Gamma_u$ is \emph{closed} in $U\times_S Y$ \sref[I]{I.5.4.3}, hence $\Gamma_\omega$ (whenever it exists) is the open $\Gamma_\omega\cap(U\times_S Y)$ \sref[I]{I.9.5.10};
...
from the class of $v$ (whose rational application associated \sref{IV.20.2.13}[(ii)] is then \emph{birational});
as $p^{-1}(\operatorname{dom}_S(\omega))$ is the graph of the $S$-morphism $u=q\circ v:\operatorname{dom}_S(\omega)\to Y$, it is schematically dense in $\Gamma_\omega$ \sref{IV.11.10.3}[(iv)], so $\omega$ can be considered as the \emph{composed} $q\circ p^{-1}$ \sref{IV.20.3.2}.
For every part $M$ of the underlying space of $\Gamma_\omega$ set $\omega(M)=q(p^{-1}(M))$, which amounts to considering $\omega$ as a ``multi-valued function''.
...
It is then said to be \emph{defined} at $x$ \sref{IV.20.4.1}.
\end{env}
...
Then $\omega\circ f$ is defined, and one has $\operatorname{dom}_S(\omega\circ f)=f^{-1}(\operatorname{dom}(\omega))$ (in other words, for $x'\in X'$ and $x=f(x')$, so that $\omega$ is defined at $x$, it suffices that $\omega\circ f$ is at $x'$).
\end{proposition}

\begin{proof}
\oldpage[IV-4]{246}
As $X'$ is reduced and $f^{-1}(\operatorname{dom}_S(\omega))$ is everywhere dense in $X'$ by virtue of \sref{IV.2.4.11}, the composite $\Gamma_{\omega\circ f}$ is a reduced prescheme \sref{IV.20.4.1}, and as by hypothesis the composed morphism $\Gamma_{\omega\circ f}\to\Gamma_\omega\times_X X'$
is an isomorphism, $p_{(X')}$ is surjective;
as $f$ is surjective, it suffices to apply \sref{IV.20.4.4} that $\omega$ is defined at $x$.
\end{proof}
...
we note that then $Y$ is locally Noetherian, and we can evidently (since $X$ is locally integre and locally Noetherian) suppose $X=S$ is irreducible.
...
Note that for every $x'\in X\cap U$, $h(x')$ is a specialisation of $h'_x(x)=y$.
\oldpage[IV-4]{248}
As $\Gamma_\omega$ is formed of the adherences of $h(U)$ and $h(U)$ formed of points adherent to $h(\xi)$, where $\xi$ is the generic point of $X$, $\Gamma_\omega$ is formed of adherences of $h(\xi)$;
indeed there exists a spectrum $S_1$ of a discrete valuation ring and a morphism $u:S_1\to X$ such that $u(a)=x$, $u(b)=\xi$ $(\mathbf{II}$, 7.1.9), and we set $v=p\circ u$, so that $v(a)=z$, $v(b)=h(\xi)$.
Applying the condition of the statement to $u$, we see that one obtains a morphism $h'_1:S_1\to Y$ such that $h'_1(a)=y$ and $h'_1(b)=h(\xi)$;
but this means $z=y$ by virtue of $(\mathbf{II}$, 7.2.3), since $Y$ is separated on $X$ and $v$ and $h'_1$ must coincide, since they are equal at $b$.
C.Q.F.D.
\end{proof}
...
Since $V_i$ is separated on $X$ and reduced, for two points $x'$, $x''$ of $E_i$, $h^{(x')}$ and $h^{(x'')}$ coincide in $W^{(x')}\cap W^{(x'')}$ since the preceding morphisms coincide in an open everywhere dense $U\cap W^{(x')}\cap W^{(x'')}$ of $U_i\cap U_j$, they are equal.
The morphism $h'$, equal to $h$ in $U$, to $h_i$ in each $U_i$, answers the question.
\end{proof}

\begin{remark}[20.4.8]
\label{IV.20.4.8}
When $E=X-U$, it is clear that if, for every open affine $T$ of $X$, there exists an $S$-morphism $h'_T:T\to Y$ extending $h|(U\cap T)$ and such that for every $x\in T-(U\cap T)$, the $h'_T$ are the restrictions of an $S$-\emph{morphism} $h':X\to Y$ extending $h$ (everywhere defined) by the assertion of uniqueness in \sref{IV.20.4.6}.
...
\label{IV.20.4.9}
Let $S$ be a prescheme locally Noetherian, $f:X\to S$ a morphism flat, $g:Y\to S$ a morphism locally of finite type.
Let $U$ be an open schematically dense in $S$, $h:f^{-1}(U)\to Y$ an $S$-morphism, $T=S-U$, $Z$ the closed sub-prescheme reduced of $X$ having $f^{-1}(T)$ as underlying space, $h_0:Z\to Y$ an $S$-morphism.
...
Indeed, by hypothesis all the fibres of the projection morphism $X\times_S Y\to X$ are sets of isolated points, and to apply \sref{IV.20.4.4} it suffices to know that the fibre $p^{-1}(x)$ is non-empty, whence the conclusion.
\end{corollary}

\begin{proposition}[20.4.11]
\label{IV.20.4.11}
Let $X$ be an $S$-prescheme locally Noetherian, $Y$ an $S$-prescheme affine on $S$, $Z=X-U$ an open of $X$.
...
As $B$ is quotient of a polynomial algebra $A[(T_\lambda)]_{\lambda\in L}$, $Y$ is a closed sub-prescheme of $Y'=X[(T_\lambda)]_{\lambda\in L}$.
...
indeed, the sub-prescheme $v'^{-1}(Y)$ is closed and majorizes $U$ \sref{IV.20.3.8.8}.
...
on the other hand, given a covering open $(U_\alpha)$ of $U$, and a pseudo-$S$-morphism $\omega$ of $U$ in $Y$, for $\operatorname{dom}_S(\omega)$ to be universally schematically dense in $U$ relatively to $S$, it results immediately from the definitions (cf.\ \sref{IV.20.2.1}) that it suffices that each of the sets $\operatorname{dom}_S(\omega)\cap U_\alpha=\operatorname{dom}_S(\omega|U_\alpha)$ be universally schematically dense in $U_\alpha$ relatively to $S$;
whence~(ii).
\end{proof}
...
\oldpage[IV-4]{251}
for every pseudo-morphism $\omega\in\operatorname{Ps.hom}_{X/S}(X,Y)$, the morphisms $fu$, where $u$ runs through $\omega$, belong therefore to a single equivalence class (for the relation defined in \sref{IV.20.5.1}), which we call the pseudo-morphism of $X$ in $Z$, relatively to $S$, \emph{composed} of $f$ and $\omega$ and denoted $f\omega$.
If $g:Z\to T$ is a morphism, it is immediate that $g\circ(f\omega)=(g\circ f)\omega$.
\end{env}

\begin{env}[20.5.6]
\label{IV.20.5.6}
Suppose $Y$ \emph{separated} on $S$, and let $\omega$ be a pseudo-morphism of $X$ in $Y$ relatively to $S$.
If $f:X'\to X$ is an $S$-morphism such that $f^{-1}(\operatorname{dom}_S(\omega))$ is universally schematically dense in $X'$ \emph{relatively to $S$}, it results from \sref{IV.20.3.2} that the pseudo-$S$-morphism $\omega\circ f$ has a domain universally schematically dense relatively to $S$, hence \sref{IV.20.5.3} may be considered as a pseudo-morphism \emph{relatively to $S$}.
When $X'$ is flat and of locally of finite presentation over $S$, the condition that $f^{-1}(\operatorname{dom}_S(\omega))$ be universally schematically dense in $X'$ relatively to $S$ is equivalent to saying that for every $s\in S$, $(f^{-1}(\operatorname{dom}_S(\omega)))_s$ (notation of \sref{IV.11.10.10}) is schematically dense in $X'_s$.
...
We deduce from this that for every $S$-morphism $g:Y\to Z$, $(g\omega)\circ f$ is still defined and equal to $g\circ(\omega\circ f)$ (with the definitions of \sref{IV.20.5.1}), and that it is therefore a pseudo-morphism \emph{relatively} to $S$.
\end{env}
...
if $u_1$, $u_2$ are $S$-morphisms of the class $\omega$, defined respectively on opens $U_1$, $U_2$ of $X$, universally schematically dense in $X$ relatively to $S$, it results from what precedes that $u'_1=u_1\circ(g|g^{-1}(U_1))$ and $u'_2=u_2\circ(g|g^{-1}(U_2))$ coincide in an open $U'_3$ universally schematically dense relatively to $S'$, hence the $S'$-morphisms $v_1$ and $v_2$ correspond to the same element of $\operatorname{Ps.hom}_{X'/S'}(X',Y')$, which we call the \emph{image reciprocal} of $\omega$ by the base change $S'\to S$ and denote $\omega_{(S')}$.
...
this condition is satisfied \emph{a fortiori} when $\sh{F}$ is strictly torsion-free \sref{IV.20.1.5}.
...
\label{IV.20.6.4}
Let $\sh{F}$ be an $\sh{O}_X$-Module quasi-coherent of finite presentation and strictly torsion-free.
...
Consider the Ideal of denominators $\sh{J}$ of $\varphi$ \sref{IV.20.2.14}, which is quasi-coherent, and which defines the sub-prescheme whose underlying space is $X-\operatorname{dom}(\varphi)$.
By hypothesis, if $s$ is the image of $x$ in $S$, $\operatorname{dom}(\varphi)\cap X_s$ is schematically dense in the prescheme locally Noetherian $X_s$, hence \sref{IV.20.2.13}[(iv)] contains $\operatorname{Ass}(\sh{O}_{X_s})$;
...
But then, since $\sh{F}$ is strictly torsion-free, $t(\varphi|(U\cap\operatorname{dom}(\varphi)))$ is a section $u$ of $\sh{F}$ over $U\cap\operatorname{dom}(\varphi)$;
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item Let $\sh{L}$ be an $\sh{O}_X$-Module \emph{invertible}, and let $s$ be a meromorphic \emph{regular} section of $\sh{L}$ over $X$ \sref{IV.20.1.8};
  ...
  by virtue of~(ii), it is immediate that it suffices that for every $s\in S$, $\varphi_s$ be a regular meromorphic section \sref{IV.20.1.8} of the $\sh{O}_{X_s}$-Module invertible $\sh{L}\otimes_{\sh{O}_S}k(s)$.
  If $\varphi'$ is the inverse of $\varphi$ in $\sh{L}^{-1}$ \sref{IV.20.1.10}, $\varphi'$ is then also regular relatively to $S$, and if $\sh{L}_1$ is a second $\sh{O}_X$-Module invertible, $\varphi_1$ a meromorphic section of $\sh{L}_1$ over $X$, regular relatively to $S$, then $\varphi\otimes\varphi_1$ is a meromorphic section of $\sh{L}\otimes\sh{L}_1$ over $X$, regular relatively to $S$.
\end{enumerate}
\end{remarks}

\begin{proposition}[20.6.6]
\label{IV.20.6.6}
Let $X$ be an $S$-prescheme flat and locally of finite presentation, $S$ an $\sh{O}_X$-Module locally free of finite type;
