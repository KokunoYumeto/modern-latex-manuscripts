...
Furthermore \sref{IV.1.5.4}, we can suppose $X$ and $Y$ reduced, so $A$ and $B$ are integral.
...
Let $\eta_0$ be the generic point of $Y_0$, so that $g$ is the morphism $Y\to Y_0$, we have $g(\eta)=\eta_0$; by transitivity of fibres \sref[I]{I.3.6.4}, we have $f^{-1}(\eta)=f_0^{-1}(\eta_0)\otimes_{k(\eta_0)}k(\eta)$, and as $f^{-1}(\eta)$ is irreducible by hypothesis, so is $f_0^{-1}(\eta_0)$ \sref{IV.4.4.1}.
Our assertion will then follow from the following lemma:
\end{proof}

\begin{lemma}[13.1.2]
\label{IV.13.1.2}
Let $Y_0$, $Y$ be two irreducible preschemes of generic points $\eta_0$, $\eta$, $g:Y\to Y_0$ a dominant morphism, $f_0:X_0\to Y_0$ a dominant morphism such that $f_0^{-1}(\eta_0)$ is irreducible.
Let $X_0'$ be the unique irreducible component of $X_0$ meeting $f_0^{-1}(\eta_0)$ \sref[0]{0.2.1.8}, and let $X_0'$ also denote the closed reduced sub-prescheme of $X_0$ having $X_0'$ as underlying space.
...
On the other hand, $X_0'$ contains the fibre $f_0^{-1}(\eta_0)$, so $X_0'\times_{Y_0}Y$ contains $f^{-1}(\eta)$; note on the other hand that $f^{-1}(\eta)$ is not empty \sref[I]{I.3.4.7}, so it contains the generic point of $X$; by extension the image of $j$ is necessarily $X$ itself.
...
\oldpage[IV-3]{189}
catenary ring \sref{IV.5.6.4}; consequently, for every $y_0\in f_0(X_0)$, all the irreducible components of $f_0^{-1}(y)$ have dimension $\geq e$, since we know that $\dim(f_0^{-1}(\eta_0))=e$ by transitivity of fibres \sref{IV.4.1.4}.
For every $y\in f(X)$, we then have $y_0=g(y)\in f_0(X_0)$ and by transitivity of fibres \sref{IV.4.2.7}, we complete the proof.
...
We know then \sref{IV.8.9.1} that there exists a Noetherian subring $A_0$ of $A$, and an $A_0$-algebra of finite type $B_0$, such that $B=B_0\otimes_{A_0}A$.
Set $Y_0=\Spec(A_0)$, $X_0=\Spec(B_0)$, and let $f_0:X_0\to Y_0$ be the morphism corresponding to the homomorphism $A_0\to B_0$.
Let $g:Y\to Y_0$ be the morphism corresponding to the canonical injection $A_0\to A$, $y$ a point of $Y$, $y_0=g(y)$; we know that $f^{-1}(y)=f_0^{-1}(y_0)\otimes_{k(y_0)}k(y)$, and it follows from \sref{IV.4.2.7} that if $p$ is the canonical projection $f^{-1}(y)\to f^{-1}(y_0)$, the irreducible components of $f^{-1}(y)$ are the irreducible components of the spaces $p^{-1}(Z_0)$, where $Z_0$ runs over the set of irreducible components of $f_0^{-1}(y_0)$, and each of the irreducible components of $p^{-1}(Z_0)$ dominates $Z_0$ and has dimension equal to $\dim(Z_0)$.
...
By considering the set of closed subsets $Y'$ of $Y$ such that the theorem is true for the closed sub-prescheme of $Y$ having $Y'$ as underlying space and for $X'=f^{-1}(Y')$, we can reason by Noetherian recurrence \sref[0]{0.2.2.2}, and suppose that the theorem is true for every closed subset $Y'\neq Y$ of $Y$.
...
If $Z$ is the closed sub-prescheme of $Y$ having $\overline{f(X)}$ as underlying space, $f$ factorises as $X\xrightarrow{g}Z\xrightarrow{j}Y$, where $j$ is the canonical injection of type fini \sref{IV.1.5.4}, so it suffices to prove the theorem for $Z$ and $g$; by the hypothesis of recurrence, we are thus reduced to considering only the case where $Z=Y$, in other words where $Y$ is irreducible and $f$ dominant.
...
\oldpage[IV-3]{190}
II) We now pass to the general case, always supposing $S=\Spec(A)$ and $X=\Spec(B)$ affine, $B$ being an $A$-algebra of finite type, hence of the form $B=A[T_1,\ldots,T_n]/\mathfrak{J}$.
Let $(\mathfrak{J}_\lambda)$ be the family of ideals of finite type of $A[T_1,\ldots,T_n]$ contained in $\mathfrak{J}$, so that $\mathfrak{J}$ is the filtered union of the $\mathfrak{J}_\lambda$; if $X$ and the $X_\lambda=\Spec(A[T_1,\ldots,T_n]/\mathfrak{J}_\lambda)$ are considered as closed sub-preschemes of $Z=\Spec(A[T_1,\ldots,T_n])$, we therefore have, for the underlying spaces, $X=\bigcap X_\lambda$.
If $f_\lambda:X_\lambda\to S$ is the structural morphism, we deduce that $f^{-1}(s)=\bigcap f_\lambda^{-1}(s)$ for every $s\in S$, and as the sets $f_\lambda^{-1}(s)$ are closed in the Noetherian space, there exists a $\lambda$ (depending on $s$) such that $f^{-1}(s)=f_\lambda^{-1}(s)$.
So then, for every $x\in X$, we set $d(x)=\dim_x(f^{-1}(f(x)))$, $d_\lambda(x)=\dim_x(f_\lambda^{-1}(f_\lambda(x)))$; from what precedes we see that we have $d(x)=\inf_\lambda d_\lambda(x)$; the functions $d_\lambda$ being upper semi-continuous by the first part of the proof, so is $d$.
\end{proof}
...
\label{IV.13.1.6}
Let $X$, $Y$ be two irreducible preschemes, $f:X\to Y$ a dominant morphism of finite type.
Let $\xi$ (resp.\ $\eta$) be the generic point of $X$ (resp.\ $Y$) and let $e=\dim(f^{-1}(\eta))$.
...
The converse is true if the two members of the inequality \sref{IV.5.6.5.2} are equal, in particular if $\sh{O}_y$ is universally catenary.
\end{proposition}
...
\begin{env}[13.2.4]
\label{IV.13.2.4}
Let now, in a general fashion, $Y$ be a locally Noetherian prescheme, $f:X\to Y$ a morphism of finite type, $x$ a point of $X$, $X'$ a closed irreducible subset of $X$ containing $x$, $y=f(x)$, $Y'=\overline{f(X')}$, $\eta'$ the generic point of $Y'$.
...
\begin{enumerate}[label=\emph{\alph*)}]
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \]
  \end{enumerate}
  (a condition always implied by (iii) when $A=\sh{O}_{Y,y}$ is a universally catenary ring, by virtue of \sref{IV.5.6.5}).
  \item We have the equality
  \begin{enumerate}[label=(\roman*)]
    \item $\dim(B')=\dim(B)$.
    \item $\dim(B)=\dim(A)+\dim(B\otimes_A k(y))$.
...
\begin{env}[13.2.6]
\label{IV.13.2.6}
Recall now that the irreducible components $X_i$ of $X$ containing $x$ are finite in number \sref{IV.5.1.2.1} and \sref[0]{0.14.2.1.1}
\[
\tag{13.2.6.1}
  \dim(\sh{O}_{X,x})=\sup_i\dim(\sh{O}_{X_i,x}).
\]
The equivalence of conditions $b)$ and $c)$ in \sref{IV.13.2.5} implies therefore, taking into account \sref[0]{0.16.3.9} and \sref[0]{0.14.2.1}:
\end{env}
...
\]
For the two members of \sref{IV.13.2.7.1} to be equal, it is necessary and sufficient that there exists a closed irreducible subset $X'$ of $X$ containing $x$ and satisfying simultaneously the following conditions:
\begin{enumerate}[label=(\roman*)]
...
\label{IV.13.2.8}
If the two members of \sref{IV.13.2.7.1} are equal, the closed irreducible subsets $X'$ of $X$ containing $x$ and satisfying conditions (i) to (iv) of \sref{IV.13.2.7} are exactly the ones for which $\dim(\sh{O}_{X',x})=\dim(\sh{O}_{X,x})$ (which implies that $X'$ is necessarily an irreducible component of $X$).
\end{corollary}
...
As $f^{-1}(y)$ is locally Noetherian, there exists an open neighbourhood $U$ of $x$ in $Z$ such that $U$ meets no irreducible component of $f^{-1}(y)$ other than those containing $x$, hence \sref{IV.4.1.1.3} $\dim_x(f^{-1}(y))=n$ for every $x'\in U$; it is then clear that the conditions (i) to (iii) of \sref{IV.13.2.7} are satisfied when we replace $x$ by any point $x'\in U$, and it follows similarly from condition (iv) since $\sh{O}_{Y,y}$ is universally catenary; whence the conclusion by \sref{IV.13.2.7}.
...
\label{IV.13.2.11}
With the notations being those of \sref{IV.13.2.10}, suppose that $\sh{O}_{Y,y}$ is universally catenary.
For every $y'\in Y$, let $E(y')$ be the set of dimensions of the irreducible components of $f^{-1}(y')$ and set $d(y')=\dim(f^{-1}(y'))=\sup(E(y'))$.
Then, if the conditions $a)$, $b)$, $c)$ of \sref{IV.13.2.10} are satisfied, we have $E(y)\subset E(\eta)$, whence $d(y)\leq d(\eta)$.
\end{corollary}
...
\label{IV.13.2.12}
\begin{enumerate}[label=(\roman*)]
  \item Recall \sref{IV.6.1.2} that the relation \sref{IV.13.2.10.1} is always satisfied when the morphism $f$ is \emph{flat} at the point $x$.
...
\oldpage[IV-3]{195}
  ...
  \item[\emph{a$'$)}] There exist an integer $e$ and an open neighbourhood $U$ of $x$ such that the image by $f$ of every irreducible component of $U$ is dense in some $Y_\alpha$, and such that, if we denote by $y_\alpha$ the generic point of $Y_\alpha$, every irreducible component of the spaces $U\cap f^{-1}(y_\alpha)$, $U\cap f^{-1}(y_\alpha)$ is of dimension $e$.
...
Let us show that $a')$ implies $a'')$; we can restrict to the case where $f$ is of finite type and $X$ is reduced.
Let $U_\lambda$ be an irreducible component of $U$, and suppose that $f(U_\lambda)$ is dense in $Y_\alpha$; by the hypothesis of $a')$, on can (restricting $U$ if necessary) suppose that $f(x_\lambda)$ is one of the generic points $y_\alpha$ of the irreducible components of $Y$ containing $y$; by virtue of \sref[0]{0.2.1.8}, $U_\lambda\cap f_\alpha^{-1}(y_\alpha)$ is the unique irreducible component of $U\cap f^{-1}(y_\alpha)$ containing $x_\lambda$ and is by hypothesis of dimension $e$, equal to the dimensions of all the irreducible components of $U_\lambda\cap f^{-1}(y_\alpha)$; by Chevalley's theorem \sref{IV.13.1.3} and \sref{IV.13.1.1}, the set $V_\lambda$ of $x'\in U_\lambda$ such that $\dim_{x'}(f_\lambda^{-1}(f_\lambda(x)))=e$ is open and contains $x$, and it suffices to take the union of the $V_\lambda$ as open set satisfying the conditions of $a'')$.
...
Furthermore, for any such morphism $g$, there exists an open neighbourhood $U$ of $f^{-1}(y)$ in $X$ such that $g|U$ is quasi-finite.
\end{lemma}

\begin{proof}
Let $\mathfrak{p}=j_y$; the ring $B\otimes_A k(\mathfrak{p})$ is a $k(\mathfrak{p})$-algebra of finite type, hence by the normalisation lemma (Bourbaki, \emph{Alg.\ comm.}, chap.~V, \textsection~3, n$^{\text{o}}$~1, th.~1) there exist in $B\otimes_A k(\mathfrak{p})$ a finite sequence $(t_i)_{1\leq i\leq r}$ of elements algebraically independent over $k(\mathfrak{p})$ and such that if we set $C'=k(\mathfrak{p})[t_1,\ldots,t_r]$, $B\otimes_A k(\mathfrak{p})$ is a $C'$-algebra \emph{finite}; we therefore have $\dim(B\otimes_A k(\mathfrak{p}))=\dim(C')$ \sref[0]{0.16.1.5} and as $\dim(C')=r$ \sref{IV.5.2.1}, we have $r=e$.
...
This lemma being established, it remains to show that if $x_\lambda$ is a maximal point of $U$, $g(x_\lambda)=z_\lambda$ is a maximal point of $Z=Y[T_1,\ldots,T_e]$.
By the hypothesis of $a)$, we can (restricting $U$ if necessary) suppose that $f(x_\lambda)$ is one of the generic points $y_\alpha$ of the irreducible components of $Y$ containing $y$.
Let $u:A[T_1,\ldots,T_e]\to B$ be the homomorphism and $h:U\cap f^{-1}(y_\alpha)\to p^{-1}(y_\alpha)=\Spec(k(y_\alpha)[T_1,\ldots,T_e])$ the quasi-finite morphism deduced from $g$, by virtue of \sref[0]{0.2.1.8}; the maximal points of $Z$ that are images by $p$ of the generic points of the $Y_\alpha$ already prove that the $f(x_\lambda)$ are generic points of the $Y_\alpha$.
Furthermore, if $f(x_\lambda)=y_\alpha$, the $k(y_\alpha)$-morphism $h:U\cap f^{-1}(y_\alpha)\to p^{-1}(y_\alpha)$ deduced from $g$ is dominant and quasi-finite by hypothesis; we therefore have $\dim(U_\lambda\cap f^{-1}(y_\alpha))=e$ ($U_\lambda$ being the irreducible component of $U$ of generic point $x_\lambda$).
Similarly, for every $x'\in U_\lambda$, the morphism $U_\lambda\cap f^{-1}(f(x'))\to p^{-1}(f(x'))$ deduced from $g$ is a $k(f(x'))$-morphism, hence \sref{IV.4.1.2.1} we have $\dim(f^{-1}(f(x'))\cap U_\lambda)\leq e$; but we know on the other hand \sref{IV.13.1.6} that all the irreducible components of $U_\lambda\cap f^{-1}(f(x'))$ are of dimension $\geq e$; we see therefore that these components are of exactly dimension $e$, hence $b)$ implies $a)$.

Let us finally prove that $b)$ implies $a)$.
Note that the structural morphism $p:Z=Y[T_1,\ldots,T_e]\to Y$ is faithfully flat; hence \sref{IV.2.3.4} the maximal points of $Z$ are the images by $p$ of the maximal points of $Y$; this already proves that the $f(x_\lambda)$ are generic points of the $Y_\alpha$.
Furthermore, if $f(x_\lambda)=y_\alpha$, the $k(y_\alpha)$-morphism $h:U\cap f^{-1}(y_\alpha)\to p^{-1}(y_\alpha)$ deduced from $g$ is dominant and quasi-finite by hypothesis; we therefore have $\dim(U_\lambda\cap f^{-1}(y_\alpha))=e$.
...
If $f$ is also equidimensional at $\sh{O}_{X,x}$, then $\sh{O}_{X,x}$ is also equidimensional; the converse is true if the image by $f$ of the union of the $X_j$ is dense in a neighbourhood of $y$.
\end{corollary}

\begin{proof}
Indeed, to say that $\sh{O}_{X,y}$ is equidimensional means that the numbers $\dim(\sh{O}_{X_j,y})$ are equal for all the irreducible components $Y_j'$ of $Y$ containing $y$, as it follows from \sref{IV.5.1.1.5} applied to the local scheme $\Spec(\sh{O}_{x,y})$; as we have \sref{IV.13.3.4.1}, it follows by the same reasoning that $\sh{O}_{X,x}$ is then also equidimensional.
Conversely, if $\sh{O}_{X,x}$ is equidimensional, all the numbers $\sh{O}_{Y_j,y}$ are equal by \sref{IV.13.3.4.1}, hence $\sh{O}_{Y,y}$ is equidimensional if the $Y_j$ are all the irreducible components of $Y$ containing $y$, which follows from the supplementary hypothesis.
\end{proof}
...
Suppose that the ring $\sh{O}_y$ is equidimensional and that we have the equality
\[
\tag{13.3.6.1}
  \dim(\sh{O}_x)=\dim(\sh{O}_y)+\dim(\sh{O}_x\otimes_{\sh{O}_y}k(y))
\]
...
\begin{proof}
Keeping the notations of \sref{IV.13.3.3}; it follows from \sref{IV.13.2.8} and from the hypothesis that $\sh{O}_x$ is equidimensional, that each $Y_j$ is an irreducible component of $Y$ and that each $f_j$ is equidimensional at the point $x$; furthermore \sref{IV.13.2.8}, on has \sref{IV.13.3.4} that
\[
  \dim(\sh{O}_{X_j,x})=\dim(\sh{O}_{Y_j,y})+\dim(\sh{O}_{X_j,x}\otimes_{\sh{O}_{Y_j,y}}k(y));
\]
whence, by the inequality \sref{IV.5.6.5.2} and the equality \sref{IV.13.3.6.1}:
\[
\tag{13.3.6.2}
...
\]
But by hypothesis the first term of the right-hand side is a continuous function of $x$ \sref{IV.13.3.1} and the second is an upper semi-continuous function of $x$ by \sref{IV.13.1.3}; whence the conclusion.
\end{proof}
...
On the other hand, by transitivity of fibres \sref[I]{I.3.6.4}, it follows from \sref{IV.4.2.8} that, for every $z'\in X'$, the set of dimensions of the irreducible components of ${f'}^{-1}(f'(z'))$ is the same as the set of dimensions of the irreducible components of $f^{-1}(f(z))$, where $z$ is the projection of $z'$ in $X$; whence the conclusion by virtue of \sref{IV.13.3.1},~$a)$).
\end{proof}
