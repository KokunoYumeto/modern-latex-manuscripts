...
\begin{enumerate}
  \item[(a)] The conditions are evidently necessary.
    Conversely, if they are satisfied, then it is clear that $\theta^\sharp$ is an isomorphism from $\psi^*(\sh{O}_X)$ to $\sh{O}_Y$\footnote{\emph{[Trans.] The French source reverses the source and target here. By the definition of a morphism of preschemes and by the displayed stalk maps in Proposition~4.2.2(a), $\theta^\sharp$ has source $\psi^*(\sh{O}_X)$ and target $\sh{O}_Y$.}}, and $\psi^*(\sh{O}_X)$ is the sheaf induced by ``transport of structure'' via $\psi^{-1}$ from $\sh{O}_X|\psi(Y)$;
...
To prove (ii), let $z\in X\times_S Y$, set $z'=(f\times_S g)(z)$, let $p$ and $q$ be the projections from $X\times_S Y$, and let $p'$ and $q'$ be the projections from $X'\times_S Y'$.
