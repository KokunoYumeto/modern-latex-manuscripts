...
\xymatrix{
  ...
  E/\mathfrak{R} \ar[r]^{f'} & E'/\mathfrak{R}'
}
\]
...
Since $v_{\alpha\beta}$ is surjective, it is \emph{a fortiori} a formal epimorphism.
...
\[
  w_j:\gr^j(E_{\gamma,n})\to\gr^j(F_{\beta,n})
\]
such that
\[
  \gr^j(F_{\gamma,n})\xrightarrow{\varphi_{\gamma\gamma,j}}\gr^j(E_{\gamma,n})\xrightarrow{w_j}\gr^j(F_{\beta,n})
\]
is the transition homomorphism.
The same $\gamma$ may be chosen for all $j$, since there are only finitely many of them.
By uniqueness and because $\varphi$ is a homomorphism of graded algebras, the family $w_\bullet$ is a homomorphism of graded $C_\gamma$-algebras
\[
  \gr^\bullet(E_{\gamma,n})\to\gr^\bullet(F_{\beta,n})=F_{\beta,n}.
\]
Since $\varphi_{\gamma\gamma,0}$ and $\varphi_{\gamma\gamma,1}$ are the identity transition maps, $w_0$ and $w_1$ are the transition homomorphisms; as $\gr^\bullet(E_{\gamma,n})$ is generated by degree one, $\gr^j(v_{\alpha\beta}\circ w_\bullet)$ is the transition homomorphism for every $0\leq j\leq n$.

Apply now \sref{0.19.5.6.3},~(ii), first to $\alpha,\beta,\gamma$, and then repeat the construction after replacing $\alpha,\beta$ by the resulting later indices.
The resulting compatible surjections show that $\Ker(v_{\gamma\delta})\subset\Ker(q_{\beta\delta})$; hence $q_{\beta\delta}$ factors through $v_{\gamma\delta}$ and gives a homomorphism $E_{\gamma,n}\to F_{\beta,n}$ whose composite with $v_{\alpha\beta}$ is the transition map to $E_{\alpha,n}$.
For any $\mu\geq\gamma$, composing with $E_{\mu,n}\to E_{\gamma,n}$ gives the required $u_{\beta\mu}$.
\end{proof}
...
Since $F_{\beta,n}$ is a $C_\beta$-algebra, hence a discrete topological $C$-algebra, \sref{0.19.5.6.9} gives by composition a continuous $A$-homomorphism $r:C\to G/\mathfrak{R}$.
Formal smoothness of $C$ lifts it to $r':C\to G$, thereby giving $G$ the structure of a discrete topological $C$-algebra.
