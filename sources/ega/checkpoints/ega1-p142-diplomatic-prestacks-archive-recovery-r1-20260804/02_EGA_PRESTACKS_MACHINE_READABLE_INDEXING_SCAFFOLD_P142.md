...
## Incremental EGA I p.139 scaffold

- `provenance:EGA-I-p139-fr`: direct-authority page node. Source is NUMDAM EGA
  I PDF one-based p.138 / printed p.139, verified against the one bounded
  600-dpi page image at SHA-256
  ...
  Method is direct visual reading plus non-OCR text-object confirmation;
  `confidence=high`, `unresolved_reading_count=0`, and
  `catalogued_author_error_count=1`.
- `proof:I.5.5.10-fr`: completion of the affine-intersection proof. The open
  set `p_1^(-1)(U) intersect p_2^(-1)(V)` identifies with the underlying
  space of affine `U times_Z V`; the graph intersection is closed there and
  induces a closed affine subprescheme. Dependencies are `I.3.2.7-fr`,
  `I.3.2.2-fr`, `I.5.4.3-fr`, `I.4.2.1-fr`, and `I.4.2.3-fr`; the conclusion
  uses that `Gamma_f` is an immersion.
- `source-reading:I.5.p139.ref-4.2.1-punctuation-fr`: the direct image has a
  comma after `(4.2.1)`. The PDF text layer and bbox text misdecode that comma
  as a baseline digit `3`; the rejected secondary candidate is not a clause
  number or footnote.
- `I.5.5.11-fr`: Examples node with four typed children.
  `example:I.5.5.11.projective-line-fr` applies the two conditions of
  `I.5.5.6-fr` to the cover `(X_1,X_2)` and the ring generated by `K[s]` and
  `1/s`, proving the projective line of `I.2.3.2-fr` separated.
- `example:I.5.5.11.doubled-affine-line-fr`: glue two affine lines by the
  identity on the overlap; condition one of `I.5.5.6-fr` holds and condition
  two fails. The resulting integral prescheme is nonseparated and maps
  surjectively to `Spec(K[s])`, with two points over the origin.
- `erratum:I.5.5.11.doubled-origin-ideal-fr`: confirmed author/source error.
  Both French ideal tokens are printed `(0)`; the doubled origin has prime
  ideal `(s)`, while `(0)` is the generic point. The diplomatic French keeps
  `(0)`. English corrects both tokens to `(s)` and exposes the source reading
  in a translator note. Edge types are `printed_error`,
  `visible_source_correction`, and `repair`; `confidence=high`.
- `object:I.5.5.11.punctured-affine-plane-open-fr`: for
  `Y=Spec(K[s,t])`, the union `D(s) union D(t)` is not affine. Sections extend
  to all of `Y`; if the open were affine, `I.1.7.3-fr` would force its
  injection into `Y` to be an isomorphism. Definition-use dependency is
  `I.1.4.1-fr`.
- `example:I.5.5.11.doubled-affine-plane-fr`: glue two affine planes along
  `D(s_i) union D(t_i)`. Neither condition of `I.5.5.6-fr` holds. The
  resulting integral prescheme is the affine plane with doubled origin.
- `I.5.5.12-fr`: Remark node for a property `P` of prescheme morphisms.
  `range_status=open`; clauses (iv)--(vi) continue on printed p.140.
- `property:I.5.5.12.P-fr`: bold source symbol `P`, parent
  `I.5.5.12-fr`. `clause:I.5.5.12.i-fr` states closed immersions have `P`;
  `clause:I.5.5.12.ii-fr` states closure under composition; and
  `clause:I.5.5.12.iii-fr` states closure under products of two
  `S`-morphisms. The product formula is `f times_S g`.
- `english-repair:I.5.p139.doubled-origin-ideal`: inherited English repeated
  the printed `(0)` error. The successor uses `(s)` twice and adds a visible
  translator note explaining the generic-point contradiction.
- `english-repair:I.5.p139.neither-agreement`: restores singular `is
  satisfied` after `neither` in the first nonaffine-open example sentence.
- `english-repair:I.5.p139.doubled-plane-negative-quantifier`: replaces the
  weaker plural negation by `neither condition ... is satisfied`, restoring
  French `aucune` and the failure of each condition separately.
- `english-normalization:I.5.p139.bold-Z-macro-retained`: English `bb{Z}` and
  French `mathbf{Z}` denote the same printed bold base ring; the macro-level
  difference is explicit and is not attributed to the author.
- `english:I.5.p139-confirmation`: after three reversible line repairs,
  including one visible author-source correction, live lines 750--781 are
  source-grounded. R84 changes exactly `ega1/ega1-5.tex` from the R83-gated
  127-file tree by +184 bytes.

Exact p.139 French append coordinates are `ega1-5-fr.tex` bytes
42,954--47,116, a 4,163-byte suffix / SHA-256
`D0B70C7E7D933740BBF31F9FFE81C98CAE06626EDA37CB0EE8410206942846E1`.
The p.139 marker begins at one-based byte 42,955; marker-to-EOF is 4,162
bytes / SHA-256
`E68D4D820A37CD04328AAB304C3A39B82439D9988BBD064F0A88AFCAFD8DF6FF`.
Truncation to the first 42,953 bytes reproduces sealed p.138 at SHA-256
`2619437E655E33F819B8A965B48F8DA2D9B0F6890A0E9314EA285D8C99DF87CB`.
Paired English p.139 marker coordinates are `ega1-5.tex` lines 750--781,
3,881 bytes / SHA-256
`B760F3EA78B372E7C5C3E5BAFC834054093989E257D070FD55316558D33380C3`.
The exact live build prefix is lines 239--781, 29,803 bytes / SHA-256
`F34AA6F07CEE12F29A8C9307EABAB9F387AA6CB1BE3411FAFB6EF32DE67DFDBB`
with zero source additions. Because Remark 5.5.12 and its enumerate continue
on p.140, each bounded wrapper requires exactly two build-only closes. Live
lines 1--781 are 44,973 bytes / SHA-256
`A55F139AE0148D8696284A315E84F5F70BF516C09C7A7E470F8670B282D46B9A`.
R84 is 52,489 bytes / SHA-256
`4C4EF213763A4E9838AF2E8E23A89C8DB0FC45DEBEA0DA078908BED01EA6CFB8`;
its 7,281,856-byte ordinal source tree has SHA-256
`3DD3F9B92DBC78C7334C1F194836E9F4181DF2FE87A8A3905D64947F51576F6C`.
Final cumulative coordinates, exhaustive references, subject/formula/
terminology indices, and implication closure remain deferred to the stable
cumulative reader.

## Incremental EGA I p.140 scaffold

- provenance:EGA-I-p140-fr is the direct-authority page node for NUMDAM EGA I
  PDF one-based p.139 / printed p.140. The single bounded 600-dpi image has
  SHA-256
  53C5CB09587812441B5A5ED0C6DB1451E3A63C106EBEDFB7FD33CF421536A93B;
  the non-OCR layout witness has SHA-256
  E1171D8933412E63A22252C5CEDC2E0A3699C41A1084618163365D78088D00F1.
  Method is direct visual reading with secondary text comparison,
  confidence=high, unresolved_reading_count=0, and
  catalogued_author_error_count=0.
- clause:I.5.5.12.iv-fr states base-change stability for property P: an
  S-morphism f:X->Y with P gives every S'-morphism f_(S') obtained along
  S'->S property P. Its typed edge is base_change_stability.
- clause:I.5.5.12.v-fr is a cancellation statement: if g composed f has P
  and g is separated, then f has P. clause:I.5.5.12.vi-fr states that P
  descends to f_red and depends on I.5.1.5-fr.
- implication:I.5.5.12.iii-iv-fr records, under clauses (i) and (ii), the
  equivalence of product stability (iii) and base-change stability (iv).
  implication:I.5.5.12.v-vi-fr records that clauses (v) and (vi) follow from
  (i), (ii), and (iii).
- formula:I.5.5.12.graph-factorization-fr is
  X ->[Gamma_f] X times_Z Y ->[p_2] Y, with dependency I.5.3.13-fr.
  formula:I.5.5.12.projection-product-fr is
  p_2=(g composed f) times_Z 1_Y. Together with clause (iii),
  I.5.4.3-fr, clause (i), and composition clause (ii), these prove the
  cancellation clause (v).
- diagram:I.5.5.12.reduction-square-fr is the commutative square with
  X_red ->[f_red] Y_red above X ->[f] Y. Both vertical arrows are closed
  immersions by I.5.1.5-fr. The composite X_red->Y_red->Y has P by (i) and
  (ii); the right vertical arrow is separated by I.5.5.1.i-fr, so (v)
  yields P for f_red. The direct French diagram has no comma after its
  bottom-right Y; the initially inherited comma is a rejected candidate.
- clause:I.5.5.12.i-prime-fr replaces closed immersion by immersion.
  clause:I.5.5.12.v-prime-fr states cancellation from P of g composed f.
  Their typed implication edge records that (v') follows from (i'), (ii),
  and (iii). Remark I.5.5.12-fr closes on this page.
- I.5.5.13-fr is an unheaded numbered observation. Clause (ii') composes a
  closed immersion j with a P-morphism g; clauses (i), (iii), and (ii')
  imply (v) and (vi). Clause (ii'') makes the analogous statement for an
  immersion; clauses (i'), (iii), and (ii'') imply (v'). The proof-use edge
  targets the arguments of I.5.5.12-fr.
- section:I.6-fr opens Conditions de finitude. subsection:I.6.1-fr opens
  Preschemes noetheriens et localement noetheriens.
- I.6.1.1-fr defines a Noetherian prescheme as a finite union of affine opens
  V_alpha whose induced scheme rings are Noetherian, and a locally
  Noetherian prescheme by dropping finiteness of the union. Typed terms are
  term:noetherien-fr, term:localement-noetherien-fr, and
  object:I.6.1.1.affine-cover-fr.
- statement:I.6.1.1.coherent-structure-sheaf-fr uses I.1.5.2-fr to infer
  that O_X is a coherent sheaf of rings when X is locally Noetherian.
  range_status=open: printed p.140 and the French cursor end at the exact
  fragment Tout sous-O_X-, whose sentence continues on printed p.141.
- english:I.5-I.6.p140-confirmation records zero source mutation after direct
  comparison of ega1-5.tex lines 782--827 and ega1-6.tex lines 1--13.
  english-normalization:I.6.p140-page-boundary records that English preposes
  quasi-coherent and therefore places oldpage 141 after the complete phrase
  quasi-coherent O_X-submodule rather than splitting that reordered phrase
  at the literal French compound boundary. This is a translation-coordinate
  normalization, not an authorial word-order claim.
- english-normalization:I.6.p140-noetherian-capitalization retains standard
  English capitalization Noetherian for French noetherien. R86 seals all 127
  source rows unchanged from R85.
- english-normalization:I.5.p140-diagram-comma retains the English comma after
  the displayed square as sentence punctuation before the following where
  clause; it is explicitly not attributed as a printed French glyph.

Exact p.140 French section-5 append coordinates are ega1-5-fr.tex bytes
47,117--50,232, a 3,116-byte suffix / SHA-256
67AC729828067E29A52579C8056A65DCE05B6AC1C4C5027AFB7721830F725A15.
The p.140 marker begins at one-based byte 47,118; marker-to-EOF is 3,115
bytes / SHA-256
4C3B9D6DE635AB3BC9353C859DF6A73A4349EE1A5994AB0A3F92D9FE64D80A8F.
Truncation to the first 47,116 bytes reproduces sealed p.139 at SHA-256
D8168125192DF12B1765D4F81E8DD2A15D37378370454F4370E0A3F18C3BC055.
The new section-aligned ega1-6-fr.tex is 694 bytes / SHA-256
0292AFD987807F3045A61D94F56A2344684A539439013DE19091181E369F859F.
Its exact reversal is removal only when that hash still matches.

Paired English p.140 coordinates are ega1-5.tex lines 782--827, 2,565 bytes /
SHA-256
5591365273645FDB7E7AD4D56CBA6636538567C0F176629BBF5C7E67C99D58AB,
and ega1-6.tex lines 1--13, 649 bytes / SHA-256
742F394381659739D885472D80DC02B21BCEB738815289955E6B0410646D4E52.
Their ordered concatenation is 3,214 bytes / SHA-256
AECF8D9C3BD19AA2CDB86B074270F09921732D829417AFA04F67C41FB0E775A7.
R86 is 53,233 bytes / SHA-256
1AACEE47C3D247A51FAC9790F44B8B4291AD670DB299986CEC6F056638063F8B;
its unchanged 7,281,856-byte ordinal tree has SHA-256
3DD3F9B92DBC78C7334C1F194836E9F4181DF2FE87A8A3905D64947F51576F6C.
Final cumulative coordinates, exhaustive references, subject/formula/
terminology indices, and implication closure remain deferred to the stable
cumulative reader.

## Incremental EGA I p.141 scaffold

- provenance:EGA-I-p141-fr is the direct-authority node for NUMDAM EGA I PDF
  one-based p.140 / printed p.141. Its single bounded 600-dpi image has
  SHA-256
  4346F82331781A0876FAE61518278E9D62A5D14C25F90D172CBFF9DAD6AE73A5;
  the non-OCR layout witness has SHA-256
  99709BB3993FAC9555F0EA594BDC01D4B9DAE48CF727F7AC59987FB904B71AA0.
  Method is direct visual reading with secondary text comparison,
  confidence=high, unresolved_reading_count=0, and
  catalogued_author_error_count=0.
- typography:I.6.1.1.crosspage-coherent-module-fr links the p.140 fragment
  Tout sous-O_X- to the p.141 continuation Module ... quasi-coherent. Direct
  authority proves the full phrase italic. A seven-byte p.140-tail fidelity
  successor repairs only that emphasis and has an exact fragment inverse.
- statement:I.6.1.1.coherent-subquotients-fr states that a quasi-coherent
  O_X-submodule, or quasi-coherent quotient O_X-module, of a coherent
  O_X-module F is coherent. Its typed definition-use edges target I.1.5.1-fr,
  I.1.4.1-fr, and I.1.3.10-fr plus the finite-type subquotient fact over a
  Noetherian ring. consequence:I.6.1.1.coherent-ideal-sheaf-fr specializes to
  every quasi-coherent ideal sheaf of O_X.
- statement:I.6.1.1.local-cover-gluing-fr records that a finite cover by
  Noetherian induced preschemes makes X Noetherian, while an arbitrary cover
  by locally Noetherian induced preschemes makes X locally Noetherian. The
  finite-versus-arbitrary distinction is source-certain.
- I.6.1.2-fr is the biconditional criterion
  Noetherian(X) iff locally-Noetherian(X) and quasi-compact(|X|), with the
  additional conclusion Noetherian(|X|). Its proof-use edges target
  I.1.1.10.ii-fr, I.1.1.6-fr, and 0.2.2.3-fr.
- I.6.1.3-fr equates three nodes for an affine scheme X of ring A:
  Noetherian(X), locally-Noetherian(X), and Noetherian(A). The b-to-a and
  a-to-b dependencies use I.6.1.2-fr and I.1.1.10-fr; c-to-a is direct.
- object:I.6.1.3.affine-cover-fr is the finite affine cover (V_i) with
  Noetherian induced rings A_i. sequence:I.6.1.3.ideals-fr is the increasing
  sequence (a_n) in A. sequence:I.6.1.3.ideal-sheaves-fr is the canonically
  bijective increasing sequence (tilde-a_n) supplied by I.1.3.7-fr.
  identification:I.6.1.3.tildeA-OX-fr preserves the printed
  tilde-A equiv O_X rather than collapsing the identification to an
  unrecorded equality.
- restriction:I.6.1.3.local-ideal-sheaves-fr identifies
  tilde-a_n restricted to V_i as the inverse image along V_i->X
  (0.5.1.4-fr), hence as tilde-a_ni for an ideal a_ni of A_i
  (I.1.3.7-fr). Noetherian(A_i) makes each local sequence stationary, and
  the finite cover makes the global sequence stationary. This is the
  proof-dependency chain for a-to-c.
- consequence:I.6.1.3.coherent-ideal-ACC-fr records the source-emphasized
  statement that every increasing sequence of coherent ideal sheaves on a
  Noetherian prescheme is stationary.
- I.6.1.4-fr states that every subprescheme of a Noetherian (respectively
  locally Noetherian) prescheme has the same finiteness property. The proof
  reduces by I.6.1.1-fr and I.4.1.3-fr to an affine X and a closed or
  open-induced Y.
- formula:I.6.1.4.closed-quotient-fr is A/J, with J an ideal, and uses
  I.4.2.3-fr plus I.6.1.3-fr. cover:I.6.1.4.principal-opens-fr is the finite
  D(f_i) cover of the open Y obtained from I.6.1.2-fr. The proof and this
  range remain open after the exact printed-p.141 terminal semicolon;
  range_status=open toward printed p.142.
- english-repair:I.6.1.3.sequence-precision records one reversible live-source
  line repair: add the omitted increasing property of the sheaf sequence,
  restore the governing I.1.3.7 citation, and replace bare equality by the
  printed canonical equivalence symbol. This is classified as a mathematical
  and notational precision repair, not an author correction.
- english-repair:I.6.p142-marker restores the omitted oldpage 142 marker
  immediately after the p.141 D(f_i) semicolon. Its exact inverse removes the
  unique 17-byte marker line. english-normalization:I.6.1.3.condition-letters
  retains parenthesized (a)/(b)/(c) navigation for French a)/b)/c).

Exact p.141 French coordinates are ega1-6-fr.tex bytes 702--5,074 after the
701-byte emphasis-repaired prefix, a 4,373-byte append / SHA-256
5ADC5AE2ED53F87CB81A5E197993C3069E4FF78AA3236172489B47AA1AF38259.
The p.141 marker begins at one-based byte 703. The full source is 5,074 bytes
/ SHA-256
75A77003BDC90E8F0809F0DBF324A1F45268BC7A39C557D5A78C62816168B95B.
Truncation to 701 bytes and the recorded emphasis-fragment inverse reproduce
sealed p.140 at SHA-256
0292AFD987807F3045A61D94F56A2344684A539439013DE19091181E369F859F.

Paired English p.141 content is ega1-6.tex lines 15--65, 3,999 bytes /
SHA-256
DD22C85E9CCBFEAA250B253852BB73F43F585C1E8D6CEE2E4639F8457532A8AE.
Including oldpage markers 141 and 142 gives lines 14--66, 4,033 bytes /
SHA-256
9234D3A321A90E2C4D71F38B6AD6CAF1C724C19DDAE629A13B46422436B3AAC9.
R88 is 54,107 bytes / SHA-256
76E4ACFF554773770EBC1C53C87E02A3F4CC54D3EA8CE81898DA6D5B9BF9B0E6;
its 7,281,911-byte ordinal tree has SHA-256
5AAE163319428FB1DDB52411C7F5CAB6AFA90235FB32C7ADE2AFD6203E6D4C25.
Final cumulative coordinates, exhaustive references, subject/formula/
terminology indices, and implication closure remain deferred to the stable
cumulative reader.

## Incremental EGA I p.142 scaffold

- provenance:EGA-I-p142-fr is the direct-authority node for NUMDAM EGA I PDF
  one-based p.141 / printed p.142. Its single bounded 600-dpi image has
  SHA-256
  F7FCFB5BF65576AC2607AF7126910447C4F3C16971D53E76554C6DC6BC43F8AE;
  the non-OCR layout witness has SHA-256
  DB51B3CED6B447BAAC5413AF1A854880BE005C6C66A73C0AB23A7B2576DA1598.
  Method is direct visual reading with secondary text comparison,
  confidence=high, unresolved_reading_count=0, and
  catalogued_author_error_count=0.
- proof:I.6.1.4-fr closes the p.141 open range by reducing the open
  subprescheme case to Y=D(f). object:I.6.1.4.principal-open-fr has affine
  ring A_f. Its typed proof-use edges target I.1.3.6-fr and I.6.1.3-fr.
- I.6.1.5-fr is an unheaded numbered observation. It records that the product
  of two Noetherian S-preschemes need not be Noetherian even when both are
  affine. object:I.6.1.5.tensor-product-fr is the tensor product of the two
  Noetherian algebras; its failure to be a Noetherian ring supplies the
  countermechanism. The forward reference I.6.3.8-fr remains open until
  section 6.3 is admitted.
- I.6.1.6-fr states nilpotence of the nilradical sheaf N_X on a Noetherian
  prescheme. cover:I.6.1.6.affine-U_i-fr is a finite affine cover.
  formula:I.6.1.6.local-nilpotence-fr is
  (N_X restricted to U_i) raised to n_i equals zero.
  construction:I.6.1.6.maximum-exponent-fr sets n=max_i(n_i), and
  formula:I.6.1.6.global-nilpotence-fr is N_X raised to n equals zero.
  Proof-use edges target I.5.1.1-fr, I.1.3.13-fr, and
  external:I-11:p127:cor4. The printed argument is unheaded.
- I.6.1.7-fr is the biconditional affine(X) iff affine(X_red) for a
  Noetherian prescheme. Its typed dependencies target I.6.1.6-fr and
  I.5.1.10-fr; reduction:I.6.1.7-Xred-fr links to the established reduced
  prescheme construction.
- I.6.1.8-fr is the neighbourhood-connectedness lemma. Given a neighbourhood
  U with finitely many irreducible components, construction:I.6.1.8-V-fr
  removes the components not containing x. For every open W with x in W and
  W contained in V, components:I.6.1.8-W-fr are the traces on W of precisely
  those irreducible components of U which meet W. Both component-use edges
  target 0.2.1.6-fr. Since all contributing components contain x, their union
  W is connected.
- english-repair:I.6.1.8.meeting-components records the reversible precision
  repair adding which meet W. The inherited English quantified over every
  irreducible component of U and therefore falsely included empty
  intersections as irreducible components of W. This is a translation
  mathematical-precision correction, not an author correction.
- I.6.1.9-fr states that every locally Noetherian topological space is locally
  connected. consequence:I.6.1.9.open-connected-components-fr records that
  its connected components are open.
- I.6.1.10-fr has condition nodes a through e:
  condition:I.6.1.10.a.open-irreducible-components-fr;
  condition:I.6.1.10.b.irreducible-equals-connected-fr;
  condition:I.6.1.10.c.connected-components-irreducible-fr;
  condition:I.6.1.10.d.distinct-irreducible-components-disjoint-fr; and
  condition:I.6.1.10.e.irreducible-stalk-spectrum-fr. The e node is
  equivalently primality of the nilradical of O_x. Admitted implication edges
  are a->b and b->c; the converse c->b argument begins and remains open after
  the source-certain words un ensemble ferme F contenant.
- proof:I.6.1.10-fr is a typed semantic range despite being an unheaded
  printed argument. range_status=open toward printed p.143. French source has
  no open TeX environment at this cursor because unheaded arguments follow
  the established diplomatic plain-paragraph encoding.
- english-normalization:I.6.p142-proof-style retains semantic English proof
  environments, visible Proof headings, and QED marks for the unheaded French
  arguments after 6.1.6, 6.1.7, 6.1.8, and 6.1.10.
  english-normalization:I.6.p142-nilradical-case retains conventional
  lower-case English nilradical where French prints an initial capital.
  english-normalization:I.6.1.10-condition-letters retains parenthesized
  (a)--(e) navigation for French a)--e).

Exact p.142 French coordinates are ega1-6-fr.tex bytes 5,075--8,918 after the
5,074-byte sealed-p.141 prefix, a 3,844-byte append / SHA-256
3F80CD616F841F25010329244BD545AA03A652B5F18BAE50BA863CF3696BE95E.
The p.142 marker begins at one-based byte 5,075. The full source is 8,918
bytes / SHA-256
00E79CA7426EA6320FF63BF0ED4207272B5210C94CF8F4E6840358A9D5A9A622.
Truncation to 5,074 bytes reproduces sealed p.141 at SHA-256
75A77003BDC90E8F0809F0DBF324A1F45268BC7A39C557D5A78C62816168B95B.

Paired English p.142 content is ega1-6.tex lines 67--137, 3,598 bytes /
SHA-256
FA92D3476F9D64F664607B90E4249F3473E719698E4B171447D4321AFA4497D8.
Including oldpage markers 142 and 143 gives lines 66--138, 3,632 bytes /
SHA-256
485A4C7865B016E208DC9AEE9ACD23BA9F4D23B6606D44CE6410EC7B3C12C87F.
R90 is 54,931 bytes / SHA-256
9C7A39B176AADF819DC164BDAEA8C23EEB6E7B5851D2F2C318952B1C837D3E7D;
its 7,281,925-byte ordinal tree has SHA-256
F25D638DCFAA5654BF5ED53481009D6E2B3D94C4B1AB99AF5AAFECC0C1F1335A.
Final cumulative coordinates, exhaustive references, subject/formula/
terminology indices, and implication closure remain deferred to the stable
cumulative reader.
