...
To satisfy the conditions $f(a)=y'$ and $f(b)=y$ as well as the additional condition (i) (resp. (ii)), we take $Y'=\Spec(A')$, where $A'$ is a complete valuation ring that dominates $\sh{O}_{y'}$ and whose algebraically closed residue field extends $\kres(x)$ (resp. a valuation ring that dominates $\sh{O}_{y'}$ and whose field of fractions is $\kres(x)$);
the existence of such an $A'$ is guaranteed by \sref{II.7.1.2}.
\end{proof}
