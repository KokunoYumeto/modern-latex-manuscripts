...
so that the phrase reads ``satisfying (7.8.2, (ii) and (iii)).''
...
hence also over $A_M$; one concludes again by (16.1.5).
...
of $A$.  Thus in the proof of (16.5.12) one may reduce to the case in
...
gives $af'(x)d_{B/A}x=0$; the assertion follows from (20.4.7).
...
is a quasi-coherent $\sh{O}_Y$-Module (1.7.4).  Then
\begin{align}
 \operatorname{Ass}(f_*(\sh{F}))&\subset f(X), \tag{3.1.10.3}\\
 \operatorname{Ass}(f_*(\sh{F}))&\subset f(\operatorname{Ass}(\sh{F}))
       \quad\text{if $X$ is locally Noetherian}. \tag{3.1.10.4}
\end{align}

Indeed, let $y\in\operatorname{Ass}(f_*(\sh{F}))$.  By (2.3.1) and the
...
\]
by (3.1.7), and hence $y\in f(\operatorname{Ass}(\sh{F}))$.
\end{remark}
...
one maximal point; flatness (2.3.4) then gives the same conclusion for
...
follows from (2.6.1) that $k'/k(y)$ is radicial, so $A$ is geometrically
unibranch as well.

\paragraph{(Err\textsubscript{IV}, 33).}
The example in (IV, 12.1.2, (i)) is incorrect: the morphism considered
...
is glued to $b_2$ and $b_1$ to $a_2$ (cf. (6.5.5, (ii))).  It is easy
...
``locally equidimensional.''  In (IV, 13.3.9), on the third line from
...
present statement of (IV, 14.4.1) is false even when $f:X\to Y$ is a
...
conditions b) and b') of (IV, 14.4.1).
...
In (IV, 14.4.4), on the fifteenth and sixteenth lines from the bottom of
...
\begin{quote}
By (1.10.3), it is enough to prove that for every $x\in X_y^0$ and
every generalization $y'$ of $y$, there exists a generalization $x'$ of
$x$ such that $f(x')=y'$.  By (15.6.2), it is enough to take for $x'$
the generic point of $X_{y'}^0$.
\end{quote}

\paragraph{(Err\textsubscript{IV}, 47).}
In (IV, 15.6.7), on line 2 of p.~241, after ``Noetherian,'' add ``or if
condition $\beta$) of (15.6.6, (iii)) is satisfied.''  After line 9 of
p.~241, add:
\begin{quote}
When condition $\beta$) of (15.6.6, (iii)) is satisfied, the implication
a)$\Rightarrow$b') follows from (15.6.6, (ii)), and the implication
b')$\Rightarrow$a) follows from (15.6.6, (iii)).
\end{quote}
...
(IV, 20.1.5) coincides with that of (I, 7.4.1).
\end{quote}
...
(1.7.4).  Its fibre is zero at every point
...
Under the hypotheses of (4.2.1), one also has
\begin{equation}
 \dim(K\otimes_kL)=inf(\operatorname{tr.deg}_kK,
                           \operatorname{tr.deg}_kL). \tag{4.2.1.5}
\end{equation}
...
$\operatorname{tr.deg}_kK=n<+\infty$.  In the proof of (4.2.1) we saw
...
\]
which has dimension $n$ (5.5.4), so
...
(5.2.3), or equivalently (I, 1.7.4) an $L$-homomorphism
...
otherwise $X$ would be geometrically irreducible by (4.5.19).  Let
...
the $X'_i$ (4.5.11), so it meets none of the $X'_{ij}$; hence $Y$ is
nowhere dense.
...
Under the hypotheses of (6.2.3), let in addition $M$ be a finite
$A$-module, and suppose that $M\neq0$ and $N\neq0$.  Then
\begin{equation}
 \operatorname{pd}_B(M\otimes_AN)=
 \operatorname{pd}_A(M)+
 \operatorname{pd}_{B\otimes_Ak}(N\otimes_Ak). \tag{6.2.4.1}
\end{equation}
\end{proposition}

Consider the left resolution $L_\bullet$ of $N$ introduced in (6.2.3),
...
In (IV, 6.14.4), the reference on the twelfth line from the bottom of
...
ring.  It is then universally catenary (7.2.9), and hence, by (7.3.19),
...
$A$.  By (7.6.4) and (7.7.2), this is also equivalent to saying that
...
The hypothesis in (8.10.3) that the sets
...
\begin{quote}
It follows from (8.12.3) that the existence of the factorisation in the
...
$g(\eta)=\xi$, since $f$ is birational (6.15.4).  As $g$ is an
...
of $X$ at which $f$ is quasi-finite; this is open in $X$ (13.1.4).\footnote{The
reader will verify that neither (8.12.12) nor any of its consequences is
used in the proof of (13.1.4).}  By (8.12.10), the restriction
...
(8.8.2, (i) and (ii)) by the method of (8.1.2, a), we obtain an open
...
In (IV, 10.4), after Proposition (10.4.11), add:
...
of $X_0$ by the base change $S\to S_0$ (8.9.1).  We may moreover
suppose $f_0$ radicial (8.10.5, (vii)).  It is enough to prove that
...
$X$ are excellent preschemes ((7.8.3) and (7.8.6)), and in particular
that $X$ is universally Japanese ((5.11.4) and (7.8.3, (vi))).
...
it is separated (1.8.7.1); since $f_{\mathrm{red}}$ is proper, $f$ is
...
Write $f=(\psi,\theta)$.  We already know from (10.4.11) that $\psi$ is
...
diagram (10.4.12.1) also shows that $f'$ is quasi-finite.  The
$X_i'$ being integral and normal, (8.12.11) gives a factorisation
\[
 X_i'\xrightarrow{f_i''}X_{\sigma(i)}''
...
Consequently $f'$ is radicial, and (10.4.11) shows that it is
...
Japanese (7.8.3).  Thus $p\circ f'=f\circ p$ is finite; as $p$ is
surjective and $f$ separated, $f$ is proper (II, 5.4.3), and being
quasi-finite it is finite (8.11.1).

\textup{(ii)} By (8.10.5, (i)), applied by the method of (8.1.2, a), we
...
the same notation, one obtains the commutative diagram (10.4.12.1), in
...
reader will verify that (10.4.12) is not used in that theory).
\end{proof}
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item In (10.4.12) and (10.4.13), the hypothesis that $X$ is reduced
...
(10.4.12) shows that $f$ is finite.  If moreover $X$ is reduced and
