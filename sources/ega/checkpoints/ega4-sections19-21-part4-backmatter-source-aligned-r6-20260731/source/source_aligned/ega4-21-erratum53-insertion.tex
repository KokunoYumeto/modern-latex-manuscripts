...
(21.4.4).  For every open subset $U$ of $Y$, write $f_U$ for the
restriction $f^{-1}(U)\to U$ of $f$.  By (21.4.8), the map
\[
...
Using the quasi-compactness of $f$ and (20.3.8), by the method of
(8.1.2, a), we are reduced to proving---in the notation of the
...
over $U$ (21.2.11).  Hence, by (21.4.1), $D'|f^{-1}(U)$ is the
...
$\sh{L}'$ (2.3.1).  Moreover, flatness and the inequality
...
\]
and (21.4.8) gives $D_\alpha=D_\beta$.  If $D$ denotes their common
...
Assume the general hypotheses of (21.4.9), and suppose in addition that
...
$\sh{O}_{Y,y}$ is a discrete valuation ring (5.8.6).
...
onto $\operatorname{Div}(Y)$.  The condition of (21.4.9) says that
...
$\operatorname{prof}(\sh{O}_{X,x})\geq 2$ by (6.3.1).  The desired
equality follows from (21.1.8).
\end{proof}
...
\begin{proof}
It is enough to check that condition 2\textdegree{} of (21.4.10) is then
automatic.  If $\dim(\sh{O}_{Y,y})=1$ and $X_y$ is integral, then at
the unique maximal point $x$ of $X_y$ one has
$\dim(\sh{O}_{X,x})=1$ by (6.1.3).  Moreover,
$\sh{O}_{Y,y}$ is regular and $\sh{O}_{X_y,x}$ is a field, so
$\sh{O}_{X,x}$ is regular (6.5.1), hence a discrete valuation ring.
...
2\textdegree{} of (21.4.10), since $X_y$ has only one maximal point.
\end{proof}

\begin{remark}[21.4.12]
If in (21.4.11) one assumes only that the $X_y$ are reduced, one must
...
Assume the hypotheses of (21.4.11).  For every invertible
...
$\eta$ of $Y$.  Applying (21.4.11) to $D'$ then gives the result by
(21.4.2).

The points of $\operatorname{Ass}(\sh{O}_X)$ lie over the maximal
points of $Y$ (3.3.1).  Since $Y$ is reduced and locally Noetherian,
...
(5.10.2).  We may therefore restrict to the case in which $Y$ is
affine and integral.
...
is schematically dense in $X$ (20.2.13, (iv)), $s$ identifies with a
uniquely determined regular meromorphic section of $\sh{L}$ over $X$.
It is clear from (20.2.11) and (20.3.5) that this section induces on
