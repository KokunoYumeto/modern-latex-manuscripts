...
It is clear from base change (17.3.3) that $c')$ implies $c'')$.
Conversely, since the projection morphism
\[
f^{-1}(y)\otimes_{k(y)}k'\longrightarrow f^{-1}(y)
\]
is open (2.4.10), hypothesis $c'')$ implies that the space $f^{-1}(y)$ is discrete.
Thus, for every point $x\in X_y=f^{-1}(y)$, the local ring $\sh{O}_{X_y,x}=A$ is a finite $k(y)$-algebra, hence an Artinian local ring.
Moreover, it follows from $c'')$ that $\Spec(A\otimes_{k(y)}k')$ is a sum of spectra of fields isomorphic to $k'$, which is possible only if $A$ is a field that is a finite separable extension of $k(y)$ (4.6.1).
\end{proof}
...
\begin{proof}
The equivalence of $e)$ with each of the conditions of (17.6.1) follows at once from (17.4.4,~$f')$) and (17.5.3,~$d')$).
That $e)$ implies $e')$ follows from (17.4.4,~$f)$) and ($\mathbf{0}$,~19.7.1), since $\hat{B}$ is then a finite $\hat{A}$-algebra (17.4.4), and saying that it is a flat $\hat{A}$-module is equivalent to saying that it is a free $\hat{A}$-module ($\mathbf{0}_\mathbf{III}$,~10.1.3).
Conversely, the implication $e')\Rightarrow e)$ follows from (17.4.4) and ($\mathbf{0}$,~19.7.1).
Finally, $e')$ implies that the homomorphism $\hat{A}\to\hat{B}$ is injective; if $k(x)=k(y)$, or if $k$ is separably closed, this homomorphism is surjective by (17.4.4).
...
(ii) Since $f'$ is flat at $x'$ (17.5.1), the hypothesis that $g$ is flat at $y'$ implies that $f$ is flat at $x$: indeed, the projection $g':X'\to X$ is flat at $x'$, and $f\circ g'=g\circ f'$ is flat at $x'$, whence the conclusion (2.2.11,~(iv)).
...
To say that $f'$ is smooth at $x'$ then means (17.5.1) that $X'$ is geometrically regular over $k'$ at $x'$, and this implies (6.7.8) that $X$ is geometrically regular over $k$ at $x$; hence $f$ is smooth at $x$.
Suppose moreover that $f'$ is \'etale at $x'$, so that $x'$ is isolated in $f'^{-1}(y')$ (17.6.1).
Since the projection $f'^{-1}(y')\to f^{-1}(y)$ is an open morphism (2.4.10), $x$ is isolated in $f^{-1}(y)$; as $f$ is already known to be smooth at $x$, it is \'etale at that point (17.6.1).
\end{proof}
...
In (ii), the case where $f$ is locally of finite presentation follows from (17.7.1).
...
\begin{proof}
Property (iii) is immediate and does not even require the hypothesis that $g$ be locally of finite presentation (2.2.11,~(iv)), since the projection $g':X'\to X$ is flat.
By (17.7.2), the assertions concerning properties (iv), (v), and (vi) are consequences of the assertion concerning property (ii).
...
The sets $V$ and $V'$ are open, and $W=g'(V')$ is also open in $X$ by (2.4.6).
...
The assertion is therefore reduced to the following lemma, which strengthens (11.3.16).
\end{proof}
...
Then $f(U)$ is open in $Y$ (2.4.6); if $V\subset f(U)$ is an affine neighbourhood of $y$, the morphism
\[
...
It then suffices to repeat the argument of the first paragraph of the proof of (11.3.16).
\end{proof}
...
Moreover, since $h$ is then flat at $x$ (17.5.1), $f$ is flat at $y$ by (2.2.11,~(iv)).
...
Since $g$ is locally of finite presentation (I.4.3,~(v)), there is an open neighbourhood $U$ of $x$ in $X$ on which $g$ is flat (11.3.1) and $h$ is smooth.
Moreover, $g(U)$ is an open neighbourhood of $y$ in $Y$ (2.4.6); replacing $X$ by $U$ and $Y$ by $g(U)$, one may therefore suppose that $g$ is faithfully flat and that $h$ is smooth, and one is reduced to proving that $f$ is smooth.
If $k'$ is an algebraically closed extension of $k$, then $X\otimes_k k'$ is smooth over $k'$; by (17.7.3,~(ii)), it is enough to prove that $Y\otimes_k k'$ is smooth over $k'$ (since $\Spec(k')\to\Spec(k)$ is faithfully flat).
Thus one may suppose that $k$ is algebraically closed.
The set of $k$-rational points of $Y$ is then very dense in $Y$ (10.4.8), and the locus where $Y$ is smooth over $k$ is open; it is therefore enough to prove that $Y$ is smooth over $k$ at every $k$-rational point $y$.
At such a point, saying that $Y$ is smooth over $k$ is equivalent to saying that $Y$ is regular at $y$ (17.5.1 and 6.7.8).
Now $y=g(x)$ for some $x\in X$, and by hypothesis $X$ is regular at $x$ (17.5.1); since $X$ and $Y$ are locally Noetherian and $g$ is flat, $Y$ is indeed regular at $y$ (6.5.1,~(i)).

\oldpage[IV]{75}

\emph{(ii) Case of \'etale morphisms.}
By (i), one already knows that $f$ is smooth at $y$; by (17.6.1), it is therefore enough to prove that $f$ is quasi-finite at $y$, or equivalently that $\sh{O}_{Y,y}$ is a finite $k$-algebra.
...
\label{IV.17.7.8}
With the notation of (8.8.1), suppose that $X_\alpha$ and $Y_\alpha$ are locally of finite presentation over $S_\alpha$.
...
\]
The assertion concerning non-ramified morphisms therefore follows from (17.7.1,~(i)), and it is enough to consider smooth morphisms.
Since $f$ and $f_\lambda$ are locally of finite presentation, (17.5.1) says that it amounts to the same thing to require that $f^{-1}(y)$ be geometrically regular at $x$ or that $f_\lambda^{-1}(y_\lambda)$ be geometrically regular at $x_\lambda$ (6.7.8).
The proposition then follows from (17.5.1) and (11.2.6).
...
Moreover, by (17.3.3), $V_\lambda\subset V_\mu$ for $\lambda\leq\mu$; since $X$ is quasi-compact, there is therefore an index $\mu$ such that $X=V_\mu$.
...
In view of (11.2.7) and (17.7.9), one may moreover suppose that $S$, $X$, and $Y$ are Noetherian; finally, one may restrict to the case $S=\Spec(\sh{O}_{S,s})$, where $s=f(y)=h(x)$.
...
Replacing $X$ and $Y$ by $X\times_S\Spec(B)$ and $Y\times_S\Spec(B)$, one concludes from (2.5.1) and (17.7.1) that one may restrict to the case where $A$ is complete and has algebraically closed residue field.
The hypothesis that $f$ is non ramified at $y$ then implies (17.4.4) that $\sh{O}_{Y,y}$ is a finite $\sh{O}_{S,s}$-algebra and a complete Noetherian local ring ($\mathbf{0}_\mathbf{I}$,~7.4.2); since the residue field of $\sh{O}_{S,s}$ is algebraically closed, the homomorphism $\sh{O}_{S,s}\to\sh{O}_{Y,y}$ is surjective (17.4.4).
On the other hand, the hypothesis that $h$ is flat at $x$ implies that the composite homomorphism $\sh{O}_{S,s}\to\sh{O}_{Y,y}\to\sh{O}_{X,x}$ is injective ($\mathbf{0}_\mathbf{I}$,~6.5.1); consequently $\sh{O}_{S,s}\to\sh{O}_{Y,y}$ is bijective, which proves that $f$ is \'etale at $y$ (17.6.3).
...
The set $F$ of points $y\in Y$ satisfying the corresponding property in (ii) is none other than $Y-f(X-E)$; hence (ii) follows from (i) and Chevalley's theorem when $f$ is of finite presentation (1.8.4).
...
Reasoning as at the beginning of (9.9.1) and using (17.7.2,~(ii)), one is reduced to the case where $A$ is Noetherian.
...
Since smoothness of a morphism of finite presentation at a point depends only on the local ring at that point and the local ring at its image (17.5.1), the hypothesis $x\in E$ means that $f$ is smooth at $x$.
It then has this property at every point of an open neighbourhood of $x$ in $X$, and (17.3.3,~(iii)) yields the conclusion.

$2\textsuperscript{o}$ Suppose next that $x\in X-E$ and that $f_\eta$ is not flat at $x$.
The conclusion follows from the following lemma, which sharpens (11.2.8):
...
\begin{proof}[Proof of Lemma 17.7.11.1]
Reasoning again as at the beginning of (9.9.1) and using (2.5.1), one is reduced to the case where $S$, $X$, and $Y$ are Noetherian.
...
The case $x\in E$ is an immediate consequence of (11.1.1).
To treat the case $x\notin E$, one reasons as in (9.4.7.1): after possibly replacing $Y$ and $X$ by neighbourhoods of $f(x)$ and $x$, respectively, there exist two coherent $\sh{O}_Y$-modules $\sh{G}$ and $\sh{H}$ and an $\sh{O}_Y$-homomorphism $u:\sh{G}\to\sh{H}$ such that, for every $s\in S$, the homomorphism $u_s:\sh{G}_s\to\sh{H}_s$ is injective, whereas
\[
...
\]
(I.9.1.13.1).
It follows from (9.5.2) that, for $s$ in a neighbourhood of $\eta$, one has $\Supp(\Ker(1\otimes u_s))\supset T_s$, which proves the lemma.
\end{proof}

$3\textsuperscript{o}$ Suppose now that $x\in X-E$, that $f_\eta$ is flat at $x$, but that $f_\eta$ is not smooth at $x$.
To say that $f_\eta$ is flat at $x$ is equivalent to saying that $f$ itself is flat at $x$; replacing $X$ by a neighbourhood of $x$, one may therefore suppose that $f$ is flat (11.1.1).
...
But the locus where $f$ is smooth is open in $X$ (12.1.7); since it does not contain $x$ by hypothesis, it does not contain $\overline{\{x\}}$ either.
This completes the proof for the first property considered in (i).

\oldpage[IV]{78}

Let us next prove (i) for the property of being \'etale.
For $f_s$ to be \'etale at $x$ means that it is both smooth and quasi-finite at $x$ (17.6.1).
Now it amounts to the same thing to say that $f_{g(x)}$ is quasi-finite at $x$ or that $f$ itself is quasi-finite at that point; it follows from (13.1.4) that the set of points $x$ at which $f_{g(x)}$ is quasi-finite is open in $X$, hence a fortiori locally constructible.
...
It follows from (17.12.5.1)\footnote{The reader may verify that the result of (17.7.11) is not used in the remainder of \S17, so there is no vicious circle.} that $f_{g(x)}$ is differentially smooth at $x$ if and only if $p_{g(x)}$ is smooth at $\Delta_f(x)$.
Since $p$ is locally of finite presentation, the set of points $z\in X\times_Y X$ at which $p_{g(p(z))}$ is smooth is locally constructible; its intersection with the locally closed subset $\Delta_f(X)$ is therefore locally constructible in $\Delta_f(X)$ (1.8.2).
...
To say that $f_{g(x)}$ is non ramified at $x$ amounts to saying that $(\Delta_f)_{g(x)}$ is a local isomorphism at $x$ (17.4.1); since this is an immersion locally of finite presentation, it amounts to the same thing (17.9.1)\footnote{The reader may verify that the result of (17.7.11) is not used in the remainder of \S17, so there is no vicious circle.} to say that $(\Delta_f)_{g(x)}$ is \'etale at $x$.
...
\begin{proof}
Indeed, one knows from (11.3.10) that $f$ is flat if and only if $f_s$ is flat for every $s\in S$, and that in this case $g$ is flat at the points of $f(X)$.
But for a flat morphism locally of finite presentation, smoothness is a property of its fibres (17.5.1,~$b$)); and for every $y\in Y$, one has $f^{-1}(y)=f_s^{-1}(y)$ when $s=g(y)$.
\end{proof}
...
Conversely, if $Z$ is an open subset of $X$ such that $f|Z$ is surjective and radiciel, then, since $f$ is \'etale, $f|Z$ is an isomorphism by (17.9.1).
...
To say that a morphism locally of finite presentation $f:X\to Y$ is \'etale at a point $x\in X$ means that $f$ is smooth at $x$ and that $\dim_x f=0$ (17.6.1).

\begin{proposition}[17.10.2]
\label{IV.17.10.2}
Let $f:X\to Y$ be a smooth morphism.
For every $x\in X$, the locally free $\sh{O}_X$-module $\Omega^1_f$ (17.2.3) has rank at $x$ equal to $\dim_x f$ (which implies that $x\mapsto\dim_x f$ is a continuous function on $X$).
\end{proposition}

\begin{proof}
Indeed, if $y=f(x)$, then $X_y=f^{-1}(y)$ is smooth over $k(y)$; if $f_y:X_y\to\Spec(k(y))$ is the structural morphism, one has $\Omega^1_{f_y}=\Omega^1_f\otimes_{\sh{O}_X}\sh{O}_{X_y}$ (16.4.5).
Thus one may reduce to the case where $Y=\Spec(k)$ is the spectrum of a field.
Moreover, by (16.4.5) and (17.7.1), one may replace $k$ by an algebraically closed extension; in other words, one may suppose that $k$ is algebraically closed.
The set of $k$-rational points of $X$ is then dense in $X$ (10.4.8), so it is enough to consider the case where $x$ is rational over $k$.
Now (16.4.12) identifies $(\Omega^1_{X/k})_x\otimes_{\sh{O}_{X,x}}k(x)$ with $\mathfrak{m}_x/\mathfrak{m}_x^2$; since $\sh{O}_{X,x}$ is a regular local ring (17.5.1), one has $\operatorname{rg}_{k(x)}(\mathfrak{m}_x/\mathfrak{m}_x^2)=\dim(\sh{O}_{X,x})$ ($\mathbf{0}$,~17.1.1).
Hence $\operatorname{rg}_{\sh{O}_{X,x}}(\Omega^1_{X/k})_x=\dim(\sh{O}_{X,x})$.
Since $k(x)=k$, one also has $\dim_x f=\dim(\sh{O}_{X,x})$ (5.2.3).
Q.E.D.
\end{proof}
...
\begin{proof}
Indeed, $g\circ f$ is smooth (17.3.3), so the three $\sh{O}_X$-modules $\Omega^1_{X/Y}$, $\Omega^1_{X/Z}$, and $f^*(\Omega^1_{Y/Z})$ are locally free (17.2.3 and $\mathbf{0}_\mathbf{I}$,~5.4.5); moreover, the rank at $x$ of $f^*(\Omega^1_{Y/Z})$ equals the rank at $y=f(x)$ of $\Omega^1_{Y/Z}$.
Equality (17.10.3.1) is therefore a consequence of (17.10.2) and the exactness of the sequence (17.2.3.1).
\end{proof}
...
\begin{proof}
Indeed, $\Omega^1_{X/Y}\otimes_{\sh{O}_X}\sh{O}_{X'}$ and $\Omega^1_{X'/Y}$ are locally free, and the exact sequence (17.2.5.1) splits locally in a suitable neighbourhood of every point of $X'$.
Thus $\sh{N}_{X'/X}$ is locally free, and relation (17.10.4.1) follows at once from the exactness of (17.2.5.1).
\end{proof}
...
To see that $c)$ and $c')$ are equivalent, note that under either condition $\Omega^1_{X/S}$ is locally free of finite type, by \sref{IV.17.2.3},~(ii); it is therefore enough to apply ($\mathbf{0}$, \sref{IV.19.1.12}) to the local ring $\sh{O}_{X,x}$ and to the homomorphism \sref{IV.17.11.1.1} of finite-type $\sh{O}_{X,x}$-modules.
...
$\Omega^1_{B'/A}\to\Omega^1_{B/A}$ ($\mathbf{0}$, \sref{IV.20.5.2}) sends the basis elements $d_{B'/A}(\mathrm{T}_i)$ ($\mathbf{0}$, \sref{IV.20.4.13}) to the $d_{B/A}(s_i)$, and is therefore surjective.
...
Since $\sh{O}_{Y,y}$ is then a formally smooth $k$-algebra for the discrete topologies and $\sh{O}_{Y,y}/\mathfrak m_y=k$, it follows from ($\mathbf{0}$, \sref{IV.20.5.14}) that the canonical homomorphism
\[
...
\]
is injective.
Since $\sh{O}_{X,x}$ is regular, the conclusion follows from ($\mathbf{0}$, \sref{IV.17.3.3}).
\end{proof}
...
$\operatorname{Spec}(C)\to\operatorname{Spec}(k(y))=\operatorname{Spec}(k)$ obtained from $g$ by base change ($\mathbf{I}$, \sref{IV.3.3.12}).
...
The images of the $t_i$ in $B$ consequently form a regular system of parameters of $B$ ($\mathbf{0}$, \sref{IV.17.1.7}); condition \sref{IV.17.6.3},~$e''$), is therefore satisfied by $g$ at $x$ ($\mathbf{0}$, \sref{IV.17.1.1}, and Bourbaki, \emph{Alg. comm.}, chap.~III, \S~2, n\textsuperscript{o}~8, cor.~3 to th.~1), which completes the proof.
\end{proof}
...
It is then enough to apply the Jacobian criterion ($\mathbf{0}$, \sref{IV.22.6.1}) together with ($\mathbf{0}$, \sref{IV.19.1.14}), bearing in mind that $\Omega^1_{X/S}$ is locally free in a neighbourhood of $x$ and that $\sh{N}_{Y/X}$ is of finite type \sref{IV.16.1.6}.
...
By ($\mathbf{0}$, \sref{IV.19.3.8}), $B'$ is formally smooth when $A'$ is given the discrete topology and $B'$ the $\mathfrak J'$-preadic topology.
Consequently ($\mathbf{0}$, \sref{IV.19.5.4}), $\mathfrak J'/\mathfrak J'^2$ is a projective $B'/\mathfrak J'$-module and the canonical homomorphism
\[
...
The $t_i$ form part of a system of parameters of $\sh{O}_{X,x}$ ($\mathbf{0}$, \sref{IV.16.4.1}); the conclusion follows from ($\mathbf{0}$, \sref{IV.17.1.7}).
\end{proof}
...
$(X\times_Y X)\times_XY'$ ($\mathbf{I}$, \sref{IV.3.3.9}).
...
The homomorphism $\operatorname{gr}_1(g)$ is defined in \sref{IV.16.2.1}, and the commutativity of the two squares follows from ($\mathbf{0}$, \sref{IV.20.5.7.3}) and ($\mathbf{0}$, \sref{IV.20.5.11.3}).
...
\]
is injective, taking account of ($\mathbf{0}$, \sref{IV.19.1.12}) and of the fact that $\Omega^1_{X'/S}$ is locally free at $x'$ \sref{IV.17.2.3}.
...
Since $\operatorname{gr}_1(g):g^*(\sh{N}_{Y/X})\to\sh{N}_{Y'/X'}$ is surjective, the preceding conditions are equivalent to its being bijective at $x'$ (Bourbaki, \emph{Alg. comm.}, chap.~II, \S~3, n\textsuperscript{o}~2, cor. to prop.~6), and therefore also in a neighbourhood of $x'$ ($\mathbf{0}_{\mathrm I}$, \sref{IV.5.2.7}).
...
\begin{proof}[Proof of Proposition \textup{17.15.14}]
