\section{Supplement on \'etale morphisms. Henselian local rings and strictly local rings}
\label{section:IV.18}

\subsection{A remarkable equivalence of categories.}
\label{subsection:IV.18.1}

\begin{theorem}[18.1.1]
\label{IV.18.1.1}
Let $\vphi:A\to B$ be a local homomorphism of local rings, $\mathfrak{m}$ the maximal ideal of $A$, $B_0=B/\mathfrak{m}B$.
The following conditions are equivalent:
\begin{enumerate}[label=\emph{\alph*)}]
  \item $\vphi$ is a formally \'etale homomorphism (for the preadic topologies defined by $\mathfrak{m}$ and $\mathfrak{m}B$ respectively).
  \item $B$ is a \emph{Noetherian} ring, $B$ is $\mathfrak{m}B$-adically \emph{separated and complete}, and $B_0$ is a formally \'etale $A/\mathfrak{m}$-algebra (for the discrete topologies).
\end{enumerate}
\end{theorem}

\begin{proof}
\oldpage[IV-4]{109}
We have already seen (\sref[0]{0.19.7.1}) that the conditions are equivalent when $A$ is a \emph{Noetherian} complete local ring (with $B$ also Noetherian complete local).
Now let us handle the general case.

We first prove that $a)$ implies $b)$.
By \sref[0]{0.19.3.5}[ii] and \sref[0]{0.19.6.4}, we see that $B$ is Noetherian and $\mathfrak{m}B$-adically separated and complete, and it remains to show that $B_0$ is a formally \'etale $(A/\mathfrak{m})$-algebra.
But if $C_0$ is an $(A/\mathfrak{m})$-algebra and $\mathfrak{J}_0$ a nilpotent ideal of $C_0$, the ring $C_0$ can be considered as an $A$-algebra, and $\mathfrak{J}_0$ as an ideal of this $A$-algebra;
moreover, $C_0$ is separated and complete for the $\mathfrak{m}$-preadic topology since $\mathfrak{m}C_0=0$.
The hypothesis on $\vphi$ then implies that the canonical map $\operatorname{Hom}_{A\text{-}\mathrm{alg}}(B,C_0)\to\operatorname{Hom}_{A\text{-}\mathrm{alg}}(B,C_0/\mathfrak{J}_0)$ is bijective;
but since $\mathfrak{m}C_0=0$, both sides identify respectively with $\operatorname{Hom}_{(A/\mathfrak{m})\text{-}\mathrm{alg}}(B_0,C_0)$ and $\operatorname{Hom}_{(A/\mathfrak{m})\text{-}\mathrm{alg}}(B_0,C_0/\mathfrak{J}_0)$, hence $B_0$ is formally \'etale over $A/\mathfrak{m}$.

Conversely, suppose $b)$ is satisfied, and let $C$ be an $A$-algebra, separated and complete for the $\mathfrak{m}$-preadic topology, $\mathfrak{J}$ an ideal of $C$ such that $\mathfrak{J}^2=0$ (which is sufficient by \sref[0]{0.19.3.6}[ii]).
We must show that the canonical map $\operatorname{Hom}(B,C)\to\operatorname{Hom}(B,C/\mathfrak{J})$ is bijective (the $\operatorname{Hom}$ being taken in the category of continuous $A$-algebra homomorphisms for the $\mathfrak{m}$-preadic topologies on $B$, $C$, and $C/\mathfrak{J}$).
Let us pose $A_n=A/\mathfrak{m}^{n+1}$, $B_n=B/\mathfrak{m}^{n+1}B$, $C_n=C/\mathfrak{m}^{n+1}C$, $\mathfrak{J}_n$ the image of $\mathfrak{J}$ in $C_n$;
since $B$ and $C$ are separated and complete, we have $B=\varprojlim B_n$ and $C=\varprojlim C_n$, and since the $B_n$ are Artinian, and $C_n/\mathfrak{J}_n=(C/\mathfrak{J})_n$, we are reduced to proving that $\operatorname{Hom}_{A_n\text{-}\mathrm{alg}}(B_n,C_n)\to\operatorname{Hom}_{A_n\text{-}\mathrm{alg}}(B_n,C_n/\mathfrak{J}_n)$ is bijective for all $n$, i.e.\ that $B_n$ is a formally \'etale $A_n$-algebra (for the discrete topologies).
Now, by the already established special case (\sref[0]{0.19.7.1}), $B$ is formally \'etale over the completion $\hat{A}$ of $A$;
in particular, $B_n$ is a formally \'etale $A_n$-algebra, since $A_n=\hat{A}/\mathfrak{m}^{n+1}\hat{A}$, which concludes the proof.
\end{proof}

\begin{env}[18.1.2]
\label{IV.18.1.2}
Under the hypotheses and with the notation of \sref{IV.18.1.1}, the formally \'etale homomorphism $\vphi:A\to B$ is also \emph{faithfully flat} (\sref[0]{0.19.7.1}[i]).
In particular, the image in $B$ of every non-zero-divisor of $A$ is a non-zero-divisor of $B$;
the image in $B$ of every element of $A$ not belonging to $\mathfrak{m}$ is \emph{invertible} in $B$;
therefore $\vphi$ extends to a homomorphism $\vphi':A_\mathfrak{m}\to B$, where $A_\mathfrak{m}$ is the localisation of $A$ at $\mathfrak{m}$ (which coincides with $A$ when $A$ is already local).
It is immediate that $\vphi'$ is also a formally \'etale homomorphism.
On the other hand, if $\mathfrak{n}$ is a maximal ideal of $B$, then $\mathfrak{n}\supset\mathfrak{m}B$ since $\mathfrak{m}B$ is contained in the radical of $B$ (\sref[0]{0.7.1.10});
the quotient $B/\mathfrak{n}$ is therefore a quotient of $B_0$, and conversely, any maximal ideal of $B_0$ is the image of a maximal ideal of $B$;
in other words, the maximal ideals of $B$ correspond bijectively to those of $B_0$.
Since $B_0$ is a formally \'etale $\kappa(\mathfrak{m})$-algebra (where $\kappa(\mathfrak{m})=A/\mathfrak{m}$), $B_0$ is a \emph{finite} product of \emph{finite separable extensions} of $\kappa(\mathfrak{m})$ (\sref[0]{0.20.7.3}).
\end{env}

\begin{env}[18.1.3]
\label{IV.18.1.3}
We now specialise to the case where $B$ is a \emph{local} ring.
Then $B_0$ is a finite separable extension of $\kappa(\mathfrak{m})$, therefore there exists a unique (by \sref[0]{0.20.5.9}) polynomial $F\in A[\mathrm{T}]$ \emph{monic} and such that $B_0$ is isomorphic to $\kappa(\mathfrak{m})[\mathrm{T}]/(F_0)$, where $F_0$ is the canonical image of $F$ in $\kappa(\mathfrak{m})[\mathrm{T}]$;
\oldpage[IV-4]{110}
moreover, $F_0$ is a product of \emph{distinct} irreducible factors in $\kappa(\mathfrak{m})[\mathrm{T}]$, the image $\bar{\xi}$ of $\mathrm{T}$ in $B_0$ is a root of $F_0$, and $B_0$ is isomorphic to $\kappa(\mathfrak{m})[\mathrm{T}]/(g_0)$ where $g_0$ is the minimal polynomial of $\bar{\xi}$ over $\kappa(\mathfrak{m})$, so $g_0$ is \emph{separable} and divides $F_0$.
By a well-known property of Henselian rings (\sref[0]{0.I.8.3}), $B$ is isomorphic to $\hat{A}[\mathrm{T}]/(g)$ where $g\in\hat{A}[\mathrm{T}]$ is a monic polynomial lifting $g_0$, $\hat{A}$ being the completion of $A$.
\end{env}

\begin{env}[18.1.4]
\label{IV.18.1.4}
We return to the general case.
To say that $B_0=B/\mathfrak{m}B$ is a formally \'etale $\kappa(\mathfrak{m})$-algebra is equivalent to saying (in the notation of \sref{IV.18.1.2}) that the set $\operatorname{Hom}_{\kappa(\mathfrak{m})\text{-}\mathrm{alg}}(B_0,\Omega)$ is \emph{finite} for every algebraically closed extension $\Omega$ of $\kappa(\mathfrak{m})$, and that $\Omega^1_{B_0/\kappa(\mathfrak{m})}=0$ (\sref[0]{0.20.7.3}).
Furthermore, the formally \'etale homomorphism $\vphi:A\to B$ is a formally smooth homomorphism (\sref[0]{0.19.3.1}), hence a flat homomorphism (\sref[0]{0.19.7.1}[i]).
\end{env}

\begin{theorem}[18.1.2]
\label{IV.18.1.2.thm}
Let $S$ be a prescheme, $S_0$ a closed sub-prescheme of $S$ whose underlying space is identical to that of $S$.
Then the functor
\[
X\mapsto X\times_S S_0
\]
from the category of $S$-preschemes \'etale over $S$, to the category of $S_0$-preschemes \'etale over $S_0$, is an equivalence of categories.
\end{theorem}

\begin{proof}
Let us first show that this functor is \emph{fully faithful}.
Let $X$, $Y$ be two $S$-preschemes \'etale over $S$, and set $X_0=X\times_S S_0$, $Y_0=Y\times_S S_0$.
If $Z=X\times_S Y$, the set $\operatorname{Hom}_S(X,Y)$ is in canonical bijective correspondence with the set of $X$-sections $\Gamma(Z/X)$, and likewise $\operatorname{Hom}_{S_0}(X_0,Y_0)$ is in canonical bijective correspondence with $\Gamma(Z_0/X_0)$, where $Z_0=Z\times_S S_0=X_0\times_{S_0}Y_0$.
Now $Z$ is \'etale over $X$, $Z_0$ \'etale over $X_0$, and $X_0$ (resp.\ $Z_0$) is a closed sub-prescheme of $X$ (resp.\ $Z$) having the same underlying space.
The open subsets of $Z$ are therefore the \emph{same} as the open subsets of $Z_0$ having the corresponding properties, and our assertion follows from \sref{IV.17.9.3}.

To complete the proof, it suffices to show that for every $S_0$-prescheme $X_0$ \'etale over $S_0$, there exists an $S$-prescheme $X$ \'etale over $S$ and an $S_0$-isomorphism $X_0\xrightarrow{\sim}X\times_S S_0$.
By virtue of Proposition~\sref{IV.18.1.1}, there exists an open covering $(U_\alpha)$ of $X_0$ and for each $\alpha$, an $S$-prescheme $V_\alpha$ which is \'etale over $S$, and finally an $S_0$-isomorphism $\theta_\alpha:U_\alpha\xrightarrow{\sim}V_\alpha\times_S S_0$.
Furthermore, by virtue of the first part of the proof, there exists a unique $S_0$-isomorphism $\varphi_{\alpha\beta}$ from $\theta_\alpha(U_\alpha\cap U_\beta)$ to $\theta_\beta(U_\alpha\cap U_\beta)$, corresponding to the identity automorphism of $U_\alpha\cap U_\beta$, and it is immediate, for the same reason, that these isomorphisms satisfy the glueing condition (\sref[0]{0.4.1.7}).
Hence there exists an $S$-prescheme $X$ such that the $V_\alpha$ identify canonically with sub-preschemes induced on the open subsets of $X$, the $\theta_\alpha$ identify with $S_0$-isomorphisms which coincide on the intersections $U_\alpha\cap U_\beta$, and define an $S_0$-isomorphism $X_0\xrightarrow{\sim}X\times_S S_0$.
It is clear that $X$ is \'etale over $S$ \sref{IV.17.3.2}, which completes the proof.
\end{proof}

\begin{corollary}[18.1.3]
\label{IV.18.1.3.cor}
Let $S$ be a prescheme, $X$ an $S$-prescheme \'etale over $S$, $S'$ an $S$-prescheme, $S_0'$ a closed sub-prescheme of $S'$ having the same underlying space.
Then the canonical map $X(S')_S\to X(S_0')_S$ \sref[I]{I.3.4.3} is bijective.
\end{corollary}

\begin{proof}
Indeed, set $X'=X\times_S S'$, $X_0'=X\times_S S_0'$, so that $X(S')_S=\Gamma(X'/S')$ and $X(S_0')_S=\Gamma(X_0'/S_0')$;
the corollary follows from the fact that the functor defined in \sref{IV.18.1.2.thm} (with $S$ and $S_0$ replaced by $S'$ and $S_0'$ respectively) is fully faithful (or directly from \sref{IV.17.9.3}).
\end{proof}

\subsection{Coverings.}
\label{subsection:IV.18.2}

\begin{env}[18.2.1]
\label{IV.18.2.1}
Given a ring $A$ and a commutative $A$-algebra $B$ which is \emph{finite} and a \emph{free} $A$-module, recall (Bourbaki, \emph{Alg.}, ch.~VIII, \textsection~12, no.~2) that one defines on $B$ an \emph{$A$-linear form} $\operatorname{Tr}_{B/A}$, the ``trace form'';
one deduces from it the definition of a \emph{symmetric $A$-bilinear form} (also called ``trace form'')
\[
\label{IV.18.2.1.1}
(x,y)\mapsto\operatorname{Tr}_{B/A}(xy)
\tag{18.2.1.1}
\]
\oldpage[IV-4]{111}
whose datum is equivalent to that of the associated $A$-linear map $\operatorname{astr}_{B/A}:B\to\check{B}$, where $\check{B}$ denotes the \emph{dual} of the $A$-module $B$, equal to its transpose.
When $A$ is a \emph{field}, it is equivalent to say that this bilinear form is \emph{non-degenerate} or that $B$ is a \emph{separable} $A$-algebra (Bourbaki, \emph{Alg.}, ch.~IX, \textsection~2, prop.~5).

Let $f:A\to A'$ be a ring homomorphism;
set $B'=B\otimes_A A'$, and let $g:B\to B'$ be the canonical homomorphism;
the image by $g$ of a base of the $A$-module $B$ is then a base of the $A'$-module $B'$, and it follows from the definitions that, for all $x\in B$,
\[
\label{IV.18.2.1.2}
\operatorname{Tr}_{B'/A'}(g(x))=f(\operatorname{Tr}_{B/A}(x)).
\tag{18.2.1.2}
\]
\end{env}

\begin{env}[18.2.2]
\label{IV.18.2.2}
Consider now a \emph{ringed space} $(X,\sh{O}_X)$ and let $\sh{B}$ be an $\sh{O}_X$-Algebra which, as an $\sh{O}_X$-Module, is \emph{locally free of finite rank};
then, for every open subset $U\subset X$ such that $\sh{B}|U$ is (as an $\sh{O}_U$-Module) isomorphic to $\sh{O}_U^n$ (for some $n$ depending on $U$), $\Gamma(U,\sh{B})$ is a $\Gamma(U,\sh{O}_X)$-algebra which, as a $\Gamma(U,\sh{O}_X)$-module, is free of finite rank, and thus defines a $\Gamma(U,\sh{O}_X)$-linear form $\operatorname{Tr}_{\Gamma(U,\sh{B})/\Gamma(U,\sh{O}_X)}$, which we also denote by $\operatorname{Tr}_{\sh{B}|\sh{O}_X,U}$;
from this we deduce an associated linear map
\[
\operatorname{astr}_{\sh{B}|\sh{O}_X,U}:\Gamma(U,\sh{B})\to\Gamma(U,\sh{B})^\vee=\Gamma(U,\check{\sh{B}}).
\]

Furthermore, it follows from \sref{IV.18.2.1.2} that these linear maps are compatible with the restriction operations from $U$ to a smaller open subset, and thus define an \emph{$\sh{O}_X$-Module homomorphism}, also called the \emph{trace homomorphism}:
\[
\label{IV.18.2.2.1}
\operatorname{Tr}_{\sh{B}|\sh{O}_X}:\sh{B}\to\sh{O}_X
\tag{18.2.2.1}
\]
and also an \emph{$\sh{O}_X$-Module homomorphism}
\[
\label{IV.18.2.2.2}
\operatorname{astr}_{\sh{B}|\sh{O}_X}:\sh{B}\to\check{\sh{B}}
\tag{18.2.2.2}
\]
called the \emph{homomorphism associated to the trace}, and equal to its transpose.
It also follows from \sref{IV.18.2.1.2} that for every $x\in X$, we have
\[
\label{IV.18.2.2.3}
(\operatorname{Tr}_{\sh{B}|\sh{O}_X})_x=\operatorname{Tr}_{\sh{B}_x|\sh{O}_{X,x}}
\tag{18.2.2.3}
\]
\[
\label{IV.18.2.2.4}
(\operatorname{astr}_{\sh{B}|\sh{O}_X})_x=\operatorname{astr}_{\sh{B}_x|\sh{O}_{X,x}}.
\tag{18.2.2.4}
\]

Finally, under the conditions of \sref{IV.18.2.1}, if we set $X=\operatorname{Spec}(A)$ and $\sh{B}=\widetilde{B}$, the form $\operatorname{Tr}_{\sh{B}|\sh{O}_X}$ (resp.\ the homomorphism $\operatorname{astr}_{\sh{B}|\sh{O}_X}$) corresponds to the form $\operatorname{Tr}_{B/A}$ (resp.\ the $A$-module homomorphism $\operatorname{astr}_{B/A}$), as also follows from \sref{IV.18.2.1.2}.
\end{env}

\begin{proposition}[18.2.3]
\label{IV.18.2.3}
Let $f:X\to Y$ be a finite morphism of preschemes and set $\sh{B}=f_*(\sh{O}_X)$.
The following conditions are equivalent:
\begin{enumerate}[label=\emph{\alph*)}]
  \item $f$ is \'etale.
  \item[a$'$)] $f$ is a flat morphism of finite presentation, and for every $x\in X$, if we set $y=f(x)$, $\sh{O}_{X,x}/\mathfrak{m}_y\sh{O}_{X,x}$ is a finite separable extension of $\kappa(y)$.
  \item $\sh{B}$ is a locally free $\sh{O}_Y$-Module and for every $y\in Y$, $\sh{B}_y\otimes_{\sh{O}_{Y,y}}\kappa(y)$ is a finite separable $\kappa(y)$-algebra (hence the direct composite of a finite number of fields, finite separable extensions of $\kappa(y)$).
  \item $\sh{B}$ is a locally free $\sh{O}_Y$-Module and the homomorphism $\operatorname{astr}_{\sh{B}|\sh{O}_Y}:\sh{B}\to\check{\sh{B}}$ \sref{IV.18.2.2} is bijective.
\end{enumerate}
\end{proposition}

\begin{proof}
\oldpage[IV-4]{112}
Since $f$ is quasi-compact, the equivalence of $a)$ and $a')$ has already been shown \sref{IV.17.6.2}.
To prove the rest of the proposition, we may restrict to the case where $Y=\operatorname{Spec}(A)$ and $X=\operatorname{Spec}(B)$ are affine, $B$ being a finite $A$-algebra and $\sh{B}=\widetilde{B}$.
To say that $f$ is a morphism of finite presentation is equivalent to saying that $B$ is an $A$-module of finite presentation \sref{IV.1.4.7}.
If in addition $f$ is flat, then $B$ is a flat $A$-module, so $B$ is an $A$-module projective, known (Bourbaki, \emph{Alg.\ comm.}, ch.~II, \textsection~5, no.~2, cor.~2 du th.~1) to be equivalent to saying that $B$ is an $A$-Module \emph{projective};
thus $\sh{B}$ is a locally free $\sh{O}_Y$-Module (\emph{loc.\ cit.}, no.~2, th.~1), and the converse is immediate.
On the other hand, $f^{-1}(y)$ is nothing but the spectrum of the $\kappa(y)$-algebra $\sh{B}(y)=\sh{B}_y/\mathfrak{m}_y\sh{B}_y$, which completes the proof of the equivalence of $a')$ and $b)$.
To see that $b)$ is equivalent to $c)$, let us note that the second assertion of $b)$ is equivalent to the fact that $\operatorname{astr}_{\sh{B}(y)|\kappa(y)}:\sh{B}(y)\to\sh{B}(y)^\vee$ is bijective;
since $\sh{B}(y)=\sh{B}_y/\mathfrak{m}_y\sh{B}_y$ and $\check{\sh{B}}_y$ are free $\sh{O}_{Y,y}$-modules, it follows from \sref{IV.18.2.2.4} and Bourbaki, \emph{Alg.\ comm.}, ch.~II, \textsection~3, no.~3, cor.\ of prop.~6, that the homomorphism $\operatorname{astr}_{\sh{B}_y|\sh{O}_{Y,y}}:\sh{B}_y\to\check{\sh{B}}_y$ is itself bijective;
the converse being evident, this completes the proof.

When an $\sh{O}_X$-Algebra $\sh{B}$ satisfies the equivalent conditions $b)$ and $c)$ of \sref{IV.18.2.3}, we say that $\sh{B}$ is a \emph{finite \'etale $\sh{O}_X$-Algebra}.
When $X=\operatorname{Spec}(A)$ is affine and $\sh{B}=\widetilde{B}$, this amounts, by virtue of \sref{IV.18.2.3}, to saying that $B$ is a finite \'etale $A$-algebra, or that $B$ is a finite $A$-algebra that is \'etale (in the sense of \sref{IV.17.3.2}).
\end{proof}

\begin{corollary}[18.2.4]
\label{IV.18.2.4}
Let $f:X\to Y$ be a morphism of finite type and of finite presentation, and set $\sh{B}=f_*(\sh{O}_X)$.
Let $y$ be a point of $Y$.
The following conditions are equivalent:
\begin{enumerate}[label=\emph{\alph*)}]
  \item There exists an open neighbourhood $U$ of $y$ in $Y$ such that the restriction $f^{-1}(U)\to U$ of $f$ is an \'etale morphism.
  \item $\sh{B}_y$ is a locally free $\sh{O}_{Y,y}$-module of finite type and $\sh{B}_y\otimes_{\sh{O}_{Y,y}}\kappa(y)$ is a separable $\kappa(y)$-algebra.
\end{enumerate}
\end{corollary}

\begin{proof}
It is clear that $a)$ implies $b)$ by virtue of \sref{IV.18.2.3}.
On the other hand, $\sh{B}$ is an $\sh{O}_Y$-Module of finite presentation \sref{IV.1.6.3} and \sref{IV.1.4.7}, hence, if $\sh{B}_y$ is a locally free $\sh{O}_{Y,y}$-module, there exists an open neighbourhood $U$ of $y$ in $Y$ such that $\sh{B}|U$ is a locally free $\sh{O}_U$-Module (\sref[0]{0.5.2.7});
moreover, by hypothesis, the homomorphism $\operatorname{astr}_{\sh{B}_y|\sh{O}_{Y,y}}:\sh{B}_y\to\check{\sh{B}}_y$ being bijectif, one can also suppose by \sref[0]{0.5.2.7} that one can choose $U$ so that
\oldpage[IV-4]{113}
the homomorphism $\operatorname{astr}_{\sh{B}|U,\sh{O}_U}$ is bijective.
The fact that $b)$ implies $a)$ then follows from \sref{IV.18.2.3}.
\end{proof}

\begin{corollary}[18.2.5]
\label{IV.18.2.5}
Let $Y$ be a quasi-compact or locally Noetherian prescheme, $f:X\to Y$ a morphism of finite type and of finite presentation;
set $\sh{B}=f_*(\sh{O}_X)$.
Suppose that for every \emph{closed} point $y$ of $Y$, $\sh{B}_y$ is a locally free $\sh{O}_{Y,y}$-module and $\sh{B}_y\otimes_{\sh{O}_{Y,y}}\kappa(y)$ is a separable $\kappa(y)$-algebra.
Then $f$ is \'etale.
\end{corollary}

\begin{proof}
Indeed, it follows from \sref{IV.18.2.4} that every closed point $y$ of $Y$ has an open neighbourhood $U$ such that the restriction $f^{-1}(U)\to U$ of $f$ is \'etale;
the conclusion follows from the fact that in the two cases considered, every non-empty closed subset of $Y$ contains a closed point (\sref{IV.5.1.11} and \sref[0]{0.2.1.3}).
\end{proof}

\begin{corollary}[18.2.6]
\label{IV.18.2.6}
If $f:X\to Y$ is a finite \'etale morphism, and if we set $\sh{B}=f_*(\sh{O}_X)$, then the $\sh{O}_Y$-homomorphism $\operatorname{Tr}_{\sh{B}|\sh{O}_Y}:\sh{B}\to\sh{O}_Y$ \sref{IV.18.2.2} (also denoted $\operatorname{Tr}_f$) is surjective.
\end{corollary}

\begin{proof}
The question being local, we may, by virtue of \sref{IV.18.2.3}, suppose that $Y=\operatorname{Spec}(A)$, $X=\operatorname{Spec}(B)$, with $B$ a free $A$-module;
as by virtue of \sref{IV.18.2.3} the bilinear form \sref{IV.18.2.1.1} is non-degenerate, this implies in particular that the linear form $\operatorname{Tr}_{B/A}$ is surjective.
\end{proof}

\begin{remarks}[18.2.7]
\label{IV.18.2.7}
\begin{enumerate}
  \item[(i)] When $f:X\to Y$ is a finite morphism such that $f_*(\sh{O}_X)$ is a locally free $\sh{O}_Y$-Module (resp.\ locally free of rank $n$), we say further that $f$ is a \emph{finite locally free} morphism (resp.\ \emph{locally free of rank $n$}).
This condition, by virtue of \sref{IV.18.2.3}, is satisfied if $f$ is a finite \'etale morphism, but does not imply by itself that $f$ is \'etale, as shown by the example where $X=\operatorname{Spec}(K)$ and $Y=\operatorname{Spec}(k)$ are spectra of fields, $K$ being a finite non-separable extension of $k$.
When $f:X\to Y$ is a finite \'etale morphism, we say further that $X$ is an \emph{\'etale covering} of $Y$.
One notes that in this case, $f$ is universally open and universally closed, and in particular $f(X)$ is a subset that is both open and closed of $Y$.

We say that an \'etale covering $X$ of $Y$ is \emph{trivial} if $X$ is a \emph{sum} of a finite number of preschemes \emph{isomorphic} to $Y$.
We say that an \'etale covering $X$ of $Y$ is \emph{locally trivial} if the morphism $f:X\to Y$ is such that for every point $y\in Y$ there is an open neighbourhood $U$ for which the covering $f^{-1}(U)$ of $U$ is trivial.

  \item[(ii)] Let $f:X\to Y$ be a finite morphism, locally free of rank $n$;
set $\sh{B}=f_*(\sh{O}_X)$ and let $u=\operatorname{astr}_{\sh{B}|\sh{O}_Y}:\sh{B}\to\check{\sh{B}}$;
we deduce from this an $n$-th exterior power homomorphism
$\bigwedge^n u:\bigwedge^n\sh{B}\to\bigwedge^n\check{\sh{B}}=(\bigwedge^n\sh{B})^\vee$ of \emph{invertible} $\sh{O}_Y$-Modules, and by \sref[0]{0.5.4.2} an element
\[
\label{IV.18.2.7.1}
d_{X/Y}\in\Gamma(Y,(\bigwedge^n\check{\sh{B}})\otimes_{\sh{O}_Y}(\bigwedge^n\check{\sh{B}}))
\tag{18.2.7.1}
\]
which we call the \emph{discriminant} of $X$ over $Y$.
Furthermore, since $(\bigwedge^n\check{\sh{B}})\otimes_{\sh{O}_Y}(\bigwedge^n\check{\sh{B}})$ is the dual of $(\bigwedge^n\sh{B})\otimes_{\sh{O}_Y}(\bigwedge^n\sh{B})$, $d_{X/Y}$ can also be identified with a homomorphism
\[
\label{IV.18.2.7.2}
(\bigwedge^n\sh{B})\otimes_{\sh{O}_Y}(\bigwedge^n\sh{B})\to\sh{O}_Y
\tag{18.2.7.2}
\]
and we denote by $\mathscr{D}_{X/Y}$ the quasi-coherent ideal of $\sh{O}_Y$ of finite type, \emph{image} of the homomorphism \sref{IV.18.2.7.2}, also called the \emph{discriminant ideal} of $X$ over $Y$.
\end{enumerate}
\end{remarks}

\oldpage[IV-4]{114}
For the homomorphism $u$ to be bijective, it is necessary and sufficient that $\bigwedge^n u$ be bijective, or again that the section $d_{X/Y}$ have an \emph{invertible} germ at every point $y\in Y$, which is also written $d_{X/Y}(y)\neq 0$ for every $y$ (\sref[0]{0.5.5.2}).
It amounts to the same to say that the discriminant ideal $\mathscr{D}_{X/Y}$ is \emph{equal} to $\sh{O}_Y$.

The terminology ``covering'' introduced in \sref{IV.18.2.7}, (i), is justified by the following proposition:

\begin{proposition}[18.2.8]
\label{IV.18.2.8}
Let $f:X\to Y$ be a morphism \'etale, separated and of finite type, and for every $y\in Y$, let $n(y)$ be the geometric number of points of $f^{-1}(y)$.
Then the function $y\mapsto n(y)$ is lower semi-continuous on $Y$.
For it to be continuous at a point $y$ (and hence constant in a neighbourhood of $y$), it is necessary and sufficient that there exists an open neighbourhood $U$ of $y$ such that the restriction $f^{-1}(U)\to U$ of $f$ is a \emph{finite} (\'etale) morphism.
\end{proposition}

\begin{proof}
Since $f$ is quasi-finite \sref{IV.17.6.1} and locally of finite presentation, it amounts to saying that $f$ is finite or that $f$ is proper \sref{IV.8.11.1}:
moreover, each fibre $f^{-1}(y)$ is geometrically reduced over $\kappa(y)$.
The conclusions therefore follow from \sref{IV.15.5.9}, (i) and (ii) and from the fact that $f$ is flat.
\end{proof}

\begin{corollary}[18.2.9]
\label{IV.18.2.9}
Let $Y$ be a connected prescheme, $f:X\to Y$ a morphism \'etale, separated and of finite type.
For $f$ to be finite (in other words, for $X$ to be an \'etale covering of $Y$), it is necessary and sufficient that all the fibres of $f$ have the same geometric number of points.
\end{corollary}

\begin{remarks}[18.2.10]
\label{IV.18.2.10}
\begin{enumerate}
  \item[(i)] The example of the ``affine line with a doubled point'' (\sref[I]{I.5.5.11}) shows that a morphism \'etale of finite type of Noetherian preschemes may not be separated;
for this example, the first assertion of \sref{IV.18.2.8} is no longer exact.

  \item[(ii)] For a separated morphism of finite type and \'etale $f:X\to Y$ to make $X$ a \emph{locally trivial} covering, it is necessary and sufficient that for every $x\in X$ there exist an open neighbourhood $U$ of $y=f(x)$ and a $U$-section $g$ of $f^{-1}(U)$ such that $g(y)=x$.
Indeed, the condition is obviously necessary;
the fact that it is also sufficient follows from the fact that every fibre $f^{-1}(y)$ is finite (\sref{IV.17.6.1}) and \sref[I]{I.6.2.2}, from the characterisation of sections of a $Y$-scheme \'etale \sref{IV.17.9.3} and from Proposition~\sref{IV.18.2.8}.
\end{enumerate}
\end{remarks}


\subsection{Finite algebras and \'etale algebras.}
\label{subsection:IV.18.3}

\begin{proposition}[18.3.1]
\label{IV.18.3.1}
Let $A$ be a ring, $B$ an $A$-algebra of finite presentation.
\begin{enumerate}
  \item[(i)] For $B$ to be an unramified $A$-algebra, it is necessary and sufficient that $B$ be an $A$-module of finite presentation and that $(B\otimes_A B)$ be a \emph{projective} $(B\otimes_A B)$-module.
  \item[(ii)] Suppose moreover that $B$ is a finite $A$-algebra. For $B$ to be an \'etale $A$-algebra, it is necessary and sufficient that $B$ be an $A$-module projective and a $(B\otimes_A B)$-module projective.
\end{enumerate}
\end{proposition}

\begin{proof}
\oldpage[IV-4]{115}
The $(B\otimes_A B)$-module structure on $B$ is that coming from the canonical ring homomorphism $B\otimes_A B\to B$, which is surjective and has kernel $\mathfrak{J}_{B/A}$ (\sref[0]{0.20.4.1}).

(i) To say that the morphism $\operatorname{Spec}(B)\to\operatorname{Spec}(A)$ is locally of finite presentation \sref{IV.1.4.6}.
To say that $B$ is an unramified $A$-algebra is equivalent to saying that it is an $A$-algebra of finite presentation \sref{IV.1.4.7}.
To say that $B$ is unramified is equivalent to saying that $(B\otimes_A B)/\mathfrak{J}_{B/A}$ is an open and closed subset of $\operatorname{Spec}(B\otimes_A B)$ and we know that for this, it is necessary and sufficient that $\mathfrak{J}_{B/A}$ be a \emph{direct factor ideal} of $B\otimes_A B$ (Bourbaki, \emph{Alg.\ comm.}, ch.~II, \textsection~4, no.~3, prop.~15);
but this amounts to saying that the $(B\otimes_A B)$-module quotient $(B\otimes_A B)/\mathfrak{J}_{B/A}$ is projective (Bourbaki, \emph{Alg.}, ch.~II, $3^{\text{e}}$ \'ed., \textsection~2, no.~2, prop.~4).

(ii) Recall that a flat $A$-module of finite presentation is projective, and conversely (Bourbaki, \emph{Alg.\ comm.}, ch.~II, \textsection~5, no.~2, cor.~2 du th.~1), so the assertion (ii) follows from that of (i) and from \sref{IV.17.6.2}.
\end{proof}

\begin{proposition}[18.3.2]
\label{IV.18.3.2}
Let $A$ be a ring, $\mathfrak{J}$ an ideal of $A$ such that, for the $\mathfrak{J}$-preadic topology, $A$ is separated and complete;
set $A_0=A/\mathfrak{J}$.
Then the functor
\[
B\mapsto B\otimes_A A_0
\]
is an equivalence of the category of finite \'etale $A$-algebras, to the category of finite \'etale $A_0$-algebras.
\end{proposition}

\begin{proof}
We will first prove the following lemma:

\begin{lemma}[18.3.2.1]
\label{IV.18.3.2.1}
Let $A$ be a ring, $\mathfrak{J}$ an ideal of $A$ such that, for the $\mathfrak{J}$-preadic topology, $A$ is separated and complete.
\begin{enumerate}
  \item[(i)] Every projective $A$-module of finite type $M$ is separated and complete for the $\mathfrak{J}$-preadic topology, hence a projective limit of the $(A/\mathfrak{J}^{n+1})$-modules projective $M/\mathfrak{J}^{n+1}M$.
  \item[(ii)] Conversely, set $A_n=A/\mathfrak{J}^{n+1}$, and let $(M_n)$ be a projective system of $A_n$-modules, such that, for every $n$, the homomorphism $M_{n+1}\otimes_{A_{n+1}}A_n\to M_n$ deduced from the di-homomorphism of transition $M_{n+1}\to M_n$ is bijective.
Suppose moreover that the $M_n$ are projective and $M_0$ of finite type.
Then $M=\varprojlim M_n$ is a projective $A$-module of finite type such that the canonical homomorphism $M\otimes_A A_0\to M_0$ is bijective.
\end{enumerate}
\end{lemma}

\begin{proof}
(i) There is a free $A$-module of finite type $L$ such that $M$ is isomorphic to a direct factor of $L$;
as $L$ is separated for the $\mathfrak{J}$-preadic topology, so is every sub-module $N$ of $L$ since $\mathfrak{J}^{n+1}N\subset\mathfrak{J}^{n+1}L$;
in particular $M$ is separated for this topology, and since the surjective homomorphism $f:L\to M$ is continuous for the $\mathfrak{J}$-preadic topology, its kernel $N$ is closed for the topology induced by that of $L$;
since $L$ is complete and $f$ a strict morphism, we conclude that $M$ is complete (Bourbaki, \emph{Top.\ g\'en.}, ch.~IX, $2^{\text{e}}$ \'ed., \textsection~3, no.~1, prop.~4).

(ii) It follows from Nakayama's lemma that $M_0$ is generated by a finite family $(x_{i0})$ of $r$ elements and if for all $n$, $x_{in}$ is an element of $M_n$ whose image $x_{i,n-1}$ is the image in $M_{n-1}$, then $(x_{in})$ ($1\leq i\leq r$) is a system of generators of $M_n$ (Bourbaki, \emph{Alg.\ comm.}, ch.~II, \textsection~3, cor.~2 de la prop.~4).
Setting, for all $n$, $L_n=A_n^r$;
if $(e_{in})_{1\leq i\leq r}$ is the canonical base of $L_n$, let $u_n:L_n\to M_n$ be the $A$-linear map such that $u_n(e_{in})=x_{in}$ for all $i$.
By hypothesis, we have a split exact sequence
\[
0\to N_n\xrightarrow{v_n} L_n\xrightarrow{u_n} M_n\to 0
\]
\oldpage[IV-4]{116}
and since $L_n=L_{n+1}/\mathfrak{J}^{n+1}L_{n+1}$ and $M_n=M_{n+1}/\mathfrak{J}^{n+1}M_{n+1}$, the vertical arrows in the commutative diagram
\[
\xymatrix{
0\ar[r] & N_{n+1}\ar[r]^{v_{n+1}}\ar[d] & L_{n+1}\ar[r]^{u_{n+1}}\ar[d] & M_{n+1}\ar[r]\ar[d] & 0\\
0\ar[r] & N_n\ar[r]^{v_n} & L_n\ar[r]^{u_n} & M_n\ar[r] & 0
}
\]
are all three surjective.
Now we have $M=\varprojlim M_n$ and $L=A^r=\varprojlim L_n$;
setting $N=\varprojlim N_n$, $u=\varprojlim u_n$, $v=\varprojlim v_n$, by \sref[0]{0.13.2.2} the exact sequence
\[
\label{IV.18.3.2.2}
0\to N\xrightarrow{v} L\xrightarrow{u} M\to 0
\tag{18.3.2.2}
\]
is exact.
Moreover, since for every $n$, $v_n$ is left-invertible and $M_n$ is a projective $A_n$-module, by virtue of (\sref[0]{0.19.1.8}) the exact sequence \sref{IV.18.3.2.2} is split, which proves the lemma.
\end{proof}

This being established, let us first show that the functor of \sref{IV.18.3.2} is \emph{fully faithful}.
Set, as in the lemma, $A_n=A/\mathfrak{J}^{n+1}$;
let $B$, $C$ be two finite \'etale $A$-algebras, and set, for every $n$, $B_n=B\otimes_A A_n$, $C_n=C\otimes_A A_n$;
by virtue of \sref{IV.18.3.1} and \sref{IV.18.3.2.1}, $B$ and $C$ are separated and complete for the $\mathfrak{J}$-preadic topology, and we have $B=\varprojlim B_n$, $C=\varprojlim C_n$;
moreover, every $A$-algebra homomorphism $u:B\to C$ is continuous for the $\mathfrak{J}$-preadic topologies, and hence gives a projective system of $A_n$-algebra homomorphisms $u_n=u\otimes 1:B_n\to C_n$, of which it is the projective limit;
the converse being evident, we have a canonical bijection
\[
\operatorname{Hom}_{A\text{-}\mathrm{alg.}}(B,C)\xrightarrow{\sim}\varprojlim\operatorname{Hom}_{A_n\text{-}\mathrm{alg.}}(B_n,C_n).
\]
But since $B$ and $C$ are finite \'etale $A$-algebras, it follows also from \sref{IV.18.1.2.thm} that the canonical map
\[
\operatorname{Hom}_{A_{n+1}\text{-}\mathrm{alg.}}(B_{n+1},C_{n+1})\to\operatorname{Hom}_{A_n\text{-}\mathrm{alg.}}(B_n,C_n)
\]
is \emph{bijective} for $n\geq 0$, which proves that the canonical map $\operatorname{Hom}_{A\text{-}\mathrm{alg.}}(B,C)\to\operatorname{Hom}_{A_0\text{-}\mathrm{alg.}}(B_0,C_0)$ is bijective.

To complete the proof of \sref{IV.18.3.2}, it suffices to show that for every finite \'etale $A_0$-algebra $B_0$, there exists a finite \'etale $A$-algebra $B$ and an $A_0$-isomorphism $B_0\xrightarrow{\sim}B\otimes_A A_0$.
Now, by \sref{IV.18.1.2.thm}, there is a projective system $(B_n)$ such that $B_n$ is an \'etale $A_n$-algebra and that the homomorphisms $B_{n+1}\otimes_{A_{n+1}}A_n\to B_n$ are bijective.
It follows from \sref{IV.18.3.1} and \sref{IV.18.3.2.1} that the $A$-algebra $B=\varprojlim B_n$ is a projective $A$-module of finite type and that $B_0$ is isomorphic to $B\otimes_A A_0$.
To prove that $B$ is a finite \'etale $A$-algebra, it suffices, by \sref{IV.18.2.5}, to show that for every maximal ideal $\mathfrak{m}$ of $A$, $B_\mathfrak{m}/\mathfrak{m}B_\mathfrak{m}$ is a separable $(A/\mathfrak{m})$-algebra.
Now, since $\mathfrak{J}$ is contained in the radical of $A$ (\sref[0]{0.7.1.10}),
\oldpage[IV-4]{117}
we have $\mathfrak{J}\subset\mathfrak{m}$, so $\mathfrak{m}_0=\mathfrak{m}/\mathfrak{J}$, we have $A_0/\mathfrak{m}_0=A/\mathfrak{m}$ and $B_\mathfrak{m}/\mathfrak{m}B_\mathfrak{m}=(B_0)_{\mathfrak{m}_0}/\mathfrak{m}_0(B_0)_{\mathfrak{m}_0}$;
the conclusion therefore follows from the fact that $B_0$ is a finite \'etale $A_0$-algebra \sref{IV.18.2.5}.
\end{proof}

\begin{env}[18.3.3]
\label{IV.18.3.3}
\emph{Example.} ---
Proposition \sref{IV.18.3.2} applies in particular when $A$ is a \emph{local} ring, separated and complete, $\mathfrak{J}$ being the maximal ideal of $A$, so that $A_0$ is a field and the category of finite \'etale $A_0$-algebras is identical to that of finite separable $A_0$-algebras, hence isomorphic to direct composites of fields, finite separable extensions of $A_0$.
In particular, if the field $A_0$ is \emph{separably closed}, these extensions are all identical to $A_0$, and hence every \'etale covering of $\operatorname{Spec}(A)$ is \emph{trivial} \sref{IV.18.2.7} by virtue of \sref{IV.18.3.2}.
\end{env}

\begin{theorem}[18.3.4]
\label{IV.18.3.4}
Let $A$ be a Noetherian ring, $\mathfrak{J}$ an ideal of $A$ such that $A$ is separated and complete for the $\mathfrak{J}$-preadic topology, $A_0=A/\mathfrak{J}$.
Let $S=\operatorname{Spec}(A)$, $S_0=\operatorname{Spec}(A_0)$.
Let $X$ be a proper $S$-scheme, and set $X_0=X\times_S S_0$.
Then the functor
\[
Z\mapsto Z\times_X X_0
\]
from the category of finite \'etale $X$-schemes over $X$, to the category of finite \'etale $X_0$-schemes over $X_0$, is an equivalence of categories.
\end{theorem}

\begin{proof}
Let us first show that this functor is \emph{fully faithful}.
Let $Z'$ and $Z''$ be two finite \'etale $X$-schemes over $X$.
Set $S_n=\operatorname{Spec}(A/\mathfrak{J}^{n+1})$, $X_n=X\times_S S_n$, $Z'_n=Z'\times_S S_n$, $Z''_n=Z''\times_S S_n$.
It follows from (\sref[III]{III.5.4.1}) that we have a canonical bijection $\operatorname{Hom}_X(Z',Z'')\xrightarrow{\sim}\varprojlim\operatorname{Hom}_{X_n}(Z'_n,Z''_n)$.
Now, by virtue of \sref{IV.18.1.2.thm}, the canonical map $\operatorname{Hom}_{X_{n+1}}(Z'_{n+1},Z''_{n+1})\to\operatorname{Hom}_{X_n}(Z'_n,Z''_n)$ is bijective, which proves our assertion.

It remains to show that if $\sh{B}_0$ is a finite \'etale $\sh{O}_{X_0}$-Algebra, there exists an $\sh{O}_X$-Algebra $\sh{B}$, finite and \'etale, and an isomorphism $\sh{B}_0\xrightarrow{\sim}\sh{B}\otimes_{\sh{O}_X}\sh{O}_{X_0}$.
It follows from \sref{IV.18.1.2.thm} that there is a projective system $(\sh{B}_n)$, where $\sh{B}_n$ is an $\sh{O}_{X_n}$-Algebra finite and \'etale, and the second comparison theorem (\sref[III]{III.5.1.4}) proves the existence of a coherent $\sh{O}_X$-Module $\sh{B}$ and of a projective system of isomorphisms $\sh{B}_n\xrightarrow{\sim}\sh{B}\otimes_{\sh{O}_X}\sh{O}_{X_n}$.
The data of an Algebra structure on $\sh{B}$ is equivalent to that of a homomorphism $\sh{F}\otimes\sh{F}\to\sh{F}$ rendering the tensor powers commutative diagrams where only tensor powers of $\sh{F}$ appear;
by \sref[III]{III.5.1.3}, \sref[I]{I.10.11.4} and \sref[I]{I.10.11.7}, $\sh{B}$ is endowed in a natural way with a structure of $\sh{O}_X$-Algebra for which the isomorphism $\sh{B}_0\xrightarrow{\sim}\sh{B}\otimes_{\sh{O}_X}\sh{O}_{X_0}$ is an isomorphism of Algebras.
Moreover, $\sh{B}$ is an $\sh{O}_X$-Module \emph{locally free};
this also follows from \sref[III]{III.5.1.3}, \sref[I]{I.10.11.4}, \sref[I]{I.10.11.7} and from the fact that, in the category of $\sh{O}_X$-Modules, the locally free $\sh{O}_X$-Modules $\sh{F}$ can be defined as those for which the functors $\sh{G}\mapsto\shHom_{\sh{O}_X}(\sh{F},\sh{G})$ are \emph{exact}.
Finally, to see that $\sh{B}$ is a finite \'etale $\sh{O}_X$-Algebra, it suffices \sref{IV.18.2.5} to show that for every \emph{closed} point $x\in X$, $\sh{B}_x\otimes_{\sh{O}_{X,x}}\kappa(x)$ is a separable $\kappa(x)$-algebra.
But since the structural morphism $f:X\to S$ is \emph{proper}, $f(x)$ is a closed point of $S$, so it belongs to $S_0$, since $\mathfrak{J}$ is contained in the radical of $A$ (\sref[0]{0.7.1.10});
the conclusion therefore follows from the fact that $X_0=f^{-1}(S_0)$ and that $\sh{B}_0$ is a finite \'etale $\sh{O}_{X_0}$-algebra.
\end{proof}


\subsection{Local structure of unramified morphisms and of \'etale morphisms.}
\label{subsection:IV.18.4}

\oldpage[IV-4]{118}

\begin{lemma}[18.4.1]
\label{IV.18.4.1}
Let $A$ be a ring, $B$ a finite monogenic $A$-algebra, $u$ a generator of the $A$-algebra $B$, $F\in A[\mathrm{T}]$ a polynomial such that $F(u)=0$, $F'$ the derivative polynomial;
set $u'=F'(u)$.
Then the ideal of $B$, annihilator of $\Omega^1_{B/A}$, contains $u'B$;
it is equal to $u'B$ if the ideal $\mathfrak{J}$ of $A[\mathrm{T}]$ of polynomials $G$ such that $G(u)=0$, is generated by $F$, in other words if the surjective canonical homomorphism $\varphi:A[\mathrm{T}]/F.A[\mathrm{T}]\to B$ transforming the image of $\mathrm{T}$ into $u$, is bijective.
\end{lemma}

\begin{proof}
Set $C=A[\mathrm{T}]$, so that $B=C/\mathfrak{J}$.
We have the exact sequence (\sref[0]{0.20.5.12.1})
\[
\mathfrak{J}/\mathfrak{J}^2\to\Omega^1_{C/A}\otimes_C B\to\Omega^1_{B/A}\to 0
\]
and $\Omega^1_{B/A}$ therefore identifies with the quotient $B/\mathfrak{J}'$, where $\mathfrak{J}'$ is the ideal generated by the elements $G'(u)$, where $G$ ranges over a system of generators of the ideal $\mathfrak{J}$ (\sref[0]{0.20.5.13});
whence the lemma immediately follows.
\end{proof}

\begin{proposition}[18.4.2]
\label{IV.18.4.2}
Let the notation be those of \sref{IV.18.4.1}, let $\mathfrak{q}$ be a prime ideal of $B$.
Then:
\begin{enumerate}
  \item[(i)] If $\mathfrak{q}$ does not contain $u'$, $B_\mathfrak{q}$ is an $A_\mathfrak{p}$-algebra formally unramified ($\mathfrak{p}$ being the inverse image of $\mathfrak{q}$ in $A$);
in other terms, $\operatorname{Spec}(B_u)$ is formally unramified over $\operatorname{Spec}(A)$.
  \item[(ii)] Suppose moreover that $F$ is unitaire and generates $\mathfrak{J}$.
Then, for $\operatorname{Spec}(B)$ to be \'etale over $\operatorname{Spec}(A)$ at the point $\mathfrak{q}$, it is necessary and sufficient that $u'\notin\mathfrak{q}$.
\end{enumerate}
\end{proposition}

\begin{proof}
The hypothesis that $u'\notin\mathfrak{q}$ implies that $\Omega^1_{B_\mathfrak{q}/A_\mathfrak{p}}=0$ (\sref[0]{0.20.5.9}), hence (i) follows from \sref{IV.17.2.1}.
Moreover, under the hypotheses of (ii), $B$ is a free $A$-module by virtue of the Euclidean division;
as the annihilator $\mathfrak{J}'$ of $\Omega^1_{B/A}$ is then equal to $u'B$ by virtue of \sref{IV.18.4.1} and $\Omega^1_{B/A}$ is a $B$-module of finite presentation \sref{IV.16.4.22}, the annihilator of $\Omega^1_{B_\mathfrak{q}/A_\mathfrak{p}}$ is equal to $u'B_\mathfrak{q}$ (Bourbaki, \emph{Alg.\ comm.}, ch.~II, \textsection~2, no.~4, formule (g)), and (ii) follows from (i) and from the implication $c)\Rightarrow a)$ in \sref{IV.17.6.1}.
\end{proof}

\begin{corollary}[18.4.3]
\label{IV.18.4.3}
Let the notation be those of \sref{IV.18.4.2}, suppose that $F$ is unitaire and generates $\mathfrak{J}$.
Then, for $B$ to be an \'etale $A$-algebra, it is necessary and sufficient that $u'$ be invertible in $B$ (or, what amounts to the same, that the ideal of $A[\mathrm{T}]$ generated by $F$ and $F'$ be equal to $A[\mathrm{T}]$).
\end{corollary}

\begin{proof}
In view of \sref{IV.18.4.2},~(ii), saying that $\operatorname{Spec}(B)$ is \'etale over $\operatorname{Spec}(A)$ means that $u'$ does not belong to any prime ideal of $B$, i.e.\ that it is invertible in $B$.
\end{proof}

We say that a monic polynomial $F\in A[\mathrm{T}]$ such that the ideal of $A[\mathrm{T}]$ generated by $F$ and $F'$ is equal to $A[\mathrm{T}]$ is \emph{separable};
it is immediate that this definition coincides with the usual definition (Bourbaki, \emph{Alg.}, ch.~V, \textsection~7, no.~6) when $A$ is a field.

\begin{lemma}[18.4.4]
\label{IV.18.4.4}
Let $A$ be a local ring, $B$ a finite monogenic $A$-algebra, $u$ a generator of the $A$-algebra $B$.
Let $\mathfrak{n}_i$ ($1\leq i\leq r$) be the maximal ideals of $B$ such that $\operatorname{Spec}(B)$ is formally unramified over $\operatorname{Spec}(A)$ at the points $\mathfrak{n}_i$.
Then there exists a monic unitaire polynomial $F\in A[\mathrm{T}]$ such that $F(u)=0$;
such a polynomial $F$ satisfies the conditions $F'(u)\notin\mathfrak{n}_i$ for all $i$ ($1\leq i\leq r$).
Moreover, if $k$ is the
\oldpage[IV-4]{119}
residue field of $A$, $f\in k[\mathrm{T}]$ the minimal polynomial of the image of $u$ in $B\otimes_A k$, there exists an $F\in A[\mathrm{T}]$ such that $F(u)=0$;
such an $F$ satisfies the conditions $F'(u)\notin\mathfrak{n}_i$ for $1\leq i\leq r$.
\end{lemma}

\begin{proposition}[18.4.5]
\label{IV.18.4.5}
Let $A$ be a local ring, $k$ its residue field, $B$ an $A$-algebra.
Suppose moreover that the residue field $k$ is \emph{infinite}, or else that $B$ is a local ring.
Let $n$ be the rank of $L=B\otimes_A k$ over $k$.
For $B$ to be a formally unramified $A$-algebra (resp.\ \'etale), it is necessary and sufficient that there exist a monic separable polynomial $F\in A[\mathrm{T}]$ such that $B$ is isomorphic to a quotient of $A[\mathrm{T}]/F.A[\mathrm{T}]$ (resp.\ isomorphic to $A[\mathrm{T}]/F.A[\mathrm{T}]$).
Moreover, one can (and necessarily must) suppose that $F$ is of degree $n$ (resp.\ of degree $n$).
\end{proposition}

\begin{proof}
The conditions are sufficient by virtue of \sref{IV.18.4.2}, without assuming either that $k$ is infinite or that $B$ is local.
To see that the conditions are necessary, note that if $B$ is a formally unramified $A$-algebra, $L$ is a finite algebra and separable over $k$, hence a direct composite of a finite number of fields, finite separable extensions $k_j$ of $k$ ($1\leq j\leq r$), $k_j$ being generated by an element $\bar{\xi}_j$, of minimal polynomial $f_j$ ($1\leq j\leq r$) (Bourbaki, \emph{Alg.}, ch.~V, \textsection~11, no.~4, prop.~4).
Let us show that by virtue of the hypotheses made on $k$ or $B$, there exists an element $\bar{\xi}$ of $L$ generating the $k$-algebra $L$.
This is immediate if $B$ is local, since then $r=1$.
Otherwise, $k$ being assumed infinite, one can suppose that the irreducible polynomials $f_j\in k[\mathrm{T}]$ are all \emph{distinct} (by replacing each $\bar{\xi}_j$ by $\bar{\xi}_j+a_j$, for a suitable element $a_j\in k$);
if we set $f=f_1 f_2\ldots f_r$, it is clear that $L$ is isomorphic to $k[\mathrm{T}]/f.k[\mathrm{T}]$ in both cases, hence generated by an element $\bar{\xi}$ of minimal polynomial $f\in k[\mathrm{T}]$ of degree $n$.
Note now that $\mathfrak{m}B$ is contained in the radical of $B$;
as $1,\bar{\xi},\ldots,\bar{\xi}^{n-1}$ form a base of $L$ over $k$, it follows by Nakayama's lemma that $1,u,\ldots,u^{n-1}$ generate the $A$-module $B$, and hence there exists a monic unitaire polynomial $F\in A[\mathrm{T}]$ of degree $n$ such that $F(u)=0$;
moreover, since $\bar{\xi}$ is a root of the canonical image of $F$ in $k[\mathrm{T}]$, this image is necessarily equal to $f$.
But then $F'(\bar{\xi})$ of $F$ in $L$ is $f'(\bar{\xi})$, and as $f'(\bar{\xi})\neq\mathfrak{n}_i$ by \sref{IV.18.4.4}, this proves that $F'(u)$ is invertible in $B$, hence $F$ is a separable polynomial.
Finally, if $B$ is an \'etale $A$-algebra, $B$ being an $A$-module flat of finite presentation \sref{IV.1.4.7}, and $1,u,\ldots,u^{n-1}$ form a base of the $A$-module $B$ (Bourbaki, \emph{Alg.\ comm.}, ch.~II, \textsection~3, no.~2, prop.~5), in other words the $A$-algebra $B$ is isomorphic to $A[\mathrm{T}]/F.A[\mathrm{T}]$.
\end{proof}

\begin{theorem}[18.4.6]
\label{IV.18.4.6}
(Chevalley). ---
\begin{enumerate}
  \item[(i)] Let $f:X\to Y$ be a morphism locally of finite type, $x$ a point of $X$, $y=f(x)$, and set $A=\sh{O}_{Y,y}$.
For $\sh{O}_{X,x}$ to be an $A$-algebra formally unramified (resp.\ formally \'etale), it is necessary and sufficient that there exist a monic unitaire polynomial $F\in A[\mathrm{T}]$ and a maximal ideal $\mathfrak{n}$ such that $B=A[\mathrm{T}]/F.A[\mathrm{T}]$ (resp.\ an algebra quotient $B$ of $A[\mathrm{T}]/F.A[\mathrm{T}]$) such that $\sh{O}_{X,x}$ is $A$-isomorphic to $B_\mathfrak{n}$ and that, if $u$ is the image of $\mathrm{T}$ in $B$, one has $F'(u)\notin\mathfrak{n}$.
  \item[(ii)] Suppose in addition that $f$ is locally of finite presentation.
Then, for $f$ to be \'etale at the point $x$, it is necessary and sufficient that we can moreover take $B=A[\mathrm{T}]/F.A[\mathrm{T}]$.
\end{enumerate}
\end{theorem}

\begin{proof}
\oldpage[IV-4]{120}
The conditions are sufficient by virtue of \sref{IV.18.4.2}.
To see that they are necessary, and taking into account the remark \sref{IV.17.1.2}, when $f$ is formally unramified and quasi-finite, since $f$ is affine, it follows from \sref{IV.8.12.8} that there exists a finite $A$-algebra $C$ and a maximal ideal $\mathfrak{r}$ such that $\sh{O}_{X,x}$ is $A$-isomorphic to $C_\mathfrak{r}$.
Moreover, by \sref{IV.17.4.1.2} the residue field $C_\mathfrak{r}/\mathfrak{r}C_\mathfrak{r}$ is a finite separable extension of $k=A/\mathfrak{m}$.
Let $\mathfrak{r}_i$ ($1\leq i\leq h$) be the maximal ideals of the semi-local ring $C$ \emph{other than} $\mathfrak{r}$;
there exists an element $u\in C$ belonging to all the $\mathfrak{r}_i$ and such that its image in $C/\mathfrak{r}$ equals $v$ (Bourbaki, \emph{Alg.\ comm.}, ch.~II, \textsection~1, no.~2, prop.~5).
We shall now show that the sub-$A$-algebra $B=A[u]$ of $C$ is finite on $B$ and $\mathfrak{n}=\mathfrak{r}\cap B$ answers the question.

\oldpage[IV-4]{121}
To handle the case where $\sh{O}_{X,x}$ is a formally ramified $A$-algebra, it suffices to prove that $B_\mathfrak{n}$ is isomorphic to $C_\mathfrak{r}$;
indeed, $B_\mathfrak{n}$ will then be formally unramified over $A$ and the existence of the polynomial $F\in A[\mathrm{T}]$ having the properties of the statement will follow from \sref{IV.18.4.4}.
Note now that, since $f$ is formally unramified, $B_\mathfrak{n}$ is a $B'_\mathfrak{n}$-algebra formally \'etale;
with the preceding notations, since $B'$ does not belong to $\mathfrak{n}'$ by hypothesis, the morphism $\operatorname{Spec}(B')\to\operatorname{Spec}(A)$ is \'etale at the point $\mathfrak{n}'$ by \sref{IV.18.4.2},~(ii).
As by hypothesis $\operatorname{Spec}(B)\to\operatorname{Spec}(A)$ is \'etale at the point $\mathfrak{n}$, one concludes by \sref{IV.17.3.4} that $\operatorname{Spec}(B)\to\operatorname{Spec}(B')$ is \'etale at the point $\mathfrak{n}$;
but since this morphism is an immersion, it cannot be \'etale at a point unless it is a local isomorphism at that point \sref{IV.17.9.1}, hence $B_\mathfrak{n}$ and $B'_{\mathfrak{n}'}$ are isomorphic.

Finally, suppose that $\sh{O}_{X,x}$ is a formally \'etale $A$-algebra;
with the preceding notations, by \sref{IV.17.1.5} that $B_\mathfrak{n}$ is a $B'_\mathfrak{n}$-algebra formally \'etale;
but since the homomorphism $B'_{\mathfrak{n}'}\to B_\mathfrak{n}$ is surjective, this can only happen if this homomorphism is \emph{bijective} (\sref[0]{0.19.10.3}, (i)).
This completes the proof of \sref{IV.18.4.6}.
\end{proof}

\begin{corollary}[18.4.7]
\label{IV.18.4.7}
Let $f:X\to Y$ be a morphism locally of finite type, $x$ a point of $X$.
For $f$ to be formally unramified at the point $x$, it is necessary and sufficient that there exist an open neighbourhood $U$ of $x$ such that $f|U$ factorises as $U\xrightarrow{j}X'\xrightarrow{h}Y$, where $h$ is an \'etale morphism and $j$ is a closed immersion.
\end{corollary}

\begin{proof}
\oldpage[IV-4]{122}
We can obviously reduce to the case where $Y=\operatorname{Spec}(R)$ is affine and $f$ is of finite type.
If $A=\sh{O}_{Y,y}$, with $y=f(x)$, the condition for $f$ to be formally unramified at the point $x$ is equivalent, by virtue of \sref{IV.17.4.1.2}, to saying that $\sh{O}_{X,x}$ is a formally unramified $A$-algebra.
If it is so, we can apply \sref{IV.18.4.6}, (i);
replacing $Y$ as needed by an affine neighbourhood of $y$, we may suppose (with the notation of \sref{IV.18.4.6}) that the polynomial $F$ is the image in $A[\mathrm{T}]$ of a monic unitaire polynomial $G\in R[\mathrm{T}]$.
We then set $X'=\operatorname{Spec}(R[\mathrm{T}]/G.R[\mathrm{T}])$;
let $x'$ be the image of the point $\mathfrak{n}$ of $\operatorname{Spec}(B)$ by the morphism corresponding to the composite homomorphism $R[\mathrm{T}]/G.R[\mathrm{T}]\to A[\mathrm{T}]/F.A[\mathrm{T}]\to B$.
It follows from \sref{IV.18.4.2} that the canonical morphism $h:X'\to Y$ is \'etale at the point $x$, hence, restricting as needed $U$ and $X'$, we can suppose $h$ \'etale.
On the other hand, by virtue of \sref[I]{I.6.5.1}, (ii)) and \sref{IV.1.7.2}, it follows from the fact that we have a local homomorphism $\varphi:\sh{O}_{X',x'}\to\sh{O}_{X,x}$ that this homomorphism corresponds to a morphism $j:U\to X'$ from an open neighbourhood $U$ of $x$ in $X'$;
in applying \sref[I]{I.6.5.1}, (i)), suppose that $h\circ j=f|U$, hence $j$ is of finite type (\sref[I]{I.6.3.4}, (v));
finally, the homomorphism $\varphi$ is surjective by \sref{IV.18.4.6},~(i), hence it follows from (\sref[I]{I.6.5.4}, (i)) that we can, restricting further $U$ and $X'$, suppose that $j$ is a closed immersion.
This proves the necessity of the condition stated;
the sufficiency is immediate \sref{IV.17.1.3}, (i) and (ii).
\end{proof}

\begin{corollary}[18.4.8]
\label{IV.18.4.8}
Let $f:X\to Y$ be a morphism locally of finite presentation.
For $f$ to be non ramified (resp.\ \'etale), it is necessary and sufficient that there exist a family of flat morphisms $g_\alpha:Y'_\alpha\to Y$ and, for each $\alpha$, an open subset $U_\alpha$ in $X'_\alpha=X\times_Y Y'_\alpha$, such that, if $g'_\alpha:X'_\alpha\to X$ and $f'_\alpha:X'_\alpha\to Y'_\alpha$ are the canonical projections, the $g'_\alpha(U_\alpha)$ form a covering of $X$, and that each of the composed morphisms $U_\alpha\to X'_\alpha\xrightarrow{f'_\alpha}Y'_\alpha$ is a closed immersion (resp.\ open).
One can moreover take the $g_\alpha$ to be \'etale.
\end{corollary}

\begin{proof}
The necessity of the condition for \'etale morphisms is trivial, taking a single $Y'_\alpha$ equal to $X$, the morphism $g_\alpha$ corresponding to the diagonal, and the open subset $U\subset X\times_Y X$ being the diagonal.
When $f$ is non ramified, the necessity of the condition follows from \sref{IV.18.4.7};
on the other hand we need to find an open covering $(V_\alpha)$ of $X$ such that for each $\alpha$, $f|V_\alpha$ factorises as $V_\alpha\xrightarrow{j_\alpha}X'_\alpha\xrightarrow{g_\alpha}Y$, where $j_\alpha$ is a closed immersion and $g_\alpha$ an \'etale morphism.
Then $j_\alpha:V_\alpha\to Y'_\alpha$ is a $V_\alpha$-section of $X'_\alpha$, and since $f|V_\alpha$ is \'etale, $s_\alpha$ is a $V_\alpha$-section of $X'_\alpha$, and since $g_\alpha$ is \'etale, the morphism $s_\alpha$ is an immersion ouverte \sref{IV.17.4.1}, and it suffices to take $U_\alpha=s_\alpha(V_\alpha)$ to answer the question.

The sufficiency of the conditions follows from \sref{IV.17.7.1};
one concludes at each point $x$ of $g'(U_\alpha)$, hence in all of $X$ since the $g'_\alpha(U_\alpha)$ cover $X$.
\end{proof}

\begin{proposition}[18.4.9]
\label{IV.18.4.9}
Let $S$ be a prescheme, $f:X\to S$ a morphism, $h:Y\to S$ an $S$-morphism, $g:X\to Y$ an $S$-morphism, $x$ a point of $X$, $y=g(x)$.
The following conditions are equivalent:
\begin{enumerate}[label=\emph{\alph*)}]
  \item $h$ is \'etale at the point $y$ and $g$ is flat at the point $x$.
  \item $h$ is non ramified at the point $y$ and $f$ is flat at the point $x$.
\end{enumerate}
\end{proposition}

\begin{proof}
Since $f=h\circ g$, $a)$ implies that $f$ is flat at the point $x$ \sref{IV.2.1.6}, and it is obvious that $h$ being non ramified at the point $y$, hence $a)$ implies $b)$.

To prove that $b)$ implies $a)$, we can first suppose that $h$ is non ramified (replacing $Y$ by an open neighbourhood of $y$);
then, replacing $S$ by an open neighbourhood of $s=h(y)=f(x)$, one can suppose that there exists an \'etale morphism $u:S'\to S$, a point $y'$ of $Y'=Y_{(S')}$ above $y$ in $Y'$ and an open neighbourhood $V$ of $y'$ in $Y'$ such that, if the restriction of $h'$ to $V$ is a closed immersion \sref{IV.18.4.8}.
If one proves that $h'$ is \'etale at the point $y'$, it will follow that $h$ is \'etale at the point $y$ \sref{IV.17.7.1}, (ii);
moreover, $f'=f_{(S')}:X_{(S')}\to S'$ is flat at every point $x'$ above $x$.
As the projections $v:Y'\to Y$, $w:X'\to X$ are \'etale morphisms (hence flat), if one proves that $g'=g_{(S')}:X_{(S')}\to Y_{(S')}$ is flat at the point $x'$, it will follow from \sref{IV.2.2.11}, (iv) that $g$ is flat at the point $x$.
We can hence reduce to the case where $h$ is a \emph{closed immersion}
\oldpage[IV-4]{123}
of finite presentation, $f$ being supposed flat at the point $x$ and $f$ is of finite type.
Let $\sh{J}$ be the quasi-coherent Ideal of $\sh{O}_S$ of finite type defining the closed sub-prescheme $Y$ of $S$.
The hypothesis that $f$ is flat at the point $x$ implies that the homomorphism $\sh{O}_{S,s}\to\sh{O}_{X,x}$ is injective (\sref[0]{0.6.5.1});
\emph{a fortiori} the homomorphism $\sh{O}_{S,s}\to\sh{O}_{Y,y}$ is injective, which means that $\sh{J}_s=0$, since it is the kernel of the preceding homomorphism.
Since $\sh{J}$ is of finite type, there exists an open neighbourhood $U$ of $s$ in $S$ such that $\sh{J}|U=0$ (\sref[0]{0.5.2.2}).
We can hence suppose that $h$ is an open immersion, and it is clear that $g$ is flat at the point $x$.
\end{proof}

\begin{proposition}[18.4.10]
\label{IV.18.4.10}
Let us denote by $\mathbf{P}(f,x)$ a property satisfying the following conditions:
\begin{enumerate}
  \item[$1^\circ$] For every morphism $f:X\to Y$ and every local isomorphism $h:Y\to Z$, $\mathbf{P}(f,x)$ is equivalent to $\mathbf{P}(h\circ f,x)$ for $x\in X$.
  \item[$2^\circ$] For every morphism $f:X\to Y$, every \'etale morphism $g:Y'\to Y$, every point $x\in X$, if we set $X'=X\times_Y Y'$, $f'=f_{(Y')}:X'\to Y'$ and if $x'\in X'$ is above $x$, the properties $\mathbf{P}(f,x)$ and $\mathbf{P}(f',x')$ are equivalent (``invariance by \'etale base change'').
\end{enumerate}
Let then $S$ be a prescheme, $f:X\to S$ and $h:Y\to S$ two morphisms, $g:X\to Y$ an $S$-morphism, $x$ a point of $X$, and suppose $h$ \'etale at the point $y$.
Then the properties $\mathbf{P}(f,x)$ and $\mathbf{P}(g,x)$ are equivalent.
\end{proposition}

\begin{proof}
With the same notations as in the proof of \sref{IV.18.4.9} and replacing $S$ (resp.\ $Y$) by an open neighbourhood of $s=h(y)$ (resp.\ $y$), we may find, by \sref{IV.18.4.8}, an \'etale morphism $u:S'\to S$, a point $y'$ of $Y'=Y_{(S')}$ and an open neighbourhood $V$ of $y'$ in $Y'$ such that $h'$ restricted to $V$ is an open immersion;
if $x'\in X'$ is any point above $x$, $\mathbf{P}(f',x')$ (resp.\ $\mathbf{P}(g',x')$) is then equivalent to $\mathbf{P}(f,x)$ (resp.\ $\mathbf{P}(g,x)$).
The hypothesis implies that $\mathbf{P}(g,x)$ is then equivalent to $\mathbf{P}(h\circ g,x)$, and we deduce the conclusion.
\end{proof}

Let then $S$ be a prescheme, $f:X\to S$ and $h:Y\to S$ two morphisms, $g:X\to Y$ an $S$-morphism, $x$ a point of $X$, and suppose $h$ \'etale at the point $y$.
Then the properties $\mathbf{P}(f,x)$ and $\mathbf{P}(g,x)$ are equivalent.

\begin{env}[18.4.11]
\label{IV.18.4.11}
\emph{Examples.} ---
One can take for the property $\mathbf{P}(f,x)$ any one of the following, taking into account \sref{IV.17.7.4},~(ii):
\begin{enumerate}
  \item[(i)] $f$ is flat at the point $x$ \sref{IV.2.2.11}, (iv);
  \item[(ii)] $f$ is locally of finite presentation and of coprofondeur $\leq n$ at the point $x$ (\sref{IV.6.8.1} and \sref{IV.6.7.8});
  \item[(iii)] $f$ is locally of finite presentation and of Cohen-Macaulay at the point $x$ (\sref{IV.6.8.1} and \sref{IV.6.7.8});
  \item[(iv)] $f$ is locally of finite presentation and possesses the property $(S_n)$ at the point $x$ (\sref{IV.6.8.1} and \sref{IV.6.7.8});
  \item[(v)] $f$ is locally of finite presentation and possesses the property $(R_n)$ at the point $x$ (\sref{IV.6.8.1} and \sref{IV.6.7.8});
  \item[(vi)] $f$ is locally of finite presentation and normal at the point $x$ (\sref{IV.6.8.1} and \sref{IV.6.7.8});
  \item[(vii)] $f$ is locally of finite presentation and reduced at the point $x$ (\sref{IV.6.8.1} and \sref{IV.6.7.8});
  \item[(viii)] $f$ is non ramified at the point $x$ \sref{IV.17.7.4};
  \item[(ix)] $f$ is smooth at the point $x$ \sref{IV.17.7.4};
  \item[(x)] $f$ is \'etale at the point $x$ \sref{IV.17.7.4}.
\end{enumerate}
\end{env}

\begin{corollary}[18.4.12]
\label{IV.18.4.12}
\begin{enumerate}
  \item[(i)] Let $f:X\to Y$ be a morphism locally of finite type, $x$ a point of $X$.
If $f$ is flat and formally unramified at the point $x$, then, in every factorisation $f|U:U\xrightarrow{j}X'\xrightarrow{h}Y$, where $U$ is an open neighbourhood of $x$, $j$ a closed immersion and $h$ an \'etale morphism, the homomorphism $\sh{O}_{X',j(x)}\to\sh{O}_{X,x}$ corresponding to $j$ is bijective (in particular $\sh{O}_{X,x}$ is an essentially \'etale $\sh{O}_{Y,f(x)}$-algebra \sref{IV.18.6.1}).
  \item[(ii)] For a morphism $f:X\to Y$ to be \'etale, it is necessary and sufficient that it be locally of finite type, formally unramified and flat.
\end{enumerate}
\end{corollary}

\begin{proof}
\oldpage[IV-4]{124}
(i) Since the homomorphism $\sh{O}_{X',j(x)}\to\sh{O}_{X,x}$ is surjective, it suffices to prove that it is injective, and for that it suffices to prove that $\sh{O}_{X,x}$ is an $\sh{O}_{X',j(x)}$-module \emph{faithfully flat}, or merely \emph{flat} (\sref[0]{0.6.5.1} and \sref{IV.6.6.2});
in other words, it amounts to showing that $j$ is a flat morphism at the point $x$;
but since $h\circ j=f$ is flat at the point $x$ by hypothesis and $h$ is \'etale, this follows from \sref{IV.18.4.10} and \sref{IV.18.4.11}, (i).

(ii) It suffices to prove the sufficiency of the stated conditions.
For every $x\in X$, we have in a neighbourhood open $U$ of $x$ a factorisation of $f|U$ having the properties stated in (i), and the fact that $f$ is flat and formally non ramified at \emph{all} the points of $U$.
For every $x\in X$, the quasi-coherent Ideal of $\sh{O}_{X'}$ corresponding to the closed sub-prescheme $X'$ associated to $j$, we have $\sh{J}_{j(x)}=0$ for every $z\in U$, hence $j$ is an open immersion, and $f$ is hence \'etale at every point of $X$.
\end{proof}

\begin{corollary}[18.4.13]
\label{IV.18.4.13}
Let $f:X\to Y$ be a morphism locally of finite type, $x$ a point of $X$, $y=f(x)$.
Suppose that $y$ admits an open neighbourhood which is a reduced prescheme and has only a finite number of irreducible components.
Then, for $f$ to be \'etale at the point $x$, it is necessary and sufficient that $f$ be flat and formally non ramified at the point $x$.
\end{corollary}

\begin{proof}
There is only the sufficiency of the condition to prove.
The question being local on $X$ and $Y$, one can suppose \sref{IV.18.4.7} that $f$ factorises as $X\xrightarrow{j}X'\xrightarrow{h}Y$ where $h$ is \'etale and $j$ a closed immersion.
Then $X'$ is reduced \sref{IV.17.5.7} and, replacing $X'$ by an open neighbourhood of $j(x)$, one can suppose that $X'$ has only a finite number of irreducible components.
Then the maximal points of $X'$ are above the maximal points of $Y$ \sref{IV.2.3.4} and since they are finite, their number is finite.
It all amounts to showing, with the notations of the proof of \sref{IV.18.4.12}, that $\sh{J}_{x'}=0$ for all points $x'$ in a neighbourhood of $j(x)$ in $X'$, knowing that $\sh{J}_{j(x)}=0$.
Now, replacing $X'$ by an affine neighbourhood of $j(x)$, we can suppose that all the irreducible components of $X'$ contain $j(x)$;
if $X'=\operatorname{Spec}(A')$, and if $\mathfrak{p}'$ is the prime ideal of $A'$ corresponding to the point $j(x)$, the morphism $\operatorname{Spec}(A'_{\mathfrak{p}'})\to\operatorname{Spec}(A')$ is dominant, hence the corresponding homomorphism $A'\to A'_{\mathfrak{p}'}$ is \emph{injective} since $A'$ is reduced \sref[I]{I.1.2.7}.
If $\sh{J}=\widetilde{\mathfrak{J}}$, where $\mathfrak{J}$ is an ideal of $A'$, $\mathfrak{J}$ identifies with an ideal of $A'$, $\mathfrak{J}_{\mathfrak{p}'}=0$ by hypothesis, hence $\mathfrak{J}=0$.
\end{proof}

\begin{corollary}[18.4.14]
\label{IV.18.4.14}
Let $A$ be a local ring with maximal ideal $\mathfrak{m}$, residue field $k$, $B$ a finite $A$-algebra.
\begin{enumerate}
  \item[(i)] For $B$ to be a formally unramified $A$-algebra, it is necessary and sufficient that $B\otimes_A k$ be a $k$-algebra \'etale.
  \item[(ii)] For $B$ to be a finite \'etale $A$-algebra, it is necessary and sufficient that $B\otimes_A k$ be an \'etale $k$-algebra and that $B$ be a flat $A$-module (which is equivalent to saying (\sref[0]{0.6.3.3}) that for every maximal ideal $\mathfrak{n}$ of $B$ (necessarily above $\mathfrak{m}$), $B_\mathfrak{n}$ is a flat $A$-module).
\end{enumerate}
\end{corollary}

\begin{proof}
\oldpage[IV-4]{125}
It is clear that (ii) follows from (i) and \sref{IV.18.4.12},~(ii), taking into account \sref{IV.17.1.2},~(i).
To prove (i), let us note that if $B\otimes_A k$ is an \'etale $k$-algebra, we have $\Omega^1_{B\otimes_A k/k}=0$ \sref{IV.17.2.1}.
Since $\Omega^1_{B\otimes_A k/k}=\Omega^1_{B/A}\otimes_B k$ (\sref[0]{0.20.5.5}) and since $B$ is an $A$-algebra of finite type, $\Omega^1_{B/A}$ is a $B$-module of finite type (\sref[0]{0.20.4.7}).
But since $B$ is a finite $A$-algebra, $\mathfrak{m}B$ is contained in the radical of $B$ (Bourbaki, \emph{Alg.\ comm.}, ch.~V, \textsection~2, no.~1, prop.~1), hence the lemma of Nakayama proves that $\Omega^1_{B/A}=0$, and hence $B$ is a formally unramified $A$-algebra \sref{IV.17.2.1}.
\end{proof}


\subsection{Henselian local rings.}
\label{subsection:IV.18.5}

\begin{env}[18.5.1]
\label{IV.18.5.1}
Let $X$ be a prescheme, $\sh{E}$ a locally free $\sh{O}_X$-Module of finite rank;
the dual $\check{\sh{E}}=\shHom_{\sh{O}_X}(\sh{E},\sh{O}_X)$ is therefore a locally free $\sh{O}_X$-Module whose rank at every point is equal to that of $\sh{E}$ at that point, and the canonical homomorphism $\sh{E}\to\shHom_{\sh{O}_X}(\check{\sh{E}},\sh{O}_X)=(\check{\sh{E}})^\vee$ is \emph{bijective}.
For every morphism $X'\to X$, set $\sh{E}_{(X')}=\sh{E}\otimes_{\sh{O}_X}\sh{O}_{X'}$ which is a locally free $\sh{O}_{X'}$-Module, and consider the \emph{set} $\Gamma(X',\sh{E}_{(X')})$ of sections of this $\sh{O}_{X'}$-Module over $X'$.
We shall see that this defines a \emph{contravariant representable functor}
\[
\label{IV.18.5.1.1}
{}^t\! V_{\sh{E}}:X'\mapsto\Gamma(X',\sh{E}_{(X')})
\tag{18.5.1.1}
\]
\oldpage[IV-4]{126}
from the category of $X$-preschemes to the category of \emph{sets} (\sref[0]{0.8.1.8}).

First of all, this is indeed a well-defined functor, for if $f:X''\to X'$ is an $X$-morphism, we have $\sh{E}_{(X'')}=f^*(\sh{E}_{(X')})$, whence (\sref[0]{0.4.3.2}) a map $\Gamma(X',\sh{E}_{(X')})\to\Gamma(X'',\sh{E}_{(X'')})$ which completes the definition of the functor ${}^t\! V$.
Let us then show that the $X$-prescheme $\mathbf{V}(\check{\sh{E}})$ (\sref[II]{II.1.7.8}) represents the functor ${}^t\! V$.
It is immediate that $(\sh{E}_{(X')})^\vee=(\check{\sh{E}})_{(X')}$, hence $\mathbf{V}(\check{\sh{E}})\times_X X'=\mathbf{V}((\check{\sh{E}})_{(X')})$;
taking into account \sref[I]{I.3.3.14}, we are reduced to defining a bijection $\Gamma(\mathbf{V}(\check{\sh{E}}_{(X')})/X')\xrightarrow{\sim}\Gamma(X',\sh{E})$, and to verify that the bijection $\Gamma(\mathbf{V}(\check{\sh{E}})/X)\xrightarrow{\sim}\Gamma(X,\sh{E})$ is functorial in $X'$.
Now, on the one hand a section $u\in\Gamma(X,\sh{E})$ identifies canonically with an $\sh{O}_X$-homomorphism $u:\sh{O}_X\to\sh{E}$, to which corresponds by transposition an $\sh{O}_X$-homomorphism ${}^t u:\check{\sh{E}}\to\sh{O}_X$, and hence an $\sh{O}_X$-Algebra homomorphism $v:\mathbf{S}_{\sh{O}_X}(\check{\sh{E}})\to\sh{O}_X$.
Let $\mathrm{Al}(X,\sh{E},\sh{J})$ denote the set of $u\in\Gamma(X,\sh{E})$ such that $\sh{J}$ is contained in the kernel of $v$;
it follows immediately from these definitions that
\[
X'\mapsto\mathrm{Al}(X',\sh{E}_{(X')},\sh{J}\otimes_{\sh{O}_X}\sh{O}_{X'})
\]
is a representable functor by $\operatorname{Spec}(\mathbf{S}_{\sh{O}_X}(\check{\sh{E}})/\sh{J})$.
If $X'=\operatorname{Spec}(A)$ is affine and such that $\sh{E}_{(X')}$ is isomorphic to $\sh{O}_{X'}^n$, $\Gamma(X',\sh{E}_{(X')})$ can be identified with the set $A^n$;
in an obvious way, we can say that the object $\operatorname{Spec}(\mathbf{S}_{\sh{O}_X}(\check{\sh{E}})/\sh{J})$ represents ``the algebraic subset of the twisted affine space over $X$ defined by $\sh{E}$''.
We write this $X$-prescheme $\mathbf{Al}(\sh{E},\sh{J})$.
We note also that if $\sh{J}$ is a type-finite Ideal of $\mathbf{S}_{\sh{O}_X}(\check{\sh{E}})$, $\mathbf{Al}(\sh{E},\sh{J})$ is an $X$-prescheme of finite presentation, since $\mathbf{S}_{\sh{O}_X}(\check{\sh{E}})$ is an $\sh{O}_X$-Algebra of finite presentation.
\end{env}

\begin{env}[18.5.2]
\label{IV.18.5.2}
Recall (\sref[II]{II.1.7.8}) that by definition $\mathbf{V}(\check{\sh{E}})=\operatorname{Spec}(\mathbf{S}_{\sh{O}_X}(\check{\sh{E}}))$.
Let $\sh{J}$ be a quasi-coherent Ideal of $\mathbf{S}_{\sh{O}_X}(\check{\sh{E}})$, so that $\operatorname{Spec}(\mathbf{S}_{\sh{O}_X}(\check{\sh{E}})/\sh{J})$ is a closed sub-prescheme of $\mathbf{V}(\check{\sh{E}})$;
we will interpret it as \emph{representing a functor} from the category of $X$-preschemes to that of sets.
Let us note for this that a section $u\in\Gamma(X,\sh{E})$ identifies canonically with an $\sh{O}_X$-homomorphism $u:\sh{O}_X\to\sh{E}$, to which corresponds by transposition an $\sh{O}_X$-Algebra homomorphism $v:\mathbf{S}_{\sh{O}_X}(\check{\sh{E}})\to\sh{O}_X$.
Let $\mathrm{Al}(X,\sh{E},\sh{J})$ denote the set of $u\in\Gamma(X,\sh{E})$ such that $\sh{J}$ is \emph{contained in the kernel} of $v$;
if $X'=\operatorname{Spec}(A)$ is affine and such that $\sh{E}_{(X')}$ is isomorphic to $\sh{O}_{X'}^n$ and $\sh{J}\otimes_{\sh{O}_X}\sh{O}_{X'}$ is a $\sh{O}_{X'}$-Module of the form $\widetilde{\mathfrak{J}}$, where $\mathfrak{J}$ is an ideal of the polynomial algebra $C=A[\mathrm{T}_1,\ldots,\mathrm{T}_n]$;
the set $\mathrm{Al}(X',\sh{E}_{(X')},\sh{J}\otimes_{\sh{O}_X}\sh{O}_{X'})$ then identifies with the subset of $A^n$ consisting of points $(t_1,\ldots,t_n)$ such that $\mathrm{P}(t_1,\ldots,t_n)=0$ for all polynomials $\mathrm{P}\in\mathfrak{J}$;
thus we can say that the object $\operatorname{Spec}(\mathbf{S}_{\sh{O}_X}(\check{\sh{E}})/\sh{J})$ represents ``the algebraic subset of the twisted affine space over $X$ defined by $\sh{E}$''.
We also write this $X$-prescheme $\mathbf{Al}(\sh{E},\sh{J})$.
We note that if $\sh{J}$ is a type-finite Ideal of $\mathbf{S}_{\sh{O}_X}(\check{\sh{E}})$, $\mathbf{Al}(\sh{E},\sh{J})$ is an $X$-prescheme of finite presentation, since $\mathbf{S}_{\sh{O}_X}(\check{\sh{E}})$ is an $\sh{O}_X$-Algebra of finite presentation.
\end{env}

\begin{lemma}[18.5.3]
\label{IV.18.5.3}
Let $S$ be a prescheme, $f:X\to S$ a finite and locally free morphism.
Consider the contravariant functor from the category of $S$-preschemes to the category of sets
\[
\label{IV.18.5.3.1}
S'\mapsto\operatorname{Of}(X\times_S S')
\tag{18.5.3.1}
\]
where $\operatorname{Of}(X\times_S S')$ denotes the set of subsets that are both open and closed of the underlying space of $X\times_S S'$.
Then this functor is representable by an $S$-prescheme $\mathbf{Of}(X)$, which is affine, \'etale and of finite presentation over $S$.
\end{lemma}

\begin{proof}
\oldpage[IV-4]{127}
We have by hypothesis $X=\operatorname{Spec}(\sh{B})$, where $\sh{B}=f_*(\sh{O}_X)$ is a finite $\sh{O}_S$-Algebra and locally free.
Set $X'=X\times_S S'$, $f'=f_{(S')}$, $\sh{B}'=\sh{B}\otimes_{\sh{O}_S}\sh{O}_{S'}=f'_*(\sh{O}_{X'})$, so that $X'=\operatorname{Spec}(\sh{B}')$;
there is then a canonical bijection, functorial in $S'$, from the set $\operatorname{Of}(X')$ to the set $\operatorname{Id}(\sh{B}')$ of \emph{idempotents} of the ring $\Gamma(S',\sh{B}')=\Gamma(X',\sh{O}_X)$.
Indeed, by virtue of the equivalence of the category of affine $S'$-schemes and the opposite category of quasi-coherent $\sh{O}_{S'}$-Algebras (\sref[II]{II.1.2.7} and \sref{IV.1.3.1}), there is a canonical bijective correspondence between the decompositions of $X'$ into a \emph{sum} $X'_1\sqcup X'_2$ of two open subsets induced by open subsets of $X'$ and the decompositions of $\sh{B}'$ into a \emph{direct composite} of two Ideals $\sh{B}'_1$, $\sh{B}'_2$;
these latter in turn form a set in canonical bijective correspondence with $\operatorname{Id}(\sh{B}')$.
It suffices therefore to prove the lemma for the functor $S'\mapsto\operatorname{Id}(\sh{B}')$.

For this, we shall show that there exists a type-finite Ideal $\sh{J}$ of $\mathbf{S}_{\sh{O}_S}(\sh{B})$ such that $\operatorname{Id}(\sh{B}')$ is of the form $\mathrm{Al}(S',\check{\sh{B}},\sh{J}\otimes_{\sh{O}_S}\sh{O}_{S'})$ \sref{IV.18.5.2}.
Note to this end that since $\sh{B}$ is an $\sh{O}_S$-Algebra, its inverse image $\sh{B}_{\mathbf{V}(\check{\sh{B}})}$ is an $\sh{O}_{\mathbf{V}(\check{\sh{B}})}$-Algebra, and we can form the ``square'' $c^2$, in this Algebra, of the canonical section $c$ \sref{IV.18.5.1};
it corresponds canonically to a homomorphism $u^{(2)}:\mathbf{S}_{\sh{O}_S}(\check{\sh{B}})\to\mathbf{S}_{\sh{O}_S}(\check{\sh{B}})\otimes_{\sh{O}_S}\sh{B}$, and one verifies immediately that for every affine open subset $W$ of $S$, with the notations of \sref{IV.18.5.1}, $u^{(2)}$ corresponds to the $C$-module homomorphism such that $u^{(2)}(1)=\sum_k(\sum_{i,j}c_{ijk}\mathrm{T}_i\mathrm{T}_j)\otimes e_k$, where $(c_{ijk})$ is the table of multiplication of the algebra $\Gamma(W,\sh{B})$.
Let us show that the Ideal $\sh{J}$ of $\mathbf{S}_{\sh{O}_S}(\sh{B})$ generated by the kernel of the homomorphism $u-u^{(2)}$ answers the question.
Indeed, it suffices to note that the ideal $\Gamma(W,\sh{J})$ of $C$ is generated by the polynomials $\mathrm{P}_k(\mathrm{T}_1,\ldots,\mathrm{T}_n)=\mathrm{T}_k-\sum_{i,j}c_{ijk}\mathrm{T}_i\mathrm{T}_j$ and that the idempotents of $\Gamma(W,\sh{B})$ are precisely the elements $\sum_i t_i e_i$ of this algebra such that $(t_1,\ldots,t_n)$ annihilates all the polynomials $\mathrm{P}_k$ ($1\leq i\leq n$).
We deduce from \sref{IV.18.5.2} that the $S$-prescheme affine $\mathbf{Of}(X)=\mathbf{Al}(\sh{B},\sh{J})$ does represent the functor \sref{IV.18.5.3.1}.
It remains to show that $\mathbf{Of}(X)$ is \'etale over $S$, or equivalently, that it is \emph{formally \'etale} over $S$.
But if $S'$ is an $S$-prescheme, $S_0'$ a closed sub-prescheme of $S'$ defined by a locally nilpotent Ideal (and having the same underlying space as $S'$), it is clear that $X'=X\times_S S'$ and $X_0'=X\times_S S_0'$ have the same underlying space, hence the canonical map $\operatorname{Of}(X')\to\operatorname{Of}(X_0')$ is \emph{bijective}, which completes the proof \sref{IV.17.1.1}.
\end{proof}

\begin{proposition}[18.5.4]
\label{IV.18.5.4}
\oldpage[IV-4]{128}
Let $S$ be a prescheme, $S_0$ a closed sub-prescheme of $S$;
consider the following properties:
\begin{enumerate}[label=\emph{\alph*)}]
  \item For every finite morphism $g:S'\to S$, the canonical map
\[
\label{IV.18.5.4.1}
\operatorname{Of}(S')\to\operatorname{Of}(S'\times_S S_0)
\tag{18.5.4.1}
\]
is bijective {\upshape(}cf.\ \sref{IV.18.5.3}{\upshape)}.
  \item[a$'$)] For every finite and locally free morphism $g:S'\to S$, the map \sref{IV.18.5.4.1} is bijective.
  \item For every \'etale and separated morphism $g:S'\to S$, the canonical map
\[
\label{IV.18.5.4.2}
\Gamma(S'/S)\to\Gamma(S'\times_S S_0/S_0)
\tag{18.5.4.2}
\]
is bijective.
\end{enumerate}
\emph{Condition $b)$ implies a$'$);
if moreover $S$ is quasi-compact and quasi-separated, condition $a)$ implies $b)$.}
\end{proposition}

\begin{proof}
Let us first prove that $b)$ implies $a')$.
Suppose therefore $b)$ satisfied, and let $g:S'\to S$ be a finite and locally free morphism;
set $S_0'=S'\times_S S_0$, so that $g_0=g_{(S_0)}:S_0'\to S_0$ is finite and locally free.
Then it follows from \sref{IV.18.5.3} that $\mathrm{P}=\mathbf{Of}(S')$ is an $S$-prescheme \emph{\'etale and separated};
moreover, by the definition of the functor $\operatorname{Of}$, we see immediately that if we set $\mathrm{P}_0=\mathbf{Of}(S_0')$ (for the category of $S_0$-preschemes), we have $\mathrm{P}_0=\mathrm{P}\times_S S_0$.
This being so, by definition we have the commutative diagram
\[
\xymatrix{
\Gamma(\mathrm{P}/S)\ar[r]\ar[d]^l & \Gamma(\mathrm{P}_0/S_0)\ar[d]^l\\
\operatorname{Of}(S')\ar[r] & \operatorname{Of}(S_0')
}
\]
where the vertical arrows are the bijections.
Since hypothesis $b)$, applied to the morphism $\mathrm{P}\to S$, implies that the first row is a bijection, the same holds for the second, which establishes our assertion.

Before proving that $a)$ implies $b)$ when $S$ is quasi-compact and quasi-separated, we will establish the following lemma.

\begin{lemma}[18.5.4.3]
\label{IV.18.5.4.3}
If $S$ and $S_0$ satisfy condition $a)$ of \sref{IV.18.5.4}, then, for every finite morphism $g:S'\to S$, $S'$ is the unique open and closed neighbourhood of $S_0'=g^{-1}(S_0)=S'\times_S S_0$ in $S'$.
\end{lemma}

\begin{proof}
Indeed, it amounts to saying that if $T'$ is a closed subset of $S'$ such that $T'\cap S_0'=\varnothing$, then $T'=\varnothing$.
Now, if we also denote by $T'$ the closed sub-prescheme of $S'$ having $T'$ as underlying space, the composite morphism $h:T'\to S'\xrightarrow{g}S$ is finite and $h^{-1}(S_0)$ is empty;
condition $a)$, applied to the morphism $h$, implies that $T'$ is necessarily empty.
\end{proof}

This lemma being established, let us first prove that, under hypothesis $a)$, the map \sref{IV.18.5.4.2} is \emph{injective}.
Indeed, if $u'$, $u''$ are two $S$-sections of $S'$, the fact that the morphism $S'\to S$ is non ramified implies that the prescheme of coincidences of $u'$ and $u''$ is induced on an open subset $U$ of $S$ \sref{IV.17.4.6}.
If the restrictions to $S_0$ of $u'$ and $u''$ are equal, then $U$ contains $S_0$, hence is equal to $S$ by virtue of the lemma \sref{IV.18.5.4.3} applied to the case where $S'=S$.

\oldpage[IV-4]{129}
It remains to show that, under hypothesis $a)$, the map \sref{IV.18.5.4.2} is \emph{surjective} (when $S$ is quasi-compact and quasi-separated).
Let therefore $u_0:S_0\to S_0'$ be an $S_0$-section of $S_0'$;
$u_0(S_0)$ being quasi-compact in $S'$, can be covered by a finite number of open affine subsets $V_i$ such that the restriction $g|V_i$ is a morphism of finite type;
if $V$ is the union of the $V_i$, we conclude that $g|V:V\to S$ is a morphism \emph{of finite presentation}, being separated and locally of finite presentation \sref{IV.1.6.1}.
Replacing $S'$ by $V$, one can hence suppose that $g$ is of finite presentation.
Since $S$ is quasi-compact and quasi-separated ($g$ quasi-finite and separated) \sref{IV.17.6.1}, it follows from the ``Main theorem'' \sref{IV.8.12.6} that $g$ factorises as $S'\xrightarrow{j}S''\xrightarrow{f}S$, where $j$ is an open immersion and $f$ a \emph{finite} morphism.
Set $S_0''=S''\times_S S_0$, $j_0=j_{(S_0)}:S_0'\to S_0''$, which is an open immersion and $f_0=f_{(S_0)}:S_0''\to S_0$, which is also an $S_0$-section of $S_0''$.
Then $u_0$ is an open immersion of $S_0$ into $S_0'$ \sref{IV.17.4.1}, hence $g_0:S_0'\to S_0$ is \'etale, hence an open immersion of $S_0$ into $S_0'$, hence $X_0=u_0(S_0)$ is \emph{open and closed} in $S_0''$.
In virtue of condition $a)$, there exists an open and closed subset $X$ of $S''$ such that $X\cap S_0''=X_0$.
Let us show that the morphism $f:S''\to S$ is \emph{\'etale at the points of $X$}:
indeed, the set of points of $X$ where $f|X$ is \'etale is open and contains $X_0\subset S_0'$.
But the lemma \sref{IV.18.5.4.3} applied to the finite morphism $f|X$ proves that $U=X$.
On the other hand, $S'\cap X$ is open in $X$ and contains $X_0$, hence by the same reasoning proves that $S'\cap X=X$, i.e.\ $X\subset S'$.
It remains to show that, for every $s\in S$, the geometric number $n(s)$ of points of $X\cap f^{-1}(s)$ is equal to $1$, for it will follow that $f|X$ is an isomorphism of the open subset $X\subset S'$ onto $S$, whose inverse will be the $S$-section sought extending $u_0$.
But since $f|X$ is \'etale and finite, $s\mapsto n(s)$ is continuous on $S$ \sref{IV.18.2.8}, and since $X\cap S_0'=X_0$, we have $n(s)=1$ in $S_0$;
the set of points $s\in S$ such that $n(s)=1$ being open in $S$ and containing $S_0$, is equal to $S$ by \sref{IV.18.5.4.3}.
\end{proof}

\begin{remark}[18.5.4.4]
\label{IV.18.5.4.4}
One can show that the statement \sref{IV.18.5.4} remains valid when, in condition $b)$, one supposes merely the morphism $g$ \'etale (but not necessarily separated) \cite[exp.~XII]{SGA1}.
\end{remark}

\begin{definition}[18.5.5]
\label{IV.18.5.5}
We say that a prescheme $S$ and a closed sub-prescheme $S_0$ of $S$ form a \emph{Henselian couple} if they satisfy condition $a)$ of \sref{IV.18.5.4}.
\end{definition}

Taking into account \sref[I]{I.5.1.8}, it amounts to the same to say that $(S,S_0)$ is a Henselian couple or that $(S_{\mathrm{red}},(S_0)_{\mathrm{red}})$ is one.

\begin{proposition}[18.5.6]
\label{IV.18.5.6}
\begin{enumerate}
  \item[(i)] If $(S,S_0)$ is a Henselian couple, then, for every finite morphism $f:S'\to S$, if $S_0'$ is the closed sub-prescheme $f^{-1}(S_0)$ of $S'$, the couple $(S',S_0')$ is Henselian.
  \item[(ii)] Let $S=\coprod_\alpha S^{(\alpha)}$ be a sum of preschemes, $S_0$ a closed sub-prescheme of $S$, the sum of sub-preschemes $S_0^{(\alpha)}$ of the $S^{(\alpha)}$.
For the couple $(S,S_0)$ to be Henselian, it is necessary and sufficient that each of the couples $(S^{(\alpha)},S_0^{(\alpha)})$ be Henselian.
\end{enumerate}
\end{proposition}

\begin{proof}
\oldpage[IV-4]{130}
Assertion (i) is an immediate consequence of the definition, since for every finite morphism $g:S''\to S'$, $f\circ g:S''\to S$ is a finite morphism.
Similarly, under the conditions of (ii), for every morphism $g:S'\to S$ to be finite, it is necessary and sufficient that each of its restrictions $g^{(\alpha)}:S'^{(\alpha)}\to S^{(\alpha)}$ be finite, and the conclusion follows from \sref{IV.18.5.6}, (ii) and the remark \sref{IV.18.5.7}.
\end{proof}

\begin{remark}[18.5.7]
\label{IV.18.5.7}
Let $S=\operatorname{Spec}(A)$ be an affine scheme, $S_0$ a closed sub-prescheme defined by an ideal $\mathfrak{J}$ of $A$.
Then, if the couple $(S,S_0)$ is Henselian, the ideal $\mathfrak{J}$ is necessarily contained in the \emph{radical} of $A$.
Indeed, if $\mathfrak{m}$ is a maximal ideal of $A$, $\mathfrak{m}$ must belong to $V(\mathfrak{J})=S_0$, by virtue of \sref{IV.18.5.4.3}, whence the conclusion.
In particular, suppose that $S_0$ is \emph{reduced to a point}, then $\mathfrak{J}$ must be the radical of $A$, so $A$ must be a \emph{local} ring, and $S_0$ the unique closed point of $\operatorname{Spec}(A)$.
\end{remark}

\begin{definition}[18.5.8]
\label{IV.18.5.8}
We say that a ring $A$ is \emph{Henselian} if it is semi-local and if, denoting by $\mathfrak{r}$ the radical of $A$, the couple $(\operatorname{Spec}(A),\operatorname{Spec}(A/\mathfrak{r}))$ is Henselian.
We call a \emph{Henselian local scheme} a scheme isomorphic to the spectrum of a Henselian local ring.
\end{definition}

\begin{proposition}[18.5.9]
\label{IV.18.5.9}
\begin{enumerate}
  \item[(i)] For a semi-local ring $A$ to be Henselian, it is necessary and sufficient that it be a direct composite of Henselian local rings.
  \item[(ii)] For a local ring $A$ to be Henselian, it is necessary and sufficient that every finite $A$-algebra $B$ be isomorphic to a product of local rings.
\end{enumerate}
\end{proposition}

\begin{proof}
(i) Indeed, the definition, applied to $S=\operatorname{Spec}(A)$ and $S_0=\operatorname{Spec}(A/\mathfrak{r})$, shows that $S_0$ is a finite discrete set, closed in $S$, and $S$ is the union of a finite number of open and closed subsets $S_i$ ($1\leq i\leq n$), pairwise disjoint, each containing exactly one of the maximal ideals $\mathfrak{m}_i$ of $A$;
the conclusion follows from \sref{IV.18.5.6}, (ii) and the remark \sref{IV.18.5.7}.

(ii) Every finite morphism $S'\to\operatorname{Spec}(A)$ is of the form $\operatorname{Spec}(B)\to\operatorname{Spec}(A)$, where $B$ is a finite $A$-algebra.
If $k$ is the residue field of $A$, $\operatorname{Spec}(B\otimes_A k)$ is a finite Artinian spectrum, hence finite and discrete.
To say that the couple $(\operatorname{Spec}(A),\operatorname{Spec}(k))$ is Henselian means that $B$ is a direct composite of rings $A_i$ such that $\operatorname{Spec}(A_i\otimes_A k)$ is reduced to a point, i.e.\ that each $A_i$ (which is a finite $A$-algebra) must have only a single maximal ideal (Bourbaki, \emph{Alg.\ comm.}, ch.~V, \textsection~2, no.~1, prop.~1).
\end{proof}

The study of Henselian rings is thus essentially reduced to that of Henselian \emph{local} rings.

\begin{proposition}[18.5.10]
\label{IV.18.5.10}
If $A$ is a Henselian ring, every finite $A$-algebra $B$ is a Henselian ring (hence a direct composite of Henselian local rings \sref{IV.18.5.9}).
\end{proposition}

\begin{proof}
Indeed, if $\mathfrak{r}'$ is the radical of $B$, the inverse image of $\mathfrak{r}'$ in $A$ is the radical $\mathfrak{r}$ of $A$, and every maximal ideal of $B$ is a maximal ideal of $B$, hence the set $V(\mathfrak{r}')$ in $\operatorname{Spec}(B)$ is the inverse image of the set $V(\mathfrak{r})$ in $\operatorname{Spec}(A)$.
The proposition is then a consequence of \sref{IV.18.5.6}, (i).
\end{proof}

\begin{theorem}[18.5.11]
\label{IV.18.5.11}
Let $A$ be a local ring, $\mathfrak{m}$ its maximal ideal.
The following conditions are equivalent:
\begin{enumerate}[label=\emph{\alph*)}]
  \item $A$ is Henselian, in other words, every finite $A$-algebra $B$ is isomorphic to a product of local rings.
  \item[a$'$)] Condition $a)$ is satisfied for all $A$-algebras $B$ of the form $A[\mathrm{T}]/F.A[\mathrm{T}]$, where $F\in A[\mathrm{T}]$ is a monic polynomial.
  \item Let $S=\operatorname{Spec}(A)$, $S_0=\operatorname{Spec}(k)$.
For every \'etale morphism $g:S'\to S$, if we set $S_0'=S'\otimes_A k$, every $S_0$-section $u_0$ of $S_0'$ is the restriction of an $S$-section $u$ of $S'$.
  \item For every morphism locally of finite type $f:X\to S$, separated, and every point $x\in X$ such that $f(x)$ is equal to the closed point $s$ of $S$ and that $f$ is quasi-finite at the point $x$ (\sref[III]{III.Err.20}), $X$ is the sum of two preschemes $X'$, $X''$ such that $X'=\operatorname{Spec}(\sh{O}_{X,x})$ and that $f|X':X'\to S$ is a finite morphism.
  \item[c$'$)] For every morphism locally of finite type $f:X\to S$, and every point $x\in X$ such that $f$ be quasi-finite at the point $x$ and $f(x)$ equal to the closed point $s$ of $S$, $\sh{O}_{X,x}$ is a finite algebra over $\sh{O}_{S,s}=A$.
  \item[c$''$)] For every morphism locally of finite presentation $f:X\to S$, and every point $x\in X$ such that $f$ be quasi-finite at the point $x$ and $f(x)$ equal to the closed point $s$ of $S$, $\sh{O}_{X,x}$ is a finite algebra and of finite presentation over $\sh{O}_{S,s}=A$.
\end{enumerate}
\end{theorem}

\begin{proof}
\oldpage[IV-4]{131}
Let us first note that $c')$ (resp.\ $c''$) is equivalent to the same condition where moreover $f$ is supposed \emph{separated}, the question being local on $X$.
Similarly, condition $b)$ is equivalent to the same condition where moreover $g$ is supposed \emph{separated}:
indeed, it suffices to apply the latter to the restriction of $g$ to an open neighbourhood affine of the point $u_0(S_0)$ in $S'$.
Let us henceforth restrict to the case where, in $b)$, $c')$ and $c''$, the morphisms are separated.

The fact that $a)$ implies $b)$ and that $b)$ implies $a')$ is a particular case of \sref{IV.18.5.4}.
Let us moreover show that $a')$ implies $a)$.
It is a matter of proving that if $e_0$ is an idempotent of $C=B\otimes_A k$, there is an idempotent $e\in B$ whose image is $e_0$ in $C$.
If $b$ is an element of $B$ whose image in $C$ is $e_0$, the sub-$A$-algebra $A[b]=B'$ of $B$ is finite, and the canonical image $C'$ of $B'\otimes_A k$ in $C$ contains $e_0$.
Now, $B'\otimes_A k$ is a $k$-algebra, hence a direct composite of local $k$-algebras, and consequently $e_0$ is the image of an idempotent $e_0'$ of $B'\otimes_A k$.
We are thus reduced to the case where $B$ is monogenic, and hence by $a')$, a quotient of an algebra of the form $A[\mathrm{T}]/F.A[\mathrm{T}]$;
now, by $a')$, $A[\mathrm{T}]/F.A[\mathrm{T}]$ is a direct composite of local rings, hence so is its quotient, which proves the existence of the idempotent $e$.

It is immediate that $c)$ implies $c')$, and that $c')$ implies $c'')$ when $B$ is of finite presentation.
Conversely, let us show that $c'')$ implies $a')$.
For if $B=A[\mathrm{T}]/F.A[\mathrm{T}]$, the morphism $X=\operatorname{Spec}(B)\to\operatorname{Spec}(A)=Y$ is finite and of finite presentation, and the preceding demonstration shows that $B$ is a direct composite of finite $A$-algebras $\sh{O}_{X,x_i}$, where the $x_i$ are the points of the fibre of the closed point of $\operatorname{Spec}(A)$.

It is trivial that $c)$ implies $c')$, by virtue of the remark at the beginning.
Let us prove that $c')$ implies $c'')$.
Suppose $c')$ satisfied and let us prove that under the conditions of $c'')$, the set $Z=\operatorname{Spec}(\sh{O}_{X,x})$ identifies with an open and closed subset of $X$, which will establish $c'')$ \sref[I]{I.2.4.2}.
In the first place, the composite morphism $Z\xrightarrow{j}X\xrightarrow{f}S$, where $j$ is the canonical map \sref[I]{I.2.4.1}, is finite and of finite presentation by hypothesis, and since $f$ is separated and locally of finite presentation, $j$ is also a finite morphism closed \sref[II]{II.6.1.10}, hence this proves that $Z$ is closed in $X$.
Moreover, since $j$ is a closed immersion \sref[I]{I.4.2.2} and (\sref[I]{I.4.2.2}), $j$ is a closed immersion.
But if $\sh{J}$ is the Ideal of $\sh{O}_X$ defining $Z$, we also have $\sh{J}_z=0$ for every point $z$ in a neighbourhood open $V$ of $x$ in $X$, since by hypothesis $\sh{J}$ is a quasi-coherent $\sh{O}_X$-Module of finite type (\sref{IV.1.4.7}) and (\sref[0]{0.5.2.2}).
Now, in an open neighbourhood $V$ containing $Z$, hence $Z$ is open in $X$.

\oldpage[IV-4]{132}
Finally, $c'')$ implies $a')$:
indeed, if $B=A[\mathrm{T}]/F.A[\mathrm{T}]$, the morphism $X=\operatorname{Spec}(B)\to\operatorname{Spec}(A)=Y$ is finite and of finite presentation, and the preceding argument shows that $B$ is a direct composite of finite $A$-algebras $\sh{O}_{X,x_i}$, where the $x_i$ are the points of the fibre of the closed point of $\operatorname{Spec}(A)$.
\end{proof}

\begin{corollary}[18.5.12]
\label{IV.18.5.12}
Let $A$ be a semi-local ring, $\mathfrak{r}$ its radical;
set $S=\operatorname{Spec}(A)$, $S_0=\operatorname{Spec}(A/\mathfrak{r})$.
For $A$ to be Henselian, it is necessary and sufficient that, for every finite morphism $f:X\to S$ and every \'etale and separated morphism $g:Y\to S$, if we set $X_0=X\times_S S_0$ and $Y_0=Y\times_S S_0$, the canonical map
\[
\operatorname{Hom}_S(X,Y)\to\operatorname{Hom}_{S_0}(X_0,Y_0)
\]
is bijective.
\end{corollary}

\begin{proof}
The sufficiency of the condition follows from the equivalence of $a)$ and $b)$ in \sref{IV.18.5.11}, by applying this condition to the case where $f=1_S$.
To see that the condition is necessary, note that if $A$ is Henselian, the couple $(X,X_0)$ is Henselian by \sref{IV.18.5.6}, (i);
moreover $\operatorname{Hom}_S(X,Y)=\Gamma(X\times_S Y/X)$ and $\operatorname{Hom}_{S_0}(X_0,Y_0)=\Gamma(X_0\times_{S_0}Y_0/X_0)$;
since $X\times_S Y$ is \'etale and separated over $X$, the conclusion follows from the fact that $a)$ implies $b)$ in \sref{IV.18.5.4}.
\end{proof}

\begin{remarks}[18.5.13]
\label{IV.18.5.13}
The equivalent conditions $a)$, $a')$, $b)$ of Theorem~\sref{IV.18.5.11} are also equivalent to the following (``Hensel's lemma''):
\begin{enumerate}
  \item[a$''$)] For every monic polynomial $F\in A[\mathrm{T}]$, with canonical image $F_0\in k[\mathrm{T}]$, and every decomposition $F_0=G_0 H_0$ of $F_0$ into a product of two monic polynomials that are coprime $G_0$, $H_0$ in $k[\mathrm{T}]$, there exists a unique couple $(G,H)$ of monic polynomials of $A[\mathrm{T}]$ possessing the following properties:
$G_0$ and $H_0$ are the canonical images respectively of $G$ and $H$, we have $F=GH$, and the ideal of $A[\mathrm{T}]$ generated by $G$ and $H$ is equal to $A[\mathrm{T}]$.
\end{enumerate}
\end{remarks}

\begin{lemma}[18.5.13.1]
\label{IV.18.5.13.1}
Let $A$ be a local ring with residue field $k$, $F\in A[\mathrm{T}]$ a monic polynomial, $B$ the $A$-algebra $A[\mathrm{T}]/F.A[\mathrm{T}]$.
There exists a canonical bijective correspondence between the decompositions of $B$ into direct composite of two $A$-algebra quotients $B'$, $B''$ and the decompositions $F=GH$ of $F$ into a product of two monic polynomials $G$, $H$ of $A[\mathrm{T}]$, such that the ideal generated by $G$ and $H$ is equal to $A[\mathrm{T}]$;
the quotient algebras $B'$, $B''$ corresponding to such a couple of polynomials $G$, $H$ are respectively $A[\mathrm{T}]/H.A[\mathrm{T}]$ and $A[\mathrm{T}]/G.A[\mathrm{T}]$.
\end{lemma}

\begin{proof}
\oldpage[IV-4]{133}
If $F=GH$ and $G$ and $H$ generate the ideal $A[\mathrm{T}]$, there are two polynomials $P$, $Q$ of $A[\mathrm{T}]$ such that $1=PG+QH$.
We deduce that the intersection of the principal ideals $\mathfrak{a}=G.A[\mathrm{T}]$ and $\mathfrak{b}=H.A[\mathrm{T}]$ is equal to $\mathfrak{c}=F.A[\mathrm{T}]$:
indeed, if $R\in G.A[\mathrm{T}]\cap H.A[\mathrm{T}]$, we can write $R=PRG+QRH$;
since $RG$ (resp.\ $RH$) is a multiple of $F$ since $R$ is a multiple of $H$ (resp.\ $G$), therefore $R\in F.A[\mathrm{T}]$.
Since $A[\mathrm{T}]=\mathfrak{a}+\mathfrak{b}$, $A[\mathrm{T}]/\mathfrak{c}$ is the direct sum of ideals $\mathfrak{a}/(\mathfrak{a}\cap\mathfrak{b})$ and $\mathfrak{b}/(\mathfrak{a}\cap\mathfrak{b})$, canonically isomorphic respectively to $A[\mathrm{T}]/\mathfrak{b}$ and $A[\mathrm{T}]/\mathfrak{a}$.

Conversely, suppose given a decomposition of $B$ into a direct composite of two $A$-algebras $B'$, $B''$, which identify canonically with two ideals $e'B$, $e''B$ of $B$, corresponding to a decomposition $1=e'+e''$ into orthogonal idempotents $e'$, $e''$ of $B$.
Set further $B_0=B\otimes_A k=k[\mathrm{T}]/F_0.k[\mathrm{T}]$, where $F_0$ is the canonical image of $F$ in $k[\mathrm{T}]$, of the same degree $n$ as $F$;
if $e_0'$, $e_0''$ are the canonical images of $e'$, $e''$ in $B_0$, these are two orthogonal idempotents such that $1=e_0'+e_0''$, and $B_0$ is a direct composite $B_0'=e_0'B_0$ and $B_0''=e_0''B_0$.
Let $t$ and $t_0$ be the canonical images of $\mathrm{T}$ in $B$ and $B_0$;
$B_0'$ (resp.\ $B_0''$) is a $k$-algebra generated by $t_0'=e_0't_0$ (resp.\ $t_0''=e_0''t_0$), and it admits a base of the form $\{e_0',t_0',t_0'^2,\ldots,t_0'^{s-1}\}$ (resp.\ $\{e_0'',t_0'',t_0''^2,\ldots,t_0''^{r-1}\}$) with $r+s=n$.
On the other hand, $B$ being a free $A$-module (of base $\{1,t,\ldots,t^{n-1}\}$), $B'$ and $B''$ are projective $A$-modules, hence free since $A$ is a local ring (Bourbaki, \emph{Alg.\ comm.}, ch.~II, \textsection~5, no.~3, cor.\ de la prop.~5);
it follows from the above and from Bourbaki, \emph{Alg.\ comm.}, ch.~II, \textsection~3, no.~3, prop.~5, that if we set $t'=e't$, $t''=e''t$, $\{e',t',\ldots,t'^{s-1}\}$ (resp.\ $\{e'',t'',\ldots,t''^{r-1}\}$) is a base of the $A$-module $B'$ (resp.\ $B''$).
There is hence a monic polynomial $H$ (resp.\ $G$) of degree $s$ (resp.\ $r$) in $A[\mathrm{T}]$ such that $e'H(t')=0$ and $e''G(t'')=0$;
as $t^h=e't'^h+e''t''^h$, $t''^h=e''t''^h$ for every integer $h\geq 1$, and as $t'^h=e't'^h$, we also have $G(t)=e'G'(t')$ and $H(t)=e''H(t'')$, whence $G(t)H(t)=0$;
we conclude that the polynomial $G(T)H(T)$ is divisible by $F(T)$;
but since these two monic polynomials have the same degree, we have $GH=F$.
Moreover, $B'$ (resp.\ $B''$) is isomorphic to $A[\mathrm{T}]/H.A[\mathrm{T}]$ (resp.\ $A[\mathrm{T}]/G.A[\mathrm{T}]$).
Finally, there are two polynomials $R$, $S$ in $A[\mathrm{T}]$ such that $e'=R(t)$ and $e''=S(t)$;
as $e'S(t')=0$ and also $e'S(t'')=0$, we have necessarily $e'R(t')=0$ and $e''R(t'')=0$, so that, by definition of $G$ and $H$, $R=QH$ and $S=PG$, where $P$, $Q$ belong to $A[\mathrm{T}]$;
the relation $1=R(t)+S(t)$ in $B$ gives $PG+QH=1+LF$ for some polynomial $L\in A[\mathrm{T}]$, and since $F=GH$, this proves that the ideal generated by $G$ and $H$ is equal to $A[\mathrm{T}]$, and completes the proof of the lemma.
\end{proof}

This lemma being established, it suffices to apply it, to the ring $A$ on the one hand, to the field $k$ on the other, to see immediately that conditions $a')$ and $a'')$ are equivalent.

\begin{proposition}[18.5.14]
\label{IV.18.5.14}
Every semi-local ring, separated and complete for the $\mathfrak{r}$-preadic topology (where $\mathfrak{r}$ is the radical of $A$) is Henselian.
\end{proposition}

\begin{proof}
Indeed, $A$ is a direct composite of local rings separated and complete (Bourbaki, \emph{Alg.\ comm.}, ch.~III, \textsection~2, no.~13, cor.\ of prop.~19), so we are reduced to the case where $A$ is a local ring.
Let us verify criterion $a')$ of \sref{IV.18.5.11}.
Since $B$ is an $A$-module libre of finite type, $B$ is separated and complete for the $\mathfrak{J}$-preadic topology (\sref[0]{0.7.3.6}).
Replacing $B$ by $A$ and $\mathfrak{J}B$ by $\mathfrak{J}$, it all amounts to seeing that the map that, to every idempotent of $A$, associates its class mod~$\mathfrak{J}$, gives a correspondence with its image, and one verifies immediately that for every affine open subset $W$ of $S$ with the notations of \sref{IV.18.5.1}, is \emph{bijective}.
Let us write $\operatorname{Idem}(A)$ for the set of idempotents of $A$, and for every homomorphism of rings $\varphi:A\to B$, let $\operatorname{Idem}(\varphi)$ denote the map $\operatorname{Idem}(A)\to\operatorname{Idem}(B)$ restriction of $\varphi$;
it follows from the definition of the projective limit that $\operatorname{Idem}(A)=\varprojlim\operatorname{Idem}(A/\mathfrak{J}^n)$, under the maps $\psi_{nm}:\operatorname{Idem}(A/\mathfrak{J}^m)\to\operatorname{Idem}(A/\mathfrak{J}^n)$ restriction of the canonical maps $A/\mathfrak{J}^m\to A/\mathfrak{J}^n$.
But since $\operatorname{Spec}(A/\mathfrak{J}^m)\to\operatorname{Spec}(A/\mathfrak{J}^n)$ is a homeomorphism, the $\psi_{nm}$ are bijections (as seen in the proof of \sref{IV.18.5.3});
this proves our assertion.
\end{proof}

\begin{proposition}[18.5.15]
\label{IV.18.5.15}
\oldpage[IV-4]{134}
Let $A$ be a Henselian ring, $\mathfrak{r}$ its radical.
Then the functor $B\mapsto B/\mathfrak{r}B$ is an equivalence of the category of finite \'etale $A$-algebras with the category of finite \'etale $(A/\mathfrak{r})$-algebras.
\end{proposition}

\begin{proof}
The fact that the functor is fully faithful is a particular case of \sref{IV.18.5.12}.
To show that this functor is an equivalence of categories, we may reduce to the case where $A$ is local;
it suffices then to apply \sref{IV.18.1.2.thm} (for \'etale morphisms), and $S_0=\operatorname{Spec}(A/\mathfrak{r})$, reduced to a single point.
\end{proof}

\begin{remarks}[18.5.16]
\label{IV.18.5.16}
\begin{enumerate}
  \item[(i)] We do not know if Proposition~\sref{IV.18.5.15} generalises to a Henselian couple $(S,S_0)$, even when $S$ is affine and Noetherian.
  \item[(ii)] Let $A$ be a Noetherian ring, $\mathfrak{J}$ an ideal of $A$ such that $A$ is \emph{separated and complete} for the $\mathfrak{J}$-preadic topology.
Then the couple $(\operatorname{Spec}(A),\operatorname{Spec}(A/\mathfrak{J}))$ is Henselian:
indeed, for every finite $A$-algebra $B$, $B$ is separated and complete for the $\mathfrak{J}$-preadic topology (\sref[0]{0.7.3.6}).
Replacing $B$ by $A$ and $\mathfrak{J}B$ by $\mathfrak{J}$, everything amounts to seeing that the map that to every idempotent of $A$ associates its class mod $\mathfrak{J}$, is \emph{bijective}.
Let us write $\operatorname{Idem}(A)$ for the set of idempotents of $A$, and $\operatorname{Idem}(A)=\varprojlim\operatorname{Idem}(A/\mathfrak{J}^n)$ for the projective limit;
but since $\operatorname{Spec}(A/\mathfrak{J}^n)\to\operatorname{Spec}(A/\mathfrak{J}^m)$ is a homeomorphism, the $\psi_{nm}$ are bijections (as seen in the proof of \sref{IV.18.5.3});
this proves our assertion.
\end{enumerate}
\end{remarks}

\begin{theorem}[18.5.17]
\label{IV.18.5.17}
Let $A$ be a Henselian local ring, $S=\operatorname{Spec}(A)$, $s$ the closed point.
For every smooth morphism $f:X\to S$, the canonical map
\[
\Gamma(X/S)\to\Gamma(X_s/\kappa(s))
\]
(where $X_s=f^{-1}(s)$) is surjective.
\end{theorem}

\begin{proof}
The datum of a $\kappa(s)$-section of $X_s$ is equivalent to that of a point $x\in X$ above $s$ whose residue field $\kappa(x)$ is isomorphic to $\kappa(s)$, and it is a matter of proving that there exists an $S$-section $u:S\to X$ such that $u(s)=x$.
Taking into account \sref{IV.17.16.3}, (i), we can suppose that $f$ is \'etale.
Then the conclusion follows from the criterion \sref{IV.18.5.11}, $b)$.
(The reader will note that by virtue of this criterion, the validity of \sref{IV.18.5.17} for a local ring is necessary and sufficient for $A$ to be Henselian.)
\end{proof}

\begin{remark}[18.5.18]
\label{IV.18.5.18}
The conditions of \sref{IV.18.5.11} are also equivalent to the following:
\begin{enumerate}
  \item[d)] For every morphism locally of finite type $f:X\to S$ and every point $x\in X$ such that $f(x)$ be equal to the closed point $s$ of $S$ and that $\sh{O}_{X,x}/\mathfrak{m}_s\sh{O}_{X,x}$ be a field canonically isomorphic to $k=\kappa(s)$, there exists an open neighbourhood $U$ of $x$ in $X$ such that $f|U$ is a closed immersion.
\end{enumerate}
\end{remark}

\begin{proof}
It is immediate that $d)$ implies condition $b)$ of \sref{IV.18.5.11} since a closed immersion \'etale is an open immersion \sref{IV.18.9.1}.
Conversely, suppose the conditions of \sref{IV.18.5.11} satisfied, and let us prove $d)$.
The hypothesis of $d)$ implies that $f$ is quasi-finite at the point $x$ (\sref[III]{III.Err.20}), hence by virtue of condition $c)$ of \sref{IV.18.5.11}, $\sh{O}_{X,x}$ is a finite $A$-algebra and $\operatorname{Spec}(\sh{O}_{X,x})$ a neighbourhood open $U$ of $x$ in $X$.
Moreover, since $\sh{O}_{X,x}/\mathfrak{m}_s\sh{O}_{X,x}$ is isomorphic to $k=A/\mathfrak{m}_s$, the Nakayama lemma proves that the homomorphism $A\to\sh{O}_{X,x}$ is surjective, hence $f|U$ is a closed immersion.
\end{proof}

\begin{proposition}[18.5.19]
\label{IV.18.5.19}
Let $A$ be a Noetherian local ring and Henselian, $S=\operatorname{Spec}(A)$, $s$ the closed point of $S$, $f:X\to S$ a proper morphism;
set $X_s=f^{-1}(s)$.
Then the map $Y\to Y\cap X_s$ is a bijection from the set of connected components of $X$ to the set of connected components of $X_s$.
\end{proposition}

\begin{proof}
By considering a closed sub-prescheme of $X$ having for underlying space a connected component of $X$, we are reduced to showing that if $X$ is \emph{connected and non-empty}, then $X_s$ is \emph{connected and non-empty}.
The fact that $X_s$ is non-empty follows from (\sref[II]{II.7.2.1});
to show that $X_s$ is connected, we argue by contradiction, considering the Stein factorisation $f:X\xrightarrow{f'}S'\xrightarrow{g}S$ of the proper morphism $f$ (\sref[III]{III.4.3.3});
by hypothesis the finite discrete set $g^{-1}(s)$ would contain at least two points.
Since $A$ is Henselian and $g$ separated and of finite type, it would then follow from \sref{IV.18.5.11}, $c)$ that $S'$ would be the sum of two non-empty preschemes $S_1'$, $S_2'$ (since the intersection of any one of them with $g^{-1}(s)$ is reduced to a single point).
Since $f'$ is surjective (\sref[III]{III.4.3.1}), we would conclude that $X$ is a sum of two non-empty preschemes, contradicting the hypothesis.
\end{proof}
\subsection{Henselization.}
\label{subsection:IV.18.6}

\oldpage[IV-4]{136}

\begin{env}[18.6.1]
\label{IV.18.6.1}
Given a local ring A, there exists a \emph{set} $\mathfrak{E}$ of local A-algebras that are essentially \'{e}tale such that every A-algebra belonging to $\mathfrak{E}$ is A-isomorphic to an algebra belonging to $\mathfrak{E}$.
It suffices indeed to observe that there exists a set $\mathfrak{F}$ of A-algebras of finite type such that every A-algebra of finite type is isomorphic to an algebra belonging to $\mathfrak{F}$; one can take $\mathfrak{F}$ to be the set of quotients of polynomial algebras $\mathrm{A}[\mathrm{T}_1,\ldots,\mathrm{T}_n]$ ($n\in\bb{N}$).

In what follows, if A and B are two local rings, we denote by $\Hom.\operatorname{loc}(\mathrm{A},\mathrm{B})$ the set of \emph{local} homomorphisms from A to B.
\end{env}

\begin{lemma}[18.6.2]
\label{IV.18.6.2}
Let $\mathrm{A}$, $\mathrm{A}'$ be two local rings, $k$, $k'$ their residue fields,
$\vphi:\mathrm{A}\to\mathrm{A}'$ a local homomorphism making $\mathrm{A}'$ an A-algebra essentially \'{e}tale \sref{IV.18.6.1}
and such that the corresponding homomorphism $k\to k'$ is bijective.
Then, for every local Henselian ring $\mathrm{B}$, the canonical map
\[
  \Hom(\vphi,\,1_{\mathrm{B}}):\Hom.\operatorname{loc}(\mathrm{A}',\mathrm{B})\to\Hom.\operatorname{loc}(\mathrm{A},\mathrm{B})
\]
is bijective.
\end{lemma}

\begin{proof}
Let $\mathrm{S}=\Spec(\mathrm{A})$, $\mathrm{Y}=\Spec(\mathrm{B})$, and let $s$ and $y$ be the closed points of S and Y respectively.
By hypothesis, $\mathrm{A}'$ is isomorphic to a local ring $\sh{O}_{\mathrm{X},x}$ of an S-scheme X \'{e}tale over S, at a point $x\in\mathrm{X}$ above $s$.
Suppose given a local homomorphism $\psi:\mathrm{A}\to\mathrm{B}$, making Y an S-scheme;
it amounts to showing that there exists \emph{one and only one} S-morphism $f:\mathrm{Y}\to\mathrm{X}$ such that $f(y)=x$.
Let $\mathrm{X}'=\mathrm{X}\times_{\mathrm{S}}\mathrm{Y}$, and note that since $k(x)=k(s)$, there exists a unique point $x'\in\mathrm{X}$ above $x$ and above $y$, and that $k(x')=k(y)$.
It remains to show that there exists a unique Y-section $f'$ of $\mathrm{X}'$ such that $f'(y)=x'$.
Now, the morphism $g:\mathrm{X}'\to\mathrm{Y}$ is \'{e}tale and separated, and the fibre $\mathrm{X}'_0=g^{-1}(y)$ at the point $x'$ has the residue field $k(x')=k(y)$.
If we set $\mathrm{Y}_0=\Spec(k(y))$, there exists a unique $\mathrm{Y}_0$-section $f'_0$ of $\mathrm{X}'_0$ such that $f'_0(y)=x'$, and the conclusion follows from the fact that B is assumed Henselian and from \sref{IV.18.5.11},~$b$).
\end{proof}

We say that a local A-algebra $\mathrm{A}'$ satisfying the conditions of \sref{IV.18.6.2} is \emph{strictly essentially \'{e}tale}.
Note that the criterion \sref{IV.18.5.11},~$b$) means that, \emph{for A to be Henselian, it is necessary and sufficient that every strictly essentially \'{e}tale A-algebra be A-isomorphic to} A.

\begin{lemma}[18.6.3]
\label{IV.18.6.3}
Let $\mathrm{A}$ be a local ring, $\mathrm{A}_1$, $\mathrm{A}_2$ two A-algebras that are local and strictly essentially \'{e}tale.
\begin{enumerate}[label=\emph{\roman*)}]
  \item There exists at most one A-homomorphism (necessarily local) from $\mathrm{A}_1$ to $\mathrm{A}_2$.
  \item There exists a local A-algebra $\mathrm{A}_3$ that is strictly essentially \'{e}tale, and two A-homomorphisms $\mathrm{A}_1\to\mathrm{A}_3$, $\mathrm{A}_2\to\mathrm{A}_3$.
\end{enumerate}
\end{lemma}

\begin{proof}
Let $\mathrm{S}=\Spec(\mathrm{A})$;
by hypothesis there are two S-schemes \'{e}tale over S, $\mathrm{X}_1$, $\mathrm{X}_2$, and two points $x_1\in\mathrm{X}_1$, $x_2\in\mathrm{X}_2$ above the closed point $s$ of S and such that $\mathrm{A}_1=\sh{O}_{\mathrm{X}_1,x_1}$, $\mathrm{A}_2=\sh{O}_{\mathrm{X}_2,x_2}$;
the hypotheses $k(x_1)=k(x_2)=k(s)$ \sref[I]{I.3.4.9} imply that there exists a unique point $x_3\in\mathrm{X}_3$ above $x_1$ and $x_2$, and that $k(x_3)=k(s)$.
Let $\mathrm{X}_3=\mathrm{X}_1\times_{\mathrm{S}}\mathrm{X}_2$;
the hypotheses $k(x_1)=k(x_2)=k(s)$ \sref[I]{I.3.4.9} imply that there exists a unique point $x_3\in\mathrm{X}_3$ above $x_1$ and $x_2$, and that $k(x_3)=k(s)$.
Furthermore, $\mathrm{X}_3$ is \'{e}tale over S \sref{IV.17.3.3}, so $\mathrm{A}_3=\sh{O}_{\mathrm{X}_3,x_3}$ satisfies the conditions of (ii).
Moreover,
\oldpage[IV-4]{137}
as $\mathrm{X}'_2$ is connected, the uniqueness of $f'$ follows from \sref{IV.17.4.9}, whence (i).
\end{proof}

\begin{env}[18.6.4]
\label{IV.18.6.4}
Let us denote by $\mathfrak{S}$ the subset of the set $\mathfrak{E}$ defined in \sref{IV.18.6.1} formed by the A-algebras that are strictly essentially \'{e}tale belonging to $\mathfrak{E}$.
It follows from \sref{IV.18.6.3} that the relation ``there exists an A-homomorphism from $\mathrm{A}_1$ to $\mathrm{A}_2$'' is a \emph{preorder} relation on $\mathfrak{S}$, and makes $\mathfrak{S}$ a \emph{filtered increasing} set.
Indexing $\mathfrak{S}$ by itself by means of the identity map, it follows from \sref{IV.18.6.3},~(i) that if $\lambda\leq\mu$ in $\mathfrak{S}$, there exists a \emph{unique} $\vphi_{\mu\lambda}:\mathrm{A}_\lambda\to\mathrm{A}_\mu$ and $(\mathrm{A}_\lambda,\vphi_{\mu\lambda})$ is clearly an \emph{inductive system} of local A-algebras, the $\vphi_{\mu\lambda}$ being local homomorphisms.
Furthermore, it follows from \sref{IV.17.3.5} that for $\lambda\leq\mu$, $\mathrm{A}_\mu$ is an $\mathrm{A}_\lambda$-algebra that is strictly essentially \'{e}tale.
\end{env}

\begin{definition}[18.6.5]
\label{IV.18.6.5}
The \emph{Henselization} of a local ring $\mathrm{A}$ is the inductive limit of the inductive system $(\mathrm{A}_\lambda,\vphi_{\mu\lambda})$, denoted ${}^h\!\mathrm{A}$, defined in \sref{IV.18.6.4}.
\end{definition}

This definition does not depend on the choice of $\mathfrak{E}$; if $\mathfrak{E}'$ is another set having the same property as $\mathfrak{E}$, and $\mathfrak{S}'$ the subset of $\mathfrak{E}'$ consisting of the strictly essentially \'{e}tale A-algebras, it follows from \sref{IV.18.6.3},~(ii) that, for the preorder relations considered, $\mathfrak{S}$ and $\mathfrak{S}'$ are \emph{cofinal} in $\mathfrak{S}''=\mathfrak{S}\cup\mathfrak{S}'$, so they give the same inductive limit up to an isomorphism.
We shall moreover see below \sref{IV.18.6.6},~(i) and~(ii) that ${}^h\!\mathrm{A}$ and the canonical homomorphism $\mathrm{A}\to{}^h\!\mathrm{A}$ are the solution of a universal problem, and are therefore determined up to a unique isomorphism.

One will observe that if A is a field, then evidently ${}^h\!\mathrm{A}=\mathrm{A}$.
In the general case, ${}^h\!\mathrm{A}$ is also the Henselization of all the A-algebras that are strictly essentially \'{e}tale $\mathrm{A}_\lambda$ ($\lambda\in\mathfrak{S}$), since if B is an $\mathrm{A}_\lambda$-algebra that is strictly essentially \'{e}tale, B is also an A-algebra that is strictly essentially \'{e}tale \sref{IV.18.6.1}, so (up to an A-isomorphism) one of the $\mathrm{A}_\mu$ for $\mu\geq\lambda$, and the $\mathrm{A}_\mu$ such that $\mu\geq\lambda$ and that $\mathrm{A}_\mu$ is an $\mathrm{A}_\lambda$-algebra that is strictly essentially \'{e}tale form a cofinal set in the preordered set of the $\mathrm{A}_\lambda$, by virtue of \sref{IV.18.6.1} and \sref{IV.18.6.3}.

\begin{theorem}[18.6.6]
\label{IV.18.6.6}
Let $\mathrm{A}$ be a local ring, ${}^h\!\mathrm{A}$ its Henselization.
\begin{enumerate}[label=\emph{\roman*)}]
  \item ${}^h\!\mathrm{A}$ is a Henselian local ring and the structural homomorphism $\mathrm{A}\to{}^h\!\mathrm{A}$ is local.
  \item For every Henselian local ring $\mathrm{B}$, the canonical map
  \[
    \Hom.\operatorname{loc}({}^h\!\mathrm{A},\mathrm{B})\to\Hom.\operatorname{loc}(\mathrm{A},\mathrm{B})
  \]
  is bijective.
  \item ${}^h\!\mathrm{A}$ is a faithfully flat A-module, and if $\mathfrak{m}$ is the maximal ideal of $\mathrm{A}$, $\mathfrak{m}.{}^h\!\mathrm{A}$ is the maximal ideal of ${}^h\!\mathrm{A}$, and the homomorphism $\mathrm{A}/\mathfrak{m}\mathrm{A}\to{}^h\!\mathrm{A}/\mathfrak{m}.{}^h\!\mathrm{A}$ of residue fields is bijective.
  \item If $\hat{\mathrm{A}}$ and $({}^h\!\mathrm{A})\hat{\phantom{x}}$ are the separated completions of the local rings $\mathrm{A}$ and ${}^h\!\mathrm{A}$, the homomorphism $\hat{\mathrm{A}}\to({}^h\!\mathrm{A})\hat{\phantom{x}}$ deduced from the structural homomorphism $\mathrm{A}\to{}^h\!\mathrm{A}$ by completion is bijective.
  \item For ${}^h\!\mathrm{A}$ to be Noetherian, it is necessary and sufficient that A be so.
  \item If A is Henselian, the canonical homomorphism $\mathrm{A}\to{}^h\!\mathrm{A}$ is bijective.
\end{enumerate}
\end{theorem}

\begin{proof}
\oldpage[IV-4]{138}
Let $\mathfrak{m}_\lambda$ be the maximal ideal of $\mathrm{A}_\lambda$;
the fact that, in \sref{IV.17.6.1},~$a$) implies $c'$), that for $\lambda\leq\mu$, on has $\mathfrak{m}_\mu=\mathfrak{m}_\lambda\mathrm{A}_\mu$, the homomorphism $\mathrm{A}_\lambda/\mathfrak{m}_\lambda\to\mathrm{A}_\mu/\mathfrak{m}_\mu$ is bijective and $\mathrm{A}_\mu$ is a flat $\mathrm{A}_\lambda$-module.
The fact that ${}^h\!\mathrm{A}$ is local, that the homomorphism $\mathrm{A}\to{}^h\!\mathrm{A}$ is local, assertion~(iii) and the sufficiency of~(v) follow therefore from \sref[0]{0.10.3.1.3};
the necessity of~(v) follows from the fact that ${}^h\!\mathrm{A}$ is a flat A-module \sref{IV.6.5.2}.

To prove that ${}^h\!\mathrm{A}$ is Henselian, let us apply the criterion \sref{IV.18.5.11},~$b$).
Let $\mathrm{S}=\Spec({}^h\!\mathrm{A})$, $\mathrm{S}_0=\Spec({}^h\!\mathrm{A}/\mathfrak{m}.{}^h\!\mathrm{A})$, and let $g:\mathrm{S}'\to\mathrm{S}$ be an \'{e}tale morphism;
reasoning as in \sref{IV.18.5.4}, one can suppose that $g$ is of finite presentation.
It follows then from \sref{IV.8.8.2} and \sref{IV.17.7.5} that there exists an index $\lambda\in\mathrm{S}$, an \'{e}tale morphism $g_\lambda:\mathrm{S}^{(\lambda)}\to\Spec(\mathrm{A}_\lambda)$ and, if one sets $\mathrm{S}_0^{(\lambda)}=g^{-1}(\mathrm{S}_0^{(\lambda)})$, an $\mathrm{S}_0^{(\lambda)}$-section $f_0^{(\lambda)}:\mathrm{S}_0^{(\lambda)}\to\mathrm{S}'^{(\lambda)}$ such that $\mathrm{S}'=\mathrm{S}'^{(\lambda)}\times_{g^{(\lambda)}}\mathrm{S}$, $g=g^{(\lambda)}\times 1$ and $f_0=f_0^{(\lambda)}\times 1$.
Let $s$ be the closed point of S, $x=f_0(s)$, let $s_\lambda$ be the closed point of $\mathrm{S}^{(\lambda)}$;
as $x_\lambda$ is above the closed point $s_\lambda$ of $\mathrm{S}^{(\lambda)}$, the local ring $\mathrm{C}_\lambda$ of $\mathrm{S}'^{(\lambda)}$ at the point $x_\lambda$ is an $\mathrm{A}_\lambda$-algebra that is strictly essentially \'{e}tale;
furthermore, as $f_0^{(\lambda)}(s_\lambda)=x_\lambda$, in other words, $\mathrm{C}_\lambda$ is an $\mathrm{A}_\lambda$-algebra that is local and strictly essentially \'{e}tale, and by \sref{IV.18.6.1} is an A-algebra $\mathrm{A}_\mu$ with $\mu\geq\lambda$.
There is therefore an $\mathrm{A}_\lambda$-homomorphism $\mathrm{C}_\lambda\to{}^h\!\mathrm{A}$, that is to say there exists a well-determined S-morphism $f:\mathrm{S}\to\mathrm{S}'$ of S';
by \sref{IV.18.5.4}, there exists a neighbourhood of $\mathfrak{n}_\lambda$ in $\Spec(\mathrm{C}_\lambda)$ isomorphic to $\Spec(\mathrm{A}_\lambda)$ \sref[I]{I.6.5.4};
one thus concludes that there is a neighbourhood of $\mathfrak{n}$ in $\Spec(\mathrm{C})$ isomorphic to $\Spec(\mathrm{A}')$, hence $\mathrm{C}_\mathfrak{n}$ is isomorphic to $\mathrm{A}'$, which proves that $\mathrm{A}'$ is Henselian \sref{IV.18.6.2} and completes the proof.

To prove~(ii), it suffices to note that $\Hom.\operatorname{loc}({}^h\!\mathrm{A},\mathrm{B})\cong\varprojlim\Hom.\operatorname{loc}(\mathrm{A}_\lambda,\mathrm{B})$ and that by \sref{IV.18.6.2} the canonical homomorphisms $\Hom.\operatorname{loc}(\mathrm{A}_\lambda,\mathrm{B})\leftarrow\Hom.\operatorname{loc}(\mathrm{A},\mathrm{B})$ are bijective.

To prove~(iv), note that for every $\lambda$ and every integer $n>0$, $\mathfrak{m}_\lambda^n=\mathfrak{m}^n\mathrm{A}_\lambda$, and $(\mathfrak{m}.{}^h\!\mathrm{A})^n=\mathfrak{m}^n.{}^h\!\mathrm{A}=\varinjlim(\mathrm{A}_\lambda/\mathfrak{m}_\lambda^n)$, and by the exactness of the functor $\varinjlim$ in the category of A-modules, that ${}^h\!\mathrm{A}/(\mathfrak{m}.{}^h\!\mathrm{A})^n=\varinjlim(\mathrm{A}_\lambda/\mathfrak{m}_\lambda^n)$, and it therefore suffices to show that, for every $n$ and every index $\lambda$, the homomorphism $\mathrm{A}/\mathfrak{m}^n\to\mathrm{A}_\lambda/\mathfrak{m}_\lambda^n$ is bijective.
On the other hand, since $\mathrm{A}_\lambda$ is a flat A-module, one has
\[
  \mathfrak{m}_\lambda^n/\mathfrak{m}_\lambda^{n+1}=(\mathfrak{m}^n/\mathfrak{m}^{n+1})\otimes_{\mathrm{A}}\mathrm{A}_\lambda
  =(\mathfrak{m}^n/\mathfrak{m}^{n+1})\otimes_{\mathrm{A}/\mathfrak{m}}(\mathrm{A}_\lambda/\mathfrak{m}\mathrm{A}_\lambda)
\]
and as $\mathrm{A}/\mathfrak{m}\to\mathrm{A}_\lambda/\mathfrak{m}\mathrm{A}_\lambda$ is bijective, the homomorphism $\mathfrak{m}^n/\mathfrak{m}^{n+1}\to\mathfrak{m}_\lambda^n/\mathfrak{m}_\lambda^{n+1}$ is also bijective;
the conclusion then follows from Bourbaki, \emph{Alg. comm.}, chap.~III, \S~2, no~8, cor.~3 du th.~1.

Finally, if A is Henselian, it follows from the remark preceding \sref{IV.18.6.3} that the canonical homomorphisms $\mathrm{A}\to\mathrm{A}_\lambda$ are bijective, which proves~(vi) by definition of ${}^h\!\mathrm{A}$.
\end{proof}

\begin{env}[18.6.7]
\label{IV.18.6.7}
\oldpage[IV-4]{138}
Let A be a \emph{semi-local} ring, $\mathfrak{m}_i$ ($1\leq i\leq r$) its maximal ideals.
The \emph{Henselization} of A is the ring denoted again ${}^h\!\mathrm{A}$, defined as the \emph{product} $\prod_i{}^h(\mathrm{A}_{\mathfrak{m}_i})$ of the Henselizations of the local rings $\mathrm{A}_{\mathfrak{m}_i}$.
It is a faithfully flat A-module and a semi-local A-algebra, whose maximal ideals are the $\mathfrak{m}_i.{}^h\!\mathrm{A}$ by virtue of \sref{IV.18.6.6},~(iii);
furthermore, if $\mathfrak{r}=\bigcap\mathfrak{m}_i$ is the radical of A, it follows from the above that $\mathfrak{r}.{}^h\!\mathrm{A}$
\oldpage[IV-4]{139}
is the radical of ${}^h\!\mathrm{A}$ and that the canonical map $\mathrm{A}/\mathfrak{r}\to{}^h\!\mathrm{A}/\mathfrak{r}.{}^h\!\mathrm{A}$ is bijective.
As the separated completion $\hat{\mathrm{A}}$ of A for the $\mathfrak{r}$-preadic topology is the product of the separated completions $\hat{\mathrm{A}}_{\mathfrak{m}_i}$ (Bourbaki, \emph{Alg. comm.}, chap.~III, \S~2, no~13, prop.~18), the canonical homomorphism $\hat{\mathrm{A}}\to({}^h\!\mathrm{A})\hat{\phantom{x}}$ is bijective by \sref{IV.18.6.6},~(iv), and it is clear by \sref{IV.18.6.6},~(v) that for ${}^h\!\mathrm{A}$ to be Noetherian, it is necessary and sufficient that A be so.

To obtain the analogue of the universal property \sref{IV.18.6.6},~(ii), when A and B are two \emph{semi-local} rings of radicals $\mathfrak{r}$, $\mathfrak{s}$, let us agree to call a \emph{semi-local homomorphism} $\vphi$ from A to B any homomorphism such that $\vphi(\mathfrak{r})\subset\mathfrak{s}$.
In the particular case where B is a \emph{product} of local rings $\mathrm{B}_j$ ($1\leq j\leq n$), the data of such a homomorphism amounts to the data of its projections $\vphi_j:\mathrm{A}\to\mathrm{B}_j$, which are homomorphisms subject only to the condition that $\vphi_j(\mathfrak{r})\subset\mathfrak{n}_j$, where $\mathfrak{n}_j$ is the maximal ideal of $\mathrm{B}_j$.
Moreover, since $\mathfrak{m}_i$ ($1\leq i\leq m$) are the maximal ideals of A, the prime ideal $\vphi_j^{-1}(\mathfrak{n}_j)$ must contain one of the $\mathfrak{m}_i$, since it must contain their intersection $\mathfrak{r}$, hence it is equal to one of the $\mathfrak{m}_i$ and $\vphi_j$ factorises as $\mathrm{A}\to\mathrm{A}_{\mathfrak{m}_i}\xrightarrow{\psi_{ij}}\mathrm{B}_j$, where $\psi_{ij}$ is a \emph{local} homomorphism.
If $\Hom.\operatorname{sloc}(\mathrm{A},\mathrm{B})$ denotes the set of semi-local homomorphisms from A to B, one can thus identify this set with $\prod_j\left(\bigcup_i\Hom.\operatorname{loc}(\mathrm{A}_{\mathfrak{m}_i},\mathrm{B}_j)\right)$;
in the case considered, since a semi-local Henselian ring is a direct product of Henselian local rings, one sees that the universal property \sref{IV.18.6.6},~(ii) is still valid for a semi-local ring A, provided one replaces ``local homomorphisms'' by ``semi-local homomorphisms'' \sref{IV.18.6.7}.
\end{env}

\begin{proposition}[18.6.8]
\label{IV.18.6.8}
Let $\mathrm{A}$ be a semi-local ring, $\mathrm{B}$ an A-algebra that is semi-local and entire over $\mathrm{A}$.
Then $\mathrm{B}\otimes_{\mathrm{A}}({}^h\!\mathrm{A})$ is a semi-local B-algebra isomorphic to ${}^h\!\mathrm{B}$.
\end{proposition}

\begin{proof}
As $\mathrm{B}\otimes_{\mathrm{A}}({}^h\!\mathrm{A})$ is entire over ${}^h\!\mathrm{A}$, each of its maximal ideals lies above one of the maximal ideals of ${}^h\!\mathrm{A}$, hence above a maximal ideal of A, and since B is entire over A, the prime ideals of B lying above a maximal ideal of A are maximal ideals of $\mathrm{B}\otimes_{\mathrm{A}}({}^h\!\mathrm{A})$.
Taking account of \sref{IV.18.6.6},~(iii) and \sref[I]{I.3.4.9}, one concludes that the ring $\mathrm{B}\otimes_{\mathrm{A}}({}^h\!\mathrm{A})$ is semi-local.
Furthermore, for every Henselian local ring C, $\Hom.\operatorname{sloc}(\mathrm{B}\otimes_{\mathrm{A}}({}^h\!\mathrm{A}),\mathrm{C})$ is in bijection with the set of pairs $(\vphi,\psi)$ of homomorphisms
$\psi:{}^h\!\mathrm{A}\to\mathrm{C}$, $\vphi:\mathrm{B}\to\mathrm{C}$, $\mathrm{A}\to{}^h\!\mathrm{A}\xrightarrow{\psi}\mathrm{C}$
such that the composites $\mathrm{A}\to\mathrm{B}\xrightarrow{\vphi}\mathrm{C}$ and $\mathrm{A}\to{}^h\!\mathrm{A}\xrightarrow{\psi}\mathrm{C}$ are equal.
But by virtue of the bijection between $\Hom.\operatorname{sloc}(\mathrm{A},\mathrm{C})$ and $\Hom.\operatorname{sloc}({}^h\!\mathrm{A},\mathrm{C})$, one sees that for every $\vphi\in\Hom.\operatorname{sloc}(\mathrm{B},\mathrm{C})$ there exists a unique $\psi$ having the preceding property, hence the map $\Hom.\operatorname{sloc}(\mathrm{B}\otimes_{\mathrm{A}}({}^h\!\mathrm{A}),\mathrm{C})\to\Hom.\operatorname{sloc}(\mathrm{B},\mathrm{C})$ is bijective, which proves the proposition by virtue of the uniqueness of the solution of a universal problem.
\end{proof}

\begin{theorem}[18.6.9]
\label{IV.18.6.9}
Let $\mathrm{A}$ be a semi-local ring.
\begin{enumerate}[label=\emph{\roman*)}]
  \item For ${}^h\!\mathrm{A}$ to be reduced (resp. normal), it is necessary and sufficient that A be so.
  \item Suppose A Noetherian.
  Then, for every prime ideal $\mathfrak{p}$ of A, the ring $({}^h\!\mathrm{A})_{\mathfrak{p}}/\mathfrak{p}.({}^h\!\mathrm{A})_{\mathfrak{p}}$ is a composed direct product of a finite number of separable algebraic extensions of $k(\mathfrak{p})$ (which implies that the fibres of the canonical morphism $\Spec({}^h\!\mathrm{A})\to\Spec(\mathrm{A})$ are \emph{geometrically regular} \sref{IV.18.6.9} and are \emph{discrete} spaces).
\end{enumerate}
\end{theorem}

\begin{proof}
\oldpage[IV-4]{140}
(i) As ${}^h\!\mathrm{A}$ is a faithfully flat A-module, it follows from \sref{IV.2.1.13} that if ${}^h\!\mathrm{A}$ is reduced (resp. normal), so is A.
To prove the converse, one can restrict to the case where A is local, so that, with the notations of \sref{IV.18.6.4}, one has ${}^h\!\mathrm{A}=\varinjlim\mathrm{A}_\alpha$.
Now, it follows from the definition of the $\mathrm{A}_\alpha$ and from \sref{IV.17.5.7} that if A is reduced (resp. integral and integrally closed), so is each of the $\mathrm{A}_\alpha$;
furthermore, the homomorphisms $\mathrm{A}_\alpha\to\mathrm{A}_\beta$ for $\alpha\leq\beta$ are injective by virtue of \sref[0]{0.6.5.1} since $\mathrm{A}_\beta$ is a flat $\mathrm{A}_\alpha$-module;
hence the morphisms $\Spec(\mathrm{A}_\beta)\to\Spec(\mathrm{A}_\alpha)$ are dominant, and one concludes from \sref{IV.5.13.2} (resp. \sref{IV.5.13.4}) that ${}^h\!\mathrm{A}$ is reduced (resp. integral and integrally closed).

(ii) One can restrict to the case where A is local.
By virtue of the fact that the functor $\varinjlim$ commutes with tensor products, the fibre of the morphism $\Spec({}^h\!\mathrm{A})\to\Spec(\mathrm{A})$ at a point $\mathfrak{p}$ is the projective limit of the fibres of the morphisms $\Spec(\mathrm{A}_\alpha)\to\Spec(\mathrm{A})$ at this point.
As ${}^h\!\mathrm{A}$ is Noetherian, one is reduced to proving the following lemma:
\end{proof}

\begin{lemma}[18.6.9.1]
\label{IV.18.6.9.1}
Let $(\mathrm{B}_\alpha)$ be an inductive system of Artinian rings, $\mathrm{B}=\varinjlim\mathrm{B}_\alpha$ its inductive limit.
If $\mathrm{B}$ is Noetherian, then $\mathrm{B}$ is Artinian;
if moreover, for $\alpha\leq\beta$, the homomorphisms $\vphi_{\beta\alpha}:\mathrm{B}_\alpha\to\mathrm{B}_\beta$ are injective, then there exists $\lambda$ such that, for $\alpha\geq\lambda$, the number of local components $\mathrm{B}_\alpha^{(i)}$ of $\mathrm{B}_\alpha$ is constant, that $\vphi_{\beta\alpha}(\mathrm{B}_\alpha^{(i)})\subset\mathrm{B}_\beta^{(i)}$ for every $i$ and that the $\mathrm{B}^{(i)}=\varinjlim\mathrm{B}_\alpha^{(i)}$ are the local components of $\mathrm{B}$.
\end{lemma}

\begin{proof}
Indeed $\mathrm{Y}_\alpha=\Spec(\mathrm{B}_\alpha)$, $\mathrm{Y}=\Spec(\mathrm{B})$, which, as a topological space, is equal to $\varprojlim\mathrm{Y}_\alpha$ \sref{IV.8.2.10}.
The spaces $\mathrm{Y}_\alpha$ are finite and discrete;
furthermore, if the $\vphi_{\beta\alpha}$ are injective, the canonical morphisms $\mathrm{Y}_\beta\to\mathrm{Y}_\alpha$ are dominant, hence surjective.
The lemma then follows from the following purely topological lemma:
\end{proof}

\begin{lemma}[18.6.9.2]
\label{IV.18.6.9.2}
Let $(\mathrm{Y}_\alpha,\psi_{\alpha\beta})$ be a projective system of finite topological spaces, and let Y be discrete.
If $\mathrm{Y}=\varprojlim\mathrm{Y}_\alpha$ is Noetherian, Y is finite and discrete.
If moreover the $\psi_{\alpha\beta}$ are surjective, then there exists $\lambda$ such that for $\alpha\geq\lambda$, the number of elements of $\mathrm{Y}_\alpha$ is constant and equal to the number of elements of $\mathrm{Y}$.
\end{lemma}

\begin{proof}
Indeed, Y being compact and Noetherian, every subset of Y is closed, hence compact, hence finite since it is compact.
If moreover the $\psi_{\alpha\beta}$ are surjective, the same holds for $\psi_\alpha:\mathrm{Y}\to\mathrm{Y}_\alpha$, so $\operatorname{Card}(\mathrm{Y})\geq\operatorname{Card}(\mathrm{Y}_\alpha)$, and as $\operatorname{Card}(\mathrm{Y}_\alpha)$ is an increasing function of $\alpha$, there exists $\lambda$ such that for $\alpha\geq\lambda$, $\operatorname{Card}(\mathrm{Y}_\alpha)=\operatorname{Card}(\mathrm{Y}_\lambda)$;
the $\psi_{\alpha\beta}$ are then bijective for $\alpha\geq\lambda$ and one therefore has $\operatorname{Card}(\mathrm{Y})=\operatorname{Card}(\mathrm{Y}_\lambda)$.
\end{proof}

\begin{corollary}[18.6.10]
\label{IV.18.6.10}
Let $\mathrm{A}$ be a local Noetherian ring.
For ${}^h\!\mathrm{A}$ to possess one of the following properties:
\begin{enumerate}[label=\emph{\alph*)}]
  \item being a Cohen-Macaulay ring \sref[0]{0.16.5.3};
  \item satisfying the property $(\mathrm{S}_n)$ \sref{IV.5.7.3};
  \item being regular;
  \item satisfying the property $(\mathrm{R}_n)$ \sref{IV.5.8.2};
\end{enumerate}
it is necessary and sufficient that A possess the same property.
\end{corollary}

\begin{proof}
\oldpage[IV-4]{141}
It follows immediately from the fact that the fibres of the morphism $\Spec({}^h\!\mathrm{A})\to\Spec(\mathrm{A})$ are geometrically regular \sref{IV.18.6.9} and from \sref{IV.6.4.1},~\sref{IV.6.5.1} and \sref{IV.6.5.3}.
\end{proof}

\begin{corollary}[18.6.11]
\label{IV.18.6.11}
Let $\mathrm{A}$ be a semi-local Noetherian ring.
For a prime ideal $\mathfrak{p}$ of ${}^h\!\mathrm{A}$ to belong to $\operatorname{Ass}({}^h\!\mathrm{A})$, it is necessary and sufficient that $\mathfrak{p}\cap\mathrm{A}$ belong to $\operatorname{Ass}(\mathrm{A})$.
\end{corollary}

\begin{proof}
Taking account of the description of the fibres of the morphism $\Spec({}^h\!\mathrm{A})\to\Spec(\mathrm{A})$, this also follows immediately from \sref{IV.3.3.1}.
\end{proof}

\begin{proposition}[18.6.12]
\label{IV.18.6.12}
Let $\mathrm{A}$ be a local ring;
the following properties are equivalent:
\begin{enumerate}[label=\emph{\alph*)}]
  \item A is unibranch \sref[0]{0.23.2.1} (resp. reduced and unibranch).
  \item For every A-algebra $\mathrm{A}'$ that is local and strictly essentially \'{e}tale, $\Spec(\mathrm{A}')$ is irreducible (resp. integral).
  \item $\Spec({}^h\!\mathrm{A})$ is irreducible (resp. integral).
\end{enumerate}
\end{proposition}

\begin{proof}
Note first that ${}^h(\mathrm{A}_{\mathrm{red}})=({}^h\!\mathrm{A})_{\mathrm{red}}$, since ${}^h(\mathrm{A}_{\mathrm{red}})=({}^h\!\mathrm{A})\otimes_{\mathrm{A}}\mathrm{A}_{\mathrm{red}}$ \sref{IV.18.6.8}, and ${}^h(\mathrm{A}_{\mathrm{red}})$ is reduced \sref{IV.18.6.9}.
This allows, in the proof, to consider only the case where A is reduced, hence also $\mathrm{A}'$ \sref{IV.17.5.7} and ${}^h\!\mathrm{A}$ \sref{IV.18.6.9}.

With the notations of \sref{IV.18.6.4}, the condition $b)$ means that all the $\mathrm{A}_\alpha$ are integral;
one therefore concludes that $b)$ implies $c)$ \sref{IV.5.13.3}, hence $b)$ implies $c)$.

It is clear that if ${}^h\!\mathrm{A}$ is integral, so are each of the $\mathrm{A}_\alpha\subset{}^h\!\mathrm{A}$, hence $c)$ implies $b)$.

Let us show that $c)$ implies $a)$.
Note first that $\mathrm{A}\subset{}^h\!\mathrm{A}$ is integral;
let K and L be the fields of fractions of A and ${}^h\!\mathrm{A}$ respectively.
It suffices to show that for every \emph{finite} A-algebra B, $\mathrm{B}\subset\mathrm{K}$, B is an anneau local, whose Henselization is therefore $\mathrm{B}\otimes_{\mathrm{A}}({}^h\!\mathrm{A})$ \sref{IV.18.6.8}.
Now B is a semi-local ring, whose Henselization is therefore $\mathrm{B}\otimes_{\mathrm{A}}({}^h\!\mathrm{A})$ \sref{IV.18.6.8}.
By flatness, $\mathrm{B}\otimes_{\mathrm{A}}({}^h\!\mathrm{A})$ identifies with a subring of L, hence ${}^h\!\mathrm{B}$ is integral.
But as ${}^h\!\mathrm{B}$ is a semi-local Henselian ring, it is a composed direct product of local rings \sref{IV.18.5.9}, hence it can only be integral if B is local, which shows that B is local.

Let us finally prove that $a)$ implies $c)$.
Let C be the integral closure of the integral ring A.
As C is a local ring by hypothesis, $\mathrm{C}\otimes_{\mathrm{A}}({}^h\!\mathrm{A})$ is the Henselization of C \sref{IV.18.6.8}, hence is integral and integrally closed \sref{IV.18.6.9}.
As A is a subring of C, hence ${}^h\!\mathrm{A}$ identifies with a subring of ${}^h\!\mathrm{C}$ by flatness, hence is also integral, which completes the proof.
\end{proof}

\begin{corollary}[18.6.13]
\label{IV.18.6.13}
Let $\mathrm{A}$ be a local Henselian ring.
Then, for A to be unibranch, it is necessary and sufficient that $\mathrm{A}_{\mathrm{red}}$ be integral.
\end{corollary}

\begin{proposition}[18.6.14]
\label{IV.18.6.14}
\begin{enumerate}[label=\emph{\roman*)}]
  \item Every filtered inductive limit of Henselian local rings (where the transition homomorphisms are local) is a Henselian local ring.
  \item The functor $\mathrm{A}\leadsto{}^h\!\mathrm{A}$ in the category of local rings (where the morphisms are local homomorphisms) commutes with filtered inductive limits.
  \item Every Henselian local ring A is an inductive limit of an inductive filtered system of Henselian and Noetherian local rings $\mathrm{A}_\lambda$ (the transition homomorphisms $\mathrm{A}_\lambda\to\mathrm{A}_\mu$ being local).
\end{enumerate}
\end{proposition}

\begin{proof}
\oldpage[IV-4]{141}
(i) Let $(\mathrm{A}_\lambda)$ be a filtered inductive system of Henselian local rings, where the transition homomorphisms are local.
Let us show that $\mathrm{A}'=\varinjlim\mathrm{A}_\lambda$, which is local \sref[0]{0.10.3.1.3}, is Henselian.
Now, let C be an $\mathrm{A}'$-algebra \'{e}tale, $\mathfrak{n}$ a prime ideal
\oldpage[IV-4]{142}
of C above the maximal ideal $\mathfrak{m}'$ of $\mathrm{A}'$, such that $\mathrm{C}_{\mathfrak{n}}$ is strictly essentially \'{e}tale over $\mathrm{A}'$ \sref{IV.18.6.1}, and in virtue of \sref{IV.8.8.2},~(ii) and \sref{IV.17.7.8}, there exist an index $\lambda$ and an $\mathrm{A}_\lambda$-algebra $\mathrm{C}_\lambda$ \'{e}tale such that $\mathrm{C}=\mathrm{C}_\lambda\otimes_{\mathrm{A}_\lambda}\mathrm{A}'$ up to an isomorphism.
Furthermore, if $\mathfrak{n}_\lambda$ is the inverse image of $\mathfrak{n}$ in $\mathrm{C}_\lambda$, and $\mathfrak{m}_\lambda$ the maximal ideal of $\mathrm{A}_\lambda$, one can, by virtue of \sref{IV.8.8.2.4} and of the transitivity of fibres \sref[I]{I.3.6.4}, suppose that $(\mathrm{C}_\lambda)_{\mathfrak{n}_\lambda}$ is strictly essentially \'{e}tale over $\mathrm{A}_\lambda$;
since $\mathrm{A}_\lambda$ is Henselian, $(\mathrm{C}_\lambda)_{\mathfrak{n}_\lambda}$ is necessarily isomorphic to $\mathrm{A}_\lambda$ \sref{IV.18.5.11},~$b$);
there exists therefore a neighbourhood of $\mathfrak{n}_\lambda$ in $\Spec(\mathrm{C}_\lambda)$ isomorphic to $\Spec(\mathrm{A}_\lambda)$ \sref[I]{I.6.5.4};
hence $\mathrm{C}_\mathfrak{n}$ is isomorphic to $\mathrm{A}'$, which proves that $\mathrm{A}'$ is Henselian \sref{IV.18.6.2} and completes the proof.

(ii) Suppose that $\mathrm{B}=\varinjlim\mathrm{B}_\lambda$, where the $\mathrm{B}_\lambda$ are local rings, the transition morphisms $\mathrm{B}_\lambda\to\mathrm{B}_\mu$ ($\lambda\leq\mu$) being local.
Set $\mathrm{A}_\lambda={}^h\!\mathrm{B}_\lambda$;
the $\mathrm{A}_\lambda$ are Henselian local rings and Noetherian local rings \sref{IV.18.6.6},~(v), and by virtue of \sref{IV.18.6.6},~(vi),~(ii) the transition morphisms $\mathrm{B}_\lambda\to\mathrm{B}_\mu$ uniquely determine local homomorphisms $\mathrm{A}_\lambda\to\mathrm{A}_\mu$, so that $(\mathrm{A}_\lambda)$ is an inductive system.
It amounts to showing that $\mathrm{A}'=\varinjlim\mathrm{A}_\lambda$ is canonically isomorphic to $\mathrm{A}={}^h\!\mathrm{B}$.
By virtue of \sref{IV.18.6.6},~(ii) and of the definition of inductive limits, one has, for every Henselian local ring E,
\[
  \Hom.\operatorname{loc}(\mathrm{A}',\mathrm{E})=\varprojlim\Hom.\operatorname{loc}(\mathrm{A}_\lambda,\mathrm{E})=\varprojlim\Hom.\operatorname{loc}(\mathrm{B}_\lambda,\mathrm{E})=\Hom.\operatorname{loc}(\mathrm{B},\mathrm{E}).
\]
But as $\mathrm{A}'$ is Henselian by~(i), this proves our assertion.

(iii) The ring A is a filtered inductive limit of its sub-local Noetherian rings $\mathrm{B}_\lambda$, the transition homomorphisms being local \sref{IV.5.13.3},~(iii).
As the ${}^h\!\mathrm{B}_\lambda$ are Henselian and Noetherian local rings \sref{IV.18.6.6},~(v) and that $\mathrm{A}={}^h\!\mathrm{A}$ \sref{IV.18.6.6},~(vi), it suffices to apply~(ii) to the inductive system $(\mathrm{B}_\lambda)$.
\end{proof}

\begin{corollary}[18.6.15]
\label{IV.18.6.15}
Let $\mathrm{A}$ be a Henselian local ring, $\mathrm{X}$ an A-prescheme of finite presentation over $\mathrm{A}$.
Then there exists a Noetherian Henselian local ring $\mathrm{A}_0$, a local homomorphism $\mathrm{A}_0\to\mathrm{A}$, an $\mathrm{A}_0$-prescheme $\mathrm{X}_0$ of finite type over $\mathrm{A}_0$ and an A-isomorphism $\mathrm{X}_0\otimes_{\mathrm{A}_0}\mathrm{A}\simto\mathrm{X}$.
\end{corollary}

\begin{proof}
This follows from \sref{IV.18.6.14} and \sref{IV.8.8.2},~(ii).
\end{proof}


\subsection{Henselization and excellent rings.}
\label{subsection:IV.18.7}

\begin{env}[18.7.1]
\label{IV.18.7.1}
\oldpage[IV-4]{142}
In this paragraph we denote by $\boldsymbol{P}(\mathrm{Z},k)$ a property of the form considered in \sref{IV.7.3.1}, where we further suppose that the property $\boldsymbol{Q}(\mathrm{A},k)$ satisfies the following condition:

For every separable algebraic extension $k'$ of $k$, and every $k'$-local algebra that is Noetherian, the property $\boldsymbol{Q}(\mathrm{A},k)$ is \emph{equivalent} to $\boldsymbol{Q}(\mathrm{A},k')$.

It is immediate that all the properties $\boldsymbol{P}$ considered in \sref{IV.7.3.8} satisfy the preceding condition, since if K is a finite extension of $k$, $k'\otimes_k\mathrm{K}$ is a composed direct product of a finite number of separable algebraic extensions of K.
\end{env}

\begin{proposition}[18.7.2]
\label{IV.18.7.2}
Let $\mathrm{A}$ be a local Noetherian ring.
For ${}^h\!\mathrm{A}$ to be a $\boldsymbol{P}$-ring \sref{IV.7.3.13}, it is necessary and sufficient that A be so.
\end{proposition}

\begin{proof}
\oldpage[IV-4]{143}
Indeed, by virtue of \sref{IV.18.6.6},~(iv), the completion $\hat{\mathrm{A}}$ of A is also the completion of ${}^h\!\mathrm{A}$ and one therefore has $\mathrm{A}\subset{}^h\!\mathrm{A}\subset\hat{\mathrm{A}}$.
By \sref{IV.18.6.9}, for every $x\in\Spec(\mathrm{A})$, the fibre at $x$ of the morphism $\Spec({}^h\!\mathrm{A})\to\Spec(\mathrm{A})$ is discrete, finite, and each of the points $y_i$ of this fibre is a separable algebraic extension of $k(x)$.
The formal fibre of A at the prescheme \emph{sum} of the formal fibres of ${}^h\!\mathrm{A}$ at the points $y_i$.
The conclusion then follows from the hypothesis on $\boldsymbol{P}$ in \sref{IV.18.7.1}.
\end{proof}

\begin{corollary}[18.7.3]
\label{IV.18.7.3}
Let $\mathrm{A}$ be a local Noetherian ring.
For ${}^h\!\mathrm{A}$ to be universally Japanese, it is necessary and sufficient that A be so.
\end{corollary}

\begin{proof}
Indeed, for a Noetherian local ring B, it amounts to the same to say that B is universally Japanese, or that the formal fibres of B are geometrically reduced \sref{IV.7.6.4} and \sref{IV.7.7.1};
the conclusion therefore follows from \sref{IV.18.7.2}.
\end{proof}

\begin{corollary}[18.7.4]
\label{IV.18.7.4}
Let $\mathrm{A}$ be a local Noetherian ring.
For the formal fibres of ${}^h\!\mathrm{A}$ to be geometrically regular, it is necessary and sufficient that those of A be so.
\end{corollary}

\begin{proof}
This is a particular case of \sref{IV.18.7.2}.
\end{proof}

\begin{proposition}[18.7.5]
\label{IV.18.7.5}
Let $\mathrm{A}$ be a local Noetherian ring.
If A is universally catenary \sref{IV.5.6.2} (resp. formally catenary \sref{IV.7.1.9}, resp. strictly formally catenary \sref{IV.7.2.6}), so is ${}^h\!\mathrm{A}$.
\end{proposition}

\begin{proof}
With the notations of \sref{IV.18.6.4}, if A is universally catenary, the same holds for the $\mathrm{A}_\alpha$, which are A-algebras essentially of finite type \sref{IV.5.6.3}.
To prove that in this case ${}^h\!\mathrm{A}$ is universally catenary, it will therefore suffice to establish the following lemma:
\end{proof}

\begin{lemma}[18.7.5.1]
\label{IV.18.7.5.1}
Let $(\mathrm{B}_\alpha,\vphi_{\beta\alpha})$ be an inductive system of Noetherian rings and suppose that, for $\alpha\leq\beta$, $\vphi_{\beta\alpha}$ makes $\mathrm{B}_\beta$ a flat $\mathrm{B}_\alpha$-module and that $\mathrm{B}=\varinjlim\mathrm{B}_\alpha$ is Noetherian.
Then, if the $\mathrm{B}_\alpha$ are catenary (resp. universally catenary), the same holds for $\mathrm{B}$.
\end{lemma}

\begin{proof}
To say that B is universally catenary is equivalent to saying that $\mathrm{B}[\mathrm{T}_1,\ldots,\mathrm{T}_n]$ is catenary for all $n$ \sref{IV.5.6.2}, and it is immediate that the inductive system $(\mathrm{B}_\alpha[\mathrm{T}_1,\ldots,\mathrm{T}_n])$ satisfies the same conditions as $(\mathrm{B}_\alpha)$;
it therefore suffices to prove that the $\mathrm{B}_\alpha$ are catenary.
If $\mathfrak{p}_0\supset\mathfrak{p}_1\supset\cdots\supset\mathfrak{p}_r$ is a saturated chain of prime ideals, the inverse images $\mathfrak{p}_{0\alpha}\supset\mathfrak{p}_{1\alpha}\supset\cdots\supset\mathfrak{p}_{r\alpha}$ of these ideals in $\mathrm{B}_\alpha$ form a chain of prime ideals for each $\alpha$ sufficiently large, and it suffices to show that for $\alpha$ sufficiently large, this chain is saturated.
Now, each $\mathfrak{p}_i$ is an ideal of finite type, hence there exists $\lambda$ such that, for $\alpha\geq\lambda$, on has $\mathfrak{p}_i=\mathfrak{p}_{i\alpha}.\mathrm{B}$.
On has therefore $1=\codim(\mathrm{V}(\mathfrak{p}_i),\mathrm{V}(\mathfrak{p}_{i+1}))=\codim(\mathrm{V}(\mathfrak{p}_{i\alpha}),\mathrm{V}(\mathfrak{p}_{i+1,\alpha}))$ since B is a flat $\mathrm{B}_\alpha$-module \sref{IV.6.1.4} and \sref[0]{0.6.2.3}, which completes the proof of the lemma.

Suppose now A formally catenary.
Let $\mathfrak{p}$ be a prime ideal of ${}^h\!\mathrm{A}$, and $\mathfrak{q}$ its inverse image in A, so that ${}^h\!\mathrm{A}/\mathfrak{q}.{}^h\!\mathrm{A}$ is a quotient ring of ${}^h\!\mathrm{A}$, and as ${}^h\!\mathrm{A}/\mathfrak{q}.{}^h\!\mathrm{A}={}^h(\mathrm{A}/\mathfrak{q})$ by \sref{IV.18.6.8}, it suffices to show that if A is \emph{integral} and formally catenary, ${}^h\!\mathrm{A}$ is formally \emph{equidimensional}.
But as the completion of ${}^h\!\mathrm{A}$ is equal to $\hat{\mathrm{A}}$, this follows from the hypothesis that A is formally catenary.
\end{proof}

\begin{corollary}[18.7.6]
\label{IV.18.7.6}
If a local Noetherian ring A is excellent \sref{IV.7.8.2}, so is ${}^h\!\mathrm{A}$.
\end{corollary}

\begin{remark}[18.7.7]
\label{IV.18.7.7}
It can happen that ${}^h\!\mathrm{A}$ is an excellent local ring without A being universally catenary (nor \emph{a fortiori} excellent).
To see this, let us take the example of A from \sref{IV.5.6.11}, with the same notations.
The ring E constructed in \sref{IV.5.6.11} is excellent when $k_0$ is of characteristic 0 \sref{IV.7.8.3},~(ii) and~(iii);
consider two rings $\mathrm{E}_1$, $\mathrm{E}_2$ isomorphic to E, and ``glue'' $\Spec(\mathrm{E}_1)$ and $\Spec(\mathrm{E}_2)$ such that if $\mathfrak{m}_i$ and $\mathfrak{m}'_i$ are the maximal ideals of $\mathrm{E}_i$ ($i=1,2$), corresponding to $\mathfrak{m}$ and $\mathfrak{m}'$, the point $\mathfrak{m}_1$ is ``glued'' to $\mathfrak{m}'_2$ and the point $\mathfrak{m}_2$ to $\mathfrak{m}'_1$;
to be precise, if $\varepsilon_i$ and $\varepsilon'_i$ are the homomorphisms from $\mathrm{E}_i$ on $k(\mathfrak{m}_i)$ and $k(\mathfrak{m}'_i)$ ($i=1,2$), the schema obtained is $\Spec(\mathrm{R})$, where R is the subring of $\mathrm{E}_1\times\mathrm{E}_2$ formed by the pairs $(x_1,x_2)$ such that $\varepsilon'_1(x_1)=\sigma(\varepsilon_2(x_2))$ and $\varepsilon_1(x_1)=\sigma(\varepsilon'_2(x_2))$.
One verifies easily (for example with the help of \sref{IV.17.6.3}) that R is a finite C-algebra that is \'{e}tale, and that there are two maximal ideals $\mathfrak{r}_1$, $\mathfrak{r}_2$ of R above the maximal ideal $\mathfrak{n}$ of C, with completions of $\mathrm{R}_{\mathfrak{r}_1}$ and $\mathrm{R}_{\mathfrak{r}_2}$, and that the Henselization is equal to ${}^h\!\mathrm{A}$, having $\mathrm{A}=\mathrm{C}_\mathfrak{n}$;
$\mathrm{R}_{\mathfrak{r}_i}$ is therefore an A-algebra that is essentially \'{e}tale, and its Henselization is also equal to ${}^h\!\mathrm{A}$.
Furthermore, one has, by \sref{IV.7.8.4},~(ii) that the formal fibres of A are geometrically regular, hence R is universally catenary.
Finally, the ring R is universally catenary, and the same holds for the quotients by its two minimal prime ideals, whence the conclusion by \sref{IV.5.6.3},~(iii).
The ring $\mathrm{R}_{\mathfrak{r}_i}$ is therefore excellent, and by the same token ${}^h\!\mathrm{A}$ \sref{IV.18.7.6}, while A is not universally catenary.
\end{remark}


\subsection{Strictly local rings and strict Henselization.}
\label{subsection:IV.18.8}

\oldpage[IV-4]{144}

\begin{proposition}[18.8.1]
\label{IV.18.8.1}
Let $\mathrm{A}$ be a local ring.
The following conditions are equivalent:
\begin{enumerate}[label=\emph{\alph*)}]
  \item A is a Henselian ring and its residue field $k$ is separably closed.
  \item A is a Henselian ring and every \'{e}tale covering of $\mathrm{S}=\Spec(\mathrm{A})$ is trivial.
  \item For every \'{e}tale morphism $f:\mathrm{X}\to\mathrm{S}$ and every point $x\in\mathrm{X}$ such that $f(x)$ is the closed point $s$ of S, there exists an S-section $u:\mathrm{S}\to\mathrm{X}$ of X such that $u(s)=x$.
\end{enumerate}
\end{proposition}

\begin{proof}
The hypothesis $c)$ implies that for every \'{e}tale morphism $f:\mathrm{X}\to\mathrm{S}$, the residue fields at the points of $f^{-1}(s)$ are all isomorphic to $k$, and moreover the condition $b)$ of \sref{IV.18.5.11} is satisfied.
Hence A is Henselian, and the fact that every \'{e}tale covering of S is trivial follows from \sref{IV.18.2.10},~(ii);
hence $c)$ implies $b)$.
As \'{e}tale coverings of $\Spec(k)$ are trivial if and only if $k$ is separably closed, $b)$ implies $a)$ by virtue of \sref{IV.18.5.11},~$b$).
Finally, it follows also from \sref{IV.18.5.11},~$b$) that $a)$ implies $c)$.
\end{proof}

\begin{definition}[18.8.2]
\label{IV.18.8.2}
A local ring is said to be \emph{strictly local} if it satisfies the equivalent conditions of \sref{IV.18.8.1}.
A scheme isomorphic to the spectrum of a strictly local ring is called a \emph{strictly local scheme}.
\end{definition}

\begin{remark}[18.8.3]
\label{IV.18.8.3}
The conditions of \sref{IV.18.8.1} are also equivalent to the following:
\begin{enumerate}
  \item[d)] For every morphism locally of finite type $f:\mathrm{X}\to\mathrm{S}$ and every point $x\in\mathrm{X}$ such that $f(x)$ is the closed point $s$ of S and that $\sh{O}_{\mathrm{X},x}/\mathfrak{m}_x\sh{O}_{\mathrm{X},x}$ is a field, a finite separable extension of $k(s)=k$, there exists an open neighbourhood U of $x$ in X such that $f|U$ is a closed immersion.
\end{enumerate}
\end{remark}

(One will note that the hypothesis of $d)$ is satisfied if $f$ is \emph{formally unramified} at the point $x$ \sref{IV.17.4.1.2}.)

We leave the reader the proof, which is substantially the same as that of \sref{IV.18.5.18}.

\begin{lemma}[18.8.4]
\label{IV.18.8.4}
\oldpage[IV-4]{145}
Let $\mathrm{A}$, $\mathrm{A}'$ be two local rings, $\vphi:\mathrm{A}\to\mathrm{A}'$ a local homomorphism making $\mathrm{A}'$ an A-algebra essentially \'{e}tale \sref{IV.18.6.1}.
Then, for every strictly local ring $\mathrm{B}$, every local homomorphism $\psi:\mathrm{A}\to\mathrm{B}$ and every homomorphism of $k(\mathrm{A})$-algebras $\alpha:k(\mathrm{A}')\to k(\mathrm{B})$, there exists one and only one local homomorphism $\psi':\mathrm{A}'\to\mathrm{B}$ such that $\psi=\psi'\circ\vphi$ and that $\alpha$ is equal to the homomorphism $\overline{\psi}'$ deduced from $\psi'$ by passage to quotients.
\end{lemma}

\begin{proof}
With the notations of \sref{IV.18.6.2}, the homomorphism $\alpha$ corresponds to a $k(s)$-morphism $\Spec(k(y))\to\Spec(k(x))$, hence to a point $x'$ well determined in $\mathrm{X}'$ above $y$.
The existence of the Y-section $f'$ of $\mathrm{X}'$ such that $f'(y)=x'$ then follows from \sref{IV.18.8.1},~$c$) since $f'$ is \'{e}tale and B strictly local, and its uniqueness follows from this and from \sref{IV.17.4.9}.
\end{proof}

\begin{lemma}[18.8.5]
\label{IV.18.8.5}
Let $\mathrm{A}$ be a local ring, $\mathrm{A}_1$, $\mathrm{A}_2$ two A-algebras that are local and essentially \'{e}tale, K a field that is an extension of $k(\mathrm{A})$.
\begin{enumerate}[label=\emph{\roman*)}]
  \item For every $k(\mathrm{A})$-homomorphism $\gamma:k(\mathrm{A}_1)\to k(\mathrm{A}_2)$, there exists at most one A-homomorphism (necessarily local) $\psi:\mathrm{A}_1\to\mathrm{A}_2$ such that $\gamma$ is the homomorphism $\overline{\psi}$ deduced from $\psi$ by passage to quotients.
  \item For every pair of local A-homomorphisms $\beta_1:\mathrm{A}_1\to\mathrm{K}$, $\beta_2:\mathrm{A}_2\to\mathrm{K}$ there exists a local A-algebra $\mathrm{A}_3$ that is essentially \'{e}tale, two A-homomorphisms $\vphi_1:\mathrm{A}_1\to\mathrm{A}_3$, $\vphi_2:\mathrm{A}_2\to\mathrm{A}_3$ and an A-homomorphism $\beta:\mathrm{A}_3\to\mathrm{K}$ such that $\beta_1=\beta\circ\vphi_1$ and $\beta_2=\beta\circ\vphi_2$.
\end{enumerate}
\end{lemma}

\begin{proof}
With the notations of \sref{IV.18.6.3}, the homomorphisms $\beta_1$, $\beta_2$ in~(ii) correspond to two S-morphisms $\Spec(\mathrm{K})\to\mathrm{X}_1$, $\Spec(\mathrm{K})\to\mathrm{X}_2$, with respective images $x_1$, $x_2$;
one thus deduces a well-determined $\Spec(\mathrm{K})\to\mathrm{X}_3$ \sref[I]{I.3.2.1} whose image $x_3$ lies above $x_1$ and $x_2$;
the A-algebra $\mathrm{A}_3=\sh{O}_{\mathrm{X}_3,x_3}$ and the homomorphism $\mathrm{A}_3\to\mathrm{K}$ corresponding to these data then satisfy the conditions of~(ii).
On the other hand, the data of $\gamma$ in~(i) corresponds to an S-morphism $\Spec(k(x_2))\to\Spec(k(x_1))$ or also to a point $x'_3$ of $\mathrm{X}_3$ above $x_2$;
the uniqueness of $\psi$ follows from this and from \sref{IV.17.4.9}.
\end{proof}

\begin{env}[18.8.6]
\label{IV.18.8.6}
Let A be a local ring, $\mathfrak{m}$ its maximal ideal, $k$ its residue field, $\Omega$ a separable closure of $k$.
Consider the set $\mathfrak{E}$ of A-algebras essentially \'{e}tale defined in \sref{IV.18.6.1}, and denote by L the set of A-\emph{homomorphisms} $\mathrm{A}'\to\Omega$, where $\mathrm{A}'$ ranges over $\mathfrak{E}$;
one will note that such a homomorphism factorises as $\mathrm{A}\to k(\mathrm{A}')\xrightarrow{\omega_\lambda}\Omega$, where $\omega_\lambda:k(\mathrm{A}')\to\Omega$ is a $k$-homomorphism (since $\mathfrak{m}\mathrm{A}'$ is the maximal ideal of $\mathrm{A}'$);
conversely the data of a homomorphism $\omega_\lambda$ uniquely determines a homomorphism $\lambda\in\mathrm{L}$.
For every $\lambda\in\mathrm{L}$, we denote by $\mathrm{A}_\lambda$ the A-algebra essentially \'{e}tale belonging to $\mathfrak{E}$ on which $\lambda$ is defined.
The set L is \emph{preordered} by the relation ``there exists a homomorphism $\vphi_{\mu\lambda}:\mathrm{A}_\lambda\to\mathrm{A}_\mu$ such that $\lambda=\mu\circ\vphi_{\mu\lambda}$'', and it follows from \sref{IV.18.8.5},~(i) that this homomorphism $\vphi_{\mu\lambda}$ is \emph{unique}.
Furthermore, L is \emph{filtered increasing} for the preceding preorder relation (which one also denotes $\lambda\leq\mu$), by virtue
\oldpage[IV-4]{146}
of \sref{IV.18.8.5},~(ii).
It is clear that $(\mathrm{A}_\lambda,\vphi_{\mu\lambda})$ is an \emph{inductive system} of local A-algebras, the $\vphi_{\mu\lambda}$ being local homomorphisms;
furthermore, it follows from \sref{IV.17.3.5} that for $\lambda\leq\mu$, $\mathrm{A}_\mu$ is an $\mathrm{A}_\lambda$-algebra that is essentially \'{e}tale.
If $\overline{\vphi}_{\mu\lambda}:k(\mathrm{A}_\lambda)\to k(\mathrm{A}_\mu)$ is the $k$-homomorphism deduced from $\vphi_{\mu\lambda}$ by passage to quotients, $(k(\mathrm{A}_\lambda),\overline{\vphi}_{\mu\lambda})$ is an inductive system of separable algebraic extensions of $k$, and the $\omega_\lambda:k(\mathrm{A}_\lambda)\to\Omega$ form an inductive system of homomorphisms of $k$.
Furthermore, the inductive limit
\begin{equation}
  \omega:\varinjlim k(\mathrm{A}_\lambda)\to\Omega
  \tag{18.8.6.1}
\end{equation}
is a $k$\emph{-isomorphism}.
It suffices indeed to prove that for every finite separable extension $k'$ of $k$, there exists an A-algebra $\mathrm{A}'$ such that $k'$ is the residue field of $\mathrm{A}'$ and that the homomorphism $k\to k'$ is deduced from $\mathrm{A}\to\mathrm{A}'$ by passage to quotients.
But this follows from \sref{IV.18.1.1} applied to \'{e}tale morphisms, since $\Spec(\mathrm{A})$ is the unique neighbourhood of its closed point.

Note now that if $\mathfrak{m}_\lambda$ is the maximal ideal of $\mathrm{A}_\lambda$, it follows that $\mathfrak{m}_\mu=\mathfrak{m}_\lambda\mathrm{A}_\mu$ and that $\mathrm{A}_\mu$ is a flat $\mathrm{A}_\lambda$-module.
Hence it follows from \sref[0]{0.10.3.1.3} that the ring $\varinjlim\mathrm{A}_\lambda$ is \emph{local}, that the homomorphism $w:\mathrm{A}\to\varinjlim\mathrm{A}_\lambda$ is local, and that one has a $k$-isomorphism
\begin{equation}
  \varinjlim k(\mathrm{A}_\lambda)\xrightarrow{\sim}k(\varinjlim\mathrm{A}_\lambda);
  \tag{18.8.6.2}
\end{equation}
identifying these two fields, one deduces a $k$-isomorphism
\[
  \omega^{-1}:\Omega\xrightarrow{\sim}k(\varinjlim\mathrm{A}_\lambda).
\]
\end{env}

\begin{definition}[18.8.7]
\label{IV.18.8.7}
Let $\mathrm{A}$ be a local ring, $k=k(\mathrm{A})$ its residue field, $i:k\to\Omega$ a homomorphism from $k$ into a separable closure $\Omega$ of $k$;
the \emph{strict Henselization} of $\mathrm{A}$ relative to $i$, denoted ${}^{hs}\!\mathrm{A}_{(i)}$ (or ${}^{hs}\!\mathrm{A}$ when this causes no confusion), is the inductive limit of the inductive system $(\mathrm{A}_\lambda,\vphi_{\mu\lambda})$ defined in \sref{IV.18.8.6}.
\end{definition}

As one has a $k$-isomorphism $\omega^{-1}:\Omega\xrightarrow{\sim}k({}^{hs}\!\mathrm{A}_{(i)})$, $\Omega$ is endowed canonically with the structure of a ${}^{hs}\!\mathrm{A}_{(i)}$-algebra by the local homomorphism
\[
  {}^{hs}\!\mathrm{A}_{(i)}\to k({}^{hs}\!\mathrm{A}_{(i)})\xrightarrow{\omega}\Omega.
\]

As in \sref{IV.18.6.5} one sees that the definition given in \sref{IV.18.8.7} does not depend on the choice of $\mathfrak{E}$;
we shall moreover see below \sref{IV.18.8.8},~(i) and~(ii) that ${}^{hs}\!\mathrm{A}_{(i)}$ is an object representing a functor entirely defined by the data of A and $i$ \sref[0]{0.8.1.8}, and is hence defined as such up to an isomorphism.

\begin{proposition}[18.8.8]
\label{IV.18.8.8}
Let $\mathrm{A}$ be a local ring, $k$ its residue field, $i:k\to\Omega$ a homomorphism from $k$ into a separable closure $\Omega$ of $k$, ${}^{hs}\!\mathrm{A}_{(i)}$ the strict Henselization of $\mathrm{A}$ corresponding to $i$.
\begin{enumerate}[label=\emph{\roman*)}]
  \item ${}^{hs}\!\mathrm{A}_{(i)}$ is a strictly local ring \sref{IV.18.8.2} and the structural homomorphism $w:\mathrm{A}\to{}^{hs}\!\mathrm{A}_{(i)}$ is local.
  \item For every local homomorphism $u:\mathrm{A}\to\mathrm{B}$, where $\mathrm{B}$ is a strictly local ring, and every $k$-homomorphism $\psi:\Omega\to k(\mathrm{B})$, there exists one and only one A-homomorphism $v:{}^{hs}\!\mathrm{A}_{(i)}\to\mathrm{B}$ such that $\psi$ factorises as $\Omega\xrightarrow{\omega^{-1}}k({}^{hs}\!\mathrm{A}_{(i)})\xrightarrow{\overline{v}}k(\mathrm{B})$, where $\overline{v}$ is deduced from $v$ by passage to quotients.
  \item ${}^{hs}\!\mathrm{A}_{(i)}$ is a faithfully flat A-module, and if $\mathfrak{m}$ is the maximal ideal of A, $\mathfrak{m}.{}^{hs}\!\mathrm{A}_{(i)}$ is the maximal ideal of ${}^{hs}\!\mathrm{A}_{(i)}$, and $k({}^{hs}\!\mathrm{A}_{(i)})$ is a separable closure of $k$.
  \item For ${}^{hs}\!\mathrm{A}_{(i)}$ to be Noetherian, it is necessary and sufficient that A be so.
  \item If A is strictly local (hence $\Omega=k$), one has ${}^{hs}\!\mathrm{A}\cong\mathrm{A}$.
  \item If $i':k\to\Omega'$ is a second homomorphism from $k$ into a separable closure of $k$, for every $k$-isomorphism $\sigma:\Omega\xrightarrow{\sim}\Omega'$, the corresponding A-homomorphism (by~(ii)) $v_\sigma:{}^{hs}\!\mathrm{A}_{(i)}\to{}^{hs}\!\mathrm{A}_{(i')}$ is an isomorphism.
\end{enumerate}
\end{proposition}

\begin{proof}
\oldpage[IV-4]{147}
We have already seen above that ${}^{hs}\!\mathrm{A}_{(i)}$ is a local ring and that $w$ is a homomorphism local;
the fact that ${}^{hs}\!\mathrm{A}_{(i)}$ is Henselian (and hence strictly local) is demonstrated as in \sref{IV.18.6.6}.
The assertions of~(iii) also follow from \sref[0]{0.10.3.1.3};
the necessity of the condition~(iv);
the necessity of condition results from \sref[0]{0.6.5.2}.

To prove~(ii), remark, with the notations of \sref{IV.18.8.6}, that for every $\lambda\in\mathrm{L}$, there exists one and only one local homomorphism $v_\lambda:\mathrm{A}_\lambda\to\mathrm{B}$, such that the composite $k(\mathrm{A}_\lambda)\xrightarrow{\omega_\lambda}\Omega\xrightarrow{\psi}k(\mathrm{B})$ is deduced from $v_\lambda$ by passage to quotients, by virtue of \sref{IV.18.8.4} and the hypothesis that B is strictly local.
The uniqueness of $v_\lambda$ implies moreover that the $v_\lambda$ form an inductive system, whence the existence of the homomorphism $v$ satisfying the conditions of~(ii);
uniqueness results from~(vi) asserting the composites $v_\alpha\circ v_{\alpha-1}$ and $v_{\alpha-1}\circ v_\alpha$.
Finally, if A is strictly local, and $\mathrm{A}'=\mathrm{B}_n$ is an A-algebra local and essentially \'{e}tale, where B is an A-algebra \'{e}tale, it follows from \sref{IV.18.8.1} that there exists a $\Spec(\mathrm{A})$-section of $\Spec(\mathrm{B})$ taking the value $\mathfrak{n}$ at the closed point of $\Spec(\mathrm{A})$, hence by \sref{IV.17.4.1} that the homomorphism $\mathrm{A}\to\mathrm{A}'$ is bijective, which proves~(v).
\end{proof}

\begin{env}[18.8.8.1]
\label{IV.18.8.8.1}
Let $\mathcal{C}$ be the category of strictly local rings, with local homomorphisms as morphisms;
for every $\mathrm{B}\in\mathcal{C}$, denote by $\mathrm{F}(\mathrm{B})$ the set of pairs $(u,\psi)$ consisting of a local homomorphism $u:\mathrm{A}\to\mathrm{B}$ and a $k$-isomorphism $\psi:k=k(\mathrm{A})=k\xrightarrow{\psi}\Omega\xrightarrow{\psi}k(\mathrm{B})$ such that the composite $k(\mathrm{A})=k\xrightarrow{i}\Omega\xrightarrow{\psi}k(\mathrm{B})$ is deduced from $u$ by passage to quotients.
One can say that the object ${}^{hs}\!\mathrm{A}_{(i)}$ \emph{represents} the covariant functor $\mathrm{F}:\mathcal{C}\to\cat{Ens}$ \sref[0]{0.8.1.11}.

One will remark that by virtue of \sref{IV.18.8.8},~(vi), the strict Henselizations ${}^{hs}\!\mathrm{A}_{(i)}$ are all A-isomorphic (for the various $k$-homomorphisms $i$ of $k$ into separable closures of $k$), but for given $i$ and $i'$, there is in general an infinity of A-isomorphisms ${}^{hs}\!\mathrm{A}_{(i)}\xrightarrow{\sim}{}^{hs}\!\mathrm{A}_{(i')}$.
To be precise, the group of A-automorphisms of ${}^{hs}\!\mathrm{A}_{(i)}$ is isomorphic to the Galois group of $\Omega$ over $k$.
\end{env}

\begin{env}[18.8.9]
\label{IV.18.8.9}
Let now A be a \emph{semi-local} ring, $\mathfrak{m}_j$ ($1\leq j\leq r$) its maximal ideals, and for every $j$, consider a homomorphism $i_j:k(\mathrm{A}_{\mathfrak{m}_j})\to\Omega_j$ of the residue field into a separable closure.
The \emph{strict Henselization} of A relative to the $i_j$ is the product $\prod_{j=1}^{r}({}^{hs}(\mathrm{A}_{\mathfrak{m}_j}))$, denoted ${}^{hs}\!\mathrm{A}$ (when there is no risk of confusion).
\oldpage[IV-4]{148}
It is therefore a faithfully flat A-module and a semi-local A-algebra, whose maximal ideals are the $\mathfrak{m}_j.{}^{hs}\!\mathrm{A}$ and the radical $\mathfrak{r}.{}^{hs}\!\mathrm{A}$ (if $\mathfrak{r}$ denotes the radical of A).
Finally, the universal property \sref{IV.18.8.8},~(ii) still holds when replacing ``local homomorphism'' by ``semi-local homomorphism'' \sref{IV.18.6.7}, and replacing $\Omega$ by $\prod_j\Omega_j$.
\end{env}

\begin{proposition}[18.8.10]
\label{IV.18.8.10}
Let $\mathrm{A}$ be a semi-local ring, $\mathrm{B}$ a finite A-algebra.
Let $\mathfrak{n}_j$ ($1\leq j\leq r$) be the maximal ideals of $\mathrm{B}$.
Then there is, for every $j$, a maximal ideal $\mathfrak{q}_j$ of $\mathrm{B}\otimes_{\mathrm{A}}({}^{hs}\!\mathrm{A})$ above $\mathfrak{n}_j$, such that ${}^{hs}\!\mathrm{B}$ is isomorphic to the composed direct product of local rings $(\mathrm{B}\otimes_{\mathrm{A}}({}^{hs}\!\mathrm{A}))_{\mathfrak{q}_j}$ ($1\leq j\leq r$).
\end{proposition}

\begin{proof}
One can evidently restrict to the case where A is local, replacing A by $\mathrm{A}_{\mathfrak{m}_j}$.
On the other hand, let $\mathrm{C}=\mathrm{B}_{\mathfrak{n}_j}$;
by virtue of the definition of ${}^{hs}\!\mathrm{B}$ \sref{IV.18.8.1}, everything amounts to seeing that ${}^{hs}\!\mathrm{C}$ is C-isomorphic to $(\mathrm{C}\otimes_{\mathrm{A}}({}^{hs}\!\mathrm{A}))_{\mathfrak{q}}$, where $\mathfrak{q}$ is one of the maximal ideals of $\mathrm{C}\otimes_{\mathrm{A}}({}^{hs}\!\mathrm{A})$.
Let $k$ and K be the residue fields of A and C, $\Omega$ the separable closure of $k$, and let us suppose K and $\Omega$ contained in the same closure of $k$;
as K is a finite extension of $k$, it is immediate that $\mathrm{K}\otimes_k\Omega$ is a composed direct product of a finite number of fields, all isomorphic to the separable closure $\mathrm{K}\Omega$ of K.
On the other hand, the ring ${}^{hs}\!\mathrm{A}$ being Henselian, $\mathrm{C}\otimes_{\mathrm{A}}({}^{hs}\!\mathrm{A})$ is a composed direct product of local rings $(\mathrm{C}\otimes_{\mathrm{A}}({}^{hs}\!\mathrm{A}))_{\mathfrak{q}_i}$, where $\mathfrak{q}_i$ ($1\leq i\leq s$) ranges over the maximal ideals of $\mathrm{C}\otimes_{\mathrm{A}}({}^{hs}\!\mathrm{A})$, and these local rings are Henselian \sref{IV.18.5.10} and have $\mathrm{K}\Omega$ for residue field, hence they are strictly local \sref{IV.18.8.1}.
Let $\mathrm{C}'=\mathrm{C}\otimes_{\mathrm{A}}({}^{hs}\!\mathrm{A})$ and $\mathrm{C}'_i=\mathrm{C}'_{\mathfrak{q}_i}$, which is therefore a quotient ring of $\mathrm{C}'$;
let $f:\mathrm{C}\to\mathrm{C}'$, $g:{}^{hs}\!\mathrm{A}\to\mathrm{C}'$, $\vphi_i:\mathrm{C}'\to\mathrm{C}'_i$ be the canonical maps.
\[
  \xymatrix{
    & \mathrm{C}'_i \ar[r]^{w} & {}^{hs}\!\mathrm{C} \\
    \mathrm{C} \ar[r]^{f} & \mathrm{C}' \ar[u]^{\vphi_i} \\
    \mathrm{A} \ar[u] \ar[r] & {}^{hs}\!\mathrm{A} \ar[u]_{g}
  }
\]
Let $v:\mathrm{C}\to{}^{hs}\!\mathrm{C}$ be the canonical homomorphism local;
there exists a unique $u:{}^{hs}\!\mathrm{A}\to{}^{hs}\!\mathrm{C}$ which, by passage to quotients, gives the canonical injection $\Omega\to\mathrm{K}\Omega$ and such that the composite $\mathrm{A}\to{}^{hs}\!\mathrm{A}\xrightarrow{u}{}^{hs}\!\mathrm{C}$ is equal to the composite $\mathrm{A}\to{}^{hs}\!\mathrm{A}\xrightarrow{u}{}^{hs}\!\mathrm{C}$ \sref{IV.18.8.8};
hence there exists a unique local homomorphism $w_0:\mathrm{C}'\to{}^{hs}\!\mathrm{C}$ such that $w_0\circ g=u$, $w_0\circ f=v$, and since ${}^{hs}\!\mathrm{C}$ is a local ring, this homomorphism factorises as $\mathrm{C}'\to\mathrm{C}'_i\xrightarrow{w}{}^{hs}\!\mathrm{C}$ for a well-determined index $i$, $w$ being a local homomorphism.
On the other hand, the ring $\mathrm{C}'_i$ being strictly local, there exists a local homomorphism $w':{}^{hs}\!\mathrm{C}\to\mathrm{C}'_i$ which, by passage to quotients, gives the identity automorphism of $\mathrm{K}\Omega$ and which is such that $w'\circ v=\vphi_i\circ f$ \sref{IV.18.8.8}.
One deduces first that $w\circ w'$ is an endomorphism of ${}^{hs}\!\mathrm{C}$ which, by passage to quotients, gives the identity automorphism of $\mathrm{K}\Omega$, hence
\oldpage[IV-4]{149}
is the identity \sref{IV.18.8.8}.
On the other hand, $w'\circ w\circ\vphi_i\circ f=w'\circ v=\vphi_i\circ f$ and $w'\circ w\circ\vphi_i\circ g=w'\circ u=\vphi_i\circ g$;
whence $w'\circ w\circ\vphi_i=\vphi_i$, which is to say $w'\circ w$ is the identity automorphism of $\mathrm{C}'_i$.
\end{proof}

\begin{remark}[18.8.11]
\label{IV.18.8.11}
The proof of \sref{IV.18.8.10} uses the fact that B is a \emph{finite} A-algebra, namely to establish that $\mathrm{K}\otimes_k\Omega$ is a composed direct product of a finite number of fields.
However, this last property is still satisfied when one supposes that B is an \emph{entire} semi-local A-algebra whose maximal ideals $\mathfrak{n}_j$ have residue fields of finite separable degree $[k(\mathfrak{n}_j):k]_s$ finite.
\end{remark}

\begin{proposition}[18.8.12]
\label{IV.18.8.12}
Let $\mathrm{A}$ be a semi-local ring.
\begin{enumerate}[label=\emph{\roman*)}]
  \item For ${}^{hs}\!\mathrm{A}$ to be reduced (resp. normal), it is necessary and sufficient that A be so.
  \item Suppose A Noetherian.
  Then, for every prime ideal $\mathfrak{p}$ of A, the ring $({}^{hs}\!\mathrm{A})_{\mathfrak{p}}/\mathfrak{p}.({}^{hs}\!\mathrm{A})_{\mathfrak{p}}$ is a composed direct product of a finite number of separable algebraic extensions of $k(\mathfrak{p})$ (which implies that the fibres of the canonical morphism $\Spec({}^{hs}\!\mathrm{A})\to\Spec(\mathrm{A})$ are \emph{geometrically regular} and are \emph{discrete} spaces).
\end{enumerate}
\end{proposition}

\begin{proof}
The proofs are the same as those of \sref{IV.18.6.9},~\sref{IV.18.6.10} and \sref{IV.18.6.11} respectively.
\end{proof}

\begin{corollary}[18.8.13]
\label{IV.18.8.13}
Let $\mathrm{A}$ be a local ring.
For ${}^{hs}\!\mathrm{A}$ to possess one of the following properties:
\begin{enumerate}[label=\emph{\alph*)}]
  \item being a Cohen-Macaulay ring \sref[0]{0.16.5.3};
  \item satisfying the condition $(\mathrm{S}_n)$ \sref{IV.5.7.3};
  \item being regular;
  \item satisfying the condition $(\mathrm{R}_n)$ \sref{IV.5.8.2};
\end{enumerate}
it is necessary and sufficient that A possess the same property.
\end{corollary}

\begin{corollary}[18.8.14]
\label{IV.18.8.14}
Let $\mathrm{A}$ be a semi-local Noetherian ring.
For a prime ideal $\mathfrak{p}$ of ${}^{hs}\!\mathrm{A}$ to belong to $\operatorname{Ass}({}^{hs}\!\mathrm{A})$, it is necessary and sufficient that $\mathfrak{p}\cap\mathrm{A}$ belong to $\operatorname{Ass}(\mathrm{A})$.
\end{corollary}

\begin{proof}
The proofs are the same as those of \sref{IV.18.6.9},~\sref{IV.18.6.10} and \sref{IV.18.6.11} respectively.
\end{proof}

\begin{proposition}[18.8.15]
\label{IV.18.8.15}
Let $\mathrm{A}$ be a local ring;
the following properties are equivalent:
\begin{enumerate}[label=\emph{\alph*)}]
  \item A is geometrically unibranch (resp. reduced and geometrically unibranch).
  \item For every A-algebra $\mathrm{A}'$ that is local and essentially \'{e}tale, $\Spec(\mathrm{A}')$ is irreducible (resp. integral).
  \item $\Spec({}^{hs}\!\mathrm{A})$ is irreducible (resp. integral).
\end{enumerate}
\end{proposition}

\begin{proof}
One reduces to the case where A is reduced, using the relation ${}^{hs}(\mathrm{A}_{\mathrm{red}})=({}^{hs}\!\mathrm{A})\otimes_{\mathrm{A}}\mathrm{A}_{\mathrm{red}}$ \sref{IV.18.8.11}, and the equivalence of $b)$ and $c)$ is proved as in \sref{IV.18.6.12};
the same holds, the fact that $a)$ implies $c)$, taking into account the definition of a geometrically unibranch integral local ring \sref{IV.18.8.11}.
Finally, to prove that $c)$ implies $a)$, using the same notations as in \sref{IV.18.6.12}, it suffices to see that ${}^{hs}\!\mathrm{B}$ is integral.
But, by \sref{IV.18.8.10}, ${}^{hs}\!\mathrm{B}$ is a localised ring of the semi-local ring $\mathrm{B}\otimes_{\mathrm{A}}({}^{hs}\!\mathrm{A})$, which identifies by flatness with a subring of L, hence is integral;
it follows that ${}^{hs}\!\mathrm{B}$ is integral, hence also ${}^{hs}\!\mathrm{B}$, which completes the proof.
\end{proof}

\begin{corollary}[18.8.16]
\label{IV.18.8.16}
Let $\mathrm{A}$ be a local Henselian ring (resp. strictly local).
For A to be unibranch (resp. geometrically unibranch), it is necessary and sufficient that $\Spec(\mathrm{A})$ be irreducible.
\end{corollary}

\begin{proposition}[18.8.17]
\label{IV.18.8.17}
Let $\mathrm{A}$ be a local Noetherian ring.
If A is universally catenary, the same holds for ${}^{hs}\!\mathrm{A}$.
\end{proposition}

\begin{proof}
The proof is the same as that of \sref{IV.18.7.5}.
\end{proof}

\begin{proposition}[18.8.18]
\label{IV.18.8.18}
\begin{enumerate}[label=\emph{\roman*)}]
  \item Every filtered inductive limit of strictly local rings (where the transition homomorphisms are local) is a strictly local ring.
  \item The functor $\mathrm{A}\leadsto{}^{hs}\!\mathrm{A}$ in the category of local rings (where the morphisms are local homomorphisms) commutes with filtered inductive limits.
  \item Every strictly local ring A is an inductive limit of a filtered inductive system of Noetherian strictly local rings (the transition homomorphisms being local).
\end{enumerate}
\end{proposition}

\begin{proof}
The proofs are modelled on those of \sref{IV.18.6.14}.
\end{proof}

\oldpage[IV-4]{150}


\subsection{Formal fibres of Noetherian Henselian rings.}
\label{subsection:IV.18.9}

\begin{theorem}[18.9.1]
\label{IV.18.9.1}
Let $\mathrm{A}$ be a local Noetherian Henselian ring, whose formal fibres are geometrically normal.
Then all the fibres of $\mathrm{A}$ are geometrically connected (hence geometrically integral).
\end{theorem}

\begin{proof}
The two assertions stated are in fact equivalent, a prescheme that is locally Noetherian connected and normal being integral.
As every finite integral A-algebra is a Henselian local ring \sref{IV.18.5.9} and \sref{IV.18.5.10}, it follows from \sref{IV.7.3.16.2} that one is reduced to proving that if one further supposes A \emph{integral}, then the fibre of the morphism $\Spec(\hat{\mathrm{A}})\to\Spec(\mathrm{A})$ at the generic point of $\Spec(\mathrm{A})$ is \emph{integral}, which follows from the
\end{proof}

\begin{corollary}[18.9.2]
\label{IV.18.9.2}
Let $\mathrm{A}$ be a local Noetherian Henselian integral ring, whose formal fibres are geometrically normal.
Then $\hat{\mathrm{A}}$ is integral.
\end{corollary}

\begin{proof}
Indeed, it follows from \sref{IV.18.8.16} that A is unibranch, hence the theorem follows from the hypothesis made on the formal fibres of A and from \sref{IV.7.6.3}.
\end{proof}

\begin{corollary}[18.9.3]
\label{IV.18.9.3}
Under the hypotheses of \sref{IV.18.9.2}, the field of fractions L of $\hat{\mathrm{A}}$ is algebraically closed in L.
\end{corollary}

\begin{proof}
This follows from the fact that the fibre of the morphism $\Spec(\hat{\mathrm{A}})\to\Spec(\mathrm{A})$ at the generic point is geometrically integral and from \sref{IV.4.3.2} and \sref{IV.4.3.5}.
\end{proof}

\begin{corollary}[18.9.4]
\label{IV.18.9.4}
Let $\mathrm{A}$ be a local Noetherian ring, Henselian, and whose formal fibres are geometrically normal (for example a Noetherian Henselian local ring that is excellent), and let $\mathrm{A}'=\hat{\mathrm{A}}$.
Let $\mathrm{X}$ be an A-prescheme, $\mathrm{X}'=\mathrm{X}\otimes_{\mathrm{A}}\mathrm{A}'$, $g:\mathrm{X}'\to\mathrm{X}$ the canonical projection.
Then the map $\mathrm{U}\mapsto g^{-1}(\mathrm{U})$ is a bijection from the set of subsets of $\mathrm{X}$ that are both open and closed to the set of subsets of $\mathrm{X}'$ that are both open and closed.
\end{corollary}

\begin{proof}
As $g$ is a faithfully flat and quasi-compact morphism \sref{IV.2.3.12}, one knows that the topology of X is the \emph{quotient} of that of $\mathrm{X}'$ by the equivalence relation defined by $f$.
It therefore amounts to seeing that an open and closed subset $\mathrm{U}'$ of $\mathrm{X}'$ is \emph{saturated} for this relation.
Now, as the morphism $\Spec(\mathrm{A}')\to\Spec(\mathrm{A})$ has its fibres geometrically connected \sref{IV.18.9.1}, the fibres of $g$ are connected, whence our assertion immediately follows.

In particular, if X is locally connected (which will be the case if X is locally Noetherian), then, for every connected component U of X (which is open and closed), $g^{-1}(\mathrm{U})$ is connected (Bourbaki, \emph{Top. g\'{e}n.}, chap.~I\textsuperscript{er}, 3\textsuperscript{e} \'{e}d., \S~11, no~3, prop.~7).
If one denotes by $\pi_0(\mathrm{X})$ the set of connected components of X, the map $\pi_0(\mathrm{X}')\to\pi_0(\mathrm{X})$ canonically deduced from $g$ is therefore \emph{bijective}.
\end{proof}

\begin{corollary}[18.9.5]
\label{IV.18.9.5}
Under the hypotheses of \sref{IV.18.9.4}, the functor
\begin{equation}
  \mathrm{Z}\mapsto\mathrm{Z}\otimes_{\mathrm{A}}\mathrm{A}'
  \tag{18.9.5.1}
\end{equation}
from the category of \'{e}tale preschemes over $\mathrm{X}$, to that of \'{e}tale preschemes over $\mathrm{X}'$, is fully faithful.
\end{corollary}

\begin{proof}
\oldpage[IV-4]{151}
Let $\mathrm{Z}_1$, $\mathrm{Z}_2$ be two \'{e}tale preschemes over X, and set $\mathrm{Z}'_i=\mathrm{Z}_i\otimes_{\mathrm{A}}\mathrm{A}'$ ($i=1,2$);
it is a matter of proving that every $\mathrm{X}'$-morphism $\mathrm{Z}'_1\to\mathrm{Z}'_2$ comes, by base change, from a unique X-morphism $\mathrm{Z}_1\to\mathrm{Z}_2$.
Suppose first that $\mathrm{Z}_2$ is \emph{separated} over X;
then, as $\Hom_{\mathrm{X}}(\mathrm{Z}_1,\mathrm{Z}_2)$ identifies with $\Gamma((\mathrm{Z}_1\times_{\mathrm{X}}\mathrm{Z}_2)/\mathrm{Z}_1)$, and that $\mathrm{Z}_1\times_{\mathrm{X}}\mathrm{Z}_2$ is \'{e}tale and separated over $\mathrm{Z}_1$, by virtue of \sref{IV.17.9.3}, to the set of open and closed subsets U of $\mathrm{Z}_1\times_{\mathrm{X}}\mathrm{Z}_2$ such that the restriction to U of the projection $p:\mathrm{Z}_1\times_{\mathrm{X}}\mathrm{Z}_2\to\mathrm{Z}_1$ is a surjective and radicial morphism.
The assertion thus follows from \sref{IV.18.9.4} and the fact that $\mathrm{Z}'_1\times_{\mathrm{X}'}\mathrm{Z}'_2=(\mathrm{Z}_1\times_{\mathrm{X}}\mathrm{Z}_2)\otimes_{\mathrm{A}}\mathrm{A}'$.

Let us pass to the general case:
it will suffice to prove that the graph $\Gamma$ of an $\mathrm{X}'$-morphism $\mathrm{Z}'_1\to\mathrm{Z}'_2$ in $\mathrm{Z}'_1\times_{\mathrm{X}'}\mathrm{Z}'_2$ is of the form $\Gamma\otimes_{\mathrm{A}}\mathrm{A}'$, where $\Gamma$ is induced on an \emph{open} subset of $\mathrm{Z}=\mathrm{Z}_1\times_{\mathrm{X}}\mathrm{Z}_2$.
Indeed, the restriction to $\Gamma$ of the projection $p:\mathrm{Z}\to\mathrm{Z}_1$ is an isomorphism;
by base change $\mathrm{A}\to\mathrm{A}'$, this restriction becomes the isomorphism restriction $p':\mathrm{Z}'\to\mathrm{Z}'_1$ (with $\mathrm{Z}'=\mathrm{Z}\otimes_{\mathrm{A}}\mathrm{A}'$), and it suffices to apply \sref{IV.2.7.1},~(viii);
the conclusion then follows from the characterisation of graphs \sref[I]{I.5.3}.

If $q:\mathrm{Z}'\to\mathrm{Z}$ is the canonical projection, to prove $\mathrm{Z}'$ is of the form $q^{-1}(\Gamma)$, where $\Gamma$ is open in Z, it suffices, since $q$ is a faithfully flat and quasi-compact morphism, to prove that $q^{-1}(\Gamma)=q^{-1}(\mathrm{U})$ for an open $\mathrm{U}\subset\mathrm{Z}$.
Let $\mathrm{Z}''=\mathrm{Z}'\times_{\mathrm{Z}}\mathrm{Z}'=\mathrm{Z}\times_{\mathrm{X}}\mathrm{X}''$ where $\mathrm{X}''=\mathrm{X}'\times_{\mathrm{X}}\mathrm{X}'$, and let $q_1:\mathrm{Z}''\to\mathrm{Z}'$, $q_2:\mathrm{Z}''\to\mathrm{Z}'$ be the canonical projections.
Applying \sref{IV.4.5.19.1}, it suffices to show that $q_1^{-1}(\Gamma')=q_2^{-1}(\Gamma')$.
But this is a property which is true if and only if it is true after each base change $\Spec(k(x))\to\mathrm{X}$, where $x$ ranges over X.
In other words, it suffices to prove the corollary when X is the \emph{spectrum of a field};
but as every X-prescheme that is \'{e}tale is automatically \emph{separated} over X \sref{IV.17.6.2},~$c'$), one is reduced to the case considered at the beginning of the proof.
\end{proof}

\begin{remarks}[18.9.6]
\label{IV.18.9.6}
\begin{enumerate}[label=\emph{\roman*)}]
  \item It is possible that, if the residue field of A is of characteristic 0, the functor \sref{IV.18.9.5.1} induces even an \emph{equivalence} of the category of \'{e}tale coverings of X and the category of \'{e}tale coverings of $\mathrm{X}'$.
  One can indeed prove this when A is \emph{excellent}, using the resolution of singularities of Hironaka (M.~Artin).
  It is plausible that an analogous statement is still true without restriction on the characteristic of the residue field, provided one restricts to the \'{e}tale coverings whose ``principal Galois groups'' have order prime to the residual characteristic (cf.~[41]).

  \item One does not know whether the formal fibres of A are geometrically connected (or even geometrically irreducible) when A is any Henselian local ring.
  It would suffice that, for every Noetherian Henselian local ring, Henselian and \emph{integral} A, $\Spec(\hat{\mathrm{A}})$ be irreducible.
  One does not know if this is always the case.

  \item One does not know whether, when A is an excellent local ring, its strict Henselization ${}^{hs}\!\mathrm{A}$ has geometrically regular formal fibres.
  One can indeed elucidate this question, one can reduce to the case where $\mathrm{A}=k[[\mathrm{T}_1,\ldots,\mathrm{T}_n]]$, $k$ being a field of characteristic $p>0$.
  Taking account of \sref{IV.18.8.17}, the question is whether the formal fibres of ${}^{hs}\!\mathrm{A}$ are geometrically regular.
  The answer is affirmative when $n=1$, but is not known for $n=2$.
\end{enumerate}
\end{remarks}

The connectedness assertion made in \sref{IV.18.9.1} generalises in the following way:

\begin{theorem}[18.9.7]
\label{IV.18.9.7}
\oldpage[IV-4]{152}
Let $\mathrm{A}$, $\mathrm{B}$ be two local Noetherian rings, $\rho:\mathrm{A}\to\mathrm{B}$ a local homomorphism, $f:\Spec(\mathrm{B})=\mathrm{X}\to\Spec(\mathrm{A})=\mathrm{Y}$ the corresponding morphism.
Suppose the following conditions satisfied:
\begin{enumerate}[label=\emph{\roman*)}]
  \item $f$ is flat and all the fibres $\mathrm{X}_y=f^{-1}(y)$ ($y\in\mathrm{Y}$) are geometrically reduced \sref{IV.4.6.2}.
  \item Either the ring $\mathrm{A}$ is geometrically unibranch \sref[0]{0.23.2.1}, or else the ring A is unibranch \sref[0]{0.23.2.1} and $k(\mathrm{B})$ is a primary extension \sref{IV.4.3.1} of $k(\mathrm{A})$.
\end{enumerate}
Then, if $\eta$ is the generic point of $\mathrm{Y}$, $\mathrm{X}_\eta$ is connected, and for every closed subset $\mathrm{T}\subset f^{-1}(y)$, $\mathrm{X}-\mathrm{T}$ is connected.
\end{theorem}

\begin{proof}
The two conclusions stated are in fact equivalent;
indeed, the hypothesis that $f(\mathrm{T})$ is rare in Y, T does not contain any maximal point of X \sref{IV.2.3.4}, hence is rare in X;
as X is a local scheme, hence connected, to prove that $\mathrm{X}-\mathrm{T}$ is connected, it suffices, by virtue of \sref{IV.15.5.6.1}, to show that, for every $x\in f^{-1}(y)$, $\Spec(\sh{O}_{\mathrm{X},x})-\{x\}$ is connected.
One can therefore restrict to the case where $\mathrm{X}=\Spec(\mathrm{B})$, B being a local Noetherian ring and $\mathrm{A}\to\mathrm{B}$ the corresponding local homomorphism;
let $\mathrm{U}=\mathrm{Y}-\{y\}$, and note that $\mathrm{V}=f^{-1}(\mathrm{U})$ is dense in X \sref{IV.2.3.10};
it suffices therefore to prove that V is \emph{connected}.
Keeping the notations of \sref{IV.18.9.7.5}, we thus have the commutative diagram
\[
  \xymatrix{
    \mathrm{X} & \mathrm{X}_1 \ar[l]_{g_1} \\
    \mathrm{Y} \ar[u]^{f} & \mathrm{Y}_1 \ar[l]^{g} \ar[u]_{f_1}
  }
\]
\end{proof}

\begin{remark}[18.9.7.1]
\label{IV.18.9.7.1}
When the ring A is \emph{Japanese}, one can, in the case preceding \sref{IV.18.9.7.2}, take $\mathrm{A}''=\mathrm{A}'$ by definition \sref[0]{0.23.1.1};
one sees therefore that in this case one would even be able to prove the theorem when A is \emph{integrally closed}.
\end{remark}

\begin{env}[18.9.7.2]
\label{IV.18.9.7.2}
Suppose therefore now that Y is integral and geometrically unibranch.
Note that if T is a closed subset of X such that $f(\mathrm{T})$ is rare in Y, T contains no maximal point of X \sref{IV.2.3.4}, hence is rare in X;
as X is a local scheme, hence connected, to prove that $\mathrm{X}-\mathrm{T}$ is connected, it suffices, by virtue of \sref{IV.15.5.6.1}, to show that for every $x\in f^{-1}(y)$, $\Spec(\sh{O}_{\mathrm{X},x})-\{x\}$ is connected.
One can therefore restrict to the case where $\mathrm{X}=\Spec(\mathrm{B})$, B being a local Noetherian ring and the homomorphism $\mathrm{A}\to\mathrm{B}$ being local;
let $\mathrm{U}=\mathrm{Y}-\{y\}$, and note that $\mathrm{V}=f^{-1}(\mathrm{U})$ is dense in X \sref{IV.2.3.10};
it suffices therefore to prove that V is \emph{connected}.
\end{env}

\begin{lemma}[18.9.7.3]
\label{IV.18.9.7.3}
Let $\mathrm{A}$, $\mathrm{B}$ be two local Noetherian rings, $\mathrm{Y}=\Spec(\mathrm{A})$, $\mathrm{X}=\Spec(\mathrm{B})$, $\rho:\mathrm{A}\to\mathrm{B}$ a local homomorphism, $f:\mathrm{X}\to\mathrm{Y}$ the corresponding morphism.
Suppose the following conditions satisfied:
\begin{enumerate}[label=\emph{\roman*)}]
  \item $f$ is flat and all the fibres $\mathrm{X}_y$ ($y\in\mathrm{Y}$) are geometrically reduced.
  \item A is integral, geometrically unibranch and $\dim(\mathrm{A})\geq 1$.
\end{enumerate}
Then, if $b$ is the closed point of $\mathrm{X}$, $\mathrm{X}-\{b\}$ is connected.
\end{lemma}

\begin{proof}
\oldpage[IV-4]{153}
Note first that by virtue of the theorem of Hartshorne \sref{IV.5.10.7}, $\mathrm{X}-\{b\}$ is connected if $\operatorname{prof}(\mathrm{B})\geq 2$.
Now on has \sref{IV.6.3.1}
\begin{equation}
  \operatorname{prof}(\mathrm{B})=\operatorname{prof}(\mathrm{A})+\operatorname{prof}(\mathrm{B}\otimes_{\mathrm{A}}k)
  \tag{18.9.7.4}
\end{equation}
where $k$ denotes the residue field of A.
On the other hand, since A is integral and $\dim(\mathrm{A})\geq 1$, one has $\operatorname{prof}(\mathrm{A})\geq 1$, hence $\operatorname{prof}(\mathrm{B})\geq 2$ \emph{unless} $\operatorname{prof}(\mathrm{B}\otimes_{\mathrm{A}}k)=0$;
moreover, $\mathrm{B}\otimes_{\mathrm{A}}k$ is reduced by~(i), hence the relation $\operatorname{prof}(\mathrm{B}\otimes_{\mathrm{A}}k)=0$ means that $\mathrm{B}\otimes_{\mathrm{A}}k$ is a \emph{field} \sref[0]{0.16.4.7}.
We shall henceforth suppose that this last condition is satisfied.
We shall begin by treating a case where the proof is very simple.

A) \emph{Case where A is integrally closed.}
Then, if $\dim(\mathrm{A})\geq 2$, one has $\operatorname{prof}(\mathrm{A})\geq 2$ \sref[0]{0.16.5.1}, hence $\operatorname{prof}(\mathrm{B})\geq 2$.
One therefore only has to consider the case where $\dim(\mathrm{A})=1$ and where $\mathrm{B}\otimes_{\mathrm{A}}k$ is a field;
A is then a discrete valuation ring, hence regular, and since $\mathrm{B}\otimes_{\mathrm{A}}k$ is a field, B is also a discrete valuation ring \sref[0]{0.17.3.3};
but in addition \sref{IV.6.1.1.1}, one has $\dim(\mathrm{B})=\dim(\mathrm{A})+\dim(\mathrm{B}\otimes_{\mathrm{A}}k)=\dim(\mathrm{A})=1$, hence B is also a discrete valuation ring, hence $\mathrm{X}-\{b\}$ is reduced to a single point, whence the lemma \sref{IV.18.9.7.3} in this case.

One will observe that this proves the statement of the theorem \sref{IV.18.9.7} when one supposes the ring A \emph{Japanese}, for by virtue of the remark \sref{IV.18.9.7.1}, one is reduced to the case where A is \emph{integrally closed}, and then, in the reduction \sref{IV.18.9.7.3}, the ring $\mathrm{A}_1=\sh{O}_{\mathrm{X},y}$ is also integrally closed and one can apply the result just proved.

B) \emph{Case where} $\dim(\mathrm{A})\geq 2$.
The result of \sref{IV.18.9.7.3} will follow from the two following lemmas.
\end{proof}

\begin{lemma}[18.9.7.5]
\label{IV.18.9.7.5}
Let $\mathrm{A}$ be a semi-local Noetherian ring, reduced, $\mathrm{A}'$ its integral closure in its total ring of fractions, $\mathrm{X}=\Spec(\mathrm{A})$, $\mathrm{X}'=\Spec(\mathrm{A}')$;
let S be the set of closed points of $\mathrm{X}$ and $\mathrm{U}=\mathrm{X}-\mathrm{S}$.
Suppose that for every point $x'\in\mathrm{X}'$ above a point of $\mathrm{U}$, $\dim(\sh{O}_{\mathrm{X}',x'})\geq 2$;
then $\mathrm{A}^{(1)}=\Gamma(\mathrm{U},\sh{O}_{\mathrm{X}})$ is entire over $\mathrm{A}$ and is a sub-A-algebra isomorphic to $\mathrm{A}'$.
\end{lemma}

\begin{proof}
\oldpage[IV-4]{154}
Recall that the $\mathfrak{p}_i$ ($1\leq i\leq m$) are the minimal prime ideals of A, $\mathrm{A}'$ is the composed direct product of the integral closures $\mathrm{A}_i=\mathrm{A}/\mathfrak{p}_i$, so that $\mathrm{X}'$ is the sum of the schemes $\mathrm{X}'_i=\Spec(\mathrm{A}'_i)$ ($1\leq i\leq m$).
If $\mathrm{X}_0$ is the sum of the schemes $\mathrm{X}_i=\Spec(\mathrm{A}_i)$ ($1\leq i\leq m$) and $\mathrm{U}_0$ the inverse image of U in $\mathrm{X}_0$, the canonical homomorphism $\Gamma(\mathrm{U},\sh{O}_{\mathrm{X}})\to\Gamma(\mathrm{U}_0,\sh{O}_{\mathrm{X}_0})$ is injective since X is reduced and the morphism $\mathrm{X}'\to\mathrm{X}$ surjective.
As $\Gamma(\mathrm{U}_0,\sh{O}_{\mathrm{X}_0})$ is the composed direct product of the $\Gamma(\mathrm{U}_i,\sh{O}_{\mathrm{X}_i})$ (where $\mathrm{U}_i$ is the inverse image of U in $\mathrm{X}_i$), it suffices to prove that for each $i$, $\Gamma(\mathrm{U}_i,\sh{O}_{\mathrm{X}_i})$ is isomorphic to a sub-$\mathrm{A}_i$-algebra of $\mathrm{A}'_i$.
As X is reduced and the morphism $\mathrm{X}'\to\mathrm{X}$ surjective, the homomorphism $\Gamma(\mathrm{U},\sh{O}_{\mathrm{X}})\to\Gamma(\mathrm{U}',\sh{O}_{\mathrm{X}'})$ (where $\mathrm{U}'$ is the inverse image of U in $\mathrm{X}'$) is injective, and everything amounts to verifying that the canonical homomorphism $\mathrm{A}'=\Gamma(\mathrm{X}',\sh{O}_{\mathrm{X}'})\to\Gamma(\mathrm{U}',\sh{O}_{\mathrm{X}'})$ is \emph{bijective}.
Now \sref[I]{I.8.2.1.1}, $\Gamma(\mathrm{U}',\sh{O}_{\mathrm{X}'})$ is the intersection of the local rings $\mathrm{A}'_{\mathfrak{p}'}$, where $\mathfrak{p}'$ ranges over U', and by virtue of the hypothesis, among these local rings are included all those of height 1.
But the reasoning of \sref[0]{0.23.2.7} applies also to a semi-local Noetherian integral ring, and is therefore a \emph{semi-local Krull ring}, and is therefore the intersection of the local rings $\mathrm{A}'_{\mathfrak{p}'}$, where $\mathfrak{p}'$ ranges over the set of prime ideals of height 1 of $\mathrm{A}'$ (Bourbaki, \emph{Alg. comm.}, chap.~VII, \S~1, no~6, th.~4);
\emph{a fortiori}, $\mathrm{A}'$ is the intersection of the $\mathrm{A}'_{\mathfrak{p}'}$ for $\mathfrak{p}'\in\mathrm{U}'$, which completes the proof of the lemma.
\end{proof}

\begin{remark}[18.9.7.6]
\label{IV.18.9.7.6}
We know \sref[0]{0.23.2.5} that there exists a finite sub-A-algebra $\mathrm{A}''$ of K, the field of fractions of A, such that the morphism $\Spec(\mathrm{A}')\to\Spec(\mathrm{A}'')$ is \emph{radicial};
as this morphism is also entire and dominant, hence surjective \sref[II]{II.6.1.10}, it is a \emph{homeomorphism} \sref{IV.2.4.5}.
By virtue of the geometric interpretation of $\dim(\sh{O}_{\mathrm{X}',x'})$ \sref{IV.5.1.2}, one has therefore, for every point $x''$ of $\mathrm{X}''=\Spec(\mathrm{A}'')$ above a point $x\in\mathrm{X}$, $\dim(\sh{O}_{\mathrm{X}'',x''})=\dim(\sh{O}_{\mathrm{X}',x'})$, whence $\dim(\sh{O}_{\mathrm{X}'',x'})\geq 2$ if $x'$ is the unique point of $\mathrm{X}'$ above $x$.
\oldpage[IV-4]{155}
The same conclusion holds if $\sh{O}_{\mathrm{X},x}$ is \emph{geometrically unibranch}, for the fibre of the morphism $\mathrm{X}'\to\mathrm{X}$ at the point $x$ is then reduced to a single point, and $\sh{O}_{\mathrm{X}',x'}$ is an \emph{entire} $\sh{O}_{\mathrm{X},x}$-algebra;
it suffices then to apply \sref[0]{0.16.1.5}.
\end{remark}

\begin{lemma}[18.9.7.7]
\label{IV.18.9.7.7}
Let $\mathrm{A}$ be a local Noetherian ring, of dimension $\geq 2$, integral and geometrically unibranch, $\mathrm{Y}=\Spec(\mathrm{A})$, $y$ the closed point of Y.
Let $\mathrm{X}$ be a locally Noetherian prescheme and $f:\mathrm{X}\to\mathrm{Y}$ a flat morphism.
Then, for every closed subset $\mathrm{T}\subset f^{-1}(y)$, $\mathrm{X}-\mathrm{T}$ is connected.
\end{lemma}

\begin{proof}
\oldpage[IV-4]{155}
The same reasoning as at the beginning of \sref{IV.18.9.7.2} reduces to proving that, for every point $x\in f^{-1}(y)$, $\Spec(\sh{O}_{\mathrm{X},x})-\{x\}$ is connected.
One can therefore restrict to the case where $\mathrm{X}=\Spec(\mathrm{B})$, B being a local Noetherian ring and the homomorphism $\mathrm{A}\to\mathrm{B}$ being local;
let $\mathrm{U}=\mathrm{Y}-\{y\}$, and note that $\mathrm{V}=f^{-1}(\mathrm{U})$ is dense in X \sref{IV.2.3.10};
it suffices therefore to prove that V is \emph{connected}.
Keeping the notations of \sref{IV.18.9.7.5}, and setting $\mathrm{A}^{(1)}=\Gamma(\mathrm{U},\sh{O}_{\mathrm{X}})$, $\mathrm{X}_1=\mathrm{X}\times_{\mathrm{Y}}\mathrm{Y}_1=\Spec(\mathrm{B}\otimes_{\mathrm{A}}\mathrm{A}^{(1)})$, we have the commutative diagram
\[
  \xymatrix{
    \mathrm{X} & \mathrm{X}_1 \ar[l]_{g_1} \\
    \mathrm{Y} \ar[u]^{f} & \mathrm{Y}_1 \ar[l]^{g} \ar[u]_{f_1}
  }
\]
By virtue of \sref{IV.2.3.1} and of the definition of $\mathrm{A}^{(1)}$, one has a canonical isomorphism $\Gamma(\mathrm{V},\sh{O}_{\mathrm{X}})\xrightarrow{\sim}\mathrm{B}\otimes_{\mathrm{A}}\mathrm{A}^{(1)}$, and it suffices therefore to prove that this last ring is \emph{local}, such a ring being a product of two nonzero rings \sref[III]{III.7.8.6.1}.
But we have seen \sref{IV.18.9.7.5} that $\mathrm{A}^{(1)}$ is a sub-A-algebra of the integral closure $\mathrm{A}'$ of A, hence $\dim(\mathrm{A})\geq 2$.
The hypothesis that A is geometrically unibranch implies that the morphism $\Spec(\mathrm{A}')\to\Spec(\mathrm{A})$ is radicial at the point $y$ \sref{IV.6.15.3};
\emph{a fortiori} $g_1$ is radicial at the closed point of X;
furthermore $g_1$ is an entire morphism, hence the closed points of $\mathrm{X}_1$ are above $x$, hence there is only one closed point of $\mathrm{X}_1$, which completes the proof of \sref{IV.18.9.7.7}.

It is clear that \sref{IV.18.9.7.7} proves \sref{IV.18.9.7.3} when $\dim(\mathrm{A})\geq 2$ (even without supposing the fibres geometrically reduced).

C) \emph{Case where} $\dim(\mathrm{A})=1$.
One has seen at the beginning of the proof of \sref{IV.18.9.7.3} that one can suppose $\dim(\mathrm{B}\otimes_{\mathrm{A}}k)=0$, and by flatness \sref{IV.6.1.1.1}, one therefore has
\[
  \dim(\mathrm{B})=\dim(\mathrm{A})+\dim(\mathrm{B}\otimes_{\mathrm{A}}k)=1.
\]
Since A is integral, Y consists of two points, the generic point $\eta$ and the closed point $a$.
Furthermore, since $f$ is flat, every irreducible component of X dominates Y \sref{IV.2.3.4}, hence $\mathrm{X}-\{b\}$, which is the set of maximal points $\xi_i$ of X, is equal to
\oldpage[IV-4]{156}
the underlying set of $f^{-1}(\eta)$, and the fibre $f^{-1}(\eta)$ is the sum of the $\Spec(k(\xi_i))$.
The hypothesis on the $\mathrm{X}_y$ and the fact that these fibres are of dimension 0, show that they are \emph{geometrically regular} \sref{IV.6.7.6}, and \emph{a fortiori} geometrically normal, in other words $f$ is a \emph{normal morphism}.
But since A is integral and geometrically unibranch, it follows then from \sref{IV.6.15.10} that B is also integral and geometrically unibranch, hence $f^{-1}(\eta)=\mathrm{X}-\{b\}$ is reduced to a single point, and \emph{a fortiori} connected;
this concludes the proof of \sref{IV.18.9.7.3} and of \sref{IV.18.9.7}.
\end{proof}

\begin{remark}[18.9.7.8]
\label{IV.18.9.7.8}
In case C) of the proof of \sref{IV.18.9.7.3}, one can avoid making appeal to the delicate result \sref{IV.6.15.10} by reasoning as follows:
since A is integral and unibranch of dimension 1, it follows from the theorem of Krull-Akizuki (Bourbaki, \emph{Alg. comm.}, chap.~VII, \S~2, no~5, prop.~5) that its integral closure $\mathrm{A}'$ is a \emph{Noetherian} ring.
The same reasoning as at the beginning of the proof of \sref{IV.18.9.7} shows that $\mathrm{B}'=\mathrm{B}\otimes_{\mathrm{A}}\mathrm{A}'$ is a local ring;
furthermore, by flatness, $\mathrm{B}'$ is contained in the total ring of fractions R of the reduced $\mathrm{B}$ \sref{IV.3.3.5}.
Now, the theorem of Krull-Akizuki (Bourbaki, \emph{loc. cit.}) also applies to a local Noetherian \emph{reduced} ring of dimension 1, and shows that for every such ring B, everything contained between B and its total ring of fractions is \emph{Noetherian}.
The ring $\mathrm{B}'$ being a local Noetherian ring and the morphism $\Spec(\mathrm{B}')\to\Spec(\mathrm{A}')$ being normal, $\mathrm{B}'$ is an integral ring and integrally closed \sref{IV.6.5.4}, and one concludes as at the beginning of the proof of \sref{IV.18.9.7}.
\end{remark}

\begin{corollary}[18.9.8]
\label{IV.18.9.8}
With the notations of \sref{IV.18.9.7}, suppose verified the condition~(i) of \sref{IV.18.9.7} and one of the two conditions:
\begin{enumerate}
  \item[\rm(ii$'$)] A is a strictly local ring \sref{IV.18.8.2}.
  \item[\rm(ii$''$)] A is a Henselian local ring and $k(\mathrm{B})$ is a primary extension of $k(\mathrm{A})$.
\end{enumerate}
Then all the fibres $\mathrm{X}_y$ ($y\in\mathrm{Y}$) are geometrically connected.
\end{corollary}

\begin{proof}
Let $\mathfrak{p}$ be the prime ideal of A corresponding to the point $y$, and set $\mathrm{A}'=\mathrm{A}/\mathfrak{p}$ and $\mathrm{B}'=\mathrm{B}\otimes_{\mathrm{A}}\mathrm{A}'=\mathrm{B}/\mathfrak{p}\mathrm{B}$, which are evidently local Noetherian rings;
it is clear that the morphism $f':\Spec(\mathrm{B}')\to\Spec(\mathrm{A}')$ satisfies condition~(i) of \sref{IV.18.9.7};
and as $y$ is the generic point of $\Spec(\mathrm{A}')$ and $k(\mathrm{A}')=k(\mathrm{A})$, $k(\mathrm{B}')=k(\mathrm{B})$, one sees that it suffices to verify that, in the case~(ii$'$) (resp.~(ii$''$)), $\mathrm{A}'$ is geometrically unibranch (resp. unibranch).
But $\mathrm{A}'$ is integral, and in the case~(ii$'$) (resp.~(ii$''$)) it is strictly local (resp. Henselian) by virtue of \sref{IV.18.5.10}.
One therefore concludes that $\mathrm{A}'$ is geometrically unibranch (resp. unibranch) by virtue of \sref{IV.18.8.15} (resp. \sref{IV.18.6.12}).
\end{proof}

\begin{corollary}[18.9.9]
\label{IV.18.9.9}
Let $\mathrm{A}$ be a local Noetherian ring, Henselian, universally Japanese (i.e.~\sref{IV.7.6.4} and \sref{IV.7.7.2}), whose formal fibres \sref{IV.7.3.13} are geometrically reduced;
then all the fibres formelles of A are geometrically connected.
\end{corollary}

\begin{proof}
It suffices to apply \sref{IV.18.9.8} in the case where $\mathrm{B}=\hat{\mathrm{A}}$, since B is an A-module that is flat and that $k(\mathrm{B})=k(\mathrm{A})$.
\end{proof}

\begin{remark}[18.9.10]
\label{IV.18.9.10}
The proofs of \sref{IV.18.9.4} and \sref{IV.18.9.5} do not use the fact that the formal fibres of A are geometrically connected.
The conclusions
\oldpage[IV-4]{157}
of these two propositions are therefore still valid when one supposes the local ring A Noetherian, Henselian and universally Japanese.
\end{remark}

\begin{corollary}[18.9.11]
\label{IV.18.9.11}
Let $\mathrm{X}$, $\mathrm{Y}$ be two locally Noetherian preschemes, $\mathrm{Y}$ being supposed geometrically unibranch and X connected.
Let $f:\mathrm{X}\to\mathrm{Y}$ be a reduced morphism (i.e.~\sref{IV.6.8.1}), flat and with geometrically reduced fibres.
Then, for every closed subset $\mathrm{T}$ of $\mathrm{X}$ such that $f(\mathrm{T})$ is rare in $\mathrm{Y}$, $\mathrm{X}-\mathrm{T}$ is connected.
\end{corollary}

\begin{proof}
As $f$ is flat and $f(\mathrm{T})$ is rare, T cannot contain any maximal point of X \sref{IV.2.3.4}, hence is rare in X.
Utilising \sref{IV.15.5.6.1}, it suffices to show that, for every $x\in\mathrm{T}$, $\Spec(\sh{O}_{\mathrm{X},x})-\{x\}$ is connected.
Setting $y=f(x)$, as the ring $\sh{O}_{\mathrm{Y},y}$ is geometrically unibranch, one can apply \sref{IV.18.9.7} to the morphism
\[
  f_1:\Spec(\sh{O}_{\mathrm{X},x})\to\Spec(\sh{O}_{\mathrm{Y},y}),
\]
which satisfies conditions~(i) and~(ii) of this theorem, and to the closed subset $\{x\}$ of $\Spec(\sh{O}_{\mathrm{X},x})$, since $f_1(\{x\})=\{y\}$ is rare in $\Spec(\sh{O}_{\mathrm{Y},y})$.
\end{proof}
\subsection{\'Etale preschemes over a geometrically unibranch or normal prescheme}
\label{subsection:IV.18.10}

\begin{theorem}[18.10.1]
\label{IV.18.10.1}
Let $f:X\to Y$ be a locally of finite presentation morphism, $x$ a point of $X$, $y=f(x)$.
Suppose that $Y$ is integral and geometrically unibranch at the point $y$.
Then, for $f$ to be \'etale at the point $x$, it is necessary and sufficient that the following two conditions be satisfied:
\begin{enumerate}[label=\emph{\roman*)}]
  \item $f$ is formally unramified at the point $x$;
  \item the homomorphism $\sh{O}_{Y,y}\to\sh{O}_{X,x}$ is injective.
\end{enumerate}

Moreover, if this is the case, $X$ is integral and geometrically unibranch at the point $x$.

Finally, if $\sh{O}_{Y,y}$ is Noetherian, one can, in the preceding criterion, replace the condition \emph{(ii)} by the condition

\emph{(ii bis)} $\dim\sh{O}_{Y,y}\leq\dim\sh{O}_{X,x}$.
\end{theorem}

\begin{proof}
\oldpage[IV-4]{157}
The fact that, if $f$ is \'etale, $X$ is integral and geometrically unibranch at the point $x$ is a particular case of \sref{IV.17.5.7}.
It is clear on the other hand that the conditions (i) and (ii) are necessary for $f$ to be \'etale at the point $x$, since $\sh{O}_{X,x}$ is then an $\sh{O}_{Y,y}$-module faithfully flat \sref{IV.17.6.1}.
Let us prove that these conditions are sufficient.

Set $\mathrm{A}=\sh{O}_{Y,y}$, $\mathrm{A}'={}^{\mathrm{hs}}\mathrm{A}$ \sref{IV.18.8.7}, $\mathrm{Y}'=\operatorname{Spec}(\mathrm{A}')$, $\mathrm{X}'=\mathrm{X}\times_{\mathrm{Y}}\mathrm{Y}'$, $f'=f_{(\mathrm{Y}')}:\mathrm{X}'\to\mathrm{Y}'$.
Let $y'$ be the closed point of $\mathrm{Y}'$, $x'$ a point of $\mathrm{X}'$ above $x$ and $y'$; since the morphism $g:\mathrm{Y}'\to\mathrm{Y}$ is \emph{flat} \sref{IV.18.8.8}, it amounts to showing that $f'$ is \'etale at the point $x'$ \sref{IV.17.7.1},(ii)).
The hypothesis (ii) implies that the morphism $\operatorname{Spec}(\sh{O}_{X,x})\to\operatorname{Spec}(\sh{O}_{Y,y})$ is dominant \sref[I]{I.1.2.7}; as $\sh{O}_{Y,y}$ is integral, $\operatorname{Spec}(\sh{O}_{Y,y})$ has a single generic point $y_1$, and there is a generization $x_1$ of $x$ in $\mathrm{X}$ above $y_1$.
The projection morphism $\mathrm{X}'\to\mathrm{X}$ being flat, there exists a generization $x_1'$ of $x'$ above $x_1$ \sref{IV.2.3.4}; set $y_1'=f'(x_1')$.
Now, the hypothesis on $y$ implies that $\mathrm{A}'$ is \emph{integral} \sref{IV.18.8.15}, hence $\mathrm{Y}'$ has a single generic point $\eta$, necessarily above $y_1$ since $g$
\oldpage[IV-4]{158}
is flat \sref{IV.2.3.4}; on the other hand, $\eta$ is also the generic point of $g^{-1}(y_1)$ \sref[0]{0.2.1.8}, and one knows on the other hand that the fibres of $g$ are discrete spaces \sref{IV.18.8.12}, hence $g^{-1}(y_1)=\{\eta\}$, and consequently $y_1'=\eta$.
The morphism $\operatorname{Spec}(\sh{O}_{X',x'})\to\operatorname{Spec}(\sh{O}_{Y',y'})$, restriction of $f'$, is therefore dominant, and since $\sh{O}_{Y',y'}=\mathrm{A}'$ is integral, the corresponding homomorphism $\sh{O}_{Y',y'}\to\sh{O}_{X',x'}$ is injective \sref[I]{I.1.2.7}.
On the other hand, $f'$ is not ramified at the point $x'$, (hence \sref{IV.18.8.3},~$d$)), there exists a neighbourhood $\mathrm{U}$ of $x'$ in $\mathrm{X}'$ such that $f'|\mathrm{U}$ is a \emph{closed immersion} \sref{IV.18.8.3}; \emph{a fortiori} the homomorphism $\sh{O}_{Y',y'}\to\sh{O}_{X',x'}$ is surjective, and since it is injective, it is therefore \emph{bijective}.
But then $\sh{O}_{X,x'}$ is an $\sh{O}_{Y',y'}$-module \emph{flat}, and $f'$ is therefore \'etale at the point $x'$ \sref{IV.17.6.1}.

Finally, when $\sh{O}_{Y,y}$ is Noetherian, we know that if $f$ is \'etale at the point $x$, we have $\dim\sh{O}_{Y,y}=\dim\sh{O}_{X,x}$ \sref{IV.17.6.4}.
Conversely, suppose the conditions (i) and (ii~bis) satisfied, and let us show that they imply conditions (i) and (ii).
Let $\mathfrak{J}$ be the kernel of the canonical homomorphism $\sh{O}_{Y,y}\to\sh{O}_{X,x}$; restricting $Y$ as needed to a neighbourhood of $y$, we may suppose that $\mathfrak{J}=\sh{J}_y$, where $\sh{J}$ is an Ideal of $\sh{O}_Y$; if $\mathrm{Y}_1$ is the closed sub-prescheme of $\mathrm{Y}$ defined by $\sh{J}$, one can, restricting further $\mathrm{X}$ and $\mathrm{Y}$ to neighbourhoods of $x$ and $y$ respectively, suppose that $f$ factorises as $\mathrm{X}\xrightarrow{h}\mathrm{Y}_1\to\mathrm{Y}$ \sref[I]{I.6.5.1}.
It is clear that $f_1$ is still not ramified at the point $x$, hence \sref{IV.17.4.1} quasi-finite at this point; the demonstration of \sref{IV.5.4.1},~(i)) then shows that $\dim(\sh{O}_{X,x})\leq\dim(\sh{O}_{Y,y}/\mathfrak{J})$.
But if one had $\mathfrak{J}\neq 0$, one would conclude that $\dim(\sh{O}_{Y,y}/\mathfrak{J})<\dim(\sh{O}_{Y,y})$ since $\mathrm{Y}$ is integral \sref[0]{0.16.1.2.2}; one would then have $\dim\sh{O}_{X,x}<\dim\sh{O}_{Y,y}$, contradicting the hypothesis, which proves (ii).
\end{proof}

\begin{remark}[18.10.2]
\label{IV.18.10.2}
\medskip\noindent
\begin{enumerate}[label=(\roman*)]
  \item When $\mathrm{A}=\sh{O}_{Y,y}$ is Noetherian, and one knows that its completion $\hat{\mathrm{A}}$ is integral (for example if $\mathrm{A}$ is regular), one can, in the preceding proof, replace $\mathrm{A}'$ by $\hat{\mathrm{A}}$.
  \item When $\mathrm{Y}$ is locally Noetherian, one can give a faster proof of \sref{IV.18.10.1}, without using strict henselisation, but invoking the rather delicate results of \textsection\textsection~14 and 15.
As $f$ is quasi-finite at the point $x$ by hypothesis \sref{IV.17.4.1}, one can, replacing $\mathrm{X}$ by an open neighbourhood of $x$, suppose that $f^{-1}(y)=\{x\}$.
The hypothesis (ii) implies, as we have seen, the existence of an irreducible component $\mathrm{Z}$ of $\mathrm{X}$ containing $x$ and dominating the unique irreducible component $\mathrm{Y}_0$ of $\mathrm{Y}$ containing $y$.
The morphism $f|\mathrm{Z}$ being quasi-finite at the point $x$, hence equidimensional at this point \sref{IV.13.2.2}, it follows from the hypothesis on $\mathrm{Y}$ and from the Chevalley criterion \sref{IV.14.4.4} that $g=f|\mathrm{Z}$ is universally open at the point $x$.
Furthermore since $f$ (and hence also $g$) is not ramified at the point $x$, $g^{-1}(y)$ is geometrically reduced on $k(y)$ \sref{IV.17.4.1}; as $\sh{O}_{Y,y}$ is integral, one concludes from \sref{IV.15.2.3} that $g$ is \emph{flat} at the point $x$, hence $f$ is \emph{\'etale} at this point \sref{IV.18.4.9}.
  \item With the same notations and hypotheses on $\mathrm{Y}$ as in \sref{IV.18.10.1}, suppose only that $f$ is \emph{locally of finite type} (and not necessarily locally of finite presentation).
Then, for $\sh{O}_{X,x}$ to be an $\sh{O}_{Y,y}$-algebra \emph{essentially \'etale} \sref{IV.18.6.1}, it is necessary and sufficient that $f$ satisfy the two following conditions:
\end{enumerate}
\end{remark}

\oldpage[IV-4]{159}
\begin{enumerate}[label=\arabic*\textsuperscript{o}]
  \item $f$ is formally unramified at the point $x$;
  \item the homomorphism $\sh{O}_{Y,y}\to\sh{O}_{X,x}$ is injective.
\end{enumerate}

There is nothing more than proving the sufficiency of these conditions.
Taking into account \sref{IV.18.4.12}, it suffices to show that $f$ is \emph{flat} at the point $x$.
Now, by going back to the proof of \sref{IV.18.10.1} and the notations used in this proof, it suffices to show that $f'$ is flat at the point $x'$ \sref{IV.2.5.1}; by \sref{IV.17.1.3} furthermore $f'$ is formally unramified at the point $x'$.
One can therefore restrict to carrying out the proof when $\mathrm{Y}=\operatorname{Spec}(\mathrm{A})$, where $\mathrm{A}$ is a \emph{strictly local} local ring and $y=f(x)$ the closed point of $\mathrm{Y}$.
Using then \sref{IV.18.8.3} we see that $\sh{O}_{Y,y}\to\sh{O}_{X,x}$ is surjective, hence \emph{bijective} by virtue of the hypothesis, and this suffices to show the flatness of $f$ at the point $x$.

Suppose furthermore that $\mathrm{Y}$ is \emph{locally integral} at the point $y$ \sref[I]{I.2.1.8}.
Then the conditions $1^{\circ}$ and $2^{\circ}$ above (together with the fact that $\mathrm{Y}$ is geometrically unibranch at the point $y$ and $f$ locally of finite type) already imply that $f$ is \emph{\'etale at the point} $x$.
Indeed, this follows from what precedes and from \sref{IV.18.4.13}.

\begin{corollary}[18.10.3]
\label{IV.18.10.3}
Let $\mathrm{Y}$ be an integral prescheme, integral and geometrically unibranch, $\eta$ its generic point, $\mathrm{X}$ a connected prescheme, $f:\mathrm{X}\to\mathrm{Y}$ a locally of finite type morphism, and such that $f^{-1}(\eta)$ is non-empty.
Then, if $f$ is formally unramified, $f$ is \'etale, and $\mathrm{X}$ is integral and geometrically unibranch.
\end{corollary}

\begin{proof}
It follows from the hypothesis and from \sref{IV.18.4.13} that $f$ is \'etale at all the points where it is flat, and in particular at the points of $f^{-1}(\eta)$, since $\sh{O}_{Y,\eta}=k(\eta)$ is a field.
The open set $\mathrm{U}$ of points of $\mathrm{X}$ where $f$ is \'etale is therefore \emph{non-empty}.
Let us show on the other hand that for all $x\in\mathrm{U}$, the homomorphism $\sh{O}_{Y,f(x)}\to\sh{O}_{X,x}$ is \emph{injective}.
Indeed let $s$ be a section of $\sh{O}_Y$ above an open neighbourhood of $f(x)$, that one can always suppose to be $\mathrm{X}$ and $\mathrm{Y}$ themselves (the question being local on $\mathrm{X}$ and $\mathrm{Y}$), and suppose that its image $t\in\Gamma(\mathrm{Y},f_*(\sh{O}_X))=\Gamma(\mathrm{X},\sh{O}_X)$ has zero germ at the point $x$, hence vanishes in a neighbourhood $\mathrm{V}$ of $x$; then the restriction of $t$ to the open set $\mathrm{U}\cap\mathrm{V}\neq\varnothing$ is zero; but the restriction $f|(\mathrm{U}\cap\mathrm{V})$ is \'etale, hence $\mathrm{W}=f(\mathrm{U}\cap\mathrm{V})$ is open in $\mathrm{Y}$ and contains by construction the generic point $\eta$; the homomorphism $\sh{O}_{Y,\eta}\to\sh{O}_{X,z}$ being injective for all points $z\in f^{-1}(\eta)$, the hypothesis that $t|(\mathrm{U}\cap\mathrm{V})=0$ implies that $s_\eta=0$, and as $\mathrm{Y}$ is integral, this implies $s=0$, whence our assertion.
Applying now \sref{IV.18.10.2},~(iii)), we conclude that $f$ is \emph{flat} at the point $x$, in other words $x\in\mathrm{U}$, hence $\mathrm{U}$ is both open and closed in $\mathrm{X}$; as it is non-empty and $\mathrm{X}$ is connected, we have $\mathrm{U}=\mathrm{X}$.

Note now that since $f$ is \'etale, hence locally quasi-finite and $\mathrm{Y}$ is irreducible, the maximal points of $\mathrm{X}$ belong to $f^{-1}(\eta)$ \sref{IV.2.3.4} and for all $x\in\mathrm{X}$, there is a neighbourhood of $x$ that contains only a finite number of these maximal points; in other words, the set of irreducible components of $\mathrm{X}$ is locally finite.
But by \sref{IV.18.10.1} $\mathrm{X}$ is integral and geometrically unibranch at every point, hence every point of $\mathrm{X}$ belongs to only a single irreducible component, and since these latter form a locally finite set of open (and closed) subsets in $\mathrm{X}$; since $\mathrm{X}$ is connected, it is integral.
\end{proof}

\oldpage[IV-4]{160}
\begin{remark}[18.10.3.1]
\label{IV.18.10.3.1}
If $f:\mathrm{X}\to\mathrm{Y}$ is locally of finite presentation, dominant and quasi-compact, the fibre $f^{-1}(\eta)$ is non-empty by virtue of \sref[I]{I.1.1.5}.
On the other hand, if one does not suppose $f$ quasi-compact, $f$ \emph{can be non-ramified and dominant without $f$ being \'etale} (always assuming the hypotheses of \sref{IV.18.10.3} verified for $\mathrm{X}$ and $\mathrm{Y}$).
This is what the following example shows: one takes for $\mathrm{Y}$ the affine plane $\operatorname{Spec}(\mathbf{C}[\mathrm{T},\mathrm{U}])$; one considers on the other hand a family $(\mathrm{X}_i)_{i\in\mathbf{Z}}$ of preschemes isomorphic to the affine line $\operatorname{Spec}(\mathbf{C}[\mathrm{T}])$, and one forms the prescheme obtained by ``gluing together'' $\mathrm{X}_{2j}$ and $\mathrm{X}_{2j+1}$ at the point $-1$ and $\mathrm{X}_{2j+1}$ and $\mathrm{X}_{2j+2}$ at the point $+1$.
Let us define on the other hand $f:\mathrm{X}\to\mathrm{Y}$ as being equal on $\mathrm{X}_{2j}$ to the closed immersion whose image is the line of equation $y=2j$, transforming the point $-1$ to $(2j-1,2j)$ and the point $1$ to $(2j+1,2j)$; on $\mathrm{X}_{2j+1}$, $f_j$ is the closed immersion whose image is the line of equation $x=2j+1$, transforming the point $-1$ to $(2j+1,2j)$ and the point $1$ to $(2j+1,2j+2)$.
It is clear that $f$ is a local immersion (hence is not ramified) and is dominant but the fibre of the generic point of $\mathrm{Y}$ is empty and $f$ is not \'etale.
\end{remark}

\begin{corollary}[18.10.4]
\label{IV.18.10.4}
Let $g:\mathrm{X}\to\mathrm{S}$, $h:\mathrm{X}\to\mathrm{S}$ be two locally of finite presentation morphisms, $f:\mathrm{X}\to\mathrm{Y}$ an $\mathrm{S}$-morphism, $x$ a point of $\mathrm{X}$, $y=f(x)$, $s=g(y)=h(x)$.
Suppose that $h$ is flat at the point $x$ and $g^{-1}(s)$ normal at the point $y$.
Then, for $f$ to be \'etale at the point $x$, it is necessary and sufficient that $f$ be formally unramified at the point $x$ and that $\dim_xh^{-1}(s)=\dim_yg^{-1}(s)$.
\end{corollary}

\begin{proof}
The conditions are necessary by virtue of the fact that if $f$ is \'etale at the point $x$, it follows from this that $f_s$ is also flat and quasi-finite at the point $x$ \sref{IV.6.1.2}.
Conversely, if the conditions of the statement are satisfied, in order to see that $f$ is \'etale at the point $x$, it suffices, by virtue of \sref{IV.17.8.3}, to see that $f_s$ is so.
One can therefore restrict to the case where $\mathrm{S}$ is the spectrum of a field.
It then follows from the hypothesis $\dim_x\mathrm{X}=\dim_y\mathrm{Y}$ and the fact that $f$ is not ramified at the point $x$ that one has (by \sref{IV.5.2.3} and \sref{IV.17.4.1}) $\dim\sh{O}_{X,x}=\dim\sh{O}_{Y,y}$ integral, and we deduce from \sref[0]{0.7.4.4} and \sref[0]{0.16.3.10} that the homomorphism $\sh{O}_{Y,y}\to\sh{O}_{X,x}$ is injective; it then suffices to apply \sref{IV.18.10.1}.
\end{proof}

\begin{corollary}[18.10.5]
\label{IV.18.10.5}
With the notations of \sref{IV.18.10.4}, suppose that $h$ is flat and that $g^{-1}(s)$ is normal for all $s\in\mathrm{S}$ (which will be the case for example if $g$ is smooth).
For $f$ to be an open immersion, it is necessary and sufficient that $f$ be a monomorphism and that, for all $x\in\mathrm{X}$, $\dim_xh^{-1}(s)=\dim_yg^{-1}(s)$, where $y=f(x)$ and $s=g(y)=h(x)$.
\end{corollary}

\begin{proof}
One needs only to prove the sufficiency of the conditions; now, it follows from \sref{IV.17.1.3} that a monomorphism locally of finite presentation is formally unramified, hence the hypotheses imply that $f$ is \'etale \sref{IV.18.10.4}; on the other hand, by \sref{IV.17.9.1}, we conclude from these and from \sref{IV.18.10.4}.
\end{proof}

\begin{lemma}[18.10.6]
\label{IV.18.10.6}
Let $f:\mathrm{X}\to\mathrm{Y}$ be a flat and locally quasi-finite morphism.
\begin{enumerate}[label=(\roman*)]
  \item The set $\mathrm{Max}(\mathrm{X})$ of maximal points of $\mathrm{X}$ is in canonical bijection with the set $\coprod_{y\in\mathrm{Max}(\mathrm{Y})}f^{-1}(y)$.
  \item If $\mathrm{Y}$ is irreducible with generic point $\eta$, the set of irreducible components of $\mathrm{X}$ is in canonical bijection with $f^{-1}(\eta)$, and is in particular finite if and only if $f^{-1}(\eta)$ is finite.
\oldpage[IV-4]{161}
  \item If the set of irreducible components of $\mathrm{Y}$ is locally finite, so is the set of irreducible components of $\mathrm{X}$, and in particular $\mathrm{X}$ is locally connected.
\end{enumerate}
\end{lemma}

\begin{proof}
The assertion (i) follows immediately from \sref{IV.2.3.4} and \sref[0]{0.2.1.6} and from the fact that the fibres $f^{-1}(y)$ are discrete sets.
It is clear that (ii) is a particular case of (i).
Finally, to prove (iii), one can, by virtue of (i), restrict to the case where $f$ is quasi-finite and where $\mathrm{Y}$ is irreducible \sref[0]{0.2.1.6} and the conclusion follows from (ii) and from the fact that the fibres of $f$ are finite sets \sref[II]{II.6.2.2}.
\end{proof}

\begin{proposition}[18.10.7]
\label{IV.18.10.7}
Let $\mathrm{Y}$ be a geometrically unibranch and irreducible prescheme (resp.\ integral), $f:\mathrm{X}\to\mathrm{Y}$ an \'etale morphism.
Then $\mathrm{X}$ is isomorphic to a sum of preschemes irreducible (resp.\ integral) $\mathrm{X}_\lambda$, where $\lambda$ ranges over the fibre $f^{-1}(\eta)$ of the generic point $\eta$ of $\mathrm{Y}$.
The $\mathrm{X}_\lambda$ are geometrically unibranch; if $\mathrm{Y}$ is normal, the $\mathrm{X}_\lambda$ are normal.
\end{proposition}

\begin{proof}
Indeed, $\mathrm{X}$ is geometrically unibranch \sref{IV.17.5.7}, hence every point of $\mathrm{X}$ belongs to only a single irreducible component; moreover, by virtue of \sref{IV.18.10.6} and \sref{IV.17.6.1} the set of irreducible components of $\mathrm{X}$ is locally finite, hence \sref[0]{0.2.1.6} these are the connected components $\mathrm{X}_\lambda$ of $\mathrm{X}$ ($\lambda\in f^{-1}(\eta)$), and $\mathrm{X}$ is the sum of the $\mathrm{X}_\lambda$; by virtue of \sref{IV.17.5.7}, if $\mathrm{Y}$ is integral (resp.\ normal), the $\mathrm{X}_\lambda$ are integral (resp.\ normal).
\end{proof}

\begin{proposition}[18.10.8]
\label{IV.18.10.8}
Let $\mathrm{Y}$ be a normal and integral prescheme, of generic point $\eta$, $\mathrm{K}=k(\eta)$ its field of rational functions, $f:\mathrm{X}\to\mathrm{Y}$ an \'etale morphism.
Suppose that $f^{-1}(\eta)$ is finite and non-empty, so that the $\mathrm{K}$-scheme $f^{-1}(\eta)$ is the spectrum of a finite separable $\mathrm{K}$-algebra $\mathrm{L}$, composed as a direct product of a finite number of fields $\mathrm{K}_i$ ($1\leq i\leq n$), finite separable extensions of $\mathrm{K}$ \sref{IV.17.6.2}.
Let $\mathrm{Y}'$ be the integral closure of $\mathrm{Y}$ in $\mathrm{L}$, isomorphic to the sum of the integral closures $\mathrm{Y}_i$ of $\mathrm{Y}$ in $\mathrm{K}_i$ \sref[II]{II.6.3.6}.
Then the morphism $f$ factorises uniquely as $\mathrm{X}\xrightarrow{f'}\mathrm{Y}'\xrightarrow{g}\mathrm{Y}$, such that the restriction $f_\eta':f^{-1}(\eta)\to g^{-1}(\eta)$ is the canonical isomorphism.
Moreover $f'$ is a local isomorphism; for $f'$ to be an open immersion, it is necessary and sufficient that $f$ be separated.
\end{proposition}

\begin{proof}
By virtue of \sref{IV.18.10.7}, one can restrict to the case where $\mathrm{X}$ is integral and normal.
The existence and uniqueness of the factorisation considered for $f$ follow from \sref[II]{II.6.3.9}.
As $f'$ is birational and locally quasi-finite, the final assertions follow from \sref{IV.8.12.10}.
\end{proof}

\begin{corollary}[18.10.9]
\label{IV.18.10.9}
Under the hypotheses of \sref{IV.18.10.8}, for $f$ to be a finite morphism (in other words, for $\mathrm{X}$ to be an \'etale covering of $\mathrm{Y}$), it is necessary and sufficient that $\mathrm{X}$ be isomorphic to the integral closure $\mathrm{Y}'$ of $\mathrm{Y}$ in $\mathrm{L}$.
\end{corollary}

\begin{proof}
If $f$ is finite, the canonical injection $j:\mathrm{X}\to\mathrm{Y}'$ (which is an open immersion) is also a finite morphism \sref[II]{II.6.1.5},~(v)), hence a closed morphism \sref[II]{II.6.1.10}, and as $j(\mathrm{X})$ is dense in $\mathrm{Y}'$ by \sref{IV.18.10.8}, we have $j(\mathrm{X})=\mathrm{Y}'$.
Conversely, as $\mathrm{Y}$ is normal and $\mathrm{L}$ is a composed direct product of finite separable extensions of $\mathrm{K}$, $\mathrm{Y}'$ is finite over $\mathrm{Y}$ (Bourbaki, \emph{Alg.\ comm.}, chap.~V, \textsection~1, n\textsuperscript{o}~6, cor.~1 of prop.~18), whence the corollary.
\end{proof}

\begin{env}[18.10.10]
\label{IV.18.10.10}
Let $\mathrm{Y}$ be a normal and integral prescheme, $\mathrm{K}=\mathrm{R}(\mathrm{Y})$ its field of rational functions.
We say that a $\mathrm{K}$-algebra of finite rank $\mathrm{L}$ is \emph{non-ramified over $\mathrm{Y}$} if: $1^{\circ}$ $\mathrm{L}$ is a separable $\mathrm{K}$-algebra, hence composed as a direct product of finite separable extensions $\mathrm{K}_i$ of $\mathrm{K}$ ($1\leq i\leq n$); $2^{\circ}$ the integral closure $\mathrm{Y}'$ of $\mathrm{Y}$ in $\mathrm{L}$ (sum of the preschemes that are integral closures of $\mathrm{Y}$ in the $\mathrm{K}_i$) is \emph{non-ramified over} $\mathrm{Y}$ (which, by \sref{IV.18.10.3},
\oldpage[IV-4]{162}
is equivalent to saying that $\mathrm{Y}'$ is \'etale over $\mathrm{Y}$).
When $\mathrm{Y}=\operatorname{Spec}(\mathrm{A})$, where $\mathrm{A}$ is an integral ring, integrally closed, $\mathrm{K}$ its field of fractions, one says also (by a slight abuse of language, and when there is no confusion to be feared with the terminology of \sref{IV.17.3.2},~(ii)) that $\mathrm{L}$ is non-ramified over $\mathrm{A}$ instead of ``non-ramified over $\mathrm{Y}$''.
\end{env}

\begin{remark}[18.10.11]
\label{IV.18.10.11}
Instead of saying that $\mathrm{L}$ is non-ramified over $\mathrm{Y}$, certain authors say also that $\mathrm{L}$ is ``non-ramified over $\mathrm{K}$''; we shall not follow this usage, which lends itself to confusions.
\end{remark}

One can then express \sref{IV.18.10.9} in the following form:

\begin{corollary}[18.10.12]
\label{IV.18.10.12}
Let $\mathrm{Y}$ be a normal and integral prescheme, $\mathrm{K}=\mathrm{R}(\mathrm{Y})$ its field of rational functions.
Then the functor $\mathrm{X}\mapsto\mathrm{R}(\mathrm{X})$ is an equivalence of the category of \'etale coverings of $\mathrm{Y}$ and of the category of finite $\mathrm{K}$-algebras that are non-ramified over $\mathrm{Y}$.
One obtains a quasi-inverse functor by making correspond to every finite \'etale $\mathrm{K}$-algebra $\mathrm{L}$, non-ramified over $\mathrm{Y}$, the integral closure $\mathrm{Y}'$ of $\mathrm{Y}$ in $\mathrm{L}$.
\end{corollary}

\begin{proposition}[18.10.13]
\label{IV.18.10.13}
Let $\mathrm{Y}$ be a normal and integral prescheme, $\mathrm{K}=\mathrm{R}(\mathrm{Y})$ its field of rational functions.
\begin{enumerate}[label=(\roman*)]
  \item The field $\mathrm{K}$ is a $\mathrm{K}$-algebra non-ramified over $\mathrm{Y}$.
  \item Let $\mathrm{L}$ be a finite extension of $\mathrm{K}$, non-ramified over $\mathrm{Y}$, and let $\mathrm{Z}$ be the integral closure of $\mathrm{Y}$ in $\mathrm{L}$.
Then, if $\mathrm{M}$ is an $\mathrm{L}$-algebra non-ramified over $\mathrm{Z}$, $\mathrm{M}$ is also a $\mathrm{K}$-algebra non-ramified over $\mathrm{Y}$ (``transitivity'' of non-ramification).
  \item Let $\mathrm{Y}'$ be a normal and integral prescheme, $\mathrm{K}'$ its field of rational functions, $g:\mathrm{Y}'\to\mathrm{Y}$ a dominant morphism.
If $\mathrm{L}$ is a $\mathrm{K}$-algebra non-ramified over $\mathrm{Y}$, then $\mathrm{L}\otimes_{\mathrm{K}}\mathrm{K}'$ is a $\mathrm{K}'$-algebra non-ramified over $\mathrm{Y}'$ (``translation'' property).
Furthermore if $\mathrm{Y}=\operatorname{Spec}(\mathrm{A})$ and $\mathrm{Y}'=\operatorname{Spec}(\mathrm{A}')$ are affine, and if $\mathrm{C}$ is the integral closure of $\mathrm{A}$ in $\mathrm{L}$, then $\mathrm{C}'=\mathrm{C}\otimes_{\mathrm{A}}\mathrm{A}'$ is the integral closure of $\mathrm{A}'$ in $\mathrm{L}\otimes_{\mathrm{K}}\mathrm{K}'$.
\end{enumerate}
\end{proposition}

\begin{proof}
The assertion (i) is immediate, $\mathrm{Y}$ being its own normalisation in $\mathrm{K}$.
To prove (ii), let $\mathrm{Z}'$ be the integral closure of $\mathrm{Z}$ in $\mathrm{M}$, which is by hypothesis an \'etale covering of $\mathrm{Z}$.
As $\mathrm{Z}$ is \'etale and separated over $\mathrm{Y}$, so is $\mathrm{Z}'$ \sref{IV.17.3.3}, and it is clearly the integral closure of $\mathrm{Y}$ in $\mathrm{M}$.
As $\mathrm{M}$ is a $\mathrm{K}$-algebra, finite and separable, it follows from \sref{IV.18.10.10} that $\mathrm{M}$ is non-ramified over $\mathrm{Y}$.
Finally, to prove (iii), let us note that if $\mathrm{Z}$ is the integral closure of $\mathrm{Y}$ in $\mathrm{L}$, $\mathrm{Z}\times_{\mathrm{Y}}\mathrm{Y}'$ is \'etale and separated over $\mathrm{Y}'$ \sref{IV.17.3.3}, finite over $\mathrm{Y}'$ since $\mathrm{Z}$ is finite over $\mathrm{Y}$, and admits for ring of rational functions $\mathrm{L}\otimes_{\mathrm{K}}\mathrm{K}'$ \sref[I]{I.3.4.9}; as $\mathrm{Y}'$ is normal, it follows from \sref{IV.17.5.7}, which finishes the proof.
\end{proof}

\begin{corollary}[18.10.14]
\label{IV.18.10.14}
Let $\mathrm{Y}$ and $\mathrm{Y}'$ be two normal preschemes and integral, $\mathrm{K}$, $\mathrm{K}'$ their respective fields of rational functions, $g:\mathrm{Y}'\to\mathrm{Y}$ a dominant morphism, so that $\mathrm{K}'$ is an extension of $\mathrm{K}$.
Let $\mathrm{L}$ be a finite extension of $\mathrm{K}$ non-ramified over $\mathrm{Y}$, $\mathrm{L}_1$ a composite extension (Bourbaki, \emph{Alg.}, chap.~VIII, \textsection~8, def.~1) of $\mathrm{L}$ and $\mathrm{K}'$.
Then $\mathrm{L}_1$ is an extension of $\mathrm{K}'$ non-ramified over $\mathrm{Y}'$.
Furthermore if $\mathrm{Y}=\operatorname{Spec}(\mathrm{A})$, $\mathrm{Y}'=\operatorname{Spec}(\mathrm{A}')$ are affine, and if $\mathrm{C}$ is the integral closure of $\mathrm{A}$ in $\mathrm{L}$, then the subring $\mathrm{C}_1=\mathrm{A}[\mathrm{C},\mathrm{A}']$ of $\mathrm{L}_1$ is the integral closure of $\mathrm{A}'$ in $\mathrm{L}_1$.
\end{corollary}

\begin{proof}
\oldpage[IV-4]{163}
Indeed, $\mathrm{L}\otimes_{\mathrm{K}}\mathrm{K}'$ is composed as a direct product of finite separable extensions of $\mathrm{K}'$, and $\mathrm{L}_1$ is one of these extensions; hence the integral closure of $\mathrm{Y}$ in $\mathrm{L}$ is the sum of preschemes, one of which is the integral closure of $\mathrm{Y}$ in $\mathrm{L}_1$.
The corollary therefore follows from \sref{IV.18.10.14}.

When for example $\mathrm{A}$ is the ring of integers of an algebraic number field $\mathrm{K}$, $\mathrm{K}'$ and $\mathrm{L}$ algebraic extensions of $\mathrm{K}$, there are classical examples showing that the relation $\mathrm{C}_1=\mathrm{A}[\mathrm{C},\mathrm{A}']$ does not hold when $\mathrm{L}$ is not ramified over $\mathrm{A}[\quad]$.
\end{proof}

\begin{env}[18.10.15]
\label{IV.18.10.15}
Suppose that $\mathrm{Y}=\operatorname{Spec}(\mathrm{A})$ is affine, $\mathrm{A}$ being integral and integrally closed; let $\mathrm{K}$ be its field of fractions, $\mathrm{L}$ a finite separable extension of $\mathrm{K}$, and we suppose that the integral closure $\mathrm{C}$ of $\mathrm{A}$ in $\mathrm{L}$ is a \emph{projective} $\mathrm{A}$-module of finite type (which will be the case for example if $\mathrm{A}$ is a Dedekind ring (Bourbaki, \emph{Alg.\ comm.}, chap.~VII, \textsection~4, n\textsuperscript{o}~10, prop.~22)).
To say that $\mathrm{L}$ is non-ramified over $\mathrm{A}$ means that $\mathrm{C}$ is \'etale \sref{IV.18.10.10} and by virtue of \sref{IV.18.2.7},~(ii)), this is equivalent to saying that the \emph{discriminant} $d_{\mathrm{C}/\mathrm{A}}$ of $\operatorname{Spec}(\mathrm{C})$ over $\operatorname{Spec}(\mathrm{A})$ is \emph{invertible} in $\mathrm{A}$.
In the particular case where $\mathrm{C}$ is a \emph{free} $\mathrm{A}$-module and $(x_i)_{1\leq i\leq n}$ a basis of $\mathrm{C}$ over $\mathrm{A}$, this means that $\det(\operatorname{Tr}_{\mathrm{C}/\mathrm{A}}(x_ix_j))$ is invertible in $\mathrm{A}$.
\end{env}

\begin{theorem}[18.10.16]
\label{IV.18.10.16}
Let $\mathrm{Y}$ be an integral prescheme, $\eta$ its generic point, $f:\mathrm{X}\to\mathrm{Y}$ a separated and locally of finite type morphism.
Suppose that every irreducible component of $\mathrm{X}$ dominates $\mathrm{Y}$ and that the generic fibre $\mathrm{X}_\eta=f^{-1}(\eta)$ is a finite prescheme over $\mathrm{K}=k(\eta)$, hence $(\mathrm{X}_\eta)_{\mathrm{red}}$ is the spectrum of a finite separable $\mathrm{K}$-algebra $\mathrm{L}$, where $\mathrm{L}=\prod_i\mathrm{L}_i$ is a product of finite extensions $\mathrm{L}_i$ of $\mathrm{K}$.
Set
\[
  n=[\mathrm{L}:\mathrm{K}]=\sum_i[\mathrm{L}_i:\mathrm{K}],\quad n_s=\sum_i[\mathrm{L}_i:\mathrm{K}]_s\quad\text{(sum of separable degrees of the $\mathrm{L}_i$)}.
\]
Let $y$ be a geometrically unibranch point of $\mathrm{Y}$, and denote by $n(y)$ the sum of the separable degrees over $k(y)$ of the residue fields of the isolated points of $f^{-1}(y)$.
Then:
\begin{enumerate}[label=(\roman*)]
  \item One has $n(y)\leq n_s\leq n$.
  \item Suppose $\mathrm{X}$ reduced and the point $y$ normal in $\mathrm{Y}$.
For $n(y)=n$, it is necessary and sufficient that there exist a neighbourhood $\mathrm{U}$ of $y$ in $\mathrm{Y}$ such that the restriction $f^{-1}(\mathrm{U})\to\mathrm{U}$ of $f$ is an \'etale and finite morphism.
\end{enumerate}
\end{theorem}

\begin{proof}
\emph{A) Reduction to the case where $f$ is quasi-finite.} ---
One knows \sref{IV.13.1.4} that the set $\mathrm{Z}$ of isolated points $x\in\mathrm{X}$ of $f^{-1}(f(x))$ is open in $\mathrm{X}$ and by hypothesis $\mathrm{Z}$ contains $f^{-1}(\eta)$.
Since $f^{-1}(\eta)$ is finite, $\mathrm{Z}$ is an increasing filtered union of quasi-compact open sets $\mathrm{V}_\lambda$ containing $f^{-1}(\eta)$ (dense in $\mathrm{X}$ by hypothesis).
If $n_\lambda(y)$ is the sum of the separable degrees over $k(y)$ of the residue fields of the points of $\mathrm{V}_\lambda\cap f^{-1}(y)$, we have $n(y)=\sup n_\lambda(y)$; to prove (i), it will therefore suffice to show that $n_\lambda(y)\leq n_s$.
On the other hand, if $n_\lambda(y)=n$, suppose that we have shown that there exists a neighbourhood $\mathrm{U}$ of $y$ in $\mathrm{Y}$ such that the restriction $\mathrm{V}_\lambda\cap f^{-1}(\mathrm{U})\to\mathrm{U}$ of $f$ is an \'etale and finite morphism; since $f$ is separated, the canonical injection $\mathrm{V}_\lambda\cap f^{-1}(\mathrm{U})\to f^{-1}(\mathrm{U})$ is then also a finite morphism \sref[II]{II.6.1.5},~(v)), hence $\mathrm{V}_\lambda\cap f^{-1}(\mathrm{U})$ is closed in $f^{-1}(\mathrm{U})$; but being dense everywhere in $f^{-1}(\mathrm{U})$, it is necessarily equal to it.

\emph{B) Reduction to the case where $\mathrm{X}$ is reduced, $f$ finite, and $\mathrm{Y}=\operatorname{Spec}(\sh{O}_{Y,y})$.} ---
We note that the hypothesis on $f$ implies that it is also satisfied for $f_{\mathrm{red}}$ and that the conclusion of (i) does not concern $f_{\mathrm{red}}$.
One can therefore replace $f$ everywhere by $f_{\mathrm{red}}$ and thus suppose $\mathrm{X}$ reduced.

\oldpage[IV-4]{164}
On the other hand, by virtue of \sref{IV.18.12.13} (${}^1$), one can (after having replaced $\mathrm{Y}$ by an open affine neighbourhood $\mathrm{V}$ of $y$ and $f$ by its restriction $f^{-1}(\mathrm{V})\to\mathrm{V}$) factorise $f$ as $\mathrm{X}\xrightarrow{j}\mathrm{X}'\xrightarrow{g}\mathrm{Y}$, where $g$ is \emph{finite} and $j$ an open immersion.
Since $\mathrm{X}$ is reduced, one can replace $\mathrm{X}'$ by the reduced closed sub-prescheme with underlying space $\overline{j(\mathrm{X})}$ \sref[I]{I.5.2.2}, hence suppose $\mathrm{X}'$ reduced and such that every irreducible component of $\mathrm{X}'$ dominates $\mathrm{Y}$.
Moreover, $j(\mathrm{X})$ is an open dense set in $\mathrm{X}'$ and has only a finite number of irreducible components by virtue of the hypothesis; hence \sref[0]{0.1.2.7} the irreducible components of $\mathrm{X}'$ are the closures of those of $j(\mathrm{X})$, and by construction the fibre $g^{-1}(\eta)$ identifies with $f^{-1}(\eta)$.
If one supposes the theorem proved for $f$ finite, one can apply it to $g$, and as $f^{-1}(y)$ identifies with a sub-prescheme of $g^{-1}(y)$, the relation (i) for $g$ implies the same relation for $f$.
Suppose on the other hand that $n(y)=n$; then, by virtue of what precedes, one can apply the conclusion of (ii) to the morphism $g$, and replacing $\mathrm{Y}$ as needed by a neighbourhood of $y$, one can suppose $g$ \'etale; it follows in addition, on the one hand that $f^{-1}(y)=g^{-1}(y)$.
As the morphism $g$ is closed and $j(\mathrm{X})$ is open in $\mathrm{X}'$, there exists a neighbourhood $\mathrm{U}$ of $y$ in $\mathrm{Y}$ such that $j(\mathrm{X})\cap g^{-1}(\mathrm{U})=j(f^{-1}(\mathrm{U}))$, which proves that (ii) is also true for $f$.

We are thus reduced to the case where $f$ is moreover a \emph{finite} morphism.
It is immediate that, to prove (i), one can restrict to the case where $\mathrm{Y}=\operatorname{Spec}(\sh{O}_{Y,y})$.
Let us show that the same is true for proving (ii).
Indeed, suppose we have shown that the condition $n(y)=n$ implies that $f$ is flat and formally unramified at the points of $f^{-1}(y)$, and that $\mathrm{Y}$ is \emph{integral}, we conclude from \sref{IV.18.4.13} that $f$ is \emph{\'etale at the points} of $f^{-1}(y)$, hence also in a neighbourhood $\mathrm{V}$ of $f^{-1}(y)$ in $\mathrm{X}$; but since $f$ is a finite morphism, hence closed, $\mathrm{V}$ contains an open set of the form $f^{-1}(\mathrm{U})$, where $\mathrm{U}$ is a neighbourhood of $y$ in $\mathrm{Y}$.

\emph{C) Reduction to the case where $\mathrm{Y}=\operatorname{Spec}(\sh{O}_{Y,y})$ and $\sh{O}_{Y,y}$ is strictly local.} ---
Set $\mathrm{A}=\sh{O}_{Y,y}$, and let $\mathrm{A}'={}^{\mathrm{hs}}\mathrm{A}$ be the strict henselisation of $\mathrm{A}$ \sref{IV.18.8.7}, which is a geometrically unibranch local integral ring \sref{IV.18.8.15}.
Let $\mathrm{Y}'=\operatorname{Spec}(\mathrm{A}')$, and let $y'$ and $\eta'$ be the closed point and the generic point of $\mathrm{Y}'$; $y'$ is above $y$ and since the morphism $g:\mathrm{Y}'\to\mathrm{Y}$ is flat \sref{IV.18.8.8}, $\eta'$ is above $\eta$ \sref{IV.2.3.4}.
Set $\mathrm{X}'=\mathrm{X}\times_{\mathrm{Y}}\mathrm{Y}'$, $f'=f_{(\mathrm{Y}')}:\mathrm{X}'\to\mathrm{Y}'$; $f'$ is finite and since $g$ is flat, every irreducible component of $\mathrm{X}'$ dominates $\mathrm{Y}'$ \sref{IV.2.3.7}.
If $n'(y')$, $n'$ and $n_s'$ are the numbers defined for $f'$ in the same way as $n(y)$, $n$ and $n_s$ for $f$, we have $n_s'=n_s$ and $n'(y')=n(y)$ \sref[I]{I.6.4.8}, and it therefore amounts to proving the assertion (i) for $f$ or for $f'$.
Moreover, if $\mathrm{X}$ is reduced and $y$ a normal point of $\mathrm{Y}$ (hence $\mathrm{A}$ integrally closed \sref{IV.18.8.12}).
On the other hand, it follows from the definition \sref{IV.18.8.7}, from the remark \sref{IV.8.1.2},~$a$)) and from the theorem of the double inductive limit, that $\mathrm{A}'$ is the inductive limit of rings $\mathrm{A}_\alpha$ such that the morphisms $\operatorname{Spec}(\mathrm{A}_\alpha)\to\operatorname{Spec}(\mathrm{A})=\mathrm{Y}$ are \'etale; consequently $\mathrm{X}'$ is the projective limit of the $\mathrm{X}_\alpha=\mathrm{X}\otimes_{\mathrm{A}}\mathrm{A}_\alpha$, which are \'etale over $\mathrm{X}$, hence reduced since $\mathrm{X}$ is \sref{IV.17.5.7}; by passage to the limit,

\footnotetext{(${}^1$) The reader will verify that \sref{IV.18.10.16} is not used in the proof of \sref{IV.18.12.13}. If $f$ is supposed locally of finite presentation (resp.\ quasi-projective), one can replace \sref{IV.18.12.13} by \sref{IV.8.12.8} (resp.\ \sref{IV.8.12.6}).}

\oldpage[IV-4]{165}
we deduce that $\mathrm{X}'$ is reduced \sref{IV.8.7.1}.
This being so, if $n(y)=n$, on has $n_s=n$ and consequently the residue fields at the points of $\mathrm{X}_\eta$ are necessarily separable over $k(\eta)$; the residue fields at the points of $\mathrm{X}_{\eta'}'$ are then algebraic and separable over $k(\eta')$ \sref{IV.4.6.1}, in other words $n_s'=n'=n$.
The morphism $f'$ has therefore the same properties as $f$, and if one proves that $f'$ is \'etale, it will follow by \sref{IV.17.7.1}.

\emph{D) End of the proof.} ---
Suppose therefore $\mathrm{A}$ strictly local.
Since $\mathrm{A}$ is henselian and the morphism $f$ finite, it follows from \sref{IV.18.5.11} that if $x_j$ ($1\leq j\leq m$) are the distinct points of $f^{-1}(y)$, $\mathrm{X}$ is the \emph{sum} of open sub-preschemes $\operatorname{Spec}(\sh{O}_{X,x_j})=\mathrm{X}_j$, which are the connected components of $\mathrm{X}$.
By hypothesis, each of them dominates $\mathrm{Y}$, hence every irreducible component of $\mathrm{X}$ meets $f^{-1}(\eta)$, and so the number of points of $f^{-1}(\eta)$ is $\geq m$; \emph{a fortiori} we have $n_s\geq m$.
Suppose now the hypotheses of (ii) satisfied; the relations $n(y)=n_s=n$ imply first of all, since every irreducible component of $\mathrm{X}$ dominates $\mathrm{Y}$, that each of the $\mathrm{X}_j$ is \emph{irreducible}; moreover, as the $\mathrm{X}_j$ are \emph{reduced}, they are \emph{integral}; finally the relations $m=n_s=n$ imply that for the generic point $\xi_j$ of $\mathrm{X}_j$, $k(\xi_j)$ is a separable extension of $k(\eta)$ of degree $1$, hence isomorphic to $k(\eta)$; in other words, the restriction $f|\mathrm{X}_j:\mathrm{X}_j\to\mathrm{Y}$ of $f$ is \emph{finite and birational}; moreover, $\mathrm{X}_j$ is \emph{integral} and by hypothesis $\mathrm{Y}$ is \emph{normal}; hence \sref{IV.8.12.10.1} $f|\mathrm{X}_j$ is an \emph{isomorphism}, which proves that $f$ is \'etale, and finishes the proof of \sref{IV.18.10.16}.
\end{proof}

\begin{remark}[18.10.17]
\label{IV.18.10.17}
The method of ``\'etale localisation'' used in the proof of \sref{IV.18.10.16} also allows one to improve the results of \sref{IV.15.5.1} by eliminating the Noetherian hypothesis.
One has first of all the following result which generalises \sref{IV.15.5.2}:
\end{remark}

\begin{env}[18.10.17.1]
\label{IV.18.10.17.1}
Let $f:\mathrm{X}\to\mathrm{Y}$ be a separated and quasi-finite morphism, $y$ a point of $\mathrm{Y}$ such that $f$ is universally open at the points of $f^{-1}(y)$.
Then, for every generisation $y'$ of $y$, we have $n(y)\leq n(y')$.
\end{env}

\begin{proof}
One can replace $\mathrm{Y}$ by the reduced closed sub-prescheme $\overline{\{y'\}}$ and $\mathrm{X}$ by $f^{-1}(\mathrm{Z})$, hence suppose $\mathrm{Y}$ integral with generic point $y'$.
Using \sref{IV.18.12.13} (as in \sref{IV.18.10.16},~B)), one can suppose that $\mathrm{X}$ is an open dense subset of a prescheme $\mathrm{X}'$ and that $f$ is the restriction to $\mathrm{X}$ of a finite morphism $g:\mathrm{X}'\to\mathrm{Y}$.
Let us show that $g^{-1}(y')=f^{-1}(y')$.
Indeed, let $i:g^{-1}(y')\to\mathrm{X}'$ be the canonical injection, which is a flat morphism since $y'$ is the generic point of $\mathrm{Y}$ \sref[I]{I.3.6.5}; one can write $f^{-1}(y')=i^{-1}(\mathrm{X})$.
On the other hand, the canonical injection $j:\mathrm{X}\to\mathrm{X}'$ is a morphism of finite type since the composite $g\circ j=f$ is of finite type and $g$ is separated \sref{IV.1.5.4}; hence $\mathrm{X}$ is a retrocompact open subset of $\mathrm{X}'$, and consequently pro-constructible in $\mathrm{X}'$ \sref[I]{I.1.9.5},~(v)).
One concludes therefore from \sref{IV.2.3.10} that $i^{-1}(\overline{\mathrm{X}})=\overline{i^{-1}(\mathrm{X})}$, in other words $f^{-1}(y')$ is dense in $g^{-1}(y')$; but as $g^{-1}(y')$ is discrete, we necessarily have $f^{-1}(y')=g^{-1}(y')$.

We are thus reduced to proving that the sum of the numbers $[k(x):k(y)]_s$ where $x$ ranges over the points of $g^{-1}(y)$ where $g$ is universally open, is at most equal to
\oldpage[IV-4]{166}
$n(y')$ (for the morphism $g$).
Using then the fact that the property of being universally open at a point is preserved by base change, one shows, as in \sref{IV.18.10.16},~C)), that one can reduce to the case where $\mathrm{Y}=\operatorname{Spec}(\sh{O}_{Y,y})$, with $\sh{O}_{Y,y}$ \emph{strictly local}.
Applying \sref{IV.18.5.11},~$c$)), we see that if $x_j$ ($1\leq j\leq n$) are the points of $f^{-1}(y)$, $\mathrm{X}$ is the \emph{sum} of $n$ sub-preschemes $\mathrm{X}_j=\operatorname{Spec}(\sh{O}_{X,x_j})$; the hypothesis that $g$ is open in each of the points $x_j$ implies that the sub-prescheme $\mathrm{X}_j$ corresponding dominates $\mathrm{Y}$ \sref{IV.1.10.3}; one concludes as in \sref{IV.18.10.16},~D)).

We then deduce from \sref{IV.18.10.17.1} that \emph{the conclusions of \sref{IV.15.5.1} are still valid when one no longer supposes $\mathrm{Y}$ locally Noetherian, but only that $f$ is separated, quasi-finite, and of finite presentation}.
\end{proof}

Indeed, to prove assertion (i) of \sref{IV.15.5.1}, one remarks that, since $f$ is of finite presentation, the set $\mathrm{E}$ of points $z\in\mathrm{Y}$ such that $n(z)\geq n(y)$ is locally constructible \sref{IV.9.7.9}; to show that $y$ is interior to $\mathrm{E}$, it suffices, by virtue of \sref{IV.1.10.1}, to see that every generisation $y'$ of $y$ belongs to $\mathrm{E}$, which is nothing other than \sref{IV.18.10.17.1}.

To prove assertion (ii) of \sref{IV.15.5.1}, one remarks that, since $f$ is of finite presentation, using the method of \sref{IV.8.10.5},~(xii)) applied following \sref{IV.8.1.2},~$a$)), and one can therefore suppose that $\mathrm{Y}=\operatorname{Spec}(\sh{O}_{Y,y})$, and the ring $\sh{O}_{Y,y}$ is \emph{strictly local}.
Applying \sref{IV.18.5.11},~$c$)), we see then that if $x_j$ ($1\leq j\leq n$) are the points of $f^{-1}(y)$, $\mathrm{X}$ is the \emph{sum} of $n$ open sub-preschemes $\mathrm{X}_j=\operatorname{Spec}(\sh{O}_{X,x_j})$ which are \emph{finite} over $\mathrm{Y}$, and of an open prescheme $\mathrm{X}''$.
If one proves that $\mathrm{X}''=\varnothing$, we will have shown that $f$ is a finite morphism, and the conclusion follows from the fact that, since $\mathrm{X}$ is open in each of the points $x_j$, the restriction of $f$ to $\mathrm{X}_j$, being a finite morphism \sref[II]{II.6.1.10}, is \emph{surjective}; the hypothesis that $z\mapsto n(z)$ is constant implies \sref{IV.17.4.1}, hence one terminates as in \sref{IV.18.10.16},~D)).

Finally, the proof of (iii) reproduces without change that given in \sref{IV.15.5.1}.

\begin{env}[18.10.17.2]
\label{IV.18.10.17.2}
By analogous methods using strict henselisation, one can, in most of the results of \textsection\textsection~14 and 15, eliminate the Noetherian hypotheses.
\end{env}

\begin{lemma}[18.10.18]
\label{IV.18.10.18}
Let $\mathrm{X}$, $\mathrm{Y}$ be two preschemes, $f:\mathrm{X}\to\mathrm{Y}$ a birational morphism (${}^1$), $x$ a point of $\mathrm{X}$.
For $f$ to be an isomorphism local at the point $x$, it is necessary and sufficient that $f$ be \'etale at the point $x$.
For $f$ to be an open immersion, it is necessary and sufficient that $f$ be \'etale and separated.
\end{lemma}

\footnotetext{(${}^1$) We understand by this the notion defined in \sref{IV.6.15.4}, but where we do not suppose $\mathrm{X}$ and $\mathrm{Y}$ reduced.}

\begin{proof}
The conditions stated being trivially necessary, all that is needed is to see that they are sufficient.
For the first assertion, the question is local on $\mathrm{X}$ and $\mathrm{Y}$, hence one can suppose $\mathrm{X}$ and $\mathrm{Y}$ affine, hence $f$ \emph{separated}, and we are thus reduced to proving the second assertion.
By virtue of \sref{IV.17.9.1}, it is a question of proving that $f$ is \emph{radicial}.
Let $y\in\mathrm{Y}$, and let us prove that $f^{-1}(y)$ is radicial on $k(y)$.
The base change $\operatorname{Spec}(\sh{O}_{Y,y})\to\mathrm{Y}$ not changing the fact that $f$ is \'etale, separated, and birational, one can restrict to the case where $\mathrm{Y}=\operatorname{Spec}(\mathrm{A})$, with $\mathrm{A}=\sh{O}_{Y,y}$.
\oldpage[IV-4]{167}
Let $\mathrm{A}'={}^{\mathrm{hs}}\mathrm{A}$, which is a local ring and an $\sh{O}_{Y,y}$-module faithful flat.
Set $\mathrm{Y}'=\operatorname{Spec}(\mathrm{A}')$, $f'=f_{(\mathrm{Y}')}:\mathrm{X}'\to\mathrm{Y}'$, so that $f'$ is \'etale and separated; moreover, as the morphism $\mathrm{Y}'\to\mathrm{Y}$ is flat, the reasoning of \sref{IV.6.15.4.1} shows that $f'$ is also birational.

Suppose therefore $\mathrm{Y}=\operatorname{Spec}(\mathrm{A})$, $\mathrm{A}$ strictly local, $f$ separated, \'etale and birational, and let us show that $f^{-1}(y)$ can contain only a single point.
Indeed, if $f^{-1}(y)$ contained two distinct points $x_1$, $x_2$, there would exist two $\mathrm{Y}$-sections $u_1$, $u_2$ of $f$ such that $u_1(y)=x_1$ and $u_2(y)=x_2$ \sref{IV.18.8.1}; but since $f$ is \'etale and separated, $u_1$ and $u_2$ would be isomorphisms of $\mathrm{Y}$ onto two connected components (open) of $\mathrm{X}$ \sref{IV.17.9.4} and there would then be two maximal points of $\mathrm{X}$ at least above every maximal point of $\mathrm{Y}$, which contradicts the hypothesis that $f$ is \emph{birational}.
\end{proof}

\begin{proposition}[18.10.19]
\label{IV.18.10.19}
Let $\mathrm{Y}$ be a reduced prescheme whose set of irreducible components is locally finite, $f:\mathrm{X}\to\mathrm{Y}$ a separated morphism of finite type and formally unramified, $g$ a $\mathrm{Y}$-rational section of $f$ (i.e.\ a $\mathrm{Y}$-rational map from $\mathrm{Y}$ to $\mathrm{X}$), $\mathrm{U}$ the domain of definition of $g$ \sref[I]{I.7.2.1}, and let $\mathrm{Z}$ be the closure of $g(\mathrm{U})$ in $\mathrm{X}$.
Then, for all points $y\in\mathrm{Y}-\mathrm{U}$ such that $y$ is geometrically unibranch, $\mathrm{Z}_y=\mathrm{Z}\cap f^{-1}(y)=\varnothing$.
In particular, if $\mathrm{Y}$ is geometrically unibranch, $g(\mathrm{U})$ is a subset of $\mathrm{X}$ that is both open and closed.
If, for every closed irreducible $\mathrm{T}$ of $\mathrm{X}$, $f(\mathrm{T})$ is closed in $\mathrm{Y}$, $g$ is defined at every geometrically unibranch point of $\mathrm{Y}$.
\end{proposition}

\begin{proof}
Since $\mathrm{Y}$ is reduced and $f$ separated, it follows from \sref[I]{I.7.2.2} that there exists a $\mathrm{U}$-section $u$ of $f^{-1}(\mathrm{U})$ belonging to the class $g$; by \sref{IV.17.4.1.2}, moreover, $f$ is formally unramified, hence $u$ is an isomorphism of $\mathrm{U}$ onto the prescheme induced on the open set $u(\mathrm{U})$ of $\mathrm{X}$.
If one denotes also by $\mathrm{Z}$ the reduced sub-prescheme of $\mathrm{X}$ having $\mathrm{Z}$ for underlying space, $g(\mathrm{U})=u(\mathrm{U})$, being reduced, is induced also by $\mathrm{Z}$, and by construction $f'=f|\mathrm{Z}$ is a \emph{birational} morphism of $\mathrm{Z}$ to $\mathrm{Y}$, moreover formally unramified since $f$ is \sref{IV.17.1.3}.
The question being local over $\mathrm{Y}$ at the point $y$, one can restrict (replacing $\mathrm{Y}$ by a neighbourhood of $y$) to the case where $\mathrm{Y}$ is \emph{integral}; then $u(\mathrm{U})$ is irreducible, hence also $\mathrm{Z}$, and by construction $\mathrm{Z}$ is also integral.
Then \sref[I]{I.8.2.7} since $f'$ is dominant, the homomorphism $\sh{O}_{Y,y}\to\sh{O}_{Z,x}$ is injective for all $x\in\mathrm{Z}_y$, and one concludes from \sref{IV.18.10.2},~(iii)) that $f'$ is an \'etale isomorphism local at the point $x$; but this would imply that the $\mathrm{U}$-section $u$ would be extendable to an open set distinct from $\mathrm{U}$, contrary to the definition of the latter.
We therefore necessarily have $\mathrm{Z}_y=\varnothing$, which establishes the first assertion; the second is an obvious consequence.
Finally, under the last hypotheses of the statement, as $\mathrm{Z}$ is closed and irreducible, $f(\mathrm{Z})$ is closed in $\mathrm{Y}$, hence equal to $\mathrm{Y}$, i.e.\ $\mathrm{Z}_y\neq\varnothing$ for all $y\in\mathrm{Y}$, which finishes the proof.
\end{proof}

\begin{remark}[18.10.20]
\label{IV.18.10.20}
Let $\mathrm{Y}$ be a locally Noetherian prescheme.
One says that a morphism $f:\mathrm{X}\to\mathrm{Y}$ is \emph{essentially proper} if it is locally of finite type and if, for every base change $\mathrm{Y}''\to\mathrm{Y}$, where $\mathrm{Y}''$ is \emph{locally Noetherian}, the image by $f_{(\mathrm{Y}'')}:\mathrm{X}_{(\mathrm{Y}'')}\to\mathrm{Y}''$ of every closed irreducible subset of $\mathrm{X}''$ is \emph{closed}.
\oldpage[IV-4]{168}
To say that $f$ is essentially proper for every $\mathrm{Y}$-scheme $\mathrm{Y}'=\operatorname{Spec}(\mathrm{A})$ where $\mathrm{A}$ is a discrete valuation ring, the canonical map \sref[II]{II.7.3.2.2} relative to the $\mathrm{Y}'$-prescheme $\mathrm{X}\times_{\mathrm{Y}}\mathrm{Y}'$, is bijective.
The reasoning of \sref[II]{II.7.3.8} proves that this is the same as saying that $f$ (supposed locally of finite type) is \emph{separated} and that for every base change $\mathrm{Y}''\to\mathrm{Y}$, where $\mathrm{Y}''$ is \emph{locally Noetherian}, the image by $f_{(\mathrm{Y}'')}: \mathrm{X}_{(\mathrm{Y}'')}\to\mathrm{Y}''$ of every closed irreducible subset of $\mathrm{X}''$ is \emph{closed}.
To say that $f$ is \emph{proper} (for $\mathrm{Y}$ locally Noetherian) means therefore that $f$ is \emph{essentially proper and of finite type} \sref[II]{II.7.3.8}; but one encounters important examples of essentially proper morphisms that are not of finite type (for example certain ``Picard preschemes'' or certain ``preschemes of N\'eron-Severi'' (chap.~VI)).
It evidently follows from \sref{IV.18.10.19} that if $\mathrm{Y}$ is \emph{locally Noetherian, reduced and geometrically unibranch}, and if $f:\mathrm{X}\to\mathrm{Y}$ is \emph{essentially proper and not ramified}, then every $\mathrm{Y}$-rational section of $f$ is \emph{defined everywhere}.
\end{remark}

\begin{proposition}[18.10.21]
\label{IV.18.10.21}
Let $f:\mathrm{X}\to\mathrm{Y}$ be a locally of finite presentation morphism, $x$ a point of $\mathrm{X}$, $y=f(x)$, and suppose the ring $\sh{O}_{Y,y}$ integral and geometrically unibranch.
Let $r$ be the minimum number of generators of the $\sh{O}_{X,x}$-module $(\Omega^1_{\mathrm{X}/\mathrm{Y}})_x$ (equal to the rank of the $k(x)$-vector space $(\Omega^1_{\mathrm{X}/\mathrm{Y}})_x\otimes_{\sh{O}_{X,x}}k(x)$).
Then, for $f$ to be smooth at the point $x$, it is necessary and sufficient that, if $y'$ is the generic point of the unique irreducible component of $\mathrm{Y}$ containing $y$, there exists a generisation $x'$ of $x$ such that $f(x')=y'$ and $\dim_{x'}f^{-1}(y')\geq r$.
\end{proposition}

\begin{proof}
If $f$ is smooth at the point $x$, it is also so in an open neighbourhood $\mathrm{U}$ of $x$, hence in every generisation $x'$ of $x$, and $\dim_{x'}(f^{-1}(f(x')))=r$ in each of these points, by virtue of \sref{IV.17.10.2}.
As $f$ is moreover flat in $\mathrm{U}$, there exists a generisation of $x$ above every generisation of $y$ \sref{IV.2.3.4}, which proves that the condition is necessary.
To show that it is sufficient, let us first note that by \sref{IV.17.5.1}, it suffices to prove that the morphism deduced from $f$ by the base change $\operatorname{Spec}(\sh{O}_{Y,y})\to\mathrm{Y}$ is smooth at the point $x$; one can therefore suppose that $\mathrm{Y}=\operatorname{Spec}(\sh{O}_{Y,y})$.

The question is local on $\mathrm{X}$, hence \sref{IV.16.4.22} and \sref[0]{0.5.2.2}) one can suppose that $\mathrm{X}$ is affine, and that there exist $r$ sections $s_i$ ($1\leq i\leq r$) of $\sh{O}_X$ above $\mathrm{X}$ such that the sections $d_{\mathrm{X}/\mathrm{Y}}(s_i)$ generate $\Omega^1_{\mathrm{X}/\mathrm{Y}}$.
Let
\[
  g:\mathrm{X}\to\mathrm{Z}=\mathrm{Y}[\mathrm{T}_1,\ldots,\mathrm{T}_r]=\mathbf{V}_{\mathrm{Y}}
\]
be the $\mathrm{Y}$-morphism corresponding to the homomorphism $\sh{O}_{\mathrm{Y}}^r\to f_*(\sh{O}_X)$ of $\sh{O}_Y$-Modules defined by the $s_i$ (considered as sections of $f_*(\sh{O}_X)$ above $\mathrm{Y}$) \sref[II]{II.1.2.7}.
We have the exact sequence
\[
  g^*(\Omega^1_{\mathrm{Z}/\mathrm{Y}})\to\Omega^1_{\mathrm{X}/\mathrm{Y}}\to\Omega^1_{\mathrm{X}/\mathrm{Z}}\to 0.
\]
As the $d\mathrm{T}_i$ ($1\leq i\leq r$) generate $\Omega^1_{\mathrm{Z}/\mathrm{Y}}$, it follows from the definition of $g$ that the homomorphism $g^*(\Omega^1_{\mathrm{Z}/\mathrm{Y}})\to\Omega^1_{\mathrm{X}/\mathrm{Y}}$ is surjective, hence $\Omega^1_{\mathrm{X}/\mathrm{Z}}=0$, and $g$ is consequently \emph{formally unramified} \sref{IV.17.4.1}.
We shall now see that $g$ is \emph{\'etale} at the point $x$, and taking into account \sref{IV.17.3.8}, this will prove that $f$ is smooth at the point $x$.
As $\sh{O}_{Y,y}$ is supposed integral and geometrically
\oldpage[IV-4]{169}
unibranch, so is $\sh{O}_{Z,z}$ by \sref{IV.17.5.7} where $z=g(x)$, since the structural morphism $\mathrm{Z}\to\mathrm{Y}$ is smooth \sref{IV.17.3.8}.
By virtue of \sref{IV.18.10.1}, it suffices to show that the homomorphism $\sh{O}_{Z,z}\to\sh{O}_{X,x}$ is injective; it amounts to the same thing to see that the morphism $\operatorname{Spec}(\sh{O}_{X,x})\to\operatorname{Spec}(\sh{O}_{Z,z})$ corresponding is \emph{dominant}, since $\sh{O}_{Z,z}$ is integral \sref[I]{I.1.2.7}.
Let $y'$ be the generic point of $\mathrm{Y}=\operatorname{Spec}(\sh{O}_{Y,y})$ and let $z'$ be the generic point of $\mathrm{Z}=\operatorname{Spec}(\sh{O}_{Y,y}[\mathrm{T}_1,\ldots,\mathrm{T}_r])$ which is integral; if $h:\mathrm{Z}\to\mathrm{Y}$ is the structural morphism, on has $h(z')=y'$.
It suffices to prove that there exists in $f^{-1}(y')$ a generisation $x'$ of $x$ such that the image of $x'$ by the morphism $g_{y'}:f^{-1}(y')\to h^{-1}(y')$ equals $z'$.
Moreover $z'$ is also the generic point of $h^{-1}(y')=\operatorname{Spec}(k(y')[\mathrm{T}_1,\ldots,\mathrm{T}_r])$; as the morphism $g_{y'}$ is not ramified, hence quasi-finite \sref{IV.17.4.2}, it follows from \sref{IV.5.4.1} that the restriction of $g_{y'}$ to every irreducible component of \emph{dimension} $\geq r$ of $f^{-1}(y')$ is a dominant morphism.
Now, there exists by hypothesis a component containing a generisation $x'$ of $x$; \emph{a fortiori} its generic point is a generisation of $x$, whence the conclusion.
\end{proof}

\subsection{Application to Noetherian local algebras complete over a field}
\label{subsection:IV.18.11}

The following lemma generalises \sref[0]{0.21.9.1} and \sref[0]{0.21.9.2}:

\begin{lemma}[18.11.1]
\label{IV.18.11.1}
Let $k$ be a field, $\mathrm{A}$ a Noetherian complete local ring which is a $k$-algebra, $\mathrm{K}$ the residue field of $\mathrm{A}$.
\begin{enumerate}[label=(\roman*)]
  \item If the $\mathrm{K}$-vector space $\Omega^1_{\mathrm{K}/k}$ is of finite rank, the $\mathrm{A}$-module $\hat{\Omega}^1_{\mathrm{A}/k}$ is of finite type.
  \item If moreover $\mathrm{A}$ is a $k$-algebra formally smooth (for the adic topology), $\hat{\Omega}^1_{\mathrm{A}/k}$ is a free $\mathrm{A}$-module of rank equal to
\[
  \dim(\mathrm{A})+\operatorname{rg}_{\mathrm{K}}(\Omega^1_{\mathrm{K}/k})-\operatorname{rg}_{\mathrm{K}}(\Upsilon_{\mathrm{K}/k}).
\]
\end{enumerate}
Furthermore, for every subfield $k_0$ of $k$ such that $\Omega^1_{\mathrm{K}/k_0}$ is a $k$-vector space of finite rank, $\hat{\Omega}^1_{\mathrm{A}/k_0}$ is a free $\mathrm{A}$-module of rank equal to
\[
  \dim(\mathrm{A})+\operatorname{rg}_{\mathrm{K}}(\Omega^1_{\mathrm{K}/k_0})-\operatorname{rg}_{\mathrm{K}}(\Upsilon_{\mathrm{K}/k_0})+\operatorname{rg}_{\mathrm{K}}(\Omega^1_{k/k_0}).
\]
\end{lemma}

\begin{proof}
The assertion (i) is noted as a reminder, having been proved in \sref[0]{0.20.7.15}.
To prove (ii), note that this assertion has been in fact established by the demonstration of \sref[0]{0.21.9.2}, making only intervene the finiteness of the transcendence degree of $\mathrm{K}$ over $k$ (by way of the Cartier equality \sref[0]{0.21.7.1}).
\end{proof}

\begin{lemma}[18.11.2]
\label{IV.18.11.2}
Let $k$ be a field of characteristic $p>0$, $\mathrm{C}$ a $k$-algebra, local Noetherian, complete, whose residue field is a finite extension of $k$, and which is integral; let $n$ be its dimension, $\mathrm{L}$ its field of fractions.
Then $\Omega^1_{\mathrm{L}/k}$ and $\Upsilon_{\mathrm{L}/k}$ are $\mathrm{L}$-vector spaces of finite rank, and one has
\begin{equation}
\label{IV.18.11.2.1}
\tag{18.11.2.1}
  \operatorname{rg}_{\mathrm{L}}(\Omega^1_{\mathrm{L}/k})-\operatorname{rg}_{\mathrm{L}}(\Upsilon_{\mathrm{L}/k})\geq n.
\end{equation}
\oldpage[IV-4]{170}
Suppose furthermore that $[k:k^p]<+\infty$.
Then one has the equality
\begin{equation}
\label{IV.18.11.2.2}
\tag{18.11.2.2}
  \operatorname{rg}_{\mathrm{L}}(\Omega^1_{\mathrm{L}/k})-\operatorname{rg}_{\mathrm{L}}(\Upsilon_{\mathrm{L}/k})=n.
\end{equation}
Furthermore, $\Omega^1_{\mathrm{C}/k}$ is then isomorphic to $\hat{\Omega}^1_{\mathrm{C}/k}$, hence \sref{IV.18.11.1} is a $\mathrm{C}$-module of finite type.
\end{lemma}

\begin{proof}
We know in fact \sref[0]{0.19.8.9} that there exists a sub-$k$-algebra $\mathrm{C}_0$ of $\mathrm{C}$ isomorphic to a formal power series algebra in $n$ variables $k[[\mathrm{T}_1,\ldots,\mathrm{T}_n]]$ and such that $\mathrm{C}$ is a $\mathrm{C}_0$-algebra finite.
Hence, $\mathrm{L}$ is a \emph{finite} extension of the field of fractions $\mathrm{L}_0=k((\mathrm{T}_1,\ldots,\mathrm{T}_n))$ of $\mathrm{C}_0$.
Now, one has the exact sequence of $\mathrm{L}$-vector spaces obtained by applying \sref[0]{0.20.6.17.1} to the three fields $k\subset\mathrm{L}_0\subset\mathrm{L}$ (and taking into account \sref[0]{0.20.4.13}):
\[
  0\to\Upsilon_{\mathrm{L}/k}\otimes_{\mathrm{L}_0}\mathrm{L}\to\Upsilon_{\mathrm{L}/k}\to\Upsilon_{\mathrm{L}/\mathrm{L}_0}\to\Omega^1_{\mathrm{L}/k}\otimes_{\mathrm{L}_0}\mathrm{L}\to\Omega^1_{\mathrm{L}/k}\to\Omega^1_{\mathrm{L}/\mathrm{L}_0}\to 0.
\]
Since $\mathrm{L}$ is a finite extension of $\mathrm{L}_0$, $\Omega^1_{\mathrm{L}/\mathrm{L}_0}$ and $\Upsilon_{\mathrm{L}/\mathrm{L}_0}$ are $\mathrm{L}$-vector spaces of finite rank having the same rank, by the Cartier equality \sref[0]{0.21.7.1}; from the preceding exact sequence we therefore already deduce that
\[
  \operatorname{rg}_{\mathrm{L}}(\Omega^1_{\mathrm{L}/k})-\operatorname{rg}_{\mathrm{L}}(\Upsilon_{\mathrm{L}/k})=\operatorname{rg}_{\mathrm{L}_0}(\Omega^1_{\mathrm{L}_0/k})-\operatorname{rg}_{\mathrm{L}_0}(\Upsilon_{\mathrm{L}_0/k}).
\]

To prove \sref{IV.18.11.2.1} or \sref{IV.18.11.2.2}, one can therefore restrict to proving these relations by replacing $\mathrm{C}$ and $\mathrm{L}$ by $\mathrm{C}_0$ and $\mathrm{L}_0$.
As $\mathrm{L}_0$ is separable over $k$ \sref[0]{0.21.7.1}, we have $\Upsilon_{\mathrm{L}_0/k}=0$ \sref[0]{0.20.6.3}; one deduces hence already that in all cases $\operatorname{rg}_{\mathrm{L}_0}(\Omega^1_{\mathrm{L}_0/k})\geq n$, with equality when $[k:k^p]<+\infty$; this already proves \sref{IV.18.11.2.1} and \sref{IV.18.11.2.2}.

Finally, to see that $\Omega^1_{\mathrm{C}/k}$ is isomorphic to $\hat{\Omega}^1_{\mathrm{C}/k}$ when $[k:k^p]<+\infty$, it suffices to prove that $\Omega^1_{\mathrm{C}/k}$ is a $\mathrm{C}$-module of finite type, since $\mathrm{C}$ is a Noetherian complete local ring \sref[0]{0.7.3.5}, taking into account \sref[0]{0.20.4.5}.
Since $\Omega^1_{\mathrm{C}/k}$ is isomorphic to $\hat{\Omega}^1_{\mathrm{C}/k}$ and that $k[\mathrm{C}_0^p]\subset k[\mathrm{C}^p]$, the result follows that $\Omega^1_{\mathrm{C}/k}$ is a $\mathrm{C}$-module of finite type and $\mathrm{C}_0$ a $k[\mathrm{C}_0^p]$-module of finite type in the hypothesis $[k:k^p]<+\infty$.
\end{proof}

\begin{lemma}[18.11.3]
\label{IV.18.11.3}
Let $\mathrm{A}$ be a Noetherian local ring, $\mathfrak{p}$ a prime ideal of $\mathrm{A}$ such that $\mathrm{A}_\mathfrak{p}$ is a Cohen-Macaulay ring.
For every system of parameters $(t_i)_{1\leq i\leq s}$ of $\mathrm{A}_\mathfrak{p}$, there exists a sequence $(x_i)_{1\leq i\leq s}$ of elements of $\mathfrak{p}$ such that the $x_i$ form part of a system of parameters of $\mathrm{A}$ \oldpage[IV-4]{172}and that, for all $i$, $x_i/1$ is congruent to $t_i$ modulo the ideal $t_1\mathrm{A}_\mathfrak{p}+\ldots+t_{i-1}\mathrm{A}_\mathfrak{p}$.
In particular, if $\mathrm{A}_\mathfrak{p}$ is regular and if the $t_i$ form a regular system of parameters of $\mathrm{A}_\mathfrak{p}$, so it is also the case of the $x_i/1$.
\end{lemma}

\begin{proof}
This lemma itself will be a consequence of the following:
\end{proof}

\begin{lemma}[18.11.3.4]
\label{IV.18.11.3.4}
Let $\mathrm{A}$ be a Noetherian local ring, $\mathfrak{p}$ a prime ideal of $\mathrm{A}$, $x$ an element of $\mathrm{A}$; then there exists an element $x'\in\mathrm{A}$ such that $x'/1=x/1$ in $\mathrm{A}_\mathfrak{p}$ and that $x'$ does not belong to any of the minimal prime ideals of $\mathrm{A}$ not contained in $\mathfrak{p}$.
\end{lemma}

\begin{proof}
Let us first show how \sref{IV.18.11.3.4} implies \sref{IV.18.11.3}.
Multiplying the $t_i$ if necessary by invertible elements of $\mathrm{A}_\mathfrak{p}$, one can suppose that they are of the form $x_i/1$, with $x_i\in\mathfrak{p}$ for $1\leq i\leq s$.
We reason by induction on $s$.
As $t_1$ forms part of a system of parameters of $\mathrm{A}_\mathfrak{p}$ \sref[0]{0.16.5.5}, there is no $x_1'\in\mathrm{A}$ such that $x_1'/1=t_1$ that can belong to a minimal prime ideal of $\mathrm{A}_\mathfrak{p}$, hence it cannot belong to any of the minimal prime ideals of $\mathrm{A}$ contained in $\mathfrak{p}$.
By virtue of \sref{IV.18.11.3.4}, one can moreover choose $x_1'$ such that $x_1'/1=t_1$ (hence $x_1'\in\mathfrak{p}$) and that $x_1'$ does not belong to \emph{any} minimal prime ideal of $\mathrm{A}$, hence $x_1'$ forms part of a system of parameters of $\mathrm{A}$ \sref[0]{0.16.3.4} and \sref{IV.16.3.7}.
One reasons then by induction, considering the ring $\mathrm{A}'=\mathrm{A}/x_1'\mathrm{A}$ and the prime ideal $\mathfrak{p}'=\mathfrak{p}/x_1'\mathrm{A}$; as $\mathrm{A}_{\mathfrak{p}'}'\cong\mathrm{A}_\mathfrak{p}/t_1\mathrm{A}_\mathfrak{p}$, $\mathrm{A}_{\mathfrak{p}'}'$ is also a Cohen-Macaulay ring \sref[0]{0.16.5.5}, and the images $t_i'$ ($2\leq i\leq s$) of the $t_i$ in $\mathrm{A}_{\mathfrak{p}'}'$ form a system of parameters of this ring; it suffices to apply $\mathrm{A}'$ and to the $t_i'$ ($2\leq i\leq s$) the induction hypothesis.
The last assertion of \sref{IV.18.11.3} follows from the fact that in $\mathrm{A}_\mathfrak{p}$, a system of parameters that generates the maximal ideal is a regular system of parameters \sref[0]{0.17.1.1}.
\end{proof}

\begin{proof}[Proof of \sref{IV.18.11.3.4}]
Let $(p_k')_{1\leq k\leq r}$ be the suite of minimal prime ideals of $\mathrm{A}$ not contained in $\mathfrak{p}$, and let $(\mathfrak{p}_j)_{1\leq j\leq n}$ be the suite of other minimal prime ideals of $\mathrm{A}$; one can suppose that $r\geq 1$.
As $\mathfrak{p}_k'$ does not contain $\bigcap_{1\leq i\leq n}\mathfrak{p}_j$, there exists an element $y\in\bigcap_{1\leq i\leq n}\mathfrak{p}_j$ that does not belong to any of the $\mathfrak{p}_k'$ (Bourbaki, \emph{Alg.\ comm.}, chap.~II, \textsection~1, n\textsuperscript{o}~1, prop.~2); moreover $y/1$ belongs to all the minimal prime ideals of $\mathrm{A}_\mathfrak{p}$, hence is nilpotent; if $(y/1)^h=0$, the element $x'=x+y^h$ will satisfy the requirement, since $x'/1=x/1$ by definition of the $\mathfrak{p}_k'$, on has $x'\notin\mathfrak{p}_k'$ for $1\leq k\leq r$, and on the other hand, if $\mathfrak{p}_j$ is a prime ideal of $\mathrm{A}$ not contained in $\mathfrak{p}$ but such that $x\notin\mathfrak{p}_j$, we also have $x'\notin\mathfrak{p}_j$ since $y\in\mathfrak{p}_j$.
\end{proof}

\begin{proof}[Proof of {\sref{IV.18.11.3}}, continued]
We return to the proof of \sref{IV.18.11.3} when $p=1$.
By virtue of \sref{IV.18.11.3.4}, there exists a regular system of parameters $(t_i)_{1\leq i\leq s}$ of $\mathrm{A}_\mathfrak{p}$ where the $x_i$ ($1\leq i\leq s$) belong to $\mathfrak{p}$ and form part of a system of parameters $(x_j)_{1\leq j\leq n}$ of $\mathrm{A}$.
Set $\mathrm{A}_0=k[[\mathrm{T}_1,\ldots,\mathrm{T}_n]]$; as $\mathrm{A}$ is a $k$-algebra complete and the $x_j$ belong to the maximal ideal $\mathfrak{m}$ of $\mathrm{A}$, there exists a local $k$-homomorphism $u:\mathrm{A}_0\to\mathrm{A}$ such that $u(\mathrm{T}_j)=x_j$ for $1\leq j\leq n$ (Bourbaki, \emph{Alg.\ comm.}, chap.~III, \textsection~4, n\textsuperscript{o}~5, prop.~6); if $n$ is the ideal of $\mathrm{A}$ generated by the $x_j$ ($1\leq j\leq n$), it is by hypothesis an ideal of definition of $\mathrm{A}$; one deduces therefore from \sref[0]{0.7.4.4} and \sref{IV.7.4.3} that $u$ makes $\mathrm{A}$ an $\mathrm{A}_0$-algebra \emph{finite}.

Set $\mathfrak{p}_0=\sum_{j=1}^{s}\mathrm{A}_0\mathrm{T}_j$, $\mathrm{B}_0=(\mathrm{A}_0)_{\mathfrak{p}_0}$ and $\mathrm{B}=\mathrm{A}\otimes_{\mathrm{A}_0}\mathrm{B}_0$, so that $u_{\mathfrak{p}_0}:\mathrm{B}_0\to\mathrm{B}$ makes
\oldpage[IV-4]{173}
$\mathrm{B}$ a $\mathrm{B}_0$-algebra finite; moreover, if $\mathfrak{p}'$ is the ideal of $\mathrm{B}$ generated by $\mathfrak{p}$, on has $\mathrm{B}_{\mathfrak{p}'}=\mathrm{A}_\mathfrak{p}$; as $\mathfrak{p}'$ contains $\mathfrak{p}_0\mathrm{B}_0$ by construction, it is above the maximal ideal $\mathfrak{p}_0\mathrm{B}_0$ of $\mathrm{B}_0$.
Let us show that the morphism $\operatorname{Spec}(\mathrm{B})\to\operatorname{Spec}(\mathrm{B}_0)$ is \emph{not ramified} at the point $\mathfrak{p}'$: indeed this follows from the fact that $k(\mathfrak{p}')$ is a finite extension of the field $k(\mathfrak{p}_0)$ of characteristic $0$, hence is \emph{necessarily separable}, and that $\mathrm{B}_{\mathfrak{p}'}/\mathfrak{p}_0\mathrm{B}_{\mathfrak{p}'}=k(\mathfrak{p}')$ by virtue of the choice of the $x_i$ for $1\leq j\leq s$ \sref{IV.17.4.1}.

Let us now note the following lemma:

\begin{lemma}[18.11.3.5]
\label{IV.18.11.3.5}
Let $k$ be a field, $\mathrm{R}$, $\mathrm{S}$ two Noetherian complete local $k$-algebras, such that, if $\mathrm{K}$ is the residue field of $\mathrm{S}$, $\Omega^1_{\mathrm{K}/k}$ is of finite rank over $\mathrm{K}$; let $u:\mathrm{R}\to\mathrm{S}$ be a $k$-homomorphism making $\mathrm{S}$ an $\mathrm{R}$-algebra finite; then $\hat{\Omega}^1_{\mathrm{S}/\mathrm{R}}=\Omega^1_{\mathrm{S}/\mathrm{R}}$, and the sequence
\begin{equation}
\label{IV.18.11.3.6}
\tag{18.11.3.6}
  \hat{\Omega}^1_{\mathrm{R}/k}\otimes_{\mathrm{R}}\mathrm{S}\xrightarrow{v}\hat{\Omega}^1_{\mathrm{S}/k}\xrightarrow{w}\Omega^1_{\mathrm{S}/\mathrm{R}}\to 0
\end{equation}
(cf.\ \sref[0]{0.20.7.17.3}) is exact.
\end{lemma}

\begin{proof}
The first assertion follows from the fact that $\Omega^1_{\mathrm{S}/\mathrm{R}}$ is an $\mathrm{S}$-module of finite type \sref[0]{0.20.4.7} and that $w$ is surjective; but it follows from \sref{IV.18.11.1} that $\hat{\Omega}^1_{\mathrm{S}/k}$ is an $\mathrm{S}$-module of finite type; on concludes that every sub-$\mathrm{S}$-module of $\hat{\Omega}^1_{\mathrm{S}/k}$ is closed \sref[0]{0.7.3.5}, whence the lemma.
\end{proof}

Applying this lemma to the case where $\mathrm{R}=\mathrm{A}_0$, $\mathrm{S}=\mathrm{A}$, and noting that $\hat{\Omega}^1_{\mathrm{A}_0/k}$ is an $\mathrm{A}_0$-module free of rank $n$ \sref[0]{0.21.9.3}; hence on a $\hat{\Omega}^1_{\mathrm{A}_0/k}\otimes_{\mathrm{A}_0}\mathrm{A}=\hat{\Omega}^1_{\mathrm{A}_0/k}\otimes_{\mathrm{A}_0}\mathrm{A}$ and this is an $\mathrm{A}$-module free of rank $n$.
Since $\operatorname{Spec}(\mathrm{B})$ is not ramified on $\operatorname{Spec}(\mathrm{B}_0)$ at the point $\mathfrak{p}'$, on a $(\Omega^1_{\mathrm{A}/\mathrm{A}_0})_\mathfrak{p}=\Omega^1_{\mathrm{B}_{\mathfrak{p}'}/\mathrm{B}_0}=0$ \sref{IV.16.4.15} and \sref{IV.17.4.1}.
If one localises the exact sequence \sref{IV.18.11.3.6} (applied to $\mathrm{A}_0$ and $\mathrm{A}$) at $\mathfrak{p}$, one obtains therefore a surjective homomorphism $(\hat{\Omega}^1_{\mathrm{A}_0/k}\otimes_{\mathrm{A}_0}\mathrm{A})_\mathfrak{p}\to(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$, and the conclusion follows that $\hat{\Omega}^1_{\mathrm{A}/k}$ is a $\mathrm{B}$-module of rank $n$, and the ring being complete and integral by the Cohen theorem \sref[0]{0.19.8.8},~(ii)) and the fact that the field of fractions of $\mathrm{A}$ is of characteristic $0$.
Hence the $\mathrm{A}_\mathfrak{p}$-module $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ admits a system of $n$ generators.
But the $\mathrm{A}_\mathfrak{p}$-module $\hat{\Omega}^1_{\mathrm{A}/k}$ is of \emph{rank} $n$, by virtue of \sref[0]{0.21.9.5}, which is applicable to the ring being complete and integral by the Cohen theorem \sref[0]{0.19.8.8},~(ii)) and the fact that the field of fractions of $\mathrm{A}$ is of characteristic $0$.
Hence the $\mathrm{A}_\mathfrak{p}$-module $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ admits also a system of $n$ generators, and since it is of rank $n$, this quotient is necessarily \emph{free}, whence the conclusion.
\end{proof}

\begin{lemma}[18.11.4]
\label{IV.18.11.4}
Let $k$ be a field, $\mathrm{A}$ a $k$-algebra, local Noetherian and complete, whose residue field is a finite extension of $k$.
Let $k'$ be an arbitrary extension of $k$, and set $\mathrm{A}'=\mathrm{A}\widehat{\otimes}_kk'$.
Then:
\begin{enumerate}[label=(\roman*)]
  \item $\mathrm{A}'$ is a semi-local Noetherian ring, complete, composed as a direct product of complete local rings $\mathrm{A}_i'$ ($1\leq i\leq r$) which are $\mathrm{A}$-modules faithfully flat, and whose residue fields are finite extensions of $k'$; if $\mathfrak{m}$ is the maximal ideal of $\mathrm{A}$, $\mathfrak{m}\mathrm{A}'$ is an ideal of definition of $\mathrm{A}'$; and $\dim(\mathrm{A}_i')=\dim(\mathrm{A})$ for all $i$.
  \item For all $i$, $\hat{\Omega}^1_{\mathrm{A}/k}$ is canonically isomorphic to $\hat{\Omega}^1_{\mathrm{A}/k}\otimes_{\mathrm{A}}\mathrm{A}_i'$.
\end{enumerate}
\end{lemma}

\begin{proof}
\oldpage[IV-4]{174}
The first assertions follow immediately from \sref{IV.7.5.5} and \sref[0]{0.6.6.2}; the fact that $\mathfrak{m}\mathrm{A}'$ is an ideal of definition of $\mathrm{A}'$ also follows from \sref{IV.7.5.5}, since $\mathrm{K}$ is the residue field of $\mathrm{A}$, $\mathrm{K}\otimes_k k'$ is a finite $k'$-algebra.
Finally $\mathrm{A}_i'/\mathfrak{m}\mathrm{A}_i'$ is one of the Artinian local rings whose direct product is $\mathrm{A}'\otimes_k k'$; hence as $\mathrm{A}_i'$ is an $\mathrm{A}$-module flat, the equality of dimensions of $\mathrm{A}$ and $\mathrm{A}_i'$ follows from \sref{IV.6.1.2}.

(ii) As $\hat{\Omega}^1_{\mathrm{A}/k}$ is, by the hypothesis on $\mathrm{K}$, an $\mathrm{A}$-module of finite type \sref{IV.18.11.1}, $\hat{\Omega}^1_{\mathrm{A}/k}\otimes_{\mathrm{A}}\mathrm{A}'$ is complete and identifies with the completed tensor product $\hat{\Omega}^1_{\mathrm{A}/k}\widehat{\otimes}_{\mathrm{A}}\mathrm{A}'$; by virtue of the associativity of the completed tensor product \sref[0]{0.7.7.4}, $\hat{\Omega}^1_{\mathrm{A}/k}\otimes_{\mathrm{A}}\mathrm{A}'=\hat{\Omega}^1_{\mathrm{A}/k}\otimes_{\mathrm{A}}(\mathrm{A}\widehat{\otimes}_kk')$ identifies with $\hat{\Omega}^1_{\mathrm{A}/k}\widehat{\otimes}_kk'$.
But $\Omega^1_{\mathrm{A}/k}\otimes_kk'$ identifies with $\Omega^1_{\mathrm{A}''/k'}$, where $\mathrm{A}''=\mathrm{A}\otimes_kk'$ \sref[0]{0.20.5.5}, and as $\mathrm{A}'$ is by definition the separated completion of $\mathrm{A}''$, the separated completion $\hat{\Omega}^1_{\mathrm{A}''/k'}$ identifies by construction to $\hat{\Omega}^1_{\mathrm{A}'/k'}$ \sref[0]{0.20.7.4}; furthermore, if $\mathfrak{r}$ is the radical of $\mathrm{A}'$, the $\mathfrak{r}$-preadique topology on $\Omega^1_{\mathrm{A}'/k'}$ identifies with the product of the $\mathfrak{m}_i'$-preadique topologies on the $\Omega^1_{\mathrm{A}_i'/k'}$ (where $\mathfrak{m}_i'$ is the maximal ideal of $\mathrm{A}_i'$); finally, it follows that the separated complete $\hat{\Omega}^1_{\mathrm{A}'/k'}$ for the $\mathfrak{r}$-preadique topology identifies with the product of the separated completions $\hat{\Omega}^1_{\mathrm{A}_i'/k'}$ for the $\mathfrak{m}_i'$-preadique topologies, and it suffices to use \sref[0]{0.20.4.5}.

The conclusion of (ii) follows now from the fact that $\Omega^1_{\mathrm{A}'/k'}$ is a direct sum of the $\Omega^1_{\mathrm{A}_i'/k'}$ \sref[0]{0.20.4.13}, and if $\mathfrak{r}$ is the radical of $\mathrm{A}'$, the $\mathfrak{r}$-preadique topology on $\Omega^1_{\mathrm{A}'/k'}$ identifies with the product of the $\mathfrak{m}_i'$-preadique topologies on the $\Omega^1_{\mathrm{A}_i'/k'}$ (where $\mathfrak{m}_i'$ is the maximal ideal of $\mathrm{A}_i'$); the separated complete $\hat{\Omega}^1_{\mathrm{A}'/k'}$ for the $\mathfrak{r}$-preadique topology identifies therefore with the product of the separated completions $\hat{\Omega}^1_{\mathrm{A}_i'/k'}$ for the $\mathfrak{m}_i'$-preadique topologies, hence one has a canonical isomorphism
\[
  \hat{\Omega}^1_{\mathrm{A}'/k'}\xrightarrow{\sim}\hat{\Omega}^1_{\mathrm{A}/k}\otimes_{\mathrm{A}}\mathrm{A}'.
\]
\end{proof}

\begin{proposition}[18.11.5]
\label{IV.18.11.5}
Let $k$ be a field, $\mathrm{A}$ a $k$-algebra, local Noetherian and complete, whose residue field is a finite extension of $k$, $\mathfrak{p}$ a prime ideal of $\mathrm{A}$ distinct from the maximal ideal $\mathfrak{m}$, such that there exists a unique minimal prime ideal $\mathfrak{q}\subset\mathfrak{p}$ for which $\dim(\mathrm{A}/\mathfrak{q})=\dim(\mathrm{A})$ (which will in particular hold when $\mathrm{A}$ is equidimensional), $m$ an integer $\geq 0$.
The following conditions are equivalent:
\begin{enumerate}[label=\alph*)]
  \item The $\mathrm{A}_\mathfrak{p}$-module $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ admits a system of $m$ generators.
  \item There exists a local $k$-homomorphism $u:\mathrm{B}\to\mathrm{A}$, where $\mathrm{B}=k[[\mathrm{T}_1,\ldots,\mathrm{T}_m]]$, making $\mathrm{A}$ a $\mathrm{B}$-algebra finite, and such that the corresponding morphism $\operatorname{Spec}(\mathrm{A})\to\operatorname{Spec}(\mathrm{B})$ is \'etale at the point $\mathfrak{p}$.
\end{enumerate}
\end{proposition}

\begin{proof}
To prove that $b)$ implies $a)$, let us note that by virtue of the lemma \sref{IV.18.11.3.5}, on has the exact sequence
\[
  \hat{\Omega}^1_{\mathrm{B}/k}\otimes_\mathrm{B}\mathrm{A}\to\hat{\Omega}^1_{\mathrm{A}/k}\to\Omega^1_{\mathrm{A}/\mathrm{B}}\to 0
\]
since $\mathrm{A}$ is a $\mathrm{B}$-algebra finite; localising at $\mathfrak{p}$ and noting that by hypothesis $(\Omega^1_{\mathrm{A}/\mathrm{B}})_\mathfrak{p}=0$ \sref{IV.17.4.1}, one obtains a surjective homomorphism $(\hat{\Omega}^1_{\mathrm{B}/k}\otimes_\mathrm{B}\mathrm{A})_\mathfrak{p}\to(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$, and
\oldpage[IV-4]{175}
the conclusion follows that $\hat{\Omega}^1_{\mathrm{B}/k}$ is a $\mathrm{B}$-module of rank $m$ \sref[0]{0.21.9.3}.

To prove that $a)$ implies $b)$, let us first prove the following lemmas.
\end{proof}

\begin{lemma}[18.11.5.1]
\label{IV.18.11.5.1}
Let $k$ be a field, $\mathrm{A}$ a $k$-algebra, local Noetherian and complete, whose residue field is a finite extension of $k$, $\mathfrak{p}$ a prime ideal of $\mathrm{A}$, $\mathfrak{q}$ a minimal prime ideal of $\mathrm{A}$ contained in $\mathfrak{p}$.
Then the minimum number of generators of the $\mathrm{A}_\mathfrak{p}$-module $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ is at least equal to $\dim(\mathrm{A}/\mathfrak{q})$.
\end{lemma}

\begin{proof}
Set $n=\dim(\mathrm{A}/\mathfrak{q})$, and let $m$ be the minimum number of generators of the $\mathrm{A}_\mathfrak{p}$-module $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$, which is equal to $\operatorname{rg}_{k(\mathfrak{p})}((\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}\otimes_{\mathrm{A}_\mathfrak{p}}k(\mathfrak{p}))$ (Bourbaki, \emph{Alg.\ comm.}, chap.~II, \textsection~3, n\textsuperscript{o}~2, cor.~2 of prop.~4).
Note that $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{q}=(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}\otimes_{\mathrm{A}_\mathfrak{p}}\mathrm{A}_\mathfrak{q}$, hence the minimum number of generators of the $\mathrm{A}_\mathfrak{q}$-module $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{q}$ is \emph{at most} equal to $m$.
It suffices therefore to consider the case where $\mathfrak{p}=\mathfrak{q}$ is minimal.
In second place, let us show one can restrict to the case where $k$ is \emph{algebraically closed}.
Indeed, let $k'$ be an algebraic closure of $k$, and set $\mathrm{A}'=\mathrm{A}\widehat{\otimes}_kk'$, which is composed as a direct product of $k'$-local algebras $\mathrm{A}_i'$ ($1\leq i\leq r$), whose residue fields are isomorphic to $k'$, and which are $\mathrm{A}$-modules faithfully flat \sref{IV.18.11.4}.
Applying \sref{IV.2.3.4} and \sref{IV.6.1.1}, one sees that in a given $\mathrm{A}_i'$, there exists a minimal prime ideal $\mathfrak{q}_i'$ above $\mathfrak{q}$ such that $n'=\dim(\mathrm{A}_i'/\mathfrak{q}_i')\geq\dim(\mathrm{A}/\mathfrak{q})=n$.
On the other hand by \sref{IV.18.11.4}, the minimum $m'$ of generators of the $(\mathrm{A}_i')_{\mathfrak{q}_i'}$-module $(\hat{\Omega}^1_{\mathrm{A}_i'/k'})_{\mathfrak{q}_i'}$ is \emph{at most} equal to $m$; whence our assertion.

Suppose therefore $k$ algebraically closed and $\mathfrak{q}=0$.
In the case where $\mathfrak{q}=0$, one can moreover restrict to the case where $\mathrm{A}$ is integral and $\mathrm{K}$ separable over the algebraically closed field $k$, hence $\mathrm{A}$ is a geometrically regular algebra \sref{IV.6.7.6}; one can therefore apply \sref{IV.18.11.3} for $\mathfrak{p}=0$ since $k=k^p$ and in this case $m=n$.
\end{proof}

\begin{lemma}[18.11.5.2]
\label{IV.18.11.5.2}
Let $k$ be a field, $\mathrm{A}$ a $k$-algebra, local Noetherian and complete, whose residue field is a finite extension of $k$, $\mathfrak{m}$ its maximal ideal, $\mathfrak{p}\subset\mathfrak{m}$ a prime ideal of $\mathrm{A}$.
Let $m$ be the minimum number of generators of the $\mathrm{A}_\mathfrak{p}$-module $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$, and let $(x_i)_{1\leq i\leq m}$ be a family of elements of $\mathfrak{m}$ not belonging to $\mathfrak{p}$.
Then there exist $m$ invertible elements $u_i$ ($1\leq i\leq m$) of $\mathrm{A}$ such that if $y_i=u_ix_i$, the canonical images of the $dy_i$ in $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ generate this $\mathrm{A}_\mathfrak{p}$-module.
\end{lemma}

\begin{proof}
For all $x\in\mathrm{A}$, denote by $\delta(x)$ the canonical image of $dx$ $(=d_{\mathrm{A}/k}x)$ in $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}\otimes_{\mathrm{A}_\mathfrak{p}}k(\mathfrak{p})=\hat{\Omega}^1_{\mathrm{A}/k}\otimes_{\mathrm{A}}k(\mathfrak{p})$; as this is a $k(\mathfrak{p})$-vector space of dimension $m$ by hypothesis, it suffices (by virtue of the Nakayama lemma) to prove that one can determine the $u_i$ such that the $\delta(u_ix_i)$ form a free system.
We reason by induction and suppose for an integer $r<m$, we have determined the $u_i$ ($1\leq i\leq r$) such that the $\delta(u_ix_i)$ for $1\leq i\leq r$ are linearly independent over $k(\mathfrak{p})$; if $\delta(x_{r+1})$ is not a linear combination of the $\delta(u_ix_i)$ for $1\leq i\leq r$, one takes $u_{r+1}=1$ to continue the induction.
In the contrary case, note that for $u\in\mathrm{A}$, $\delta(ux_{r+1})$ is the canonical image of $(du)x_{r+1}+u(dx_{r+1})$; as $u(dx_{r+1})$ has a canonical image which is a linear combination
\oldpage[IV-4]{176}
of the $\delta(u_ix_i)$ for $1\leq i\leq r$, there exists therefore $z\in\mathrm{A}$ such that $\delta(z)$ is not a linear combination of the $\delta(u_ix_i)$ for $1\leq i\leq r$; if $z\notin\mathfrak{m}$, one takes $u=z$; otherwise, $1+z$ is invertible and $d(1+z)=d(z)$ since $(du)x_{r+1}=\delta(z)$; one takes then $u=1+z$, which finishes the proof of \sref{IV.18.11.5.2}.
\end{proof}

\begin{proof}[Proof of {\sref{IV.18.11.5}}, continued]
We now return to the proof of the implication $a)\Rightarrow b)$ in \sref{IV.18.11.5}.
Let us first note that since $\mathfrak{p}\neq\mathfrak{m}$, there exists a system of parameters $(x_i)_{1\leq i\leq n}$ of $\mathrm{A}$ (with $n=\dim(\mathrm{A})$) such that $x_i\notin\mathfrak{p}$ for all $i$, without which $\mathrm{A}/\mathfrak{p}$ would be of finite length, and $\mathfrak{p}$ would be maximal, contrary to the hypothesis.
But if for example $x_1\notin\mathfrak{p}$, it suffices, for each index $i$ such that $x_i\in\mathfrak{p}$, to replace $x_i$ by $x_1+x_i$, to have a system of parameters none of whose elements belong to $\mathfrak{p}$.
The relation $\dim(\mathrm{A}/\mathfrak{q})=n$ implies, by virtue of \sref{IV.18.11.5.1}, that $m\geq n$; one can therefore consider a family $(x_i)_{1\leq i\leq m}$ of elements $x_i\notin\mathfrak{p}$, whose $n$ first form a system of parameters of $\mathrm{A}$.
Multiplying furthermore the $x_i$ by invertible elements $u_i$ of $\mathrm{A}$, one can, by \sref{IV.18.11.5.2}, suppose that the images of $dx_i$ ($1\leq i\leq m$) in $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ generate this $\mathrm{A}_\mathfrak{p}$-module, and the multiplication by the $u_i$ has not altered the fact that the $x_i$ for $1\leq i\leq n$ form a system of parameters.
We shall consider the local $k$-homomorphism $u:\mathrm{B}\to\mathrm{A}$ such that $u(\mathrm{T}_i)=x_i$ for $1\leq i\leq m$ (Bourbaki, \emph{Alg.\ comm.}, chap.~III, \textsection~4, n\textsuperscript{o}~5, prop.~6); as the $x_i$ generate an ideal of definition of $\mathrm{A}$, it follows from \sref[0]{0.7.4.4} and \sref{IV.7.4.3} that $u$ makes $\mathrm{A}$ a $\mathrm{B}$-algebra \emph{finite}.

But the $d_\mathrm{A}x_i$ are the canonical images by $v$ of the elements $d_\mathrm{B}\mathrm{T}_i\otimes 1$ \sref[0]{0.20.5.2.6}.
If one localises the exact sequence preceding at $\mathfrak{p}$, we see therefore that $v_\mathfrak{p}:(\hat{\Omega}^1_{\mathrm{B}/k}\otimes_\mathrm{B}\mathrm{A})_\mathfrak{p}\to(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ is \emph{surjective}, and consequently $0=(\Omega^1_{\mathrm{A}/\mathrm{B}})_\mathfrak{p}=\Omega^1_{\mathrm{A}_\mathfrak{p}/\mathrm{B}_\mathfrak{r}}$ (where $\mathfrak{r}$ is the inverse image of $\mathfrak{p}$ in $\mathrm{A}$, cf.\ \sref{IV.16.4.15}), thus the morphism $\operatorname{Spec}(\mathrm{A})\to\operatorname{Spec}(\mathrm{B})$ is not ramified at $\mathfrak{p}$ by \sref{IV.17.4.1}, and the property $b)$ of \sref{IV.18.11.5} follows.
\end{proof}

\begin{remark}[18.11.6]
\label{IV.18.11.6}
In the statement of \sref{IV.18.11.5}, one can drop the hypothesis $\mathfrak{p}\neq\mathfrak{m}$.
Indeed, for every local $k$-homomorphism $u:\mathrm{B}\to\mathrm{A}$, where $\mathrm{B}=k[[\mathrm{T}_1,\ldots,\mathrm{T}_r]]$ ($r$ any integer) making $\mathrm{A}$ a $\mathrm{B}$-algebra finite, $\mathfrak{m}$ is the only point of $\operatorname{Spec}(\mathrm{A})$ above the maximal ideal $\mathfrak{n}$ of $\mathrm{B}$; the morphism $\operatorname{Spec}(\mathrm{A})\to\operatorname{Spec}(\mathrm{B})$ cannot be unramified at $\mathfrak{m}$ unless $\mathrm{A}/\mathfrak{n}\mathrm{A}$ is a field, an extension separable of $k$ \sref{IV.17.4.1}, which implies that the residue field $\mathrm{K}$ of $\mathrm{A}$ is a separable extension of $k$.
If this condition is not satisfied, the conclusion of \sref{IV.18.11.5} can never be satisfied for $\mathfrak{p}=\mathfrak{m}$, whatever the integer $m$.
\end{remark}

\begin{corollary}[18.11.7]
\label{IV.18.11.7}
Let $k$ be a field, $\mathrm{A}$ a $k$-algebra, local Noetherian and complete, equidimensional, whose residue field is a finite extension of $k$, $\mathfrak{p}$ a prime ideal of $\mathrm{A}$.
Then the minimum number of generators of the $\mathrm{A}_\mathfrak{p}$-module $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ is at least equal to $\dim(\mathrm{A})$.
\end{corollary}

\begin{proof}
It suffices to set $\mathfrak{p}=0$ in \sref{IV.18.11.7}.
\end{proof}

\begin{corollary}[18.11.8]
\label{IV.18.11.8}
Let $k$ be a field, $\mathrm{A}$ a $k$-algebra, local Noetherian and complete, integral, whose residue field is a finite extension of $k$.
If $\mathrm{K}$ is the field of fractions of $\mathrm{A}$, one has $\operatorname{rg}_{\mathrm{K}}((\hat{\Omega}^1_{\mathrm{A}/k})\otimes_{\mathrm{A}}\mathrm{K})\geq\dim(\mathrm{A})$.
\end{corollary}

\begin{proof}
It suffices to set $\mathfrak{p}=0$ in \sref{IV.18.11.7}.
\end{proof}

\oldpage[IV-4]{177}
\begin{corollary}[18.11.9]
\label{IV.18.11.9}
Let $k$ be a field, $\mathrm{A}$ a $k$-algebra, local Noetherian and complete, of dimension $n$, whose residue field is a finite extension of $k$.
Let $\mathfrak{p}$ be a prime ideal of $\mathrm{A}$ distinct from the maximal ideal, and containing a minimal prime ideal $\mathfrak{q}$ such that $\dim(\mathrm{A}/\mathfrak{q})=n$.
Suppose that the $\mathrm{A}_\mathfrak{p}$-module $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ admits $n$ generators.
Then:
\begin{enumerate}[label=(\roman*)]
  \item The $\mathrm{A}_\mathfrak{p}$-module $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ is free of rank $n$.
  \item There exists a local $k$-homomorphism $u:\mathrm{B}\to\mathrm{A}$, where $\mathrm{B}=k[[\mathrm{T}_1,\ldots,\mathrm{T}_n]]$, making $\mathrm{A}$ a $\mathrm{B}$-algebra finite, and such that the corresponding morphism $\operatorname{Spec}(\mathrm{A})\to\operatorname{Spec}(\mathrm{B})$ is \'etale at the point $\mathfrak{p}$.
  \item The $k$-algebra $\mathrm{A}_\mathfrak{p}$ is geometrically regular over $k$.
\end{enumerate}
\end{corollary}

\begin{proof}
Let us prove first of all (ii); by virtue of \sref{IV.18.11.5}, there exists a local $k$-homomorphism $u:\mathrm{B}=k[[\mathrm{T}_1,\ldots,\mathrm{T}_n]]\to\mathrm{A}$ making $\mathrm{A}$ a $\mathrm{B}$-algebra finite, and such that the corresponding morphism $\operatorname{Spec}(\mathrm{A})\to\operatorname{Spec}(\mathrm{B})$ is not ramified at the point $\mathfrak{p}$; set $\mathfrak{r}=u^{-1}(\mathfrak{p})$.
The hypothesis on $\mathfrak{q}\subset\mathfrak{p}$ and the fact that $\mathrm{A}$ is a quotient ring of a regular ring \sref[0]{0.19.8.8} imply that $\dim(\mathrm{A}_\mathfrak{p})=n-\dim(\mathrm{A}/\mathfrak{p})$ \sref[0]{0.16.5.12}.
Similarly $\dim(\mathrm{B}_\mathfrak{r})=n-\dim(\mathrm{B}/\mathfrak{r})$.
Finally, since the morphism $\operatorname{Spec}(\mathrm{A})\to\operatorname{Spec}(\mathrm{B})$ is not ramified at the point $\mathfrak{p}$, the fibre of this morphism at the point $\mathfrak{r}$ is of dimension $0$, hence \sref[0]{0.16.3.9} $\dim(\mathrm{A}/\mathfrak{p})\leq\dim(\mathrm{B}/\mathfrak{r})$ and consequently $\dim(\mathrm{B}_\mathfrak{r})\leq\dim(\mathrm{A}_\mathfrak{p})$.
We conclude therefore from \sref{IV.18.10.1} that the morphism $\operatorname{Spec}(\mathrm{A})\to\operatorname{Spec}(\mathrm{B})$ is \'etale at $\mathfrak{p}$.

The assertion (iii) follows from the fact that $\mathrm{A}_\mathfrak{p}$ is formally smooth and that $\mathrm{B}_\mathfrak{r}$ is formally smooth on $k$ for their preadique topologies \sref[0]{0.19.3.4} and \sref{IV.19.3.5}), hence $\mathrm{A}_\mathfrak{p}$ is formally smooth on $k$ for its preadique topology, and consequently geometrically regular over $k$ \sref[0]{0.22.5.8}.

Let us finally prove (i).
Let $k'$ be an algebraically closed extension of $k$, and let us consider the $k'$-algebra semi-local $\mathrm{A}'=\mathrm{A}\widehat{\otimes}_kk'$.
As a $\mathrm{B}$-module of finite type via $u$, $\mathrm{A}$ is considered as a $\mathrm{B}$-algebra finite; since the morphism $\operatorname{Spec}(\mathrm{A})\to\operatorname{Spec}(\mathrm{B})$ is finite and \'etale at every point $\mathfrak{p}'$ above $\mathfrak{p}$ \sref{IV.17.3.3}; furthermore, $\mathrm{A}'$ is composed as a direct product of local rings $\mathrm{A}_i'$ of dimension $n$ \sref{IV.18.11.4} and $\mathfrak{p}'$ identifies with a prime ideal $\mathfrak{p}_i'$ of one of the $\mathrm{A}_i'$.
The same reasoning as above proves that $(\mathrm{A}_i')_{\mathfrak{p}_i'}$ is geometrically regular over $k'$, and as $k'$ is perfect, the fact $d)$ implies $b)$, hence by faithful flatness \sref{IV.18.11.4} and \sref{IV.2.5.2}, $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ is an $\mathrm{A}_\mathfrak{p}$-module free of finite type.
But $(\hat{\Omega}^1_{\mathrm{A}_i'/k'})_{\mathfrak{p}_i'}=(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}\otimes_{\mathrm{A}_\mathfrak{p}}(\mathrm{A}_i')_{\mathfrak{p}_i'}$ \sref{IV.18.11.4} and $(\mathrm{A}_i')_{\mathfrak{p}_i'}$ is a faithfully flat $\mathrm{A}_\mathfrak{p}$-module; one sees that $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ is an $\mathrm{A}_\mathfrak{p}$-module \emph{free} of finite type \sref{IV.2.5.2}; this finishes the proof.
\end{proof}

\begin{theorem}[18.11.10]
\label{IV.18.11.10}
\oldpage[IV-4]{178}
Let $k$ be a field of characteristic exponent $p$, $\mathrm{A}$ a $k$-algebra, local Noetherian and complete, whose residue field is a finite extension of $k$, $\mathfrak{p}$ a prime ideal of $\mathrm{A}$ distinct from the maximal ideal.
The following conditions are equivalent:
\begin{enumerate}[label=\alph*)]
  \item For every extension $k'$ of $k$, and every prime ideal $\mathfrak{p}'$ of $\mathrm{A}'=\mathrm{A}\widehat{\otimes}_kk'$ above $\mathfrak{p}$, $\mathrm{A}_{\mathfrak{p}'}$ is a regular ring.
\item[a$'$)] There exists a perfect extension $k'$ of $k$ and a prime ideal $\mathfrak{p}'$ of $\mathrm{A}'=\mathrm{A}\widehat{\otimes}_kk'$ above $\mathfrak{p}$, such that $\mathrm{A}_{\mathfrak{p}'}$ is regular.
  \item Let $n$ be the largest of the dimensions of the irreducible components of $\operatorname{Spec}(\mathrm{A})$ to which $\mathfrak{p}$ belongs; then $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ is an $\mathrm{A}_\mathfrak{p}$-module free of rank $n$.
\end{enumerate}
Suppose furthermore that $n=\dim(\mathrm{A})$ (which will be the case if $\mathrm{A}$ is equidimensional); then the preceding conditions are also equivalent to:
\begin{enumerate}[label=\alph*)]
  \setcounter{enumi}{2}
  \item There exists a local $k$-homomorphism $u:\mathrm{B}\to\mathrm{A}$, where $\mathrm{B}=k[[\mathrm{T}_1,\ldots,\mathrm{T}_n]]$, making $\mathrm{A}$ a $\mathrm{B}$-algebra finite, and such that the corresponding morphism $\operatorname{Spec}(\mathrm{A})\to\operatorname{Spec}(\mathrm{B})$ is \'etale at the point $\mathfrak{p}$.
  \item The ring $\mathrm{A}_\mathfrak{p}$ is geometrically regular over $k$.
\end{enumerate}
Furthermore, if $[k:k^p]<+\infty$, the condition $d)$ is equivalent to $a')$, $b)$, and $c)$.
\end{theorem}

\begin{proof}
The fact that $d)$ implies $b)$ when $[k:k^p]<+\infty$ is nothing other than \sref{IV.18.11.3}.
It is clear that $a)$ implies $a')$.
Let us show $c)$ implies $a)$ when $\dim(\mathrm{A})=n$.
One says that $\mathrm{A}$ is an $\mathrm{A}$-module flat \sref{IV.18.11.4} and $\dim(\mathrm{A}')=\dim(\mathrm{A})=n$; it follows from \sref{IV.2.3.4} and \sref{IV.6.1.1} that there exists a minimal prime ideal $\mathfrak{q}'$ of $\mathrm{A}'$ contained in $\mathfrak{p}'$, above $\mathfrak{q}$ and such that $\dim(\mathrm{A}'/\mathfrak{q}')=\dim(\mathrm{A}/\mathfrak{q})=n$.
Furthermore, by \sref{IV.18.11.4}, $(\hat{\Omega}^1_{\mathrm{A}'/k'})_{\mathfrak{p}'}=(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}\otimes_{\mathrm{A}_\mathfrak{p}}\mathrm{A}_{\mathfrak{p}'}'; $ as $k'$ is perfect, the hypothesis implies that it is geometrically regular over $k'$, one can apply to $\mathrm{A}'$ and $\mathfrak{p}'$ the fact $d)$ implies $b)$, hence $(\hat{\Omega}^1_{\mathrm{A}'/k'})_{\mathfrak{p}'}$ is an $\mathrm{A}_{\mathfrak{p}'}$-module free of rank $n$; by faithful flatness \sref{IV.18.11.4} and \sref{IV.2.5.2}, $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ is an $\mathrm{A}_\mathfrak{p}$-module free of rank $n$.
This shows the equivalence of $a)$, $a')$, $b)$, and $c)$ when $\dim(\mathrm{A})=n$.
It remains to prove that $a)$, $a')$, and $b)$ are still equivalent in the general case.
Let $\mathfrak{J}$ be the kernel of $\mathrm{A}$, the noyau of the canonical homomorphism $\mathrm{A}\to\mathrm{A}_\mathfrak{p}$, and set $\mathrm{A}_1=\mathrm{A}/\mathfrak{J}$; we necessarily have $\mathfrak{J}\subset\mathfrak{p}$, and if we set $\mathfrak{p}_1=\mathfrak{p}/\mathfrak{J}$, the canonical homomorphism $\mathrm{A}_\mathfrak{p}\to(\mathrm{A}_1)_{\mathfrak{p}_1}$ is \emph{bijective}; it follows from \sref[I]{I.6.5.4} that the canonical injection $\operatorname{Spec}(\mathrm{A}_1)\to\operatorname{Spec}(\mathrm{A})$ is an \emph{isomorphism local} at the point $\mathfrak{p}_1$, and one has $\mathfrak{J}_\mathfrak{p}=0$.
One sees as in \sref{IV.18.11.3} that the sequence is exact, and that one concludes that the condition $a)$ (resp.\ $a')$)) for the ring $\mathrm{A}$ and the prime ideal $\mathfrak{p}$ is \emph{equivalent} to the condition $a)$ (resp.\ $a')$)) for the ring $\mathrm{A}_1$ and the prime ideal $\mathfrak{p}_1$.
Now, all the minimal prime ideals of $\mathrm{A}$ contained in $\mathfrak{p}$ contain $\mathfrak{J}$ since $\operatorname{Spec}(\mathrm{A}_1)\to\operatorname{Spec}(\mathrm{A})$ is an isomorphism local at $\mathfrak{p}_1$; on the other hand the $\mathrm{Ass}_\mathrm{A}(\mathrm{A}/\mathfrak{J})$ are the prime ideals of $\mathrm{Ass}(\mathrm{A})$ which are contained in $\mathfrak{p}$ (Bourbaki, \emph{Alg.\ comm.}, chap.~IV, \textsection~1, n\textsuperscript{o}~2, prop.~6); hence all the minimal prime ideals of $\mathrm{A}_1$ are contained in $\mathfrak{p}_1$, and consequently $\dim(\mathrm{A}_1)=n$.
It suffices then to apply to $\mathrm{A}_1$ and $\mathfrak{p}_1$ which has been proved above.
\end{proof}

\begin{remark}[18.11.11]
\label{IV.18.11.11}
\oldpage[IV-4]{179}
\begin{enumerate}[label=(\roman*)]
  \item The equivalence of conditions $d)$ and $b)$ in \sref{IV.18.11.10} is no longer valid when one no longer supposes that $[k:k^p]<+\infty$.
Indeed, by the example of \sref[0]{0.22.7.7},~(ii)), the ring $\mathrm{B}=\mathrm{A}/\mathfrak{q}$ is integral and of dimension $1$; on the other hand, the sequence
\[
  (\mathfrak{q}/\mathfrak{q}^2)\otimes_{\mathrm{A}}\mathrm{L}\xrightarrow{j}\hat{\Omega}^1_{\mathrm{A}/k}\otimes_{\mathrm{A}}\mathrm{L}\to\hat{\Omega}^1_{\mathrm{B}/k}\otimes_{\mathrm{B}}\mathrm{L}\to 0
\]
is exact (same demonstration as in \sref{IV.18.11.3}), and we know that $j$ \emph{is not injective}, hence $j=0$ since $(\mathfrak{q}/\mathfrak{q}^2)\otimes_\mathrm{A}\mathrm{L}$ is of rank $1$.
We conclude that $\operatorname{rg}_{\mathrm{L}}(\hat{\Omega}^1_{\mathrm{B}/k}\otimes_{\mathrm{B}}\mathrm{L})=2$; as $\mathrm{B}_\mathfrak{p}$ is a $k$-algebra geometrically regular, one sees that the condition $d)$ does not imply here the condition $b)$.
  \item With the notations of \sref{IV.18.11.10} and supposing that $n=\dim(\mathrm{A})$ and $(x_i)_{1\leq i\leq n}$ a system of parameters of $\mathrm{A}$ not belonging to $\mathfrak{p}$; one deduces a local $k$-homomorphism $u:\mathrm{B}\to\mathrm{A}$ such that $u(\mathrm{T}_i)=x_i$ for $1\leq i\leq n$, making $\mathrm{A}$ a $\mathrm{B}$-algebra finite.
For the morphism $\operatorname{Spec}(\mathrm{A})\to\operatorname{Spec}(\mathrm{B})$ to be \emph{\'etale} at the point $\mathfrak{p}$, it is necessary and sufficient that the images of $d_\mathrm{A}x_i$ in $(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ form a \emph{system of generators} of this $\mathrm{A}_\mathfrak{p}$-module.
Indeed, as one has seen in the proof of \sref{IV.18.11.5} that if the morphism $\operatorname{Spec}(\mathrm{A})\to\operatorname{Spec}(\mathrm{B})$ is not ramified at $\mathfrak{p}$, the canonical homomorphism $(\hat{\Omega}^1_{\mathrm{B}/k}\otimes_\mathrm{B}\mathrm{A})_\mathfrak{p}\to(\hat{\Omega}^1_{\mathrm{A}/k})_\mathfrak{p}$ is surjective and the images of the elements $d_\mathrm{B}\mathrm{T}_i\otimes 1$, which generate $(\hat{\Omega}^1_{\mathrm{B}/k}\otimes_\mathrm{B}\mathrm{A})_\mathfrak{p}$, are the $d_\mathrm{A}x_i$; whence the \emph{necessity} of the condition.
Conversely, the same reasoning shows that if this condition is satisfied, the morphism $\operatorname{Spec}(\mathrm{A})\to\operatorname{Spec}(\mathrm{B})$ is not ramified at $\mathfrak{p}$, and the reasoning of \sref{IV.18.11.9} proves in fact that this morphism is \'etale at $\mathfrak{p}$.
\end{enumerate}
\end{remark}

\begin{corollary}[18.11.12]
\label{IV.18.11.12}
Let $k$ be a field of characteristic exponent $p$ such that $[k:k^p]<+\infty$, $\mathrm{A}$ a $k$-algebra, local Noetherian and complete and integral, which is not a field, and whose residue field is a finite extension of $k$.
Then there exists a finite radicial extension $k'$ of $k$ such that, setting $\mathrm{A}'=\mathrm{A}\otimes_kk'$, the $k'$-algebra $\mathrm{A}'_{\mathrm{red}}$ and the prime ideal $\mathfrak{o}$ of this algebra satisfy the equivalent conditions $a)$, $a')$, $b)$, $c)$, and $d)$ of \sref{IV.18.11.10}.
In particular, if $n=\dim(\mathrm{A})=\dim(\mathrm{A}')$,
\oldpage[IV-4]{180}
there exists a $k'$-homomorphism local $\mathrm{B}'=k'[[\mathrm{T}_1,\ldots,\mathrm{T}_n]]\to\mathrm{A}'_\mathrm{red}$ making $\mathrm{A}'_\mathrm{red}$ a $\mathrm{B}'$-algebra finite, and such that the field of fractions $\mathrm{K}'$ of $\mathrm{A}'_\mathrm{red}$ is a finite separable extension of the field of fractions $k'((\mathrm{T}_1,\ldots,\mathrm{T}_n))$ of $\mathrm{B}'$.
\end{corollary}

\begin{proof}
The morphism $\operatorname{Spec}(\mathrm{A}')\to\operatorname{Spec}(\mathrm{A})$ being complete, radicial, and finite, $\mathrm{A}'$ is a local ring, complete and its nilradical is the only prime ideal above $\mathfrak{o}$ of $\mathrm{A}$; by flatness, $\mathrm{A}'$ identifies with a sub-ring of $\mathrm{K}\otimes_kk'$, which is also the total ring of fractions of $\mathrm{A}'$; on has therefore $\mathrm{K}'=(\mathrm{K}\otimes_kk')_{\mathrm{red}}$.
It is a question of proving, by the equivalence $d)\Leftrightarrow c)$, that there exists a finite radicial extension $k'$ of $k$ such that $(\mathrm{K}\otimes_kk')_\mathrm{red}$ is a separable extension of $k'$.
Or one knows \sref[0]{0.19.8.9} that, under all the hypotheses made, there exists a sub-$k$-algebra $\mathrm{C}=k[[\mathrm{T}_1,\ldots,\mathrm{T}_n]]$ of $\mathrm{A}$ such that $\mathrm{A}$ is a finite $\mathrm{C}$-algebra; hence, $\mathrm{K}$ is a \emph{finite} extension of the field of fractions $\mathrm{K}_1=k((\mathrm{T}_1,\ldots,\mathrm{T}_n))$ of $\mathrm{C}$, which is \emph{separable} on $k$ \sref[0]{0.21.9.6.4}.
If $p$ is the characteristic exponent of $k$, one can therefore write $\mathrm{K}\otimes_kk^{p^{-\infty}}=\mathrm{K}\otimes_{k_1}(\mathrm{K}_1\otimes_kk^{p^{-\infty}})$ and $\mathrm{K}_1\otimes_kk^{p^{-\infty}}$ is a \emph{field}, a radicial extension of $\mathrm{K}_1$; on concludes that $\mathrm{K}\otimes_kk^{p^{-\infty}}$ is a finite algebra over the field $\mathrm{K}_1\otimes_kk^{p^{-\infty}}$, hence is \emph{Artinian}.
The conclusion follows therefore from the following more general lemma:
\end{proof}

\begin{lemma}[18.11.12.1]
\label{IV.18.11.12.1}
Let $k$ be a field of characteristic $p>0$, $\mathrm{K}$ an extension of $k$.
The following conditions are equivalent:
\begin{enumerate}[label=\alph*)]
  \item The ring $\mathrm{K}\otimes_kk^{p^{-\infty}}$ is Artinian.
  \item There exists an extension $k'$ of $k$ of finite type such that $(\mathrm{K}\otimes_kk')_{\mathrm{red}}$ is a $k'$-algebra separable.
  \item There exists a finite radicial extension $k'$ of $k$ such that $(\mathrm{K}\otimes_kk')_\mathrm{red}$ is a separable extension of $k'$.
\end{enumerate}
\end{lemma}

\begin{proof}
Let us first show that $c)$ implies $a)$.
The ring $\mathrm{A}=\mathrm{K}\otimes_kk'$ is then a local Artinian ring, and if $\mathfrak{N}$ is nilradical, the residue field $\mathrm{L}=\mathrm{A}/\mathfrak{N}$ is separable over $k'$; hence $\mathrm{L}\otimes_{k'}k^{p^{-\infty}}$ is separable over $k'$, and as it is equal to $(\mathrm{A}\otimes_{k'}k^{p^{-\infty}})/(\mathfrak{N}\otimes_{k'}k^{p^{-\infty}})$, one sees that $\mathfrak{N}\otimes_{k'}k^{p^{-\infty}}$ is the nilradical of $\mathrm{K}\otimes_kk^{p^{-\infty}}$; as $\mathfrak{N}$ is a nilpotent ideal of finite type, the nilradical of $\mathrm{K}\otimes_kk^{p^{-\infty}}$ is of finite type, hence the ring $\mathrm{K}\otimes_kk^{p^{-\infty}}$ is Artinian.

Conversely, let us prove that $a)$ implies $c)$.
Let $\mathfrak{N}$ be the nilradical of the local Artinian ring $\mathrm{B}=\mathrm{K}\otimes_kk^{p^{-\infty}}$, which is by hypothesis generated by a finite number $d$ of elements of the form $y_i=\sum_j x_{ij}\otimes\xi_{ij}$, $x_{ij}\in\mathrm{K}$, $\xi_{ij}\in k^{p^{-\infty}}$.
Let $k'$ be the finite radicial extension of $k$ generated by the $\xi_{ij}$, $\mathfrak{N}_0$ the ideal of $\mathrm{B}_0=\mathrm{K}\otimes_kk'$ generated by the $y_i$; it is clear that $\mathfrak{N}_0\otimes_{k'}k^{p^{-\infty}}=\mathfrak{N}$, and hence $\mathfrak{N}\cap\mathrm{B}_0=\mathfrak{N}_0$, and so it is equal to $\mathrm{B}_0/\mathfrak{N}_0=(\mathrm{K}\otimes_kk')_{\mathrm{red}}$ is separable over $k$ \sref{IV.4.6.1}.

It is clear that $c)$ implies $b)$.
Conversely, suppose $b)$ is satisfied, and let us note that there exists a separable extension $k_1$ of $k$ such that $k'$ is a finite radicial extension of $k_1$; set $\mathrm{K}_1=\mathrm{K}\otimes_kk_1$, which is a field.
Applying the equivalence of $a)$ and $c)$ to the extension $\mathrm{K}_1$,
\oldpage[IV-4]{181}
on sees that $\mathrm{K}_1\otimes_{k_1}k_1^{p^{-\infty}}$ is also Artinian (Bourbaki, \emph{Alg.\ comm.}, chap.~I, \textsection~3, n\textsuperscript{o}~5, cor.\ of prop.~8); we have thus shown that $b)$ implies $a)$, which finishes the proof.
\end{proof}


\subsection{Applications of \'etale localisation to quasi-finite morphisms}
\label{subsection:IV.18.12}

The results of this section were communicated by P.~Deligne.

\begin{theorem}[18.12.1]
\label{IV.18.12.1}
Let $f:\mathrm{X}\to\mathrm{Y}$ be a locally of finite type morphism, $x$ a point of $\mathrm{X}$, $y=f(x)$.
Suppose that $x$ is an isolated point of the space $f^{-1}(y)$.
Then there exist an \'etale morphism $\mathrm{Y}'\to\mathrm{Y}$, a point $x'$ of $\mathrm{X}'=\mathrm{X}\times_{\mathrm{Y}}\mathrm{Y}'$ above $x$ and a neighbourhood $\mathrm{V}'$ of $x'$ in $\mathrm{X}'$ such that, if $f'=f_{(\mathrm{Y}')}:\mathrm{X}'\to\mathrm{Y}'$, $f'|\mathrm{V}'$ is a finite morphism.
Furthermore $\mathrm{V}'$ is both open and closed, and $\mathrm{X}'$ is therefore the sum of two sub-preschemes induced on open subsets of $\mathrm{X}'$, of which one is finite on $\mathrm{Y}'$ and contains $x'$.
\end{theorem}

\begin{proof}
The last assertion follows from the fact that, if $f$ is separated, it is the same for $f'$; hence, if $j':\mathrm{V}'\to\mathrm{X}'$ is the injection canonical, and the fact that $f'$ is finite implies that it is the same for $j'$ \sref[II]{II.6.1.5}, and $\mathrm{V}'=j'(\mathrm{V}')$ is closed in $\mathrm{X}'$ \sref[II]{II.6.1.10}.

The question being local on $\mathrm{X}$, one can suppose $\mathrm{Y}=\operatorname{Spec}(\mathrm{A})$ and $\mathrm{X}=\operatorname{Spec}(\mathrm{B})$ affine, $\mathrm{B}$ being an $\mathrm{A}$-algebra of finite type, and one can then suppose $f^{-1}(y)=\{x\}$.
Hence the question is local on $\mathrm{Y}$, hence of the form $\mathrm{C}/\mathfrak{J}$, where $\mathrm{C}=\mathrm{A}[\mathrm{T}_1,\ldots,\mathrm{T}_r]$ and $\mathfrak{J}$ is an ideal of type finite of $\mathrm{E}$; we then have $\mathrm{A}'=\mathrm{E}'/\mathfrak{J}\mathrm{E}'$, where $\mathrm{E}'=\mathrm{C}'[\mathrm{T}_1,\ldots,\mathrm{T}_r]$.
Let $(\mathfrak{J}_\lambda)$ be the family of ideals of type fine of $\mathrm{C}$ contained in $\mathfrak{J}$, and denote the corresponding $\mathrm{X}_\lambda=\operatorname{Spec}(\mathrm{C}/\mathfrak{J}_\lambda)$ considered as closed sub-preschemes of $\mathrm{Z}=\operatorname{Spec}(\mathrm{C})$, so that $\mathfrak{J}$ is the filtered union of the $\mathfrak{J}_\lambda$.
If $\mathrm{X}$ and the $\mathrm{X}_\lambda=\operatorname{Spec}(\mathrm{C}/\mathfrak{J}_\lambda)$ are considered as closed sub-preschemes, $\mathrm{X}=\bigcap\mathrm{X}_\lambda$; if $f_\lambda:\mathrm{X}_\lambda\to\mathrm{Y}$ is the structural morphism, and as the sets $f_\lambda^{-1}(y)$ are closed in the Noetherian space $\mathrm{Z}_y=\operatorname{Spec}(k(y)[\mathrm{T}_1,\ldots,\mathrm{T}_r])$, there exists an index $\lambda$ such that $f_\lambda^{-1}(y)=f^{-1}(y)=\{x\}$.
One can then suppose that for a given $\mathrm{X}_\lambda$, the condition holds that $\mathrm{X}$ is at point $x$; if one proves the proposition for such an $\mathrm{X}_\lambda$, it will follow for $\mathrm{X}$, since $x'$ is a point above $x$ in $\mathrm{Z}'=\mathrm{Z}\times_\mathrm{Y}\mathrm{Y}'$, $\mathrm{X}_\lambda'=p^{-1}(\mathrm{X}_\lambda)$, where $p:\mathrm{Z}'\to\mathrm{Z}$ is the projection (from the restriction of $f_\lambda$ to $\mathrm{X}_\lambda$), the same will hold for the open set $\mathrm{V}_\lambda'$ of $x'$ in $\mathrm{X}_\lambda'$ which will be finite over $\mathrm{Y}'$.
Furthermore $\mathrm{V}'$ and $\mathrm{X}'$ are sums of two open sub-preschemes, $\mathrm{V}'$ and $\mathrm{X}_\lambda'$; one answers the question by taking $\mathrm{V}'$ as a sum of $\mathrm{U}'$ and $\mathrm{V}'$.
In particular, note that the set of isolated points of $\mathrm{X}$ in their fibre et open in $\mathrm{X}$ \sref{IV.13.1.4}, hence $f$ is \emph{quasi-finite}.
Set $\mathrm{A}''=\sh{O}_{Y,y}$, and set $\mathrm{Y}''=\operatorname{Spec}(\mathrm{A}'')$.
Set $\mathrm{X}''=\mathrm{X}\times_{\mathrm{Y}}\mathrm{Y}''$, $f''=f_{(\mathrm{Y}'')}:\mathrm{X}''\to\mathrm{Y}''$; $\mathrm{X}''\to\mathrm{Y}''$ is \emph{separated}, quasi-finite and of presentation finite; $\mathrm{A}''$ is henselian, for every $x''\in\mathrm{X}''$ above $x$ an open and closed neighbourhood $\mathrm{V}''$ of $x''$ in $\mathrm{X}''$ which is finite on $\mathrm{Y}''$ \sref{IV.18.5.11},~$c$)); as the immersion $\mathrm{V}''\to\mathrm{X}''$ is open and closed, it is also quasi-compact, hence $f''|\mathrm{V}''$ is of presentation finite since $\mathrm{V}''\to\mathrm{X}''$ is open, and $\mathrm{X}''\to\mathrm{Y}''$ is of presentation finite.
$f''$ is finite and quasi-compact over $\mathrm{Y}''$ \sref{IV.1.6.2}, hence $f''|\mathrm{V}''$ is of presentation finite.

Since, by definition, $\mathrm{A}''$ is the inductive limit of a filtered inductive family $(\mathrm{A}_\lambda)$ of $\sh{O}_{Y,y}$-algebras strictly \'etales (\sref{IV.18.6.5}) and \sref{IV.8.1.2},~$a$))); also $\mathrm{B}_{\mathrm{M}_\lambda}$ can be assumed to be a $\sh{O}_{Y,y}$-algebra of presentation finite, and it follows from \sref{IV.18.6.1} and \sref{IV.8.1.2},~$a$)) and the result of \sref{IV.17.7.8} that $\mathrm{B}_{\mathrm{M}_\lambda}$ is a $\sh{O}_{Y,y}$-algebra \'etale for all $\lambda$; Namely, $\mathrm{Y}''$ is the inductive limit projective of a family $(Y_\alpha)$ of affine $\mathrm{Y}$-schemes \'etale.
If $x_\alpha''$ is the projection of $x''$ on $\mathrm{X}_\alpha'=\mathrm{X}\times_{\mathrm{Y}}\mathrm{Y}_\alpha$, on can suppose that $\mathrm{X}_\alpha'\cap f'^{-1}(\mathrm{U})$ where $\mathrm{V}_\alpha'$ is a neighbourhood of $x_\alpha''$ in $\mathrm{X}_\alpha'$, is finite on $\mathrm{Y}_\alpha$ \sref{IV.8.2.11}.
Finally, $\mathrm{V}''$ being of presentation finite over $\mathrm{Y}''$, on concludes from \sref{IV.8.10.5},~(x)) that for a suitable $\alpha$, $\mathrm{V}_\alpha'$ is finite over $\mathrm{Y}_\alpha'$.
\end{proof}

\begin{remark}[18.12.2]
\label{IV.18.12.2}
In the preceding proof, one sees (taking into account \sref{IV.18.6.2}) that one has constructed $\mathrm{Y}'$ answering the question, and in addition such that if $y'=f'(\mathrm{X}')$, the homomorphism $k(y)\to k(y')$ is bijective.
\end{remark}

\begin{corollary}[18.12.3]
\label{IV.18.12.3}
Let $f:\mathrm{X}\to\mathrm{Y}$ be a locally of finite type morphism and separated, $y$ a point of $\mathrm{Y}$ such that the underlying space $f^{-1}(y)$ is finite and discrete; in particular, a proper and quasi-finite morphism.
Then there exist an \'etale morphism $\mathrm{Y}'\to\mathrm{Y}$, a point $y'\in\mathrm{Y}'$ above $y$ such that $k(y')=k(y)$, and a decomposition of $\mathrm{X}'=\mathrm{X}\times_{\mathrm{Y}}\mathrm{Y}'$ as a sum of two sub-preschemes $\mathrm{X}_1'$, $\mathrm{X}_2'$ induced on open subsets of $\mathrm{X}'$, such that the restriction $f'=f_{(\mathrm{Y}')}:\mathrm{X}_1'\to\mathrm{Y}'$ is a finite morphism and that $\mathrm{X}_2'\cap f'^{-1}(y')=0$.
\end{corollary}

\begin{proof}
\oldpage[IV-4]{182}
Indeed, one can apply the corollary \sref{IV.18.12.3} at any point whatsoever $y$ of $\mathrm{Y}$.
Since $f'$ is a closed morphism, hence, since $\mathrm{X}_2'$ is closed in $\mathrm{X}'$, $f'(\mathrm{X}_2')$ is closed in $\mathrm{Y}'$ and does not contain $y'$; hence there exists by construction an open neighbourhood $\mathrm{U}'$ of $y'$ in $\mathrm{Y}'$ such that $f'^{-1}(\mathrm{U}')=\mathrm{X}\times_\mathrm{Y}\mathrm{U}'$ is finite on $\mathrm{Y}'$, and the morphism $\mathrm{U}'\to\mathrm{Y}$ is \'etale; the morphism $f'^{-1}(\mathrm{U}')\to\mathrm{U}'$ is faithfully flat and quasi-compact, hence the conclusion.
\end{proof}

\begin{corollary}[18.12.4]
\label{IV.18.12.4}
Let $f:\mathrm{X}\to\mathrm{Y}$ be a finite morphism, it is necessary and sufficient that it be separated, universally closed and, for all $y\in\mathrm{Y}$, $f^{-1}(y)$ is a finite and discrete space; in particular, a proper and quasi-finite morphism.
\end{corollary}

\begin{proof}
Indeed, one can apply the corollary \sref{IV.18.12.3} at any point whatsoever $y$ of $\mathrm{Y}$.
Since $f'$ is a closed morphism, hence, since $\mathrm{X}_2'$ is closed in $\mathrm{X}'$, $f'(\mathrm{X}_2')$ is closed in $\mathrm{Y}'$ and does not contain $y'$; hence there exists by construction an open neighbourhood $\mathrm{U}'$ of $y'$ in $\mathrm{Y}'$ such that $f'^{-1}(\mathrm{U}')=\mathrm{X}\times_\mathrm{Y}\mathrm{U}'$ is finite over $\mathrm{Y}'$, and the morphism $\mathrm{U}'\to\mathrm{Y}$ is \'etale; the morphism $\mathrm{U}'\to\mathrm{Y}$ is faithfully flat and quasi-compact, hence by \sref{IV.2.7.1},~(xv)), the restriction of $f$ to $\mathrm{X}_\lambda$, being a finite morphism \sref[II]{II.6.1.10}, hence the conclusion results from \sref{IV.11.3.1}, since $f$ restricted is finite and quasi-compact.
\end{proof}

\begin{remark}[18.12.5]
\label{IV.18.12.5}
One notes that in the proof of \sref{IV.18.12.4} one has not used the hypothesis \sref{IV.18.12.4} in its general form, but only that $f_{(\mathrm{Y}')}$ is a universally closed morphism for every \'etale morphism $\mathrm{Y}'\to\mathrm{Y}$.
\end{remark}

\begin{corollary}[18.12.6]
\label{IV.18.12.6}
Let $f:\mathrm{X}\to\mathrm{Y}$ be a morphism of preschemes.
The following conditions are equivalent:
\begin{enumerate}[label=\alph*)]
  \item $f$ is a closed immersion.
  \item $f$ is a proper monomorphism.
  \item $f$ is proper and, for all $y\in\mathrm{Y}$, the $k(y)$-prescheme $\mathrm{X}_y=f^{-1}(y)$ is radicial and geometrically reduced on $k(y)$ (i.e.\ empty or $k(y)$-isomorphic to $\operatorname{Spec}(k(y))$).
\end{enumerate}
\end{corollary}

\begin{proof}
This is deduced from \sref{IV.18.12.4} as \sref{IV.18.11.5} is deduced from \sref{IV.8.11.1}.
\end{proof}

\begin{proposition}[18.12.7]
\label{IV.18.12.7}
Let $f:\mathrm{X}\to\mathrm{Y}$ be a locally of finite type morphism, $y$ a point of $\mathrm{Y}$.
For there to exist a neighbourhood $\mathrm{U}$ of $y$ such that the restriction $f^{-1}(\mathrm{U})\to\mathrm{U}$ of $f$ is a closed immersion, it is necessary and sufficient that $\mathrm{X}_y$ be a $k(y)$-prescheme radicial and geometrically reduced on $k(y)$ (i.e.\ empty or $k(y)$-isomorphic to $\operatorname{Spec}(k(y))$) and that there exist a neighbourhood $\mathrm{V}$ of $y$ such that the restriction $f^{-1}(\mathrm{V})\to\mathrm{V}$ of $f$ is a universally closed morphism.
\end{proposition}

\begin{proof}
The conditions being evidently necessary, let us prove that they are sufficient.
One can, by the second condition, already suppose $f$ universally closed.
Let us show, moreover, hence separated; this follows from the following lemma:
\end{proof}

\begin{lemma}[18.12.7.1]
\label{IV.18.12.7.1}
Let $f:\mathrm{X}\to\mathrm{Y}$ be a closed morphism, $y\in\mathrm{Y}$ a point such that every neighbourhood of $\mathrm{X}_y$ contains an affine neighbourhood of $\mathrm{X}_y$ (condition always satisfied when $\mathrm{X}_y$ is empty or reduced to a single point).
Then there exists a neighbourhood $\mathrm{U}$ of $y$ such that the restriction $f^{-1}(\mathrm{U})\to\mathrm{U}$ of $f$ is an affine morphism.
\end{lemma}

\begin{proof}
Indeed, let $\mathrm{U}_0$ be an affine open neighbourhood of $y$ in $\mathrm{Y}$ and let $\mathrm{V}$ be a neighbourhood of $\mathrm{X}_y$ contained in $f^{-1}(\mathrm{U}_0)$ such that $f^{-1}(\mathrm{U})\subset\mathrm{V}$, which is affine.
As $f$ is closed and $\mathrm{V}$ open, there exists a neighbourhood $\mathrm{U}\subset\mathrm{U}_0$ of $y$ such that $f^{-1}(\mathrm{U})\subset\mathrm{V}$.
As the restriction $f^{-1}(\mathrm{U})\to\mathrm{U}$ is also an affine morphism \sref[II]{II.1.2.5}.
\end{proof}

Suppose therefore $f$ affine and universally closed, and let us show that for all $y\in\mathrm{Y}$ such that the restriction $f^{-1}(\mathrm{U})\to\mathrm{U}$ of $f$ is a closed immersion, it suffices to prove that $f$ is quasi-finite \sref{IV.18.12.4}, and since $\mathrm{X}_y$ is empty or reduced to a single point, $f$ is quasi-finite \sref{IV.18.12.4}, taking into account \sref{IV.18.12.6}.

\begin{proposition}[18.12.8]
\label{IV.18.12.8}
Let $f:\mathrm{X}\to\mathrm{Y}$ be a morphism of preschemes.
For $f$ to be entire, it is necessary and sufficient that $f$ be affine and universally closed.
\end{proposition}

\begin{proof}
We can suppose $\mathrm{X}=\operatorname{Spec}(\mathrm{B})$, and it suffices to prove that every element $b\in\mathrm{B}$ is entire on $\mathrm{A}$ \sref[II]{II.6.1.1}.
Let $\mathrm{B}'$ be the sub-$\mathrm{A}$-algebra of $\mathrm{B}$ generated by $b$, which is an $\mathrm{A}$-algebra of finite type; set $\mathrm{X}'=\operatorname{Spec}(\mathrm{B}')$, so that $f:\mathrm{X}\to\mathrm{Y}$ factorises as $\mathrm{X}\xrightarrow{g}\mathrm{X}'\xrightarrow{h}\mathrm{Y}$, where $h$ is of finite type, and $g$ is \emph{dominant} since the homomorphism $\mathrm{B}'\to\mathrm{B}$ is injective \sref[I]{I.1.2.7}.
As $h$ is separated and $h\circ g$ universally closed, $g$ is universally closed \sref[II]{II.5.4.3} and
\oldpage[IV-4]{183}
\sref{IV.5.4.9}); hence $h$ is also universally closed, therefore proper since it is of finite type and separated.
But then, for all $y\in\mathrm{Y}$, the morphism $h^{-1}(y)\to\operatorname{Spec}(k(y))$ deduced from $h$ is proper and affine, hence \emph{finite} \sref[III]{III.4.4.2}, in other words $h$ is \emph{quasi-finite}; one deduces therefore from \sref{IV.18.12.4} that $h$ is finite, and consequently $b$ is entire on $\mathrm{A}$.
\end{proof}

\begin{remark}[18.12.9]
\label{IV.18.12.9}
One can say that a morphism $f:\mathrm{X}\to\mathrm{Y}$ is entire when one supposes only separated, universally closed, and such that for all $y\in\mathrm{Y}$, the morphism $f^{-1}(y)\to\operatorname{Spec}(k(y))$ deduced from $f$ is entire.
It would suffice to prove that these conditions imply that $f$ is affine, or again that every fibre $\mathrm{X}_y$ is contained in an affine open neighbourhood.
\end{remark}

\begin{corollary}[18.12.10]
\label{IV.18.12.10}
A morphism $f:\mathrm{X}\to\mathrm{Y}$ which is injective and universally closed is entire.
\end{corollary}

\begin{proof}
It suffices, by \sref{IV.18.12.8}, to prove that $f$ is affine, which follows from the lemma \sref{IV.18.12.7.1} and the hypothesis.
\end{proof}

\begin{corollary}[18.12.11]
\label{IV.18.12.11}
Let $f:\mathrm{X}\to\mathrm{Y}$ be a morphism of preschemes (resp.\ a locally of finite type morphism).
For $f$ to be a universal homeomorphism \sref{IV.2.4.2}, it is necessary and sufficient that $f$ be entire (resp.\ finite), radicial, and surjective.
\end{corollary}

\begin{proof}
One knows that the conditions are sufficient \sref{IV.2.4.5}; they are necessary by virtue of \sref{IV.18.12.10} and \sref{IV.18.12.4}.
The last proposition improves \sref{IV.8.11.2}.
\end{proof}

\begin{proposition}[18.12.12]
\label{IV.18.12.12}
Every morphism $f:\mathrm{X}\to\mathrm{Y}$ which is quasi-finite and separated is quasi-affine.
\end{proposition}

\begin{proof}
Set $\mathscr{A}=f_*(\sh{O}_X)$, which is an $\sh{O}_Y$-Algebra quasi-coherent and separated \sref{IV.1.7.4}; and $\mathrm{Z}=\operatorname{Spec}(\mathscr{A})$ \sref[II]{II.1.3.1}, so that $f$ factorises canonically as $\mathrm{X}\xrightarrow{g}\mathrm{Z}\xrightarrow{h}\mathrm{Y}$, $h$ being affine and $g$ corresponding to the automorphism identical to $\mathscr{A}$ \sref[II]{II.1.2.7}; it suffices to prove that $g$ is an \emph{open immersion}, or, what comes to the same \sref{IV.17.9.1}, that the morphism $g$ is \'etale and \emph{radicial}.
It suffices evidently to prove that, for all $y\in\mathrm{Y}$, the morphism $g$ is \'etale and radicial at each point of $f^{-1}(y)$.
One can apply the result to $\mathrm{X}$, $f$ the restriction and $\mathrm{Y}$ by $\mathrm{X}$, $\mathrm{Y}$, and the sections of $f$ of $f$; hence $f$ is quasi-compact and separated, and the fact that $f$ is \'etale and radicial at each point of $f^{-1}(y)$ is part of \sref{IV.18.10.2},~(iii)), quasi-separable,  from $\mathrm{X}$, $f$, and $g$ as in \sref{IV.18.12.3}, whose $\mathrm{X}'$ is the extension $\mathrm{Z}'=\mathrm{Z}\times_{\mathrm{Y}}\mathrm{Y}'$ a $\sh{O}_{Y'}$-Module $\sh{O}_{X'}/\mathfrak{J}\mathrm{E}'$, so that as $\mathrm{X}$ and $\mathrm{Y}$ are both $\mathrm{Y}$-schemes \'etale \sref{IV.17.3.2}; hence by \sref[II]{II.1.2.7} $f$ factorises as $\mathrm{X}\xrightarrow{g}\mathrm{Z}\xrightarrow{h}\mathrm{Y}$, where $g$ is the canonical injection from the identity on $\mathscr{A}$, and $g$ is an \emph{open immersion}, and $g$ is \emph{radicial}, whence the result.
\end{proof}

\begin{corollary}[18.12.13]
\label{IV.18.12.13}
(``Main Theorem'' of Zariski).
Let $\mathrm{Y}$ be a prescheme quasi-compact and quasi-separated.
Then there exists a factorisation of $f$
\[
  \mathrm{X}\xrightarrow{g}\mathrm{Z}\xrightarrow{u}\mathrm{Y}
\]
where $g$ is an open immersion (necessarily quasi-compact) and $u$ a finite morphism.
\end{corollary}

\begin{proof}
Indeed, it follows from \sref{IV.18.12.12} that $f$ is quasi-affine, hence quasi-projective.
One can therefore apply \sref{IV.8.12.8}, where the hypothesis on $\sh{O}_Y$-Module ample can in fact be replaced by the sole hypothesis that $\mathrm{Y}$ is quasi-compact and quasi-separated: indeed, it follows from \sref{IV.8.12.3} that the existence of the factorisation announced is a \emph{local} property on $\mathrm{Y}$, and it suffices to prove it when $\mathrm{Y}$ is affine.
\end{proof}

\begin{remark}[18.12.14]
\label{IV.18.12.14}
One can give a proof of \sref{IV.18.12.13} analogous to that of \sref{IV.18.12.12} (but not using \sref{IV.8.12.3}, hence \sref{IV.18.5.11}), which itself uses the ``Main Theorem'' under its local form \sref{IV.8.12.9}).
Keeping indeed the notations of the proof of \sref{IV.18.12.12}, and let $\mathscr{B}$ be the $\sh{O}_Y$-Algebra quasi-coherente, integral closure of $\sh{O}_Y$ in $\mathscr{A}$ \sref[II]{II.6.3.2}.
If one sets $\mathrm{T}=\operatorname{Spec}(\mathscr{B})$, it suffices, by virtue of \sref{IV.8.12.3}, to prove that $g:\mathrm{X}\to\mathrm{T}$ corresponding to the canonical injection $\mathscr{B}\to\mathscr{A}$ is an immersion, and radicial.
With the notations of \sref{IV.18.12.12}, and one can suppose that $\mathrm{Y}=\operatorname{Spec}(\mathrm{C})$ and $\mathrm{Y}'=\operatorname{Spec}(\mathrm{C}')$ are affine, $\mathrm{C}'$ being a $\mathrm{C}$-algebra \'etale \sref{IV.17.3.2}; hence $\mathscr{A}=\overline{\mathrm{A}}$, where $\mathrm{A}$ is a $\mathrm{C}$-algebra, $\mathrm{B}$ the integral closure of $\mathrm{C}$ in $\mathrm{A}$.
The algebra $\mathrm{A}'$ is a $\mathrm{C}'$-algebra, $\mathrm{A}'=\mathrm{A}\otimes_\mathrm{C}\mathrm{C}'$, and $\mathscr{B}=\overline{\mathrm{B}}$, where $\mathrm{B}$ is the integral closure of $\mathrm{C}$ in $\mathrm{A}$.
As $\mathrm{Z}'=\mathrm{Z}\times_\mathrm{Y}\mathrm{Y}'$ and the $\mathrm{X}_\lambda'$ and $\mathrm{X}_\lambda$ are \'etale over $\mathrm{X}$, the conclusion follows in this case.

When $\mathrm{C}$ is Noetherian, the demonstration of \sref{IV.6.14.4} no longer requires the use of \sref{IV.6.14.1}.
Indeed, the reductions successives of the proof of \sref{IV.6.14.4} lead back, without using \sref{IV.6.14.1}, to the case where $\mathrm{C}$ is \emph{integral}, $\mathrm{A}$ the field of fractions of $\mathrm{C}$, and the morphism $\operatorname{Spec}(\mathrm{B}')\to\operatorname{Spec}(\mathrm{B})$ is \'etale, it follows from \sref{IV.17.5.7} that $\mathrm{B}'$ is a normal ring, and the reasoning terminates by making appeal only to the easy lemma \sref{IV.6.14.1}, and not to the difficult part of the proof of \sref{IV.6.14.1}.
\end{remark}

\oldpage[IV-4]{184}
\begin{proposition}[18.12.15]
\label{IV.18.12.15}
Let $\mathrm{G}$ be a ring, $\mathrm{C}'$ a $\mathrm{C}$-algebra \'etale, $\mathrm{A}$ a $\mathrm{C}$-algebra, $\mathrm{B}$ the integral closure of $\mathrm{C}$ in $\mathrm{A}$; on sets $\mathrm{A}'=\mathrm{A}\otimes_\mathrm{C}\mathrm{C}'$, $\mathrm{B}'=\mathrm{B}\otimes_\mathrm{C}\mathrm{C}'$, $\mathrm{B}'$ identifying to a sub-algebra of $\mathrm{A}'$; then $\mathrm{B}'$ is the integral closure of $\mathrm{C}'$ in $\mathrm{A}'$.
\end{proposition}

\begin{proof}
Considering the filtered increasing family of sub-$\mathrm{C}$-algebras of type finite of $\mathrm{A}$ and using the fact that $\mathrm{A}$ is a $\mathrm{C}$-algebra of type fine, hence of the form $\mathrm{E}/\mathfrak{J}$, where $\mathrm{E}=\mathrm{C}[\mathrm{T}_1,\ldots,\mathrm{T}_r]$ and $\mathfrak{J}$ is an ideal of type fine of $\mathrm{E}$; we then have $\mathrm{A}'=\mathrm{E}'/\mathfrak{J}\mathrm{E}'$, where $\mathrm{E}'=\mathrm{C}'[\mathrm{T}_1,\ldots,\mathrm{T}_r]$.
Let $(\mathfrak{J}_\lambda)$ be the increasing filtered family of the ideals of type fine of $\mathrm{C}$ contained in $\mathfrak{J}$ and denote $\mathrm{B}_\lambda$ the integral closure of $\mathrm{C}$ in $\mathrm{E}/\mathfrak{J}_\lambda$; if $\mathrm{B}_\lambda$ is the inductive limit of the integral closures $\mathrm{B}_\lambda$, $\mathrm{B}$ is the inductive limit of the $\mathrm{B}_\lambda$, as one sees by the reasoning of \sref{IV.5.13.4}; same reasoning for $\mathrm{C}$ and the $\mathrm{B}_\lambda\otimes_\mathrm{C}\mathrm{C}'$ for $\beta\geq\alpha$.
The same, $\mathrm{A}'$ is the inductive limit of the integral closures of $\mathrm{C}'$ in $\mathrm{E}'/\mathfrak{J}_\lambda\mathrm{E}'$, hence the same reasoning as the preceding shows that $\mathrm{B}'=\mathrm{B}\otimes_\mathrm{C}\mathrm{C}'$ is the integral closure of $\mathrm{C}'$ in $\mathrm{A}'$ once we have reduced to the case where $\mathrm{C}$ is Noetherian, the proposition becomes a particular case of \sref{IV.6.14.4}.
It is necessary however to observe that, when $\mathrm{C}'$ is supposed \emph{\'etale} on $\mathrm{C}$ Noetherian, the demonstration of \sref{IV.6.14.4} no longer requires that one use \sref{IV.6.14.1}.
Indeed, the successive reductions in the proof of \sref{IV.6.14.4} lead back, \emph{without using} \sref{IV.6.14.1}, to the case where $\mathrm{C}$ is \emph{integral}, $\mathrm{A}$ the field of fractions of $\mathrm{C}$, hence $\mathrm{B}$ is a normal ring and the morphism $\operatorname{Spec}(\mathrm{B}')\to\operatorname{Spec}(\mathrm{B})$ is \'etale, it follows from \sref{IV.17.5.7} that $\mathrm{B}'$ is a normal ring, and the reasoning terminates by making appeal only to the elementary lemma \sref{IV.6.14.1}, and not the difficult part of the proof of \sref{IV.6.14.1}.
\end{proof}

\begin{proposition}[18.12.16]
\label{IV.18.12.16}
Let $g:\mathrm{Y}\to\mathrm{S}$ be a quasi-compact morphism, $f:\mathrm{X}\to\mathrm{Y}$ a separated and quasi-finite morphism.
For every $\sh{O}_Y$-Module invertible $\mathscr{L}$, ample relative to $\mathrm{S}$ \sref[II]{II.4.6.1}, the $\sh{O}_X$-Module $f^*(\mathscr{L})$ is ample relative to $\mathrm{S}$.
\end{proposition}

\begin{proof}
Indeed, by virtue of \sref{IV.18.12.12}, the morphism $f$ is quasi-affine, hence \sref[II]{II.5.1.6} the $\sh{O}_X$-Module $\sh{O}_X$ is ample for $f$.
We deduce from \sref[II]{II.4.6.13},~(iii)) that $\sh{O}_X\otimes_{\sh{O}_X}f^*(\mathscr{L}^{\otimes n})=f^*(\mathscr{L}^{\otimes n})$ is ample relative to $\mathrm{S}$ for $n$ large enough.
But as $f^*(\mathscr{L}^{\otimes n})=(f^*(\mathscr{L}))^{\otimes n}$, this implies that $f^*(\mathscr{L})$ is ample relative to $\mathrm{S}$ \sref[II]{II.4.6.9},~(i)).
\end{proof}

\begin{corollary}[18.12.17]
\label{IV.18.12.17}
Let $\mathrm{Z}$ be an affine scheme, $h:\mathrm{X}\to\mathrm{Z}$ a morphism of finite type, $\mathscr{L}$ an $\sh{O}_X$-Module invertible.
With the notations of \sref[II]{II.4.5.2}, the conditions of \sref[II]{II.4.5.2} (which are equivalent to saying that $\mathscr{L}$ is ample) are also equivalent to each of the following:

\begin{enumerate}
  \item[\rm b$''$)] $h$ is separated, one has $\mathrm{G}(\varepsilon)=\mathrm{X}$ and the canonical morphism $u:\mathrm{G}(\varepsilon)\to\operatorname{Proj}(\mathrm{S})$ has finite and discrete fibres.
  \item[\rm b$'''$)] One has $\mathrm{G}(\varepsilon)=\mathrm{X}$ and the canonical morphism $u$ is radicial.
\end{enumerate}

If $\mathrm{Z}=\operatorname{Spec}(\mathrm{A})$, the ring $\mathrm{S}$ is canonically equipped with a structure of graded $\mathrm{A}$-algebra, hence $h=g\circ u$.
Since every radicial morphism is separated \sref[I]{I.1.8.7.1}, $b''')$ implies $b'')$.
As the condition $b)$ of \sref[II]{II.4.5.2} implies evidently $b'')$, it remains to see that $b'')$ implies $\mathscr{L}$ ample.
Since $h=g\circ u$ is of finite type by hypothesis, $u$ is also quasi-finite and separated \sref[I]{I.6.3.4}, hence by the hypothesis $b'')$ implies that $u$ is quasi-finite and separated \sref[I]{I.5.5.1}.
To prove that $\mathscr{L}$ is ample, we shall now apply \sref{IV.18.12.16}.
Set $\mathrm{Y}=\operatorname{Proj}(\mathrm{S})$.
Since $\mathrm{X}$ is quasi-compact, there exists an integer $n>0$ and a finite number of elements $f_j\in\mathrm{S}_n$ such that the inverse images $u^{-1}(\mathrm{D}_+(f_j))$ cover $\mathrm{X}$; one can therefore consider $u$ as a quasi-finite and separated morphism of $\mathrm{X}$ in the open $\mathrm{Y}'=\bigcup\mathrm{D}_+(f_j)$ of $\mathrm{Y}$.
Let us consider then the $\sh{O}_{Y'}$-Module invertible $\mathscr{L}'=\sh{O}_Y(n)\,|\,\mathrm{Y}'$ \sref[II]{II.2.5.8}; it is very ample relative to $\mathrm{Z}$ \sref[II]{II.4.4.3}, and consequently it is the same for $u^*(\mathscr{L}')$ \sref[II]{II.4.4.3}.

$\mathscr{L}'=\sh{O}_Y(n)\,|\,\mathrm{Y}'$ \sref[II]{II.2.5.8}; it is very ample relative to $\mathrm{Z}$ \sref[II]{II.4.4.3}, and as $u^*(\mathscr{L}')=\mathscr{L}^{\otimes n}$ by \sref[II]{II.4.4.3}, $\mathscr{L}^{\otimes n}$ is thus ample relative to $\mathrm{S}$ (this amounts to saying that it is ample \sref[II]{II.4.6.6}), and consequently the same is true of $\mathscr{L}$ \sref[II]{II.4.5.6},~(i)).

We leave it to the reader to state analogous conditions equivalent to those of \sref[II]{II.4.6.3} for the $\sh{O}_X$-Modules relatively ample.
\end{corollary}
