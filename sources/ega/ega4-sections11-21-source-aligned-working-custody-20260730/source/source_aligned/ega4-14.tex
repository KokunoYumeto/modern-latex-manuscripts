...
We have already seen \sref{IV.2.4.6} that a \emph{flat} morphism and locally of finite presentation is universally open, the converse being inexact.
...
Finally, when $f$ admits a \emph{section} $g$ (which will be the case when $X$ is a $Y$-\emph{scheme in groups}) and for every $y\in Y$, one denotes by $X_y^0$ the connected component of the fibre $X_y=f^{-1}(y)$ at the point $g(y)$ (``neutral component'' in the case of groups), one studies the union $X^0$ of the $X_y^0$ for $y\in Y$, and one shows \sref{IV.15.6.4} that if $f$ is universally open and the fibres $X_y$ geometrically reduced, then $X^0$ is an \emph{open} set in $X$.
...
Now, if $U$ is a neighbourhood of $x$ such that $\psi(U)\cap Y_i$ is a neighbourhood of $\psi(x)$ in $Y_i$, there exists a neighbourhood $V\subset W$ of $\psi(x)$ in $Y$ such that for all such $i$, $\psi(x)\in Y_i$,
\oldpage[IV-3]{201}
...
\begin{enumerate}[label=(\roman*)]
  ...
  For example, let $K$ be a field, $A$ the polynomial ring $K[S,T]$, $V$ the affine plane $\Spec(A)$, $X$ the closed sub-prescheme of $V$ which is the ``union'' of the line $X_1$ defined by $T=0$ and of the line $X_2$ defined by $S=0$, i.e.\ $\Spec(A/\mathfrak{a})$, where $\mathfrak{a}=\mathrm{AST}$; let $Y=X_2=\Spec(A/\mathrm{AS})$ and for $f$ the projection corresponding to the canonical injection $K[T]\to A/\mathfrak{a}$; then we have $Z=X_2$, which is not open in $X$.
  ...
  \item We do not know whether, when $X$ and $Y$ are locally Noetherian and $f:X\to Y$ is a morphism locally of finite type, the set of points of $X$ where $f$ is open is open or not \emph{locally constructible}.
\end{enumerate}
\end{remarks}
...
\begin{remark}[14.1.5]
\label{IV.14.1.5}
Even if $X$ and $Y$ are locally Noetherian and $f$ a morphism \emph{fini}, it can happen that $f$ is open at every point of a fibre $f^{-1}(y)$, without there existing a \emph{neighbourhood} of $f^{-1}(y)$ at every point of which $f$ is open.
...
As the union of the $Y_i$ is a neighbourhood of $y$ in $Y$, we see that one can restrict to proving the corollary when $Y$ is irreducible; let $\eta$ be its generic point.
It follows from \sref{IV.14.2.4} that $d(y)\leq d(\eta)$; on the other hand, as $f$ is proper, we deduce from \sref{IV.13.1.5} that $z\mapsto\dim(f^{-1}(z))$ is upper semi-continuous; in particular $d(y)\geq d(\eta)$, hence $d(y)=d(\eta)$.
Moreover, there is a neighbourhood $V$ of $y$ such that $d(z)\leq d(y)$ for every $z\in V$; but as every $z$ is a specialisation of $\eta$, we have on the other hand $d(z)\geq d(\eta)$, whence finally $d(z)=d(y)$ for $z\in V$.
\end{proof}
...
\label{IV.14.2.6}
Let $Y$ be a locally Noetherian prescheme, $f:X\to Y$ an open morphism locally of finite type, $y$ a point of $Y$.
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item There exists a neighbourhood $U$ of $f^{-1}(y)$ such that the function $z\mapsto\dim(U\cap f^{-1}(z))$ is constant in a neighbourhood of $y$.
\end{enumerate}
\end{corollary}
...
Conversely, let us show that if $f$ is locally of finite presentation, suppose that $f$ is locally of finite presentation, and let us show that for every integer $e$, if one sets $Y''=Y[\mathrm{T}_1,\ldots,\mathrm{T}_e]$, $f_{(Y'')}$ is an open morphism.
...
\begin{enumerate}[label=(\roman*)]
  ...
  For $f$ to be universally open at the point $x$, it is necessary and sufficient that $f_1$ be so (recall \sref[I]{I.3.6.5} that $X_1$ is canonically identified with a sub-espace of $X$).
\end{enumerate}
\end{proposition}
...
On the other hand, if it is satisfied, and if $g:Y'\to Y$ is any morphism, $X'=X_{(Y')}$, $f'=f_{(Y')}$, $x'$ a point of $X'$ above $x$, then $f'$ is locally of finite presentation, and to see that it is open at the point $x'$, it suffices to apply the criterion \sref{IV.1.10.3},~$b)$).
...
\begin{proof}
Taking into account the definition, this follows from the analogous proposition \sref{IV.14.1.2},~(ii)) for open morphisms at a point.
\end{proof}
...
Since $Y''$ is locally Noetherian, it follows from \sref[II]{II.7.1.9} and $(\mathbf{0}_{\mathrm{III}},~10.3.1)$ that there exists a scheme $Y'=\Spec(A)$, where $A$ is a discrete valuation ring, and a morphism $g:Y'\to Y''$ such that, if $s$ and $y'$ are the generic point and the closed point respectively, we have $g(s)=t$ and $g(y')=y''$.
There is then a point $x'$ of $X'=X''\times_{Y''}Y'$ whose projections in $X''$ and $Y'$ are $x''$ and $y'$ respectively, and which is rational over $k(y')$ \sref[I]{I.3.4.9}.
The hypothesis implies that there is a generization $z'$ of $x'$ in $X'$ whose projection in $Y'$ is $s$; if $z''$ is the projection of $z'$ in $X''$, $z''$ is a generization of $x''$ and its projection in $Y''$ is $t$; we conclude therefore from \sref{IV.1.10.3} that $f''$ is open at the point $x''$, hence $f$ is universally open at the point $x$ \sref{IV.14.3.3.1},~(i)).
\end{proof}
...
\begin{proof}
As $f_{\mathrm{red}}$ is locally of finite type \sref{IV.1.3.4}, $a)$ implies that $f_{\mathrm{red}}$ is flat in a neighbourhood of $f^{-1}(y)$ \sref{IV.11.1.1}, and it suffices to apply \sref{IV.2.4.6} in such a neighbourhood.
...
\oldpage[IV-3]{207}
affine; if $X_i$ ($1\leq i\leq n$) are the closed sub-preschemes (integral) of $X$ having as underlying spaces the irreducible components of $X$, then, for every $x\in X$, $\sh{O}_{X,x}$ is a sub-ring of the composed direct product of the $\sh{O}_{X_i,x}$; if $y=f(x)$, it will suffice to show that each of the $\sh{O}_{X_i,x}$ is an $\sh{O}_y$-module \emph{without torsion}, for it will then be of the same for $\sh{O}_{X,x}$; as by hypothesis $\sh{O}_y$ is a regular local ring of dimension $1$, i.e.\ a discrete valuation ring \sref[II]{II.7.1.6}, it will then follow from $(\mathbf{0}_1,~6.3.4)$ that $\sh{O}_{X,x}$ is a \emph{flat} $\sh{O}_y$-module.
...
\begin{enumerate}[label=(\roman*)]
  ...
  With the notations of \sref{IV.11.7.5}, take indeed $Y=\Spec(A)$, $X=\Spec(\overline{A})$, so that $f:X\to Y$ is a morphism \emph{fini surjectif}; as $A$ is a local ring of dimension $1$, and as $\overline{A}$ is a local ring of dimension $1$, it follows immediately from \sref{IV.1.10.4} that the morphism $f$ is \emph{open}.
  However $f$ is not universally open (nor \emph{a fortiori} flat) \sref{IV.11.7.5}.
  One could give an analogous example by taking for $Y$ the local scheme of an algebraic curve having an ``ordinary double point'' and for $X$ the normalised of $Y$.

  \item The example of \sref{IV.14.1.3},~(i)) shows that of a morphism $f$ where the set of points of $X$ where the morphism is universally open is universally open at all the points but the set of points where the morphism is universally open is not necessarily closed, for it is immediate that all the points of $X$ \emph{except} the morphism $f$ is universally open (it is even a local isomorphism).
...
Let $n=\dim(X_0)\geq 0$; using \sref{IV.4.1.1.3}, it follows from \sref{IV.13.3.1} that there exists a neighbourhood $U$ of $X_0$ in $X$ and an $S$-morphism quasi-finite $g:U\to S[\mathrm{T}_1,\ldots,\mathrm{T}_n]=Z$ such that the morphism restriction $g_0:U_0=U\cap X_0\to Z_0=\Spec(k[\mathrm{T}_1,\ldots,\mathrm{T}_n])$ is finite and \emph{surjective}.
By base change to the open $U_1=U\cap X_1$, we deduce a quasi-finite morphism $g_1:U_1\to Z_1=\Spec(K[\mathrm{T}_1,\ldots,\mathrm{T}_n])$.
...
If $\omega$ is a valuation on $K$ associated to $A$, and if $(a_\alpha)$ is the family of coefficients of $\mathrm{F}_1$, after multiplication of $\mathrm{F}_1$ by a non-zero element of $K$, we can suppose that $\inf(\omega(a_\alpha))=0$; in other words, $\mathrm{F}_1$ comes from a polynomial $\mathrm{F}\in A[\mathrm{T}_1,\ldots,\mathrm{T}_n]$ (with which it is identified) such that the image $\mathrm{F}_0$ of $\mathrm{F}$ in $k[\mathrm{T}_1,\ldots,\mathrm{T}_n]$ is \emph{nonzero}.
Consider then \emph{in} $Z$ the closed set $\mathrm{V}(\mathrm{F})$; we have $\mathrm{V}(\mathrm{F})\cap Z_1=\mathrm{V}(\mathrm{F}_1)$, and as $U_1$
\oldpage[IV-3]{208}
...
For every $s\in S$, set $X_s=X\times_S\Spec(k(s))$, $(Y_\lambda)_s=Y_\lambda\times_S\Spec(k(s))$, and let $(u_\lambda)_s:(Y_\lambda)_s\to X_s$ be the morphism $u_\lambda\times 1$ deduced from $u_\lambda$ by base change.
...
\begin{proof}
We know \sref[II]{II.7.1.4} that there exists a valuation ring $A'$ and a morphism $h:S'=\Spec(A')\to S$ such that, if $a$ (resp.\ $b$) is the closed point (resp.\ the generic point) of $S'$, we have $h(a)=s$, $h(b)=s'$.
Furthermore, the morphism projection $p:X_s{}^{\otimes_{k(s)}}k(a)\to X_s$ is surjective and open \sref{IV.2.4.10}, hence for every subset $M$ of $X_s$, $\overline{p^{-1}(M)}$ is therefore equal to the closure of $X_s{}^{\otimes_{k(s)}}k(a)$ by an open equivalence relation; for every $M$, one reasons in the same way for $X_{s'}$, and from \sref[I]{I.3.4.8}, \sref{IV.4.2.7} and from the fact that the $g_\lambda$ are universally open, we see that one can reduce to proving the proposition on the situation obtained after base change $S'\to S$.
Suppose therefore $S'=S$, $s$ being the closed point and $s'$ the generic point of $S$.
The hypothesis that $g_\lambda$ is open implies that every irreducible component of $Y_\lambda$ dominates $S$ \sref{IV.1.10.4}, hence its generic point is a maximal point of $(Y_\lambda)_s$; if $Z$ denotes the closure of $X$ in $Z(s)$, we have therefore $u_\lambda(Y_\lambda)\subset Z$, and consequently $(u_\lambda)_s((Y_\lambda)_s)\subset Z\cap X_s$; in other words, we have $Z(s)\subset Z_s$, where $Z_s=Z\cap X_s$.
...
On the other hand, since $Z(s')\supset Z_{s'}$ by definition, we have $\dim(Z(s'))\geq\dim(Z_{s'})$, which completes the proof.
\end{proof}

\begin{remark}[14.3.12]
\label{IV.14.3.12}
The case considered by M.\ Artin was that where $Y_\lambda=S$ for every $\lambda$, in other words the case where $(u_\lambda)$ is a family of $S$-\emph{sections} of $X$.
...
\begin{enumerate}[label=\emph{\alph*)}]
  \item $f$ is universally open at the point $x$ (or even at every point of the irreducible component of $X_y$ of generic point $x$ \sref{IV.14.3.3.1},~(ii))).
...
The relation $\dim_x(Z\cap X_y)=\dim_x(X_y)$ follows then from \sref{IV.13.1.6}.

It remains to show that $a)$ implies $b'$).
Taking into account \sref[II]{II.7.1.4} and the invariance of the hypotheses and of the conclusion by base change (by virtue of \sref{IV.4.2.7}), we can restrict to the case where $Y$ is the spectrum of a valuation ring, of closed point $y$ and of generic point $y'$, and where $U=X$.
...
\begin{enumerate}[label=\emph{\alph*)}]
  \item $f$ is universally open at $x$ (or equivalently at every point of the irreducible component of $X_y$ with generic point $x$ \sref{IV.14.3.3.1},~(ii)).
\oldpage[IV-3]{210}
...
The fact that \emph{b)} and \emph{b$'$)} are equivalent and that \emph{a)} implies \emph{b$'$)} results from \sref{IV.14.3.13}; on the other hand, if $Y$ is locally Noetherian, we have seen in \sref{IV.14.3} that \emph{c)} implies \emph{b)}.
...
\label{IV.14.4.1.2}
Let $A$ be a local integral ring, unibranch, $B$ an integral ring containing $A$, $\mathfrak{n}$ a prime ideal of $B$ above the maximal ideal $\mathfrak{m}$ of $A$.
...
If $x\in X$ is such that $y=f(x)$ is unibranch over $Y$, then $f$ is open at the point $x$.
\end{lemma}

It suffices to prove that $f(\Spec(\sh{O}_{X,x}))=\Spec(\sh{O}_{Y,y})$ \sref{IV.1.10.3}.
One can thus restrict, by the base change $\Spec(\sh{O}_{X,y})\to Y$, to the case where $Y=\Spec(A)$, where $A$ is a local ring and $y$ is the closed point of $Y$ (taking into account \sref[I]{I.3.6.5} and \sref[0]{0.1.2.1.8}), which proves that $X\times_Y\Spec(\sh{O}_{Y,y})$ is irreducible; replacing $f$ by $f_{\mathrm{red}}$, we may suppose $X$ and $Y$ reduced, hence integral.
Replacing $X$ and $Y$ eventually by affine neighbourhoods of $x$ and $y$ respectively, we may suppose \sref{IV.8.12.9} that the morphism $f$ factorises as $X\xrightarrow{j}X_1\xrightarrow{g}Y$, where $j$ is an \emph{open immersion} and $g$ a \emph{finite} morphism (evidently dominant); as $X$ and $X_1$ are affine, $j$ is affine, hence separated and quasi-compact, and consequently $f$ factorises as $X\xrightarrow{u}X_2\xrightarrow{h}X_1$, where $X_2$ is the closed image of $X$ by $j$, $h$ the canonical injection and $u$ an open immersion \sref[I]{I.9.5.3}.
In other terms, one can suppose that $X_1=\Spec(B)$, or even that $B$ is an $A$-algebra integral and finite, containing $A$ and corresponding to the point $x$; the hypothesis that $A$ is unibranch then implies \sref{IV.14.4.1.2} that the morphism $\Spec(B_\mathfrak{n})\to\Spec(A)$ is surjective.
...
The following conditions are equivalent:
\begin{enumerate}[label=\emph{\alph*)}]
  \item $f$ is universally open at all points of $X_y$ (or, what amounts to the same, at the maximal points of $X_y$ \sref{IV.14.3.3.1},~(ii)).
  ...
  \item[\emph{b$'$)}] For every $x\in X_y$ and every open neighbourhood $U$ of $x$ in $X$, we have
    ...
    \]
  \item[\emph{b$''$)}] For every open set $U$ of $X$, we have $\dim(U\cap X_{y'})\geq\dim(U\cap X_y)$.
\end{enumerate}
\oldpage[IV-3]{212}
When moreover $Y$ is locally Noetherian, these conditions are also equivalent to the following:
\begin{enumerate}[label=\emph{\alph*)}]
  \item[\emph{c)}] $f$ is open at all points of $X_y$ (or, what amounts to the same \sref{IV.14.3.3.1},~(ii), at the maximal points of $X_y$).
\end{enumerate}
The equivalence of \emph{a)} and \emph{c)} when $Y$ is locally Noetherian results from \sref{IV.14.4.1}; the conditions \emph{b)} or \emph{b$'$)}, applied to the \emph{maximal} points of $X_y$, imply \emph{a)} by virtue also of \sref{IV.14.3.3.1},~(iii); finally, \emph{b$'$)} and \emph{b$''$)} are equivalent, since
\[
...
As to assertion (i), it follows from the fact that the hypothesis implies that condition \emph{b)} of \sref{IV.14.4.1} is satisfied at the generic point of every irreducible component of $X_y$ containing $x$, taking into account \sref{IV.13.3.1}; it therefore suffices to apply \sref{IV.14.4.1}.
...
Indeed, \emph{c)} implies trivially \emph{b)}, and \emph{a)} implies \emph{c)} by virtue of \sref{IV.14.4.4}; finally, \emph{b)} implies \emph{a)} by \sref{IV.14.2.2}.
More generally:
...
Condition \emph{a)} implies that $f$ is equidimensional at all the generic points of the irreducible components of $f^{-1}(y)$ containing $x$ \sref{IV.13.3.1}, and consequently \sref{IV.14.4.4} universally open at these points; this proves the last assertion of the proposition, taking into account \sref{IV.14.3.3}) applied to each $f_i$.
Furthermore, by \sref{IV.14.2.1}, we have the relations
\begin{align}
  \dim(\sh{O}_{X_i,x}) &= \dim(\sh{O}_y) + \dim(\sh{O}_{X_y,x}\otimes_{\sh{O}_y}\kappa(y))
...
To show that \emph{a$'$)} implies \emph{a$''$)}, note that we can write $Y''=\Spec(\sh{B})$, where $\sh{B}$ is a quasi-coherent $\sh{O}_Y$-Algebra integral over $\sh{O}_Y$; as $Y$ is Noetherian, $\sh{B}$ is the inductive limit of its sub-$\sh{O}_Y$-Algebras $\sh{B}_\lambda$, which are quasi-coherent and of finite type \sref[I]{I.9.6.6}; but then the $\sh{B}_\lambda$ are \emph{finite} $\sh{O}_Y$-Algebras \sref[II]{II.6.1.2}; we can therefore write $Y''=\varprojlim Y'_\lambda$, where $Y'_\lambda=\Spec(\sh{B}_\lambda)$, with $X'_\lambda=X\times_Y Y'_\lambda$.
By virtue of \emph{a$'$)}, the morphisms $f'_\lambda:X'_\lambda\to Y'_\lambda$ are open at all the points of $X''$ above $x$; we conclude that $f''$ is open at all the points of $X''$ above $x$, by \sref{IV.8.10.1} and \sref{IV.14.3.3.1},~(i).

As the prescheme $Y''$ is \emph{normal} by definition, the fact that \emph{b)} applied to the equidimensional irreducible component of the statement and to the restriction of $f''$ to this component implies \emph{a$''$)} and implies that \emph{b)} is satisfied.
It remains therefore to show that \emph{a$''$)} implies \emph{a)} and \emph{b)}.
...
\]
of the structural morphism, such that $v$ is a \emph{finite} morphism \emph{surjective}, $u$ an integral, \emph{radicial} and dominant morphism; therefore surjective \sref[II]{II.6.1.10} and consequently a universal \emph{homeomorphism} (universal) \sref{IV.2.4.5}.
...
But $Y_1$ is Noetherian and furthermore geometrically unibranch since $Y''$ is normal and it is radicial \sref{IV.6.15.1}; the property \emph{b)} therefore results from \sref{IV.14.4.1}.
...
\oldpage[IV-3]{215}
an open neighbourhood $U$ of $x$ in $X$; as $v$ is closed and surjective, for $f(U)$ to be an open neighbourhood of $y=f(x)$, it is necessary and sufficient that $v^{-1}(f(U))$ be a neighbourhood of $v^{-1}(y)$.
...
\begin{enumerate}[label=\emph{\alph*)}]
  ...
  \item For every point $x''$ of $X''=X\times_Y Y''$, there exists an irreducible component $Z''$ of $X''$ containing $x''$ and equidimensional over $Y''$ at the point $x''$ (cf.\ \sref{IV.14.4.10},~(ii)).
\end{enumerate}
\end{corollary}
...
(ii) One can give examples of morphisms $f:X\to Y$ with the following properties: $Y$ is Noetherian, regular and of dimension~2, $f$ is \emph{universally open} and of finite type, $X$ has two irreducible components $X_1$, $X_2$, but the restriction $X_i\hookrightarrow Y$ of $f$ to one of them is \emph{not a universally open morphism}.
...
On the other hand, let us set $Y_2=\mathbf{P}^1_Y$, and let $f_2:Y_2\to Y$ be the structural morphism; since $f_2$ is flat, hence universally open \sref{IV.2.4.6}; it suffices then to ``glue'' $Y_1$ and $Y_2$ along the fibres isomorphic to $\mathbf{P}^1_{\kappa(y)}$, $f_1^{-1}(y)$ and $f_2^{-1}(y)$, this gives a morphism $f:X\to Y$ where the irreducible components of $X$ identify with $f_1$ and $f_2$, and the restrictions of $f$ to these components are $f_1$ and $f_2$.
...
The question being local on $X$, we can suppose that $f$ is of finite type and \emph{equidimensional}, so that for \emph{every} $z\in Y$, \emph{all} the irreducible components of $f^{-1}(z)$ are of dimension $e$ \sref{IV.13.1.1},~$a''$).
...
\end{equation}
and if the two sides are equal, we necessarily have $\codim(X',X)=n$ and $Z$ contains an irreducible component of $X$, which implies \sref{IV.13.3.1} that $f(X')$ is dense in $Y$ and hence $Z=X$.
...
Then there exists an open neighbourhood $U$ of $x$ in $X$, an affine open set $U$ of $x$ in $X$, and a section $g\in\Gamma(U,\sh{O}_X)$ such that the set $X'$ of $x'\in U$ such that $g(x')=0$ contains $x$ and does not contain any maximal point of $f^{-1}(y)$.
...
Then there exists a closed irreducible subspace $X'$ of $X$, locally closed in $X$, containing $x$, and such that the restriction $X'\to Y$ of $f$ (where $X'$ is the sub-prescheme reduced of $X$ having $X'$ as underlying space) is a \emph{quasi-finite and dominant} morphism.
\end{proposition}

Indeed, with the notation of \sref{IV.14.5.2}, the hypothesis that $x$ is closed in $f^{-1}(y)$ implies that $e-1\geq 1$.
...
\oldpage[IV-3]{218}
  ...
  \item $f$ is open in a neighbourhood of $f^{-1}(y)$.
  \item Every irreducible component of $X$ meeting $f^{-1}(y)$ dominates $Y$.
  \item For every point $x\in f^{-1}(y)$, closed in $f^{-1}(y)$, and every irreducible component $X_i$ of $X$ containing $x$, there exists an irreducible closed subspace $X'$ of $X$, locally closed in $X$, containing $x$, contained in $X_i$, and such that the restriction $X'\to Y$ of $f$ is a quasi-finite and dominant morphism.
\end{enumerate}
\end{proposition}
...
As the restriction $f_i:X_i\to Y$ of $f$ to $X_i$ is a dominant morphism, it follows from \emph{c)} and \emph{d)} for $f_i$ that this morphism is open at the generic points of $X_i\cap f^{-1}(y)$, hence from \sref{IV.14.4.2} that $f_i$ is equidimensional at the point $x$, and we conclude with the aid of \sref{IV.14.5.3}.

\begin{remark}[14.5.7]
\label{IV.14.5.7}
If, in the statement of \sref{IV.14.5.5}, we suppose that $Y=\Spec(A)$, where $A$ is a complete discrete valuation ring, and that $y$ is the closed point of $Y$, we can moreover suppose that $X'=\Spec(A')$, where $A'$ is a discrete valuation ring which is a finite $A$-algebra, as the proof of \sref{IV.14.5.4} shows, and the fact that a local integral ring, regular and of dimension $1$ is a discrete valuation ring \sref[II]{II.7.1.6}.
\end{remark}
...
For every complete discrete valuation ring $A$, of algebraically closed residue field, every morphism $g:Y'=\Spec(A)\to Y$ such that the image by $g$ of the closed point $y'$ of $Y'$ is equal to $y$, and every element $u_0\in X(\Spec(\kappa(y')))$ such that $u_0(y')=x$, there exists a discrete valuation ring $B$, a finite $A$-algebra, and, if we set $Z'=\Spec(B)$, an element $u\in X(Z')$ such that, if $z'$ is the closed point of $Z'$, the diagram
\[
  \xymatrix{
    \Spec(\kappa(z')) \ar[r] \ar[d] & Z' \ar[d]^{u} \\
    \Spec(\kappa(y')) \ar[r]^-{u_0} & X
  }
\]
is commutative.
\end{proposition}
...
\oldpage[IV-3]{219}
Let us note that if $A$ and $B$ satisfy the conditions of the statement, $B$ is a complete discrete valuation ring (Bourbaki, \emph{Alg.\ comm.}, chap.~III, \S~3, n\textsuperscript{o}~3, prop.~7 and chap.~IV, \S~2, n\textsuperscript{o}~2, cor.~3 of the prop.~9) of residue field isomorphic to that of $A$, hence algebraically closed; and since $u(z')=x$, it is in $X''=X\times_Y Z'$ a point $x''$, whose projections in $X$ and $Z'$ are $x$ and $z'$ and which is rational over $\kappa(z')$; furthermore, since $u$ is a $Z'$-section $v$ of $X''$ such that $v(z')=x''$, the image by $v$ of the generic point $s$ of $Z'$ is $s$; applying \sref{IV.14.3.6}, we see that the condition of the statement is \emph{sufficient}.
...
By virtue of \sref{IV.14.3.6}, there is an irreducible component $T$ of $X'$ containing $x'$ and dominant over $Y'$, hence \sref{IV.14.3.8} and \sref{IV.14.3.13} the restriction $T\to Y'$ of $f'$ is equidimensional at the point $x$.
We deduce from \sref{IV.14.5.5} that there is a finite $A$-algebra $B$ which is an integral local ring, integrally closed, and a $Z'$-section $v:Z'\to X''$ such that the image of $v$ is an immersion, and setting $Z'=\Spec(B)$ and $X''=X\times_Y Z'=X'\times_{Y'} Z'$, a $Z'$-section $v:Z'\to X''$ such that the image by $v$ of the closed point $z'$ of $Z'$ is $x'$; the composite morphism $u:Z'\xrightarrow{v}X''\to X'$ gives the answer to the question.

\begin{proposition}[14.5.9]
\label{IV.14.5.9}
Let $Y$ be a locally Noetherian prescheme, irreducible, $f:X\to Y$ a morphism locally of finite type, $y$ a point geometrically unibranch of $Y$.
Then the following conditions are equivalent:
\begin{enumerate}[label=\emph{\alph*)}]
  \item $f$ is universally open at every point of $f^{-1}(y)$.
  \item $f$ is open at every point of $f^{-1}(y)$.
  \item For every irreducible component $Z$ of $f^{-1}(y)$, of generic point $z$, there exists a component of $X_i$ of $X$, containing $z$ and equidimensional over $Y$ at the point $z$.
  \item For every closed point $x$ of $f^{-1}(y)$, there exists an irreducible closed subspace $X'$ of $X$, locally closed in $X$, containing $x$, and such that the restriction $X'\to Y$ of $f$ is a quasi-finite and dominant morphism.
\end{enumerate}
\end{proposition}
...
Conversely, suppose \emph{d)} is satisfied; by the Chevalley criterion \sref{IV.14.4.4}, the restriction $X'\to Y$ of $f$ is a universally open morphism at the point $x$, and \emph{a fortiori} $f$ is open at all the closed points of $X_y=f^{-1}(y)$.
...
We will show that if one can cover $Y$ by open sets $U_j$ in finite number, such that, for every $j$, the conclusion of the proposition holds for the morphism $f^{-1}(U_j)\to U_j$, restriction of $f$, then the conclusion holds also for $f$.
...
Applying once more \sref[0]{0.23.2.5} and \sref{IV.6.15.5}, the morphism $\Spec(B)\to\Spec(A)$ is surjective, and one can therefore evidently replace $Y$ by $\Spec(B)$ and $X$ by $X\otimes_A B$ in order to prove the proposition; the reduction~III) permits then even to suppose from the outset that $A$ is local, integral and $Y=\Spec(A)$ geometrically unibranch.
...
Let $Y=\Spec(A)$, $\mathfrak{p}=\mathfrak{j}_y$, and set $X_1=X\times_Y Y_1$, where $Y_1=\Spec(A_\mathfrak{p})$; by hypothesis a finite surjective morphism $g_1:Y'_1=\Spec(B_1)\to Y_1$ exists, where $Y'_1=\Spec(B_1)$ and $g'=g_{(Y_1)}:Y'_1\to Y_1$, every point of $Y'_1$ admits an open neighbourhood $U_1$ such that there exists a $U_1$-section of $X\times_Y U'_1$.
If $y'_j$ ($1\leq j\leq r$) are the closed points of $Y'_1$, there is therefore a covering of $Y'_1$ by open sets $U'_j$ and a $U'_j$-section $h'_j$ of $X\times_Y U'_j$ ($1\leq j\leq r$).
The $A_\mathfrak{p}$-module $B_1$ admits a finite system of generators of the form $z_i/s$ (with $s\in A-\mathfrak{p}$,
\oldpage[IV-3]{221}
$z_i$ integral over $A$), which one may suppose (multiplying as needed $s$ by an element of the field of fractions of $B_1$, integral over $A$, as an element of $A_s$), so that if $V$ is the open affine set $D(s)=\Spec(A_s)\subset Y$, $Y'_1$ identifies with $Y_1\times_Y V'$, where $V'$ is the spectrum of the finite $A_s$-algebra generated by the $z_i/s$; $g:V'\to V$ is therefore a finite surjective morphism and $g_1=g_{(Y_1)}$.
Moreover, applying the method of \sref{IV.8.1.2},~$a$), one can suppose that each of the $U'_j$ is the inverse image by $p:Y'_1\to Y'_1$ of an open set $W'_j$ of $V'$, and that each of the sections $h'_j$ is of the form $(v'_j)_{(Y_1)}$, where $v'_j$ is a $W'_j$-section of $X\times_Y W'_j$ \sref{IV.8.8.2},~(i).
It is therefore clear that the proposition holds when $Y=\Spec(A)$, $A$ being a local integral Noetherian ring.
We then know that there is a finite $A$-algebra $B$, with $A\subset B$ and such that $\Spec(B)$ is geometrically unibranch \sref[0]{0.23.2.5} and \sref{IV.6.15.5}; as the morphism $\Spec(B)\to\Spec(A)$ is surjective, one may therefore evidently replace $Y$ by $\Spec(B)$ and $X$ by $X\otimes_A B$ to prove the proposition; the reduction~III) therefore allows even to suppose from the outset that $A$ is local, integral and $Y=\Spec(A)$ geometrically unibranch.
...
There exists then a reduced irreducible sub-prescheme $X_0$ of $X$ such that the restriction $f_0:X_0\to Y$ of $f$ is a quasi-finite and \emph{dominant} morphism and that $f_0(X_0)$ contains the closed point $y$ of $Y$ \sref{IV.14.5.9}; as $Y$ is geometrically unibranch, it follows from \sref{IV.14.4.1} that $f_0$ is still \emph{universally open}.
Furthermore we may suppose $X_0$ reduced, hence integral, and we see that we may, by replacing $X$ by $X_0$, suppose that $X$ is integral and $f$ \emph{quasi-finite and dominant}, and such that $f(X)$ contains $y$; as $f$ is \emph{open} and surjective.

Let $\xi$, $\eta$ be the generic points of $X$ and $Y$ respectively; $\kappa(\xi)$ is therefore a finite extension of $K=\kappa(\eta)$.
By \sref{IV.4.6.8}, there is a \emph{finite} extension $K'$ of $K$ such that $(\Spec(\kappa(\xi)\otimes_K K'))_{\mathrm{red}}$ is geometrically reduced over $K$ and that its irreducible components are geometrically irreducible; it follows \sref{IV.4.5.9} and \sref{IV.4.6.1} that the residue fields of $\Spec(\kappa(\xi)\otimes_K K')$ are finite extensions, primary and separable of $K'$.
...
If we set $X_1=X\times_Y Y_1=X'\times_{Y'}Y'_1$, the canonical projection $p:X_1\to X$ is a finite surjective morphism, hence, if we prove that $X_1$ is a scheme, it will follow first of all that $X$ is quasi-compact, hence Noetherian, then that $X$ is affine by virtue of the theorem of Chevalley \sref[II]{II.6.7.1}.
It suffices therefore to prove that the morphism $f_1=f_{(Y_1)}:X_1\to Y_1$ is affine.
Or, if we set
\[
  X'_1=X_1\times_{Y_1}Y'_1=X'\times_{Y'}Y'_1 \qquad\text{and}\qquad f'_1=(f_1)_{(Y_1)}=f'_{(Y'_1)}:X'_1\to Y'_1,
\]
$f'_1$ is affine by virtue of the hypothesis.
We are therefore reduced to proving the corollary in $y$ by replacing $Y$, $X$, $f$ and $Y'$ by $Y_1$, $X_1$, $f_1$ and $Y'_1$; in other words, it suffices to prove that $f$ is affine when, in the statement of \sref{IV.14.5.11}, the hypothesis that every point of $Y$ admits an open neighbourhood $U$ such that there exists a $U$-section of $g^{-1}(U)$.
Or, we have the following elementary lemma (valid in any category admitting fibre products):
