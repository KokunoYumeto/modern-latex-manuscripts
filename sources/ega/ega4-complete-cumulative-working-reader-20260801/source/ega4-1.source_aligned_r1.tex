...
\begin{proof}
Assertion~\hyperref[IV.1.4.3]{(i)} is trivial.
To prove~\hyperref[IV.1.4.3]{(iii)}, we may limit to the case where $S=Y$ \sref[I]{I.3.3.11}; let $x'$ be a point of $X_{(S')}$, $s'$ and $x$ its projections on $S'$ and $X$ respectively, $s$ the common projection of $s'$ and $x$ on $S$.
...
To prove~\hyperref[IV.1.4.3]{(ii)}, consider a point $x\in X$, and let $y=f(x)$, $z=g(f(x))$.
...
If we know that $\Delta_g$ is locally of finite presentation, it will follow by~\hyperref[IV.1.4.3]{(iii)} that so is $\Gamma_f$.
On the other hand, we have the factorisation of $f$ as $X\xrightarrow{\Gamma_f} X\times_Z Y\xrightarrow{p_2} Y$ \sref[I]{I.5.3.13}, where the projection $p_2$ is equal to $(g\circ f)\times_Z 1_Y$; as $g\circ f$ is assumed locally of finite presentation, so is $p_2$ by~\hyperref[IV.1.4.3]{(iv)}, and it will follow finally from~\hyperref[IV.1.4.3]{(ii)} that $f$ is also locally of finite presentation.
...
\begin{proof}
This follows immediately (except for the case~\hyperref[IV.1.5.4]{(v)} where $X$ is Noetherian, proved in \sref[I]{I.6.3.6}) from the definitions and from \sref{IV.1.1.2}, \sref{IV.1.2.4}, and \sref{IV.1.3.4}.
\end{proof}
...
\begin{enumerate}
  ...
  \item[\rm(ii)] $f$ is quasi-compact (which, when~\hyperref[IV.1.6.1]{(i)} is satisfied, is equivalent to saying that $f$ is of finite type~\sref{IV.1.5.2});
...
Condition~\hyperref[IV.1.6.1]{(iii)} is automatically satisfied if $f$ is separated, or if $X$ is locally Noetherian~\sref{IV.1.2.8}; when $Y$ is locally Noetherian, it amounts to the same to say that $f$ is \emph{of finite presentation} or that $f$ is \emph{of finite type} (the latter condition implying that $X$ is locally Noetherian~\sref{IV.1.3.3}).
...
\begin{remark}[1.6.4]
\label{IV.1.6.4}
In the definition~\sref{IV.1.6.1}, condition~\hyperref[IV.1.6.1]{(iii)} is not a consequence of the two others.
...
It is clear that $f$ satisfies condition~\hyperref[IV.1.6.1]{(i)} of~\sref{IV.1.6.1}, being a local isomorphism; it also satisfies~\hyperref[IV.1.6.1]{(ii)}, the inverse image of an open quasi-compact subset of $Y$ being the union of its images in $Y_1$ and $Y_2$; but since $Y_1\cap Y_2$ is not quasi-compact, $f$ is not quasi-separated~\sref{IV.1.2.7}.
\end{remark}
