...
\]
If $E$ (resp.~$E_0$) is the set of $x\in X$ (resp.~$x_0\in X_0$) for which $(\sh{H}_n((\sh{L}_\bullet)_{f(x)}))_x=0$ (resp.~$(\sh{H}_n((\sh{L}_\bullet^{(0)})_{f(x_0)}))_{x_0}=0$), then $E=h^{-1}(E_0)$, where $h:X\to X_0$ is the canonical projection.
