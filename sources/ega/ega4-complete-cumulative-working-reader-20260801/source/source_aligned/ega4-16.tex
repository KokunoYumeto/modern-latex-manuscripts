...
\oldpage[IV-4]{6}
projective system of sheaves of rings on $Y$; for $n \leq m$, the transition homomorphism $\vphi_{nm}:\sh{O}_{Y^{(m)}} \to \sh{O}_{Y^{(n)}}$ identifies $\sh{O}_{Y^{(n)}}$ with the quotient of $\sh{O}_{Y^{(m)}}$ by the power $(\sh{I}_f/\sh{I}_f^{n+1})^m$ of the \emph{augmentation ideal} of $\sh{O}_{Y^{(n)}}$, the kernel of $\vphi_{0n}: \sh{O}_{Y^{(n)}} \to \sh{O}_{Y}$.
...
\medskip\noindent
\begin{enumerate}
  \item[(i)] First note that $f':Y'\to X'$ and $u:Y'\to Y$ jointly identify $Y'$ with the product $Y\times_X X'$ (for the structure morphisms $f:Y\to X$ and $v:X'\to X$) \sref{IV.14.5.11.1}.\footnote{The printed source cites (14.5.12.1); the relevant product lemma is (14.5.11.1).}
...
\medskip\noindent
\begin{enumerate}
  \item[(i)] The reasoning of \sref{IV.16.2.2}[(i)] still applies when the maps in \sref{IV.16.2.1.1} are morphisms of \emph{locally ringed spaces} ($\mathbf{Err_{III}}$, \sref[I]{I.1.8.2}).
...
this follows readily from the definitions of $u$ \sref{IV.16.4.8.2} and $\delta$ \sref{IV.16.4.7.1}, together with the definitions of the $\sh{O}_X$-algebra structure on $\sh{P}_{X/S}^n$ and of $d_{X/S}^n$ \sref{IV.16.3.5} and \sref{IV.16.4.3.6}.
...
\label{IV.16.4.11}  
Let $f:X \to S$ be a morphism, let $g: S \to X$ be an $S$-section of $X$, and let $S^{(n)}$ be the $n$-th infinitesimal neighbourhood of $S$ with respect to the immersion $g$ \sref{IV.16.1.2}.
...
Indeed, for every $s \in S$, there is an open neighbourhood $W$ of $s$ in $S$ such that $g(W)$ is closed in an open set $U$ of $X$, and it is clear that $g|W$ is a $W$-section of the morphism $U \cap f^{-1}(W) \to W$, the restriction of $f$.
...
\label{IV.16.5.8.1}
  ...
  \tag{16.5.8.1}
\]
Every $\Gamma(S, \sh{O}_S)$-derivation of the $\Gamma(S, \sh{O}_S)$-algebra $\Gamma(U, \sh{O}_X)$ is therefore written uniquely as
\[
...
Then $\sh{P}$ has the structure of a pseudo-torsor under the $\sh{O}_{Y_0}$-module $\sh{G} = \shHom_{\sh{O}_{Y_0}}(u_0^*(\Omega_{X/S}^1), \sh{I})$.
\end{proposition}
...
Now (Bourbaki, \emph{Alg.}, chap.~III, 3\textsuperscript{rd} ed., \textsection10), to define an $A$-antiderivation $D$ of degree $+1$ on an exterior algebra $\bigwedge(M)$ (where $M$ is a $B$-module and $B$ an $A$-algebra), with values in a \emph{graded anticommutative} $A$-algebra $C = \bigoplus_{n=0}^\infty C_n$ whose elements of degree $1$ are square-zero, it suffices to choose arbitrarily an $A$-derivation $D_0$ from $B$ to $C_1$ and an $A$-homomorphism $D_1$ from $M$ to $C_2$;
...
Now, since $z = 1 \otimes (xy) + (xy) \otimes 1 - x \otimes y - y \otimes x$, formula \sref{IV.16.6.2.4} shows that it is 
\oldpage[IV-4]{36}
...
Now this element can be written as $\sum_{H \subset I_{n+1}} (\prod_{i \in H} a_i) \otimes ((\prod_{i \notin H} a_i ) t )$, and its image under $D'$ is the left-hand side of \sref{IV.16.8.8.2}; this proves the equivalence of (a) and (c).
...
\begin{proof}[Proof of Lemma 16.8.9.1]
...
\begin{proof}
The two conditions in the statement merely express the definition given in \sref[0]{0.15.2.2}, taking into account the definition of the canonical homomorphisms \sref{IV.16.2.1.1}.
...
\begin{env}[16.9.7]
\label{IV.16.9.7}
If $X = \Spec(A)$ is an affine scheme, we say that an ideal $\mathfrak{I}$ of $A$ is \emph{regular} (resp. \emph{quasi-regular}) if the ideal $\sh{J} = \widetilde{\mathfrak{I}}$ of $\sh{O}_X$ is regular (resp. quasi-regular).
...
\begin{proof}
  \begin{enumerate}
    ...
    Conversely, if $\sh{I}_y$ is generated by a regular sequence $(s_i)_y$, where the $s_i$ are sections of $\sh{I}$ over an open neighbourhood $U$ of $y$ in $X$, we may suppose that the $s_i$ generate $\sh{I}|U$ \sref[0]{0.5.5.2} and form a regular sequence \sref[0]{0.15.2.4}, whence the assertion.
...
Put $A = \sh{O}_{X,x}$. We may write $\sh{O}_{Y,x} = A' = A/\mathfrak{I}$, where $\mathfrak{I}$ is an ideal contained in the maximal ideal of $A$, and then $\sh{O}_{Y',x} = A'/\mathfrak{K}'$, where $\mathfrak{K}'$ is generated by an \emph{$A'$-regular} sequence of elements of $A'$ which are the images of elements of an $A$-regular sequence in $A$ belonging to the maximal ideal of $A$.
...
It follows from \sref[0]{0.20.4.7} that every $x \in X$ has an open neighbourhood such that there is a finite family $(z_\lambda)_{\lambda \in L}$ of sections of $\sh{O}_X$ over $U$ for which $(dz_\lambda)_{\lambda \in L}$ forms a \emph{basis} of the $\Gamma(U, \sh{O}_X)$-module $\Gamma(U, \Omega_{X/S}^1)$.
\end{remark}
