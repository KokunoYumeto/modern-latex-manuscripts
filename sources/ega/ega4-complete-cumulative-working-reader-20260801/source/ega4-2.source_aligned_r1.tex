...
\label{IV.2.3.2}
Let $S$ be a prescheme, $f:X\to Y$ a quasi-compact and quasi-separated $S$-morphism, and let $Z$ be the sub-prescheme of $Y$ which is the closed image of $X$ under $f$~\textnormal{(}\sref[I]{I.9.5.3} and~(1.7.8)\textnormal{)}.
...
\]
of~\sref{IV.2.3.1.1}.
The conclusion follows from~\sref{IV.2.3.1} and~\sref[I]{I.9.5.2}, since $f$ is quasi-compact and quasi-separated~(1.1.2 and 1.2.2).
\end{proof}
...
\begin{enumerate}
  \item[$1^\circ$] $B$ is an $A$-algebra essentially of finite type~(1.3.8).
...
\begin{remark}[2.3.9]
\label{IV.2.3.9}
We will see further on~(7.8.3, (vii)) that for the Noetherian local rings $A$ most commonly used in Algebraic Geometry, the hypothesis that $A$ is integral and integrally closed implies that the same is true of $\widehat{A}$.
...
Then $f\circ j$ is quasi-compact~(1.1.2), so $Z=f(F)$ is pro-constructible in $Y$~(1.9.5,~(vii)), and the corollary follows from the fact that $F$ is closed.
