...
If $A\neq 0$, this amounts to saying that $A$ \emph{contains a field of characteristic $p$}, the image of $P$ being necessarily the only sub-field of $A$ isomorphic to $P$ (and moreover the only sub-field of $A$ isomorphic to $P$).
\end{env}
...
We set $A^p=\mathrm{F}_A(A)$ (sub-ring of $A$ formed by the $x^p$ for $x\in A$); one can naturally consider $A$ as an $A^p$-algebra.
...
It is clear that for every ring homomorphism $u:A\to B$ the couple $(u,u)$ is a \emph{di-homomorphism} of $A$-algebras $A^{(p)}\to B^{(p)}$.
...
When $\mathrm{F}_A$ is \emph{injective}, one can identify $A^{(p)}$ as an algebra over its \emph{sub-ring} $A^p$.
...
The conclusion results from the fact that the elements $1\otimes x-x\otimes 1$ generate $\mathfrak{J}_{B/A}$ \sref{0.20.4.4}.
\end{proof}

It results immediately from \sref{0.21.1.5} that one has, for every pair of ring homomorphisms $A\to B\to C$,
\begin{equation}
\tag{21.1.5.1}
  \label{0.21.1.5.1}
...
\label{0.21.1.6.1}
Let $R$ be a ring, $S$ an $R$-algebra finite; if the canonical homomorphism $\pi:S\otimes_R S\to S$ is bijective, then the structural homomorphism $u:R\to S$ is surjective.
\end{lemma}

\begin{proof}
It suffices to show that for every maximal ideal $\mathfrak{m}$ of $R$, if one sets $T=R-\mathfrak{m}$, the homomorphism $u_\mathfrak{m}:R_\mathfrak{m}\to T^{-1}S$ is surjective (Bourbaki, \emph{Alg.\ comm.}, chap.~II, \S\,3, n$^\circ$\,3, th.~1); or the hypothesis implies that the homomorphism $(T^{-1}S)\otimes_{R_\mathfrak{m}}(S/\mathfrak{m}S)\to T^{-1}S$ is bijective, and as $T^{-1}S$ is an $R_\mathfrak{m}$-algebra finite, one sees that one can reduce to the case where $R$ is a local ring.
...
The fact that a) implies c) results from \sref{0.21.1.5}, (ii) and \sref{0.20.4.7}.

Suppose conversely c) verified and that $B$ is an $A[B^p]$-algebra of finite type; let $B'$ be the sub-$A[B^p]$-algebra of $B$ generated by the $x_\alpha$; in the exact sequence \sref{0.20.5.7.1}
\[
...
When one takes for $A$ the \emph{prime field} $\bb{F}_p$ (in which case $A[B^p]=B^p$), one omits the mention of $A$ in the preceding definition (or says also that a family is ``absolutely'' $p$-free, resp.\ an absolute system of $p$-generators, resp.\ an absolute $p$-base).
...
\begin{enumerate}
  ...
  \item[\rm(ii)] Suppose that $M$ is a $p$-base of $C$ over $A$; then, for $N$ to be $p$-free over $C$, it is necessary and sufficient that $M\cup N$ be $p$-free over $A$.
\end{enumerate}
\end{lemma}
...
\label{0.21.2.1}
Let $B$ be a ring, $A$ a sub-ring of $B$ such that $B^p\subset A\subset B$.
Let $E$ be an $A$-algebra, $u:A\to E$ the structural homomorphism, $q:E\to B$ an $A$-homomorphism surjective, $\mathfrak{R}$ its kernel, and suppose that $\mathfrak{R}^p=0$.
Then:
\begin{enumerate}
...
The sufficiency of condition (i) will result from (ii).
...
But, one can write $(\prod_\alpha x_\alpha^{m(\alpha)})(\prod_\alpha x_\alpha^{n(\alpha)})=a.\prod_\alpha x_\alpha^{r(\alpha)}$, where $r(\alpha)=m(\alpha)+n(\alpha)$ if $m(\alpha)+n(\alpha)<p$, $r(\alpha)=m(\alpha)+n(\alpha)-p$ in the contrary case, and $a\in A$ is the product of the $x_\alpha^p$ for the $\alpha\in I$ such that $m(\alpha)+n(\alpha)\geq p$; it suffices to see that $v$ is a ring homomorphism, and by construction, on has $f(q(z)^p)=z^p$ for every $z\in E$, the condition (i) of \sref{0.21.2.1} is therefore satisfied by construction, and by the application of \sref{0.21.2.1}, one is therefore in the conditions of application of the theorem in this case.
...
For every couple $(\alpha,\lambda)$, denote by $z_{\alpha\lambda}$ the canonical image of $z_\alpha$ in $E_\lambda$, and by $p_\lambda:C_\lambda\to C_\lambda$ the canonical homomorphism and by $q_\lambda:E_\lambda\to C_\lambda$ the canonical homomorphism.
...
But one has $u(q(z)^p)=v(q(z)^p)=(v(q(z)))^p$; but by the relation $L^2=0$, one also has $L^p=0$, and as $v(q(z))-z\in L$, on has $(v(q(z)))^p=z^p$; the condition of \sref{0.21.2.1}, (i) is therefore satisfied, and $E$ is $B$-trivial.
\end{proof}

\begin{corollary}[21.2.3]
\label{0.21.2.3}
Let $B$ be a ring, $A$ a sub-ring of $B$ such that $B^p\subset A\subset B$, $L$ a $B$-module.
Then:
\begin{enumerate}
...
\begin{proof}
This results immediately from \sref{0.21.2.3} and from the formula $\Der_A(B,L)=\Hom_B(\Omega^1_{B/A},L)$.
\end{proof}
...
One assumes the following conditions verified:
\begin{enumerate}
...
Indeed, one can reduce to the case where $\mathfrak{R}^2=0$; as $\Der_A(B,\mathfrak{R})=\Hom_B(\Omega^1_{B/A},\mathfrak{R})$ by \sref{0.20.4.8}, there exists an $A$-derivation $D$ of $B$ in $\mathfrak{R}$ and a unique one such that $D(x_\alpha)=z'_\alpha$ for every family $(z'_\alpha)$ of elements of $\mathfrak{R}$; the conclusion results from \sref{0.20.1.1}.
\end{remark}
...
\end{align}
Most often, $i$ will be \emph{injective}, so that $A$ is identified with a sub-ring of $B$; once this identification made, the existence of $j$ implies that $B^p\subset A$, and $j$ is then identified with $\mathrm{F}_B$.
\end{env}
...
\end{equation}
so that the couple $(j,\mathrm{F}_A)$ can be considered as a \emph{di-homomorphism} of the $A$-algebra $B$ (for $i$) in the $A^p$-algebra $A$ (for $h$).
...
\begin{proof}
This results from the definition of flatness.
The proposition \sref{0.21.3.4} applies in particular when $B^p\subset A\subset B$, $i$ and $j$ being respectively the canonical injection and $\mathrm{F}_B$ (so that $B$ is then an $A$-module libre).
...
\label{0.21.3.5}
Let $A$ be a \emph{reduced} ring, $B$ a sub-ring of $A$ such that $B^p\subset A\subset B$.
...
\]
\sref{0.20.5.5}; one deduces an $A'$-isomorphism canonical
\begin{equation}
\tag{21.3.6.1}
  \label{0.21.3.6.1}
...
\begin{env}[21.4.0]
\label{0.21.4.0}
Let $K$ be a field of characteristic $p>0$, $k$ a sub-field of $K$; then the ring $k[K^p]$ is equal to the field $k(K^p)$ since $k$ is algebraic over $K^p$.
...
It suffices to see that the sub-field $K'$ of $K$ generated by $k$ and $B$ is equal to $K$.
In the contrary case, there would exist $x\in S$ not in $K'$; as $K^p\subset k$, one has $K'=K'(K^p)$; hence $x$ would be $p$-free over $K'$ \sref{0.21.4.1}, and hence $B\cup\{x\}$ would be $p$-free over $k$ \sref{0.21.1.10}, contradicting the maximality of $B$.
...
Let $K'$ be the sub-field (equal to the sub-ring) of $K$ generated by $k(K^p)$ and the $x_\alpha$; taking into account \sref{0.21.4.3}, it suffices to see that, for each $\alpha$, $x_\alpha$ does not belong to $K_\alpha$.
...
\label{0.21.4.7}
Let $L$ be a field of characteristic $p>0$, $K$ a sub-field of $L$ such that $L^p\subset K\subset L$.
...
Indeed, this results from \sref{0.19.8.6}, (i) applied to the composed homomorphism $v':A\to A/\mathfrak{m}^2\xrightarrow{u'_1} B/\mathfrak{n}^2$;
...
\begin{enumerate}
  ...
  \item[(ii)] In the same way, let $W$ be a Cohen ring, $C$ a complete local noetherian ring, $\mathfrak{J}$ a nonzero ideal of $C$ contained in the maximal ideal; for the factorization $W\xrightarrow{v} C\to C/\mathfrak{J}$ in \sref{0.19.8.6}, (i) to be \emph{unique}, it is necessary and sufficient that the residue field $K$ of the Cohen ring $W$ be such that $\Omega^1_K=0$.
    ...
    If on the contrary $\Omega^1_K=0$, the uniqueness of $v$ already results from (i) when $W$ is a field of characteristic $0$.
    In the contrary case, one has $\widehat{\Omega}^1_W=0$; in fact, one then has $\mathfrak{m}=pW$, $p$ being the characteristic of $K$ \sref{0.19.8.5}, and the canonical homomorphism $\mathfrak{m}/\mathfrak{m}^2\to\Omega^1_W/\mathfrak{m}\Omega^1_W$ \sref{0.20.5.12.1} applied to $W$ and to $K=W/\mathfrak{m}$ implies that $\Omega^1_W=\mathfrak{m}\Omega^1_W$, hence by Nakayama's lemma.
...
\begin{env}[21.6.1]
\label{0.21.6.1}
Given four fields $k_0\subset k\subset K\subset L$, it results from \sref{0.20.6.16} and \sref{0.20.6.17} that one has an exact sequence
\begin{equation}
\tag{21.6.1.1}
  \label{0.21.6.1.1}
...
\begin{proof}
\begin{enumerate}
  \item[(i)] The assertions result trivially from the exactness of the sequence \sref{0.21.6.1.1}.
...
\]
It then results from \sref{0.20.6.17}, (ii) that one has a canonical homomorphism
\begin{equation}
\tag{21.6.3.1}
  \label{0.21.6.3.1}
...
This results immediately from the relations \sref{0.21.6.4.2} and \sref{0.21.6.4.4}, the values of $d$ being $\geqslant 0$.
\end{corollary}
...
\]
Since moreover $\deg.\operatorname{tr}_K L=\deg.\operatorname{tr}_K L'+\deg.\operatorname{tr}_{L'} L$, one sees, by recurrence on the number of generators of the extension $L$, that one is reduced to proving \sref{0.21.7.1.1} when $L=K(x)$.
...
\emph{c)} $L$ is a finite algebraic inseparable extension of $K$; the reasoning at the beginning shows that one can restrict to the case where $L^p\subset K$; it then results from \sref{0.21.4.8} that one has $\operatorname{rg}_L\Upsilon_{L/K}=\operatorname{rg}_L\Omega^1_{L/K}$, hence again \sref{0.21.7.1.1}.
\end{proof}

\begin{corollary}[21.7.2]
\label{0.21.7.2}
Let $K$ be a field, $L$ a finite type extension of $K$, $k$ a sub-field of $K$.
...
\begin{proof}
Indeed, as the homomorphism $\Upsilon_{L/K}\to\Upsilon_{L/K/k}$ is surjective, one has by definition \sref{0.21.6.1.2} $\operatorname{rg}\Upsilon_{L/K}-\operatorname{rg}\Upsilon_{L/K/k}=d(L/K,k)$, and the corollary results immediately from \sref{0.21.7.1} and from \sref{0.21.6.2}.
\end{proof}
...
We shall see moreover below \sref{0.21.8.6} that for every finite type extension $L$ of $K$, there exists a sub-field $k$ of $K$ such that $[K:k]$ is finite, and which is admissible for the extension $L$ of $K$, so that the two members of \sref{0.21.7.3.2} are then equal.
...
\begin{remark}[21.7.5]
\label{0.21.7.5}
By virtue of \sref{0.20.6.3.2} and ($\mathbf{0_{III}}$, 11.10.2) the first member of \sref{0.21.7.1.1} is none other than the \emph{Euler-Poincar\'e characteristic} of the complex $\mathrm{K}_\bullet(C/B/A)$ introduced in \sref{0.20.6.3}, for $C=L$, $B=K$ and $A=P$ (prime field of $K$).
...
\begin{lemma}[21.8.1]
\label{0.21.8.1}
Let $K$ be a field, $k$ a sub-field of $K$, $(k_\alpha)_{\alpha\in I}$ a decreasing filtering family of sub-fields of $K$ such that $k=\bigcap_\alpha k_\alpha$.
...
Let $r$ be the rank of the family $(a_i)_{1\leqslant i\leqslant n}$ over $K$, and let us reason by recurrence on $n-r$; the proposition is evident for $n=r$, for the family $(a_i)_{1\leqslant i\leqslant r}$ is then free over $K$, hence over every sub-field of $K$.
...
It now suffices to apply the recurrence hypothesis by replacing $K$ by $k_\beta$ and the family $(k_\alpha)_{\alpha\in I}$ by the sub-family of the $k_\alpha$ contained in $k_\beta$.
\end{proof}

\begin{lemma}[21.8.2]
\label{0.21.8.2}
Let $K$ be a field, $p$ its exponential characteristic, $k_0$ a sub-field of $K$, $(k_\alpha)_{\alpha\in I}$ a decreasing filtering family of sub-fields of $K$ such that $\bigcap_\alpha k_\alpha(K^p)=k_0(K^p)$.
...
\begin{proof}
Indeed, to say that the family $(x_i)$ is $p$-free over a sub-field $k$ of $K$ means that the finite family of monomials $\prod_{i=0}^n x_i^{m(i)}$ with $0\leqslant m(i)<p$ is free over $k(K^p)$; it suffices therefore to apply the lemma \sref{0.21.8.1} to this family of monomials in the vector space $V=K$, and to the sub-fields $k_\alpha(K^p)$ and $k_0(K^p)$ of $K$.
\end{proof}

\begin{theorem}[21.8.3]
\label{0.21.8.3}
Let $K$ be a field of characteristic $p>0$, $k_0$ a sub-field of $K$, $(k_\alpha)_{\alpha\in I}$ a decreasing filtering family of sub-fields of $K$ containing $k_0$.
The following conditions are equivalent:
\begin{enumerate}
  \item[\rm a)] $\bigcap_\alpha k_\alpha(K^p)=k_0(K^p)$.
  \item[\rm b)] For every extension $L$ of $K$ such that $\Upsilon_{L/K/k_\alpha}$ is an $L$-vector space of finite rank \textup{(}which holds in particular if $L$ is a finite type extension in virtue of \sref{0.21.7.2}\textup{)}, there exists $\alpha\in I$ such that $k_\alpha$ is $k_0$-admissible for the extension $L$ of $K$.
  ...
  \item[\rm c)] The canonical application
  \begin{equation}
  \tag{21.8.3.1}
  \label{0.21.8.3.1}
...
\begin{proof}
The canonical application \sref{0.21.8.3.1} is understood as passage to the projective limit in the projective system of homomorphisms $\Omega^1_{K/k_0}\to\Omega^1_{K/k_\alpha}$ \sref{0.20.5.3.3}.
...
Or, one has the commutative diagram
\[
  \xymatrix{
...
To prove that $c)$ implies $b)$, it suffices therefore to show that the canonical application
\begin{equation}
\tag{21.8.3.2}
  \label{0.21.8.3.2}
...
On the other hand, for every element $z$ of the first member of \sref{0.21.8.3.2} there exists a sub-space $V'$ of $V$ of finite rank containing $z$ (identified canonically with a sub-space of $\Omega^1_{K/k_0}\otimes_K V'$); if $z\neq 0$, its image $\varprojlim(\Omega^1_{K/k_\alpha}\otimes_K V')$ is also non-zero; as the $\varprojlim$ functor is exact on the left, the image of $z$ in the second member of \sref{0.21.8.3.2} is therefore also $\neq 0$, which finishes the proof that $c)$ implies $b)$.

It is trivial that $b)$ implies $b')$; let us show that $b')$ implies $a)$.
Using the notations of $b')$, on can suppose $L\neq K$.
It then results from \sref{0.21.4.7} that one has $\operatorname{rg}\Upsilon_{L/K}\leqslant 1$.
If $\Upsilon_{L/K/k_\alpha}=0$ there is nothing to prove by virtue of \sref{0.21.6.1.1}.
Otherwise, $\Upsilon_{L/K/k_\alpha}$ is identified canonically with $\Upsilon_{L/K}$, and if one sets $a=x^p$, $\Upsilon_{L/K}$ is a one-dimensional space generated by the single element $d_K(a)\otimes 1$ \sref{0.21.4.7}; $\Upsilon_{L/K/k_\alpha}$ is thus a one-dimensional space generated by the single element $d_{K/k_\alpha}(a)\otimes 1$, i.e., to say that a sub-field $k_\alpha$ is $k_0$-admissible for the extension $L$ of $K$ means that one has $d_{K/k_\alpha}(a)\neq 0$, or equivalently \sref{0.21.4.6} that $a\notin k_\alpha(K^p)$.
...
\label{0.21.8.4}
Let $K$ be a field of characteristic $p>0$, $k_0$ a sub-field of $K$, $(k_\alpha)_{\alpha\in I}$ a decreasing filtering family of sub-fields of $K$, containing $k_0$, such that
\[
...
Let $L$ be an extension of $K$ such that $\Upsilon_{L/K/k_0}$ is an $L$-vector space of finite rank; then there exists a sub-field $k$ of $K$, containing $k_0(K^p)$, such that $[K:k]$ is finite, and which is $k_0$-admissible for the extension $L$ of $K$.
\end{corollary}

\begin{proof}
It suffices indeed, by virtue of \sref{0.21.8.3}, to construct a decreasing filtering family $(k_\alpha)_{\alpha\in I}$ of sub-fields of $K$, containing $K^p$ and $k_0$, for which $[K:k_\alpha]<+\infty$ and $\bigcap_\alpha k_\alpha=k_0(K^p)$.
For this one considers a $p$-base $(x_\lambda)_{\lambda\in J}$ of $K$ over $k_0$ and, for every finite subset $H$ of $J$, one considers the sub-field $k_H$ of $K$ generated by $k_0(K^p)$ and the $x_\lambda$ for $\lambda\in J-H$; it results from this definition that $(x_\lambda)_{\lambda\in H}$ is a $p$-base of $K$ over $k_H$, and one concludes immediately that the $k_H$ satisfy the required conditions.
\end{proof}

\begin{remarks}[21.8.7]
\label{0.21.8.7}
\begin{enumerate}
  \item[(i)] One has already seen \sref{0.21.7.2} that if $L$ is a \emph{finite type} extension of $K$, $\Upsilon_{L/K/k_0}$ is of finite rank for every sub-field $k_0$ of $K$.
    It is the same if $L$ is a \emph{separable} extension of $K$, for in virtue of \sref{0.20.6.19}, one has $\Upsilon_{L/K/k_0}=0$.
    Finally, if $L$ is a \emph{finite type extension of a separable extension $L_0$ of $K$}, the same reasoning as in \sref{0.21.8.5} shows that $\Upsilon_{L/K/k_0}$ is still of finite rank (and in fact is isomorphic to a sub-space of $\Upsilon_{L/L_0/k_0}$).
  \item[(ii)] In the statement of \sref{0.21.8.5}, and consequently in that of \sref{0.21.8.3}, $b)$), one can omit the hypothesis that $\Upsilon_{L/K/k_0}$ is of finite rank over $L$.
    Let us take for $k_0$ the prime field $\mathbf{F}_p$, for $K$ a field such that $[K:K^p]$ is countably infinite (for example the field of rational fractions $\mathbf{F}_p(X_1,\ldots,X_n,\ldots)$ with a countable infinity of indeterminates);
    the construction procedure of \sref{0.21.8.6} then shows that there exists an infinite strictly decreasing sequence $(K_n)$ of sub-fields of $K$ such that $K_0=K$ and $\bigcap_n K_n=K^p$.
    ...
    Namely, take an element $x$ of $K-K^p$, set $M_0=K^p$, and $M_n=K^p(a_1 x,a_2 x,\ldots,a_n x)$ for $n\geqslant 1$, where the $a_n$ are constructed by recurrence so that $a_n\in K_n$ and $a_{n+1}\notin M_n(x)$ for all $n$: this is possible, since $M_n(x)$ is of finite degree over $K^p$, while $K_n$ is not of the same for $K_n$.
...
Then there exists a decreasing filtering family $(A_\alpha)$ of sub-rings noetherian of $A$ such that $A$ is an $A_\alpha$-module \emph{free of finite type} for all $\alpha$ and that, if $K_\alpha$ is the field of fractions of $A_\alpha$, one has $\bigcap_\alpha K_\alpha=K^p$.
\end{proposition}

\begin{proof}
One can write $K^p=k^p((T_1^p,\ldots,T_r^p))$; as one has seen in the proof of \sref{0.21.8.6}, there exists a decreasing family $(k_\alpha)$ of sub-fields of $k$ such that $[k:k_\alpha]$ is finite for all $\alpha$ and $\bigcap_\alpha k_\alpha=k^p$; it is clear that if one sets $A_\alpha=k_\alpha[[T_1^p,\ldots,T_r^p]]$, $A$ is an $A_\alpha$-module \emph{free of finite type}; the whole thing therefore reduces to proving the relation
\begin{equation}
\tag{21.8.8.1}
  \label{0.21.8.8.1}
...
Since $\bigcap_\alpha k_\alpha=k^p$, this will result from the following two lemmas:
\end{proof}
...
\label{0.21.8.8.3}
Let $k$ be a field, $(k_\alpha)$ a decreasing filtering family of sub-fields of $k$, and set $k_0=\bigcap_\alpha k_\alpha$.
...
Now, by hypothesis, $K$ is the residue field of a $k$-algebra of finite type $B$; as $\Omega^1_{B/k}$ is a $B$-module of finite type by virtue of \sref{0.20.4.7}, the conclusion results from \sref{0.20.5.9}.
\end{proof}
...
Furthermore, for every sub-field $k$ of $k_0$ such that $\Omega^1_{k_0/k}$ is of finite rank, $\widehat{\Omega}^1_{A/k}$ is an $A$-module free of rank equal to $\dim(A)+\deg.\operatorname{tr}_{k_0} K+\operatorname{rg}_{k_0}(\Omega^1_{k_0/k})$.
\end{proposition}

\begin{proof}
It is clear that, if $P$ is the prime sub-field of $k_0$, $K$ is a $P$-algebra formally smooth \sref{0.19.6.1}.
Let us note on the other hand that since $A$ is integral over $A_0$, $A_0$ integrally closed and $E$ is separable over $L_0$, $\Omega^1_{A/A_0}$ is an $A$-module of \emph{torsion} \sref{0.20.4.13}, (iv)); tensoring the exact sequence \sref{0.21.9.2} by $E$, it comes an exact sequence
\[
...
Now, let $D_i=\partial/\partial T_i$ be the canonical derivations of $A$ into $P$, which are restrictions of $k$-derivations continuous of $A$ into $A$-modules topological of type fini \sref{0.20.3.7.1};
as $E$ is a separable finite extension of $L_0$, they extend uniquely to derivations of $A$ into $E$ (still denoted $D_i$); by restriction to $A$, these derivations give derivations of $A$ into $E$, and it is immediate that these derivations take their values in the same finite type sub-$A$-module of $E$; as they are continuous in $A_0$ and the topology of $A$ is the $\mathfrak{m}_0$-adic topology, on has thus formed $r$ \emph{continuous} derivations of $A$ into a topological $A$-module, which are evidently linearly independent, since $D_i(T_j)=\delta_{ij}$; this finishes the proof of the proposition.
\end{proof}
...
Then $\widehat{\Omega}^1_{A/k}$ is an $A$-module free of rank $r=\dim(A)$.
\end{corollary}
...
If $B_1$ is the sub-algebra $k[[T_1^p,\ldots,T_r^p]]$ of $B$, $\widehat{\Omega}^1_{A/k}$ identifies canonically with $\Omega^1_{A/B_1}$.
\end{lemma}

\begin{proof}
Every continuous derivation of $B_1$ into a topological $A$-module $P$, which is the restriction of a $k$-derivation continuous of $A$ into $P$, is \emph{zero}, for it is zero in the sub-ring $k[T_1^p,\ldots,T_r^p]$ of polynomials, and this last is dense in $B_1$.
...
\]
is bijective; the conclusion results from \sref{0.20.14.4}, taking into account that $\Omega^1_{A/B_1}$ is an $A$-module of finite type \sref{0.20.4.7}, hence separated and complete since $A$ is a local noetherian complete ring.
\end{proof}

\begin{proposition}[21.9.5]
\label{0.21.9.5}
Let $A$ be a local noetherian complete ring integral, $A_0$ a sub-ring of $A$ isomorphic to a formal power series ring $k[[T_1,\ldots,T_r]]$ over a field $k$, such that $A$ is an $A_0$-algebra finite and that the field of fractions $E$ of $A$ is a separable extension of the field of fractions $L_0$ of $A_0$.
Then one has
\begin{equation}
\tag{21.9.5.1}
  \label{0.21.9.5.1}
  \operatorname{rg}_A\widehat{\Omega}^1_{A/k}=\operatorname{rg}_E(\widehat{\Omega}^1_{A/k_0}\otimes_A E)=\dim(A).
\end{equation}
\end{proposition}

\begin{proof}
Let us note that if $\mathfrak{m}_0$ is the maximal ideal of $A_0$, the topology of $A$ is the $\mathfrak{m}_0$-adic topology since $A$ is an $A_0$-algebra finite (Bourbaki, \emph{Alg.\ comm.}, chap.~IV, \S~2, no.~5, cor.~3 of prop.~9) and it induces on $A_0$ the $\mathfrak{m}_0$-adic topology (Bourbaki, \emph{Alg.\ comm.}, chap.~III, \S~3, no.~4, th.~3).
One knows \sref{0.21.9.1} that $\widehat{\Omega}^1_{A/k}$ is an $A$-module of finite type and, moreover, $\Omega^1_{A/A_0}$ is an $A$-module of \emph{torsion} \sref{0.20.4.13}, (iv)); since the exact sequence \sref{0.20.5.2}
\begin{equation}
\tag{21.9.5.2}
  \label{0.21.9.5.2}
  \widehat{\Omega}^1_{A_0/k}\otimes_{A_0} A\xrightarrow{u}\widehat{\Omega}^1_{A/k}\xrightarrow{v}\Omega^1_{A/A_0}\to 0
\end{equation}
is known to be exact by virtue of \sref{0.20.7.17}, and since $\Omega^1_{A/A_0}$ is a torsion module, we have
$\operatorname{rg}_E(\widehat{\Omega}^1_{A/k}\otimes_A E)\leqslant r$.

It remains to show that there exist, in $\widehat{\Omega}^1_{A/k}$, $r$ elements linearly independent over $E$.
Now, let $D_i=\partial/\partial T_i$ be the canonical derivations of $A_0$ into itself; since $E$ is a separable finite extension of $L_0$, they extend uniquely to derivations of $E$ (still denoted $D_i$); by restriction to $A$, these derivations give derivations of $A$ into $E$, and it is immediate that these derivations take their values in the same finite type sub-$A$-module of $E$; as they are continuous in $A_0$ and the topology of $A$ is the $\mathfrak{m}_0$-adic topology, one has thus formed $r$ continuous derivations of $A$ into a topological $A$-module, which are evidently linearly independent, since $D_i(T_j)=\delta_{ij}$; this finishes the proof of the proposition.
\end{proof}
...
\begin{enumerate}
  ...
  \end{equation}
  Furthermore, if the two members of \sref{0.21.9.6.2} are equal, they remain so when one replaces $k$ by a sub-field $k'$ such that $k_0\subset k'\subset k$.
  \item[\rm (ii)] Suppose moreover that $[k_0:k_0^p]<+\infty$.
  Let $(k_\alpha)$ be a decreasing filtering family of sub-fields of $K_0$ containing $k_0$, such that $[K_0:k_\alpha]<+\infty$ for all $\alpha$ and that $\bigcap_\alpha k_\alpha(K_0^p)=k_0(K_0^p)$; then there exists $\alpha$ such that, for $k=k_\alpha$, the two members of \sref{0.21.9.6.2} are equal.
\end{enumerate}
\end{proposition}
...
\begin{enumerate}
  ...
  It results from \sref{0.21.7.3} that one has
  \[
    \operatorname{rg}_E\Omega^1_{E/M}-\operatorname{rg}_{M_0}\Omega^1_{M_0/M_0}=\deg.\operatorname{tr}_{M_0} E+d(E/M_0,M)
  \]
  ...
  \[
    \operatorname{rg}_{M_0}\Omega^1_{M_0/M}=r+\operatorname{rg}_{K_0}\Omega^1_{K_0/k}
  \]
  by virtue of \sref{0.21.4.5} and \sref{0.21.1.10}, which proves \sref{0.21.9.6.5}.

  On a again, by \sref{0.21.6.4.2},
  \[
  ...
  \]
  But as $M_0$ and $L_0$ are separable extensions of $K_0$ \sref{0.21.9.6.4}, one has $d(L_0/K_0,k)=0$, and by \sref{0.21.6.4.4}
  \[
    d(L_0/M_0,k)=0.
  \]
  It follows therefore
  \[
    \delta=d(E/L_0,M)+d(E/M_0,N/k_0)\geqslant 0
  \]
  ...
  Furthermore, when $k$ is replaced by a sub-extension $k'$ of $k_0$, $N$ is replaced by a sub-extension $N'$ of $k_0$, hence $d(E/M_0,N'/k_0)\leqslant d(E/M_0,N/k_0)$ \sref{0.21.6.4.2}, which proves the last assertion of (i).
  ...
  To finish the proof of \sref{0.21.9.6}, (ii), it remains therefore to prove
\end{enumerate}
\end{proof}

\begin{lemma}[21.9.6.4]
\label{0.21.9.6.4}
For every field $k$, the field of series $K=k((T_1,\ldots,T_r))$ is a separable extension of $k$.
\end{lemma}

\begin{proof}
Let us recall rapidly the proof of this classical lemma.
It suffices (Bourbaki, \emph{Alg.}, chap.~VIII, \S~7, no.~3, proof of th.~1) to prove that for every \emph{finite} extension $k'$ of $k$, $K\otimes_k k'$ is without nilpotent element; but if one sets $A=k[[T_1,\ldots,T_r]]$ and $S=A-\{0\}$, $K\otimes_k k'$ is equal to $S^{-1}(A\otimes_k k')$, and $A\otimes_k k'$ identifies canonically with the integral ring $A'=k'[[T_1,\ldots,T_r]]$ and $A$ to a sub-ring of $A'$, hence the conclusion.
\end{proof}

Using this lemma and \sref{0.21.6.2}, (ii)), one has $d(L_0/K_0,k)=0$, and by \sref{0.21.6.4.4}
also $d(L_0/M_0,k)=0$ \sref{0.21.6.2}.
It follows therefore
\[
  \delta=d(E/L_0,M)+d(E/M_0,N/k_0)\geqslant 0
\]
which finishes the proof of \sref{0.21.9.6}, (i).

\begin{lemma}[21.9.6.7]
\label{0.21.9.6.7}
Let $k_0$ be a field of characteristic $p>0$ such that $[k_0:k_0^p]<+\infty$, $K_0$ an extension of $k_0$.
Then one has
\begin{equation}
\tag{21.9.6.8}
  \label{0.21.9.6.8}
  (k_0(K_0^p))((T_1,\ldots,T_r))=k_0(K_0^p((T_1,\ldots,T_r))).
\end{equation}
\end{lemma}

\begin{proof}
As $k_0^p\subset K_0^p$, one has $k^p_0(K_0^p((T_1,\ldots,T_r)))=K_0^p((T_1,\ldots,T_r))$.
If $(c_i)_{1\leqslant i\leqslant n}$ is a base of $k_0$ over $k_0^p$, it is immediate that $(c_i)$ is also a system of generators of both members of \sref{0.21.9.6.8} over $K_0^p((T_1,\ldots,T_r))$.
\end{proof}

\begin{remark}[21.9.7]
\label{0.21.9.7}
The assertion of \sref{0.21.9.6}, (ii) \emph{does not extend} to the case where $[k_0:k_0^p]$ is infinite.
Let us take for $k_0$ the field $\mathbf{F}_p(X_n)_{n\geqslant 0}$ of rational fractions with a countable infinity of indeterminates, so that the $X_n^p$ form a $p$-base of $k_0$ over $\mathbf{F}_p$, and therefore $[k_0:k_0^p]=+\infty$.
In the statement of \sref{0.21.9.6}, take $K_0=k_0$ (hence necessarily $k=k_0$), $L_0=L=k_0((T))$, for $E$ the extension $L_0(y)$, where $y$ is a root of the polynomial $Y^p-\sum_{n=0}^\infty X_n T^{np}$.
Then $E$ is \emph{separable} over $k_0$, without being an element of $k_0(L_0^p)$, and one sees immediately that this is not possible; one has therefore $d(E/K_0,k_0)=0$, the formula \sref{0.21.9.6.3} reduces to $\delta=d(E/M_0,M_0)-1$, and it is enough to verify that this rank is $2$.
But a $p$-base of $E$ over $L_0$ is formed of the single element $y$, and $L_0$ has evidently a $p$-base on $M_0$ formed of the single element $T$; hence, since $E$ is an algebraic extension of $M_0$, our assertion results from \sref{0.21.7.1} and \sref{0.21.4.5}.
\end{remark}

\begin{proposition}[21.9.8]
\label{0.21.9.8}
Let $k_0$ be a field \textup{(}of arbitrary characteristic\textup{)}, $K_0$ an extension separable of $k_0$, $A$ a local noetherian complete ring, which is a $k_0$-algebra and whose residue field is a finite extension of $K_0$.
...
Then one can find a field $k$ such that $k_0\subset k\subset K_0$ and $[K_0:k]<+\infty$, for which the two members of \sref{0.21.9.8.1} are equal.
\end{proposition}

\begin{proof}
As $A/\mathfrak{p}$ has the same residue field $K$ that $A$ and is complete, one can restrict to the case where $A$ is integral and $\mathfrak{p}=0$; one will then set $E=k(\mathfrak{p})$, field of fractions of $A$.
...
One has $\Upsilon_{K_0/k_0}=0$ since $K_0$ is separable over $k_0$; moreover, as $A$ is an $A_0$-algebra finite, $\dim(A)=\dim(A_0)$ \sref{0.16.1.5}.
...
Suppose now $p>1$; if $B=k[[T_1,\ldots,T_r]]$, $A$ is a $B$-algebra, and setting $B_1=k[[T_1^p,\ldots,T_r^p]]$ and $M$ the field of fractions $k((T_1^p,\ldots,T_r^p))$, it results from \sref{0.20.5.9} that $\widehat{\Omega}^1_{A/k}\otimes_A E$ identifies with $\Omega^1_{E/M}$ \sref{0.21.9.4}, and the inequality \sref{0.21.9.8.1} is none other than \sref{0.21.9.6.2}, and the last assertion follows from \sref{0.21.9.6}, (ii).
\end{proof}
