...
\]
is bijective; the conclusion follows from \sref{0.20.14.4}, taking into account that $\Omega^1_{A/B_1}$ is an $A$-module of finite type \sref{0.20.4.7}, hence separated and complete since $A$ is a local Noetherian complete ring.
\end{proof}
...
It suffices (Bourbaki, \emph{Alg.}, chap.~VIII, \S~7, no.~3, proof of th.~1) to prove that for every \emph{finite} extension $k'$ of $k$, $K\otimes_k k'$ is without nilpotent element; but if one sets $A=k[[T_1,\ldots,T_r]]$ and $S=A-\{0\}$, $K\otimes_k k'$ is equal to $S^{-1}(A\otimes_k k')$, and $A\otimes_k k'$ identifies canonically with the integral ring $A'=k'[[T_1,\ldots,T_r]]$ and $A$ to a subring of $A'$, hence the conclusion.
\end{proof}
...
Let us take for $k_0$ the field $\mathbf{F}_p(X_n)_{n\geqslant 0}$ of rational functions with a countable infinity of indeterminates, so that the $X_n^p$ form a $p$-base of $k_0$ over $\mathbf{F}_p$, and therefore $[k_0:k_0^p]=+\infty$.
...
But a $p$-base of $E$ over $L_0$ is formed of the single element $y$, and $L_0$ has evidently a $p$-base on $M_0$ formed of the single element $T$; hence, since $E$ is an algebraic extension of $M_0$, our assertion follows from \sref{0.21.7.1} and \sref{0.21.4.5}.
\end{remark}
...
One has $\Upsilon_{K_0/k_0}=0$ since $K_0$ is separable over $k_0$; moreover, as $A$ is a finite $A_0$-algebra, $\dim(A)=\dim(A_0)$ \sref{0.16.1.5}.
...
Suppose now $p>1$; if $B=k[[T_1,\ldots,T_r]]$, $A$ is a $B$-algebra, and setting $B_1=k[[T_1^p,\ldots,T_r^p]]$ and $M$ the field of fractions $k((T_1^p,\ldots,T_r^p))$, it follows from \sref{0.20.5.9} that $\widehat{\Omega}^1_{A/k}\otimes_A E$ identifies with $\Omega^1_{E/M}$ \sref{0.21.9.4}, and the inequality \sref{0.21.9.8.1} is none other than \sref{0.21.9.6.2}, and the last assertion follows from \sref{0.21.9.6}, (ii).
\end{proof}
