...
We thus have $\dim(\mathrm{M})=\dim(\mathrm{M}/\mathfrak{q}^k\mathrm{M})$ for every $k>0$.
...
\begin{proof}
Indeed, there exists $n_0$ such that $\mathrm{M}_{n+1}=\mathfrak{q}\mathrm{M}_n$ for $n\geq n_0$, whence the inclusions $\mathfrak{q}^{n+n_0}\mathrm{M}\subset\mathrm{M}_n\subset\mathfrak{q}^n\mathrm{M}\subset\mathrm{M}_{n-n_0}$ for $n$ sufficiently large, which implies
\[
...
Then there exist $d$ homogeneous elements of degree $\geq 1$ in $\mathrm{B}$, $t_1,\ldots,t_d$, forming a family $(t_i)_{1\leq i\leq d}$ such that $\mathrm{E}/(\sum_i^d t_i\mathrm{E})$ is of finite rank over $k$.
\end{proposition}
...
Every element of $\mathrm{B}-\mathfrak{a}$ determines an injective homothety in the Artinian $\mathrm{B}$-module $\mathrm{E}/\mathrm{E}_n'$, and this homothety is thus bijective (Bourbaki, \emph{Alg.}, chap.~VIII, \textsection~2, n\textsuperscript{o}~2, lemma~3), from which one deduces that $(\mathrm{E}/\mathrm{E}_n')_\mathfrak{q}$ is a polynomial in $n$ of degree $d$ for $n$ sufficiently large; as the residue field of $\mathrm{B}_\mathfrak{q}$ is $k$, one deduces from the hypothesis that $\operatorname{long}_{\mathrm{B}_\mathfrak{q}}((\mathrm{E}/\mathrm{E}_\mathfrak{q}')_\mathfrak{q})$ is a polynomial in $n$ of degree $d$; moreover, $\mathrm{E}$ is a $\mathrm{B}$-module of finite type and $\mathrm{B}$ is generated over $k$ by its homogeneous elements of degree $1$, hence the filtration $(\mathrm{E}_n')$ is $\mathfrak{a}$-good (Bourbaki, \emph{Alg.~comm.}, chap.~III, \textsection~1, n\textsuperscript{o}~3, lemma~1), hence the filtration $(\mathrm{E}_n')_\mathfrak{q}$ on $\mathrm{E}_\mathfrak{q}$ is $\mathfrak{a}\mathrm{B}_\mathfrak{q}$-good.
Since $\mathrm{B}_\mathfrak{q}$ is a local Noetherian ring, one concludes from \sref{0.16.2.3} and \sref{0.16.2.3.1} that $\dim(\mathrm{E}_\mathfrak{q})=d$, the lemma being evident if $d=0$.
...
For a prime ideal in a Noetherian ring to be of height $\leq 1$, it is necessary and sufficient that it be minimal in the set of prime ideals containing a principal ideal.
\end{corollary}
...
Let $\mathfrak{p}_i$ $(1\leq i\leq n)$ be the minimal prime ideals among those containing $f$ (which are finite in number since $\mathrm{A}/f\mathrm{A}$ is Noetherian); it suffices to prove that the $\mathfrak{p}_i$ are \emph{maximal} ideals, since $\mathrm{A}_f$ is a field.
...
Let $\mathrm{A}$ be a local Noetherian ring, $\mathfrak{m}$ its maximal ideal, $\mathrm{M}$ an $\mathrm{A}$-module of finite type.
\begin{enumerate}
  \item[\rm{(i)}] For $\operatorname{prof}(\mathrm{M})=0$, it is necessary and sufficient that $\mathfrak{m}\in\operatorname{Ass}(\mathrm{M})$ (or equivalently that there exists a submodule of $\mathrm{M}$ isomorphic to the residue field $k=\mathrm{A}/\mathfrak{m}$ of $\mathrm{A}$).
...
(ii) If $(x_i)_{1\leq i\leq m}$ is a $(\mathrm{M}/x\mathrm{M})$-regular maximal sequence of elements of $\mathfrak{m}$, the sequence formed of $x$ and the $x_i$ is $\mathrm{M}$-regular, and it follows from the definition that it is maximal when the first sequence is; whence the conclusion.
