...
\medskip
\noindent 5)\enspace
The kernel of $v^1$ is the set of classes of $A$-extensions $E$ of $C$ by $L$ which trivialise on $v(B)$, that is, for which there is an $A$-homomorphism $w:B\to E$ such that $v$ factors as $B\xrightarrow{w}E\to C$; or an $A$-homomorphism defining on $E$ a structure of $B$-extension whose class has for image by $u^1$ the class of the $A$-extension given; the converse being trivially true, this proves the exactness at $\operatorname{Exan}_A(C,L)$.
...
\end{equation}
where in the first member the inductive limit is taken over the ordered filtered set of couples $(\mathfrak{J},\mathfrak{R})$ of bilateral ideals such that $\mathfrak{R}.L=L.\mathfrak{R}=0$, $\mathfrak{J}.B\subset\mathfrak{R}$, $B.\mathfrak{J}\subset\mathfrak{R}$.
\end{proposition}
...
Every topological ring $B$ can be considered as a $\mathbf{Z}$-algebra topological (where $\mathbf{Z}$ is equipped with the discrete topology), so one can define the topological $B$-module $\Omega^1_{B/\mathbf{Z}}$, which is sometimes also called the $B$-module of \emph{absolute differentials} over $B$ and which one denotes $\Omega^1_B$.
...
\label{0.20.4.6}
Let $p_1$ and $p_2$ be the composed maps $B\xrightarrow{j_1}B\otimes_A B\to\mathrm{P}^1_{B/A}$, $B\xrightarrow{j_2}B\otimes_A B\to\mathrm{P}^1_{B/A}$, which are continuous $A$-homomorphisms such that $\varepsilon\circ p_1=\varepsilon\circ p_2=\mathrm{I}_B$.
...
\label{0.20.4.8}
Let $A$ be a topological ring, $B$ a topological $A$-algebra topological.
\begin{enumerate}
...
To prove the universal property, recall that $\operatorname{D\acute{e}r.\ cont}_A(B,L)$ identifies canonically with the set of continuous $B$-algebra homomorphisms $u:B\to D_B(L)$ such that the composed $B\xrightarrow{u}D_B(L)\xrightarrow{q}B$ is the identity (where $q(b,0)=b$ is the projection; cf.\ \sref{0.20.1.6} and \sref{0.20.3.2}).
On the other hand, by virtue of the isomorphism $\varphi$, $\operatorname{Hom.\ cont}_B(\Omega^1_{B/A},L)$ identifies canonically with the set of continuous $B$-algebra homomorphisms $v:\mathrm{P}^1_{B/A}\to D_B(L)$ such that the composed $\mathrm{P}^1_{B/A}\xrightarrow{v}D_B(L)\xrightarrow{q}B$ is the augmentation $\varepsilon$.
...
\end{equation}
such that the couple $(u'',u)$ is a \emph{di-homomorphism} for the $B$-module structure of $\Omega^1_{B/A}$ and the $C$-module structure of $\Omega^1_{C/A}$; this last fact allows one to deduce canonically a \emph{homomorphism of $C$-modules}
\begin{equation}
\tag{20.5.2.4}
  \label{0.20.5.2.4}
...
\]
is exact; taking into account \sref{0.20.5.6} and \sref{0.20.2.4.2}, this condition is equivalent to $\operatorname{Exalcom}_{B/A}(C,L)=0$ for every $C$-module $L$, and the conclusion follows from \sref{0.19.9.8.1}.
\end{proof}
...
\]
But if $x,y$ are two elements of $\mathfrak{R}$ and $\zeta$ the class of $xy$ modulo $\mathfrak{R}^4$, the image $\delta'(\zeta\otimes 1)$ is $(xd_{B/A}(y)+yd_{B/A}(x))\otimes 1$; but as the images of $x$ and of $y$ in $C$ are zero, on has also $d_{B/A}(xy)\otimes 1=0$ in $\Omega^1_{B/A}\otimes_B C$, which proves our assertion.
...
Conversely, $c)$ implies $b)$, since every $B$-extension of $C$ by $L$ is $B$-equivalent to $E\otimes_{\mathfrak{R}/\mathfrak{R}^2}L$ \sref{0.18.3.8}, otherwise said its class is the image of the class of $E$ (considered as $B$-extension) by the homomorphism
...
The fact that $c)$ implies $b)$ then results from the commutativity of the diagram \sref{0.18.3.6.5}
\[
\xymatrix{
...
By tensorisation with $C$, one deduces that the composite
\[
...
On the other hand, from the definition it results immediately that the image of $\mathfrak{R}/\mathfrak{R}^2$ by $\delta_{C/B/A}$ is the sub-$C$-module generated by the
\[
...
\end{equation}
is \emph{bijective}; this indeed results from the exact sequence \sref{0.20.5.7.1}, since $\Omega^1_{B/A}=0$ \sref{0.20.4.12}.
\end{remark}
...
It then results from the exact sequence \sref{0.20.6.1.1} and \sref{0.20.5.9} that one has
\begin{equation}
\tag{20.6.1.2}
  \label{0.20.6.1.2}
...
\begin{proof}
This results from \sref{0.20.5.7}, (ii).
\end{proof}
...
\begin{proof}
Indeed, it is equivalent to say that $K$ is separable over $k$ or that $K$ is a $k$-algebra formally smooth \sref{0.19.6.1}; on the other hand, if $P$ is the prime field of $k$, $K$ is separable over $P$, hence also a $P$-algebra formally smooth, and it follows that $K$ is a $k$-algebra formally smooth relative to $P$.
...
The proposition results from the exact sequence \sref{0.20.2.4.2} and from the definition of the cohomology modules
\begin{equation}
\tag{20.6.6.3}
  \label{0.20.6.6.3}
...
The commutativity of the right-hand square of \sref{0.20.6.9.1} results from the relation $v=p\circ q$ \sref{0.20.5.2.7}; the image by $q_{E/B/A}\otimes 1_C$ of the kernel $\Upsilon_{C/B/A}$ of $v_{C/B/A}$ is therefore contained in the kernel $L$ of $p_{C/E/A}$.
...
By virtue of \sref{0.20.5.2.6}, on has $q_{E/B/A}(d_{B/A}(b))=d_{E/A}(q(b))=d_{E/A}(h(v(b)))+d_{E/A}(j(f(d_{B/A}(b))))$ for $b\in B$.
...
Suppose one of the following conditions verified:
\begin{enumerate}
...
\end{enumerate}
Then the homomorphism canonical \sref{0.20.6.7.1}
\[
  \operatorname{Exalcom}_{B/A}(C,L)\to\Hom_C(\Upsilon_{C/B/A},L)
\]
is bijective.
...
\begin{proof}
The fact that each of the conditions (i), (ii) implies that \sref{0.20.6.7.1} is bijective results in both cases from the definition of $\alpha_1$.
...
\begin{proof}
Indeed, it results from the hypothesis on $C$ that $\operatorname{Exalcom}_B(C,L)=\operatorname{Exalcom}_{B/A}(C,L)$, and the homomorphism \sref{0.20.6.12.1} is none other than \sref{0.20.6.7.1}.
\end{proof}
...
Our assertion results from the commutativity of the diagram
\begin{equation}
\tag{20.6.17.3}
  \label{0.20.6.17.3}
\xymatrix{
...
\]
is exact and split \sref{0.20.5.7}; by tensorisation with $C$, the sequence
\[
...
\begin{enumerate}
  \item[\rm(i)] If $F$ is a separable extension of $E$, on has $\Upsilon_{F/E/K}=0$ \textup{(}or equivalently, the canonical homomorphism $\Omega^1_{E/K}\otimes_E F\to\Omega^1_{F/K}$ is injective\textup{)}.
...
\begin{proof}
(i) For every $x\in K$, let $f$ be the minimal polynomial of $x$ over $k$; as $f'(x)\neq 0$ and $d_{K/k}(f(x))=f'(x)d_{K/k}(x)=0$, one has $d_{K/k}(x)=0$ and our assertion results from \sref{0.20.4.7}.

(ii) There exists a pure extension $L$ of $k$ such that $k\subset L\subset K$ and $K$ is an algebraic extension of $L$.
As $K$ is separable over $L$, it results from \sref{0.20.6.19}, (i) that the sequence
\[
  0\to\Omega^1_{L/k}\otimes_L K\to\Omega^1_{K/k}\to\Omega^1_{K/L}\to 0
\]
is exact, and from (i) that $\Omega^1_{K/L}=0$.
The relation $\Omega^1_{K/k}=0$ is therefore equivalent to $\Omega^1_{L/k}=0$, and as $L$ is a pure extension of $k$, it results from \sref{0.20.5.10} that the relation $\Omega^1_{L/k}=0$ is equivalent to $L=k$.
\end{proof}
...
Moreover, it results from \sref{0.20.6.10} that $\Upsilon_{C/B/\Lambda}$ is canonically identified with $\mathfrak{R}/\mathfrak{R}^2$.
...
One has thus made precise the \emph{kernel} of $\delta_{C/B/A}$ in the case where there exists a ring $\Lambda$ such that $A$ is a $\Lambda$-algebra and $B/\mathfrak{R}^2$ is $\Lambda$-trivial; one observes that this is the case in particular when $C$ is a \emph{$\Lambda$-algebra formally smooth} (for the discrete topology).
...
\begin{proof}
This results from \sref{0.20.6.18}, (ii).
\end{proof}

\begin{env}[20.6.24]
\label{0.20.6.24}
When the hypotheses of \sref{0.20.6.22} are satisfied, one says also that the characteristic homomorphism $\chi_E$ is the \emph{characteristic homomorphism of the $A$-algebra $B$, relative to $\Lambda$ and to the ideal $\mathfrak{R}$} (suppressing these last precisions when there is no danger of confusion); one denotes it sometimes by $\chi_B$ or $\chi_{B/A}$.
\end{env}
...
\]
by definition; the identification of $u^1$ and $v^1$ with the homomorphisms of \sref{0.18.3.4.2} results from \sref{0.20.6.6.4}.
\end{proof}
...
In reality, these complexes, considered as objects of the category of complexes of $C$-modules ``up to homotopy'' (that is to say where the morphisms are the classes of homotopic homomorphisms) are finer invariants than the couple formed by $\Omega^1_{C/B}$ and $\Upsilon_{C/B/A}$.
...
\begin{proof}
To say that $v_{C/B/A}$ is formally left-invertible means, given that the topologies of $\Omega^1_{C/A}$ and $\Omega^1_{B/A}\otimes_B C$ are finer than those deduced from the topology of $C$ \sref{0.20.4.5}, that for every $C$-module discrete $L$, annihilated by an open ideal of $C$, the canonical homomorphism
\[
...
For $C$ to be a $B$-algebra formally smooth relative to $A$, it is necessary and sufficient that, if one denotes by $(\mathfrak{R}_\lambda)$ a fundamental system of neighborhoods of $0$ formed of ideals of $C$, then for every $\lambda$, the homomorphism
\begin{equation}
\tag{20.7.3.1}
  \label{0.20.7.3.1}
...
\begin{proof}
Indeed, by virtue of \sref{0.19.1.1}, this homomorphism is surjective, and by injectivity in virtue of \sref{0.20.7.2}, it is also bijective; it results that the quotient of $\Omega^1_{C/A}$ by the closure of $\operatorname{Im}(v_{C/B/A})$ would be separated and $\neq 0$ and would therefore have a discrete quotient $L\neq 0$, contradicting what one has just seen (taking into account \sref{0.20.4.8}).
By virtue of $v_{C/B/A}$, which is a monomorphism in virtue of \sref{0.20.7.2}, it is also surjective in virtue of the image of $v_{C/B/A}$ being necessarily dense in $\Omega^1_{C/A}$, otherwise the quotient of $\Omega^1_{C/A}$ by the closure of $\operatorname{Im}(v_{C/B/A})$ would be separated and $\neq 0$ (taking into account \sref{0.20.4.8}); by virtue of $v_{C/B/A}$, which is a formal monomorphism by virtue of \sref{0.20.7.2}, it is therefore a formal bimorphism \sref{0.19.1.2}.
\end{proof}

\begin{proposition}[20.7.4]
\label{0.20.7.4}
Let $A$ be a topological ring, $B$ a topological $A$-algebra.
For $B$ to be formally non-ramified \sref{0.19.10.2}, it is necessary and sufficient that the separated completion $\widehat{\Omega}^1_{B/A}$ be zero.
\end{proposition}

\begin{proof}
Indeed, it results immediately from \sref{0.19.10.4} and \sref{0.20.1.1} that, for $B$ to be formally non-ramified, it is necessary and sufficient that for every open ideal $\mathfrak{R}$ of $B$, every open ideal $\mathfrak{H}$ of $A$ such that $\mathfrak{H}.B\subset\mathfrak{R}$ and every $(B/\mathfrak{R})$-module $L$, one has $\Der_{A/\mathfrak{H}}(B/\mathfrak{R},L)=0$, that is to say $\operatorname{D\acute{e}r.\ cont}_A(B,L)=0$ for every $B$-module discrete $L$ annihilated by an open ideal; by virtue of \sref{0.20.4.8}, this is equivalent to $\Hom.\operatorname{cont}_B(\Omega^1_{B/A},L)=0$ for every such $B$-module, whence the proposition.
...
\begin{proof}
Indeed, for every $C$-module discrete $L$ annihilated by an open ideal of $C$, one has then \sref{0.20.7.4} $\operatorname{D\acute{e}r.\ cont}_B(C,L)=0$ and by virtue of \sref{0.20.3.6.1} the canonical homomorphism
\oldpage[0\textsubscript{IV-1}]{149}
$\operatorname{D\acute{e}r.\ cont}_A(C,L)\to\operatorname{D\acute{e}r.\ cont}_A(B,L)$ is injective; it is moreover surjective by virtue of \sref{0.20.7.2}, hence it is bijective; it results that the image of $v_{C/B/A}$ is necessarily dense in $\Omega^1_{C/A}$, otherwise the quotient of $\Omega^1_{C/A}$ by the closure of $\operatorname{Im}(v_{C/B/A})$ would be separated and $\neq 0$ and would therefore have a discrete quotient $L\neq 0$, contradicting what one has just seen (taking into account \sref{0.20.4.8}).
...
the homomorphism $\delta_{C/B/A}$ is continuous and the homomorphism $v_{C/B/A}$ is a strict morphism of topological $C$-modules.
  \item[\rm(ii)] For $\delta_{C/B/A}$ to be formally left-invertible \sref{0.19.1.5}, it is necessary and sufficient that for every $C$-module discrete $L$ annihilated by an open ideal of $C$, the canonical homomorphism
\begin{equation}
\tag{20.7.8.1}
  \label{0.20.7.8.1}
...
On the other hand, the canonical homomorphism $v\otimes v:B\otimes_A B\to C\otimes_A C$ is a strict morphism by definition of the tensor product topology, and one deduces immediately that it is the same of $v_{C/B/A}$.
...
\]
is surjective, and the conclusion results from the exact sequence \sref{0.20.3.6.1}.
\end{proof}

\begin{corollary}[20.7.9]
\label{0.20.7.9}
If the $A$-algebra topological $C=B/\mathfrak{R}$ is formally smooth, the canonical homomorphism $\delta_{C/B/A}$ is formally left-invertible.
\end{corollary}

\begin{env}[20.7.10]
\label{0.20.7.10}
In \sref{0.20.6.1}, when $A$, $B$, $C$ are topological rings and $u$ and $v$ continuous homomorphisms, one equips $\Upsilon_{C/B/A}$ with the topology induced by that of $\Omega^1_{B/A}\otimes_B C$; the homomorphisms \sref{0.20.6.4.2} and \sref{0.20.6.4.4} are then continuous provided that, in \sref{0.20.6.7}, on supposes $L$ a $C$-module discrete annihilated by an open ideal of $C$, one deduces, by passage to the inductive limit, the canonical $C$-homomorphism
\begin{equation}
\tag{20.7.10.1}
  \label{0.20.7.10.1}
  \operatorname{Exalcotop}_{B/A}(C,L)\to\Hom.\operatorname{cont}_C(\Upsilon_{C/B/A},L)
\end{equation}
which can be made explicit in the following way (taking into account \sref{0.18.5.3.1}): for every open ideal $\mathfrak{M}$ of $A$, every open ideal $\mathfrak{R}$ of $B$ such that $\mathfrak{M}B\subset\mathfrak{R}$, every open ideal $\mathfrak{P}$ of $C$ such that $\mathfrak{R}C\subset\mathfrak{P}$ and $\mathfrak{P}.L=0$, every $(B/\mathfrak{R})$-extension $E$ of $C/\mathfrak{P}$ by $L$ which is $(A/\mathfrak{M})$-trivial comes from the datum of a continuous $C$-homomorphism $f$ of $\Omega^1_{B/A}\otimes_B C$ in $L$, and the homomorphism \sref{0.20.7.10.1} makes correspond to the class of $E$ the restriction of $f$ to $\Upsilon_{C/B/A}$, said \emph{characteristic homomorphism of $E$} and again denoted $\chi_E$.
\end{env}
...
\begin{env}[20.7.13]
\label{0.20.7.13}
If, in \sref{0.20.6.23}, one supposes that the rings $\Lambda$, $A$, $B$, $C$ are topological, the homomorphisms $s$, $u$, $v$ continuous and $L$ a $C$-module discrete annihilated by an open ideal of $C$, one can pass to the inductive limit as in \sref{0.20.3.6}, and one obtains an exact sequence
\begin{equation}
\tag{20.7.13.1}
  \label{0.20.7.13.1}
\begin{split}
...
\begin{env}[20.7.14]
\label{0.20.7.14}
Let $A$ be a topological ring, $B$ a topological $A$-algebra, $\mathfrak{M}'$ an open ideal of $A$, $\mathfrak{R}'$ an open ideal of $B$ such that $\mathfrak{M}'B\subset\mathfrak{R}'$; set $A'=A/\mathfrak{M}'$, $B'=B/\mathfrak{R}'$; the kernel of the homomorphism $B\otimes_A B\to B'\otimes_{A'} B'$ is $\mathfrak{U}=\operatorname{Im}(\mathfrak{R}'\otimes B+B\otimes\mathfrak{R}')$, whence it results immediately that the kernel of the homomorphism
\begin{equation}
\tag{20.7.14.1}
  \label{0.20.7.14.1}
  \varphi_{(\mathfrak{M}',\mathfrak{R}')}:\Omega^1_{B/A}\to\Omega^1_{B'/A'}
\end{equation}
is $((\mathfrak{J}\cap\mathfrak{U})+\mathfrak{J}^2)/\mathfrak{J}^2$; moreover, the homomorphism \sref{0.20.7.14.1} is \emph{surjective}, as it results from \sref{0.20.4.7}.
...
If one notices that the $((\mathfrak{J}\cap\mathfrak{U}')+\mathfrak{J}^2)/\mathfrak{J}^2$ form a fundamental system of neighborhoods of $0$ in $\Omega^1_{B/A}$, one sees therefore that the \emph{separated completion} $\widehat{\Omega}^1_{B/A}$ of the topological $B$-module $\Omega^1_{B/A}$ is given, up to a canonical isomorphism, by
\begin{equation}
\tag{20.7.14.2}
  \label{0.20.7.14.2}
...
\end{equation}
is \emph{surjective} for every couple $(\mathfrak{M}',\mathfrak{R}')$.
One can moreover replace $A'$ by $A$ everywhere in the above.

Finally, if $L$ is a separated and \emph{complete} topological $B$-module, every continuous $B$-homomorphism of $\Omega^1_{B/A}$ in $L$ extends in a unique fashion to a continuous $\widehat{B}$-homomorphism of $\widehat{\Omega}^1_{B/A}$ in $L$ and reciprocally such a homomorphism gives by composition with $\Omega^1_{B/A}\to\widehat{\Omega}^1_{B/A}$ a continuous $B$-homomorphism, so that one has a canonical isomorphism
\[
  \Hom.\operatorname{cont}_B(\Omega^1_{B/A},L)\xrightarrow{\sim}\Hom.\operatorname{cont}_{\widehat{B}}(\widehat{\Omega}^1_{B/A},L).
\]
More particularly, if $L$ is a $B$-module discrete annihilated by an open ideal of $B$, one sees that the canonical isomorphism \sref{0.20.4.8.2} can also be written
\begin{equation}
\tag{20.7.14.4}
  \label{0.20.7.14.4}
...
\label{0.20.7.15}
Let $A$ be a discrete ring, $B$ a topological $A$-algebra, $\mathfrak{n}$-adique ($\mathbf{0_I}$, 7.1.9), $\mathfrak{n}$ an ideal of definition of $B$, $B_0=B/\mathfrak{n}$.
One supposes that $\Omega^1_{B/A}$ and $\mathfrak{n}/\mathfrak{n}^2$ are $B_0$-modules of finite type.
Then $\widehat{\Omega}^1_{B/A}$ is a $B$-module of finite type.
\end{proposition}
...
Taking into account the hypothesis that $B$ is an adique ring, it suffices, by virtue of ($\mathbf{0_I}$, 7.2.7 and 7.2.9), to prove that $\Omega^1_{B_0/A}$ is a $B_0$-module of finite type, which follows from \sref{0.20.5.12.1} and the hypotheses.
\end{proof}
