...
\label{0.22.4.3}
With the hypotheses and notations of \sref{0.22.4.1}, suppose that $\mathrm{A}$ and $\mathrm{K}$ are of characteristic $p>0$ (in other words, that $p.1\in\mathfrak{m}$ in $\mathrm{A}$), and that the $\mathrm{F}_i$ are of the form
\[
\label{0.22.4.3.1}
...
The residue field of $\mathrm{B}_\mathfrak{p}$ is $k(\mathfrak{p})$; as $k((\mathrm{X}_1,\ldots,\mathrm{X}_s))$ is separable over $k$, $k(\mathfrak{p})$ is separable over $k$ by hypothesis. It follows therefore from \sref{0.22.6.2}, (ii) that the condition $a$) is equivalent to the fact that the homomorphism $j_0$ defined in \sref{0.22.7.2.1} (where $\mathrm{L}=k(\mathfrak{p})$) be \emph{injective}.
...
Consider further the composite homomorphism
\[
\label{0.22.7.2.2}
