...
The results of this section will be completed in \textbf{IV}, \sref[IV]{IV.7.6} and \sref[IV]{IV.7.7}.
...
This being the case, the $xA'$-preadic topology of $A'$ being separated, $A'$ is a sub-ring of its completion $\hat{A}'$ for this topology; but since $A'/xA'$ is an $A$-module of finite type and $A$ is separated and complete for the $xA$-preadic topology, $\hat{A}'$ is an $A$-module of finite type by virtue of \sref[0\textsubscript{I}]{0.7.2.9}, hence so is $A'$.
\end{proof}
...
\begin{proof}
We know \sref{0.19.8.8}[ii] that there exists a sub-ring $B$ of $A$ that is a complete local
...
Since $A_1$ is a flat $A$-module \sref[0\textsubscript{I}]{0.7.3.5}, $A'_1$ is identified with a sub-ring of $K'_1$ and is evidently integral over $A_1$.
On the other hand, if we set $K_1=K\otimes_A A_1$, we can write $K'_1=K'\otimes_K K_1$; since $A$ is integral and $A_1$ is a flat $A$-module, every element $\neq 0$ of $A$ is $A_1$-regular \sref[0\textsubscript{I}]{0.6.3.4}; since $K=S^{-1}A$, where $S=A\setmin\{0\}$, we have $K_1=S^{-1}A_1$, and the preceding remark proves that $K_1$ is identified with a \emph{sub-ring} of the total ring of fractions $R$ of $A_1$.
Since every $K$-module is flat, $K'_1$ is identified with a \emph{sub-ring} of $K'\otimes_K R$, which by hypothesis is \emph{reduced} and is a \emph{finite} $R$-algebra.
...
\begin{lemma}[23.2.2]
\label{0.23.2.2}
Let $A$ be an integral ring, $A'$ its integral closure; for the canonical morphism $f:\Spec(A')\to\Spec(A)$ to be radiciel, it is necessary and sufficient that for every $\mathfrak{p}\in\Spec(A)$, $A_\mathfrak{p}$ be geometrically unibranch; when this is so, $f$ is a homeomorphism.\footnote{In the rest of this section, we use notions and results from chap.~IV, \textsection\textsection2 and~5; since the results of this section are not used before chap.~IV, \textsection6, this does not lead to circular reasoning.}
\end{lemma}
...
Let $K'$ be a finite extension of $K$, and let $(C_\alpha)$ be the increasing filtering family of sub-rings of $K'$ that are finite $A$-algebras and admit $K'$ as their field of fractions; the union $A'$ of the $C_\alpha$ is thus the integral closure of $A$ in $K'$.
Then:
\begin{enumerate}
...
Let $(C_\alpha)$ be the increasing filtering family of sub-rings of $K'$ that are finite $A$-algebras (hence Noetherian semi-local rings \textnormal{(Bourbaki, \emph{Alg.~comm.}, chap.~IV, \textsection2, n\textsuperscript{o}~5, cor.~3 of prop.~9)}) and admit $K'$ as their field of fractions.
...
Moreover, there exists a sub-algebra $C$ of $K$ which is a finite $A$-algebra, and such that the $V_\lambda$ are the integral closures of the rings $C_{\mathfrak{p}_\lambda}$, where $(\mathfrak{p}_\lambda)$ is the family of prime ideals of height $1$ of the local ring $C$.
\end{proposition}
...
Every $a\neq 0$ in $A$ being a non-zero-divisor does not become a zero-divisor in $B$ by flatness \sref[0\textsubscript{I}]{0.6.3.4}; we deduce that $a$ is not a non-zero-divisor in $C$ either, since if $ax\in\mathfrak{R}$ for an $x\notin\mathfrak{R}$ in $B$, we would have $a^nx^n=0$ for some integer $n$, which contradicts the above since $x^n\neq 0$.
...
Conversely, let $x\in C'\cap K$; $x$ is then integral over $C$, and \emph{a fortiori} over $B$, that is to say $B[x]$ is a finite $B$-algebra; moreover, $K$ is identified with a sub-ring of $B\otimes_A K$, and the sub-ring $B[x]$ of $B\otimes_A K$ is identified with $B\otimes_A A[x]$ by flatness; we conclude that $A[x]$ is an $A$-module of finite type (Bourbaki, \emph{Alg.~comm.}, chap.~I\textsuperscript{er}, \textsection3, n\textsuperscript{o}~6, prop.~11), hence $x\in A'$.
\end{proof}
...
We know (Bourbaki, \emph{Alg.~comm.}, chap.~VII, \textsection1, n\textsuperscript{o}~3, cor.~du th.~2) that $B'_i$ is a Krull ring.
...
To prove the last assertion of the proposition, note that there exists a finite $A$-algebra $C\subset K$ such that the morphism $\Spec(A')\to\Spec(C)$ is radiciel and a homeomorphism \sref{0.23.2.4}; for every prime ideal $\mathfrak{p}'\in\Spec(A')$, $\mathfrak{p}'$ is thus the only prime ideal of $A'$ above $\mathfrak{p}=\mathfrak{p}'\cap C$ and $A'_{\mathfrak{p}'}$ the integral closure of $C_\mathfrak{p}$; on the other hand the fact that $\Spec(A')\to\Spec(C)$ is a homeomorphism implies that the application $\mathfrak{p}'\mapsto\mathfrak{p}'\cap C$ is a \emph{bijection} of the set of prime ideals of height~$1$ of $A'$ onto the set of prime ideals of height~$1$ of $C$; whence the conclusion, by virtue of Bourbaki, \emph{Alg.~comm.}, chap.~VII, \textsection1, n\textsuperscript{o}~6, th.~4.
\end{proof}
