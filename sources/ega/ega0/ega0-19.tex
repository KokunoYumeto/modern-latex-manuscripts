...
For \emph{topological algebras} over a topological ring $A$, there is an analogous notion of \emph{formally smooth algebra} that we will not make precise here (cf.~\sref{0.19.3.1}).
...
For $A$ to be formally smooth, it is necessary and sufficient that $A$ be \emph{geometrically regular} over $k$, that is to say (cf.~$\mathbf{IV}$, 6.7.6), for every finite extension $k'$ of $k$, the semi-local ring $A\otimes_k k'$ be regular \sref{0.17.3.6}.
...
Given a topological ring $A$ and a topological $A$-module $M$, the sets $\mathfrak{J}M$, where $\mathfrak{J}$ ranges over a fundamental system of open ideals in $A$, form a fundamental system of open submodules of a topology on $M$ making $M$ a topological $A$-module, said to be \emph{derived} from the topology of $A$.
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item[a$''$)] For all $\lambda$, the composed homomorphism $M\xrightarrow{u}N\to N/W_\lambda$ is surjective.
  \end{enumerate}
...
For $u$ to be a formal bimorphism, it is necessary and sufficient that for all $\lambda$, the homomorphism $u_\lambda:M/\mathfrak{J}_\lambda M\to N/\mathfrak{J}_\lambda N$ induced by $u$ by passing to quotients be bijective.
\end{corollary}
...
suppose one of the following conditions is verified:
\begin{enumerate}
...
the composed homomorphism $N\xrightarrow{u}M\to M/\mathfrak{J}M$ factorizes as $N\xrightarrow{\tilde{u}}M/\mathfrak{J}M$ since $N$ is projective;
...
\begin{proof}
One will note that $\hat{u}$ is then an isomorphism of $\widehat{M}$ onto a \emph{direct summand} of $\widehat{N}$.
...
Suppose that, for all $n$, $P_n=\operatorname{Coker}(u_n)$ is a $(A/\mathfrak{J}_n)$-module projective and that $M$ is separated and complete.
...
Let $M$, $N$ be two topological $A$-modules whose topologies are derived from those of $A$, and such that for all $\lambda$, $N/\mathfrak{J}_\lambda N$ be an $(A/\mathfrak{J}_\lambda)$-module projective (cf.~\sref{0.19.2.3}).
...
For $u$ to be left invertible, it is necessary and sufficient that there exist a system of generators $(x_i)_{1\leq i\leq m}$ of $M$ such that the images $x_i\otimes 1:M\otimes_A k\to N\otimes_A k$ of the $x_i\otimes 1$ be linearly independent in $N\otimes_A k$;
...
\label{0.19.1.13}
Suppose the hypotheses of \sref{0.19.1.9} verified, and suppose in addition that $(\mathfrak{J}_\lambda)$ be a decreasing sequence $(\mathfrak{J}_n)$ and that $M$ be separated and complete.
...
for every open submodule $W'$ of $N$, there exist an open submodule $V''\subset V'\cap u^{-1}(W')$ of $M$, an open submodule $W''\subset W'$ of $N$, and a homomorphism $h:N/W''\to M/V'$ such that the canonical homomorphism $N/W''\to N/W'$ factorizes as
\[
...
\begin{env}[19.2.2]
\label{0.19.2.2}
To verify the condition of \sref{0.19.2.1}, one can obviously restrict to the case where $v$ is itself surjective (replacing $Q$ by $v(M)$ and $P$ by $u^{-1}(v(M))$) at the case where $v$ is itself surjective;
$Q$ is then isomorphic to $M/V$, where $V$ is an open submodule of $M$ such that $\mathfrak{J}M\subset V$;
the condition of \sref{0.19.2.1} then amounts to saying that for every $(A/\mathfrak{J})$-module discrete $P$ and every $A$-homomorphism $u:P\to M/V$, there exists an open submodule $V'\subset V$ and an $A$-homomorphism $w:M/V'\to P$, such that the canonical homomorphism $M/V'\to M/V$ factorizes as $M/V'\xrightarrow{w}P\xrightarrow{u}M/V$.
One will note that it suffices to verify this condition when $\mathfrak{J}$ ranges over a fundamental system of neighbourhoods of $0$ formed of ideals in $A$.
\end{env}

\begin{env}[19.2.3]
\label{0.19.2.3}
Suppose that there exists a fundamental system $(\mathfrak{J}_\lambda)$ of open ideals in $A$ and a fundamental system $(V_\lambda)$ of open submodules in $M$, having the same index set, such that $\mathfrak{J}_\lambda\subset V_\lambda$ for all $\lambda$, and such that $M/V_\lambda$ be an $(A/\mathfrak{J}_\lambda)$-module \emph{projective}.
...
as $P$ is also an $(A/\mathfrak{J}_\lambda)$-module, the factorization of the canonical homomorphism $M/V_\lambda\to M/V$ into $M/V_\lambda\to P\to M/V$ results from what $M/V_\lambda$ is supposed to be an $(A/\mathfrak{J}_\lambda)$-module projective.
...
let us give ourselves an open ideal $\mathfrak{R}$ of $B$ and a continuous $A$-homomorphism $u':B/\mathfrak{R}\to C/\mathfrak{J}$;
...
As every discrete $A$-algebra $C$ is separated and complete, it is also a $\widehat{A}$-algebra (by canonical prolongation of the homomorphism of $A$ in $C$), and a continuous $A$-homomorphism of $B$ in $C$ gives by prolongation a canonical $\widehat{A}$-homomorphism (and \emph{a fortiori} an $A$-homomorphism) of $\widehat{B}$ in $C$ (in other terms, every continuous $A$-homomorphism $B\to C$ factorizes as $B\to\widehat{B}\to C$ uniquely).
These remarks and the definition \sref{0.19.3.1} immediately imply the proposition.
\end{proof}

\begin{corollary}[19.3.7]
\label{0.19.3.7}
Under the hypotheses of \sref{0.19.3.5}, (iv)), the $A\{S^{-1}\}$-algebra topological $B\{T^{-1}\}$ is formally smooth.
\end{corollary}
...
Let $A'$, $B'$ be the rings $A$ and $B$ equipped with topologies finer than the initial topologies, and suppose that the canonical homomorphism $\varphi:A\to B$ is still a continuous homomorphism of $A'$ in $B'$.
...
Then, if a composed homomorphism $v:B\xrightarrow{u}C\to C/\mathfrak{J}$ is continuous, the homomorphism $u$ is continuous.
\end{lemma}
...
Let $u:B\to E/\mathfrak{R}$ be a continuous $A$-homomorphism, $u'$ the composed homomorphism $B\xrightarrow{u}E/\mathfrak{R}\xrightarrow{f'}E'/\mathfrak{R}'$, and let $v':B\to E'$ be a continuous $A$-homomorphism such that $u'$ factorizes as $B\xrightarrow{v'}E'\to E'/\mathfrak{R}'$.
...
\begin{enumerate}
  ...
  As $B$ is a formally projective $A$-module, the canonical continuous $A$-linear application $\rho:B\to B/\mathfrak{R}$ factorizes as $B\xrightarrow{w}E\to B/\mathfrak{R}$, where $w$ is a continuous $A$-linear application \sref{0.19.2.1},
...
To see that it is sufficient, we first note that, by considering a fundamental system of open neighborhoods of $0$ in $A$ formed by ideals $\mathfrak{J}_\lambda$ contained in $\mathfrak{J}$, one can by virtue of \sref{0.19.4.2} reduce to the case where $A$ is discrete, since $B/\mathfrak{J}_\lambda B$ is a formally projective $(A/\mathfrak{J}_\lambda)$-module \sref{0.19.2.5};
...
By hypothesis, there exists an open ideal $\mathfrak{R}'\subset\mathfrak{R}$ in $B$ and an $A_0$-linear application $g_0:B_0/\mathfrak{R}'B_0\to L$ such that $f_0(\varphi_0(x),\varphi_0(y))=xg_0(y)-g_0(xy)+g_0(x)y$ for $x,y$ in $B_0/\mathfrak{R}'B_0$, where $\varphi_0:B_0/\mathfrak{R}'B_0\to B_0/\mathfrak{R}B_0$ is the canonical homomorphism.
One concludes immediately that the composed $A$-linear application $g:B/\mathfrak{R}'\to B_0/\mathfrak{R}'B_0\xrightarrow{g_0}L$ is such that $dg=f\circ(\varphi\times\varphi)$, where $\varphi:B/\mathfrak{R}'\to B/\mathfrak{R}$ is the canonical homomorphism.

Let us now pass to the general case, and consider first the $(B/\mathfrak{R})$-module $L/\mathfrak{J}L=L'$, for which $\mathfrak{J}L'=0$;
if $f'$ is the $A$-bilinear composed application $(B/\mathfrak{R})\times(B/\mathfrak{R})\xrightarrow{f}L\to L'$, $f'$ is again a symmetric Hochschild $2$-cocycle, and by virtue of what precedes, there exists an open ideal $\mathfrak{R}'\subset\mathfrak{R}$ in $B$ and an $A$-linear application $g':B/\mathfrak{R}'\to L'$ satisfying $dg'=f'\circ(\varphi'\times\varphi')$ for the canonical application $\varphi':B/\mathfrak{R}'\to B/\mathfrak{R}$.
As $B$ is a formally projective $A$-module, there exists an $A$-linear application $g_1:B/\mathfrak{R}'_1\to L$ lifting $B/\mathfrak{R}'_1\to B/\mathfrak{R}'\xrightarrow{g'}L'$.
Consider then the Hochschild $2$-cocycle
\[
  f_1(x,y) = f(\varphi_1(x),\varphi_1(y)) - xg_1(y) + g_1(xy) - g_1(x)y,
\]
a symmetric $A$-bilinear application of $(B/\mathfrak{R}'_1)\times(B/\mathfrak{R}'_1)$ in $L$.
The choice of $g_1$ implies that $f_1$ takes its values in $\mathfrak{J}L$.
As $\mathfrak{J}(\mathfrak{J}L)=0$, one can again apply the first case, so there exists an open ideal $\mathfrak{R}''\subset\mathfrak{R}'_1$ and an $A$-linear application $g_2:B/\mathfrak{R}''\to\mathfrak{J}L$ such that
\[
  f_1(\varphi_2(x),\varphi_2(y)) = xg_2(y) - g_2(xy) + g_2(x)y
\]
in $(B/\mathfrak{R}'')\times(B/\mathfrak{R}'')$, where $\varphi_2:B/\mathfrak{R}''\to B/\mathfrak{R}'_1$ is the canonical application;
the $A$-linear application $g=g_2+g_1\circ\varphi_2:B/\mathfrak{R}''\to L$ therefore satisfies $dg=f\circ(\varphi\times\varphi)$ for the canonical application $\varphi:B/\mathfrak{R}''\to B/\mathfrak{R}$.
\end{proof}
...
we take as fundamental system of neighborhoods of $0$ in $\mathbf{S}_C^*(V)$ the set of sums $\mathfrak{a}.\mathbf{S}_C^*(V)+U.\mathbf{S}_C^*(V)$, where $\mathfrak{a}$ (resp.\ $U$) runs through a fundamental system of open ideals (resp.\ of open submodules) of $C$ (resp.\ $V$).
...
in this case the topology induced on each $\mathbf{S}_C^n(V)$ for $n\geq 1$ has as fundamental system of neighborhoods of $0$ the $U.\mathbf{S}_C^{n-1}(V)$, where $U$ runs through the open submodules of $V$;
...
On the other hand, the restriction to $\mathfrak{J}/\mathfrak{J}^2$ of the $C$-homomorphism $f'_n:F_n\xrightarrow{v}E_n\xrightarrow{f_n}G/\mathfrak{R}$ is a $C$-linear application $g:\mathfrak{J}/\mathfrak{J}^2\to G/\mathfrak{R}$.
As $\mathfrak{J}/\mathfrak{J}^2$ is a projective $C$-module, $g$ factorizes as $\mathfrak{J}/\mathfrak{J}^2\xrightarrow{h}G\to G/\mathfrak{R}$, where $h$ is a $C$-linear application.
By extension, one deduces a homomorphism of $C$-algebras $w:\mathbf{S}_C^*(\mathfrak{J}/\mathfrak{J}^2)\to G$, and by construction every element of degree $m$ has for image by $w$ an element of $\mathfrak{R}$, so every element of degree $n=2m$ has zero image since $\mathfrak{R}^2=0$;
...
Then:
\begin{enumerate}
  \item[\rm(i)] For every $\alpha$, there exists $\beta\geq\alpha$ and an $A$-homomorphism surjective of algebras
...
\]
  ...
  $2^\circ$ the composite $E_{\lambda,n}\xrightarrow{w_{\beta\lambda}}F_{\beta,n}\xrightarrow{v_{\alpha\beta}}E_{\alpha,n}$ is the homomorphism of transition.

  \item[\rm(ii)] If $\beta$ and $v_{\alpha\beta}$ satisfy the conditions of~{\rm(i)}, then for every $\gamma\geq\beta$, there exists $\delta\geq\gamma$ and an $A$-homomorphism surjective of algebras $v_{\gamma\delta}:F_{\delta,n}\to E_{\gamma,n}$ satisfying the conditions of~{\rm(i)} (for $\gamma$ and $\delta$) and making commutative the diagram
...
\begin{enumerate}
  ...
  The hypothesis that $\mathfrak{J}/\mathfrak{J}^2$ is a formally projective $C$-module then implies the existence of a continuous $C$-linear application $g_\alpha:\mathfrak{J}/\mathfrak{J}^2\to\mathfrak{J}_\alpha/\mathfrak{J}_\alpha^{n+1}$ making commutative the diagram
...
\]
  ...
  This implies in particular that the kernel of the homomorphism of transition $E_{\gamma,n}\to E_{\alpha,n}$, equal to $(\mathfrak{b}_\gamma+\mathfrak{J}^{n+1})/(\mathfrak{b}_\alpha+\mathfrak{J}^{n+1})$, is \emph{nilpotent};
...
\]
  ...
  (where the vertical arrow is the homomorphism of transition) is commutative.
...
As $B$ is a formally smooth $A$-algebra, the $A$-homomorphism canonical $\rho_{\beta,n}:B\to E_{\beta,n}$, which is continuous, factorizes as $B\xrightarrow{w}F_{\beta,n}\xrightarrow{v_{\beta\delta}}E_{\beta,n}$, where $w$ is a continuous $A$-homomorphism.
...
Consider the composed homomorphism $u'_\lambda:B/\mathfrak{b}_\lambda\xrightarrow{w'_\lambda}F_{\beta,n}\xrightarrow{v_{\alpha\beta}}E_{\alpha,n}$, where $q_{\beta\delta}$ is the homomorphism of transition;
as the composite $v_{\beta\delta}\circ w_{\beta\lambda}$ is the homomorphism of transition $C_\lambda\to C_\beta$, this $A$-homomorphism is the canonical homomorphism $B/\mathfrak{b}_\lambda\to B/(\mathfrak{b}_\alpha+\mathfrak{J}^{n+1})=E_{\alpha,n}$.
...
From $w'_\lambda$, noting that $\gr^0(q_{\beta\delta})=\gr^0(v_{\beta\delta}):C_\delta\to C_\beta$ is the homomorphism of transition, the composite $C_\lambda\to C_\delta\to C_\beta$ is the homomorphism of transition.
Similarly, $\gr^1(v_{\beta\delta}\circ w_{\beta\lambda})$ is the homomorphism of transition, and the required properties follow.
\end{proof}
...
moreover, for $j\leq n$, as $\gr^j(v_{\alpha\beta})=\varphi_j$ for all $j$, the homomorphism composed

\[
\gr^j(F_{\lambda,n}) \xrightarrow{\varphi_{\lambda\lambda,j}} \gr^j(E_{\lambda,n}) \xrightarrow{\gr^j(w_{\beta\lambda})} \gr^j(F_{\beta,n})
\]

is the homomorphism of transition, since this is true for $j=0$ and $j=1$ by \sref{0.19.5.6.5}, and as $\gr^*(E_{\lambda,n})$ is generated by $\gr^1(E_{\lambda,n})$; composing with the homomorphism of transition $\gr^n(F_{\beta,n})\to\gr^n(F_{\alpha,n})$, one sees therefore, for $j=n$, that one has factorized the homomorphism of transition $\gr^n(F_{\lambda,n})\to\gr^n(F_{\alpha,n})$ as
...
Applying the lemma \sref{0.19.5.6.7} to $v_{\alpha\beta}$, there exists a $\delta\geq\gamma$ and a $u_{\gamma\delta}:E_{\delta,n}\to F_{\gamma,n}$, so that the composite $v_{\alpha\beta}\circ u_{\gamma\delta}$ is the homomorphism of transition.
...
Let $\mathfrak{r}$ be an ideal of definition of $C$, so that the $\mathfrak{r}^n$ form a fundamental system of neighborhoods of $0$ in $C$.
...
On the other hand, the $C$-modules $\mathbf{S}_C^n(\mathfrak{J}/\mathfrak{J}^2)$ and $\gr_\mathfrak{J}^n(B)$ are of finite type, and when $C$ is a Zariski ring it is equivalent to say that $\varphi_n$ is bijective or that $\varphi_n\otimes 1_{C_\lambda}$ is bijective for all $\lambda$ (Bourbaki, \emph{Alg. comm.}, chap.~III, \textsection3, n\textsuperscript{o}~5, prop.~9);
...
as $K$ is a field and the composed homomorphism $K\xrightarrow{\varphi}B\to A$ (where $\varphi$ is the canonical injection) is not zero, it is injective.
...
Reciprocally, $b)$ implies $a)$, for the composite of the canonical homomorphisms $\mathbf{Z}\xrightarrow{j}A\to k$ has kernel $p\mathbf{Z}$, and one has $j(p\mathbf{Z})=0$, hence $\Ker(j)=p\mathbf{Z}$ and $A$ contains a ring $A'$
...
\begin{enumerate}
  ...
  \item[b)] The completion $\widehat{A}$ of $A$ is $k$-isomorphic to a formal power series ring $K[[T_1,\ldots,T_n]]$ (where the $k$-algebra structure is defined by the composed homomorphism $k\xrightarrow{\varphi}K\xrightarrow{\psi}K[[T_1,\ldots,T_n]]$, where $\psi$ is the canonical injection and $\varphi$ the homomorphism defining the extension structure of $K$).
...
Finally, $c)$ is verified: it follows from \sref{0.17.1.1},~$a$) and from \sref{0.19.5.4} (equivalence of $b)$ and $a)$), applied to $K=A/\mathfrak{m}$, replacing $C$ by $A$, that $A$ is a formally smooth $k$-algebra.
\end{proof}
...
\begin{proof}
Indeed, for every finite extension $k'$ of $k$, the ring $K\otimes_k k'$ is identified with a sub-ring of $A'=A\otimes_k k'$;
...
One can in certain cases make the corollary \sref{0.19.6.5} more precise:
\end{remark}
...
If one sets $B'=K'[[T_1,\ldots,T_n]]$, the hypothesis that $K'$ is separable over $k'$ entails, by the remarks of the beginning, that for every finite extension $k''$ of $k'$, the local components of the semi-local complete ring $B'\otimes_{k'} k''$ (equal to formal power series algebras $(K'\otimes_{k'}k'')[[T_1,\ldots,T_n]]$) are the field components of $K'\otimes_{k'}k''$, and the $K_i''$ are separable over $k_0'$; since each of these local rings is $k_0'$-isomorphic to a formal power series algebra $K_i''[[T_1,\ldots,T_n]]$, where $K'$ is separable, one sees that the hypothesis $b')$ is still verified when one replaces $k'$ by any of its finite extensions.
...
If $(B',u')$ is a couple satisfying the same conditions as $(B,u)$, there exists an $A$-isomorphism $v:B\xrightarrow{\sim} B'$ making the diagram
\[
\xymatrix{
...
\begin{enumerate}
  \item[\rm(i)] If the structural homomorphism $\rho:A\to B$ is \emph{surjective}, $B$ is a \emph{formally unramified} $A$-algebra, for with the notation of \sref{0.19.10.2}, the composed homomorphism $A\xrightarrow{j}B\xrightarrow{v}E$ must be the structural homomorphism, hence $v$ is unique (if it exists).
