...
We will see further on \sref{IV.17.9.7} that when one further supposes that $X$ is an $S$-prescheme of finite presentation, and on the other hand that $g$ is a monomorphism, then one can assert that $g$ is an \emph{automorphism} of $X$.
\end{proof}
...
Then the prescheme induced on the open subset $X-Y$ is a Jacobson prescheme.
\end{proposition}
...
Suppose first that $A$ is integral and of dimension $1$; by virtue of the Nagata theorem \sref[0]{0.23.1.5} and \sref{IV.23.1.6}, the integral closure $A'$ of $A$ is a complete local integral ring and
...
\begin{env}[10.7.1]
\label{IV.10.7.1}
Let $S$ be a locally Noetherian prescheme of dimension $\leq 1$ and suppose that $S$ is a \emph{prescheme de Jacobson}; when $S$ is Noetherian, this amounts to saying that the irreducible components of $S$ of dimension $1$ are infinite, for every $x\in S$ which is not a closed point is the generic point of such a component \sref{IV.10.4.5} and \sref{IV.10.5.4}.
...
Then $S$ satisfies conditions $1^\circ$, $2^\circ$, and $3^\circ$ of \sref{IV.10.6.1}: indeed, one has already seen that $S$ is a prescheme de Jacobson \sref{IV.10.5.9}; as $A$ is universally catenary, so are all the local rings $A_\mathfrak{p}$ at the prime ideals of $A$ \sref{IV.5.6.3}.
On the other hand, an irreducible component $S'$ of $S$ is the complement of $a$ in an irreducible component $X'$ of $X$; for every closed point $x$ of $S$, the closure of $x$ in $X$ is therefore $\{x,a\}$, hence $\dim(\sh{O}_{X',x})=1$ and hence, as $X'$ is by hypothesis biequidimensional \sref[0]{0.14.3.3}, on~has $\dim(\sh{O}_{X',x})=\dim(X')-1$ \sref{IV.5.1.2}, which proves that $S'$ is equicodimensional.
\end{env}
...
As $A[T_1,\ldots,T_{n+1}]$ is an $A[T_1,\ldots,T_n]$-module free of rank one, there are in $A[T_1,\ldots,T_{n+1}]$ two maximal ideals $\mathfrak{m}'$, $\mathfrak{n}'$ of respective heights $n+1$ and $n+2$; furthermore, by \sref{IV.5.5.3}, these ideals are necessarily of heights $n+1$ and $n+2$, whence our assertion.
Suppose $n\geq 2$ in what follows.
Let $\mathfrak{J}$ be the ideal $\mathfrak{m}\cap\mathfrak{n}=\mathfrak{m}\mathfrak{n}$, which is contained in the \emph{radical} of $B$; if one sets $B'=R^{-1}B$, the ideal $\mathfrak{J}'=R^{-1}\mathfrak{J}$ is contained in the radical of $B'$ as a topological space $X'=\operatorname{Spec}(B')$ is identified with a subspace of $X=\operatorname{Spec}(B)$, and as the points $x$ of $X'$, the local rings $\sh{O}_{X',x}$ and $\sh{O}_{X,x}$ are the same \sref{IV.10.5.7}; one knows \sref{IV.10.5.7} that $S$ is a prescheme de Jacobson, evidently irreducible and Noetherian; furthermore all the local rings $\sh{O}_{S,s}$ are universally catenary for every $s\in S$ by virtue of \sref{IV.5.6.3}, since $A$ is universally catenary.
However, there exist two closed points $a$, $b$ of $S$ such that $\sh{O}_{S,a}$ and $\sh{O}_{S,b}$ \emph{do not have the same dimension}, otherwise $S$ would not be biequidimensional \sref{IV.5.6.4}.
To see this, consider the two maximal ideals $\mathfrak{m}'=R^{-1}\mathfrak{m}$, $\mathfrak{n}'=R^{-1}\mathfrak{n}$ of $B'$, which are of heights $n$ and $n+1$ respectively; we have $\mathfrak{J}'=\mathfrak{m}'\cap\mathfrak{n}'$, and $\mathfrak{J}'$ is contained in no prime ideal of $B'$ distinct from $\mathfrak{m}'$ and $\mathfrak{n}'$, which are therefore the only maximal ideals of $B'$.
Let $a'$, $b'$ be the two closed points of $X'$ corresponding to $\mathfrak{m}'$ and $\mathfrak{n}'$; the closures of $a'$ in $X'$ are $\{a,a'\}$; as $B'_{\mathfrak{m}'}$ is biequidimensional, $\mathfrak{p}'$ is then an ideal of $B'$ of height $n$ such that its closure in $X'$ is $\{a,a'\}$; one constructs in the same way an ideal $\mathfrak{q}'$ of $B'$ of height $n$ such that the closure of $b$ in $X'$ is $\{b,b'\}$.
This being so, $a$ and $b$ are closed points in $S$, hence the question is answered.
\end{env}
...
\begin{proof}
Let us note that for every $s\in S$, the fibre $X_s$ being a prescheme locally of finite type over $k(s)$, is a Jacobson prescheme \sref{IV.10.4.7}; as furthermore $\operatorname{Spec}(k(s))$ satisfies the conditions of \sref{IV.10.6.1}, on~has $\operatorname{prof}^*_x(\sh{F}_{f(x)})=\dim_x(\operatorname{Supp}(\sh{F}_{f(x)}))-\operatorname{coprof}((\sh{F}_{f(x)})_x)$ \sref{IV.10.8.7}.
...
It is clear that in this equivalence, the $\sh{O}_X$-Modules that are locally free (resp.\ coherent) correspond to the $\sh{O}_{S(X)}$-Modules that are locally free (resp.\ coherent); furthermore, if $\sh{L}$ is an $\sh{O}_X$-Module locally free and if $U$ is an open of $X$ such that $\sh{L}|U$ is free, then $j_X^*(\sh{L})|(U\cap S(X))$ is isomorphic to $(\sh{O}_X|U)^n$, i.e.\ is isomorphic to $((\sh{O}_{S(X)}|(U\cap S(X)))^n$.
\end{env}
