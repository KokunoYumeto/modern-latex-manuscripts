...
Set $\mathrm{A}'=\widehat{\mathrm{A}}$, and let $\mathfrak{q}'$ be one of the minimal prime ideals of $\mathrm{A}'$ among those containing $\mathfrak{q}\mathrm{A}'$; if we set $\mathrm{B}=\mathrm{A}_\mathfrak{q}$, $\mathrm{B}'=\mathrm{A}'_{\mathfrak{q}'}$, then $\mathrm{B}'$ is a B-module flat \sref[0]{0.6.3.2}.
Set $\mathrm{C}=\widehat{\mathrm{B}}$, $\mathrm{C}'=\widehat{\mathrm{B}'}$; as $\mathrm{B}'$ is a flat B-module plat, we know that $\mathrm{C}'$ is a flat $\mathrm{C}$-module (Bourbaki, \emph{Alg.\ comm.}, chap.~III, \textsection\,5, n\textsuperscript{o}\,4, prop.~4); as $\mathrm{B}'$ is a flat B-module, it is known that $\mathrm{C}'$ is a flat $\mathrm{C}$-module.
...
$(\mathrm{R_{II}})$ Let $\varphi:\mathrm{A}\to\mathrm{B}$ be a homomorphism of local Noetherian rings making $\mathrm{B}$ a flat $\mathrm{A}$-module \emph{plat}; then $\boldsymbol{R}(\mathrm{B})$ implies $\boldsymbol{R}(\mathrm{A})$.

Moreover, the property $(\mathrm{P_{III}})$ results from the fact that $\boldsymbol{R}(k)$ is true for every field $k$; finally, when $\boldsymbol{P}$ is of the second type, the condition $(\mathrm{P_{IV}})$ is a consequence of the definition of $\boldsymbol{P}$ and of the transitivity of finite extensions of type fini.
\end{env}
...
On the other hand, if $\mathrm{B}$ is a finite A-algebra, $\mathrm{B}$ is a semi-local ring, on has $\widehat{\mathrm{B}}=\mathrm{B}\otimes_\mathrm{A}\widehat{\mathrm{A}}$, and~(ii) follows from \sref{IV.7.3.7}.
\end{proof}
...
\oldpage[IV-2]{197}
On the other hand, if we specialise $c)$ to the case where $\mathrm{B}$ is integral and $y$ the generic point of $\mathrm{Y}=\Spec(\mathrm{B})$, on has $\sh{O}_{\mathrm{X},y}=\boldsymbol{k}(y)$, hence $y\in\mathrm{U}_{\boldsymbol{R}}(\mathrm{Y})$, hence $\boldsymbol{R}(k)$ is true for every field $k$; by expressing that every point $y'\in g^{-1}(y)$ belongs to $\mathrm{U}_{\boldsymbol{R}}(\mathrm{Y}')$, we obtain the statement $b)$, hence $c)$ implies $b)$.
...
If we set $\mathfrak{p}=\mathfrak{I}_x$, $\boldsymbol{k}(x)$ is the field of fractions of $\mathrm{A}/\mathfrak{p}$, and if there is a base of $k$ over $\boldsymbol{k}(x)$ formed of integral elements over $\mathrm{A}/\mathfrak{p}$; if $\mathrm{B}$ is the subring of $k$ generated by these elements, $\mathrm{B}$ is an integral finite A-algebra, and $k$ is the field $\boldsymbol{k}(\xi)$ at the generic point $\xi$ of $\Spec(\mathrm{B})$; as $x$ is the image of $\xi$ in $\Spec(\mathrm{A})$, the fibre $g^{-1}(\xi)$ is nothing other than $f^{-1}(x)\otimes_{\boldsymbol{k}(x)}k$, which completes the proof of the lemma.
\end{proof}
...
\begin{enumerate}[label=\emph{\roman*)}]
  ...
  For~(i), (ii) and~(iii), this follows from \sref{IV.6.11.2} and the theorem of Cohen \sref[0]{0.19.8.8}; when $\boldsymbol{P}$ is one of the properties~(iv), (iv'), (v), (v'), this follows from \sref{IV.6.12.7} and \sref{IV.6.12.9}, and when $\boldsymbol{P}$ is one of the properties~(vi) and (vi'), the assertion of \sref{IV.7.3.18} is trivial, being true for every locally Noetherian prescheme.

  \item We have already noted \sref{IV.6.4.3} that when $\boldsymbol{P}$ is one of the properties~(ii) or $(\mathrm{iii}_1)$ of \sref{IV.7.3.8}, we do not know if every local Noetherian ring is a $\boldsymbol{P}$-ring.
  We also do not know if, in the equivalent conditions $c)$, $d)$, $e)$ of \sref{IV.7.2.5}, one can replace the hypothesis that $\mathrm{A}$ is universally catenary by the hypothesis that $\mathrm{A}$ is catenary; one can show that this is the case when $\mathrm{A}$ is Henselian \sref{IV.18.9.6}.
\end{enumerate}
\end{remarks}
...
\begin{proof}
Let $(x_i)_{1\leqslant i\leqslant m}$ be a system of generators of the A-algebra $\mathrm{C}$; let us show that we can reason by induction on $m$.
Let $\mathrm{C}'$ be the sub-A-algebra of $\mathrm{C}$ generated by $x_1,\ldots,x_{m-1}$, and let $\mathfrak{n}'=\mathfrak{n}\cap\mathrm{C}'$.
The homomorphism $\mathrm{A}\to\mathrm{C}'_{\mathfrak{n}'}$ and $\mathrm{C}'_{\mathfrak{n}'}\to\mathrm{C}_\mathfrak{n}$ are local homomorphisms; if $k''$ is the residue field of $\mathrm{C}'_{\mathfrak{n}'}$, $k\to k''\to k'$ is factored as $k\to k''\to k'$, hence $k''$ is a finite extension of $k$ and $k'$ a finite extension of $k''$.
The hypothesis of induction implies that $\mathrm{C}'_{\mathfrak{n}'}$ is a $\boldsymbol{P}$-ring, and the hypothesis of induction shows again that $\mathrm{C}_\mathfrak{n}$ is a $\boldsymbol{P}$-ring, which reduces the problem to the case where $\mathrm{C}$ is an A-algebra generated by a \emph{single} element $t$.

Applying \sref{IV.7.4.3},~(ii) with $\mathrm{A}'=\widehat{\mathrm{A}}$, $\mathrm{B}'=\mathrm{B}\otimes_\mathrm{A}\mathrm{A}'$ is a local ring of $\mathrm{C}\otimes_\mathrm{A}\mathrm{A}'$ at a prime ideal above the maximal ideal $\mathfrak{m}$; as $\mathrm{A}$ and $\mathrm{A}'$ have the same residue field $k$, the residue field of $\mathrm{C}\otimes_\mathrm{A}\mathrm{A}'$ at this prime ideal is equal to $k'$.
...
Let $\mathrm{B}_1$ be the integral finite B-algebra, hence generated by a finite number of integral elements over $\mathrm{B}$.
...
We see therefore (taking into account \sref{IV.7.3.14} and of~(i)) that we are reduced to the following question: let $\mathrm{C}$ be a local Noetherian integral ring, containing a sub-ring $\mathrm{C}_0$ which is an A-algebra generated by a single element $t$ and such that $\mathrm{C}$ is a $\mathrm{C}_0$-algebra \emph{finite}; if $\mathfrak{n}$ is a maximal ideal of $\mathrm{C}$
\oldpage[IV-2]{201}
...
Let us consider the case where $\mathrm{B}$ is a finite A-algebra; if $\mathfrak{p}$ is the prime ideal of $\mathrm{A}$ above which the maximal ideal of $\mathrm{B}$ lies, $\mathrm{B}$ is also an $\mathrm{A}_\mathfrak{p}$-algebra essentially of finite type \sref{IV.1.3.8}; by virtue of~(i), it is enough to show that $\mathrm{C}_\mathfrak{n}$ is a $\boldsymbol{P}$-ring, hence reduce to proving the lemma for the case where $\mathrm{A}$ is complete and $\mathrm{C}$ generated by a single element.
In the general case, if we set $\mathfrak{p}=\mathfrak{I}_x$, $\mathrm{C}/\mathfrak{q}$ and $\mathrm{B}/\mathfrak{p}$ are respectively $(\mathrm{A}/\mathfrak{r})[\bar{t}]$-algebra finite, where $\mathfrak{r}=\mathfrak{p}\cap\mathrm{A}$; in the general case, if we set $\mathfrak{q}=\mathfrak{p}\cap\mathrm{C}$, $\mathrm{C}/\mathfrak{q}$ is a $k$-algebra of finite type, hence $k'$ of $\mathrm{C}$ at the maximal ideal $\mathfrak{n}$ is an extension \emph{finite} of the residue field $k$ of $\mathrm{A}$ \sref[I]{I.6.4.2}; by virtue of (i), it suffices therefore to show that $\mathrm{B}$ is a $\boldsymbol{P}$-ring.
This was carried out in \sref[0]{0.22.3.3} and \sref[0]{0.22.5.8}.
\end{proof}

\begin{lemma}[7.4.4.2]
\label{IV.7.4.4.2}
Let $\mathrm{A}$ be a local Noetherian integral ring and complete, $k$ its residue field, $\mathrm{C}$ an integral ring containing $\mathrm{A}$, such that there exists $t\in\mathrm{C}$ for which $\mathrm{C}$ is an $\mathrm{A}[t]$-algebra finite.
...
\begin{proof}
The assertion to prove in order to obtain \sref{IV.7.4.4.1} will be deduced from this lemma by observing that, since $\mathrm{B}$ is integral, the generic point of $\mathrm{X}$ belongs to $\mathrm{U}$.
We note that since $\mathrm{C}$ is an A-algebra of finite type, and $\mathfrak{n}$ a maximal ideal of $\mathrm{C}$, the residue field $k'$ of $\mathrm{C}_\mathfrak{n}=\mathrm{B}$ (hence also of $\mathrm{B}'$) is a finite extension of $k$ \sref[I]{I.6.4.11} and \sref{IV.6.4.2}.

It follows from \sref{IV.6.12.8} that $\operatorname{Reg}(\mathrm{Y})$ is open in $\mathrm{Y}$; on the other hand \sref{IV.6.12.7}, $\mathrm{U}'$ is open in $\mathrm{X}'$, hence $\mathrm{S}'=\mathrm{X}'-\mathrm{U}'$ is closed; hence $\mathrm{S}'\cap f^{-1}(\mathrm{U})$ is \emph{locally closed} in $\mathrm{X}'$, and everything comes down to proving that this set is \emph{empty}.
We know \sref{IV.5.1.10} that in the contrary case, there would exist in $\mathrm{S}'\cap f^{-1}(\mathrm{U})$ a prime ideal $\mathfrak{p}'$ such that $\dim(\mathrm{B}'/\mathfrak{p}')\leqslant 1$.
Let us remark first that $\mathfrak{p}'$ cannot be the maximal ideal $\mathfrak{m}\mathrm{B}'$ of $\mathrm{B}'$, where $\mathfrak{m}$ is the maximal ideal of $\mathrm{B}$.
Indeed, this would mean that $\mathrm{B}=\mathrm{B}_\mathfrak{m}$ would be regular, hence also $\mathrm{B}'=\widehat{\mathrm{B}}$ \sref[0]{0.17.1.5}, and we would have $\mathfrak{m}\mathrm{B}'\in\mathrm{U}'$ contrary to the hypothesis.
We must therefore have $\dim(\mathrm{B}'/\mathfrak{p}')=1$.
Set $\mathfrak{p}=\mathfrak{p}\cap\mathrm{C}$, $\mathfrak{r}=\mathfrak{p}\cap\mathrm{A}$, $\mathrm{C}/\mathfrak{q}$ is a $(\mathrm{A}/\mathfrak{r})[\bar{t}]$-algebra finite (where $\bar{t}$ is the class of $t$ mod.~$\mathfrak{q}$); as $\mathfrak{p}=\mathfrak{q}\mathrm{B}$, $\mathrm{B}/\mathfrak{p}$ is equal to $(\mathrm{C}/\mathfrak{q})_{\mathfrak{n}/\mathfrak{q}}$, and $\mathfrak{n}/\mathfrak{q}$ is a maximal ideal of $\mathrm{C}$ (necessarily above $\mathfrak{m}/\mathfrak{r}$ of $\mathrm{A}/\mathfrak{r}$); we see therefore that the hypotheses of \sref{IV.7.4.4.2} are fulfilled by $\mathrm{A}/\mathfrak{r}$, $\mathrm{C}/\mathfrak{q}$ and $\mathrm{B}/\mathfrak{p}$, and as the completion of $\mathrm{B}/\mathfrak{p}$ is $\mathrm{B}'/\mathfrak{p}\mathrm{B}'$, this proves our assertion.
Let us suppose therefore $\mathfrak{p}=0$.
But then $\mathrm{A}\subset\mathrm{V}$ and $\mathrm{V}$ is \emph{a fortiori} an A-algebra finite, and as the local ring $\mathrm{A}$ is complete, it is the same for $\mathrm{B}$ (Bourbaki, \emph{Alg.\ comm.}, chap.~IV, \textsection\,2, n\textsuperscript{o}\,5, cor.~3 of prop.~9), whence $\mathrm{B}'=\mathrm{B}$ is a field, and so by $\mathrm{B}$ a subring of $\mathrm{B}$ also reduced.
\end{proof}
...
Let $\mathrm{A}$ be a Noetherian ring, $\mathfrak{J}$ an ideal of $\mathrm{A}$, $\widehat{\mathrm{A}}$ the separated completion of $\mathrm{A}$ for the $\mathfrak{J}$-preadic topology.
...
We know (Bourbaki, \emph{Alg.\ comm.}, chap.~III, \textsection\,3, n\textsuperscript{o}\,4, prop.~8) that the canonical homomorphism $\mathrm{A}_\mathfrak{m}\to(\widehat{\mathrm{A}})_\mathfrak{n}$ is injective, that the topology $\mathfrak{m}\mathrm{A}_\mathfrak{m}$-preadic on $\mathrm{A}_\mathfrak{m}$ is induced by the topology $\mathfrak{n}(\widehat{\mathrm{A}})_\mathfrak{n}$-preadic, and that $\mathrm{A}_\mathfrak{m}$ is dense in $(\widehat{\mathrm{A}})_\mathfrak{n}$ for this topology, so that the completion of $\mathrm{A}_\mathfrak{m}$ for the $\mathfrak{m}\mathrm{A}_\mathfrak{m}$-preadic topology identifies with that of $(\widehat{\mathrm{A}})_\mathfrak{n}$ for the $\mathfrak{n}(\widehat{\mathrm{A}})_\mathfrak{n}$-preadic topology.
...
\[
  \Spec(\mathrm{A}[[\mathrm{T}_1,\ldots,\mathrm{T}_r]])\xrightarrow{l}\Spec(\mathrm{A}[\mathrm{T}_1,\ldots,\mathrm{T}_r])\xrightarrow{g}\Spec(\mathrm{A}).
\]
It is clear that the morphism $g$ is regular \sref{IV.0.17.3.7}; by virtue of \sref{IV.7.4.5}, $\mathrm{A}[\mathrm{T}_1,\ldots,\mathrm{T}_r]$ is a $\boldsymbol{P}$-ring, hence it follows from \sref{IV.7.4.6} that $f$ is a $\boldsymbol{P}$-morphism; as $g$ is regular, hence also a $\boldsymbol{P}$-morphism \sref{IV.7.3.5},~(i), it follows from $(\mathrm{P}'_\mathrm{I})$ that $g\circ f$ is a $\boldsymbol{P}$-morphism.

One will note that the conclusion is still valid if instead of supposing that $\boldsymbol{P}$ satisfies $(\mathrm{P}'_\mathrm{I})$, one only supposes that a composite $g\circ f$ is a $\boldsymbol{P}$-morphism when $g$ is regular and $f$ a $\boldsymbol{P}$-morphism (symmetric condition to that of $(\mathrm{P}'_\mathrm{I})$ in some sense).
\end{proof}
...
\begin{enumerate}[label=\rm\Alph*)]
  ...
  It would follow that for every $\boldsymbol{P}$-ring Noetherian $\mathrm{A}$ and every ideal $\mathfrak{J}$ of $\mathrm{A}$, the separated completed $\widehat{\mathrm{A}}$ for the $\mathfrak{J}$-preadic topology would also be a $\boldsymbol{P}$-ring.
...
\end{enumerate}
The properties (i), (iii) and the first assertion of (ii) are particular cases of \sref[0]{0.19.7.1.2}. To prove that $\mathrm{C}$ is a flat $\mathrm{B}$-module, let us note that for all $h>0$, $\mathrm{C}/\mathfrak{m}^h\mathrm{C}=(\mathrm{A}/\mathfrak{m}^h)\otimes_k\mathrm{B}$ is a flat $\mathrm{B}$-module, since $k$ is a field; it therefore suffices to apply \sref[0]{0.10.2.6} to each of the local components of $\mathrm{C}$ (whose $\mathrm{C}$ is the direct composite). Finally, if $\mathfrak{n}$ is the maximal ideal of $\mathrm{B}$, the residue fields of the components of $\mathrm{C}$ are also those of the Artinian ring $(\mathrm{A}/\mathfrak{m})\otimes_k(\mathrm{B}/\mathfrak{n})$, which, by hypothesis, are finite extensions of $\mathrm{B}/\mathfrak{n}$.
\end{lemma}

\begin{proposition}[7.5.6]
\label{IV.7.5.6}
Let $k$ be a field, $\mathrm{A}$, $\mathrm{B}$ two local Noetherian complete rings containing $k$, and whose residue fields are finite extensions of $k$. Let $\boldsymbol{R}$ be a property satisfying the conditions $(\mathrm{R}'_i)$,~$(\mathrm{R}_{\mathrm{III}})$ and $(\mathrm{R}_{\mathrm{IV}})$ (the property $\boldsymbol{P}$ being defined from $\boldsymbol{R}$ by \sref{IV.7.5.0}). Suppose that $\boldsymbol{R}(\mathrm{A})$ is true, as well as $\boldsymbol{P}(\operatorname{Spec}(\mathrm{B}),k)$ (which means that for every finite extension $k'$ of $k$ and for each of the complete local rings $\mathrm{C}_j$ whose direct composite is $\mathrm{B}\otimes_k k'$, $\boldsymbol{R}(\mathrm{B}_j)$ is true). Then, for each of the complete local rings $\mathrm{C}_j$ whose direct composite is $\mathrm{B}\widehat{\otimes}_k\mathrm{B}$, $\boldsymbol{R}(\mathrm{C}_j)$ is true.
\end{proposition}

\begin{proof}
It follows from \sref{IV.7.5.5},~(iii) that each of the homomorphisms $\mathrm{A}\to\mathrm{C}_j$ is local and from \sref{IV.7.5.5},~(ii) that each of the $\mathrm{C}_j$ is a flat $\mathrm{A}$-module; finally, by \sref{IV.7.5.5},~(iv), the residue fields of the $\mathrm{C}_j$ are finite extensions of $\mathrm{A}/\mathfrak{m}$. On the other hand, the property $\boldsymbol{P}(\operatorname{Spec}(\mathrm{C}\otimes_\mathrm{A}(\mathrm{A}/\mathfrak{m})),\mathrm{A}/\mathfrak{m})$ is true, as this is the case when the residue fields of $\mathrm{B}$ are among those of $\mathrm{C}/\mathfrak{m}\mathrm{C}=(\mathrm{A}/\mathfrak{m})\otimes_k\mathrm{B}$, where $y$ ranges over $\operatorname{Spec}(\mathrm{B})$ and $x=f(y)$, from the definition of $\boldsymbol{P}$ and the hypothesis that $\boldsymbol{R}(\mathrm{A})$ is true. All the conditions of \sref{IV.7.5.1} are satisfied for the homomorphisms $\mathrm{A}\to\mathrm{C}_j$ and the corresponding morphisms $\operatorname{Spec}(\mathrm{C}_j)\to\operatorname{Spec}(\mathrm{A})$; the conclusion then follows from $(\mathrm{R}'_i)$ and the hypothesis that $\boldsymbol{R}(\mathrm{A})$ is true.
\end{proof}
...
\begin{proof}
I) Suppose $k$ perfect, $\mathrm{A}$ and $\mathrm{B}$ reduced; let $\mathrm{A}_i$ (resp.\ $\mathrm{B}_j$) be the quotients of $\mathrm{A}$ (resp.\ $\mathrm{B}$) by its prime minimal ideals $(1\leqslant i\leqslant r$, $1\leqslant j\leqslant s)$, which are complete; the hypothesis entails that $\mathrm{A}$ (resp.\ $\mathrm{B}$) identifies with a subring of the direct composite of the $\mathrm{A}_i$ (resp.\ $\mathrm{B}_j$); the tensor products being taken over the base field, and as the tensor products of integral domains are subrings, $\mathrm{A}\otimes_k\mathrm{B}$ identifies with a subring of the direct composite of the $\mathrm{A}_i\otimes_k\mathrm{B}_j$. It follows that $\mathrm{A}\widehat{\otimes}_k\mathrm{B}$ identifies with a subring of the direct composite of the $\mathrm{A}_i\widehat{\otimes}_k\mathrm{B}_j$, and we are thus reduced to the case where $\mathrm{A}$ and $\mathrm{B}$ are integral. By the theorem of Nagata de finitude \sref[0]{0.23.1.6}, $\mathrm{A}'$ (resp.\ $\mathrm{B}'$) is an $\mathrm{A}$-module (resp.\ $\mathrm{B}$-module) of finite type and a complete local ring. Furthermore, $\mathrm{A}\widehat{\otimes}_k\mathrm{B}$ identifies with a subring of $\mathrm{A}'\widehat{\otimes}_k\mathrm{B}'$: indeed, it will suffice to prove that $\mathrm{A}\widehat{\otimes}_k\mathrm{B}$ identifies with a subring of $\mathrm{A}'\widehat{\otimes}_k\mathrm{B}$, and the latter with a subring of $\mathrm{A}'\widehat{\otimes}_k\mathrm{B}'$. This follows from the lemma below:
...
\begin{proof}
Indeed, on has a canonical isomorphism $\mathrm{M}\otimes_k\mathrm{B}\xrightarrow{\sim}\mathrm{M}\otimes_\mathrm{A}(\mathrm{A}\otimes_k\mathrm{B})$ hence a canonical composite homomorphism
\[
  \varphi:\mathrm{M}\widehat{\otimes}_k\mathrm{B}\to\mathrm{M}\otimes_\mathrm{A}(\mathrm{A}\widehat{\otimes}_k\mathrm{B})\to\mathrm{M}\otimes_\mathrm{A}(\mathrm{A}\widehat{\otimes}_k\mathrm{B})
\]
...
II) Suppose $k$ algebraically closed, $\mathrm{A}$ and $\mathrm{B}$ integral. The reasoning of I) brings us again to the case where $\mathrm{A}$ and $\mathrm{B}$ are \emph{integrally closed}; it follows then from \sref{IV.7.5.6} that $\operatorname{Spec}(\mathrm{A}\widehat{\otimes}_k\mathrm{B})$ is \emph{normal}, hence $\mathrm{A}\widehat{\otimes}_k\mathrm{B}$ is a direct composite of integrally closed local complete rings, and the whole thing comes down to seeing that $\mathrm{A}\widehat{\otimes}_k\mathrm{B}$ is an integral \emph{local} ring; it suffices for this to verify that if $\mathfrak{m}$ and $\mathfrak{n}$ are the maximal ideals of $\mathrm{A}$ and $\mathrm{B}$, $(\mathrm{A}/\mathfrak{m})\otimes_k(\mathrm{B}/\mathfrak{n})$ is an integral local ring (see the proof of \sref[0]{0.19.7.1.2}); but as $\mathrm{A}/\mathfrak{m}$ and $\mathrm{B}/\mathfrak{n}$ are finite extensions of $k$, they are identical to $k$, since $k$ is algebraically closed, whence the conclusion.
\end{proof}
...
The formal fibres of $\mathrm{A}$ are \emph{a fortiori} geometrically reduced, hence the hypothesis that $\mathrm{A}$ is reduced entails that so is $\widehat{\mathrm{A}}$, by virtue of \sref{IV.7.3.17}. Let $\mathfrak{p}_i$ $(1\leqslant i\leqslant n)$ be the prime minimal ideals of $\mathrm{A}$, and let $\mathrm{B}_i=\mathrm{A}/\mathfrak{p}_i$; the formal fibres of the local rings $\mathrm{B}_i$ are then also geometrically normal \sref{IV.7.3.15}, hence the $\widehat{\mathrm{B}}_i$ are also reduced and it follows from \sref[0]{0.23.1.7},~(i) that the integral closure $\mathrm{B}'_i$ of $\mathrm{B}_i$ in its field of fractions is a $\mathrm{B}_i$-module of finite type, hence an $\mathrm{A}$-module of finite type; as $\mathrm{A}'$ is the direct composite of the $\mathrm{B}'_i$ (\textbf{II}, $6.3.8$), one sees that $\mathrm{A}'$ is an $\mathrm{A}$-module of finite type. Let $\mathfrak{m}_j$ $(1\leqslant j\leqslant r)$ be the maximal ideals of the semi-local ring $\mathrm{A}'$; one knows that the completion $\widehat{\mathrm{A}}'$ of $\mathrm{A}'$ identifies with the direct composite of the completions $\widehat{\mathrm{A}}'_{\mathfrak{m}_j}$ of $\mathrm{A}'_{\mathfrak{m}_j}$ (Bourbaki, \emph{Alg.\ comm.}, chap.~III, \S~2, no~13, cor.\ de la prop.~19). Now, it follows from the hypothesis and from \sref{IV.7.3.15} that the formal fibres of the $\mathrm{A}'_{\mathfrak{m}_j}$ are geometrically normal; as $\operatorname{Spec}(\mathrm{A}')$ is normal by definition, it follows from each $\operatorname{Spec}(\mathrm{A}'_{\mathfrak{m}_j})$, and one deduces hence from \sref{IV.7.3.17} that $\operatorname{Spec}(\widehat{\mathrm{A}}'_{\mathfrak{m}_j})$ is normal for every $j$, hence also $\operatorname{Spec}(\widehat{\mathrm{A}}')$. On the other hand, $\widehat{\mathrm{A}}'$ identifies with $\mathrm{A}'\otimes_\mathrm{A}\widehat{\mathrm{A}}$ since $\mathrm{A}'$ contains $\mathrm{A}$ and is contained in the total ring of fractions $\mathrm{R}$ of $\mathrm{A}$, and that $\widehat{\mathrm{A}}$ is a flat $\mathrm{A}$-module, every regular element of $\mathrm{A}$ is also $\widehat{\mathrm{A}}$-regular; finally, as $\widehat{\mathrm{A}}$ is a flat $\mathrm{A}$-module, $\widehat{\mathrm{A}}$ contains $\widehat{\mathrm{A}}$ and is contained in $\mathrm{R}'=\mathrm{R}\otimes_\mathrm{A}\widehat{\mathrm{A}}$; hence $\mathrm{R}'$ identifies canonically with a subring of the total ring of fractions $\mathrm{R}''$ of $\widehat{\mathrm{A}}$ (Bourbaki, \emph{Alg.\ comm.}, chap.~IV, \S~2, no~5, cor.~3 de la prop.~9 and chap.~III, \S~3, no~4, th.~3), $\widehat{\mathrm{A}}'$ is normal and that $\widehat{\mathrm{A}}'$ is indeed the integral closure of $\widehat{\mathrm{A}}$ in $\mathrm{R}''$.
\end{proof}

\begin{corollary}[7.6.2]
\label{IV.7.6.2}
Under the hypotheses of \sref{IV.7.6.1}, there is a bijective correspondence between the maximal ideals $\mathfrak{m}_j$ of $\mathrm{A}'$ (in other words, the set of closed points of $\operatorname{Spec}(\mathrm{A}')$ above the closed point of $\mathrm{A}$) and the set of prime minimal ideals $\mathfrak{q}_j$ of $\widehat{\mathrm{A}}$ (in other words, the set of maximal points of $\operatorname{Spec}(\widehat{\mathrm{A}})$); in this correspondence, the completion $\widehat{\mathrm{A}}'_{\mathfrak{m}_j}$ of $\mathrm{A}'_{\mathfrak{m}_j}$ is isomorphic to the integral closure of $\widehat{\mathrm{A}}/\mathfrak{q}_j$.
\end{corollary}

\begin{proof}
Indeed, one knows that the integral closure of $\widehat{\mathrm{A}}$ in its total ring of fractions is the direct composite of the integral closures of the $\widehat{\mathrm{A}}/\mathfrak{q}_j$, which are local complete rings \sref[0]{0.23.1.6}.
\end{proof}

\begin{corollary}[7.6.3]
\label{IV.7.6.3}
Under the hypotheses of \sref{IV.7.6.1}, for $\widehat{\mathrm{A}}$ to be integral, it is necessary and sufficient that $\mathrm{A}$ be unibranch; for $\widehat{\mathrm{A}}$ to be geometrically unibranch, it is necessary and sufficient that $\mathrm{A}$ be.
\end{corollary}
...
To show that $a)$ implies $b)$, it suffices to verify that the $\mathrm{L}_i$ are separable extensions of $\mathrm{K}$, or still \sref{IV.4.6.1} that for every finite extension $\mathrm{K}'$ of $\mathrm{K}$, the ring $\widehat{\mathrm{B}}\otimes_\mathrm{B}\mathrm{K}'$ is reduced. It is thus when $\mathrm{B}'$ is a finite integral extension of $\mathrm{B}$, the nonzero elements of $\mathrm{B}'$ are $\widehat{\mathrm{B}}'$-regular, which entails that $\widehat{\mathrm{B}}'\otimes_{\mathrm{B}'}\mathrm{K}'$ identifies with a subring of the total ring of fractions of $\widehat{\mathrm{B}}'$, and \emph{a fortiori} is reduced.

To see that $a')$ implies $a'')$, consider any point $x$ of $\mathrm{X}=\operatorname{Spec}(\mathrm{A})$, and let $\mathrm{Y}$ be the reduced integral closed sub-prescheme of $\mathrm{X}$ having $\{x\}$ as underlying space; one has $\mathrm{Y}=\operatorname{Spec}(\mathrm{B})$, where $\mathrm{B}$ is an integral quotient ring of $\mathrm{A}$, and $\operatorname{Spec}(\widehat{\mathrm{B}})=\mathrm{Y}\times_\mathrm{X}\operatorname{Spec}(\widehat{\mathrm{A}})$, so that the formal fibre of $\mathrm{A}$ at the point $x$ is the same as that of $\mathrm{B}$, or still equal to $\operatorname{Spec}(\widehat{\mathrm{B}}\otimes_\mathrm{B}\mathrm{K})$. As the local rings of $\operatorname{Spec}(\widehat{\mathrm{B}}\otimes_\mathrm{B}\mathrm{K})$ are those of $\operatorname{Spec}(\widehat{\mathrm{B}})$ at the points of the fibre of $x$, the hypothesis implies that $\widehat{\mathrm{B}}\otimes_\mathrm{B}\mathrm{K}$ is reduced, and the conditions of $a')$ imply $\widehat{\mathrm{B}}\otimes_\mathrm{B}\mathrm{K}$ is a separable $\mathrm{K}$-algebra \sref{IV.4.6.1}.
...
The fact that $a')$ implies $b)$ is a particular case of \sref[0]{0.23.1.7}. It remains to show, to prove the equivalence of $a)$, $b)$ and $c)$, that $b)$ implies $a)$. Let us note that if $\mathrm{C}$ is an $\mathrm{A}$-algebra finite, for every prime ideal $\mathfrak{q}$ of $\mathrm{C}$, $\mathrm{C}/\mathfrak{q}$ is an $(\mathrm{A}/\mathfrak{p})$-algebra finite, where $\mathfrak{p}$ is the inverse image of $\mathfrak{q}$ in $\mathrm{A}$ which is a prime ideal, hence the hypothesis $b)$ entails that every integral quotient ring of $\mathrm{C}$ is a Japanese ring. One is thus reduced to proving that under the hypothesis $b)$, \emph{the completion of every integral quotient ring of $\mathrm{A}$ is reduced}. We argue by induction on $n=\dim(\mathrm{A})$, the assertion being trivial for $n=0$; by replacing $\mathrm{A}$ by the quotients of $\mathrm{A}$ by its prime minimal ideals $\mathfrak{p}_i$, one can restrict to the case where $\mathrm{A}$ is integral (every integral quotient ring of $\mathrm{A}$ being a quotient of one of the $\mathrm{A}/\mathfrak{p}_i$). For every prime ideal $\mathfrak{p}\neq 0$, the hypothesis of induction shows already that the completion of $\mathrm{A}/\mathfrak{p}$ is reduced, and it suffices therefore to prove that $\widehat{\mathrm{A}}$ is reduced. Furthermore, the integral closure $\mathrm{A}'$ of $\mathrm{A}$ is by hypothesis an $\mathrm{A}$-module of finite type, hence a semi-local Noetherian ring and $\widehat{\mathrm{A}}$ identifies with a subring of $(\sref[0]{0.7.3.3})$ and Bourbaki, \emph{Alg.\ comm.}, chap.~IV, \S~2, no~5, cor.~3 de la prop.~9); it will therefore suffice to prove that $\widehat{\mathrm{A}}'$ is reduced; as one has seen above, the hypothesis $b)$ is also satisfied by $\mathrm{A}'$, which is moreover of dimension $n$ \sref[0]{0.16.1.5}; one can therefore restrict to the case where $\mathrm{A}$ is \emph{integrally closed}. Let $t\neq 0$ be an element of the radical of $\mathrm{A}$, and let $\mathfrak{q}_j$ $(1\leqslant j\leqslant n)$ be the prime minimal ideals among those containing $t\mathrm{A}$; one has the following properties:
\begin{enumerate}[label=(\roman*)]
  \item $t$ is regular.
...
\begin{proof}
Set for simplicity $\mathrm{A}'=\widehat{\mathrm{A}}$, and consider $t$ as an element of $\mathrm{A}'$; one has $\mathrm{A}'/t\mathrm{A}'=(\mathrm{A}/t\mathrm{A})\otimes_\mathrm{A}\mathrm{A}'$, and as $\mathrm{A}'$ is a flat $\mathrm{A}$-module, it follows from \sref{IV.3.3.1} that the prime ideals of $\mathrm{A}'$ associated to the $\mathrm{A}'$-module $\mathrm{A}'/t\mathrm{A}'$ are the prime ideals $\mathfrak{q}'_{jh}$ such that $\mathfrak{q}'_{jh}$ lies above $\mathfrak{q}_j$ and is associated to the $k_j$-module $(\mathrm{A}'/t\mathrm{A}')\otimes_\mathrm{A} k_j$, where $k_j$ is the field of fractions of $\mathrm{A}/\mathfrak{q}_j$. Now, $\operatorname{Spec}((\mathrm{A}'/t\mathrm{A}')\otimes_\mathrm{A} k_j)$ is the formal fibre of $\mathrm{A}$ at the point $\mathfrak{q}_j$, or also that of $\mathrm{A}/\mathfrak{q}_j$ (generic point of $\operatorname{Spec}(\mathrm{A}/\mathfrak{q}_j)$). By virtue of (iv) the completion $\mathrm{A}'/\mathfrak{q}_j\mathrm{A}'$ of $\mathrm{A}/\mathfrak{q}_j$ is reduced, it follows that it is of the same for the formal fibre of $\mathrm{A}/\mathfrak{q}_j$ at the generic point of $\operatorname{Spec}(\mathrm{A}/\mathfrak{q}_j)$; this formal fibre therefore has no prime cycle, its irreducible components being fields \sref{IV.3.2.1}. One sees hence by \sref{IV.3.3.3} and the hypothesis (ii) that $\mathfrak{q}_j\mathrm{A}'_{2_{jh}}$ is the maximal ideal of $\mathrm{A}'_{\mathfrak{q}'_{jh}}$ for every $h$; as $\mathrm{A}_{\mathfrak{q}_j}$ is a discrete valuation ring, its maximal ideal $\mathfrak{q}_j\mathrm{A}_{\mathfrak{q}_j}$ is principal, hence the maximal ideal of the local Noetherian ring $\mathrm{A}'_{\mathfrak{q}'_{jh}}$ is principal, which entails that this ring is a discrete valuation ring (Bourbaki, \emph{Alg.\ comm.}, chap.~VI, \S~3, no~6, prop.~9). We have just verified the hypotheses (i) to (iv) for the \emph{complete local ring} $\mathrm{A}'$. It therefore suffices to show that if $\mathrm{A}$ is complete and satisfies the hypotheses (i) to (iii) ((iv) being automatically satisfied in this case),

then $\mathrm{A}$ is reduced. Now, the hypotheses (i) and (ii) imply that, if $\varphi:\mathrm{A}\to\prod_{j=1}^n\mathrm{A}_{\mathfrak{q}_j}$ is the canonical homomorphism, one has $t^n\mathrm{A}=\varphi^{-1}(\varphi(t^n\mathrm{A}))$ \sref{IV.3.4.9}; as the canonical image of $t\mathrm{A}$ in the discrete valuation ring $\mathrm{A}_{\mathfrak{q}_j}$ is a power $\mathfrak{q}_j^h\mathrm{A}_{\mathfrak{q}_j}$ of the maximal ideal, one sees that on $\mathrm{A}$ the $(t\mathrm{A})$-preadic topology is the inverse image by $\varphi$ of the topology induced by the product on $\prod_{j=1}^n\mathrm{A}_{\mathfrak{q}_j}$. Now, as $t$ belongs to the radical of $\mathrm{A}$, the $(t\mathrm{A})$-preadic topology is separated, hence $\varphi$ is \emph{injective}. But by virtue of the hypothesis (iii), the $\mathrm{A}_{\mathfrak{q}_j}$ are integral, hence $\prod_{j=1}^n\mathrm{A}_{\mathfrak{q}_j}$ is reduced, and \emph{a fortiori} it follows that so is $\mathrm{A}$.
\end{proof}
\end{proof}
...
be reduced and that, if $\mathfrak{q}_j$ $(1\leqslant j\leqslant n)$ are the prime minimal ideals of $\widehat{\mathrm{A}}$, the fields of fractions of the integral rings $\widehat{\mathrm{A}}/\mathfrak{q}_j$ be separable extensions of $\mathrm{K}$.
\end{corollary}
...
(v) Let $\mathrm{A}$ be an excellent ring, $\mathfrak{J}$ an ideal of $\mathrm{A}$, $\widehat{\mathrm{A}}$ the $\mathfrak{J}$-separated completion of $\mathrm{A}$ for the $\mathfrak{J}$-preadic topology. Then the canonical morphism $f:\operatorname{Spec}(\widehat{\mathrm{A}})\to\operatorname{Spec}(\mathrm{A})$ is regular (in other words, flat and with geometrically regular fibres). If we set $\mathrm{X}=\operatorname{Spec}(\mathrm{A})$, $\mathrm{X}'=\operatorname{Spec}(\widehat{\mathrm{A}})$, and use the notations of (iv) and further setting $\sh{F}'=\sh{F}\otimes_{\sh{O}_\mathrm{X}}\sh{O}_{\mathrm{X}'}$, we have
...
The first assertion of (ix) is a consequence of \sref{IV.5.6.3},~(i) and \sref{IV.5.11.4}; the second is a consequence of (vi) and \sref{IV.5.11.4}, if one observes that when $\sh{F}$ is torsion-free, $\operatorname{Ass}(\sh{F})$ is reduced to the generic point of $\operatorname{Spec}(\mathrm{A})$. Finally, to prove (x), it suffices to observe that $\mathrm{B}$ is universally catenary and universally Japanese by (ii); as $\mathrm{B}$ is an excellent local ring by (ii), the ring $\mathrm{B}^{(1)}$ is a finite $\mathrm{B}$-algebra \sref{IV.5.11.2}, hence $\mathrm{A}_\mathfrak{m}$ is formally catenary, and the same holds for every $\mathrm{A}_\mathfrak{p}$ for every prime ideal $\mathfrak{p}$ of $\mathrm{A}$, hence also $\mathrm{A}$. This shows that $\mathrm{A}$ is strictly formally catenary by \sref{IV.7.2.5},~$c)$) and completes the proof of (x).
\end{proof}
...
(ii) The catenary local ring $\mathrm{A}$ constructed in \sref{IV.5.6.11} satisfies the conditions (ii) and (iii) (but not condition (i) of the definition \sref{IV.7.8.2}). Indeed, the formal fibre at the maximal ideal $\mathfrak{n}\mathrm{C}_\mathfrak{n}$ of $\mathrm{A}$ is geometrically regular, and that at the other prime ideals $\mathfrak{p}$ of $\mathrm{A}$ coincides with that of the ring $\mathrm{E}$ at these prime ideals;

\oldpage[IV-2]{217}

hence the condition \sref{IV.6.12.4},~$a)$) is also satisfied for $\mathrm{A}$ by virtue of \sref{IV.7.8.3},~(v); this proves hence that $\mathrm{A}$ satisfies the condition \sref{IV.7.8.2},~(ii). On the other hand, the complement of the closed point in $\operatorname{Spec}(\mathrm{A})$ is isomorphic to an open affine, whose ring of affine functions is the ring of a

polynomial ring; by taking the body $k_0$ of characteristic~$0$ \sref{IV.7.8.3},~(iii); consequently $\mathrm{E}$ is an excellent ring by \sref{IV.7.8.3},~(ii) and this proves that $\mathrm{A}$ satisfies the condition \sref{IV.7.8.2},~(ii).
...
(i$'$) \emph{For every prime minimal ideal} $\mathfrak{p}_i$ \emph{of} $\mathrm{A}$ $(1\leqslant i\leqslant r)$, \emph{let} $\mathrm{A}'_i$ \emph{be the integral closure of the integral ring} $\mathrm{A}_i=\mathrm{A}/\mathfrak{p}_i$; \emph{then, for every maximal ideal} $\mathfrak{m}'$ \emph{of} $\mathrm{A}'_i$, \emph{denoting by} $\mathfrak{m}$ \emph{the inverse image of} $\mathfrak{m}'$ \emph{in} $\mathrm{A}$,
...
Indeed, one has seen in a Remark (i) that under only the hypotheses (ii) and (iii) of \sref{IV.7.8.2}, the analogues of the properties (ii), (v), (vi) and (vii) of \sref{IV.7.8.3} are still valid. One deduces hence from (vi) that the $\mathrm{A}'_i$ satisfy (ii) and (iii), and that $((\mathrm{A}'_i)_{\mathfrak{m}'})^\wedge$ are integral and integrally closed. Let now $\mathfrak{m}$ be an arbitrary maximal ideal of $\mathrm{A}$; for every $i$ such that $\mathfrak{p}_i\subset\mathfrak{m}$, the integral closure of $\mathrm{A}_\mathfrak{m}/\mathfrak{p}_i\mathrm{A}_\mathfrak{m}$ is an $(\mathrm{A}_\mathfrak{m}/\mathfrak{p}_i\mathrm{A}_\mathfrak{m})$-algebra finite by (vi), hence semi-local, and its local components are of the form $(\mathrm{A}'_i)_{\mathfrak{m}'}$, where $\mathfrak{m}'$ is a prime ideal (not necessarily maximal) of $\mathrm{A}'_i$ above $\mathfrak{m}/\mathfrak{p}_i$; it follows then from (vii), from what precedes and from \sref{IV.7.8.4.1} that the quotients of the completion $(\mathrm{A}_\mathfrak{m}/\mathfrak{p}_i\mathrm{A}_\mathfrak{m})^\wedge$ by its prime minimal ideals all have the same dimension equal to $\dim(\mathrm{A}_\mathfrak{m})$. Consequently (\sref{IV.7.1.9} and \sref{IV.7.1.8},~$b)$), $\mathrm{A}_\mathfrak{m}$ is formally catenary, and \emph{a fortiori} \sref{IV.7.1.11} universally catenary; it follows that so is $\mathrm{A}_\mathfrak{p}$ for every prime ideal $\mathfrak{p}$ of $\mathrm{A}$ \sref{IV.5.6.3},~(i), hence also $\mathrm{A}$.
...
Let us note that these conditions are satisfied when one takes for $\mathcal{C}$ the category of \emph{excellent} preschemes and for $\boldsymbol{R}$ one of the properties (i) to (viii) of \sref{IV.7.5.3}; for condition 1${}^\circ$, this follows from \sref{IV.7.8.3},~(ii)); for condition 2${}^\circ$, from the reasoning of \sref{IV.7.5.3}, and for conditions 3${}^\circ$ and 4${}^\circ$, from the reasoning of \sref{IV.7.5.3}, taking into account that every excellent ring is catenary. Finally, for condition 4${}^\circ$, it follows, for (i), (ii) and (iii) of \sref{IV.6.7.1}; for (iv), (v), (vi) of \sref{IV.6.7.7}, and for (viii) of \sref{IV.4.6.1}; as concerns property (vii) of \sref{IV.7.5.3}, the corresponding property $\boldsymbol{P}(\mathrm{Z},k)$ implies that $\mathrm{Z}$ is locally integral (being locally Noetherian) and that each of the integral sub-preschemes of $\mathrm{Z}$ whose sum is geometrically integral, one also concludes the property 4${}^\circ$ in this case.

With these notations and hypotheses:

\begin{proposition}[7.9.8]
\label{IV.7.9.8}
Let $\mathrm{A}$ be a local Noetherian ring, $k$ its residue field, $\mathrm{B}$ an integral quotient ring of $\mathrm{A}$ such that $\operatorname{Spec}(\mathrm{B})\in\mathcal{C}$, $\varphi:\mathrm{A}\to\mathrm{B}$ a local homomorphism making $\mathrm{B}$ a flat $\mathrm{A}$-module; set $\mathrm{Y}=\operatorname{Spec}(\mathrm{A})$, $\mathrm{X}=\operatorname{Spec}(\mathrm{B})$ and let $f:\mathrm{X}\to\mathrm{Y}$ be the morphism corresponding to $\varphi$. Suppose that:
...
This being so, there exists by hypothesis a regular prescheme $\mathrm{Y}'$ and a morphism proper and birational $g:\mathrm{Y}'\to\mathrm{Y}$; as $\mathrm{Y}'$ is Noetherian and locally integral, the hypothesis that $g$ is birational implies that $\mathrm{Y}'$ is integral. As the morphism $g':\mathrm{X}'=\mathrm{X}\times_\mathrm{Y}\mathrm{Y}'\to\mathrm{X}$ is also birational, and let $f':\mathrm{Y}'\to\mathrm{X}'$ and $g':\mathrm{X}'\to\mathrm{X}'$ be the canonical projections, so that one has a commutative diagram
...
that $\mathrm{U}_{\boldsymbol{R}}(\mathrm{X}')=\mathrm{X}'$, then, designating by $a$ the closed point of $\mathrm{X}$, that $g'^{-1}(a)\subset\mathrm{U}_{\boldsymbol{R}}(\mathrm{X}')$. Now, let $x'\in g'^{-1}(a)$, and set $z'=f'(x')$, so that $b=g(z')$ is the closed point of $\mathrm{Y}$. One has therefore $f'^{-1}(z')=f^{-1}(b)\otimes_{\boldsymbol{k}(b)}\boldsymbol{k}(z')$, and as $\boldsymbol{k}(z')$ is an extension \emph{of finite type} of $k$ (since $g$ is proper, hence of finite type), the hypothesis implies, by virtue of condition 4${}^\circ$ of \sref{IV.7.9.7}, that it follows that $\boldsymbol{P}(f'^{-1}(z'),\boldsymbol{k}(z'))$ is true, and as $\sh{O}_{x'}$ is a regular ring and that $\operatorname{Spec}(\sh{O}_{x'})\in\mathcal{C}$ by virtue of condition 1${}^\circ$ of \sref{IV.7.9.7}, the lemma \sref{IV.7.5.1.1} proves that $\boldsymbol{R}(\sh{O}_{\mathrm{X}',x'})$ is true, which completes the proof.
\end{proof}
...
\begin{proof}
We will apply \sref{IV.7.9.8} taking for $\mathcal{C}$ the category of all locally Noetherian preschemes, for $\boldsymbol{R}$ the property of being integral; the reasoning of \sref{IV.7.5.3} and \sref{IV.7.9.7} show that conditions 1${}^\circ$, 2${}^\circ$, 3${}^\circ$ and 4${}^\circ$ of \sref{IV.7.9.7} are satisfied in this case. The hypothesis on $\mathrm{B}\otimes_\mathrm{A} k$ and the proposition \sref{IV.7.9.8} show hence (with the notations of \sref{IV.7.9.8}) that the fibres $f^{-1}(y)$ are geometrically punctually integral for $y\in\mathrm{Y}$; \emph{a fortiori} the $f^{-1}(y)$ for $y\in\mathrm{Y}$ are reduced preschemes, and as $\mathrm{A}$ is integral (and \emph{a fortiori} reduced) \sref{IV.3.3.5}. It remains to prove that $\mathrm{X}=\operatorname{Spec}(\mathrm{B})$ is \emph{irreducible}. Now, the proof of \sref{IV.7.9.8} proves that $\mathrm{X}'$ is locally integral (being locally Noetherian). As the morphism $g':\mathrm{X}'\to\mathrm{X}$ is surjective, it suffices hence to see that $\mathrm{X}'$ is irreducible, or (since $\mathrm{X}'$ is locally integral), that $\mathrm{X}'$ is \emph{connected}. But it suffices for this to prove that the fibre $g'^{-1}(a)$ is \emph{connected}. Indeed, if this point is established, $\mathrm{X}'$ cannot be the sum of two open non-empty preschemes $\mathrm{X}'_1$, $\mathrm{X}'_2$, for one would then have for example $g'^{-1}(a)\cap\mathrm{X}'_2=\varnothing$; but the restriction $\mathrm{X}'_2\to\mathrm{X}$ of $g'$ being proper (since $\mathrm{X}'_2$ is a closed sub-prescheme of $\mathrm{X}'$), $g'(\mathrm{X}'_2)$ would be a closed non-empty part of $\mathrm{X}$ not containing the closed point $a$, which is absurd. Now, one has $g'^{-1}(a)=g^{-1}(b)\otimes_{\boldsymbol{k}(b)}\boldsymbol{k}(a)$; but $g:\mathrm{Y}'\to\mathrm{Y}$ is proper and birational, hence, in the Stein factorisation $\mathrm{Y}'\xrightarrow{v}\mathrm{Y}''\xrightarrow{u}\mathrm{Y}$ of the morphism $g$ (\textbf{III}, $4.3.3$), the morphism finite $u$ is also birational and consequently $\mathrm{Y}''$ is integral. As $\mathrm{A}$ is supposed geometrically unibranch, it follows then from (\textbf{III}, $4.3.4$) that $g^{-1}(b)$ is \emph{geometrically connected}, which completes the proof.
\end{proof}
