...
\label{IV.15.1.1}
\oldpage[IV-3]{223}
Let $\mathrm{Y}$ be a locally Noetherian irreducible prescheme, of generic point $\eta$, $f:\mathrm{X}\to\mathrm{Y}$ a morphism locally of finite type, $y$ a point of $\mathrm{Y}$, $\sh{F}$ a coherent $\sh{O}_\mathrm{X}$-Module;
...
If $\lambda_z(\sh{F}_y)$ (resp.~$\lambda_i(\sh{F}_\eta)$) denotes the geometric length of $\sh{F}_y$ at a point $x\in f^{-1}(y)$ (resp.~that of $\sh{F}_\eta$ at a point $t\in f^{-1}(\eta)$) \sref{IV.4.7.5}, then we have
\[
\tag{15.1.1.1}
\label{IV.15.1.1.1}
...
Finally, if $y=\eta$, there is only a single irreducible component $\mathrm{X}_i$ of $\mathrm{X}$ containing $z$ \sref[0]{0.2.1.8} and on a $z_i=z$, which renders \sref{IV.15.1.1.1} and \sref{IV.15.1.1.2} trivial (without hypotheses on $f$).
...
Furthermore, the morphism projection $f^{-1}(\eta)\otimes_\mathrm{K}\mathrm{K}'\to f^{-1}(\eta)$ is finite, dominant, and radicial \sref[I]{I.3.5.7}, hence is a homeomorphism \sref{IV.2.4.5}.
So $z_i'$ is the unique point of $f^{-1}(\eta)\otimes_\mathrm{K}\mathrm{K}'$ above $z_i$, and it follows from \sref{IV.4.7.9} and \sref{IV.4.7.5} that we have
\[
\tag{15.1.1.3}
\label{IV.15.1.1.3}
...
\tag{15.1.1.5}
\label{IV.15.1.1.5}
  \operatorname{length}((\sh{F}_{y'}')_z)\geqslant\sum_{i=1}^{n}\operatorname{length}((\sh{F}_{\eta'}')_{z_i'}).
\]
...
Let $\zeta$ be the generic point of $h^{-1}(s)$; if $t_i'$ ($1\leqslant i\leqslant e$) are the elements of $\mathfrak{m}$ whose classes $\bmod\mathfrak{m}^2$ are the $t_i$, $\mathrm{B}_{(t_j)}$ is the ring of an affine open neighbourhood of $\zeta$ in $\mathrm{P}$, and we have $\mathbf{k}(\zeta)=k(t_1,\ldots,t_{e-1})$; on the other hand, as $\mathrm{A}$ is integrally closed, one easily verifies that $\mathrm{B}$ is as well (Bourbaki, \emph{Alg.\ comm.}, chap.~V, \S\,1, no~8, cor.~1 of prop.~21), so $\mathrm{B}_{(t_j)}$ is also integrally closed, and hence so is the local ring $\sh{O}_{\mathrm{P},\zeta}$.
Finally, as $\mathrm{A}=\sh{O}_s$ is regular, hence a universally catenary ring \sref{IV.5.6.4}, we have, by virtue of \sref{IV.5.6.5}, $\dim(\sh{O}_{\mathrm{P},\zeta})=\dim(\sh{O}_s)-(e-1)$, since $h$ is a birational morphism, and the local ring of the fibre $h^{-1}(s)$ at its generic point $\zeta$ is of dimension $0$.
...
Suppose that: $1^\circ$~$x$ belongs to a single irreducible component $\mathrm{Z}$ of $f^{-1}(y)$; $2^\circ$~$\sh{F}_y=\sh{F}\otimes_{\sh{O}_\mathrm{Y}}\mathbf{k}(y)$ is geometrically reduced on $\mathbf{k}(y)$, in other words, if $z$ is the generic point of $\mathrm{Z}$, $\lambda_z(\sh{F}_y)=1$ (resp.~$\sh{O}_y$ is regular and $\operatorname{length}((\sh{F}_y)_z)=1$).
...
Let us denote by $t$ a uniformiser of $\mathrm{A}$, set $\mathrm{B}=\sh{O}_z$ and $\sh{F}_z=\mathrm{M}$; condition~(i) implies that $\operatorname{Supp}(\mathrm{M})=\operatorname{Spec}(\mathrm{B})$ \sref[I]{I.9.1.13}.
...
It remains to show that under these conditions $\mathrm{M}$ is an $\mathrm{A}$-module \emph{without torsion} \sref[0]{0.6.3.4}, and for this it suffices to show that $t$ is an $\mathrm{M}$-\emph{regular} element; but this follows from \sref{IV.3.4.6.1}.
\end{proof}
...
\begin{enumerate}[label=(\roman*)]
  \item $f$ is universally open at the generic points of the irreducible components of $f^{-1}(y)$ containing $x$.
  \item $f^{-1}(y)$ is geometrically reduced (on $\mathbf{k}(y)$) at the point $x$ \sref{IV.4.6.9}.
  \item The ring $\sh{O}_y$ is reduced.
\end{enumerate}
...
However, $\sh{O}_y$ is not an $\sh{O}_y$-module without torsion, $\sh{O}_y$ identifies with a subring of $\sh{O}_z$ that contains the zero divisors of $0$ in $\sh{O}_z$; hence $f$ is not flat at the point $x$.
\oldpage[IV-3]{228}
...
An example is provided by taking for $\mathrm{Y}$ a curve having a node, for example $\mathrm{Y}=\operatorname{Spec}(\mathrm{K}[\mathrm{S},\mathrm{T}]/(\mathrm{S}^3-\mathrm{T}^2))$, $\mathrm{K}$ being an algebraically closed field, and for $\mathrm{X}$ its \emph{normalisation} \sref[II]{II.6.3.8}.
If $s$ and $t$ are the classes of $\mathrm{S}$ and $\mathrm{T}$ in $\mathrm{A}=\mathrm{K}[\mathrm{S},\mathrm{T}]/(\mathrm{S}^3-\mathrm{T}^2)$, one verifies immediately that $\mathrm{X}=\operatorname{Spec}(\mathrm{B})$, where $\mathrm{B}=\mathrm{K}[t,l,u]$, $u$ being the element $t/s$ of the field of fractions of $\mathrm{A}$, and since $s=u^2$, $t=u^3$, we also have $\mathrm{B}=\mathrm{K}[u]$, isomorphic to a polynomial ring in one indeterminate over $\mathrm{K}$.
The only point $x$ of $\mathrm{X}$ above the point $y$ corresponding to the maximal ideal $(s)+(t)$ is such that, as the class $\bar{u}$ of $u$ in $\sh{O}_z/\mathfrak{m}_y\sh{O}_z$ satisfies $\bar{u}^2=0$, $f^{-1}(y)$ is not a reduced $\mathbf{k}(y)$-prescheme.
Here $f$ is a finite morphism \sref{IV.2.4.5}, but it is not flat at the point $x$, for if $\sh{O}_z$ were an $\mathrm{A}$-module flat, it would be a free $\sh{O}_y$-module (Bourbaki, \emph{Alg.\ comm.}, chap.~II, \S\,3, no~2, prop.~5), which is absurd.
...
\begin{enumerate}[label=\arabic*)]
  ...
  \item $\mathrm{A}'$ is the integral closure of $\mathrm{A}$, a $\mathrm{A}$-algebra whose maximal ideal is $\mathfrak{m}\mathrm{A}'$, and the residue field $k'$ a non-trivial finite radicial extension of the residue field $k=\mathrm{A}/\mathfrak{m}$ of $\mathrm{A}$.
\end{enumerate}
One can then take $\mathrm{Y}=\operatorname{Spec}(\mathrm{A})$, $\mathrm{X}=\operatorname{Spec}(\mathrm{A}')$, $\sh{F}=\sh{O}_{e_+}$, $y$ and $x$ being the closed points of $\mathrm{Y}$ and $\mathrm{X}$ respectively; the hypotheses~(i) and (iv) of \sref{IV.15.2.2} are trivially satisfied; as $\mathrm{A}$ is of dimension $1$, $f:\mathrm{X}\to\mathrm{Y}$ is radicial, finite, and surjective, hence a universal homeomorphism \sref{IV.2.4.5}, which proves hypothesis~(i) of \sref{IV.15.2.3}.
Finally, it is clear that $f^{-1}(y)$ is reduced.
However, $f$ is not flat at the point $x$, for if $\mathrm{A}'$ were an $\mathrm{A}$-module flat, it would be a free $\mathrm{A}$-module of rank $1$ (since it has the same field of fractions as $\mathrm{A}$) generated by the element $1$ of $\mathrm{A}$ (Bourbaki, \emph{Alg.\ comm.}, chap.~II, \S\,3, no~2, prop.~5), which is absurd.

To construct the rings $\mathrm{A}$ and $\mathrm{A}'$, let us start from an imperfect field $k$ of characteristic $p>0$; let $\mathrm{K}_0=k((\mathrm{T}))$ be the field of formal power series over $k$, $\mathrm{A}_0$ the valuation ring corresponding to the discrete valuation of $\mathrm{K}_0$; the residue field of $\mathrm{A}_0$ is thus $k$.
...
If $\mathfrak{m}'$ is the maximal ideal of $\mathrm{A}'$, one will satisfy conditions~$1)$ and $2)$ by taking $\mathrm{A}=\mathrm{A}_0+\mathfrak{m}'$: indeed, as $\mathrm{A}_0$ is Noetherian and $\mathrm{A}'$ an $\mathrm{A}_0$-module of finite type, $\mathrm{A}$ is a $\mathrm{A}_0$-algebra finite, hence a Noetherian ring; $\mathrm{A}'$ is finite over $\mathrm{A}$, $\mathfrak{m}'$ is the only maximal ideal of $\mathrm{A}$, which is thus a complete local ring, evidently of dimension $1$ (being finite over $\mathrm{A}_0$) and geometrically unibranch since $\mathrm{A}'$ is integrally closed and having the same field of fractions $\mathrm{K}$ as $\mathrm{A}$.
  \item The proof of \sref{IV.15.2.2} shows however that one can weaken condition~(iii) by replacing ``geometrically reduced'' by ``reduced'' when one can apply the valuative criterion of flatness \sref{IV.11.8.1} using only discrete valuation rings $\mathrm{A}'$ whose residue field $k'$ is \emph{separable} over $\mathbf{k}(y)$; indeed, if $x'$ and $z'$ have the same meanings as in \sref{IV.15.2.2}, the radicial multiplicities $\mu_1(\mathbf{k}(z')|k')$ and $\mu_1(\mathbf{k}(z)|\mathbf{k}(y))$ are the same, by virtue of \sref{IV.4.7.3}, hence it follows from \sref{IV.4.7.9} that $\operatorname{length}((\sh{F}_y)_z)$ and $\operatorname{length}((\sh{F}_{y'}')_{z'})$ are equal, and we are again reduced to the case where $\mathrm{Y}$ is the spectrum of a discrete valuation ring and $y$ its closed point.
For example, one will be able to obtain this result further when $\sh{O}_y$ is \emph{unibranch} and dominated by a discrete valuation ring whose residue field is a separable extension of $\mathbf{k}(y)$, by virtue of \sref{IV.11.6.4}.
...
Then:
\begin{enumerate}[label=(\roman*)]
  \item There exists a neighbourhood $\mathrm{U}$ of $x$ in $\mathrm{X}$ such that $f|\mathrm{U}$ is universally open and that, for all $x'\in\mathrm{U}$, $\sh{F}_{f(x')}$ is geometrically reduced on $\mathbf{k}(f(x'))$ at the point $x'$.
...
\oldpage[IV-3]{229}
Taking into account \sref[I]{I.5.1.8}, \sref{IV.4.2.7}, and \sref{IV.4.7.11}, the hypotheses do not change, neither in conclusion~(i), when we replace $\mathrm{Y}$ by $\mathrm{Y}_\mathrm{red}$ and $\mathrm{X}$ by $\mathrm{X}\times_\mathrm{Y}\mathrm{Y}_\mathrm{red}$, so we can suppose $\mathrm{Y}$ \emph{reduced}.
It follows then from \sref{IV.15.2.2} that $\sh{F}$ is $f$-flat at the point $x$, and hence also \sref{IV.11.1.1} in a neighbourhood of $x$, and \emph{a~fortiori} $f$ is universally open in this neighbourhood \sref{IV.2.4.6}.
...
\begin{enumerate}[label=(\roman*)]
  \item $\operatorname{Supp}(\sh{F})=\mathrm{X}$.
  \item $f$ is universally open at the generic points of the irreducible components of $f^{-1}(y)$ containing $x$.
  \item $\sh{F}_y$ is geometrically pointwise integral on $\mathbf{k}(y)$ at the point $x$ \sref{IV.4.6.22}.
...
It follows thus from \sref{IV.14.4.7} and from \sref{IV.15.3.2} that for every irreducible component $\mathrm{X}_i$ of $\mathrm{X}$ containing $x$ (and hence $\mathrm{Z}$), the restriction $f_i$ of $f$ to $\mathrm{X}_i$ is an equidimensional morphism and universally open at the generic point of $\mathrm{Z}$.
...
If we disregard in this statement the hypothesis that $\sh{O}_y$ is regular, one can replace the hypothesis that $\mathrm{Y}$ is geometrically unibranch by that that it is integral, or even integrally closed.
\end{enumerate}
\end{remarks}
...
Indeed (\sref{IV.11.1.1} and \sref{IV.12.1.1},~(vi))), there exists a neighbourhood $\mathrm{U}$ of $x$ such that $\sh{F}|\mathrm{U}$ is $f$-flat and that for all $x'\in\mathrm{U}$, $\sh{F}_{f(x')}$ is a Cohen-Macaulay $\sh{O}_{\mathrm{X}_{f(x')}}$-Module at the point $x'$; this latter property implies that $\sh{F}_{f(x')}$ is equidimensional at the point $x'$ \sref[0]{0.16.5.4}.
...
\label{IV.15.4.3}
Let $f:\mathrm{X}\to\mathrm{Y}$ be a morphism locally of finite presentation, $\sh{F}$ a coherent $\sh{O}_\mathrm{X}$-Module of finite presentation, such that $\operatorname{Supp}(\sh{F})=\mathrm{X}$, $x$ a point of $\mathrm{X}$, $y=f(x)$.
Then (with the notations of \sref{IV.15.4.1}) the following conditions are equivalent:
\begin{enumerate}[label=\emph{\alph*})]
  \item $\sh{F}$ is $f$-flat at the point $x$ and $\sh{F}_y$ is a Cohen-Macaulay $\sh{O}_{\mathrm{X}_y}$-Module at the point $x$.
  \item There exists an open neighbourhood $\mathrm{U}$ of $x$ in $\mathrm{X}$, and a $\mathrm{Y}$-morphism quasi-finite $g:\mathrm{U}\to\mathrm{Y}'=\mathrm{Y}[\mathrm{T}_1,\ldots,\mathrm{T}_n]$ ($\mathrm{T}_i$ indeterminates), such that $\sh{F}|\mathrm{U}$ is $g$-flat at the point $x$.
\end{enumerate}
\end{proposition}
...
The hypothesis that the morphism $g$ is quasi-finite implies that it is the same for $g_y:\mathrm{X}_y\to\mathrm{Y}_y'$ \sref[II]{II.6.2.4}, hence $\mathrm{M}\otimes_\mathrm{A} k$ is a $(\mathrm{B}\otimes_\mathrm{A} k)$-module of finite length \sref[II]{II.6.2.2}; as by hypothesis $\mathrm{M}$ is a Cohen-Macaulay $\mathrm{A}$-module flat and $\mathrm{A}$ a regular ring, $\mathrm{M}$ is a $\mathrm{B}$-module Cohen-Macaulay \sref{IV.6.3.3}.
The fact that $\sh{F}$ is $f$-flat at the point $x$ follows from that it is $g$-flat and from the fact that the morphism $h$ is flat \sref{IV.2.1.6}.

Let us now show that $a)$ implies $b)$.
The question being local on $\mathrm{X}$ and $\mathrm{Y}$, using \sref{IV.8.9.1} and \sref{IV.11.2.7}, we reduce to the case where $\mathrm{X}$ and $\mathrm{Y}$ are Noetherian, taking into account \sref{IV.6.7.1}.
The hypothesis implies that $f$ is open at the generic points of the irreducible components of $\mathrm{X}_y$ containing $x$, by \sref{IV.15.4.2},~$e)$; it follows from \sref{IV.2.3.4} and \sref{IV.13.3.1},~$a)$) that $f$ is equidimensional at the point $x$.
\end{proof}
\subsection{Separable rank of fibres of a quasi-finite and universally open morphism. Application to connected components of fibres of a proper morphism}
...
\label{IV.15.5.2.2}
Under the general hypotheses of \sref{IV.15.5.1}, if $y$ is a point of $Y$, $y'$ a generization of $y$ distinct from $y$, there exists a spectrum of a discrete valuation ring $Z$, of closed point $z$ and generic point $z'$, and a morphism $g:Z\to Y$ such that:
$1^\circ$ $g(z)=y$, $g(z')=y'$;
$2^\circ$ if one sets $f'=f_{(\mathrm{Z})}$, $n(y)$ is the number of points of $f'^{-1}(z)$ and $n(y')$ is the number of points of $f'^{-1}(z')$.
\end{lemma}
...
Denote then by $X_i$ the sub-preschemes of $X$ having for underlying spaces the irreducible components of $X$ ($1\leq i\leq m$).
By virtue of \sref{IV.1.10.4}, the restriction $X_i\to Y$ of $f$ to each $X_i$ is a \emph{dominant} morphism, and since $f^{-1}(y')$ is discrete, each of the intersections $X_i\cap f^{-1}(y')$ is reduced to the generic point of $X_i$, whence (denoting by $n_i(z)$ the geometric number of points of $X_i\cap f^{-1}(z)$ for every $z\in Y$), $n_i(y')=1$ for every $i$, and $n(y')=\sum_{i=1}^{m} n_i(y')=m$.
...
But then, for every $x\in f^{-1}(y)$, we have $\sh{O}_y\subset\sh{O}_z\subset K$, where $K=\kappa(y')$ is the field of fractions of $\sh{O}_y$.
Since $\sh{O}_y$ is a valuation ring and $\sh{O}_z$ dominates $\sh{O}_y$, we necessarily have $\sh{O}_y=\sh{O}_z$ (\sref[II]{II.7.1.1}).
...
It follows from \sref{IV.1.10.4} that if $y'$ is the generic point of $Y$, each of the $X_i\cap f^{-1}(y')$ is reduced to a single point; the restriction $f_i:X_i\to Y$ of $f$ being a quasi-finite and dominant morphism and every point of $Y$ being geometrically unibranch, it follows from the criterion of Chevalley \sref{IV.14.4.4} that $f_i$ is \emph{universally open}.
So then for any point $y$ of $Y$, we have $n_i(y)\leq n_i(y')$; on the other hand, since the $X_i\cap f^{-1}(y)$ are pairwise disjoint, $\sum_i n_i(y')=n(y')$; we therefore still have the relations \sref{IV.15.5.2.4}.
...
Combining this proposition with the connectivity theorem of Zariski and its consequences (\sref[III]{III.4.3}), we obtain the following results that complement (\sref[III]{III.4.3.7} and \sref{IV.4.3.10}):
...
Suppose that the restriction of $f$ to every irreducible component of $X$ is a dominant morphism in $Y$.
Then, if $y_0$ is a geometrically unibranch point of $Y$, the function $y\mapsto n(y)$ is lower semi-continuous at the point $y_0$.
\end{proposition}

\begin{proof}
Consider the Stein factorisation $f=g\circ f'$ of $f$ (\sref[III]{III.4.3.3}), where $g:Y'\to Y$ is a finite morphism and $f':X\to Y'$ is a surjective morphism whose fibres are geometrically connected (\sref[III]{III.4.3.4}).
Since $f'$ is surjective, the inverse image by $f'$ of every irreducible component of $Y'$ contains an irreducible component of $X$ at least; therefore each irreducible component of $Y'$ dominates $Y$.
...
We have an example by taking $Y=\Spec(k[\mathrm{T}])$ where $k$ is algebraically closed, and $X=\Spec(A)$, where $A=k[\mathrm{S},\mathrm{T}]/(\mathrm{S}^2+\mathrm{T}^3-1)$; at the points $y'$, $y''$ of $Y$ corresponding to the maximal ideals $(\mathrm{T}-1)$ and $(\mathrm{T}+1)$, we have $n(y')=n(y'')=1$, but $n(y)=2$ at all other points of $Y$.
...
(iv)
Finally, in \sref{IV.15.5.1},~(iv), one cannot dispense with the hypothesis that the morphism $f$ is \emph{separated}, as shown by the example where $X$ is the affine line one of whose points has been ``doubled'' at a point $x$ (\sref[I]{I.5.5.11}), and $Y$ the affine line: here $f$ is a local isomorphism (hence flat) and is quasi-finite, but $z\mapsto n(z)$ is not lower semi-continuous.
\end{remarks}

\begin{lemma}[15.5.6]
\label{IV.15.5.6}
Let $Y$ be the spectrum of a discrete valuation ring, $a$ its closed point, $b$ its generic point.
Let $f:X\to Y$ be a locally of finite type and open morphism.
...
Let $Z$ be a closed rare subset of $X$ having the following property: for every $x\in Z$, if one denotes by $\mathrm{T}_x$ the set of generizations of $x$ in $X$, then $\mathrm{T}_x-\{x\}$ is connected.
Under these conditions, if $X$ is connected, $X-Z$ is connected.
\end{lemma}

\begin{proof}
Let us give an independent proof.
Reasoning by the absurd, suppose that the open set $U=X-Z$, which is dense in $X$ since $Z$ is everywhere dense, is the union of two nonempty open sets $U'$, $U''$ without common points, and denote by $X'$ and $X''$ the closures in $X$ of $U'$ and $U''$.
...
\oldpage[IV-3]{235}
\begin{proof}[Proof of {\rm (15.5.6)}]
To prove \sref{IV.15.5.6}, we apply the lemma \sref{IV.15.5.6.1} to $X$ and to its closed subset $Z=f^{-1}(a)$ which is rare since $f$ is open.
Note that by hypothesis, taking into account \sref{IV.15.2.2.1}, this implies that $f$ is \emph{flat} at the points of $f^{-1}(a)$.
For any such point $x$, $\sh{O}_x$ is therefore an $\sh{O}_a$-module without torsion ($\mathbf{0}_{\mathrm{I}},~6.3.4$), and in particular, if $t$ is a uniformiser of $\sh{O}_a$, $t$ is an $\sh{O}_x$-regular element; moreover $\sh{O}_x/t\sh{O}_x$ is reduced by hypothesis; if it is of depth $0$, it is therefore a field ($\mathbf{0},~16.4.7$), and as $t\sh{O}_x$ is then the maximal ideal of $\sh{O}_x$, $\Spec(\sh{O}_x)-\{x\}$ is reduced to a point, and \emph{a fortiori} is connected.
If on the contrary $\operatorname{prof}(\sh{O}_x/t\sh{O}_x)\geq 1$, we have $\operatorname{prof}(\sh{O}_x)\geq 2$ since $t$ is $\sh{O}_x$-regular ($\mathbf{0},~16.4.6$); it then follows from the theorem of Hartshorne \sref{IV.5.10.7} that $\Spec(\sh{O}_x)-\{x\}$ is connected, which completes the proof.
\end{proof}
...
\begin{proof}
We already know \sref{IV.15.5.4} that under the conditions of the statement, $z\mapsto n(z)$ is lower semi-continuous at the point $y$; it remains to see that it is also upper semi-continuous, and by the same reasoning as at the beginning of the proof of \sref{IV.15.5.1},~(i), it suffices to show that if $y'$ is a generization of $y$, we have
\begin{equation}
\tag{15.5.7.1}
\label{IV.15.5.7.1}
  n(y') \leq n(y).
...
Using the beginning of the proof of \sref{IV.15.5.2} and the lemma \sref{IV.15.5.2.2} applied to the finite morphism $g$ of the Stein factorisation $f=g\circ f'$ of $f$ (recalling that the geometric number of points of connected components of $f^{-1}(y)$ is equal to that of $g^{-1}(y)$ (\sref[III]{III.4.3.4})), we reduce to the case where $n(y)$ and $n(y')$ are respectively the number of connected components of $f^{-1}(y)$ and $f^{-1}(y')$, and where $Y$ is the spectrum of a discrete valuation ring, $y$ the closed point and $y'$ the generic point of $Y$.
We can moreover replace $X$ by any one of its connected components, these being open and closed in $X$, and proper over $Y$.
Suppose therefore that $X$ is connected; the hypothesis that $f$ is open at the points of $f^{-1}(y)$ implies that $f^{-1}(y)\neq\varnothing$, and we also have $f^{-1}(y')\neq\varnothing$ ($\mathbf{0}_{\mathrm{I}},~1.10.3$); the hypothesis that $f$ is closed implies that if $f^{-1}(y)\neq\varnothing$, we also have $f^{-1}(y')\neq 0$ (\sref[II]{II.7.2.1}).
...
\begin{enumerate}[label=(\roman*)]
  \item If $f$ is separated and quasi-finite, the function $y\mapsto n(y)$ (equal then to the number of geometric points of $f^{-1}(y)$) is lower semi-continuous in $Y$; if it is constant in a neighbourhood of $y_0$, $f$ is proper at the point $y_0$.
  \item If $f$ is proper, the function $y\mapsto n(y)$ is lower semi-continuous in $Y$; if in addition $f^{-1}(y_0)$ is geometrically reduced over $\kappa(y_0)$, $n(y)$ is constant in a neighbourhood of $y_0$.
\end{enumerate}
\end{proposition}
...
Moreover $f$ is then universally open \sref{IV.2.4.6}, it suffices to apply \sref{IV.15.5.1}, \sref{IV.15.5.4} and \sref{IV.15.5.7} to conclude.
\end{proof}

\begin{remarks}[15.5.10]
\label{IV.15.5.10}
We note that we have thus obtained another proof of \sref{IV.12.2.4},~(vi).
Conversely, one can deduce \sref{IV.15.5.7} from \sref{IV.12.2.4},~(vi): indeed, on can restrict to the case where $Y$ is reduced (replacing $f$ by $f_\mathrm{red}$), and it then follows from the hypotheses of \sref{IV.15.5.7} and from \sref{IV.15.2.3} that $f$ is flat in an open neighbourhood of $f^{-1}(y)$; since moreover $f$ is proper, we can suppose that this neighbourhood is of the form $f^{-1}(U)$, where $U$ is an open neighbourhood of $y$ in $Y$.
...
Let $y$ be a point of $Y$, $v'$ a generization of $y$.
Then:
\begin{enumerate}[label=(\roman*)]
  \item There exists an irreducible component $Z$ of $X_y^0$, of dimension equal to $\dim(X_y^0)$ and containing $g(y')$ (which will be the case if $X_y^0$ is irreducible); we have
\begin{equation}
\tag{15.6.1.1}
\label{IV.15.6.1.1}
  \dim(X_{y'}^0) \geq \dim(X_y^0).
\end{equation}
...
\begin{proof}
(i)
Let $\overline{Z}$ be the closure of $Z$ in $X$; since $y$ is adherent to $y'$ and $g$ is continuous, $g(y)\in\overline{Z}$.
...
Then the function $z\mapsto\dim(X_y^0)$ is upper semi-continuous in $Y$.
...
\begin{proof}
Let $y$ be a point of $Y$, and let $U$ be a neighbourhood of $g(y)$ in $X$; then there is a neighbourhood $V$ of $y$ in $Y$ such that $g(V)\subset U$, and for every $z\in V$, $U\cap X_z^0$ is dense in $X_z^0$, hence $\dim(X_z^0)=\dim(U\cap X_z^0)$ (\sref{IV.4.1.1.3}).
Replacing $X$ by $U$, we can suppose that $f$ is a morphism of finite type.
We then know \sref{IV.9.7.10} that the function $z\mapsto\dim(X_z^0)$ is locally constructible, and the assertions of the corollary follow from \sref{IV.15.6.1} and from $(\mathbf{0}_{\mathrm{III}},~9.3.4)$.
\end{proof}
...
Then, for every $y\in Y$ such that the function $z\mapsto\dim(X_y^0)$ is constant in a neighbourhood of $y$, $f$ is universally open at the points of $X_y^0$.
\end{proposition}

\begin{proof}
Apply the criterion \sref{IV.14.3.7}.
Let therefore $h:Y'\to Y$ be a morphism, $Y'$ the spectrum of a discrete valuation ring, whose points we denote by $t$, $t'$ with $t$ the closed point and $t'$ the generic point, so that $y'=h(t')$, and $y'$ is a generization of $y$.
Set $X'=X\times_Y Y'$, $f'=f_{(Y')}:X'\to Y'$, $g'=g_{(Y')}:Y'\to X'$.
With the same notations as in \sref{IV.15.6.1}, the hypothesis on $X_y^0$ implies that $X_t'^0=X_y^0\otimes_{\kappa(y)}\kappa(t)$ and $X_{t'}'^0=X_{y'}^0\otimes_{\kappa(y')}\kappa(t')$, and that $X_t'^0$ and $X_{t'}'^0$ are irreducible \sref{IV.4.4.1}; since $\dim(X_y^0)=\dim(X_t'^0)$ and $\dim(X_{y'}^0)=\dim(X_{t'}'^0)$ \sref{IV.4.1.4}, we see that it suffices to prove the proposition for $f'$ and $X_t'^0$, in other words we can restrict to the case where $Y$ is the spectrum of a discrete valuation ring, $y$ its closed point and $y'$ its generic point; if $x'$ is the generic point of $X_y^0$, it follows from \sref{IV.15.6.2} that every point of $X_y^0$ is a specialisation of $x'$, whence the conclusion.
\end{proof}
...
Let $x$ be a point of $X_y^0$; it follows from \sref{IV.5.10.8.1} (with $\mathfrak{F}=\{\varnothing\}$) that there is a finite sequence $(Z_i)_{0\leq i\leq n}$ of irreducible components of $f^{-1}(y)$ such that $g(y)\in Z_0$, $x\in Z_n$ and $Z_i\cap Z_{i+1}\neq\varnothing$ for $0\leq i\leq n-1$; the $Z_i$ being irreducible, there is a finite sequence $(U_i)$ ($1\leq i\leq n$) of open affine subsets of $f^{-1}(y)$, $x\in U_n$, $U_i$ being an open affine neighbourhood of a point $x_i$ of $Z_i\cap Z_{i-1}$ for $1\leq i\leq n$.
...
Since $g$ is continuous, there is an open affine neighbourhood $V$ of $y$ in $Y$ such that $g(V)\subset W$; if $X'=W\cap g^{-1}(V)$, the restriction of $f$ to $X'$ is of finite type; moreover, for every $z\in V$, $X_z'^0$ is the connected component of $X'\cap f^{-1}(z)$ containing $g(z)$, and we have $X_z'^0\subset X_y^0$, and $x$ belongs to $X_y^0$.
Indeed, each of the $U_i$ contains the two irreducible sets $U_i\cap Z_{i-1}$
\oldpage[IV-3]{238}
and $U_i\cap Z_i$ which intersect, and the $U_i\cap Z_i$ for $1\leq i\leq n$ is connected and contains $x$ and $g(y)$.
Since $f|X'$ is universally open at the points of $X_y'^0$, this proves our assertion, for if the proposition is proved when $f$ is of type fini, the union $X'^0$ of the $X_z'^0$ for $z\in V$ will be a neighbourhood of $x$, and we are done since $X'^0\subset X^0$.
...
Set $X'=X\times_Y Y'$, $f'=f_{(Y')}:X'\to Y'$, and $g'=g_{(Y')}:Y'\to X'$.
If $p_y:f'^{-1}(z)\to f^{-1}(y)$ (resp.\ $p_y:f'^{-1}(z')\to f^{-1}(y')$) is the canonical projection, $p_y^{-1}(X_y^0)$ (resp.\ $p_{y'}^{-1}(X_{y'}^0)$) is the connected component of $f'^{-1}(z)$ (resp.\ $f'^{-1}(z')$) containing $g'(z)$ (resp.\ $g'(z')$), since the $X_y^0$ are geometrically connected \sref{IV.4.5.13} and it suffices to apply \sref{IV.4.4.1}.
Moreover, the morphism $u$ corresponds to a $Y'$-section $v$ of $X'$ such that $p_Y(v(y))=x$, $p_Y(v(y'))=x'$, and $v(y')$ is a generization of $v(y)$, and it will suffice to prove that $v(y')$ belongs to $p_{y'}^{-1}(X_y^0)$.
...
Since $f^{-1}(y)-X_y^0$ is then closed in $X$, replacing $X$ by the open set $X-(f^{-1}(y)-X_y^0)$, we can suppose that $X_y^0=f^{-1}(y)$; similarly, one can replace $X$ by the open set, connected component of $X$, which contains $g(Y)$; this connected component evidently contains $X^0$; in other words, we can suppose $X$ \emph{connected}.
The proposition will then be established if we show that $X^0=X$, or again that $X_y$ is connected; but $f^{-1}(y')$ is geometrically reduced over $\kappa(y')$, and \emph{a fortiori} \emph{reduced}; and moreover $f$ is open at the points of $f^{-1}(y)$, so we can apply the lemma \sref{IV.15.5.6}, whence the conclusion.
\end{proof}
...
\label{IV.15.6.6}
Let $f:X\to Y$ be a morphism such that $Y$ is locally Noetherian and $f$ locally of finite type, or that $f$ is of finite presentation.
...
If the function $y\mapsto\dim(X_y^0)$ is constant in a neighbourhood of $y_0$, there exists a neighbourhood $U$ of $y_0$ such that $f$ is universally open at all points of $X^0\cap f^{-1}(U)$.
If moreover $Y$ is reduced and $f^{-1}(y_0)$ geometrically reduced over $\kappa(y_0)$ at the point $x_0$, $f$ is flat at the point $x_0$.
  ...
  \begin{enumerate}[label=$\alpha$)]
    \item The fibre $f^{-1}(y_0)$ is geometrically reduced over $\kappa(y_0)$ at the point $g(y_0)$ and the ring $\sh{O}_{Y,y_0}$ is Noetherian.
    \item For every generic point $y'$ of an irreducible component of $Y$ containing $y_0$, we have $\dim(f^{-1}(y'))=\dim(X_{y'}^0)$.
  \end{enumerate}
...
We have $\dim(X_y^0)=\dim((X_1)_{y_1}^0)$ \sref{IV.4.1.4}, and applying once more \sref{IV.9.5.5} and \sref{IV.9.7.12}, we can suppose (restricting as needed $Y$ to a neighbourhood of $y_0$) that if $\dim(X_y^0)$ is constant in $Y$, then $\dim((X_1)_{y_1}^0)$ is constant in $Y_1$.
If $Y$ is reduced, so is $Y_1$ since $A_1\subset A$; moreover, if $f^{-1}(y_0)$ is geometrically reduced over $\kappa(y_0)$ at the point $x_0$, $f_1^{-1}(y_{01})$ at the point $x_{01}$ ($x_{01}$ and $y_{01}$ being the respective projections of $x_0$ and $y_0$) in $X_1$ and $Y_1$ \sref{IV.4.6.10}.

These remarks show that to prove (i) and (ii), we can restrict to the case where $Y$ is Noetherian and $f$ locally of finite type.
But in this case, the assertion of (i) follows from \sref{IV.15.6.2} and the first assertion of \sref{IV.15.6.3}.
Moreover, if $Y$ is reduced
\oldpage[IV-3]{240}
and $f^{-1}(y_0)$ geometrically reduced over $\kappa(y_0)$ at the point $x_0$, we deduce from \sref{IV.15.6.3} and \sref{IV.15.2.3} that if $\dim(X_y^0)$ is constant in a neighbourhood of $y_0$, $f$ is flat at the point $x_0$.
...
Apply then \sref{IV.1.10.1} to $E$: it suffices to prove that $\dim(X_{y'}^0)=\dim(X_{y_0}^0)$ for the generic point $y'$ of an irreducible component of $Y$ containing $y_0$; this already allows us to suppose that $Y=\Spec(\sh{O}_{X,y_0})$.

If we are in the case $\alpha$), we reduce immediately, by virtue of (\sref[II]{II.7.1.9}), and using \sref{IV.15.6.4}, to the case where $Y$ is the spectrum of a discrete valuation ring, $y_0$ its closed point and $y'$ its generic point.
...
To prove our assertion, by $V$, for $V\cap X_y^0$ is nonempty by virtue of \sref{IV.2.3.4}, hence is an everywhere dense open set in $X_y^0$ and has therefore the same dimension \sref{IV.4.1.1.3}; moreover, since $X_y^0$ is also the connected component of $f^{-1}(y')$ containing $g(y')$, we can suppose $V$ chosen such that $V$ meets no other irreducible component of $f^{-1}(y_0)$ nor of $f^{-1}(y')$, in other words we can suppose that $X^0=X$; but then the conclusion follows from \sref{IV.12.1.1},~(i), since by hypothesis $X_{y_0}^0$ is integral.
...
\label{IV.15.6.7}
Suppose the preliminary conditions of \sref{IV.15.6.6} satisfied on $f$, and suppose moreover that for every $y\in Y$, $X_y^0$ is geometrically integral over $\kappa(y)$.
...
Then, if every $y\in Y$ is such that $X_y^0$ is proper over $\kappa(y)$, $X^0$ is proper over $Y$ at the point $y$ (i.e.\ \sref{IV.15.7.1}, there exists an open neighbourhood $U$ of $y$ in $Y$ such that $X^0\cap f^{-1}(U)$ is closed in $f^{-1}(U)$ and proper over $U$).
\end{proposition}

\begin{proof}
Proceeding as at the beginning of \sref{IV.15.6.6} (where one uses \sref{IV.8.10.5},~(v), and where one replaces \sref{IV.9.6.7} by \sref{IV.9.6.7}), we reduce to the case where $Y$ is Noetherian and $f$ of finite type; moreover, the $X_y^0$ are geometrically connected (for every $z\in Y$) by virtue of \sref{IV.4.5.13}; finally, as $g(Y)\subset X^0$, $X^0$ is universally submersive over $Y$ \sref{IV.15.7.8}.
...
\begin{proof}
Suppose first that $f$ is of finite presentation; then replacing as needed $Y$ by a neighbourhood of $y$, we can suppose that $f|X^0$ is \emph{proper} \sref{IV.15.6.8}, and it suffices to apply \sref{IV.13.1.5} to this restriction.
If $Y$ is locally Noetherian and $f$ locally of finite type, $X$ is locally Noetherian; moreover the hypothesis that $X_y^0$ is proper over $\kappa(y)$ implies that $X_y^0$ is Noetherian; therefore, by $V$ an open Noetherian set containing $X_y^0$; it exists then a neighbourhood open $U$ of $y$ in $Y$ such that $g(W)\subset V$.
Moreover, if $z$ is a maximal point of $X_y^0$, the restriction of $f$ to $V\cap f^{-1}(W)$ is a morphism of finite type, and applying to this morphism the result already obtained, we see that if $Z$ is the irreducible component of $X_y^0$ for which $z$ is a maximal point whatever, we have $\dim(X_{y'}^0)\leq\dim(X_y^0)$.
\end{proof}

\begin{remarks}[15.6.10]
\label{IV.15.6.10}
(i)
The supplementary hypothesis on the existence of an irreducible component of $X$ containing $g(y')$ and of dimension equal to $\dim(X_y^0)$, made in \sref{IV.15.6.1}, is not superfluous, as shown by the following example.
Let $A$ be a discrete valuation ring, $\pi$ a uniformiser of $A$, $K$ the field of fractions of $A$, $k$ its residue field, and designate by $y$ and $y'$ the closed point and the generic point of $Y$.
...
In the ring $A[t]$, the principal ideal $(\pi t-1)$ is \emph{maximal}, and projects to the \emph{generic point} $y'$ of $Y$ and is such that $\kappa(x_1)=K$.
\oldpage[IV-3]{242}
...
As one will see in Ch.\ V, one can therefore \emph{reglue} $X_1$ and $X_2$ along the two sub-preschemes $\{x_1\}$ and $\{x_2\}$ having respectively $\{x_1\}$ and $\{x_2\}$ as underlying spaces, following the unique $Y$-isomorphism of $Z_1$ on $Z_2$.
Designate by $X$ the $Y$-prescheme thus obtained, by $x$ the point of $X$ which is the image of $x_1$ and $x_2$.
The ``null section'' $g$ of $X_1$ (\sref[II]{II.1.7.9}) is then also a $Y$-section of $X$; $f^{-1}(y)$ is equal to $(X_1)_y$, so $f^{-1}(y)$ has two irreducible components, respectively isomorphic to $(X_1)_y^0$ and $(X_2)_y^0$, having a unique point in common, see however the prop.\ \sref{IV.15.6.9}.

(ii)
Consider the morphism $f$ defined in \sref{IV.12.2.3},~(ii), which is proper and flat; moreover, the reciprocal morphism $g:Y\to X$ is the $Y$-section of $X$.
We have then $\dim(X_{y_0}^0)=1$ whereas $\dim(X_y^0)=0$ for $y\neq y_0$, although $X_y^0$ is geometrically irreducible for every $y\in Y$ (since $X_y^0$ is not reduced); one sees therefore that as in \sref{IV.15.6.6},~(iii), one cannot suppress the hypotheses $\alpha$) and $\beta$).
Moreover, $X^0$ is not a neighbourhood of $X_{y_0}^0$ in $X$, so we see that as in \sref{IV.15.6.4}, one cannot dispense with the hypothesis that $X'$ is reduced.
...
If $G$ is such a prescheme, there exists the ``unit section'' $g$, and one shows that $G_y$ is always geometrically irreducible and $\dim(G_y^0)=\dim(G_y)$, setting $G_y=f^{-1}(y)$.
...
This section gives complements to the valuative criterion of properness proved in (\sref[II]{II.7.3.10}); it is independent from the rest of \S~15.
...
\label{IV.15.7.2}
Let $Y$ be an integral locally Noetherian prescheme, $X$ an integral prescheme, $f:X\to Y$ a separated, dominant, and of finite type morphism, $y$ a point of $Y$.
...
\begin{enumerate}[label=\emph{\alph*)}]
  ...
  \item Every discrete valuation ring having $R(X)$ as field of fractions and dominating a local ring $\sh{O}$ dominates an $\sh{O}_x$ where necessarily $f(x)=y$.
\end{enumerate}
\end{proposition}
...
The question being local on $Y$, hence on the Noetherian $Y$, by virtue of the lemma of Chow (\sref[II]{II.5.6.1}), there exists an integral prescheme $X'$, a projective morphism $p:P\to Y$, an open dominant immersion $j:X'\to P$, and a projective and birational (hence surjective) morphism $g:X'\to X$ such that the diagram
\[
  \xymatrix{
    P & X' \ar[l]_-{j} \\
    Y \ar[u]^-{p} & X \ar[l]^-{f} \ar[u]_-{g}
  }
\]
is commutative.
Since $X'$ is integral and $j$ dominant, $P$ is irreducible, and one can, replacing $p$ by $p_\mathrm{red}$, suppose $P$ integral (\sref[I]{I.5.2.2} and \sref[II]{II.5.5.5},~(vi)).
It amounts to proving that there exists an open neighbourhood $U$ of $y$ such that $p^{-1}(U)=g^{-1}(f^{-1}(U))=V$ will be an isomorphism on $p^{-1}(U)$, hence the restriction $V\to U$ of $f\circ g$ will be proper, and as the restriction $V\to f^{-1}(U)$ of $g$ is surjective, the restriction $f^{-1}(U)\to U$ of $f$ will be proper (\sref[II]{II.5.4.3}).
...
Since $g$ is proper, there exists $x'\in X'$ such that $g(x')=x$ and $A$ dominates $\sh{O}_{x'}$ (\sref[II]{II.7.3.10}); therefore $z'$ and $j(x')$ are two points of $P$ whose local rings $\sh{O}_z$ and $\sh{O}_{j(x')}=\sh{O}_{z'}$ are related by (\sref[I]{I.8.1.4}); since $P$ is a scheme, this implies $z'=j(x')$ (\sref[I]{I.8.2.2}), which completes the proof.
\end{proof}

\begin{remarks}[15.7.3]
\label{IV.15.7.3}
One can in \sref{IV.15.7.2} suppress the hypothesis that $Y$ is locally Noetherian, replacing in condition $c)$ the discrete valuation rings by valuation rings of any kind; the proof is then unchanged, taking into account (\sref[II]{II.7.3.10}).
\end{remarks}
...
We can evidently also, taking into account (\sref[I]{I.5.2.2}) and (\sref[II]{II.5.4.6}), suppose that $X$ and $Y$ are reduced; then (by definition of proper part (\sref[II]{II.5.4.10})) suppose that $\overline{Z}=X=\{z\}$ and (applying again (\sref[I]{I.5.2.2})) that $f$ is dominant, hence $X$ and $Y$ integral and Noetherian; it therefore suffices to prove that $f$ is proper at the point $y$ \sref{IV.15.7.2}.
Let $A'$ be a discrete valuation ring having $R(X)=\kappa(z)$ as field of fractions and dominating $\sh{O}_y$; with the notations of $c)$, we have therefore $g(y')=y$, and $g(\eta')$ is the generic point of $Y$.
...
We conclude therefore from hypothesis $c)$ that there exists a $Y'$-section $h'$ of $X'$ such that $h'(\eta')=z'$; hence $h=g'\circ h':Y'\to X$ is the $Y$-morphism corresponding, and since $x=h(y')$; it is clear that $f(x)=y$ and that $A'=\sh{O}_{y'}$ dominates $\sh{O}_x$, which completes the proof.
\end{proof}
...
\begin{enumerate}[label=\emph{\alph*)}]
  \item $f$ is proper at the point $y$.
  \item If $Y_1=\Spec(\sh{O}_y)$, $X_1=X\times_Y Y_1$, the morphism $f_1=f_{(Y_1)}:X_1\to Y_1$ is proper.
  \item For every scheme $Y'=\Spec(A')$, where $A'$ is a discrete valuation ring, and every morphism $g:Y'\to Y$, such that the image $g(y')$ of the closed point $y'$ of $Y'$ is $y$, and denoting by $\eta'$ the generic point of $Y'$, there exists a $Y'$-section of $X'$ containing $x'$, for every $x'\in f'^{-1}(\eta')$, rational over $\kappa(\eta')$.
\end{enumerate}
\end{corollary}

\begin{corollary}[15.7.6]
\label{IV.15.7.6}
Let $Y$ be a locally Noetherian prescheme, $f:X\to Y$ a locally of finite type morphism.
The following conditions are equivalent:
\begin{enumerate}[label=\emph{\alph*)}]
  \item There exists a proper morphism $g:X\to Z$ and an immersion $h:Z\to Y$ such that $f=h\circ g$.
  \item The sub-space $f(X)$ is locally closed in $Y$, and if $Z'$ is the sub-prescheme of $Y$ induced by the image $f(X)$ of $Z'$, so that $f$ factorises uniquely as $f=j\circ g$ (where $j:Z''\to Y$ is the canonical injection), then $g:X\to Z''$ is proper.
  \item For every $y\in f(X)$, $f$ is proper at the point $y$.
  \item For every scheme $Y'=\Spec(A')$, where $A'$ is a discrete valuation ring, and every morphism $g:Y'\to Y$ such that $g(Y')\subset f(X)$, every $Y'$-section rationnelle (\sref[I]{I.7.1.2}) of $X'=X\times_Y Y'$ extends uniquely to a $Y'$-section of $X'$.
\end{enumerate}
\end{corollary}

\begin{proof}
It is clear that $a)$ implies $c)$, let us first remark that $a)$ implies that $h(Z)$ is locally closed in $Y$ and $g(X)$ is closed in $Z$, hence $f(X)$ is locally closed.
Moreover, since $f$ is locally closed in $Y$; for every $y\in f(X)$, there is therefore a neighbourhood $U$ of $y$ in $Y$ such that $f(X)\cap U$ is closed in $U$, hence $f(X)$ is locally closed.
Since this last is the underlying space of $Z'$, and $f$ factorises as in the statement of $b)$; the fact that $g$ is proper results then from the local character of \sref{IV.15.7.5},~(a).
It is clear that $c)$ implies $d)$, by the uniqueness of the extension of a $Y'$-section rationnelle resulting from the hypothesis that $f$ is separated (\sref[I]{I.7.2.2}).
Let us finally prove that $d)$ implies $c)$; in the first place, from $d)$, it follows that $f$ is separated; on the other hand, we can then apply the criterion \sref{IV.15.7.5},~$c$), which shows that $f$ is proper at every point of $f(X)$.
\end{proof}
...
\label{IV.15.7.7}
(i)
In \sref{IV.15.7.4},~$c$), \sref{IV.15.7.5},~$c$), and \sref{IV.15.7.6},~$d$), one can restrict to the case where the discrete valuation ring $A'$ is \emph{complete} and admits an algebraically closed residue field.
Indeed, in the proof of \sref{IV.15.7.4}, if one considers a complete discrete valuation ring $A''$ dominating $A'$ and having an algebraically closed residue field, $X''=X'\times_{Y'}Y''$ and $f''=f_{(Y'')}$; but $X''=X'\times_{Y'}\times_{Y'}Y''$, and it follows then from the remark (\sref[II]{II.7.3.9},~(i)) that $f'$ is proper; by suite $Y'$, $X'$ and $f'$ also verify the hypothesis $c)$ (\sref[II]{II.7.3.8}).
...
We say that $Z$ is \emph{universally submersive over $f(Z)$} if, for every base change $g:Y'\to Y$, the inverse image $Z'=p^{-1}(Z)$ of $Z$ by the projection $p:X\times_Y Y'\to Y'$ is submersive over $f'(Z')=g^{-1}(f(Z))$.
...
Let us first note that if $A'$ is a local Noetherian ring, $\varphi:A\to A'$ a local homomorphism and $g:Y'\to Y$ the corresponding morphism, the inverse image $Z'$ of $Z$ by the canonical projection $p:X\times_Y Y'\to X$ has the properties (i), (ii), (iii) of the statement vis-\`a-vis $Y'$; using \sref{IV.1.8.2}, taking into account \sref{IV.2.7.1},~(vii), it therefore amounts to the same to prove that $Z'$ is proper over $Y'$, provided that $A'$ is a flat $A$-module.
...
Since $Z_y$ is a connected component of $f^{-1}(y)$, proper over $\kappa(y)$, it follows from (\sref[III]{III.5.5.2}) that $X$ is the \emph{sum} of two sub-preschemes $X_0$, $X_1$ induced on open and closed subsets of $X$, such that $X_0\cap f^{-1}(y)=Z_y$.
Set $Z_0=X_0\cap Z$, $Z_1=X_1\cap Z$, so that these sets form a partition of $Z$ into two open and closed subsets.
Since the $Z_y$ are connected, we necessarily have $Z_y\subset Z_0$ or $Z_y\subset Z_1$, hence $f(Z_0)\cap f(Z_1)=\varnothing$.
...
In other words, it will suffice to prove that a \emph{constructible} subset $Z$ of $X$ \emph{proper} over $Y$ that satisfies only the two \emph{sole} conditions (i) and (iii), is \emph{closed} in $X$.
...
Let $A_1$ be a complete discrete valuation ring, $u$ a morphism of $Y_1=\Spec(A_1)$ into $X$ such that, if $y_1$ and $y_1'$ are the closed point and the generic point of $Y_1$, $u(y_1)=x$, $u(y_1')=x'$ (\sref[II]{II.7.1.7}); set $g=f\circ u:Y_1\to Y$ and $X_1=X\times_Y Y_1$ with $f_1=f_{(Y_1)}$, so that $u=p\circ u_1$, where $p:X_1\to X$ is the canonical projection.
Moreover, $x_1=u_1(y_1)$, $x_1'=u_1(y_1')$, and then $p(x_1)=x$ and $p(x_1')=x'$, and $x_1$ is a specialisation of $x_1'$.
Moreover, $x_1'\in Z_1=p^{-1}(Z)$; since we have remarked that the conditions (i) and (iii) are stable under \emph{every} base change, it is seen that we are reduced to the following situation: $Y$ is the spectrum of a complete discrete valuation ring, $f(x)=y$, $y'=f(x')$ its generic point, there exists a $Y$-section $h$ of $X$ such that $h(y)=x$, $h(y')=x'$, and finally $x'\in Z$; we must prove that $x\in Z$.
...
\oldpage[IV-3]{247}
we can suppose $X'$ connected (replacing as needed $X'$ by that of its connected components containing $Z_y$).
...
Then, if every $y\in Y$ is such that $X_y$ is proper over $\kappa(y)$, $f$ is proper at the point $y$.
\end{corollary}
...
\label{IV.15.7.11}
Let $f:X\to Y$ be a separated morphism of finite presentation; suppose that there exists a surjective morphism of finite presentation $g:Y'\to Y$, which is either proper or flat and quasi-compact, and such that if one sets $X'=X\times_Y Y'$, there exists a $Y'$-section of $X'$ (or even a $Y$-morphism $Y'\to X$ (\sref[I]{I.3.3.14})).
...
\begin{proof}
The question being local on $Y$, we can suppose $Y=\Spec(A)$ affine.
Applying \sref{IV.8.9.1} to $f$ and to $g$, one sees that there exists a Noetherian sub-ring $A_0$ of $A$ and two morphisms of finite type $f_0:X_0\to Y_0=\Spec(A_0)$, $g_0:Y_0'\to Y_0$ such that $X=X_0\times_{Y_0}Y$, $Y'=Y_0'\times_{Y_0}Y$, $f=(f_0)_{(Y)}$ and $g=(g_0)_{(Y)}$; moreover, using \sref{IV.8.10.5},~(v),~(vi) and (xii) and \sref{IV.11.2.7}, we can suppose $f_0$ separated and proper (resp.\ flat) if $g$ is proper (resp.\ flat).
Furthermore, we can suppose that there exists a $Y_0'$-section $g_0$ of $X_0$ such that $g=(g_0)_{(Y)}$ \sref{IV.8.8.2}.
For every $y\in Y$, let $y_0$ be its projection in $Y_0$; it follows from \sref{IV.4.5.13} that the $f_0^{-1}(y_0)$ are geometrically connected for every $y_0\in Y_0$.
...
\label{IV.15.7.11.1}
Let $f:X\to Y$ be a morphism, $g:Y'\to Y$ a morphism surjective, which is either proper or flat and quasi-compact.
...
Indeed, suppose prouved in this case, and let us show the lemma also holds when $f'$ is universally submersive.
Let $Y_1\to Y$ be any morphism, and set $Y_1'=Y'\times_Y Y_1$; if $X_1'=X'\times_{Y'} Y_1'$, we have $f_1'=(f')_{(Y_1)}:X_1'\to Y_1'$; by hypothesis $g_1:Y_1'\to Y_1$ is surjective, and proper (resp.\ flat and quasi-compact) if $g$ is.
