...
If $\mathrm{N}$ is a flat $\mathrm{A}$-module \textnormal{(}resp.\ if $\mathrm{B}$ is an $\mathrm{A}$-module plat\textnormal{)}, then we have
\begin{equation}
...
If in addition there exists a $\mathrm{B}$-module of finite type $\mathrm{N}$ that is a flat $\mathrm{A}$-module and has support equal to $\Spec(\mathrm{B})$ (which holds in particular if $\mathrm{B}$ is an $\mathrm{A}$-module \emph{plat}), then $\dim(\mathrm{B})=\dim(\mathrm{A})$.
\end{corollary}
...
\begin{proof}
In addition, if $f$ is quasi-flat \sref{IV.2.3.3} the two sides of \sref{IV.6.1.4} are equal.
...
\]
and by virtue of \sref{IV.5.5.1.2} we have
\[
...
Consider an element $y\in\mathfrak{m}$ which is ($\mathrm{N}\otimes_\mathrm{A} k$)-regular; it follows from \sref[0]{0.10.2.4} that $y$ is N-regular and that we then have
\[
  \mathrm{N}'=\mathrm{N}/y\mathrm{N}
\]
is a flat $\mathrm{A}$-module \emph{plat}, since $\mathrm{N}$ is supposed to be a flat $\mathrm{A}$-module.
...
Taking into account \sref[I]{I.3.6.5}, on sees that if we set $\mathrm{A}''=\mathrm{A}'\otimes_\mathrm{A}\mathrm{A}_\mathfrak{p}$ and $\mathrm{M}''=\mathrm{M}'\otimes_\mathrm{A}\mathrm{A}_\mathfrak{p}$, $\mathrm{M}''$ is a Cohen-Macaulay $\mathrm{A}''$-module.
For every prime ideal $\mathfrak{p}''$ of $\mathrm{A}''$ above $\mathfrak{p}$, $\mathrm{M}''_{\mathfrak{p}''}=\mathrm{M}_\mathfrak{p}\otimes_{\mathrm{A}_\mathfrak{p}}\mathrm{A}''_{\mathfrak{p}''}$ is a Cohen-Macaulay $\mathrm{A}''_{\mathfrak{p}''}$-module \sref[0]{0.16.5.10}; on the other hand, $\mathrm{M}_\mathfrak{p}$ is by hypothesis a Cohen-Macaulay $\mathrm{A}_\mathfrak{p}$-module and $\mathrm{A}''_{\mathfrak{p}''}$ is a flat $\mathrm{A}_\mathfrak{p}$-module plat since $\mathrm{A}'$ is a flat $\mathrm{A}$-module \sref[0]{0.7.3.3} and \sref{IV.6.3.2}.
...
Moreover, as the ``gluing'' process commutes with localisation and completion, one easily verifies that the canonical homomorphisms $\hat{\mathrm{A}}_\mathfrak{m}\to\hat{\mathrm{B}}_{\mathfrak{r}_1}$ and $\hat{\mathrm{A}}_\mathfrak{m}\to\hat{\mathrm{B}}_{\mathfrak{r}_2}$ are bijective, and therefore $\mathrm{B}_{\mathfrak{r}_1}$ and $\mathrm{B}_{\mathfrak{r}_2}$ are flat $\mathrm{A}_\mathfrak{m}$-modules (Bourbaki, \emph{Alg.\ comm.}, chap.~III, \textsection\,3, n\textsuperscript{o}\,5, prop.~10) $\mathrm{B}_{\mathfrak{r}_1}$ and $\mathrm{B}_{\mathfrak{r}_2}$ are flat $\mathrm{A}_\mathfrak{m}$-modules.
...
B) Let $(\mathrm{L}_\lambda)$ be the filtering family of sub-extensions of $\mathrm{L}$ which are of finite type over $k$; by virtue of A), each of the rings $\mathrm{C}_\lambda=\mathrm{K}\otimes_k\mathrm{L}_\lambda$ is regular; furthermore, for $\mathrm{L}_\lambda\subset\mathrm{L}_\mu$, $\mathrm{L}_\mu$ is an $\mathrm{L}_\lambda$-module flat, so $\mathrm{C}_\mu$ is an $\mathrm{C}_\lambda$-module flat \sref[0]{0.6.2.1}. Since $\mathrm{C}=\mathrm{K}\otimes_k\mathrm{L}=\varinjlim(\mathrm{K}\otimes_k\mathrm{L}_\lambda)$ is Noetherian, we can apply \sref{IV.5.13.7}, and $\mathrm{C}$ is therefore regular.
\end{proof}
...
\oldpage[IV-2]{153}
  \item $\sh{O}_x$ is a formally smooth $\sh{O}_y$-algebra \sref[0]{0.19.3.1} for the preadic topologies on $\sh{O}_x$ and $\sh{O}_y$.
...
In the second place, for $\sh{O}_x$ to be a formally smooth $\sh{O}_y$-algebra for the preadic topologies, it is necessary and sufficient that $\sh{O}_x$ be a flat $\sh{O}_y$-module and that $\sh{O}_x\otimes_{\sh{O}_y}\boldsymbol{k}(y)$ be a $\boldsymbol{k}(y)$-algebra formally smooth for its preadic topology \sref[0]{0.19.7.1}; but for $\sh{O}_x\otimes_{\sh{O}_y}\boldsymbol{k}(y)$ to be a $\boldsymbol{k}(y)$-algebra formally smooth for its preadic topology, it is necessary and sufficient that it be a $\boldsymbol{k}(y)$-algebra geometrically regular \sref[0]{0.19.6.6}; this proves the equivalence of $b)$ and $c)$. Finally, to prove the equivalence of $c)$ and $c')$, one can limit oneself to the case where $\mathrm{Y}=\operatorname{Spec}(\mathrm{A})$ and $\mathrm{X}=\operatorname{Spec}(\mathrm{C})$ are affine, $\mathrm{A}$ being Noetherian and $\mathrm{C}$ a Noetherian $\mathrm{A}$-algebra of finite type, which can therefore be written in the form $\mathrm{A}[\mathrm{T}_1,\ldots,\mathrm{T}_n]/\mathfrak{J}$. As here $\mathfrak{J}/\mathfrak{J}^2$ is a $\mathrm{C}$-module of finite presentation, the equivalence of $c)$ and $c')$ follows from \sref[0]{0.22.6.4} applied to $\mathrm{A}$, $\mathrm{B}=\mathrm{A}[\mathrm{T}_1,\ldots,\mathrm{T}_n]$ and $\mathrm{C}=\mathrm{B}/\mathfrak{J}$.
\end{proof}
...
\begin{lemma}[6.9.2]
\label{IV.6.9.2}
Let $\mathrm{A}$ be an integral Noetherian ring, $\mathrm{B}$ an $\mathrm{A}$-algebra of finite type, $\mathrm{M}$ a $\mathrm{B}$-module of finite type. Then there exists $f\neq 0$ in $\mathrm{A}$ such that $\mathrm{M}_f$ is an $\mathrm{A}_f$-module free.
\end{lemma}
...
\]
where $\mathrm{M}'$ is a $\mathrm{C}$-module \emph{of torsion} of finite type; the support of $\mathrm{M}'$ does not therefore contain the generic point of $\operatorname{Spec}(\mathrm{C})$ \sref[I]{I.7.4.6} and hence the support of $\mathrm{M}'\otimes_\mathrm{A}\mathrm{K}$ does not contain the generic point of $\operatorname{Spec}(\mathrm{C}\otimes_\mathrm{A}\mathrm{K})$ \sref[I]{I.9.1.13.1}; one concludes \sref{IV.4.1.2.1} that its dimension is $<n$. By the induction hypothesis, there exists $f\neq 0$ in $\mathrm{A}$ such that $\mathrm{M}'_f$ is an $\mathrm{A}_f$-module free; hence it is the same for $\mathrm{C}^h_f$, which is the same as $\mathrm{B}_f$, and who in turn of \sref[0]{0.1.3.2} is an extension of free modules.
\end{proof}
...
\begin{proof}
We proceed by Noetherian induction \sref[0]{0.2.2.2} on the set of closed subsets $\mathrm{T}$ of $\mathrm{S}$ such that the reduced sub-prescheme having $\mathrm{T}$ for underlying space satisfies the conclusion of \sref{IV.6.9.3}. One can therefore limit oneself to proving the corollary for $\mathrm{S}$ by supposing that it is true for every closed sub-prescheme reduced and having as underlying space a closed subset $\neq\mathrm{S}$. Since the morphism $\mathrm{S}_\mathrm{red}\to\mathrm{S}$ is of finite type and surjective, one can replace $\mathrm{S}$ by $\mathrm{S}_\mathrm{red}$, hence suppose $\mathrm{S}$ reduced and non-empty. Since $\mathrm{S}$ is Noetherian, the interior $\mathrm{T}$ of an irreducible component of $\mathrm{S}$ is non-empty, and the prescheme induced on $\mathrm{T}$ is integral; there therefore exists by virtue of \sref{IV.6.9.1} a non-empty open $\mathrm{U}\subset\mathrm{T}$

\oldpage[IV-2]{155}

such that $\sh{F}|u^{-1}(\mathrm{U})$ is flat over $\mathrm{U}$. If $\mathrm{Y}$ is then the sub-prescheme reduced of $\mathrm{S}$ having for underlying space $\mathrm{S}-\mathrm{U}$, there exists by hypothesis a partition $(\mathrm{Y}_i)$ of $\mathrm{Y}$ in locally closed subsets of $\mathrm{Y}$ (hence of $\mathrm{S}$) such that $\sh{F}_i=\sh{F}\otimes_{\sh{O}_\mathrm{S}}\sh{O}_{\mathrm{Y}_i}$ is flat over $\mathrm{Y}_i$ for every $i$; it is clear that the $\mathrm{Y}_i$ and $\mathrm{U}$ form a partition answering the question.
\end{proof}
...
\begin{proof}
Indeed, let $\sh{J}$ be the coherent Ideal of $\sh{O}_\mathrm{X}$ defining $\mathrm{Y}$; the $\sh{O}_\mathrm{Y}$-Algebra $\sh{B}=\operatorname{gr}^{\bullet}_\sh{J}(\sh{O}_\mathrm{X})$ is quasi-coherent and \emph{of finite type} car elle est engendr\'ee par $\operatorname{gr}^1_\sh{J}(\sh{O}_\mathrm{X})$, inverse image of $\sh{J}/\sh{J}^2$; as $\operatorname{gr}^{\bullet}_\sh{J}(\sh{F})$ is a $\sh{B}$-module quasi-coherent generated by $\operatorname{gr}^{\bullet}_\sh{J}(\sh{F})$, it is a $\sh{B}$-Module \emph{of finite type}. If one sets $\mathrm{Z}=\operatorname{Spec}(\sh{B})$, the structural morphism $u:\mathrm{Z}\to\mathrm{Y}$ is of finite type, and if $\sh{H}$ is the coherent $\sh{O}_\mathrm{Z}$-Module such that $u_*(\sh{H})=\operatorname{gr}^{\bullet}_\sh{J}(\sh{F})$, it suffices to apply to $u$ and to $\sh{H}$ the theorem of generic flatness \sref{IV.6.9.1} to prove the proposition.
\end{proof}
...
\begin{proof}
\oldpage[IV-2]{157}
As $\mathrm{Y}$ is assumed connected, the first assertion of (ii) follows from \sref{IV.6.10.3},~(i). If $\operatorname{Supp}(\sh{F})\supset\mathrm{Y}$, and if $y$ is a maximal point of $\mathrm{Y}$; as on a module $\sh{F}_y$ of finite length \sref{IV.3.1.4}; setting $s(n)=\operatorname{long}(\operatorname{gr}^n_{\sh{J}_y}(\sh{F}_y))$, we know that there exists a polynomial $\mathrm{R}\in\bb{Q}[\mathrm{T}]$ such that $s(n)=\mathrm{R}(n)$ for $n$ large enough, namely the Hilbert-Samuel polynomial of $\sh{F}_y$ for the $\sh{J}_y$-filtration, $\sh{J}_y$-preadic \sref[0]{0.16.2.1}. Since the $\sh{O}_{\mathrm{Y},y}$-modules $\operatorname{gr}^n_{\sh{J}_y}(\sh{F}_y)$ are free by hypothesis, we have $r(n)=s(n)/m$, denoting by $m$ the length of the $\sh{O}_{\mathrm{X},y}$-module $\sh{O}_{\mathrm{Y},y}=\sh{O}_{\mathrm{X},y}/\sh{J}_y$; so $r(n)$ satisfies (i) in taking $\mathrm{P}=\frac{1}{m}\mathrm{R}$. We know furthermore \sref[0]{0.16.2.3} that $\deg(\mathrm{H})=\dim(\sh{F}_y)$, whence the relation \sref{IV.6.10.5.2}; the relation \sref{IV.6.10.5.3} follows from \sref{IV.5.1.12.2}.

It remains to prove (iii) for any point $x\in\mathrm{Y}$. Set $\mathrm{A}=\sh{O}_{\mathrm{X},x}$, $\mathfrak{J}=\sh{J}_x$, so that $\sh{O}_{\mathrm{Y},x}=\mathrm{A}/\mathfrak{J}$; let $\mathrm{S}=\operatorname{gr}^{\bullet}_\mathfrak{J}(\mathrm{A})$, which is an $(\mathrm{A}/\mathfrak{J})$-algebra graded of finite type, with positive degrees, such that $\mathrm{S}_0=\mathrm{A}/\mathfrak{J}$, and generated by its elements of degree $1$; let finally $\mathrm{N}=\operatorname{gr}^{\bullet}_\mathfrak{J}(\mathrm{M})$, which is a graded $\mathrm{S}$-module of finite type, each homogeneous component $\mathrm{N}_n$ being by hypothesis a free $(\mathrm{A}/\mathfrak{J})$-module of length $r(n)$. Let $\mathfrak{m}$ be the maximal ideal of $\mathrm{A}$, $k=\mathrm{A}/\mathfrak{m}$ its residue field; $\mathrm{B}=\mathrm{S}\otimes_\mathrm{A}k$ is a $k$-algebra graded of finite type, with positive degrees, such that $\mathrm{B}_0=k$, so that $\mathfrak{q}=\mathrm{B}^+=\bigoplus_{n\geqslant 1}\mathrm{B}_n$ is the maximal ideal in $\mathrm{B}$; $\mathrm{E}=\mathrm{N}\otimes_\mathrm{A}k$ is a graded $\mathrm{B}$-module of finite type such that $\operatorname{rg}_k(\mathrm{E}_n)=r(n)$. Applying \sref[0]{0.16.2.7}, and noting that $f_i\in\mathfrak{J}^{\mathrm{n}_i}$ ($1\leqslant i\leqslant d$) is an element whose image $t_i$ is the image in $\mathrm{B}_{\mathrm{n}_i}$. For $n\geqslant\sup(n_i)$, consider the $\mathrm{S}$-sub-module $\sum_{i=1}^d f_i\mathfrak{J}^{n-n_i}\mathrm{M}$ of $\mathrm{M}$; as the homogeneous component of degree $n$ of the sub-module $\sum_i t_i\mathrm{E}$ of $\mathrm{E}$ is equal to $\mathrm{E}_n$ as soon as $n$ is large enough, one sees that, for $n$ large enough, we have
\[
...
\label{IV.6.11.9}
(i) If $\mathrm{X}$ satisfies the hypothesis (CMU) of \sref{IV.6.11.8}, then, for every morphism $u:\mathrm{X}'\to\mathrm{X}$
locally of finite type, $\mathrm{X}'$ also satisfies (CMU). Let indeed $\mathrm{Y}'$ be a closed integral sub-prescheme of $\mathrm{X}'$, $y'$ its
generic point, and let $\mathrm{Y}$ be the closed integral sub-prescheme of $\mathrm{X}$ having for underlying space $\overline{\{y\}}$; then $u|\mathrm{Y}'$
factorizes as $\mathrm{Y}'\overset{v}{\to}\mathrm{Y}\overset{j}{\to}\mathrm{X}$, where $j$ is the canonical injection $({\mathbf{I}},~5.2.2)$, and $v$ is locally of finite type $(\mathbf{I},~6.6.6)$.
Restricting to affine open neighbourhoods of $y$ and $y'$ respectively, one can therefore restrict to the case
where $\mathrm{X}=\operatorname{Spec}(\mathrm{A})$, where $\mathrm{A}$ is an integral Cohen-Macaulay ring, and $\mathrm{X}'=\operatorname{Spec}(\mathrm{A}')$, where $\mathrm{A}'$ is an $\mathrm{A}$-algebra
\oldpage[IV-2]{163}
of finite type (with $f\neq 0$), one can suppose furthermore that $\mathrm{A}'$ contains a polynomial ring $\mathrm{A}[\mathrm{T}_1,\ldots,\mathrm{T}_n]=\mathrm{A}''$, and is an
$\mathrm{A}''$-algebra \emph{finite} (Bourbaki, \emph{Alg.~comm.}, chap.~V, \S~3, no~1, cor.~1 of th.~1). But $\mathrm{A}''$ is a Cohen-Macaulay ring
$(6.3.6)$; hence one can further restrict to the case where $\mathrm{A}'$ is also an $\mathrm{A}$-module \emph{free} of finite type. But then $\mathrm{A}'$ is an $\mathrm{A}$-module of
finite type, and as $\mathrm{A}'$ is also an $\mathrm{A}$-module \emph{free} of finite type. But $\mathrm{A}'$ is an $\mathrm{A}$-module of finite type,
hence one can further suppose that $\mathrm{A}'$ is also an $\mathrm{A}$-module \emph{libre}. But then $\mathrm{A}'$ is an $\mathrm{A}$-module free of finite type; as $\mathrm{A}$'
is an $\mathrm{A}$-module of finite type, and as $\mathrm{A}'$ is also an $\mathrm{A}'$-module of Cohen-Macaulay
$(\mathbf{0},~16.5.3)$, hence a Cohen-Macaulay ring.
...
a closed integral sub-prescheme $\mathrm{Z}$ of $\mathrm{Y}$ of generic point $z$; if $\mathrm{Y}'$ is the
sub-prescheme integral of $\mathrm{X}$ having for underlying space the closure $\mathrm{Y}'=\overline{\{z\}}$ of $z$
...
knows (\emph{loc.~cit.}) that $\mathrm{A}'(z)$ is a $\boldsymbol{k}(z)$-algebra \emph{separable}, hence a composed direct of field
extensions finite and separable of $\boldsymbol{k}(z)$. Such an algebra being a \emph{regular} ring, one sees
...
subring of $\mathrm{K}'$, hence $\mathrm{X}''=\operatorname{Spec}(\mathrm{A}'\otimes_\mathrm{A}\mathrm{K})$ is a $\mathrm{K}$-prescheme integral, whose field of
...
\label{IV.6.14.1.1}
Let $\mathrm{R}$ be a ring, composed direct of a finite number of fields.
\begin{enumerate}[label=(\roman*)]
...
a composed direct of a finite number of fields, hence every non-zero divisor
...
if $\mathrm{R}_\mathfrak{p}$ is composed direct of $2$ rings at least, the integral closure of $\mathrm{A}_\mathfrak{p}$ in $\mathrm{R}_\mathfrak{p}$ is
composed direct of $2$ rings at least non-reduced to $0$, which is absurd since $\mathrm{A}_\mathfrak{p}$
...
a composed direct of a finite number of integral rings and integrally closed; the direct composite
...
\begin{enumerate}[label=(\roman*)]
  \item If $\mathfrak{q}'_j$ $(1\leqslant j\leqslant r)$ are the minimal ideals of $\mathrm{A}'$, $\mathrm{K}'=\mathrm{K}\otimes_\mathrm{A}\mathrm{A}'$ is composed direct
...
is identified with the composed direct of the $\mathrm{L}'_j$ and $\mathrm{A}'$ is a subring of $\mathrm{K}'$.
...
$(5.13.3)$. Denote by $\mathrm{L}''$ the composed direct of the fields $\mathrm{L}'_j$ such that $\mathfrak{q}'_j\not\subset\mathfrak{p}'$; then for
...
But $\overline{\mathrm{B}'_{\mathfrak{q}'}}$, composed direct of normal rings $\mathrm{L}''$ and $\mathrm{B}'_{\mathfrak{q}'}$, is normal, and as
...
hence a composed direct of integral rings $\mathrm{C}_i$ in finite number, so that if $\mathrm{E}_i$ is the
field of fractions of $\mathrm{C}_i$, the composed direct $\mathrm{E}$ of the $\mathrm{E}_i$ is the total ring of fractions
...
closure of $\mathrm{A}'$ in $\mathrm{B}'_0$. But $\mathrm{C}_0$ is composed direct of the $\mathrm{C}_i$, integral closures of $\mathrm{A}$
in the $\mathrm{B}_i=\mathrm{B}/\mathfrak{q}_i$; hence $\mathrm{C}'_0$ is composed direct of the $\mathrm{C}'_i=\mathrm{C}_i\otimes_\mathrm{A}\mathrm{A}'$ and it suffices to show
...
normal $\mathrm{A}$-algebra, hence a composed direct of integral rings and integrally closed $\mathrm{A}'_i$, the
...
is therefore composed direct of the $\mathrm{B}\otimes_\mathrm{A}\mathrm{A}'_i=\mathrm{B}'_i$. One can therefore restrict to the case where $\mathrm{A}'$ is
...
hence $\mathrm{B}'$ is composed direct of integral rings; the total ring of fractions $\mathrm{L}'$ of $\mathrm{B}'$
is then composed direct of a finite number of fields. This being so, as $\mathrm{B}$ is the field of
...
appealing to \sref{IV.6.14.1}, even when $\mathrm{X}=\mathrm{Y}$, for in that case the integral
closure is the ring $\mathrm{B}$, which is not necessarily a Noetherian ring even if $\mathrm{B}$
is.
