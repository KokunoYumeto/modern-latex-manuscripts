...
Assuming only $f$ \emph{locally of finite presentation}, we will also give~(9.9) criteria for the set of points $x\in X$ where the fibre $X_{f(x)}$ (or the $\sh{O}_{X_{f(x)}}$-Module $\sh{F}_{f(x)}$) has a certain property, to be \emph{locally constructible}.
...
We know, by the theorem of zeros of Hilbert (Bourbaki, \emph{Alg.\ comm.}, chap.~V, \textsection3, n\textsuperscript{o}~1, cor.~2 du th.~1) that $K'$ is a finite extension of $k$.
...
\]
is true; then, by virtue of~\sref{IV.9.2.2},~(iv)), we have $E=p^{-1}(E_0)$; on the other hand, one can restrict further (1.8.2) to a $\mathbf{Z}$-algebra of finite type, hence a Noetherian scheme.
...
Indeed, the restriction to the case where $S$ is affine shows that one can restrict to the case where these parts are \emph{constructible}: applying~(8.3.11) then~(with the previous notations) one obtains a constructible part of $X_0$ for a suitable choice of $A_0$.
...
Using~\sref{IV.8.9.1} and~\sref{IV.8.5.2},~(i)), one sees that one can reduce to the case where $S$ (hence also $X$) is Noetherian, and by
\oldpage[IV-3]{63}
consequence (since $S$ is integral) and taking into account that $\sh{O}_\eta=\mathbf{k}(\eta)$ (so that $\operatorname{Ker}(v_s)=(\operatorname{Ker} v)_s$ and $\operatorname{Im}(u_s)=(\operatorname{Im} u)_s$), the sequence
\[
  0\to(\operatorname{Im} u)_\eta\to(\operatorname{Ker} v)_\eta\to(\operatorname{Ker} v/\operatorname{Im} u)_\eta\to 0
\]
is exact.

One can evidently restrict to a complex of three terms of degrees $-1$, $0$, $+1$: $\sh{M}\xrightarrow{u}\sh{N}\xrightarrow{v}\sh{P}$ and to $i=0$; the canonical homomorphism to consider is then $(\operatorname{Ker} v/\operatorname{Im} u)_s\to\operatorname{Ker}(v_s)/\operatorname{Im}(u_s)$ for every $s\in S$.
As $\sh{F}$, $\sh{G}$, $\sh{H}$ are coherent~($\mathbf{0}_{\mathrm{I}}$,~5.3.4), $\operatorname{Im}(u)$ and $\operatorname{Ker}(v)$ are also coherent~($\mathbf{0}_{\mathrm{I}}$,~5.3.4) and furthermore $\operatorname{Im}(u_s)=(\operatorname{Im}(u))_s$ and $\operatorname{Ker}(v_s)=(\operatorname{Ker}(v))_s$; and from the exact sequence $0\to\operatorname{Im}(u)\to\operatorname{Ker}(v)\to\operatorname{Ker}(v)/\operatorname{Im}(u)\to 0$, the conclusion follows from~\sref{IV.9.4.2} applied to this exact sequence (since $S$ is integral), which completes the proof.
...
Let $\sh{L}^\bullet=(\sh{L}^i)_{i\in\mathbf{Z}}$ be a complex of quasi-coherent $\sh{O}_X$-Modules of finite presentation.
For every $i$, there exist canonical homomorphisms
\begin{equation}
...
\label{IV.9.4.7}
Let $f:X\to S$ be a morphism of finite presentation, $\sh{F}$ a quasi-coherent $\sh{O}_X$-Module of finite presentation.
The set $E$ \textnormal{(}resp.\ $E'$\textnormal{)} of $s\in S$ such that $\sh{F}_s$ is an $\sh{O}_{X_s}$-Module locally free \textnormal{(}resp.\ locally free of rank $n$\textnormal{)} is locally constructible.
\end{proposition}
...
$3^\circ$ $\eta\in S-E$.
As $\sh{F}_\eta$ is a coherent $\sh{O}_{X_\eta}$-Module, to say that it is not locally free means that it is not flat over $\sh{O}_{X_\eta}$ (Bourbaki, \emph{Alg.\ comm.}, chap.~II, \textsection5, n\textsuperscript{o}~2, cor.~2 du th.~1).
...
Or, it follows from~\sref{IV.8.1.2}, applied following the method of~\sref{IV.8.1.2},~$a$)), that one can suppose (replacing $S$ by a neighbourhood of $\eta$) that $X$ and $X_\eta$ have the same connected components, the connected components of $X_\eta$ being the intersections of $X_\eta$ and the connected components of $X$.
...
Then the set $E$ \textnormal{(}resp.\ $E'$\textnormal{)} of $s\in S$ such that $\sh{F}_s$ is an $\sh{O}_{X_s}$-Module of torsion \textnormal{(}resp.\ an $\sh{O}_{X_s}$-Module without torsion\textnormal{)} is locally constructible.
\end{proposition}
...
Note furthermore that $X$ is a finite union of affine open sets $U_i$, of finite type over $S$, and each of the $(U_i)_\eta$ is either empty (resp.\ non-empty), one knows that $(U_i)_s$ is also empty (resp.\ non-empty) in a neighbourhood of $\eta$~\sref{IV.9.2.6}.
We can therefore suppose all the $(U_i)_\eta$ non-empty and integral, and to say that $\sh{F}_s$ is of torsion (resp.\ without torsion) is equivalent to saying that each $\sh{F}_s|(U_i)_s$ is of torsion (resp.\ without torsion).
We are therefore reduced to the case where $X=\operatorname{Spec}(B)$ and $\sh{F}=\widetilde{M}$, where $M$ is a $B$-module of finite type; we will set $B_s=B\otimes_A\mathbf{k}(s)$, $M_s=M\otimes_A\mathbf{k}(s)$ and one can suppose $B_\eta$ integral.
There are then four cases to consider:

$1^\circ$ $\eta\in E$; $M_\eta$ is therefore a $B_\eta$-module of torsion of finite type, and there is therefore an $h\neq 0$ in $B_\eta$ such that $hM_\eta=0$; by virtue of~\sref{IV.8.5.2},~(i)), applied following the general method of~\sref{IV.8.1.2},~$a$)), there exists a neighbourhood $U$ of $\eta$ in $S$ such that $hM_s=0$ for every $s\in U$; hence $E$ is a neighbourhood of $\eta$.
...
$3^\circ$ $\eta\in S-E$.
As $\sh{F}_\eta$ is a coherent $\sh{O}_{X_\eta}$-Module, to say that it is not locally free is equivalent to saying that it is not flat over $\sh{O}_{X_\eta}$ (Bourbaki, \emph{Alg.\ comm.}, chap.~II, \textsection5, n\textsuperscript{o}~2, cor.~2 du th.~1).
The fact that $S-E$ is a neighbourhood of $\eta$ then follows from the lemma~\sref{IV.9.4.7.1} applied to the case where $g$ is the identity.
...
As $Z_i$ is locally closed in $X$, there is a sub-prescheme of $X$ having $Z_i$ as underlying space (and still denoted $Z_i$), which is of type fini over $S$ \sref[I]{I.6.3.5}.
...
We know that $B$ is a Dedekind ring, and that all the maximal ideals of $k[\mathrm{T}]$, except for a finite number, are not ramified in $B$; as furthermore the residue field of every maximal ideal of $B$ is necessarily $k$ (since $k$ is a finite extension of $k$), we see that for almost all the maximal ideals $\mathfrak{j}_s$ of $k[\mathrm{T}]$, $B_s=B/\mathfrak{j}_s B$ is a direct composition of $[L:K]$ copies of the field $k$, in other words $X_s$ is not irreducible, although $X_\eta=\operatorname{Spec}(L)$ is.
...
\begin{enumerate}[label=\upshape{$\arabic{enumi}^\circ$}]
  ...
  \item\oldpage[IV-3]{73} $f_\eta(X_\eta)$ is locally closed in $Y_\eta$ but $f_\eta$ is not an immersion.
  ...
  As $Y$ is Noetherian, $V$ is of finite type over $S$, so we can reduce to the case where $f$ is \emph{surjective} by replacing $Y$ by $f(X)$.
  But then $f_s(X_s)=(f(X))_s$ is closed in $Y_s$ for every $s\in S$, so we can say that $f_s$ is a closed immersion, meaning that $f_s$ defines $X$ as a sub-prescheme of $Y_s$ defined by an Ideal coherent $\mathscr{J}$ of $\sh{O}_Y$.
  By hypothesis $f_\eta$ is not an isomorphism, therefore $\mathscr{J}_\eta\neq 0$; we conclude \sref{IV.9.4.5} that in a neighbourhood of $\eta$, $\mathscr{J}_s\neq 0$, and consequently the closed immersion $f_s$ is not open.
\end{enumerate}
...
\begin{enumerate}[label=\upshape{$\arabic{enumi}^\circ$}]
  ...
  But then $f_s(X_s)=(f(X))_s$ is open in $Y_s$ for every $s\in S$, so we can reduce to the case where $f$ is \emph{surjective} and $f$ is a closed immersion, in other words, that $X$ is a sub-prescheme of $Y$ defined by an Ideal coherent $\mathscr{J}$ of $\sh{O}_Y$.
...
The conclusion follows then from the stability of the properties considered under base change \sref[II]{II.4.6.13} and \sref{IV.4.4.10}.
...
As $f_s$ is proper and, for every $y\in Y$ above $s$, $\sh{L}_y=(\sh{L}_s)_y$, this being the case since $f_s$ is proper, we have $F_s\neq\varnothing$.
It suffices to show that $F_s\neq\varnothing$ for every $s$ in a neighbourhood of $\eta$ in $S$, and as $F$ is closed in $Y$ and $F_\eta\neq\varnothing$, it follows from \sref{IV.9.5.1} that the set of $s\in S$ such that $F_s\neq\varnothing$ is a neighbourhood of $\eta$ in $S$.
\end{proof}
...
\begin{proof}
As $X_\eta$ is Noetherian, there exists an injective homomorphism $v:\sh{F}_\eta\to\bigoplus_{i=1}^m\sh{G}_i$, where each of the $\sh{G}_i$ is irredondant and $\operatorname{Ass}(\sh{F}_\eta)$ is the set of the $x_i$, where $\{x_i\}=\operatorname{Ass}(\sh{G}_i)$ \sref{IV.3.2.6}; if $Z_i$ is the closure of $\{x_i\}$ in $X_\eta$, we have $Z_i=\operatorname{Supp}(\sh{G}_i)$ \sref{IV.3.1.4}, and by hypothesis the $Z_i$ are the irreducible components of $Z=\operatorname{Supp}(\sh{F}_\eta)$.
...
Then the elements of $\operatorname{Ass}((\sh{F}_i)_s)$ are the maximal points of $(Y_i)_s$; no two of them can therefore belong to a $(Y_j)_s$, for $i\neq j$, and the proposition will then be irredondant; furthermore, we can suppose that $\sh{F}_\eta$ is \emph{irredondant}; in addition, we can reduce to the case where $S=\operatorname{Spec}(A)$ is affine and where the morphism $f:X\to S$ is dominant; $A$ is then a Noetherian integral ring, \emph{sub-ring} of a finite A-algebra $B$, and $\sh{F}=\widetilde{M}$, where $M$ is a B-module of finite type; by hypothesis, if $K$ is the field of fractions of $A$, the $B_{(K)}$-module $M_{(K)}$ is irredondant.
...
As $B_{(K)}$ is a $C_{(K)}$-module without torsion, we can apply once more the preceding reasoning, replacing $B$, $M$, and $\mathfrak{q}$ by $C$, $B$, and $(0)$ respectively, and it suffices to show that for $s$ close to $\eta$, $C_{(\boldsymbol{k}(s))}$ has no associated prime cycle.
...
Furthermore, for all $s\in S$, if $s_0$ is the projection of $s$ in $S_0=\operatorname{Spec}(A_0)$, and $h_s$ the projection $X_s\to(X_0)_{s_0}$, we have $Z_s=h_s^{-1}((Z_0)_{s_0})$.
...
\oldpage[IV-3]{89}
containing $x$, we have $\operatorname{codim}_x(Z_s,X_s)=\inf_{\beta,\alpha}(\sup(\operatorname{codim}(Z_s^{(\beta)},X_s^{(\alpha)})))$, $(\alpha,\beta)$ varying over the set of pairs such that $x\in Z_s^{(\beta)}\in X_s^{(\alpha)}$ ($0$, \sref{IV.14.2.6}).
As the algebraic preschemes are biequidimensional \sref{IV.5.2.1}, we can write, by virtue of ($0$, \sref{IV.14.3.3.1})
\begin{equation}
\tag{9.9.1.1}
\label{IV.9.9.1.1}
  \operatorname{codim}_x(Z_s,X_s)=\inf_{\beta,\alpha}(\sup(\dim(X_s^{(\alpha)})-\dim(Z_s^{(\beta)})))
\end{equation}
...
We thus see that if $E$ (resp.~$E_0$) is the set of $x\in X$ (resp.~$x_0\in X_0$) satisfying one of the conditions (i), (ii), (iii) of the statement (resp.~the same condition), we have $E=h^{-1}(E_0)$, and by virtue of \sref{IV.1.8.2}, we see that one can restrict to considering the case where $A$ is \emph{Noetherian}, as well as that of \sref{IV.9.9.1.1}, and we have $(0_{\mathrm{III}}$, \sref{IV.9.2.3}), and also of \sref{IV.9.9.1}, (iii).

By hypothesis $Z_\eta$ is closed in $X_\eta$; as $Z$ is constructible, it follows from \sref{IV.8.3.11}, applied following the method of \sref{IV.8.1.2},~$a$)), that we can, by replacing $S$ by an open neighbourhood of $\eta$, suppose that $Z$ is \emph{closed} in $X$.
Suppose then that the irreducible components of $X$ (resp.~$Z$) are $X_i$ (resp.~$Z_j$) the irreducible components of $X_\eta$ (resp.~$Z_\eta$); by virtue of ($0_I$, \sref{IV.2.1.8}), the $X_i\cap X_\eta$ (resp.~$Z_j\cap X_\eta$) are the irreducible components of $X_\eta$ (resp.~$Z_\eta$) containing $x$; by virtue of \sref{IV.9.5.1}, we can suppose further, by restricting $S$ to a neighbourhood, that for all $s\in S$, the $X_i$ (resp.~$Z_j$) are exactly the irreducible components of $X$ (resp.~$Z$) meeting $\overline{\{x\}}$ and that the $(X_i)_s\cap\overline{\{x\}}$ and $(Z_j)_s\cap\overline{\{x\}}$ are non-empty for all $s\in S$.
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item The set of $x\in X$ such that $\sh{F}_{f(x)}$ possesses property ($S_n$) at the point $x$ \sref{IV.5.7.2}.
...
(ii) All the properties considered are local on $X$, and we will therefore restrict to the case where $X=\operatorname{Spec}(B)$ and $S=\operatorname{Spec}(A)$ are affine and $f$ a morphism of finite presentation.
We keep the notations from the beginning of the proof of \sref{IV.9.9.1}, and we suppose furthermore $A_0$ chosen so that there exists a coherent $\sh{O}_{X_0}$-Module $\sh{F}_0$ such that $\sh{F}$ is isomorphic to $\sh{F}_0\otimes_{A_0}A$.
Then (\sref{IV.4.2.7} and \sref{IV.4.7.9}) the set of geometric lengths of $(\sh{F}_0)_{f(x_0)}$ at the generic points of the irreducible components of $\operatorname{Supp}((\sh{F}_0)_{f(x_0)})$ which contain $x_0$ is the same as the analogous set for $\sh{F}_{f(x)}$; in other words, if $E$ (resp.~$E_0$) is the set of $x\in X$ (resp.~$x_0\in X_0$) satisfying the condition (ii) of the statement, we have $E=h^{-1}(E_0)$, and by virtue of \sref{IV.1.8.2}, we see that one can restrict to considering the case where $A$ is \emph{Noetherian}; in other words, say that $S$ is \emph{integral} and $\eta=f(x)$ its generic point.
By further reducing, if $S'$ is the sub-prescheme reduced of $S$ having $\{\overline{f(x)}\}$ for underlying space, and if $X'=f^{-1}(S')$, the fibres of $X$ and $X'$ at the points of $S'$ are the same, hence we can replace $S$ by $S'$ and $X$ by $X'$, in other words suppose that $S$ is \emph{integral} and $\eta=f(x)$ its generic point.

Now, if one sets $Z=\operatorname{Supp}(\sh{F})$, the same reasoning as in the proof of \sref{IV.9.9.1} shows that, if $Z_i$ are the irreducible components of $Z$ containing $x$, we can suppose that ce are exactly the irreducible components of $Z$ meeting $\overline{\{x\}}$ and that $(Z_i)_s\cap\overline{\{x\}}\neq\varnothing$ for all $s\in S$.
The conclusion follows from \sref{IV.9.8.3} and \sref{IV.9.8.6}.

(iii) We reason as in (iii), using this time \sref{IV.4.7.11}.
With the same notations as in (iii), the prime cycles associated to $\sh{F}_s$ that are irreducible components of $(Z'_i)_s$ are immersed if and only if $(Z'_i)_\eta$ is an associated immersed prime cycle of $\sh{F}_\eta$.
We conclude already that if $x$ belongs to a (resp.~does not belong to any) associated immersed prime cycle of $\sh{F}_\eta$, the set of $x'\in\overline{\{x\}}$ such that $x'$ belongs to a (resp.~does not belong to any) associated immersed prime cycle of $\sh{F}_{f(x')}$ is a neighbourhood of $x$ in $\overline{\{x\}}$.
The conclusion follows from this remark, from the characterisation of the points where a Module is geometrically reduced \sref{IV.4.7.10}, and of (ii).

(iv) Taking into account \sref{IV.4.7.11}, we reduce still again to the case where $S$ is Noetherian and integral and where $\eta=f(x)$ is its generic point.
Let $Y$ be a closed sub-prescheme of $X$ having $\operatorname{Supp}(\sh{F})$ for underlying space.
Reasoning as at the beginning of the proof of \sref{IV.9.7.7}, we see that there exists a finite integral $A$-algebra $A'$ (such that if $S'=\operatorname{Spec}(A')$, the morphism $S'\to S$ is finite and surjective) such that if $Y'=Y_{(S')}$ and $\eta'$ is the generic point of $S'$, the irreducible components of $Y_{\eta'}$ are \emph{geometrically irreducible}.
On the other hand, if $X'=X_{(S')}$, the morphism projection $h:X'\to X$ is finite and surjective; consequently, if $x'$ is a point of $X'$ above $x$, we have $h(\{x'\})=\{x\}$ and the image by $h$ of a neighbourhood of $x'$ in $\{x'\}$ is a neighbourhood of $x$ in $\{x\}$.
Taking into account \sref{IV.4.7.11} and setting $\sh{F}'=\sh{F}_{(S')}$, we are thus reduced to proving that if $\sh{F}_{\eta'}$ is (resp.~is not) geometrically integral at the point $x'$, the set of $y'\in\overline{\{x'\}}$ such that $\sh{F}'_{f'(y')}$ (where $f'=f_{(S')}:X'\to S'$) is (resp.~is not) geometrically integral at the point $y'$ is a neighbourhood of $x'$ in $\overline{\{x'\}}$.
We can thus restrict to the case where $X'=X$ and where the irreducible components of $Y_\eta$ are \emph{geometrically irreducible}; if one denotes by $Z_k$ the closed subsets of $X$ such that $Z_k\cap X_\eta$ are the
\oldpage[IV-3]{91}
irreducible components of $Y_\eta$, it follows from \sref{IV.9.7.7}, \sref{IV.9.7.8} and \sref{IV.9.5.3} that for all $s$ sufficiently close to $\eta$, the $(Z_k)_s$ are the irreducible components of $Y_s$ and that they are \emph{geometrically irreducible}.
To say that at a point $y\in X_s$, $\sh{F}_s$ is geometrically integral means that $\sh{F}_s$ is geometrically reduced at this point and furthermore that $y$ belongs to a \emph{single} of the $(Z_i)_s$ \sref{IV.4.6.22}.
The conclusion follows thus from part (iv) and of \sref{IV.9.5.1} applied to the intersection of $\overline{\{x\}}$ and of each $Z_i$.

(v) Taking into account \sref{IV.4.7.11}, we reduce once more to the case where $S$ is Noetherian and integral and where $\eta=f(x)$ is its generic point.
Let $Y$ be a closed sub-prescheme of $X$ having $\operatorname{Supp}(\sh{F})$ for underlying space.
Reasoning as at the beginning of the proof of \sref{IV.9.7.7}, we see that there exists a finite integral $A$-algebra $A'$ (such that if $S'=\operatorname{Spec}(A')$, the morphism $S'\to S$ is finite and surjective) such that if $Y'=Y_{(S')}$ and if $\eta'$ is the generic point of $S'$, the irreducible components of $Y_{\eta'}$ are \emph{geometrically irreducible}.
On the other hand, if $X'=X_{(S')}$, the morphism projection $h:X'\to X$ is finite and surjective; consequently, if $x'$ is a point of $X'$ above $x$, we have $h(\{x'\})=\{x\}$ and the image by $h$ of a neighbourhood of $x'$ in $\{x'\}$ is a neighbourhood of $x$ in $\{x\}$.
Taking into account \sref{IV.4.7.11} and setting $\sh{F}'=\sh{F}_{(S')}$, we are thus reduced to proving that if $\sh{F}_{\eta'}$ is (resp.~is not) geometrically integral at the point $x'$, the set of $y'\in\overline{\{x'\}}$ such that $\sh{F}'_{f'(y')}$ (where $f'=f_{(S')}:X'\to S'$) is (resp.~is not) geometrically integral at the point $y'$ is a neighbourhood of $x'$ in $\overline{\{x'\}}$.
We can thus restrict to the case where $X'=X$ and where the irreducible components of $Y_\eta$ are \emph{geometrically irreducible}; if one denotes by $Z_k$ the closed subsets of $X$ such that $Z_k\cap X_\eta$ are the irreducible components of $Y_\eta$, it follows from \sref{IV.9.7.7}, \sref{IV.9.7.8} and \sref{IV.9.5.3} that for all $s$ sufficiently close to $\eta$, the $(Z_k)_s$ are the irreducible components of $Y_s$ and that they are \emph{geometrically irreducible}.
To say that at a point $y\in X_s$, $\sh{F}_s$ is geometrically integral means that $\sh{F}_s$ is geometrically reduced at this point and furthermore that $y$ belongs to a \emph{single} of the $(Z_i)_s$ \sref{IV.4.6.22}.
The conclusion follows thus from part (iv) and of \sref{IV.9.5.1} applied to the intersection of $\overline{\{x\}}$ and of each $Z_i$.

(vi) Keeping the same notations as in (ii), it follows from \sref{IV.6.2.1} that we have $\dim.\operatorname{proj}((\sh{F}_x)_x)=\dim.\operatorname{proj}((\sh{F}_{f(x)})_x)$; one can thus still restrict to the case where $A$ is \emph{Noetherian}.
Furthermore, on the other hand, we reduce still to showing that, for all $x\in X$, there exists a neighbourhood $V$ of $x$ in $\overline{\{x\}}$ such that, for all $x'\in V$, we have $\dim.\operatorname{proj}((\sh{F}_{f(x')})_{x'})=\dim.\operatorname{proj}((\sh{F}_{f(x)})_x)$; and as above, we can suppose that $S$ is \emph{integral} and that $\eta=f(x)$ is its generic point, in virtue of the generic flatness theorem \sref{IV.6.9.1}, we can, by replacing $S$ by a neighbourhood open of $\eta$, suppose that the morphism $f$ is \emph{flat} and that $\sh{F}$ is $f$-\emph{flat}; one has then $\dim.\operatorname{proj}((\sh{F}_{f(x')})_{x'})=\dim.\operatorname{proj}((\sh{F}_{f(x)})_x)$ for \emph{all} $x'\in X$ by virtue of \sref{IV.6.2.3}.
This being so, on can (by replacing $S$ by a neighbourhood open of $x$) suppose that $\dim.\operatorname{proj}(\sh{F}_x)\leqslant\dim.\operatorname{proj}(\sh{F}_x)$ for \emph{all} $x'\in X$.
On the other hand, if $\dim.\operatorname{proj}(\sh{F}_x)=n$, there is by hypothesis a finite $\sh{O}_x$-module of type $M$ such that $\operatorname{Ext}^n_{\sh{O}_x}(\sh{F}_x,M)\neq 0$ ($0$, \sref{IV.17.2.4}).
Now, there exists an $\sh{O}_X$-Module coherent $\sh{G}$ such that $M=\sh{G}_x$ (by replacing $X$ by a neighbourhood open of $x$ ($0_I$, \sref{IV.5.3.8}));
\oldpage[IV-3]{92}
...
(vii) As $B$ is an $A$-algebra of finite type, $X$ is $S$-isomorphic to a \emph{closed sub-scheme} of an $S$-scheme of the form $Y=\operatorname{Spec}(A[T_1,\ldots,T_r])$; if $j:X\to Y$ is the canonical injection, and $\sh{G}_s=(j_s)_*(\sh{F}_s)$ for all $s\in S$, and, by virtue of ($0$, \sref{IV.16.4.11}), we can restrict to proving the assertion for $Y$ and $\sh{G}$.
...
But as $W$ is constructible \sref{IV.8.9.1} and each $W_s$ closed, it follows from \sref{IV.9.9.1},~(ii)) that the two functions from the right-hand side of \sref{IV.9.9.2.1} are constructible, which finishes the proof of the proposition in the case (vii).

(viii) Let $U_n$ be the set of $x\in X$ such that $\operatorname{coprof}((\sh{F}_{f(x)})_x)\leqslant n$, and set $Z_n=X-U_n$; it follows from (vii) that the $Z_n$ are \emph{constructible}; furthermore, as the function $x\mapsto\dim_x(W_{f(x)})$ takes only a finite number of values, the numbers $\dim(W_{f(x)})$ have a finite upper bound $m$ when $x$ ranges over $X$; as $\operatorname{coprof}((\sh{F}_{f(x)})_x)\leqslant\dim((\sh{F}_{f(x)})_x)\leqslant\dim(W_{f(x)})$, we have $Z_n=\varnothing$ for $n\geqslant m$.
...
For all $s\in S$, if $s_0$ is the projection of $s$ in $S_0=\operatorname{Spec}(A_0)$, and $h_s$ the projection $X_s\to(X_0)_{s_0}$, we have $(\sh{L}_\bullet)_s=(\sh{L}_\bullet^{(0)})_{s_0}\otimes_{\mathbf{k}(s_0)}\mathbf{k}(s)$, and consequently, by virtue of \sref{IV.1.8.2}, if $x\in X$ is such that $(\sh{H}_n((\sh{L}_\bullet)_{f(x)}))_x=0$ (resp.~$\neq 0$), there exists a neighbourhood open $V$ of $x$ in $\overline{\{x\}}$ such that, for all $x'\in V$, we have $(\sh{H}_n((\sh{L}_\bullet)_{f(x')}))_{x'}=0$ (resp.~$\neq 0$).
Replacing $S$ by the sub-prescheme reduced of $S$ having $\overline{\{f(x)\}}$ for underlying space, we can suppose that $S$ is integral and that $f(x)=\eta$ is its generic point.
Then, by restricting $S$ to a neighbourhood open of $\eta$, we can suppose that for all $s\in S$, on a $(\sh{H}_n(\sh{L}_\bullet))_s=\sh{H}_n((\sh{L}_\bullet)_s)$ \sref{IV.9.4.3}, and by extension, if $Z$ is the support of $\sh{H}_n(\sh{L}_\bullet)$, the support of $\sh{H}_n((\sh{L}_\bullet)_s)$ is $Z_s=Z\cap X_s$ \sref[I]{I.9.1.13.1}.
