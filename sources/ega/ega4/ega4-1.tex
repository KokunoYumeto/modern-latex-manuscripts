...
\begin{enumerate}[label=(\roman*)]
  ...
  \item If $f:X\to Y$ is an $S$-morphism quasi-compact, the same is true of $f_{(S')}:X_{(S')}\to Y_{(S')}$
    for every extension $g:S'\to S$ of the base prescheme.
  \item If $f:X\to X'$ and $g:Y\to Y'$ are two $S$-morphisms quasi-compacts,
    \[
      f\times_S g: X\times_S Y \to X'\times_S Y'
    \]
    is quasi-compact.
  \item If the composition $g\circ f$ of two morphisms $f:X\to Y$, $g:Y\to Z$ is quasi-compact, and
    if $g$ is separated, or the underlying space of $X$ is locally Noetherian, $f$ is quasi-compact.
...
\label{IV.1.1.6}
Let $f':X'\to Y$, $f'':X''\to Y$ be two morphisms, $X$ the prescheme
which is the direct sum $X'\coprod X''$, $f:X\to Y$ the morphism equal to $f'$ in $X'$ and to $f''$ in $X''$. For $f$ to be
...
By definition, every separated morphism is quasi-separated, $\Delta_f$ being a closed immersion~\sref[I]{I.5.4.1}, (i)); a \emph{scheme} $X$ is a quasi-separated prescheme, being separated over
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item If $f:X\to Y$ and $g:Y\to Z$ are two quasi-separated morphisms, $g\circ f:X\to Z$ is
    quasi-separated.
  \item Let $X$, $Y$ be two $S$-preschemes, $f:X\to Y$ an $S$-morphism quasi-separated. Then,
...
\[
\xymatrix{
  X\times_Y X \ar[r]^-{i} & X\times_Z X \\
  Y \ar[r]_-{\Delta_g} & Y\times_Z Y
}
\]
\oldpage[IV-1]{227}
is the same as that of $i$~\sref{IV.1.1.2}, (iii)); if furthermore $f$ is quasi-separated, $\Delta_f$ is quasi-compact,
so it is the same for $i\circ\Delta_f$~\sref{IV.1.1.2}, (ii)) which is equal to $\Delta_{g\circ f}$.
\end{proof}
...
\label{IV.1.2.4}
Let $f:X\to Y$ be a morphism, $g:Y\to Z$ a morphism.
If $g\circ f$ is quasi-compact, then so is $f$.
\end{proposition}

\begin{proof}
We know that $f$ is the morphism composed $X\xrightarrow{\Gamma_f} X\times_Z Y\xrightarrow{\mathrm{pr}_2} Y$~\sref[I]{I.5.3.13}; on the other hand,
...
\tag{1.2.4.1}
\]
which identifies $X\times_Y Y$-preschemes $Y$ and $X\times_Z Y$~\sref[I]{I.5.3.7}. As
...
\begin{proof}
The inverse image of $X\times_Y X$ of $U_\alpha$ is $X_\alpha\times_{U_\alpha} X_\alpha$, writing $X_\alpha=f^{-1}(U_\alpha)$,
...
is quasi-separated, it amounts to saying that $f_\alpha$ is quasi-separated or that the composition
$X_\alpha\xrightarrow{f_\alpha} U_\alpha\to\Spec(\mathbf{Z})$ is~\sref{IV.1.2.2}, (ii) and~(v)), hence the proposition.
\end{proof}
...
\label{IV.1.2.9}
Let $X'$, $X''$ be two preschemes, $X=X'\coprod X''$ their direct sum,
$f':X'\to Y$, $f'':X''\to Y$ two morphisms, $f:X\to Y$ the morphism which coincides with $f'$ in $X'$
and with $f''$ in $X''$. For $f$ to be quasi-separated, it is necessary and sufficient that $f'$ and $f''$ be so.
\end{proposition}

\begin{proof}
Indeed, $X\times_Y X$ is the direct sum of the four preschemes $X'\times_Y X'$, $X'\times_Y X''$, $X''\times_Y X'$
and $X''\times_Y X''$, and $\Delta_f$ is the morphism which coincides with $\Delta_{f'}$ in $X'$ and with $\Delta_{f''}$ in $X''$;
...
\medskip\noindent
\begin{enumerate}[label=(\roman*)]
  \item Every local isomorphism is locally of finite type.
  \item If two morphisms $f:X\to Y$, $g:Y\to Z$ are locally of finite type, it is the same for $g\circ f$.
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item If $B$ is a ring of fractions of an $A$-algebra of finite type $C$, $B'$ is a ring
    of fractions of the $A'$-algebra of finite type $C\otimes_A A'$, hence~(ii).
\end{enumerate}
\end{proof}
...
$S=A-\mathfrak{p}$, $C_{\mathfrak{q}}$ is also a local ring over a prime ideal of $S^{-1}C$; as $S^{-1}C$
...
$A$-isomorphic to a quotient of an $A$-algebra of the form $C_{\mathfrak{q}}$, where $C$ is an algebra of
polynomials $A[T_1,\ldots,T_n]$ and $\mathfrak{q}$ a prime ideal of $C$.
\end{proposition}
...
\begin{proof}
Assertion~(i) is trivial.
To prove~(iii), we may limit to the case where $S=Y$ \sref[I]{I.3.3.11}; let $x'$ be a point of $X_{(S')}$, $s'$ and $x$ its projections on $S'$ and $X$ respectively, $s$ the common projection of $s'$ and $x$ on $S$.
...
To prove~(ii), consider a point $x\in X$, and let $y=f(x)$, $z=g(f(x))$.
...
We know that~(iv) follows from~(i), (ii), and~(iii) \sref[I]{I.3.5.1}.
To prove~(v), consider the commutative diagram
\[
\xymatrix{
...
If we know that $\Delta_g$ is locally of finite presentation, it will follow by~(iii) that so is $\Gamma_f$.
On the other hand, we have the factorisation of $f$ as $X\xrightarrow{\Gamma_f} X\times_Z Y\xrightarrow{p_2} Y$ \sref[I]{I.5.3.13}, where the projection $p_2$ is equal to $(g\circ f)\times_Z 1_Y$; as $g\circ f$ is assumed locally of finite presentation, so is $p_2$ by~(iv), and it will follow finally from~(ii) that $f$ is also locally of finite presentation.
...
\begin{proof}
By hypothesis, there exists a $C$-algebra of polynomials $C'=C[T_1,\ldots,T_n]$ and a surjective $C$-homomorphism $v:C'\to C_s/\mathfrak{a}_s$ whose kernel $\mathfrak{b}$ is of finite type.
...
\label{IV.1.4.7}
Let $\rho:A\to B$ be a homomorphism of rings making $B$ an $A$-algebra of finite presentation.
For $B$ to be an $A$-module of finite presentation, it is necessary and sufficient that it be an $A$-module of finite type.
\end{proposition}
...
\begin{lemma}[1.4.7.1]
\label{IV.1.4.7.1}
If $B$ is an $A$-algebra of finite presentation, there exists a surjective $A$-homomorphism of algebras $u:B'\to B$, where $B'$ is an $A$-algebra of finite presentation and a free $A$-module.
\end{lemma}
...
This lemma being proved, let us keep the same notation, and let $w=v\circ u:B''\to B$.
Let $\mathfrak{b}$ (resp.~$\mathfrak{a}$) be the kernel of $w$ (resp.~$u$); we have $\mathfrak{a}=v(\mathfrak{b})$ since $v$ is surjective.
Now by~\sref{IV.1.4.4}, if $B$ is an $A$-algebra of finite presentation, $\mathfrak{b}$ is an ideal of finite type of $B''$, so $\mathfrak{a}$ is an $A$-module of finite type since $B$ is an $A$-algebra of finite presentation.
...
\label{IV.1.5.4}
\begin{enumerate}
  \item[\rm(i)] Let $j:X\to Y$ be a quasi-compact immersion.
  If $j$ is a closed immersion, or if the underlying space of $X$ is Noetherian, or if the space underlying $Y$ is locally Noetherian, then $j$ is of finite type.
...
\begin{proof}
This follows immediately (except for the case~(v) where $X$ is Noetherian, proved in \sref[I]{I.6.3.6}) from the definitions and from \sref{IV.1.1.2}, \sref{IV.1.2.4}, and \sref{IV.1.3.4}.
\end{proof}
...
\begin{enumerate}
  ...
  \item[\rm(ii)] $f$ is quasi-compact (which, when~(i) is satisfied, is equivalent to saying that $f$ is of finite type~\sref{IV.1.5.2});
...
Condition~(iii) is automatically satisfied if $f$ is separated, or if $X$ is locally Noetherian~\sref{IV.1.2.8}; when $Y$ is locally Noetherian, it amounts to the same to say that $f$ is \emph{of finite presentation} or that $f$ is \emph{of finite type} (the latter condition implying that $X$ is locally Noetherian~\sref{IV.1.3.3}).
...
\begin{remark}[1.6.4]
\label{IV.1.6.4}
In the definition~\sref{IV.1.6.1}, condition~(iii) is not a consequence of the two others.
...
Let $X$ be the prescheme obtained by gluing two preschemes $Y_1$, $Y_2$ isomorphic to $Y$ along the open subsets $U_1$, $U_2$ corresponding to $U$ \sref[0]{0.4.1.7}, so that $X$ is the union of two open subsets isomorphic respectively to $Y_1$ and $Y_2$ (and that we identify with them), with $Y_1\cap Y_2=U$.
Furthermore, there is a morphism $f:X\to Y$ that coincides in $Y_i$ with the isomorphism $Y_i\to Y$ ($i=1,2$).
It is clear that $f$ satisfies condition~(i) of~\sref{IV.1.6.1}, being a local isomorphism; it also satisfies~(ii), the inverse image of an open quasi-compact subset of $Y$ being the union of its images in $Y_1$ and $Y_2$; but as $Y_1\cap Y_2$ is not quasi-separated~\sref{IV.1.2.7}.
\end{remark}
...
\begin{env}[1.7.1]
\label{IV.1.7.1}
In the statements of \sref[I]{I.6.4.2}, \sref[I]{I.6.4.3}, and \sref[I]{I.6.4.9}, we can, instead of assuming that $X$ and $Y$ are of finite type over the field $K$, assume only that they are \emph{locally of finite type} over $K$; for \sref[I]{I.6.4.2}, it suffices to observe that every point of a prescheme $X$ that is closed in every open affine of $X$ is closed in $X$, and a scheme affine locally of finite type over $K$ is \emph{ipso facto} of finite type over $K$ \sref{IV.1.5.1}.
For \sref[I]{I.6.4.9}, we use \sref{IV.1.3.7} and \sref{IV.1.5.4},~(v).
\end{env}
...
\begin{env}[1.7.4]
\label{IV.1.7.4}
In \sref[I]{I.9.2.2}, we can replace the three hypotheses (each implying the others) by the single hypothesis that $f$ is \emph{quasi-compact and quasi-separated}, by virtue of~\sref{IV.1.2.6} and~\sref{IV.1.2.7}.
...
\begin{env}[1.7.6]
\label{IV.1.7.6}
In \sref[I]{I.9.3.4} and \sref[I]{I.9.3.5}, it suffices to assume $X$ quasi-compact, $\sh{F}$ and $\sh{G}$ quasi-coherent and of finite type.
\end{env}
...
\begin{env}[1.7.12]
\label{IV.1.7.12}
In \sref[II]{II.3.8.5}, we can similarly suppose only that $Y$ is quasi-compact and quasi-separated, this hypothesis being covered by \sref[I]{I.9.4.7}.
\end{env}
...
\begin{env}[1.7.14]
\label{IV.1.7.14}
In \sref[II]{II.4.5.2} and \sref[II]{II.4.5.5}, it suffices to assume $X$ \emph{quasi-compact and quasi-separated}, this hypothesis being covered by \sref[I]{I.9.3.1} and \sref[I]{I.9.3.2}.
\end{env}

\begin{env}[1.7.15]
\label{IV.1.7.15}
In \sref[II]{II.4.6.8}, it suffices still to assume $X$ \emph{quasi-compact and quasi-separated}, this hypothesis being covered by \sref[I]{I.9.4.7}.
\end{env}

\begin{env}[1.7.16]
\label{IV.1.7.16}
In \sref[II]{II.5.1.2}, it suffices to assume that $X$ is quasi-compact and quasi-separated (the hypothesis being covered by \sref[II]{II.4.5.2} and \sref[II]{II.4.5.5}).
Similarly in \sref[II]{II.5.1.9}, where we use \sref[I]{I.9.6.5}.
\end{env}

\begin{env}[1.7.17]
\label{IV.1.7.17}
In \sref[II]{II.5.2.1}, it suffices always to assume $X$ quasi-compact and quasi-separated (the hypothesis being covered by \sref[II]{II.4.5.2}).
\end{env}

\begin{env}[1.7.18]
\label{IV.1.7.18}
In \sref[II]{II.5.2.2}, one must assume $f$ \emph{quasi-compact} and $X$ \emph{quasi-separated}; the current proof is in fact insufficient (with the hypotheses made), for from the fact that for every quasi-coherent $\sh{O}_X$-Module $\sh{F}$ we have $\mathrm{R}^1 f_*(\sh{F})=0$, it does not follow, necessarily, for an open affine $U$ of $Y$, that the same property holds for the ($\sh{O}_X|f^{-1}(U)$)-Modules quasi-coherent, if one does not know that such a Module is the restriction of a quasi-coherent $\sh{O}_X$-Module (a remark applying to conditions~b) and~c')); when $f$ is quasi-compact and $X$ quasi-separated, this extension is possible by virtue of \sref[I]{I.9.4.7}, modified above~\sref{IV.1.7.7}.
\end{env}

\begin{env}[1.7.19]
\label{IV.1.7.19}
In \sref[II]{II.5.3.2}, it suffices to assume $Y$ \emph{quasi-compact and quasi-separated} (using \sref[II]{II.4.4.6} and \sref[II]{II.4.4.7}).
...
For an open subset $U$ of $X$ to be \emph{retrocompact} in $X$ \sref[0]{0.9.1.1}, it is necessary and sufficient that for every \emph{open affine} subset $V$ of $X$, $U\cap V$ be quasi-compact.
When $X$ is \emph{quasi-separated} \sref{IV.1.2.1}, every quasi-compact open subset of $X$ is retrocompact in $X$ \sref{IV.1.2.7}, so every locally constructible part of $X$ is retrocompact in $X$ \sref[0]{0.9.1.13}; if \emph{in addition} $X$ is \emph{quasi-compact}, there is identity between constructible parts and locally constructible parts of $X$ \sref[0]{0.9.1.12}, and between constructible open parts and quasi-compact open parts \sref[0]{0.9.1.5}.
\end{env}
...
It therefore suffices to consider the case where $Z$ is constructible, and taking into account \sref[0]{0.9.1.3}, we are reduced to the case where $Z$ is open and \emph{retrocompact} in $Y$, in other words such that the canonical injection $Z\to Y$ is a quasi-compact morphism; it then suffices to see that $f^{-1}(Z)$ is \emph{retrocompact in} $X$, in other words such that the canonical injection $f^{-1}(Z)\to X$ is a quasi-compact morphism; but this follows from \sref{IV.1.1.2}, (iii)), since $f^{-1}(Z)=Z\times_Y X$ \sref[I]{I.4.4.1}.
\end{proof}
...
Since $Z\cap X_i$ is constructible in $X_i$ \sref[0]{0.9.1.8}, we see that we are reduced to proving the lemma when $X_i$ is \emph{affine} of ring $A$.
Since $Z$ is then a finite union of sets of the form $U\cap\complement V$, where $U$ and $V$ are
\oldpage[IV-1]{335}
quasi-compact open subsets \sref[0]{0.9.1.3}, one sees, by considering yet another suitable sum of preschemes, that one can reduce to the case where $Z=U\cap\complement V$; since moreover $U$ is a finite union of affine open subsets, one can even restrict ourselves to the case where $U$ is \emph{affine}.
Since $V$ is quasi-compact, it is a finite union of open subsets of the form $X_{f_i}$, where $f_i\in A$; let $\mathfrak{J}$ be the ideal of $A$ generated by the $f_i$, and let $Z'$ be the closed affine sub-scheme of $X$ that it defines (and which is by construction of finite presentation over $X$); one has by definition $V=\complement Z'$, so $U\cap\complement V=U\cap Z'$.
...
\begin{proof}
Let $y\in Y$ and $V$ an open affine neighbourhood of $y$; since $f$ is quasi-compact and quasi-separated, the same is true of its restriction $f^{-1}(V)\to V$, so $f^{-1}(V)$ is a prescheme quasi-compact and quasi-separated \sref{IV.1.2.3}; since $Z\cap f^{-1}(V)$ is constructible in $f^{-1}(V)$ \sref{IV.1.8.1}, we see that one can restrict ourselves to the case where $Y$ is affine and $Z$ constructible; $X$ is then quasi-compact and quasi-separated, so \sref{IV.1.8.3} there exists a morphism of finite presentation $g:X'\to X$ such that $Z=g(X')$; since $f\circ g$ is of finite presentation, we see that we are reduced to the case where $Z=X$; in other words, it will suffice to prove the
\end{proof}
...
\begin{proof}
Since $X$ is quasi-compact, it is a finite union of affine open subsets; one can therefore restrict ourselves to the case where $Y=\Spec(A)$, $X=\Spec(B)$, $B$ being an $A$-algebra of finite type \sref{IV.1.4.6}.
Or, one has the
\end{proof}

\begin{lemma}[1.8.4.2]
\label{IV.1.8.4.2}
Let $A_0$ be a ring, $(A_\lambda)_{\lambda\in\mathrm{L}}$ an inductive system of $A_0$-algebras, $A=\varinjlim A_\lambda$; if $B$ is an $A_0$-algebra of finite presentation, there exist a $\lambda$ and an $A_\lambda$-algebra of finite presentation $B_\lambda$ such that $B$ is isomorphic to $B_\lambda\otimes_{A_\lambda} A$.
\end{lemma}
...
If $\mathfrak{J}_\lambda$ is the ideal of $A_\lambda[\mathrm{T}_1,\ldots,\mathrm{T}_n]$ generated by the $\mathrm{F}_{j\lambda}$, $\mathfrak{J}$ is the image of $\mathfrak{J}_\lambda\otimes_{A_\lambda} A$ in $A[\mathrm{T}_1,\ldots,\mathrm{T}_n]=A_\lambda[\mathrm{T}_1,\ldots,\mathrm{T}_n]\otimes_{A_\lambda} A$; the ring $B_\lambda=A_\lambda[\mathrm{T}_1,\ldots,\mathrm{T}_n]/\mathfrak{J}_\lambda$ answers to the question.
\end{proof}

One will apply this lemma here by considering $A$ as inductive limit of its sub-$\bb{Z}$-algebras \emph{of finite type}.
...
Let us apply the criterion \sref[0]{0.9.2.3}: one must prove that for every irreducible closed part $T$ of $Y$, $T\cap f(X)=f(f^{-1}(T))$ is rare in $T$ or contains a non-empty open subset of $T$; taking $T$ as a sub-prescheme of $Y$ with underlying space $T$ \sref[I]{I.5.2.1} and considering its inverse image by $f$, one sees finally (taking into account \sref{IV.1.5.4}, (iii)) that one is reduced to proving that if one supposes $Y$ \emph{irreducible and} $f$ \emph{dominant}, $f(X)$ contains a non-empty open subset of $Y$.
...
It suffices to show that there exists $g\in A$ such that, for every $y\in\mathrm{D}(g)$ (that is to say such that $g\notin\mathfrak{i}_y$) the ideal $\mathfrak{i}_y$ is the intersection of $A$ and a prime ideal of $B$, for this will show that $\mathrm{D}(g)\subset f(X)$.
...
If $\sh{F}$ is an $\sh{O}_X$-Module quasi-coherent of finite type, and if $\sh{F}_\eta=\sh{F}\otimes_{\sh{O}_Y} k(\eta)$ is not zero, then there exists a neighbourhood $U'$ of $\eta$ in $Y$ such that $\sh{F}_y=\sh{F}\otimes_{\sh{O}_Y} k(y)$ is not zero for every $y\in U'$.
\end{corollary}

\begin{proof}
If $p_y$ is the canonical projection of the fibre $f^{-1}(y)=X\times_Y\Spec(k(y))$ in $X$, one has $\sh{F}_y=p_y^*(\sh{F})$, so $\mathrm{Supp}(\sh{F}_y)=p_y^{-1}(\mathrm{Supp}(\sh{F}))=\mathrm{Supp}(\sh{F})\cap f^{-1}(y)$ by virtue of \sref[I]{I.9.1.13.1} and \sref[I]{I.3.6.1}, since $\sh{F}$ is of finite type; moreover $\mathrm{Supp}(\sh{F})$ is closed in $X$ \sref[0]{0.5.2.2}, and if $Z$ is a closed sub-prescheme of $X$ having as underlying space $\mathrm{Supp}(\sh{F})$ \sref[I]{I.5.2.1}, and $j$ the canonical immersion $Z\to X$, $f\circ j$ is locally of finite type \sref{IV.1.3.4}, (vi)); this proves that the first assertion implies the second.
...
Moreover, if $Z$ is the reduced closed sub-prescheme of $X$ having $\{\overline{\xi}\}$ as underlying set \sref[I]{I.5.2.1}, one can, as above, replace $X$ by $Z$, in other words suppose $B$ \emph{integral} \sref[I]{I.5.1.4}.
...
Then the set of $s\in S$ such that $f_s$ is surjective (resp. radical) is locally constructible.
\end{proposition}
...
To prove (ii), note that, since $u$ factors as $X\xrightarrow{u_\alpha} X_\alpha\xrightarrow{} Y$, the first member is contained in the second.
...
\label{IV.1.9.3}
Let $X$ be a prescheme quasi-compact and quasi-separated, $(X_\alpha)_{\alpha\in\mathrm{I}}$ an open cover of $X$, and for every $\alpha\in\mathrm{I}$, let $E_\alpha=E\cap X_\alpha$.
The following conditions are equivalent:
\begin{enumerate}
  ...
  \item[b)] $E$ is intersection of constructible parts of $X$.
  \item[b$'$)] $F=X-E$ is union of constructible parts of $X$.
\end{enumerate}
\end{proposition}
...
Since $E$ is the union of the $E\cap X_\alpha$ for $\alpha\in\mathrm{J}$, one has $E=f(X')$ and it remains to see that $f$ is a quasi-compact morphism; but the hypothesis that $X$ is quasi-separated implies that $j_\alpha$ is quasi-compact \sref{IV.1.2.7}, so it is the same for $j_\alpha\circ f_\alpha$ \sref{IV.1.1.1} and \sref{IV.1.1.2} and finally of $f$ \sref{IV.1.1.6}.

\oldpage[IV-1]{338}

It remains therefore to prove the equivalence of $a'')$ and $b$).
Let us prove first that $a'')$ implies $b$): let us consider a finite part $\mathrm{J}$ of $\mathrm{I}$ such that the $X_\alpha$ for $\alpha\in\mathrm{J}$ cover $X$; it will suffice to show that, for every $\alpha\in\mathrm{J}$, $E_\alpha$ is intersection of constructible parts $\mathrm{F}_{\alpha\lambda}$ of $X_\alpha$ ($\lambda\in\mathrm{L}_\alpha$); indeed, every $\mathrm{F}_{\alpha\lambda}$ is also a constructible part of $X$ by virtue of \sref[0]{0.9.1.8}, (ii)), since the hypothesis that $X$ is quasi-separated implies that every $X_\alpha$ is retrocompact in $X$ \sref{IV.1.2.7}; $E$ being union of the $E_\alpha$ for $\alpha\in\mathrm{J}$, is intersection of the finite unions $\bigcup_{\alpha\in\mathrm{J}} \mathrm{F}_{\alpha,\lambda(\alpha)}$, for all choices of $\lambda(\alpha)\in\mathrm{L}_\alpha$; these finite unions being constructible parts of $X$, this establishes our assertion.
One can therefore restrict ourselves to the case
\oldpage[IV-1]{339}
where $X=\Spec(A)$ and where $E=f(X')$, where $X'=\Spec(A')$ is affine, $f:X'\to X$ a morphism corresponding to a homomorphism of rings $A\to A'$.
Let us note now that $A'$ is inductive limit of the family $(A_\lambda')$ of its sub-$A$-algebras \emph{of finite type}, ordered by inclusion; if $X_\lambda'=\Spec(A_\lambda')$, $f$ factors as $X'\to X_\lambda'\xrightarrow{f_\lambda} X$ and it follows from \sref{IV.1.9.2}, (ii)) that $f(X')=\bigcap_\lambda f_\lambda(X_\lambda')$; if one shows that $f_\lambda(X_\lambda')$ is intersection of constructible parts of $X$, it will be the same for $f(X')$.
One can therefore restrict ourselves to the case where $A'$ is an $A$-algebra \emph{of finite type}.
Or, one has the following lemma:
\end{proof}
...
If $E$ is intersection of a family $(G_\lambda)_{\lambda\in\mathrm{L}}$ of constructible parts of $X$, each $E_\alpha$ is intersection of the $G_\lambda\cap X_\alpha$, which are constructible in $X_\alpha$ \sref[0]{0.9.1.8}, (i)), and one is reduced to the case where $X=\Spec(A)$ is affine.
...
We say that a part $E$ of $X$ is \emph{pro-constructible} (resp. \emph{ind-constructible}) in $X$ if, for every $x\in X$, there exists an open neighbourhood $U$ of $x$ in $X$ such that $E\cap U$ is intersection (resp. union) of locally constructible parts of $U$.
\end{definition}

Taking into account \sref{IV.1.2.7} and \sref[0]{0.9.1.8} and \sref[0]{0.9.1.12}, the equivalent conditions of the proposition \sref{IV.1.9.3} are therefore expressed by saying that $E$ is \emph{pro-constructible} in $X$, the locally constructible parts of $X$ being identical to the constructible parts of $X$ under the hypotheses of \sref{IV.1.9.3}.
...
\begin{enumerate}
  ...
  \item[\rm(ii)] Every finite union and every finite intersection of pro-constructible parts of $X$ are pro-constructible.
  Every finite intersection and every finite union of ind-constructible parts of $X$ are ind-constructible.
...
If $X$ is quasi-compact and quasi-separated and $E$ is pro-constructible in $X$, there is a finite open cover $(U_i)$ of $X$ formed by affine open subsets such that $E\cap U_i$ is intersection of constructible parts $\mathrm{M}_{i\lambda}$ of $U_i$ ($\lambda\in\mathrm{L}_i$); the $\mathrm{M}_{i\lambda}$ are also constructible parts of $X$ by virtue of \sref[0]{0.9.1.8}, (ii)), since the hypothesis that $X$ is quasi-separated implies that every $X_\alpha$ is retrocompact in $X$ \sref{IV.1.2.7}; $E$ being union of the $E_\alpha$ for $\alpha\in\mathrm{J}$, is intersection of the finite unions $\bigcup \mathrm{M}_{i,\lambda(i)}$ (where $\lambda(i)\in\mathrm{L}_i$ for every $i$), which are constructible parts of $X$; this proves the second assertion of (iii).
The assertion (ix) follows from (iii) and \sref{IV.1.9.3}.
To prove the first assertion of (v), one can limit ourselves to the case where $X$ is affine, and prove then that $E$ is quasi-compact \sref[0]{0.9.1.1}; but since $X$ is then quasi-separated, there exists by virtue of (ix) a quasi-compact morphism $f:X'\to X$ such that $E=f(X')$; since $X'$ is quasi-compact, the same is true of $E$ by a continuous map.
...
Finally, if $X$ is a \emph{Noetherian} prescheme, therefore quasi-separated \sref{IV.1.2.8}, it follows from \sref{IV.1.9.5}, (iii)) and \sref[0]{0.9.1.7} that the ind-constructible parts of $X$ are the \emph{unions of locally closed parts} of $X$.
\end{env}
...
\begin{proof}
Note that the sets $F=X-E$ and $F\cap\mathrm{F}_\lambda$ are pro-constructible, so one is reduced to the
\end{proof}
...
\begin{proof}
Suppose $E$ ind-constructible, and let $Y$ be the intersection of $\overline{\{x\}}$ and an open affine subset of $X$ containing $x$; then there is thus a sub-prescheme of $X$ having $Y$ for underlying space \sref[I]{I.5.2.1}, and if $j:Y\to X$ is the canonical injection, $E\cap Y=j^{-1}(E)$ is ind-constructible in $Y$ \sref{IV.1.9.5}, (vi)).
As $Y$ is quasi-compact and separated, $E\cap Y$ is therefore a union of constructible parts of $Y$ \sref{IV.1.9.5}, (iii)); consequently, there are two open subsets $U$, $V$ in $Y$ such that $x\in U\cap\complement V\subset E\cap Y$ \sref[0]{0.9.1.3}.
...
\begin{proof}
We know that the condition is necessary \sref{IV.1.8.2} and \sref{IV.1.9.5}, (vi))); to show that it is sufficient, one is reduced to the case where $X$ is affine, by virtue of \sref{IV.1.9.5}, (iv)); in addition, by virtue of \sref{IV.1.9.11}, one can restrict to the case where $f^{-1}(E)$ is pro-constructible, \emph{or} the case where $f^{-1}(E)$ is ind-constructible.
...
\begin{env}[1.9.13]
\label{IV.1.9.13}
Let $X$ be a prescheme, $\mathscr{T}$ its topology; it follows from \sref{IV.1.9.5}, (i) and (ii)) that the ind-constructible (resp. pro-constructible) parts of $X$ are the open parts (resp. closed parts) for a topology on $X$, called the \emph{constructible topology} and which we will denote by $\mathscr{T}^{\mathrm{cons}}$; we will denote by $X^{\mathrm{cons}}$ the set $X$ equipped with the topology $\mathscr{T}^{\mathrm{cons}}$.
\end{env}
...
\begin{proof}
Indeed, (i) follows from the fact that every open set for $\mathscr{T}$ is open for $\mathscr{T}^{\mathrm{cons}}$ \sref{IV.1.9.6}, and (ii) is a restatement of \sref{IV.1.9.11}; (iii), (iv) and (v) are restatements respectively of \sref{IV.1.9.5}, (vi), (vii) and (viii).
...
(ii) Suppose $X$ quasi-separated, and let us show that $X^{\mathrm{cons}}$ is totally disconnected: indeed, if $x$, $y$ are two distinct points of $X$ and if for example $x\notin\overline{\{y\}}$, there exists an open affine subset $U$ of $X$ not containing $y$; as $X$ is quasi-separated, $U$ is retrocompact in $X$ \sref{IV.1.2.7}, hence $U$ and $X-U$ are locally constructible in $X$, and therefore open in $X^{\mathrm{cons}}$ by virtue of \sref{IV.1.9.11}, whence our assertion.
As for every open affine subset $U$ of $X$, the topology induced by $\mathscr{T}^{\mathrm{cons}}$ is that of $U^{\mathrm{cons}}$ by virtue of \sref{IV.1.9.14}, (vi)), it follows from the above that $X^{\mathrm{cons}}$ is locally compact, since $U$ is open in $X^{\mathrm{cons}}$ and $U^{\mathrm{cons}}$ is compact.
...
\]
$Z$ is quasi-compact \sref{IV.1.1.2}, (iii)) and as the canonical morphism $\rho:Z\to X$ is injective, it follows from (i) and from the fact that the map $\rho^{\mathrm{cons}}$ is continuous, that the topology induced by that of $X^{\mathrm{cons}}$ on $f^{-1}(y)$ makes $f^{-1}(y)$ a quasi-compact space.
...
\begin{env}[1.9.16]
\label{IV.1.9.16}
We will show later how one can, for every prescheme $X$, equip the space $X^{\mathrm{cons}}$ with a sheaf of rings which is in fact a prescheme whose local rings are all fields, identical to the residue fields at the points of $X$.
...
For $x$ to be interior to $E$, it is necessary and sufficient that every generisation \sref[0]{0.2.1.2} $x'$ of $x$ belong to $E$, in other words \sref[I]{I.2.4.2} that $\Spec(\mathcal{O}_{X,x})\subset E$.
\end{theorem}

\begin{proof}
The condition is evidently necessary, every neighbourhood of $x$ containing the generisations of $x$.
To show that it is sufficient, one can evidently restrict to the case where $X=\Spec(A)$ is affine, $x$ being a prime ideal $\mathfrak{p}$ of $A$.
...
As $A_\mathfrak{p}=\varinjlim A_t$ \sref[0]{0.1.4.5}, where $t$ runs through $A-\mathfrak{p}$, and the functor $\varinjlim$ commutes with the tensor product, by \sref{IV.1.9.1}, there exists $t\in A-\mathfrak{p}$ such that $A_t'=0$, and $\mathrm{D}(t)$ is then an open neighbourhood of $x$ contained in $E$.
\end{proof}
...
\begin{enumerate}
  ...
  \item[\rm b$'$)] The image by $f$ of $\Spec(\mathcal{O}_{X,x})$ is $\Spec(\mathcal{O}_{Y,y})$.
  \item[\rm c)] For every irreducible closed part $Y'$ of $Y$ containing $y$, there exists a component of $X'=f^{-1}(Y')$ containing $x$ and dominating $Y'$.
\end{enumerate}
...
\begin{proof}
By definition of a generisation, the image by $f$ of $\Spec(\mathcal{O}_{X,x})$ is always contained in $\Spec(\mathcal{O}_{Y,y})$, so b) and b$'$) are equivalent.
It is immediate that c) implies b), since if $y'$ is a generisation of $y$, $Y'=\overline{\{y'\}}$ is an irreducible closed part of $Y$ containing $y$; if $x'$ is the generic point of an irreducible component of $f^{-1}(Y')$ containing $x$ and dominating $Y'$, we have $f(x')=y'$ \sref[0]{0.2.1.5} and $x'$ is a generisation of $x$.
...
One has, by \sref{IV.1.9.2}, $f(\Spec(\mathcal{O}_{X,x}))=\bigcap_t f(\Spec(B_t))=\bigcap_t f(\mathrm{D}(t))$, and by hypothesis, $y$ is interior to $f(\mathrm{D}(t))$, hence $f(\mathrm{D}(t))$ contains $\Spec(\mathcal{O}_{Y,y})$; whence $\Spec(\mathcal{O}_{Y,y})\subset f(\Spec(\mathcal{O}_{X,x}))$, which establishes our assertion.
Finally, when $f$ is locally of finite presentation, b) implies a) by virtue of \sref{IV.1.10.2} applied to the restriction of $f$ to an arbitrary open neighbourhood $U$ of $x$.
\end{proof}
...
\begin{enumerate}
  ...
  \item[\rm b$'$)] For every $x\in X$, the image by $f$ of $\Spec(\mathcal{O}_{X,x})$ is $\Spec(\mathcal{O}_{Y,f(x)})$.
  \item[\rm c)] For every irreducible closed part $Y'$ of $Y$, every component of $f^{-1}(Y')$ dominates $Y'$.
\end{enumerate}
Then we have the implications $\mathrm{a)}\Rightarrow\mathrm{b)}\Leftrightarrow\mathrm{b')}\Leftrightarrow\mathrm{c)}$.
If in addition $f$ is locally of finite presentation, the four conditions are equivalent.
\end{corollary}
