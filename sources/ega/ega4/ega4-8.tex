...
If one sets $w_\lambda=(\rho_\lambda,\omega_\lambda)$, the $\rho_\lambda$ form a projective system of continuous maps and $\rho=\varprojlim\rho_\lambda$ a continuous map $\mathrm{Y}\to\mathrm{S}$, by virtue of \sref{IV.8.2.9} and \sref{IV.8.2.12}.
The unique homomorphism $\omega^\sharp:\rho^*(\sh{O}_\mathrm{S})\to\sh{O}_\mathrm{Y}$ such that $\omega_\lambda^\sharp=\omega^\sharp\circ\rho^*(\theta_\lambda^*)$ for every $\lambda$, and there is thus a unique morphism of ringed spaces $w:\mathrm{Y}\to\mathrm{S}$ satisfying these relations.
\end{remark}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item If, for some index $\lambda$, the morphisms $u_{\lambda\mu}:\mathrm{S}_\mu\to\mathrm{S}_\lambda$ are flat (resp.\ faithfully flat), then for every $\mu\geqslant\lambda$, the morphism $u_\lambda:\mathrm{S}\to\mathrm{S}_\lambda$ is flat (resp.\ faithfully flat).
...
One notes that we can arrange for $u_\lambda$ to be open without the $u_{\lambda\mu}$ being open and without $u_\lambda$ being surjective either.
...
\begin{align}
  &\varinjlim\mathfrak{C}(\mathrm{S}_\lambda)\to\mathfrak{C}(\mathrm{S}) \tag{8.3.9.2}\\
  &\varinjlim\mathfrak{Dc}(\mathrm{S}_\lambda)\to\mathfrak{Dc}(\mathrm{S}) \tag{8.3.9.3}\\
  &\varinjlim\mathfrak{Fc}(\mathrm{S}_\lambda)\to\mathfrak{Fc}(\mathrm{S}) \tag{8.3.9.4}\\
...
One easily verifies that the inductive limit of this system of rings is the canonical homomorphism corresponding to the second projection $\mathrm{A}\times\mathrm{K}\to\mathrm{K}$.
...
Then the assertion is equivalent to the lemma \sref{IV.8.5.7.1}.
...
exact, it is necessary and sufficient that there exist $\mu\geqslant\lambda$ such that the sequence $\sh{F}_\mu\xrightarrow{f_\mu}\sh{G}_\mu\xrightarrow{g_\mu}\sh{H}_\mu$ be so (in which case the same holds for $\nu\geqslant\mu$).
  \item Let $f_\lambda:\sh{F}_\lambda\to\sh{G}_\lambda$ be an $\sh{O}_{\mathrm{S}_\lambda}$-Module homomorphism quasi-coherent, and suppose that $\operatorname{Im} f_\lambda$ is of finite type.
...
\end{enumerate}
\end{corollary}\subsection{Sub-preschemes of finite presentation of a projective limit of preschemes}
...
For $Z=u_\lambda^{-1}(Z_\lambda)$ to be a sub-prescheme induced on an open (resp.\ a closed sub-prescheme) of $S$, it is necessary and sufficient that there exist $\mu\geqslant\lambda$ such that $Z_\mu=u_{\mu\lambda}^{-1}(Z_\lambda)$ be induced on an open (resp.\ a closed sub-prescheme) of $S_\mu$.
\end{corollary}
...
For $f:X\to Y$ to be an isomorphism, it is necessary and sufficient that there exist $\lambda\geqslant\alpha$ such that $f_\lambda:X_\lambda\to Y_\lambda$ be an isomorphism.
\end{corollary}

\begin{proof}
The condition is evidently sufficient; conversely, note that since the question is local on $S_0$ (since $L$ is filtered), one can always suppose $S_0$ affine, hence quasi-separated.
There is by hypothesis an $S$-morphism $g:Y\to X$ such that $g\circ f=1_X$ and $f\circ g=1_Y$.
On the other hand, it follows also from \sref{IV.8.8.2},~(i) and from the relations $g\circ f=1_X$ and $f\circ g=1_Y$ that there exists $\nu\geqslant\mu$ such that $g_\nu\circ f_\nu=1_{X_\nu}$ and $f_\nu\circ g_\nu=1_{Y_\nu}$, since $X_\alpha$ and $Y_\alpha$ are of finite type over $S_\alpha$ and quasi-compact.
This means that $f_\nu:X_\nu\to Y_\nu$ is an isomorphism, whence the corollary.
\end{proof}

\begin{corollary}[8.8.5]
\label{IV.8.8.5}
Suppose $S_0$ quasi-compact and quasi-separated, $X_\alpha$ and $Y_\alpha$ of finite presentation over $S_\alpha$.
For every $S$-isomorphism $f:X\xrightarrow{\sim}Y$, it is necessary and sufficient that there exist $\lambda\geqslant\alpha$ such that $X_\lambda$ and $Y_\lambda$ be $S_\lambda$-isomorphic, and an $S_\mu$-isomorphism $f_\mu:X_\mu\to Y_\mu$ such that $f=f_\mu\times 1_S$.
\end{corollary}

\begin{proof}
The condition is evidently sufficient; conversely, note that since the question is local on $S_0$ (since $L$ is filtered), one can always suppose $S_0$ affine, hence quasi-separated.
There is by hypothesis an $S$-morphism $g:Y\to X$ such that $g\circ f=1_X$ and $f\circ g=1_Y$.
As $X_\alpha$ is of finite presentation over $S_\alpha$, and $Y_\alpha$ quasi-compact and quasi-separated, it results also from \sref{IV.8.8.2},~(i) that $f$ is of the form $f_\mu\times 1_S$ for some $\mu\geqslant\alpha$ and an $S_\mu$-morphism $f_\mu:X_\mu\to Y_\mu$.
Similarly for $g$.
Applying \sref{IV.8.8.2.4} one obtains that there exists $\nu\geqslant\mu$ such that $g_\nu\circ f_\nu=1_{X_\nu}$ and $f_\nu\circ g_\nu=1_{Y_\nu}$, and the conclusion follows.
\end{proof}

\begin{proof}[Proof of \sref{IV.8.8.2}, (ii)]
Consider first the case where $S_0=\operatorname{Spec}(A_0)$ and $X=\operatorname{Spec}(B)$ are affine; then the $S_\lambda=\operatorname{Spec}(A_\lambda)$ and $S=\operatorname{Spec}(A)$ are also affine, with $A=\varinjlim A_\lambda$.
As $X$ is of finite presentation over $S$, $B$ is an $A$-algebra of finite presentation, hence by \sref{IV.8.8.2.3} applied to $G=B$ and $F_\lambda=A_\lambda$, there exist $\lambda$ and an $A_\lambda$-algebra $B_\lambda$ of finite presentation such that $B\simeq B_\lambda\otimes_{A_\lambda}A$.
Setting $X_\lambda=\operatorname{Spec}(B_\lambda)$, we obtain an $S_\lambda$-prescheme of finite presentation such that $X\simeq X_\lambda\times_{S_\lambda}S$.

To prove (ii) in the general case, note that $S$ is quasi-compact and quasi-separated, and $X$ is of finite presentation over $S$.
As $X$ is quasi-compact, it is a finite union of open affines, and by the affine case there exist $\lambda$ and affine $S_\lambda$-preschemes of finite presentation that descend each piece.
By \sref{IV.8.8.2},~(i) and the fact that $L$ is filtered, we can glue these pieces for a large enough $\lambda$ to obtain $X_\lambda$ of finite presentation over $S_\lambda$ with $X\simeq X_\lambda\times_{S_\lambda}S$.
\end{proof}
...
\begin{proof}
Indeed, by \sref{IV.8.9.1} there is a sub-$\mathbf{Z}$-algebra of finite type $A_0$ of $A$, a $B_0$-algebra of finite type and a $B_0$-module of finite type $M_0$ such that $B$ is $A$-isomorphic to $B_0\otimes_{A_0}A$ and that $M$ is $B$-isomorphic to $M_0\otimes_{A_0}A$; by \sref{IV.6.9.2}, there exists $f_0\neq 0$ in $A_0$ such that $(M_0)_{f_0}$ is an $(A_0)_{f_0}$-module free.
As $M_{f_0}=(M_0)_{f_0}\otimes_{A_0}A$ and $A_{f_0}=(A_0)_{f_0}\otimes_{A_0}A$, $M_{f_0}$ is an $A_{f_0}$-module free.
\end{proof}
\end{proof}
...
Then there exists a partition $(S_i)_{1\leqslant i\leqslant n}$ of $S$ into a finite number of locally closed sets in $S$ such that, for $1\leqslant i\leqslant n$, there exists a sub-prescheme $S'_i$ of $S$, having $S_i$ as underlying space, of finite presentation over $S_i$, such that if one sets $X_i=X\times_S S'_i$, the $\sh{O}_{X_i}$-Module $\sh{F}_i=\sh{F}\otimes_{e_S}\sh{O}_{S'_i}$ be flat over $S'_i$.
\end{corollary}

\begin{proof}
Consider a finite covering $(U_j)_{1\leqslant j\leqslant m}$ of $S$ by open affines and define by induction $T_1=U_1$, $T_j=U_j-\bigcup_{h<j}(U_j\cap U_h)$ for $j\geqslant 2$; each $T_j$ is closed in the open affine $U_j$, and the $T_j$ are pairwise disjoint; furthermore the $T_j$ are also quasi-compact since $S$ is quasi-separated, and consequently (since $S$ is also quasi-compact), they are \emph{constructible} in $S$ \sref[I]{I.1.8.1}, and hence also each of the $T_j$.
It will therefore evidently suffice to prove the corollary for a sub-prescheme $T'_j$ of $S$ having $T_j$ as underlying space and for the morphism and the Module deduced
\oldpage[IV-3]{36}
respectively from $u$ and $\sh{F}$ by the base change $T'_j\to S$.
Note for this that if on $U_j$ one takes the prescheme structure induced by that of $S$, the open immersion $U_j\to S$ is quasi-separated since $S$ is quasi-separated \sref[I]{I.1.2.7}, hence it is of finite presentation.
Then \sref{IV.8.9.1}, with the same notations, there exist $\lambda$, a morphism of finite type $u_\lambda:X_\lambda\to S_\lambda$ and an $\sh{O}_{X_\lambda}$-Module coherent $\sh{F}_\lambda$ such that $X$ is isomorphic to $X_\lambda\times_{S_\lambda}S$, $\sh{F}$ to $\sh{F}_\lambda\otimes_{e_{S_\lambda}}\sh{O}_X$.
One can then apply to $S_\lambda$, $X_\lambda$, and $\sh{F}_\lambda$ the result of \sref{IV.6.9.3} and it follows that there is a partition of $S_\lambda$ into sub-preschemes $S'_{\lambda i}$ of $S_\lambda$, whose underlying sets form a partition of $S_\lambda$, and which are such that $\sh{F}_{M}=\sh{F}_\lambda\otimes_{e_{S_\lambda}}\sh{O}_{S'_{\lambda M}}$ be flat over $S'_{\lambda M}$.
The $S'_i=S'_{\lambda i}\times_{S_\lambda}S$ are then sub-preschemes of $S$ satisfying the condition by virtue of \sref{IV.2.1.4}.
\end{proof}
...
If one sets $Z_\lambda=w_{\alpha\lambda}^{-1}(Z_\alpha)$ for $\lambda\geqslant\alpha$ and $Z=w_\alpha^{-1}(Z_\alpha)$, on has $Z_\lambda=f_\lambda(X_\lambda)$ and $Z=f(X)$ \sref[I]{I.3.4.8}.
...
\label{IV.8.10.4}
Suppose that there exists $\alpha$ such that $Y_\alpha$ is quasi-compact of finite type and $f_\alpha$ of type fini and quasi-separated.
...
\begin{proof}
The case where $\mathbf{P}$ is one of the properties (i) or (v) was not inserted in the statement for the record; in these cases, the theorem follows from what has been proved respectively in \sref{IV.8.8.4} and \sref{IV.8.10.4}.
Furthermore, taking into account \sref[I]{I.5.4.1} and \sref{IV.5.3.4}, (v) also results from (iv).
...
\label{IV.8.10.5.1}
(Chow's lemma for morphisms of finite presentation). --- Let $A$ be a ring, $X$, $Y$ two $A$-preschemes of finite presentation, $f:X\to Y$
\oldpage[IV-3]{39}
\end{proposition}
...
\begin{env}[8.11.4]
\label{IV.8.11.4}
The hypotheses and the conclusions of \sref{IV.8.11.1} and \sref{IV.8.11.2} are all local on $Y$ \sref[I]{I.1.6.1},~\sref[I]{I.1.2.6},~\sref[II]{II.5.1.1},~\sref[II]{II.5.4.1} and \sref[II]{II.6.2.2}), so one can suppose $Y=\operatorname{Spec}(A)$ affine.
...
Let $\mathfrak{m}$ be the maximal ideal of $A$, $\hat{A}$ the completion of $A$ for the $\mathfrak{m}$-preadic topology; one knows that $\hat{A}$ is a local Noetherian ring and that the morphism $\operatorname{Spec}(\hat{A})\to\operatorname{Spec}(A)$ is faithfully flat and quasi-compact $(\mathbf{0}_I,~7.3.5)$; applying \sref{IV.2.7.1},~(i),~(vii),~(xiv),~(xv) and (xvi)), one sees furthermore that one can restrict to the case where $A$ is \emph{complete}.
...
But as by hypothesis $\sh{B}'$ is an $\sh{O}_Y$-Algebra finite, it identifies by definition of $\sh{B}$ with one of the sub-$\sh{O}_Y$-Algebras $\sh{C}_\lambda$, which proves \emph{c)}.
...
\begin{proof}
(i) It is clear that $f_{(Y')}$ is of type finite \sref{IV.1.5.4}); furthermore, a factorisation $X\xrightarrow{g}Z\xrightarrow{u}Y$ where $g$ is an immersion and $u$ is finite, gives a factorisation $X'\xrightarrow{g'}Z'\xrightarrow{u'}Y'$ of $f_{(Y')}$, where $Z'=Z_{(Y')}$, $g'=g_{(Y')}$ and $u'=u_{(Y')}$; $g'$ is an immersion \sref[I]{I.4.3.2}) and $u'$ is finite \sref[II]{II.6.1.5}); so $f_{(Y')}$ is pseudo-finite.

(ii) The condition is necessary by virtue of (i), the $U_\lambda$ being quasi-separated since $Y$ is.
To see that it is sufficient, let us observe (with the notations of \sref{IV.8.12.3}) that if one sets $X_\lambda=f^{-1}(U_\lambda)$, on has $\sh{B}|U_\lambda=(f_\lambda)_*(\sh{O}_{X_\lambda})$, $\sh{C}|U_\lambda$ is the integral closure of $\sh{O}_{U_\lambda}$ in $\sh{B}|U_\lambda$, and consequently, if $h:Z\to Y$ is the canonical morphism, $Z_\lambda=\operatorname{Spec}(\sh{C}_\lambda)$ identifies with $h^{-1}(U_\lambda)=\operatorname{Spec}(\sh{C}|U_\lambda)$, the $Y$-morphism $g_\lambda:X_\lambda\to Z_\lambda=h^{-1}(U_\lambda)$ being the $U_\lambda$-morphism corresponding to $f_\lambda$.
...
It follows from this that $B'$ is the direct composite of the two $A'$-algebras $\Gamma(X'_1,\sh{O}_{X'_1})=B'_1$ and $\Gamma(X'_2,\sh{O}_{X'_2})=B'_2$; one concludes immediately that the integral closure $C'$ of $A'$ in $B'$ is the direct composite of the integral closures $C'_1$, $C'_2$ of $A'$ in $B'_1$, $B'_2$ respectively, whence $Z'=Z'_1\sqcup Z'_2$, where $Z'_i=\operatorname{Spec}(C'_i)$ ($i=1,2$); and the canonical morphism $g':X'\to Z'$ is such that $g'|X'_i$ is the canonical morphism $g'_i:X'_i\to Z'_i$ ($i=1,2$).
...
\label{IV.8.12.10}
Let $Y$ be a prescheme integral and normal, $X$ a prescheme integral, $f:X\to Y$ a birational morphism and locally quasi-finite $(\mathbf{Err}_{\text{III}},~20)$.
...
To prove the first assertion, one can suppose $X$ and $Y$ affine and $f$ quasi-finite; consider the factorisation $f=u\circ f'$ of \sref{IV.8.12.8}), which allows one to identify $X$ by $f'$ with a sub-prescheme induced on an open subset of $Y'$.
...
\begin{proof}
Setting $\sh{A}=u_*(\sh{O}_{Y'})$, so that $\sh{A}$ is an $\sh{O}_Y$-Algebra finite, $Y'$ identifying with $\operatorname{Spec}(\sh{A})$ \sref[II]{II.1.3.6}).
...
\label{IV.8.12.11}
Let $Y$ be a prescheme integral and normal, $X$ a prescheme integral and normal, $f:X\to Y$ a dominant and locally quasi-finite morphism.
...
Let $q_\lambda:C\to C_\lambda$ be the canonical homomorphism, and set $t_i=p_\lambda(q_\lambda(T_i))$, in other words $u(t_i)=q_\lambda(T_i)$.
There exists therefore a $\mu\geqslant\lambda$ such that the $n$ elements $u(t_i)-q_\lambda(T_i)$ belong to $\mathfrak{J}_\mu/\mathfrak{J}_\lambda$ ($1\leqslant i\leqslant n$); if $p_{\mu\lambda}:C_\lambda\to C_\mu$ is the canonical homomorphism, replacing $\lambda$ by $\mu$ and $u$ by $p_{\mu\lambda}\circ u$, one can therefore suppose that $u(t_i)=q_\lambda(T_i)$ for all $i$, and if $r=p_\lambda\circ q_\lambda$ is the canonical homomorphism $C\to C/\mathfrak{J}=B$, one can therefore write $u(r(T_i))=q_\lambda(T_i)$ for all $i$, whence $q_\lambda=u\circ r$.
But this implies necessarily that $\mathfrak{J}=\mathfrak{J}_\lambda$, for if $z\in\mathfrak{J}$, one has $r(z)=0$; hence on has $B=C_\lambda$.
...
\[
Z\xrightarrow{v_\lambda}Z_\lambda\xrightarrow{v_\lambda}V,\qquad Z\xrightarrow{u_\lambda}Z_\lambda\xrightarrow{v'_\lambda}V
\]
