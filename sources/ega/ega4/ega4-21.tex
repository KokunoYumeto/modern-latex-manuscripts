...
We retain the notation of \sref{IV.20.1}:
$\sh{S}(\sh{O}_X)$ denotes the subsheaf of $\sh{O}_X$ whose sections over an open $U$ are the \emph{regular} elements of $\Gamma(U,\sh{O}_X)$;
$\sh{M}_X=\sh{O}_X[\sh{S}(\sh{O}_X)^{-1}]$ the sheaf of germs of meromorphic functions on $X$;
$\sh{M}_X^*$ the sheaf of \emph{invertible} elements of $\sh{M}_X$, which is also equal to the sheaf $\sh{S}(\sh{M}_X)$ of regular elements of $\sh{M}_X$ \sref{IV.20.1.8};
$\sh{O}_X^*$ the sheaf of invertible elements of $\sh{O}_X$, which is a \emph{subsheaf of groups} of $\sh{M}_X^*$.
\end{env}

\begin{env}[21.1.2]
\label{IV.21.1.2}
We call the quotient sheaf $\sh{M}_X^*/\sh{O}_X^*$ the \emph{sheaf of germs of divisors on $X$} and denote it by $\sh{D}iv_X$;
it is a sheaf of \emph{commutative groups} (written additively);
the sections of $\sh{D}iv_X$ over $X$ are called the \emph{divisors on $X$} and form a commutative group denoted $\operatorname{Div}(X)$.
For every regular meromorphic function $f\in\Gamma(X,\sh{M}_X^*)$, the image of $f$ in $\Gamma(X,\sh{D}iv_X)=\operatorname{Div}(X)$ is a divisor, called the \emph{divisor of $f$} and denoted $\operatorname{div}(f)$;
\oldpage[IV-4]{255}
by definition, for every open $U$ of $X$ and every $x\in U$, we have $\operatorname{div}(f)|U=0$ if and only if $f|U\in\Gamma(U,\sh{O}_X^*)$;
by the definition of quotient sheaves, for every $x\in X$ there exists an open neighbourhood $U$ of $x$ such that $\operatorname{div}(f)|U=\operatorname{div}_U(g)$ for some $g\in\Gamma(U,\sh{M}_X^*)$, and more precisely $g=f|U$;
conversely, every divisor $D$ on $X$ is \emph{locally} of the form $\operatorname{div}(g)$ for a regular meromorphic function $g$.
\end{env}

\begin{env}[21.1.2.1]
\label{IV.21.1.2.1}
For every point $x\in X$, the stalk $(\sh{D}iv_X)_x$ of the sheaf $\sh{D}iv_X$ at $x$ is the quotient group $\sh{M}_{X,x}^*/\sh{O}_{X,x}^*$;
the image of a divisor $D$ on $X$ in $(\sh{D}iv_X)_x$ is denoted $D_x$;
to say that $D_x=0$ means that there exists an open neighbourhood $U$ of $x$ such that $D|U=0$.
\end{env}
...
By virtue of \sref{IV.21.2.3},~(i), we will identify invertible $\sh{O}_X$-Modules and their inverses $\sh{J}$, $\sh{J}^{-1}$ considered as sub-$\sh{O}_X$-Modules of $\sh{M}_X$.
...
\begin{env}[21.2.5.4]
\label{IV.21.2.5.4}
We deduce that $f\in\Gamma(X,\sh{J}_X(D))$, for every divisor $D\in\operatorname{Div}(X)$, we have the relation
\[
...
It follows from \sref{IV.21.2.10} that if $s$, $s'$ are two regular meromorphic sections of $\sh{L}$ over $X$, the relation $\operatorname{cl}(\sh{L},s)=\operatorname{cl}(\sh{L},s')$ is equivalent to the existence of a section $t\in\Gamma(X,\sh{O}_X^*)$ such that $s'=ts$.
...
on sees immediately that these isomorphisms are compatible with the restriction operations, and thus define an isomorphism $v:\sh{L}\to\sh{O}_X(D)$ such that $\overline{v}(s)=s_D$.
\end{proof}
...
we denote this set by $\operatorname{Pic}(X)$, and for every invertible $\sh{O}_X$-Module $\sh{L}$, we denote by $\operatorname{cl}(\sh{L})$ the class of equivalence of $\sh{O}_X$-Modules isomorphic to $\sh{L}$;
...
In case $b)$, we take for each of the maximal points $x_\lambda$ of $X$ an open $U_\lambda$ containing $x_\lambda$, distinct from $\overline{\{x_\lambda\}}$ and such that $\sh{L}|U_\lambda$ is isomorphic to $\sh{O}_{U_\lambda}$;
...
\label{IV.21.3.5}
If $X$ is a Noetherian prescheme, such that there exists an invertible $\sh{O}_X$-Module ample (\textbf{II}, 4.5.3) (for example a quasi-projective prescheme over a Noetherian ring spectre (\textbf{II}, 5.3.1 and 4.6.6)), the canonical homomorphism $l:\operatorname{Div}(X)\to\operatorname{Pic}(X)$ is surjective.
\end{corollary}

\begin{proof}
Indeed (\textbf{II}, 4.5.4), there exists then a neighbourhood of $\operatorname{Ass}(\sh{O}_X)$ which is an affine open of the finite set $\operatorname{Ass}(\sh{O}_X)$.
\end{proof}
...
$\partial(D)$ is the cohomology class of the cocycle $(c_{\alpha\beta})$, where $c_{\alpha\beta}=g_\alpha|_\beta g_\beta|_\alpha^{-1}$, $g_{\alpha|\beta}$ denoting the restriction of $g_\alpha$ to $U_\alpha\cap U_\beta$.
...
to say that the inverse image $s_0f$ of $s$ by $f$ exists \sref{IV.20.1.11} and is a regular meromorphic section of $\sh{L}'$ over $X'$ amounts to saying that the inverse images by $f$ of $s$ and $s^{\otimes(-1)}$ exist, in other words that $s\in\Gamma(X,\sh{M}_f(\sh{L}))$ and $s^{\otimes(-1)}\in\Gamma(X,\sh{M}_f(\sh{L}^{-1}))$;
the remark made above shows that if $(\sh{L}_1,s_1)$ is a pair equivalent to $(\sh{L},s)$ in the sense of \sref{IV.21.2.10}, the inverse image $s_1\circ f$ exists and is a regular meromorphic section of $\sh{L}_1'=f^*(\sh{L}_1)$ over $X'$, and the pairs $(\sh{L}',s_0f)$ and $(\sh{L}_1',s_1of)$ are equivalent.
...
\begin{env}[21.4.3]
\label{IV.21.4.3}
The definition \sref{IV.21.4.2} also shows immediately that, for $f^*(D)$ to exist, it suffices that for every open $U$ of $X$ such that $D|U=\operatorname{div}(g)$ (restriction of $f$ to $f^{-1}(U)\to U$), where $g$ is a regular meromorphic function on $X$, to say that the inverse image of $s_D$ by $f$ exists and is a regular meromorphic section on $X'$.
...
Then if $f=(\psi,0)$, the canonical homomorphism \sref{IV.20.1.11} $\psi^*(\sh{M}_f)\to\sh{M}_{X'}$ gives by restriction a homomorphism of sheaves of groups $\psi^*(\sh{M}_f^*)\to\sh{M}_{X'}^*$.
...
\begin{enumerate}[label=(\roman*)]
  \item $f$ is flat.
  \item $X$ and $X'$ are locally Noetherian preschemes and $f(\operatorname{Ass}(\sh{O}_{X'}))\subset\operatorname{Ass}(\sh{O}_X)$ and $g'(\operatorname{Ass}(\sh{O}_{X'}))\subset\operatorname{Ass}(\sh{O}_X)$.
...
By hypothesis we have $u_y\sh{O}_{X,x}\subset v_y\sh{O}_{X,x}$, hence, for every $x\in X$, setting $y=f(x)$, we have $u_y\sh{O}_{Y,y}\subset v_y\sh{O}_{Y,y}$;
we conclude that $u_y\sh{O}_{Y,y}\subset v_y\sh{O}_{Y,y}$ by virtue of the hypothesis that $\sh{O}_{X,x}$ is a faithfully flat $\sh{O}_{Y,y}$-module, and since $f$ is surjective, we have $u\sh{O}_Y\subset v\sh{O}_Y$, whence $D\geq 0$.
\end{proof}
...
\begin{enumerate}[label=\Roman*)]
  \item $f$ is a finite morphism \emph{locally free}, in other words \sref{IV.18.2.7} the $\sh{O}_X$-Module $f_*(\sh{O}_{X'})$ is \emph{locally free}.
...
This being so, let $u'$ be a section of $\sh{L}'$ over $X'$;
...
the element $\mathrm{N}(t')/\mathrm{N}(s')$ (where $\mathrm{N}(t')$ is the section of $\sh{L}$ defined in (\textbf{II}, 6.5.3)) is then a section of $\sh{L}$ over $U$, and from what precedes and the multiplicative properties of the norm (\textbf{II}, 6.5.3.1) this section does not depend on the choice of $u'|f^{-1}(U)$ but only on its representation in the form $t'/s'$;
...
\begin{proof}
Taking into account the definition \sref{IV.21.5.5}, it suffices to prove the following:
\end{proof}
...
\label{IV.21.5.7.3}
  ...
  \tag{21.5.7.3}
\]
in the category of invertible $\sh{O}_{X''}$-Modules.
\end{lemma}

\begin{proof}
Indeed, taking into account the definition of the norm of a section $\sh{L}''$ (\textbf{II}, 6.5.3) and of the definition \sref{IV.21.5.5}, we will obtain \sref{IV.21.5.7.3}, and it suffices to prove that for every section $s$ of $f_*''(\sh{O}_{X''})=f_*(f_*'(\sh{O}_{X''}))$ over an open $U$ of $X$, we have
\[
\label{IV.21.5.7.4}
...
\]
\oldpage[IV-4]{270}
which follows from the transitivity of the norm (Bourbaki, \emph{Alg.}, chap.~VIII, \textsection~12, n\textsuperscript{o}~2, prop.~7) when $f$ satisfies hypothesis (I) and $A$ is Noetherian and reduced, one can suppose that $A'$ is an $A$-module free of rank $m$, and then $A''$ is an $A$-module projective of rank $mn$, and restricting $X$ to a suitable open, one can suppose that $A''$ is an $A$-module free of rank $mn$;
the formula \sref{IV.21.5.7.4} follows then again from the transitivity of the norms.
When $f$ satisfies hypothesis (II), $A$ is Noetherian and reduced, its total ring of fractions $A'\otimes_A\mathrm{R}$ is an $\mathrm{R}$-module projective of rank $m$, hence $A''\otimes_A\mathrm{R}=A''\otimes_{A'}\mathrm{R}$ is an $\mathrm{R}$-module projective of rank $mn$;
the proposition also follows then from the transitivity of the norms.
\end{proof}
...
\]
in the category of invertible $\sh{O}_{X'}$-Modules;
moreover (\textbf{II}, 6.5.4), if $s'$ is the corresponding section of $\sh{O}_{X'}$ over $f^{-1}(U)$, $s''$ the corresponding section of $\sh{O}_{Y'}$ over $g'^{-1}(f^{-1}(U))$ (where $U$ is open of $X$), $\mathrm{N}_{Y'/Y}(s'')$ is a section of $\sh{O}_Y$ over $U$.
The formula \sref{IV.21.5.8.1} will thus follow from the definitions, provided one proves that $g^*(D')$ and $g'^*(D')$ are both defined, for arbitrary divisors $D'$ on $X'$ and $D$ on $X$.
As far as $D$ is concerned, this results from the hypotheses made and from \sref{IV.21.4.5}.
As far as $D'$ is concerned, in case (i) $g'$ is flat, hence the conclusion by \sref{IV.21.4.5};
in what concerns $D'$, in case (ii), the conclusion results again from \sref{IV.21.4.5} by the hypotheses;
in case (iii), $g'$ is flat, hence the conclusion again by \sref{IV.21.4.5}.
\end{proof}
...
\begin{env}[21.6.1]
\label{IV.21.6.1}
We call \emph{multiplicity of $\mathrm{Z}$ at the point $x$} and denote by $\operatorname{mult}_x(\mathrm{Z})$ the integer $n_x$ (positive or negative).
To say that $\mathrm{Z}\geq 0$ means that $\operatorname{mult}_x(\mathrm{Z})\geq 0$ for all $x\in\mathrm{X}$.
We call the \emph{support} of $\mathrm{Z}$ and denote by $\Supp(\mathrm{Z})$ the union of the $\overline{\{x\}}$ such that $\operatorname{mult}_x(\mathrm{Z})\neq 0$;
by definition of $\mathfrak{Z}(\mathrm{X})$, this is a closed part of $\mathrm{X}$, as the union of a locally finite family of closed parts.
We call the \emph{dimension} (resp.\ \emph{codimension}) of $\mathrm{Z}$ and denote by $\dim(\mathrm{Z})$ (resp.\ $\codim(\mathrm{Z})$) the dimension (resp.\ codimension in $\mathrm{X}$) of $\Supp(\mathrm{Z})$.
\end{env}
...
in the latter case, we consider $\mathfrak{Z}(\mathrm{X})$ as \emph{graded} by the $\mathrm{Z}^p(\mathrm{X})$ for $p\in\bb{N}$.
\end{env}
...
It is clear that $\mathrm{Z}\hookrightarrow\mathrm{Z}|\mathrm{U}$ is a homomorphism of ordered groups from $\mathfrak{Z}(\mathrm{X})$ to $\mathfrak{Z}(\mathrm{U})$ (resp.\ from $\mathfrak{Z}^p(\mathrm{X})$ to $\mathfrak{Z}^p(\mathrm{U})$), and that if $\mathrm{V}$ is an open subset contained in $\mathrm{U}$, we have $\mathrm{Z}|\mathrm{V}=(\mathrm{Z}|\mathrm{U})|\mathrm{V}$;
the map $\mathrm{U}\hookrightarrow\mathfrak{Z}^+(\mathrm{U})$ (resp.\ $\mathrm{U}\hookrightarrow\mathfrak{Z}^{p+}(\mathrm{U})$) is therefore a \emph{presheaf} on $\mathrm{X}$ of ordered commutative groups.
...
\oldpage[IV-4]{272}
of monoids $\mathrm{U}\hookrightarrow\mathfrak{Z}^+(\mathrm{U})$ (resp.\ $\mathrm{U}\hookrightarrow\mathfrak{Z}^{p+}(\mathrm{U})$) of $\mathscr{Z}_\mathrm{X}$ (resp.\ $\mathscr{Z}_\mathrm{X}^p$).
...
\]
are called respectively the \emph{cycle of zeros} (or \emph{polar cycle}) of $f$, and we evidently have $\operatorname{cyc}(\operatorname{div}(f))=\mathrm{Z}^+(f)-\mathrm{Z}^-(f)$.
...
On the other hand, the regular meromorphic functions on $\mathrm{X}$ are identified with the nonzero elements of the field of fractions $\mathrm{K}$ of $\mathrm{A}$, the map $f\mapsto\operatorname{cyc}(\operatorname{div}(f))$ in $\mathrm{M}(\mathrm{X})$ is identified in $\mathfrak{Z}^1(\mathrm{X})$ with the map denoted $f\mapsto\operatorname{div}(f)$ in Bourbaki (\emph{loc.\ cit.}, \textsection~1, n\textsuperscript{o}~1);
...
\begin{env}[21.6.9.2]
\label{IV.21.6.9.2}
When $\mathrm{A}$ is a Noetherian \emph{factorial} ring, it is clear that $\mathrm{X}=\Spec(\mathrm{A})$ is locally factorial (Bourbaki, \emph{Alg.\ comm.}, chap.~VII, \textsection~3, n\textsuperscript{o}~3, prop.~3).
...
\begin{proof}
It is clear that if $\mathrm{A}$ is factorial, it is normal, whence $\dim(\mathrm{A})\geq 2$ \sref[0]{0.16.5.1}{} and \sref{IV.21.6.13}{} shows that \emph{a)} implies \emph{b)}.
...
\oldpage[IV-4]{278}
  \item $\mathrm{Y}$ satisfies the condition $(\mathrm{S}_1)$, in other words \sref{IV.5.7.5}{} has no associated prime cycle.
\end{enumerate}
We then have
\[
\label{IV.21.7.2.1}
...
let $\sh{J}$ denote the Ideal of $\sh{O}_\mathrm{X}$ defining $\mathrm{Y}$, and hypothesis (ii) implies that $\operatorname{Ass}(\sh{O}_\mathrm{X}/\sh{J})$ is identical to the set $\mathrm{F}$ of maximal points of $\mathrm{Y}$, and hypothesis (i) implies that $\mathrm{F}$ is contained in $\mathrm{X}^{(1)}$;
hence \sref{IV.3.2.6}{} $\sh{O}_\mathrm{X}/\sh{J}$ is an $\sh{O}_\mathrm{X}$-Module which is irredundant such that $\operatorname{Ass}(\sh{S}(x))=\{x\}$.
Now, since $\mathrm{X}$ satisfies $(\mathrm{R}_1)$, for each $x\in\mathrm{F}$, $\sh{O}_{\mathrm{X},x}$ is a discrete valuation ring and so the only primary ideals are the powers $\mathfrak{m}_x^{n_x}$ of the maximal ideal, where the $\mathfrak{p}_i$ correspond to the points of $\mathrm{F}$.
We see then that $\mathrm{Y}$ is of the form $\mathrm{Y}(\mathrm{C})$.
\end{proof}

\begin{corollary}[21.7.3]
\label{IV.21.7.3}
Let $\mathrm{X}$ be a locally Noetherian prescheme such that $\mathrm{X}$ satisfies $(\mathrm{R}_1)$.
For every positive divisor $\mathrm{D}$ on $\mathrm{X}$, the maximal points of the closed sub-prescheme $\mathrm{Y}(\mathrm{D})$ \sref{IV.21.2.12}{} majorise the closed sub-prescheme $\mathrm{Y}(\operatorname{cyc}(\mathrm{D}))$ \sref{IV.21.7.1}, and this has the same underlying space;
...
\begin{proof}
Indeed, the question is local, and for all $x\in\mathrm{T}$, $\mathrm{D}_x=0$ implies the existence of an open neighbourhood $\mathrm{U}$ of $x$ such that $\mathrm{D}|\mathrm{U}=0$, whence $\operatorname{cyc}(\mathrm{D})|\mathrm{U}=0$, which proves our assertion.
We may suppose $\mathrm{X}=\Spec(\mathrm{A})$, and that $t$ is a regular section of $\sh{O}_\mathrm{X}$ over $\mathrm{X}$, such that $\sh{J}_\mathrm{X}(\mathrm{D})=t\sh{O}_\mathrm{X}$, meaning that $\mathrm{Y}(\mathrm{D})$ satisfies $(\mathrm{S}_1)$, at every maximal point $x$ of $\mathrm{Y}(\mathrm{D})$, $\sh{O}_{\mathrm{X},x}$ is by hypothesis a discrete valuation ring, therefore $t_x\sh{O}_{\mathrm{X},x}=\mathfrak{m}_x^{n_x}$, where $n_x=\operatorname{mult}_x(\mathrm{D})$.
We can assume $\mathrm{X}=\Spec(\mathrm{A})$, and then, if $\sh{J}_\mathrm{X}(\operatorname{cyc}(\mathrm{D}))=\sh{J}'$, it follows from what precedes and from \sref{IV.21.7.1}{} that $\mathfrak{J}$ and $\mathfrak{J}'$ have the same \emph{non-immersed primary ideals}, whence $\mathfrak{J}'\subset\mathfrak{J}$, since $\mathfrak{J}$ is the intersection of
\oldpage[IV-4]{279}
...
Since for all $x\in\mathrm{Y}(\mathrm{D})$, it is a condition on the local ring, and since by hypothesis $\operatorname{prof}(\sh{O}_{\mathrm{X},z})\geq 2$ at all non-maximal points $z\in\mathrm{Y}(\mathrm{D})$, the condition $(\mathrm{S}_1)$ is then also satisfied.
\end{proof}
...
\label{IV.21.7.4}
Let $\mathrm{X}$ be a locally Noetherian prescheme, reduced and with isolated points.
The following conditions are equivalent:
\begin{enumerate}[label=\emph{\alph*)}]
  \item The canonical homomorphism $c:\sh{D}iv_\mathrm{X}\to\mathscr{Z}_\mathrm{X}^1$ \sref{IV.21.6.4}{} is an isomorphism of sheaves of ordered groups, and we have $\operatorname{Div}(\mathrm{X})=\operatorname{Div.princ}(\mathrm{X})$.
  \item[\rm a$'$)] The canonical homomorphism $c:\sh{D}iv_\mathrm{X}\to\mathscr{Z}_\mathrm{X}^1$ is injective, and every prime $1$-codimensional cycle on $\mathrm{X}$ is the image by $\operatorname{cyc}$ of a positive principal divisor, and the canonical homomorphism $c:\sh{D}iv_\mathrm{X}\to\mathscr{Z}_\mathrm{X}^1$ is injective.
  \item[\rm a$''$)] For every integral closed sub-prescheme $\mathrm{Y}$ of $\mathrm{X}$, of codimension $1$, the canonical immersion $\mathrm{Y}\to\mathrm{X}$ is regular.
  \item $\mathrm{X}$ is normal and the homomorphism $\operatorname{cyc}:\operatorname{Div}(\mathrm{X})\to\mathfrak{Z}^1(\mathrm{X})$ is bijective.
  \item $\mathrm{X}$ is locally factorial and $\operatorname{Pic}(\mathrm{X})=0$.
  \item $\mathrm{X}$ is normal, and for every open $\mathrm{U}$ of $\mathrm{X}$, we have $\operatorname{Pic}(\mathrm{U})=0$.
\end{enumerate}
\end{proposition}

\begin{proof}
The equivalence of \emph{a)}, \emph{a$'$)}, \emph{a$''$)}, \emph{b)} and \emph{c)} follows at once from \sref{IV.21.7.4}{} and from \sref{IV.21.6.11}.
Furthermore, it follows at once that every non-empty open $\mathrm{U}$ of $\mathrm{X}$ also satisfies these conditions, in other words \emph{c)} implies \emph{d)}.
It remains to show that \emph{d)} implies \emph{b)}.
Now, let us denote by $\mathrm{U}_\lambda$ the open complements of finite unions of sets of the form $\overline{\{x\}}$, where $\dim(\sh{O}_{\mathrm{X},x})\geq 2$;
it is clear that the $\mathrm{U}_\lambda$ form a decreasing filtered family and that for all $x\in\bigcap_\lambda\mathrm{U}_\lambda$, we have $\dim(\sh{O}_{\mathrm{X},x})\leq 1$, so $\sh{O}_{\mathrm{X},x}$ is a principal ring and therefore one can apply to the family $(\mathrm{U}_\lambda)$ the proposition \sref{IV.21.6.12}, which shows that $\mathfrak{Cl}(\mathrm{X})$ is isomorphic to $\varinjlim\operatorname{Pic}(\mathrm{U}_\lambda)$, hence $\mathfrak{Cl}(\mathrm{X})=0$ by virtue of the hypothesis, which proves \emph{b)} by definition.
\end{proof}
...
with the notation introduced in \sref{IV.14.1.5}, the prime $1$-codimensional cycles of $\mathrm{X}$ are those of the plane $\mathrm{X}_2$ and the line $\mathrm{X}_3$;
...
The following conditions are equivalent:
\begin{enumerate}[label=\emph{\alph*)}]
  \item The canonical homomorphism $c:\sh{D}iv_\mathrm{X}\to\mathscr{Z}_\mathrm{X}^1$ is an isomorphism of sheaves of ordered groups, and we have $\operatorname{Div}(\mathrm{X})=\operatorname{Div.princ}(\mathrm{X})$.
  \item $\mathrm{X}$ is normal and every $1$-codimensional cycle on $\mathrm{X}$ is principal.
  \item $\mathrm{X}$ is locally factorial and $\operatorname{Pic}(\mathrm{X})=0$.
  \item $\mathrm{X}$ is normal, and for every open $\mathrm{U}$ of $\mathrm{X}$, we have $\operatorname{Pic}(\mathrm{U})=0$.
\end{enumerate}
\end{corollary}
...
For every $\sh{O}_{\mathrm{X}}$-Module locally free $\sh{E}'$ of constant rank $n$ and every $x\in\mathrm{X}$ such that, if we set $\mathrm{U}'=f^{-1}(\mathrm{U})$, $\sh{E}'|\mathrm{U}'$ is isomorphic to $\sh{O}_{\mathrm{U}'}^n$.
\end{lemma}
...
we conclude that the $\sh{O}_{\mathrm{X}'_0}$-Module locally free $\sh{E}'_0=\sh{E}'\otimes_{\sh{O}_{\mathrm{X}'}}\sh{O}_{\mathrm{X},x}$ is isomorphic to $\sh{O}_{\mathrm{X}'_0}^n$ (Bourbaki, \emph{Alg.\ comm.}, chap.~II, \textsection~5, n\textsuperscript{o}~3, prop.~5).
...
\begin{proof}
\begin{enumerate}[label=(\roman*)]
  \item (i) is the sheaf associated to the presheaf of commutative groups $\mathrm{U}\mapsto\HH^1(f^{-1}(\mathrm{U}),\sh{O}^*_{\mathrm{X}'})$ on $\mathrm{X}$ (\emph{loc.\ cit.});
the fibre of this sheaf at a point $x$ can be identified with the commutative group $\varinjlim\operatorname{Pic}(f^{-1}(\mathrm{U}))$, where $\mathrm{U}$ ranges over the open neighbourhoods of $x$ in $\mathrm{X}$, and for every element $\zeta\in\operatorname{Pic}(f^{-1}(\mathrm{U}'))$, for every open neighbourhood $\mathrm{U}'$ of $x$ in $\mathrm{X}$, and every element $\zeta\in\operatorname{Pic}(f^{-1}(\mathrm{U}'))$, the transition homomorphisms $\operatorname{Pic}(f^{-1}(\mathrm{U}'))\to\operatorname{Pic}(f^{-1}(\mathrm{U}))$ being defined by \sref{IV.21.3.2.4}.
Now, for all $x\in\mathrm{X}$, every open neighbourhood $\mathrm{U}'$ of $x$ in $\mathrm{X}$, and every element $\zeta\in\operatorname{Pic}(f^{-1}(\mathrm{U}'))$, it follows from \sref{IV.21.8.1}{} that there is an open neighbourhood $\mathrm{U}\subset\mathrm{U}'$ of $x$ such that the image of $\zeta$ in $\operatorname{Pic}(f^{-1}(\mathrm{U}))$ at the point $x$ is zero.
Therefore the fibre of $\RR^1 f_*(\sh{O}^*_{\mathrm{X}'})$ at the point $x$ is zero.

  \item (ii) The Leray spectral sequence for the composed functor $\sh{F}'\mapsto\Gamma(\mathrm{X},f_*(\sh{F}'))$ of sheaves of commutative groups on $\mathrm{X}'$ (T,~2.4) gives the exact sequence of low degree
\[
...
\label{IV.21.8.4}
If, in addition to the hypotheses of \sref{IV.21.8.3}, the homomorphism $\theta'$ is an isomorphism of sheaves of rings, then $\theta^*:\sh{O}^*_\mathrm{X}\to f_*(\sh{O}^*_{\mathrm{X}'})$ is injective, $\theta''':\sh{D}iv_\mathrm{X}\to f_*(\sh{D}iv_{\mathrm{X}'})$ is surjective, $\operatorname{Ker}(\theta''')$ is isomorphic to $\operatorname{Coker}(\theta^*)$.
\end{corollary}
...
is injective, the homomorphism $\theta''':\sh{D}iv_\mathrm{X}\to f_*(\sh{D}iv_{\mathrm{X}'})$ is surjective, $\operatorname{Ker}(\theta''')$ is isomorphic to $\sh{N}=\operatorname{Coker}(\theta^*)$ and the support of the sheaf of multiplicative groups $\sh{N}$ is contained in $\mathrm{S}=\mathrm{X}-\mathrm{U}$.
  \item Suppose moreover that the set $\mathrm{S}$ is discrete and set, for brevity, $\sh{O}^*_\mathrm{X}=f_*(\sh{O}_{\mathrm{X}'})$.
Then, on a commutative diagram
\[
\label{IV.21.8.5.1}
  \xymatrix{
    ...
    1 \ar[r] & (\prod_{s\in\mathrm{S}}\sh{O}^{*''}_{\mathrm{X},s}/\sh{O}^*_{\mathrm{X},s})/\operatorname{Im}\Gamma(\mathrm{X}',\sh{O}^*_{\mathrm{X}'}) \ar[r]^-{} & \operatorname{Pic}(\mathrm{X}) \ar[r] & \operatorname{Pic}(\mathrm{X}') \ar[r] & 0
  }
  \tag{21.8.5.1}
\]
...
\begin{proof}
\begin{enumerate}[label=(\roman*)]
  \item The hypothesis implies that we have a canonical isomorphism $\sh{M}'_\mathrm{X}\simto f_*(\sh{M}_{\mathrm{X}'})$ for the sheaves of germs of pseudo-functions \sref{IV.20.2.2}, whence the assertion relative to $\theta'$, given the existence of the canonical isomorphisms $\sh{M}_\mathrm{X}\simto\sh{M}'_\mathrm{X}$ and $\sh{M}_{\mathrm{X}'}\simto\sh{M}'_{\mathrm{X}'}$ \sref{IV.20.2.11}.
The rest of assertion (i) follows from \sref{IV.21.8.4}, except what concerns the support of $\sh{N}$, which follows directly from the hypothesis on $\mathrm{U}$.
  ...
  \item (ii) Applying the exact sequence of cohomology to the two exact sequences of sheaves of commutative groups
\[
\label{IV.21.8.5.2}
...
\]
\[
  \Gamma(\mathrm{X}',\sh{O}^*_{\mathrm{X}'})\to\Gamma(\mathrm{X},\sh{N})\xrightarrow{\partial}\HH^1(\mathrm{X},\sh{O}^*_\mathrm{X})
\]
\oldpage[IV-4]{283}
...
Now, as the support of $\sh{N}$ is contained in the discrete set $\mathrm{S}$, closed in $\mathrm{X}$, we have $\HH^1(\mathrm{X},\sh{N})=\HH^1(\mathrm{S},\sh{N}|\mathrm{S})=\prod_{s\in\mathrm{S}}\HH^1(\{s\},\sh{N}_s)=0$ by definition of the cohomology (G,~II,~4.9.2) and $\HH^1(\mathrm{S},\sh{N}|\mathrm{S})=\prod_{s\in\mathrm{S}}\HH^1(\{s\},\sh{N}_s)=0$ by definition of the cohomology (G,~II,~4.4).
Similarly, we have $\Gamma(\mathrm{X},\sh{N})=\prod_{s\in\mathrm{S}}\sh{N}_s$ and $\sh{N}_s=\sh{O}^{*''}_{\mathrm{X},s}/\sh{O}^*_{\mathrm{X},s}$.
By the second exact sequence \sref{IV.21.8.5.3}, the image of $t$ by $\partial:\Gamma(\mathrm{X},\sh{N})\to\HH^1(\mathrm{X},\sh{O}^*_\mathrm{X})$ comes from the cocycle obtained as follows: we describe a section $t$ of $\sh{N}$ over $\mathrm{X}$, and we describe a covering of $\mathrm{X}$ formed of $\mathrm{U}$ and of opens $\mathrm{U}_i$ such that $\mathrm{U}_i\cap\mathrm{S}$ reduces to a single point $s_i$ and that $\mathrm{U}_i\cap\mathrm{U}_j\subset\mathrm{U}$ for $i\neq j$, then considering for each $i$ a section $t_i$ of $\sh{N}$ over $\mathrm{U}_i$, which may be considered as a section of $\sh{M}^*_{\mathrm{X}'}$ over $f^{-1}(\mathrm{U}_i)$, thus also as a section of $\sh{M}^*_\mathrm{X}$ over $\mathrm{U}_i$, and finally considering a section $u_i$ of $\sh{O}^*_{\mathrm{X}'}$ over $f^{-1}(\mathrm{U}_i)$, the germ $(t_i)_{s_i}$ comes from a section of $\sh{O}^*_{\mathrm{X}'}$ over $f^{-1}(\mathrm{U}_i)$;
it canonically corresponds to this section a section $d_i\in\Gamma(\mathrm{U}_i,\sh{D}iv_\mathrm{X})$ the restriction of $u_i$ in $\mathrm{U}\cap\mathrm{U}_i$ to $(u_j|(\mathrm{U}_i\cap\mathrm{U}_j))(u_i|(\mathrm{U}_i\cap\mathrm{U}_j))^{-1}$ in $\mathrm{U}_i\cap\mathrm{U}_j$, whence the expression of $j'(t)$ and the commutativity of the diagram \sref{IV.21.8.5.1}.
\end{enumerate}
\end{proof}
...
\label{IV.21.8.6}
\begin{enumerate}[label=(\roman*)]
  \item The conditions of application of \sref{IV.21.8.5},{}~(i) are in particular satisfied when $\mathrm{X}$ is a locally Noetherian prescheme which is \emph{reduced} with only a finite number of irreducible components, $\mathrm{X}'$ is \emph{normalisied} and that $\mathrm{X}'$ is also locally Noetherian (\sref[II]{II.6.3.8});
the $\sh{O}_\mathrm{X}$-Module $\sh{D}iv_\mathrm{X}$ is then an \emph{extension} of $f_*(\sh{D}iv_{\mathrm{X}'})$ by the cokernel $\sh{N}$ of $\sh{O}^*_\mathrm{X}\to f_*(\sh{O}^*_{\mathrm{X}'})$, which we can consider as known.
If moreover $\mathrm{X}$ is an algebraic curve (reduced) over a field $k$, we are in the conditions of application of \sref{IV.21.8.5},{}~(ii).
  ...
  \item When the conditions of application of \sref{IV.21.8.5},{}~(ii) are satisfied and in addition that the canonical homomorphism globally $\Gamma(\mathrm{X},\sh{O}_\mathrm{X})\to\Gamma(\mathrm{X}',\sh{O}_{\mathrm{X}'})$ is \emph{bijective}, we see that the kernels of the surjective homomorphisms $\operatorname{Div}(\mathrm{X})\to\operatorname{Div}(\mathrm{X}')$ and $\operatorname{Pic}(\mathrm{X})\to\operatorname{Pic}(\mathrm{X}')$ are \emph{isomorphic}.

  \item When the conditions of application of \sref{IV.21.8.5},{}~(ii) are satisfied, we see that the homomorphism $\operatorname{Div}(\mathrm{X})\to\operatorname{Div}(\mathrm{X}')$ cannot be injective (in which case it is bijective) unless $\sh{O}^{*''}_{\mathrm{X},s}=\sh{O}^*_{\mathrm{X},s}$ for all $s\in\mathrm{S}$.
For an $s\in\mathrm{S}$ such that $\sh{O}_{\mathrm{X},s}$ is \emph{local} (which, since $\mathrm{X}'=\Spec(\sh{O}^*_\mathrm{X})$ \sref[II]{II.1.3}, means that there exists no point $s'\in\mathrm{X}'$ above $s$, i.e.\ the residue fields $\boldsymbol{k}(s)$ and $\boldsymbol{k}(s')$ are equal;
moreover, if this condition is satisfied, it is furthermore needed that the multiplicative group $1+\mathfrak{m}_s$ has for image $1+\mathfrak{m}_s$, or again $\mathfrak{m}_s=\mathfrak{r}$, which implies that $\sh{O}^{*''}_{\mathrm{X},s}\otimes_{\sh{O}_{\mathrm{X},s}}\boldsymbol{k}(s)$ is a composed direct of fields isomorphic to $\boldsymbol{k}(s)$ (which, when the morphism $f$ is \emph{finite}, implies in the general case, by \sref[0]{0.23.2.6}, the canonical homomorphism $\sh{O}^{*''}_{\mathrm{X},s}\to\sh{O}^*_{\mathrm{X},s}$ defines, by passage to quotients, a homomorphism of the multiplicative group $(\boldsymbol{k}(s))^*$ into the product of the multiplicative groups $(\boldsymbol{k}(s'_j))^*$, the $s'_j$ being the points (in
\oldpage[IV-4]{284}
number $>1$) of $\mathrm{X}'$ above $s$.
It is immediate that this homomorphism cannot be bijective if $\boldsymbol{k}(s)$ and all the $\boldsymbol{k}(s'_j)$ are equal to the field $\bb{F}_2$ with $2$ elements;
moreover, if this condition is satisfied, it is furthermore needed that the multiplicative group $1+\mathfrak{m}_s$ has for image $1+\mathfrak{m}_s$, in other words that $\mathfrak{m}_s=\mathfrak{r}$, which implies that $\sh{O}^{*''}_{\mathrm{X},s}$ is \emph{non ramified} at the points $s'_j$ \sref{IV.17.4.1}.
If for example \emph{no} residue field of $\mathrm{X}$ is isomorphic to $\bb{F}_2$, the canonical homomorphism $\operatorname{Div}(\mathrm{X})\to\operatorname{Div}(\mathrm{X}')$ cannot be bijective if $f$ is an \emph{isomorphism}.
...
If $\mathrm{A}(x)$ is a commutative group, we can consider the commutative group on the space $\{x\}$ reduced to a single point, and take its direct image sheaf of commutative groups on $\mathrm{X}$ \sref[0]{0.3.4.1};
...
\end{enumerate}
When this is the case, for all $x\in\mathrm{X}_0$, the group $\mathrm{A}(x)$ is determined to a unique isomorphism close to $\sh{F}_x$.
...
for every open Noetherian $\mathrm{U}$, every support of a section of $\sh{F}$ over $\mathrm{U}$ is \emph{finite}.
We deduce that for every open $\mathrm{U}$ which is discrete and quasi-compact, the image of the homomorphism \sref{IV.21.9.1.1}{} for $\mathrm{Y}=\mathrm{X}_0$ is contained in $\bigoplus_{x\in\mathrm{X}_0}\Gamma(\mathrm{U},(i_x)_*(\sh{F}_x))$ and that this homomorphism is injective, which establishes the hypothesis \emph{b)}.
...
There then exists a unique section $t$ of $\mathrm{X}$ such that $t_x=a(x)$ for all $x\in\mathrm{E}$ and that the support of $t$ is contained in $\mathrm{E}$.
...
\label{IV.21.9.4}
Let $\mathrm{X}$ be a locally Noetherian prescheme, $\mathrm{X}^{(1)}$ the set of points $x\in\mathrm{X}$ such that $\dim(\sh{O}_{\mathrm{X},x})\leq 1$.
We then have a canonical isomorphism
\[
\label{IV.21.9.4.1}
...
\oldpage[IV-4]{286}
For the 1\textsuperscript{o}, it suffices to note that for every open $\mathrm{U}$ of $\mathrm{X}$ and every divisor $\mathrm{D}\in\operatorname{Div}(\mathrm{U})$, the maximal points of the support $\mathrm{S}$ of $\mathrm{D}$ are such that $\operatorname{prof}(\sh{O}_{\mathrm{X},x})\leq 1$ (for $\mathrm{S}$ is equal to $\mathrm{S}$, and $\mathrm{S}$ is therefore discrete, the set of irreducible components of $\mathrm{S}$ being locally finite.
\end{proof}
...
then, if $\mathrm{A}'$ is the \emph{normalised} of $\mathrm{A}$ (the integral closure of $\mathrm{A}$ in its total ring of fractions), $\mathrm{A}'$ is Noetherian by virtue of the Krull--Akizuki theorem (Bourbaki, \emph{Alg.\ comm.}, chap.~VII, \textsection~2, n\textsuperscript{o}~5, prop.~5), and we have seen in \sref{IV.21.8.6}{} how
\oldpage[IV-4]{287}
$\operatorname{Div}(\mathrm{A})$ can be obtained as an \emph{extension} of $\operatorname{Div}(\mathrm{A}')$ and this last group is of the form $\bb{Z}'$.
\end{env}
...
if moreover $\mathrm{D}_0\geq 0$, we can assume $\mathrm{D}\geq 0$.
  ...
  \item Suppose in addition that there exists in $\mathrm{X}_0$ an affine open $\mathrm{U}_0$ containing $(\operatorname{Ass}(\sh{O}_{\mathrm{X}_0}))\cup\mathrm{Z}_0$.
...
\begin{proof}
\begin{enumerate}[label=(\roman*)]
  \item Taking into account \sref{IV.21.9.6}, we can restrict to showing the assertion relative to divisors $\mathrm{D}_0\geq 0$;
the support $\mathrm{T}$ of $\mathrm{D}_0$ is a finite discrete and closed set in $\mathrm{X}_0$, that is $\mathrm{T}\neq\emp$, there is nothing to show.
For all $x\in\mathrm{T}$, there exists an element $s_x\in\sh{O}_{\mathrm{X}_0,x}$ which is not a divisor of zero and whose image in $(\sh{D}iv_\mathrm{X})_x$ is $(\mathrm{D}_0)_x$.
There exists an open affine neighbourhood $\mathrm{U}^{(x)}$ of $x$ in $\mathrm{X}$ and a section $g^{(x)}$ of $\sh{O}_\mathrm{X}$ over $\mathrm{U}^{(x)}$ such that $s_x$ is the image of the germ $(g^{(x)})_x\in\sh{O}_{\mathrm{X},x}$;
taking $\mathrm{U}^{(x)}$ small enough, we can arrange that $g^{(x)}$ is not a zero divisor in $\Gamma(\mathrm{U}^{(x)},\sh{O}_\mathrm{X})$, in other words \sref{IV.3.1.9}, writing $\mathrm{V}^{(x)}$ for the closed part of $\mathrm{X}$ defined by $g^{(x)}=0$, that $\mathrm{V}^{(x)}\cap\operatorname{Ass}(\sh{O}_\mathrm{X})=\emp$.
Now, if $z\in\operatorname{Ass}(\sh{O}_\mathrm{X})$, the hypothesis $x\notin\overline{\{z\}}$, that is $x\notin\overline{\{z\}}\cap\mathrm{X}_0$, either $x$ is not isolated in $\overline{\{z\}}\cap\mathrm{X}_0$.
Now, if $y$ is a generization of $x$ such that $\overline{\{y\}}$ is of codimension $1$ in $\mathrm{X}$, the sub-prescheme reduced $\mathrm{Y}$ of $\mathrm{X}$ having $\overline{\{y\}}$ for underlying space, $\mathrm{X}_0\cap\mathrm{Y}$ being by hypothesis a finite closed part of $\mathrm{X}$ and open in $\mathrm{Y}$, containing $\mathrm{Y}\cap\mathrm{X}_0$, the condition $(\mathrm{S}_1)$ is then also satisfied.

We can define a divisor $\mathrm{D}'^{(x)}=\operatorname{div}(g^{(x)})$ on $\mathrm{U}^{(x)}$ by $\mathrm{D}'^{(x)}=\operatorname{div}(g^{(x)})$, and $\mathrm{D}'^{(x)}|(X-\mathrm{V}^{(x)})=0$, which has a meaning since $\mathrm{U}^{(x)}\cap(X-\mathrm{V}^{(x)})$ is the complement of the zeros of $g^{(x)}$ in $\mathrm{U}^{(x)}$ ;
by the hypothesis $x\notin\mathrm{Z}_0$ we have $x\notin\overline{\{z\}}$;
as $\mathrm{V}^{(x)}\cap\mathrm{X}_0$ is reduced to $\{x\}$, $\mathrm{V}^{(x)}$ is closed in $\mathrm{X}$.
On can then define a divisor $\mathrm{D}=\sum_{x\in\mathrm{T}}\mathrm{D}'^{(x)}$, which has a meaning since $\mathrm{T}$ is finite.

\oldpage[IV-4]{288}

  \item Taking into account the commutative diagram \sref{IV.21.4.2.1}, the conclusion will follow if we prove that every $\sh{O}_{\mathrm{X}_0}$-Module invertible $\sh{L}_0$ is isomorphic to an $\sh{O}_{\mathrm{X}_0}$-Module of the form $\sh{O}_{\mathrm{X}_0}(\mathrm{D}_0)$ \sref{IV.21.2.8}, where $\mathrm{D}_0$ is a divisor on $\mathrm{X}_0$ whose support does not meet $\mathrm{Z}_0$.
Now, as $\mathrm{U}_0$ is schematically dense in $\mathrm{X}_0$ \sref{IV.20.2.13},{}~(iv), it suffices to prove that there exists a section $t\in\Gamma(\mathrm{U}_0,\sh{L}_0)$ such that $t(z_0)\neq 0$ at the points of $(\operatorname{Ass}(\sh{O}_{\mathrm{X}_0}))\cup\mathrm{Z}_0$, the conclusion results from (\sref[II]{II.5.1.4}) and, as $\mathrm{U}_0$ is affine, $\sh{L}_0$ is ample \sref[II]{II.4.5.4}, the conclusion results from \sref[II]{II.5.3.1}{} and \sref[II]{II.5.5.3}.
\end{enumerate}
\end{proof}
...
since $\dim(\mathrm{X}_0)\leq 1$ by hypothesis and \sref{IV.4.1.4}, we thus have $\dim(\mathrm{X}'_{0\lambda})=\dim(\mathrm{X}_0')\leq 1$ by transitivity of the fibres and \sref{IV.4.1.4}.
It amounts to seeing that if we have proved that the homomorphism $\operatorname{Pic}(\mathrm{X}_\lambda)\to\operatorname{Pic}(\mathrm{X}'_{0\lambda})$ is surjective for all $\lambda\geq\alpha$, we have the same conclusion for $\mathrm{X}$ and $\mathrm{X}'_0$.
...
since $\dim(\mathrm{X}'_0)\leq 1$ by hypothesis and $\dim(\mathrm{X}'_{0\lambda})=\dim(\mathrm{X}_0')\leq 1$ by transitivity of the fibres and \sref{IV.4.1.4}.
...
\]
For every invertible $\sh{O}_{\mathrm{X}'_0}$-Module $\sh{L}'_0$, there exists $\lambda\geq\alpha$ and an invertible $\sh{O}_{\mathrm{X}'_{0\lambda}}$-Module $\sh{L}'_{0\lambda}$ such that $\sh{L}'_0$ is deduced by base change $\mathrm{S}\to\mathrm{S}_\lambda$ \sref{IV.8.5.2}{} and \sref{IV.8.5.5};
it suffices then to take the $\sh{O}_\mathrm{X}$-Module invertible $\sh{L}$ deduced from $\sh{L}_\lambda$ by base change $\mathrm{S}\to\mathrm{S}_\lambda$, and by virtue of the commutativity of the preceding diagram, $\operatorname{cl}(\sh{L}'_0)$ will be the image of $\operatorname{cl}(\sh{L})$.
...
Since by hypothesis $\dim(\mathrm{X}_0')\leq 1$;
on the other hand, for verifying the condition 2\textsuperscript{o} of \sref{IV.21.9.11}, consider a sub-prescheme $\mathrm{Y}$ of $\mathrm{X}$ having $\mathrm{Y}$ as underlying set, the morphism $g:\mathrm{Y}\to\mathrm{S}$, restriction of $f$, being quasi-finite at each of the points $x_i$ of $\mathrm{Y}\cap\mathrm{X}_0$ ($1\leq i\leq n$), we can apply \sref{IV.18.5.11},{}~\emph{c)}) and we see that $\mathrm{Y}$ is the \emph{sum} of sub-preschemes \emph{open} $\mathrm{Y}_i=\Spec(\sh{O}_{\mathrm{Y},x_i})$ ($1\leq i\leq n$) and of a prescheme $\mathrm{Y}''$ such that $\mathrm{Y}''\cap\mathrm{X}_0=\emp$, and in addition that the canonical injections $j_i:\mathrm{Y}_i\to\mathrm{X}$ are such that $f\circ j_i$ is a finite morphism, thus $\mathrm{Y}_i$ is \emph{closed} in $\mathrm{X}$;
on the question by taking $\mathrm{Y}'=\bigcup_{1\leq i\leq n}\mathrm{Y}_i$.

It remains to verify the supplementary hypothesis of \sref{IV.21.9.11},{}~(ii).
Now, $\mathrm{X}_0'$ being a separated curve over the field $\boldsymbol{k}(s_0)$, is a $\boldsymbol{k}(s_0)$-scheme quasi-projective (chap.~V) ($^1$), thus there exists an $\sh{O}_{\mathrm{X}'_0}$-Module \sref[II]{II.5.3.1}, and this achieves the proof.
\end{proof}
...
indeed, if $\sh{L}_0$ is an $\sh{O}_{\mathrm{X}'_0}$-Module ample, it exists, by virtue of \sref{IV.21.9.12}{} an $\sh{O}_\mathrm{X}$-Module invertible $\sh{L}$ whose inverse image in $\mathrm{X}_0'$ is isomorphic to $\sh{L}_0$, hence is ample and, as every neighbourhood of $s_0$ in $\mathrm{S}$ is necessarily the whole of $\mathrm{S}$, we deduce then from \sref{IV.9.6.4}{} that $\sh{L}$ is an $\sh{O}_\mathrm{X}$-Module ample, whence the conclusion (\sref[II]{II.5.3.1}{} and \sref[II]{II.5.5.3}).
\end{remark}
...
\label{IV.21.10.1}
\oldpage[IV-4]{289}
Let $\mathrm{X}$, $\mathrm{X}'$ be two locally Noetherian preschemes, $f:\mathrm{X}'\to\mathrm{X}$ a morphism, $\mathrm{T}$ a closed rare subset of $\mathrm{X}$ contained in $f(\mathrm{X}')$, and let $\mathrm{Z}$ be a $1$-codimensional cycle on $\mathrm{X}$ with support contained in $\mathrm{T}$.
We propose to define, under suitable hypotheses on $f$, a $1$-codimensional cycle $f^*(Z)$ on $\mathrm{X}'$, the \emph{inverse image} of $\mathrm{Z}$ by $f$.

Let us first suppose that $Z\mapsto f^*(Z)$ is a \emph{homomorphism} of ordered groups from the subgroup of $\mathfrak{Z}^1(\mathrm{X})$ formed by cycles with support contained in $\mathrm{T}$, into the ordered group $\mathfrak{Z}^1(\mathrm{X}')$.
For this, let
\[
\label{IV.21.10.1.1}
  Z = \sum_{x\in\mathrm{T}\cap\mathrm{X}^{(1)}} n_x.\overline{\{x\}}
  \tag{21.10.1.1}
\]
where the family of the $x\in\mathrm{T}\cap\mathrm{X}^{(1)}$ such that $n_x\neq 0$ is locally finite.
For all $x'\in\mathrm{X}'^{(1)}$, we define an integer $n_{x'}$ in the following way, setting $x=f(x')$:
\begin{enumerate}[label=\arabic*\textsuperscript{o}]
  ...
  \item if $\sh{O}_{\mathrm{X},x}$ is factorial and $\mathfrak{m}_{x'}\notin\operatorname{Ass}(\sh{O}_{\mathrm{X}',x'})$, we know \sref{IV.21.6.9} that the canonical homomorphism $\operatorname{cyc}:\operatorname{Div}(\sh{O}_{\mathrm{X},x})\to\mathfrak{Z}^1(\operatorname{Spec}(\sh{O}_{\mathrm{X},x}))$ is bijective, and on the other hand since $\dim(\sh{O}_{\mathrm{X}',x'})=1$ and $\mathfrak{m}_{x'}\notin\operatorname{Ass}(\sh{O}_{\mathrm{X}',x'})$, $\operatorname{Ass}(\sh{O}_{\mathrm{X}',x'})$ consists solely of the maximal points of $\operatorname{Spec}(\sh{O}_{\mathrm{X}',x'})$, so the hypothesis on $f$ implies that $f(\operatorname{Ass}(\sh{O}_{\mathrm{X}',x'}))\subset\operatorname{Ass}(\sh{O}_{\mathrm{X},x})$ and it follows from \sref{IV.21.4.5},~(ii) that the homomorphism $f^*:\operatorname{Div}(\sh{O}_{\mathrm{X},x})\to\operatorname{Div}(\sh{O}_{\mathrm{X}',x'})$ is defined; finally, $x'$ being the unique closed point of $\operatorname{Spec}(\sh{O}_{\mathrm{X}',x'})$, $\mathfrak{Z}^1(\operatorname{Spec}(\sh{O}_{\mathrm{X}',x'}))$ is canonically isomorphic to $\bb{Z}$.
\end{enumerate}
We have thus defined a canonical composite homomorphism
\[
\label{IV.21.10.1.2}
  \mathfrak{Z}^1(\operatorname{Spec}(\sh{O}_{\mathrm{X},x})) \xleftarrow{\operatorname{cyc}^{-1}} \operatorname{Div}(\sh{O}_{\mathrm{X},x}) \xrightarrow{f'} \operatorname{Div}(\sh{O}_{\mathrm{X}',x'}) \xrightarrow{\operatorname{cyc}^0} \mathfrak{Z}^1(\operatorname{Spec}(\sh{O}_{\mathrm{X}',x'})) \xrightarrow{\sim} \bb{Z}
  \tag{21.10.1.2}
\]
If $\mathrm{Z}_x$ is the cycle $\sum_{y\in\operatorname{Spec}(\sh{O}_{\mathrm{X},x})\cap\mathrm{X}^{(1)}} n_y.(\overline{\{y\}}\cap\operatorname{Spec}(\sh{O}_{\mathrm{X},x}))$ on $\operatorname{Spec}(\sh{O}_{\mathrm{X},x})$, we take for $n_{x'}$ the \emph{image} of $\mathrm{Z}_x$ by the homomorphism \sref{IV.21.10.1.2}.
\end{env}
...
One can evidently restrict to the case where $n_x=1$, and then by the definition \sref{IV.21.6.5.1} the image of $\mathrm{Z}_x$ by \sref{IV.21.10.1.1} is the number $\lambda_{x'}$, which finishes the proof of A).
...
If the point $x'$ is in case $3^{\mathrm{o}}$ of \sref{IV.21.10.1}, the equality of the multiplicities in $x'$ of the two members of \sref{IV.21.10.4.1} follows from the definition of $n_{x'}$ by means of the homomorphism \sref{IV.21.10.1.1}.
...
Note now that $f^*(\mathrm{D})=\operatorname{div}(t')$, where $t'$ is the image of $t$ in $\Gamma(\mathrm{X}',\sh{O}_{\mathrm{X}'})$, and $\mathrm{Y}(f^*(\mathrm{D}))=f^{-1}(\mathrm{Y}(\mathrm{D}))$; the multiplicity of $f^*(\mathrm{D})$ at the point $x'$ is therefore the length $n_{x'}$ of the $\sh{O}_{\mathrm{X}',x'}$-module $\sh{O}_{\mathrm{X}',x'}/t'_{x'}\sh{O}_{\mathrm{X}',x'}=(\sh{O}_{\mathrm{X},y}/t_y\sh{O}_{\mathrm{X},y})\otimes_{\sh{O}_{\mathrm{X},y}}\sh{O}_{\mathrm{X}',x'}$; it follows from \sref{IV.4.7.1} that $n_{x'}=\lambda_{x'}n_y$, so the multiplicities at the point $x'$ of the two members of \sref{IV.21.10.4.1} are again equal, which finishes the proof.
\end{proof}
...
Noting that every $1$-codimensional cycle with rare support in $\mathrm{X}$ is defined for \emph{every} $1$-codimensional cycle $\mathrm{Z}$ on $\mathrm{X}$; by virtue of \sref{IV.21.10.3.1}, we have thus defined a homomorphism of sheaves of ordered commutative groups
\[
...
The second follows from the fact that $f$ (resp.\ $g$, resp.\ $f\circ g$) transforms maximal points of $\mathrm{X}$ (resp.\ $\mathrm{X}'$, resp.\ $\mathrm{X}''$) into maximal points of $\mathrm{X}$ (resp.\ $\mathrm{X}$, resp.\ $\mathrm{X}$) and from the fact that, if $x''=g(x'')\in\mathrm{X}''^{(1)}$ and $x=f(x')\in\mathrm{X}^{(1)}$ is such that $x'=g(x'')\in\mathrm{X}'^{(1)}$, we have
\[
...
\begin{enumerate}
  \item[(iv)] $x\in\mathrm{X}^{(1)}$, $\mathfrak{m}_{x'}\notin\operatorname{Ass}(\sh{O}_{\mathrm{X},x})$ \emph{and furthermore, if we set $\mathrm{A}=\widehat{\sh{O}}_{\mathrm{X},x}$, $\mathrm{A}'=\widehat{\sh{O}}_{\mathrm{X}',x'}$, and if $\mathrm{K}=\mathrm{A}'\otimes_{\mathrm{A}}\mathrm{K}$ denotes the total ring of fractions of $\mathrm{A}$, then $\mathrm{A}'$ is a finite $\mathrm{A}$-algebra and $\mathrm{K}'=\mathrm{A}'\otimes_{\mathrm{A}}\mathrm{K}$ is a free $\mathrm{K}$-module}.
\end{enumerate}
...
To study the other two cases, let us note that $\mathfrak{m}_{x'}\sh{O}_{\mathrm{X}',x'}$ is closed for the $\mathfrak{m}_{x'}$-preadic topology, so the completion of $\sh{O}_{\mathrm{X}',x'}/\mathfrak{m}_x\sh{O}_{\mathrm{X}',x'}$ for this last topology is $\sh{O}_{\mathrm{X}',x'}/\mathfrak{m}_x\sh{O}_{\mathrm{X}',x'}$.
If one is simultaneously in cases $2^{\mathrm{o}}$ and $4^{\mathrm{o}}$, $\sh{O}_{\mathrm{X}',x'}/\mathfrak{m}_x\sh{O}_{\mathrm{X}',x'}$ is discrete for the $\mathfrak{m}_{x'}$-preadic topology, so isomorphic to $\mathrm{A}'/\mathfrak{m}_x\mathrm{A}'$.
Since $\mathrm{A}'$ is a finite A-algebra and flat $(\mathbf{0}_{\mathrm{III}}$, $10.2.3)$, it is a free A-module (\emph{ibid.}, $10.1.3$), and the rank of $\mathrm{A}'/\mathfrak{m}_x\mathrm{A}'$ over $\mathrm{A}/\mathfrak{m}_x\mathrm{A}=\boldsymbol{k}(x)$ is equal to the rank of $\mathrm{A}'$ over $\mathrm{A}$, hence also to the rank of $\mathrm{K}'$ over $\mathrm{K}$ on $\mathrm{A}$, hence also $\lambda_{x'}$ of the $\sh{O}_{\mathrm{X}',x'}$-module $\sh{O}_{\mathrm{X}',x'}/\mathfrak{m}_x\sh{O}_{\mathrm{X}',x'}$ (or of $\boldsymbol{k}(x')$-module) by the rank $[\boldsymbol{k}(x'):\boldsymbol{k}(x)]=q_{x'}$, which proves the relation $\mu_{x'}=r_{x'}/q_{x'}=\lambda_{x'}$ in this case.
...
We have thus again defined a homomorphism $f^*$ of ordered groups, satisfying \sref{IV.21.10.3.1}, and hence a homomorphism of sheaves of commutative ordered groups
\[
  \psi^*(\mathscr{Z}^1_{\mathrm{X}})\to\mathscr{Z}^1_{\mathrm{X}'}\otimes_{\bb{Z}}\bb{Q}
\]
whence, by tensorisation, a homomorphism of sheaves of $\bb{Q}$-vector spaces ordered
\[
\label{IV.21.10.11.1}
...
We have an example by taking the integral ring and its integral closure $\mathrm{A}'$ of $(\mathbf{0}$, $6.15.11$,~(ii)) and $\sref{IV.21.10.10}$,~(iv) of $\operatorname{Spec}(\mathrm{A}')$ and one has $\mu_{x'}=\frac{1}{2}$.
\end{remark}
...
Since $\mathrm{A}$ is of dimension $1$ and $\mathrm{A}'$ is an A-module of finite type such that $\mathrm{A}'\otimes_{\mathrm{A}}\mathrm{K}$ is a free $\mathrm{K}$-module of rank $r_{x'}$, we can apply \sref{IV.21.10.13},~(v) to $\mathrm{M}=\mathrm{A}'$ and to $t_y$, and the length of $\mathrm{A}'/t_y\mathrm{A}'$ as an $\mathrm{A}'$-module is therefore $r_{x'}n_y$.
...
To prove \sref{IV.21.10.16.1}, we are immediately reduced to the case where $\mathrm{X}$, $\mathrm{X}'$ and $\mathrm{X}_1$ are spectra of local Noetherian rings of dimension $1$, $\mathrm{A}$, $\mathrm{A}'$ and $\mathrm{A}_1$, $\mathrm{A}'$ being a finite flat A-module.
...
Since we have
\[
  [\boldsymbol{k}(x'):\boldsymbol{k}(x)]\operatorname{length}_{\mathrm{A}'}(\mathrm{A}_1'/\mathfrak{m}_{x'}\mathrm{A}_1')=\operatorname{length}_{\mathrm{A}_1}(\mathrm{A}_1'/\mathfrak{m}_{x'}\mathrm{A}_1')
\]
where we have set $\mathrm{A}_1'=\mathrm{A}'\otimes_{\mathrm{A}}\mathrm{A}_1$.
We have thus $\mathrm{A}_1'/\mathfrak{m}_{x'}\mathrm{A}_1'=(\mathrm{A}'/\mathfrak{m}_{x'}\mathrm{A}')\otimes_{\mathrm{A}}\mathrm{A}_1$, and as $\mathrm{A}_1$ is a flat A-module, by \sref{IV.4.7.1},
\[
  \operatorname{length}_{\mathrm{A}_1}(\mathrm{A}_1'/\mathfrak{m}_{x'}\mathrm{A}_1')=\operatorname{length}_{\mathrm{A}_1}(\mathrm{A}_1/\mathfrak{m}_x\mathrm{A}_1)\operatorname{length}_{\mathrm{A}_1}(\mathrm{A}_1'/\mathfrak{m}\mathrm{A}_1')
...
We can restrict to the case where $t'$ is not invertible in $\mathrm{A}'$, the ring $\mathrm{A}/t\mathrm{A}$ is then of finite length, and $\mathrm{A}'/t'\mathrm{A}'$ is a direct sum of local Artinian rings $\mathrm{A}_i'$ ($1\leq i\leq r$), the residue field of $\mathrm{A}_i'$ being $\boldsymbol{k}(x_i')$, where $x_i'$ ($1\leq i\leq r$) are the points of $\mathrm{X}'$ lying above $x$.
If $t_i'$ ($1\leq i\leq r$) is the image of $t'$ in $\mathrm{A}_i'$, $\mathrm{A}'/t'\mathrm{A}'$ is a direct sum of the $\mathrm{A}_i'/t_i'\mathrm{A}_i'$; since the product $\operatorname{length}_{\mathrm{A}_i'}(\mathrm{A}_i'/t_i'\mathrm{A}_i')\cdot[\boldsymbol{k}(x_i'):\boldsymbol{k}(x)]$ is equal to $\operatorname{length}_{\mathrm{A}}(\mathrm{A}_i'/t_i'\mathrm{A}_i')$, we see that the multiplicity at point $x$ of the first member of \sref{IV.21.10.17.1} is $\operatorname{length}_{\mathrm{A}}(\mathrm{A}'/t'\mathrm{A}')$, so that the formula to prove reduces to
\[
\label{IV.21.10.17.2}
...
\]
of A-modules, where $\mathrm{M}''$ is of \emph{finite length}: indeed by localising the preceding exact sequence at each of the $\mathfrak{p}_i$, we obtain $\mathrm{M}''_{\mathfrak{p}_i}=0$, since $(\mathfrak{q}_i)_{\mathfrak{p}_i}=0$ and $(\mathfrak{q}_j)_{\mathfrak{p}_i}=\mathrm{A}_{\mathfrak{p}_j}$ for $j\neq i$ (Bourbaki, \emph{Alg.\ comm.}, chap.~IV, \textsection2, n$^{\mathrm{o}}$~4, prop.~6), so $(\mathrm{M}_i')_{\mathfrak{p}_i}=\mathrm{M}_{\mathfrak{p}_i}$ and $(\mathrm{M}_j')_{\mathfrak{p}_i}=0$; the support of $\mathrm{M}''$ is therefore reduced to $\mathfrak{m}$, hence $\mathrm{M}''$ is of finite length $(\mathbf{0}$, $16.1.10)$.
...
\]
and that each of the numbers $\chi(\mathrm{M}_j,v_j)$ is defined and one has
\[
...
Since $\operatorname{Coker}(u_i)$ is an image of $\operatorname{Coker}(u)$, it is also of finite length and $\chi(\mathrm{M}_i,u_i)$ is therefore defined for all $i$; we have therefore, by the formula \sref{IV.21.10.17.8}, $\chi(\mathrm{M},u)+\chi(\mathrm{M}'',u'')=\sum_i\chi(\mathrm{M}_i',u_i')$ and, since $\mathrm{M}''$ is of finite length, $\chi(\mathrm{M}'',u'')=0$.
...
\begin{proof}
\oldpage[IV-4]{302}
By virtue of the definitions, we are immediately reduced to the case of a local Noetherian ring $\mathrm{A}$ of dimension $1$, of closed point $x$, and where $\mathrm{Z}=\overline{\{x\}}$; set $\mathrm{X}'=\operatorname{Spec}(\mathrm{A}')$ where $\mathrm{A}'$ is a finite A-algebra and for every minimal prime ideal $\mathfrak{p}_i$ of $\mathrm{A}$, $\mathrm{A}_{\mathfrak{p}_i}'$ is a free $\mathrm{A}_{\mathfrak{p}_i}$-module of rank $n$.
Let us show that we can furthermore restrict to the case where $\mathrm{A}$ is \emph{complete}; let us make the base change $h:\mathrm{Y}\to\mathrm{X}$, where $\mathrm{Y}=\operatorname{Spec}(\mathrm{B})$, $\mathrm{B}=\widehat{\mathrm{A}}$, and set $\mathrm{Y}'=\mathrm{X}'\times_{\mathrm{X}}\mathrm{Y}=\operatorname{Spec}(\mathrm{B}\otimes_{\mathrm{A}}\mathrm{A}')$ and $g=f_{\{Y\}}:\mathrm{Y}'\to\mathrm{Y}$; the morphism $g$ is then finite, and it satisfies the same conditions as $f$; if we prove the formula \sref{IV.21.10.18.1} for $g$ and $\overline{\{x\}}$, we verify from \sref{IV.21.10.16.1}.
We can therefore suppose that $\mathrm{A}$ is complete, and hence so is $\mathrm{A}'$ which is also decomposed as a direct product of complete local rings; one can therefore also restrict ourselves to the case where $\mathrm{A}'$ is local, and verify \sref{IV.21.10.18.1} for each of the conditions (ii), (iii), (iv) of $\mathrm{X}'$.

In case (ii), $\mathrm{A}'$ is a flat A-module of finite type, of rank $n$ by hypothesis, hence by definition \sref{IV.21.10.1} and \sref{IV.21.10.3} $f^*(Z)=\lambda_{x'}\cdot\overline{\{x'\}}$, where $\lambda_{x'}$ is the length of the $\mathrm{A}'$-module $\mathrm{A}'/\mathfrak{m}_x\mathrm{A}'$, hence $f_*(f^*(Z))=(\lambda_{x'}[\boldsymbol{k}(x'):\boldsymbol{k}(x)])\cdot\overline{\{x\}}$; but $\lambda_{x'}[\boldsymbol{k}(x'):\boldsymbol{k}(x)]$ is the length of $\mathrm{A}'/\mathfrak{m}_x\mathrm{A}'$ as A-module, or its rank as $\boldsymbol{k}(x)$-vector space, which is equal to $n$.

In case (iii), $\mathrm{A}$ is a discrete valuation ring, hence regular, and the hypothesis $\mathfrak{m}_{x'}\notin\operatorname{Ass}(\sh{O}_{\mathrm{X}',x'})$ implies that $\mathrm{A}'$ is a Cohen-Macaulay ring $(\mathbf{0}$, $16.4.6)$; since $\dim(\mathrm{A}')=\dim(\mathrm{A})=1$ that $\mathrm{A}'/\mathfrak{m}_x\mathrm{A}'$ is an Artinian ring, it follows from \sref{IV.6.1.5} that $\mathrm{A}'$ is a flat A-module and one is reduced to case (ii).

In case (iv), if $\mathrm{K}$ is the total ring of fractions of $\mathrm{A}$, $\mathrm{K}'=\mathrm{A}'\otimes_{\mathrm{A}}\mathrm{K}$ is by hypothesis a free $\mathrm{K}$-module of rank $n$ and by definition \sref{IV.21.10.10}, we have $f^*(Z)=(n/[\boldsymbol{k}(x'):\boldsymbol{k}(x)])\cdot\overline{\{x'\}}$, whence $f_*(f^*(Z))=n\cdot\overline{\{x\}}$.
...
\]
an exact sequence of $\sh{O}_{\mathrm{X}}$-Modules locally free of finite rank; then the invertible $\sh{O}_{\mathrm{X}}$-Module $\bigotimes_{0\leq i\leq h}(\Lambda^{\max}\mathscr{E}_i)^{\otimes(-1)^i}$ is isomorphic to $\sh{O}_{\mathrm{X}}$.
\end{lemma}
...
\[
  \operatorname{Hom}_{\mathrm{X}}(\mathrm{U},\mathrm{V})\xrightarrow{\sim}\operatorname{Hom}_{\sh{O}_x}(\mathscr{A}(\mathrm{V}),\mathscr{A}(\mathrm{U}))
\]
and $\operatorname{Hom}_{\mathrm{X}}(\mathrm{U}^0,\mathrm{V})\xrightarrow{\sim}\operatorname{Hom}_{\sh{O}_x}(\mathscr{A}(\mathrm{V}),\mathscr{A}(\mathrm{U}^0))$ $(\mathbf{II}$, $1.2.7)$.
...
\label{IV.21.12.3}
For every quasi-compact and quasi-separated morphism $f:\mathrm{U}\to\mathrm{X}$ and all $x\in\mathrm{X}$, there is a canonical isomorphism up to canonical isomorphism
\[
...
We will, when $\mathrm{X}$ is locally Noetherian, give precisions on the nature of the set $\operatorname{Daf}(\mathrm{U}/\mathrm{X})$, which will show for example that when $\mathrm{U}$ is not everywhere dense in $\mathrm{X}$, $\operatorname{Daf}(\mathrm{U}/\mathrm{X})$ is not any rare closed subset:
...
Indeed, $x$ cannot evidently be a maximal point of $\mathrm{X}$, not being contained in $\overline{\mathrm{U}}$; it is therefore a matter of seeing that we cannot have $\dim(\sh{O}_{\mathrm{X},x})=1$.
Now, this relation would imply, by \sref{IV.21.12.5.3}, $x\in\operatorname{Daf}(\mathrm{U}^{(a)}/\operatorname{Spec}(\sh{O}_{\mathrm{X},x}))$, where $\mathrm{U}^{(a)}=\mathrm{U}\cap\operatorname{Spec}(\sh{O}_{\mathrm{X},x})$.
...
Now, the open set reduced to a single maximal point of $\operatorname{Spec}(\sh{O}_{\mathrm{X},x})$ is affine, by definition of preschemes; we conclude that \emph{all} open subsets of $\operatorname{Spec}(\sh{O}_{\mathrm{X},x})$ are affine, hence $\operatorname{Daf}(\mathrm{U}^{(a)}/\operatorname{Spec}(\sh{O}_{\mathrm{X},x}))=\varnothing$, contradicting the hypothesis.

To prove (i), it remains to show more, namely that if $\dim(\sh{O}_{\mathrm{X},x})=1$ is such that $x\in\mathrm{X}$, there exists a neighbourhood open $\mathrm{W}$ of $x$ in $\mathrm{X}$ such that $\mathrm{W}$ does not meet $\operatorname{Daf}(\mathrm{U}/\mathrm{X})$, i.e.\ such that the canonical injection $f_{\mathrm{W}}:\mathrm{U}\cap\mathrm{W}\to\mathrm{W}$ is affine.
But, with the same notations as above, we see that the injection $f^{(a)}:\mathrm{U}^{(a)}\to\operatorname{Spec}(\sh{O}_{\mathrm{X},x})$ is affine.
We can evidently restrict to the case where $\mathrm{X}$ is Noetherian, and $x$ the closed point of $\mathrm{X}$.
For this, it suffices to make the change of base $h:\mathrm{X}'=\operatorname{Spec}(\mathrm{A})\to\mathrm{X}$, where $\mathrm{A}=\widehat{\sh{O}}_{\mathrm{X},x}$; if we set $\mathrm{U}'=h^{-1}(\mathrm{U})$, $f'=f_{\{X'\}}$ the canonical injection $\mathrm{U}'\to\mathrm{X}'$, and since the morphism $h$ is \emph{flat}, it follows from \sref{IV.21.12.2} that if we prove that $x$ belongs to $f'^0(\mathrm{U}'^0)$, then $x\in f^0(\mathrm{U}^0)$.
By virtue of \sref{IV.6.1.1}, we have made the desired reduction.

Let then $\mathrm{X}_1$ be a closed reduced sub-prescheme of $\mathrm{X}$ having as underlying space an irreducible component of $\mathrm{X}$, of \emph{maximal} dimension, among those that contain $\mathrm{T}$, and set $\mathrm{U}_1=\mathrm{U}\cap\mathrm{X}_1$, $\mathrm{T}_1=\mathrm{T}\cap\mathrm{X}_1=\mathrm{X}_1-\mathrm{U}_1$; we have therefore $\operatorname{codim}(\mathrm{T}_1,\mathrm{X}_1)\geq 2$ $(\mathbf{0}$, $14.2.1)$, and the pair $(\mathrm{U}_1,\mathrm{X}_1)$ satisfies the same hypotheses that the pair $(\mathrm{U},\mathrm{X})$ ($\mathrm{X}_1$ being the spectrum of a quotient of $\mathrm{A}$, hence a local Noetherian complete ring).
\oldpage[IV-4]{307}
We have on the other hand a commutative diagram
\[
  \xymatrix{
    \mathrm{U}_1 \ar[r]^i \ar[d]_h & \mathrm{U} \ar[d]^f \\
    \mathrm{X}_1 \ar[r]_i & \mathrm{X}
  }
\]
where $i$ and $j$ are the canonical injections, $i$ being thus an \emph{affine} morphism; we deduce from \sref{IV.21.12.1} the existence of a morphism $j^0:\mathrm{U}_1^0\to\mathrm{U}^0$ making commutative the diagram
\[
  \xymatrix{
...
\]
and hence, to prove that $x\in f^0(\mathrm{U}^0)$, it suffices to prove that $x\in f_1^0(\mathrm{U}_1^0)$.
We can therefore replace $\mathrm{X}$ by $\mathrm{X}_1$, and we can hence suppose that the ring $\mathrm{A}$ is \emph{integral}.
But $\mathrm{A}$ is a quotient of a regular Noetherian ring by the Cohen structure theorem $(\mathbf{0}$, $19.8.8$,~(i)) and since it is integral, we can apply $(5.11.1)$ with the family $(x_a)$ reduced to the single maximal point of $\mathrm{X}$; since $\operatorname{codim}(\mathrm{T},\mathrm{X})\geq 2$ by hypothesis, we see that $f_*(\sh{O}_{\mathrm{U}})$ is a \emph{coherent} $\sh{O}_{\mathrm{X}}$-Module, hence the morphism $f^0:\mathrm{U}^0\to\mathrm{X}$ is \emph{finite}; since this morphism is dominant ($\mathrm{U}$ being everywhere dense), it is surjective $(\mathbf{II}$, $6.1.10)$, and hence $x\in f^0(\mathrm{U}^0)$.

(ii) It is a matter of proving that for every point $x\in\mathrm{T}$, we have $x\in f^0(\mathrm{U}^0)$; we will first show that we can reduce to the case where $\mathrm{X}=\operatorname{Spec}(\mathrm{A})$, where $\mathrm{A}$ is a \emph{complete} local Noetherian ring, and $x$ the closed point of $\mathrm{X}$.
For this, it suffices to make the change of base $h:\mathrm{X}'=\operatorname{Spec}(\mathrm{A})\to\mathrm{X}$, where $\mathrm{A}=\widehat{\sh{O}}_{\mathrm{X},x}$; if we set $\mathrm{U}'=h^{-1}(\mathrm{U})$, $f'=f_{\{X'\}}$ the canonical injection $\mathrm{U}'\to\mathrm{X}'$, and since the morphism $h$ is \emph{flat}, it follows from \sref{IV.21.12.2} that if we prove that $x$ belongs to $f'^0(\mathrm{U}'^0)$, then $x\in f^0(\mathrm{U}^0)$.
By virtue of \sref{IV.6.1.1}, we have made the desired reduction.

Let then $\mathrm{X}_1$ be a closed reduced sub-prescheme of $\mathrm{X}$ having as underlying space an irreducible component of $\mathrm{X}$, of \emph{maximal} dimension, among those that contain $\mathrm{T}$, and set $\mathrm{U}_1=\mathrm{U}\cap\mathrm{X}_1$, $\mathrm{T}_1=\mathrm{T}\cap\mathrm{X}_1=\mathrm{X}_1-\mathrm{U}_1$; we have therefore $\operatorname{codim}(\mathrm{T}_1,\mathrm{X}_1)\geq 2$ $(\mathbf{0}$, $14.2.1)$, and the pair $(\mathrm{U}_1,\mathrm{X}_1)$ satisfies the same hypotheses (the pair $(\mathrm{U},\mathrm{X})$ ($\mathrm{X}_1$ being the spectrum of a quotient of $\mathrm{A}$, hence a local Noetherian complete ring).
On the other hand, by the commutative diagram above we see that $x\in f^0(\mathrm{U}^0)$ as we have seen, and we can hence suppose that the ring $\mathrm{A}$ is integral.
But $\mathrm{A}$ is a quotient of a regular Noetherian ring by the Cohen structure theorem $(\mathbf{0}$, $19.8.8$,~(i)) and since it is integral, we can apply $(5.11.1)$ with the family $(x_a)$ reduced to the single maximal point of $\mathrm{X}$; since $\operatorname{codim}(\mathrm{T},\mathrm{X})\geq 2$ by hypothesis, we see that $f_*(\sh{O}_{\mathrm{U}})$ is a \emph{coherent} $\sh{O}_{\mathrm{X}}$-Module, hence the morphism $f^0:\mathrm{U}^0\to\mathrm{X}$ is \emph{finite}; since this morphism is dominant ($\mathrm{U}$ being everywhere dense), it is surjective $(\mathbf{II}$, $6.1.10)$, and hence $x\in f^0(\mathrm{U}^0)$.
\end{proof}
...
Then every irreducible component of $\mathrm{T}=\mathrm{X}-\mathrm{U}$ is of codimension $\leq 1$ (and hence everywhere dense).
\end{corollary}

\begin{proof}
Suppose indeed that one of the irreducible components $\mathrm{T}_1$ of $\mathrm{T}$ is of codimension $\geq 2$ in $\mathrm{X}$.
Replacing $\mathrm{X}$ if needed by a neighbourhood open of the generic point of $\mathrm{T}_1$, we can suppose $\mathrm{T}$ irreducible and of codimension $\geq 2$.
...
Indeed, if (W') is satisfied and if $\mathrm{U}$ is an open of $\mathrm{Y}$ satisfying the hypothesis of (W), it follows from \sref{IV.21.12.7} that no irreducible component of $\mathrm{T}=\mathrm{Y}-\mathrm{U}$ can be of codimension $\geq 2$; since on the other hand $\mathrm{U}$ contains no irreducible component of $\mathrm{Y}_i-\{y\}$ of $\mathrm{Y}$, condition (W') on $\mathrm{Y}$, shows that $\mathrm{U}$ is affine.
...
But there are local Noetherian rings which are not factorial, for example those of dimension $\leq 1$: indeed, as remarked in the proof of \sref{IV.21.12.6},~(i), that \emph{all} opens of $\mathrm{Y}$ are affine.
One can on the other hand prove by using the theory of local duality (chap.~III, $3^{\text{e}}$ partie) that every local Noetherian ring of dimension $2$ satisfies (W').
\end{env}

The interest of condition (W) resides in the following result:
\begin{proposition}[21.12.10]
\label{IV.21.12.10}
Let $\mathrm{X}$, $\mathrm{Y}$ be two locally Noetherian preschemes, $g:\mathrm{X}\to\mathrm{Y}$ a morphism, $y$ a closed point of $\mathrm{Y}$, $\mathrm{X}'=g^{-1}(Y')$; suppose that the morphism $g':\mathrm{X}'\to\mathrm{Y}'$ restriction of $g$ is an open immersion, and that the local ring $\sh{O}_{\mathrm{Y},y}$ satisfies the condition \textup{(W)} of \sref{IV.21.12.8}.
Then for every irreducible $\mathrm{Z}$ of $\mathrm{X}_y=g^{-1}(y)$, either $\mathrm{Z}$ is of codimension $\leq 1$ in $\mathrm{X}$, or its generic point is isolated in $\mathrm{Z}$.
...
\oldpage[IV-4]{309}
Suppose that there exists an irreducible component $\mathrm{Z}$ of $\mathrm{X}_y$ whose generic point $z$ is not isolated in $\mathrm{Z}$, and such that $\operatorname{codim}(\mathrm{Z},\mathrm{X})\geq 2$.
The question being local on $\mathrm{X}$ and $\mathrm{Y}$, suppose $\mathrm{X}$ and $\mathrm{Y}$ affine; since on the other hand $z\in\mathrm{X}_y$ is the generic point of $\mathrm{T}$, we have $g(\mathrm{T})\subset\overline{\{y\}}=\{y\}$, i.e.\ $\mathrm{T}=\mathrm{X}_y$.
We are then exactly in the conditions of application of \sref{IV.21.12.10}, in other words $\mathrm{T}=\mathrm{X}_y$.
Set $\mathrm{X}_i=\overline{\{x_i\}}$.
By hypothesis, $z$ is not isolated in $\mathrm{X}_y$, and since $z$ is not a maximal point of $\mathrm{X}$ (this would imply $\operatorname{codim}(\mathrm{Z},\mathrm{X})=0$), it follows that there exists $x_i$ in $\mathrm{X}_y$ with $x_i\neq z$.
Since $\mathrm{X}_i\neq\mathrm{Z}$, there exists $\mathrm{X}_i$ a point $t_i$ such that if we set $\mathrm{T}_i=\overline{\{t_i\}}$, we have $\dim(\sh{O}_{\mathrm{T}_i,z})=1$ \sref{IV.10.5.9}; this implies by hypothesis $t_i\notin\mathrm{Z}$, so $t_i$ is not a generisation of $z$.
Replacing $\mathrm{X}$ by $\mathrm{X}-\bigcup_i\mathrm{T}_i$, we see that $g$ of $\mathrm{X}'\to\mathrm{X}_y'$ does not contain the images $g(t_i)$, so does not contain $y$.
This being so, since $\mathrm{X}$ and $\mathrm{Y}$ are affines, the morphism $g:\mathrm{X}\to\mathrm{Y}$ is affine, hence $g':\mathrm{X}'\to\mathrm{Y}'$ is also an affine morphism of the restriction $g':\mathrm{X}'\to\mathrm{Y}'$.
Set $\mathrm{Y}_1=\operatorname{Spec}(\sh{O}_{\mathrm{Y},y})$, $\mathrm{Y}_1'=\mathrm{Y}_1-\{y\}=\mathrm{Y}'\cap\mathrm{Y}_1$, $\mathrm{U}_1=\mathrm{U}\cap\mathrm{X}_1$, $\mathrm{T}_1=\mathrm{T}\cap\mathrm{X}_1=\mathrm{X}_1-\mathrm{U}_1$; we have therefore $\operatorname{codim}(\mathrm{T}_1,\mathrm{X}_1)\geq 2$ $(\mathbf{0}$, $14.2.1)$, and the pair $(\mathrm{U}_1,\mathrm{X}_1)$ satisfies the same hypotheses as the pair $(\mathrm{U},\mathrm{X})$.
This implies that $\mathrm{U}_1$ is an open affine in $\mathrm{Y}_1$ by virtue of (W); one would conclude from this that the open $\mathrm{X}-\mathrm{X}_y$ of $\mathrm{X}$, isomorphic to $\mathrm{U}$, would be affine, which contradicts \sref{IV.21.12.7}; the proposition is thus proved.
\end{proof}
...
But, up to restringing to an open neighbourhood of $z$, we can restrict to the case $\mathrm{T}=\mathrm{X}_y$; then all the fibres $g^{-1}(y')$ ($y'\in\mathrm{Y}$) would be empty or reduced to a single point, and it would follow from the ``Main theorem'' \sref{IV.8.12.10} that $g$ would be a local isomorphism, contradicting the hypothesis $z\in\mathrm{T}$.
\end{proof}

\begin{corollary}[21.12.13]
\label{IV.21.12.13}
Suppose satisfied the hypotheses of \sref{IV.21.12.12} and in addition, suppose that $g$ is quasi-finite at each of the points of $\mathrm{X}^{(1)}$ (recall that these are the points where $\dim(\sh{O}_{\mathrm{X},x})=1$); if furthermore $g$ is separated, $g$ is an open immersion.
\end{corollary}
...
\oldpage[IV-4]{311}
(iii)
Without supposing that $\mathrm{Y}$ is locally factorial, but instead assuming by contrast that $\mathrm{X}$ is intersection complete relative to $\mathrm{Y}$ (chap.~V), we will see in chap.~V that one has a more precise result than \sref{IV.21.12.12}, expressing $\mathrm{T}$ as a $1$-codimensional cycle of an $\sh{O}_{\mathrm{X}}$-Module invertible, canonically associated to $g$.
...
(iv) One may ask whether \sref{IV.21.12.10} admits a converse; in other words, for a local Noetherian ring $\mathrm{A}$, set $\mathrm{Y}=\operatorname{Spec}(\mathrm{A})$, $y$ denoting the closed point of $\mathrm{Y}$, and for every locally Noetherian prescheme $\mathrm{X}$ and every morphism $g:\mathrm{X}\to\mathrm{Y}$ such that $g':\mathrm{X}'\to\mathrm{Y}'$ is an open immersion, is every irreducible component of $\mathrm{X}_y$ of codimension $\leq 1$ in $\mathrm{X}$ or reduced to a single point, is it true that $\mathrm{A}$ satisfies condition (W) of \sref{IV.21.12.8}?
...
One can show that the preceding conjecture would be a consequence of the following (and it would even be equivalent): for a regular local ring $\mathrm{A}$ contained in a local integral integrally closed ring $\mathrm{B}$, such that $\mathrm{B}$ is a finite A-module, the open $\mathrm{U}$ of the points of $\operatorname{Spec}(\mathrm{B})$ for which $\operatorname{Spec}(\mathrm{B})$ is unramified over $\operatorname{Spec}(\mathrm{A})$ is affine, or equivalently (or also by the same argument $(18.10.1)$, \'etale on $\operatorname{Spec}(\mathrm{A})$) is \emph{affine}.
\end{remarks}
...
For every $s\in\mathrm{S}$, by \sref{IV.17.9.5}, it suffices to prove that for all $s\in\mathrm{S}$, $f_s:\mathrm{X}_s\to\mathrm{Y}_s$ is an open immersion.
Since every $s\in\mathrm{S}$ is a generisation of a maximal point $\eta$, by making the change of base $\operatorname{Spec}(\sh{O}_{\mathrm{S},s})\to\mathrm{S}$ we can reduce to the case where $\mathrm{S}$ is a local integral scheme of closed point $s$ and generic point $\eta$.
Since $\mathrm{X}_s$ and $\mathrm{X}_\eta$ are geometrically irreducible, the irreducible components of $\mathrm{X}_s$ and $\mathrm{Y}_s$ are of the same dimension.
But by hypothesis $\mathrm{X}_s$ is irreducible, and since $f_\eta$ is an isomorphism, $\mathrm{X}$ is irreducible and reduced $(\mathbf{I}$, $3.2.1)$ and by flatness $(\mathbf{I}$, $3.3.2)$, hence $\mathrm{X}$ is reduced (\emph{ibid.}, $3.2.1$) and by flatness, $\mathrm{X}$ is integral.
Since $g:\mathrm{X}\to\mathrm{S}$ is flat, the maximal points of $\mathrm{X}$ are contained in $\mathrm{X}_\eta$ \sref{IV.2.3.4} and since $f_\eta$ is an isomorphism, $\mathrm{X}$ is irreducible and the local ring at its generic point is a field; moreover, by flatness \sref{IV.3.3.2}, $\mathrm{X}$ has no prime cycles of dimension $1$, hence $\mathrm{X}$ is reduced (\emph{ibid.}, $3.2.1$).
To prove that $f$ is an open immersion, we note that the assertion is evident at the points of $\mathrm{X}^{(1)}$ that belong to $\mathrm{X}_\eta$; the only point of $\mathrm{X}^{(1)}$ belonging to $\mathrm{X}_s$ is the generic $x$ of $\mathrm{X}_s$, by virtue of \sref{IV.6.1.1}.
Now, it follows from \sref{IV.2.4.6} and \sref{IV.14.2.2} that the morphisms $g$ and $h$ are equidimensional; as the irreducible components of $\mathrm{X}_s$ and $\mathrm{Y}_s$ are all of the same dimension.
But by hypothesis $\mathrm{X}_s$ is irreducible, and since $f_\eta$ is surjective, it follows from the same of $f_s:\mathrm{X}_s\to\mathrm{Y}_s$, and $\dim(f_s^{-1}(y))=0$, i.e.\ $f$ is well \emph{quasi-finite} at the point $x$, and the conclusion follows from the fact that we can apply the criterion \sref{IV.21.12.13}; to see that $f$ is quasi-finite at the points of $\mathrm{X}^{(1)}$, we remark that the assertion is evident at those that belong to $\mathrm{X}_\eta$; the only point of $\mathrm{X}^{(1)}$ belonging to $\mathrm{X}_s$ is the generic $x$ of $\mathrm{X}_s$, by virtue of \sref{IV.6.1.1}.
Now, it follows from \sref{IV.2.4.6} and \sref{IV.14.2.2} that the morphisms $g$ and $h$ are equidimensional; as $\mathrm{X}_\eta$ and $\mathrm{Y}_\eta$ are isomorphic, the irreducible components of $\mathrm{X}_s$ and of $\mathrm{Y}_s$ all have the same dimension.
But by hypothesis $\mathrm{X}_s$ is irreducible, and since $f_\eta$ is surjective, hence so is $f_s$, and $\dim(f_s^{-1}(y))=0$, i.e.\ $f$ is well \emph{quasi-finite} at the point $x$, and the conclusion follows from \sref{IV.21.12.13}.
\end{proof}
...
\oldpage[IV-4]{313}
Stability by specialisation follows from \sref{IV.21.12.15} (noting that the projection of the closed point of $\mathrm{S}$ is the closed point of $\mathrm{S}_0$) in order to reduce to the case where moreover $\mathrm{S}$ is \emph{Noetherian}; but then the conclusion follows from $(\mathbf{III}$, $4.6.7$,~(ii)).

Stability by generisation follows from \sref{IV.21.12.15}.
\end{proof}
...
Denote moreover $s$ and $\eta$ the closed point and the generic point of $\mathrm{S}$; let $z$ be a closed point of $\mathrm{Y}_s$, for example a rational point $k=\boldsymbol{k}(s)$, and let $\mathrm{X}$ be the $\mathrm{Y}$-prescheme obtained by blowing up the point $z$.
As in the proof of \sref{IV.15.1.6}, if $f:\mathrm{X}\to\mathrm{Y}$ is the structural morphism, $f^{-1}(z)$ in $\mathrm{X}_s$ is isomorphic to the complement of $\mathrm{Z}_1$, so $\mathrm{Z}_1=f^{-1}(z)$ and $\mathrm{Z}_2$, the closure in $\mathrm{X}_s$ of the complement of $\mathrm{Z}_1$, are the two irreducible components of $\mathrm{X}_s$.
On the other hand, $f$ is evidently proper and $g=h\circ f$ is flat, since $\mathrm{X}$ is integral $(\mathbf{II}$, $8.1.4)$ and for every affine open $\mathrm{U}$ of $\mathrm{X}$, $\Gamma(\mathrm{U},\sh{O}_{\mathrm{X}})$ is a torsion-free A-module, hence flat since $\mathrm{A}$ is a discrete valuation ring $(\mathbf{0}$, $6.3.4)$.
It is clear that $f_\eta:\mathrm{X}_\eta\to\mathrm{Y}_\eta$ is an isomorphism, but $f_s$ is not an isomorphism, although $f_s:\mathrm{X}_s\to\mathrm{Y}_s$ is not reduced to a single point for the closed point $z$, $f''$ is not an isomorphism.

(ii)
The conclusion of \sref{IV.21.12.15} also fails if, under the hypotheses, we suppress the hypothesis that $f$ is \emph{proper}.
It suffices for this to see to define $\mathrm{S}$ and $\mathrm{Y}$ as in (i), but to replace $\mathrm{X}$ by the complement $\mathrm{X}_2'$ of $\mathrm{Z}_2$ in the prescheme $\mathrm{X}$ defined in (i), $f$ by the restriction $f':\mathrm{X}'\to\mathrm{Y}$ of $f$; the morphism $g':\mathrm{X}'\to\mathrm{S}$, restriction of $g$, is again flat, and this time $\mathrm{X}_2'$ is geometrically irreducible; $\mathrm{X}_2'$ is moreover an affine scheme isomorphic to the complement of a closed point in $\mathbb{P}^1_k$; since its image under $f_s'$ is not reduced to the closed point $z$, $f'$ is not a proper morphism and $f_s'$ is not an isomorphism, although $f_\eta'$ is.
...
\begin{enumerate}[label=\arabic*\textsuperscript{o}]
  ...
  \]
  is \emph{bijective};
  \item the functor $\mathscr{L}\mapsto\mathscr{L}|(\mathrm{U}\cap\mathrm{V})$ is \emph{essentially surjective}, in other words, for every $\sh{O}_{\mathrm{U}\cap\mathrm{V}}$-Module invertible $\mathscr{L}_0$, there exists an $\sh{O}_{\mathrm{V}}$-Module invertible $\mathscr{L}$ such that $\mathscr{L}_0$ is
\oldpage[IV-4]{314}
...
\begin{enumerate}[label=\emph{\alph*)}]
  ...
  \item[\emph{b')}] The canonical homomorphism $\sh{O}_{\mathrm{X}}\to f_*(\sh{O}_{\mathrm{X}'})$ is a monomorphism (resp.\ an isomorphism) for every $\sh{O}_{\mathrm{V}}$-Module locally free of finite rank $\mathscr{E}$.
\end{enumerate}
...
\begin{enumerate}[label=(\roman*)]
  \item $f_*(\mathscr{E}')$ is an $\sh{O}_{\mathrm{X}}$-Module locally free;
  \item the canonical homomorphism $(\mathbf{0}_{\mathrm{I}}$, $4.4.3.3)$ $f^*(f_*(\mathscr{E}'))\to\mathscr{E}'$ is an isomorphism.
\end{enumerate}
When these two conditions are satisfied, $\mathscr{E}$ is isomorphic to $f_*(\mathscr{E}')$.
\end{lemma}

\begin{proof}
\oldpage[IV-4]{315}
To say that the functor $\mathscr{E}\mapsto(j_{\mathrm{V}})_*(\mathscr{E}|(\mathrm{U}\cap\mathrm{V}))$ is injective (resp.\ bijective) for every open $\mathrm{V}$ of $\mathrm{X}$, the canonical homomorphism $\mathscr{E}\to(j_{\mathrm{V}})_*(j_{\mathrm{V}}^*(\mathscr{E}))$ is injective (resp.\ bijective); since this must take place in replacing $\mathrm{V}$ by an open $\mathrm{W}\subset\mathrm{V}$ and $\mathscr{E}_1$, $\mathscr{E}_2$ by $\mathscr{E}_1|\mathrm{W}$, $\mathscr{E}_2|\mathrm{W}$, and since $\operatorname{Hom}_{\sh{O}_{\mathrm{V}}}(\mathscr{E}_1,\mathscr{E}_2|\mathrm{W})=\Gamma(\mathrm{W},\shHom_{\sh{O}_{\mathrm{V}}}(\mathscr{E}_1,\mathscr{E}_2))$, the condition also means that the canonical homomorphism of sheaves
\[
\label{IV.21.13.2.1}
...
But since $\mathscr{E}_1$ and $\mathscr{E}_2$ are locally free of finite rank, $\shHom_{\sh{O}_{\mathrm{V}}}(\mathscr{E}_1,\mathscr{E}_2)$, isomorphic to $\mathscr{E}_1\otimes_{\sh{O}_{\mathrm{V}}}\mathscr{E}_2$ $(\mathbf{0}_{\mathrm{I}}$, $5.4.2)$ is also locally free of finite rank; this already shows that $b)$ implies $a)$, and $b')$ is a particular case of $a')$.
It is clear that $a')$ is a particular case of $a)$.
Conversely, if $b)$ is a local property, it can, for the verification, be restricted to the case where $\mathscr{E}=\sh{O}_{\mathrm{V}}^n$, and this shows that $b')$ implies $b)$.

If the canonical homomorphism $\sh{O}_{\mathrm{X}}\to f_*(\sh{O}_{\mathrm{X}'})$ is bijective, and if the conditions (i) and (ii) are satisfied, it is clear that $\mathscr{E}'=f^*(\mathscr{E})$ with $\mathscr{E}=f_*(\mathscr{E}')$, up to canonical isomorphism.
\end{proof}
...
\label{IV.21.13.5}
Let $\mathrm{X}$ be a ringed space, $\mathrm{Y}$ a closed subset of $\mathrm{X}$, $\mathrm{U}=\mathrm{X}-\mathrm{Y}$, $j:\mathrm{U}\to\mathrm{X}$ the canonical injection.
For the pair $(\mathrm{X},\mathrm{Y})$ to be parafactorial, it is necessary and sufficient that the canonical homomorphism $\sh{O}_{\mathrm{X}}\to j_*(\sh{O}_{\mathrm{U}})$ is bijective, and that for every open $\mathrm{V}$ of $\mathrm{X}$ and every $\sh{O}_{\mathrm{U}\cap\mathrm{V}}$-Module invertible $\mathscr{L}$, $(j_{\mathrm{V}})_*(\mathscr{L})$ is an $\sh{O}_{\mathrm{V}}$-Module invertible (notation of \sref{IV.21.13.3}).
\end{proposition}
...
\begin{proof}
\oldpage[IV-4]{316}
(i) Since the fact that the canonical homomorphism $\sh{O}_{\mathrm{X}}\to j_*(\sh{O}_{\mathrm{U}})$ is bijective is a local property on $\mathrm{X}$, it all comes down to seeing (by virtue of \sref{IV.21.13.5}) by denoting by $j_\alpha$ the injection canonical $\mathrm{U}\cap\mathrm{W}_\alpha\to\mathrm{W}_\alpha$, that for all $\alpha$ and every $\sh{O}_{\mathrm{U}}$-Module invertible $\mathscr{L}$, $(j_\alpha)_*(\mathscr{L}|(\mathrm{U}\cap\mathrm{W}_\alpha))$ is a $\sh{O}_{\mathrm{W}_\alpha}$-Module invertible.
But the property of being an $\sh{O}_{\mathrm{X}}$-Module invertible is local on $\mathrm{X}$; since $j_*(\mathscr{L})|\mathrm{W}_\alpha=(j_\alpha)_*(\mathscr{L}|(\mathrm{U}\cap\mathrm{W}_\alpha))$, this proves our assertion.

(ii) Set $\mathrm{U}'=\mathrm{X}-\mathrm{Y}'$ and let $j'':\mathrm{U}'\to\mathrm{X}$ be the canonical injection.
To say that the canonical homomorphism $\sh{O}_{\mathrm{X}}\to j''_*(\sh{O}_{\mathrm{U}'})$ is bijective amounts to saying that for every open $\mathrm{V}$ of $\mathrm{X}$, the homomorphism $\Gamma(\mathrm{V},\sh{O}_{\mathrm{X}})\to\Gamma(\mathrm{V}\cap\mathrm{U}',\sh{O}_{\mathrm{X}})$ is bijective; but the homomorphism composed
\[
...
\]
is bijective by hypothesis (by replacing $\mathrm{V}$ by $\mathrm{V}\cap\mathrm{U}'$, we see that $\Gamma(\mathrm{V}\cap\mathrm{U}',\sh{O}_{\mathrm{X}})\to\Gamma(\mathrm{V}\cap\mathrm{U},\sh{O}_{\mathrm{X}})$ is bijective.
Noting then that if $j'':\mathrm{U}'\to\mathrm{X}$ is the canonical injection and if $\mathscr{L}'$ is an $\sh{O}_{\mathrm{U}}$-Module invertible, then $j_*''(\mathscr{L}')|\mathrm{U}$ is an $\sh{O}_{\mathrm{X}}$-Module invertible.
If we denote by $\mathscr{L}_1$ the $\sh{O}_{\mathrm{V}_1}$-Module invertible equal to $\mathscr{L}''$ in $\mathrm{W}$ and to $\mathscr{L}'$ in $\mathrm{V}\cap\mathrm{U}'$, on concludes from the fact that $\sh{O}_{\mathrm{X}}\to j_*(\sh{O}_{\mathrm{U}})$ is bijective and from \sref{IV.21.13.2},~$b)$) that $j_*(\mathscr{L})|V_1$ is an $\sh{O}_{\mathrm{X}}$-Module invertible.
It suffices then to replace $\mathrm{X}$ by an open $\mathrm{V}$ and $\mathrm{X}'$ by $f^{-1}(\mathrm{V})$ in what precedes.

(iii) Set $\mathrm{U}'=\mathrm{X}'-\mathrm{Y}'=f^{-1}(\mathrm{U})$ and note that since it is a question of preschemes, on can write $\mathrm{U}'=\mathrm{U}\times_{\mathrm{X}}\mathrm{X}'$; let $f_{\mathrm{U}}:f^{-1}(\mathrm{U})\to\mathrm{U}$ be the restriction of $f$ and let $j':\mathrm{U}'\to\mathrm{X}'$ be the canonical injection, which we also write $j_{(\mathrm{X}')}$.
Let us first show that the canonical homomorphism $\rho:\sh{O}_{\mathrm{X}}\to j_*(\sh{O}_{\mathrm{U}})$ is bijective; for this, it is necessary that for every open $\mathrm{V}$ of $\mathrm{X}$, the canonical homomorphism $\sh{O}_{\mathrm{X}',x'}\to j'_*(\sh{O}_{\mathrm{U}'})$ is bijective; since $j$ is quasi-compact and separated by hypothesis, the morphism $f$ is quasi-compact and separated, hence by \sref{IV.2.3.1}, $\sh{O}_{\mathrm{X}'}=f^*(\sh{O}_{\mathrm{X}})$, we see that $\rho$ is bijective since $f^*(\rho)$ is bijective and $f$ is faithfully flat \sref{IV.2.2.7}.
Let then $\mathscr{L}'=f_{\mathrm{U}}^*(\mathscr{L})$ be a $\sh{O}_{\mathrm{U}}$-Module invertible $\mathscr{L}$.
By hypothesis $j'_*(\mathscr{L}')=j'_*(f_{\mathrm{U}}^*(\mathscr{L}))$ is an $\sh{O}_{\mathrm{X}'}$-Module invertible, which is isomorphic to $f^*(j_*(\mathscr{L}))$ by \sref{IV.2.3.1} as $\sh{O}_{\mathrm{X}'}=f^*(\sh{O}_{\mathrm{X}})$; we conclude then that $\rho$ is bijective since $f$ is faithfully flat.
Since $f$ is faithfully flat and quasi-compact, this implies that $j_*(\mathscr{L})$ is an $\sh{O}_{\mathrm{X}}$-Module locally free \sref{IV.2.5.2}.
For the proof, it suffices to replace $\mathrm{X}$ by an open $\mathrm{V}$ and $\mathrm{X}'$ by $f^{-1}(\mathrm{V})$ in what precedes.
\end{proof}
...
\begin{proof}
Indeed, the only open of $\mathrm{X}$ containing $a$ is $\mathrm{X}$ itself, and hence every $\sh{O}_{\mathrm{X}}$-Module invertible is isomorphic to $\sh{O}_{\mathrm{X}}$, i.e.\ $\operatorname{Pic}(\mathrm{X})=0$; the first assertion follows therefore from the definition \sref{IV.21.13.1} and from \sref{IV.21.13.3}.
...
For every $z\in\mathfrak{J}$, set $\varphi(z)=1+z$; since $(1+z)(1-z)=1$, $\varphi$ is the kernel of the surjective canonical homomorphism of groups of multiplicative groups $\mathrm{A}^*\to\mathrm{B}^*$; in other words, we have an exact sequence of commutative groups
\[
...
\]
denoting by $t_0$ the image of $t$ in $\mathrm{B}$, since $\mathrm{B}_t=\mathrm{A}_t/\mathfrak{J}_t$; in other words, we have an exact sequence of sheaves of commutative groups on the topological space $\mathrm{X}$
\[
...
\[
\label{IV.21.13.9.1}
  \mathrm{H}^1(\mathrm{U},\mathscr{J})\to\mathrm{H}^1(\mathrm{U},\sh{O}_{\mathrm{U}}^*)\xrightarrow{u}\mathrm{H}^1(\mathrm{U},\sh{O}_{\mathrm{U}_0}^*).
  \tag{21.13.9.1}
\]
But since $\mathfrak{J}$ is a $\mathrm{B}$-module isomorphic to $\mathrm{B}$, we have $\mathrm{H}^1(\mathrm{U}_0,\sh{O}_{\mathrm{U}_0})=\mathrm{H}^1(\mathrm{U}_0,\sh{O}_{\mathrm{U}_0})$, and it follows from the chap.~III, $3^{\text{e}}$ partie (see also \cite[III, Exemple~III-1]{IV-41}) that we have
\oldpage[IV-4]{318}
...
Let indeed $\mathrm{B}$ be a local Noetherian ring, complete, integral, integrally closed, of dimension $\geq 2$ and not factorial (for example the completion of the localised polynomial algebra $k[\mathrm{U},\mathrm{V},\mathrm{W}]/(\mathrm{U}^2-\mathrm{U}\mathrm{V})$ at the ideal image of $(\mathrm{U})+(\mathrm{V})+(\mathrm{W})$).
...
(v) One can show that an intersection complete local ring of absolute dimension \sref{IV.19.3.1} of dimension $\geq 4$, is parafactorial (cf.\ \cite[XI, $3.13$~(i)]{IV-41}).
...
One can show that, for an arbitrary local Noetherian ring $\mathrm{A}$ of dimension $2$ to be factorial and not parafactorial, it is necessary and sufficient that the following three conditions be satisfied:
\begin{enumerate}[label=\arabic*\textsuperscript{o}]
  ...
  \item Let $\mathfrak{J}$ be the conductor of $\mathrm{A}$ in $\mathrm{A}'$, or equivalently the largest ideal of $\mathrm{A}'$ contained in $\mathrm{A}$); set $\mathrm{B}=\mathrm{A}/\mathfrak{J}$, $\mathrm{B}'=\mathrm{A}'/\mathfrak{J}$; then $\dim(\mathrm{B})=1$ (which implies $\mathrm{A}'\neq\mathrm{A}$, i.e.\ $\mathrm{A}$ is not integrally closed), and the canonical map $\operatorname{Div}(\mathrm{B})\to\operatorname{Div}(\mathrm{B}')$ \sref{IV.21.4.5} is \emph{surjective}.
\end{enumerate}
One can furthermore show that these conditions imply the following property: $4^{\mathrm{o}}$ the ring $\mathrm{B}'$ is also reduced, and the morphism $\operatorname{Spec}(\mathrm{B}')\to\operatorname{Spec}(\mathrm{B})$ is bijective.

If we set $\mathrm{X}=\operatorname{Spec}(\mathrm{A})$, $\mathrm{X}'=\operatorname{Spec}(\mathrm{A}')$, $\mathrm{Y}=\operatorname{Spec}(\mathrm{B})$, $\mathrm{Y}'=\operatorname{Spec}(\mathrm{B}')$, so that $\mathrm{Y}$ (resp.\ $\mathrm{Y}'$) is defined by the ideal $\mathfrak{J}$ of $\mathrm{A}$ (resp.\ $\mathrm{A}'$), the structural morphism $f:\mathrm{X}'\to\mathrm{X}$ is an isomorphism on $\mathrm{X}'-\mathrm{Y}'$ over $\mathrm{X}-\mathrm{Y}$ since $f$ is \emph{bijective} and in other words, $\mathrm{X}$ is a \emph{local} Noetherian \emph{unibranch} prescheme (and in particular $\mathrm{A}'$ is a \emph{local} Noetherian ring); in general $\mathrm{X}$ is not geometrically unibranch.
The space $\mathrm{Y}$, of dimension $1$, is formed of the closed point $x$ of $\mathrm{X}$ and the maximal points $y_i$ ($1\leq i\leq r$) of $\mathrm{Y}$, and the unique point $y_i'$ of $\mathrm{Y}'$ above $y_i$ is also a maximal point of $\mathrm{Y}'$; since $\mathrm{B}$ and $\mathrm{B}'$ are reduced, we deduce from this that $\mathfrak{m}_{\sh{O}_{\mathrm{X}',y_i'}}=\mathfrak{m}_{y_i}$; for $z=y_i$ and $z'$ the unique point of $f^{-1}(z)$, hence for all $z\in\mathrm{U}=\mathrm{X}-(x)$ and all $z'$ the unique point of $f^{-1}(z)$; this relation is also verified obviously for $z\notin\mathrm{Y}$ and $z'$ the unique point of $f^{-1}(z)$ (which is not characteristic $0$ \sref{IV.21.1.1}) on $\mathrm{U}$ (but \emph{not} \'etale in general).
\end{env}

\oldpage[IV-4]{319}
We will now limit ourselves to demonstrating that conditions $1^{\mathrm{o}}$, $2^{\mathrm{o}}$, $3^{\mathrm{o}}$ above are \emph{sufficient} for $\mathrm{A}$ to be parafactorial.
Since $\mathrm{A}'$ is factorial by virtue of condition $1^{\mathrm{o}}$ and \sref{IV.21.13.8}, it suffices to show that $\operatorname{Pic}(\mathrm{U})=0$.
Since $\mathrm{A}'$ is factorial, by virtue of $2^{\mathrm{o}}$, we obtain and we deduce from \sref{IV.21.8.5},~(ii) an exact sequence
\[
  1\to(\prod_{s\in\mathrm{S}}\sh{O}_{\mathrm{U},s}^*/\operatorname{Im}\Gamma(\mathrm{U}',\sh{O}_{\mathrm{U}'}^*))\to\operatorname{Pic}(\mathrm{U})\to\operatorname{Pic}(\mathrm{U}')\to 0
\]
and it is a matter of showing that the second term of this exact sequence is reduced to the neutral element, i.e.\ for the set $y_i$ ($1\leq i\leq r$) of $\mathrm{A}$ and $\mathrm{A}'$ corresponding to points $y_i$, $y_i'$, and we note that $\mathfrak{p}_i\cap\mathrm{A}=\mathfrak{p}_i$; denoting for each $i$ an element inversible $a_i'/s_i$ of $\mathrm{A}_{\mathfrak{p}_i}'$, with $a_i'\in\mathrm{A}'$, $s_i\notin\mathfrak{p}_i$; since we have only to examine that the quotient groups $\mathrm{A}_{\mathfrak{p}_i}^*/\mathrm{A}_{\mathfrak{p}_i}^*$ are the same; we can suppose $s_i=1$ for all $i$; let $b_i'\in\mathrm{B}_i'$ be the canonical image of $a_i'$, so that $b_i'/1$ is an element $\neq 0$ of $\boldsymbol{k}(y_i')=\sh{O}_{v',y_i'}$.
The open $\mathrm{U}\cap\mathrm{Y}$ formed by the $y_i$ is an open affine of the form $\mathrm{D}(t)$ in $\mathrm{Y}$, with $t\in\mathfrak{m}$, the maximal ideal of $\mathrm{A}$; it exists therefore an invertible element $b'\in\mathrm{B}'$ such that the canonical images of $b_i'/1$ are the images in $\mathrm{A}_{\mathfrak{p}_i}'/\mathrm{A}_{\mathfrak{p}_i}$ are the same, which finishes the proof.
...
Taking into account the preceding remark, it all amounts to seeing that if $\mathrm{T}_y=\operatorname{Spec}(\sh{O}_{\mathrm{X},y})$ and $\mathrm{U}_y=\mathrm{T}_y-\{y\}$, every $\sh{O}_{\mathrm{U}_y}$-Module invertible $\mathscr{L}_0$ is isomorphic to $\sh{O}_{\mathrm{U}_y}$.
Now, when $\mathrm{V}$ ranges over the open neighbourhoods of $y$ in $\mathrm{X}$, it follows from \sref{IV.8.2.13} and \sref{IV.1.2.4.2} that the prescheme $\mathrm{U}_y$ is projective limit of the preschemes induced by $\mathrm{X}$ on the opens $\mathrm{V}-(\mathrm{V}\cap\overline{\{y\}})$, which are quasi-compact since $\mathrm{X}$ is Noetherian.
Since the $\mathrm{U}_y=\mathrm{V}-(\mathrm{V}\cap\overline{\{y\}})$ are separated, it follows then from \sref{IV.8.5.2},~(ii) and \sref{IV.8.5.5} that there is a neighbourhood open affine $\mathrm{V}$ of $y$ in $\mathrm{X}$ and an $\sh{O}_{\mathrm{V}}$-Module invertible $\mathscr{L}_{\mathrm{V}}'$ such that the sheaf induced by $j_*(\mathscr{L})$ on $\mathrm{U}_y$ is $\mathscr{L}_0$.
But $\sh{O}_{\mathrm{X},y}$ is parafactorial, so the pair $(\mathrm{V},\mathrm{V}\cap\overline{\{y\}})$ is parafactorial by \sref{IV.21.13.6},~(i)), so there exists an $\sh{O}_{\mathrm{V}}$-Module invertible $\mathscr{L}_{\mathrm{V}}$ inducing $\mathscr{L}_{\mathrm{V}}'$ on $\mathrm{U}_y$.
But as $\mathrm{T}_y$ is projective limit, by replacing $\mathrm{V}$ by a smaller neighbourhood open of $y$, one can suppose that $\mathscr{L}_{\mathrm{V}}$ is isomorphic to $\sh{O}_{\mathrm{V}}$, whence $\mathscr{L}_0$ is isomorphic to $\sh{O}_{\mathrm{U}_y}$, hence invertible.
...
This is evidently reduced, taking into account the remark at the beginning, to showing that for every $\sh{O}_{\mathrm{U}}$-Module invertible $\mathscr{L}$, $j_*(\mathscr{L})$ is an $\sh{O}_{\mathrm{X}}$-Module invertible.
The set of $x\in\mathrm{X}$ such that the restriction of $j_*(\mathscr{L})$ to a neighbourhood open of $x$ in $\mathrm{X}$ is invertible is evidently open in $\mathrm{X}$; it is a matter of showing that $\mathrm{V}=\mathrm{X}$.
...
Replacing $\mathrm{X}$ if needed by a neighbourhood open of $y$, we can suppose that the restriction of $j_*(\mathscr{L})$ to $\mathrm{W}-(\mathrm{W}\cap\overline{\{y\}})$ is equal.
If we set $\mathrm{V}_1=\mathrm{V}\cup\mathrm{W}$ and if we denote by $\mathscr{L}_1$ the $\sh{O}_{\mathrm{V}_1}$-Module equal to $j_*(\mathscr{L})|W$ in $\mathrm{W}$, $\mathscr{L}_{\mathrm{V}}$ in $\mathrm{V}$, we have $\mathrm{U}\subset\mathrm{V}_1$ and $\mathscr{L}_1|\mathrm{U}=\mathscr{L}$.
We conclude from the fact that $\sh{O}_{\mathrm{X}}\to j_*(\sh{O}_{\mathrm{U}})$ is bijective and from \sref{IV.21.13.2},~$b)$) that $j_*(\mathscr{L})|\mathrm{V}_1$ is an $\sh{O}_{\mathrm{X}}$-Module invertible.
But this contradicts the definition of $\mathrm{V}$, and concludes the proof of \sref{IV.21.13.10}.
\end{proof}
...
\begin{proof}
We already know that $\mathrm{A}$ is integral and integrally closed \sref{IV.6.5.4}, so we can restrict ourselves by induction on $n=\dim(\mathrm{A})$.
...
The local rings $\sh{O}_{\mathrm{X}',x'}$ at points $x'\in f^{-1}(a)$ are factorial and of dimension $\geq 2$ \sref{IV.6.1.2}, hence parafactorial \sref{IV.21.13.9},~(ii)); we conclude \sref{IV.21.13.10} that the pair $(\mathrm{X}',f^{-1}(a))$ is parafactorial.
...
When $\mathscr{G}$ is a sheaf of commutative groups, by introducing the cohomology sheaves $\mathscr{H}^i_{\mathrm{Y}}(\mathscr{G})$ defined in chap.~III, $3^{\text{e}}$ partie, saying that $(\mathrm{X},\mathrm{Y})$ is $i$-pure for $i\leq 2$ means that $\mathscr{H}^i_{\mathrm{Y}}(\mathscr{G})=0$ for $p\leq i$; under this form, the notion generalises immediately for every integer $i$.
\end{remark}
...
\begin{proof}
\oldpage[IV-4]{322}
It is clear that the set $\mathrm{S}'$ of $\mathrm{X}$ generisations of points of $\mathrm{S}$ is the intersection of the open neighbourhoods of $\mathrm{S}$.
We can restrict to the case where $\mathrm{X}$ is Noetherian (the properties $a)$ and $b)$ being local on $\mathrm{X}$); as every $1$-codimensional cycle on $\mathrm{X}$ which is principal at the points of $\mathrm{S}$ is also principal at the points of a neighbourhood open of $\mathrm{S}$, we see that condition $a)$ of \sref{IV.21.13.15} amounts to condition $a)$ of \sref{IV.21.13.14} applied to the family $(\mathrm{U}_\lambda)$ of open neighbourhoods of $\mathrm{S}$.
The corollary follows then from the equivalence of $a)$ and $b)$ in \sref{IV.21.13.14}.

Let us note on the other hand that $b')$ implies $b)$ by virtue of \sref{IV.21.13.10}; conversely, if $b')$ is satisfied and if $x\notin\mathrm{U}_\lambda$, then $\mathrm{Y}=\{x\}$ is contained in $\mathrm{X}-\mathrm{U}_\lambda$ and one has $\operatorname{codim}(\mathrm{Y},\mathrm{X})\geq 2$, hence $\sh{O}_{\mathrm{X},x}$ is parafactorial, which proves the equivalence of $a')$ and $b')$.

Let us prove that $a)$ implies $c)$, i.e.\ $a')$ implies $b'')$.
First we prove that $a'')$ implies $b'')$.
Let us note first that $b')$ implies $b)$ by virtue of \sref{IV.21.13.10}; conversely, if $b')$ is satisfied and if $x\notin\mathrm{U}_\lambda$, then $\mathrm{Y}=\{x\}$ is contained in $\mathrm{X}-\mathrm{U}_\lambda$ and one has $\operatorname{codim}(\mathrm{Y},\mathrm{X})\geq 2$; if we note that $\mathrm{U}=\mathrm{X}-\mathrm{Y}$, $\sh{O}_{\mathrm{X}}\to j_*(\sh{O}_{\mathrm{U}})$ is bijective \sref{IV.21.13.2}, hence condition $a)$ of \sref{IV.21.13.15} amounts to the condition $a)$ of \sref{IV.21.13.14}.

$a'')$ For every $1$-codimensional cycle $\mathrm{Z}$ on $\mathrm{X}$ which is principal at the points of $\mathrm{S}$ of $\mathrm{X}$, $\mathrm{Z}$ is locally principal.

$b'')$ The canonical homomorphism $\operatorname{Pic}(\mathrm{X})\to\operatorname{Pic}(\mathrm{U})$ is surjective.

Let us first prove that $a)$ implies $c)$, since if the $1$-codimensional cycle $\mathrm{Z}$ on $\mathrm{X}$ has its support in one of the sets $\mathrm{X}-\mathrm{U}_\lambda$, $\mathrm{Z}|\mathrm{U}_\lambda=0$, hence $\mathrm{Z}|\mathrm{U}_\lambda=\operatorname{cyc}(\mathrm{D}_\lambda)$, where $\mathrm{D}_\lambda$ is a divisor on $\mathrm{U}_\lambda$; since $\mathrm{X}$ is normal, $\mathrm{Z}|\mathrm{U}_\lambda$ is locally principal by \sref{IV.21.6.10},~(i), and the hypothesis implies that $\mathrm{Z}$ is locally principal on $\mathrm{X}$.

Inversely $c)$ implies $a)$, since if $\mathrm{Z}$ is a $1$-codimensional cycle on $\mathrm{X}$ such that $\mathrm{Z}|\mathrm{U}_\lambda$ is locally principal, the image of $\mathrm{Z}|\mathrm{U}$ in $\mathfrak{Cl}(\mathrm{U})$ is therefore the image of a unique element $\zeta_0\in\operatorname{Pic}(\mathrm{U})$ \sref{IV.21.6.11}; it follows from \sref{IV.21.6.11} that the image of $\mathrm{Z}$ in $\mathfrak{Cl}(\mathrm{X})$ is the image of a unique element $\zeta\in\operatorname{Pic}(\mathrm{X})$, and it is clear then that $\zeta_0$ is the image of $\zeta$.
\end{proof}
...
\begin{enumerate}[label=\emph{\alph*)}]
  ...
  \oldpage[IV-4]{323}
  \item For all $x\in\mathrm{X}$ which is not a generisation of a point of $\mathrm{S}$ (in other words, such that $\{x\}\cap\mathrm{S}=\varnothing$) and such that $\dim(\sh{O}_{\mathrm{X},x})\geq 2$, the local ring $\sh{O}_{\mathrm{X},x}$ is parafactorial.
\end{enumerate}
\end{corollary}
...
If $\mathrm{D}_\mu=\operatorname{div}(f_\mu)$, $f_\mu$ is a regular rational function on $\mathrm{U}_\mu$; the hypothesis implies (by virtue of \sref{IV.21.3.4}) that there is a set $\mathrm{U}_\mu\subset\mathrm{U}_\lambda$ such that $\mathrm{D}_\mu|\mathrm{U}_\mu=\mathrm{D}_\lambda|\mathrm{U}_\mu$ is equivalent to $0$.
...
The condition $\varinjlim\operatorname{Pic}(\mathrm{U}_\lambda)=0$ will in particular be satisfied if $(\mathrm{U}_\lambda)$ is the family of open neighbourhoods of a point $z\in\mathrm{X}$, since then $\mathrm{U}_\lambda$ is isomorphic to $\sh{O}_{\mathrm{U}_\lambda}$; there exists $\mathrm{U}_\mu\subset\mathrm{U}_\lambda$ such that $\mathscr{L}_\lambda|\mathrm{U}_\mu\cong\sh{O}_{\mathrm{U}_\mu}$.
...
If moreover $\operatorname{Pic}(\mathrm{X})=0$, condition $a)$ of \sref{IV.21.13.15} is also equivalent to the following:
\begin{enumerate}
  \item[c)] Every $1$-codimensional cycle on $\mathrm{X}$ whose support is contained in one of the sets $\mathrm{X}-\mathrm{U}_\lambda$ is locally principal.
\end{enumerate}
\end{remark}
...
Let $\mathrm{B}$ be a local Noetherian ring and $\rho:\mathrm{A}\to\mathrm{B}$ a local homomorphism making $\mathrm{B}$ a smooth A-algebra (for the preadic topologies $(\mathbf{0}$, $19.3.1)$) and such that the residue field of $\mathrm{B}$ is finite over that of $\mathrm{A}$.
...
\begin{proof}
\oldpage[IV-4]{324}
If $k=\mathrm{A}/\mathfrak{m}$ is the residue field, $\mathrm{B}/\mathfrak{m}\mathrm{B}$ is a formally smooth $k$-algebra (for the preadic topologies), and in particular integral, in other words $\mathfrak{p}=\mathfrak{m}\mathrm{B}$ is indeed a prime ideal of $\mathrm{B}$, which justifies the statement.
...
Let $k'$ be the residue field of $\widehat{\mathrm{B}}$, a finite extension of $k$; there is a local homomorphism $\widehat{\mathrm{A}}\to\mathrm{C}$, where $\mathrm{C}$ is a local Noetherian ring which is an $\widehat{\mathrm{A}}$-module \emph{finite} (and \emph{flat}) and $\mathrm{C}/\mathfrak{m}\mathrm{C}$ isomorphic to $k'$ $(\mathbf{0}_{\mathrm{III}}$, $10.3.1)$; we conclude then from \sref{IV.7.5.1}, \sref{IV.7.5.3} and \sref{IV.6.5.4},~(ii) that $\mathrm{C}$ is \emph{integral} and \emph{integrally closed}.
Furthermore, $\mathrm{D}=\widehat{\mathrm{B}}\otimes_{\bb{Z}}\mathrm{C}$ is a semi-local complete ring, a direct sum of complete local rings whose one, $\mathrm{D}_0$, has for residue field $k'$ (since $k'\otimes_k k'$ is isomorphic to $k'$).
Since $\mathrm{D}$ is formally smooth over $\mathrm{C}$ for the preadic topologies; by $\operatorname{D}_0/\mathrm{m}\mathrm{D}_0$ is a $k'$-algebra formally smooth of residue field $k'$, which implies that it is a $k'$-isomorphic algebra of power series $k'[[\mathrm{T}_1,\ldots,\mathrm{T}_n]]$ $(\mathbf{0}$, $19.6.4)$; we conclude, by $(\mathbf{0}$, $19.7.5)$, that $\mathrm{D}_0$ is an algebra of formal power series $\mathrm{C}[[\mathrm{T}_1,\ldots,\mathrm{T}_n]]$, and hence \emph{integral and integrally closed} (Bourbaki, \emph{Alg.\ comm.}, chap.~V, \textsection1, n$^{\mathrm{o}}$~4, prop.~14).
Since the morphisms $\operatorname{Spec}(\mathrm{D}_0)\to\operatorname{Spec}(\widehat{\mathrm{B}})$ and $\operatorname{Spec}(\mathrm{D}_0)\to\operatorname{Spec}(\mathrm{C})$ are faithfully flat, we deduce that $\mathrm{B}$ and $\widehat{\mathrm{B}}$ are also \emph{integral and integrally closed}.
\oldpage[IV-4]{325}
We will thus prove the theorem in the particular case where $\mathrm{A}$ is \emph{complete} and $\mathrm{B}=\mathrm{A}[[\mathrm{T}_1,\ldots,\mathrm{T}_n]]$.
Let us first show that we can reduce to the case where $n=1$.
Indeed, we proceed by induction on $n$, and let $f\in\mathfrak{q}$ be an element not belonging to $\mathfrak{p}=\mathfrak{m}\mathrm{B}$; it is a formal power series whose reduced series $\overline{f}_0$ is nonzero; hence, by virtue of the preparation theorem (Bourbaki, \emph{Alg.\ comm.}, chap.~VII, \textsection3, n$^{\mathrm{o}}$~8, prop.~5), for every $f\in\mathfrak{q}$, there exists $g\in\mathrm{B}$ and a polynomial $r\in\mathrm{A}[\mathrm{T}]$ such that $f=gf_0+r$ and one has thus $r\in\mathfrak{q}\cap\mathrm{A}[\mathrm{T}]$; on the other hand (\emph{loc.\ cit.}, prop.~6) there exists a distinguished non-constant polynomial $\mathrm{F}_0\in\mathrm{A}[\mathrm{T}]$ and an invertible element $u\in\mathrm{B}$ such that $f_0=u\mathrm{F}_0$, hence also $\mathrm{F}_0\in\mathfrak{q}\cap\mathrm{A}[\mathrm{T}]$; since $\mathrm{B}$ is flat over $\mathrm{A}[\mathrm{T}]$ $(\mathbf{0}_{\mathrm{I}}$, $7.3.3)$, it follows from Bourbaki, \emph{Alg.\ comm.}, chap.~VII, \textsection1, n$^{\mathrm{o}}$~10, prop.~15, that $\mathfrak{q}_1$ is a prime ideal of height $1$ in $\mathrm{A}[\mathrm{T}]$.
But on the other hand, we have necessarily $\mathfrak{q}_1\cap\mathrm{A}=0$; otherwise, $\mathfrak{q}_1\cap\mathrm{A}$ would necessarily be of height $\geq 1$, and it would follow from \sref{IV.5.5.3} that $\mathfrak{q}_1=(\mathfrak{q}_1\cap\mathrm{A})\mathrm{A}[\mathrm{T}]$.
But then, since $\mathfrak{q}_1\subset\mathfrak{m}\mathrm{A}[\mathrm{T}]$, we would have $\mathfrak{q}_1\subset\mathfrak{m}\mathrm{A}[\mathrm{T}]$, contradicting the hypothesis on $\mathfrak{q}$.
If $\mathrm{K}$ is the field of fractions of $\mathrm{A}$, $\mathfrak{q}_1\mathrm{K}[\mathrm{T}]$ is therefore a prime ideal distinct from $0$ and of $\mathrm{K}[\mathrm{T}]$, hence of the form $h\cdot\mathrm{K}[\mathrm{T}]$, where $h(\mathrm{T})=\mathrm{T}^m+a_1\mathrm{T}^{m-1}+\ldots+a_m$ with $m\geq 1$ and the $a_i\in\mathrm{K}$, $h$ being irreducible in $\mathrm{K}[\mathrm{T}]$.
But we have seen above that there exists in $\mathfrak{q}_1$ a distinguished non-constant $\mathrm{F}_0\in\mathrm{A}[\mathrm{T}]$.
$t$ is a root of $\mathrm{F}_0$ in an extension of $\mathrm{K}$ and hence divides $\mathrm{F}_0$ \emph{in} $\mathrm{K}[\mathrm{T}]$; but since $h$ and $\mathrm{F}_0$ are unitary, the coefficients $a_i$ of $h$ are integers over $\mathrm{A}$ (Bourbaki, \emph{Alg.\ comm.}, chap.~V, \textsection1, n$^{\mathrm{o}}$~3, prop.~11), hence belong to $\mathrm{A}$ since $\mathrm{A}$ is integrally closed.

The statement \sref{IV.21.14.1} is equivalent to the following:
\end{proof}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item Let $\mathrm{Y}$ be a closed subset of $\mathrm{X}$ containing no irreducible component of any fibre $\mathrm{X}_s$, $\mathrm{Y}_\eta=\mathrm{Y}\cap\mathrm{X}_\eta$ is of codimension $\geq 2$ in $\mathrm{X}_\eta$.
  Then the pair $(\mathrm{X},\mathrm{Y})$ is parafactorial.
\end{enumerate}
\end{proposition}
...
To prove \sref{IV.21.14.3}, let us note first that under the hypotheses of (i), if we set $\mathrm{Y}=\overline{\{x\}}$, it suffices to verify the conditions of (ii); and we have $\mathrm{Y}\cap\mathrm{X}_s$ is of codimension $\geq 2$ in $\mathrm{X}_s$ for every $s$ by the hypotheses on $x$.

For the proof of (i), we first show that $\sh{O}_{\mathrm{X},x}$ is parafactorial; since $\mathrm{X}$ is normal \sref{IV.17.5.7}, hence $\mathrm{A}$ is integral and integrally closed, and since $\sh{O}_{\mathrm{X},x}$ is a formally smooth $\sh{O}_{\mathrm{S},s}$-algebra for the preadic topologies \sref{IV.17.5.3}; since $\mathrm{Y}_s$ contains no irreducible component of $\mathrm{X}_s$, and since $\mathrm{Y}_s$ is of codimension $\geq 2$ in $\mathrm{X}_s$, the prime ideal of $\sh{O}_{\mathrm{X},x}$ corresponding to the point $x$ is not contained in $\mathfrak{m}_s\sh{O}_{\mathrm{X},x}$ since $\mathrm{Y}$ contains no irreducible component of $\mathrm{X}_s$.
Furthermore, $\dim(\sh{O}_{\mathrm{X},x})\geq\dim(\sh{O}_{\mathrm{S},s})+2\geq 2$, so we can apply \sref{IV.21.14.2}.

For the proof of (ii), since the conditions and the conclusion are local on $\mathrm{S}$ and on $\mathrm{X}$ by virtue of \sref{IV.21.13.6},~(i), we can restrict to the case where $\mathrm{S}$ and $\mathrm{X}$ are affine, $f$ is therefore of finite presentation.
Since the fibres of $f$ are regular, the hypothesis on $\mathrm{Y}$ implies that, for every point $x\in\mathrm{Y}$, $\operatorname{prof}(\sh{O}_{\mathrm{X}_{f(s)},x})=\dim(\sh{O}_{\mathrm{X}_{f(s)},x})\geq 1$; it follows from \sref{IV.19.9.8} that the canonical homomorphism $\sh{O}_{\mathrm{X}}\to j_*(\sh{O}_{\mathrm{U}})$ is \emph{injective}.
Furthermore, replacing $\mathrm{Y}$ by a larger closed subset following the method of the proof of \sref{IV.19.9.8}, and using \sref{IV.21.13.6},~(ii), we can suppose that $\mathrm{Y}$ is defined by an ideal of \emph{finite type} of the Picard scheme of $\mathrm{X}$, hence the open $\mathrm{U}=\mathrm{X}-\mathrm{Y}$ is \emph{quasi-compact and quasi-separated} and the closed set $\mathrm{Y}$ is \emph{constructible}.

To prove (ii), it suffices, for every $\sh{O}_{\mathrm{U}}$-Module invertible $\mathscr{L}$ and every point $x\in\mathrm{Y}$, to establish the existence of a neighbourhood open $\mathrm{V}$ of $x$ in $\mathrm{X}$ such that: $1^{\mathrm{o}}$ the canonical homomorphism $\sh{O}_{\mathrm{V}}\to(j_{\mathrm{V}})_*(\sh{O}_{\mathrm{U}\cap\mathrm{V}})$ is surjective; $2^{\mathrm{o}}$ there exists an $\sh{O}_{\mathrm{V}}$-Module invertible $\mathscr{L}_{\mathrm{V}}'$ such that $\mathscr{L}_{\mathrm{V}}'|\mathrm{U}\cap\mathrm{V}=\mathscr{L}|(\mathrm{U}\cap\mathrm{V})$ (and \sref{IV.21.13.3}).

Set $s=f(x)$; we will see that we can restrict to the case where $\mathrm{S}=\mathrm{S}_0=\operatorname{Spec}(\sh{O}_{\mathrm{S},s})$.
Let $(S_\nu)$ be a fundamental system of affine open neighbourhoods of $s$ in $\mathrm{S}$, and set $\mathrm{X}_\nu=f^{-1}(\mathrm{S}_\nu)$, $\mathrm{Y}_\nu=\mathrm{Y}\cap\mathrm{X}_\nu$, $\mathrm{U}_\nu=\mathrm{X}_\nu-\mathrm{Y}_\nu$, $j_\nu:\mathrm{U}_\nu\to\mathrm{X}_\nu$ being the canonical injection; $\mathrm{X}_0=\mathrm{X}\times_{\mathrm{S}}\mathrm{S}_0$ is projective limit of the $\mathrm{X}_\nu$ and, denoting by $\mathrm{Y}_0=\mathrm{Y}\cap\mathrm{X}_0$ and $\mathrm{U}_0=\mathrm{X}_0-\mathrm{Y}_0$, $j_0:\mathrm{U}_0\to\mathrm{X}_0$ the canonical injection, $p_\nu:\mathrm{X}_0\to\mathrm{X}_\nu$ being the affine morphism, one has $j_*(\sh{O}_{\mathrm{U}})=p_\nu^*((j_\nu)_*(\sh{O}_{\mathrm{U}_\nu}))$ $(\mathbf{II}$, $1.5.2)$, and the canonical homomorphism $\rho:\sh{O}_{\mathrm{X}}\to j_*(\sh{O}_{\mathrm{U}})$ is none other that $p_\nu^*(\rho_\nu)$, where $\rho_\nu$ is the canonical homomorphism.
Since by hypothesis $\rho_0$ is surjective, there exists a sufficiently large $\nu$ and $\mathscr{L}_\nu$ such that $\mathscr{L}_0=p_\nu^*(\mathscr{L}_\nu)$ is invertible. The proof finishes like that of the proposition, when $\mathrm{A}$ is a \emph{local} ring: since $\mathrm{X}$ is normal and $f$ smooth, $\mathrm{S}$ is normal \sref{IV.17.5.7}, hence $\mathrm{A}$ is integral and integrally closed \sref{IV.17.5.7}; since the local rings $\sh{O}_{\mathrm{S},s}$ are excellent integrally closed rings $(\mathbf{0}$, $7.8.3$,~(ii)), (iii) and (vi)), $\mathrm{A}$ is an inductive limit of its sub-$\bb{Z}$-algebras \emph{of finite type}; by virtue of \sref{IV.1.8.4.2}, there exists an index $\lambda$ and an $\mathrm{A}_\lambda$-algebra of finite type $\mathrm{B}_\lambda$ such that $\mathrm{B}=\mathrm{B}_\lambda\otimes_{\mathrm{A}_\lambda}\mathrm{A}$.
By virtue of \sref{IV.1.8.4.2}, there exists an index $\lambda$ and an $\mathrm{A}_\lambda$-isomorphism near.
Set $\mathrm{S}_\lambda=\operatorname{Spec}(\mathrm{A}_\lambda)$, $\mathrm{X}_\lambda=\operatorname{Spec}(\mathrm{B}_\lambda)$; if $\rho_\lambda:\mathrm{X}\to\mathrm{X}_\lambda$ is the canonical projection, one has $j_*(\sh{O}_{\mathrm{U}})=\rho_\lambda^*((j_\lambda)_*(\sh{O}_{\mathrm{U}_\lambda}))$; it all amounts to proving that $\mathrm{U}=\mathrm{X}-\mathrm{Y}$ is retrocompact in $\mathrm{X}$.
Since the fibres of $f$ are regular, the hypothesis on $\mathrm{Y}$ implies that for every point $x\in\mathrm{Y}$, $\sh{O}_{\mathrm{X},x}$ is parafactorial by (i), and the result then follows from \sref{IV.21.13.10}.
\end{proof}
...
The statement with this generality would be equivalent, by virtue of \sref{IV.21.13.15} and \sref{IV.21.13.12.1} to the following: let $\mathrm{A}$ be a local Noetherian ring, complete, integral, integrally closed, $\mathrm{B}$ an arbitrary Noetherian ring which is a formally smooth A-algebra, such that $\dim(\mathrm{B})>\dim(\mathrm{A})$; then $\mathrm{B}$ is parafactorial.

(ii) We will see in chap.~VI, employing a technique of ``finite descent'' applied to \sref{IV.21.14.1} (or to \sref{IV.21.14.2}), that the conclusion of \sref{IV.21.14.1} holds valid by replacing the hypothesis that $\widehat{\mathrm{A}}$ is \emph{reduced}.
If we replace this last condition by $\dim\mathrm{B}\geq\dim\mathrm{A}+3$, we can even suppress the hypothesis that $\widehat{\mathrm{A}}$ is reduced.
...
(iii) In chap.~III, $3^{\text{e}}$ partie, we prove that if $f:\mathrm{X}\to\mathrm{S}$ is a smooth morphism, $\mathrm{Y}$ a closed locally constructible subset of $\mathrm{X}$ such that, for all $s\in\mathrm{S}$, with the notations of \sref{IV.21.14.3} $\operatorname{codim}(\mathrm{Y}_s,\mathrm{X}_s)\geq 3$, then the pair $(\mathrm{X},\mathrm{Y})$ is parafactorial.
...
For example, let $k$ be a field, $\mathrm{A}=k[\mathrm{T}]/(\mathrm{T}^2)$, $\mathrm{S}=\operatorname{Spec}(\mathrm{A})$ ($\mathrm{T}_1$, $\mathrm{T}_2$ indeterminates), so that $\mathrm{X}=\mathbb{V}_{\mathrm{S}}^3$, $\mathrm{Y}$ being the ``zero section''; if $s$ is the unique closed point of $\mathrm{S}$, on a $\mathrm{X}_s=\mathbb{V}_k^2$ and $\mathrm{Y}_s$ is the ``zero section'' of $\mathrm{X}_s$.
...
Let $\mathrm{S}$, $\mathrm{T}$ be two preschemes locally of finite type over a field $k$, $\mathrm{Z}$ a $1$-codimensional cycle on $\mathrm{X}$ (considered as S-prescheme); suppose the following conditions are satisfied:
\begin{enumerate}[label=\arabic*\textsuperscript{o}]
  \item $\mathrm{S}$ and $\mathrm{T}$ are geometrically normal over $k$ \sref{IV.6.7.6};
  \item For every maximal point $\eta$ of $\mathrm{S}$, the $1$-codimensional cycle $\mathrm{Z}_\eta$ on the fibre $\mathrm{X}_\eta$, having even multiplicity that $\mathrm{Z}$ at every point of $\mathrm{X}^{(1)}\cap\mathrm{X}_\eta$, is locally principal (in other words, is the image of a divisor of $\mathrm{X}_\eta$, since $\mathrm{X}_\eta$ is normal);
...
In other words, $\mathrm{X}$ being normal \sref{IV.6.14.1}, every $1$-codimensional cycle $\mathrm{Z}$ on $\mathrm{X}$ such that for every maximal $x\in\mathrm{X}$ which is not maximal in its fibre $\mathrm{X}_s$ and which does not belong to any of the ``generic fibres'' $\mathrm{X}_\eta$ (which implies $\dim(\sh{O}_{\mathrm{X},x})\geq 2$ by virtue of \sref{IV.6.1.1}), the local ring $\sh{O}_{\mathrm{X},x}$ is parafactorial, by virtue of \sref{IV.21.12.15} ${}^{(1)}$.
\end{env}
...
Let $\mathscr{M}_{\mathrm{X}/\mathrm{S}}^*$ be the sheaf (in multiplicative groups) of germs of invertible sections of $\mathscr{M}_{\mathrm{X}}^*$ which is thus a sub-sheaf of $\mathscr{M}_{\mathrm{X}}^*$ and contains $\sh{O}_{\mathrm{X}}^*$ as sub-sheaf.
\end{env}
...
\label{IV.21.15.4}
Let $\mathrm{D}$ be a divisor on $\mathrm{X}$, $\mathscr{J}_{\mathrm{X}}(\mathrm{D})$ the ideal fractionnaire invertible corresponding \sref{IV.21.2.8} and $\sref{IV.21.2.9}$.
...
The interpretation of the divisors on $\mathrm{X}$ by means of the classes $\operatorname{cl}(\mathscr{L},s)$ \sref{IV.21.2.11} allows us therefore to interpret the elements of $\operatorname{Div}(\mathrm{X}/\mathrm{S})$ as the pairs $(\mathscr{L},s)$ (up to isomorphism) such that $\mathscr{L}$ is an $\sh{O}_{\mathrm{X}}$-Module invertible and that $s$ is a meromorphic section of $\mathscr{L}$ regular relative to $\mathrm{S}$ \sref{IV.20.6.5},~(iii)).

\begin{proposition}[21.15.6]
\label{IV.21.15.6}
Let $\mathrm{D}$ be a divisor on $\mathrm{X}$ relative to $\mathrm{S}$, and suppose that for all $x\in\mathrm{X}$ such that $\operatorname{prof}(\sh{O}_{\mathrm{X}_{f(s)},x})=1$, we have $\mathrm{D}_x\geq 0$ (resp.\ $\mathrm{D}_x=0$).
Then $\mathrm{D}\geq 0$ (resp.\ $\mathrm{D}=0$).
\end{proposition}

\begin{proof}
As in \sref{IV.21.1.8}, we can restrict to the case where $\mathrm{D}=\operatorname{div}(\varphi)$, where $\varphi$ is a meromorphic regular function relative to $\mathrm{S}$, and it all amounts to seeing that if $\mathrm{D}_x\geq 0$ everywhere then $\varphi$ is everywhere defined in $\mathrm{X}$.
But this hypothesis means that, if $\mathrm{T}=\mathrm{X}-\operatorname{dom}(\varphi)$, we have $\operatorname{prof}(\sh{O}_{\mathrm{X}_{f(s)},x})\geq 2$ for all $x\in\mathrm{T}$, and it suffices to conclude by applying \sref{IV.20.6.6}.
\end{proof}
...
We deduce then from \sref{IV.21.5.2} and \sref{IV.20.6.1} that for every open $\mathrm{U}$ of $\mathrm{X}$ and every section $t'\in\Gamma(g^{-1}(\mathrm{U}),\mathscr{T}_{\mathrm{X}'/\mathrm{S}})$, the norm $\mathrm{N}_{\mathrm{X}'/\mathrm{X}}(t')$ belongs to $\Gamma(\mathrm{U},\mathscr{T}_{\mathrm{X}/\mathrm{S}})$; the reasoning of \sref{IV.21.5.3} proves then that for every $\sh{O}_{\mathrm{X}'}$-Module invertible $\mathscr{L}'$ and every meromorphic section $u'$ of $\mathscr{L}'$ above $\mathrm{S}$, the norm $\mathscr{L}=\mathrm{N}_{\mathrm{X}'/\mathrm{X}}(\mathscr{L}')$ is a meromorphic section of $\mathrm{X}$, regular relative to $\mathrm{S}$.
The interpretation of the direct image of divisors relatives to $\mathrm{S}$ given in \sref{IV.21.15.5} and the definition of a direct image of divisors \sref{IV.21.5.5} prove then that \emph{for every divisor} $\mathrm{D}'\in\operatorname{Div}(\mathrm{X}'/\mathrm{S})$, we have $g_*(\mathrm{D}')\in\operatorname{Div}(\mathrm{X}/\mathrm{S})$.
\end{env}
