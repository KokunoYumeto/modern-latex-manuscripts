...
{\Large\bfseries La conjecture de Weil pour les surfaces K3}\\[1.5em]
...
Soit $X$ un schéma, séparé et de type fini, sur $\Fp$, et soit $\overline X$ le schéma sur $\overline{\F}_q$ qui s'en déduit par extension des scalaires. Pour tout point fermé $x$ de $X$, soit $\deg(x)=[k(x):\Fp]$ le degré sur $\Fp$ de l'extension résiduelle. La fonction zêta $Z(X,t)\in\Z[[t]]$ est définie par
\[
...
\emph{(ii) On a $H^2(X,T_X^1)=0$ (les déformations infinitésimales ne sont pas obstruées), l'application déduite du produit intérieur
\[
\tag{1.1.1}
...
\item \emph{Il existe un anneau de valuation discrète complet d'inégale caractéristique $V$, de corps résiduel fini $k$, un morphisme propre et lisse $f_V:Y\to\Spec(V)$ de fibre générale une surface K3 et $i:\Fp\to k$ tel que $Y_0\otimes_{\Fp}k\simeq Y\otimes_V k$.}
\item \emph{Il existe un schéma intègre $S$ de corps des fractions de caractéristique $0$, un point $s\in S$, un morphisme propre et lisse $f:Y\to S$ de fibre générale une surface K3, et $i:\Fp\to k(s)$ tel que $Y_0\otimes_{\Fp}k(s)\simeq Y_s$.}
\end{enumerate}
...
\paragraph{1.3. Théorème.} \emph{Une surface projective non singulière sur un corps fini $\Fp$ qui se relève en une surface K3 vérifie la conjecture de Weil.}

Ce théorème a été obtenu indépendamment par Pjateckii-Shapiro et Safarevitch. Il s'applique par exemple aux surfaces de degré $4$ dans $\mathbb P^3$. Les nombres de Betti d'une K3 sont $(1,0,22,0,1)$; le théorème implique donc que
\[
...
\paragraph{2.1.} Soient $\underline S$ le groupe algébrique réel des éléments inversibles de la $\R$-algèbre $\C$, $w:\mathbb G_{m,\R}\to\underline S$ l'inclusion de $\R^*$ dans $\C^*$, et $t:\underline S\to\mathbb G_{m,\R}$ l'inverse de la norme; on a $tw(x)=x^{-2}$. Soit $V$ un vectoriel réel de dimension finie. Il revient au même de se donner:
\begin{enumerate}[label=\alph*)]
\item $\rho:\underline S\to\GL(V)$ de poids $n$: $\rho w(x)*v=x^n v$;
...
\]
On pose $C=\rho(i)$: c'est l'opérateur $C$ de Weil. On a $Cv^{p,q}=i^{p-q}v^{p,q}$. Pour $\mathscr S$ une partie de $\Z\times\Z$, une bigraduation de Hodge est dite de type $\mathscr S$ si les nombres de Hodge $h^{pq}=\dim V^{p,q}$ sont nuls pour $(p,q)\notin\mathscr S$.

\paragraph{2.2.} On appellera ici structure de Hodge de poids $n$ $H$ la donnée d'un $\Z$-module libre de type fini $H_\Z$ et d'une bigraduation de Hodge de poids $n$ de $H_\C=H_\Z\otimes\C$. La structure de Hodge de Tate $\Z(n)$ est la structure de Hodge de type $\{(-n,-n)\}$ donnée par
\[
\Z(n)_\C=\C,
\qquad
...
Une structure de Hodge de poids $n$ rationnelle consiste en un $\Q$-vectoriel $H_\Q$ et une bigraduation de Hodge de poids $n$ de $H_\C=H_\Q\otimes\C$. On désigne par $\Q(n)$ la structure de Hodge rationnelle sous-jacente à $\Z(n)$. La définition des polarisations est laissée au lecteur.
...
De la fin de ce numéro, nous n'utiliserons que $2.11(iii')\Rightarrow(ii)$. Pour prouver cette implication, on peut se limiter à ne considérer que des groupes connexes, ce qui rend 2.5 inutile ou évident.

\paragraph{2.4.} Soit $G$ un groupe algébrique linéaire réel. On dit que $G$ est compact si $G(\R)$ est compact et que toute composante connexe de $G(\C)$ a un point réel. Le foncteur évident est alors une équivalence de la catégorie des représentations linéaires algébriques de $G$ avec la catégorie des représentations linéaires du groupe compact $G(\R)$.
...
\item \emph{pour toute représentation linéaire réelle, respectivement complexe, de $G$, il existe une forme bilinéaire, respectivement sesquilinéaire, symétrique, respectivement hermitienne, définie positive et $G$-invariante;}
...
\paragraph{2.7.} Soit $G$ un groupe algébrique linéaire réel. Pour tout automorphisme involutif $\sigma$ de $G$, soit $G^{(\sigma)}$ la forme réelle de $G_\C$ définie par l'involution antilinéaire $g\mapsto\sigma(\bar g)$. On dit que $\sigma$ est une involution de Cartan si $G^{(\sigma)}$ est compact.

Soit $C$ un élément de $G(\R)$ de carré central. Une représentation réelle, respectivement complexe, de $G$ sera dite $C$-polarisable s'il existe sur $V$ une forme bilinéaire, respectivement sesquilinéaire, $G$-invariante $\psi$ telle que la forme $\psi(x,Cy)$ soit symétrique, respectivement hermitienne, et définie positive. On dit alors que $\psi$ est une $C$-polarisation de $V$. Un cas particulier du lemme suivant est énoncé sans démonstration dans [4].
...
\item \emph{toute représentation réelle, respectivement complexe, de $G$ est $C$-polarisable;}
\item \emph{$G$ admet une représentation réelle, respectivement complexe, $C$-polarisable fidèle.}
\end{enumerate}
\emph{Si $G$ est connexe, ces conditions équivalent encore à}
\begin{enumerate}[label=(iii\textquotesingle)]
\item \emph{$G$ admet une représentation réelle, respectivement complexe, $C$-polarisable de noyau fini.}
\end{enumerate}
...
\]
(remplacer $y$ par $Cy$), c'est-à-dire à la $G^{(\ad C)}$-invariance de $\psi(x,Cy)$. La forme hermitienne de 2.8 résulte donc de 2.6. La forme non hermitienne s'en déduit:
\begin{enumerate}[label=\alph*)]
\item la complexifiée, respectivement la réellifiée, d'une représentation réelle, respectivement complexe, $C$-polarisable est $C$-polarisable;
\item une polarisation de $V$ induit une polarisation de toute sous-représentation de $V$;
\item pour $V$ réel, respectivement complexe, on a $V\into V\otimes_\R\C$.
\end{enumerate}
...
Une représentation définie sur $\Q$, $V_\Q$, de $G$ est dite homogène de poids $n$ si $w(x)*v=x^n v$. Toute représentation est somme de représentations homogènes. Soit $V_\Q$ une représentation de poids $n$. Alors $h$ munit $V_\Q$ d'une structure de Hodge rationnelle de poids $n$.

On regardera $\Q(n)_\Q$ comme une représentation de $G$, par la règle
\[
g*x=t(g)^n x.
\]
Cette convention est raisonnable, car $\underline S$ agit sur $\Q(n)_\R$ par la même formule. Une polarisation d'une représentation de poids $n$ $V_\Q$ de $G$ est une forme $G$-invariante
\[
...
Soit $P_\Q$, respectivement $P_\R$, l'espace des formes bilinéaires $(-1)^n$-symétriques $G$-invariantes sur $V_\Q$, respectivement $V_\R$. On a $P_\R=\R\otimes P_\Q$. Les $h(i)$-polarisations forment un ouvert de $P_\R$, et les polarisations de $V_\Q$ forment la trace de cet ouvert sur $P_\Q$. Le lemme en résulte.
...
Pour les rappels contenus dans ce numéro, on pourra consulter [3].
...
\paragraph{3.2.} Supposons que $A$ soit un corps de caractéristique $0$, et que $Q$ soit non dégénérée. On note $\CSpin(V)$ le groupe de Clifford. C'est le groupe algébrique des éléments inversibles $g$ de $C^+(V)$ tels que $gVg^{-1}=V$. On définit un morphisme de $\CSpin(V)$ dans $\SO(V)$ par
\[
...
1) $\dim(V)$ impair: $C^+(V)$ est ici une algèbre de matrices. Soit $W$ un $C^+(V)$-module simple; $W$ est une représentation spinorielle de $\CSpin(V)$:
\[
g*w=\alpha(g)\cdot w.
\]
...
2) $\dim(V)$ pair: $C^+(V)$ est ici produit de deux algèbres de matrices. Soient $W_1$ et $W_2$ deux $C^+(V)$-modules simples non isomorphes et soit $W=W_1+W_2$; $W_1$ et $W_2$ sont les représentations semi-spinorielles. On a des isomorphismes de représentations
\[
\tag{3.4.3}
(C^+(V))_s=\text{somme de copies de }W,
\]
...
Il suffit de traiter le cas où $A$ est un corps algébriquement clos et où $\Gamma$ est le groupe $\Spin$ tout entier; en effet, commuter avec $a$ est une condition Zariski-fermée. Avec les notations de 3.4, identifions $C^+(V)$ à une sous-algèbre de $\End_A(W)$.
...
1) $\dim(V)$ impair: ici, $C^+(V)=\End_A(W)$, et $\det_W((\alpha(\gamma),\bar a))=1$, de sorte que $\alpha(\gamma)\bar a\alpha(\gamma)^{-1}\bar a^{-1}$ est une racine de l'unité $\nu$. Puisque $\Spin(V)$ est connexe, et que, pour $\gamma=e$, on a $\nu=1$, on a même
\[
(\alpha(\gamma),\bar a)=1,
\qquad\text{i.e.}\qquad
...
2) $\dim(V)$ pair: ici, $C^+(V)=\End_A(W_1)\times\End_A(W_2)$ et soit $\bar a$ permute les $W_i$, soit il les respecte. Dans les deux cas, on déduit de ce que
\[
...
\paragraph{4.3. Lemme.} Relativement à $\tilde h$, toute représentation de $\CSpin(V_\Q)$ est polarisable (2.9).

On applique le critère 2.11 b)(iii') à la représentation $V_\Q$ de $\CSpin(V_\Q)$.

\paragraph{4.4. Lemme.} La structure de Hodge de $C^+(V_\Q)_{\mathrm{ad}}$ définie par $\tilde h$ est de type $\{(-1,1),(0,0),(1,-1)\}$.
...
\paragraph{4.5. Proposition.} La structure de Hodge de $C^+(V_\Q)_s$ définie par $\tilde h$ est de type $\{(0,1),(1,0)\}$. La structure de Hodge $C^+(V_\Q)_s$, munie du réseau $C^+(V_\Z)$, définit donc une variété abélienne.

Supposons tout d'abord $n$ impair. Après extension des scalaires à $\C$, $C^+(V_\Q)$ devient une algèbre de matrices
\[
C^+(V_\C)\simeq\End(W).
\]
Sur $W$, $\CSpin(V)_\C$ agit par $g\cdot w=\alpha(g)w$: c'est la représentation spinorielle. La représentation $C^+(V_\Q)_s$ est somme de $\dim(W)$ copies de $W$, de sorte qu'il suffit de prouver que la représentation de poids un $W$ est purement de type $\{(0,1),(1,0)\}$. Sinon, la représentation $\End(W)\simeq C^+(V_\C)_{\mathrm{ad}}$ ne pouvait pas être purement de type $\{(-1,1),(0,0),(1,-1)\}$.

Pour $n$ pair, on a de même
\[
C^+(V_\C)\simeq\End(W')\oplus\End(W''),
\]
où $W'$ et $W''$ sont les représentations semi-spinorielles. Comme plus haut, on prouve que $W'$ et $W''$ sont de type $\{(0,1),(1,0)\}$, donc aussi $C^+(V_\Q)_s$.

La seconde assertion résulte de 2.3 et 3.3.

\paragraph{4.6.} La construction donnée dans ce numéro cache le fait, dont nous ne ferons pas usage, suivant, qui se lit sur le diagramme de Dynkin et est la vraie raison pour laquelle tout marche.
...
{\draw (\x,0.08)--(\x,-0.08); \node at (\x,-0.55) {$\a$};}
\node at (4.1,0) {$\cdots$};
\node at (11.9,0) {pour $\SO(2,2\ell-1)$,};
\end{tikzpicture}
\]
\[
\begin{tikzpicture}[baseline={(current bounding box.center)},scale=0.9]
...
\node at (12.0,0) {pour $\SO(2,2\ell-2)$.};
\end{tikzpicture}
\]
...
\paragraph{5.1. Théorème (A. Borel).} Soit $X/\Gamma$ le quotient d'un domaine hermitien symétrique par un groupe arithmétique sans torsion. On sait [2] que $X/\Gamma$ est de façon naturelle une variété algébrique quasi-projective. Soient $S$ un schéma réduit sur $\C$ et $f:S^{an}\to X/\Gamma$ un morphisme d'espaces analytiques. Alors $f$ est algébrique.

Soit $S$ lisse sur $\C$. Pour la notion de variation de structures de Hodge $\underline H$ sur $S$, et celle de polarisation de $\underline H$, on renvoie à Griffiths [6]. On notera $H_\Z$ le système local de $\Z$-modules libres sur $S$ défini par $\underline H$, et par $H_s$ la structure de Hodge fibre de $\underline H$ en $s\in S$.

Pour $X$ le demi-espace de Siegel, le théorème 5.1 a le corollaire suivant, qui généralise 2.3.

\paragraph{5.2. Scholie.} Le foncteur $(a:A\to S)\mapsto R^1a_*\Z$ identifie les schémas abéliens polarisés $A$ sur $S$ aux variations de structures de Hodge polarisées de type $\{(0,1),(1,0)\}$ sur $S$.
...
\]
\item si $\underline H$ est une variation de structures de Hodge de poids $0$, et $\psi$ une polarisation de $\underline H$, ou un quelconque morphisme de structures de Hodge symétrique $\underline H\otimes\underline H\to\Z$, on dispose de $C(\underline H,\psi)$ et $C^+(\underline H,\psi)$. En tout point $t\in S$, on a avec les notations de 3.4
\[
C^+(\underline H,\psi)_t=C^+(H_t,\psi_t).
\]
\end{enumerate}

\paragraph{5.4.} Soit $(V_\Z,B)$ comme en 4.1, et soit $X$ l'ensemble des $h:\underline S\to\SO(V_\R)$ du type considéré en 4.1. Se donner un tel $h$ revient à se donner un sous-espace $V^-_\R$ de dimension $2$ de $V_\R$, sur lequel $B$ est défini négatif, et une orientation de $V^-_\R$: pour $z=r e^{i\theta}\in\C^*$, $h(z)$ induit l'identité sur l'orthogonal de $V^-_\R$ et la rotation d'angle $2\theta$ sur $V^-_\R$. Le groupe $\mathrm O(V_\R)$ agit sur $X$ par transport de structure, $(g*h)(z)=g h(z)g^{-1}$, et la description ci-dessus rend géométriquement évident que $X$ est un espace homogène sous $\SO(V_\R)$. Le stabilisateur de $h\in X$ est un sous-groupe $\SO(2)\times\SO(n)$, composante neutre d'un sous-groupe compact maximal.
...
\paragraph{5.6.} Soit $W_\Q$ une représentation de poids $n$ du groupe algébrique $\CSpin(V_\Q)$: $w(x)y=x^n y$. Soit $W_\Q$ le système local constant sur $X$, de fibre $W_\Q$. Le groupe $\CSpin(V_\Q)$ agit sur $(X,W_\Q)$ par
\[
\gamma*(w\text{ en }h\in X)=(\gamma w\text{ en }\gamma h\gamma^{-1}\in X).
\]
Chaque $h\in X$ définit $h:\underline S\to\CSpin(V_\R)$, d'où une $\Q$-structure de Hodge sur la fibre de $W_\Q$ en $h\in X$. On obtient ainsi une variation de $\Q$-structures de Hodge de poids $n$ sur $X$, encore notée $W_\Q$:
\begin{enumerate}[label=\alph*)]
\item la filtration de Hodge est fonction holomorphe de $h$;
\item l'axiome de transversalité est vérifié, et nous ne l'utiliserons que dans un cas trivial.
\end{enumerate}
Cette construction est compatible au produit tensoriel. En particulier, une polarisation $\psi:W_\Q\otimes W_\Q\to\Q(-n)$ de la représentation $W_\Q$ définit une polarisation de $W_\Q$.

Soient $W_\Z$ un réseau entier dans $W_\Q$ et $\Gamma$ un sous-groupe de $\CSpin(V_\Q)$, discret dans $\CSpin(V_\R)$, agissant librement sur $X$ et tel que $\Gamma W_\Z=W_\Z$. Alors le système local constant $W_\Z$ sur $X$, de fibre $W_\Z$, est sous-jacent à une variation de structure de Hodge $\Gamma$-équivariante $W$ sur $X$, comme plus haut. Par passage au quotient, elle définit une variation de structures de Hodge $W$ sur $X/\Gamma$.

Si $W_\Q$ est une représentation de $\SO(V_\Q)$, la même construction s'applique aux sous-groupes discrets de $\SO(V_\Q)$.

\paragraph{5.7. Proposition.} Soit $(\underline H,\psi)$ une variation de structures de Hodge polarisée de type $\{(-1,1),(0,0),(1,-1)\}$, avec $h^{1,-1}=1$, sur un schéma lisse et connexe $S$. Il existe:
\begin{enumerate}[label=\alph*)]
\item un revêtement fini étale surjectif $u:S_1\to S$;
\item un schéma abélien $A$ sur $S_1$;
\item une $\Z$-algèbre $C$, et $\mu:C\to\End_{S_1}(A)$;
\item un isomorphisme de variations de structures de Hodge
\[
...
Il existe un revêtement fini étale $v:S'_1\to S$ tel que le système local $v^*H_{\Z/n\Z}$ soit trivial. Choisissons un isomorphisme $\sigma_n$ entre ce système local et le système local constant $V_{\Z/n\Z}$. Soient $s\in S'_1$ et $\sigma:V_\Z\iso H_{s\Z}$ une isométrie telle que $\sigma=\sigma_n\pmod n$. La structure de Hodge $h_s$ de $H_s$ définit $\sigma^{-1}(h_s)\in X$. L'image de $\sigma^{-1}(h_s)$ dans $X/\Gamma$ ne dépend que de $s$. On définit ainsi
\[
...
\begin{enumerate}[label=\alph*)]
\item un isomorphisme $(vp_1)^*\underline H\simeq (mp_1)^*\underline V$;
...
Soit $C$ l'algèbre $C^+(V_\Z)$. Le schéma abélien construit en b) est à multiplication complexe par $C$, et la variation de structures de Hodge $\End_C(R^1a_*\Z)$ est
\[
...
\paragraph{6.2.} Une famille de surfaces $K3$ paramétrée par un schéma $S$ de caractéristique $0$ est un morphisme propre et plat $f:Y\to S$ dont les fibres sont des surfaces $K3$. Une polarisation de $f:Y\to S$ est une section $\eta$ de $\Pic_S(Y)$, qui en chaque $s\in S$ est une polarisation de $Y_s=f^{-1}(s)$.
...
\]
Pour tout $s\in S$, la fibre $(P^2f_*\Z(1))_s$ est la partie primitive de la cohomologie $H^2(Y_s,\Z)$. La structure de Hodge de $(P^2f_*\Z(1))_s$ est de type $\{(-1,1),(0,0),(1,-1)\}$, avec $h^{-1,1}=1$ ((1.1)(i)). L'action de $\pi_1(S,s)$ sur $H^2(Y_s,\Z)$ induit
\[
\tag{6.3.2}
...
\paragraph{6.4. Proposition.} Pour toute surface $K3$ polarisée complexe $Y_0$, il existe une famille de surfaces $K3$ polarisées $f:Y\to S$ comme plus haut, avec $Y_s\simeq Y_0$ pour un $s\in S$, et telle que l'image de $\pi$ (6.3.2) soit d'indice fini.

Soit $f^\wedge:Y^\wedge\to T^\wedge$ la déformation formelle verselle de $Y_0$. Puisque $H^0(Y_0,T^1)=0$, $T^\wedge$ est même un schéma formel de modules pour $Y_0$, mais peu importe. Puisque $H^2(Y_0,T^1)=0$ ((1.1)(i)), $T^\wedge$ est le spectre d'un anneau de séries formelles sur $\C$. De $f^\wedge$, on déduit une variation de structures de Hodge non polarisée sur $T^\wedge$. D'après (1.1.i), celle-ci est la déformation universelle de la structure de Hodge $H^2(Y_0,\Z)$ munie du cup-produit.

Soit $\tilde\eta$ l'image de $\eta$ par l'application composée
\[
H^2(Y_0,\Z)\longrightarrow H^2_{DR}(Y^\wedge/T^\wedge)\longrightarrow R^2f^\wedge_*\mathcal O.
\]
Soit $S^\wedge$ le sous-schéma formel de $T^\wedge$ d'équation $\tilde\eta=0$ et soit $f^\wedge:Y^\wedge\to S^\wedge$ induit par $f^\wedge$. Puisque $h^{0,2}=1$, $S^\wedge\subset T^\wedge$ est défini par une équation; celle-ci fait partie d'un système régulier de paramètres, de sorte que $S^\wedge$ est un anneau de séries formelles. Sur $S^\wedge$, $\eta$ provient d'un faisceau inversible $\mathcal O(1)$ ample sur $Y^\wedge$; $f^\wedge$ est donc algébrisable. En outre, sur $S^\wedge$, la variation de structure de Hodge polarisée
\[
(P^2(Y^\wedge/S^\wedge,\Z)(1),\psi)
\]
est une déformation universelle de $(P^2(Y_0,\Z)(1),\psi)$.
...
D'après Borel (5.1), $m$ est un morphisme de schémas. Le morphisme $m$ est dominant, car le composé $m^\wedge:S^\wedge\to X/\Gamma$ l'est. Il en résulte, voir par exemple P. Deligne, Théorie de Hodge II, Publ. Math. IHES 40, lemme 4.4.17, que $m(\pi_1(S,s))$ est d'indice fini dans
\[
\pi_1(X/\Gamma,m(s))=\Gamma,
\]
lui-même d'indice fini dans le groupe orthogonal.

\paragraph{6.5. Proposition.} Soit $Y_0$ une surface $K3$ polarisée sur un corps $K$ de caractéristique $0$. Il existe:
\begin{enumerate}[label=\alph*)]
\item un schéma $S$ de type fini sur $\Q$, et une famille $f:Y\to S$ de surfaces $K3$ polarisées, paramétrée par $S$;
\item une extension finie $K'$ de $K$ et $v:\Spec(K')\to S$ tel que $v^*Y$ soit isomorphe à $Y_0\otimes_K K'$;
\item un schéma abélien $a:A\to S$ sur $S$;
\item une $\Z$-algèbre $C$ et $\mu:C\to\End_S(A)$;
\item un isomorphisme de $\Z_\ell$-faisceaux d'algèbres sur $S$
\[
u:C^+(P^2f_*\Z_\ell(1),\psi_\ell)\iso \End_C(R^1a_*\Z_\ell).
\]
\end{enumerate}

On se ramène à supposer $K$ de type fini sur $\Q$. Choisissons un plongement complexe $\sigma:K\to\C$ et appliquons 6.4 à $Y_0\otimes_{K,\sigma}\C$ pour obtenir une famille de surfaces $K3$ polarisées $f_1:Y_1\to S_1$. Appliquons 5.7 à $f_1$; on obtient une famille $f_2:Y_2\to S_2$, vérifiant encore la conclusion de 6.4, et, sur $S_2$, un schéma abélien $a_2:A_2\to S_2$ à multiplication complexe $\mu_2:C\to\End_{S_2}(A_2)$ et un isomorphisme
\[
...
\]
est défini par un nombre fini d'équations. Il existe donc une extension de type fini $K_3$ de $K$, avec $K\subset K_3\subset\C$, et, sur $K_3$, un système
\[
\Xi_3=(S_3;f_3,Y_3,a_3,A_3,\mu_3)
\]
qui, par extension des scalaires de $K_3$ à $\C$, fournit $\Xi_2$.

\paragraph{6.5.1. Lemme.} Il existe un unique isomorphisme de $\Z_\ell$-faisceaux d'algèbres
\[
...
Soit $T$ un schéma intègre de type fini sur $\Q$, de corps des fonctions $K_3$. Pour $T$ convenable, $S_3$ est la fibre générique d'un système
\[
\Xi=(S,f,Y,a,A,\mu)
\]
sur $T$, et $u_3$ se prolonge en
\[
u:C^+(P^2f_*\Z_\ell(1),\psi_\ell)\iso \End_C(R^1a_*\Z_\ell).
\]
On vérifie facilement l'existence de $K'$ comme en b), et ceci achève la démonstration.

\paragraph{6.6.} Prouvons le théorème 1.3. Soit donc $f_V:Y_V\to\Spec(V)$ comme en 1.2(i). Soit $K$ le corps des fractions de $V$, et appliquons 6.5 à la surface
\[
Y_K=Y\otimes_VK
\]
sur $K$, munie d'une quelconque polarisation. Quitte à remplacer $V$ par son normalisé dans une extension finie de $K$, on peut supposer qu'il existe
\[
\Xi=(S,f,Y,a,A,\mu,u)
\]
comme en 6.5, tel que $Y_K$ soit l'image réciproque de $Y$ par $v:\Spec(K)\to S$. Par image réciproque, on en déduit une variété abélienne à multiplication complexe $(A_K,\mu)$ sur $K$.
...
\End_C(H^1(A_{\overline k},\Z_\ell)).
\]
D'après la théorie de Weil pour $A_k$, les valeurs propres de Frobenius agissant sur
\[
C^+(P^2(Y_{\overline k},\Z_\ell)(1),\psi_\ell)
\]
sont des nombres algébriques dont tous les conjugués complexes sont de valeur absolue $1$. On a
\[
...
\[
(*)\quad
\text{La structure de Hodge de la représentation adjointe }\Lie(G)\text{ de }G
\text{ est purement de type }\{(-1,1),(0,0),(1,-1)\}.
\]

\paragraph{7.3. Proposition.} Si une structure de Hodge rationnelle $H$ s'exprime à l'aide de variétés abéliennes, alors la condition $(*)$ est vérifiée.
...
\item Šafarevič, I. R., Averbuch, B. C., Vaĭnberg, Ju. R., Južtchenko, A. B., Manin, Ju. I., Moishezon, B. C., Tjurina, C. I., Tjurin, A. I.: Algebraic surfaces. Trudi Mat. Inst. Steklov LXXV.
