...
% Source file: letters/Letter_Millson_1987.tex
% ------------------------------------------------------------
...
Nori brought me a copy of your paper with Goldman one deformations of flat bundles. It seems to me that it contains a number of mistakes, but also that, when corrected, it gives the strong results you ask for $p\,8\,l.\,10\,\grave{a}\,-7$. I begin with the latter.
...
The formal group $\widehat{\underline{L}}^0$ acts one $L^1$ (thanks to $[,]\colon L^0 \otimes L^1 \to L^1$), hence $\widehat{\underline{L}}^0(A)$ one $L^1 \otimes A$. one has also a map
\[
...
\noindent\textbf{To be checked:} for a square zero ideal $I$ in $A$, and an object $\omega$ one $A/I$, there is an obstruction in $H^2(L \otimes I)$ to extend $\omega$ to $A$; if the obstruction vanishes, isomorphism classes of extensions form a principal homogeneous space under $H^1(L \otimes I)$; automorphisms of any extension (trivial one $A/I$) are $H^0(L^* \otimes I)$. This is functorial in $L^*$, hence the fact that quasi-isomorphisms induce equivalences of fibred categories.

\vspace{0.3cm}

\noindent\textbf{Map here:} Let $X$ be an analytic variety and $(V,\nabla)$ be a (holomorphic) vector bundle with connection. The deformation problem is
\[
A \longmapsto \text{category of vector bundles with connection in the $X$-direction,}
\]
one $X \otimes \Sp(A)$, deforming $(V,\nabla)$.

It is given by the previous construction applied to $\Omega^{**}(\End V)$.

\newpage

\noindent\textbf{Rmq:} In your map, because you have the explicit quasi-isomorphism (1), you have an explicit equivalence of fibred categories between
...
\section*{II. Comments one your paper}
...
Hodge theory is available for variations of Hodge structures (complex variations, one compact K\"ahler manifolds). The useful bigrading of $\Omega^{**}(\End V)$ is obtained by mixing that of $\Omega^*$ and that of $\End V$. It is useful now that (1) is a filtered quasi-isomorphism. One gets:

\vspace{0.2cm}

\begin{itemize}[leftmargin=1cm]
\item[a)] The same result as before one deformations of a representation of $\pi_1$ given by a complex variation.
