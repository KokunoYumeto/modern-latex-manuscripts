\documentclass[10pt]{article}
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{lmodern}
\usepackage{amsmath,amssymb,mathtools}
\usepackage{geometry}
\usepackage{enumitem}
\geometry{margin=0.86in}
\setlength{\parindent}{1.15em}
\setlength{\parskip}{0.18em}
\emergencystretch=3em
\setlist{leftmargin=2em,itemsep=0.12em,topsep=0.2em}
\newcommand{\Z}{\mathbb Z}
\newcommand{\N}{\mathbb N}
\newcommand{\Q}{\mathbb Q}
\newcommand{\C}{\mathbb C}
\newcommand{\Fp}{\mathbb F_p}
\newcommand{\ord}{\operatorname{ord}}
\newcommand{\Stab}{\operatorname{Stab}}
\newcommand{\supp}{\operatorname{supp}}
\newcommand{\dd}{\mathrel{\doteq}}
\newcommand{\vfun}{v}

\newcommand{\og}{\guillemotleft\,}
\newcommand{\fg}{\,\guillemotright}

\newcommand{\nn}{\mathbf n}
\newcommand{\rr}{\mathbf r}
\newcommand{\ii}{\mathbf i}
\newcommand{\xx}{\mathbf X}
\newcommand{\monom}[1]{X^{#1}}
\begin{document}

\begin{center}
{\large Extrait de \og Mathematica \& Paedagogia\fg\ n\textsuperscript{o} 46}\\[2.4em]
{\Large\bfseries UNE CONGRUENCE ENTRE COEFFICIENTS MULTINOMIAUX}\\[1.0em]
par P. DELIGNE\\
{\itshape Institut des Hautes Études Scientifiques, Bures sur Yvette}
\end{center}

\medskip

Soit $p$ un nombre premier. Pour tout entier $m\ne0$, on désigne par $v(m)$ l'exposant de la plus haute puissance de $p$ qui divise $m$. On pose de plus $v(0)=\infty$.

Pour $\nn=(n_1,\ldots,n_k)$ un $k$-uple d'entiers $\ge0$, on pose
\[
|\nn|=\sum_{i=1}^k n_i,\qquad
a\nn=(an_1,\ldots,an_k),\qquad
X^\nn=\prod_{i=1}^k X_i^{n_i},\qquad
\nn! = \prod_{i=1}^k n_i!,
\]
et on désigne par $C(\nn)$ le coefficient du monôme $X^\nn$ dans
\[
\left(\sum_{i=1}^k X_i\right)^{|\nn|}
\]
(coefficients multinômiaux).

On a donc, pour tout entier $n\ge0$,
\[
\left(\sum_{i=1}^k X_i\right)^n
=\sum_{|\nn|=n} C(\nn)X^\nn, \tag{1}
\]
\[
C(\nn)=\frac{|\nn|!}{\nn!}. \tag{2}
\]
Pour $k=2$, ce sont là les coefficients binômiaux:
\[
C(n_1,n_2)=\binom{n_1+n_2}{n_1}. \tag{3}
\]

\paragraph{Proposition.} Soient $\nn$ un $k$-uple d'entiers $\ge0$, $n=|\nn|$, et $l\ge1$. Si $p$ est un nombre premier $\ge5$, on a
\[
C(p^l\nn)\equiv C(\nn)\pmod {p^{v(n)+3}}. \tag{4}
\]

\paragraph{Exemple.} Pour $p=5$, $k=2$, $l=1$ et $\nn=(1,1)$, on a
\[
\binom{10}{5}=252=\binom21+2\cdot5^3.
\]

\paragraph{Preuve.} On sait, ou on déduit de (2), que
\[
\left(\sum_{i=1}^k X_i\right)^{p^l}
=
\left(\sum_{i=1}^k X_i^{p^l}\right)+pQ, \tag{5}
\]
où $Q$ est un polynôme à coefficients entiers, de degré $<p^l$ en chaque variable. La formule du binôme fournit alors
\[
\left(\sum_{i=1}^k X_i\right)^{p^l n}
=
\left(\sum_{i=1}^k X_i^{p^l}\right)^n
+\sum_{i=1}^n p^i\binom ni R_i, \tag{6}
\]
avec
\[
R_i=
\left(\sum_{j=1}^k X_j^{p^l}\right)^{n-i}Q^i. \tag{7}
\]

Pour $A$ et $B$ deux polynômes à coefficients entiers, on écrira
\[
A\dd B \pmod r
\]
si les coefficients de $X^{p^l\nn}$ dans $A$ et $B$ sont congrus modulo $r$. Avec cette notation, (4) se réécrit
\[
\left(\sum_{i=1}^k X_i\right)^{p^l n}
\dd
\left(\sum_{i=1}^k X_i^{p^l}\right)^n
\pmod {p^{v(n)+3}}. \tag{8}
\]

\paragraph{Lemme 1.}
\[
v\binom ni \ge v(n)-v(i). \tag{9}
\]

Faisons agir le groupe additif $\Z/(n)$ sur lui-même par translation\footnote{À tout $x\in\Z/(n)$ est donc associée la permutation de $\Z/(n)$ qui applique $y$ sur $x+y$.} Si une partie à $i$ éléments $P$ de $\Z/(n)$ a pour stabilisateur\footnote{Si un groupe $G$ opère dans un ensemble $E$, le stabilisateur d'une partie $P$ de $E$ est l'ensemble des $g\in G$ pour lesquels $P$ est stable.} un sous-groupe $H$ de $\Z/(n)$, $H\cdot P=P$, alors $P$ est réunion de classes latérales de $H$, et $\#H$ divise $i$. Le cardinal
\[
r_P=\frac{n}{\#H}
\]
de l'orbite de $P$ sous $\Z/(n)$, dans $\mathcal P(\Z/(n))$, vérifie donc
\[
p^{v(n)-v(i)}\mid r_P.
\]
D'où, puisque l'ensemble des parties à $i$ éléments de $\Z/(n)$ est somme disjointe d'orbites,
\[
p^{v(n)-v(i)}\mid\binom ni.
\]

\paragraph{Lemme 2.} Pour $p\ne2,3$ et $i\ge3$, on a
\[
p^i\binom ni\equiv0\pmod {p^{v(n)+3}}. \tag{10}
\]

On a
\[
v\left(p^i\binom ni\right)
=i+v\binom ni
\ge v(n)+(i-v(i))
\]
par le lemme 1. Pour $3\le i<p$, on a $v(i)=0$ et $i-v(i)\ge3$. Pour $i\ge p$, on a
\[
i-v(i)\ge i-\frac{\log i}{\log p}.
\]
Cette dernière fonction de $i$ est croissante pour $i\ge p$, et vaut $p-1\ge3$ pour $i=p$.

\paragraph{Lemme 3.} Le coefficient de $X^{p^l\nn}$ dans $R_1$ est nul.

Aucun des monômes $X^\rr$ apparaissant dans $Q$ n'a un exposant $\rr$ divisible par $p^l$. Le polynôme
\[
R_1=\left(\sum_{j=1}^k X_j^{p^l}\right)^{n-1}Q
\]
hérite de cette propriété.

Il résulte des lemmes 2 et 3 que
\[
\left(\sum_{i=1}^k X_i\right)^{p^l n}
\dd
\left(\sum_{i=1}^k X_i^{p^l}\right)^n
+\binom n2 p^2R_2
\pmod {p^{v(n)+3}}. \tag{11}
\]
De cette congruence résulte déjà, d'après le lemme 1, que
\[
C(p^l\nn)\equiv C(\nn)\pmod {p^{v(n)+2}}. \tag{12}
\]

Les seuls monômes $X^\rr$ d'exposant divisible par $p^l$ qui figurent dans $(pQ)^2$ sont les monômes
\[
X_i^{p^l}X_j^{p^l}\qquad(i<j).
\]
Le coefficient de chacun d'eux est
\[
\sum_{a\ne0,p^l}\binom{p^l}{a}\binom{p^l}{p^l-a}
=
\sum_a \binom{p^l}{a}^2-2
=
\binom{2p^l}{p^l}-2, \tag{13}
\]
et
\[
\binom{2p^l}{p^l}-2
\equiv
\binom{2p}{p}-2
\pmod {p^3}
\]
d'après (12).

On a donc
\[
\left(\sum_{i=1}^k X_i\right)^{p^l n}
\dd
\left(\sum_{i=1}^k X_i^{p^l}\right)^n
+\binom n2\left[\binom{2p}{p}-2\right]R_2
\pmod {p^{v(n)+3}},
\]
avec $R_2$ polynôme à coefficients entiers. Puisque
\[
v\binom n2\ge v(n)
\]
par le lemme 1, la proposition résulte du lemme suivant.

\paragraph{Lemme 4.} On a, pour $p$ premier $\ge5$,
\[
\binom{2p}{p}\equiv2\pmod {p^3}.
\]

On a
\[
p^{-1}\binom pi\equiv(-1)^{i-1}i^{-1}\pmod p
\]
pour $1\le i\le p-1$, et donc
\[
p^{-2}\left[\binom{2p}{p}-2\right]
=
\sum_{i=1}^{p-1}p^{-2}\binom pi^2
\equiv
\sum_{i=1}^{p-1}\frac{1}{i^2}
\pmod p. \tag{14}
\]
Définissons $z\in\Z/(p)$ par
\[
z=\sum_{i\in(\Z/(p))^*}\frac1{i^2}.
\]
Pour $u\in(\Z/(p))^*$, on a
\[
z=\sum_{i\in(\Z/(p))^*}\frac{1}{(u^{-1}i)^2}=u^2z
\]
et
\[
(u^2-1)z=0.
\]
Puisque $p\ne2,3$, il existe $u$ tel que $u^2\ne1$, et $z=0$. La formule (14) se simplifie donc en
\[
p^{-2}\left[\binom{2p}{p}-2\right]\equiv0\pmod p,
\]
ce qui achève la démonstration.

\end{document}
