"""Deterministic, offline packager for the six-file D033 release payload.

This module performs no Git, TeX, network, publication, or credential access.
The forbidden first-name token is accepted only in memory (the CLI reads it
from stdin) and is never included in output or exception text.
"""
...
import d033_contract as contract
...
RELEASE_FILENAMES = (
    ...
    "Deligne_FR.tex",
    "Deligne_Source.zip",
    "DELIGNE_PROVENANCE_AUDIT_D033_GAPFILL.zip",
)
...
D033_MAP_MEMBER = "D033/D033_PUBLIC_EVIDENCE_MAP.json"
...
PAYLOAD_MANIFEST_NAME = "D033_RELEASE_PAYLOAD_MANIFEST.json"
...
def validate_provenance_tree(members: list[tuple[str, Path]], inherited_archive: Path) -> dict[str, object]:
    ...
    require(required <= set(by_name), "provenance tree lacks required inherited or D033 members")
    require(any(name.startswith("D033/evidence/") for name in by_name), "provenance tree lacks D033 privacy-safe evidence")
    ...
    require(identity(by_name[INHERITED_MEMBER]) == inherited_identity, "inherited D019 provenance bytes differ")
    return {
        ...
        "d033_gate_archive_members": archive_inventory(
            by_name[D033_GATE_MEMBER],
            require_deterministic_metadata=False,
            require_sorted_names=False,
        ),
    }
...
def split_large_archive(archive: Path, parts_directory: Path) -> list[Path]:
    ...
    with archive.open("rb") as source:
        index = 1
        while True:
            block = source.read(MAX_PART_BYTES)
            if not block:
                break
            require(len(block) <= MAX_PART_BYTES, "split part exceeds maximum size")
...
def github_release_asset_contract(files: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "schema": "d033-github-release-assets-v1",
...
def _write_manifest(
    ...
    result = {
        "schema": "d033-release-payload-manifest-v1",
        "status": "PASS",
...
def _write_release_metadata(
    ...
    with tsv_path.open("xb") as stream:
        stream.write(("\n".join(lines) + "\n").encode("ascii"))
    receipt = {
        "schema": "d033-build-release-receipt-v1",
        "status": "PASS",
...
def package_release(
    ...
    inherited_d019_provenance: Path,
    release_directory: Path,
    manifest_path: Path,
    forbidden_first_name: str,
    parts_root: Path | None = None,
) -> dict[str, object]:
    """Create, replay, privacy-scan, and atomically expose the D033 payload."""
    ...
    provenance_validation = validate_provenance_tree(provenance_members, inherited_d019_provenance.resolve())
    ...
    staging = Path(tempfile.mkdtemp(prefix=".d033-package-", dir=release_directory.parent))
    split_parts: list[Path] = []
    archive_members: dict[str, list[dict[str, object]]] = {}
...
def _read_secret_from_stdin() -> str:
    require(not sys.stdin.isatty(), "pipe the forbidden first-name token on stdin")
    value = sys.stdin.readline()
    require(value.endswith("\n") or value != "", "forbidden first-name token is unavailable")
    return value.rstrip("\r\n")
...
def main() -> None:
    ...
    parser.add_argument("--inherited-d019-provenance", type=Path, required=True)
    ...
    result = package_release(
        ...
        inherited_d019_provenance=args.inherited_d019_provenance,
        release_directory=args.release_directory,
        manifest_path=args.payload_manifest,
        forbidden_first_name=_read_secret_from_stdin(),
