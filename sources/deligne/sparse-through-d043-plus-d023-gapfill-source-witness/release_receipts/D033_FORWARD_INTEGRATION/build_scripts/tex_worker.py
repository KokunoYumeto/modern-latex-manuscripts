...
from d033_contract import Failure, require
...
def tex_pass(mutex, engine, slot, name, environment, timeout_seconds=900):
    """Return output only after the memory-capped captured job is empty."""
    ...
    try:
        ...
        proc = subprocess.Popen([str(engine), "-no-shell-escape", "-interaction=nonstopmode", "-halt-on-error", "-file-line-error", name], cwd=slot, env=env, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, creationflags=CREATE_SUSPENDED | CREATE_NO_WINDOW)
        ...
        raw, _ = proc.communicate(timeout=timeout_seconds)
        ...
        return {"return_code": proc.returncode, "captured_processes": accounting.TotalProcesses, "active_descendants_at_return": 0, "elapsed_seconds": round(time.monotonic()-started, 3), "memory_policy": job_memory_policy(), "peak_job_memory_bytes": peak_job_memory, "anomalies": log_anomalies(raw.decode("utf-8", "replace"))}, raw
    finally:
