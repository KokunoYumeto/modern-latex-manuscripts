"""Fail-closed outer Windows Job runner for D033 build and QA stages.

The production CLI accepts only ``build`` or ``qa`` and constructs the exact
...
from d033_contract import Failure, TASK, identity, reject_reparse, require

OUTER_JOB_MEMORY_LIMIT_BYTES = 3_221_225_472
...
BUILD_SCRIPT = SCRIPTS / "build_d033_integration.py"
...
def production_receipt(stage):
    require(stage in ("build", "qa"), "outer runner stage is not allowlisted")
    return TASK / "build/cumulative/audit" / f"D033_OUTER_{stage.upper()}_JOB_RECEIPT.json"
...
def _validate_command(command, profile, probe_receipt=None):
    ...
    if profile in ("build", "qa"):
        expected = [str(python), str(build), profile]
...
def _run_allowlisted(command, receipt_path, profile, timeout_seconds, probe_receipt=None):
    """Run one exact command in an outer 3 GiB Job and write a sanitized receipt."""
    require(os.name == "nt", "outer Windows Job runner requires Windows")
    require(isinstance(timeout_seconds, int) and 0 < timeout_seconds <= BUILD_TIMEOUT_SECONDS, "invalid bounded wall timeout")
    ...
    receipt = {
        "schema": "d033-outer-process-tree-job-v1",
        "status": "RUNNING",
        "started_utc": now(),
        "allowlist_profile": profile,
        "stage": profile if profile in ("build", "qa") else "NON_TEX_NESTED_JOB_PROBE",
...
def run_stage(stage):
    ...
    require(stage in ("build", "qa"), "outer runner stage is not allowlisted")
    timeout = BUILD_TIMEOUT_SECONDS if stage == "build" else QA_TIMEOUT_SECONDS
    command = [str(bundled_python()), str(BUILD_SCRIPT.resolve()), stage]
...
def main():
    ...
    parser.add_argument("stage", choices=("build", "qa"))
