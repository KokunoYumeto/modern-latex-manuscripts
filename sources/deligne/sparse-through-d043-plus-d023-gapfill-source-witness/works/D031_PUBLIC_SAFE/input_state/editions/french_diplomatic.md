...
Diplomatic French completed and frozen for physical pages 1-43 (printed 247-289). Page boundaries are explicit. Repeated running heads and folios, neighboring-page slivers, scanner borders, the proceedings citation line, the AMS classification line, and the copyright line are excluded from the reader body and recorded in the apparatus.
...
l’involution d’opposition. Il s’agit d’exprimer que $\langle\mu,\beta\rangle$ ne prend que deux valeurs $a$ et $a+1$, pour $\beta$ un poids de $W$. Ces poids étant tous de la forme $(\alpha+\text{ une combinaison }\mathbf Z\text{-linéaire de racines})$, et les $\langle\mu,\gamma\rangle$, pour $\gamma$ une racine, étant entiers, la condition s’exprime par $\langle\mu,-\tau(\alpha)\rangle=\langle\mu,\alpha\rangle-1$, soit
...
$A_{p+q-1}$ (sommet spécial en $p^{\mathrm{ième}}$ position)
...
Nous noterons $^{\wedge}(\mathrm{rel.}\,G_1)$, ou simplement $^{\wedge}$, la complétion pour cette topologie. Soit $\rho:\widetilde G_0\to G_1$ l’application naturelle, notons $^{-}$ l’adhérence dans $G_1(A^f)$, et posons $\Gamma=\rho\widetilde G_0(A^f)\cap G_1(\mathbf Q)$. Puisque $\widetilde G_0(\mathbf R)$ est connexe, $\Gamma\subset G_1(\mathbf Q)_+$. On a (2.0.9, 2.0.14)
...
(a) Soient des systèmes $(G_i,G'_i,X_i^+)$ comme en 2.7.10, en nombre fini, et $E\subset\overline{\mathbf Q}$ un corps de nombres contenant les $E(G_i,X_i^+)$. Si les $M^0_{\overline{\mathbf Q},i}$ sont des modèles faiblement canoniques, sur $E$, des $M^0(G_i,G'_i,X_i^+)$, leur produit est un modèle faiblement canonique de $M^0(\prod G_i,\prod G'_i,\prod X_i^+)$ sur $E$.

(b) Soient $(G,G',X^+)$ comme en 2.7.10, et $G''$ un revêtement de $G$, quotient de $G'$. Si $M^0_{\overline{\mathbf Q}}$ est un modèle faiblement canonique, sur $E$, de $M^0(G,G',X^+)$, son quotient par $\operatorname{Ker}(G(\mathbf Q)^{+\wedge}(\mathrm{rel.}\,G')\to G(\mathbf Q)^{+\wedge}(\mathrm{rel.}\,G''))$ est un modèle faiblement canonique, sur $E$, de $M^0(G,G'',X^+)$.
