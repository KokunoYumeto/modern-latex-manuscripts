...
\paragraph{6.} Soient $X$ une variété projective et lisse sur $\C$, $D=\sum_1^n D_i$ un diviseur à croisements normaux dans $X$, somme de diviseurs lisses, et $j$ l'inclusion dans $X$ de $U=X-D$. Pour $Q\subset[1,n]$, on pose
\[
...
\]
soit un morphisme de structure de Hodge. On le prouve en interprétant $d_2$ comme un morphisme de Gysin. Dès lors, $E_3^{pq}$ est muni d'une structure de Hodge de poids $p+2q$. D'après 3, on s'attend à ce que, modulo torsion, la suite spectrale (6.4) dégénère au terme $E_3$ ($E_3=E_\infty$), et à ce que la nullité des $d_r$ ($r\ge3$) soit une application de 5.1. Ce programme est mené à bien dans [1] 3.2. On y définit la filtration par le poids de $H^*(U,\Q)$ comme aboutissement de (7.1), à la renumérotation (6.3) près.
...
\end{array}
\]
On notera que $\wt X$ est homotopiquement équivalent à chacune des fibres $X_t=f^{-1}(t)$ ($t\in D^*$): $H^i(X_K,\Zl)$ a encore pour analogue $H^i(X_t,\Z)$ et à l'action de $I$ correspond la transformation de monodromie $T$.
