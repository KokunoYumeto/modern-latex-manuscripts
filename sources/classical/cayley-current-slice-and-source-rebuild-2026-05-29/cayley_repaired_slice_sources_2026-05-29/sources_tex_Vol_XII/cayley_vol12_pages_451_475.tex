...
\[
\frac{du}{dz} = u\,P(u), \qquad \frac{dz}{du} = \frac{1}{u}\,P(u),\quad\text{or say}\quad m\frac{dz}{du} = \frac{1}{u} + P u,
\]
whence
\[
m z = \log u + u\,P(u),
...
I take a particular instance, the point $(0,0)$ for the curve $y = \dfrac{x-\tfrac{1}{3}x^{3}}{1-x^{2}}$; here
\[
\frac{du}{dz} = \frac{u-\tfrac{1}{3}u^{3}}{1-u^{2}}, \qquad \frac{dz}{du} = \frac{1-u^{2}}{u-\tfrac{1}{3}u^{3}},
\]
\[
z = \log(u-\tfrac{1}{3}u^{3}), \quad\text{or}\quad u-\tfrac{1}{3}u^{3}=e^{z};
\]
the point is permissive. The equation just obtained shows that $u$ is not a monotropic function of $z$; but this is by reason of the prohibitive points $(\pm1,\infty)$.

\medskip

\noindent 9.\quad A point $y = (x-a)^{2}\,P(x-a)$ is permissive. Here
\[
\frac{du}{dz} = u^{2}P(u), \qquad \frac{dz}{du} = \frac{1}{u^{2}}P(u) = \frac{A}{u^{2}}+\frac{B}{u}+P(u),
\]
whence
\[
...
\[
e^{z/B}=u e^{-\frac{A}{B u}+uP(u)};
\]
in the latter case,
\[
z = -\frac{A}{u}+uP(u);
\]
and in each case we can, by reversion, express $u$ as a one-valued series in $z$. We may take as examples the two curves $y=\dfrac{x^{2}}{1-x}$ and $y=\dfrac{x^{2}}{1-x^{2}}$; in each case the point $(0,0)$ is a permissive point, but, by reason of the prohibitive points $(1,\infty)$ and $(\pm1,\infty)$, $u$ is not a monotropic function of $z$.
...
\[
y=(x-a)^{1+\varepsilon/n}P\bigl((x-a)^{1/n}\bigr),\quad\text{or}\quad \frac{du}{dz}=u^{1+\varepsilon/n}P(u^{1/n}),
\]
...
\[
y^{m} + a_{1}x^{2}y^{m-1} + a_{2}x^{4}y^{m-2} + \cdots + a_{m}x^{2m} = 0;
\]
...
\[
\left(\frac{du}{dz}\right)^{3} + 3\left(\frac{du}{dz}\right)^{2} + 27u^{2} - 4 = 0;
\]
...
\begin{tikzpicture}[scale=0.82, line cap=round, line join=round]
  \draw[->] (-1.45,0) -- (1.65,0) node[right] {$x$};
  \draw[->] (0,-2.20) -- (0,1.70) node[above] {$y$};
  \draw
    (0,1.30)
      .. controls (-0.68,1.20) and (-0.70,0.25) .. (0,-1.25)
      .. controls (0.70,0.25) and (0.68,1.20) .. (0,1.30);
  \draw (0,-1.25) .. controls (-0.20,-1.55) and (-0.43,-1.90) .. (-0.58,-2.15);
  \draw (0,-1.25) .. controls (0.20,-1.55) and (0.43,-1.90) .. (0.58,-2.15);
  \draw[fill=white] (0,0) circle (1.5pt);
  \node at (0,-2.50) {\small Fig.\ 1.};
\end{tikzpicture}
\end{center}

For the points at infinity, we have $y = -3x^{\frac{2}{3}} - 1 - \tfrac{1}{3}x^{-\frac{2}{3}} + \&c.$, which is of the form $y = x^{\frac{2}{3}}P\!\left(\dfrac{1}{x^{1/3}}\right)$, included in $y = x^{1-1/n}\,P\!\left(\dfrac{1}{x^{1/n}}\right)$, and the point is thus permissive; there is no prohibitive point, and the differential equation has a monotropic solution. This is, in fact, the rational solution $u = (z-c) - (z-c)^{3}$, or say $u = z - z^{3}$; the curve is thus given by the two equations $x = z - z^{3}$, $y = 1 - 3z^{2}$.

\medskip

\noindent 17.\quad As a second example, take the differential equation
\[
\left(\frac{du}{dz}\right)^{3} - \left(\frac{du}{dz}\right)^{2} + 4u^{2} - 27u^{4} = 0.
\]
We have here the curve
\[
...
\]
hence, putting $\theta = \dfrac{e^{z}-e^{-z}}{e^{z}+e^{-z}}$, the curve is given by the two equations
\[
x = \theta-\theta^{3}, \qquad y = (1-3\theta^{2})(1-\theta^{2}) = 1-4\theta^{2}+3\theta^{4};
\]
...
\[
(11)\qquad
\vartheta(0,0,\ldots,0)
= \sqrt{\frac{|A^{(\lambda)}_{\lambda}|}{(2\pi i)^{p}}}\,
  \sqrt[4]{\operatorname{Discr.}(0,0,\ldots,0)\operatorname{Discr.}'(0,0,\ldots,0)},
\]
...
\]
The determinant $|A^{(\lambda)}_{\lambda}|$ is a single term $=A$, and the formula becomes
\[
\vartheta(0) = \sqrt{\frac{A}{\pi i}}\,\sqrt[4]{(k_{3}-k_{1})(k_{4}-k_{2})},
\]
where $k_{3}-k_{1}$, $k_{4}-k_{2}$ are each $=1+\dfrac{1}{k}$, and we have therefore
\[
\vartheta(0) = \sqrt{\frac{A}{\pi i}\left(1+\frac{1}{k}\right)}.
\]
$A$ also denotes the integral
\[
\int_{k_{1}}^{k_{2}} \frac{dx}{\sqrt{x-k_{1}\cdot x-k_{2}\cdot x-k_{3}\cdot x-k_{4}}}
= \int_{-\frac{1}{k}}^{-1} \frac{k\,dx}{\sqrt{(1-x^{2})(1-k^{2}x^{2})}}
= \int_{1}^{\frac{1}{k}} \frac{k\,dx}{\sqrt{(1-x^{2})(1-k^{2}x^{2})}}
= ikK',
\]
...
\[
\vartheta(x) = \Sigma e^{a n^{2}+2mnx},
\]
$a$ is used to denote the value
\[
a = a'_{1}B,\qquad = \frac{2\pi}{A}B,
\]
where $A$ is the above-mentioned integral, and $B$ is the integral
\[
B = \int_{k_{2}}^{k_{3}} \frac{dx}{\sqrt{x-k_{1}\cdot x-k_{2}\cdot x-k_{3}\cdot x-k_{4}}}
= \int_{-1}^{1} \frac{k\,dx}{\sqrt{(1-x^{2})(1-k^{2}x^{2})}}
= 2kK,
\]
...
\[
q=e^{-\pi K'/K},\qquad r=e^{-\pi K/K'},
\]
the $e^{a}$ of the theta-function is not $q$, but it is $r^{2}$; and the zero-value $\vartheta(0)$ is
$=1+2r^{2}+2r^{8}+2r^{18}+\cdots$. The equation thus is
\[
1+2r^{2}+2r^{8}+2r^{18}+\cdots
= \sqrt{\frac{K'(1+k)}{\pi}},
\]
which is right; in fact, writing $k'$ in place of $k$, and consequently $K$, $q$ in place of $K'$, $r$ respectively, the equation becomes
\[
1+2q^{2}+2q^{8}+2q^{18}+\cdots
= \sqrt{\frac{K(1+k')}{\pi}};
\]
we have
\[
...
= \sqrt{\frac{2K}{\pi}},
\]
and changing $q$ into $q^{2}$, then (\textit{Fund.\ Nova}, p.~92) $K$ is changed into $\dfrac{K(1+k')}{2}$, and we have the formula in question. As a verification for small values of $q$, observe that we have
\[
\frac{2K}{\pi}=1+4q+4q^{2},\qquad \frac{1+k'}{2}=1-4q+16q^{2},
\]
and thence
\[
\frac{K(1+k')}{\pi}=1+4q^{2},
\qquad\text{or}\qquad
\sqrt{\frac{K(1+k')}{\pi}}=1+2q^{2}.
\]
...
\]
\[
B = 1 + \frac{\beta}{2\sqrt{\alpha}},
\]
\[
C = \frac{\gamma}{2\sqrt{\alpha}} - \frac{\beta}{4\alpha} - \frac{\beta^{2}}{8\alpha\sqrt{\alpha}},
\]
\[
D = \frac{\delta}{2\sqrt{\alpha}} - \frac{\gamma}{2\alpha}
    + \frac{1}{4\alpha\sqrt{\alpha}}(\beta-\beta\gamma)
    + \frac{\beta^{2}}{4\alpha^{2}}
    + \frac{\beta^{3}}{16\alpha^{2}\sqrt{\alpha}},
\]
\[
E = \frac{\varepsilon}{2\sqrt{\alpha}} - \frac{3\delta}{4\alpha}
  + \frac{1}{4\alpha\sqrt{\alpha}}\left(3\gamma-\beta\delta-\tfrac{1}{2}\gamma^{2}\right)
  + \frac{1}{4\alpha^{2}}\left(-\tfrac{3}{2}\beta+3\beta\gamma\right)
  + \frac{1}{16\alpha^{2}\sqrt{\alpha}}\left(-\tfrac{17}{2}\beta^{2}+3\beta^{2}\gamma\right)
  - \frac{\beta^{3}}{4\alpha^{3}}
  - \frac{5}{128}\frac{\beta^{4}}{\alpha^{3}\sqrt{\alpha}},\quad \&c.
\]
...
\]
where the second exponential is to be expanded in the form $1+C'x+D'x^{2}+\cdots$; there are of course two values, say $y_{1}$ and $y_{2}$, corresponding to the values $+\sqrt{\alpha}$ and $-\sqrt{\alpha}$ of the radical respectively.
...
we have
\[
z' = -\tfrac{3}{2}A t^{5/2} - B t^{2} - \tfrac{1}{2}C t^{3/2}
     + \tfrac{1}{2}E t^{1/2} + \cdots,
\]
...
\[
A = \sqrt{\beta},\quad B = \tfrac{3}{4},\quad
C = \frac{1}{2\sqrt{\beta}}\left(\gamma+\tfrac{3}{16}\right),\quad
D = -\frac{1}{4\beta}\left(\gamma+\tfrac{3}{16}\right),\quad
E = \frac{1}{2\beta\sqrt{\beta}}\left(\delta+\tfrac{9}{32}\gamma-\tfrac{1}{4}\gamma^{2}+\tfrac{63}{1024}\right),\quad \&c.,
\]
...
\noindent We have
\[
z = \frac{1}{y}\,\frac{dy}{dx} = \frac{A}{x^{3/2}} + \frac{B}{x}
  + \frac{C}{x^{1/2}} + D + Ex^{1/2} + Fx + \cdots,
\]
giving
\[
...
\[
y_{1} = e^{-2A/x^{1/2}}\,x^{B}\,(1 + C'x^{1/2} + D'x + E'x^{3/2} + \cdots),
\]
and then also, changing the sign of $\sqrt{\beta}$, the integral
\[
y_{2} = e^{+2A/x^{1/2}}\,x^{B}\,(1 - C'x^{1/2} + D'x - E'x^{3/2} + \cdots).
\]
Writing for a moment
\[
Q = e^{-2A/x^{1/2}},\qquad M = 1 + D'x + F'x^{2} + \cdots,\qquad
N = C' + E'x + G'x^{2} + \cdots,
\]
...
y_{1} - y_{2} = M x^{B}\,(Q - Q^{-1}) + N x^{B + 1/2}\,(Q + Q^{-1}),
\]
where $Q + Q^{-1}$ is a rational function of $x$, and $Q - Q^{-1}$ is $=x^{1/2}$ multiplied by a rational function of $x$; hence $y_{1} + y_{2}$ is $=x^{B}$ multiplied by a rational function of $x$, but $y_{1} - y_{2}$ is $=x^{B+\frac{1}{2}}$ multiplied by a rational function of $x$.
...
\[
P\bigl(x^{e}f\bigr) = x^{e}\,\bigl\{(e+n)(e-n-1) - (e^{2}+e)x^{2}
  + (x^{2}-x^{4})x^{-e}F'' - (n^{2}+n)x^{-e}F\bigr\}
\]
