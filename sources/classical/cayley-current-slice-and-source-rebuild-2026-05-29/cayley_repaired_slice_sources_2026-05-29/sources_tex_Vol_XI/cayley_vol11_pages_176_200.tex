...
\begin{tikzpicture}[scale=0.9,line cap=round,line join=round]
  \coordinate (A) at (-1.25,-0.20);
  \coordinate (B) at (1.15,-1.28);
  \coordinate (C) at (0.18,1.25);

  \draw (-4.0,-1.85) -- (-1.25,-1.85);
  \foreach \x/\lab in {-3.35/$a$,-2.55/$b$,-1.55/$c$}
    {
      \draw (\x,-1.93) -- (\x,-1.77);
      \node[below] at (\x,-1.93) {\lab};
    }

  \draw[dashed] (0,0) circle (1.62);
  \draw (A) .. controls (-0.85,-1.02) and (0.12,-1.54) .. (B);
  \draw (B) .. controls (1.45,-0.15) and (0.95,0.94) .. (C);
  \draw (C) .. controls (-0.70,0.92) and (-1.32,0.40) .. (A);
  \draw[dashed] (A) -- (B);
  \draw[dashed] (B) -- (C);
  \draw[dashed] (C) -- (A);
...
\begin{tikzpicture}[scale=0.82,line cap=round,line join=round]
  ...
  \fill[gray!25] (O) -- (0:\r) arc[start angle=0,end angle=60,radius=\r] -- cycle;
  \fill[gray!25] (O) -- (120:\r) arc[start angle=120,end angle=180,radius=\r] -- cycle;
  \fill[gray!25] (O) -- (240:\r) arc[start angle=240,end angle=300,radius=\r] -- cycle;
  \fill[gray!25] (-2.70,2.75) -- (120:\r) arc[start angle=120,end angle=60,radius=\r] -- (2.70,2.75) -- cycle;
  \fill[gray!25] (-3.55,0) -- (180:\r) arc[start angle=180,end angle=240,radius=\r] -- (-2.55,-2.45) -- cycle;
  \fill[gray!25] (3.55,0) -- (0:\r) arc[start angle=0,end angle=300,radius=\r] -- (2.55,-2.45) -- cycle;

  \draw[dashed] (O) circle (\r);
  \draw (-3.55,0) -- (3.55,0);
  \draw[dashed] (120:3.05) -- (300:3.05);
  \draw (60:3.05) -- (240:3.05);
  \draw (O) -- (0:\r);
  \draw (O) -- (60:\r);
  \draw (O) -- (120:\r);
  \draw (O) -- (180:\r);
  ...
  \node[right] at (0:\r) {$A$};
  \node[left] at (180:\r) {$C$};
  \node[above right] at (60:\r) {$C$};
  \node[above left] at (120:\r) {$A$};
  \node[below left] at (240:\r) {$A$};
  \node[below right] at (300:\r) {$C$};
  \node[below left] at (O) {$B$};
  \node[right] at (0.82,0.12) {$K$};

  \node[right] at (3.50,0) {$B$};
  \node[left] at (-3.50,0) {$B$};
  \node[above right] at (60:3.05) {$B$};
  \node[above left] at (120:3.05) {$B'$};
  \node[below right] at (300:3.05) {$B$};
  \node[below left] at (240:3.05) {$B$};
\end{tikzpicture}
\end{center}
