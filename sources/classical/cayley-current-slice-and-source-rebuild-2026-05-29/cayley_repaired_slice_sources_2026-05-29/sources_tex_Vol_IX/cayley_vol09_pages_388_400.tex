...
\begin{tikzpicture}[scale=1.05,line cap=round,line join=round]
  ...
  \coordinate (M) at (1.48,1.34);
  \draw (-2,0) -- (2,0);
  \draw (-2,0) arc[start angle=180,end angle=0,radius=2];
  \draw (O) -- (M);
  \draw (P) -- (M);
  \draw (0.45,0) arc[start angle=0,end angle=42,radius=0.45];
  \node[below] at (O) {$O$};
  \node[below] at (P) {$P$};
  \node[above right] at (M) {$M$};
  \node at (0.44,0.18) {$\theta$};
  \node[right] at (1.0,0.68) {$\rho$};
\end{tikzpicture}
\]
...
\]
\[
\sin\phi = \frac{f}{\rho}\sin\theta
= \frac{f\sin\theta}{\sqrt{\kappa^{2}+f^{2}-2\kappa f\cos\theta}},
\]
