...
\newcommand{\cayleysquarediag}[5]{%
\begin{tikzpicture}[x=#5cm,y=#5cm,baseline=(current bounding box.center)]
  \node[anchor=south west] at (0,#2+0.35) {$w=#1$};
  \draw[thin] (0,0) grid (#2,#2);
  \foreach \lab [count=\i] in {#3} {
    \node[left=2pt] at (0,#2-\i+0.5) {$\lab$};
  }
  \foreach \lab [count=\j] in {#4} {
    \node[below,rotate=90] at (\j-0.5,0) {$\lab$};
  }
  \foreach \i in {1,...,#2} {
    \node at (\i-0.5,#2-\i+0.5) {$1$};
  }
\end{tikzpicture}%
}

\newcommand{\cayleysquareten}{%
\begin{tikzpicture}[x=0.42cm,y=0.42cm,baseline=(current bounding box.center)]
  \node[anchor=south west] at (0,12.45) {$w=10$};
  \draw[thin] (0,0) grid (12,12);
  \draw[dashed] (0,12) -- (12,0);
  \draw[dashed] (0,0) -- (12,12);
  \foreach \lab [count=\i] in {k,ci,dh,eg,{c^2g},{f^2},cdf,{ce^2},{d^2e},{c^3e},{c^2d^2},{c^5}} {
    \node[left=2pt] at (0,12-\i+0.5) {$\lab$};
  }
  \foreach \lab [count=\j] in {{f^2},{ce^2},{b^2e^2},{bd^3},{c^2d^2},{b^2cd^2},{c^5},{b^4d^2},{b^2c^4},{b^4c^3},{b^6c^2},{b^{10}}} {
    \node[below,rotate=90] at (\j-0.5,0) {$\lab$};
  }
  \foreach \r/\c/\v in {1/1/1,2/2/1,3/3/1,4/4/1,5/6/2,6/5/1,7/8/2,8/7/1,9/9/1,10/10/1,11/11/1,12/12/1} {
    \node at (\c-0.5,12-\r+0.5) {$\v$};
  }
\end{tikzpicture}%
}
...
\begin{tabular}{l|rrrrr}
 & LHS & $S\alpha^5\beta$ & $S\alpha^4\beta^2$ & $S\alpha^3\beta^3$ & sum \\
\hline
$g$ & $+15$ & $+6$ & $+6$ & $+3$ & $+15$ \\
$bf$ & $-10$ & $-1$ & $-6$ & $-3$ & $-10$ \\
$ce$ & $-7$ & $-6$ & $+2$ & $-3$ & $-7$ \\
...
& {} + 6e(c - b^2) \\
& {} + 10f(b)+ 15g(-1) \\
& {} - 5b(-f + be + cd - b^2d - bc^2 + b^3c - \tfrac{1}{5}b^5).
\end{align*}
...
\begin{center}
\begin{tabular}{cccc}
\cayleysquarediag{2}{1}{c}{{b^2}}{0.58} &
\cayleysquarediag{3}{1}{d}{{b^3}}{0.58} &
\cayleysquarediag{4}{2}{e,{c^2}}{{c^2},{b^4}}{0.50} &
\cayleysquarediag{5}{2}{f,cd}{{bc^2},{b^5}}{0.50}
\\[1.2cm]
\multicolumn{2}{c}{\cayleysquarediag{6}{4}{g,ce,{d^2},{c^3}}{{d^2},{c^3},{b^2c^2},{b^6}}{0.55}} &
\multicolumn{2}{c}{\cayleysquarediag{7}{4}{h,cf,de,{c^2d}}{{bd^2},{bc^3},{b^3c^2},{b^7}}{0.55}}
\end{tabular}
\end{center}

% ----- printed page 294 (PDF 316) -----
\begin{center}
\cayleysquarediag{8}{7}{i,cg,df,{e^2},{c^2e},{cd^2},{c^4}}{{e^2},{cd^2},{b^2d^2},{c^4},{b^2c^3},{b^4c^2},{b^8}}{0.43}
\qquad
\cayleysquarediag{9}{8}{j,ch,dg,ef,{c^2f},cde,{d^3},{c^3d}}{{be^2},{d^3},{bcd^2},{b^3d^2},{bc^4},{b^3c^3},{b^5c^2},{b^9}}{0.40}
...
\begin{center}
\begin{tabular}{rcl}
$k$ & $\mathrm{ao}$ & $f^2$,\quad ``1'' \\
$ci$ & $\mathrm{ao}$ & $ce^2$,\quad ``1'' \\
$dh$ & $\mathrm{ao}$ & $b^2 e^2$,\quad ``1'' \\
$eg$ & $\mathrm{ao}$ & $bd^3$,\quad ``1'' \\
$f^2$ & $\mathrm{ao}$ & $c^2 d^2$,\quad ``1'' \\
$c^2 g$ & $\mathrm{ao}$ & $b^2 c d^2$,\quad ``2'' \\
$ce^2$ & $\mathrm{ao}$ & $c^5$,\quad ``1'' \\
$cdf$ & $\mathrm{ao}$ & $b^4 d^2$,\quad ``2'' \\
$d^2 e$ & $\mathrm{ao}$ & $b^2 c^4$,\quad ``1'' \\
$c^3 e$ & $\mathrm{ao}$ & $b^4 c^3$,\quad ``1'' \\
$c^2 d^2$ & $\mathrm{ao}$ & $b^6 c^2$,\quad ``1'' \\
$c^5$ & $\mathrm{ao}$ & $b^{10}$,\quad ``1'' \\
\end{tabular}
\end{center}
