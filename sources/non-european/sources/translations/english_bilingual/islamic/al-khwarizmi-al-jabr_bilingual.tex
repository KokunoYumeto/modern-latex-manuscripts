...
\cnote{Editorial Note:} This is the celebrated method of ``\textbf{completion of the square}'' (\ar{إتمام المربع}), which gives algebra its very name: \textit{al-jabr} refers to the ``restoration'' of the deficient corners. The identity underlying the proof is: $\left(x + \frac{b}{2}\right)^2 = x^2 + bx + \left(\frac{b}{2}\right)^2$.
