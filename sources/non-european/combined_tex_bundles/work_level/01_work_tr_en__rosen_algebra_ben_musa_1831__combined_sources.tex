...
%% ========================================================================
%% The Algebra of Mohammed ben Musa — ENHANCED EDITION
%% Edited and Translated by Frederic Rosen (1831)
%% Enhanced Edition with Original Arabic Terms Restored
%%
%% London: Printed for the Oriental Translation Fund, 1831
%% Enhanced Edition with bilingual Arabic--English terminology
%% ========================================================================

\input{/mnt/agents/output/master-preamble.tex}

%% -------- Font override: avoid HTML contamination from Linux Libertine O --------
...
\definecolor{arabicbg}{RGB}{250,247,240}
\definecolor{glosscolor}{RGB}{60,50,40}

%% Header/Footer
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textit{The Algebra of Mohammed ben Musa — Enhanced Edition}}
...
%% ========================================================================
%% Primary Arabic terms with English translations

%% Core algebraic terms — displayed as Arabic (English)
\newcommand{\term}[2]{#1\marginpar{\raggedright\footnotesize\textcolor{notecolor}{#2}}}
\newcommand{\arterm}[2]{\fbox{\arabicfont\beginR #1\endR}\ \texten{#2}}

%% Inline Arabic term with English translation (first occurrence / emphasis)
\newcommand{\aeterm}[2]{\textbf{\arabicfont\beginR #1\endR}\ \textit{(#2)}}
\newcommand{\ea-term}[2]{#1\ \textbf{[\arabicfont\beginR #2\endR]}}

%% Parenthetical Arabic
\newcommand{\parab}[1]{\textnormal{(\arabicfont\beginR #1\endR)}}

%% Shorthand commands for the six canonical terms
\newcommand{\MfM}[0]{\textbf{m\=al}\ \parab{مال}}          %% squares / māl
\newcommand{\Jdh}[0]{\textbf{jidhr}\ \parab{جذر}}           %% roots / jidhr
\newcommand{\Adad}[0]{\textbf{`adad}\ \parab{عدد}}         %% numbers / `adad
\newcommand{\Shay}[0]{\textbf{shay'}\ \parab{شيء}}          %% thing / shay'
\newcommand{\Jbr}[0]{\textbf{al-jabr}\ \parab{الجبر}}       %% restoration / al-jabr
\newcommand{\Muq}[0]{\textbf{al-muq\=abala}\ \parab{المقابلة}} %% reduction / al-muqabala

%% Additional algebraic terms
\newcommand{\Wajib}[0]{\textbf{w\=ajib}\ \parab{واجب}}     %% absolute term / wājib
\newcommand{\Dirhem}[0]{\textbf{dirhem}\ \parab{درهم}}      %% dirhem / currency unit
\newcommand{\Murabba}[0]{\textbf{murabba`}\ \parab{مربع}}   %% quadrate / square figure
\newcommand{\Zij}[0]{\textbf{z\=ij}\ \parab{زيج}}           %% astronomical table
\newcommand{\ShayPrime}[0]{\textbf{shay'}\ \parab{شيء}}      %% thing (alternative)

%% Geometric terms
\newcommand{\Khat}[0]{\textbf{kha\d{t}\d{t}}\ \parab{خط}}   %% line
\newcommand{\Mustawi}[0]{\textbf{mustaq\=im}\ \parab{مستقيم}} %% straight
\newcommand{\Qaema}[0]{\textbf{q\=a'ima}\ \parab{قائمة}}    %% perpendicular
\newcommand{\Qaemat}[0]{\textbf{q\=a'imat\=an}\ \parab{قائمتان}} %% perpendiculars
\newcommand{\Ard}[0]{\textbf{`ar\d{d}}\ \parab{عرض}}       %% breadth
\newcommand{\Tul}[0]{\textbf{\d{t}\=ul}\ \parab{طول}}      %% length
\newcommand{\Daer}[0]{\textbf{d\=a'ira}\ \parab{دائرة}}     %% circle
\newcommand{\Nisba}[0]{\textbf{nisba}\ \parab{نسبة}}       %% ratio/proportion

%% Mercantile terms
\newcommand{\Thaman}[0]{\textbf{thaman}\ \parab{ثمن}}      %% price
\newcommand{\Kail}[0]{\textbf{kayl}\ \parab{كيل}}           %% measure
\newcommand{\Miqdar}[0]{\textbf{miqd\=ar}\ \parab{مقدار}}   %% quantity
\newcommand{\Jumla}[0]{\textbf{jumla}\ \parab{جملة}}       %% sum/total
\newcommand{\Istirad}[0]{\textbf{istir\=l}\ \parab{استبدال}} %% exchange

%% Legacy terms
\newcommand{\Wasiyya}[0]{\textbf{wasiyya}\ \parab{وصية}}    %% legacy/bequest
\newcommand{\Mirath}[0]{\textbf{m\=ir\=ath}\ \parab{ميراث}}   %% inheritance
\newcommand{\Faraez}[0]{\textbf{far\=a'i\d{d}}\ \parab{فرائض}} %% legal shares
\newcommand{\Taaruf}[0]{\textbf{ta`r\=if}\ \parab{تعريف}}   %% computation/settlement
\newcommand{\Itq}[0]{\textbf{`itq}\ \parab{عتق}}            %% emancipation
\newcommand{\Mukataba}[0]{\textbf{muk\=ataba}\ \parab{مكاتبة}} %% manumission contract
\newcommand{\Mahr}[0]{\textbf{mahr}\ \parab{مهر}}           %% dowry
\newcommand{\Radd}[0]{\textbf{radd}\ \parab{رد}}            %% return

%% ------------------------------------------------------------------------
%% Bilingual environment for case statements
\newenvironment{bilingual-case}[2]{%
  \begin{center}
  \begin{minipage}{0.9\textwidth}
  \begin{center}
  \Large\scshape\color{titlecolor} #1\\[0.3cm]
  \large\color{sectioncolor} \arabicfont\beginR #2\endR
  \end{center}
  \vspace{0.3cm}
}{%
  \end{minipage}
...
%% Highlight box for key Arabic terms
\newenvironment{arabic-highlight}{%
  \begin{center}
  \begin{minipage}{0.9\textwidth}
  \begin{center}
  \colorbox{arabicbg}{%
...
{\fontsize{12}{14}\selectfont\scshape\color{notecolor} Enhanced Bilingual Edition}\\[0.3cm]
{\fontsize{10}{12}\selectfont\color{notecolor} Original Arabic Terms Restored}\\[1cm]
...
{\fontsize{28}{34}\selectfont\scshape\color{titlecolor} Mohammed ben Musa}\\[0.5cm]

{\large\color{sectioncolor}
\textit{Kit\=ab al-Mukhta\d{s}ar f\=i \d{H}is\=ab al-Jabr wal-Muq\=abala}}\\[0.3cm]
{\normalsize\arabicfont\beginR كتاب المختصر في حساب الجبر والمقابلة\endR}\\[1.5cm]

\rule{0.5\textwidth}{1pt}\\[1cm]

{\Large\scshape Edited and Translated}\\[0.3cm]
{\Large\scshape by}\\[0.3cm]
{\Large\scshape Frederic Rosen}\\[1.5cm]
...
\textit{Enhanced Edition with Original Arabic Terminology}\\[0.2cm]
\textit{Six Canonical Cases in Bilingual Presentation}\\[0.2cm]
\textit{Geometrical Demonstrations with Arabic Labels}\\[0.2cm]
\textit{Complete Arabic--English Glossary}
\end{minipage}
...
{\small\textit{Enhanced Edition, 2025}}
...
%% ========================================================================
%% ENHANCED PREFACE — Arabic Terminology Overview
%% ========================================================================
\thispagestyle{empty}
\section*{\textsc{Preface to the Enhanced Edition}}
\addcontentsline{toc}{section}{Preface to the Enhanced Edition}

\lettrine{T}{his enhanced edition} of Frederic Rosen's classic 1831 translation restores the original Arabic terminology that Rosen rendered into English. Where Rosen translated al-Khw\=arizm\=i's technical terms into their English equivalents, this edition presents \textit{both} the original Arabic \textit{and} Rosen's English translation, enabling readers to appreciate the precise mathematical vocabulary of the Arabic tradition.

\begin{arabic-highlight}
\textbf{The Six Canonical Terms}\\[0.3cm]
\begin{tabular}{lll}
\textbf{Arabic} & \textbf{Transliteration} & \textbf{Rosen's English} \\
\arabicfont\beginR مال\endR & \textit{m\=al} & ``squares'' \\
\arabicfont\beginR جذر\endR & \textit{jidhr} & ``roots'' \\
\arabicfont\beginR عدد\endR & \textit{`adad} & ``numbers'' \\
\arabicfont\beginR شيء\endR & \textit{shay'} & ``thing'' (the unknown) \\
\arabicfont\beginR الجبر\endR & \textit{al-jabr} & ``restoration/completion'' \\
\arabicfont\beginR المقابلة\endR & \textit{al-muq\=abala} & ``reduction/opposition'' \\
\end{tabular}
\end{arabic-highlight}

\vspace{0.5cm}
\noindent The Arabic title of the work is:
\begin{center}
\Large\arabicfont\beginR كتاب المختصر في حساب الجبر والمقابلة\endR\\[0.3cm]
\textit{Kit\=ab al-Mukhta\d{s}ar f\=i \d{H}is\=ab al-Jabr wal-Muq\=abala}\\[0.2cm]
``A Compendious Book on Calculation by Completion and Reduction''
\end{center}

\vspace{0.3cm}
\noindent Throughout this enhanced edition:
\begin{itemize}[leftmargin=2em]
\item Arabic terms are presented \textbf{[\arabicfont\beginR thus\endR]} alongside Rosen's English translation on first occurrence in each section.
\item The six canonical cases of equations are given with bilingual headings.
\item Geometrical demonstrations include the original Arabic labels.
\item A complete Arabic--English glossary is appended.
\end{itemize}

\vspace{0.3cm}
\noindent The goal is to honour both al-Khw\=arizm\=i's original mathematical vocabulary and Rosen's pioneering English translation, presenting them together for the benefit of historians of mathematics, Arabists, and mathematicians alike.
...
\addcontentsline{toc}{section}{The Author's Preface \textnormal{[\arabicfont\beginR كلام المؤلف\endR]}}
...
{\Large\scshape Completion and Reduction.}\\[0.5cm]
{\normalsize\color{sectioncolor}
\textit{Kit\=ab al-Mukhta\d{s}ar f\=i \d{H}is\=ab al-Jabr wal-Muq\=abala}}\\[0.2cm]
{\normalsize\arabicfont\beginR كتاب المختصر في حساب الجبر والمقابلة\endR}
\end{center}
...
\addcontentsline{toc}{section}{Introduction \textnormal{[\arabicfont\beginR المقدمة\endR]}}
...
I observed that the numbers which are required in calculating by Completion and Reduction \textbf{[\arabicfont\beginR الجبر والمقابلة\endR\ \textit{(al-jabr wal-muq\=abala)}]} are of three kinds, namely, \textbf{roots} \textbf{[\arabicfont\beginR جذر\endR\ \textit{(jidhr)}]}, \textbf{squares} \textbf{[\arabicfont\beginR مال\endR\ \textit{(m\=al)}]}, and \textbf{simple numbers relative to neither root nor square} \textbf{[\arabicfont\beginR عدد\endR\ \textit{(`adad)}]}.

A \textit{root} \textbf{[\arabicfont\beginR جذر\endR\ \textit{(jidhr)}]} is any quantity which is to be multiplied by itself, consisting of units, or numbers ascending, or fractions descending.\footnote{By the word root, is meant the simple power of the unknown quantity.}

A \textit{square} \textbf{[\arabicfont\beginR مال\endR\ \textit{(m\=al)}]} is the whole amount of the root multiplied by itself.

A \textit{simple number} \textbf{[\arabicfont\beginR عدد\endR\ \textit{(`adad)}]} is any number which may be pronounced without reference to root or square.
...
\hfill\textbf{[\arabicfont\beginR المال يساوي الجذور\endR\ \textit{(al-m\=al yus\=awi al-judh\=ur)}]}
...
\hfill\textbf{[\arabicfont\beginR المال يساوي الأعداد\endR\ \textit{(al-m\=al yus\=awi al-a`d\=ad)}]}
...
\hfill\textbf{[\arabicfont\beginR الجذور تساوي الأعداد\endR\ \textit{(al-judh\=ur tus\=awi al-a`d\=ad)}]}
...
\vspace{0.5cm}
I found that these three kinds; namely, \textbf{roots} \textbf{[\arabicfont\beginR جذور\endR\ \textit{(judh\=ur)}]}, \textbf{squares} \textbf{[\arabicfont\beginR أموال\endR\ \textit{(amw\=al)}]}, and \textbf{numbers} \textbf{[\arabicfont\beginR أعداد\endR\ \textit{(a`d\=ad)}]}, may be combined together, and thus three compound species arise;\footnote{The three cases considered are,\\\null\hspace{1cm}$1^{st}$. $cx^2 + bx = a$\\\null\hspace{1cm}$2^{d}$. $cx^2 + a = bx$\\\null\hspace{1cm}$3^{d}$. $cx^2 = bx + a$} that is, ``\textit{squares and roots equal to numbers};'' ``\textit{squares and numbers equal to roots};'' ``\textit{roots and numbers equal to squares}.''

\vspace{0.3cm}
\noindent\textbf{Case IV.---Roots and Squares are equal to Numbers.}\
\hfill\textbf{[\arabicfont\beginR الجذور والمال يساويان الأعداد\endR\ \textit{(al-judh\=ur wal-m\=al yus\=awiy\=an al-a`d\=ad)}]}
...
\hfill\textbf{[\arabicfont\beginR المال والأعداد يساويان الجذور\endR\ \textit{(al-m\=al wal-a`d\=ad yus\=awiy\=an al-judh\=ur)}]}
...
\hfill\textbf{[\arabicfont\beginR الجذور والأعداد تساوي المال\endR\ \textit{(al-judh\=ur wal-a`d\=ad tus\=awi al-m\=al)}]}
...
\addcontentsline{toc}{section}{Geometrical Demonstrations \textnormal{[\arabicfont\beginR البراهين الهندسية\endR]}}
\addcontentsline{toc}{section}{Geometrical Demonstrations}

\lettrine{D}{emonstration of the Case:} ``\textit{a Square \textbf{[\arabicfont\beginR مال\endR\ \textit{(m\=al)}]} and ten Roots \textbf{[\arabicfont\beginR جذور\endR\ \textit{(judh\=ur)}]} are equal to thirty-nine Dirhems \textbf{[\arabicfont\beginR درهم\endR\ \textit{(dirham)}]}.}''\footnote{Geometrical illustration of the case, $x^2 + 10x = 39$.}
...
\addcontentsline{toc}{section}{On Multiplication \textnormal{[\arabicfont\beginR في الضرب\endR]}}
...
For instance: ``Ten less \textbf{thing} \textbf{[\arabicfont\beginR شيء\endR\ \textit{(shay')}]}, the signification of \textbf{root} \textbf{[\arabicfont\beginR جذر\endR\ \textit{(jidhr)}]}, to be multiplied by ten.''\footnote{$(10 - x) \times 10 = 10 \times 10 - 10x = 100 - 10x$.} You begin by taking ten times ten, which is a hundred; less thing by ten is ten roots negative; the product is therefore a hundred less ten things.
...
\addcontentsline{toc}{section}{On Addition and Subtraction \textnormal{[\arabicfont\beginR في الجمع والطرح\endR]}}
...
%% ========================================================================
\section*{On Division}
\addcontentsline{toc}{section}{On Division \textnormal{[\arabicfont\beginR في القسمة\endR]}}
...
\addcontentsline{toc}{section}{Demonstrations \textnormal{[\arabicfont\beginR البراهين\endR]}}
...
\addcontentsline{toc}{section}{Of the Six Problems \textnormal{[\arabicfont\beginR المسائل الست\endR]}}
...
\addcontentsline{toc}{section}{Various Questions \textnormal{[\arabicfont\beginR مسائل شتى\endR]}}
...
\addcontentsline{toc}{section}{On Mercantile Transactions \textnormal{[\arabicfont\beginR في المعاملات التجارية\endR]}}
...
%% ========================================================================
\section*{Mensuration}
\addcontentsline{toc}{section}{Mensuration \textnormal{[\arabicfont\beginR في المساحة\endR]}}
...
%% ========================================================================
\section*{On Legacies}
\addcontentsline{toc}{section}{On Legacies \textnormal{[\arabicfont\beginR في الوصايا\endR]}}
...
%% APPENDIX: Notes
%% ========================================================================
\section*{Notes on the Arabic Text \textnormal{[\arabicfont\beginR ملاحظات على النص العربي\endR]}}
...
%% ========================================================================
%% APPENDIX: COMPLETE ARABIC--ENGLISH GLOSSARY
%% ========================================================================
\section*{Appendix: Arabic--English Glossary of Algebraic Terms}
\addcontentsline{toc}{section}{Appendix: Arabic--English Glossary}

\lettrine{T}{he following glossary} presents the complete Arabic mathematical terminology employed by Mohammed ben Musa al-Khw\=arizm\=i in his \textit{Kit\=ab al-Mukhta\d{s}ar f\=i \d{H}is\=ab al-Jabr wal-Muq\=abala}, together with Rosen's English equivalents and modern mathematical interpretations.

\vspace{0.5cm}

\begin{center}
\begin{longtable}{>{\raggedright}p{3.2cm} >{\raggedright}p{2.8cm} >{\raggedright\arraybackslash}p{6.5cm}}
\toprule
...
%% ===== CORE ALGEBRAIC TERMS =====
\multicolumn{3}{c}{\textsc{I. Core Algebraic Terms}} \\
\midrule

\arabicfont\beginR الجبر\endR & \textit{al-jabr} & \textbf{Restoration, completion.} The operation of transferring negative quantities to the other side of an equation as positive. One of the two fundamental operations of algebra, giving the science its name. \\
\addlinespace[0.3cm]

\arabicfont\beginR المقابلة\endR & \textit{al-muq\=abala} & \textbf{Reduction, opposition, balancing.} The process of cancelling like quantities on both sides of an equation; also the reduction of equations to standard form. \\
\addlinespace[0.3cm]

\arabicfont\beginR مال\endR & \textit{m\=al} & \textbf{Square, property, wealth.} The square of the unknown quantity ($x^2$). Literally ``property'' or ``possession,'' reflecting the mercantile context of early algebra. Rosen consistently renders this as ``square.'' \\
\addlinespace[0.3cm]

\arabicfont\beginR جذر\endR & \textit{jidhr} & \textbf{Root.} The unknown quantity itself ($x$); also used for square roots of known numbers. Literally ``root,'' ``base,'' or ``origin.'' \\
\addlinespace[0.3cm]

\arabicfont\beginR شيء\endR & \textit{shay'} & \textbf{Thing.} The unknown quantity ($x$). The most common designation for the unknown in Arabic algebra. Corresponds to the Latin \textit{res} and the Italian \textit{cosa}. \\
\addlinespace[0.3cm]

\arabicfont\beginR عدد\endR & \textit{`adad} & \textbf{Number.} The absolute or constant term in an equation; a known quantity unrelated to the unknown. Rosen renders this as ``simple number'' or ``number.'' \\
\addlinespace[0.3cm]

\arabicfont\beginR واجب\endR & \textit{w\=ajib} & \textbf{Obligatory, requisite.} The constant term or absolute number in certain contexts. Used by later writers; al-Khw\=arizm\=i himself uses \textit{`adad}. \\
\addlinespace[0.3cm]

\midrule
\multicolumn{3}{c}{\textsc{II. Geometrical Terms}} \\
\midrule

\arabicfont\beginR مربع\endR & \textit{murabba`} & \textbf{Quadrate, square figure.} A geometric square used in demonstrations. Distinguished from \textit{m\=al} which refers to the algebraic square. \\
\addlinespace[0.3cm]

\arabicfont\beginR خط\endR & \textit{kha\d{t}\d{t}} & \textbf{Line.} A straight or curved line. \\
\addlinespace[0.3cm]

\arabicfont\beginR مستقيم\endR & \textit{mustaq\=im} & \textbf{Straight, rectilinear.} Describes a straight line or straight angle. \\
\addlinespace[0.3cm]

\arabicfont\beginR قائمة\endR & \textit{q\=a'ima} & \textbf{Perpendicular, cathetus.} A line standing at right angles to another. Literally ``standing upright.'' \\
\addlinespace[0.3cm]

\arabicfont\beginR طول\endR & \textit{\d{t}\=ul} & \textbf{Length.} The longer dimension of a figure. \\
\addlinespace[0.3cm]

\arabicfont\beginR عرض\endR & \textit{`ar\d{d}} & \textbf{Breadth, width.} The shorter dimension of a figure. \\
\addlinespace[0.3cm]

\arabicfont\beginR دائرة\endR & \textit{d\=a'ira} & \textbf{Circle.} Literally ``that which revolves.'' \\
\addlinespace[0.3cm]

\arabicfont\beginR قطر\endR & \textit{qa\d{t}r} & \textbf{Diagonal, diameter.} The line passing through the centre of a figure. \\
\addlinespace[0.3cm]

\midrule
\multicolumn{3}{c}{\textsc{III. Mercantile and Practical Terms}} \\
\midrule

\arabicfont\beginR درهم\endR & \textit{dirham} & \textbf{Dirhem.} A silver coin; the standard monetary unit in algebraic calculations. Derived from the Greek drachma. \\
\addlinespace[0.3cm]

\arabicfont\beginR ثمن\endR & \textit{thaman} & \textbf{Price, value.} The cost of goods in mercantile problems. \\
\addlinespace[0.3cm]

\arabicfont\beginR كيل\endR & \textit{kayl} & \textbf{Measure.} A unit of dry measure for grain, etc. \\
\addlinespace[0.3cm]

\arabicfont\beginR مقدار\endR & \textit{miqd\=ar} & \textbf{Quantity, amount.} A general term for an unknown amount. \\
\addlinespace[0.3cm]

\arabicfont\beginR جملة\endR & \textit{jumla} & \textbf{Sum, total.} The aggregate of quantities. \\
\addlinespace[0.3cm]

\arabicfont\beginR نسبة\endR & \textit{nisba} & \textbf{Ratio, proportion.} The relation between two quantities. \\
\addlinespace[0.3cm]

\midrule
\multicolumn{3}{c}{\textsc{IV. Legal and Legacy Terms}} \\
\midrule

\arabicfont\beginR وصية\endR & \textit{wasiyya} & \textbf{Legacy, bequest.} A gift made by will, especially on death-bed. Subject to the restriction that it may not exceed one-third of the estate without heirs' consent. \\
\addlinespace[0.3cm]

\arabicfont\beginR ميراث\endR & \textit{m\=ir\=ath} & \textbf{Inheritance, heritage.} The estate left by a deceased person. \\
\addlinespace[0.3cm]

\arabicfont\beginR فرائض\endR & \textit{far\=a'i\d{d}} & \textbf{Obligatory shares.} The legally fixed portions of an estate due to prescribed heirs according to Islamic law. \\
\addlinespace[0.3cm]

\arabicfont\beginR عتق\endR & \textit{`itq} & \textbf{Emancipation, manumission.} The freeing of a slave, especially by a master's death-bed declaration. \\
\addlinespace[0.3cm]

\arabicfont\beginR مكاتبة\endR & \textit{muk\=ataba} & \textbf{Manumission contract.} An agreement whereby a slave purchases freedom by instalments. \\
\addlinespace[0.3cm]

\arabicfont\beginR مهر\endR & \textit{mahr} & \textbf{Dowry, marriage settlement.} The payment made by a groom to the bride, part of which may revert to the husband's heirs. \\
\addlinespace[0.3cm]

\midrule
\multicolumn{3}{c}{\textsc{V. Book Title and Formulaic Expressions}} \\
\midrule

\arabicfont\beginR كتاب المختصر في حساب الجبر والمقابلة\endR & \textit{Kit\=ab al-Mukhta\d{s}ar f\=i \d{H}is\=ab al-Jabr wal-Muq\=abala} & \textbf{The Compendious Book on Calculation by Completion and Reduction.} The full Arabic title of al-Khw\=arizm\=i's algebra. \\
\addlinespace[0.3cm]

\arabicfont\beginR بسم الله الرحمن الرحيم\endR & \textit{Bi-smi Ll\=ahi r-Ra\d{h}m\=ani r-Ra\d{h}\=im} & \textbf{In the Name of God, the Gracious, the Merciful.} The traditional opening formula (basmala) of Arabic books. \\
\addlinespace[0.3cm]

\arabicfont\beginR الحمد لله\endR & \textit{Al-\d{h}amdu li-Ll\=ah} & \textbf{Praise be to God.} The opening formula of the author's preface. \\
\addlinespace[0.3cm]
...
\noindent\textbf{Note on Transliteration.} The system of transliteration employed above follows the standard academic convention for Arabic: macrons (\=a, \=i, \=u) denote long vowels; dots beneath consonants (\d{t}, \d{h}, \d{s}, \d{d}) denote the Arabic emphatic consonants; the hamza (') represents the glottal stop; and the `ayn (`) represents the voiced pharyngeal fricative.
...
\centering
\vspace{0.3cm}
\textbf{The Six Canonical Forms of Equations}\\[0.3cm]
\begin{tabular}{lll}
\textbf{Rosen's English} & \textbf{Arabic} & \textbf{Modern Form} \\
Squares equal Roots & \arabicfont\beginR المال يساوي الجذور\endR & $ax^2 = bx$ \\
Squares equal Numbers & \arabicfont\beginR المال يساوي الأعداد\endR & $ax^2 = c$ \\
Roots equal Numbers & \arabicfont\beginR الجذور تساوي الأعداد\endR & $bx = c$ \\
Squares and Roots equal Numbers & \arabicfont\beginR المال والجذور يساويان الأعداد\endR & $ax^2 + bx = c$ \\
Squares and Numbers equal Roots & \arabicfont\beginR المال والأعداد يساويان الجذور\endR & $ax^2 + c = bx$ \\
Roots and Numbers equal Squares & \arabicfont\beginR الجذور والأعداد تساوي المال\endR & $bx + c = ax^2$ \\
\end{tabular}
\vspace{0.3cm}
\end{minipage}}
\end{center}
...
\begin{center}
{\small\textit{Published under the auspices of the Oriental Translation Fund\\London, 1831}}\\[0.3cm]
{\footnotesize\textit{Enhanced Bilingual Edition with Original Arabic Terminology Restored, 2025}}
\end{center}
