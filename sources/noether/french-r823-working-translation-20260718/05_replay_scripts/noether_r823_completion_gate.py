...
STRONG_AUTHORITY_IDENTITY = re.compile(
    r"(?P<authority_file>(?:Noether_R823_cum_de|cum_de)\.tex)|"
    r"(?:R823\s+authority|authority\s+R823|autorité\s+R823|"
    r"German\s+authority|autorité\s+allemande)",
    re.IGNORECASE,
)
AUTHORITY_LINE_MARKER = re.compile(
    r"(?:(?:lines?|lignes?|ligne)\b|l\.)(?:\s*[:.]?)\s*",
    re.IGNORECASE,
)
AUTHORITY_LINE_SPAN = re.compile(
    r"(?<![A-Za-z])(\d+)(?:\s*(?:--|[-–—])\s*(\d+))?"
)
SHA256_CLAIM = re.compile(r"\bsha256\s*=\s*([0-9a-fA-F]{64})\b", re.IGNORECASE)
...
def meaningful_review_text(value: str) -> bool:
    text = value.strip()
    tokens = [token.casefold() for token in re.findall(r"\b\w+\b", text)]
    if len(text) < 80 or len(tokens) < 10 or len(set(tokens)) < 8:
        return False
    most_common = max(Counter(tokens).values(), default=0)
...
def has_strong_source_locator(
    ...
    *,
    ...
    """Accept only locators that resolve to live, in-range R823 authority lines.

    Native PDF/TeX-looking strings are deliberately nonqualifying until a
    resolver can bind their path, range, and hash.  The current terminology
    floor is supported by exact R823 authority lines and does not need those
    weaker alternatives.
    """
    if not authority_path.is_file() or authority_line_count < 1:
        return False
    expected_names = {
        authority_path.name.casefold(),
        "noether_r823_cum_de.tex",
        "cum_de.tex",
    }
    expected_hash = authority_sha256.upper()
    value_sha_claims = [claim.upper() for claim in SHA256_CLAIM.findall(value)]
    if value_sha_claims and any(claim != expected_hash for claim in value_sha_claims):
        return False
    for segment in re.split(r"[;\n]+", value.strip()):
        identity = STRONG_AUTHORITY_IDENTITY.search(segment)
        if identity is None:
            continue
        cited_file = identity.groupdict().get("authority_file")
        if cited_file and cited_file.casefold() not in expected_names:
            continue
        sha_claims = [claim.upper() for claim in SHA256_CLAIM.findall(segment)]
        if sha_claims and any(claim != expected_hash for claim in sha_claims):
            continue
        marker = AUTHORITY_LINE_MARKER.search(segment, identity.end())
        if marker is None:
            continue
        locator_tail = SHA256_CLAIM.sub("", segment[marker.end() :])
        spans = [
            (int(start), int(end or start))
            for start, end in AUTHORITY_LINE_SPAN.findall(locator_tail)
        ]
        if spans and all(
            1 <= start <= end <= authority_line_count for start, end in spans
        ):
            return True
    return False
...
def main() -> int:
    ...
    try:
        ...
        supplied_pdf = args.pdf.resolve()
        success_basename_ok = bool(success_output) and (
            Path(success_output).name.casefold() == args.pdf.name.casefold()
        )
        # LuaLaTeX reports a relative output name in the log even when
        # -output-directory makes the recorder emit an absolute OUTPUT path.
        # The supplied PDF's exact presence in the FLS is the authoritative
        # same-build binding; resolving that relative log name against FLS PWD
        # would invent a path that was never written.
        success_record = supplied_pdf if success_basename_ok else None
        recorder_ok = (
            not fls_warnings
            and args.tex.resolve() in fls_inputs
            and args.log.resolve() in fls_outputs
            and supplied_pdf in fls_outputs
        ...
        if success_record is not None and success_record.suffix.casefold() == ".xdv":
            ...
            recorder_ok = recorder_ok and success_record == supplied_pdf
        checks.append(
            Check(
                "fls_build_binding",
                recorder_ok,
    ...
    if args.terminology_ledger.is_file():
        ...
        authority_line_count = len(authority_text.splitlines())
        locator_kwargs = {
            "authority_path": args.authority_tex,
            "authority_line_count": authority_line_count,
            "authority_sha256": authority_hash,
        }
        strict_source_locator_lines = [
            index + 2
            for index, row in enumerate(terminology)
            if has_strong_source_locator(
                row.get("source_evidence", ""), **locator_kwargs
            )
        ]
        weak_source_locator_lines = [
            ...
            and not has_strong_source_locator(
                row.get("source_evidence", ""), **locator_kwargs
            )
        ]
        term_keys = [
