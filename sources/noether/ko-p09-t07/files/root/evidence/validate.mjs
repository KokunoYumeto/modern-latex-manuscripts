...
const tranche7 = lineBytes(7023, 7133);
if (tranche7.length !== route.tranche7.bytes || sha(tranche7) !== route.tranche7.sha256) errors.push('tranche7 slice');
for (const gap of route.tranche7.excluded_blank_lines) if (lines[gap - 1] !== '') errors.push(`nonblank gap ${gap}`);
...
if (manifest.target_count !== 64 || manifest.target_bytes !== sortedTargets.reduce((sum, item) => sum + item.data.length, 0) || manifest.target_tree_stream_bytes !== tree.length || manifest.target_tree_sha256 !== sha(tree)) errors.push('target aggregate');
...
if (structure.length !== 266 || structure.at(-1)?.id !== 'CJK-KO-P09-STR-0266') errors.push('structure count/head');
...
if (difficulty.length !== 41 || difficulty.at(-1)?.id !== 'CJK-KO-P09-HARD-041') errors.push('difficulty count/head');
...
if (visualStatus.records !== 0 || visualStatus.files !== 0 || visualStatus.bytes !== 0 || visualStatus.render_calls !== 0 || visualStatus.next_authority_line !== 7134) errors.push('visual status');
...
const report = {
  ...
  scope: 'Korean Noether P09 T01-U01 through T07-U64',
  authority: { lines: [6348, 7133], bytes: manifest.scope.bytes, sha256: manifest.scope.sha256 },
