...
const hard = [
  ...
  return {
    ...entry, recorded_time: '2026-08-04', time_precision: 'day', work_unit: Number(entry.id.slice(-3)) >= 29 ? 'Noether P09 Korean T06' : Number(entry.id.slice(-3)) >= 22 ? 'Noether P09 Korean T05' : Number(entry.id.slice(-3)) >= 18 ? 'Noether P09 Korean T04' : 'Noether P09 Korean T01', authority: { edition_id: 'NOETH-DE-ED-0002', whole_sha256: editionSha, pointer_sha256: pointerSha },
    ...
    related_decision_ids: Number(entry.id.slice(-3)) >= 29 ? ['CJK-KO-P09-006'] : Number(entry.id.slice(-3)) >= 22 ? ['CJK-KO-P09-005'] : Number(entry.id.slice(-3)) >= 18 ? ['CJK-KO-P09-004'] : ['CJK-KO-P09-001'], review_state: entry.resolution.startsWith('resolved') ? 'resolved_not_reviewed' : 'held_for_independent_checker', supersession: null,
...
const visualStatus = {
  scope: 'Korean Noether P09 T01--T06 producer translation', records: 0, files: 0, bytes: 0, render_calls: 0,
  reason: 'Translation-only producer created and used no visual artifacts; zero is not visual QA.',
  rights_disposition: { project_generated: 0, rights_cleared: 0, rights_blocked: 0 }, next_authority_line: 7023,
};
...
const manifest = {
  ...
  scope: { tranches: ['T01','T02','T03','T04','T05','T06'], lines: [6348,7022], bytes: lineBytes(authorityLines,6348,7022).length, sha256: sha(lineBytes(authorityLines,6348,7022)), next_line: 7023 },
  state: { translation: 'T01_T06_complete_producer_draft', review: 'unchecked', source_check: 'not_performed', formula_check: 'not_performed', build: 'not_run', render: 'not_run', visual_qa: 'not_run', assembly: 'not_run', approval: 'not_approved' },
  ...
  excluded_blank_lines: [...route.tranche.excluded_blank_lines, ...route.tranche2.excluded_blank_lines, ...route.tranche3.excluded_blank_lines, ...route.tranche4.excluded_blank_lines, ...route.tranche5.excluded_blank_lines, ...route.tranche6.excluded_blank_lines],
...
const report = {
  ...
  target_tree_stream_bytes: manifest.target_tree_stream_bytes, target_tree_sha256: manifest.target_tree_sha256, next_authority_line: 7023,
};
