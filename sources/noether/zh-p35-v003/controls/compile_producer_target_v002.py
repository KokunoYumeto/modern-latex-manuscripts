#!/usr/bin/env python3
"""Run two serial producer-only XeLaTeX passes for one Paper 35 v002 target."""
...
TARGETS = {
    "hans": {
        "label": "zh-Hans-CN",
        "short": "HANS",
        "tex": ROOT / "build/zh-Hans-CN-v002/Noether_Paper35_Chinese_CurrentAuthority_zh-Hans-CN_v002.tex",
        "upstream": ROOT / "controls/HANS_ASSEMBLY_RECORD_v002.json",
        "upstream_key": ("output", "sha256"),
        "record": ROOT / "controls/HANS_MECHANICAL_BUILD_RECORD_v002.json",
        "localization": "PRC-oriented Simplified Chinese producer translation",
    },
    "hant": {
        "label": "zh-Hant-controlled",
        "short": "HANT",
        "tex": ROOT / "build/zh-Hant-controlled-v002/Noether_Paper35_Chinese_CurrentAuthority_zh-Hant-controlled_v002.tex",
        "upstream": ROOT / "controls/OPENCC_PRODUCER_RECORD_v002.json",
        "upstream_key": ("output_sha256",),
        "record": ROOT / "controls/HANT_MECHANICAL_BUILD_RECORD_v002.json",
...
if stale_outputs:
    raise RuntimeError(
        "Refusing to reuse pre-existing v002 build outputs: "
...
for number in (1, 2):
    ...
    transcript = ROOT / "controls" / f"P35_{cfg['short']}_V002_XELATEX_PASS{number}.stdout.log"
    ...
    retained_log = ROOT / "controls" / f"P35_{cfg['short']}_V002_XELATEX_PASS{number}.engine.log"
    ...
    if run.returncode != 0:
        failure = ROOT / "controls" / f"P35_{cfg['short']}_V002_FAILED_BUILD_PASS{number}.json"
        failure.write_text(
...
record = {
    ...
    "operation": f"producer_only_two_pass_xelatex_v002_{cfg['label']}",
    "decision_id": "ZH-D131",
    "checker_return_id": "ZHCHK-NOETHER-P35-RETURN-001",
