...
if (manifest.target_count !== 11 || manifest.target_bytes !== sortedTargets.reduce((sum, item) => sum + item.data.length, 0) || manifest.target_tree_stream_bytes !== tree.length || manifest.target_tree_sha256 !== sha(tree)) errors.push('target aggregate');
...
if (structure.length !== 63 || structure.at(-1)?.id !== 'CJK-KO-P09-STR-0063') errors.push('structure count/head');
...
if (difficulty.length !== 9 || difficulty.at(-1)?.id !== 'CJK-KO-P09-HARD-009') errors.push('difficulty count/head');
...
if (visualStatus.records !== 0 || visualStatus.files !== 0 || visualStatus.bytes !== 0 || visualStatus.render_calls !== 0 || visualStatus.next_authority_line !== 6492) errors.push('visual status');
...
const report = {
  ...
  scope: 'Korean Noether P09 T01-U01 through T01-U11',
  authority: { lines: [6348, 6491], bytes: route.tranche.bytes, sha256: route.tranche.sha256 },
