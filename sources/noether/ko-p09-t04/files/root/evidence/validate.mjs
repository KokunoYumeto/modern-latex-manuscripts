...
if (manifest.target_count !== 29 || manifest.target_bytes !== sortedTargets.reduce((sum, item) => sum + item.data.length, 0) || manifest.target_tree_stream_bytes !== tree.length || manifest.target_tree_sha256 !== sha(tree)) errors.push('target aggregate');
...
if (structure.length !== 137 || structure.at(-1)?.id !== 'CJK-KO-P09-STR-0137') errors.push('structure count/head');
...
if (difficulty.length !== 21 || difficulty.at(-1)?.id !== 'CJK-KO-P09-HARD-021') errors.push('difficulty count/head');
...
if (visualStatus.records !== 0 || visualStatus.files !== 0 || visualStatus.bytes !== 0 || visualStatus.render_calls !== 0 || visualStatus.next_authority_line !== 6726) errors.push('visual status');
...
const report = {
  ...
  scope: 'Korean Noether P09 T01-U01 through T04-U29',
  authority: { lines: [6348, 6725], bytes: manifest.scope.bytes, sha256: manifest.scope.sha256 },
