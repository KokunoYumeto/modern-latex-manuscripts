...
- Resolution: reuse the loader-provided `node_modules` junction already established earlier in this same production session at `C:/tmp/codex_p01_csv_019f757c_a43b/node_modules`, whose recorded target is `${PRIVATE_USER_ROOT}/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules`.
...
- Time precision: 2026-08-04 (date precision only).
- Exact runtime resolution supplied by the active root session: Node `${PRIVATE_USER_ROOT}/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin/node.exe`; node_modules `${PRIVATE_USER_ROOT}/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules`.
