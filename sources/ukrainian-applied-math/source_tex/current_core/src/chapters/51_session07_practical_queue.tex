\chapter{Черга після сесії 07}
\label{chap:session07-queue}

Після інтеграції повного auxiliary local run payload порядок роботи змінився. Тепер не треба витрачати глибокі токени на те, що вже зроблено як source-preserved модуль. Потрібно витрачати їх на контроль якості, формули, терміни, і вузли, які склеюють корпус.
...
\item \textbf{Micro Lie full paper.} Використати Spark fragments як старт, але пройти весь paper file-by-file з компіляцією.
...
\section{Release staging split}

Public: open-source or source-backed educational TeX, compiled PDFs, terminology, build scripts, QA notes.

source-review/staged: scan OCR drafts, copyrighted local chunks, anything requiring source repair from non-open PDFs, and any material requiring additional source review before release.
