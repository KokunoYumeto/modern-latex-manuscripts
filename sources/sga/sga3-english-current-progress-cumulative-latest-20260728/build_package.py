#!/usr/bin/env python3
"""Build the compact latest SGA3 current-progress package."""
...
def excluded(relative: str) -> bool:
    ...
    if parts_lower and parts_lower[0] in {
        "build",
        "build_latest",
        "qa",
        "qa_latest",
        ...
        return True
    if "standalone_build" in parts_lower:
        return True
...
def main() -> int:
    ...
    reader = root / "build_latest" / f"{master.stem}.pdf"
    ...
    source_validation = {
        "schema": "sga3_current_progress_source_history_latest_v1",
    ...
    package_validation = {
        "schema": "sga3_current_progress_outer_package_latest_v1",
        "status": "PASS" if not errors else "FAIL",
        "errors": errors,
        "reader": {
            "filename": PDF_NAME,
