#!/usr/bin/env python3
"""Build exact same-concept Zenodo controls for the idx327-330 crop increment."""
...
PREDECESSOR_ZIP_COUNT = 21
PREDECESSOR_ZIP_MEMBERS = 3152
PREDECESSOR_ZIP_UNCOMPRESSED_BYTES = 314900818
REPLACED_PREDECESSOR_ZIP_MEMBERS = 11
REPLACED_PREDECESSOR_ZIP_UNCOMPRESSED_BYTES = 205768
PARENT_SHA256 = (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--correction-upload-manifest", type=Path, required=True)
    parser.add_argument("--correction-validation", type=Path, required=True)
    parser.add_argument("--correction-zip-dir", type=Path, required=True)
...
def main() -> int:
    ...
    correction_zip_dir = args.correction_zip_dir.resolve()
    ...
    prior_rows_by_name = {row["filename"]: row for row in prior_rows}
    replaced_names = CONTROL_NAMES | CORRECTED_ARCHIVE_NAMES
    retained_rows = [
    ...
    for filename in CORRECTED_ARCHIVE_NAMES:
        if filename not in prior_rows_by_name or filename not in predecessor_files:
            raise RuntimeError(f"missing predecessor archive to correct: {filename}")
        row = prior_rows_by_name[filename]
        identity = predecessor_files[filename]
        if (int(row["bytes"]), row["sha256"]) != (
            int(identity["bytes"]),
            identity["sha256"],
        ):
            raise RuntimeError(f"predecessor correction identity mismatch: {filename}")

    correction_rows, correction_validation = load_crop_bundle(
        args.correction_upload_manifest,
        args.correction_validation,
        correction_zip_dir,
    )
    correction_names = {row["filename"] for row in correction_rows}
    if correction_names != CORRECTED_ARCHIVE_NAMES:
        raise RuntimeError(
            "correction archive set mismatch: "
            f"missing={sorted(CORRECTED_ARCHIVE_NAMES - correction_names)} "
            f"extra={sorted(correction_names - CORRECTED_ARCHIVE_NAMES)}"
        )
...
adds the next compact SGA6 source-audit crop increment. All 41 files outside
the three release controls and two corrected crop archives from predecessor
...
Five image archives now preserve 1,936 targeted formula, glyph, punctuation,
diagram, and prose-detail crops actually used during the active SGA6 source
audit. Four metadata archives preserve parent identity, page/crop provenance,
exact manifests, and 1,778 routine whole-page or page-band derivatives as
...
index 329 and provenance for all 21 associated audit images from indices 327
through 330. The temporal snapshot closes after cold reverification reached
index 330 and before index 331 began. Earlier targeted archives also contain
crops from prior same-parent audit phases, so no continuous index-coverage claim
is made.

The two idx322--326 archives are replaced under their existing filenames solely
to correct machine provenance links from older first-pass audit entries
#319--#323 to the controlling cold-reverification entries #1074--#1078. The
targeted PNG, parent identity, interval, rights boundary, and audit conclusions
are unchanged. Predecessor record {args.predecessor_record} remains immutable.
...
"""
    ...
    for row in correction_rows + crop_rows:
        is_metadata = "RightsBlocked_Metadata" in row["filename"]
        is_correction = row["filename"] in CORRECTED_ARCHIVE_NAMES
        provenance_prefix = (
            "SGA6 idx322-326 provenance-corrected source-audit archive; "
            if is_correction
            else "SGA6 incremental high-detail source-audit crop release; "
        )
        final_rows.append(
            {
                ...
                "provenance": (
                    f"{provenance_prefix}GitHub commit {args.github_commit}; "
                    "parent scan SHA-256 "
                    f"{PARENT_SHA256}"
                ),
                "status": (
                    "corrected_provenance_and_rights_blocked_metadata"
                    if is_correction and is_metadata
                    else "corrected_targeted_visual_evidence_no_pixel_change"
                    if is_correction
                    else "provenance_and_rights_blocked_metadata"
                    if is_metadata
    ...
    correction_zip_results = correction_validation["zip_validation"]
    correction_zip_members = sum(
        int(item["members"]) for item in correction_zip_results.values()
    )
    correction_zip_uncompressed = sum(
        int(item["uncompressed_bytes"])
        for item in correction_zip_results.values()
    )
    ...
    validation = {
        ...
        "provenance_corrected_sga6_crop_archives": [
            row["filename"] for row in correction_rows
        ],
        ...
        "github": {
            ...
            "package": (
                ...
                "coldreverify-idx327-330-20260723"
            ),
            "correction_package": (
                "sources/sga/sga6-high-detail-source-audit-crops-"
            ...
            "metadata_files": 14,
            "readback_files": 14,
            "correction_package_files": 15,
            "correction_readback_files": 15,
            "readback_errors": [],
        },
        "sga6_source_audit_crops": {
            ...
            "cumulative_targeted_images": 1936,
            "cumulative_rights_blocked_routine_page_derivatives": 1778,
            "provenance_corrected_zip_count": len(correction_zip_results),
            "provenance_corrected_zip_members": correction_zip_members,
            "provenance_corrected_zip_uncompressed_bytes": (
                correction_zip_uncompressed
            ),
            "new_zip_count": len(new_zip_results),
            "new_zip_members": new_zip_members,
            "new_zip_uncompressed_bytes": new_zip_uncompressed,
            "correction_producer_validation_sha256": sha256(
                args.correction_validation
            ),
        ...
        "zip_member_count": (
            ...
            - REPLACED_PREDECESSOR_ZIP_MEMBERS
            + correction_zip_members
            + new_zip_members
        ),
        "zip_uncompressed_bytes": (
            PREDECESSOR_ZIP_UNCOMPRESSED_BYTES
            - REPLACED_PREDECESSOR_ZIP_UNCOMPRESSED_BYTES
            + correction_zip_uncompressed
