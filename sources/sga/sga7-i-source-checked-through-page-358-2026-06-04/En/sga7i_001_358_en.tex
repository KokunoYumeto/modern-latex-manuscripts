...
\begin{equation}\qquad\text{(6.2.1)}
\begin{tikzcd}[column sep=large,row sep=large]
...
\begin{equation}\qquad\text{(1.1.1)}
...
\begin{equation}\qquad\text{(1.1.3.0)}
\begin{aligned}
...
\begin{equation}\qquad\text{(1.1.3.1)}
...
\begin{equation}\qquad\text{(1.1.3.2)}
...
\begin{equation}\qquad\text{(1.1.4.1)}
\begin{tikzcd}[column sep=large,row sep=large]
...
\begin{equation}\qquad\text{(1.1.5.3)}
...
\begin{equation}\qquad\text{(1.1.5.4)}
...
\begin{equation}\qquad\text{(1.1.5.5)}
...
\begin{equation}\qquad\text{(1.1.5.10)}
...
\begin{equation}\qquad\text{(1.2.1)}
\begin{tikzcd}[column sep=large,row sep=large]
...
\begin{equation}\qquad\text{(1.3.1.1)}
  \Hompt(P,G)\simeq (e),
\end{equation}
...
\begin{equation}\qquad\text{(1.3.1.2)}
...
\begin{equation}\qquad\text{(1.3.4.1)}
...
\begin{equation}\qquad\text{(1.3.4.2)}
...
\begin{equation}\qquad\text{(1.3.7.3)}
...
\begin{equation}\qquad\text{(1.3.8.1)}
...
\begin{equation}\qquad\text{(1.4.1)}
  P=\Z[I],
\end{equation}
...
\begin{equation}\qquad\text{(1.4.2)}
  \EXT(P,G)\longrightarrow \TORS(I,G_I).
\end{equation}
This functor is an equivalence of categories.  Equivalently, for every \(G\) the maps
\begin{align}
  \Hom_{\Z}(P,G)&\longrightarrow H^0(I,G_I)=G(I), \qquad\text{(1.4.3)}\\
  \operatorname{Ext}^{1}_{\Z}(P,G)&\longrightarrow H^1(I,G_I) \qquad\text{(1.4.4)}
\end{align}
...
\begin{equation}\qquad\text{(1.4.5)}
...
\begin{equation}\qquad\text{(2.0.1)}
\begin{tikzcd}
...
\begin{equation}\qquad\text{(2.0.2)}
  P\times Q=Q_P,
\end{equation}
and symmetrically as the inverse-image \(Q\)-Group of \(P\),
\begin{equation}\qquad\text{(2.0.3)}
  P\times Q=P_Q.
\end{equation}
...
\begin{equation}\qquad\text{(2.0.4)}
...
\begin{equation}\qquad\text{(2.0.5)}
\begin{tikzcd}[column sep=large,row sep=large]
...
\begin{equation}\qquad\text{(2.0.6)}
\begin{tikzcd}[column sep=large,row sep=large]
...
\begin{equation}\qquad\text{(2.0.7)}
...
\begin{equation}\qquad\text{(2.0.8)}
\begin{tikzcd}[column sep=large,row sep=large]
...
\end{equation}
and
\begin{equation}\qquad\text{(2.0.9)}
\begin{tikzcd}[column sep=large,row sep=large]
...
\begin{equation}\qquad\text{(2.1.1)}
...
\begin{equation}\qquad\text{(2.1.1.2)}
...
\begin{equation}\qquad\text{(2.1.1.1)}
...
\begin{equation}\qquad\text{(2.2.3)}
  e_E\in\Gamma(E),
\end{equation}
...
\begin{equation}\qquad\text{(2.3.3)}
...
\begin{equation}\qquad\text{(2.4.1)}
  \BIEXT(P,Q;G)
\end{equation}
...
\begin{equation}\qquad\text{(2.4.2)}
...
\begin{equation}\qquad\text{(2.5.1.1)}
  P\times Q\times G,
\end{equation}
...
\begin{equation}\qquad\text{(2.5.3.2)}
...
\begin{equation}\qquad\text{(2.5.4.0)}
...
\begin{equation}\qquad\text{(2.5.4.1)}
...
\begin{equation}\qquad\text{(2.6.1)}
...
\begin{equation}\qquad\text{(2.6.2)}
...
\begin{equation}\qquad\text{(2.7.1)}
  {}^s({}^sE)=E.
\end{equation}
...
Let
\begin{equation}\qquad\text{(2.8.0)}
...
\begin{equation}\qquad\text{(2.8.1)}
...
\begin{equation}\qquad\text{(2.9.1)}
...
\begin{equation}\qquad\text{(2.9.2)}
  P\times Q,
...
\begin{equation}\qquad\text{(2.9.4.1)}
...
\begin{equation}\qquad\text{(2.9.4.2)}
...
\begin{equation}\qquad\text{(2.9.5.1)}
...
\begin{equation}\qquad\text{(2.9.6.0)}
...
\begin{equation}\qquad\text{(2.9.6.1)}
  \Biext(P,Q;G)
...
\begin{equation}\qquad\text{(2.9.6.2)}
...
\begin{equation}\qquad\text{(2.10.3.1)}
...
\begin{equation}\qquad\text{(3.0.1)}
...
\begin{equation}\qquad\text{(3.1.1)}
...
\begin{equation}\qquad\text{(3.1.2)}
...
\begin{equation}\qquad\text{(3.1.3)}
\begin{tikzcd}[column sep=large,row sep=large]
...
The obvious functor
\begin{equation}\qquad\text{(3.1.4)}
...
\begin{equation}\qquad\text{(3.1.6)}
...
\begin{equation}\qquad\text{(3.1.8)}
  \cPsi^0_L(A)
...
\begin{equation}\qquad\text{(3.2.1)}
...
\begin{equation}\qquad\text{(3.2.4)}
...
\begin{equation}\qquad\text{(3.2.5.1)}
...
\begin{equation}\qquad\text{(3.2.5.2)}
\begin{tikzcd}[column sep=large,row sep=large]
...
\begin{equation}\qquad\text{(3.3.3.1)}
...
\begin{equation}\qquad\text{(3.3.6.1)}
\begin{aligned}
...
\begin{equation}\qquad\text{(3.4.3)}
...
\begin{equation}\qquad\text{(3.4.7.1)}
...
\begin{equation}\qquad\text{(3.4.8.1)}
...
\begin{equation}\qquad\text{(3.4.8.2)}
...
\begin{equation}\qquad\text{(3.4.9.1)}
\begin{aligned}
0&\to\Ext^0(L'',A)\to\Ext^0(L,A)\to\Ext^0(L',A)\\
...
\begin{equation}\qquad\text{(3.5.1)}
...
\begin{equation}\qquad\text{(3.5.3.1)}
...
\begin{equation}\qquad\text{(3.5.3.2)}
  \pr_1^*E\;\pr_2^*E\;(+)^*E^{-1}
\end{equation}
...
\begin{equation}\qquad\text{(3.6.1)}
  L_\bullet(P)\to P,
  \qquad
  L_\bullet(Q)\to Q,
\end{equation}
and hence
\begin{equation}\qquad\text{(3.6.2)}
...
\begin{equation}\qquad\text{(3.6.4.1)}
  \cPsi_{L(P,Q)}.
\end{equation}
...
\begin{equation}\qquad\text{(3.6.5.1)}
...
\begin{equation}\qquad\text{(3.7.4)}
...
\begin{equation}\qquad\text{(3.7.5.3)}
...
\begin{equation}\qquad\text{(3.7.5.4)}
 \partial:\Biext^0(P',Q;G)\longrightarrow \Biext^1(P'',Q;G).
\end{equation}
...
\begin{equation}\qquad\text{(3.7.6.1)}
...
\begin{equation}\qquad\text{(3.8.1)}
...
\begin{equation}\qquad\text{(3.8.2)}
\begin{tikzcd}[column sep=large,row sep=large]
...
\begin{equation}\qquad\text{(3.8.3)}
...
\begin{equation}\qquad\text{(3.8.9)}
...
\begin{equation}\qquad\text{(3.8.10)}
...
\begin{equation}\qquad\text{(3.8.11)}
...
\begin{equation}\qquad\text{(1.1.2)}
...
\begin{equation}\qquad\text{(1.1.3)}
\begin{gathered}
...
\begin{equation}\qquad\text{(1.1.4)}
\begin{gathered}
...
\]
Then
\begin{equation}\qquad\text{(1.3.4)}
        P\otimes Q=0,
        \qquad
...
\end{equation}
and hence
\begin{equation}\qquad\text{(1.3.5)}
        \Biext^1(P,Q;G)
        \simeq \Hom(T_n(P),D(Q))
        \simeq \Hom(Q,D(T_n(P))).
\end{equation}
...
\begin{equation}\qquad\text{(1.3.6)}
...
If
\begin{equation}\qquad\text{(1.5.1)}
        \Hom(P,\Ext^1(Q,G))=0,
\end{equation}
then (1.1.4) yields the canonical isomorphism
\begin{equation}\qquad\text{(1.5.2)}
...
\begin{equation}\qquad\text{(1.6.3)}
        \Ext^1(P,Q')\times Q
...
\end{equation}
It is the composite
\begin{equation}\qquad\text{(2.1.2)}
        {}_nP\otimes{}_nQ
...
\begin{equation}\qquad\text{(2.1.4)}
        \Torop_*^{\mathbb Z}(P,Q)
        \Longleftarrow
...
\begin{equation}\qquad\text{(2.1.5)}
...
\begin{equation}\qquad\text{(2.1.8.1)}
...
\begin{center}
Continuation from source scan page 237 through source scan page 270.
\end{center}
...
% Additions for SGA 7-I Expose VIII continuation
...
\providecommand{\Tate}{\operatorname{T}}
...
\section*{Exposé VIII, continuation. Complements on biextensions and group schemes}
\begin{center}
Original source scan pages 271--317; completion of Exposé VIII.
\end{center}

\subsection*{4. The case of a base field: divisor correspondences and separation}
...
\textbf{4.4.}  It is not known here whether the cube condition just mentioned is automatic.  At least one knows from Raynaud's theorem that, for a given \(L\), there is an integer \(n\geq 1\), a power of the characteristic exponent of \(k\), such that \(L^{\otimes n}\) satisfies this condition; hence \(L^{\otimes n}\) comes from a biextension of \((P,Q)\) by \(\Gm\).  One can also find a radicial extension \(k'/k\) for which the required condition becomes true after base extension.  Thus, after this extension, \(L\) gives a birigidified invertible module which comes from a biextension.  In particular, if \(k\) is perfect, the condition of 4.3 is always satisfied.  This last assertion, which formally implies the preceding ones by a trace argument, also follows from 4.7 below; see 4.9.

We recall the following lemma, in the form needed below, from EGA, Errata IV, 53, 21.4.13.

\textbf{Lemma 4.5.}  Let \(f:X\to Y\) be a smooth surjective morphism with integral fibres, with \(Y\) normal integral of generic point \(\eta\).  Let \(L\) be an invertible module on \(X\) whose restriction to \(X_\eta\) is trivial.  Then there exists an invertible module \(M\) on \(Y\) such that \(L\simeq f^*M\).

\textbf{Proposition 4.6.}  Let \(P,Q\) be smooth connected algebraic groups over \(k\), with \(P\) affine.  Suppose moreover that \(P\) admits a composition series whose successive factors are either tori or copies of the additive group \(\Ga\), a condition satisfied for example if \(k\) is perfect.  Suppose, alternatively, that \(Q\) is an extension of an abelian variety \(B\) by a smooth connected affine algebraic group satisfying the same condition.  Under these hypotheses every birigidified invertible module \(L\) on \(P\times Q\) is trivial.  A fortiori every biextension of \((P,Q)\) by \(\Gm\) is trivial.

Indeed, by descent and by 4.3 we may pass to a finite separable extension and then to the case where \(k\) is separably closed.  Then every torus is split.  If \(P\) has a composition series with quotients isomorphic to \(\Ga\) or \(\Gm\), repeated use of Lemma 4.5, together with \(\Pic(\Ga)=\Pic(\Gm)=0\), shows that \(\Pic(P)=0\).  Applying 4.5 to \(P\times Q\to Q\) and using the trivialization on \(e_P\times Q\), one obtains that \(L\) is trivial.

In the second case one applies Lemma 4.5 to the morphism \(P\times Q\to P\times B\).  Its generic fibre is a torsor under the affine group occurring in the extension of \(Q\), and it is trivial by the standard relations \(H^1(K,\Ga)=0\) and \(H^1(K,\Gm)=0\).  After replacing the resulting line bundle on \(P\times B\) by an equivalent birigidified one, one reduces to the case where \(Q\) itself is an abelian variety.  Then \(L\) defines a morphism \(P\to Q'\), where \(Q'\) is the dual abelian variety of \(Q\).  A morphism from a smooth connected affine algebraic group to an abelian variety is constant; since it is pointed, it is zero.  Therefore \(L\) is pulled back from \(P\), and the rigidification forces this pullback to be trivial.

\textbf{Proposition 4.7.}  Let \(P,Q\) be smooth connected algebraic groups over \(k\).  Assume that \(P\) is an extension of an abelian variety \(A\) by a smooth connected affine algebraic group \(L\) which has a composition series with factors either tori or copies of \(\Ga\).  Then every birigidified invertible module on \(P\times Q\) comes from a biextension of \((P,Q)\) by \(\Gm\).  Moreover the functor
\[
  \Biext^1(A,Q;\Gm)\longrightarrow \Biext^1(P,Q;\Gm)
\]
induces an isomorphism of groups.

The first assertion is reduced, again by descent, to the case where \(k\) is separably closed.  The preceding proof shows that the given module is the inverse image of a birigidified invertible module on \(A\times Q\).  If \(P=A\) is abelian, this module defines a morphism \(Q\to A'\), and is the pullback of the Weil biextension on \(A\times A'\).  The result follows from VII 2.9.6, where divisor correspondences on a product of abelian varieties are shown to come from biextensions.  For general \(P\), the asserted isomorphism follows from the exact sequence of \(\Biext^1\)'s associated with
\[
...
\textbf{Corollary 4.8.}  Under the hypotheses of 4.7, suppose that \(Q\) is an extension of an algebraic group \(B\) by a smooth connected affine group \(M\).  Then
\[
  \Biext^1(A,B;\Gm)\longrightarrow \Biext^1(P,Q;\Gm)
\]
is an equivalence of rigid categories, hence an isomorphism on groups of isomorphism classes.  This follows as above from the exact \(\Biext\)-sequence attached to \(0\to M\to Q\to B\to 0\) and from the vanishing supplied by 4.6.

\textbf{Remark 4.9.}  If \(k\) is perfect and \(P,Q\) are smooth connected algebraic groups, each of them is an extension of an abelian variety by a smooth connected affine group, and that affine group is a product of a torus by an iterated extension of groups \(\Ga\).  Hence the theory of biextensions of \((P,Q)\) by \(\Gm\) is reduced, by 4.8, to the classical theory for the pair of abelian varieties \((A,B)\).

\textbf{Corollary 4.10.}  Let \(P,Q\) be smooth connected algebraic groups over \(k\), let \(L\) be a smooth connected affine algebraic subgroup of \(P\), and let \(E\) be a biextension of \((P,Q)\) by \(\Gm\).  The pairing on Tate modules deduced from \(E\),
\[
  T_\ell(P)\times T_\ell(Q)\longrightarrow T_\ell(\Gm),
\]
...
\textbf{4.11. Separating biextensions.}  Let \(P,Q\) be smooth algebraic groups over \(k\), let \(E\) be a biextension of \((P,Q)\) by \(\Gm\), and let \(T\) be the maximal subtorus of \(P\).  We seek the left annihilator of the pairing
\[
  T_\ell(P)\times T_\ell(Q)\longrightarrow T_\ell(\Gm).
\]
...
\[
  A'\longrightarrow (B')'
\]
associated with \(F'\), then the left annihilator is the inverse image of \(T_\ell(C')\).  We shall say that \(E\) is \emph{left separating} if this annihilator is exactly \(T_\ell(T)\); equivalently, the associated morphism \(A'\to (B')'\) has finite kernel.  Dually, \(E\) is \emph{right separating} if the dual associated morphism has finite kernel.  It is \emph{separating} if both conditions hold, equivalently if the associated morphism is an isogeny.  These notions are invariant under arbitrary extension of the base field.

\textbf{4.12.}  Let \(P\) be a smooth connected group scheme and \(L\) an invertible module on \(P\).  Put
\[
  \delta(L)=m^*L\,\pr_1^*L^{-1}\,\pr_2^*L^{-1},
\]
with notation as in 4.2.  This is a birigidified invertible module on \(P\times P\).  If it comes from a biextension, that biextension is unique by 4.3 and is symmetric.  When \(P\) is an extension of an abelian variety \(A\) by a group built from tori and additive groups, and when the maximal torus is split, the construction reduces to the abelian case: one may write \(L=f^*M\), for \(f:A\to P\), and then \(\delta(L)\) is the pullback of \(\delta(M)\).

\textbf{Proposition 4.13.}  In the preceding situation, assume \(L\) is ample.  If the biextension \(\delta(L)\) of \((P,P)\) by \(\Gm\) is defined, then it is separating.  If \(P\) is an extension of an abelian variety \(A\) by a smooth connected affine group satisfying 4.7, the biextension of \((A,A)\) from which \(\delta(L)\) comes is of the form \(\delta(M)\), with \(M\) ample on \(A\).  This is reduced to the algebraically closed case and then to Raynaud's theorem that ampleness descends to the abelian quotient; Weil's theory then gives that the corresponding homomorphism \(A\to A'\) is an isogeny.

\textbf{Remark 4.14.}  In the favourable case where \(P\) is an extension of an abelian variety \(A\) by a group built from tori and additive groups, the composite
\[
  \Pic(P)\longrightarrow \Biext^1(P,P;\Gm)
     \xrightarrow{\sim}\Biext^1(A,A;\Gm)
\]
identifies the Neron-Severi group \(\NS(A)\), or equivalently \(\NS(P)=\Pic(P)/\Pic^0(P)\), with the group of symmetric biextensions of \((P,P)\) by \(\Gm\).  An ample divisor correspondence, or ample biextension, is one coming from an ample element of this Neron-Severi group.  Raynaud's theorem says that an element \(\xi\in\Pic(P)\) is ample precisely when its image in the group of symmetric biextension classes is ample in this sense.

\section*{5. Extensions and biextensions by truncated constant group schemes without torsion}

The next two sections record auxiliary results used in Section 7, especially in the case \(\ell=p\).

\textbf{Proposition 5.1.}  Let \(S\) be geometrically unibranch and irreducible with generic point \(\eta\), and let \(Z\) be a torsion-free group, viewed as a constant group scheme \(Z_S\) on \(S\).  Every torsor under \(Z_S\) is trivial, and the functor sending a torsor to its generic fibre is an equivalence with the category of torsors under \(Z_\eta\).

The proof first uses descent to see that such torsors are represented and étale-locally trivial.  It is then enough to prove \(H^1(S,Z_S)=0\).  Passing to the strict localization and then to the spectrum of a field reduces the assertion to the elementary fact that, for a profinite Galois group \(\Gamma\),
\[
  H^1(\Gamma,Z)=\Hom_{\mathrm{cont}}(\Gamma,Z)=0
\]
when \(Z\) is torsion-free and discrete.

\textbf{Corollary 5.2.}  With the same hypotheses, let \(P\) be smooth over \(S\).  Then torsors under the induced constant group on \(P\) are controlled by the generic fibres.  Indeed, \(P\) is a locally finite sum of geometrically unibranch irreducible preschemes whose generic points lie over \(\eta\), and 5.1 applies component by component.  The analogous assertion for bitorsors follows in the same way.

\textbf{Proposition 5.3.}  Let \(S,Z\) be as in 5.1 and let \(P\) be a smooth group scheme over \(S\).  Restriction to the generic fibre gives an equivalence from the category of group-scheme extensions of \(P\) by \(Z_S\) to the corresponding category over \(\eta\).  If \(P\) and \(Z\) are commutative, it also gives an equivalence for commutative extensions.  The proof is the bitorsor description of VII 1.1.6, applied to \(P\), \(P\times P\), and \(P\times P\times P\).

\textbf{Corollary 5.4.}  If \(P,Q\) are smooth commutative group schemes over \(S\), restriction to \(\eta\) gives an equivalence between biextensions of \((P,Q)\) by \(Z_S\) and biextensions of \((P_\eta,Q_\eta)\) by \(Z_\eta\).

\textbf{Proposition 5.5.}  Let \(k\) be a field, \(Z\) a torsion-free group, and \(P\) a group scheme locally of finite type over \(k\).  Let \(P^0\) be the neutral connected component and \(\Phi=P/P^0\) the group of connected components.
\begin{enumerate}[label=(\roman*)]
\item The natural functor from extensions of \(\Phi\) by \(Z_k\) to extensions of \(P\) by \(Z_k\), and similarly for commutative extensions, is an equivalence.  Hence, if \(P\) is connected, every such extension is trivial and the category is rigid.
\item If \(\Phi\) is torsion and \(P,Z\) are commutative, the category of extensions of \(P\) by \(Z_k\) is rigid.  If some \(n>0\) kills \(\Phi\), then the group of commutative extension classes is canonically
\[
  \Ext^1(P,Z_k)\simeq \Hom\bigl(\Phi,\QQ/Z\bigr),
\]
the isomorphism being the boundary map attached to \(0\to Z\to \QQ\to\QQ/Z\to 0\).
\end{enumerate}

\textbf{Corollary 5.6.}  Let \(P,Q\) be commutative group schemes locally of finite type over \(k\), and put \(\Phi=P/P^0\), \(\Psi=Q/Q^0\).  Biextensions of \((P,Q)\) by \(Z_k\) are equivalent to biextensions of \((\Phi,\Psi)\) by \(Z_k\).  If one of \(\Phi,\Psi\) is torsion, the category is rigid; if one is killed by an integer \(n>0\), then
\[
  \Biext^1(P,Q;Z_k)\simeq \Hom(\Phi\otimes\Psi,\QQ/Z).
\]

\textbf{5.7. Truncated constant schemes.}  Let \(S\) be a scheme, \(T\subset S\) a closed subscheme, and \(U=S-T\).  For a set \(Z\), the \emph{constant scheme on \(S\) truncated on \(T\)} is obtained by gluing the disjoint sum of copies of \(S\), indexed by \(Z\), along the common open \(U\).  It will be denoted \(Z_{S,T}\).  It is étale over \(S\), and for a variable \(S\)-scheme \(S'\) one has functorially
\[
...
\textbf{5.8.}  Assume \(Z\) is commutative.  Restriction to \(T\) gives an equivalence from torsors under \(Z_{S,T}\) to torsors under \(Z_T\).  Full faithfulness reduces by fpqc descent to the trivial torsor and is the preceding Hom formula; essential surjectivity is reduced to the cohomological vanishing \(R^1i_*Z_T=0\) for the closed immersion \(i:T\to S\).

\textbf{Proposition 5.9.}  For a commutative group scheme \(P\) over \(S\), restriction to \(T\) gives an equivalence between extensions of \(P\) by \(Z_{S,T}\) and extensions of \(P_T\) by \(Z_T\).

\textbf{Corollary 5.10.}  If \(Q\) is another commutative group scheme over \(S\), restriction to \(T\) gives an equivalence between biextensions of \((P,Q)\) by \(Z_{S,T}\) and biextensions of \((P_T,Q_T)\) by \(Z_T\).

\section*{6. Extensions and biextensions by the Neron-Raynaud model of \(\Gm\)}

\textbf{6.1.}  Associated with \(Z_{S,T}\) is a canonical surjection from the ordinary constant sheaf \(Z_S\).  Let \(\widetilde Z_{S,T}\) denote its kernel; thus there is an exact sequence, functorial in \(Z\),
\[
  0\longrightarrow \widetilde Z_{S,T}\longrightarrow Z_S\longrightarrow Z_{S,T}\longrightarrow 0.
\]
The kernel is the disjoint sum of pieces \(S_n\), \(n\in Z\), with \(S_0=S\) and \(S_n=U\) for \(n\neq0\).  It follows that for any group \(G\) on \(S\), restriction to \(U\) induces a bijection
\[
  \Hom(\widetilde Z_{S,T},G)\simeq \Hom(Z_U,G_U).
\]

\textbf{Lemma 6.2.}  Let \(G\) be a commutative group on \(S\).  The category of commutative group extensions of \(\widetilde{\ZZ}_{S,T}\) by \(G\) is equivalent to the category of pairs \((F,s)\), where \(F\) is a torsor under \(G\) and \(s\) is a section of \(F|_U\).  Under this equivalence, the extension corresponding to \((F,s)\) has, over the component indexed by an integer \(n\), the torsor \(F^{\otimes n}\), with the trivialization over \(U\) deduced from \(s^{\otimes n}\).
...
\textbf{6.3.}  We now take \(G=\Gm\).  A torsor under \(\Gm\) is an invertible module \(L\), and a section over \(U\) is a trivialization of \(L|_U\).  Thus the data are a pair \((L,s)\).  When \(U\) is schematically dense in \(S\), with \(S\) locally noetherian, or more generally reduced with locally finite irreducible components, such a pair is the same thing as a divisor \(D\) supported on \(T\): one associates
\[
  (L,s)=(\mathcal O_S(-D),\tau|_U),
\]
where \(\tau\) is the canonical rational section.  In all cases a divisor \(D\) supported on \(T\) defines in this way an extension \(\cG\) of \(\widetilde{\ZZ}_{S,T}\) by \(\Gm\).  This is called the \emph{Neron-Raynaud model of \(\Gm\)} associated with \(D\) and \(T\); when \(T=\supp D\), it is omitted from the notation.  Its sections are the regular meromorphic functions on \(S\) whose divisor is locally an integral multiple of \(D\).

\textbf{6.4.}  Suppose \(S\) is normal with locally finite irreducible components, and that \(D\) is positive with all multiplicities equal to one.  Put \(T=\supp D\), and suppose \(T\) is locally irreducible and geometrically unibranch.  Then every rational function on \(S\), regular on \(U\), has divisor locally an integral multiple of \(D\); hence
\[
  \cG(S)=\Gamma(U,\mathcal O_U^*).
\]
Moreover, for every smooth \(S\)-scheme \(S'\), one obtains functorially
\[
  \Hom_S(S',\cG)\simeq \Gamma(S'_U,\mathcal O^*_{S'_U}).
\]
This characterizes \(\cG\) uniquely and justifies the name Neron-Raynaud model.

\textbf{Theorem 6.5.}  Let \(S\) be normal and locally integral, let \(D\) be a positive divisor without multiple components, with geometrically unibranch and locally integral support, and let \(\cG\) be the associated Neron-Raynaud model.  The restriction functor from torsors under \(\cG\) on \(S\) to torsors under \(\Gm\) on \(U\) is fully faithful.  If the local rings of \(S\) are factorial, for instance if \(S\) is regular, this functor is an equivalence.

Full faithfulness is local for the Zariski topology and reduces to the trivial torsor; then it is exactly the equality \(\cG(S)=\Gamma(U,\mathcal O_U^*)\).  Essential surjectivity in the factorial case is the assertion that every invertible module on \(U\) extends to \(S\).

\textbf{Corollary 6.6.}  Let \(S\) be locally noetherian and regular, and let \(D\subset S\) be a reduced geometrically unibranch closed subscheme, pure of codimension one.  Let \(\cG\) be the associated Neron-Raynaud model and let \(P\) be a smooth commutative group scheme over \(S\).  Restriction to \(U=S-D\) induces an equivalence between commutative group extensions of \(P\) by \(\cG\) and extensions of \(P_U\) by \(\Gm\).  The non-commutative version is analogous, with bitorsors in place of torsors.

\textbf{Corollary 6.7.}  Under the same hypotheses, if \(P,Q\) are smooth commutative group schemes over \(S\), restriction to \(U\) gives an equivalence between biextensions of \((P,Q)\) by \(\cG\) and biextensions of \((P_U,Q_U)\) by \(\Gm\).

\textbf{Remarks 6.8.}  When \(S\) is a trait and \(D\) is its closed point, \(U\) is the spectrum of the fraction field and \(\cG\) is the usual Neron-Raynaud model of the multiplicative group.  The proofs of 6.6 and 6.7 use Theorem 6.5 after smooth base change, so bases more general than traits enter formally.  More generally, if \(U\) is only a schematically dense open in a normal locally noetherian scheme \(S\), the corresponding Neron-Raynaud model may be a sheaf rather than a representable scheme; it is described by the exact sequence
\[
  0\longrightarrow \Gm\longrightarrow \cG\longrightarrow \cD\longrightarrow 0,
\]
where \(\cD\) is the sheaf of divisors supported in \(S-U\).  Compatibility with smooth base change follows from EGA IV, Errata 53, 21.4.9.

\textbf{6.9. Autocritique, after observations of P. Deligne.}  Deligne points out that Sections 5 and 6 conceal a simple geometric statement behind a hand-made formalism.  The core assertions are these: if \(j:U\hookrightarrow S\) is a schematically dense open immersion in a normal noetherian scheme, then \(\Gm{}_{S}\) is a subsheaf of \(j_*\Gm{}_{U}\), and the quotient sheaf of divisors commutes with smooth base change.  If \(U=S-Y\) with \(Y\) a geometrically unibranch divisor, the quotient is represented by the truncated constant group scheme associated with the closed part.  Finally, the truncated constant sheaf attached to a closed subscheme is representable and compatible with base change.  These statements give, on the smooth site, the Neron-Raynaud model used above and allow the results of Section 6 to be deduced formally.

\section*{7. Canonical prolongations of extensions and biextensions by \(\Gm\)}

\textbf{7.0.}  Let \(S\) be a trait, \(\eta\) its generic point, \(s\) its closed point, and \(k=k(s)\).  Let \(P\) be a smooth commutative group scheme over \(S\), and put \(P_s=P\times_S s\).  In this section all groups are commutative.

\textbf{Theorem 7.1.}
\begin{enumerate}[label=\alph*)]
\item Assume that the group of connected components
\[
  \Phi=P_s/P_s^0
\]
is torsion, for example if \(P\) is of finite type over \(S\).  Then the functor from extensions of \(P\) by \(\Gm\) to extensions of \(P_\eta\) by \(\Gm\) is fully faithful.  If some \(n>0\) kills \(\Phi\), then every extension \(E_\eta\) of \(P_\eta\) by \(\Gm\) has a canonical obstruction
\[
  d(E_\eta):\Phi\longrightarrow \QQ/Z,
\]
whose vanishing is necessary and sufficient for \(E_\eta\) to be the generic fibre of an extension over \(S\).  If \(P_s\) is connected, the restriction functor is an equivalence.
\item Let \(P,Q\) be smooth commutative group schemes over \(S\), put
\[
  \Phi=P_s/P_s^0,
  \qquad
  \Psi=Q_s/Q_s^0,
\]
and assume that \(\Phi\) or \(\Psi\) is torsion.  Then restriction from biextensions of \((P,Q)\) by \(\Gm\) to biextensions of \((P_\eta,Q_\eta)\) by \(\Gm\) is fully faithful.  If an integer \(n>0\) kills \(\Phi\) or \(\Psi\), a biextension \(E_\eta\) has a canonical obstruction pairing
\[
  d(E_\eta):\Phi\otimes\Psi\longrightarrow \QQ/Z,
\]
...
The proof replaces \(\Gm\) by the Neron-Raynaud model \(\cG\).  By 6.6 and 6.7, restriction from extensions or biextensions by \(\cG\) to the generic fibre is an equivalence.  The only remaining question is the passage from the kernel \(\Gm\) to \(\cG\), and this is measured by the exact sequence
\[
  0\longrightarrow \Gm\longrightarrow \cG\longrightarrow Z_{S,T}\longrightarrow 0
\]
and by the associated exact sequences of \(\Ext^1\) and \(\Biext^1\).  The groups which control full faithfulness become \(\Hom(\Phi,Z)\) or \(\Hom(\Phi\otimes\Psi,Z)\), hence vanish under the torsion hypotheses; the obstruction groups become \(\Hom(\Phi,\QQ/Z)\) and \(\Hom(\Phi\otimes\Psi,\QQ/Z)\), as in 5.5 and 5.6.

\textbf{Remark 7.2.}  The same idea treats a normal connected regular base \(S\).  To decide whether an extension over the generic fibre prolongs to \(S\), it suffices to test after base change to the local traits \(\Spec(\mathcal O_{S,x})\) for codimension-one points \(x\).  The condition is the vanishing of the corresponding homomorphisms \(\Phi_x\to\QQ/Z\).  The analogous statement for biextensions is obtained by the same argument and the codimension at least two extension property for line bundles.

\textbf{7.3. Functorialities of the obstructions.}  Let \(u:P'\to P\) be a morphism of smooth group schemes over \(S\).  If \(E_\eta\) is an extension of \(P_\eta\) by \(\Gm\), the obstruction to prolonging its inverse image along \(u_\eta\) is the pullback of the obstruction of \(E_\eta\).  In diagrammatic form:
\[
\begin{tikzcd}[column sep=large,row sep=large]
\Phi' \arrow[d,"u_s"'] \arrow[dr,"d(E'_\eta)"]&\\
\Phi \arrow[r,"d(E_\eta)"']&\QQ/Z.
\end{tikzcd}
\]
Similarly, for morphisms \(u:P'\to P\) and \(v:Q'\to Q\), the obstruction pairing for the inverse image of a biextension is obtained by composing with \(u_s\otimes v_s\):
\[
\begin{tikzcd}[column sep=large,row sep=large]
\Phi'\otimes\Psi' \arrow[d,"u_s\otimes v_s"'] \arrow[dr,"d(E'_\eta)"]&\\
\Phi\otimes\Psi \arrow[r,"d(E_\eta)"']&\QQ/Z.
\end{tikzcd}
\]

For an extension \(E_\eta\) of \(P_\eta\) there is a largest open subgroup \(V\subset P\), containing \(P_\eta\), over which \(E_\eta\) prolongs.  It is defined by
\[
  V_s/V_s^0=\Ker d(E_\eta)\subset \Phi.
\]
For a biextension \(E_\eta\) of \((P_\eta,Q_\eta)\), there is similarly a largest open subgroup \(V\subset P\) such that the restriction to \((V_\eta,Q_\eta)\) prolongs; it is defined by the left annihilator of the pairing
\[
  d(E_\eta):\Phi\otimes\Psi\longrightarrow\QQ/Z.
\]
There are symmetric statements interchanging \(P\) and \(Q\).

The obstruction is compatible with symmetry.  If \({}^sE_\eta\) denotes the symmetric biextension of \((Q_\eta,P_\eta)\), then the square
\[
\begin{tikzcd}[column sep=large,row sep=large]
\Phi\otimes\Psi \arrow[d,"\mathrm{sym}"'] \arrow[dr,"d(E_\eta)"]&\\
\Psi\otimes\Phi \arrow[r,"d({}^sE_\eta)"']&\QQ/Z
\end{tikzcd}
\]
...
The obstruction for a biextension can be reduced to the obstruction for extensions by evaluating on sections of the second group.  If \(q\in Q(S)\) has image \(q_s\in\Psi(k)\), and if \(q^*E_\eta\) denotes the extension of \(P_\eta\) induced by \(q\), then
\[
  d(q^*E_\eta)(x)=d(E_\eta)(x,q_s).
\]
Conversely, the extension case is obtained from the biextension case by taking \(Q=\widetilde{\ZZ}_{S,T}\) and evaluating at the unit section.

If \(S'\to S\) is a local dominant morphism of traits, with ramification index
\[
  e(S'/S)=\operatorname{length}_{V'}(V'/\mathfrak m_VV'),
\]
...
\textbf{Proposition 7.4.}  Let \(S\) be reduced.
\begin{enumerate}[label=\alph*)]
\item Let \(P\) be a smooth group scheme over \(S\) whose fibres over maximal points are connected.  The natural functor
\[
  \Ext(P,\Gm)\longrightarrow \operatorname{TORSRIG}(P,\Gm)
\]
from extensions of \(P\) by \(\Gm\) to \(\Gm\)-torsors on \(P\) rigidified along the unit section is fully faithful.  If \(S\) is normal and \(P\) has connected fibres, such a rigidified torsor comes from an extension if and only if it does so on the fibres over all maximal points.
\item Let \(P,Q\) be smooth group schemes over \(S\) whose fibres over maximal points are connected.  The natural functor
\[
  \Biext(P,Q;\Gm)\longrightarrow \operatorname{TORSBIRIG}(P,Q;\Gm)
\]
from biextensions to birigidified \(\Gm\)-torsors on \(P\times Q\) is fully faithful.  If \(S\) is normal and one of \(P,Q\) has connected fibres, an object of the right hand side comes from a biextension if and only if this is true over every maximal point of \(S\).
\end{enumerate}

For (a), the full faithfulness follows from the fact that a morphism from a flat product \(X\times_SY\) to the separated group \(\Gm\) is trivial as soon as it is trivial on the fibres over the maximal points.  This is the same argument as in 4.1.  The essential-image assertion is local on \(S\); after reducing to the noetherian normal case, Theorem 7.1 gives an open complement of codimension at least two over which the desired extension exists, and normality then extends the required isomorphism over all of \(P\times_SP\).  The proof of (b) is identical, with biextension data in place of extension data.

\textbf{Corollary 7.5.}  Let \(S\) be normal and let \(P,Q\) be smooth group schemes over \(S\), with connected fibres over maximal points, and with one of \(P,Q\) having connected fibres everywhere.  Suppose moreover that for every maximal point \(\eta\) of \(S\), \(P_\eta\) or \(Q_\eta\) is an extension of an abelian variety by a torus, more generally admits a composition series whose factors are abelian varieties, tori, or copies of \(\Ga\).  Then the functor in 7.4(b) is an equivalence.  This is an immediate consequence of 7.4(b) and 4.7.

\subsection*{Bibliography}
\begin{enumerate}[label={[\arabic*]}]
\item M. Demazure, J. Giraud, M. Raynaud, \emph{Schémas abéliens}, Séminaire Orsay 1967/68, to appear.
\item M. Raynaud, \emph{Faisceaux amples sur les schémas en groupes et les espaces homogènes}, Paris thesis, 1968, to appear in Lecture Notes, Springer.
\item M. Rosenlicht, Some basic theorems on algebraic groups, \emph{Amer. J. Math.} 78, no. 2 (1956), 401--443.
\item M. Rosenlicht, Some rationality questions on algebraic groups, \emph{Annali di Matematica}, Serie 4, 43 (1957), 25--50.
\item J.-P. Serre, \emph{Corps locaux}, Act. Sci. Ind. 1296, Hermann, Paris, 1962.
\end{enumerate}



% SGA 7-I Expose IX macro additions
\providecommand{\NS}{\operatorname{NS}}
\providecommand{\Biext}{\operatorname{Biext}}
...
\providecommand{\Id}{\operatorname{Id}}
\providecommand{\I}{I}
\providecommand{\Tate}{T}
\providecommand{\Ph}{\Phi}
\providecommand{\Zell}{\mathbb Z_\ell}
\providecommand{\Ql}{\mathbb Q_\ell}
\providecommand{\Gmk}{\mathbb G_m}


\section*{Expos\'e IX. N\'eron models and monodromy}
\begin{center}
by A. Grothendieck\\
...
\(\Phi\times\Phi'\to(\Q/\Z)_k\), and the canonical biextension
\(\cW^0\) of \((A^0,A^{\prime 0})\).
\item Fixed part and toric part of \(T_\ell(A_K)\).  Criteria for good reduction.  Orthogonality theorem for \(\ell\ne p\).
\item The semi-stable reduction case.  The semi-stable reduction theorem.
\item Application to a conjecture of Serre--Tate and to the conductor.
\item The orthogonality theorem in the case \(\ell=p\), in the semi-stable case.  Duality of the abelian schemes \(B\) and \(B'\).  Characterization of the fixed part.  Criteria for good reduction, essential good reduction, and semi-stable reduction.
\item Remarks on construction of the toric part and the fixed part of \(T_\ell(A)\) in the non-semi-stable case.
\item Raynaud's extension over \(S\) attached to an abelian scheme with semi-stable reduction.  Duality of the abelian schemes \(B,B'\) over \(S\).
\item Raynaud's extension in the semi-stable case, and the associated ind-group.
\item Definition of the monodromy pairing \(M'\otimes M\to\Z\).
\item Properties of the monodromy pairing.  Integrality and positivity theorem.
\item Connected components of the N\'eron model: duality relation and asymptotic behaviour.
\item N\'eron model of a Jacobian and the Picard--Lefschetz formula.
\item Transcendental links: the complex analytic case.
\item Transcendental links: the rigid-analytic case.
\end{enumerate}

\subsection*{Guide to the expos\'e}
The first four sections review the ingredients leading to semi-stable reduction; the following sections develop the algebraic study of the N\'eron model.  The guide in the original source is the directed graph
\[
\begin{gathered}
1\to2\to3,\qquad 3\to4,\qquad 3\to5,\\
5\to6,\quad 5\to7,\quad 5\to9,\quad 7\to8,\\
7\to13,\quad 7\to14,\quad 8\to13,\quad 8\to14,\\
9\to10,\quad 10\to11,\quad 10\to12,\quad 10\to14.
\end{gathered}
\]
...
\subsection*{0. Introduction}
In this expos\'e we apply the results of the preceding expos\'es to the study of the N\'eron model \(A\) of an abelian scheme \(A_K\) over the function field \(K\) of a henselian trait \(S\).  The most important properties of the N\'eron model can be interpreted in terms of the local monodromy group \(I\) acting on the Tate module \(T_\ell(A_K(\bar K))\), where \(\ell\) is a prime distinct from the residual characteristic \(p\).  Since the dual of this Tate module is the \(\ell\)-adic cohomology group \(H^1(A_{\bar K},\Z_\ell)\), and since for a smooth projective \(K\)-scheme \(X_K\), the group \(H^1(X_{\bar K},\Z_\ell)\) identifies with \(H^1(\operatorname{Alb}_{X/K,\bar K},\Z_\ell)\), this expos\'e may also be read as a detailed study of local monodromy phenomena for the first \(\ell\)-adic, or better motivic, cohomology of smooth projective varieties over \(K\).  In this perspective the results below should ultimately belong to a N\'eron theory for motives of arbitrary weight.

The principal result is the semi-stable reduction theorem, Theorem 3.6: after a finite separable extension \(K'/K\), the N\'eron model of \(A_{K'}\) has special fibre whose neutral component is an extension of an abelian variety by a torus.  Serre conjectured this result in 1964, and the principle of the proof used here goes back to the same year.  The condition is equivalent to saying that the monodromy action on \(T_\ell(A_{\bar K})\) is essentially unipotent of step two.  In that form it is a special case of the monodromy theorem proved earlier.  To pass between the group-theoretic and N\'eron-model formulations one needs the orthogonality theorem 2.4, for which the theory of biextensions developed in the preceding expos\'es is the decisive tool.

Mumford found a different proof of the semi-stable reduction theorem using theta functions, though, in the form then available, it was restricted to residual characteristic different from \(2\).  The theorem is indispensable not only in the algebraic study of N\'eron models, but also in the rigid-analytic or theta-function approach to abelian varieties over valued fields, as developed by Tate, Morikawa, Mumford and Raynaud.  It is also a key ingredient in the Artin--Deligne--Mumford semi-stable reduction theorem for curves, already used for the monodromy action on the fundamental group and used again below in the proof of the integrality and positivity theorem 10.4.

A special feature of the present expos\'e is that, in studying \(T_\ell(A)\), the case \(\ell=p\) is also systematically taken into account.  The language of Barsotti--Tate groups, or \(p\)-divisible groups, allows one to extend to this case many of the phenomena established when \(\ell\ne p\), subject sometimes to Tate's fundamental theorem, presently known only in characteristic zero.  In characteristic zero the pro-\(p\)-Barsotti--Tate group \(T_p(A)\) can still be interpreted by means of the monodromy action on the \(p\)-adic Tate module.  Nevertheless the analogy with \(\ell\ne p\) is deceptive: for example the monodromy action need not be essentially unipotent.  With more suitable translations, such as 5.13 below, the study of geometrically defined Barsotti--Tate groups over the function field of a trait belongs to the local monodromy theory of \(p\)-adic local systems.  For higher cohomology a suitable enlargement of the category of Barsotti--Tate groups should be inspired by crystalline cohomology.

Finally, the exposition is close to the oral lectures up to Section 4 and becomes more detailed from Section 5 onward.  Some results below, especially 10.4 and 12.5, use the Picard--Lefschetz formula, which is established only later in the expos\'e.

\section*{1. N\'eron models of abelian schemes; notation; the canonical pairings}

\subsection*{1.0. Poincar\'e biextension and Tate pairings}
Throughout this expos\'e let \(S\) be a trait, with generic point \(\eta\), closed point \(s\), residue field \(k\), and function field \(K\).  Let \(A_K\) be an abelian scheme over \(K\), and let \(A'_K\) denote its dual.  The duality between \(A_K\) and \(A'_K\) is given by the Poincar\'e divisor correspondence \(\cW_K\) on \(A_K\times A'_K\), i.e. by an invertible sheaf birigidified along the unit sections.  Its class is
\begin{equation}\tag{1.0.1}
 w_K\in \Biext^1(A_K,A'_K;\Gm{}_K).
\end{equation}
The corresponding pairing is
\begin{equation}\tag{1.0.2}
 \psi_\ell:T_\ell(A_K)\times T_\ell(A'_K)\longrightarrow T_\ell(\Gm{}_K).
\end{equation}
For \(\ell\ne p=\Car k\), this is equivalently a pairing of free \(\Z_\ell\)-modules of finite type with Galois action,
\begin{equation}\tag{1.0.3}
 \psi_\ell:T_\ell(A_K(\bar K))\times T_\ell(A'_K(\bar K))\longrightarrow \Z_\ell(1),
\end{equation}
compatible with \(\Gamma=\Gal(\bar K/K)\).  By the preceding expos\'e this pairing is perfect.

Sometimes a polarization \(\xi\in\NS(A_K/K)\) is chosen.  It gives the usual homomorphism
\begin{equation}\tag{1.0.5}
 t_\xi:A_K\longrightarrow A'_K
\end{equation}
and therefore
\begin{equation}\tag{1.0.6}
 T_\ell(t_\xi):T_\ell(A_K)\longrightarrow T_\ell(A'_K).
\end{equation}
The induced alternating form is
\begin{equation}\tag{1.0.7}
 \psi_{\ell,\xi}(x,y)=\psi_\ell(x,T_\ell(t_\xi)y):T_\ell(A_K)\times T_\ell(A_K)\longrightarrow \Z_\ell(1).
\end{equation}
For \(\ell\ne p\) this is a bilinear form on free \(\Z_\ell\)-modules with operators.  The fact that \(\xi\) is a polarization means that \(t_\xi\) is an isogeny; equivalently, \(T_\ell(t_\xi)\) is injective with finite cokernel, or the form \(\psi_{\ell,\xi}\) is separated.

\subsection*{1.1. The N\'eron model and the component groups}
Recall that there is a smooth \(S\)-scheme \(A\), the N\'eron model of \(A_K\), representing the functor
\begin{equation}\tag{1.1.1}
 S'\longmapsto \Hom_K(S'_K,A_K)
\end{equation}
for smooth relative \(S\)-schemes \(S'\).  Thus, for \(S'\) smooth over \(S\), there is a canonical bijection
\begin{equation}\tag{1.1.2}
 \Hom_S(S',A)=\Hom_K(S'_K,A_K).
\end{equation}
N\'eron constructed \(A\) when \(k\) is perfect, and Raynaud extended the existence theorem to arbitrary residue fields.  We shall use the existence theorem, and the fact that \(A\) is quasi-projective over \(S\).  The generic fibre of \(A\) is canonically \(A_K\), and the restriction map in (1.1.2) is simply restriction to generic fibres.
...
\begin{equation}\tag{1.1.5}
 \Phi=A_s/A_s^0
\end{equation}
is a finite \(\etale\) group scheme over \(k\), equivalently, after choosing a separable closure \(\bar k\), a finite ordinary group \(\Phi(\bar k)\) with its natural action of \(\Gal(\bar k/k)\).

The dual abelian scheme \(A'_K\) has its N\'eron model \(A'\), with special fibre \(A'_s\), neutral component \(A_s^{\prime 0}\), and component group
\begin{equation}\tag{1.1.8}
 \Phi'=A'_s/A_s^{\prime 0}.
\end{equation}

\subsection*{1.2--1.5. The component pairing and the canonical biextension}
Applying the preceding theory of prolongation of biextensions to the pair \((A,A')\) and to \(\cW_K\), one obtains the canonical pairing
\begin{equation}\tag{1.2.1}
 \Phi\times\Phi'\longrightarrow (\Q/\Z)_k,
\end{equation}
or equivalently a pairing of \(\Gal(\bar k/k)\)-modules
\begin{equation}\tag{1.2.2}
 \Phi(\bar k)\times\Phi'(\bar k)\longrightarrow \Q/\Z.
\end{equation}
It is the obstruction to prolonging \(\cW_K\) to a biextension of \((A,A')\) by \(\Gm{}_S\).

\textbf{Conjecture 1.3.}  The pairing (1.2.1) is a perfect duality: it identifies each component group with the dual of the other, with values in \((\Q/\Z)_k\).
...
If \(U\subset A\) and \(U'\subset A'\) are open subgroup schemes such that \(\cW_K\) prolongs to a biextension of \((U,U')\) by \(\Gm{}_S\), the subgroups \(U/U^0\subset\Phi\) and \(U'/U^{\prime 0}\subset\Phi'\) are orthogonal for (1.2.1).  The resulting biextension is denoted by \(\cW\).  Taking \(U=A^0\), \(U'=A^{\prime 0}\), one obtains a canonical biextension
\begin{equation}\tag{1.4.4}
 \cW^0\in\Biext^1(A^0,A^{\prime 0};\Gm{}_S).
\end{equation}
All other prolongations restrict to \(\cW^0\) on \((A^0,A^{\prime 0})\).

\textbf{Proposition 1.5.}  The special fibre \(\cW_s\), a biextension of \((A_s^0,A_s^{\prime 0})\) by \(\Gm{}_k\), is separated, i.e. separated on the left and on the right.

Indeed, choose an ample invertible sheaf \(L\) on \(A_s^0\).  It defines a homomorphism \(A_s^0\to A_s^{\prime 0}\), and the inverse image of \(\cW_s\) by \((\Id,A_s^0\to A_s^{\prime 0})\) is the biextension attached to the birigidified sheaf \(L\).  Since \(L\) is ample, the separatedness theorem for biextensions applies.  Symmetry gives separatedness on both sides.  This will be sharpened below when \(A_s^0\) has unipotent rank zero.

\section*{2. Fixed part and toric part of \(T_\ell(A_K)\); good reduction; orthogonality for \(\ell\ne p\)}

\subsection*{2.1. Toric, unipotent, and abelian ranks}
...
\begin{equation}\tag{2.1.2}
 0\longrightarrow U_s\longrightarrow A_s^0/T_s\longrightarrow B_s\longrightarrow0.
\end{equation}
More generally, whenever \(A_s^0\) is an extension
\begin{equation}\tag{2.1.3}
 0\longrightarrow L_s\longrightarrow A_s^0\longrightarrow B_s\longrightarrow0
\end{equation}
...
For \(\ell\ne\Car k\), the inclusion of the torus gives an exact sequence of pro-group schemes
\begin{equation}\tag{2.1.6}
 0\longrightarrow T_\ell(T_s)\longrightarrow T_\ell(A_s^0)\longrightarrow T_\ell(A_s^0/T_s)\longrightarrow0.
\end{equation}
When (2.1.2) is available this becomes
\begin{equation}\tag{2.1.8}
 0\longrightarrow T_\ell(T_s)\longrightarrow T_\ell(A_s^0)\longrightarrow T_\ell(B_s)\longrightarrow0.
\end{equation}
Put
\begin{equation}\tag{2.1.9}
 n=\dim A_K,\\quad \mu=\dim T_s,
 \quad \lambda=\dim U_s,
 \quad a=\dim B_s.
\end{equation}
Then
...
\begin{equation}\tag{2.1.11}
 \rk T_\ell(T_s)=\mu,
 \quad \rk T_\ell(B_s)=2a,
 \quad \rk T_\ell(A_s^0)=\mu+2a.
\end{equation}
...
The dual N\'eron model \(A'\) gives analogous objects \(T'_s,U'_s,B'_s\).  Later it will follow that the corresponding integers \(\mu,\lambda,a\) are the same for \(A\) and for \(A'\), so no new notation is needed.

\subsection*{2.2. The fixed part}
We compare \(T_\ell(A_K)\), \(T_\ell(A_s^0)\), and \(T_\ell(A^0)\).  First recall the following lemma.

\textbf{Lemma 2.2.1.}  Let \(m>0\).  For the multiplication endomorphism of \(A^0\), the following conditions are equivalent:
\begin{enumerate}[label=(\roman*)]
\item \(m\Id_{A^0}\) is flat;
\item \(m\Id_{A^0}\) is surjective;
\item \(m\Id_{A^0}\) is quasi-finite;
\item \(m\) is prime to \(\Car k\), or \(A_s^0\) has unipotent rank zero.
\end{enumerate}
When these conditions hold for \(m\), they hold for all powers \(m^\nu\), and the group schemes
...
Take now a prime \(\ell\) satisfying the conditions of the lemma and put
\begin{equation}\tag{2.2.2.1}
 T_\ell(A^0)=\varprojlim_\nu {}_{\ell^\nu}A^0.
\end{equation}
After base change to \(\eta\) and to \(s\), this recovers respectively \(T_\ell(A_K)\) and \(T_\ell(A_s^0)=T_\ell(A_s)\).
...
\begin{equation}\tag{2.2.3.1}
 X=X^f\amalg X'
\end{equation}
canonically, with \(X^f\) finite over \(S\) and \(X'_s=\emptyset\).  If \(X\) is a group scheme, \(X^f\) is a subgroup scheme; if \(X\) is flat or \(\etale\), so is \(X^f\).  Applying this to the terms of \(T_\ell(A^0)\), one obtains a projective subsystem
\begin{equation}\tag{2.2.3.2}
 T_\ell(A^0)^f\subset T_\ell(A^0),
\end{equation}
called the fixed part of \(T_\ell(A^0)\).  It satisfies
\begin{equation}\tag{2.2.3.3}
 T_\ell(A^0)^f\times_S s = T_\ell(A_s^0).
\end{equation}
...
For \(\ell\ne p\), this is an \(\ell\)-adic locally constant twisted sheaf on \(S\), hence corresponds to a \(\Z_\ell\)-submodule of \(T_\ell(A_K(\bar K))\) stable under \(\Gamma\).

\textbf{Proposition 2.2.5.}  Suppose \(\ell\ne p\) and \(S\) henselian.  The submodule of \(T_\ell(A_K(\bar K))\) corresponding to the fixed part \(T_\ell(A_K)^f\) is precisely
\begin{equation}\tag{2.2.5}
 T_\ell(A_K(\bar K))^I,
\end{equation}
where \(I\) is the inertia group.

Indeed it is enough to prove, for each \(\nu\),
\begin{equation}\tag{2.2.5.0}
 ({}_{\ell^\nu}A^0)^f(\bar K)={}_{\ell^\nu}A_K(\bar K)^I.
\end{equation}
Let \(\widetilde S\) be the strict localization of \(S\), with fraction field the maximal unramified extension of \(K\) in \(\bar K\).  Since \(({}_{\ell^\nu}A^0)^f\) is finite \(\etale\) over \(S\), its \(\bar K\)-points are its \(\widetilde S\)-points.  By the N\'eron mapping property these are the same as the \(\widetilde K\)-points of \({}_{\ell^\nu}A_K\), which are exactly the inertia-invariant \(\bar K\)-points.  Passing to the projective limit gives the assertion; the final equality with the special fibre follows because an element infinitely divisible by \(\ell\) in \(A_s(k)\) lies in \(A_s^0(k)\).

\textbf{Proposition 2.2.6.}  Let \(C_K\) be another abelian scheme over \(K\), with N\'eron model \(C\), and let \(u_K:C_K\to A_K\) be an isogeny, extending to \(u:C\to A\).  For every prime \(\ell\), the induced map on fixed parts
\begin{equation}\tag{2.2.6.0}
 T_\ell(C_K)^f\longrightarrow T_\ell(A_K)^f
\end{equation}
is an isogeny.  Also \(T_\ell(C_s)\to T_\ell(A_s)\) is an isogeny; the map on the greatest subtori is an isogeny; and, after an extension of \(k\) over which the abelian parts are defined, the induced map between abelian parts is an isogeny.
...
\textbf{Corollary 2.2.7.}  If \(A_K\) and \(C_K\) are isogenous, the invariants \(\mu,\lambda,a\) are the same for both.  In particular they are the same for \(A_K\) and its dual \(A'_K\).

These assertions about special fibres remain valid without the henselian hypothesis, since formation of the N\'eron model and of the special fibre invariants commutes with passing to the henselization of \(S\).

\textbf{Corollary 2.2.9 (good reduction).}  Let \(S\) be a trait, \(A_K\) an abelian scheme over \(K\), \(A\) its N\'eron model, and \(\ell\ne\Car k\).  The following conditions are equivalent:
\begin{enumerate}[label=(\roman*)]
\item there exists an abelian scheme over \(S\) with generic fibre isomorphic to \(A_K\);
\item \(A\) is an abelian scheme;
\item \(A^0\) is an abelian scheme;
\item \(A_s\) is an abelian scheme;
\item \(A_s^0\) is an abelian scheme, equivalently \(\lambda=\mu=0\);
\item \(A\) is proper over \(S\);
\item \(A^0\) is proper over \(S\);
\item \(T_\ell(A_K)\) is unramified over \(S\), i.e. inertia acts trivially;
\item equivalently, all \(\ell^\nu\)-torsion \({}_{\ell^\nu}A_K\) extends to an \(\etale\) covering of \(S\).
\end{enumerate}
When these conditions hold, the abelian scheme over \(S\) is the N\'eron model and is unique.  The equivalence with unramifiedness is the N\'eron--Ogg--Shafarevich criterion.
...
\begin{equation}\tag{2.3.1}
 T_s\subset A_s^0
\end{equation}
gives a sub-\(\ell\)-adic sheaf
\begin{equation}\tag{2.3.2}
 T_\ell(A^0)^t\subset T_\ell(A^0)^f,
\end{equation}
characterized by the special fibre condition
\begin{equation}\tag{2.3.3}
 T_\ell(A^0)^t\times_S s=T_\ell(T_s)\subset T_\ell(A_s^0).
\end{equation}
...
\end{equation}
The middle terms are, by Proposition 2.2.5, the inertia invariants.  The toric parts will be described by orthogonality with respect to the Poincar\'e pairing.

\textbf{Theorem 2.4 (orthogonality theorem).}  Assume \(S\) henselian and \(\ell\ne p\).  For the filtrations (2.3.4) and (2.3.5), the toric part \(T_\ell(A_K)^t\) is the intersection of the fixed part \(T_\ell(A_K)^f\) with the orthogonal of the fixed part \(T_\ell(A'_K)^f\) under the canonical pairing (1.0.3).  Symmetrically, \(T_\ell(A'_K)^t\) is the intersection of \(T_\ell(A'_K)^f\) with the orthogonal of \(T_\ell(A_K)^f\).

\emph{Proof.}  The restriction of the pairing to the fixed parts is obtained from the canonical biextension \(\cW^0\) of (1.4.4).  More precisely, \(\cW^0\) gives an \(\ell\)-adic pairing
\begin{equation}\tag{2.4.1}
 T_\ell(A^0)\times T_\ell(A^{\prime 0})\longrightarrow T_\ell(\Gm{}_S),
\end{equation}
...
\begin{equation}\tag{2.4.3}
 T_\ell(A_s^0)\times T_\ell(A_s^{\prime 0})\longrightarrow T_\ell(\Gm{}_k)
\end{equation}
attached to \(\cW_s\).  By the definition of the toric part, the desired vanishing is equivalent to the vanishing of
\begin{equation}\tag{2.4.5}
 T_\ell(T_s)\times T_\ell(A_s^{\prime 0})\longrightarrow T_\ell(\Gm{}_k),
\end{equation}
which follows from the general theorem on biextensions of algebraic groups.  This proves
\begin{equation}\tag{2.4.6}
 T_\ell(A_K)^t\subset T_\ell(A_K)^f\cap\bigl(T_\ell(A'_K)^f\bigr)^\perp.
\end{equation}
To see that equality holds, it is enough, again because the objects are \(\ell\)-adic locally constant sheaves, to check the special fibres.  The assertion becomes that the inclusion
\begin{equation}\tag{2.4.7}
 T_\ell(T_s)\subset T_\ell(A_s^0)\cap T_\ell(A_s^{\prime 0})^\perp
\end{equation}
is an equality, or equivalently that \(\cW_s\) is non-degenerate on the left.  This is precisely Proposition 1.5, ultimately a consequence of Raynaud's thesis.  Symmetry gives the statement for \(A'_K\).

\subsection*{2.5--2.6. Polarized form and remarks}
Using a polarization \(\xi\) of \(A_K\), the theorem may be expressed as a statement about the alternating form \(\psi_{\ell,\xi}\) on
\(L=T_\ell(A_K)\).  Let
...
The orthogonality theorem lets one read the most important properties of the N\'eron model of \(A_K\) directly from the inertia representation on \(T_\ell(A_K(\bar K))\).  The good-reduction criterion 2.2.9 is the first example.  The next section gives further examples, beginning with the semi-stable reduction criterion.
