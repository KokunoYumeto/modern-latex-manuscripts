...
est émph{lisse} si toute application surjective
...
Dans tout ce qui suit, on travaille, sauf mention expresse du contraire, dans un \(\cU\)-topos fixé, noté \(\cT\).  Autrement dit, dans la terminologie de SGA 4 IV, \(\cT\) est une catégorie équivalente à la catégorie des faisceaux sur un \(\cU\)-site convenable, qu'on peut prendre égal à \(\cT\) muni de sa topologie canonique.  On identifiera souvent les objets de \(\cT\) avec les faisceaux d'ensembles sur \(\cT\).  Les Groupes, extensions de Groupes, etc. dont il sera question seront tous relatifs à ce topos \(\cT\).

\section*{1. Compléments sur les extensions de Groupes}

\subsection*{1.1. Extensions et bitorseurs}

Soient \(P\) et \(G\) deux Groupes de \(\cT\), et
...
\[
  g\,x\,g'=i(g)\,x\,i(g')
\]
pour \(g,g'\in G(S)\), \(x\in E(S)\), et \(S\) objet quelconque de \(\cT\).  Si l'on considère \(E\), grâce à \(j\), comme un objet au-dessus de \(P\), c'est-à-dire comme un objet du topos induit \(\cT/P\), cela revient à dire que le groupe induit
\[
  G_P=G\times P
\]
...
Les raisonnements précédents prouvent que, pour un topos fixé \(\cT\), les constructions ci-dessus fournissent une équivalence entre la catégorie des extensions de Groupes et la catégorie des systèmes \((P,G,E,\varphi)\) décrits ci-dessus.  Il est laissé au lecteur l'explicitation de l'image inverse ou directe d'une extension par un morphisme de Groupes, ainsi que la compatibilité avec l'image inverse par un morphisme de topos \(f:\cT'\to\cT\).
...
Soit \(P\) un objet de \(\cT\) muni d'une section \(e_P\), soit \(G\) un Groupe de \(\cT\), et soit \(E\) un torseur sous \(G_P\).  Une \emph{rigidification} de \(E\), relativement à \(e_P\), est une trivialisation de \(E_1=e_P^*(E)\).  Cette terminologie est inspirée par le cas où \(G\) est commutatif et où
\[
  \Hompt(P,G)\simeq e,\qquad
  \Gamma(G)\xrightarrow{\sim}\operatorname{Hom}(P,G).
\]
Soient maintenant \(P\) et \(Q\) deux objets pointés; pour un torseur \(E\) sous \(G_{P\times Q}\), des rigidifications partielles le long de \(P\times e_Q\) et \(e_P\times Q\) sont dites compatibles si elles coïncident sur \(e_P\times e_Q\).  Un tel couple est une émph{birigidification}.  La terminologie s'étend aussitôt aux bitorseurs.
...
\textbf{Proposition 1.3.5.}  Soient \(P,Q\) deux Groupes de \(\cT\).
\begin{enumerate}[label=\alph*)]
...
Soit \(I\) un objet de \(\cT\), et prenons
\[
  P=\Z[I],
\]
...
\end{align}
sont des isomorphismes.  Plus généralement, si \(A\) est un Anneau de \(\cT\), non nécessairement commutatif, et si \(P=A[I]\), on a des isomorphismes canoniques fonctoriels
\[
...
\section*{Expos\'e VII, \S3. Compl\'ements d'alg\`ebre homologique}

Le but principal de ce num\'ero est la d\'efinition de l'isomorphisme
\begin{equation}\qquad\text{(3.0.1)}
  \Biext^1(P,Q;G)\simeq \Ext^1(P\otimes^{L}Q,G),
\end{equation}
annonc\'e plus haut.  On aura besoin pour cela de compl\'ements d'alg\`ebre homologique, d'int\'er\^et ind\'ependant, qui occupent les sections 3.1 \`a 3.4.

\subsection*{3.1. Cat\'egorie cofibr\'ee additive associ\'ee \`a un complexe}
Soit \(\cC\) une cat\'egorie ab\'elienne et soit
\begin{equation}\qquad\text{(3.1.1)}
  \cdots\longrightarrow L_2\xrightarrow{d_2}L_1\xrightarrow{d_1}L_0
\end{equation}
un complexe de cha\^ines de \(\cC\), avec diff\'erentielle de degr\'e \(-1\).  On d\'efinit une cat\'egorie cofibr\'ee \(\cPsi_L\) sur \(\cC\).  Les objets de \(\cPsi_L(A)\) sont les couples \((X,a)\), o\`u
\begin{equation}\qquad\text{(3.1.2)}
  0\longrightarrow A\longrightarrow X\xrightarrow{j}L_0\longrightarrow0
\end{equation}
est une extension, et o\`u \(a:L_1\to X\) est un rel\`evement de \(d_1\), soumis \`a la condition \(a d_2=0\):
...
\end{tikzcd}
\end{equation}
Les morphismes sont les morphismes d'extensions compatibles \`a ces donn\'ees.  Le foncteur \((X,a)\mapsto A\) est cofibrant, et \(\cPsi_L\) est additive.  L'objet nul de \(\cPsi_L(A)\) est d\'efini par l'extension triviale \(L_0\oplus A\), et l'on a
\begin{equation}\qquad\text{(3.1.8)}
  \cPsi_L^0(A)\simeq
  \Ker\bigl(\Hom(L_0,A)\to\Hom(L_1,A)\bigr)
  \simeq\Hom(H_0(L_\bullet),A).
\end{equation}
L'ensemble des classes d'isomorphie de \(\cPsi_L(A)\) est un groupe commutatif not\'e \(\cPsi_L^1(A)\), fonctoriel additivement en \(A\).

\subsection*{3.2. D\'etermination de \(\cPsi_L^1(A)\)}
Pour \((X,a)\in\cPsi_L(A)\), le complexe
\begin{equation}\qquad\text{(3.2.1)}
  K^\bullet=[X\xrightarrow{j}L_0]
\end{equation}
repr\'esente \(A\) dans \(\Dcat(\cC)\).  Le diagramme de \((X,a)\) donne un morphisme de degr\'e \(1\)
...
\end{equation}
dans la cat\'egorie d\'eriv\'ee.  On obtient ainsi
\begin{equation}\qquad\text{(3.2.4')}
  \cPsi_L^1(A)\longrightarrow \Ext^1(L_\bullet,A).
\end{equation}

\textbf{Th\'eor\`eme 3.2.5.}  L'application pr\'ec\'edente est un isomorphisme de groupes, fonctoriel en \(A\).

La preuve est celle du texte original.  Si le morphisme de complexes associ\'e \`a \((X,a)\) est nul dans \(\Dcat(\cC)\), il est d\'ej\`a nul \`a homotopie pr\`es: il existe \(s:L_0\to X\) tel que
\begin{equation}\qquad\text{(3.2.5.1)}
  js=\id_{L_0},\qquad s d_1=-a.
\end{equation}
Un tel \(s\) est exactement une trivialisation de l'extension compatible \`a \(a\).  Inversement, tout morphisme \(L_\bullet\to A[1]\) se repr\'esente, apr\`es choix d'une r\'esolution \(C^\bullet\) de \(A\), par un diagramme de carr\'es anticommutatifs; en tirant en arri\`ere l'extension \(C^0\) de \(C^1\) par \(A\), on construit \((X,a)\) dont la classe est le morphisme donn\'e.

\subsection*{3.3. Propri\'et\'es de \(\cPsi_L\)}
La cat\'egorie cofibr\'ee additive \(\cPsi_L\) est exacte \`a gauche, admet un objet quasi-maximal, et tout objet poss\`ede un complexe typique.  Pour une suite exacte
\begin{equation}\qquad\text{(3.3.2.1)}
  0\to A'\to A\xrightarrow{v}A'',
\end{equation}
le foncteur
\begin{equation}\qquad\text{(3.3.2.2)}
  \cPsi_L(A')\longrightarrow\cPsi_L([A\to A''])
\end{equation}
est une équivalence.  Concr\`etement, une trivialisation au-dessus de \(A''\) est donn\'ee par un morphisme \(p:X\to A''\) avec \(pi=v\) et \(pa=0\), et l'on remonte alors \`a \(X'=\Ker p\).

Un objet quasi-maximal est donn\'e par \(X_0=L_0\oplus L'_1\), avec \(L'_1=\Coker(d_2:L_2\to L_1)\), muni du rel\`evement \((d_1,L_1\to L'_1)\).  Le complexe typique d'un objet \((X,a)\) au-dessus de \(A\) est
\begin{equation}\qquad\text{(3.3.4.2)}
  A\longrightarrow\Coker(a),
\end{equation}
et le complexe typique de la cat\'egorie \(\cPsi_L\) est canoniquement le tronqu\'e de \(L_\bullet\).

Les isomorphismes (3.1.8) et (3.2.5) identifient la suite exacte des \(\Ext\) associ\'ee \`a une suite exacte courte de coefficients avec la suite exacte g\'eom\'etrique provenant de l'exactitude \`a gauche de \(\cPsi_L\).  Le texte signale seulement une convention de signe dans le morphisme bord.

\subsection*{3.4. Variance avec \(L\)}
A tout morphisme de complexes
\begin{equation}\qquad\text{(3.4.1)}
  f:L'\to L
\end{equation}
on associe un foncteur cocart\'esien
\begin{equation}\qquad\text{(3.4.3)}
  f^*:\cPsi_L\to\cPsi_{L'}.
\end{equation}
Il est obtenu en tirant en arri\`ere l'extension de \(L_0\) par \(A\) au moyen de \(f_0:L'_0\to L_0\), et en transportant la trivialisation partielle au moyen de \(f_1\).

Pour \(i=0,1\), les homomorphismes induits sur les \(\cPsi^i\) sont compatibles aux homomorphismes
\begin{equation}\qquad\text{(3.4.5)}
  \Ext^i(L,-)\to\Ext^i(L',-),
\end{equation}
ce qui se traduit par les carr\'es commutatifs
...
\textbf{Proposition 3.4.7.}  Le foncteur (3.4.3) est fid\`ele, pleinement fid\`ele, ou une équivalence, suivant que le morphisme sur \(H_0\) est un \'epimorphisme, que le morphisme sur \(H_0\) est un isomorphisme et celui sur \(H_1\) un épimorphisme, ou enfin que les morphismes sur \(H_0\) et \(H_1\) sont des isomorphismes.
...
\begin{equation}\qquad\text{(3.4.8.2)}
  \cPsi_{L'}(A)\simeq \cPsi_L(A)\times_{\cPsi_{L''}(A)}\{0\},
\end{equation}
et la suite exacte longue des \(\Ext\) re\c coit une interpr\'etation g\'eom\'etrique correspondante.  Le texte ram\`ene la compatibilit\'e des signes \`a la th\'eorie g\'en\'erale de [4].

\subsection*{3.5. R\'esolution partielle plate canonique d'un \(\Z\)-module}
Soit \(P\) un groupe commutatif du topos \(\cT\).  On pose
\begin{equation}\qquad\text{(3.5.1)}
  L_0(P)=\Z[P],\quad L_1(P)=\Z[P\times P],\quad
  L_2(P)=\Z[P^3]\oplus\Z[P^2],
\end{equation}
et l'on d\'efinit
\begin{align}\qquad\text{(3.5.2)}
 ...
 d_2'([p,q])&=[p,q]-[q,p].
\end{align}
Ces formules d\'efinissent un complexe augment\'e vers \(P\).  Les relations sont exactement la commutativit\'e et l'associativit\'e de la loi de groupe de \(P\).

\textbf{Proposition 3.5.3.}  On a \(H_i(L_\bullet(P))=0\) pour \(i>0\), et \(H_0(L_\bullet(P))\simeq P\).

La preuve consiste \`a appliquer 3.4.7 \`a l'augmentation.  La cat\'egorie \(\cPsi_{L(P)}(G)\) s'interpr\`ete, par 1.4, comme la cat\'egorie des torseurs \(E\) sous \(G_P\) munis d'une trivialisation de
\begin{equation}\qquad\text{(3.5.3.2)}
  \pr_1^*E\;\pr_2^*E\;(+)^*E^{-1},
\end{equation}
satisfaisant les conditions de commutativit\'e et d'associativit\'e.  C'est le dictionnaire de 1.1 pour les extensions commutatives de \(P\) par \(G\).

\textbf{Remarque 3.5.6.}  Il serait int\'eressant de prolonger cette r\'esolution partielle en une r\'esolution plate fonctorielle infinie, par des sommes de modules libres engendr\'es par des puissances cart\'esiennes de \(P\).  Une variante analogue existe pour les \(A\)-modules, avec
\begin{align}\qquad\text{(3.5.9)}
L_0(P)&=A[P],\\
L_1(P)&=A[P^2]\oplus A[P],\\
L_2(P)&=A[P^3]\oplus A[P^2]\oplus A[P^2]\oplus A[P],
\end{align}
et des formules suppl\'ementaires exprimant la compatibilit\'e \`a l'action de \(A\).

\subsection*{3.6. Application aux biextensions}
Soient \(P,Q\) deux \(\Z\)-modules de \(\cT\).  Choisissons des r\'esolutions plates \(L(P)\) et \(L(Q)\), co\'incidant en dimensions \(\leq2\) avec les complexes partiels ci-dessus, et posons
\begin{equation}\qquad\text{(3.6.2)}
  L(P,Q)=L(P)\otimes L(Q)\longrightarrow P\otimes^{L}Q.
\end{equation}

\textbf{Th\'eor\`eme 3.6.4.}  La cat\'egorie cofibr\'ee additive, sur \(\GrAb(\cT)\), des biextensions de \((P,Q)\) par un groupe ab\'elien variable \(G\), est canoniquement équivalente \`a \(\cPsi_{L(P,Q)}\).

\textbf{Corollaire 3.6.5.}  On a un isomorphisme canonique, fonctoriel en \(G\),
...
L_0(P,Q)&=\Z[P\times Q],\\
L_1(P,Q)&=\Z[P\times Q^2]\oplus\Z[P^2\times Q],\\
L_2(P,Q)&=\Z[P\times Q^3]\oplus\Z[P\times Q^2]\oplus\Z[P^2\times Q^2]
\oplus\Z[P^3\times Q]\oplus\Z[P^2\times Q].
\end{align}
Ce dictionnaire identifie les objets de \(\cPsi_{L(P,Q)}(G)\) aux torseurs sur \(P\times Q\) munis des deux lois partielles de composition et des compatibilit\'es: ce sont exactement les biextensions.

\textbf{Remarque 3.6.7.}  Les variantes de la notion de biextension signal\'ees en 2.10 donnent des variantes correspondantes du th\'eor\`eme.  Dans le cas non commutatif, on associe \`a un groupe \(P\) un complexe tronqu\'e \(K_\bullet(P)\), analogue \`a \(L_\bullet(P)\), qui est le tronqu\'e d'ordre deux du complexe des cha\^ines de \(P\) \`a coefficients dans \(\Z\).
...
\begin{center}
Continuation des pages 237--270 du scan source original.
\end{center}
...
\section*{Exposé VIII, suite. Compléments sur les biextensions et les schémas en groupes}
\begin{center}
Pages 271--317 du scan original ; fin de l'Exposé VIII.
\end{center}

\subsection*{4. Cas d'un corps de base : correspondances divisorielles et séparation}

Nous continuons la discussion du numéro précédent.  Pour qu'un module inversible birigidifié \(L\) sur \(P\times Q\) provienne d'une biextension, il faut et il suffit que ses images inverses sur \(P\times P\times Q\) et sur \(P\times Q\times Q\), par les deux morphismes de multiplication naturels, satisfassent au théorème du cube.  Cette condition n'est autre que la traduction des conditions de VII 2.9.3 b), dans l'explicitation donnée en VII 3.5 b).

\textbf{4.4.}  On ne sait pas ici si la condition du cube est automatique.  On sait cependant que, pour \(L\) donné, il existe un entier \(n\geq 1\), puissance de l'exposant caractéristique de \(k\), tel que \(L^{\otimes n}\) satisfasse à cette condition, donc provienne d'une biextension de \((P,Q)\) par \(\Gm\).  On peut aussi trouver une extension radicielle \(k'/k\) pour laquelle la condition devient vraie après extension de la base.  En particulier, si \(k\) est parfait, la condition de 4.3 est toujours vérifiée; on peut également le voir par 4.7, cf. 4.9.

\textbf{Lemme 4.5.}  Soit \(f:X\to Y\) un morphisme lisse surjectif à fibres intègres, avec \(Y\) normal intègre de point générique \(\eta\).  Si \(L\) est un module inversible sur \(X\), trivial sur \(X_\eta\), il existe un module inversible \(M\) sur \(Y\) tel que \(L\simeq f^*M\).

\textbf{Proposition 4.6.}  Soient \(P,Q\) deux groupes algébriques lisses et connexes sur \(k\), avec \(P\) affine.  Supposons que \(P\) admette une suite de composition dont les quotients soient des tores ou des groupes \(\Ga\), ou que \(Q\) soit extension d'une variété abélienne \(B\) par un groupe lisse connexe affine satisfaisant la même condition.  Alors tout module inversible birigidifié sur \(P\times Q\) est trivial; par suite toute biextension de \((P,Q)\) par \(\Gm\) est triviale.

La démonstration procède par descente au cas séparablement clos.  Les tores y sont déployés; la suite de composition et \(\Pic(\Ga)=\Pic(\Gm)=0\) donnent \(\Pic(P)=0\).  Le lemme 4.5 appliqué à \(P\times Q\to Q\), puis la rigidification sur \(e_P\times Q\), forcent la trivialité.  Dans le cas où \(Q\) est extension de \(B\), on se ramène d'abord à \(P\times B\), puis à la situation où \(Q\) est abélien; le module définit alors un morphisme du groupe affine connexe \(P\) vers la variété abélienne duale de \(Q\), donc un morphisme constant, nécessairement nul.

\textbf{Proposition 4.7.}  Soient \(P,Q\) lisses et connexes sur \(k\).  Supposons que \(P\) soit extension d'une variété abélienne \(A\) par un groupe lisse connexe affine ayant une suite de composition à quotients tores ou \(\Ga\).  Alors tout module inversible birigidifié sur \(P\times Q\) provient d'une biextension de \((P,Q)\) par \(\Gm\), et
\[
  \Biext^1(A,Q;\Gm)\xrightarrow{\sim}\Biext^1(P,Q;\Gm).
\]
On se ramène au cas abélien, où le module correspond à une correspondance divisorielle et donc à une biextension de Weil.  L'isomorphisme découle de la suite exacte des \(\Biext\) associée à \(0\to L\to P\to A\to0\), avec les annulations fournies par 4.6.

\textbf{Corollaire 4.8.}  Si de plus \(Q\) est extension d'un groupe algébrique \(B\) par un groupe lisse connexe affine, alors
\[
  \Biext^1(A,B;\Gm)\xrightarrow{\sim}\Biext^1(P,Q;\Gm).
\]

\textbf{Remarque 4.9.}  Sur un corps parfait, tout groupe algébrique lisse connexe est extension d'une variété abélienne par un groupe affine lisse connexe, produit d'un tore par une extension multiple de groupes \(\Ga\).  La théorie des biextensions de \((P,Q)\) par \(\Gm\) se ramène ainsi à la théorie classique des variétés abéliennes.

\textbf{Corollaire 4.10.}  Si \(L\subset P\) est un sous-groupe affine lisse connexe et si \(E\) est une biextension de \((P,Q)\) par \(\Gm\), l'accouplement
\[
  T_\ell(P)\times T_\ell(Q)\longrightarrow T_\ell(\Gm)
\]
est nul sur \(T_\ell(L)\times T_\ell(Q)\).

\textbf{4.11.}  Pour une biextension \(E\) de \((P,Q)\) par \(\Gm\), on définit les notions de biextension séparante à gauche, à droite, et séparante.  Après une extension du corps de base où \(P,Q\) se dévissent en parties abéliennes \(A',B'\), la biextension provient d'une biextension \(F'\) de \((A',B')\).  Dire que \(E\) est séparante à gauche signifie que l'homomorphisme \(A'\to(B')'\) défini par \(F'\) a noyau fini; la condition droite est duale, et la condition séparante équivaut à dire que l'homomorphisme est une isogénie.  Ces notions sont invariantes par extension du corps de base.

\textbf{4.12--4.14.}  Si \(L\) est un module inversible sur \(P\), on pose
\[
  \delta(L)=m^*L\,\pr_1^*L^{-1}\,\pr_2^*L^{-1}.
\]
C'est un module inversible birigidifié sur \(P\times P\).  Lorsqu'il provient d'une biextension, celle-ci est unique et symétrique.  Si \(L\) est ample, cette biextension est séparante; dans le cas abélien, c'est l'énoncé classique selon lequel le morphisme associé \(A\to A'\) est une isogénie.  On obtient en outre un isomorphisme canonique
\[
  \NS(P)\simeq \Biext^1(P,P;\Gm)_{\mathrm{sym}},
\]
les classes amples de \(\NS(P)\) correspondant aux biextensions amples.

\section*{5. Extensions et biextensions par des schémas constants tronqués sans torsion}

\textbf{Proposition 5.1.}  Soient \(S\) géométriquement unibranche et irréductible de point générique \(\eta\), et \(Z\) un groupe sans torsion.  Tout torseur sous le schéma constant \(Z_S\) est trivial, et le passage à la fibre générique donne une équivalence de catégories.  La démonstration se réduit à \(H^1(S,Z_S)=0\), puis au cas d'un corps, où \(H^1(\Gamma,Z)=0\) pour \(Z\) sans torsion.

\textbf{Corollaire 5.2.}  Si \(P\) est lisse sur \(S\), l'assertion analogue vaut sur \(P\), composante irréductible par composante irréductible.  Le même résultat vaut pour les bitorseurs.

\textbf{Proposition 5.3.}  Si \(P\) est un schéma en groupes lisse sur \(S\), les extensions de \(P\) par \(Z_S\) se restreignent équivalemment aux extensions de la fibre générique.  La variante commutative s'obtient de même.

\textbf{Corollaire 5.4.}  Pour deux schémas en groupes commutatifs lisses \(P,Q\), le même résultat vaut pour les biextensions de \((P,Q)\) par \(Z_S\).

\textbf{Proposition 5.5.}  Sur un corps \(k\), pour \(P\) localement de type fini et \(\Phi=P/P^0\), les extensions de \(P\) par \(Z_k\) sont les mêmes que celles de \(\Phi\).  Si \(\Phi\) est de torsion, la catégorie est rigide; si \(n\Phi=0\), alors
\[
  \Ext^1(P,Z_k)\simeq \Hom(\Phi,\QQ/Z).
\]

\textbf{Corollaire 5.6.}  Pour \(P,Q\) commutatifs localement de type fini, de groupes de composantes \(\Phi,\Psi\), les biextensions par \(Z_k\) se ramènent à \((\Phi,\Psi)\); si un exposant tue \(\Phi\) ou \(\Psi\),
\[
  \Biext^1(P,Q;Z_k)\simeq \Hom(\Phi\otimes\Psi,\QQ/Z).
\]

\textbf{5.7--5.10.}  Étant donné un fermé \(T\subset S\), on définit le schéma constant tronqué \(Z_{S,T}\) en recollant les copies de \(S\), indexées par \(Z\), le long de \(U=S-T\).  Il représente
\[
  S'\longmapsto \Hom_T(S'_T,Z_T),
\]
et commute aux changements de base.  Les torseurs sous \(Z_{S,T}\), les extensions de \(P\) par \(Z_{S,T}\), et les biextensions de \((P,Q)\) par \(Z_{S,T}\), se restreignent équivalemment à \(T\).

\section*{6. Extensions et biextensions par le modèle de Néron-Raynaud de \(\Gm\)}

On note \(\widetilde Z_{S,T}\) le noyau de \(Z_S\to Z_{S,T}\).  Pour un groupe commutatif \(G\), les extensions de \(\widetilde{\ZZ}_{S,T}\) par \(G\) sont équivalentes aux couples \((F,s)\), où \(F\) est un torseur sous \(G\) et \(s\) une section de \(F|_U\).  Pour \(G=\Gm\), cela revient à un module inversible muni d'une trivialisation sur \(U\), donc à un diviseur \(D\) de support contenu dans \(T\).  On obtient ainsi le modèle de Néron-Raynaud \(\cG\) de \(\Gm\) associé à \(D\).

Si \(S\) est normal, si \(D\) est positif sans composantes multiples et si \(T=\supp D\) est localement intègre et géométriquement unibranche, alors
\[
  \cG(S')\simeq \Gamma(S'_U,\mathcal O^*_{S'_U})
\]
pour tout \(S'\) lisse sur \(S\).  En particulier, restriction à \(U\) donne une équivalence pour les torseurs lorsque les anneaux locaux de \(S\) sont factoriels, et une pleine fidélité en général.  On en déduit les équivalences de restriction pour les extensions et pour les biextensions par \(\cG\).  Dans le cas d'un trait, on retrouve le modèle de Néron-Raynaud usuel du groupe multiplicatif.

L'« autocritique » de Deligne reformule les numéros 5 et 6 sur le site lisse : pour un ouvert schématiquement dense \(j:U\to S\), le quotient \(j_*\Gm{}_{U}/\Gm{}_{S}\) est le faisceau des diviseurs de support dans \(S-U\), et ce faisceau commute au changement de base lisse.  Dans le cas d'un diviseur géométriquement unibranche, il est représenté par le schéma constant tronqué correspondant.

\section*{7. Prolongements canoniques d'extensions et de biextensions par \(\Gm\)}

Pour un trait \(S\), de point générique \(\eta\) et de point fermé \(s\), on associe à une extension de \(P_\eta\) par \(\Gm\) une obstruction
\[
  d(E_\eta):\Phi=P_s/P_s^0\longrightarrow \QQ/Z.
\]
Lorsque \(\Phi\) est de torsion, le foncteur de restriction des extensions sur \(S\) vers la fibre générique est pleinement fidèle; si \(\Phi\) est tué par un entier, l'annulation de \(d(E_\eta)\) est nécessaire et suffisante pour le prolongement.

Pour une biextension de \((P_\eta,Q_\eta)\), avec \(\Phi=P_s/P_s^0\) et \(\Psi=Q_s/Q_s^0\), l'obstruction est l'accouplement
\[
  d(E_\eta):\Phi\otimes\Psi\longrightarrow \QQ/Z.
\]
Sous les mêmes hypothèses de torsion, la pleine fidélité et le critère de prolongement valent pour les biextensions.

Les obstructions sont fonctorielles.  Pour \(u:P'\to P\), le diagramme
\[
\begin{tikzcd}[column sep=large,row sep=large]
...
\end{tikzcd}
\]
commute; pour les biextensions, le diagramme analogue avec \(\Phi'\otimes\Psi'\to\Phi\otimes\Psi\) commute également.  Le plus grand sous-groupe ouvert de \(P\) sur lequel une extension se prolonge est défini par le noyau de cette obstruction; pour une biextension, on prend l'annulateur gauche ou droit de l'accouplement.  La symétrie des biextensions se traduit par le carré de symétrie correspondant sur \(\Phi\otimes\Psi\).

Après un changement de traits \(S'\to S\) de ramification \(e(S'/S)\), l'obstruction est multipliée par cet indice; en particulier elle commute aux changements de base étales, henséliens, strictement henséliens et complétés.

Enfin, si \(S\) est réduit et si les fibres maximales de \(P,Q\) sont connexes, les foncteurs naturels des extensions vers les torseurs rigidifiés, et des biextensions vers les torseurs birigidifiés, sont pleinement fidèles.  Lorsque \(S\) est normal, l'image essentielle est caractérisée fibre par fibre aux points maximaux.  Si, en ces points, l'un des groupes considérés admet une suite de composition à facteurs variétés abéliennes, tores ou groupes \(\Ga\), le foncteur des biextensions vers les torseurs birigidifiés est une équivalence.

\subsection*{Bibliographie}
\begin{enumerate}[label={[\arabic*]}]
\item M. Demazure, J. Giraud, M. Raynaud, \emph{Schémas abéliens}, Séminaire Orsay 1967/68.
\item M. Raynaud, \emph{Faisceaux amples sur les schémas en groupes et les espaces homogènes}, thèse, Paris, 1968.
\item M. Rosenlicht, Some basic theorems on algebraic groups, \emph{Amer. J. Math.} 78 (1956), 401--443.
\item M. Rosenlicht, Some rationality questions on algebraic groups, \emph{Annali di Matematica} 43 (1957), 25--50.
...
\section*{Expos\'e IX. Mod\`eles de N\'eron et monodromie}
\begin{center}
par A. Grothendieck\\
...
\setcounter{enumi}{-1}
\item Introduction.
\item Mod\`ele de N\'eron des sch\'emas ab\'eliens : notations ; l'accouplement canonique \(\Phi\times\Phi'\to(\Q/\Z)_k\), et la biextension canonique \(\cW^0\) de \((A^0,A^{\prime 0})\).
\item Partie fixe et partie torique de \(T_\ell(A_K)\). Crit\`eres de bonne r\'eduction. Th\'eor\`eme d'orthogonalit\'e pour \(\ell\ne p\).
\item Cas de la r\'eduction semi-stable. Le th\'eor\`eme de r\'eduction semi-stable.
\item Application \`a une conjecture de Serre--Tate et au conducteur.
\item Le th\'eor\`eme d'orthogonalit\'e dans le cas \(\ell=p\), cas semi-stable ; dualit\'e des sch\'emas ab\'eliens \(B\) et \(B'\), caract\'erisation de la partie fixe, crit\`eres de r\'eduction.
\item Remarques sur la construction de la partie torique et de la partie fixe dans le cas de r\'eduction non semi-stable.
\item L'extension de Raynaud attach\'ee \`a un sch\'ema ab\'elien \`a r\'eduction semi-stable.
\item L'extension de Raynaud dans le cas semi-stable et l'ind-groupe associ\'e.
\item D\'efinition de l'accouplement de monodromie \(M'\otimes M\to\Z\).
\item Propri\'et\'es de l'accouplement de monodromie.
\item Composantes connexes du mod\`ele de N\'eron.
\item Mod\`ele de N\'eron d'une jacobienne et formule de Picard--Lefschetz.
\item Liens avec la th\'eorie transcendante : cas analytique complexe.
\item Liens avec la th\'eorie transcendante : cas rigide-analytique.
\end{enumerate}
...
\subsection*{0. Introduction}
Dans le pr\'esent expos\'e, nous appliquons les r\'esultats des expos\'es pr\'ec\'edents \`a l'\'etude du mod\`ele de N\'eron \(A\) d'un sch\'ema ab\'elien \(A_K\) d\'efini sur le corps des fonctions \(K\) du trait hens\'elien \(S\).  Les propri\'et\'es importantes du mod\`ele de N\'eron s'interpr\`etent en termes de l'action du groupe de monodromie locale \(I\) sur le module de Tate \(T_\ell(A_K(\bar K))\), o\`u \(\ell\ne p\).  Comme son dual est le groupe de cohomologie \(\ell\)-adique \(H^1(A_{\bar K},\Z_\ell)\), l'expos\'e peut aussi se lire comme une \"etude de la monodromie locale des \(H^1\) des vari\'et\'es projectives et lisses sur \(K\).  Ces r\'esultats devraient s'inscrire dans une th\'eorie de N\'eron pour des motifs de poids quelconque.

Le r\'esultat principal est le th\'eor\`eme de r\'eduction semi-stable 3.6.  Apr\`es extension s\'eparable finie de \(K\), le mod\`ele de N\'eron a une fibre sp\'eciale dont la composante neutre est extension d'un sch\'ema ab\'elien par un tore.  Cette propri\'et\'e \'equivaut \`a l'unipotence essentielle d'\'echelon deux de l'action de \(I\) sur le module de Tate, et sa mise en relation avec les mod\`eles de N\'eron utilise le th\'eor\`eme d'orthogonalit\'e 2.4, obtenu au moyen de la th\'eorie des biextensions.

Le th\'eor\`eme de r\'eduction semi-stable est aussi central dans les approches analytiques rigides et dans la th\'eorie des fonctions th\^eta de Mumford et Raynaud.  Il intervient dans le th\'eor\`eme de r\'eduction semi-stable des courbes d'Artin--Deligne--Mumford et dans le th\'eor\`eme d'int\'egrit\'e et de positivit\'e 10.4.  On tiendra compte aussi du cas \(\ell=p\) au moyen des groupes de Barsotti--Tate, tout en gardant \`a l'esprit que l'analogie avec \(\ell\ne p\) est trompeuse.

\section*{1. Mod\`ele de N\'eron ; notations et accouplements canoniques}
Soit \(A_K\) un sch\'ema ab\'elien sur \(K\), et \(A'_K\) son dual.  La dualit\'e est donn\'ee par la biextension de Poincar\'e \(\cW_K\) de \((A_K,A'_K)\) par \(\Gm{}_K\), de classe
\begin{equation}\tag{1.0.1}
 w_K\in \Biext^1(A_K,A'_K;\Gm{}_K).
\end{equation}
Elle d\'efinit pour tout nombre premier \(\ell\) un accouplement
\begin{equation}\tag{1.0.2}
 \psi_\ell:T_\ell(A_K)\times T_\ell(A'_K)\longrightarrow T_\ell(\Gm{}_K),
\end{equation}
et, si \(\ell\ne p\), un accouplement parfait de \(\Z_\ell\)-modules libres \`a op\'erateurs
\begin{equation}\tag{1.0.3}
 \psi_\ell:T_\ell(A_K(\bar K))\times T_\ell(A'_K(\bar K))\longrightarrow \Z_\ell(1).
\end{equation}
Une polarisation \(\xi\in\NS(A_K/K)\) donne \(t_\xi:A_K\to A'_K\), puis une forme altern\'ee
\begin{equation}\tag{1.0.7}
 \psi_{\ell,\xi}(x,y)=\psi_\ell(x,T_\ell(t_\xi)y).
\end{equation}

Le mod\`ele de N\'eron \(A\) de \(A_K\) repr\'esente
\begin{equation}\tag{1.1.1}
 S'\longmapsto\Hom_K(S'_K,A_K)
\end{equation}
sur les \(S\)-sch\'emas lisses.  C'est un sch\'ema en groupes commutatif lisse sur \(S\), de fibre g\'en\'erique \(A_K\).  On note \(A_s\) sa fibre sp\'eciale, \(A_s^0\) sa composante neutre, et
\begin{equation}\tag{1.1.5}
 \Phi=A_s/A_s^0
\end{equation}
le groupe des composantes connexes.  Pour le dual on obtient \(A'\), \(A_s^{\prime 0}\), et \(\Phi'=A'_s/A_s^{\prime 0}\).

Le prolongement de la biextension de Poincar\'e donne l'accouplement
\begin{equation}\tag{1.2.1}
 \Phi\times\Phi'\longrightarrow (\Q/\Z)_k.
\end{equation}
\textbf{Conjecture 1.3.}  Cet accouplement est une dualit\'e parfaite.
Pour les sous-groupes ouverts \(U\subset A\), \(U'\subset A'\) auxquels \(\cW_K\) se prolonge, les sous-groupes correspondants de \(\Phi\) et \(\Phi'\) sont orthogonaux.  En particulier il existe une biextension canonique
\begin{equation}\tag{1.4.4}
 \cW^0\in\Biext^1(A^0,A^{\prime 0};\Gm{}_S).
\end{equation}
Sa fibre sp\'eciale est s\'eparante des deux c\^ot\'es.

\section*{2. Partie fixe et partie torique de \(T_\ell(A_K)\)}
Le plus grand sous-tore de \(A_s^0\) est not\'e
\begin{equation}\tag{2.1.1}
 T_s\subset A_s^0.
\end{equation}
Si \(k\) est parfait, ou si une d\'ecomposition affine-ab\'elienne est donn\'ee, on a
...
\begin{equation}\tag{2.1.10}
 n=\mu+\lambda+a,
 \qquad
 \rk T_\ell(A_s^0)=\mu+2a.
\end{equation}
Les invariants correspondants sont les m\^emes pour \(A_K\) et son dual.

\textbf{Lemme 2.2.1.}  Les conditions : \(m\Id_{A^0}\) plat, surjectif, quasi-fini, sont \'equivalentes \`a : \(m\) premier \`a \(\Car k\), ou \(A_s^0\) de rang unipotent nul.  Elles impliquent que les noyaux \({}_{m^\nu}A^0\) sont quasi-finis, s\'epar\'es et plats sur \(S\), avec transitions fid\`element plates.

On d\'efinit alors
\begin{equation}\tag{2.2.2.1}
 T_\ell(A^0)=\varprojlim_\nu {}_{\ell^\nu}A^0.
\end{equation}
Si \(S\) est hens\'elien, tout sch\'ema quasi-fini et s\'epar\'e \(X/S\) se d\'ecompose canoniquement
\begin{equation}\tag{2.2.3.1}
 X=X^f\amalg X'
\end{equation}
avec \(X^f\) fini sur \(S\) et \(X'_s=\emptyset\).  On obtient ainsi la partie fixe
\begin{equation}\tag{2.2.3.4}
 T_\ell(A_K)^f\subset T_\ell(A_K).
\end{equation}
Pour \(\ell\ne p\), c'est exactement le sous-module des invariants sous l'inertie :
\begin{equation}\tag{2.2.5}
 T_\ell(A_K(\bar K))^f=T_\ell(A_K(\bar K))^I.
\end{equation}
Les isog\'enies respectent ces parties fixes, ainsi que les parties toriques et ab\'eliennes; les entiers \(\mu,\lambda,a\) sont donc invariants par isog\'enie.  Le crit\`ere de bonne r\'eduction s'exprime par les conditions \'equivalentes : existence d'un sch\'ema ab\'elien sur \(S\) de fibre \(A_K\), propret\'e de \(A\) ou de \(A^0\), nullit\'e de \(\lambda\) et \(\mu\), ou trivialit\'e de l'action de l'inertie sur \(T_\ell(A_K)\).  C'est le crit\`ere de N\'eron--Ogg--Shafarevich.

La partie torique est d\'efinie par
\begin{equation}\tag{2.3.4}
 T_\ell(A_K)^t\subset T_\ell(A_K)^f\subset T_\ell(A_K),
\end{equation}
et de m\^eme pour \(A'_K\).

\textbf{Th\'eor\`eme 2.4 (orthogonalit\'e).}  Si \(S\) est hens\'elien et \(\ell\ne p\), alors \(T_\ell(A_K)^t\) est l'intersection de la partie fixe \(T_\ell(A_K)^f\) avec l'orthogonal de la partie fixe \(T_\ell(A'_K)^f\) pour l'accouplement canonique.  Sym\'etriquement, \(T_\ell(A'_K)^t\) est l'intersection analogue du c\^ot\'e dual.

La d\'emonstration restreint l'accouplement donn\'e par \(\cW^0\) aux parties fixes, passe aux fibres sp\'eciales, puis utilise le fait que la fibre sp\'eciale \(\cW_s\) est non d\'eg\'en\'er\'ee.  Avec une polarisation, si \(L=T_\ell(A_K)\), \(V=T_\ell(A_K)^f\), \(W=T_\ell(A_K)^t\), on obtient
\begin{equation}\tag{2.5.3}
 W=V\cap V^\perp,
\end{equation}
avec les rangs successifs \(\mu,2a,\lambda,\mu\).  Une preuve arithm\'etique donne l'inclusion par un argument de poids de Frobenius, mais non l'\'egalit\'e n\'ecessaire \`a la r\'eduction semi-stable.
