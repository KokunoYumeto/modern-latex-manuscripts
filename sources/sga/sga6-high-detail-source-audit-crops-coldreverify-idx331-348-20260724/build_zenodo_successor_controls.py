#!/usr/bin/env python3
"""Build exact same-concept Zenodo controls for the idx331-348 crop increment."""
...
PREDECESSOR_ZIP_COUNT = 23
PREDECESSOR_ZIP_MEMBERS = 3171
PREDECESSOR_ZIP_UNCOMPRESSED_BYTES = 315098030
PARENT_SHA256 = (
...
def main() -> int:
    ...
    if len(predecessor_files) != 48:
        raise RuntimeError(
            f"expected 48 predecessor files, found {len(predecessor_files)}"
        )
...
adds the next compact SGA6 source-audit crop increment. All 45 files outside
...
Six image archives now preserve 1,939 targeted formula, glyph, punctuation,
diagram, and prose-detail crops actually used during the active SGA6 source
audit. Five metadata archives preserve parent identity, page/crop provenance,
exact manifests, and 1,868 routine whole-page or page-band derivatives as
...
for all 93 associated images from parent indices 331 through 348. The targeted
crops confirm `i.j` punctuation at idx333, the source factor `{{Xt}}` rather
than `{{1+Xt}}` at idx335, and lowercase prose `v` against formula `\\nabla`
at idx339. These source readings were documented and preserved.

The temporal snapshot closes after cold reverification reached index 348 and
before index 349 began. Earlier archives include crops from separate
        ...
        final_rows.append(
            {
                ...
                "provenance": (
                    "SGA6 idx331-348 incremental high-detail source-audit "
    ...
    validation = {
        ...
        "github": {
            ...
            "package": (
                ...
                "coldreverify-idx331-348-20260724"
            ),
            "metadata_files": 14,
            "readback_files": 14,
            "readback_errors": [],
        ...
        "sga6_source_audit_crops": {
            ...
            "cumulative_targeted_images": 1939,
            "cumulative_rights_blocked_routine_page_derivatives": 1868,
