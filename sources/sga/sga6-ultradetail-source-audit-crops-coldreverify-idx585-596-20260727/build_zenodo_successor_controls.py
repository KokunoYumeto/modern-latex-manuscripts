#!/usr/bin/env python3
"""Build exact same-concept Zenodo controls for the SGA6 idx585-596 crop release."""
...
3. English SGA3 cumulative working reader through complete Expose IV, followed
   by standalone native-diagram Exposes V and VI and a bounded complete Expose
   VIII checkpoint. Expose VII and Exposes IX-XXVI remain absent.
...
re-verification. They come from parent indices 585-596 and map to the stable
12-entry audit boundary #1337-#1348. The tranche spans the Expose X appendix
from the end of section 7.11 through section 7.14. Each public crop records
...
at exact dimensions and correlate to the viewer attachments returned to Claude
during its recorded `Read` events. Byte/pixel-exact replays and disclosed
renderer-version drift are distinguished per manifest row, and the source-image
hashes are disjoint from prior public crop hashes. The {blocked_bands} routine
bands retain exact source identities and PNG dimensions without redundant
public pixels.
...
"""
    ...
    final_rows.append(
        {
            ...
            "provenance": (
                "current compact same-concept release note; SGA6 idx585-596 "
        ...
        final_rows.append(
            {
                ...
                "provenance": (
                    "SGA6 idx585-596 ultra-detail source-audit crop tranche; "
    ...
    validation = {
        ...
        "sga6_source_audit_crops": {
            "scope": "parent indices 585-596; audit entries 1337-1348",
            "expose_boundary": (
                "Expose X appendix idx585-596; end of section 7.11 through "
                "section 7.14; idx597 and later excluded"
            ),
