#!/usr/bin/env python3
"""Build the SGA6 idx585-596 rights-curated crop release.
...
START_ENTRY = 1337
END_ENTRY = 1348
SELECTED_AUDIT_ROWS = 12
SELECTED_AUDIT_SHA256 = (
    "795D51BA05ECDA45E4976DBD64EA9523C18AD39A3A424BCCD0D1F6319EEB79F4"
)
EXPECTED_TIGHT_FILES = 17
EXPECTED_OPENED_TIGHT_FILES = 16
EXPECTED_OPENED_TIGHT_BYTES = 817_300
EXPECTED_UNREAD_TIGHT_FILES = 1
EXPECTED_ROUTINE_FILES = 60
EXPECTED_ROUTINE_BYTES = 14_626_507
...
def main() -> int:
    ...
    parent_identity = {
        ...
        "claim_boundary": (
            "Sparse visual provenance and QA evidence through idx596 only; "
    ...
    readme = f"""# SGA6 targeted ultra-detail source-audit crops, indices 585-596
...
- Every included full-resolution PNG is frozen by exact identity and correlated
  to the downsampled viewer attachment returned to Claude for its recorded
...
{len(excluded_rows)} additional tight crop was generated but never opened in
...
the Expose X appendix from the end of section 7.11 through section 7.14; the
final included audit entry closes idx596. Work beginning at idx597 is excluded.
...
"""
    ...
    validation = {
        "schema": "sga6_targeted_ultradetail_idx585_596_validation_v1",
        "status": "PASS" if not errors else "FAIL",
        "errors": errors,
        "selection": {
