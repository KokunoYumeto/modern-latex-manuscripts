...
\section*{Expos\'e XII. A relative representability theorem for the Picard functor}
\begin{center}
...
The purpose of Expos\'es XII and XIII is to prove the nine finiteness theorems
...
The present Expos\'e XII was presented orally by M. Raynaud following
...
\textbf{Theorem 1.1.} Let $S$ be a noetherian scheme, let $X$ and $Y$ be two
...
\textbf{Corollary 1.2.} Let $X$ be a proper scheme over a noetherian scheme
...
$\Pic^q_{X/S}$ are of finite type over $S$. Thus 1.2 is known when $X$ is in
addition projective and integral over $S$; the general case will be deduced
using the following lemma.

\textbf{Lemma 1.3.} Let $X$ be of finite type over an integral noetherian
...
a sum of quasi-projective open subschemes of finite type. Finally, 1.1 says
...
\textbf{Lemma 1.4.} Let $k$ be a field, let $P$ and $Q$ be $k$-group schemes
...
\textbf{Corollary 1.6.} Let $S$ be an integral noetherian scheme, and let
...
\textbf{Remark 1.7.} It is not known whether 1.1 remains valid when
``quasi-affine'' is replaced by ``affine,'' as 1.5 and 1.6 might suggest.

% Current rescribe index 612; printed volume page 599; source-PDF page 602.
\section*{2. First reductions}

\textbf{2.1.} Apply 1.3 to $X/S$ and $Y/S$. One obtains a nonempty open
...
\textbf{2.2.} Consider assertion II. For every $i$, let $j=\varphi(i)$ be an
...
the proof of 1.2, proving $\sgacircled{4}$ is therefore reduced to proving
the following assertion:

\medskip
\noindent
$\mathrm{II}'$. Theorem 1.1 when $X$ and $Y$ are integral over $S$.
\medskip
...
\textbf{Proposition 2.3.} Let $S$ be a scheme, let $g:Y\to S$ be a proper
...
\textbf{Lemma 2.4.} Let $X$ be a flat, proper scheme of finite presentation
