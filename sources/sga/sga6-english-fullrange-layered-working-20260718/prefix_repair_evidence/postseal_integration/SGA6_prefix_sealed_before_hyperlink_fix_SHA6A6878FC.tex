...
\[
% Source-PDF p. 14 and the French workpass print K^\bullet(X) here.  Since the
% surrounding statement concerns perfect complexes on Y, the provisional English
% uses the mathematically typed domain Y; see the prefix-repair comparison ledger.
...
\textbf{Exercises 5.7.} \textbf{a)} Suppose that \(A\) is commutative and that \((S',A')\) is faithfully flat over \((S,A)\) (cf. (2.16.2)). In order that \(E\in\Ob\Dpx^-(A,S)\)\footnote{Here \(S\) denotes the final object of the topos \(S\).} be of flat amplitude contained in \([m,n]\), it is necessary and sufficient that \(f^*(E)\) have that property.

\textbf{b)} Let \(F\in\Ob\Dpx^+(A,S)\)\footnotemark[\value{footnote}]. Show the equivalence of the following conditions:
\begin{enumerate}[label=(\roman*)]
...
\end{equation}
\footnotetext{We write \(S\) for the final object of the topos \(S\), and \(\mathbb Z_S\) for the constant sheaf with value \(\mathbb Z\).}
...
\]
is exact. For this it is enough to prove that, for every injective \(\mathbb Z_S\)-module \(I\), the sequence obtained by applying the functor \(\Hom_{\mathbb Z_S}(\cdot,I)\) is exact. But there is a commutative diagram, where the vertical arrows are the canonical arrows of (3.17.1):
\[
...
\]
is right exact (respectively left exact) on \(\Mod(\mathcal O_{X,x})\) is equivalent, by (5.2 iii), to saying that \(Z'^{p+1}(E_x)\) (respectively \(Z'^p(E_x)\)) is flat, hence free of finite type since it is of finite presentation. Since
\[
...
\[
 ...
 K'(A_X)_{\naif}=k(\Loclib(A_X)),
\]
where \(k(\ )\) is defined by (1.5.2), the ambient abelian category being \(\Mod(A_X)\).\footnote{We shall allow ourselves to omit the index \(A\) when no confusion can result. We shall sometimes also drop the index ``naive'', but will then say so explicitly.} We say that \(K_{\bullet}(A_X)\), respectively \(K'(A_X)\), is the Grothendieck group of pseudo-coherent, respectively perfect, \(A\)-module complexes on \(X\), and that \(K_{\bullet}(A_X)_{\naif}\), respectively \(K'(A_X)_{\naif}\), is the Grothendieck group of coherent, respectively locally free, left \(A\)-modules.
...
\textbf{5.4.} In the \(\lambda\)-ring generated over \(\mathbb Z\) by \(N,x,y\), subject to \(\lambda^k(N)=0\) for \(k>d\), formula (5.3.1) gives
\begin{align*}
...
\end{equation}
Indeed, let \(A_0\) be the \(K\)-\(\lambda\)-algebra generated by \(N'_0,X'_0\), subject to \(\gamma^k(N'_0)=0\) for \(k>d\), and augmented by \(\epsilon(N'_0)=\epsilon(X'_0)=0\). This is a polynomial algebra over \(K\) in the variables \(\gamma^i(N'_0)\), \(1\le i\le d\), and \(\gamma^j(X'_0)\), \(j\ge1\), and its \(\lambda\)-filtration is the weight filtration, with weight \(i\) assigned to \(\gamma^i(N'_0)\) and \(\gamma^i(X'_0)\) (4.12 and 4.15). Put
\[
  N_0=N'_0+d,
...
\]
by (5.5.4). This holds because \(\gamma^d(N'_0)\) has zero augmentation. By 4.7 there is a \(K\)-\(\lambda\)-homomorphism \(A_0\to A\), compatible with augmentations and sending \(N'_0\) to \(N-d\) and \(X'_0\) to \(x-\epsilon(x)\), hence sending \(N_0\) to \(N\) and \(X_0\) to \(x\). Compatibility with the filtrations gives the result.
...
\textbf{Lemma 1.5.1.} Let \(A\) be a ring, let \(B\) be a positively graded \(A\)-algebra with \(B_0=A\), and let \(M\) be a graded \(B\)-module. Suppose that \(M\), regarded as an ungraded \(B\)-module, is projective of finite type. Let \(\widetilde B\) be the sheaf of algebras on \(\Spec(A)\) defined by \(B\), and let \(\widetilde M\) be the associated graded \(\widetilde B\)-module. Then, locally on \(\Spec(A)\), \(\widetilde M\) is free of finite type over \(\widetilde B\) as a graded module (1.2).
...
\textbf{Lemma 1.5.2.} Let \(A\) be a ring, let \(B\) be an \(A\)-algebra of finite presentation, and let \(M\) be a \(B\)-module of finite presentation. Suppose that \(B\) and \(M\) are flat over \(A\), and that, for every residue field \(k\) of \(A\), the \(B\otimes_A k\)-module \(M\otimes_A k\) has projective dimension at most \(n\) (as is the case if \(B=A[t_0,\ldots,t_r]\)). Then \(M\) has a resolution of length \(n\) by projective \(B\)-modules of finite type:
\[
...
\textbf{Proposition 1.6.} Let \(S\) be quasi-compact, let \(E\) be locally free of rank \(r+1\), let \(B=\Sym(E)\) with its usual grading, and let \(M\) be a graded \(B\)-module of finite presentation, flat over \(S\). Then \(M\) admits a graded resolution
\[
  0\longrightarrow L_{r+1}\longrightarrow\cdots\longrightarrow L_0\longrightarrow M\longrightarrow 0,
\]
...
\textbf{Proposition 1.7.} Let \(S\) be quasi-compact, let \(E\) be locally free of finite type, let \(B=\Sym(E)\), and let \(L\) be a graded \(B\)-module locally free of finite type. Then \(L\) admits a finite decreasing filtration by graded submodules \(L^{(m)}\) such that
\[
\begin{array}{ll}
...
\subsection*{1.8. Generation by the powers of \(\xi\)}
...
\textbf{2.3.1.} First let \(S\) be a scheme, let \(A\) be a quasi-coherent \(\mathcal O_S\)-algebra, let \(Y=\Spec(A)\), and let \(f:Y\to S\) be the structural morphism. From the isomorphism
\[
...
\textbf{2.3.2.} Now let \(B\) be a quasi-coherent graded \(\mathcal O_S\)-algebra, let \(M\) be a graded \(B\)-module, and let \(X=\Proj(B)\). For every affine open \(U\) of \(S\), and every section \(s\) of \(B\) over \(U\), homogeneous of degree \(d>0\), write \(M_{(s),U}\) for the homogeneous component of degree \(0\) of the module of fractions \(M_s\); thus
\[
  M_{(s),U}=
...
\textbf{2.3.3.} It follows from 2.3.1 and 2.3.2 that the functor \(\Proj_0\) is exact. Returning to the notation of 2.2, by passage to derived categories one therefore obtains a functor, still denoted \(\Proj_0\):
\[
...
\textbf{Proposition 4.10.} Let \(S\) be a quasi-compact scheme. Then, with the notation of 4.9, one has an isomorphism
\[
...
\]
where the \(\sigma_j\) denote the elementary symmetric functions. By 4.4, the canonical homomorphism \(A\to C\) is injective. We obtain
\[
  \gamma_t(F_1-n)
  =1+c_1t+\cdots+c_nt^n
  =\prod_{i=1}^n(1+x_it).
