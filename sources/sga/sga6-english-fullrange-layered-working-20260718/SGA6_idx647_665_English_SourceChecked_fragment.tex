...
% French control: sga6_fr_workpass.tex at commit 8ccdcf8ee, working-file
% SHA-256 77703F2D7E8FF9000C2C1E7320A903A48ADE00BF62C8F5F240FF88C42ED82703.
% Current-rescribe idx647--662 is certified; idx663--665 is a scan-checked
% draft after the certified checkpoint.
...
\textbf{Lemma 3.4.} \emph{Let \(X,Y,X',Y'\) be proper over a noetherian
...
\textbf{Theorem 3.5.} \emph{Let \(S\) be a noetherian scheme, let \(X\)
...
By 3.4 (i), one may assume that \(S\) is reduced and irreducible. There
is a nonempty open subset \(U\subset S\) such that
\(\underline{\Pic}_{Y/S}\) and \(\underline{\Pic}_{X/S}\) are
representable and the restriction of \(f^*\) is quasi-affine (XII 1.1),
hence of finite type by 3.2 (i). Give \(T=S-U\) its induced reduced
structure. By noetherian induction, one may suppose that
\[
\underline{\Pic}_{Y_T/T}\longrightarrow\underline{\Pic}_{X_T/T}
\]
induced by \(f^*\) is of finite type. The assertion follows from 3.4 (i),
applied to \(U\amalg T\to S\).

\textbf{Theorem 3.6.} \emph{Let \(S\) be a noetherian scheme, let \(X\)
...
and \(f\) is surjective. If the theorem is known for \(X'\), then 3.5 and
3.4 (ii) imply it for \(X\). We may suppose \(X/S\) projective.
...
integral over \(S'\) (XII 1.3). Give \(T=S-U\) its induced reduced
...
following easy lemma, then for \(X_{S'}/S'\) by 3.5 and 3.4 (ii), and
finally for \(X/S\) by 3.4 (i).

\textbf{Lemma 3.7.} \emph{Let \(X=\coprod_iX_i\) be of finite type over a
...
In the proof of 3.6 we reduced to \(X\) projective and integral over \(S\);
...
\textbf{Theorem 3.8.} \emph{Let \(S\) be noetherian, let \(Y\) be a
...
\textbf{Lemma 3.9.} \emph{Let \(S\) be integral noetherian and \(X\) an
...
\textbf{Remark 3.10.} Retain 3.9 and suppose there is a resolving
...
In the proof of 3.8 we have reduced to \(S\) integral and \(Y\) normal and
...
by 2.10. Consequently \((f^*)^{-1}(\mathscr L)\) is bounded by 2.11,
...
Picard functors were established in XII 1.2. An argument like XII 2,
using 3.9 and XII 1.1, reduces to \(Y\) normal over \(S\). It is enough
...
\textbf{Lemma 3.11.} \emph{Let \(k\) be algebraically closed, let \(Y\)
...
is of finite type by 3.8 (respectively, 4.7 (iii)). Its kernel \(N\) is
...
\textbf{Remark 3.12.} Retain the hypotheses of 3.11.
\begin{enumerate}[label=(\roman*)]
...
\textbf{Theorem 3.13.} \emph{Let \(S\) be noetherian and \(X\) a
projective \(S\)-scheme with a relatively ample invertible sheaf
...
\(T\) (EGA III 7.9.4). Hence (ii) holds, and 2.1 gives (iii).
...
\textbf{4.1.} Let \(S\) be a scheme and \(G\) a commutative group functor
...
\textbf{Lemma 4.2.} \emph{Let \(\Phi:G\to G'\) be a morphism of
...
\textbf{Lemma 4.3.} \emph{Let \(X,Y\) be proper over \(S\), and let
...
\]
This follows formally from 4.2, XII 1.2, 3.1, and 3.5 (respectively,
3.8).

\textbf{4.4.} Let \(k\) be algebraically closed, \(X\) proper over \(k\),
...
\textbf{Proposition 4.5 (Functorial behavior).} \emph{Let
...
The assertions for \(\tau\)-equivalence follow from 4.1 and 4.3. Let
...
\textbf{Theorem 4.6 (The numerical characterization of
...
For (i) \(\Rightarrow\) (ii), devissage (EGA III 3.1) and 4.5 reduce to
...
\(X'\), then 4.5 gives it for \(X\). Hence
...
integral (4.5, EGA III 2.6.2).

For irreducible \(X\), (ii) implies (v), (iii) implies (vi), and 2.1
...
family of classes \(L^{\otimes m}\), \(m\in\mathbb N\), satisfies 3.13
...
\textbf{Theorem 4.7.} \emph{Let \(S\) be noetherian and \(X\) proper over
\(S\). Then:}
\begin{enumerate}[label=(\roman*)]
...
\(A\)-subalgebras \(B_\alpha\subset B\). By 3.1 the morphism descends to
...
along \(R\to S\). Compatibility with base change (4.1) reduces to a
...
\(S'\) (XII 1.3). It is enough to prove (b),(iii) for \(X_{S'}/S'\), by
descent and by noetherian induction with 3.4 (i). The surjection
\(\coprod_iX'_i\to X_{S'}\), together with 4.3, 3.5, and 3.7, reduces
(b),(iii) to (c),(iii) for each \(X'_i/S'\).

Thus assume \(S\) integral with generic point \(s\), and \(X\) projective
and integral. For a very ample \(\mathcal O_X(1)\), the values
\(\chi(L_t^{\otimes m}(n))\) and \(\chi(\mathcal O_{X_t}(n))\) are
independent of \(t\) (EGA III 7.9.4). Hence (c) follows from 4.6
(i) \(\Longleftrightarrow\) (v), which also says that the classes
\(\tau\)-equivalent to zero have only one

% --- current-rescribe idx663 | printed p. 650 | source-PDF p. 653 ---
% DRAFT AFTER CERTIFIED CHECKPOINT: scan-checked; French certification ends
% at idx662 in commit 8ccdcf8ee.
Hilbert polynomial. Thus (iii) follows from 2.11.

\textbf{Remark 4.8.} Let \(X\) be proper and integral over noetherian
\(S\). Without projectivity, it is unknown whether
\(\underline{\Pic}^{\tau}_{X/S}\to\underline{\Pic}_{X/S}\) remains a
closed immersion, except when \(X\) is normal over \(S\) and
\(\underline{\Pic}_{X/S}\) is representable (FGA no.~236, 2.3).

\subsection*{5. N\'eron--Severi-type finiteness theorems}

\textbf{Theorem 5.1.} \emph{Let \(X\) be proper over noetherian \(S\).
Then the N\'eron--Severi groups}
\[
\Pic_{X_K/K}(K)/\Pic^0_{X_K/K}(K)
\]
\emph{of the geometric fibers are finitely generated, and their ranks
and the orders of their torsion subgroups}
\[
...
\(S\)-scheme (XII 1.2, 4.7 (iii)). The order of the torsion quotient is
...
\(\Pic_{X_K/K}(K)/\Pic^\tau_{X_K/K}(K)\). Reduce as in 4.7 (iii) to
\(S\) integral and \(X\) projective and integral. Let \(r\) be the common
fiber dimension. For \(r=0\), respectively \(1\), the rank is \(0\),
...
\(Y\) using 4.3. Then use 4.3 and 3.10, together with resolution of
surface singularities, and replace \(S\) by a flat cover of a nonempty
open, to make \(X\) smooth over \(S\).

By 4.6 (i) \(\Longleftrightarrow\) (iii) and finiteness and specialization
for \'etale cohomology (SGA 4 XVI 2.2), Theorem 5.1 follows from:

\textbf{Proposition 5.2.} \emph{Let \(k\) be algebraically closed and
...
\qquad(\ell,\operatorname{char}(k))=1,
\]
is a Weil cohomology: it satisfies Poincar\'e duality and the K\"unneth
formula, and receives the functorial cycle-class homomorphism from
\[
C^*_{\mathrm{rat}}(X),
\]
the ring of algebraic cycles modulo rational equivalence (see X 7.13).
...
\[
\alpha:C^p_{\mathrm{rat}}(X)\longrightarrow\mathbb Z^m,\qquad
\alpha(b)=
(\langle b\cdot a_1\rangle,\ldots,\langle b\cdot a_m\rangle).
\]
Its image identifies with \(C^p_{\mathrm{num}}(X)\).

% --- current-rescribe idx665 | printed p. 652 | source-PDF p. 655 ---
\textbf{Remark 5.3.} If \(X\) is a proper smooth surface over
algebraically closed \(k\), then
\[
b^2(X)=c_2(X)-2+2b^1(X),
\]
where \(b^1(X)=2\dim\Pic^0_{X/k}\), and \(c_2(X)\), the second Chern
...
inequality
\[
\operatorname{rank}C^1_{\mathrm{num}}(X)\le b^2(X)
\]
in this form is due, before \'etale cohomology, to Igusa, who used deep
results on the tame fundamental group of an algebraic curve minus
finitely many points.

\textbf{Theorem 5.4.} \emph{Let \(k\) be algebraically closed and \(X\)
