#!/usr/bin/env python3
"""Build the SGA6 idx632-652 rights-curated crop release.
...
START_ENTRY = 1384
END_ENTRY = 1404
SELECTED_AUDIT_ROWS = 21
SELECTED_AUDIT_SHA256 = (
    "83C3FF90D91B9FE0F6643E2FF3EDB838A5E086CC6F6A0321EE3F43E2AA0ACF86"
)
EXPECTED_TIGHT_FILES = 167
EXPECTED_OPENED_TIGHT_FILES = 156
EXPECTED_OPENED_TIGHT_BYTES = 13_304_384
EXPECTED_UNREAD_TIGHT_FILES = 11
EXPECTED_ROUTINE_FILES = 105
EXPECTED_ROUTINE_BYTES = 21_116_925
...
def main() -> int:
    ...
    for count, source in enumerate(
        ...
        replay_disposition = (
            ...
            if pixel_exact and byte_exact
                ...
                if dimensions_exact and bool(difference["dimensions_exact"])
                else (
        ...
        replay[name] = {
            ...
            "replay_dimensions": list(dimensions),
    ...
    parent_identity = {
        ...
        "claim_boundary": (
            "Sparse visual provenance and QA evidence through idx652 only; "
...
- A fresh local replay reproduced dimensions for {dimension_replay_count} of
  {len(target_rows)} crops. The remaining
  {len(target_rows) - dimension_replay_count} show disclosed renderer-version
  geometry drift.
- {exact_replay_count} matched pixel-for-pixel and PNG-byte-for-PNG-byte;
  {drift_replay_count} show disclosed renderer-version geometry or pixel drift
  while the source PNG and viewer attachment remain exact.

{len(excluded_rows)} additional tight crop(s) were generated but never opened in
the audit session. They are not included as pixels; `{EXCLUDED_MANIFEST_NAME}`
records their exact identities and provenance without treating them as used
evidence.
...
one-to-one to parent PDF indices {START_INDEX}-{END_INDEX}. The boundary covers
the continuing Expose XIII source audit from idx632 through idx652. The next
live page, idx653, is excluded.
...
"""
    ...
    validation = {
        "schema": "sga6_targeted_ultradetail_idx632_652_validation_v1",
        "status": "PASS" if not errors else "FAIL",
        "errors": errors,
        "selection": {
