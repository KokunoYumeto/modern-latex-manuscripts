#!/usr/bin/env python3
"""Build exact same-concept Zenodo controls for the SGA6 idx612-617 crop release."""
...
3. English SGA3 cumulative working reader through complete Expose IV, followed
   ...
   VIII and IX checkpoints. Expose VII and Exposes X-XXVI remain absent.
...
re-verification. They come from parent indices 612-617 and map to the stable
6-entry audit boundary #1364-#1369. The tranche spans Expose XII Section 2's
reduction diagrams through the opening of the Oort devissage and Proposition
3.1, items (i)--(iii), at idx617. Each public crop records
...
attachments returned to Claude during its recorded `Read` events. Their
...
Expose VII remains held both by its current machine-evidence failure and by
that diagram gate. Expose XI has now passed the high-zoom diagram gate, but it
still lacks a sealed privacy-clean exact public projection and full release
audit, so it is not added here. Already published bounded SGA3 checkpoints
...
"""
    ...
    final_rows.append(
        {
            ...
            "provenance": (
                "current compact same-concept release note; SGA6 idx612-617 "
        ...
        final_rows.append(
            {
                ...
                "provenance": (
                    "SGA6 idx612-617 ultra-detail source-audit crop tranche; "
    ...
    validation = {
        ...
        "sga6_source_audit_crops": {
            "scope": "parent indices 612-617; audit entries 1364-1369",
            "expose_boundary": (
                "Expose XII Section 2 reduction diagrams through opening "
                "Oort devissage and Proposition 3.1 items i-iii at idx617; "
                "idx618 excluded"
            ),
        ...
        "sga3_diagram_cold_reverify": {
            ...
            "expose_xi": (
                "high_zoom_diagram_gate_passed_awaiting_sealed_privacy_clean_"
                "public_projection_and_full_release_audit"
            ),
        },
        "zip_archive_count": (
