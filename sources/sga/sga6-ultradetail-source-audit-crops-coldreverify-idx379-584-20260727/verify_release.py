#!/usr/bin/env python3
"""Independently replay and verify the SGA6 idx379-584 crop release."""
...
START_ENTRY = 1131
END_ENTRY = 1336
AUDIT_SHA256 = "7215B447B46CF94D9DE807BB35627D008A77381F925DE3AD0E70C68705457D87"
TARGET_COUNT = 155
TARGET_BYTES = 11_951_782
EXCLUDED_COUNT = 8
ROUTINE_COUNT = 1_030
ROUTINE_BYTES = 226_575_905
...
def pixel_equal(data: bytes, source: Path) -> bool:
...
def main() -> int:
    ...
    target_manifest = files[
        "SGA6_Targeted_UltraDetail_Crops_idx379_584_Manifest_20260727.csv"
    ]
    excluded_manifest = files[
        "SGA6_GeneratedUnread_Tight_Crops_idx379_584_"
        "Excluded_Manifest_20260727.csv"
    ]
    blocked_manifest = files[
        "SGA6_PageBands_idx379_584_RightsBlocked_Manifest_20260727.csv"
    ]
    audit_manifest = files[
        "SGA6_Targeted_UltraDetail_Crops_idx379_584_"
        "Audit_Context_20260727.csv"
    ]
    validation_path = files[
        "SGA6_Targeted_UltraDetail_Crops_idx379_584_"
        "VALIDATION_20260727.json"
    ]
    upload_manifest = files[
        "SGA6_Targeted_UltraDetail_Crops_idx379_584_"
        "ZENODO_UPLOAD_MANIFEST_20260727.csv"
    ]
    readme = files[
        "SGA6_UltraDetail_Crops_idx379_584_README_20260727.md"
    ]
    parent_json = files[
        "SGA6_UltraDetail_Crops_idx379_584_PARENT_SOURCE_20260727.json"
    ]
    ...
    for index, row in enumerate(target_rows, start=1):
        ...
        pixel_exact = byte_exact or pixel_equal(data, source)
        if not byte_exact:
            errors.append(f"target byte replay mismatch: {source.name}")
        if not pixel_exact:
            errors.append(f"target pixel replay mismatch: {source.name}")
    ...
    result = {
        "schema": "sga6_targeted_ultradetail_idx379_584_independent_replay_v1",
        "status": "PASS" if not errors else "FAIL",
        "errors": errors,
    ...
    output_json = package / "INDEPENDENT_REPLAY_VALIDATION_20260727.json"
    output_json.write_text(
        json.dumps(result, indent=2, ensure_ascii=True) + "\n",
        encoding="utf-8",
    )
    report = package / "INDEPENDENT_REPLAY_PASS_20260727.md"
    report.write_text(
        "# Independent replay: SGA6 idx379-584 ultra-detail crops\n\n"
        f"Status: **{result['status']}**\n\n"
        f"- Targeted crops replayed: {len(target_rows)}/{TARGET_COUNT} "
        "pixel-exact and PNG-byte-exact.\n"
