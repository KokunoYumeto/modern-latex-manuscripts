#!/usr/bin/env python3
"""Build the SGA6 idx379-584 rights-curated crop release.
...
START_ENTRY = 1131
END_ENTRY = 1336
SELECTED_AUDIT_ROWS = 206
SELECTED_AUDIT_SHA256 = (
    "7215B447B46CF94D9DE807BB35627D008A77381F925DE3AD0E70C68705457D87"
)
EXPECTED_TIGHT_FILES = 163
EXPECTED_OPENED_TIGHT_FILES = 155
EXPECTED_OPENED_TIGHT_BYTES = 11_951_782
EXPECTED_UNREAD_TIGHT_FILES = 8
EXPECTED_ROUTINE_FILES = 1_030
EXPECTED_ROUTINE_BYTES = 226_575_905
...
def pixels_equal(left_bytes: bytes, right_path: Path) -> bool:
    Image.MAX_IMAGE_PIXELS = None
    with Image.open(io.BytesIO(left_bytes)) as left:
        with Image.open(right_path) as right:
            ...
            equal = (
                left_l.size == right_l.size
                and ImageChops.difference(left_l, right_l).getbbox() is None
            )
            left_l.close()
            right_l.close()
            return equal
...
def main() -> int:
    ...
    for count, source in enumerate(
        ...
        pixel_exact = byte_exact or pixels_equal(replay_bytes, image_path)
        if dimensions != (int(source["width_px"]), int(source["height_px"])):
    ...
    write_csv(target_manifest, target_rows, list(target_rows[0]))
    write_csv(excluded_manifest, excluded_rows, list(excluded_rows[0]))
    write_csv(blocked_manifest, blocked_rows, list(blocked_rows[0]))
    ...
    parent_identity = {
        ...
        "claim_boundary": (
            "Sparse visual provenance and QA evidence through idx584 only; "
    ...
    readme = f"""# SGA6 targeted ultra-detail source-audit crops, indices 379-584
...
- The 155 images record 158 explicit inspection events across
...
- Every included crop was independently regenerated from the exact parent
  PDF and matched pixel-for-pixel and PNG-byte-for-PNG-byte.

Eight additional tight crops were generated but never opened in the audit
session. They are not included as pixels; `{EXCLUDED_MANIFEST_NAME}` records
their exact identities and provenance without treating them as used evidence.
...
Exposes VI-X and the Expose X appendix; the final included audit entry is
idx584, the opening of section 7.11. Work beginning at idx585 is excluded.
...
"""
    ...
    validation = {
        "schema": "sga6_targeted_ultradetail_idx379_584_validation_v1",
        "status": "PASS" if not errors else "FAIL",
        "errors": errors,
        "selection": {
