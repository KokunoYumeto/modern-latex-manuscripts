...
\providecommand{\Card}{\operatorname{Card}}\begin{document}
...
Soient, pour $i=1,2$, $X_i$ un $S$-schéma, $X=X_1\times_S X_2$, $p_i:X\to X_i$ les projections, $L_i\in\ob\Dctf(X_i)$. Toute correspondance cohomologique $u$ de $L_1$ à $L_2$ donne, par application de $D_X$ et identification de $D p_2^!$ à $p_2^*D$, de $D p_1^*$ à $p_1^!D$, une correspondance $Du$ de $DL_2$ à $DL_1$, dite duale de $u$. En d'autres termes, $D$ définit un homomorphisme
\begin{equation}
\tag{5.1.1}
...
\begin{equation}
\tag{5.1.2}
D:\RHom(p_1^*L_1,p_2^!L_2)\longrightarrow \RHom(p_2^*DL_2,p_1^!DL_1)
\end{equation}
rendant commutatif le carré
\begin{equation}
\tag{5.1.3}
\begin{tikzcd}[column sep=large,row sep=large]
p_1^!DL_1\otimes^{\LL}p_2^*L_2 \arrow[r,"3.1.1"] \arrow[d] &
\RHom(p_1^*L_1,p_2^!L_2) \arrow[d,"D"]\\
p_2^*DDL_2\otimes^{\LL}p_1^!DL_1 \arrow[r,"3.1.1"] &
...
\tag{5.1.4}
\begin{tikzcd}[column sep=large,row sep=large]
\RHom_S(L_1,L_2)\otimes^{\LL}\RHom_S(L_2,L_1) \arrow[r,"4.1.3"] \arrow[d,"D\otimes D"] &
K_X \arrow[d,"\Id"]\\
\RHom_S(DL_2,DL_1)\otimes^{\LL}\RHom_S(DL_1,DL_2) \arrow[r,"4.1.3"] &
K_X .
\end{tikzcd}
\end{equation}
Soit $c:C\to X$ un $S$-morphisme, posons $c_i=p_i c$; on déduit de 5.1.2, par application de $c_*c^!$ (3.2.1), un isomorphisme
\begin{equation}
\tag{5.1.5}
D:\Hom(c_1^*L_1,c_2^!L_2)\xrightarrow{\sim}
\Hom(c_2^*DL_2,c_1^!DL_1),
\end{equation}
dont on vérifie qu'il s'identifie à celui donné par application du foncteur $D_X$ et les identifications de $Dc_2^*$ à $c_2^!D$ et de $Dc_1^!$ à $c_1^*D$. Si $c':C'\to X$, $c'':C''\to X$ sont des $S$-morphismes, il découle de 5.1.4 que l'on a, pour
\[
u\in\Hom(c_1'{}^*L_1,c_2'{}^!L_2),\qquad
v\in\Hom(c_2''{}^*L_2,c_1''{}^!L_1),
\]
...
Soient, pour $i=1,2,3$, $X_i$ un $S$-schéma, $X_{ij}=X_i\times_S X_j$, $X=X_1\times_S X_2\times_S X_3$, $p_i:X\to X_i$, $p_{ij}:X\to X_{ij}$ les projections, $L_i\in\ob\Dctf(X_i)$. On a $X=X_{12}\times_{X_2}X_{23}$, et
\[
(DL_1\otimes_S^{\LL}L_2)\otimes_{X_2}^{\LL}(DL_2\otimes_S^{\LL}L_3)
=
p_1^*DL_1\otimes^{\LL}p_2^*L_2\otimes^{\LL}p_2^*DL_2\otimes^{\LL}p_3^*L_3 .
\]
La flèche d'évaluation $L_2\otimes^{\LL}DL_2\to K_{X_2}$ définit donc un accouplement
\begin{equation}
\tag{5.2.1}
(DL_1\otimes_S^{\LL}L_2)\otimes_{X_2}^{\LL}(DL_2\otimes_S^{\LL}L_3)
\longrightarrow p_{13}^!(DL_1\otimes_S^{\LL}L_3),
\end{equation}
le second membre étant identifié à $(DL_1\otimes_S^{\LL}L_3)\otimes_S^{\LL}K_{X_2}$ par Künneth (1.7.3). Compte tenu de 3.1.1, 3.1.2, on peut récrire 5.2.1 sous la forme
\begin{equation}
\tag{5.2.2}
\RHom_S(L_1,L_2)\otimes_{X_2}^{\LL}\RHom_S(L_2,L_3)
\longrightarrow p_{13}^!\RHom_S(L_1,L_3)=\RHom(p_1^*L_1,p_3^!L_3),
\end{equation}
...
\begin{equation}
\tag{5.2.3}
\Hom_S(L_1,L_2)\otimes \Hom_S(L_2,L_3)
\longrightarrow \Hom(p_1^*L_1,p_3^!L_3),
\end{equation}
...
\]
produit tensoriel des flèches canoniques
\[
p_{12}^*\RHom_S(L_1,L_2)\longrightarrow
\RHom(p_1^*L_1,p_{12}^!p_{12,2}^*L_2)
\]
et
\[
p_{23}^*\RHom_S(L_2,L_3)\longrightarrow
\RHom(p_{23}^*p_{23,2}^*L_2,p_3^!L_3),
\]
de la flèche définie par la flèche de changement de base
\[
p_{12}^!p_{12,2}^*L_2\longrightarrow p_{23}^*p_{23,2}^*L_2,
\]
et d'une flèche de composition habituelle sur les $\RHom$.
...
\begin{equation}
\tag{5.2.4}
\RHom(c_1'{}^*L_1,c_2'{}^!L_2)\otimes_{X_2}^{\LL}
\RHom(c_2''{}^*L_2,c_3''{}^!L_3)
...
\[
\begin{tikzcd}[column sep=large,row sep=large]
C' \arrow[r] \arrow[d] & X_{12} \arrow[d,"f_{12}"]\\
D' \arrow[r] & Y_{12}
\end{tikzcd}
\qquad
\begin{tikzcd}[column sep=large,row sep=large]
C'' \arrow[r] \arrow[d] & X_{23} \arrow[d,"f_{23}"]\\
D'' \arrow[r] & Y_{23}.
\end{tikzcd}
\]
...
\arrow[r,"5.2.5"] \arrow[d,"f_{12*}\otimes f_{23*}"] &
\Hom(c_1^*L_1,c_3^!L_3) \arrow[d,"f_{13*}"]\\
\Hom(d_1'{}^*M_1,d_2'{}^!M_2)\otimes\Hom(d_2''{}^*M_2,d_3''{}^!M_3)
\arrow[r,"5.2.5"] &
\Hom(d_1^*M_1,d_3^!M_3),
\end{tikzcd}
\end{equation}
où $M_1=f_{1!}L_1$, $M_2=f_{2*}L_2$, $M_3=f_{3*}L_3$, et un carré commutatif analogue avec des $\RHom$, que le lecteur écrira.
...
\begin{equation}
\tag{5.2.7}
q_1^*DL_1\otimes^{\LL}q_2^*L_2\otimes^{\LL}q_2^*DL_2\otimes^{\LL}q_1^*L_1
\longrightarrow
q_1^*DL_1\otimes^{\LL}q_2^*K_{X_2}\otimes^{\LL}q_1^*L_1
\end{equation}
défini par $L_2\otimes^{\LL}DL_2\to K_{X_2}$ ou, ce qui revient au même, induit par 5.2.1 sur $X_{12}$ via la diagonale
\[
X_{12}\longrightarrow X_1\times_S X_2\times_S X_1
\]
($q_i:X_{12}\to X_i$ désignant la projection), et de l'accouplement
\begin{equation}
\tag{5.2.8}
q_1^*DL_1\otimes^{\LL}q_2^*K_{X_2}\otimes^{\LL}q_1^*L_1
\longrightarrow K_{X_{12}}
\end{equation}
défini par $DL_1\otimes^{\LL}L_1\to K_{X_1}$. Il en résulte que, pour $u\in\Hom_S(L_1,L_2)$, $v\in\Hom_S(L_2,L_1)$, on a, dans $\H^0(X_1\times_S X_2,K_{X_1\times_S X_2})$,
\begin{equation}
\tag{5.2.9}
\langle u,v\rangle=\langle vu,1\rangle,
\end{equation}
où $1$ désigne la correspondance identique de $L_1$ à $L_1$ sur $X_1\times_S X_1$, à support dans la diagonale (3.2 b)), et le cup-produit au second membre est pris au sens de 4.2.5, compte tenu du fait que le produit fibré $X\times_{(X_1\times_S X_1)}X_1$ (par $p_{13}:X\to X_{13}=X_1\times_S X_1$ et la diagonale $X_1\to X_1\times_S X_1$) s'identifie à $X_1\times_S X_2$. On voit de même, plus généralement, que, si $c':C'\to X_{12}$, $c'':C''\to X_{21}$ sont des correspondances, $c:C=C'\times_{X_2}C''\to X_1\times_S X_2\times_S X_1$ la correspondance composée, et $d:C'\times_{X_{12}}C''=D\to X_{12}$ la correspondance intersection, on a, pour $u\in\Hom(c_1'{}^*L_1,c_2'{}^!L_2)$, $v\in\Hom(c_2''{}^*L_2,c_1''{}^!L_1)$, l'égalité suivante dans $\H^0(D,K_D)$:
\begin{equation}
\tag{5.2.10}
\langle u,v\rangle=\langle vu,1\rangle,
\end{equation}
où $1$ désigne comme ci-dessus la correspondance diagonale, et $D$ est identifié au produit fibré de $C\to X_1\times_S X_1$ et la diagonale $X_1\to X_1\times_S X_1$. Les formules 5.2.9 et 5.2.10 généralisent une formule bien connue dans le cas des coefficients constants ([12] 1.3.6 (ii) c)).

\subsection*{5.3. Produit tensoriel externe de correspondances}

Soient, pour $1\leq i\leq 4$, $X_i$ un $S$-schéma, $X=X_1\times_S X_2\times_S X_3\times_S X_4$, $X_{ij}=X_i\times_S X_j$, $L_i\in\ob\Dctf(X_i)$. L'isomorphisme de Künneth (1.7.6, 2.2.4)
\[
DL_1\otimes_S^{\LL}DL_3\xrightarrow{\sim}D(L_1\otimes_S^{\LL}L_3)
\]
définit un isomorphisme sur $X$
\begin{equation}
\tag{5.3.1}
(DL_1\otimes_S^{\LL}L_2)\otimes_S^{\LL}(DL_3\otimes_S^{\LL}L_4)
\xrightarrow{\sim}
D(L_1\otimes_S^{\LL}L_3)\otimes_S^{\LL}(L_2\otimes_S^{\LL}L_4),
\end{equation}
qu'on peut, d'après 3.1.1, récrire sous la forme
\begin{equation}
\tag{5.3.2}
\RHom_S(L_1,L_2)\otimes_S^{\LL}\RHom_S(L_3,L_4)
\xrightarrow{\sim}
\RHom_S(L_1\otimes_S^{\LL}L_3,L_2\otimes_S^{\LL}L_4).
\end{equation}
...
\longrightarrow
\Hom_S(L_1\otimes_S^{\LL}L_3,L_2\otimes_S^{\LL}L_4),
\end{equation}
qu'on notera $(u,v)\mapsto u\otimes_S^{\LL}v$. L'accouplement 5.3.2 peut aussi s'interpréter comme composé du produit tensoriel externe
\[
\RHom(E_1,E_2)\otimes_S^{\LL}\RHom(E_3,E_4)
\xrightarrow{\sim}
\RHom(E_1\otimes_S^{\LL}E_3,E_2\otimes_S^{\LL}E_4)
\]
(où $E_1=p_{12,1}^*L_1$, etc., avec des notations évidentes) et de l'isomorphisme défini par Künneth $E_2\otimes_S^{\LL}E_4\xrightarrow{\sim}p_{24}^!(L_2\otimes_S^{\LL}L_4)$ (1.7.3).

Soient $c':C'\to X_{12}$, $c'':C''\to X_{34}$ des correspondances, $c:C=C'\times_S C''\to X=X_{13}\times_S X_{24}$ la correspondance produit. On déduit de 5.3.2, ou l'on définit de manière analogue, un accouplement
\begin{equation}
\tag{5.3.4}
\RHom(c_1'{}^*L_1,c_2'{}^!L_2)\otimes_S^{\LL}
\RHom(c_3''{}^*L_3,c_4''{}^!L_4)
\longrightarrow
\RHom(c_{13}^*(L_1\otimes_S^{\LL}L_3),c_{24}^!(L_2\otimes_S^{\LL}L_4)),
\end{equation}
d'où un accouplement produit tensoriel externe, $(u,v)\mapsto u\otimes_S^{\LL}v$,
\begin{equation}
\tag{5.3.5}
\Hom(c_1'{}^*L_1,c_2'{}^!L_2)\otimes
\Hom(c_3''{}^*L_3,c_4''{}^!L_4)
\longrightarrow
\Hom(c_{13}^*(L_1\otimes_S^{\LL}L_3),c_{24}^!(L_2\otimes_S^{\LL}L_4)).
\end{equation}

On vérifie que ces accouplements sont compatibles aux images directes, dans le sens suivant. Soient, pour $1\leq i\leq 4$, $f_i:X_i\to Y_i$ un $S$-morphisme, $c'$, $c''$, $c$ comme ci-dessus, avec $c'$ et $c''$ propres, $d':D'\to Y_{12}$, $d'':D''\to Y_{34}$ des morphismes propres, $d:D=D'\times_S D''\to Y=Y_{12}\times_S Y_{34}$, et des carrés commutatifs
\[
\begin{tikzcd}[column sep=large,row sep=large]
C' \arrow[r] \arrow[d] & X_{12} \arrow[d,"f_{12}"]\\
D' \arrow[r] & Y_{12}
...
\qquad
\begin{tikzcd}[column sep=large,row sep=large]
C'' \arrow[r] \arrow[d] & X_{34} \arrow[d,"f_{34}"]\\
D'' \arrow[r] & Y_{34}.
\end{tikzcd}
\]
On a alors un carré commutatif
...
\arrow[r,"5.3.5"] \arrow[d,"f_{12}\otimes f_{34}"] &
\Hom(c_{13}^*L_{13},c_{24}^!L_{24}) \arrow[d,"f_{13,24*}"]\\
\Hom(d_1'{}^*M_1,d_2'{}^!M_2)\otimes\Hom(d_3''{}^*M_3,d_4''{}^!M_4)
\arrow[r,"5.3.5"] &
\Hom(d_{13}^*M_{13},d_{24}^!M_{24}),
\end{tikzcd}
\end{equation}
où $M_i=f_{i!}L_i$ si $i=1,3$ et $f_{i*}L_i$ sinon, $L_{ij}=L_i\otimes_S^{\LL}L_j$, $M_{ij}=M_i\otimes_S^{\LL}M_j$. On a aussi un carré analogue avec des $\RHom$.
...
\begin{equation}
\tag{5.3.7}
\langle u,v\rangle=\langle u\otimes_S^{\LL}v,1\rangle,
\end{equation}
où $1$ désigne la correspondance diagonale de $L_{12}$ à $L_{12}$ sur $X_{12}$, et le cup-produit au second membre est pris au sens de 4.2.5. Plus généralement, si $c':C'\to X_{12}$, $c'':C''\to X_{21}$ sont des correspondances, leur ``intersection'' $C=C'\times_{X_{12}}C''$ s'identifie au produit fibré de $C'\times_S C''\to X=X_{12}\times_S X_{21}$ et de la diagonale $X_{12}\to X$, et l'on a, pour $u\in\Hom(c_1'{}^*L_1,c_2'{}^!L_2)$, $v\in\Hom(c_2''{}^*L_2,c_1''{}^!L_1)$, l'égalité suivante dans $\H^0(C,K_C)$
\begin{equation}
\tag{5.3.8}
\langle u,v\rangle=\langle u\otimes_S^{\LL}v,1\rangle,
\end{equation}
...
\]
(et d'isomorphismes de transitivité $(gf)^!\simeq f^!g^!$ pour un composé $gf$, avec condition de cocycle). Pour $f$ propre, $f^!$ est ``adjoint'' à droite du foncteur
\[
f_*:D^+(X)_{\coh}\longrightarrow D^+(Y)_{\coh}
\]
(l'indice ``coh'' désignant la sous-catégorie pleine formée de complexes à cohomologie cohérente), cette adjonction se précisant en un isomorphisme
\begin{equation}
\tag{6.1.1}
f_*\RHom(E,f^!F)\xrightarrow{\sim}\RHom(f_*E,F)
\end{equation}
pour $E\in\ob D(X)_{\coh}$, $F\in\ob D^+(Y)$, défini de manière analogue à l'isomorphisme (SGA 4 XVIII 3.1.9.6). Les considérations de ([8] App.5), jointes au théorème de dualité pour l'espace projectif, montrent que, si $f$ est lisse, purement de dimension $d$, on a un isomorphisme canonique
\begin{equation}
\tag{6.1.2}
f^!L\xrightarrow{\sim} f^*L\otimes\Omega^d_{X/Y}[d];
\end{equation}
si de plus $f$ est propre, la flèche
\begin{equation}
\tag{6.1.3}
\Tr_f:f_*(f^*L\otimes\Omega^d_{X/Y}[d])\longrightarrow L
\end{equation}
déduite, par 6.1.2, de la flèche d'adjonction $f_*f^!\to I$, coïncide avec le morphisme trace ([8] VI).
\end{statement}

\begin{statement}{6.2}
Contrairement à ce qui se passe dans le cas des coefficients discrets, il n'y a pas ici de sous-catégorie raisonnable de $D$ stable par ``les six opérations'' $(f^*,f_*,f^!,f_!,\otimes,\RHom)$. La notion de finitude la plus naturelle semble être celle de perfection relative (SGA 6 III 4). Si $X$ est un $S$-schéma, et $L\in\ob D(X)$, rappelons qu'on dit que $L$ est parfait rel. à $S$ si $L$ est de tor-dimension finie rel. à $S$ et à cohomologie cohérente. Nous noterons $D(X)_{\parf/S}$ la sous-catégorie pleine de $D(X)$ formée des complexes parfaits rel. à $S$. Elle n'est stable en général ni par $\otimes^{\LL}$ ni par $\RHom$. Cependant, si $E$ est parfait (au sens absolu), et $F$ parfait rel. à $S$, $\RHom(E,F)$ est parfait rel. à $S$ (SGA 6 III 4.9). D'autre part, soient, pour $i=1,2$, $X_i$ un $S$-schéma, $L_i\in\ob D^-(X_i)$. Nous poserons, comme 1.5,
\[
L_1\otimes_S^{\LL}L_2=Lp_1^*L_1\otimes^{\LL}Lp_2^*L_2,
\]
où $p_i:X_1\times_S X_2\to X_i$ est la projection. La considération de cet objet n'est raisonnable que si $X_1$ et $X_2$ sont tor-indépendants sur $S$, i.e. (SGA 6 III 1.5)
\[
\Tor_i^S(\OO_X,\OO_Y)=0\quad\text{pour }i\ne0.
\]
Supposons qu'il en soit ainsi: alors, si $L_1$ et $L_2$ sont parfaits rel. à $S$, il en est de même de $L_1\otimes_S^{\LL}L_2$ (SGA 6 III 4.7).

Soit $f:X\to Y$ un $S$-morphisme. Si $f$ est propre, on a
\[
f_*D(X)_{\parf/S}\subset D(Y)_{\parf/S}\quad\text{(SGA 6 III 4.8).}
\]
Si $f$ est de tor-dimension finie, on a
\[
f^*D(Y)_{\parf/S}\subset D(X)_{\parf/S},
\qquad
f^!D(Y)_{\parf/S}\subset D(X)_{\parf/S}\quad\text{(SGA 6 III 4.9).}
\]
...
K_X=a^!\OO_S,
\end{equation}
et noterons $D_X$ (ou simplement $D$) le foncteur $\RHom(-,K_X)$. Si $X$ est de tor-dimension finie sur $S$, $K_X$ est parfait rel. à $S$ et, pour tout $L\in\ob D(X)_{\parf/S}$, on a $DL\in\ob D(X)_{\parf/S}$, et la flèche canonique $L\to DDL$ est un isomorphisme (SGA 6 III 4.9).

Soient $f:X\to Y$ un $S$-morphisme, $L\in\ob D(X)$, $M\in\ob D(Y)$. Si $f^*M$ est défini (par exemple si $M\in\ob D^-(Y)$, ou si $f$ est de tor-dimension finie), on posera
\[
\Hom_f^{(1)}(L,M)=\Hom(f^*M,L),\qquad
\Hom_f^{(2)}(L,M)=\Hom(L,f^*M).
\]
Si $f^!M$ est défini (par exemple si $M\in\ob D^+(Y)$, ou si $f$ est lisse, ou fini et de tor-dimension finie, ou composé de tels morphismes, car dans ces cas $f^!$ se prolonge, par 6.1.1, 6.1.2 en un foncteur défini sur la catégorie $D(Y)$ toute entière), on posera
\[
\Hom_f^{(3)}(L,M)=\Hom(L,f^!M),\qquad
\Hom_f^{(4)}(L,M)=\Hom(f^!M,L).
\]
Les éléments de $\Hom_f^{(i)}(L,M)$ s'appelleront, comme en 1.4, flèches de type $i$ de $L$ dans $M$ au-dessus de $f$ (ou $f$-flèches de type $i$ de $L$ dans $M$). On laisse au lecteur le soin d'expliciter la composition des flèches de type $i$, pour $i$ fixé, et les catégories fibrées et cofibrées correspondantes. Rappelons seulement les formules d'adjonction partielle
\[
\Hom_f^{(1)}(L,M)=\Hom(M,f_*L)\quad
(M\in\ob D^-(Y),\ L\in\ob D^+(X)),
\]
\[
\Hom_f^{(3)}(L,M)=\Hom(f_*L,M)\quad
(f\ \text{propre},\ L\in\ob D(X)_{\coh},\ M\in\ob D^+(Y)).
\]
\end{statement}

\begin{statement}{6.3}
Dans la situation de 1.6, supposons $X_1$ et $X_2$ tor-indépendants sur $S$, ainsi que $Y_1$ et $Y_2$. Alors, pour $L_i\in\ob D(X_i)_{\parf/S}$, la flèche de Künneth 1.6.4 est définie (utiliser le fait que la tor-dimension finie relative se conserve par image directe (SGA 6 III 3.7.1)) et est un isomorphisme (pour le vérifier, on peut supposer $S$ et $Y_i$ affines, et, pour un calcul à la Čech, on se ramène à supposer aussi $X_1$ et $X_2$ affines, la conclusion est alors conséquence triviale des hypothèses de tor-indépendance). D'autre part, pour $M_i\in\ob D(Y_i)_{\parf/S}$, avec $f_1^!M_1$ ou $f_2^!M_2$ dans $D^b$, la flèche de Künneth 1.7.3 est définie,\footnote{Dans le cas propre ou lissifiable, la définition de la flèche est immédiate; on peut ramener le cas général au cas lissifiable par descente cohomologique.} et est un isomorphisme quand $f_1$ et $f_2$ sont de tor-dimension finie, comme on le vérifie facilement à l'aide de l'isomorphisme de Künneth de type 1.6.4 qu'on vient d'indiquer, conjugué à 6.1.2, et 6.1.1 dans le cas où $f_i$ est une immersion fermée. En particulier, si $X_1$ et $X_2$ sont de tor-dimension finie sur $S$ et tor-indépendants, 1.7.6 est un isomorphisme.

On définit comme en 2.1 $\RHom(u,v)$ pour $(u,v)$ de type $(2,1)$ (resp. $(3,4)$, avec $f$ propre afin d'éviter les pro-objets dûs à un $f_!$). Avec des hypothèses de degré convenables (voir par exemple ([9] V 2.3)), les flèches d'évaluation (2.2.2) et de produit tensoriel (2.2.3, 2.2.4) (sous réserve qu'elles soient définies) sont des isomorphismes dès que $E_1$ et $E_2$ sont parfaits (au sens absolu).
...
\[
f_1^!Q_1\otimes_S^{\LL}Q_2\longrightarrow (f_1\times_S Y_2)^!(Q_1\otimes_S^{\LL}Q_2),
\]
\[
f_1^!Q_1\otimes_S^{\LL}F_2\longrightarrow (f_1\times_S Y_2)^!(Q_1\otimes_S^{\LL}F_2)
\]
des isomorphismes. Alors 2.4.1 et le diagramme 2.5 sont définis, et ce dernier est commutatif (toutefois les flèches horizontales ne sont pas nécessairement des isomorphismes). La vérification est la même que pour 2.5.
\end{statement}

\begin{statement}{6.4}
Soient, pour $i=1,2$, $X_i$ un $S$-schéma, $X=X_1\times_S X_2$, $p_i:X\to X_i$, $q_i:X_i\to S$ les projections canoniques. On suppose $X_1$ et $X_2$ tor-indépendants, et $q_1$ de tor-dimension finie. Soient d'autre part $L_i\in\ob D(X_i)_{\parf/S}$. Par l'analogue de 2.2.4, on a une flèche canonique
\begin{equation}
\tag{6.4.1}
DL_1\otimes_S^{\LL}L_2
\longrightarrow
\RHom(L_1\otimes_S^{\LL}\OO_X,q_1^!\OO_S\otimes_S^{\LL}L_2),
\end{equation}
d'où, en composant avec l'isomorphisme de Künneth (6.3)
\[
q_1^!\OO_S\otimes_S^{\LL}L_2\xrightarrow{\sim}p_2^!L_2
\]
une flèche
\begin{equation}
\tag{6.4.2}
...
\Hom(p_1^*L_1,p_2^!L_2)=\Hom_S(L_1,L_2),
\end{equation}
et appellerons les éléments de $\Hom_S(L_1,L_2)$ correspondances cohomologiques de $L_1$ à $L_2$. Si $c:C\to X$ est une correspondance (propre, en pratique), on a par la formule d'induction ([8] III 8.8)
\begin{equation}
\tag{6.4.4}
...
\]
s'appelleront correspondances (cohomologiques) de $L_1$ à $L_2$ à support dans $C$. Nous verrons plus loin des exemples, analogues à ceux de 3.2.
\end{statement}

\begin{statement}{6.5}
Soient, pour $i=1,2$, $f_i:X_i\to Y_i$ un $S$-morphisme propre, $f=f_1\times_S f_2:X\to Y$. On suppose $X_i$ et $Y_i$ de tor-dimension finie sur $S$, et les carrés de 2.4.0 tor-indépendants. Soient $L_i\in\ob D(X_i)_{\parf/S}$. On définit comme en 3.3.1 un isomorphisme
\begin{equation}
\tag{6.5.1}
f_*\RHom_S(L_1,L_2)\xrightarrow{\sim}
\RHom_S(f_{1*}L_1,f_{2*}L_2),
\end{equation}
d'où, par application de $\R\Gamma(Y,-)$, un isomorphisme
\begin{equation}
\tag{6.5.2}
f_*:\Hom_S(L_1,L_2)\xrightarrow{\sim}
...
Étant donné un carré commutatif 3.7.1, avec $c$ propre, on a, pour $K\in\ob D^+(C)$, une flèche canonique
\begin{equation}
\tag{6.5.3}
f_*c_*c^!K\longrightarrow d_*d^!f_*K,
\end{equation}
