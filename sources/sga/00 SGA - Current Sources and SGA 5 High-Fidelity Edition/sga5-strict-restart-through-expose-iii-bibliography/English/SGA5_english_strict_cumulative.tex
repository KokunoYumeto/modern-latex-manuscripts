...
Let, for $i=1,2$, $X_i$ be an $S$-scheme, let $X=X_1\times_S X_2$, let $p_i:X\to X_i$ be the projections, and let $L_i\in\ob\Dctf(X_i)$. Every cohomological correspondence $u$ from $L_1$ to $L_2$ gives, by applying $D_X$ and identifying $D p_2^!$ with $p_2^*D$, and $D p_1^*$ with $p_1^!D$, a correspondence $Du$ from $DL_2$ to $DL_1$, called the dual of $u$. In other words, $D$ defines a homomorphism
\begin{equation}
\tag{5.1.1}
D:\Hom(p_1^*L_1,p_2^!L_2)\longrightarrow \Hom(p_2^*DL_2,p_1^!DL_1).
\end{equation}
One verifies that 5.1.1 comes, by application of $\H^0(X,-)$, from the isomorphism
\begin{equation}
\tag{5.1.2}
...
\end{tikzcd}
\end{equation}
commutative, where the vertical arrow on the left is the composite of the symmetry isomorphism and that defined by the biduality isomorphism $L_2\to DDL_2$. It follows easily from the definitions that the square below is commutative:
\begin{equation}
\tag{5.1.4}
\begin{tikzcd}[column sep=large,row sep=large]
...
\end{tikzcd}
\end{equation}
Let $c:C\to X$ be an $S$-morphism, and put $c_i=p_i c$; one deduces from 5.1.2, by applying $c_*c^!$ (3.2.1), an isomorphism
\begin{equation}
\tag{5.1.5}
D:\Hom(c_1^*L_1,c_2^!L_2)\xrightarrow{\sim}
\Hom(c_2^*DL_2,c_1^!DL_1),
\end{equation}
which one checks is identified with the one given by application of the functor $D_X$ and the identifications of $Dc_2^*$ with $c_2^!D$ and of $Dc_1^!$ with $c_1^*D$. If $c':C'\to X$, $c'':C''\to X$ are $S$-morphisms, it follows from 5.1.4 that, for
\[
u\in\Hom(c_1'{}^*L_1,c_2'{}^!L_2),\qquad
v\in\Hom(c_2''{}^*L_2,c_1''{}^!L_1),
\]
...
One verifies on the other hand that, in the situation of 3.3, the following square is commutative:
\begin{equation}
\tag{5.1.7}
\begin{tikzcd}[column sep=large,row sep=large]
...
\end{tikzcd}
\end{equation}
and hence that, in the situation of 3.7.6, one has
\begin{equation}
\tag{5.1.8}
f_*Du=Df_*u .
\end{equation}
...
Let, for $i=1,2,3$, $X_i$ be an $S$-scheme, $X_{ij}=X_i\times_S X_j$, $X=X_1\times_S X_2\times_S X_3$, and let $p_i:X\to X_i$, $p_{ij}:X\to X_{ij}$ be the projections; let $L_i\in\ob\Dctf(X_i)$. One has $X=X_{12}\times_{X_2}X_{23}$, and
\[
(DL_1\otimes_S^{\LL}L_2)\otimes_{X_2}^{\LL}(DL_2\otimes_S^{\LL}L_3)
=
p_1^*DL_1\otimes^{\LL}p_2^*L_2\otimes^{\LL}p_2^*DL_2\otimes^{\LL}p_3^*L_3 .
\]
The evaluation arrow $L_2\otimes^{\LL}DL_2\to K_{X_2}$ therefore defines a pairing
\begin{equation}
\tag{5.2.1}
(DL_1\otimes_S^{\LL}L_2)\otimes_{X_2}^{\LL}(DL_2\otimes_S^{\LL}L_3)
\longrightarrow p_{13}^!(DL_1\otimes_S^{\LL}L_3),
\end{equation}
the second member being identified with $(DL_1\otimes_S^{\LL}L_3)\otimes_S^{\LL}K_{X_2}$ by Künneth (1.7.3). Taking 3.1.1 and 3.1.2 into account, one may rewrite 5.2.1 in the form
\begin{equation}
\tag{5.2.2}
\RHom_S(L_1,L_2)\otimes_{X_2}^{\LL}\RHom_S(L_2,L_3)
\longrightarrow p_{13}^!\RHom_S(L_1,L_3)=\RHom(p_1^*L_1,p_3^!L_3),
\end{equation}
the identification with the second member being given by the induction isomorphism (SGA 4 XVIII 3.1.12.2), whence a pairing, called composition of correspondences,
\begin{equation}
\tag{5.2.3}
\Hom_S(L_1,L_2)\otimes \Hom_S(L_2,L_3)
\longrightarrow \Hom(p_1^*L_1,p_3^!L_3),
\end{equation}
which we shall denote by $(u,v)\mapsto vu$. One verifies that, if $p_{ij,i}:X_{ij}\to X_i$ and $p_{ij,j}:X_{ij}\to X_j$ denote the projections, 5.2.2 may also be obtained by composing the arrow
\[
\RHom_S(L_1,L_2)\otimes_{X_2}^{\LL}\RHom_S(L_2,L_3)
\longrightarrow
...
\]
the tensor product of the canonical arrows
\[
...
\RHom(p_{23}^*p_{23,2}^*L_2,p_3^!L_3),
\]
with the arrow defined by the base-change arrow
\[
p_{12}^!p_{12,2}^*L_2\longrightarrow p_{23}^*p_{23,2}^*L_2,
\]
and with a usual composition arrow on the $\RHom$'s.

Let $c':C'\to X_{12}$, $c'':C''\to X_{23}$ be correspondences, and let $c:C=C'\times_{X_2}C''\to X$ be the ``composite'' correspondence. From 5.2.2 one deduces (or defines directly, by a composition analogous to that just described) a pairing
\begin{equation}
\tag{5.2.4}
...
\end{equation}
whence a ``composition'' pairing $(u,v)\mapsto vu$:
\begin{equation}
\tag{5.2.5}
\Hom(c_1'{}^*L_1,c_2'{}^!L_2)\otimes
\Hom(c_2''{}^*L_2,c_3''{}^!L_3)
...
One verifies that the composition of correspondences is compatible with direct images, in the following sense. Let, for $1\leq i\leq 3$, $f_i:X_i\to Y_i$ be an $S$-morphism, with $f_2$ proper; let $c'$, $c''$, $c$ be morphisms as above, with $c'$ and $c''$ proper; let $d':D'\to Y_{12}$, $d'':D''\to Y_{23}$ be proper morphisms, $d:D=D'\times_{Y_2}D''\to Y$, and let there be commutative squares
\[
\begin{tikzcd}[column sep=large,row sep=large]
...
\end{tikzcd}
\end{equation}
where $M_1=f_{1!}L_1$, $M_2=f_{2*}L_2$, $M_3=f_{3*}L_3$, and there is an analogous commutative square with $\RHom$'s, which the reader will write down.
...
\end{equation}
defined by $L_2\otimes^{\LL}DL_2\to K_{X_2}$ or, what amounts to the same thing, induced by 5.2.1 on $X_{12}$ via the diagonal
\[
X_{12}\longrightarrow X_1\times_S X_2\times_S X_1
\]
($q_i:X_{12}\to X_i$ denoting the projection), and of the pairing
\begin{equation}
\tag{5.2.8}
q_1^*DL_1\otimes^{\LL}q_2^*K_{X_2}\otimes^{\LL}q_1^*L_1
\longrightarrow K_{X_{12}}
\end{equation}
defined by $DL_1\otimes^{\LL}L_1\to K_{X_1}$. It follows that, for $u\in\Hom_S(L_1,L_2)$, $v\in\Hom_S(L_2,L_1)$, one has, in $\H^0(X_1\times_S X_2,K_{X_1\times_S X_2})$,
\begin{equation}
\tag{5.2.9}
\langle u,v\rangle=\langle vu,1\rangle,
\end{equation}
where $1$ denotes the identity correspondence from $L_1$ to $L_1$ on $X_1\times_S X_1$, supported on the diagonal (3.2 b)), and the cup product in the second member is taken in the sense of 4.2.5, taking into account the fact that the fibre product $X\times_{(X_1\times_S X_1)}X_1$ (by $p_{13}:X\to X_{13}=X_1\times_S X_1$ and the diagonal $X_1\to X_1\times_S X_1$) is identified with $X_1\times_S X_2$. Similarly one sees, more generally, that if $c':C'\to X_{12}$, $c'':C''\to X_{21}$ are correspondences, $c:C=C'\times_{X_2}C''\to X_1\times_S X_2\times_S X_1$ is the composite correspondence, and $d:C'\times_{X_{12}}C''=D\to X_{12}$ is the intersection correspondence, then, for $u\in\Hom(c_1'{}^*L_1,c_2'{}^!L_2)$, $v\in\Hom(c_2''{}^*L_2,c_1''{}^!L_1)$, one has the following equality in $\H^0(D,K_D)$:
\begin{equation}
\tag{5.2.10}
\langle u,v\rangle=\langle vu,1\rangle,
\end{equation}
where $1$ denotes, as above, the diagonal correspondence, and $D$ is identified with the fibre product of $C\to X_1\times_S X_1$ and the diagonal $X_1\to X_1\times_S X_1$. Formulae 5.2.9 and 5.2.10 generalize a well-known formula in the case of constant coefficients ([12] 1.3.6 (ii) c)).

\subsection*{5.3. External tensor product of correspondences}

Let, for $1\leq i\leq 4$, $X_i$ be an $S$-scheme, let $X=X_1\times_S X_2\times_S X_3\times_S X_4$, $X_{ij}=X_i\times_S X_j$, and let $L_i\in\ob\Dctf(X_i)$. The Künneth isomorphism (1.7.6, 2.2.4)
\[
...
\end{equation}
which one may, according to 3.1.1, rewrite in the form
\begin{equation}
\tag{5.3.2}
...
\end{equation}
Hence there is a pairing, called the external tensor product,
\begin{equation}
\tag{5.3.3}
...
\end{equation}
which we shall denote by $(u,v)\mapsto u\otimes_S^{\LL}v$. The pairing 5.3.2 may also be interpreted as the composite of the external tensor product
\[
\RHom(E_1,E_2)\otimes_S^{\LL}\RHom(E_3,E_4)
\xrightarrow{\sim}
\RHom(E_1\otimes_S^{\LL}E_3,E_2\otimes_S^{\LL}E_4)
\]
(where $E_1=p_{12,1}^*L_1$, etc., with evident notation) and of the isomorphism defined by Künneth
\[
E_2\otimes_S^{\LL}E_4\xrightarrow{\sim}p_{24}^!(L_2\otimes_S^{\LL}L_4)
\]
(1.7.3).

Let $c':C'\to X_{12}$, $c'':C''\to X_{34}$ be correspondences, and let $c:C=C'\times_S C''\to X=X_{13}\times_S X_{24}$ be the product correspondence. From 5.3.2 one deduces, or defines in an analogous way, a pairing
\begin{equation}
\tag{5.3.4}
...
\end{equation}
whence an external tensor product pairing, $(u,v)\mapsto u\otimes_S^{\LL}v$,
\begin{equation}
\tag{5.3.5}
\Hom(c_1'{}^*L_1,c_2'{}^!L_2)\otimes
\Hom(c_3''{}^*L_3,c_4''{}^!L_4)
...
One verifies that these pairings are compatible with direct images, in the following sense. Let, for $1\leq i\leq 4$, $f_i:X_i\to Y_i$ be an $S$-morphism, let $c'$, $c''$, $c$ be as above, with $c'$ and $c''$ proper, let $d':D'\to Y_{12}$, $d'':D''\to Y_{34}$ be proper morphisms, $d:D=D'\times_S D''\to Y=Y_{12}\times_S Y_{34}$, and let there be commutative squares
\[
\begin{tikzcd}[column sep=large,row sep=large]
C' \arrow[r] \arrow[d] & X_{12} \arrow[d,"f_{12}"]\\
D' \arrow[r] & Y_{12}
...
\end{tikzcd}
\end{equation}
where $M_i=f_{i!}L_i$ if $i=1,3$ and $f_{i*}L_i$ otherwise, $L_{ij}=L_i\otimes_S^{\LL}L_j$, $M_{ij}=M_i\otimes_S^{\LL}M_j$. One also has an analogous square with $\RHom$'s.

Suppose that $X_3=X_2$, $X_4=X_1$, $L_3=L_2$, $L_4=L_1$. A reasoning analogous to that of the last paragraph of 5.2 shows that, for $u\in\Hom_S(L_1,L_2)$, $v\in\Hom_S(L_2,L_1)$, one has
\begin{equation}
\tag{5.3.7}
\langle u,v\rangle=\langle u\otimes_S^{\LL}v,1\rangle,
\end{equation}
where $1$ denotes the diagonal correspondence from $L_{12}$ to $L_{12}$ on $X_{12}$, and the cup product in the second member is taken in the sense of 4.2.5. More generally, if $c':C'\to X_{12}$, $c'':C''\to X_{21}$ are correspondences, their ``intersection'' $C=C'\times_{X_{12}}C''$ is identified with the fibre product of $C'\times_S C''\to X=X_{12}\times_S X_{21}$ and the diagonal $X_{12}\to X$, and one has, for $u\in\Hom(c_1'{}^*L_1,c_2'{}^!L_2)$, $v\in\Hom(c_2''{}^*L_2,c_1''{}^!L_1)$, the following equality in $\H^0(C,K_C)$:
\begin{equation}
\tag{5.3.8}
\langle u,v\rangle=\langle u\otimes_S^{\LL}v,1\rangle,
\end{equation}
where $1$ denotes, as above, the diagonal correspondence. We thus have two ``diagonal'' procedures (5.2.10, 5.3.8) for the calculation of the ``Verdier local term'' $\langle u,v\rangle$.

\section*{6. Appendix. Lefschetz formula for coherent sheaves}

In this number, $S$ denotes a noetherian scheme. The $S$-schemes considered will be assumed separated and of finite type. Recall that, by Nagata, every $S$-morphism between such schemes is compactifiable, i.e. factors as an open immersion followed by a proper morphism. For every $S$-scheme $X$, we shall denote by $D(X)$ the derived category of the category of $\OO_X$-modules.
...
\]
(and transitivity isomorphisms $(gf)^!\simeq f^!g^!$ for a composite $gf$, with the cocycle condition). For $f$ proper, $f^!$ is right ``adjoint'' to the functor
\[
f_*:D^+(X)_{\coh}\longrightarrow D^+(Y)_{\coh}
\]
(the subscript ``coh'' denoting the full subcategory formed by complexes with coherent cohomology), this adjunction being made precise by an isomorphism
\begin{equation}
\tag{6.1.1}
f_*\RHom(E,f^!F)\xrightarrow{\sim}\RHom(f_*E,F)
\end{equation}
for $E\in\ob D(X)_{\coh}$, $F\in\ob D^+(Y)$, defined in a manner analogous to the isomorphism (SGA 4 XVIII 3.1.9.6). The considerations of ([8] App.5), joined to the duality theorem for projective space, show that, if $f$ is smooth, purely of dimension $d$, one has a canonical isomorphism
\begin{equation}
\tag{6.1.2}
f^!L\xrightarrow{\sim} f^*L\otimes\Omega^d_{X/Y}[d];
\end{equation}
if, moreover, $f$ is proper, the arrow
\begin{equation}
\tag{6.1.3}
...
\begin{statement}{6.2}
Contrary to what happens in the case of discrete coefficients, there is no reasonable subcategory here of $D$ stable under the ``six operations'' $(f^*,f_*,f^!,f_!,\otimes,\RHom)$. The most natural finiteness notion seems to be relative perfection (SGA 6 III 4). If $X$ is an $S$-scheme and $L\in\ob D(X)$, recall that $L$ is said to be perfect rel. to $S$ if $L$ has finite tor-dimension rel. to $S$ and coherent cohomology. We shall denote by $D(X)_{\parf/S}$ the full subcategory of $D(X)$ formed by complexes perfect rel. to $S$. In general it is stable neither under $\otimes^{\LL}$ nor under $\RHom$. However, if $E$ is perfect (in the absolute sense), and $F$ is perfect rel. to $S$, then $\RHom(E,F)$ is perfect rel. to $S$ (SGA 6 III 4.9). On the other hand, let, for $i=1,2$, $X_i$ be an $S$-scheme and $L_i\in\ob D^-(X_i)$. We put, as in 1.5,
\[
L_1\otimes_S^{\LL}L_2=Lp_1^*L_1\otimes^{\LL}Lp_2^*L_2,
\]
where $p_i:X_1\times_S X_2\to X_i$ is the projection. Consideration of this object is reasonable only if $X_1$ and $X_2$ are tor-independent over $S$, i.e. (SGA 6 III 1.5)
\[
\Tor_i^S(\OO_X,\OO_Y)=0\quad\text{for }i\ne0.
\]
Assume this to be the case: then, if $L_1$ and $L_2$ are perfect rel. to $S$, the same is true of $L_1\otimes_S^{\LL}L_2$ (SGA 6 III 4.7).

Let $f:X\to Y$ be an $S$-morphism. If $f$ is proper, one has
\[
f_*D(X)_{\parf/S}\subset D(Y)_{\parf/S}\quad\text{(SGA 6 III 4.8).}
\]
If $f$ has finite tor-dimension, one has
\[
...
K_X=a^!\OO_S,
\end{equation}
and denote by $D_X$ (or simply $D$) the functor $\RHom(-,K_X)$. If $X$ has finite tor-dimension over $S$, $K_X$ is perfect rel. to $S$ and, for every $L\in\ob D(X)_{\parf/S}$, one has $DL\in\ob D(X)_{\parf/S}$, and the canonical arrow $L\to DDL$ is an isomorphism (SGA 6 III 4.9).

Let $f:X\to Y$ be an $S$-morphism, $L\in\ob D(X)$, $M\in\ob D(Y)$. If $f^*M$ is defined (for instance if $M\in\ob D^-(Y)$, or if $f$ has finite tor-dimension), put
\[
\Hom_f^{(1)}(L,M)=\Hom(f^*M,L),\qquad
\Hom_f^{(2)}(L,M)=\Hom(L,f^*M).
\]
If $f^!M$ is defined (for instance if $M\in\ob D^+(Y)$, or if $f$ is smooth, or finite and of finite tor-dimension, or a composite of such morphisms, since in these cases $f^!$ extends, by 6.1.1, 6.1.2, to a functor defined on the entire category $D(Y)$), put
\[
\Hom_f^{(3)}(L,M)=\Hom(L,f^!M),\qquad
\Hom_f^{(4)}(L,M)=\Hom(f^!M,L).
\]
The elements of $\Hom_f^{(i)}(L,M)$ will be called, as in 1.4, arrows of type $i$ from $L$ to $M$ over $f$ (or $f$-arrows of type $i$ from $L$ to $M$). We leave to the reader the task of making explicit the composition of arrows of type $i$, for fixed $i$, and the corresponding fibred and cofibred categories. We recall only the partial adjunction formulae
\[
...
(f\ \text{proper},\ L\in\ob D(X)_{\coh},\ M\in\ob D^+(Y)).
\]
\end{statement}

\begin{statement}{6.3}
In the situation of 1.6, assume that $X_1$ and $X_2$ are tor-independent over $S$, as are $Y_1$ and $Y_2$. Then, for $L_i\in\ob D(X_i)_{\parf/S}$, the Künneth arrow 1.6.4 is defined (use the fact that relative finite tor-dimension is preserved by direct image (SGA 6 III 3.7.1)) and is an isomorphism (to verify this, one may suppose $S$ and the $Y_i$ affine, and, for a Čech calculation, reduce to supposing also $X_1$ and $X_2$ affine; the conclusion is then a trivial consequence of the tor-independence hypotheses). On the other hand, for $M_i\in\ob D(Y_i)_{\parf/S}$, with $f_1^!M_1$ or $f_2^!M_2$ in $D^b$, the Künneth arrow 1.7.3 is defined,\footnote{In the proper or lissifiable case, the definition of the arrow is immediate; the general case may be reduced to the lissifiable case by cohomological descent.} and is an isomorphism when $f_1$ and $f_2$ have finite tor-dimension, as is checked easily with the aid of the Künneth isomorphism of type 1.6.4 just indicated, conjugated with 6.1.2, and 6.1.1 in the case where $f_i$ is a closed immersion. In particular, if $X_1$ and $X_2$ have finite tor-dimension over $S$ and are tor-independent, 1.7.6 is an isomorphism.

As in 2.1, one defines $\RHom(u,v)$ for $(u,v)$ of type $(2,1)$ (respectively $(3,4)$, with $f$ proper in order to avoid the pro-objects due to an $f_!$). With suitable degree hypotheses (see for example ([9] V 2.3)), the evaluation arrows (2.2.2) and tensor product arrows (2.2.3, 2.2.4) (assuming that they are defined) are isomorphisms as soon as $E_1$ and $E_2$ are perfect (in the absolute sense).

In the situation of 2.4, assume that the pairs $(X_1,X_2)$, $(X_1,Y_2)$, $(Y_1,X_2)$, $(Y_1,Y_2)$ are tor-independent over $S$, that $f_1$ is proper, that $E_i$, $F_i$, $P_i$, $Q_i$, $\RHom(E_i,F_i)$, $\RHom(P_i,Q_i)$ are perfect rel. to $S$, and that the Künneth arrows
\[
...
\]
are isomorphisms. Then 2.4.1 and the diagram 2.5 are defined, and the latter is commutative (however, the horizontal arrows are not necessarily isomorphisms). The verification is the same as for 2.5.
\end{statement}

\begin{statement}{6.4}
Let, for $i=1,2$, $X_i$ be an $S$-scheme, $X=X_1\times_S X_2$, $p_i:X\to X_i$, and let $q_i:X_i\to S$ be the canonical projections. Assume that $X_1$ and $X_2$ are tor-independent, and that $q_1$ has finite tor-dimension. Let moreover $L_i\in\ob D(X_i)_{\parf/S}$. By the analogue of 2.2.4, one has a canonical arrow
\begin{equation}
\tag{6.4.1}
...
\end{equation}
whence, by composing with the Künneth isomorphism (6.3)
\[
...
\Hom(p_1^*L_1,p_2^!L_2)=\Hom_S(L_1,L_2),
\end{equation}
and call the elements of $\Hom_S(L_1,L_2)$ cohomological correspondences from $L_1$ to $L_2$. If $c:C\to X$ is a correspondence (proper, in practice), the induction formula ([8] III 8.8) gives
\begin{equation}
\tag{6.4.4}
...
\]
will be called (cohomological) correspondences from $L_1$ to $L_2$ with support in $C$. We shall see examples later, analogous to those of 3.2.
\end{statement}

\begin{statement}{6.5}
Let, for $i=1,2$, $f_i:X_i\to Y_i$ be a proper $S$-morphism, and $f=f_1\times_S f_2:X\to Y$. Assume that $X_i$ and $Y_i$ have finite tor-dimension over $S$, and that the squares of 2.4.0 are tor-independent. Let $L_i\in\ob D(X_i)_{\parf/S}$. As in 3.3.1, one defines an isomorphism
\begin{equation}
\tag{6.5.1}
f_*\RHom_S(L_1,L_2)\xrightarrow{\sim}
\RHom_S(f_{1*}L_1,f_{2*}L_2),
\end{equation}
whence, by applying $\R\Gamma(Y,-)$, an isomorphism
\begin{equation}
\tag{6.5.2}
f_*:\Hom_S(L_1,L_2)\xrightarrow{\sim}
\Hom_S(f_{1*}L_1,f_{2*}L_2),
\end{equation}
which we shall simply denote by $u\mapsto u_*$ when $Y_1=Y_2=S$. There is a commutative diagram analogous to 3.4.1, as follows from the analogue of 2.5 pointed out in 6.3. One deduces an analogue of 6.5, and, in the case where $Y_1=Y_2=S$ is the spectrum of a field, a description, analogous to that of 3.6, of the operations $u_*$ on cohomology.

Given a commutative square 3.7.1, with $c$ proper, one has, for $K\in\ob D^+(C)$, a canonical arrow
\begin{equation}
\tag{6.5.3}
f_*c_*c^!K\longrightarrow d_*d^!f_*K,
\end{equation}
defined in the same manner as 3.7.3 (the isomorphism $g_*c^!\xrightarrow{\sim}d^!f_*$ is also valid in the present context: definition of the arrow by transposition of a base-change arrow, and verification that it is an isomorphism by reduction to the cases where $d$ is smooth and where it is a closed immersion). Taking $K=\RHom_S(L_1,L_2)$, one deduces from 6.5.3 and 6.5.1 canonical arrows
