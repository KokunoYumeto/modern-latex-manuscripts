#!/usr/bin/env python3
"""Build compact same-concept Zenodo successor controls for the SGA6 crops."""
...
PREDECESSOR_RECORD = 21514656
SUCCESSOR_RECORD = 21515326
...
def main() -> int:
    ...
    if len(github_commit) != 40 or any(
        character not in "0123456789abcdef" for character in github_commit
    ):
    ...
    if predecessor["record"] != PREDECESSOR_RECORD:
    ...
    manifest_url = (
        f"https://zenodo.org/api/records/{PREDECESSOR_RECORD}/files/"
    ...
    if len(crop_rows) != 3:
        raise RuntimeError("expected exactly three compact crop archives")
    ...
    readme = """# Current compact SGA release

This same-concept successor preserves the existing reader-first SGA surface and
adds a compact SGA6 high-detail source-audit crop snapshot. All unrelated files
from predecessor record 21514656 remain byte-identical.
...
Two image archives preserve 1,930 targeted formula, glyph, punctuation,
diagram, and prose-detail crops actually used during the active SGA6 source
audit. A third metadata archive preserves parent identity, page/crop
provenance, exact manifests, and 1,698 routine whole-page or page-band
derivatives as `rights_blocked_not_public`; those routine pixels are not
redistributed.

The temporal snapshot closes after the cold-reverification cursor reached
parent PDF index 314 and before index 315 began. It also includes targeted
crops from earlier same-parent audit phases, so it does not claim continuous
index coverage. The parent is the 720-page reader *Theorie des intersections
et theoreme de Riemann-Roch*, 26,833,956 bytes, SHA-256
`73FBBAD41340C12ECCDCFCF6C3A1656953FE3D712AA8E391678458CCD17B4BAA`;
the parent PDF is not duplicated in these archives.
...
"""
    ...
    final_rows.append(
        {
            ...
            "provenance": (
                "current compact same-concept release note; SGA6 crop snapshot "
        ...
        final_rows.append(
            {
                ...
                "provenance": (
                    "SGA6 temporal high-detail source-audit crop snapshot; "
                    f"GitHub commit {github_commit}; parent scan SHA-256 "
                    "73FBBAD41340C12ECCDCFCF6C3A1656953FE3D712AA8E391678458CCD17B4BAA"
                ),
                "status": (
                    "targeted_visual_evidence_no_license_grant"
                    if "RightsBlocked_Metadata" not in row["filename"]
    ...
    validation = {
        ...
        "predecessor_doi": f"10.5281/zenodo.{PREDECESSOR_RECORD}",
        ...
        "github": {
            ...
            "package": (
                ...
                "snapshot-coldreverify-idx314-20260723"
            ),
            "metadata_files": 12,
            "readback_files": 12,
            "readback_errors": [],
        ...
        "sga6_source_audit_crops": {
            ...
            "recovered_named_targeted_images": crop_validation[
                "source_selection"
            ]["recovered_named_targeted"]["files"],
        ...
        "zip_archive_count": 17,
        "zip_member_count": 1183 + new_zip_members,
        "zip_uncompressed_bytes": 156509216 + new_zip_uncompressed,
        "rights": {
    ...
    # Converge the byte-count field without making the validation self-hash.
    for _ in range(10):
        write_text(
            validation_path,
