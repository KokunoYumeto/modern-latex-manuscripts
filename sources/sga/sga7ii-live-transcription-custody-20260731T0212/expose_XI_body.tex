...
L'espace affine \'epoint\'e $\mathbb{V}(\underline{O}(1))^*$ sur $\mathbb{P}(\underline{E})$ est canoniquement isomorphe \`a $\mathbb{V}(\underline{E})^*$, de sorte que, pour tout faisceau quasi-coh\'erent de $\underline{O}$-modules $\underline{F}$ sur $\mathbb{P}(\underline{E})$, on a canoniquement CONT>>
...
\end{equation}
On prouvera (i) et (ii) par récurrence sur $j$, simultanément pour tous les $\underline{F}$ vérifiant (b) et (c). Ces assertions sont vides pour $j < 0$ ; CONT>>


%%%%%%%%%%  scan idx 52  |  folio 45  %%%%%%%%%%

<<CONT supposons donc (i) et (ii) prouvés pour $j' < j$. L'hypothèse de récurrence s'applique aux faisceaux $\stackrel{k}{\wedge} \underline{W} \otimes \underline{F}$ ; elle fournit, par (c) :
...
\right.
\end{equation}
Si $S$ est le spectre d'un corps, les assertions (iii)$_a$ (iii)$_b$ (iii)$_d$ résultent de (1.5.1) et de la dualité de Serre ([2] V 11.2 p.211) La classe de CONT>>


%%%%%%%%%%  scan idx 55  |  folio 48  %%%%%%%%%%

<<CONT cohomologie $\eta^n$ est la classe de cohomologie d'un $0$-cycle de degré $\prod_1^d a_i$ (le degré de l'intersection complète), de sorte que
...
<<CONT est bijective pour $q > r + d$ et surjective pour $q = r + d$. De là, et de la dualité de Poincaré (SGA 4 XVIII) on déduit (SGA 5 VII) que la flèche de restriction
...
(i) \underline{Si $\ell$ est premier à la caractéristique de} $k$, \underline{alors la $\ell$-torsion de} $\mathrm{NS}(Y)$ \underline{s'identifie à la $\ell$-torsion de} $H^2(Y, \mathbb{Z}_\ell(1))$. CONT>>


%%%%%%%%%%  scan idx 57  |  folio 50  %%%%%%%%%%

<<CONT (ii) \underline{Si} $k$ \underline{est de caractéristique non nulle} $p$ \underline{et si} $Y$ \underline{est lisse, alors le noyau de la multiplication par} $p$ \underline{dans} $\mathrm{Pic}(Y)$ \underline{s'identifie au sous-groupe de} $H^0(Y,\Omega^1_Y)$ \underline{formé des différentielles localement logarithmiques.}
...
Supposons par l'absurde que la classe de $\underline{O}(1)$ dans $\mathrm{NS}(Y)$ soit de la forme $nu$, avec $n > 1$. Si $\ell$ divise $n$ et est premier à la caractéristique, CONT>>


%%%%%%%%%%  scan idx 58  |  folio 51  %%%%%%%%%%

<<CONT on a encore, en cohomologie $\ell$-adique
...
La formule suivante est le cas particulier $k = 0$ de la formule (2) p.160 de Hirzebruch [3] CONT>>
...
\begin{equation*}
<<CONT N \;=\; \sum \binom{a-1}{k}\ (y^{k} \,-\, z^{k}) \;=\; (y-z) \sum_{i,j\geq 0}\binom{a-1}{i+j+1}\ y^{i}\ z^{j}
\end{equation*}
\begin{equation*}
\begin{aligned}
...
\end{equation*}
(ii) \quad \underline{Si} $p \leq q$, \underline{pour que} $h_{o}^{pq}\ (\underline{a}) \neq 0$, \underline{il faut et il suffit que} CONT>>


%%%%%%%%%%  scan idx 62  |  folio 55  %%%%%%%%%%

\begin{equation*}
<<CONT p \;>\; \left[\frac{p+q+d \;-\; \sum a_i}{\sup(a_i)}\right] \qquad .
\end{equation*}
...
\right.
\end{equation}
Pour $n = p+q$ fixé, cherchons la plus petite valeur de $p$ possible. Si $n_i = c_i + d_i$, pour un tel $p$, les conditions (2.5.4) se réécrivent, en terme des $c_i$, $n_i$, $e$ et $f$ CONT>>
...
\end{equation}
La formule (2.5.8) se réduit à (2.5.2) si $d = 1$ et est trivialement vraie si (2.5.6) est vérifié. Si $d \geq 2$, si (2.5.6) n'est pas vérifié, et si tous les $a_i$ sont égaux à $2$, le mieux qu'on puisse faire est encore de CONT>>


%%%%%%%%%%  scan idx 65  |  folio 58  %%%%%%%%%%

<<CONT prendre $n_i = 0$ $(i \neq d)$, $e = 0$, $f = d-1$ ou $d-2$, \textbf{congru} à $n$ modulo $2$, et $n_d = n-f$. Si $n \equiv d-1$ $(2)$, on prouve (2.5.7) comme plus haut. Sinon, on note que
...
Le niveau de Hodge $\ell$ est donc \textbf{congru} à $n$ modulo $2$, et est au plus égal à $\inf(n,\dim(X) - n)$. Il est $\geq 0$, sauf si $H^n(X) = 0$, auquel cas il est égal à $- \infty$.

\underline{Corollaire} 2.8. \underline{Reprenons les notations} 2.1, \underline{en faisant} $k = \mathbb{C}$, \underline{et en supposant que} $a_d = \sup (a_i)$. \underline{Soit} $\ell$ \underline{un entier} $0 \leq \ell < n$, \underline{\textbf{congru}} à $n$ \underline{modulo} $2$. \underline{Pour que le niveau de Hodge de} $H^n(V_n(\underline{a}))$ \underline{soit} $\leq \ell$ , \underline{il faut et il suffit que} CONT>>
