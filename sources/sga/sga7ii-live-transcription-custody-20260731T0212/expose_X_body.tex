...
\begin{equation*}
\chi(Rg^{!}\underline{O}_S) = \chi(\underline{O}_D) + \deg_D(Rg^{!}\underline{O}_S) \qquad , CONT>>
\end{equation*}

%%%%%%%%%%  scan idx 14  |  folio 7  %%%%%%%%%%

<<CONT on en conclut par $(1.10.2)$ que
...
\end{equation*}
On se ramène aisément au cas où $S$ est strictement local. Comme $Y$ est géométriquement connexe, $H^0(Y,\underline{O}_Y)$ est une extension radicielle de $k$, et puisque $(d,p)=1$, une au moins des composantes irréductibles de $Y$ est CONT>>


%%%%%%%%%%  scan idx 15  |  folio 8  %%%%%%%%%%

<<CONT géométriquement réduite, et on en déduit facilement que $H^0(Y,\underline{O}_Y) = k$.
...
\begin{equation*}
\omega_{X/S} \qquad \simeq \qquad \underline{O}(\Sigma \, n_i \, Y_i) \qquad , CONT>>
\end{equation*}

%%%%%%%%%%  scan idx 17  |  folio 10  %%%%%%%%%%

<<CONT où les $Y_i$ sont les composantes irréductibles de $Y$, et puisque $\mathrm{div}(\pi) \sim 0$, on peut supposer les $n_i$ positifs. Si $\Sigma \, n_i \, Y_i$ est proportionnel au diviseur $\mathrm{div}(\pi)$, il est multiple entier de $\mathrm{div}(\pi)$ et on a gagné. Sinon, on a
...
\begin{equation*}
(D,E) = (f^* \, D, \ f^* \, E) \qquad\qquad , \qquad CONT>>
\end{equation*}

%%%%%%%%%%  scan idx 19  |  folio 12  %%%%%%%%%%

<<CONT comme on le voit, par exemple, en faisant bouger $D$ et $E$.
...
En particulier, $f$ induit un isomorphisme au-dessus du complémentaire d'une partie finie de $S'$, qui contient les points génériques des composantes irréductibles de $p'^{-1}(s)$. Dès lors, $f^{-1}$ définit une injection CONT>>


%%%%%%%%%%  scan idx 20  |  folio 13  %%%%%%%%%%

<<CONT de l'ensemble $\mathrm{Irr}({p'}^{-1}(s))$ des composantes irréductibles de ${p'}^{-1}(s)$ dans $\mathrm{Irr}({p''}^{-1}(s))$.
...
\begin{equation*}
\mathrm{lg}_{k(s)} (p_*J^n/p_*(J^nq)) = \mathrm{lg}_{k(s)} (\underline{O}_T) \quad , CONT>>
\end{equation*}

%%%%%%%%%%  scan idx 31  |  folio 24  %%%%%%%%%%

<<CONT et 2.14 (i) résulte de (2.11.4) et (2.11.6).
...
Soient $f : S' \longrightarrow S$ un composé de morphismes d'éclatement de points fermés, $D$ un diviseur sur $S$, $D^* = f^* D$ son transformé total sur $S'$, CONT>>


%%%%%%%%%%  scan idx 34  |  folio 27  %%%%%%%%%%

<<CONT $E$ une courbe sur $S$, $E'$ son transformé pur sur $S'$ et supposons que $D \cap E$ soit réduit à $s$. La formule de projection donne encore
...
\end{equation*}
(ii) \underline{La catégorie des} ``faisceaux inversibles'' \underline{sur} $S$ \underline{est la limite inductive} CONT>>


%%%%%%%%%%  scan idx 35  |  folio 28  %%%%%%%%%%

<<CONT (SGA 4 VI 6) \underline{des catégories de faisceaux inversibles sur les} $(S_i)_{i \in I}$ .
...
On donne successivement les équations de la courbe, les équations CONT>>


%%%%%%%%%%  scan idx 38  |  folio 31  %%%%%%%%%%

<<CONT de la transformée pure après éclatement de l'origine, des équations paramétriques de la courbe, l'invariant $\delta$ et le diagramme d'Enriques.
