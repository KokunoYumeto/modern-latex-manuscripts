#!/usr/bin/env python3
"""Independently replay the SGA6 idx349-353 crop release boundary."""
...
def main() -> int:
    ...
    audit_path = (
        package_dir
        / "SGA6_HighDetail_Crops_idx349_353_Audit_Context_20260724.csv"
    )
    ...
    expected_explicit_indices = {
        "zoom352_ou.png": "352",
        "zoom353_deduit.png": "353",
        "zoom353_remarque.png": "353",
    }
    actual_explicit_indices = {
        row["source_basename"]: row["parent_pdf_index_0based"]
        for row in explicit_rows
    }
    if actual_explicit_indices != expected_explicit_indices:
        errors.append(
            "explicit crop locator mismatch: "
            f"actual={actual_explicit_indices} "
            f"expected={expected_explicit_indices}"
        )

    all_rows = explicit_rows + recovered_rows + blocked_rows
    indices = {int(row["parent_pdf_index_0based"]) for row in all_rows}
    if indices != set(range(349, 354)):
        errors.append(f"parent index set is {sorted(indices)}, expected 349-353")
    ...
    metadata_names = [
        "SGA6_HighDetail_SourceAudit_Crops_idx349_353_README_20260724.md",
    ...
    report = {
        "schema": "sga6_idx349_353_crop_release_independent_replay_v1",
