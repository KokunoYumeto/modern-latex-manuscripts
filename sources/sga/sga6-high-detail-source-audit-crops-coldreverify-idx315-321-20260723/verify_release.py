#!/usr/bin/env python3
"""Independently replay the SGA6 idx315-321 crop release boundary."""
...
def main() -> int:
    ...
    if len(explicit_rows) != 4:
        errors.append(f"explicit crop count is {len(explicit_rows)}, expected 4")
    if recovered_rows:
        errors.append(f"recovered crop count is {len(recovered_rows)}, expected 0")
    if len(blocked_rows) != 35:
        errors.append(f"blocked band count is {len(blocked_rows)}, expected 35")
    ...
    if indices != set(range(315, 322)):
        errors.append(f"parent index set is {sorted(indices)}, expected 315-321")
    ...
    metadata_names = [
        "SGA6_HighDetail_SourceAudit_Crops_idx315_321_README_20260723.md",
        "SGA6_HighDetail_SourceAudit_Crops_idx315_321_PARENT_SOURCE_20260723.json",
        "SGA6_Explicit_Targeted_HighDetail_Crops_idx315_321_Manifest_20260723.csv",
        "SGA6_Recovered_Named_HighDetail_Crops_idx315_321_Manifest_20260723.csv",
        "SGA6_Routine_PageBands_idx315_321_RightsBlocked_Manifest_20260723.csv",
        "SGA6_HighDetail_Crops_idx315_321_Audit_Context_20260723.csv",
    ]
    common_metadata = {
        f"metadata/{name}": (
    ...
    for row in upload_rows:
        ...
        expected = (
            explicit_expected
            if row["filename"].startswith("10k_")
    ...
    report = {
        "schema": "sga6_idx315_321_crop_release_independent_replay_v1",
