...
% French control: sga6_fr_workpass.tex (working control checked through idx646).
% Scan authority: declared sga6.pdf; high-resolution supplement used only to
...
\(p_i:X\times_YX\to X\) are the projections; by 2.4 this functor is
...
\textbf{Lemma 4.3.} Under the hypotheses of 4.1, let \(L\) be an invertible
...
\(Y\), and the hypotheses of 4.1 show that they hold for
...
\textbf{Lemma 4.4.} Let \(S\) be a scheme, let
...
\textbf{Lemma 4.5.} Let \(X\to S\) be a proper morphism of finite
...
To represent condition (3) of 4.3, note that, by (1) and (2), the formation
...
\textbf{Lemma 4.6.} Let \(\lambda:X\to S\) be a proper morphism of finite
...
\textbf{Proposition 1.3.} If a sheaf \(F\) on \(X\) is \(m\)-regular,
...
If \(n\geq m\), then \(\beta_n\) is surjective by 1.3(ii), and hence
...
\textbf{Definition 1.5.} Let \(F\) be a coherent sheaf on \(X\), let
...
\noindent\textbf{(1.5.2)} There is an \(F\)-regular sequence of \(r\)
...
\textbf{Proposition 1.6.} Let \(F\) be a \((b)\)-sheaf on \(X\). Then:
\begin{enumerate}[label=(\roman*)]
...
\textbf{Proposition 1.8.} Let \(F\) be a \((b)\)-sheaf on \(X\), let
...
by induction on \(n\geq-1\). Finally, by 1.7 and induction on \(s\),
\(a_s\leq b_s\), and \(G\) is a
\((b_1,\ldots,b_{s-1},a_s)\)-sheaf; this proves (ii).

\textbf{Definition 1.9.} The following polynomials with nonnegative rational
...
\textbf{Remark 1.10.} For the \((b)\)-polynomials, induction on \(r\geq t\)
immediately gives
\[
  P_r(X_0,\ldots,X_t,0,\ldots,0)=P_t(X_0,\ldots,X_t).
\]

\textbf{Theorem 1.11.} \emph{(The main theorem on \((b)\)-sheaves.)} Let
...
\(r\)-th \((b)\)-polynomial (1.9). Then \(m\geq0\), and \(F\) is
...
\[
  0\longrightarrow F(-1)\longrightarrow F\longrightarrow G\longrightarrow0
\]
in which \(G\) is a \((b_1,\ldots,b_r)\)-sheaf. By 1.7 and the induction
hypothesis, if \(n=P_{r-1}(c_1,\ldots,c_r)\), then \(n\geq0\) and \(G\) is
\(n\)-regular. Hence, by 1.4, \(F\) is
...
\]
since \(b_i\geq0\). Finally, by 1.3(i), \(F\) is
\[
...
\textbf{1.12.} Let \(S\) be a noetherian scheme, and let \(X\) be an
