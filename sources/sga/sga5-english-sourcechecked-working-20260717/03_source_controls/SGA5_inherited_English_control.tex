...
\textbf{1.1.} Let $X$ be a prescheme, and let $A_X$ be a sheaf of rings on $X$ (for the étale topology), locally constant, whose geometric fibres are left noetherian rings. We denote by $A^{X}$ the category of left $A_X$-Modules, and by $\D_A(X)$ (or $\D(X)$ if there is no risk of confusion) the corresponding derived category. The notion of a constructible left $A_X$-Module is defined as in (SGAA IX 2.3), by replacing the expression ``locally constant with value of finite presentation'' by ``of finite presentation as an $A_U$-Module''.
...
\end{equation}
Assume that $A_X$ is locally constant, and let $F\in\ob\D_A^+(X)$ be such that every $\H^i(F)$ is a locally constant $A_X$-Module. Then, if $i^!$ has finite cohomological dimension, the arrow (4.2), and hence the arrow (4.3) if $(Y,X)$ satisfies purity, is an isomorphism.
...
\textbf{Definition 4.4.} Given the Ring $A_X$ (locally constant and killed by $n$), we shall say that the pair $(Y,X)$ satisfies cohomological purity in the strong sense if, for every $F\in\ob\D_A^+(X)$ such that the $\H^i(F)$ are locally constant, the canonical arrow (4.3) is an isomorphism.
...
Let $F\in\ob\D_A^+(X)$ be such that the $\H^i(F)$ are locally constant, and let $x$ be a point of $X$ such that $F_{\bar{x}}\neq0$ and $\dim.\mathrm{inj}_{A_{\bar{x}}}(F_{\bar{x}})<+\infty$. Then
\[
...
\[
        \Ext^i(M_{\bar{y}},F_{\bar{y}})=0\quad\text{for }i>\dim.\mathrm{inj}(F_{\bar{x}}),
\]
and hence
\[
...
\end{tikzcd}
\]
Then $\R i_*(G)\simeq k_*\R j_*(G)$. Assume of course that $x\in Z$, since otherwise $\R i_*(G)_{\bar{x}}=0$. Let $\bar{Z}=\Spec(\mathcal{O}_{Z,\bar{x}})$ be the strict localization of $Z$ at $\bar{x}$, let $\bar{f}$ be the image of $f$ in $\mathcal{O}_{Z,\bar{x}}$, let $\bar{Y}=\bar{Z}\times_ZY=\bar{Z}\cap D(f)$, and let $\bar{G}$ be the inverse image of $G$ on $\bar{Y}$. By (SGAA VIII 5.2),
\[
...
b) If $\dim.\mathrm{inj}(F_{\bar{x}})=+\infty$, then $\dim.\mathrm{q.inj}_{\bar{x}}(F)=+\infty$; indeed, for every $p$ there exists a finite type $A_{\bar{x}}$-module $M'$ such that $\Ext^p(M',F_{\bar{x}})\neq0$, hence a constructible $A_X$-Module $M$, locally constant in a neighbourhood of $x$, such that $\underline{\Ext}^p(M,F)_{\bar{x}}\neq0$.

\subsection*{6. Constructibility of $\R\underline{\Hom}$}

The equivalence of conditions a) of (I 3.5.1) remains valid when the Ring $(\Z/n\Z)_X$ is replaced by a commutative Ring, locally constant, whose geometric fibres are noetherian rings: nothing has to be changed in the proof.
...
\[
        \Ext^i(F,K)_{\bar{x}}=0\quad\text{for }i>N(\bar{x}).
\]
...
Thanks to 1.7.3, one can define, for $u_i\in\Hom_{f_i}^{(4)}(L_i,M_i)$,
...
\section{Functoriality of $\RHom$ and external tensor products}
...
\]
is none other than the arrow deduced from the adjunction arrow $f^{*}f_*E\to E$ by applying $f_*\RHom(-,F)$. Similarly, the composite of (4) with the trivial-duality isomorphism
\[
...
\]
by applying $-\overset{\mathbf L}{\otimes}_S f'_{2*}L_2$, and (2) is the composite of the Künneth isomorphism
\[
f_{1*}DL_1\overset{\mathbf L}{\otimes}_S f'_{2*}L_2
\xrightarrow{\sim}
...
& \\
f'_{1*}\RHom(L_1\overset{\mathbf L}{\otimes}_S A_{Y_2},f_1'^!(K_{Y_1}\overset{\mathbf L}{\otimes}_S f'_{2*}L_2)) \arrow[d,"\mathrm{base\ ch.}"']
& \\
...
\]
is the arrow induced by the trace morphism relative to $c_2$. For $N=0$, this formula agrees with (SGA $4^{1/2}$ Cycle 3.6): $\operatorname{cl}(c)_*$ is the homomorphism denoted $\gamma^*_{X,X_1}$ there. If $X_1$ is smooth, purely of relative dimension $r_1$, and $c$ is a closed immersion, it follows from 3.6.1 and from the remark made at the end of 3.2 a) that one also has
\begin{equation}
...
\end{equation}
where $\EP(T_s^F)$ denotes the Euler--Poincaré characteristic of $T_s^F$, for $s$ a geometric point above $S$ (loc. cit.).
\end{statement}
...
\]
(the tensor product of the canonical arrows $p_{12}^*\RHom_S(L_1,L_2)\to\RHom(p_{12}^*L_1,p_{12}^!L_2)$ and $p_{23}^*\RHom_S(L_2,L_3)\to\RHom(p_{23}^!L_2,p_{23}^!L_3)$), the arrow defined by the base-change arrow $p_{12}^!p_{12,2}^*L_2\to p_{23}^*p_{23,2}^*L_2$, and a usual composition arrow on the $\RHom$'s.
...
Assume $g$ finite and flat (hence $d=N$). Let $\omega\in H^0(Y,\Lambda^N N_{Y/X}^{\vee}\otimes\Omega^N_{X/S})$. By 6.8.2 and 6.1, one may view $\omega$ as a homomorphism
\[
...
Let $X_0$ be a separated scheme of finite type over $\mathbb F_q$, let $X$ be the scheme deduced from $X_0$ by extension of scalars to an algebraic closure $k$ of $\mathbb F_q$, and let $F$ be the Frobenius endomorphism of $X/k$. Then, for every $L_0\in\ob D^b_{\ctf}(X_0,\mathbb F_p)$, if $L$ denotes the inverse image of $L_0$ on $X$, one has
\[
...
Let $A\to B$ be a morphism of flat $K$-algebras. It induces a morphism of $K$-algebras $A^e\to B^e$, hence an extension-of-scalars functor
\[
-\lten_{A^e}B^e:D(A^e)\longrightarrow D(B^e).
\]
...
For $E\in\ob D^-(B)$, $M\in\ob D^+(K)$, $F\in\ob D^-(B)$, define an arrow in $D(K)$
...
\]
are isomorphisms, by which (1) identifies with the tensor product over $K$ of 5.10.1 by $\Hom_K(L,M)\otimes_KL$. On the other hand, the upper horizontal arrow, respectively the lower horizontal arrow, of the square identifies under these isomorphisms with the tensor product over $K$ of $\Tr_B:B\to B_\natural$ (resp. $\mathrm{ev}:\Hom_A(B,A)\otimes_AB\to A_\natural$) by the evaluation arrow $\Hom_K(L,M)\otimes_KL\to M$. The commutativity therefore follows from that of 5.10.2, completing the proof of 5.10.9.
...
\]
and let $A$ be a $\Lambda$-algebra. For $L_1\in\Ob D^-(X_{1A})$, $L_2\in\Ob D^-(X_{2A}^o)$, there is a Künneth arrow in $D^-(Y)$
\begin{equation}
...
\]
there is an isomorphism analogous to 6.2.1 in $D^-(Y_{BC})$, for $L_1\in\Ob D^-(X_{B1A})$, $L_2\in\Ob D^-(X_{A2B})$. One checks similarly that, for $M_1\in\Ob D^-(Y_{1A})$, $M_2\in\Ob D^-(Y_{2A}^o)$, there is a Künneth isomorphism in $D^-(X)$
\begin{equation}
...
\tag{6.2.2}
\end{equation}
and an analogous isomorphism in $D^-({}_B X_C)$ for $M_1\in\Ob D^-(Y_{B1A})$, $M_2\in\Ob D^-(Y_{A2C})$.

Finally, let, for $i=1,2$, $A_i$ be a $\Lambda$-algebra, $A_2$ being flat, $E_1\in\Ob D_{\mathrm{ctf}}({}_{A_1}X_1)$, $F_2\in\Ob D^+(X_{A_2})$, $F_1\in\Ob D({}_{A_1}X_{1A_2})$, with $F_1$ of finite tor-dimension and constructible cohomology as an object of $D(X_{1A_2})$. Then
\[
...
\]
is the direct-image morphism defined by $j^c:(X-U)^c\to X^c$. Recall (III 4.3) that, if $X$ is smooth, $c$ a closed immersion, and $X^c$ finite, $\Tr_{\Lambda}(\underline c)$ is the image in $\Lambda$ of the intersection multiplicity $(C,X)$ of $C$ with the diagonal. If, moreover, $X-U$ is smooth, $\Tr_{\Lambda}(\underline c_{X-U})$ is the image in $\Lambda$ of $(c_2^{-1}(X-U),X-U)$. Note also that, if $X-U$ is proper over $S$ and $c|X-U$ is the diagonal, then the Lefschetz formula (III 4.7) implies that
\[
...
\]
and the lower horizontal arrow by $\Tr_{\Lambda[G]}(-)(e)$. By descent, one reduces to $P=\Lambda[G]_Y$, and, by the initial remark, one may then suppose that $G$ acts trivially on $Q$. The assertion is then an immediate consequence of 5.12.5.
...
\]
where $\Hom_{\cC}(X,Y)$ is regarded as a right $A$-module via the action of $A$ on $X$. Interpreting $M\otimes_AX$ and $Y$ as objects of the category $\widehat{\cC}=\Hom(\cC,\Ens)$, the preceding equality takes the more familiar form
\[
...
\begin{prooftext}[Proof]
It is clear that (i) implies (ii). Conversely, let $r$ and $t$ be integers satisfying the condition of Definition 2.2.1 for $X$ and $Z$ respectively. One sees immediately that the integer $s+t$ satisfies the conditions of Definition 2.2.1 for $Y$.
\end{prooftext}
...
\[
T_n=0\quad\text{for } n\ge r-1,
\]
and $T_n=Y_{n-r}$ otherwise, with $w_n=0$ for $n\ge r-1$ and $w_n=u_{n-r}$ otherwise.
...
\end{tikzcd}
\]
in which, as before, $\widetilde X\to X$ is an AR-isomorphism, allows us to reduce to the case where $Y$ is strict and $Z$ is $J$-adic. Then (3.1.3 (ii)) gives the conclusion.
\end{prooftext}
...
$(ii)\Rightarrow(iii)$. Let $r$ be an integer such that, for every $s\ge r$ and every $p$, the canonical morphism
\[
...
\]
Indeed, if $p\ge n+1$, then $\gr^p(X_n)=0$; if $p\le n$, the chain of inequalities $m\ge n+r\ge p+r$ shows that the canonical morphism
\[
...
\tag{1}
\]
Moreover, the functors \(R^if_!\) carry locally \(AR\)-null projective systems to locally \(AR\)-null projective systems; hence, after passage to the quotient, the \(\delta\)-functor (1) gives an exact \(\delta\)-functor
\[
...
\subsubsection*{3.2}
Let \(\ell\) be a prime number \(\ne\car k\), and let \(X\) be a smooth scheme of finite type over \(k\). If \(Z\) is an algebraic cycle of codimension \(d\) on \(X\), then in Exposé IV one associated to it, for every integer \(n\), a cohomology class
\[
...
\[
H^i_c(U,F)=0\qquad\text{for }i\ge n+d;
\]
...
Let $X$ be a smooth connected subscheme of dimension $n$ of $\mathbb P_k^{\,n+r}$, a global complete intersection of $r$ divisors $X_1,\ldots,X_r$ of $\mathbb P_k^{\,n+r}$. For each $i\in\{1,\ldots,r\}$, let $d_i$ be the degree of $X_i$. Then:
\begin{enumerate}[label=(\roman*)]
\item the cohomology groups $H^p(X,\mathbb Z_\ell)$ are free $\mathbb Z_\ell$-modules of finite type;
\item if $p$ is an integer $<2n$ and $\ne n$, then
\[
H^p(X,\mathbb Z_\ell)=
\begin{cases}
...
By analogy with what happens in the transcendental case, where for constant coefficients one has another well-known theorem of Lefschetz ([4]), one is led to conjecture the following statement. If $X$ is a smooth connected projective $k$-scheme of dimension $n$, and if $\xi$ is the class in $H^2(X,\mathbb Q_\ell(1))$ of a very ample invertible $\mathcal O_X$-module, then for every twisted constant $\mathbb Q_\ell$-sheaf $L$ and every integer $p<n$, multiplication by $\xi^{n-p}$:
\[
...
\emph{Proof.} Let $j:X\to\mathbb P^r$ be a closed immersion and let $\xi_X$ (resp. $\xi_Y$) be the class in $H^2(X,\mathbb Q_\ell(1))$ (resp. $H^2(Y,\mathbb Q_\ell(1))$) of $\mathcal O_X(1)$ (resp. $\mathcal O_Y(1)$). Let $\alpha:Y\to X$ denote the canonical injection. For $q\le n-1$, the diagram
\[
\begin{array}{ccc}
...
\item for $0\le p\le2r-2$, multiplication by $\xi=c_1(\mathcal O_P(1))\in H^2(P,\mu)$ is an isomorphism in the corresponding degree;
\item the trace morphism
\[
...
j_*(y'g^*i^*(x))=j_*(y')f^*(x),&\text{projection formula}.
\end{cases}
\]
...
\tag{9.1.5}
\]
We prove (9.1.5). If $\alpha:V(N)\hookrightarrow\widehat N$ denotes the canonical inclusion, then evidently $\alpha^*(\mathcal O_P(1))=\mathcal O_{V(N)}$, whence
\[
...
\end{tikzcd}
\]
where $Z'$ denotes the blowing-up of $Z$ along $Y$, and $\widehat N=\mathbb P_Y(N\oplus\mathcal O_Y)$ is the projective completion of the vector bundle $V(N)$. As in the statement of 9.1, let $s:Y\to\widehat N$ be the morphism obtained by composing the zero-section of $V(N)$ with the canonical inclusion $V(N)\hookrightarrow\widehat N$.
...
We now give, in the form of a lemma, a number of useful relations. Let $E$ be the locally free $\mathcal O_{\widehat N}$-module defined by the canonical exact sequence
\[
...
\tag{9.4.1}
\]
Using the projection formula ([CH] 4 I 9) for $j_1$ and (9.1.6), one is reduced to proving 9.4 when $y$ is of the form $g_1^*(a)$, with $a\in C^*(Y)$. Then $j_1^*(j_1)_*g_1^*(a)$ has in turn a unique decomposition in $C^*(\widehat N)$ of the form
\[
...
\tag{9.4.9}
\]
Finally, applying $(f_2)_*$ to (9.4.9), one obtains $\gamma_*(b_0)=0$, whence $b_0=0$, since $\gamma$ is injective, $Y$ having an evident retraction.

\subsection*{9.5}

We finally show how the preceding results imply the theorem. Let $p:X\times\mathbb P^1\to X$ be the first projection, so that $i=p\circ i_1$. If $y\in C^*(Y)$, then evidently
\[
...
\tag{9.5.2}
\]
To prove the theorem, by linearity we may suppose that $y$ is the irreducible cycle associated with a subvariety $T$ of $Y$, so that $\bar x$ corresponds to the cycle $T\times\mathbb P^1$ of $Z=X\times\mathbb P^1$. Let $\theta$ denote the strict transform of $T\times\mathbb P^1$ by $f_1$. Then
\[
...
\]
\[
\beta^*f_2^*\gamma_*(y)=f_1^*\gamma_*i_*(y)\qquad\text{for every }y\in C^*(Y),
\tag{9.7 ii}
\]
\[
