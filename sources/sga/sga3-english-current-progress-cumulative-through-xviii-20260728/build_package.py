#!/usr/bin/env python3
"""Build and verify the compact SGA3 cumulative-through-XVIII package."""
...
SOURCE_MASTER_NAME = (
    "SGA3_English_Current_Progress_Cumulative_Through_XVIII.tex"
)
EXPECTED_PDF_SHA256 = (
    "4E4EA3DABB65689EF883A93D2B91D92BFD0C9A2DECE70EB5E7C7C7910808E12F"
)
EXPECTED_PAGES = 1008
EXPECTED_DESTINATIONS = 6304
EXPECTED_GOTO_ACTIONS = 3849
...
REPLACED_SOURCE_MEMBERS = {
    ...
    "SGA3_English_Current_Progress_Cumulative_Through_XVI.tex",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--xiv-root", type=Path, required=True)
    parser.add_argument("--xviii-root", type=Path, required=True)
...
def main() -> int:
    ...
    add_tree(
        members,
        args.xiv_root / "tex" / "components",
        "xiv/tex/components",
        "*.tex",
    )
    members["xiv/tex/SGA3_Expose_XIV_body.tex"] = (
        args.xiv_root / "tex" / "SGA3_Expose_XIV_body.tex"
    ).read_bytes()
    xiv_figure = (
        args.xiv_root / "figures" / "appendix-dynkin-diagrams-loop1.png"
    )
    members["xiv/figures/appendix-dynkin-diagrams-loop1.png"] = (
        xiv_figure.read_bytes()
    )
    add_tree(
        members,
        args.xviii_root / "tex" / "components",
        "xviii/tex/components",
        "*.tex",
    )
    members["xviii/tex/SGA3_Expose_XVIII_body.tex"] = (
        args.xviii_root / "tex" / "SGA3_Expose_XVIII_body.tex"
    ).read_bytes()
    xviii_figure = (
        args.xviii_root
        / "figures"
        / "expose_xviii_p04_prop23_compatibility_diagram_loop1_final.png"
    )
    members[
        "xviii/figures/"
        "expose_xviii_p04_prop23_compatibility_diagram_loop1_final.png"
    ] = xviii_figure.read_bytes()
    for name in (
        "README.md",
        "PUBLICATION_READINESS.md",
        "BUILD_SUMMARY_PUBLIC.md",
    ...
    validation = {
        "schema": "sga3_current_progress_source_bundle_through_xviii_v1",
        ...
        "scope": {
            ...
            "absent": "Exposes XV, XVII, and XIX-XXVI",
        ...
        "additive_sources": {
            "expose_xiv_component_tex": len(
                list((args.xiv_root / "tex" / "components").glob("*.tex"))
            ),
            "expose_xiv_raster_witnesses": 1,
            "expose_xviii_component_tex": len(
                list((args.xviii_root / "tex" / "components").glob("*.tex"))
            ),
            "expose_xviii_raster_witnesses": 1,
        },
        "authority_exclusions": {
            "source_pdfs_included": 0,
            "full_source_page_images_included": 0,
