...
\usepackage{lmodern,amsmath,amssymb,amsthm,mathtools,microtype,xcolor,geometry,graphicx,tikz-cd,booktabs,longtable,array,hyperref,fancyhdr}
...
\hypersetup{
  hidelinks,
  hypertexnames=false,
  pdftitle={SGA 2 Expose V - English Source-Aligned Cumulative Assembly Checkpoint},
  pdfauthor={Interlanguage English/Germanic lane},
  pdfsubject={Complete Expose V assembly from fourteen independently sealed bounded units; independently package-audited; owner release decision pending},
  pdfkeywords={SGA 2, Expose V, English translation, source-aligned, cumulative checkpoint}
}
\setlength{\emergencystretch}{2em}
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small SGA 2 -- Expos\'e V}
\fancyhead[R]{\small Package-audited -- owner decision pending}
\fancyfoot[C]{\thepage}
\newcommand{\Homop}{\operatorname{Hom}}
\newcommand{\Extop}{\operatorname{Ext}}
\newcommand{\UH}{\underset{\raisebox{1pt}{$\sim$}}{\mathrm{H}}}
...
\newcommand{\Annop}{\operatorname{Ann}}
...
\newcommand{\srcpage}[1]{\marginpar{\raggedright\scriptsize\color{rulegray}French printed p.~#1}}
\newcommand{\srcpdfpage}[1]{\marginpar{\raggedright\scriptsize\color{rulegray}French physical source p.~#1}}
\newenvironment{sgaremark}[1]{\par\addvspace{0.8\baselineskip}\noindent\textbf{#1.}\ \itshape}{\par}
\newenvironment{sgatheorem}[1]{\par\addvspace{0.8\baselineskip}\noindent\textbf{#1.}\ \itshape}{\par}
\newenvironment{sgalemma}[1]{\par\addvspace{0.8\baselineskip}\noindent\textbf{#1.}\ \itshape}{\par}
\newenvironment{sgaexample}[1]{\par\addvspace{0.8\baselineskip}\noindent\textbf{#1.}\ \itshape}{\par}
\newtheoremstyle{sgaprop}{0.8\baselineskip}{0.6\baselineskip}{\itshape}{}% body
  {\bfseries}{.}{0.5em}{}
\theoremstyle{sgaprop}
\newtheorem*{sgaproposition}{Proposition 3.5}
\newtheoremstyle{sgacor}{0.8\baselineskip}{0.6\baselineskip}{\itshape}{}% body
  {\bfseries}{.}{0.5em}{}
\theoremstyle{sgacor}
\newtheorem*{sgacorollary}{Corollary 3.6}
\renewcommand{\qedsymbol}{\textup{Q.E.D.}}
\begin{document}

\begin{center}
  {\LARGE\bfseries SGA 2: Local Cohomology and Lefschetz Theorems}\par\medskip
  {\Large\bfseries Expos\'e V -- English Source-Aligned Cumulative Assembly Checkpoint}\par\medskip
  {\normalsize Fourteen independently sealed bounded units; French lines 1770--2136}\par
  {\small 19 July 2026}
\end{center}
...
mathematical bodies of fourteen independently source-reviewed and sealed
working units. The corrected French arXiv TeX is the translation authority;
the body occupies physical source-PDF pp.~55--63 / running pp.~47--55 and
was checked in the bounded reviews. Physical pp.~52--54 / running pp.~44--46
are retained only as predecessor context. The current
jcreinhold \texttt{e7a259f} English Markdown is one comparison lineage only.
This package passed independent cumulative audit on 19 July 2026. It remains
a bounded checkpoint, not a critical edition, source certification, or release.}}

\vspace{0.5\baselineskip}
{\scriptsize
\renewcommand{\arraystretch}{1.06}
\setlength{\tabcolsep}{3.5pt}
\begin{longtable}{@{}r >{\raggedright\arraybackslash}p{38mm}
...
\midrule
\endhead
1 & Opening; 1.1--1.3 & 1770-1812 & 60-62 & 55-56 & 47-48 \\
2 & 1.4--1.5 & 1814-1870 & 62-64 & 56-57 & 48-49 \\
3 & Section 2; Theorem 2.1 & 1872-1894 & 64-65 & 58 & 50 \\
4 & Section 3; Theorem 3.1 opening & 1896-1919 & 65-66 & 58-59 & 50-51 \\
5 & Lemma 3.2 & 1921-1951 & 66-67 & 59 & 51 \\
6 & Change-of-rings reduction & 1953-1967 & 67 & 59-60 & 51-52 \\
7 & Equivalence (a)--(c) & 1969-1980 & 67-68 & 60 & 52 \\
8 & Theorem 3.1 proof (i)--(ii) & 1982-1988 & 68 & 60 & 52 \\
9 & Theorem 3.1 proof (iii), opening & 1990-2005 & 68-69 & 60 & 52 \\
10 & Theorem 3.1 proof (iii), completion & 2007-2019 & 69 & 61 & 53 \\
11 & Lemma 3.3; Theorem 3.1 close & 2021-2038 & 69-70 & 61 & 53 \\
12 & Example 3.4 & 2040-2074 & 70 & 61-62 & 53-54 \\
13 & Proposition 3.5 & 2076-2125 & 71 & 62 & 54 \\
14 & Corollary 3.6; Expose V close & 2127-2136 & 71 & 63 & 55 \\
\bottomrule
\end{longtable}}

\noindent{\small\textbf{Continuation cursor.} Corrected French line 2139,
the opening of Expos\'e VI; line 2137 is blank and line 2138 is layout control.
Printed-volume pages, one-based physical source-PDF pages, and recomposed
running pages are distinct coordinates throughout the evidence ledgers.}

\clearpage

% BEGIN COMPONENT SGA2-V-C01 :: unit_V_1_1_1_3_opening
\section*{Expos\'e V. Local Duality and Structure of the $H^i(M)$}

\subsection*{1. Complexes of homomorphisms}

\subsubsection*{1.1}

Let $F^{\bullet}$ and $G^{\bullet}$ be two graded modules. Write\srcpage{61}
\begin{equation}\tag{1}\label{V.1}
  \Homop^{\bullet}(F^{\bullet},G^{\bullet})
\end{equation}
for the graded module of homomorphisms of graded modules from $F^{\bullet}$
to $G^{\bullet}$. Thus
\begin{equation}\tag{2}\label{V.2}
...
\begin{equation*}\tag{1.3}\label{V.1.3}
...
\begin{equation}\tag{5}\label{V.5}
  0\longrightarrow M'\longrightarrow M\longrightarrow M''\longrightarrow 0
\end{equation}
one associates an exact sequence of injective resolutions
\begin{equation}\tag{6}\label{V.6}
  0\longrightarrow R(M')\longrightarrow R(M)
    \longrightarrow R(M'')\longrightarrow 0.
\end{equation}
One checks that the isomorphism \emph{(1.3)} commutes with the homomorphisms
\begin{align}
  \UH^s(R(M'),R(N))
    &\longrightarrow \UH^{s+1}(R(M''),R(N)),\tag{8}\label{V.8}\\
  \Extop^s(R(M'),R(N))
    &\longrightarrow \Extop^{s+1}(R(M''),R(N)),\tag{9}\label{V.9}
\end{align}
deduced from \emph{(6)} and \emph{(5)}.\footnote{\textit{Source note.}
...
$q(g')=g$). To say that \emph{(1.4)} is a homomorphism of $\delta$-functors
in $M$ is to say that
\begin{equation}\tag{12}\label{V.12}
  g\circ dh'-dg'\circ h
\end{equation}
...
\end{align*}
with the obvious notation. Thus \emph{(12)} becomes
\[
...
\end{align*}
Consequently \emph{(12)} is finally written as
\[
  d\!\left(g\circ h'+(-1)^{i+1}g'\circ h\right),
\]
which completes the proof.

Thus \emph{(1.3)} and \emph{(1.4)} give a homomorphism of $\delta$-functors
in $M$:
\begin{equation*}\tag{1.5}\label{V.1.5}
...
\]
We saw in \emph{IV, 5.4} that $I=H^r(A)$ is a dualizing module for $A$;
denote the associated dualizing functor by $D$. In \emph{(1.5)}, put
$N=A/\mathfrak m^k$ and $P=A$. We then obtain a homomorphism of
$\delta$-functors in $M$,
\begin{equation}\tag{13}\label{V.13}
...
$\delta$-functors
\begin{equation}\tag{14}\label{V.14}
  \varphi\colon H^i(M)\longrightarrow
    D\!\left(\Extop^{r-i}(M,A)\right).
\end{equation}

\medskip
\noindent\textbf{Theorem 2.1 (Local duality theorem).}
The preceding functorial homomorphism $\varphi$ is an isomorphism.

\medskip
\noindent\textit{Proof.}
If $i>r$,\srcpage{65} the right-hand side of \emph{(14)} is trivially zero,
...
of \emph{(14)} vanish when $M$ is finitely generated and free. This is
...
This follows from \emph{III, 3.4}, since $\prof(A)=r$. \hfill$\square$
...
\begin{sgalemma}{Lemma 3.2}\srcpage{66}
...
\begin{equation}\tag{15}\label{V.15}
...
\]
converges, in view of \textup{(16)} and \textup{(17)}, to
\[
...
$R^qf_*(\widetilde M)=0$ for $q>0$, the spectral sequence~\textup{(15)}
degenerates, and Lemma~3.2 gives an isomorphism of $B$-modules
\begin{equation}\tag{18}\label{V.18}
...
\begin{equation}\tag{19}\label{V.19}
...
By \textup{(2.1)},\srcpage{67} we have
\begin{equation}\tag{22}\label{V.22}
...
Assertion \textup{(ii)} then follows from \textup{(22)} and
\textup{(a)}.
\end{enumerate}
...
\begin{equation}\tag{23}\label{V.23}
  \Extop_A^{r-n}(M,A)_{\mathfrak p}
  =\Extop_{A_{\mathfrak p}}^{r-n}(M_{\mathfrak p},A_{\mathfrak p})\neq 0.
\end{equation}
Taking~\textup{(22)} into account, this\srcpage{69} implies, on the one hand,
\[
  H^n(M)\neq 0,
\]
...
In view of \textup{(3.2)}, we may suppose that $A$ is regular. By
\textup{(2.1)}, we have
\[
  \HH^i(M)=\DD\bigl(\Extop^{r-i}(M,A)\bigr),
\]
where $r=\dim A$. By \textup{(IV 4.7)}, condition \textup{(a)} is
equivalent\footnote{Indeed, one must show that, if $E$ is a finitely generated
...
\begin{equation}
  \Extop^{r-i}(M,A)\text{ has finite length}. \tag{24}
\end{equation}
Condition \textup{(24)} is equivalent to
\begin{equation}
  \text{for every }x\in X-\{\mathfrak m\},\qquad
  \Extop_A^{r-i}(M,A)_x=0. \tag{25}
\end{equation}
On the other hand, $A_x$ is regular of dimension
$r-\dim\overline{\{x\}}$. Hence, by \textup{(2.1)},
\begin{align}
  ...
  \right) \notag\\
  &={\DD}\!\left(\Extop_{A_x}^{r-i}(M_x,A_x)\right). \tag{26}
\end{align}
...
\begin{proof}
\renewcommand{\qedsymbol}{}
This follows from \textup{(3.5)} and \textup{(III 3.1)}.
\end{proof}
