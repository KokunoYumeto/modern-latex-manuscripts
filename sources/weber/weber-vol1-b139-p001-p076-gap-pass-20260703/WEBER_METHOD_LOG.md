...
RE-VERIFY PROGRESS (§141-188 pass):
  ...
  • NEXT (p1-99 content pass IN PROGRESS — ERSTER ABSCHNITT §1-§17 p23-63 + Einleitung p1-20 + title p21-22 + §18-§22 p64-76 DONE):
    — CONTENT TRACK (primary): **p77** = §23 Die Unterdeterminanten im weiteren Sinne (.tex 2674-2710+): §23 heading "§. 23. Die
      Unterdeterminanten im weiteren Sinne" [★VERIFY present + exact title (LONGER, maybe no trailing period)]; "Wir können nun die Betrachtungen
      des vorigen Paragraphen… verallgemeinern. Wie wir vorhin… alle Glieder in der entwickelten Determinante A… die den Faktor a_1^{(1)} enthalten,
      so wollen wir jetzt alle die Glieder aufsuchen, die den Faktor" a_1^{(1)}a_2^{(2)}…a_ν^{(ν)} "enthalten, worin ν eine beliebige Zahl unter n
      sein kann. Diese Glieder erhalten wir aus dem Hauptgliede" a_1^{(1)}…a_ν^{(ν)}a_{ν+1}^{(ν+1)}…a_n^{(n)}… Continue gap-pass p77->p99.
      ⚠ ★VERIFY §23 heading + exact title; ★"Faktor"(k) vs "Factor"(c) — .tex 2676/2680 use "Faktor", VERIFY vs scan (Weber elsewhere = "Factor");
      ★generic-index letters (Faktor/Hauptglied constructions) per scan; ★dropped intermediate-steps/cross-refs/eq-tails; ★swapped eqs; ★Fraktur.
      ⚠ commutative reorder=skip. ⚠ matrix comma-sep=fmt. ⚠ verbal->\text. ⚠ EPSILON. ⚠ punct/cap/§./ë=skip. ⚠ EMPHASIS gesperrt(defer). ⚠ ordinal=fmt. ⚠ leqno=fmt. ⚠ grep before glyph fix.
    — SPERRUNG/EMPHASIS TRACK (owed): §22 VII/VIII/IX(p74-75)+heading(p72-73); §21 I.-VI.+title(p70-72); §20 terms(p68-69); §19(p65-67);
      §18(p64); §17-§12(p52-63); p48 Satz; p25 title; §4-§23 titles; §5-§8. VERIFIED-good: theilerfremd/relativ prim/Resultante/Discriminante. Einleitung(p1-20)+§173..188.
    — ★ FORMATTING/NOTATION PASS (owed): leqno; ordinal-superscript; double \clearpage (+Zweiter Abschnitt divider); Σ Weber-conv; earlier eq/dots-row
      p47-55; §18-19 inline->display+multi-line (p64-66); p67 footnote-flatten+eq-num(3); p68-75 matrix comma-sep/dots-row/leqno/multi-line-eq;
      **p76 eq(12) matrix comma-sep + \gathered RHS + leqno**. After content pass.
    — SCOPE: front matter NOT in .tex (body-only). German-TEXT fidelity only (layout+emphasis+math-notation+table-layout = separate passes).
    TALLY: **8 type-B Weber errata + 2 variant/typo [sic]** + 1 font-slip (𝔅); **7 GPT-norms removed + 2 prose-ellipsis (p67)**; **4 fabrications
      removed (…eq(4) matrix p69 #58)**; **12 dropped restored (…ΣM #57, cross-refs #59/#61/#63/#67, =𝔄 #62, eq5-interm #68, Vandermonde-step #74)**;
      **10 index alterations (#60,#64,#65,#66,#69,#70[eq6/7 swap],#71,#72,#73,#75[i,k relabel])**; sign fixes; Fraktur-M ×15. ~76 LANDED FIXES.
      Pages DONE: Einleitung p1-20 + title p21-22 + §1-§17 p23-63 + §18-§22 p64-76 (p48-66=19-clean; p67=2;p68=1;p69=3;p70=2;p71=1;p72=2;p73=3;p74=5;p75=2;p76=1).
      Gate: 418pp / 0 overfull / 0 underfull (PDF ~2265146 B). §22 COMPLETE.
════════════════════════════════════════════════════════════════════════════════
