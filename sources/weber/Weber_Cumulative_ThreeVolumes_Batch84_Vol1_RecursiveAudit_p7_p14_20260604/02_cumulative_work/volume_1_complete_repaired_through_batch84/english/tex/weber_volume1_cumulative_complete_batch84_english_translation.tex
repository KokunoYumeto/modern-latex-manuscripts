...
\lhead{Weber Vol. I complete, Batch84 audit}
...
Continuity as well as density are properties which, by the nature of the matter, are inaccessible to our sense perception; consequently they also cannot be demonstrated with rigor in things of the external world, in spatial magnitudes, intervals of time, or masses, however much they may seem to lie in the essence of our intuition. But pure conceptual systems can very well be constructed to which density without continuity, or also density and continuity, belong.\footnote{This was proved by Dedekind, to whom we owe in general the definition of continuity given above. Compare Dedekind's writings, \emph{Stetigkeit und irrationale Zahlen}, Braunschweig 1872, 1892, and \emph{Was sind und was sollen die Zahlen?}, Braunschweig 1888, 1893. Other manifolds to which continuity belongs were formed by Weierstrass and G. Cantor. The definition of continuity, as we here lay it down after Dedekind, is exhaustive in the following respect: a set continuous in this sense, if it also possesses the property of measurability presently to be discussed, cannot be a part of a richer continuous set. I do not know whether this property has already been proved anywhere, and hope to return to it on another occasion. I remark, however, that such a property can be proved only for measurable sets. A merely ordered set can always, however dense it may be, be regarded as a part of one still denser.}

The rational fractions furnish an example of a dense set. This manifold, which we shall denote by $\mathfrak R$, is not continuous. For take any rational fraction $\mu=m:n$, in which $m$ and $n$ have no common divisor and are not both square numbers. Then $\mu$ is not the square of a rational fraction. For if $\mu=p^2:q^2$, then $mq^2=np^2$ would follow, and from this $m=p^2$, $n=q^2$, contrary to the assumption. Therefore if we form a cut $(A,B)$ in the manifold $\mathfrak R$ by assigning to $A$ every element $a$ of $\mathfrak R$ whose square is smaller than $\mu$, and to $B$ every element $b$ whose square is greater than $\mu$, then neither $A$ contains a greatest element nor $B$ a least element, and the cut $(A,B)$ is not produced by an element of $\mathfrak R$.

For suppose that $a=p:q$ is any element of $A$, so that $p^2:q^2<m:n$, or $np^2<mq^2$. Let $y$ be any natural number and choose another natural number $x$ so that $x>y$ and $x(mq^2-np^2)>ny(2p+1)$. Then
\[
...
\[
 mq^2x^2>n(px+y)^2.
\]
Thus, if we put $a'=(px+y):qx$, then $a'>a$ and $a'^2<\mu$; hence $a'$ too is contained in $A$. In exactly the same way one can show that there is no least element in $B$.

The manifold $\mathfrak R$ can, however, serve as our starting point for constructing a continuous set. The totality of all cuts in $\mathfrak R$ is certainly a manifold, which may be denoted by $\mathfrak S$. Consider two different elements of it, $\alpha=(A,B)$ and $\alpha'=(A',B')$. Then either $A$ is a part of $A'$ or $A'$ is a part of $A$. For if some element $a$ belongs to $A$, then every smaller element of $\mathfrak R$ also belongs to $A$. If $A$ is a part of $A'$, we shall call $\alpha$ smaller than $\alpha'$, and thereby the set $\mathfrak S$ has become ordered.

If we regard the cuts produced by rational fractions as equivalent to those rational fractions themselves and call them, briefly, rational cuts, then the set $\mathfrak S$ contains the set $\mathfrak R$, and $\mathfrak S$ is therefore in any case a dense set. But the set $\mathfrak S$ is also continuous. For let us denote the cuts by the large German letters $\mathfrak A,\mathfrak B,\ldots$, and consider any cut in the manifold of cuts, $(\mathfrak A,\mathfrak B)$. We can determine an element of $\mathfrak S$, say $\alpha=(A,B)$, by putting into $A$ every rational fraction that produces one of the cuts of $\mathfrak A$, and putting into $B$ all the remaining rational fractions, which therefore produce the rational cuts in $\mathfrak B$. This cut $\alpha$ in $\mathfrak R$ produces the cut $(\mathfrak A,\mathfrak B)$ in $\mathfrak S$. This will be proved when it is shown that every element $\alpha'$ of $\mathfrak S$ which is smaller than $\alpha$ belongs to $\mathfrak A$, and every element $\beta'$ of $\mathfrak S$ which is greater than $\alpha$ belongs to $\mathfrak B$.

Let $\alpha'=(A',B')$ and $\alpha'<\alpha$. Then there are rational fractions in $A$ which are not contained in $A'$. There is therefore a rational $\mu$ such that $\alpha'<\mu<\alpha$; this $\mu$ produces a cut contained in $\mathfrak A$, hence itself belongs to $\mathfrak A$. Since $\alpha'<\mu$, $\alpha'$ also belongs to $\mathfrak A$. In exactly the same way one shows that every $\beta'$ greater than $\alpha$ belongs to $\mathfrak B$, and the continuity of $\mathfrak S$ is thereby proved.
...
\section*{Eighteenth Section. Roots of Metacyclic Equations}

\sect{183}{Statement of the Problem. Auxiliary Theorem}

In the preceding section we found general criteria by which one can decide whether a proposed equation of prime degree is metacyclic or not. This does not by any means exhaust the matter. Rather, in the form in which the problem goes back to Abel, the question is to give a procedure by which all metacyclic equations can be found. In this form the problem, of which Abel left only brief indications without proofs, was taken up by Kronecker and completely solved in such a way that first the roots, rather than the equations themselves, are found. Thus for every prime number $n$ Kronecker gave an expression, obtained from a given field $\Omega$ by repeated extraction of roots, with the double property that every root of an irreducible metacyclic equation of degree $n$ over $\Omega$ is contained in this expression, and conversely every such expression satisfies an irreducible equation of degree $n$ over $\Omega$.

The foundation of this investigation is supplied by Lagrange's resolvents. Let $n$ be a prime number, let $x_0,x_1,x_2,\ldots,x_{n-1}$ be a system of independent variables, and choose the notation so that $x_n=x_0$, $x_{n+1}=x_1$, and so forth. Finally let $\varepsilon$ be an $n$-th root of unity. We put
\begin{equation}\tag{1}
  (\varepsilon,x)=x_0+\varepsilon x_1+\varepsilon^2x_2+\cdots+\varepsilon^{n-1}x_{n-1}
  =\sum_{h=0}^{n-1}\varepsilon^h x_h .
\end{equation}
We have already met these expressions in the fourteenth section under the name of Lagrange resolvents, and there we examined their behavior under cyclic permutations.

It remains now to determine the influence on these functions of linear permutations of the kind considered in \S\,180. We first state an auxiliary theorem. The imaginary $n$-th roots of unity $\varepsilon$ are the roots of an equation $X=0$, where
\begin{equation}\tag{2}
  X=\xi^{n-1}+\xi^{n-2}+\cdots+\xi+1
\end{equation}
and $\xi$ denotes a variable. As was seen earlier, $X$ is irreducible in the field $R$ of rational numbers.

Let any field $\Omega$ be given in which $X$ is irreducible, and form the field $\Omega(\varepsilon)$ consisting of all rational functions of $\varepsilon$ with coefficients in $\Omega$. Then every element of this field can be put, in one and only one way, into the form
\begin{equation}\tag{3}
  c_0+c_1\varepsilon+c_2\varepsilon^2+\cdots+c_{n-2}\varepsilon^{n-2},
\end{equation}
where the $c$ are elements of $\Omega$. For brevity we call this the normal form of elements of $\Omega(\varepsilon)$. Thus:

\begin{enumerate}[label=\arabic*.]
\item If $\Omega$ is a field in which the cyclotomic equation $X=0$ is irreducible, then every element of $\Omega(\varepsilon)$ can be brought in only one way into normal form, with coefficients in $\Omega$.
\end{enumerate}

In this theorem we may take for $\Omega$ the field obtained from $R$ by adjoining the $n$ independent variables $x_0,x_1,\ldots,x_{n-1}$. If $X$ were reducible in this field, say $X=X_1X_2$, with $X_1,X_2$ integral functions of $\varepsilon$ and rational functions of the $x$, then the variables $x_0,x_1,\ldots,x_{n-1}$ could be specialized to rational numbers without changing the degrees in $\varepsilon$, contradicting the irreducibility of $X$ in $R$.

This field $\Omega$ has various subfields in which $X$ is still irreducible. Such subfields are obtained by fixing a permutation group $P$ of the $n$ symbols $0,1,2,\ldots,n-1$ and considering the totality of all rational functions of $x_0,x_1,\ldots,x_{n-1}$ over $R$ that admit the permutations of this group. This totality is plainly a field. We call it the field belonging to the group $P$. Every such field may be used in theorem 1 in place of $\Omega$.

Consequently:

\begin{enumerate}[resume,label=\arabic*.]
\item If a rational function $\Phi(x_0,x_1,\ldots,x_{n-1},\varepsilon)$ of $\Omega(\varepsilon)$ remains unchanged when the permutations of a group $P$ are applied to the indices of the $x$, then the coefficients $c_0,c_1,\ldots,c_{n-2}$ in the normal form of $\Phi$ also admit the permutations of $P$.
\end{enumerate}

If $\Phi(\varepsilon)$ is any element of $\Omega(\varepsilon)$, its conjugates are obtained by replacing $\varepsilon$ by another root of $X=0$, that is, by carrying out the substitutions $(\varepsilon,\varepsilon^h)$ for $h=1,2,\ldots,n-1$. All these fields are again contained in $\Omega(\varepsilon)$; hence they are identical. If an element $\Phi(\varepsilon)$ remains unchanged when $\varepsilon$ is replaced by all $\varepsilon^h$, then it already belongs to $\Omega$. This is immediate from the normal form. Thus:

\begin{enumerate}[resume,label=\arabic*.]
\item If an element of $\Omega(\varepsilon)$ admits all substitutions $(\varepsilon,\varepsilon^h)$, then it is contained in $\Omega$.
\end{enumerate}

\sect{184}{Theorems on the Resolvents}

We now consider Lagrange's resolvents $(\varepsilon,x)$ under the assumption that $\varepsilon$ is an imaginary $n$-th root of unity and that the $x$ are independent variables. We first examine the influence of linear permutations on $(\varepsilon,x)$. It is enough to determine the influence of the generating substitutions
\begin{equation}\tag{1}
  s=(h,h+1),\qquad t=(h,gh),
\end{equation}
where $h$ runs through the indices of the $x$ modulo $n$, and $g$ denotes a primitive root of the prime number $n$.
...
\]
Thus $s$ changes $(\varepsilon,x)$ into $\varepsilon^{-1}(\varepsilon,x)$.

Under $t$, replacing $x_h$ by $x_{gh}$ in (2) gives
\begin{equation}\tag{3}
  x_0+\sum_{1}^{n-1}\varepsilon^h x_{gh}.
\end{equation}
In this sum $h$ may run through any reduced residue system modulo $n$; replacing $h$ by $g^{-1}h$ gives
\[
  x_0+\sum_{1}^{n-1}\varepsilon^{g^{-1}h}x_h=(\varepsilon^{g^{-1}},x).
\]
Iterating $s$ and $t$ gives the effect of $s^h,t^h$. In particular, for $h=-1$:

\begin{enumerate}[resume,label=\arabic*.]
\item The generating permutations $s,t$ of the metacyclic group effect the substitutions
\[
\begin{array}{ll}
...
From 2 and 4, since $s$ is the generating permutation of the cyclic group, one obtains:
...
\end{equation}
are written in normal form, then their coefficients are cyclic functions of $x_0,x_1,\ldots,x_{n-1}$.
\end{enumerate}
Here $\lambda$ may be any integer not divisible by $n$.

From $F(\varepsilon)$, taking $\lambda=g$ and replacing $\varepsilon$ by its $n-1$ different values, arise the functions
\begin{equation}\tag{5}
  (\varepsilon^{g^h},x)(\varepsilon,x)^{-g^h}=f_h,
\end{equation}
so that, if $(\varepsilon^g,x)(\varepsilon,x)^{-g}=f(\varepsilon)$, then
\begin{equation}\tag{6}
  f_h=f(\varepsilon^{g^h}).
\end{equation}
Under $t^{-1}$ these functions $f_0,f_1,\ldots,f_{n-2}$ undergo the cyclic permutation $(0,1,2,\ldots,n-2)$; as long as the $x$ remain variable, they are pairwise distinct.

Consequently, if one forms a cyclic function
\begin{equation}\tag{7}
  C(f_0,f_1,f_2,\ldots,f_{n-2}),
\end{equation}
of these $n-1$ quantities, it remains invariant under both $s$ and $t^{-1}$, hence under the whole metacyclic group. Therefore:

\begin{enumerate}[resume,label=\arabic*.]
\item If a cyclic function of $f_0,f_1,\ldots,f_{n-2}$ is put into normal form, then its coefficients are metacyclic functions of $x_0,x_1,\ldots,x_{n-1}$.
\end{enumerate}

The functions $f_0,f_1,\ldots,f_{n-2}$ also pass cyclically into one another when the $x$ are left unchanged and $\varepsilon$ is replaced by $\varepsilon^g$. Hence if the cyclic function $C$ has rational coefficients, and contains no $\varepsilon$ in them, it is invariant under all substitutions $(\varepsilon,\varepsilon^h)$. By theorem 3:

\begin{enumerate}[resume,label=\arabic*.]
\item If $C(f_0,f_1,\ldots,f_{n-2})$ is a cyclic function of $f_0,f_1,\ldots,f_{n-2}$ with rational coefficients, then $C$ is a metacyclic function of $x_0,x_1,\ldots,x_{n-1}$ with rational coefficients.
\end{enumerate}

Raising the functions $f_0,f_1,\ldots,f_{n-2}$ successively to the powers $g^{n-2},g^{n-3},\ldots,1$ and multiplying, all resolvents in the product of the left sides cancel except $(\varepsilon,x)$, and one obtains
\begin{equation}\tag{8}
  (\varepsilon,x)^{g^{n-1}-1}=f_0^{g^{n-2}}f_1^{g^{n-3}}\cdots f_{n-2}.
\end{equation}
With an as yet undetermined integer exponent $\lambda$, this yields
\begin{equation}\tag{9}
...
\end{equation}
Let $g_1$ be a fixed primitive root modulo $n$. Write $g=g_1+ln$. Then
\[
...
\]
Thus $l$ may be chosen so that
\[
  {g^{n-1}-1\over n}\equiv 1\pmod n.
\]
Under this assumption put
...
\end{equation}
where $\varepsilon^\lambda=\varepsilon$ and $1-\lambda=nk$. Hence
\begin{equation}\tag{12}
  (\varepsilon,x)^n=[F(\varepsilon)]^n f_0^{g^{n-2}}f_1^{g^{n-3}}\cdots f_{n-2}.
\end{equation}
Putting $f_h=f_k$ whenever $h\equiv k\pmod{n-1}$, and setting
\begin{equation}\tag{13}
  F(\varepsilon^{g^v})=F_v=(\varepsilon^{g^v},x)^{nk},
\end{equation}
substitution of $(\varepsilon,\varepsilon^{g^v})$ in (12) gives
\begin{equation}\tag{14}
  (\varepsilon^{g^v},x)^n=F_v^n f_v^{g^{n-2}}f_{v+1}^{g^{n-3}}\cdots f_{v+n-2}.
\end{equation}
These formulas hold for every $v$.

To reduce the exponents to their least positive residues modulo $n$, put
\begin{equation}\tag{15}
  g^v=nq_v+r_v,\qquad 0<r_v<n,\qquad r_0=1.
\end{equation}
The $r_v$ are, in some order, the numbers $1,2,\ldots,n-1$. Then
...
\end{equation}
The exponents $r_v$ are independent of the auxiliary condition (10).

The characteristic properties of all the functions $f_v,F_v,\Phi_v$, denoted together by $\omega_v$, are these:
\begin{enumerate}[label=\alph*)]
\item The cyclic permutation $s=(h,h+1)$ of the indices of $x$ leaves the functions $\omega_v$ unchanged.
\item The linear permutation $t^{-1}=(h,g^{-1}h)$ of the indices of $x$ induces on the indices of the $\omega$ the cyclic permutation $(0,1,2,\ldots,n-2)$.
\item The substitution $\sigma=(\varepsilon,\varepsilon^g)$ induces the same cyclic permutation on the indices of $\omega$.
\item The cyclic functions of the $\omega_v$ are metacyclic functions of the $x$ and are free of $\varepsilon$.
\end{enumerate}

By Lagrange's theorem, every function $\Theta_v$ with these four properties can be expressed rationally, over the field $\Omega$ of metacyclic functions of the $x$, in terms of one of the functions $\omega_v$, provided $\omega_0,\omega_1,\ldots,\omega_{n-2}$ are distinct. Indeed, if
\begin{equation}\tag{19}
  (u-\omega_0)(u-\omega_1)\cdots(u-\omega_{n-2})=\varphi(u),
\end{equation}
then $\varphi(u)$ is an integral function of $u$ with metacyclic coefficients and is independent of $\varepsilon$. If $\Theta_0,\Theta_1,\ldots,\Theta_{n-2}$ is another such system, then
\begin{equation}\tag{20}
  \sum_{v=0}^{n-2}{\Theta_v\over u-\omega_v}
\end{equation}
is an integral function of degree $n-2$ in $u$ with coefficients in $\Omega$. Setting
\begin{equation}\tag{21}
  \Theta_v=\Theta(u)
\end{equation}
and then $u=\omega_v$ gives $\Theta_v=\Theta(\omega_v)$, with $\Theta$ rational and metacyclic in its coefficients.

\sect{185}{Roots of Metacyclic Equations}

We now substitute, for $x_0,x_1,\ldots,x_{n-1}$, the roots $\xi_0,\xi_1,\ldots,\xi_{n-1}$ of an irreducible metacyclic equation of degree $n$ over a field $\Omega$, choosing their order so that the metacyclic functions of the $\xi$ are rational in $\Omega$. At first we make two restrictive assumptions: no resolvent $(\varepsilon,\xi)$ vanishes, and no two of the functions $f_0,f_1,\ldots,f_{n-2}$ become equal.

Suppose substitution of the $\xi$ for the $x$ carries $f_0,f_1,\ldots,f_{n-2}$ into
\[
  k_0,k_1,\ldots,k_{n-2},
\]
which are distinct, and carries $\Phi_0,\Phi_1,\ldots,\Phi_{n-2}$ into
\[
  K_0,K_1,\ldots,K_{n-2},
\]
where equal values among the $K$ may occur. By property d) of the $f_v$, the quantities $k_0,k_1,\ldots,k_{n-2}$ are the roots of a cyclic equation of degree $n-1$,
\begin{equation}\tag{1}
  \psi(u)=(u-k_0)(u-k_1)\cdots(u-k_{n-2})=0,
\end{equation}
with rational coefficients. They can be expressed rationally in one another:
\begin{equation}\tag{2}
...
\end{equation}
Adding all these formulas and putting the rational quantity $(1,\xi)=A$, one obtains
\begin{equation}\tag{6}
  n\xi_0=A+\sum_{v=0}^{n-2}K_v\tau_v^{r_{n-2}}\tau_{v+1}^{r_{n-3}}\cdots\tau_{v+n-2}^{r_0}.
\end{equation}
This represents one of the roots. If
\[
...
\end{equation}
Thus all the radicals can be expressed rationally through one of them and the $k_v$. Formula (6), however, has the important advantage that, no matter how the $n-1$ radicals are chosen, it gives only $n$ distinct values, namely the $n$ roots $\xi$.

To prove this, let $\epsilon_0,\epsilon_1,\ldots,\epsilon_{n-2}$ be arbitrary $n$-th roots of unity and replace
\[
...
\end{equation}
where $E_v$ is an $n$-th root of unity. By definition,
\begin{equation}\tag{10}
...
\end{equation}
Thus, once the $\epsilon$ are fixed,
\begin{equation}\tag{13}
  E_0=\epsilon_{n-2}\epsilon_{n-3}^{g}\cdots \epsilon_0^{r_{n-2}},
\end{equation}
and all the remaining $E_v$ are determined. Therefore (6) gives only $n$ values. To obtain the roots $\xi_0,\xi_1,\ldots,\xi_{n-1}$ in the order underlying the cyclic and metacyclic groups, multiply in (5) before summing by $\varepsilon^{-hr_v}$ and get
\begin{equation}\tag{14}
  n\xi_h=A+
  \sum_{v=0}^{n-2}\varepsilon^{-hr_v}K_v\tau_v^{r_{n-2}}\tau_{v+1}^{r_{n-3}}\cdots\tau_{v+n-2}^{r_0}.
\end{equation}
Changing the choices of radicals only produces a cyclic permutation of the $\xi_h$.

\sect{186}{Removal of the Restrictive Assumptions}

It would be a substantial restriction on the theory if the two assumptions of the preceding paragraph had to be retained. They are not necessary. Let $\eta_0,\eta_1,\ldots,\eta_{n-1}$ be the roots of any irreducible metacyclic equation of degree $n$. Introduce new unknowns $\xi_0,\xi_1,\ldots,\xi_{n-1}$ by
\begin{equation}\tag{1}
...
\end{equation}
is an integral function of degree $n-1$ with undetermined coefficients from $\Omega$. The coefficients may be chosen so that the $\xi_h$ are distinct. Then the $\xi$ are again the roots of an irreducible metacyclic equation, and (1) is a Tschirnhausen transformation. Since $\xi_0$ is a primitive element of $\Omega(\eta_0)$, the fields $\Omega(\xi_0)$ and $\Omega(\eta_0)$ are identical, and similarly for the conjugate fields. Therefore
\begin{equation}\tag{3}
  \eta_0=\chi(\xi_0)
\end{equation}
for a function $\chi$ with coefficients in $\Omega$.

Regarded as independent variables, the $a$ determine the $\xi$ by a linear substitution with non-zero determinant, namely the product of differences of the $\eta$. Hence the coefficients can be chosen so that none of the resolvents and none of the differences $f_\alpha-f_\beta$ vanish, while the $\xi_h$ remain distinct.

Now introduce, besides the variables $x_0,x_1,\ldots,x_{n-1}$, variables $y$ by
\begin{equation}\tag{4}
  y_h=\psi(x_h),
\end{equation}
and, with $\chi$ as in (3), put
\begin{equation}\tag{5}
  x_h=\chi(y_h).
\end{equation}
If
...
\end{equation}
then $\Theta_v$ is unchanged by the cyclic permutation $s$; under $t^{-1}$ the $\Theta_v$ are cyclically permuted, and the same happens under $(\varepsilon,\varepsilon^g)$. Thus the system $\Theta_0,\Theta_1,\ldots,\Theta_{n-2}$ has properties a)--d) of the previous paragraph. Consequently, after replacing $x$ by $\xi$ and hence $y$ by $\eta$, $\Theta_v$ becomes a rational function of $k_v$:
\begin{equation}\tag{7}
  \Theta_v=Q(k_v).
\end{equation}
From (6) one has
...
\end{equation}
Substitution of the expression for $(\varepsilon^{r_v},\xi)$ from \S\,185 gives for $\eta_0$ an expression of the same form, with $Q_vK_v$ in place of $K_v$. The restrictive assumptions are thereby removed.

The general theorem is therefore:

\begin{enumerate}[label=\Roman*.]
\item Every root $\xi$ of a metacyclic equation of prime degree $n$ can be represented in the form
\begin{equation}\tag{9}
  \xi=A+
  \sum_{v=0}^{n-2}K_v\tau_v^{r_{n-2}}\tau_{v+1}^{r_{n-3}}\cdots\tau_{v+n-2}^{r_0}.
\end{equation}
Here $A$ is rational, $k_0,k_1,\ldots,k_{n-2}$ are non-zero distinct roots of a cyclic equation of degree $n-1$, $K_v$ is a rational function of $k_v$ of the same form for every $v$, and $r_0,r_1,\ldots,r_{n-2}$ are the least positive residues of $1,g,g^2,\ldots,g^{n-2}$, where $g$ is a primitive root modulo $n$. The $n$ values obtained from (9) by giving the radicals $\tau_v=\sqrt[n]{k_v}$ their different values are the $n$ roots of one and the same rational equation.
\end{enumerate}
...
\end{equation}
We examine the changes produced when the radicals are chosen differently and when $k_0,k_1,\ldots,k_{n-2}$ are cyclically permuted.

If one radical, say $\tau_\alpha$, is replaced by another value,
\begin{equation}\tag{13}
  (\tau_\alpha,\varepsilon^\beta\tau_\alpha),
\end{equation}
then in (12) this corresponds to
\begin{equation}\tag{14}
  (\sqrt[n]{R_h},\varepsilon^{\beta r_{h-\alpha-1}}\sqrt[n]{R_h}).
\end{equation}
Since $r_{\alpha+\beta}\equiv r_\alpha r_\beta\pmod n$, the index $h$ in (11) is changed to $h+\beta r_{-\alpha-1}$. Thus (13) induces on the indices of the $\xi$ the cyclic permutation
\[
  (h,h+\beta r_{-\alpha-1}).
\]

Secondly, if we make the cyclic permutation
\begin{equation}\tag{15}
...
\begin{equation}\tag{16}
  ...
  v=0,1,\ldots,n-2,
\end{equation}
and the indices of the $\xi$ undergo
\[
  (h,g^{-1}h).
\]

Now let $S(\xi_0,\xi_1,\ldots,\xi_{n-1})$ be any rational symmetric, or even merely metacyclic, function of the $\xi$. Introducing the values (10), it becomes a rational function of the radicals $\tau_0,\tau_1,\ldots,\tau_{n-2}$. It is unchanged when any one of these radicals is replaced by another of its $n$ values, and hence depends rationally on $k_0,k_1,\ldots,k_{n-2}$. It is also unchanged under their cyclic permutation. Since the $k_v$ are the roots of a cyclic equation over $\Omega$, the function itself lies in $\Omega$.

Applying this to the coefficients of the equation whose roots are the quantities (10), one sees that they are roots of an equation of degree $n$ over $\Omega$. Concerning irreducibility: the radicals $\sqrt[n]{R_v}$ are either all rational or all irrational. If one is rational, the equation for $\xi$ is reducible. If, however, $\sqrt[n]{R_0}$ is irrational, then $x^n-R_0$ is irreducible over $\Omega$, and any equation for $\xi_0$ gives an equation for $\sqrt[n]{R_0}$ that remains valid when this radical is replaced by any of its $n$ values. Then $\xi_0$ is carried into each of $\xi_0,\xi_1,\ldots,\xi_{n-1}$, so the equation of degree $n$ is irreducible.

Thus:

\begin{enumerate}[resume,label=\Roman*.]
\item Every quantity $\xi$ contained in the form (9) is the root of an equation of degree $n$ over $\Omega$; this equation is irreducible except in the special case where one of its roots is rational and the others are contained in $\Omega(\varepsilon)$.
\end{enumerate}

\sect{187}{Reality Conditions}

If the field $\Omega$ is real, then, as seen in \S\,180, there are two kinds of metacyclic equations of prime degree $n$: those with one real and $n-1$ imaginary roots, and those all of whose roots are real. Similarly, cyclic equations of even degree occur in two kinds: with all roots real, and with all roots imaginary.

If the cyclic equation whose roots are $k_0,k_1,\ldots,k_{n-2}$ is of the first kind, so that the $k_v$ are real, and if the radicals $\tau_0,\tau_1,\ldots,\tau_{n-2}$ are also taken real, formula (10) of \S\,186 shows that $\xi_0$ is real while $\xi_h$ and $\xi_{-h}$ are conjugate imaginary.

If, on the other hand, the $k$ are imaginary, then $k_v$ and $k_{v+(n-1)/2}$ are conjugate imaginary. From formula (8) of \S\,185 it follows that also
\[
  \sqrt[n]{R_{v+(n-1)/2}}\quad\text{and}\quad \sqrt[n]{R_v}
\]
are conjugate imaginary. Furthermore
\[
  r_v\equiv -r_{v+(n-1)/2}\pmod n.
\]
Formula (11) of \S\,186 then shows that in this case all roots $\xi_h$ are real. Therefore:

If the cyclic equation whose roots are $k_0,k_1,\ldots,k_{n-2}$ has real roots, then among the roots $\xi$ one is real and the others are imaginary; if that cyclic equation has imaginary roots, then all the roots $\xi$ are real.

\sect{188}{Metacyclic Equations of the Fifth Degree}

The preceding theorems have put the roots of a metacyclic equation of prime degree into a generally valid form. This permits one actually to construct all such quantities, once the roots of cyclic equations are known. Cyclotomy, for example, supplies entire systems of cyclic equations of every degree, whose roots are the periods of cyclotomy. Here, as an application and illustration, we treat the special problem of finding, in an arbitrary field $\Omega$, the roots of all metacyclic equations of the fifth degree.

It is necessary and sufficient to determine generally the roots $k_0,k_1,k_2,k_3$ of a cyclic equation of the fourth degree, under the assumption that these roots are distinct and that none of them is zero. The starting point is the function
\begin{equation}\tag{1}
  w=(k_0-k_2)(k_1-k_3),
\end{equation}
which must be non-zero and which, over a real field $\Omega$, is always real. Under the cyclic permutation $(0,1,2,3)$, $w$ changes sign. Therefore its square is cyclic; put
\begin{equation}\tag{2}
  w=4\sqrt{c}.
\end{equation}
Every function of the $k$ whose sign is changed by $(0,1,2,3)$ is a rational multiple of $\sqrt c$.

Two further cyclic functions are
\[
  (k_0-k_2)^2+(k_1-k_3)^2,
  \qquad
  { (k_0-k_2)^2-(k_1-k_3)^2\over (k_0-k_2)(k_1-k_3)}.
\]
Thus for rational quantities $a,b$,
\begin{equation}\tag{3}
\begin{aligned}
...
\end{aligned}
\end{equation}
Between $a,b,c$ there is the relation
\begin{equation}\tag{4}
  b^2=c(1+a^2).
\end{equation}
Furthermore,
...
\end{equation}
and the two square roots satisfy
\begin{equation}\tag{6}
  \sqrt c=\sqrt{b+a\sqrt c}\,\sqrt{b-a\sqrt c}.
\end{equation}
Let $C$ and $B$ again denote rational quantities. Then
\begin{equation}\tag{7}
  k_0+k_1+k_2+k_3=4C,
  \qquad
...
\end{aligned}
\end{equation}
Conversely these quantities are the roots of a cyclic biquadratic equation. Put
\begin{equation}\tag{10}
  r=\sqrt c,
  \qquad \rho=\sqrt{b+a\sqrt c},
  \qquad \rho'=\sqrt{b-a\sqrt c}.
\end{equation}
Then $r=\rho\rho'$, $\rho^2=b+ar$, and $\rho'^2=b-ar$, and every rational function of the $k$ can be expressed linearly through
\[
  1,\ r,\ \rho,\ \rho',\ r\rho,\ r\rho'.
\]
The cyclic permutation $(k_0,k_1,k_2,k_3)$ corresponds to
\[
  \begin{pmatrix} r&\rho&\rho'\\ -r&\rho'&-\rho\end{pmatrix}.
\]
It follows that any cyclic function of the $k$ has a rational expression.

The biquadratic equation whose roots are the $k$ is most simply obtained with $k-C=x$:
\begin{equation}\tag{11}
  x^4-2(B^2c+b)x^2-4Bacx+B^4c^2-2B^2bc+c=0.
\end{equation}
The representation is not yet final, because $a,b,c$ are connected by (4). First consider the special case $b=0$, whence $a=i$. Then
\[
\begin{aligned}
...
\end{aligned}
\]
and the biquadratic equation is
\[
  x^4-2B^2cx^2-4iBcx+B^4c^2+c=0.
\]
This case occurs only when $i$ belongs to the field $\Omega$.

If $b$ is not zero, introduce a new rational quantity $h$ by
\[
  b=h(1+a^2),\qquad c=h^2(1+a^2).
\]
Then, replacing $Bh$ by $B$, one obtains
\begin{equation}\tag{12}
\begin{aligned}
...
\end{aligned}
\end{equation}
This expression for $k_0$ has the further advantage that, however the signs of the square roots occurring in it are chosen, it represents only four different values.

Abel gives a slightly different expression for $k_0$,
\[
  k_0=C+B\sqrt{1+e}+\sqrt{h(1+e^2+\sqrt{1+e^2})},
\]
which follows from (12) by putting $a=1:e$ and replacing $B,h$ accordingly. To represent a root of a metacyclic equation of degree five, substitute these expressions for $k_0,k_1,k_2,k_3$ in formula (9) of \S\,186. Taking $g=2$, the exponents $r_0,r_1,r_2,r_3$ are $1,2,4,3$. If
\[
...
\end{aligned}
\end{equation}
The coefficients $K_0,K_1,K_2,K_3$ are rational functions of $k_0,k_1,k_2,k_3$ which are themselves cyclically permuted when the $k$ are. Abel takes
\[
  K_0=A_1+A_2k_0+A_3k_2+A_4k_0k_2,
\]
but this is not general enough, because a linear relation among the $k$ follows easily from (12). It is simplest to express the $k$ by the three radicals
\[
  r=\sqrt{1+a^2},\qquad
...
\]
The radical $\rho'$ can be expressed linearly through $r\rho$ and $\rho$, as follows from
\[
...
\[
  \begin{pmatrix} r&\rho&\rho'\\ -r&\rho'&-\rho\end{pmatrix}
\]
instead of the cyclic permutation of the $k$, and denoting rational quantities by $A_1,A_2,A_3,A_4$, one obtains
\begin{equation}\tag{14}
\begin{aligned}
...
\end{aligned}
\end{equation}
The quantities $K_0,K_1,K_2,K_3$ are themselves the roots of a cyclic biquadratic equation. They are only apparently more general than the $k$, since
\[
  K_0=A_1+A_2r+\sqrt{\rho^2(A_3+A_4r)^2}
\]
again has the form of $k_0$ in (9), with altered $a,b,c$.
