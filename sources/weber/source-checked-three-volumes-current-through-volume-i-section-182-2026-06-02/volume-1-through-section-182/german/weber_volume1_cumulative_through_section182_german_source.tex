...
\subsection*{Schluss von \S\,147}

Auf Grund der Eigenschaften, die im vorigen Paragraphen festgestellt worden sind, nennen wir die Gesamtheit der $\mu$ Substitutionen des Normalkörpers $\Omega(\rho)$ eine Gruppe, und zwar eine endliche Gruppe vom Grade $\mu$. Sie heisst auch die Gruppe der Substitutionen des Körpers $\Omega(\rho)$. Mit der allgemeinen Theorie der Gruppen werden wir uns in den späteren Abschnitten eingehender beschäftigen.

\sect{148}{Permutationsgruppen}

Im \S\,145 wurde gezeigt, wie man zu einer Gleichung
\[
        F(x)=0
\tag{1}
\]
vom Grade $m$ in $\Omega$, unter der Voraussetzung, dass sie keine gleichen Wurzeln besitzt, einen Normalkörper $\Omega(\rho)$ bestimmen kann, dem alle ihre Wurzeln angehören. Werden diese Wurzeln bezeichnet durch
\[
        \alpha,\alpha_1,\alpha_2,\ldots,\alpha_{m-1},
\tag{2}
\]
...
\tag{3}
\]
wobei die $\chi$ rationale Functionen in $\Omega$ sind. Setzt man für $\rho$ eine mit $\rho$ conjugirte Grösse $\rho_1$ ein, führt man also eine Substitution $\delta=(\rho,\rho_1)$ aus, so geht jede Wurzel in eine bestimmte andere über, und niemals gehen zwei verschiedene Wurzeln in dieselbe über. Denn aus $F(\alpha)=F(\chi(\rho))=0$ folgt nach \S\,146, dass $\chi(\rho_1)$ wiederum Wurzel der Gleichung ist; die Eindeutigkeit folgt aus der schon bewiesenen Thatsache, dass keine zwei verschiedenen Grössen des Körpers $\Omega(\rho)$ durch dieselbe Substitution in dieselbe Grösse übergehen können.

\begin{enumerate}[label=\arabic*.,leftmargin=2em]
\item Eine Substitution $\delta$ ruft unter den Wurzeln $\alpha$ eine gewisse Permutation hervor, indem jeder Index $0,1,2,\ldots,m-1$ durch einen bestimmten anderen ersetzt wird.
\end{enumerate}
...
\tag{4}
\]
eine andere Anordnung der Ziffern $0,1,2,\ldots,m-1$. Wir schreiben den Uebergang symbolisch als
\[
...
\]
so schreibt man die zweite Permutation mit der oberen Reihe $a_0,a_1,\ldots,a_{m-1}$ und erhält durch successive Ausführung zuerst von $\pi_a$, dann von $\pi_b$, die zusammengesetzte Permutation
\[
...
\tag{8}
\]
Denn wird ein Index $p$ durch $\pi_a$ in $q$, durch $\pi_b$ in $r$ und durch $\pi_c$ in $s$ übergeführt, so führen beide Seiten der Gleichung $p$ in $s$ über.

Zu jeder Permutation $\pi_a$ gehört eine bestimmte inverse Permutation $\pi_a^{-1}$, die die durch $\pi_a$ hervorgerufene Veränderung wieder rückgängig macht. Hat $\pi_a$ die Gestalt (5), so ist
\[
...
\begin{enumerate}[label=\arabic*.,leftmargin=2em,start=3]
\item Ein aus der Gesamtheit aller $n(m)$ Permutationen von $m$ Elementen herausgegriffenes System
\[
        Q=\{\pi_0,\pi_1,\ldots,\pi_{q-1}\}
\]
...
Sind die Wurzeln einer Gleichung $F(x)=0$ durch (3) als Grössen des Normalkörpers $\Omega(\rho)$ dargestellt, so bewirkt jede Substitution $\delta$ des Normalkörpers unter den Wurzeln eine Permutation der Indices. Verschiedene Substitutionen geben verschiedene Permutationen. Denn ist
\[
        \rho=\theta(\alpha,\alpha_1,\ldots,\alpha_{m-1}),
\tag{1}
\]
...
\tag{2}
\]
Wenn zwei Substitutionen dieselbe Permutation der Wurzeln bewirkten, so müssten auch die entsprechenden $\rho_k$ gleich sein.

Wir sagen: der Substitution $\delta_h$ entspricht die Permutation $\pi_h$, wenn $\delta_h$ unter den Wurzeln diese Permutation hervorruft. Der identischen Substitution entspricht die identische Permutation. Wenn ferner den Substitutionen $\delta_h,\delta_k$ die Permutationen $\pi_h,\pi_k$ entsprechen, so entspricht der zusammengesetzten Substitution $\delta_h\delta_k$ die zusammengesetzte Permutation $\pi_h\pi_k$.

\begin{enumerate}[label=\arabic*.,leftmargin=2em]
\item Den $\mu$ Substitutionen des Körpers $\Omega(\rho)$
\[
        \delta,\,\delta_1,\,\delta_2,\ldots,\delta_{
\mu-1}
\tag{3}
\]
entsprechen $\mu$ Permutationen von $m$ Ziffern
\[
        \pi,\,\pi_1,\,\pi_2,\ldots,\pi_{
\mu-1},
\tag{4}
\]
...
Die Permutationen (4) bilden daher eine Gruppe $P$. Wir nennen sie die Galois'sche Gruppe der Gleichung $F(x)=0$ oder auch eines der Körper $\Omega(\alpha),\Omega(\alpha_1),\ldots$. Diese Gruppe ist isomorph der Substitutionsgruppe des Normalkörpers.

Die charakteristischen Eigenschaften der Galois'schen Gruppe ergeben sich durch Umformulierung der Sätze des \S\,146:

\begin{enumerate}[label=\alph*)]
\item Jede rationale Gleichung in $\Omega$, die zwischen den $m$ Wurzeln von $F(x)$ besteht, bleibt richtig, wenn die Wurzeln einer Permutation der Galois'schen Gruppe unterworfen werden.
\item Jede rationale Function in $\Omega$ der $m$ Wurzeln, die alle Permutationen der Galois'schen Gruppe gestattet, ist eine Zahl in $\Omega$.
\item Wenn eine Permutation der Wurzeln auf alle rationalen Gleichungen in $\Omega$, die zwischen den Wurzeln bestehen, anwendbar ist, so gehört diese Permutation der Galois'schen Gruppe an. Daher ist die Galois'sche Gruppe der Inbegriff aller Permutationen, die auf alle rationalen Gleichungen zwischen den Wurzeln anwendbar sind.
\item Ist $P$ eine Permutationsgruppe der Wurzeln, die die Eigenschaften a) und b) besitzt, so ist $P$ die Galois'sche Gruppe der Gleichung.
\end{enumerate}

Zum Beweise von d) sei $P$ eine Gruppe von $\nu$ Permutationen. Wendet man diese Permutationen auf eine primitive Grösse $\rho$ an, so erhält man $\rho_1,\rho_2,\ldots,\rho_\nu$. Das Product
\[
        g'(t)=(t-\rho_1)(t-\rho_2)\cdots(t-\rho_\nu)
\]
ist durch $P$ invariant und liegt nach b) in $\Omega[t]$. Es ist zugleich ein Factor der irreduciblen Galois'schen Resolvente $g(t)$; daher ist es mit $g(t)$ identisch. Folglich ist $\nu$ nicht kleiner als der Grad der Galois'schen Gruppe, und $P$ ist diese Gruppe.

Wählt man $\rho$ als rationale Function der Wurzeln so, dass alle $n(m)$ aus Permutationen hervorgehenden Werthe
\[
        \rho,\rho',\rho'',\ldots
\]
...
\[
        G(t)=(t-\rho)(t-\rho')(t-\rho'')\cdots
\]
eine Function in $\Omega$. Jede ihrer Wurzeln ist primitive Grösse des Normalkörpers $N=\Omega(\alpha,\alpha_1,
\ldots,\alpha_{m-1})$. Die Function $G(t)$ zerfällt in irreducible Factoren vom Grade $\mu$, und daher ist $\mu$ ein Theiler von $n(m)$. Der Grad der Galois'schen Gruppe ist also ein Theiler von $m!$.

Hat der Grad der Galois'schen Resolvente einer Gleichung $m$-ten Grades den grössten Werth $n(m)$, so sagt man mit Kronecker, die Gleichung habe keinen Affect. Je kleiner der Grad $\mu$ der Galois'schen Resolvente, desto grösser ist der Affect. Den Quotienten
\[
        n(m):\mu
\]
...
\begin{enumerate}[label=\arabic*.,leftmargin=2em]
\item Die Gleichung $F(x)=0$ ist reducibel oder irreducibel, je nachdem ihre Galois'sche Gruppe intransitiv oder transitiv ist.
\end{enumerate}
...
\begin{array}{lll}
        ...
        &\vdots&\\
        S&=&\delta,\delta_1,\ldots,\delta_{r-1}
\end{array}
\tag{2}
\]
zerlegen, so dass
...
\tag{3}
\]
die conjugirten Werthe von $\vartheta$ sind. Nach \S\,144 ist
\[
        (t-\vartheta)(t-\vartheta_1)\cdots(t-\vartheta_{s-1})=0
\tag{4}
\]
irreducibel über $\Omega$.
...
\begin{enumerate}[label=\arabic*.,leftmargin=2em]
\item Ist der Körper $\Omega(\alpha)$ imprimitiv, so ist die Gruppe $P$ imprimitiv.
\end{enumerate}

Durch die zu den Reihen gehörigen symmetrischen Producte
\[
        \psi(t,A)=(t-\alpha)(t-\alpha_1)\\cdots(t-\alpha_{r-1}),\quad
        \psi(t,B)=(t-\beta)(t-\beta_1)\cdots(t-\beta_{r-1}),\ldots
\tag{9}
\]
...
\tag{10}
\]
mit Coefficienten in $\Omega$, nachdem man die $y$ geeignet als verschiedene symmetrische Functionen der Reihen gewählt hat. Für jede symmetrische Function $\omega$ der Wurzeln einer Reihe ist
\[
        \sum_i \frac{\omega_i}{u-y_i}
\]
eine rationale Function in $\Omega$, und durch Einsetzen von $u=y$ wird $\omega$ rational durch $y$ ausgedrückt. So ist insbesondere
\[
        \psi(t,A)=\psi(t,y)
\tag{11}
\]
...
Ferner ist $y$ rational durch $\alpha$ darstellbar. Denn
\[
        \psi(\alpha,y)=0,
\tag{12}
\]
während $\psi(\alpha,y_i)\ne0$ für $i>0$ ist. Die Gleichungen
\[
        \psi(\alpha,u)=0,
        \qquad
        \varphi(u)=0
\tag{13}
\]
haben also genau eine gemeinsame Wurzel $u=y$; der grösste gemeinsame Theiler ist linear und gibt $y$ rational aus $\alpha$. Damit folgt die Umkehrung:
\begin{enumerate}[label=\arabic*.,leftmargin=2em,start=3]
\item Ein primitiver Körper hat eine primitive Gruppe.
\end{enumerate}
...
Wird eine Permutation $\pi$ auf die Indices der $u$ angewandt, so kann $\psi$ unverändert bleiben. In diesem Falle sagen wir, $\psi$ gestatte $\pi$. Gestattet sie alle Permutationen, so ist sie symmetrisch.

\begin{enumerate}[label=\arabic*.,leftmargin=2em]
\item Die Permutationen, die eine gegebene Function $\psi$ gestattet, bilden eine Gruppe.
\end{enumerate}

Denn gestattet $\psi$ sowohl $\pi$ als auch $\pi'$, so gestattet sie auch $\pi\pi'$, und mit $\pi$ auch $\pi^{-1}$.

Ist $P$ eine Permutationsgruppe, so heisst eine Function zu $P$ gehörig, wenn sie alle Permutationen von $P$ gestattet, sich aber bei jeder nicht zu $P$ gehörigen Permutation ändert. Zu jeder Gruppe $P$ gibt es solche Functionen. Man nehme eine Function $\rho$ der Variablen, die bei allen $n(m)$ Permutationen verschiedene Werthe annimmt, und betrachte die durch $P$ entstehenden Werthe
\[
        \rho,\rho_1,
\ldots,\rho_{
\mu-1}.
...
Die zu der Gruppe aller Permutationen gehörigen Functionen sind die symmetrischen Functionen; die Gruppe heisst deshalb symmetrische Gruppe. Eine zu der Einheitsgruppe gehörige primitive Function $\rho$ erlaubt zugleich, jede Permutation eindeutig durch das Symbol
\[
        \pi'=(\rho,\rho')
\]
zu bezeichnen.
...
\begin{enumerate}[label=\arabic*.,leftmargin=2em]
\item Jede Permutation lässt sich, und zwar auf unendlich viele Arten, in Transpositionen zerlegen.
\item Enthält eine Permutationsgruppe von $m$ Ziffern alle Transpositionen mit einer festen Ziffer, also etwa $(0,1),(0,2),\ldots,(0,m-1)$, so ist sie die ganze symmetrische Gruppe. Denn $(1,2)=(0,1)(0,2)(0,1)$, und ebenso erhält man alle übrigen Transpositionen.
\end{enumerate}
...
Betrachten wir zugleich die Zerlegung in Cyklen und in Transpositionen. Wird eine Permutation $\pi$ mit einer Transposition $\tau$ componirt, so steigt oder fällt die Anzahl der Cyklen um eins. Liegen die beiden vertauschten Ziffern in demselben Cyklus, so spaltet sich dieser; liegen sie in verschiedenen Cyklen, so vereinigen sich diese. Hat $\pi$ bei $m$ Ziffern $\nu$ Cyklen und ist $\pi$ Product von $\mu$ Transpositionen, so ist
\[
        \mu\equiv m-\nu \pmod 2.
\]
Daher hängt die Parität der Zahl der Transpositionen nur von $\pi$, nicht von der Zerlegung ab.

\begin{enumerate}[label=\arabic*.,leftmargin=2em,start=4]
\item Die Permutationen von $m$ Ziffern zerfallen in zwei Arten: die der ersten Art sind in eine gerade, die der zweiten Art in eine ungerade Anzahl von Transpositionen zerlegbar.
\end{enumerate}

Beide Arten enthalten je $\frac12 n(m)$ Permutationen. Die Composition zweier Permutationen gleicher Art ist von erster Art; die Composition einer von erster mit einer von zweiter Art ist von zweiter Art. Somit:
\begin{enumerate}[label=\arabic*.,leftmargin=2em,start=5]
...
\item Alle Permutationen erster Art lassen sich in dreigliedrige Cyklen zerlegen.
\item Eine Permutationsgruppe, die alle dreigliedrigen Cyklen mit zwei festen Ziffern enthält, enthält die ganze alternirende Gruppe.
\item Ist $m>4$, so ist eine Permutationsgruppe, die alle Producte zweier disjunkter Transpositionen, also Permutationen der Form $(0,1)(2,3)$, enthält, durch die alternirende Gruppe theilbar.
\end{enumerate}
...
\item Enthält eine transitive Permutationsgruppe von $m$ Ziffern eine einzelne Transposition, so ist sie entweder die symmetrische Gruppe oder sie ist imprimitiv.
\item Enthält eine transitive Permutationsgruppe von $m$ Ziffern einen dreigliedrigen Cyklus, so enthält sie die ganze alternirende Gruppe oder sie ist imprimitiv.
\item Tritt der Fall der Imprimitivität ein, so enthält die Gruppe im Falle 9. die ganze symmetrische Gruppe, im Falle 10. die ganze alternirende Gruppe für jedes einzelne System der Imprimitivität.
\end{enumerate}

Schliesslich bezeichnet man wiederholte Composition einer Permutation wie Potenzen. Für eine Permutation $\pi$ gibt es einen kleinsten positiven Exponenten $e$ mit
\[
        \pi^e=1.
\]
Dann sind
...
\ldots,\pi^{e-1}
\]
verschieden und wiederholen sich periodisch. Diese Reihe heisst die Periode von $\pi$, und $e$ der Grad von $\pi$. Die Periode bildet eine Gruppe.

Ist $\pi=(0,1,2,
\ldots,n-1)$, so ist der Grad von $\pi$ gleich $n$. Ist $\pi$ in mehrere Cyklen zerlegt, so ist der Grad von $\pi$ das kleinste gemeinsame Vielfache der Längen der Cyklen. Beispielsweise hat
\[
        \pi=(0,1)(2,3,4)
\]
den Grad $6$, denn
...
\end{aligned}
\]
Wenn $\pi$ in einer Permutationsgruppe $P$ vorkommt, so enthält $P$ die ganze Periode von $\pi$.


\sect{154}{Divisoren der Gruppen. Nebengruppen und conjugirte Gruppen}

Wir haben in \S\,152 gesehen, dass eine Function von $m$ unabhängigen Veränderlichen immer eine Permutationsgruppe von $m$ Ziffern bestimmt, dass aber dies nicht immer zutrifft, wenn an Stelle der unabhängigen Veränderlichen bestimmte Grössen gesetzt werden. Durch die Galois'sche Theorie wird dieser Unterschied auf seinen Kern zurückgeführt. Es sei $F(x)=0$ eine Gleichung $m$-ten Grades in einem Körper $\Omega$, mit den verschiedenen Wurzeln
\[
        \alpha,\alpha_1,\alpha_2,\ldots,\alpha_{m-1}.
\]
Den Normalkörper
...
\alpha_{m-1})
\]
bezeichnen wir mit $N$. Unter der Galois'schen Gruppe $P$ des Körpers $N$, oder der Gleichung $F(x)=0$, verstehen wir entweder die Gruppe der Substitutionen des Körpers $N$ oder die damit isomorphe Permutationsgruppe der Indices der $\alpha$.
...
Ist $q<p$, so wählen wir ein nicht in $Q$ enthaltenes Element $\pi_1$ und bilden
\[
        Q\pi_1=\chi\pi_1,
\chi_1\pi_1,
\ldots,
...
\tag{3}
\]
Die Elemente dieses Systems sind untereinander und von den Elementen von $Q$ verschieden. Enthält $Q\pi_1$ noch nicht alle Elemente von $P$, so wählt man $\pi_2$ ausserhalb von $Q$ und $Q\pi_1$ und fährt fort. So erhält man eine Zerlegung
\[
        P=Q+Q\pi_1+Q\pi_2+\cdots+Q\pi_{j-1}.
\tag{4}
\]
Die Systeme $Q,Q\pi_1,
\ldots,Q\pi_{j-1}$ heissen Nebengruppen von $Q$ in $P$.

\begin{enumerate}[label=\arabic*.,leftmargin=2em]
\item Der Grad $p$ einer Gruppe ist durch den Grad $q$ jedes ihrer Theiler theilbar. Ist $j$ die Zahl der Nebengruppen, so ist
\[
        p=jq.
\tag{5}
\]
Der Quotient $j=p/q$ heisst der Index von $Q$ in $P$.
\item Wird eine beliebige Operation $\pi$ aus $P$ auf das System der Nebengruppen angewandt, so wird dieses System nur in anderer Ordnung wieder erhalten.
\end{enumerate}

Man kann auch Nebengruppen von der Form $\pi Q$ betrachten; für die algebraische Anwendung ist jedoch die Form (4) zweckmässiger.

Eine Grösse $\psi$ des Körpers $N$ heisst zu der Gruppe $Q$ gehörig, wenn sie durch alle Operationen von $Q$ unverändert bleibt und durch jede andere Operation aus $P$ verändert wird. Diese Zugehörigkeit ist hier relativ zu $P$ verstanden; ist $P$ die symmetrische Gruppe, so fällt sie mit der in \S\,152 gebrauchten zusammen.

\begin{enumerate}[label=\arabic*.,leftmargin=2em,start=3]
\item Zu jedem Theiler $Q$ von $P$ gehören Grössen in $N$, und jede Grösse in $N$ gehört zu einem bestimmten Theiler von $P$.
\end{enumerate}

Denn die in \S\,152 construirten Functionen liefern den ersten Theil. Für den zweiten schreiben wir, wenn $\pi$ auf $\psi$ angewandt wird, den Werth als $\psi(\pi)$. Dann ist
\[
        \psi(\pi)(\pi')=\psi(\pi\pi').
\tag{6}
\]
Die Operationen, welche $\psi$ ungeändert lassen, bilden wegen (6) eine Gruppe.

Ist $\psi$ zu $Q$ gehörig und $\pi_i$ nicht in $Q$, so erhält man durch die Operationen der Nebengruppe $Q\pi_i$ stets denselben von $\psi$ verschiedenen Werth. Umgekehrt kommt eine Operation, die denselben Werth hervorbringt, in dieser Nebengruppe vor. Also:
\begin{enumerate}[label=\arabic*.,leftmargin=2em,start=4]
\item Eine zu $Q$ gehörige Function $\psi$ geht durch alle Operationen einer Nebengruppe $Q\pi_i$ und durch keine andere Operation aus $P$ in eine bestimmte von $\psi$ verschiedene Grösse $\psi_i$ über.
\item Die Grössen
\[
        \psi,
\psi_1,
...
Die Grössen (10) heissen die conjugirten Functionen zu $\psi$. Die Gruppe, zu der $\psi_i=\psi(\pi_i)$ gehört, ist
\[
        \pi_i^{-1}Q\pi_i.
\tag{11}
\]
Die Gruppen
\[
        Q,\quad \pi_1^{-1}Q\pi_1,\quad \pi_2^{-1}Q\pi_2,
\ldots,
\pi_{j-1}^{-1}Q\pi_{j-1}
\tag{12}
\]
heissen conjugirte Theiler oder conjugirte Gruppen. Die Ableitung von $\pi^{-1}Q\pi$ aus $Q$ heisst Transformation von $Q$ durch $\pi$.

Zur Bildung der transformirten Permutationen gilt die einfache Regel:
\begin{enumerate}[label=\arabic*.,leftmargin=2em,start=6]
\item Man erhält $\pi^{-1}\chi\pi$ dadurch, dass man in den Cyklen von $\chi$ die Permutation $\pi$ auf die Ziffern ausführt.
\end{enumerate}

Ist zum Beispiel
\[
        \chi=(a,b,c,\ldots)(a',b',c',\ldots)\cdots,
\]
so ist
\[
        \pi^{-1}\chi\pi=(a\pi,b\pi,c\pi,\ldots)(a'\pi,b'\pi,c'\pi,
\ldots)\cdots.
\]

Die Nebengruppen geben noch einen nützlichen Satz:
\begin{enumerate}[label=\arabic*.,leftmargin=2em,start=7]
\item Der Grad einer transitiven Permutationsgruppe von $m$ Ziffern ist stets ein Vielfaches von $m$.
\end{enumerate}
...
Es sei $P$ die Galois'sche Gruppe vom Grade $p$, $Q$ ein Theiler vom Grade $q$ und Index $j$, und $\psi$ eine zu $Q$ gehörige Function. Ihre conjugirten Grössen seien
\[
        \psi,
\psi_1,
\psi_2,
...
\begin{enumerate}[label=\arabic*.,leftmargin=2em]
\item Die Grössen (1) sind die Wurzeln einer irreducibeln Gleichung vom Grade $j$ in $\Omega$.
\end{enumerate}
Denn
\[
...
\tag{2}
\]
ist durch alle Operationen von $P$ invariant und hat daher Coefficienten in $\Omega$. Ist eine Function in $\Omega[t]$ für $t=\psi$ null, so wird sie durch alle Operationen von $P$ auch für alle conjugirten Werthe null; daraus folgt die Irreducibilität.

Hieran schliesst sich der Satz von Lagrange:
\begin{enumerate}[label=\arabic*.,leftmargin=2em,start=2]
\item Jede Grösse des Körpers $N$, die die Permutationen der Gruppe $Q$ gestattet, ist in dem Körper $\Omega(\psi)$ enthalten, wenn $\psi$ zu $Q$ gehört.
\end{enumerate}

Eine solche Grösse $\omega$ hat zu den conjugirten Werthen (1) entsprechende Werthe
\[
        \omega,
\omega_1,
\omega_2,
...
\]
ist unter $P$ invariant, also eine Function in $\Omega(t)$. Setzt man $t=\psi$, so ergibt sich $\omega$ rational durch $\psi$.

\begin{enumerate}[label=\arabic*.,leftmargin=2em,start=3]
\item Adjungirt man eine zu $Q$ gehörige Function $\psi$ zu $\Omega$, so reducirt sich die Galois'sche Gruppe von $P$ auf $Q$.
\end{enumerate}
Denn die Grössen, die in $\Omega'=\Omega(\psi)$ liegen, sind gerade diejenigen, die alle Operationen von $Q$ gestatten; dies sind nach \S\,149 die Kennzeichen der Galois'schen Gruppe in dem neuen Körper.
...
Sind $Q,Q',Q'',\ldots$ Theiler von $P$, so heisst ihr Durchschnitt $R$ der grösste gemeinsame Theiler. Ist $\psi$ zu $Q$, $\psi'$ zu $Q'$ gehörig, so kann man die Coefficienten $x,x'$ so wählen, dass $x\psi+x'\psi'$ zu $R$ gehört. Daher gilt:
\begin{enumerate}[label=\arabic*.,leftmargin=2em,start=4]
\item Werden mehrere zu den Theilern $Q,Q',Q'',\ldots$ gehörige Functionen adjungirt, so reducirt sich die Gruppe auf ihren Durchschnitt.
\end{enumerate}
...
\[
        \pi^{-1}Q\pi=Q
\]
für jede Operation $\pi$ aus $P$ gilt.

Ist $Q$ Normaltheiler, so ist die Adjunction einer einzigen Wurzel der Hülfsgleichung (2) gleichbedeutend mit der Adjunction aller ihrer Wurzeln. Ist $Q$ nicht normal, so ist der Durchschnitt $R$ aller conjugirten Theiler $\pi^{-1}Q\pi$ ein Normaltheiler von $P$.
...
Die Hülfsgleichungen
\[
        \varphi(t)=0
\]
für eine zu $Q$ gehörige Function $\psi$ nennen wir im allgemeinen Sinne Resolventen. Wird $Q$ zur Einheitsgruppe, so ist nach Lagrange jede Grösse des Normalkörpers, also auch jede Wurzel der ursprünglichen Gleichung, rational durch $\psi$ ausdrückbar; dann ist die Resolvente eine Galois'sche Resolvente.

Es sind zwei Fälle zu unterscheiden.

\begin{enumerate}[label=\arabic*.,leftmargin=2em]
\item Haben die conjugirten Gruppen $\pi^{-1}Q\pi$ nur die Einheitsgruppe gemeinsam, so ist die gleichzeitige Adjunction aller Wurzeln der Resolvente gleichbedeutend mit der Adjunction einer zur Einheitsgruppe gehörigen Function. Die Resolvente ist dann eine Totalresolvente.
\item Haben die conjugirten Gruppen einen von der Einheitsgruppe verschiedenen gemeinsamen Theiler $R$ vom Grade $r$, so zerlegt die Adjunction aller conjugirten Functionen die Galois'sche Resolvente nur in Factoren vom Grade $r$. Die Resolvente heisst dann Partialresolvente.
\end{enumerate}
...
\tag{1}
\]
welche durch die Operationen von $P$ hervorgerufen werden. Zwei Operationen $\pi,\pi'$ von $P$ rufen unter (1) dieselbe Permutation hervor, wenn und nur wenn $\pi'\pi^{-1}$ in dem Durchschnitt $R$ der conjugirten Gruppen liegt. Die Permutationen einer Nebengruppe $R\pi$ und nur diese ergeben daher dieselbe Permutation. Somit ist der Grad der Galois'schen Gruppe der Resolvente gleich
\[
        p:r,
\]
dem Index des Normaltheilers $R$. Ist $R=1$, so ist die Resolvente total und ihre Galois'sche Gruppe hat den Grad $p$.

\sect{157}{Adjunction beliebiger Irrationalitäten}

Wir betrachten nun eine beliebige Adjunction. Es sei $\eta$ eine Grösse, die einer irreducibeln Gleichung in $\Omega$ genügt, aber nicht nothwendig eine Function der Wurzeln ist. Adjungiren wir $\eta$ zu $\Omega$, so kann die Galois'sche Gruppe einer Gleichung $F(x)=0$ kleiner werden. Um diesen Vorgang auf die vorige Theorie zurückzuführen, bilden wir den gemeinsamen Normalkörper, der sowohl den Normalkörper $N$ der Gleichung als auch die conjugirten Werthe von $\eta$ enthält. In diesem erweiterten Normalkörper entsprechen der Adjunction von $\eta$ diejenigen Substitutionen, welche $\eta$ fest lassen.

Wird $\eta$ zugleich als Function der Elemente eines erweiterten Körpers aufgefasst, so bestimmt die Adjunction von $\eta$ einen Theiler der dortigen Galois'schen Gruppe. Schneidet man diesen Theiler mit der ursprünglichen Gruppe, so erhält man genau die Gruppe, auf die sich die Galois'sche Gruppe von $F(x)$ in dem Körper $\Omega(\eta)$ reducirt. Die allgemeine Adjunction ist daher, soweit sie auf die gegebene Gleichung wirkt, durch einen Theiler der ursprünglichen Galois'schen Gruppe zu messen.
...
\sect{158}{Reduction imprimitiver Gleichungen}
...
\end{array}
\]
Es sei $\psi$ eine Function, die zu dem Theiler $Q$ gehört, welcher die Reihe $A$ in sich lässt. Dann zerfällt $f(x)$ nach Adjunction einer zu den Reihen gehörigen Grösse $y$ in Factoren
\[
        f(x,y_A),
\quad f(x,y_B),
\quad \ldots,
...
Die Gruppen der einzelnen Factoren lassen sich gleich wählen. Wählt man für jede Reihe eine Permutation $\pi_B,
\ldots,
\pi_S$, welche $A$ in diese Reihe überführt, und bezeichnet die Wurzeln der Reihe entsprechend, so erzeugen die in $Q$ vorkommenden Permutationen in allen Reihen dieselben Permutationen der Indices. Damit folgt:
\begin{enumerate}[label=\arabic*.,leftmargin=2em]
\item Bei einer irreducibeln imprimitiven Gleichung können in den einzelnen Reihen die Wurzeln so bezeichnet werden, dass die Theilgleichungen
\[
        f(x,y_A)=0,
\quad f(x,y_B)=0,
\quad \ldots
...
Nun betrachten wir die Umkehrung. Hat eine transitive Gruppe $P$ einen intransitiven Normaltheiler $Q$, so seien $A,B,\ldots$ die Systeme der Intransitivität von $Q$. Da $Q$ normal ist, werden diese Systeme durch $P$ in ganze solche Systeme übergeführt. Also bilden sie Systeme der Imprimitivität von $P$.

\begin{enumerate}[label=\arabic*.,leftmargin=2em,start=2]
\item Eine transitive Gruppe hat nur dann einen von der Einheitsgruppe verschiedenen intransitiven Normaltheiler, wenn sie imprimitiv ist. Die Systeme der Intransitivität dieses Normaltheilers sind Systeme der Imprimitivität der gegebenen Gruppe.
...
In dem so erweiterten Körper kann jede Wurzel rational durch jede andere ausgedrückt werden. Die cubische Gleichung ist nach Adjunction von $\sqrt D$ ihre eigene Galois'sche Resolvente. Das gilt in jedem speciellen Rationalitätsbereich $\Omega$, in dem $a,b,c$ und $\sqrt D$ enthalten sind, sofern $f(x)$ in $\Omega$ noch irreducibel ist.

Zur eigentlichen Auflösung sucht man eine Function $v$ der Wurzeln, deren Cubus die cyklische Permutation gestattet. Dies erfordert die Adjunction der dritten Einheitswurzeln, oder gleichbedeutend von $\sqrt{-3}$. Sei $\varepsilon$ eine primitive dritte Einheitswurzel und
\[
...
\tag{7}
\]
Dann geht $v$ durch $\pi$ in $\varepsilon^2v$, $v'$ in $\varepsilon v'$ über, während $v^3$, $v'^3$ und $vv'$ invariant bleiben. Setzen wir
\[
        A=\alpha^2\alpha_1+
\alpha_1^2\alpha_2+
\alpha_2^2\alpha,
...
überein. Aus $a=\alpha+
\alpha_1+
\alpha_2$ und (7) erhält man die Wurzeln durch die bekannten Cardanischen Ausdrücke
\[
\begin{aligned}
        3\alpha&=a+v+v',\\
...
Aus vier Ziffern $0,1,2,3$ lassen sich $24$ Permutationen bilden. Die Galois'sche Gruppe der allgemeinen Gleichung vierten Grades über dem Körper der rationalen Functionen der Coefficienten hat daher den Grad $24$. Wir schreiben die Permutationen in Cyklen, wobei $1$ die identische Permutation bedeutet:
\[
\begin{array}{llll}
...
\begin{enumerate}[label=\arabic*.,leftmargin=2em]
\item Enthält eine transitive Permutationsgruppe von vier Ziffern einen dreigliedrigen Cyklus, so ist sie die symmetrische oder die alternirende Gruppe.
\item Enthält sie zwei Transpositionen mit einem gemeinsamen Element, so ist sie die symmetrische Gruppe.
\end{enumerate}
...
\]
Diese Function ist sechswerthig; die entsprechende Gleichung sechsten Grades ist imprimitiv und reducirt sich auf den dritten und zweiten Grad. Ebenso kann eine zu $P_3$ gehörige Function $\xi$ die Gleichung unmittelbar in zwei quadratische Factoren zerlegen. Zur Vereinfachung sei $a_1=0$ und
\[
...
\end{aligned}                                                          \tag{25}
\]
und für $\xi^2$ die cubische Gleichung
\[
...
Der Grad einer transitiven Permutationsgruppe von $m$ Ziffern ist durch $m$ theilbar. Ist $P$ eine solche Gruppe und $Q_0$ der Theiler, welcher die Ziffer $0$ fest lässt, so findet man wegen der Transitivität Permutationen $\pi_1,\ldots,\pi_{m-1}$, welche $0$ in $1,\ldots,m-1$ überführen, und
\[
...
Eine Gleichung $m$-ten Grades $F(x)=0$ mit den Wurzeln $\alpha,\alpha_1,\ldots,\alpha_{m-1}$ heisst eine Abel'sche Gleichung, wenn jede Wurzel rational durch eine von ihnen, $\alpha$, ausdrückbar ist und, falls
\[
...
\]
gilt. Das Zeichen bedeutet, dass die eine Function auf den Werth angewandt wird, den die andere liefert. Die Definition ist auf einen fest gewählten rationalen Körper $\Omega$ bezogen.

Zu diesen Gleichungen gehören die Kreistheilungsgleichungen. Ist $r$ eine primitive $m$-te Einheitswurzel, so sind alle Wurzeln $r^h$; die zugehörigen Operationen $x\mapsto x^h$ vertauschen miteinander. Gehören die $m$-ten Einheitswurzeln bereits zu $\Omega$, so gehört auch die reine Gleichung
\[
        x^m-a=0
\]
...
\[
        \alpha,\alpha_1=\Theta_1(\alpha),\ldots,
...
Da
\[
\sigma'\sigma''=[\alpha,\Theta'\Theta''(\alpha)],\qquad
\sigma''\sigma'=[\alpha,\Theta''\Theta'(\alpha)],
\]
folgt aus (3), dass $\sigma'\sigma''=\sigma''\sigma'$.

Umgekehrt ist eine irreducible Gleichung mit commutativer Gruppe eine Abel'sche Gleichung. Denn ist $Q$ der Theiler, welcher eine Wurzel fest lässt, und $\pi_i$ eine Operation, welche diese Wurzel in die $i$-te Wurzel überführt, so ist wegen der Commutativität $\pi_i^{-1}Q\pi_i=Q$. Da $P$ transitiv ist, lässt $Q$ jede Ziffer fest, also ist $Q$ die Einheitsgruppe; der Grad der Gruppe stimmt mit dem Grad der Gleichung überein und jede Wurzel ist rational durch jede andere ausdrückbar.

\sect{163}{Reduction der Abel'schen Gleichungen auf cyklische Gleichungen}

Die Commutativität der Gruppe liefert eine systematische Reduction. Enthält die Gruppe eine Permutation $\pi$ von der Ordnung $r$, so zerlegt sie die Wurzeln in Cyklen
\[
        \gamma=(\alpha,\alpha_1,\ldots,\alpha_{r-1}),\quad
        \gamma_1=(\beta,\beta_1,
        \ldots,\beta_{r-1}),\quad \ldots .                              \tag{1}
\]
Da die übrigen Operationen mit $\pi$ vertauschbar sind, werden die Elemente eines Cyklus nicht getrennt, sondern nur cyklisch vertauscht, und die Cyklen selbst werden untereinander vertauscht. Ist mehr als ein Cyklus vorhanden, so ist die Gruppe imprimitiv.
...
\[
        \omega,\omega_1,
        \omega_2,
        \ldots,\omega_{s-1}                                           \tag{5}
\]
ihre conjugirten Werthe. Diese sind Wurzeln einer irreducibeln Gleichung
\[
        \Phi(t)=0                                                       \tag{7}
\]
vom Grade $s$. Die durch $P$ hervorgerufenen Permutationen von (5) bilden wieder eine commutative Gruppe; also ist $\Phi(t)=0$ eine Abel'sche Gleichung niedrigeren Grades. Adjungirt man $\omega$, so zerfällt
\[
        F(x)=F(x,\omega)F(x,\omega_1)\cdots F(x,\omega_{s-1}),
\]
wobei jeder Factor die Wurzeln eines Cyklus enthält. Der erste Factor hat als Gruppe nur die Periode der cyklischen Permutation $\gamma$.
...
Allgemein heisst eine Gleichung $m$-ten Grades $F(x)=0$ mit verschiedenen Wurzeln cyklisch in $\Omega$, wenn die Wurzeln $\alpha,\alpha_1,\ldots,\alpha_{m-1}$ nicht rational sind, sich aber so anordnen lassen, dass jede durch die cyklische Gruppe
\[
        C=1,\pi,\pi^2,\ldots,\pi^{m-1},\qquad
        \pi=(\alpha,\alpha_1,
        \ldots,\alpha_{m-1})                                           \tag{8}
\]
invariante Function in $\Omega$ rational ist. Die Galois'sche Gruppe einer solchen Gleichung ist entweder $C$ selbst, und dann ist die Gleichung irreducibel, oder ein Theiler
\[
...
Für $m=ef$ zerfällt $\pi^e$ in $e$ Cyklen von je $f$ Gliedern,
\[
\begin{aligned}
...
\end{aligned}                                                          \tag{10}
\]
und so fort. Wählt man eine zu $\gamma_0$ gehörige cyklische Function
\[
        \eta=\psi(\alpha,\alpha_e,
        \ldots,\alpha_{(f-1)e})
\]
und ihre conjugirten
\[
        \eta,\eta_1,
        \ldots,\eta_{e-1},                                             \tag{11}
\]
...
Die Auflösung cyklischer Gleichungen beruht auf den Lagrange'schen Resolventen. Sei $F(x)=0$ eine Gleichung mit den Wurzeln $\alpha,\alpha_1,
\ldots,\alpha_{m-1}$, und sei $\varepsilon$ eine $m$-te Einheitswurzel. Man setzt
\[
        (\varepsilon,\alpha)=
        \alpha+\varepsilon\alpha_1+\varepsilon^2\alpha_2+
        \cdots+\varepsilon^{m-1}\alpha_{m-1}.                          \tag{1}
\]
Kennt man diese Resolventen für alle $m$-ten Einheitswurzeln $\varepsilon$, so ist die Gleichung gelöst. Denn
\[
        \sum_{\varepsilon}\varepsilon^k=0\quad (m\nmid k),\qquad
        \sum_{\varepsilon}1=m,                                          \tag{2}
\]
und daher
\[
        m\alpha_k=\sum_{\varepsilon}\varepsilon^{-k}(\varepsilon,\alpha).\tag{4}
\]
...
an, so geht
\[
        (\varepsilon,\alpha)\quad\hbox{in}\quad
        \varepsilon^{-1}(\varepsilon,\alpha)
\]
über; durch $\pi^h$ geht sie in $\varepsilon^{-h}(\varepsilon,\alpha)$ über. Nach dem polynomischen Lehrsatz ist
\[
        (\varepsilon,\alpha)^\nu
        =A_0^{(\nu)}+
        \varepsilon A_1^{(\nu)}+
...
\]
wobei die $A_k^{(\nu)}$ Formen vom Grade $\nu$ mit ganzen Coefficienten sind, von $\varepsilon$ aber nicht abhängen. Für jede Einheitswurzel, einschliesslich $1$, gilt
\[
        mA_k^{(\nu)}=
        \sum_{\varepsilon}\varepsilon^{-k}(\varepsilon,\alpha)^\nu.       \tag{6}
\]
Wird rechts der Index von $\alpha$ um eins verschoben, so geht $A_k^{(\nu)}$ in $A_{k+\nu}^{(\nu)}$ über.
...
\]
Dann sind die $B_h$ von $\varepsilon$ unabhängige Formen, und die Substitution $\pi$ führt $B_h$ in
\[
        B_{h+\nu+\lambda_1\nu_1+\lambda_2\nu_2+\cdots}
\]
über. In diesem Satze liegt die allgemeine Form der Lagrange'schen Rechnung.

Ist $m=ef$ und $\varepsilon$ nur eine $e$-te Einheitswurzel, so zerfällt (1) in $e$-gliedrige Perioden
\[
\begin{aligned}
\eta      &=\alpha+\alpha_e+\alpha_{2e}+\cdots+\alpha_{(f-1)e},\\
\eta_1    &=\alpha_1+\alpha_{e+1}+\cdots+\alpha_{(f-1)e+1},\\
&\hspace{1.5em}\cdots,\\
\eta_{e-1}&=\alpha_{e-1}+\alpha_{2e-1}+\cdots+\alpha_{m-1}.
\end{aligned}                                                          \tag{9}
\]
Diese Grössen sind die $f$-gliedrigen Perioden. Mit ihnen schreibt sich
\[
        (\varepsilon,\\alpha)=
        \eta+\varepsilon\eta_1+
        \varepsilon^2\eta_2+
        \cdots+
        \varepsilon^{e-1}\eta_{e-1}.                                    \tag{10}
\]
Wendet man $\pi$ auf die Indices an, so ruft sie unter den Perioden $\eta$ die cyklische Permutation
\[
        \gamma=(0,1,
        \ldots,e-1)                                                     \tag{11}
\]
hervor. Für Producte von Resolventen bestimmen daher die Indices der Coefficienten die durch $\pi$ entstehende Permutation.

\sect{165}{Auflösung der cyklischen Gleichungen}

Die Lagrange'schen Resolventen führen zur Auflösung, genauer zur Reduction, der cyklischen Gleichungen. Wir betrachten die Wurzeln $\alpha$ nicht mehr als unabhängige Veränderliche, sondern als Wurzeln einer cyklischen Gleichung, so dass die cyklischen Functionen als bekannt gelten.

Nach \S\,164, 2 sind die Coefficienten von $(\varepsilon,\alpha)^m$ cyklische Functionen. Für Grössen $a_0,
\ldots,a_{m-1}$ in $\Omega$ setzen wir
\[
...
\]
Ist $\varepsilon$ primitive, so sind in dieser Form alle Resolventen enthalten. Ist
\[
        (1,\alpha)=a=\alpha+\alpha_1+\cdots+\alpha_{m-1},
\]
so gibt die Umkehrungsformel
\[
        m\alpha=a+\sqrt[m]{\psi_1}+\sqrt[m]{\psi_2}+
        \cdots+
...
Wendet man den Satz \S\,164, 3 auf zwei Factoren an, so folgt, dass
\[
        (\varepsilon,\alpha)^\nu(\varepsilon^\lambda,
        \alpha)^\mu
\]
eine Function in $\Omega(\varepsilon)$ ist, wenn $\nu+\lambda\mu\equiv0\pmod m$. Für $\mu=1$, $\nu=-\lambda$ erhält man Grössen $\chi_\lambda\in\Omega(\varepsilon)$ mit
\[
        (\varepsilon,\alpha)^{m-\lambda}
        (\varepsilon^\lambda,
        \alpha)=\chi_\lambda,
\]
und damit
\[
        \sqrt[m]{\psi_\lambda}=
        \frac{\chi_\lambda\left(\sqrt[m]{\psi_1}\right)^\lambda}{\psi_1}. \tag{4}
\]
...
\]
die Zerlegung nach den Primtheilern. Man wählt $\lambda_1$ relativ prim zu $p_1$, $\lambda_2$ relativ prim zu $p_2$, usw., so dass
\[
        (\varepsilon^{\lambda_1},\alpha),
        (\varepsilon^{\lambda_2},\alpha),
        \ldots
\]
nicht verschwinden. Nach \S\,164, 3 ist
\[
        (\varepsilon^{\lambda_1},\alpha)^{\nu_1}
        (\varepsilon^{\lambda_2},\alpha)^{\nu_2}\cdots=\chi_\lambda,
        \qquad
        \lambda=-(\lambda_1\nu_1+
        \lambda_2\nu_2+
        \cdots) \pmod m.                                                \tag{5--6}
...
Da $\lambda_1m_1+
\lambda_2m_2+
\cdots$ zu $m$ prim gewählt werden kann, bestimmt dies jeden Radicalwert eindeutig aus einer Anzahl von Radicalen von den Primgraden $p_1,p_2,
\ldots$.

Für den reellen Fall, in dem $\Omega$ der reelle Körper ist, kann man die Radicalwahl trigonometrisch beschreiben. Man schreibt
\[
        \varepsilon=e^{2\pi i/m}=
...
Dann
\[
        \rho_1^2=\varphi_1\varphi'_1,
        \qquad
        \rho_1=\sqrt{a_1^{m}},                                         \tag{11--13}
\]
und aus den reellen Grössen
\[
        \frac{\varphi_1+\varphi'_1}{2}=b_1,
        \qquad
        \frac{\varphi_1-\varphi'_1}{2i}=c_1                             \tag{14}
\]
folgt
\[
...
\]
Für die übrigen Paare erhält man ebenso Winkel $\Theta_2,
\Theta_3,
\ldots$ und setzt
\[
        m_1\Theta_1+m_2\Theta_2+
        \cdots=\Theta.                                                  \tag{16}
\]
...
\]
Es sei der Körper $\Omega$ aus den rationalen Zahlen und den rationalen Functionen von $\cos m\varphi,\sin m\varphi,\cos(2\pi/m),\sin(2\pi/m)$ gebildet.
...
\[
        \sin\varphi=\frac{\sin m\varphi}{B_m(x)}                       \tag{3}
\]
...
\[
        ...
        \quad x_0=f(x_{m-1}).
\]
Eine cyklische Function der $x_k$ geht durch die cyklische Permutation in sich über und ist daher eine symmetrische Function; sie liegt in $\Omega$. Die Theilung des Winkels hängt also von einer cyklischen Gleichung ab.

Diese Gleichung ist irreducibel. Denn ersetzt man $\varphi$ in einer rationalen Relation durch $\varphi+2k\pi/m$, so erhält man eine Relation für alle Wurzeln von (2). Wäre nur $\cos m\varphi$ adjungirt, nicht aber $\sin m\varphi$, so wäre die Gleichung im allgemeinen nicht cyklisch; bei der Dreitheilung tritt dann die Quadratwurzel aus der Discriminante hervor.
...
\[
        r,r^g,r^{g^2},\ldots,r^{g^{n-2}}.                              \tag{2}
\]
...
\end{aligned}                                                          \tag{12}
\]
Diese $\eta_i$ sind die Kreistheilungsperioden. Sie werden durch die Substitution $r\mapsto r^g$ cyklisch vertauscht; also genügen sie einer cyklischen Gleichung $e$-ten Grades, der Periodengleichung.
...
\sect{168}{Producte von Perioden. Dreizehn-Theilung}
...
Für $n=13$ ist $g=2$ eine primitive Wurzel. Da $n-1=12=3\cdot4$, erhält man im Falle $e=3$, $f=4$ die drei Perioden
\[
\begin{aligned}
...
Gauss hat schon in seiner ersten Darstellung in den \emph{Disquisitiones arithmeticae} die Kreistheilungsgleichungen mit Hülfe der Resolventen unmittelbar auf reine Gleichungen zurückgeführt. Wir setzen
\[
        m=n-1
\]
und bezeichnen mit $\varepsilon$ eine primitive $m$-te Einheitswurzel, mit $r,r_1,r_2,\ldots,r_{h-1}$ die $n$-ten Einheitswurzeln in der früher festgesetzten Reihenfolge. Ist $g$ eine primitive Congruenzwurzel modulo $n$ und $g^h\equiv s\pmod n$, so schreiben wir $h=\ind s$. Die Lagrange'schen Resolventen können dann in der Form
\[
        (\varepsilon^\lambda,r)=
        \sum_{s=1}^{n-1}\varepsilon^{\lambda\ind s}r^s                         \tag{1}
\]
geschrieben werden, wobei $\lambda$ ein nach dem Modul $m$ zu nehmender Exponent ist.

Zur weiteren Reduction ist das Product zweier solcher Resolventen zu berechnen. Seien $\lambda,\mu$ zwei Exponenten und mögen $s,t$ unabhängig voneinander die von Null verschiedenen Reste modulo $n$ durchlaufen. Dann ist
\[
...
\]
sofern $\lambda$ nicht durch $n-1$ theilbar ist; für $\lambda\equiv0$ ist dagegen $(1,r)=-1$.

Ist nun $\lambda+\mu$ nicht durch $m=n-1$ theilbar, so verschwinden in (2) die Glieder $t=-1$ nach Summation über $s$. Man erhält
\[
        (\varepsilon^\lambda,r)(\varepsilon^\mu,r)=
        (\varepsilon^{\lambda+
\mu},r)\,\psi_{\lambda,\mu}(\varepsilon),              \tag{4}
\]
wobei
\[
        \psi_{\lambda,\mu}(\varepsilon)=
        \sum_{t=1}^{n-2}
...
Wir nehmen nun $\lambda$ durch einen Factor theilbar an. Ist $m=ef$, und ersetzt man $\lambda$ durch $f\lambda$, so setze man
\[
        \alpha=\varepsilon^f,                                                    \tag{6}
\]
so dass $\alpha$ eine $e$-te Einheitswurzel ist. Schreibt man
\[
        \psi_\lambda(\alpha)=
...
\[
        \eta,\eta_1,\ldots,\eta_{e-1}
\]
die Perioden von je $f$ Gliedern. Dann ist
\[
        (\alpha^\lambda,\eta)=
        \eta+\alpha^\lambda\eta_1+
...
\end{array}                                                            \tag{15}
\]
Zur Berechnung der Functionen $\psi$ dient die Tabelle
\[
...
\end{array}.                                                           \tag{16}
\]
Ist $\alpha$ eine achte Einheitswurzel, so findet man
\[
\begin{aligned}
\psi_1(\alpha)&=\psi_6(\alpha)=2\alpha+2\alpha^2+3\alpha^4+4\alpha^5+2\alpha^6+2\alpha^7,\\
\psi_2(\alpha)&=\psi_5(\alpha)=2+3\alpha+\alpha^3+
...
\eta)(3+i\sqrt8).                          \tag{22}
\]
Hierdurch sind die in Frage kommenden Resolventen durch Quadratwurzeln bestimmt. Bei der festgesetzten Wahl von $r$ und $\alpha$ ist $(-1,
\eta)$ positiv, also
\[
        (-1,
\eta)=\sqrt{17},
...
und $ (i,
\eta)$ sowie $(\alpha,
\eta)$ haben positive reelle Theile. Insbesondere
\[
 (i,
\eta)=\sqrt[4]{17}
 \,\left(
...
Die im vorigen Paragraphen erhaltenen Formeln gestatten viele Anwendungen, ohne dass die Primzahl $n$ selbst besonders bestimmt werden muss. Für die Algebra liefern sie insbesondere cyklische Gleichungen vorgeschriebenen Grades. Wir betrachten daher einige allgemeine Eigenschaften der Functionen $\psi$.

Nach der Definition ändert $\psi_{\lambda,\mu}(\varepsilon)$ seinen Werth nicht, wenn $\lambda$ und $\mu$ vertauscht werden. Ersetzt man $\lambda,\mu$ durch $\lambda f,f$, so folgt für die Functionen $\psi_\lambda(\alpha)$: wenn $\lambda\lambda'\equiv1\pmod e$, so gilt
\[
        \psi_\lambda(\alpha^{\lambda'})=\psi_{\lambda'}(\alpha).        \tag{1}
\]
Nimmt man ferner in der Productformel den Exponenten $-\lambda-
\mu$, so erhält man nach Anwendung der Relation mit den conjugirten Resolventen
\[
        \psi_{-\lambda-
\mu,
...
\mu}(\varepsilon).
\]
In der periodischen Schreibweise heisst das: ist $\lambda+
\lambda''+1\equiv0\pmod e$, so
\[
...
Sind $\lambda,
\mu,
\nu$ so gewählt, dass keine der vorkommenden Summen von Exponenten verschwindet, so folgt aus der Vergleichung des Productes dreier Resolventen
\[
        \psi_{\lambda,
\mu}(\varepsilon)
        \psi_{\lambda+
...
Zum Schluss beweisen wir die Congruenzeigenschaft, die später gebraucht wird. Sei $\nu$ aus
\[
        \lambda+
\mu+
...
\nu\ind(t+1)}          \tag{9}
\]
geschrieben werden. Ersetzt man $\varepsilon$ durch eine primitive Congruenzwurzel $g$ modulo $n$, so entsteht die ganze Zahl
\[
        \psi_{\lambda,
\mu}(g)=
        \sum_{t=1}^{n-2} t^\mu(t+1)^\nu \pmod n.                       \tag{10}
\]
Nach Anwendung des binomischen Lehrsatzes und der geometrischen Summenformel folgt, wenn $\lambda,
\mu$ zwischen $0$ und $m$ liegen,
\[
\begin{array}{ll}
\psi_{\lambda,
\mu}(g)
...
\qquad\pmod n.                                                       \tag{11}
\]
Hier bedeutet $\Pi(k)=1\cdot2\cdots k$.

\sect{171}{Die Gauss'schen Summen}

Ist $n-1=ef$, so genügen die Perioden $\eta,
\eta_1,
\ldots,
\eta_{e-1}$ einer ganzzahligen Gleichung $e$-ten Grades. Sie hat nur reelle Wurzeln, wenn $f$ gerade ist; ist $f$ ungerade, so sind ihre Wurzeln imaginär. Für $e=2$ entstehen die klassischen Gauss'schen Summen.

Für eine ungerade Primzahl $n$ seien die beiden Perioden von je $\frac12(n-1)$ Gliedern mit
\[
        A=r+r_2+r_4+
\cdots+r_{n-3},
        B=r_1+r_3+r_5+
...
\nu)=0$ für negative $\nu$, so folgt durch Induction, dass $(\mu,
\nu)$ eine ganze Function von $x$ ist. Aus
\[
        (1-x^{\mu+1})(\mu,
\nu)
        =(
...
\mu+2).                              \tag{10}
\]
Da $f(x,1)=0$ und $f(x,2)=1-x$, folgt für gerade $\mu$:
\[
        f(x,
...
\[
        \rho={-1+\sqrt{-3}\over2}
\]
eine imaginäre dritte Einheitswurzel. Die drei Perioden $\eta,
\eta_1,
\eta_2$ von je $\frac13(n-1)$ Gliedern sind die Wurzeln einer cyklischen cubischen Gleichung. Nach §169 ist
\[
        \psi_1(\rho)=
...
Die Gleichung, deren Wurzeln $\eta,
\eta_1,
\eta_2$ sind, hat die Form
\[
        \eta^3+
\eta^2+
\beta\eta+
...
Für $e=4$ treten die Primzahlen $n=4f+1$ auf. Die vier Perioden seien $\eta,
\eta_1,
\eta_2,
\eta_3$. Mit $i$ an Stelle von $\alpha$ hat man
\[
\begin{aligned}
...
\[
        p=a^2+b^2.                                                      \tag{1}
\]
Eine Primzahl $q$ ist dagegen niemals in dieser Form darstellbar; allgemeiner ist $a^2+b^2$ nur dann durch $q$ theilbar, wenn $a$ und $b$ durch $q$ theilbar sind. Denn aus $a^2+b^2\equiv0\pmod q$ und $b\not\equiv0$ würde folgen, dass $-1$ quadratischer Rest modulo $q$ wäre, was für $q\equiv3\pmod4$ unmöglich ist.
...
\]
Sind nun $\alpha,
\alpha_1$ ganze Zahlen und $\alpha_1\ne0$, so lässt sich
\[
...
\end{aligned}                                                          \tag{6}
\]
bei dem die Normen abnehmen. Die letzte nicht verschwindende Zahl ist, bis auf einen Einheitsfactor, der grösste gemeinschaftliche Theiler. Man erhält daher: Sind $\alpha,
\beta$ zwei ganze Zahlen in $R(i)$ und $\delta$ ihr grösster gemeinschaftlicher Theiler, so gibt es ganze $\kappa,
\lambda$ mit
\[
        \alpha\kappa+
...
\]
Entweder ist $N(\pi)=n$ und $n=\pi\pi'$ zerfällt in zwei conjugirte Factoren; das geschieht genau für die Primzahlen $p=a^2+b^2$. Oder $N(\pi)=n^2$ und $\alpha$ ist eine Einheit; dann bleibt $n$ Primzahl im Körper, was für die Primzahlen $q$ eintritt. Die Primzahl $2$ hat die besondere Zerlegung
\[
        2=(1+i)(1-i)=-i(1+i)^2.                                        \tag{9}
\]

Die Primzahlen des Körpers $R(i)$ bestehen also aus den reellen Primzahlen $q$ und aus den conjugirten Factoren $a\pm bi$ der Primzahlen $p$. Ist $p$ ungerade, so ist von $a,b$ eine Zahl gerade, die andere ungerade. Wählt man $b$ gerade und das Vorzeichen so, dass $a\equiv1\pmod4$ ist, so ist ein bestimmter Factor ausgezeichnet; er heisst primär. Das Product zweier primärer Zahlen ist wieder primär.
...
Wie im Körper $R(i)$ kann man zu jeder gebrochenen Zahl eine ganze Zahl $\mu$ so finden, dass
\[
        N(\xi-
...
\rho)^2,                         \tag{4}
\]
ist also, bis auf eine Einheit, Quadrat einer Primzahl. Alle reellen Primzahlen der Form $3f+1$ zerfallen nach §172 in zwei conjugirte Primfactoren; die Primzahlen der Form $3f+2$ bleiben Primzahlen in $R(\rho)$, weil ein Ausdruck $a^2-ab+b^2$ nicht $\equiv2\pmod3$ sein kann.

Ist $a+b\rho$ eine ganze complexe Zahl, deren Norm nicht durch $3$ theilbar ist, so ist unter den associirten Zahlen stets genau eine, in welcher der Coefficient von $\rho$ durch $3$ theilbar ist. Setzt man diese in der Form
\[
        a+3B\rho
\]
...
\begin{center}\large Siebzehnter Abschnitt. Algebraische Auflösung von Gleichungen.\end{center}
...
Wir setzen voraus, dass vorher alle Adjunctionen vorgenommen sind, welche die Gruppe nicht verändern. Es sei dann durch Adjunction einer Wurzel $\varepsilon$ einer cyklischen Gleichung
\[
        \varphi(x)=0
\]
von Primzahlgrad $m$ die Gruppe $P$ auf einen echten Theiler $Q$ reducirt. Bezeichnen wir die Wurzeln der Gleichung $\varphi$ mit
\[
        \varepsilon,
\varepsilon_1,
\ldots,
\varepsilon_{m-1},
\]
so sind sie rational durch jede von ihnen darstellbar. Ist $P$ die Gruppe von $f(x)=0$ in $\Omega$, so ist $Q$ die Gruppe derselben Gleichung in $\Omega(\varepsilon)$. Nach dem Satze von §157 ist der Index von $Q$ in $P$ ein Theiler von $m$; da $m$ eine Primzahl ist, muss der Index gleich $m$ sein. Ausserdem ist $\varepsilon$ rational durch die Wurzeln von $f(x)=0$ darstellbar,
\[
        \varepsilon=\psi(x_0,x_1,
\ldots,x_{n-1}).                       \tag{2}
\]
Diese Function gehört zur Gruppe $Q$. Wendet man auf sie alle Permutationen von $P$ an, so entstehen gerade die $m$ conjugirten Functionen $\varepsilon,
\varepsilon_1,
\ldots,
\varepsilon_{m-1}$. Sie gehören zu den conjugirten Gruppen $\pi^{-1}Q\pi$. Da aber jede dieser Functionen rational durch jede andere ausdrückbar ist, gehören sie alle zu derselben Gruppe. Also ist $Q$ ein Normaltheiler von $P$. Wir haben damit bewiesen:
...
Der Satz lässt sich umkehren. Hat nämlich $P$ einen Normaltheiler $Q$ vom Index $m$, so wähle man eine zu $Q$ gehörige Function $\psi$. Die conjugirten Functionen $\psi,
\psi_1,
\ldots,
\psi_{m-1}$ gehören alle zu derselben Gruppe; der Körper $\Omega(\psi)$ ist daher ein Normalkörper, und $\psi$ ist Wurzel einer Normalgleichung. Ist $m$ eine Primzahl, so ist diese Normalgleichung cyklisch. Folglich:
...
Eine Gleichung, deren vollständige Lösung sich auf eine Kette cyklischer Gleichungen zurückführen lässt, heisse metacyklisch. Die cyklischen Gleichungen selbst sind darin als besonderer Fall enthalten. Nach dem Vorhergehenden sind die metacyklischen Gleichungen genau die durch Radicale lösbaren Gleichungen.
...
Denn sei $P$ die Gruppe der Gleichung nach allen Adjunctionen, durch welche die Gleichung noch irreducibel bleibt, und werde durch die nächste Adjunction einer Wurzel $\varepsilon$ einer cyklischen Gleichung von Primzahlgrad $m$ die Gleichung reducibel. Dann besitzt $P$ nach §175 einen Normaltheiler $Q$ vom Index $m$, und $Q$ ist intransitiv. Zerfällt
\[
        f(x)=\varphi(x,
