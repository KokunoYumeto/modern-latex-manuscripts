...
\textbf{Klase konjugacije} \index{Klasa konjugacije} se obično označavaju
simbolom $mC_n$ gdje je $m$
broj elemenata klase, a $C_n$ tipični predstavnik klase označen
Sch\"{o}nfliesovim simbolom: \index{Sch\"{o}nfliesove oznake}
\begin{center}
\begin{tabular}{rcp{10cm}}
    $E$ & = & identiteta \\  \index{Identitet!kao klasa konjugacije}
...
Ciklička grupa C$_3$: \index{C$_3$}
\begin{center}
\begin{tabular}{c|ccc}
     & $E$ & $C_3$ & $C_{3}^2$ \\ \hline
    $A$ & 1 & 1 & 1 \\
...
Ciklička grupa C$_4$: \index{C$_4$}
\begin{center}
...
Kleinova četvorna grupa D$_2$: \index{D$_2$}
\begin{center}
\begin{tabular}{c|cccc}
...
Dihedralna grupa D$_3$: \index{D$_3$}
\begin{center}
\begin{tabular}{c|ccc}
  & E & 2$C_3$  & 3$C_2$ \\ \hline
$A_1$ & 1 & 1& 1 \\
...
Dihedralna grupa D$_{4}$: \index{D$_4$}
\begin{center}
    ...
    \end{tabular}
\end{center}
Ova je grupa izomorfna grupi C$_{4v}$\index{C$_{4v}$}, samo što su u tom  slučaju
neke rotacije za $\pi$ zapravo refleksije: $C_{2}' \to \sigma_v$ i
$C_{2}'' \to \sigma_h$.


Tetrahedralna grupa T:  \index{Tetrahedralna grupa}
\begin{center}
...
Oktahedralna grupa O: \index{Oktahedralna grupa}
...
\label{sec:aksijalni}
\index{Aksijalni vektor} \index{Prostorna inverzija}
...
Važan je primjer Levi-Civita pseudotenzor \index{Tenzor!Levi-Civitin}
trećeg ranga koji je invarijantan
...
\index{Hilbertov prostor}
\index{Diracove "ket" i "bra" oznake}
...
\index{Transformacija!kvantnomehaničkog sustava}
\index{Teorem!Wignerov}
\index{Operator!unitarni}
\index{Operator!linearni}
\index{Operator!antilinearni i antiunitarni}
\begin{align*}
...
\index{Distribucije (generalizirane funkcije)}
...
\emph{Gel'fandov triplet} \cite{Ballentine:1998}. 
\index{Hilbertov prostor!opremljeni (rigged)}
Tako, sasvim strogo uzevši, Diracovi
...
\index{Tenzor!kao matematički stroj}
...
gdje su "$\rule{1em}{1pt}$" mjesta ("slotovi") gdje treba staviti konkretne vektore
...
vektorski produkt dvaju vektora \index{Tenzor!Levi-Civitin}
\begin{equation}
...
nije tenzor obzirom na rotacije \index{Tenzor!elektromagnetskog polja}
...
\index{Apstraktna indeksna notacija}
Indeksi na tenzorima najčešće označavaju kompenente tog tenzora
u nekoj bazi, vidi (\ref{eq:Gkomponente}). Međutim, moguće ih
je upotrebljavati i samo kao zamjenu za slotove iz prošlog
odjeljka koji su nespretni za zapisivanje.
...
jednak jediničnom  \index{Tenzor!metrički}
\begin{displaymath}
g = \text{diag}(1, 1, 1) =
\begin{pmatrix}
1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1
...
\label{sec:su2so3}
\index{Homomorfizam!SU(2) i SO(3)}
...
\end{equation}
jedinične $\mathbb{I}$ i Paulijevih matrica \index{Paulijeve matrice}
\begin{equation}
...
nove ačin da se vidi da je $\det R = 1$. \index{Tenzor!Levi-Civitin}
...
\index{Grupna mnogostrukost} \index{Liejeva grupa!povezana}
...
tzv. Eulerova kuta (vidi npr. \cite{Sakurai:2011}) \index{Eulerovi kutovi}
\begin{equation}
...
interpretirati pomoću tzv. \emph{kvaterniona}. \index{Kvaternioni} Kvaternioni su poopćenje 
...
\begin{align}
    ...
    \det e^{A} &= e^{\mathrm{Tr}\,A} \,. \label{eq:dettr}
\end{align}
...
\label{sec:clebsch}
\index{Clebsch-Gordanov koeficijent}
