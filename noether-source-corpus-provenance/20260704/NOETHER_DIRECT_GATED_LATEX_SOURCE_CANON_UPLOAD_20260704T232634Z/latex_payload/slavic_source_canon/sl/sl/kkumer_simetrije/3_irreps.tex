...
dvije Schurove leme.
\index{Schurova lema} \index{Schur, Issai \yrange{1875}{1941}}
...
\index{Reprezentacija grupe!ekvivalentna}
\end{teorem}
...
\item Neka je $S(V)=V'$ tj. $S$ je surjekcija. Promotrimo 
    ...
    što znači da je i $D(g)\vec{k}$ element kernela Ker($S$), odnosno
...
barem jedan svojstveni vektor tj. postoji
...
reprezentacija: \index{Teorem!ortogonalnosti matrica ireducibilnih reprezentacija}
...
$\vec{x}_{ij}$ iz novog $n$-di\-men\-zi\-o\-nal\-nog vektorskog prostora,  čije
...
da su reprezentacije ekvivalentne\footnote{Taj obrat 
    se \emph{ne} poopćuje na beskonačne kontinuirane grupe, osim
...
ireducibilnih reprezentacija. \index{Tablica karaktera} \index{Karakter!tablica|see{Tablica karaktera}}
\begin{center}
...
\item Broj ireducibilnih reprezentacija jednak je broju klasa konjugacije 
  ...
  Broj klasa pronalazimo "pješke," analizom grupe. \index{Klasa konjugacije} 
...
\item Uvijek postoji trivijalna jednodimenzionalna ireducibilna
  ...
  \index{Reprezentacija grupe!trivijalna}
...
\begin{primjer}[Tablica karaktera grupe $\mathrm{D}_3$]
    \label{pr:karakteriD3} \index{Tablica karaktera} \index{D$_3$}
...
\index{Sch\"{o}nfliesove oznake} \index{Sch\"{o}nflies, Arthur \yrange{1853}{1928}}
...
kubni korijeni jedinice $\chi(c) = \exp(2\pi \rmi k/3), k=0, 1, 2$,
odnosno mogućnosti su $\chi(c)=1, \omega\equiv \rme^{2\pi \rmi/3}, \omega^2$.
...
\index{Reprezentacija grupe!kompleksno konjugirana}
...
vjerno reprezentirano jednodimenzionalnim reprezentacijama. \index{D$_3$}
...
\begin{displaymath}
  ...
  \bigoplus_{\alpha} a_{\alpha} \Gamma_{\alpha} \,,
\end{displaymath}
gdje su $\Gamma_{\alpha}$ ireducibilne reprezentacije i
gdje je $a_{\alpha}$ \emph{multiplicitet} (broj pojavljivanja) 
$\Gamma_{\alpha}$ u $\Gamma$. \index{Ireducibilna reprezentacija!multiplicitet}
\index{Multiplicitet ireducibilne reprezentacije}
...
\begin{equation}
  ...
  \forall i  \,.
\end{equation}
Sada pomnožimo ovu relaciju s $k_i \chi^{(\beta)*}_i$, gdje je $\Gamma_{\beta}$
...
\begin{primjer}[Reprezentacija $\Gamma_V$ grupe D$_3$ na 3D euklidskom prostoru]
    \index{Euklidski prostor} \index{D$_3$}
...
označavamo s $A$, vidi dodatak \ref{sec:aksijalni}.) Kako nam trebaju samo
...
$A_2$ i $E$. \index{Invarijantni potprostor}
...
\index{Clebsch-Gordanov razvoj}
\index{Clebsch, Rudolf \yrange{1833}{1872}}
\index{Gordan, Paul \yrange{1837}{1912}}
\begin{displaymath}
  \Gamma_{\alpha}\otimes\Gamma_{\beta} =
  \bigoplus_{\gamma} \: a_{\gamma}\Gamma_{\gamma}  \,.
\end{displaymath}
...
\index{Kristal}  \index{Kristal!dipolni moment}
...
kristala nepomičnom i izlistane su u dodatku \ref{sec:kristalografija}.
...
identiteta}: \index{Reprezentacija grupe!trivijalna}
\begin{displaymath}
...
vidi dodatak \ref{sec:kristalografija}) \index{Inverzija prostora} 
\index{Refleksija|see{Inverzija prostora}} treba
uzeti u obzir činjenicu da je magnetski moment $\vec{M}$ \emph{aksijalni}
vektor (vidi dodatak \ref{sec:aksijalni}) \index{Aksijalni vektor} \index{Refleksija}
\index{Pseudovektor|see{Aksijalni vektor}}
koji se pri takvim transformacijama ponaša obrnuto od običnog
(polarnog) vektora električnog dipolnog momenta. \index{Kristal!dipolni moment}
Tako će npr. refleksija preko horizontalne ravnine $\sigma_h$ promijeniti
predznak vertikalno usmjerenog električnog, ali ne i
magnetskog dipolnog momenta, vidi sliku \ref{fig:aksijal1}.
\begin{figure}[ht]
    \centerline{\input{figures/aksijal1.tex}}
    \caption{\label{fig:aksijal1}
        Ponašanje vertikalno usmjerenog polarnog vektora $\vec{P}$
        električnog dipolnog momenta
        i aksijalnog vektora $\vec{M}$ magnetskog
    dipolnog momenta prilikom refleksije $\sigma_h$ preko horizontalne ravnine.}
\end{figure}
\begin{figure}[ht]
    \centerline{\input{figures/aksijal2.tex}}
    \caption{\label{fig:aksijal2}
        Ponašanje vertikalno usmjerenog polarnog vektora $\vec{P}$
        električnog dipolnog momenta
        i aksijalnog vektora $\vec{M}$ magnetskog
    dipolnog momenta prilikom refleksije $\sigma_h$ preko vertikalne ravnine.}
\end{figure}
...
čini se da bi ta dva smjera trebala biti ekvivalentna.}, vidi sliku \ref{fig:aksijal2}.

Uzmimo kao primjer kristal s C$_{3v}$ simetrijom. \index{C$_{3v}$}
...
a ako osi izberemo kao na slici \ref{fig:C3vravnina}
\begin{figure}[ht]
    \centerline{\input{figures/C3vravnina.tex}}
    \caption{\label{fig:C3vravnina}}
\end{figure}
onda je 
\begin{displaymath}
D^{(V)}(b)=
...
ne vrijedi, grupe mogu imati iste tablice karaktera, a da nisu izomorfne.}. Dakle,
imamo \index{D$_3$}\index{C$_{3v}$}\index{Tablica karaktera}
\begin{center}
\begin{tabular}{c|ccc}
  & E & 2$C_3$  & 3$C_2$ \\ \hline
$A_1$ & 1 & 1& 1 \\
...
moment. \index{Kristal!dipolni moment}  \index{Dipolni moment|see{Kristal!dipolni moment}}

Za aksijalne vektore, reprezentacije elemenata $e, c$ i $c^2$ su iste kao i
gore, jer rotacije ne razlikuju polarne i aksijalne vektore.  \index{Aksijalni vektor}
...
s istim energijama.
\index{Schr\"{o}dingerova jednadžba}
\index{Schr\"{o}dinger, Erwin \yrange{1987}{1961}}
...
nazivamo \emph{degeneracija}. \index{Degeneracija}
...
zapravo SO(4)=SO(3)$\otimes$SO(3) što će onda potpuno objasniti
...
degeneracija u energijskom spektru sustava.
\index{Degeneracija}
...
slučajeva smanjiti simetriju tako da će ukupni hamiltonijan $H_0 + V$ 
biti sada invarijantan na manju grupu $H<G$. Naime,
...
Na primjer, električno polje u Starkovom efektu \index{Starkov efekt} mijenja hamiltonijan
...
\item Pokažite da su sve ireducibilne reprezentacije Abelovih grupa jednodimenzionalne.
    \label{zad:1Dabelrep} \index{Grupa!Abelova}
...
\item Pokažite da je nužan i dovoljan uvjet ireducibilnosti reprezentacije
 ...
 \index{Frobeniusov kriterij} \index{Frobenius, Ferdinand \yrange{1849}{1917}}
\begin{displaymath}
    \sum_i k_i |\chi_i|^2 = n  \;,
\end{displaymath}
...
\item Pokažite da je direktni produkt dvije jednodimenzionalne
    ireducibilne reprezentacije uvijek ireducibilna reprezentacija.
    \label{zad:1D1D1D}

\item Izrazite reprezentaciju $\Gamma$ grupe $C_{4v}$ koja ima
    karaktere \index{C$_{4v}$}
\begin{displaymath}
