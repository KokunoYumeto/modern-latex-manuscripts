...
čak i jednostavnija od teorije konačnih grupa\footnote{Najspektakularniji
    ...
    grupe dovršio još \'{E}. Cartan \index{Cartan, \'{E}lie \yrange{1869}{1951}}
    i koja se često izlaže u jednom poglavlju
...
tj. da ih je lokalno moguće parametrizirati $n$-torkom realnih\footnote{Matematičari
    ...
    brojevima, no mi takve grupe ovdje nećemo sresti.}
...
skraćeno $g(a)$. \index{Parametar grupe}

Točan oblik parametrizacije, dakle način na koji su točke grupnog prostora obrojčene
parametrima ("koordinatizirane"), je u velikoj mjeri proizvoljan.
Neka ograničenja dolaze od aksioma grupe.
\begin{enumerate}
\item Zatvorenost: $g(c)=g(a)g(b) \in G$. Umjesto tablice množenja koja
    ...
    premda je to strogo uzevši grupni prostor. \index{Zatvorenost!Liejeve grupe}

\item Asocijativnost: $g(a)\big[g(b)g(c)\big]=\big[g(a)g(b)\big]g(c)$ što
    znači da funkcija kompozicije mora zadovoljavati
    $\phi(a, \phi(b,c))=\phi(\phi(a,b),c)$. \index{Asocijativnost!Liejeve grupe}
...
\item Identiteta: postoji parametar $a^0$ takav da je 
    ...
    Običaj je grupnu mnogostrukost parametrizirati tako da bude $a^0=(0, 0, \ldots, 0)
     ...
     kompozicije vrijedi $\phi(0,a)=\phi(a,0)=a$. \index{Identitet!Liejeve grupe}

\item Inverz: $\forall a\quad \exists \bar{a} \td g(a)g(\bar{a})=
        g(\bar{a})g(a)=g(0)=e$ tj. $g(\bar{a})=g(a)^{-1}$. To znači
        da postoji tzv. funkcija inverza \index{Inverz!Liejeve grupe}
...
\begin{definicija}[Liejeva grupa]
    \index{Liejeva grupa!definicija}
...
od \emph{specijalna} ($\det M = 1$) \index{Matrica!specijalna}
\emph{ortogonalna} ($M^\mathsf{T} M = 1$) grupa $2\times 2$ matrica. \index{SO(2)}
\index{Matrica!ortogonalna}
...
međusobnom vezom.  \index{SO(3)} \index{SU(2)}
No krenimo prvo s malo općenitijim razmatranjem transformacija
$\vec{x} \to R \vec{x}$ u trodimenzionalnom euklidskom prostoru,
a koje čuvaju skalarni produkt vektora $(\vec{x}, \vec{y})$.
Uvjet očuvanja skalarnog produkta znači
\begin{equation}
...
grupna mnogostrukost grupe \SU{2} 3-sfera\footnote{Kružnica je 1-sfera $S^1$,
 \index{SU(2)}  \index{Sfera!3-sfera ($S^3$)} \index{Sfera!kao grupna mnogostrukost}
...
podgrupe \U{1} je 1-sfera $S^1$ tj. kružnica. \index{Sfera!1-sfera ($S^1$)}
Kako smo naučili u odjeljku \ref{sec:podgrupe},
...
\index{Susjedna klasa} \index{Hopfova fibracija} \index{Hopf, Heinz \yrange{1894}{1971}}
...
u (\ref{eq:so2}), a \U{1} je grupa unitarnih $1 \times 1$ \index{U(1)}
...
parametrizirati kao $u = \rme^{\rmi\phi}$, $\phi\in [0, 2\pi)$ i jasno je da je
...
u \SO{2}" pomoću Eulerove formule $\rme^{\rmi\phi} = \cos\phi + \rmi\sin\phi$.
...
dakle $1/2$, $3/2$, $5/2$, itd. \index{Polucjelobrojni brojevi}} spinom,
...
\begin{equation}
   M(\epsilon)=M(0)+ \sum_{i=1}^{n}\epsilon_i \frac{\partial M(a_1, \ldots, a_n)}
 {\partial a_i}\Bigg|_{a_1=\ldots =a_n=0} + O(\epsilon^2) \,,
 \label{eq:razvojM}
\end{equation}
...
konstantne matrice (ne ovise o parametrima $a_i$) koje se nazivaju \emph{generatori} grupe
\index{Liejeva grupa!generator} \index{Generator grupe|see{Liejeva grupa!generator}}
i skraćeno ćemo ih označiti s $X_i$
\begin{equation}
X_i =  \frac{\partial M(a_1, \ldots, a_n)}
 {\partial a_i}\Bigg|_{a_1=\ldots =a_n=0}\,, \quad 
   \quad i=1, \ldots, n\,. 
   \label{eq:defXi}
\end{equation}
...
njezinih reprezentacija skoro sasvim određena
...
ili alternativno pomoću tri Eulerova kuta. \index{Eulerovi kutovi}
...
te tamo eksplicitno pokazujemo da je \index{Matrica!eksponenciranje}
\begin{equation}
\rme^{\phi X_3} = \exp\Bigg\{ 
    \phi  \begin{pmatrix}
0 & -1 & 0 \\
1 & 0 & 0 \\
0 & 0 & 0 
...
od \SO{3} možemo dobiti eksponencijacijom generatora.\index{Liejeva grupa!generator} 
...
rotaciji oko fiksne osi koja prolazi kroz tu točku. \index{Teorem!Eulerov}}
svaka \SO{3} rotacija pripada nekoj jednoparametarskoj podgrupi
...
Mogli bismo recimo pokušati konstruirati opću rotaciju kao kompoziciju tri
 ...
 generatore. \index{Eulerovi kutovi} No, ima i lakši put koji će olakšati traženje 
...
\begin{equation} \label{eq:Xantisim}
 RR^\mathsf{T}=(1+\epsilon X)(1+\epsilon 
   X^\mathsf{T}) = 1+\epsilon (X+X^\mathsf{T})+O(\epsilon^2) = 1 \,,
\end{equation}
što znači da je $X^\mathsf{T}=-X$ odnosno $X$ je antisimetrična 3$\times$3 matrica.
Obratno, ako je $X$ antisimetrična, njena eksponencijacija $R = \rme^X$ daje ortogonalnu
matricu jer je
\begin{equation}
  R R^\mathsf{T} = \rme^X (\rme^X)^\mathsf{T} = \rme^X \rme^{X^\mathsf{T}}
  = \rme^X \rme^{-X}
  = \rme^{X-X} = 1 \,,
\end{equation}
gdje je jedini netrivijalni korak predzadnji. Naime, vidi teorem \ref{th:bch},
$\rme^{A} \rme^{B} = \rme^{A+B}$ samo ukoliko matrice $A$ i $B$ komutiraju što je ovdje
srećom trivijalno slučaj.
Zahvaljujući općenitoj relaciji $\det \rme^{X} = \rme^{\textrm{\scriptsize Tr} X}$,
vidi (\ref{eq:dettr}), te činjenici da antisimetrične matrice imaju
...
infinitezimalnih transformacija tj. one nemaju generatore.\label{fus:refleksija}}.
...
barem sve elemente u nekoj okolini jedinice.\index{Liejeva grupa!generator} 
...
baza je \index{Liejeva grupa!generator} 
\begin{equation}
X_1=
\begin{pmatrix}
0 & 0 & 0 \\ 
...
\end{equation}
pa je $\rme^{\phi \vec{X}\cdot\unitn}$ onda općeniti element grupe \SO{3} koji
...
spoznaja na samu grupu.\index{Liejeva grupa!generator} 
To je dodatno pojačano još jednim njihovim važnim svojstvom.
Neka su $X,Y\in\calA$. Definirajmo njihov \emph{komutator}  \index{Komutator}
\begin{equation}
    [X, Y] \equiv X Y - Y X \,.
...
\begin{definicija}[Liejeva algebra] \index{Liejeva algebra!definicija}
...
Ovo poopćenje od matričnih grupa smo napravili samo zato da
...
nematrične (konačno-dimenzionalne) Liejeve algebre efektivno ne postoje.
\begin{teorem}[Ado] \index{Teorem!Adov} \index{Ado, Igor \yrange{1910}{1983}}
...
samo na bazu Liejeve algebre, a ne toliko i na ostatak vektorskog prostora, pa će u žargonu
reći da tri matrice iz (\ref{eq:SO3generators}) "čine algebru" ove grupe.


\begin{primjer}[strukturne konstante grupe \SO{3}]
...
\end{displaymath}
\index{Homomorfizam!Liejevih algebri}
...
\index{Izomorfizam!Liejevih algebri}
...
\begin{equation}
     \{ -\rmi\frac{\sigma_1}{2},  -\rmi\frac{\sigma_2}{2},  -\rmi\frac{\sigma_3}{2} \} \,,
\end{equation}
gdje su $\sigma_{1,2,3}$ tri Paulijeve matrice
\begin{equation}
\sigma_1=\left( \begin{array}{cc} 0 &  1 \\ 1 & 0 \end{array} \right) \,,
\quad
\sigma_2=\left( \begin{array}{cc} 0 & -\rmi \\ \rmi & 0 \end{array} \right) \,,
\quad
...
\soAlg{3} algebre
\begin{displaymath}
 \left[ -\rmi\frac{\sigma_1}{2}, -\rmi\frac{\sigma_2}{2} \right] =
 -\rmi\frac{\sigma_3}{2} \qquad \text{itd.}
\end{displaymath}
pa je pridruživanje $X_i \to -\rmi \sigma_i/2$  izomorfizam i
\soAlg{3}=\suAlg{2}.
...
dok $\sigma_1 + \rmi \sigma_2$ to nije!)
...
a odgovor je da nisu.
\end{primjer}

Pri analizi reprezentacija Liejevih grupa od koristi će biti
tzv. Casimirovi operatori.
\begin{definicija}[Casimirov operator]
    \index{Operator!Casimirov|see{Casimirov operator}}
    \index{Casimir, Hendrik \yrange{1909}{2000}}
    \index{Casimirov operator}
...
\end{equation}
    za \soAlg{3} i
\begin{equation}
    \left(\frac{-\rmi}{2}\right)^2
    \big(\sigma_{1}^2 + \sigma_{2}^2 + \sigma_{3}^2\big) = -\frac{3}{4} \cdot \Eins \,,
\end{equation}
    za \suAlg{2}.
\end{primjer}
Schurova lema\index{Schurova lema}, koja vrijedi i za Liejeve grupe, traži da Casimirov operator 
bude proporcionalan jediničnom i ispostavlja se da je koeficijent proporcionalnosti
zgodan za označavanje ireducibilnih reprezentacija kako ćemo vidjeti u 
odjeljku \ref{sec:repsu2} gdje će biti i pokazana veza Casimirovog s operatorom
momenta impulsa.


\begin{primjer}[Algebra grupe \SO{2}] \index{$\mathfrak{so}$(2)}
    ...
    tj. $\soAlg{2} = \{ aX_{3} \td a \in \mathbb{R} \}$, što je
    zapravo prostor svih antisimetričnih $2\times 2$ matrica,
    koji je kao Liejeva algebra izomorfan s $\mathbb{R}$.
...
svojstva grupe.
\begin{definicija}[Povezanost] \index{Liejeva grupa!povezana}
...
\index{Liejeva grupa!komponenta povezanosti jedinice}
...
eksponencijacije, jasno je da najbolje čemu se možemo nadati u
...
element $X$ algebre takav da je $g = \rme^{X}$?
...
dakle izračunati $\rme^X \rme^Y$? Imajući u vidu definiciju eksponencijacije
to je
\begin{equation}
   \rme^X \rme^Y = \left(\sum_{k=0}^{\infty} \frac{X^k}{k!}\right)
...
iskombinirati u $\sum_n (X+Y)^{n}/n! = \exp(X+Y)$. Za općenite
...
\begin{teorem}[Baker-Campbell-Hausdorff] \label{th:bch}
    \index{BCH formula} \index{Teorem!Baker-Campbell-Hausdorffov}
    \index{Baker, Henry \yrange{1866}{1956}}
    \index{Campbell, John \yrange{1862}{1924}}
    \index{Hausdorff, Felix \yrange{1868}{1942}}
    \[ \rme^X \rme^Y = \rme^Z \,, \]
  gdje je 
  \[ Z = X + Y + \text{red višestrukih komutatora $X$ i $Y$.} \]
\end{teorem}
Ovaj izraz se naziva BCH formula i njeni prvi članovi su
\begin{align}
   ...
   \label{eq:bch}
\end{align}
a ključna netrivijalnost je da su \emph{svi} članovi reda dati preko
...
tehnike. Relativno pristupačan algebarski dokaz može se pronaći
...
$\rme^{X_1} \rme^{X_2}\cdots \rme^{X_n}$ \cite{Stilwell:2008}.
...
\begin{definicija}[Jednostavna (jednostruka) povezanost]
    \index{Liejeva grupa!jednostavno povezana}
...
\index{Grupa!fundamentalna} \index{Prva grupa homotopija}
...
grupu $H$ s \emph{diskretnim} kernelom $K$ onda kažemo da grupa $G$ 
\emph{pokriva} grupu $H$ onoliko puta koliko elemenata ima $K$.
\index{Kernel}  \index{Liejeva grupa!pokrivanje}
...
\begin{teorem}[Univerzalna grupa pokrivanja]
    \index{Univerzalna grupa pokrivanja}
...
algebra $\mathbb{R}$.
\index{Homomorfizam!Liejevih grupa} \index{Izomorfizam!Liejevih algebri}
...
$\{\rme^{x} \td x \in \mathbb{R}\}$ s množenjem kao grupnom operacijom.
...
je $x \to \rme^{\rmi x}$ i kernel je skup $2\pi\mathbb{Z} = \{0, \pm 2\pi,
...
možemo naći sve grupe koje \index{Normalna podgrupa}
...
kernela komutiraju sa cijelom grupom.\index{Centar grupe} \index{Grupa!centar}}.
...
elementa grupe "ne vidi" ima li grupa više od jedne komponente povezanosti
ili javlja li se možda višestruko pokrivanje gdje se grupa "preklapa na samu sebe"
daleko od jedinice.
...
\item \textbf{Specijalna linearna grupa}
    \index{Specijalna linearna grupa|see{SL($n$)}} 
    \index{SL($n$)}

\begin{equation}
\SL{n, \mathbb{C}} = \{ M\in \GL{n,\mathbb{C}} \td \det M = 1\}
\end{equation}

Ovdje je na svaku matricu postavljen dodatni uvjet
\begin{equation}
\det M = f(a_1, a_2 , \dots, a_{2n^2}) = 1 \,,
\label{eq:SL}
\end{equation}
...
ima $2 n^2 - 2$ parametara. Grupa je jednostavno povezana za $n>1$ i za fiziku
je posebno važna grupa $\SL{2, \mathbb{C}}$ koja je univerzalna grupa
pokrivanja za Lorentzovu grupu $\SOsup{+}{1,3}$ na analogan način na koji je \SU{2}
...
\item \textbf{Unitarna grupa}
    \index{Unitarna grupa|see{U($n$)}} 
    \index{U($n$)}

\begin{equation}
\U{n} = \{ M\in \GL{n,\mathbb{C}} \td M M^{\dagger} = 1\}
\end{equation}
...
\item \textbf{Specijalna unitarna grupa}
    \index{Specijalna unitarna grupa|see{SU($n$)}} 
    \index{SU($n$)}

\begin{equation}
\SU{n} = \{ M\in U(n) \td \det M  = 1\}
\end{equation}
...
što daje $|\det M|^2 = 1$ odnosno $\det M = \rme^{\rmi \phi}$, 
$\phi\in [0, 2\pi)$.
...
\item \textbf{Ortogonalna grupa}
    \index{Ortogonalna grupa|see{O($n$)}} 
    \index{O($n$)}

\begin{equation}
\O{n, \mathbb{C}} = \{ M\in \GL{n,\mathbb{C}} \td M M^{\mathrm{T}} = 1\}
\end{equation}
...
\begin{equation}
\O{n} \equiv \O{n, \mathbb{R}} = \{ M\in \GL{n,\mathbb{R}} \td M M^{\mathrm{T}} = 1\} \,,
\end{equation}
...
Uvjet ortogonalnosti $M M^{\mathrm{T}} = 1$ vodi na $(\det M)^2 = 1$, odnosno
na zaključak da ortogonalne matrice imaju determinantu $\det M = \pm 1$.
Kao posljedica \O{n} ima dvije komponente povezanosti.

\item \textbf{Specijalna ortogonalna grupa}
    \index{Specijalna ortogonalna grupa|see{SO($n$)}} 
    \index{SO($n$)}

\begin{equation}
\SO{n, \mathbb{C}} = \{ M\in O(n, \mathbb{C}) \td \det M  = 1\} \,,
\end{equation}
i odgovarajuća realna grupa $\SO{n}$ imaju isti broj parametara kao
$\O{n, \mathbb{C}}$, odnosno \O{n}. Naime, ograničenje determinante
s $\det M = \pm 1$
na $\det M = 1$ ne smanjuje dimenziju parametarskog prostora.

\item \textbf{Pseudo-unitarna grupa}
    \index{Pseudo-unitarna grupa|see{U($p, q$)}} 
    \index{U($p, q$)}

Čine je $n\times n$ matrice
\begin{equation}
\U{p, q} = \{ M\in \GL{n,\mathbb{C}} \td M^{\dagger} g M = g\} \,,
\end{equation}
...
\item \textbf{Pseudo-ortogonalna grupa}
    \index{Pseudo-ortogonalna grupa|see{O($p, q$)}} 
    \index{O($p, q$)}

\begin{equation}
\O{p, q} = \{ M\in \GL{n,\mathbb{R}} \td M^{\mathrm{T}} g M = g \} \,,
\label{eq:MgM}
\end{equation}
...
\index{Lorentzova grupa}  \index{Grupa!Lorentzova|see{Lorentzova grupa}}
...
Liejeve (kontinuirane) grupe i diskretne grupe. To je na ovom nivou metodički
...
nezgodnih situacija gdje bismo operacijom kvocijenta izlazili
...
\index{Cartan, \'{E}lie \yrange{1869}{1951}}  \index{Killing, Wilhelm \yrange{1847}{1923}}
...
\index{Liejeva grupa!simplektička}
\index{Liejeva grupa!iznimna}
...
\index{Grupa!sporadična} \index{Grupa!čudovište (monster)}
\index{Janko, Zvonimir \yrange{1932}{2022}}
grupa\footnote{Četiri sporadične grupe je pronašao hrvatski matematičar Zvonimir Janko
i po njemu se zovu $\mathrm{J}_{1, 2, 3\,\mathrm{i}\, 4}$.} gdje je najveća od njih
...
\item \label{zad:svojstvapaulijevih} Pokažite da Paulijeve matrice imaju slijedeća svojstva:
    \index{Paulijeve matrice} \index{Pauli, Wolfgang \yrange{1900}{1958}}
\begin{align*}
\tag{a} \sigma_{i}^2 &= 1 \\
\tag{b} \sigma_{i}^\dagger &= \sigma_i \\
\tag{c} \det \sigma_i &= -1 \\
\tag{d} \Tr \sigma_i &=0 \\
\tag{e} [\sigma_i, \sigma_j] &= 2 \rmi \epsilon_{ijk} \sigma_k \\
\tag{f} \{\sigma_i, \sigma_j\}\equiv \sigma_i\sigma_j + \sigma_j \sigma_i &= 2
\delta_{ij} 1 \\
\tag{g} \Tr (\sigma_i \sigma_j) &= 2\delta_{ij} \\
\tag{h} \sigma_i \sigma_j &= \delta_{ij} 1 + \rmi \epsilon_{ijk}\sigma_k\\
\tag{i} \rme^{-\frac{\rmi}{2}\vec{\sigma}\cdot\unitn \theta} &=
\cos \frac{\theta}{2}-\rmi\vec{\sigma}\cdot\unitn \sin\frac{\theta}{2} \quad\in \SU{2}
\end{align*}
...
\item \label{zad:abelovaalgebra} Za Abelovu grupu vrijedi $g_1 g_2 g_{1}^{-1} g_{2}^{-1} = e$,
    ...
    \index{Komutator}
    Ako je $g_1$ generiran elementom algebre $g_1 = \rme^{s X}$, a $g_2 = \rme^{t Y}$, gdje su
...
\item Grupni prostor kvocijentne grupe $G/H$ Liejeve grupe $G$ po diskretnoj normalnoj podgrupi $H$
    ...
    antipodnim točakama $S^{3}/\mathbb{Z}_2$.  \index{Kvocijentna grupa} 
    \index{Sfera!3-sfera ($S^3$)}
    ...
    $\mathbb{RP}^{3}$. \index{Projektivni prostor}
\end{enumerate}
