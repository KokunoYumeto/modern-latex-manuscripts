...
| NOETHER_R6_B3_PACKAGE_BOUNDARY_AUDIT_20260704.csv | 45 | Per-file package classification for stable non-checksum R6 metadata outputs, excluding audit self-files and checksum manifests to avoid recursive/stale hashes. |
...
`NOETHER_R6_B3_PACKAGE_BOUNDARY_AUDIT_20260704.csv` classifies 45 stable non-checksum R6 metadata output files for package consumption. Result: 45 support-metadata files, 0 B3 raw source-body pattern hits, 0 zip primaries, 0 temp/cache/runtime pattern hits, 0 files over 5 MiB, and 0 source-body or media payload flags. The audit excludes its own two files and the checksum manifests to avoid recursive or stale hashes; the checksum manifest covers the audit files after refresh. The Bislama official source retry addendum is included as metadata only; raw captures remain in the local `work` tree.
