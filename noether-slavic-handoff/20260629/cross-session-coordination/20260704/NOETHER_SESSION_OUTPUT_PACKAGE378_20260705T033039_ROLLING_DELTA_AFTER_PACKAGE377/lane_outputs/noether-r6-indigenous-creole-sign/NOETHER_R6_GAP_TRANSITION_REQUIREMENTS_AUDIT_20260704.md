...
|---|---:|---|
| `exact_official_source_capture_required` | 27 | Official-source exact URL, local path, byte count, SHA-256, language/level/topic tags, and source-owner/reviewer route. |
...
| `current_gap_retry_required` | 5 | Successful exact source capture or updated failure payload hash, with locator provenance preserved and no R6 source-body package payload. |
...
| `url_access_repair_required` | 9 | Non-placeholder live URL, documented access restriction, verification or 404 blocker, local metadata/error capture hash, and exact route repair note. |
