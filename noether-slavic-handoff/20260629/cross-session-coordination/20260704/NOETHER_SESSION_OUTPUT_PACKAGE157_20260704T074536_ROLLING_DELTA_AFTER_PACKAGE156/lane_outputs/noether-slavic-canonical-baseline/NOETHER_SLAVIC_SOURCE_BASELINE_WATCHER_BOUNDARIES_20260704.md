...
7. Rebuild `EXTERNAL_REVIEW_RETURN_STATUS_20260628.json` if return files are added.
8. If accepted corrections exist, apply only accepted rows, rebuild affected TeX/PDF streams, rerun validators, update manifests, and regenerate package hashes.
9. Run `powershell -NoProfile -ExecutionPolicy Bypass -File outputs\NOETHER_SLAVIC_BASELINE_WATCHER_20260704.ps1`.
10. If no source changes, no accepted corrections, no sidecar/reference drift, and no render defects exist, preserve current package hashes.
