...
- `NOETHER_OPEN_MACHINE_GITHUB_COORDINATION_RULE_20260704.md` and `.json`, including the rule that GitHub-tracked artifacts and PR-visible package records are the open-machine instruction bus.
- `NOETHER_SOURCE_CANON_SUFFICIENCY_TRANSLATION_TRANSITION_20260705.md` at commit `b99286628344251e860fe889e44cc54c8ebd6f87`.
- Parent ledger, source-canon steering record, B3 steward log, and the Persianate/Tajik witness table current through the 2026-07-05 TNU Tajik ideal pass.
...
The companion governance sidecar `NOETHER_PERSIANATE_TAJIK_GITHUB_GOVERNANCE_RESPONSE_20260705.md` records the exact instruction-bus hashes, parent-ledger/B3 hashes, safe-checkout observation, B3 packaging request, and non-claim boundary for this transition. Its operational effect is:

- this lane is not an isolated translation lane;
- covered `fa_IR` and `prs_AF` rows may carry draft review support because source-canon baselines are sufficient for the offered row scope;
- uncovered exact-source gaps remain explicit source-acquisition rows;
- `tg_Cyrl_TJ` remains zero-promoted-row source-discovery only;
- publication remains B3-only and this lane does not push.
