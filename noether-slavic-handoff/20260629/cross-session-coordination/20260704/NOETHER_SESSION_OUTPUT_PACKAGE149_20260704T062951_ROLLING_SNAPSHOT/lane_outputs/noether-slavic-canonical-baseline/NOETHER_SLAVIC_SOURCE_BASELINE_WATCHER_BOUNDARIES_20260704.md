...
| --- | --- | --- | --- |
| Zenodo German/source baseline | `sources\zenodo_updates\...\zenodo_20836874_api_latest_*.json`; live `https://zenodo.org/api/records/20836874` | file key added/removed, size changed, checksum changed, source witness replaced, or version/source note materially changes | compare to stored latest record; re-anchor source inventory; rebuild affected TeX/PDF; update manifests and package hashes |
...
2. Compare file keys, sizes, checksums, modified timestamp, and source-version note against the latest stored Zenodo JSON.
...
6. If no source changes, no accepted corrections, and no render defects exist, preserve current package hashes.
