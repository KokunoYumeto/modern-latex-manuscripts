# R3 Micro-Evidence Artifact (20260704T040807Z)
...
- Route bucket ledger CSV: `R3_MICRO_EVIDENCE_ROUTE_BUCKET_LEDGER_20260704T040807Z.csv`
- Route bucket ledger JSON: `R3_MICRO_EVIDENCE_ROUTE_BUCKET_LEDGER_20260704T040807Z.json`
- Validation JSON: `R3_MICRO_EVIDENCE_VALIDATION_20260704T040807Z.json`
...
Rows may also have multi-bucket routing in oute_bucket when a source row informs more than one split lane; the primary bucket is the safest downstream owner.
...
- High-risk bridge placeholder rows preserved: `8`.

## Boundaries

- 	erm_promotion_allowed=false for every row.
- ridge_promotion_allowed=false for every row.
- 
ative_review_claim=false for every row.
- git_push=false for every row.
- Bridge candidates are carried only in ridge_candidate_nonpromoted, never as accepted surfaces.
...
Use 
oncanonical_micro_translation_support only when nonempty and only for the named route bucket. Otherwise use the row as a blocker/source question. No row may be promoted without independent source and native/domain review closure.