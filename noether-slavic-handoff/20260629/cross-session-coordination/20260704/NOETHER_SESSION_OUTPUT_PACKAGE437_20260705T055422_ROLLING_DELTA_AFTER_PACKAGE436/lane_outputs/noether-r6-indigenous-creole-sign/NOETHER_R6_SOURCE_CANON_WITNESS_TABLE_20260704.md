...
|---|---:|---|
| Indigenous Americas | 35 | Paraguay Guarani exact download, Ecuador EIB/Kichwa CNIB source-body access, Guatemala Uspanteko exact routes, Mexico CONALITEG exact routes, native mathematical source archives, GitHub/source-repository evidence, and three live Minedu URL access checks remain unresolved. |
...
`NOETHER_R6_B3_PACKAGE_BOUNDARY_AUDIT_20260704.csv` classifies 55 stable non-checksum R6 metadata output files for package consumption. Result: 55 support-metadata files, 0 B3 raw source-body pattern hits, 0 zip primaries, 0 temp/cache/runtime pattern hits, 0 files over 5 MiB, and 0 source-body or media payload flags. The audit excludes its own two files and the checksum manifests to avoid recursive or stale hashes; the checksum manifest covers the audit files after refresh. The Bislama official source retry addendum, Aruba Papiamento route retry addendum, Guatemala Uspanteko DIGEBI source capture addendum, Paraguay Guarani MEC route retry addendum, Bolivia Quechua Red Minedu source capture addendum, and Ecuador EIB/Kichwa CNIB route retry addendum are included as metadata only; raw captures or source bodies remain outside `outputs`.
...
`NOETHER_R6_TARGET_COVERAGE_CROSSWALK_20260704.csv` joins the current strict witness table, explicit gap ledger, candidate route ledger, non-source guardrail rows, and non-strict route metadata into 57 target-coverage rows. Result: 25 `explicit_blocker_only`, 9 `retry_capture_plus_explicit_blockers`, 8 `route_retry_metadata_blocker`, 9 `strict_witness_plus_explicit_blockers`, 5 `strict_witness_metadata_only`, and 1 `candidate_route_only_not_witness`.
...
`NOETHER_R6_GAP_TRANSITION_REQUIREMENTS_AUDIT_20260704.csv` maps all 78 explicit gap rows to transition requirements. Result: 17 `exact_official_source_capture_required`, 18 `exact_named_language_source_capture_required`, 2 `current_gap_retry_required`, 18 `post_capture_authority_reuse_review_required`, 17 `url_access_repair_required`, 3 `github_or_source_repository_discovery_required`, and 3 `source_archive_discovery_required`.
