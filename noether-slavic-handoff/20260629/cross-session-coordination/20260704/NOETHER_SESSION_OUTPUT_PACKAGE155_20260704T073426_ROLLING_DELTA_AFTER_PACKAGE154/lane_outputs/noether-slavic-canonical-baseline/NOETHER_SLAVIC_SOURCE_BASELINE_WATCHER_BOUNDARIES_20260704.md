...
6. Rebuild `EXTERNAL_REVIEW_RETURN_STATUS_20260628.json` if return files are added.
7. If accepted corrections exist, apply only accepted rows, rebuild affected TeX/PDF streams, rerun validators, update manifests, and regenerate package hashes.
8. Run `powershell -NoProfile -ExecutionPolicy Bypass -File outputs\NOETHER_SLAVIC_BASELINE_WATCHER_20260704.ps1`.
9. If no source changes, no accepted corrections, no sidecar drift, and no render defects exist, preserve current package hashes.
