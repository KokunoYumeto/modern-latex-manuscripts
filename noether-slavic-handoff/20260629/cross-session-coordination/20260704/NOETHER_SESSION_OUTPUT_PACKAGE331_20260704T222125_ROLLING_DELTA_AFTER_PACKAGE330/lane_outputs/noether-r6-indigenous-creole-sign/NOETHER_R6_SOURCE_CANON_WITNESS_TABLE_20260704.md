...
| Artifact | Rows | Use |
|---|---:|---|
| NOETHER_R6_SOURCE_CANON_STRICT_PROVENANCE_WITNESS_TABLE_20260704.csv | 83 | Preferred strict source-canon witness table. Every row has source URL, local path, SHA-256 hash, license/access signal, and tags. |
| NOETHER_R6_SOURCE_CANON_REQUIRED_FIELD_MIRROR_20260704.csv | 83 | Normalized mirror using the current source-canon steering fields: lane, target, owner/author route, source type, URL, local path, license/access signal, hash, source language, TeX/source-archive flag, fallback flag, blocker note, and non-claim boundary. |
| NOETHER_R6_SOURCE_CANON_WITNESS_TABLE_20260704.csv | 93 | Broad source-plus-guardrail table from the earlier pass. Includes 10 International Sign policy/question guardrail rows that are not strict source witnesses. |
...
| NOETHER_R6_SOURCE_CANON_EXPLICIT_GAP_LEDGER_20260704.csv | 72 | One row per missing or blocked source route with missing source type, known locator or error payload when available, next gate, and blocked-before boundary; includes explicit GitHub/source-repository gaps. |
...
| NOETHER_R6_STRICT_PROVENANCE_PATH_HASH_AUDIT_20260704.md | 1 | Path/hash replay audit for the 83 strict provenance rows; all local paths exist and all recorded SHA-256 values match disk. |
| NOETHER_R6_STRICT_PROVENANCE_PATH_HASH_AUDIT_20260704.csv | 83 | Per-row path existence, actual byte count, actual SHA-256, and hash match status for strict provenance rows. |
...
|---|---:|---|
| Indigenous Americas | 32 | Paraguay Guarani exact download, Ecuador EIB exact routes, Guatemala Uspanteko exact routes, Mexico CONALITEG exact routes, native mathematical source archives, and GitHub/source-repository evidence remain unresolved. |
...
| Signed language | 2 | Signed-language TeX/source archive or GitHub/source repository cannot serve as sign authority by itself; video/access source routes require media and reviewer gates. |
...
`NOETHER_R6_STRICT_PROVENANCE_PATH_HASH_AUDIT_20260704.csv` verifies the 83 strict provenance rows against local disk state. Result: 83/83 local paths found, 83/83 recorded SHA-256 hashes matched, 0 missing files, and 0 hash mismatches. This is provenance integrity only, not permission, authority, review, reuse, translation, or completion evidence.
