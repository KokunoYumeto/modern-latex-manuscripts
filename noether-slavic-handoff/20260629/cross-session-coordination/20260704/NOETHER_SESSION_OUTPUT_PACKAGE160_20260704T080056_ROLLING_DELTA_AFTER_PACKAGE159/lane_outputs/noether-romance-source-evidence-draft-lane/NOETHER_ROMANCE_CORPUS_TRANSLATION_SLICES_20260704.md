...
Post-audit reader notice: this base corpus-slices artifact preserves the first 40-covered / 6-blocked coverage state at its end. For the current completed-reader state, use `NOETHER_ROMANCE_STALE_READER_RECONCILIATION_20260704.md` and `NOETHER_ROMANCE_CURRENT_READER_COVERAGE_20260704.csv`: 44 row instances now have draft/source-note coverage and 2 tensor-product rows remain precisely blocked.
