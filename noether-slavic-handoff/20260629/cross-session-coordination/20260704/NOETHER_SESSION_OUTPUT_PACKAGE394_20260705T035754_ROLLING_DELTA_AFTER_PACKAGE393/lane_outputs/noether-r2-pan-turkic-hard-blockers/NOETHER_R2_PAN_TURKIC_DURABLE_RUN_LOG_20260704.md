...
Motivation: source-canon heartbeat continuation required another direct attack on the zero source-level TeX/archive state. During re-anchoring, the earlier `20260704T2324` source-level probe markdown was found to contain literal `$csvPath`/`$jsonPath` placeholders and NUL-rendered zero characters, so this pass produced a clean metadata-only rerun artifact rather than relying on that stale markdown note.
