...
| Artifact | Rows | Use |
|---|---:|---|
| NOETHER_R6_SOURCE_CANON_WITNESS_TABLE_20260704.csv | 93 | One row per captured source witness with URL, source format, license or permission signal, SHA-256 hash, local path, topic/language/access tags, and ethics/authority notes. |
| NOETHER_R6_SOURCE_CANON_EXPLICIT_GAP_LEDGER_20260704.csv | 69 | One row per missing or blocked source route with missing source type, known locator or error payload when available, next gate, and blocked-before boundary. |
| NOETHER_R6_SOURCE_CANON_REQUIRED_FIELD_MIRROR_20260704.csv | 93 | Normalized mirror using the current source-canon steering fields: lane, target, owner/author route, source type, URL, local path, license/access signal, hash, source language, TeX/source-archive flag, fallback flag, blocker note, and non-claim boundary. |
...
| Signed language comparator | International Sign comparator | 14 | Comparator/guardrail only; not a source authority for any local signed language. |

## Explicit Gap Families

| Target family | Gap rows | Main blockers |
|---|---:|---|
| Indigenous Americas | 31 | Paraguay Guarani exact download, Ecuador EIB exact routes, Guatemala Uspanteko exact routes, Mexico CONALITEG exact routes, and native mathematical source archives remain unresolved. |
| Creole/contact | 29 | Bislama/Vanuatu exact PDFs, Papiamento/Papiamentu exact math PDFs, MIE exact Kreol rows, Jamaican Patois exact STEM, Nigerian Pidgin/Krio/Tok Pisin and other named creole/contact rows, and named-language source archives remain unresolved. |
...
| Signed language | 1 | Signed-language TeX/source archive cannot serve as sign authority; video/access source routes require media and reviewer gates. |
