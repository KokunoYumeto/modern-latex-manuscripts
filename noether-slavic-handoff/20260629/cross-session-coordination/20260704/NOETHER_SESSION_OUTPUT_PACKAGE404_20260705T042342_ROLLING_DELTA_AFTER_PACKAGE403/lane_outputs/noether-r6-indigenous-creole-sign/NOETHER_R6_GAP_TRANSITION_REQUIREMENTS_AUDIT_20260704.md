...
|---|---:|---|
| `exact_official_source_capture_required` | 24 | Official-source exact URL, local path, byte count, SHA-256, language/level/topic tags, and source-owner/reviewer route. |
...
| `current_gap_retry_required` | 4 | Successful exact source capture or updated failure payload hash, with locator provenance preserved and no R6 source-body package payload. |
| `post_capture_authority_reuse_review_required` | 13 | Exact Bislama and Uspanteko official/source-route captures now exist in dated 20260705 retry addenda; reviewer/source-owner and reuse/license gates remain open before movement. |
| `url_access_repair_required` | 13 | Non-placeholder live URL, documented access restriction, verification, timeout, or 404 blocker, local metadata/error capture hash, and exact route repair note. |
