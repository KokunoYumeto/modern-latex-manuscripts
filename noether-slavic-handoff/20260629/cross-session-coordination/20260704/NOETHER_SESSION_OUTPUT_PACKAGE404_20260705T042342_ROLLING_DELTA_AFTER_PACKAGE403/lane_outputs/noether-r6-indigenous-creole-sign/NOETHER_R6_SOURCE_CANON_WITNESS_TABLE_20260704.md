...
| NOETHER_R6_B3_PACKAGE_BOUNDARY_AUDIT_20260704.csv | 51 | Per-file package classification for stable non-checksum R6 metadata outputs, excluding audit self-files and checksum manifests to avoid recursive/stale hashes. |
...
`NOETHER_R6_B3_PACKAGE_BOUNDARY_AUDIT_20260704.csv` classifies 51 stable non-checksum R6 metadata output files for package consumption. Result: 51 support-metadata files, 0 B3 raw source-body pattern hits, 0 zip primaries, 0 temp/cache/runtime pattern hits, 0 files over 5 MiB, and 0 source-body or media payload flags. The audit excludes its own two files and the checksum manifests to avoid recursive or stale hashes; the checksum manifest covers the audit files after refresh. The Bislama official source retry addendum, Aruba Papiamento route retry addendum, Guatemala Uspanteko DIGEBI source capture addendum, and Paraguay Guarani MEC route retry addendum are included as metadata only; raw captures or source bodies remain outside `outputs`.
...
`NOETHER_R6_TARGET_COVERAGE_CROSSWALK_20260704.csv` joins the current strict witness table, explicit gap ledger, candidate route ledger, non-source guardrail rows, and non-strict route metadata into 57 target-coverage rows. Result: 32 `explicit_blocker_only`, 5 `retry_capture_plus_explicit_blockers`, 5 `route_retry_metadata_blocker`, 9 `strict_witness_plus_explicit_blockers`, 5 `strict_witness_metadata_only`, and 1 `candidate_route_only_not_witness`.
