...
- External-review packet infrastructure anchors: `NOETHER_SLAVIC_EXTERNAL_REVIEW_PACKET_INFRASTRUCTURE_ANCHORS_20260704.csv`, with executable watcher invariant `external_review_packet_infrastructure_anchor_matches`
...
| External-review packet infrastructure | `NOETHER_SLAVIC_EXTERNAL_REVIEW_PACKET_INFRASTRUCTURE_ANCHORS_20260704.csv`; `logs\external_review_role_packets_20260628` | role packet count/form count/role set drift, packet/template/protocol hash drift, missing role packet, or added unexpected file | regenerate or revalidate review packet infrastructure before sending or ingesting returns |
...
10. Check external-review role packet infrastructure anchors before sending or ingesting returns.
11. Rebuild `EXTERNAL_REVIEW_RETURN_STATUS_20260628.json` if return files are added.
12. Run `powershell -NoProfile -ExecutionPolicy Bypass -File outputs\NOETHER_SLAVIC_BASELINE_WATCHER_20260704.ps1`.
13. If no source changes, no accepted corrections, no sidecar/reference/label-boundary/review-inbox/review-packet drift, and no render defects exist, preserve current package hashes.
