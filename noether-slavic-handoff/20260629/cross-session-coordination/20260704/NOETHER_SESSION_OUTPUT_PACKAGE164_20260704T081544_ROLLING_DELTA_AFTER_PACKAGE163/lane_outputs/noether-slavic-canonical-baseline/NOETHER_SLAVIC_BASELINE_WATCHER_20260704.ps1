...
$expectedExternalReviewPacketInfrastructure = @{
  file_count = 16
  total_bytes = [int64]2937059
  aggregate_sha256 = "CDAF0506C1ABCC950E6E70BB21834C802192032D9910EC607C6BFFE92D20F5D1"
  manifest_sha256 = "CE60A29ADFD0FBA2B0270A211FEE4CB2D632A862EC65DDC230594BACB1708114"
  role_packet_count = 4
  units_per_role = 46
  total_role_forms = 184
  role_ids = @("ukrainian_math_language", "russian_math_language", "interslavic_authority", "mathematical_source_fidelity")
}
$expectedExternalReviewReturnControlFiles = @(
...
$reviewCorrectionIntakeLedgerPath = Join-Path $CanonicalRoot "logs\REVIEW_CORRECTION_INTAKE_LEDGER_20260702T005500Z.json"
$acceptedCorrectionsLedgerTemplatePath = Join-Path $CanonicalRoot "logs\external_review_role_packets_20260628\ACCEPTED_CORRECTIONS_LEDGER_TEMPLATE_20260628.json"
$externalReviewRolePacketsDir = Join-Path $CanonicalRoot "logs\external_review_role_packets_20260628"
$externalReviewRolePacketsManifestPath = Join-Path $externalReviewRolePacketsDir "EXTERNAL_REVIEW_ROLE_PACKETS_MANIFEST_20260628.json"
...
$externalReviewPacketMismatches = New-Object System.Collections.Generic.List[object]
if (-not (Test-Path -LiteralPath $externalReviewRolePacketsDir)) {
  $externalReviewPacketMismatches.Add([pscustomobject]@{ issue = "role_packets_directory_missing"; path = $externalReviewRolePacketsDir }) | Out-Null
} else {
  $reviewPacketRows = @(Get-ChildItem -LiteralPath $externalReviewRolePacketsDir -File | Sort-Object Name |
    ForEach-Object {
      [pscustomobject]@{
        path = Get-CanonicalRelativePath $_.FullName
        bytes = [int64]$_.Length
  ...
  $reviewPacketTotalBytes = [int64](($reviewPacketRows | Measure-Object bytes -Sum).Sum)
  $reviewPacketAggregateText = ($reviewPacketRows | ForEach-Object { "$($_.path)|$($_.bytes)|$($_.sha256)" }) -join "`n"
  $reviewPacketAggregateSha = Get-StringSha256 $reviewPacketAggregateText
  if ($reviewPacketRows.Count -ne $expectedExternalReviewPacketInfrastructure.file_count) {
    $externalReviewPacketMismatches.Add([pscustomobject]@{ issue = "file_count_mismatch"; expected = $expectedExternalReviewPacketInfrastructure.file_count; actual = $reviewPacketRows.Count }) | Out-Null
  }
  if ($reviewPacketTotalBytes -ne $expectedExternalReviewPacketInfrastructure.total_bytes) {
    $externalReviewPacketMismatches.Add([pscustomobject]@{ issue = "total_bytes_mismatch"; expected = $expectedExternalReviewPacketInfrastructure.total_bytes; actual = $reviewPacketTotalBytes }) | Out-Null
  }
  if ($reviewPacketAggregateSha -ne $expectedExternalReviewPacketInfrastructure.aggregate_sha256) {
    $externalReviewPacketMismatches.Add([pscustomobject]@{ issue = "aggregate_sha_mismatch"; expected = $expectedExternalReviewPacketInfrastructure.aggregate_sha256; actual = $reviewPacketAggregateSha }) | Out-Null
  }
}
$rolePacketManifest = Read-JsonOrNull $externalReviewRolePacketsManifestPath
if ($null -eq $rolePacketManifest) {
  $externalReviewPacketMismatches.Add([pscustomobject]@{ issue = "role_packet_manifest_missing_or_unreadable"; path = $externalReviewRolePacketsManifestPath }) | Out-Null
} else {
  $manifestHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $externalReviewRolePacketsManifestPath).Hash
  if ($manifestHash -ne $expectedExternalReviewPacketInfrastructure.manifest_sha256) {
    $externalReviewPacketMismatches.Add([pscustomobject]@{ issue = "manifest_sha_mismatch"; expected = $expectedExternalReviewPacketInfrastructure.manifest_sha256; actual = $manifestHash }) | Out-Null
  }
  if ([int]$rolePacketManifest.summary.role_packet_count -ne [int]$expectedExternalReviewPacketInfrastructure.role_packet_count) {
    $externalReviewPacketMismatches.Add([pscustomobject]@{ issue = "role_packet_count_mismatch"; expected = $expectedExternalReviewPacketInfrastructure.role_packet_count; actual = $rolePacketManifest.summary.role_packet_count }) | Out-Null
  }
  if ([int]$rolePacketManifest.summary.units_per_role -ne [int]$expectedExternalReviewPacketInfrastructure.units_per_role) {
    $externalReviewPacketMismatches.Add([pscustomobject]@{ issue = "units_per_role_mismatch"; expected = $expectedExternalReviewPacketInfrastructure.units_per_role; actual = $rolePacketManifest.summary.units_per_role }) | Out-Null
  }
  if ([int]$rolePacketManifest.summary.total_role_forms -ne [int]$expectedExternalReviewPacketInfrastructure.total_role_forms) {
    $externalReviewPacketMismatches.Add([pscustomobject]@{ issue = "total_role_forms_mismatch"; expected = $expectedExternalReviewPacketInfrastructure.total_role_forms; actual = $rolePacketManifest.summary.total_role_forms }) | Out-Null
  }
  $actualRoleIds = @($rolePacketManifest.summary.role_ids | Sort-Object)
  $expectedRoleIds = @($expectedExternalReviewPacketInfrastructure.role_ids | Sort-Object)
  $roleIdDiff = @(Compare-Object -ReferenceObject $expectedRoleIds -DifferenceObject $actualRoleIds)
  if ($roleIdDiff.Count -ne 0) {
    $externalReviewPacketMismatches.Add([pscustomobject]@{ issue = "role_id_set_mismatch"; expected = $expectedRoleIds; actual = $actualRoleIds }) | Out-Null
  }
}
Add-Check "external_review_packet_infrastructure_anchor_matches" ($externalReviewPacketMismatches.Count -eq 0) "trigger" "External/native review packet infrastructure file aggregate and manifest shape match expected review-ready baseline." @{
  anchor_artifact = "NOETHER_SLAVIC_EXTERNAL_REVIEW_PACKET_INFRASTRUCTURE_ANCHORS_20260704.csv"
  expected_file_count = $expectedExternalReviewPacketInfrastructure.file_count
  expected_total_role_forms = $expectedExternalReviewPacketInfrastructure.total_role_forms
  mismatch_count = $externalReviewPacketMismatches.Count
  mismatches = $externalReviewPacketMismatches
}

$guardrailLabelPattern = "(?i)(completed|complete|current|cumulative|reader|release|handoff|draft release|zenodo latest|source[- ]baseline|review-ready)"
$guardrailBoundaryPattern = "(?i)(external/native|native/external|external review|native review|authority review|not.*authority|does not claim|not.*completion|review.*incomplete|accepted correction|file/source/render/package state|context only|authority gate)"
$guardrailRiskRows = New-Object System.Collections.Generic.List[object]
$guardrailUnresolvedRows = New-Object System.Collections.Generic.List[object]
foreach ($guardrailFile in @(Get-ChildItem -LiteralPath $outputRoot -File | Where-Object { $_.Extension -in ".md", ".csv", ".json", ".txt", ".ps1" } | Sort-Object Name)) {
  $guardrailText = Get-Content -Raw -LiteralPath $guardrailFile.FullName
  $guardrailLabelMatches = [regex]::Matches($guardrailText, $guardrailLabelPattern)
  if ($guardrailLabelMatches.Count -eq 0) {
    continue
  }
  $guardrailBoundaryMatches = [regex]::Matches($guardrailText, $guardrailBoundaryPattern)
  $pairedMarkdownBoundary = $false
  $pairedMarkdownName = $null
  if ($guardrailFile.Extension -eq ".csv") {
    $pairedMarkdownPath = Join-Path $outputRoot ([System.IO.Path]::GetFileNameWithoutExtension($guardrailFile.Name) + ".md")
    if (Test-Path -LiteralPath $pairedMarkdownPath) {
      $pairedMarkdownName = Split-Path -Leaf $pairedMarkdownPath
      $pairedMarkdownText = Get-Content -Raw -LiteralPath $pairedMarkdownPath
      $pairedMarkdownBoundary = ([regex]::Matches($pairedMarkdownText, $guardrailBoundaryPattern).Count -gt 0)
    }
  }
  $isMachineHashLedger = ($guardrailFile.Name -like "NOETHER_SLAVIC_CANONICAL_BASELINE_OUTPUT_HASHES_*.csv")
  $isResolved = ($guardrailBoundaryMatches.Count -gt 0) -or $pairedMarkdownBoundary -or $isMachineHashLedger
  $guardrailRow = [pscustomobject]@{
    artifact = $guardrailFile.Name
    risky_label_hits = $guardrailLabelMatches.Count
    direct_boundary_hits = $guardrailBoundaryMatches.Count
    paired_markdown_boundary = $pairedMarkdownBoundary
    paired_markdown_name = $pairedMarkdownName
    machine_hash_ledger_global_sidecar = $isMachineHashLedger
    resolved = $isResolved
  }
  $guardrailRiskRows.Add($guardrailRow) | Out-Null
  if (-not $isResolved) {
    $guardrailUnresolvedRows.Add($guardrailRow) | Out-Null
  }
}
Add-Check "completed_reader_label_guardrail_unresolved_zero" ($guardrailUnresolvedRows.Count -eq 0) "trigger" "Completed/current/cumulative/reader/release/handoff/Zenodo/source-baseline labels are fenced by direct, paired, or global guardrail boundaries." @{
  risk_artifact_count = $guardrailRiskRows.Count
  unresolved_count = $guardrailUnresolvedRows.Count
  guardrail_artifact = "NOETHER_SLAVIC_COMPLETED_READER_LABEL_GUARDRAIL_AUDIT_20260704.md"
  unresolved_rows = $guardrailUnresolvedRows
}

$reviewReturnsDir = Split-Path -Parent $reviewStatusPath
$reviewReturnInboxMismatches = New-Object System.Collections.Generic.List[object]
if (-not (Test-Path -LiteralPath $reviewReturnsDir)) {
  $reviewReturnInboxMismatches.Add([pscustomobject]@{ issue = "returns_directory_missing"; path = $reviewReturnsDir }) | Out-Null
} else {
  $reviewReturnInboxFiles = @(Get-ChildItem -LiteralPath $reviewReturnsDir -File | Sort-Object Name)
  $reviewReturnInboxNames = @($reviewReturnInboxFiles | Select-Object -ExpandProperty Name)
  foreach ($expectedControlFile in $expectedExternalReviewReturnControlFiles) {
    if ($expectedControlFile -notin $reviewReturnInboxNames) {
      $reviewReturnInboxMismatches.Add([pscustomobject]@{ issue = "control_file_missing"; file = $expectedControlFile }) | Out-Null
    }
  }
  foreach ($reviewReturnInboxFile in $reviewReturnInboxFiles) {
    if ($reviewReturnInboxFile.Name -notin $expectedExternalReviewReturnControlFiles) {
      $reviewReturnInboxMismatches.Add([pscustomobject]@{
        issue = "candidate_review_return_present"
        file = $reviewReturnInboxFile.Name
        bytes = $reviewReturnInboxFile.Length
        sha256 = (Get-FileHash -Algorithm SHA256 -LiteralPath $reviewReturnInboxFile.FullName).Hash
      }) | Out-Null
    }
  }
}
Add-Check "external_review_return_inbox_direct_candidate_count_zero" ($reviewReturnInboxMismatches.Count -eq 0) "trigger" "External review-return inbox contains only expected control files and no unclassified candidate returns." @{
  expected_control_files = $expectedExternalReviewReturnControlFiles
  sentinel_artifact = "NOETHER_SLAVIC_REVIEW_RETURN_INBOX_DIRECT_SENTINEL_20260704.csv"
  mismatch_count = $reviewReturnInboxMismatches.Count
  mismatches = $reviewReturnInboxMismatches
}

$acceptedCorrectionMismatches = New-Object System.Collections.Generic.List[object]
$reviewCorrectionIntakeLedger = Read-JsonOrNull $reviewCorrectionIntakeLedgerPath
if ($null -eq $reviewCorrectionIntakeLedger) {
  $acceptedCorrectionMismatches.Add([pscustomobject]@{ issue = "review_correction_intake_ledger_missing"; path = $reviewCorrectionIntakeLedgerPath }) | Out-Null
} else {
  if ([bool]$reviewCorrectionIntakeLedger.completion_claim) {
    $acceptedCorrectionMismatches.Add([pscustomobject]@{ issue = "completion_claim_true"; actual = $reviewCorrectionIntakeLedger.completion_claim }) | Out-Null
  }
  if ([int]$reviewCorrectionIntakeLedger.external_review_returns.file_count -ne 0) {
    $acceptedCorrectionMismatches.Add([pscustomobject]@{ issue = "external_review_file_count_nonzero"; actual = $reviewCorrectionIntakeLedger.external_review_returns.file_count }) | Out-Null
  }
  if ([bool]$reviewCorrectionIntakeLedger.external_review_returns.ingestion_available_now) {
    $acceptedCorrectionMismatches.Add([pscustomobject]@{ issue = "ingestion_available_now_true"; actual = $reviewCorrectionIntakeLedger.external_review_returns.ingestion_available_now }) | Out-Null
  }
  if ([bool]$reviewCorrectionIntakeLedger.current_decision.accepted_external_review_ingestion_performed) {
    $acceptedCorrectionMismatches.Add([pscustomobject]@{ issue = "accepted_external_review_ingestion_performed_true"; actual = $reviewCorrectionIntakeLedger.current_decision.accepted_external_review_ingestion_performed }) | Out-Null
  }
  if ([int]$reviewCorrectionIntakeLedger.slavic_review_state.return_file_count -ne 0) {
    $acceptedCorrectionMismatches.Add([pscustomobject]@{ issue = "slavic_return_file_count_nonzero"; actual = $reviewCorrectionIntakeLedger.slavic_review_state.return_file_count }) | Out-Null
  }
  if ([int]$reviewCorrectionIntakeLedger.slavic_review_state.schema_valid_return_file_count -ne 0) {
    $acceptedCorrectionMismatches.Add([pscustomobject]@{ issue = "slavic_schema_valid_return_file_count_nonzero"; actual = $reviewCorrectionIntakeLedger.slavic_review_state.schema_valid_return_file_count }) | Out-Null
  }
  if ([int]$reviewCorrectionIntakeLedger.slavic_review_state.accepted_pair_count -ne 0) {
    $acceptedCorrectionMismatches.Add([pscustomobject]@{ issue = "slavic_accepted_pair_count_nonzero"; actual = $reviewCorrectionIntakeLedger.slavic_review_state.accepted_pair_count }) | Out-Null
  }
  if ([bool]$reviewCorrectionIntakeLedger.slavic_review_state.rebuild_required_from_review_returns) {
    $acceptedCorrectionMismatches.Add([pscustomobject]@{ issue = "rebuild_required_from_review_returns_true"; actual = $reviewCorrectionIntakeLedger.slavic_review_state.rebuild_required_from_review_returns }) | Out-Null
  }
}
$acceptedCorrectionsTemplate = Read-JsonOrNull $acceptedCorrectionsLedgerTemplatePath
if ($null -eq $acceptedCorrectionsTemplate) {
  $acceptedCorrectionMismatches.Add([pscustomobject]@{ issue = "accepted_corrections_template_missing"; path = $acceptedCorrectionsLedgerTemplatePath }) | Out-Null
} else {
  if ($acceptedCorrectionsTemplate.ledger_id -ne "accepted_corrections_YYYYMMDD") {
    $acceptedCorrectionMismatches.Add([pscustomobject]@{ issue = "accepted_corrections_template_ledger_id_changed"; actual = $acceptedCorrectionsTemplate.ledger_id }) | Out-Null
  }
  if ($acceptedCorrectionsTemplate.source_review_batch_id -ne "") {
    $acceptedCorrectionMismatches.Add([pscustomobject]@{ issue = "accepted_corrections_template_has_source_review_batch"; actual = $acceptedCorrectionsTemplate.source_review_batch_id }) | Out-Null
  }
  $templateCorrections = @($acceptedCorrectionsTemplate.corrections)
  if ($templateCorrections.Count -ne 1 -or $templateCorrections[0].correction_id -ne "corr-0001" -or $templateCorrections[0].status -ne "pending|applied|rejected|deferred") {
    $acceptedCorrectionMismatches.Add([pscustomobject]@{ issue = "accepted_corrections_template_no_longer_placeholder"; correction_count = $templateCorrections.Count }) | Out-Null
  }
}
$acceptedLedgerFiles = @(Get-ChildItem -LiteralPath (Split-Path -Parent $acceptedCorrectionsLedgerTemplatePath) -File -Filter "*ACCEPTED*CORRECTIONS*LEDGER*.json" | Sort-Object Name)
foreach ($acceptedLedgerFile in $acceptedLedgerFiles) {
  if ($acceptedLedgerFile.Name -ne "ACCEPTED_CORRECTIONS_LEDGER_TEMPLATE_20260628.json") {
    $acceptedCorrectionMismatches.Add([pscustomobject]@{
      issue = "filled_accepted_corrections_ledger_candidate_present"
      file = $acceptedLedgerFile.Name
      bytes = $acceptedLedgerFile.Length
      sha256 = (Get-FileHash -Algorithm SHA256 -LiteralPath $acceptedLedgerFile.FullName).Hash
    }) | Out-Null
  }
}
Add-Check "accepted_correction_ingestion_direct_zero" ($acceptedCorrectionMismatches.Count -eq 0) "trigger" "Accepted-correction ingestion evidence remains zero and only the placeholder ledger template is present." @{
  sentinel_artifact = "NOETHER_SLAVIC_ACCEPTED_CORRECTION_INGESTION_DIRECT_SENTINEL_20260704.csv"
  mismatch_count = $acceptedCorrectionMismatches.Count
  mismatches = $acceptedCorrectionMismatches
}
