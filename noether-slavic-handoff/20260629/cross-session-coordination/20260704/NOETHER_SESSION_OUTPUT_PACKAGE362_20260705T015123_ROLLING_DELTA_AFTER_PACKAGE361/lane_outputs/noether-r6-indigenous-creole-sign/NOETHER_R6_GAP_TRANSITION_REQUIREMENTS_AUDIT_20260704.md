...
|---|---:|---|
| `exact_official_source_capture_required` | 30 | Official-source exact URL, local path, byte count, SHA-256, language/level/topic tags, and source-owner/reviewer route. |
| `exact_named_language_source_capture_required` | 20 | Named-language math/STEM source URL, local path, byte count, SHA-256, language/topic tags, and source-owner/reviewer route. |
| `current_gap_retry_required` | 7 | Successful exact source capture or updated failure payload hash, with locator provenance preserved and no R6 source-body package payload. |
| `post_capture_authority_reuse_review_required` | 9 | Exact Bislama official source or route capture now exists in the 20260705 retry addendum; reviewer/source-owner and reuse/license gates remain open before movement. |
| `url_access_repair_required` | 6 | Non-placeholder live URL or documented access restriction, headers-only reachability status, local metadata/error capture hash, and exact route repair note. |
