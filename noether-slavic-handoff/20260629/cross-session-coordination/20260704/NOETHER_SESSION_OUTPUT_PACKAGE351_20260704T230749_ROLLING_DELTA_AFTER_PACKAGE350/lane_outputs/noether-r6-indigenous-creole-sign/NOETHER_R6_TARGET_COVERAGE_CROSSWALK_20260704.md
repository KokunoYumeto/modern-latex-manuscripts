...
|---|---:|---|
| `explicit_blocker_only` | 42 | Target appears only in explicit gap/blocker rows. |
...
2. If a target is `strict_witness_plus_explicit_blockers`, resolve the named blocker without treating the strict witness as blanket authority.
3. If a target is `candidate_route_only_not_witness`, create a separate capture policy before reading or hashing source bodies.
4. If guardrails or non-strict route rows are present, keep them out of strict witness counts until exact URL/path/hash/source-authority criteria are met.
