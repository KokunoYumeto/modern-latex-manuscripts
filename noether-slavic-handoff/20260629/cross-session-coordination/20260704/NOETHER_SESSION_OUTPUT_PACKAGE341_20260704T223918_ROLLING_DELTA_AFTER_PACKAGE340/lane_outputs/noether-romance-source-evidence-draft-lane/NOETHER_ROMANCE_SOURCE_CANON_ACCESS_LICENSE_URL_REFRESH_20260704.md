...
| French PDF/page fallback refresh | FR-C-007 through FR-C-010 | Stable PDF or bibliographic URLs returned HTTP 200; source URLs populated where previously only live URLs were present. | Mourougane and Marche remain weak PDF-fallback license rows; Numdam terms were later narrowed in `NOETHER_ROMANCE_SOURCE_CANON_LICENSE_TERMS_DEEPENING_20260704.md`. |
...
| FR-C-008 | French | Numdam bibliographic/PDF access HTTP 200; source URL populated. | Superseded by later terms deepening: Numdam metadata/access terms recorded; PDF fallback only. |
| FR-C-009 | French | Numdam bibliographic/PDF access HTTP 200; source URL populated. | Superseded by later terms deepening: Numdam metadata/access terms recorded; historical PDF fallback. |
...
| ES-C-009 | Spanish | UBA thesis listing and direct PDF URL verified. | Superseded by later terms deepening: UBA author-authorization/copyright boundary recorded; semisimple bridge remains review-sensitive. |
| ES-C-010 | Spanish | Dialnet direct PDF URL verified. | Superseded by later terms deepening: Dialnet legal-use terms recorded; PDF fallback only. |
