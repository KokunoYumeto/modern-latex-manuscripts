...
|---|---:|---|
| `explicit_blocker_only` | 40 | Target appears only in explicit gap/blocker rows. |
| `retry_capture_plus_explicit_blockers` | 2 | Target has a dated retry-capture addendum, but reviewer/source-owner and reuse gates remain explicit blockers before support movement. |
...
3. If a target is `strict_witness_plus_explicit_blockers`, resolve the named blocker without treating the strict witness as blanket authority.
4. If a target is `candidate_route_only_not_witness`, create a separate capture policy before reading or hashing source bodies.
5. If guardrails or non-strict route rows are present, keep them out of strict witness counts until exact URL/path/hash/source-authority criteria are met.
