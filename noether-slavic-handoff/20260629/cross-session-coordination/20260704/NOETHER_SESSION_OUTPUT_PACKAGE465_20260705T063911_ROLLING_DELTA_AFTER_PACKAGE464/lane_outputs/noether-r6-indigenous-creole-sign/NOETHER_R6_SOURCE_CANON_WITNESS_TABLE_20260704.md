...
`NOETHER_R6_B3_PACKAGE_BOUNDARY_AUDIT_20260704.csv` classifies 57 stable non-checksum R6 metadata output files for package consumption. Result: 57 support-metadata files, 0 B3 raw source-body pattern hits, 0 zip primaries, 0 temp/cache/runtime pattern hits, 0 files over 5 MiB, and 0 source-body or media payload flags. The audit excludes its own two files and the checksum manifests to avoid recursive or stale hashes; the checksum manifest covers the audit files after refresh. The Bislama official source retry addendum, Aruba Papiamento route retry addendum, Guatemala Uspanteko DIGEBI source capture addendum, Paraguay Guarani MEC route retry addendum, Bolivia Quechua Red Minedu source capture addendum, Ecuador EIB/Kichwa CNIB route retry addendum, and Mexico CONALITEG Indigenous route retry addendum are included as metadata only; raw captures or source bodies remain outside `outputs`.
