...
|---|---:|---|
| `explicit_blocker_only` | 17 | Target appears only in explicit gap/blocker rows. |
...
| `route_retry_metadata_blocker` | 16 | Target has dated route-retry metadata, but source-body access is still blocked and no support movement is allowed. |
...
| `Ecuador EIB` / `Ecuador Ministry EIB pages` | `route_retry_metadata_blocker` | The Ecuador EIB/Kichwa CNIB route retry addendum records the official CNIB page and exact `KICHWA_CNIB_2017.pdf` route, but local page/PDF access timed out; no PDF source body, source-body hash, authority, clearance, term, translation, or pilot movement is created. |
| `Mexico CONALITEG Indigenous books` / `Nahuatl` / `Hnahnu/Otomi` / `Maya` / `Mexico CONALITEG/Nanahuatzin` | `route_retry_metadata_blocker` | The Mexico CONALITEG route retry addendum records official catalogue HTML captures and title/link rows, but linked book/viewer/PDF bodies remain unresolved or returned tiny 404/error payloads; no book source body, source-body hash, authority, clearance, term, translation, or pilot movement is created. |
| `Nigerian Pidgin` / `Nigerian Pidgin/Nigeria` | `strict_witness_plus_explicit_blockers` | The Nigerian Pidgin source-route addendum records NERDC/FME policy and English mathematics/access comparator captures plus public GitHub `pcm` source-file candidates, but no exact Nigerian Pidgin math/STEM source body, validated mathematical source archive, source-authority return, reuse clearance, term, translation, or pilot movement is created. |
| `Cape Verdean Kriolu` / `Kabuverdianu` | `explicit_blocker_only` through grouped T0 row | The Cape Verdean Kriolu source-route addendum records Cabo Verde Portuguese mathematics comparator captures and Kriolu/bilingual-education context PDFs, but no exact Kriolu math/STEM source body, validated mathematical source archive, source-authority return, reuse clearance, term, translation, or pilot movement is created. Belize Kriol remains separate and unresolved. |
| `Tok Pisin` / `Tok Pisin/PNG FODE` | `explicit_blocker_only` | The Tok Pisin PNG FODE addendum records official FODE Tok Pisin language-context and English mathematics comparator captures, but no exact Tok Pisin math/STEM source body, source-authority return, reuse clearance, term, translation, or pilot movement is created. |
