#!/usr/bin/env python3
"""Build compact same-concept release controls for the SGA3 R22 reader."""
...
NEW_SGA3_ZIP = "10c_SGA3_English_Source_and_History_R22_20260729.zip"
R22_FILES = {
...
def load_r22_identities() -> dict[str, dict[str, object]]:
    rows = list(
        csv.DictReader(
            io.StringIO(
                (R22_ROOT / OUTER_MANIFEST).read_text(encoding="utf-8"),
                newline="",
            )
        )
    )
    ...
    if not R22_FILES.issubset(identities):
        raise RuntimeError("R22 manifest omits a release object")
    for name in R22_FILES:
        path = R22_ROOT / name
        ...
        if (
            ...
            raise RuntimeError(f"R22 identity mismatch: {name}")
    return identities


def build() -> None:
    for path in (
        R22_ROOT / OUTER_MANIFEST,
        PREDECESSOR_RECEIPT,
        PREDECESSOR_CONTROLS,
    ):
        if not path.is_file():
    ...
    if (
        ...
        or int(receipt.get("record", -1)) != 21688473
        or receipt.get("conceptdoi") != "10.5281/zenodo.20410947"
        or int(receipt.get("file_count", -1)) != 66
    ):
        raise RuntimeError("R21 public-readback receipt is not controlling")
    current = {
        name: {
    ...
    prospective["00c_SGA3_English_Reader.pdf"] = r22[
        "00c_SGA3_English_Reader.pdf"
    ]
    prospective["02c_SGA3_English_Master.tex"] = r22[
        "02c_SGA3_English_Master.tex"
    ]
    prospective.pop(OLD_SGA3_ZIP)
    prospective[NEW_SGA3_ZIP] = r22[NEW_SGA3_ZIP]
    ...
    for name, identity in sorted(
        ...
        if name == NEW_SGA3_ZIP:
            ...
            provenance = (
                "grouped SGA3 source closure, R21 history, and completed "
                "native VIII/IX/XI/XV integration controls"
            )
            status = "archive"
        else:
...
guide, and the terminal index. R22 adds completed native-diagram successors
for Exposes VIII, IX, XI, and XV.

The direct reading flow contains mathematical and legitimate historical
editorial content. Project production, source-status, and source-reading
apparatus is kept in the grouped source/history archive.
...
source package is mirrored in the project's GitHub repository.
"""
    validation = {
        "schema": "sga_reader_clean_r22_release_controls_v1",
        "status": "PASS",
        "errors": [],
        "concept_doi": "10.5281/zenodo.20410947",
        "source_record": 21688473,
    ...
    outer_validation = {
        "schema": "sga_reader_clean_r22_controls_package_v1",
        "status": "PASS",
        "errors": [],
        "source_record": 21688473,
        "concept_doi": "10.5281/zenodo.20410947",
    ...
    (OUTPUT_ROOT / "README.md").write_text(
        """# SGA R22 compact release controls

This package updates the compact SGA1-6 release controls for the clean SGA3
R22 native VIII/IX/XI/XV successor. Only the controls ZIP is intended for
the Zenodo surface; loose files are GitHub custody and validation aids.
""",
        encoding="utf-8",
        newline="\n",
    )
