#!/usr/bin/env python3
"""Publish the SGA7 II English reader through complete Expose XIX."""
...
PREDECESSOR_RECORD = 21_747_165
PREDECESSOR_DOI = "10.5281/zenodo.21747165"
PREDECESSOR_FILES = 85
PREDECESSOR_BYTES = 684_224_664
FINAL_FILES = 85
DEFAULT_PREVIEW = "00a_SGA1_English_Reader.pdf"
OLD_CONTROLS_NAME = "10z_SGA_Current_Release_Controls_20260801_r4.zip"
CONTROLS_NAME = "10z_SGA_Current_Release_Controls_20260801_r5.zip"
VERSION = "2026-08-01 SGA7 II English through complete Expose XIX"

REPO_ROOT = Path(__file__).resolve().parents[1]
PACKAGE_REL = Path(
    "sources/sga/"
    "sga7ii-english-through-expose-xix-complete-20260801"
)
PACKAGE_ROOT = REPO_ROOT / PACKAGE_REL
TEMP_ROOT = REPO_ROOT / "tmp/zenodo/sga7ii-english-xix-complete-20260801"
...
RECEIPT_TAG = "20260801_sga7ii_english_through_xix_complete"
PREDECESSOR_RECEIPT = (
    RECEIPT_ROOT
    / "20260801_sga7ii_english_through_xviii_complete_record_21747165_public_readback.json"
)
GITHUB_PACKAGE_COMMIT = "acf581a622ff60f58c34cfb606ad63031fe56ae1"
...
EXPECTED_PACKAGE_FILES = 150
EXPECTED_PACKAGE_BYTES = 3_057_991
EXPECTED_PACKAGE_AGGREGATE = (
    "607455DC8C23D17D654BCF45DA7D0593F4D0B25EED033C5D0B15EC81F49C45E2"
)
EXPECTED_MANIFEST = (
    20_210,
    "6E12AB0A619D6A75BA9F444F7CD4FE96B1581C4F588D48970E42495D99B2CB47",
)
EXPECTED_VALIDATION = (
    2_174,
    "0924F820A28D2A17714E04C6162BB4EC927AAD6D3720660D584C70914D5A5431",
)
EXPECTED_PDF = (
    1_077_752,
...
EXPECTED_ZIP_MEMBERS = 142
EXPECTED_ZIP_MANIFEST = (
    19_362,
    "159635E1603F120657FE2FBF452110797AF7632B34207A2EC1855FD81EBA8EA7",
)
EXPECTED_PREDECESSOR_RECEIPT = (
    41_104,
    "AEB28399B2F79C56C345FB969FAA9B82CED8670FF70E0123ED1E9E9B3E587D69",
)
EXPECTED_OLD_CONTROLS = (
    19_868,
    "6B3FDF89964F5A2C03435BB7DF43BFED9EA1FD5AC8C3B05D5C129F36E7BFFB8C",
)
...
def verify_package() -> dict[str, object]:
    ...
    if (
        ...
        or int(validation["reader"]["pages"]) != 203
        or int(validation["source"]["components"]) != 138
    ...
    if len(rows) != 149 or {row["path"] for row in rows} != set(represented):
    ...
    pdf = PACKAGE_ROOT / (
        "reader/SGA7II_English_Through_Expose_XIX_Complete_20260801.pdf"
    )
    source_zip = PACKAGE_ROOT / (
        "SGA7II_English_Through_Expose_XIX_Complete_"
        "Reader_and_TeX_20260801.zip"
    )
...
This successor adds a direct 203-page SGA7 II English current-progress reader
and one compact ZIP containing that reader plus its exact 138-component
buildable TeX closure.

The reader contains complete Exposes X-XIX through the Expose XIX
bibliography. The next cursor is the opening of Expose XX; Exposes XX-XXII are
absent. The reader is not represented as complete SGA7 II
...
def public_readback(
    ...
    result = {
        ...
        "source_zip_members": EXPECTED_ZIP_MEMBERS,
        "scope": (
            "SGA7 II Exposes X-XIX complete through the Expose XIX bibliography"
        ),
        "complete_sga7ii_claim": False,
        "exhaustive_reference_claim": False,
    ...
    (RECEIPT_ROOT / f"{RECEIPT_TAG}_record_{record_id}.md").write_text(
        "\n".join(
            [
                ...
                f"- Reader: 203 pages / SHA-256 `{EXPECTED_PDF[1]}`",
                f"- Reader/source ZIP: 142 members / SHA-256 `{EXPECTED_SOURCE_ZIP[1]}`",
                "- Scope: Exposes X-XIX complete through the Expose XIX bibliography",
                "- Next cursor Expose XX; Exposes XX-XXII absent; no exhaustive-reference claim",
