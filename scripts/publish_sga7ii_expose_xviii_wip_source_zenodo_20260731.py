#!/usr/bin/env python3
"""Replace the SGA7 II source ZIP with the continuous Expose-XVIII WIP."""
...
PREDECESSOR_RECORD = 21_718_438
PREDECESSOR_DOI = "10.5281/zenodo.21718438"
PREDECESSOR_FILES = 79
PREDECESSOR_BYTES = 646_432_413
FINAL_FILES = 79
DEFAULT_PREVIEW = "00a_SGA1_English_Reader.pdf"
CONTROLS_NAME = "10z_SGA_Current_Release_Controls_20260730.zip"
VERSION = "2026-07-31 SGA7 II Expose XVIII continuous WIP source custody"
...
TEMP_ROOT = REPO_ROOT / "tmp/zenodo/sga7ii-xviii-wip-source-20260731"
...
RECEIPT_TAG = "20260731_sga7ii_expose_xviii_wip_source"
PREDECESSOR_RECEIPT = RECEIPT_ROOT / (
    "20260731_sga7_lead_opened_source_crops_"
    "record_21718438_public_readback.json"
)
GITHUB_PACKAGE_COMMIT = "2b7410e51b63f18ca257aa4e292fa43ba5f31439"
GITHUB_RECEIPT_REL = Path(
    "manifests/published-github/"
    "20260731_sga7ii_expose_xviii_wip_"
    "commit_2b7410e51_public_readback.json"
)

EXPECTED_PACKAGE_FILES = 32
EXPECTED_PACKAGE_BYTES = 4_409_172
EXPECTED_PACKAGE_MANIFEST = (
    4_591,
    "4FEE108D567A6093410D3CBB39FCBBAA4A00F358095D8C97D61F4DB99431EE4F",
)
EXPECTED_WIP = (
    114_619,
    "B4ABE7CA5A6B82F3B6927AA393AEB066C5DE43AF053718D6C85B9B8064B61E79",
)
EXPECTED_SOURCE_ZIP = (
    1_098_631,
...
DESCRIPTION_ADDITION = (
    "<p><strong>SGA7 II Expose XVIII source-work preservation:</strong> this "
    ...
    "now preserves 47 continuous Expose-XVIII source pages, scan indices "
    "262-308 / book folios 255-301; indices 309-334 remain absent, so the "
    "Expose is explicitly incomplete and excluded from the direct reader. "
    "The bounded WIP compiled in isolation with zero fatal errors, one "
    "disclosed overfull box, and is not represented as reader-level release "
    "closure.</p>"
)
NOTES_ADDITION = (
    "<p>This source-custody update corrects an intermediate WIP gap statement "
    "and preserves newer continuous source work produced before a session "
    "quota stop. No direct reader, preview, image archive, or unrelated SGA "
    "file changed.</p>"
)
...
def verify_package() -> None:
    ...
    if len(rows) != 31 or {row["path"] for row in rows} != expected_paths:
    ...
    if (
        ...
        or wip_validation.get("source_page_markers") != 47
...
The replacement archive preserves 47 continuous Expose-XVIII source pages,
scan indices 262-308. Indices 309-334 remain absent, so the WIP is not a
complete Expose and is excluded from the direct reader. The WIP compiled in
isolation with zero fatal errors and one disclosed overfull box.
""",
        encoding="utf-8",
        newline="\n",
    )
    ...
    base.save_json(
        CONTROLS_ROOT / "09b_RELEASE_VALIDATION.json",
        {
            ...
            "xviii_wip_source_indices": [262, 308],
            "xviii_wip_source_page_markers": 47,
...
def public_readback(
    ...
    (RECEIPT_ROOT / f"{RECEIPT_TAG}_record_{record_id}.md").write_text(
        "\n".join(
            [
                "# SGA7 II Expose-XVIII WIP source publication receipt",
                "",
                ...
                f"- Replacement source ZIP: 19 members / `{EXPECTED_SOURCE_ZIP[1]}`",
                "- Expose-XVIII WIP: continuous source indices 262-308; 309-334 absent",
