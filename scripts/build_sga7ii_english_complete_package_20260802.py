#!/usr/bin/env python3
"""Validate and package the complete SGA7 II English working reader."""
...
PACKAGE_ROOT = REPO_ROOT / (
    "sources/sga/sga7ii-english-complete-through-expose-xxii-20260802"
)
MASTER_REL = Path(
    "source/SGA7II_English_Complete_Through_Expose_XXII_20260802.tex"
)
PDF_REL = Path(
    "reader/SGA7II_English_Complete_Through_Expose_XXII_20260802.pdf"
)
ZIP_NAME = (
    "SGA7II_English_Complete_Reader_and_Buildable_TeX_20260802.zip"
)
...
PUBLIC_PDF_NAME = (
    "00j_SGA7II_English_Complete_Working_Reader_20260802.pdf"
)
PUBLIC_TEX_NAME = (
    "02j_SGA7II_English_Complete_Working_Master_20260802.tex"
)
PUBLIC_ZIP_NAME = (
...
def verify_source_closure() -> dict[str, object]:
    ...
    if inputs[-1] != "183_expose_XXII_proposition_6_3_and_bibliography":
        raise RuntimeError("Volume endpoint changed")
    return {
...
def build_and_compare() -> tuple[dict[str, object], str]:
    ...
    with tempfile.TemporaryDirectory(prefix="sga7ii-c183-") as directory:
        root = Path(directory)
        source = root / "source"
        ...
        if underfull_boxes != 4 or textcircled_warnings != 4:
            raise RuntimeError(
    ...
    if (geometry, streams, texts, nonempty) != (
        ...
        raise RuntimeError(
            "Fresh rebuild comparison changed: "
            f"geometry={geometry}, streams={streams}, texts={texts}, "
            f"nonempty={nonempty}"
        )
...
def write_public_docs(source: dict[str, object], reader: dict[str, object]) -> None:
    summary = "\n".join(
        [
            ...
            "- Fresh isolated build: three pdfLaTeX passes, exits 0/0/0; pass 2 and pass 3 console output byte-identical; no blocking diagnostics.",
...
def write_upload_manifest(reader: dict[str, object], archive: dict[str, object]) -> None:
    rows = [
        ...
        {
            "filename": PUBLIC_TEX_NAME,
            "source_path": MASTER_REL.as_posix(),
            "bytes": (PACKAGE_ROOT / MASTER_REL).stat().st_size,
            "sha256": sha256_path(PACKAGE_ROOT / MASTER_REL),
            "role": "direct_complete_editable_master",
        },
        {
...
def main() -> int:
    ...
    validation = {
        "status": "PASS_COMPLETE_WORKING_READER",
        "errors": [],
        "scope": {
            "included": "SGA 7 II Exposes X-XXII through volume EOF",
            "excluded": "No SGA 7 II body continuation; other SGA volumes are separate",
            "continuation": "EOF",
            "complete_sga7ii_working_translation": True,
            "complete_sga_claim": False,
        },
        "authority": {
