...
RECORD_NOTES = {
    "workflow": [
        "Latest workflow update 2026-06-12 adds a source-audit/public-surface addendum: reader-first latest records, source-image authority, derivative-PDF traps, OCR as locator rather than judge, page-map requirements, aid-package design, and reliability labels. The live workflow notes now also record the SGA OCR lesson that Surya-style GPU OCR can be a stronger math/prose locator for French mathematical typescript than ordinary CPU OCR, while OCR from a mismatched source copy must not be used for page-precise claims without a source checksum and page map. Earlier workflow files document source/edition identity checks, structured repair worklists, high-DPI crop packets, local-to-web audit loops, Lean/Lake as a selective formal-checking companion, and GitHub/Zenodo publication hygiene.",
    ],
    "bianchi": [
    ...
    "weber": [
        "Current Weber public surface: Volume I is represented as a repaired/source-scan-audited working edition; Volume II has current German/English cumulative reader PDFs through section 176, with Batch104-Batch128 covering sections 169-176 plus localized recursive repairs/ledgers. Batch128 repairs Volume II sections 29-30 from source scans pp.121-127, with p.128 as the section 31 handoff boundary, replacing a previously compressed Sylow-theorem region in the cumulative Volume II branch. Volume III remains the current repaired cumulative from the earlier baseline. OCR and display ledgers are omission guards and locator layers, not independent authority; continuation ranges remain working drafts unless the package declares source-checked coverage.",
    ],
    "noether": [
