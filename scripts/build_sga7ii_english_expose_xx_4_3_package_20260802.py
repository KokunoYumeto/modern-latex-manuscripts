#!/usr/bin/env python3
"""Validate and package SGA7 II English through Expose XX Section 4.3."""
...
PACKAGE_ROOT = REPO_ROOT / (
    "sources/sga/sga7ii-english-through-expose-xx-4-3-20260802"
)
MASTER_REL = Path(
    "source/SGA7II_English_Through_Expose_XX_4_3_20260802.tex"
)
PDF_REL = Path(
...
PUBLIC_PDF_NAME = (
    "00j_SGA7II_English_Through_Expose_XX_4_3_"
...
def verify_source_closure() -> dict[str, object]:
    ...
    if inputs[-1] != "144_expose_XX_theorem_4_1_through_section_4_3":
        raise RuntimeError("Checkpoint continuation changed")
    if "145_expose" in text:
...
def build_and_compare() -> tuple[dict[str, object], str]:
    ...
    with tempfile.TemporaryDirectory(prefix="sga7ii-c144-") as directory:
        root = Path(directory)
        source = root / "source"
...
def main() -> int:
    ...
    validation = {
        ...
        "errors": [],
        "scope": {
            "included": "SGA 7 II Exposes X-XIX complete and Expose XX through Section 4.3",
            "excluded": "the remainder of Expose XX and Exposes XXI-XXII",
            "continuation": "Expose XX Section 4.4",
