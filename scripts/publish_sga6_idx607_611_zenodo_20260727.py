#!/usr/bin/env python3
"""Publish and read back the SGA6 idx607-611 source-audit crop successor."""
...
PREDECESSOR_RECORD = 21629283
PREDECESSOR_DOI = "10.5281/zenodo.21629283"
SUCCESSOR_RECORD = 21629823
SUCCESSOR_DOI = "10.5281/zenodo.21629823"
VERSION = "2026-07-27 SGA6 ultra-detail source-audit crops idx607-611"
DEFAULT_PREVIEW = (
...
TEMP_ROOT = Path(
    os.environ.get(
        "SGA6_IDX607_611_ZENODO_STAGING_ROOT",
        Path(os.environ["LOCALAPPDATA"])
        / "Temp"
        / "sga6_idx607_611_zenodo_21629823",
    )
)
CONTROLS_ROOT = Path(
    os.environ.get(
        "SGA6_IDX607_611_ZENODO_CONTROLS_ROOT",
        TEMP_ROOT / "controls",
    )
)
ZIP_ROOT = Path(
    os.environ.get(
        "SGA6_IDX607_611_ZENODO_ZIP_ROOT",
        Path(os.environ["LOCALAPPDATA"])
        / "Temp"
        / "sga6_idx607_611_zenodo_stage_20260727",
    )
)
READBACK_ROOT = Path(
    os.environ.get(
        "SGA6_IDX607_611_ZENODO_READBACK_ROOT",
        Path(os.environ["LOCALAPPDATA"])
        / "Temp"
        / "sga6_idx607_611_zenodo_21629823_public_readback",
    )
)
...
LOCAL_FILES = {
    "09_README_CURRENT_RELEASE.md": (
        ...
        3_918,
        "7B96D04D2C7745EBEC6608B43717DF83B9B1DE859623564795BE60B27D52627E",
    ),
    "09a_RELEASE_FILE_MANIFEST.csv": (
        CONTROLS_ROOT / "09a_RELEASE_FILE_MANIFEST.csv",
        24_650,
        "E67A5299F1395A9C8C434EF5975334AFCF5620C094AF4C3841C6B473A693159C",
    ),
    "09b_RELEASE_VALIDATION.json": (
        CONTROLS_ROOT / "09b_RELEASE_VALIDATION.json",
        5_308,
        "E70FAF65101D5CE3E53FC562EE5E209899C02C022FF6C55A42E9DBAE0C9530FD",
    ),
    "10x_SGA6_SourceAudit_Targeted_UltraDetail_Crops_idx607_611_20260727.zip": (
        ZIP_ROOT
        / "10x_SGA6_SourceAudit_Targeted_UltraDetail_Crops_"
        "idx607_611_20260727.zip",
        496_371,
        "962461E012678B72D9D896E6A3EFDBDBE5926CCB7D7EF954A8C9B76D1613D35E",
    ),
    "10y_SGA6_SourceAudit_Crop_Provenance_RightsBlocked_Metadata_idx607_611_20260727.zip": (
        ZIP_ROOT
        / "10y_SGA6_SourceAudit_Crop_Provenance_RightsBlocked_"
        "Metadata_idx607_611_20260727.zip",
        43_930,
        "7F93E7A2BF140AD67F05B9F01EF02A0A74E1CC70A6332856ABE3830FC565453F",
    ),
}

EXPECTED_OUTER_FILES = 69
EXPECTED_OUTER_BYTES = 336_315_522
EXPECTED_RETAINED_FILES = 64
EXPECTED_ZIP_ARCHIVES = 41
EXPECTED_ZIP_FILE_MEMBERS = 3_696
EXPECTED_ZIP_DIRECTORY_ENTRIES = 7
EXPECTED_ZIP_ALL_ENTRIES = 3_703
EXPECTED_ZIP_UNCOMPRESSED_BYTES = 384_530_053
RELEASE_CONTROL_COMPATIBILITY_COUNTER = 3_702
GITHUB_COMMIT = "a7f215965f2e2ca130dc44ec2bc33cbefc596106"
GITHUB_PACKAGE = (
    "sources/sga/"
    "sga6-ultradetail-source-audit-crops-coldreverify-idx607-611-20260727"
)

DESCRIPTION_PARAGRAPHS = [
    (
        "This same-concept compact successor preserves the reader-first SGA "
        "surface from version 10.5281/zenodo.21629283. Sixty-four predecessor "
    ...
    (
        "The targeted archive contains six tight formula, glyph, prose, locator, "
        "and diagram-junction crops actually opened during Claude's SGA6 cold "
        "source re-verification. They map parent indices 607-611 to the stable "
        "five-entry audit boundary #1359-#1363 and span the final page and "
        "bibliography of the Expose X appendix and the opening of Expose XII "
        "through Remark 1.7. Every crop records parent "
        "identity, source page, pixel bounding box, dimensions, scaling, "
        "generator identity, linked audit entry, target context, and QA "
        "disposition."
    ),
    (
        "The paired provenance archive records 25 routine full-width page "
        "bands as rights_blocked_not_public. Their hashes, coordinates, "
        "dimensions, DPI, generator identities, and target links are public, "
        "while their pixels are withheld. No current-pass tight crop was "
        "generated but left unread."
    ),
    (
    ...
    (
        ...
        "series, 2,179 selected images are public and 3,223 routine page "
    ...
    (
        ...
        "VIII, and IX; Expose VII and Exposes X-XXVI remain absent. This successor "
        "updates only existing SGA concept 10.5281/zenodo.20410947."
    ),
]
DESCRIPTION_HTML = "\n".join(
...
NOTES_TEXT = (
    ...
    "This version has 69 files and 41 ZIP archives. Literal archive replay "
    "contains 3,696 non-directory file members plus seven directory records "
    "(3,703 ZIP records total), totaling 384,530,053 uncompressed bytes. The "
    "release-control lineage compatibility counter is 3,702: the predecessor's "
    "3,685 counter plus 17 new non-directory members. SGA1 remains the "
...
def predecessor_identities() -> dict[str, dict]:
    ...
    if len(rows) != 67:
        raise RuntimeError("Predecessor receipt does not contain 67 files")
    result = {
        row["filename"]: {
            "bytes": int(row["bytes"]),
...
def predecessor_public_file_map(session: requests.Session) -> dict[str, dict]:
    ...
    if len(rows) != 67 or len(result) != 67:
...
def anonymous_readback(
    ...
    zip_receipt = {
        ...
        "compatibility_counter_note": (
            ...
            "3,685 counter and adds 17 new non-directory members. Literal "
            "replay has 3,696 non-directory files plus seven directory records, "
            "or 3,703 total ZIP records."
        ),
    ...
    receipt = {
        ...
        "metadata": {
            ...
            "description_exact": True,
            "notes_exact": True,
            "scope": "SGA6 source-audit crops idx607-611",
            "selected_opened_crops": 6,
            "routine_page_bands_metadata_only": 25,
            "generated_unread_tight_crops_metadata_only": 0,
            "stable_audit_entries": "1359-1363",
        },
        "zip_archive_count": len(zip_summaries),
        "zip_file_member_count": len(zip_members),
        "zip_directory_entry_count": directory_entries,
