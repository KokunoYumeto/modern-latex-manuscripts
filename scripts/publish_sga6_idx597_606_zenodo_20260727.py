#!/usr/bin/env python3
"""Publish and read back the SGA6 idx597-606 source-audit crop successor."""
...
PREDECESSOR_RECORD = 21628601
PREDECESSOR_DOI = "10.5281/zenodo.21628601"
SUCCESSOR_RECORD = 21629283
SUCCESSOR_DOI = "10.5281/zenodo.21629283"
VERSION = "2026-07-27 SGA6 ultra-detail source-audit crops idx597-606"
DEFAULT_PREVIEW = (
...
TEMP_ROOT = Path(
    os.environ.get(
        "SGA6_CROP_ZENODO_STAGING_ROOT",
        Path(os.environ["LOCALAPPDATA"])
        / "Temp"
        / "sga6_idx597_606_zenodo_21629283",
    )
)
CONTROLS_ROOT = TEMP_ROOT / "controls"
ZIP_ROOT = Path(
    os.environ.get(
        "SGA6_CROP_ZENODO_ZIP_ROOT",
        Path(os.environ["LOCALAPPDATA"])
        / "Temp"
        / "sga6_idx597_606_zenodo_stage_20260727",
    )
)
READBACK_ROOT = Path(
    os.environ.get(
        "SGA6_CROP_ZENODO_READBACK_ROOT",
        Path(os.environ["LOCALAPPDATA"])
        / "Temp"
        / "sga6_idx597_606_zenodo_21629283_public_readback",
    )
)
...
LOCAL_FILES = {
    "09_README_CURRENT_RELEASE.md": (
        ...
        3_887,
        "767088D976B3B165C48C1DE3D5E54631849F0EFE7AE23C95942C930A38D2B65D",
    ),
    "09a_RELEASE_FILE_MANIFEST.csv": (
        CONTROLS_ROOT / "09a_RELEASE_FILE_MANIFEST.csv",
        23_802,
        "7C09CC88E65EC1A01A2C4AF9D968FCB8116252C5A02A23905D3AC4E7FEB0CB50",
    ),
    "09b_RELEASE_VALIDATION.json": (
        CONTROLS_ROOT / "09b_RELEASE_VALIDATION.json",
        5_295,
        "CD85250A0E14C6227D8743F2C20D8F3FDB4320A801741E5A59174BD3A4EB44AD",
    ),
    "10x_SGA6_SourceAudit_Targeted_UltraDetail_Crops_idx597_606_20260727.zip": (
        ZIP_ROOT
        / "10x_SGA6_SourceAudit_Targeted_UltraDetail_Crops_"
        "idx597_606_20260727.zip",
        1_836_899,
        "AF74BC1C47AAC1ECA1017514E6EB8A7C947E2D4F03400B3820B5E2B2A2FA6E38",
    ),
    "10y_SGA6_SourceAudit_Crop_Provenance_RightsBlocked_Metadata_idx597_606_20260727.zip": (
        ZIP_ROOT
        / "10y_SGA6_SourceAudit_Crop_Provenance_RightsBlocked_"
        "Metadata_idx597_606_20260727.zip",
        85_607,
        "6D509EABF8326D50222D5CF8E3CDD56F35FD161842F2EDD792C5FEEC502AA977",
    ),
}

EXPECTED_OUTER_FILES = 67
EXPECTED_OUTER_BYTES = 335_774_329
EXPECTED_RETAINED_FILES = 62
EXPECTED_ZIP_ARCHIVES = 39
EXPECTED_ZIP_FILE_MEMBERS = 3_679
EXPECTED_ZIP_DIRECTORY_ENTRIES = 7
EXPECTED_ZIP_ALL_ENTRIES = 3_686
EXPECTED_ZIP_UNCOMPRESSED_BYTES = 383_993_186
RELEASE_CONTROL_COMPATIBILITY_COUNTER = 3_685
GITHUB_COMMIT = "d769de0a8753de8286170cb85d016e4030e6f928"
GITHUB_PACKAGE = (
    "sources/sga/"
    "sga6-ultradetail-source-audit-crops-coldreverify-idx597-606-20260727"
)

DESCRIPTION_PARAGRAPHS = [
    (
        "This same-concept compact successor preserves the reader-first SGA "
        "surface from version 10.5281/zenodo.21628601. Sixty-two predecessor "
    ...
    (
        ...
        "source re-verification. They map parent indices 597-606 to the stable "
        "10-entry audit boundary #1349-#1358 and span the Expose X appendix from "
        "section 7.15 through the end of section 7.18. Every crop records parent "
        "identity, source page, pixel bounding box, dimensions, scaling, "
        "generator identity, linked audit entry, target context, and QA "
        "disposition."
    ),
    (
        "The paired provenance archive records 50 routine full-width page "
    ...
    (
        ...
        "series, 2,173 selected images are public and 3,198 routine page "
...
NOTES_TEXT = (
    ...
    "This version has 67 files and 39 ZIP archives. Literal archive replay "
    "contains 3,679 non-directory file members plus seven directory records "
    "(3,686 ZIP records total), totaling 383,993,186 uncompressed bytes. The "
    "release-control lineage compatibility counter is 3,685: the predecessor's "
    "3,654 counter plus 31 new non-directory members. SGA1 remains the "
...
def predecessor_identities() -> dict[str, dict]:
    ...
    if len(rows) != 65:
        raise RuntimeError("Predecessor receipt does not contain 65 files")
    result = {
        row["filename"]: {
            "bytes": int(row["bytes"]),
...
def anonymous_readback(
    ...
    zip_receipt = {
        ...
        "compatibility_counter_note": (
            ...
            "3,654 counter and adds 31 new non-directory members. Literal "
            "replay has 3,679 non-directory files plus seven directory records, "
            "or 3,686 total ZIP records."
        ),
    ...
    receipt = {
        ...
        "github": {
            ...
            "commit_pinned_metadata_readback": "16/16",
        },
        ...
        "metadata": {
            ...
            "description_exact": True,
            "notes_exact": True,
            "scope": "SGA6 source-audit crops idx597-606",
            "selected_opened_crops": 20,
            "routine_page_bands_metadata_only": 50,
            "generated_unread_tight_crops_metadata_only": 0,
            "stable_audit_entries": "1349-1358",
        },
        "zip_archive_count": len(zip_summaries),
        "zip_file_member_count": len(zip_members),
        "zip_directory_entry_count": directory_entries,
