...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--through-printed-page", type=int, required=True)
    parser.add_argument("--through-authority-physical-page", type=int, required=True)
    parser.add_argument("--next-printed-page", type=int, required=True)
    parser.add_argument("--next-authority-physical-page", type=int, required=True)
    parser.add_argument("--next-context", required=True)
    parser.add_argument("--next-line", type=int, required=True)
    parser.add_argument("--checkpoint-label", required=True)
    parser.add_argument("--checkpoint-highlights", required=True)
    parser.add_argument("--active-pdf-pages", type=int, required=True)
    parser.add_argument("--active-pdf-bytes", type=int, required=True)
    parser.add_argument("--active-pdf-sha256", required=True)
...
def main() -> None:
    ...
    readme = f"""# EGA IV Sections 16-18 active source custody through printed page {args.through_printed_page}
...
Printed pages 5-{args.through_printed_page} / authority physical pages
4-{args.through_authority_physical_page} are source-aligned. The next authority
page is printed page {args.next_printed_page} / physical page
{args.next_authority_physical_page}, {args.next_context}, at
`source/source_aligned/ega4-16.tex` line {args.next_line}. Sections
...
are not included. The active producer checkpoint `{args.checkpoint_label}`
remains a three-pass, {args.active_pdf_pages}-page PDF with SHA-256
`{args.active_pdf_sha256}`.
...
"""
    ...
    validation = {
        ...
        "scope": (
            ...
            f"alignment gate through printed page {args.through_printed_page}; "
            f"next authority page {args.next_printed_page}"
        ),
        "aligned_printed_pages": f"5-{args.through_printed_page}",
        "aligned_authority_physical_pages": (
            f"4-{args.through_authority_physical_page}"
        ),
        "next_cursor": (
            f"printed page {args.next_printed_page} / authority physical page "
            f"{args.next_authority_physical_page} / {args.next_context} / "
            f"ega4-16.tex line {args.next_line}"
        ),
        ...
        "active_checkpoint_label": args.checkpoint_label,
        "active_three_pass_build_pages": args.active_pdf_pages,
        "active_three_pass_build_bytes": args.active_pdf_bytes,
        "active_three_pass_build_sha256": args.active_pdf_sha256.upper(),
