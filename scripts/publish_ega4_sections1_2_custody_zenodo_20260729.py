#!/usr/bin/env python3
"""Publish and read back the bounded EGA IV Sections 1-2 custody ZIP."""
...
PREVIOUS_PATH = SCRIPT_DIR / "publish_ega2_through_4_4_5_zenodo_20260729.py"
SPEC = importlib.util.spec_from_file_location("ega_previous_publisher", PREVIOUS_PATH)
...
PREDECESSOR_RECORD = 21_684_972
PREDECESSOR_DOI = "10.5281/zenodo.21684972"
PUBLICATION_DATE = "2026-07-29"
VERSION = "2026-07-29 EGA readers and EGA IV Sections 1-2 custody"
TITLE = previous.TITLE

GITHUB_COMMIT = "af3eb50b58fd3ee6019a0a34533373f2e438eee6"
GITHUB_PACKAGE = (
...
READBACK_ROOT = TEMP_ROOT / "ega4_sections1_2_custody_zenodo_readback"
RECEIPT_PREFIX = "20260729_ega4_sections1_2_custody"
DRAFT_STATE = RECEIPT_ROOT / f"{RECEIPT_PREFIX}_zenodo_draft_state.json"
PREDECESSOR_RECEIPT = (
    RECEIPT_ROOT
    / "20260729_ega2_through_4_4_5_record_21684972_public_readback.json"
)
PREDECESSOR_ZIP_RECEIPT = (
    RECEIPT_ROOT
    / "20260729_ega2_through_4_4_5_record_21684972_zip_member_readback.json"
)

README_NAME = "90 EGA - README and Status.md"
SUMMARY_NAME = "91 EGA - Public Summary.json"
NEW_ZIP = "10d_EGA4_Sections1_2_SourceAligned_Inputs_20260729.zip"
DEFAULT_PREVIEW = previous.DEFAULT_PREVIEW
REPLACED_PREDECESSOR_FILES = {README_NAME, SUMMARY_NAME}
EXPECTED_PREDECESSOR_FILES = 25
EXPECTED_RETAINED_FILES = 23
EXPECTED_LOCAL_FILES = 3
EXPECTED_FINAL_FILES = 26
...
LOCAL_EXPECTED = {
    NEW_ZIP: (
        657_412,
        "D1210043C7885C426B85CA697D047014D7ABC40BA4B5E3389BB4AA5BF0D86F4A",
    ),
    README_NAME: (
        2_136,
        "2D5D8983634A021776758F28E35E63E37E9FA99192BB7D341D032064C5CCE0AF",
    ),
    SUMMARY_NAME: (
        3_409,
        "D11CCB347A380A2D60A7EC6738571C31C63A33396E36FA4024BA6ACFC6639E21",
    ),
}

EXPECTED_ZIP_MEMBERS = 90
EXPECTED_ZIP_UNCOMPRESSED_BYTES = 1_043_500
EXPECTED_ZIP_MANIFEST_ROWS = 88

DESCRIPTION_PARAGRAPHS = [
    (
        "This same-concept successor adds one compact EGA IV custody ZIP "
        "containing the completed source-aligned integration inputs for "
        "Sections 1.1-1.10 and 2.1-2.8. Section 3 is the exact continuation."
    ),
    (
        "No new direct reader is introduced by this update. The existing EGA "
        "IV working reader remains a historical and continuation surface; a "
        "later cumulative reader must consume these inputs and pass its own "
        "build and public readback before replacing it."
    ),
    (
        "The ZIP contains 90 exact members and an internal 88-row "
        "self-excluding identity manifest. It includes editable English "
        "source, the two bounded reader PDFs, and machine-readable controls. "
        "Its visual-evidence ledger records 22 authority-derived images as "
        "rights-blocked and 54 target renders as redundant; no visual pixels "
        "are included."
    ),
    (
...
NOTES_HTML = (
    ...
    "archives. The EGA IV Sections 1-2 ZIP is custody material, not a new "
    "cumulative reader. The preferred preview remains the EGA 0/III reader. "
    "Immutable predecessor versions preserve earlier bounded releases.</p>"
)
...
def inspect_new_zip(path: Path, *, include_members: bool) -> dict:
    ...
    with zipfile.ZipFile(path) as archive:
        if archive.testzip() is not None:
            raise RuntimeError("EGA IV custody ZIP CRC validation failed")
        infos = archive.infolist()
        if any(info.is_dir() or not safe_member_name(info.filename) for info in infos):
            raise RuntimeError("EGA IV custody ZIP contains an unsafe member")
        for info in infos:
        ...
        if len(rows) != EXPECTED_ZIP_MANIFEST_ROWS:
            raise RuntimeError("EGA IV custody ZIP manifest row count changed")
        ...
        if (
            ...
            raise RuntimeError("EGA IV custody ZIP manifest boundary mismatch")
        for row in rows:
            ...
            if (observed["bytes"], observed["sha256"]) != (
                ...
                raise RuntimeError(
                    f"EGA IV custody ZIP manifest mismatch: {row['relative_path']}"
                )
        ...
        if (
            ...
            or validation.get("continuation") != "EGA IV Section 3"
            or validation.get("visual_evidence", {}).get("pixels_included") != 0
            or validation.get("visual_evidence", {}).get("rights_blocked") != 22
        ):
            raise RuntimeError("EGA IV custody ZIP packaged validation changed")
...
def assert_new_zip(summary: dict) -> None:
    if (
        ...
        raise RuntimeError("EGA IV custody ZIP identity boundary changed")
...
def github_package_readback() -> None:
    ...
    for path in sorted(PACKAGE_ROOT.iterdir(), key=lambda value: value.name.casefold()):
        ...
        if (len(data), hashlib.sha256(data).hexdigest().upper()) != (
            ...
            raise RuntimeError(f"GitHub EGA IV custody mismatch: {path.name}")
...
def verify_local_files() -> dict[str, dict]:
    ...
    for name, path in LOCAL_PATHS.items():
        ...
        if (row["bytes"], row["sha256"]) != LOCAL_EXPECTED[name]:
            raise RuntimeError(f"Local EGA IV successor identity mismatch: {name}")
        result[name] = row
...
def expected_files(
    ...
    predecessor = previous.receipt_identities(predecessor_receipt)
    retained = {
        name: row
...
def anonymous_readback(
    ...
    session = base.make_session()
    record = previous.wait_for_public(session, record_id)
    ...
    previous.assert_metadata(record["metadata"])
    ...
    previous.clean_temp(READBACK_ROOT)
    ...
    try:
        for index, name in enumerate(sorted(expected, key=str.casefold), start=1):
            ...
            size, sha, md5 = previous.download_file(
                session,
...
def main() -> None:
    ...
    predecessor_receipt, predecessor_zip_receipt = previous.load_predecessor()
    expected, retained = expected_files(predecessor_receipt, local)
    session = base.make_session()

    if args.preflight:
        previous.verify_live_predecessor(session, predecessor_receipt)
        print(
            json.dumps(
                {
                    ...
                    "ega4_zip_members": EXPECTED_ZIP_MEMBERS,
                },
                indent=2,
            ),
            flush=True,
    ...
    predecessor = previous.verify_live_predecessor(session, predecessor_receipt)
    token = base.find_token()
    draft_id = previous.create_or_resume_draft(session, token, predecessor)
    staged = previous.stage_draft(session, token, draft_id, expected, local)
    published = previous.publish_draft(session, token, draft_id, expected)
    public, zipped = anonymous_readback(
        draft_id,
        expected,
        retained,
