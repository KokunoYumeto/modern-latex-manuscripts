#!/usr/bin/env python3
"""Read back the SGA7 II through Expose XX 4.3 closeout commits."""
...
SOURCE_COMMIT = "8442a86f68c03cf193d97ee577ea49f14e0b0fb6"
MERGE_COMMIT = "6f72a7ed8dea3456a0c1fd63ffde2af8877ae8eb"
PULL_REQUEST = 245
REPOSITORY = "KokunoYumeto/modern-latex-manuscripts"
RECEIPT_STEM = (
    "20260802_sga7ii_english_expose_xx_4_3_"
    "closeout_commit_6f72a7ed8_public_readback"
)
...
def main() -> int:
    ...
    for path in changed:
        ...
        source_blob = git(
            root, "rev-parse", f"{SOURCE_COMMIT}:{path}"
        ).decode("ascii").strip()
        merge_blob = git(
            root, "rev-parse", f"{MERGE_COMMIT}:{path}"
        ).decode("ascii").strip()
        ...
        results[path] = {
            ...
            "source": {
                "url": raw_url(SOURCE_COMMIT, path),
                "http_status": source_status,
                "git_blob": source_remote_blob,
                "expected_git_blob": source_blob,
            },
            "merge": {
                "url": raw_url(MERGE_COMMIT, path),
                "http_status": merge_status,
                "sha256": merge_sha256,
                "git_blob": merge_remote_blob,
                "expected_git_blob": merge_blob,
            },
    ...
    md_path.write_text(
        "\n".join(
            [
                "# SGA7 II through Expose XX Section 4.3 GitHub closeout readback",
                "",
                ...
                "",
                "Every file changed by the closeout commit matched at both public "
                "commit identities. This receipt does not mutate Zenodo or change "
                "the bounded reader's scope, preview, rights, or reference claims.",
                "",
            ]
        ),
        encoding="utf-8",
