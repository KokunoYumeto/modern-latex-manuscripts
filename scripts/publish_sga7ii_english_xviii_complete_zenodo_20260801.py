#!/usr/bin/env python3
"""Publish the SGA7 II English reader through complete Expose XVIII."""
...
PREDECESSOR_RECORD = 21_745_704
PREDECESSOR_DOI = "10.5281/zenodo.21745704"
PREDECESSOR_FILES = 85
PREDECESSOR_BYTES = 684_141_327
FINAL_FILES = 85
DEFAULT_PREVIEW = "00a_SGA1_English_Reader.pdf"
OLD_CONTROLS_NAME = "10z_SGA_Current_Release_Controls_20260801_r3.zip"
CONTROLS_NAME = "10z_SGA_Current_Release_Controls_20260801_r4.zip"
VERSION = "2026-08-01 SGA7 II English through complete Expose XVIII"

REPO_ROOT = Path(__file__).resolve().parents[1]
PACKAGE_REL = Path(
    "sources/sga/"
    "sga7ii-english-through-expose-xviii-complete-20260801"
)
PACKAGE_ROOT = REPO_ROOT / PACKAGE_REL
TEMP_ROOT = REPO_ROOT / "tmp/zenodo/sga7ii-english-xviii-complete-20260801"
...
RECEIPT_TAG = "20260801_sga7ii_english_through_xviii_complete"
PREDECESSOR_RECEIPT = (
    RECEIPT_ROOT
    / "20260801_sga7ii_english_through_xviii_5_8_7_record_21745704_public_readback.json"
)
GITHUB_PACKAGE_COMMIT = "46c222136acef06ce0f515028ee73cdfcbd2b1bf"

PDF_NAME = (
    "00j_SGA7II_English_Through_Expose_XVIII_Complete_Working_20260801.pdf"
)
SOURCE_ZIP_NAME = (
    "10g3_SGA7II_English_Through_Expose_XVIII_Complete_"
...
EXPECTED_PACKAGE_FILES = 145
EXPECTED_PACKAGE_BYTES = 2_949_541
EXPECTED_PACKAGE_AGGREGATE = (
    "3C3A0FA45FC2A2C8CE75D8D9A585DD0BEB69BD83A6FC29274524E926EA66B743"
)
EXPECTED_MANIFEST = (
    19_564,
    "57656A23E42C05665F6C34E6CF568454333642476BBCC0F0A21193FC1A84D3D3",
)
EXPECTED_VALIDATION = (
    2_204,
    "E5799EC594C4E8F163544068430B48AC1F1220040EC471F9F7B772B8B7BAEF97",
)
EXPECTED_PDF = (
    1_043_444,
...
EXPECTED_ZIP_MEMBERS = 137
EXPECTED_ZIP_MANIFEST = (
    18_714,
    "DAE6DBA07BF2437DD1A373944A3D510AD74A25E789F5A24A4BBD13A4430974AB",
)
EXPECTED_PREDECESSOR_RECEIPT = (
    40_958,
    "5C0B3089F1F35E3CB58952E4B1D98BCAECAB6DC8AC2431BB952807479B7FA4C0",
)
EXPECTED_OLD_CONTROLS = (
    19_367,
    "EBB9147F15AA0259DA5BAA8B3E7886D0B8744C7C7B79A76ABA75BC9C26245E7F",
)
...
DESCRIPTION = (
    ...
    "<p><strong>SGA7:</strong> SGA7 I has a complete 287-page English working "
    "reader for all written Exposes I, II, VI, VII, VIII, and IX. SGA7 II now "
    "has a direct 194-page English current-progress reader containing complete "
    "Exposes X-XVIII through Corollary 6.7 and the Expose XVIII bibliography, "
    "with its exact buildable TeX in one compact ZIP. The next cursor is the "
    "opening of Expose XIX; Exposes XIX-XXII are absent. French working "
...
def verify_package() -> dict[str, object]:
    ...
    if (
        ...
        or int(validation["reader"]["pages"]) != 194
        or int(validation["source"]["components"]) != 133
    ...
    if len(rows) != 144 or {row["path"] for row in rows} != set(represented):
    ...
    pdf = PACKAGE_ROOT / (
        "reader/SGA7II_English_Through_Expose_XVIII_Complete_20260801.pdf"
    )
    source_zip = PACKAGE_ROOT / (
        "SGA7II_English_Through_Expose_XVIII_Complete_"
        "Reader_and_TeX_20260801.zip"
    )
...
This successor adds a direct 194-page SGA7 II English current-progress reader
and one compact ZIP containing that reader plus its exact 133-component
buildable TeX closure.

The reader contains complete Exposes X-XVIII through Corollary 6.7 and the
Expose XVIII bibliography. The next cursor is the opening of Expose XIX;
Exposes XIX-XXII are absent. The reader is not represented as complete SGA7 II
...
def public_readback(
    ...
    result = {
        ...
        "source_zip_members": EXPECTED_ZIP_MEMBERS,
        "scope": (
            "SGA7 II Exposes X-XVIII complete through Corollary 6.7 and "
            "the Expose XVIII bibliography"
        ),
        "complete_sga7ii_claim": False,
        "exhaustive_reference_claim": False,
    ...
    (RECEIPT_ROOT / f"{RECEIPT_TAG}_record_{record_id}.md").write_text(
        "\n".join(
            [
                ...
                f"- Reader: 194 pages / SHA-256 `{EXPECTED_PDF[1]}`",
                f"- Reader/source ZIP: 137 members / SHA-256 `{EXPECTED_SOURCE_ZIP[1]}`",
                "- Scope: Exposes X-XVIII complete through Corollary 6.7 and the Expose XVIII bibliography",
                "- Next cursor Expose XIX; Exposes XIX-XXII absent; no exhaustive-reference claim",
