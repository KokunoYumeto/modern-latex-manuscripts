#!/usr/bin/env python3
"""Validate and package SGA7 II English through Expose XXI Section 4."""
...
PACKAGE_ROOT = REPO_ROOT / (
    "sources/sga/sga7ii-english-through-expose-xxi-4-20260802"
)
MASTER_REL = Path(
    "source/SGA7II_English_Through_Expose_XXI_4_20260802.tex"
)
PDF_REL = Path(
...
PUBLIC_PDF_NAME = (
    "00j_SGA7II_English_Through_Expose_XXI_4_"
    "Working_20260802.pdf"
)
PUBLIC_ZIP_NAME = (
...
def verify_source_closure() -> dict[str, object]:
    ...
    if inputs[-1] != "156_expose_XXI_section_4_and_bibliography":
        raise RuntimeError("Checkpoint continuation changed")
    if "157_expose" in text:
...
def build_and_compare() -> tuple[dict[str, object], str]:
    ...
    with tempfile.TemporaryDirectory(prefix="sga7ii-c156-") as directory:
        root = Path(directory)
        source = root / "source"
        ...
        if underfull_boxes != 3 or textcircled_warnings != 4:
            raise RuntimeError(
...
def main() -> int:
    ...
    validation = {
        ...
        "errors": [],
        "scope": {
            "included": "SGA 7 II Exposes X-XX complete and Expose XXI through Section 4 and bibliography",
            "excluded": "Expose XXI Section 5 appendix and Expose XXII",
            "continuation": "Expose XXI Section 5 appendix",
