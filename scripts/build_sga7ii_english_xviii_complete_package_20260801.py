#!/usr/bin/env python3
"""Validate and package the SGA7 II English reader through Expose XVIII."""
...
PACKAGE_ROOT = REPO_ROOT / (
    "sources/sga/sga7ii-english-through-expose-xviii-complete-20260801"
)
MASTER_REL = Path(
    "source/SGA7II_English_Through_Expose_XVIII_Complete_20260801.tex"
)
PDF_REL = Path(
...
PUBLIC_PDF_NAME = (
    "00j_SGA7II_English_Through_Expose_XVIII_Complete_"
...
def verify_source_closure() -> dict[str, object]:
    ...
    if inputs[-1] != "133_expose_XVIII_corollary_6_7_and_bibliography":
        raise RuntimeError("Checkpoint continuation changed")
    if "134_expose" in text:
...
def build_and_compare() -> tuple[dict[str, object], str]:
    ...
    with tempfile.TemporaryDirectory(prefix="sga7ii-c133-") as directory:
        root = Path(directory)
        source = root / "source"
...
def main() -> int:
    ...
    validation = {
        ...
        "errors": [],
        "scope": {
            "included": "SGA 7 II Exposes X-XVIII complete through Corollary 6.7 and the Expose XVIII bibliography",
            "excluded": "Exposes XIX-XXII",
            "continuation": "the opening of Expose XIX",
