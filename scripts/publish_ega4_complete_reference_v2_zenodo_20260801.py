#!/usr/bin/env python3
"""Publish the complete EGA IV reference-v2 same-concept successor."""
...
API = base.API
PREDECESSOR_RECORD = 21_738_394
PREDECESSOR_DOI = "10.5281/zenodo.21738394"
CONCEPT_DOI = "10.5281/zenodo.20414353"
PREDECESSOR_FILES = 40
PREDECESSOR_BYTES = 3_746_855_816
PREDECESSOR_FILE_AGGREGATE = (
    "FD12BBA44013E9774FDA3575E7F36B1466042BFA14C6EDA326717708B9D3E1E1"
)
FINAL_FILES = 40
FINAL_BYTES = 3_752_670_964
DEFAULT_PREVIEW = (
    "00a_EGA0_English_Complete_Through_Section13_Reference_v2_20260730.pdf"
)
VERSION = "2026-08-01 complete EGA IV reference-v2 reader"
PUBLICATION_DATE = "2026-08-01"
GITHUB_SOURCE_COMMIT = "81442d920e08dfe04e57666f3969c51a6cc62f66"
GITHUB_MERGE_COMMIT = "61d8455209e7c7401fbf9af7ec25b340ea89f5dc"
GITHUB_PR = 207

REPO_ROOT = Path(__file__).resolve().parent.parent
PACKAGE_ROOT = REPO_ROOT / "sources/ega/ega4-complete-reference-v2-r4-20260801"
BUNDLE_ROOT = REPO_ROOT / (
    "sources/ega/"
    "ega-current-readers-and-buildable-tex-bundle-reference-v2-20260801"
)
RECEIPT_ROOT = REPO_ROOT / "manifests/published-zenodo"
STATE_PATH = RECEIPT_ROOT / "20260801_ega4_reference_v2_zenodo_state.json"
TEMP_ROOT = Path(base.os.environ["LOCALAPPDATA"]) / "Temp" / (
    "ega4_reference_v2_zenodo_20260801"
)

OLD_NAMES = {
    "00 Current_EGA_English_Readers_and_Buildable_TeX_20260801.zip": (
        9_516_524,
        "d01eee6138fda234f8bdf22e1dba9325",
    ),
    "00e_EGAIV_English_Complete_Cumulative_Working_Reader_20260801.pdf": (
        4_157_515,
        "6c37c0177f5f3fb5e87ff0224a071fda",
    ),
    "01e_EGAIV_English_Complete_Cumulative_Master_20260801.tex": (
        1_182,
        "a4a94d036bd309425a5f7300438fa191",
    ),
    "02d_EGAIV_English_Complete_Cumulative_TeX_PDF_20260801.zip": (
        4_958_922,
        "9e70eb48a5fe1706c5cad35a6654e870",
    ),
}
...
LOCAL_SPECS = {
    "00 Current_EGA_English_Readers_and_Buildable_TeX_20260801.zip": {
        ...
        / "00 Current_EGA_English_Readers_and_Buildable_TeX_20260801.zip",
        "bytes": 9_618_706,
        "sha256": "E14482040B1A7A4D149FDF0FDD9167D0B63B78C47687A0F3C46129A62CB15D5E",
        "members": 135,
        "uncompressed_bytes": 16_599_559,
    },
    "00e_EGAIV_English_Complete_Reference_v2_Reader_20260801.pdf": {
        "path": PACKAGE_ROOT / "EGA4_English_Complete_Reference_v2_Reader_20260801.pdf",
        "bytes": 4_252_287,
        "sha256": "6087FD9475DBDE908EA2025326BC7A49AF33583C7047A7D9332648D2B6387C7A",
    },
    "01e_EGAIV_English_Complete_Reference_v2_Master_20260801.tex": {
        "path": PACKAGE_ROOT / "EGA4_English_Complete_Reference_v2_Master_20260801.tex",
        "bytes": 1_249,
        "sha256": "2540AEBDC9F3339516B783AE014F3A33F94746341DC503D9E501F4DA873A5FB0",
    },
    "02d_EGAIV_English_Complete_Reference_v2_TeX_PDF_QA_20260801.zip": {
        "path": PACKAGE_ROOT / "EGA4_English_Complete_Reference_v2_TeX_PDF_QA_20260801.zip",
        "bytes": 10_577_049,
        "sha256": "7372CD51D8106D807F407FED771573DB4CB16536CABF829671EEEDBFB269D676",
        "members": 84,
        "uncompressed_bytes": 38_155_078,
    },
}

DESCRIPTION_HTML = "\n".join(
    (
        "<p><strong>Read EGA:</strong> open any English reader PDF directly, "
        "or download <code>00 Current_EGA_English_Readers_and_Buildable_"
        "TeX_20260801.zip</code> for the current cumulative readers and their "
        "buildable TeX. EGA 0 remains the default preview.</p>",
        "<p><strong>Coverage:</strong> EGA 0 is complete through Section 13; "
        "EGA I and II are complete through their source endpoints; published "
        "EGA III is complete through 7.9.14; EGA IV is complete through "
        "Sections 1-21 and EOF, with 5,911 destinations and 7,374 resolved "
        "local links. Cross-volume references remain visible nonlinks.</p>",
...
def verify_local() -> dict[str, dict[str, object]]:
    ...
    for name, spec in LOCAL_SPECS.items():
        ...
        if observed != expected:
            raise RuntimeError(f"Local EGA IV release object changed: {name}")
        row = dict(spec)
        ...
        if name.lower().endswith(".zip"):
            ...
            if (
                ...
                or inventory["uncompressed_bytes"]
                != int(spec["uncompressed_bytes"])
            ):
                raise RuntimeError(f"Local EGA IV ZIP boundary changed: {name}")
            row["zip_inventory"] = inventory
        result[name] = row
    if sum(int(row["bytes"]) for row in result.values()) != 24_449_291:
        raise RuntimeError("Local EGA IV replacement byte boundary changed")
    return result
...
def fetch_predecessor(session) -> dict:
    ...
    for name, expected in OLD_NAMES.items():
        ...
        if row is None or (
            int(row["size"]), normalized_md5(row["checksum"])
        ) != expected:
            raise RuntimeError(f"Live EGA predecessor replacement base changed: {name}")
    latest = base.check(
        session.get(
...
def create_or_resume_draft(session, token: str, live: dict) -> int:
    if STATE_PATH.is_file():
        ...
        if active_draft(session, token, draft_id) is None:
            raise RuntimeError("Tracked EGA IV successor draft disappeared")
        return draft_id
...
def stage_and_publish(
    session, token: str, live: dict, draft_id: int, local: dict[str, dict]
) -> dict:
    if STATE_PATH.is_file():
        state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
        if state.get("published"):
    ...
    expected_draft_names = set(base.modern_entries(live))
    if set(files) != expected_draft_names:
        raise RuntimeError("Tracked EGA IV draft file set changed")
    ...
    retained_names = expected_draft_names - set(OLD_NAMES)
    if set(remaining) != retained_names or len(remaining) != 36:
        raise RuntimeError("EGA IV retained draft boundary changed")
    ...
    if (
        ...
        raise RuntimeError("Staged EGA IV successor boundary changed")
    ...
    for name, row in entries.items():
        ...
        if observed != expected:
            raise RuntimeError(f"Staged EGA IV identity mismatch: {name}")
    ...
    metadata["subjects"] = [
        ...
        {"subject": "complete EGA IV"},
        {"subject": "reference-v2"},
    ...
    if (
        ...
        raise RuntimeError("Patched EGA IV successor controls changed")
    ...
    if published["parent"]["pids"]["doi"]["identifier"] != CONCEPT_DOI:
        raise RuntimeError("Published EGA IV successor escaped the concept")
    state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
    state.update(
        {
...
def public_readback(
    ...
    for name in sorted(retained_names, key=str.casefold):
        ...
        retained[name] = {
            "bytes": observed[0],
            "md5": observed[1],
            "public_url": new["links"]["content"],
            "match_predecessor": True,
        }
    ...
    try:
        for index, (name, spec) in enumerate(local.items(), start=1):
            ...
            expected = (
                int(spec["bytes"]),
                str(spec["sha256"]).upper(),
                str(spec["md5"]).lower(),
            )
            if observed != expected:
                raise RuntimeError(f"Public EGA IV SHA-256 mismatch: {name}")
            replacements[name] = {
                "bytes": observed[0],
                "sha256": observed[1],
                "md5": observed[2],
    ...
    if (
        len(retained) != 36
        or len(replacements) != 4
        or sum(int(row["size"]) for row in entries.values()) != FINAL_BYTES
        or sum(int(row["members"]) for row in archives.values()) != 219
    ):
    ...
    result = {
        ...
        "description_bytes": len(DESCRIPTION_HTML.encode("utf-8")),
        "description_sha256": hashlib.sha256(
            DESCRIPTION_HTML.encode("utf-8")
        ).hexdigest().upper(),
        "additional_descriptions": len(
            record["metadata"].get("additional_descriptions", [])
        ),
        "github_pr": GITHUB_PR,
    ...
    receipt = RECEIPT_ROOT / (
        f"20260801_ega4_complete_reference_v2_record_{record_id}_public_readback.json"
    )
...
def readback_only() -> dict:
    if not STATE_PATH.is_file():
        raise RuntimeError("No tracked EGA IV successor state exists")
    state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
    if not state.get("published"):
        raise RuntimeError("Tracked EGA IV successor is not published")
    local = verify_local()
    token = base.find_token()
    session = base.make_session()
