#!/usr/bin/env python3
"""Build the corrected EGA IV Sections 19-21 source-aligned r6 release."""
...
ARCHIVE_NAME = "EGA4_Sections19_21_Part4_Backmatter_SourceAligned_R6_20260731"
ZIP_TIMESTAMP = (2026, 7, 31, 0, 0, 0)
HARNESS = "build_harness/ega4_sections19_21_source_aligned_successor_r1.tex"
PRODUCER_PDF = (
    "build_complete_with_backmatter_r6/"
...
EXPECTED = {
    ...
    "source/source_aligned/ega4-backmatter-notation.tex": (
        8167,
        "60AB7F2FFA02E3631944F6A448491EB2592543A8841811165A1FDBFEA1615E9D",
    ),
    ...
    "source/source_aligned/ega4-errata-addenda-list3.tex": (
        49862,
        "B25F197BFE9053AB10523DF2F26A57155BCB7C66A36C1B9B3ACB7BF824788B15",
    ),
    PRODUCER_PDF: (
        861684,
...
def main() -> int:
    ...
    reader_path = (
        destination
        / "reader/EGA4_Sections19_21_Part4_Backmatter_English_SourceAligned_R6_20260731.pdf"
    )
    ...
    if producer_facts["pages"] != 133 or producer_facts["text_empty_pages"]:
...
This corrected bounded source-aligned English package covers Sections 19-21 on printed
...
333-343 and 345-361. It contains the 133-page reader and its complete editable
TeX closure.
...
The authority scan and OCR are excluded from this compact source ZIP. The
actual publicly available scan-derived image witnesses used for the work are
preserved as pixels in separate archives on the same EGA concept, including
600/1800-dpi pages and 5000/9000-dpi detail crops through printed page 336.
They are not reader screenshots and are not represented by a ledger alone.
...
- Reader: 133 US-letter pages / {reader_path.stat().st_size} bytes / SHA-256 `{sha256_path(reader_path)}`.
...
- Fresh/producer page text and decoded content streams: exact on 133/133 pages.
...
- Producer reader: 133 pages / {reader_path.stat().st_size} bytes / SHA-256 `{sha256_path(reader_path)}`.
- Fresh copied-source build: four XeLaTeX passes, return codes 0/0/0/0.
- Pass 3/4 console convergence: {str(convergence).lower()}.
- Final fresh console SHA-256: `{final_console_sha}`.
- Hard TeX diagnostics: 0.
- Standalone undefined cross-volume hyperreference warnings: {undefined_hyperrefs}.
- Fresh and producer page text/content: exact on 133/133 pages.
...
""",
    ...
    validation = {
        ...
        "source_image_disposition": "actual scan-derived pixels already public in separate same-concept archives through printed page 336; not ledger-only and not duplicated in this compact source ZIP",
