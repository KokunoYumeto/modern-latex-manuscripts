#!/usr/bin/env python3
"""Publish and read back the SGA6 idx612-617 source-audit crop successor."""
...
PREDECESSOR_RECORD = 21629823
PREDECESSOR_DOI = "10.5281/zenodo.21629823"
SUCCESSOR_RECORD = 21630394
SUCCESSOR_DOI = "10.5281/zenodo.21630394"
VERSION = "2026-07-27 SGA6 ultra-detail source-audit crops idx612-617"
DEFAULT_PREVIEW = (
...
TEMP_ROOT = Path(
    os.environ.get(
        "SGA6_IDX612_617_ZENODO_STAGING_ROOT",
        Path(os.environ["LOCALAPPDATA"])
        / "Temp"
        / "sga6_idx612_617_zenodo_21630394",
    )
)
CONTROLS_ROOT = Path(
    os.environ.get(
        "SGA6_IDX612_617_ZENODO_CONTROLS_ROOT",
        TEMP_ROOT / "controls",
    )
)
ZIP_ROOT = Path(
    os.environ.get(
        "SGA6_IDX612_617_ZENODO_ZIP_ROOT",
        Path(os.environ["LOCALAPPDATA"])
        / "Temp"
        / "sga6_idx612_617_zenodo_stage_20260727",
    )
)
READBACK_ROOT = Path(
    os.environ.get(
        "SGA6_IDX612_617_ZENODO_READBACK_ROOT",
        Path(os.environ["LOCALAPPDATA"])
        / "Temp"
        / "sga6_idx612_617_zenodo_21630394_public_readback",
    )
)
...
LOCAL_FILES = {
    "09_README_CURRENT_RELEASE.md": (
        ...
        4_430,
        "6D30E84372ACBCDAA39E8F22559EA60C3CB7CCFCA7ABE517981BC9DE89F1E8D9",
    ),
    "09a_RELEASE_FILE_MANIFEST.csv": (
        CONTROLS_ROOT / "09a_RELEASE_FILE_MANIFEST.csv",
        25_498,
        "46CAD63566CB8B736F6F29E15DCEF0F9B749B4B4C665263DCF563846DCB9B06C",
    ),
    "09b_RELEASE_VALIDATION.json": (
        CONTROLS_ROOT / "09b_RELEASE_VALIDATION.json",
        5_683,
        "DB12D83B00629B37ADC791A6159296F7397C48DBE33422922556C7AFEEC300D5",
    ),
    "10x_SGA6_SourceAudit_Targeted_UltraDetail_Crops_idx612_617_20260727.zip": (
        ZIP_ROOT
        / "10x_SGA6_SourceAudit_Targeted_UltraDetail_Crops_"
        "idx612_617_20260727.zip",
        409_098,
    ...
    "10y_SGA6_SourceAudit_Crop_Provenance_RightsBlocked_Metadata_idx612_617_20260727.zip": (
        ...
        "Metadata_idx612_617_20260727.zip",
        47_036,
        "F58EFEED6C6152F5F616A86191BD95480FEAA96740B5DF61B28C6F5D8879465A",
    ),
}

EXPECTED_OUTER_FILES = 71
EXPECTED_OUTER_BYTES = 336_773_391
EXPECTED_RETAINED_FILES = 66
EXPECTED_ZIP_ARCHIVES = 43
EXPECTED_ZIP_FILE_MEMBERS = 3_711
EXPECTED_ZIP_DIRECTORY_ENTRIES = 7
EXPECTED_ZIP_ALL_ENTRIES = 3_718
EXPECTED_ZIP_UNCOMPRESSED_BYTES = 384_983_055
RELEASE_CONTROL_COMPATIBILITY_COUNTER = 3_717
GITHUB_COMMIT = "8dc0b67398e6d1518704e3563b263109c4be4a8f"
GITHUB_PACKAGE = (
    "sources/sga/"
    "sga6-ultradetail-source-audit-crops-coldreverify-idx612-617-20260727"
)

DESCRIPTION_PARAGRAPHS = [
    (
        "This same-concept compact successor preserves the reader-first SGA "
        "surface from version 10.5281/zenodo.21629823. Sixty-six predecessor "
    ...
    (
        "The targeted archive contains four tight formula, glyph, prose, and "
        "locator crops actually opened during Claude's SGA6 cold source "
        "re-verification. They map parent indices 612-617 to the stable "
        "six-entry audit boundary #1364-#1369 and span Expose XII Section 2 "
        "reductions through the opening of Oort Proposition 3.1, items i-iii; "
        "the continuation cursor is idx618. Every crop records parent "
        "identity, source page, pixel bounding box, dimensions, scaling, "
        "generator identity, linked audit entry, target context, and QA "
        "disposition."
    ),
    (
        "The paired provenance archive records 30 routine full-width page "
        "bands and two generated-but-unread alternative tight crops as "
    ...
    (
        ...
        "series, 2,183 selected images are public and 3,253 routine page "
    ...
    (
        ...
        "VIII, and IX. Expose VII remains held for a machine-evidence repair; "
        "Expose XI has passed the high-zoom diagram gate but still needs a "
        "sealed public projection; Exposes X and XII-XXVI remain absent. This "
...
NOTES_TEXT = (
    ...
    "This version has 71 files and 43 ZIP archives. Literal archive replay "
    "contains 3,711 non-directory file members plus seven directory records "
    "(3,718 ZIP records total), totaling 384,983,055 uncompressed bytes. The "
    "release-control lineage compatibility counter is 3,717: the predecessor's "
    "3,702 counter plus 15 new non-directory members. SGA1 remains the "
...
def predecessor_identities() -> dict[str, dict]:
    ...
    if len(rows) != 69:
        raise RuntimeError("Predecessor receipt does not contain 69 files")
    result = {
        row["filename"]: {
            "bytes": int(row["bytes"]),
...
def predecessor_public_file_map(session: requests.Session) -> dict[str, dict]:
    ...
    if len(rows) != 69 or len(result) != 69:
...
def stage_files(session: requests.Session, token: str) -> dict:
    ...
    save_json(
        RECEIPT_ROOT
        / "20260727_sga6_idx612_617_record_21630394_draft_files.json",
        receipt,
    )
    return receipt
...
def inspect_preflight(session: requests.Session, token: str) -> dict:
    ...
    save_json(
        RECEIPT_ROOT
        / "20260727_sga6_idx612_617_record_21630394_preflight.json",
        receipt,
    )
    return receipt
...
def patch_and_publish(session: requests.Session, token: str) -> dict:
    ...
    save_json(
        RECEIPT_ROOT
        / "20260727_sga6_idx612_617_record_21630394_draft_patch.json",
        patch_receipt,
    )
...
def anonymous_readback(
    ...
    zip_receipt = {
        ...
        "compatibility_counter_note": (
            ...
            "3,702 counter and adds 15 new non-directory members. Literal "
            "replay has 3,711 non-directory files plus seven directory records, "
            "or 3,718 total ZIP records."
        ),
    ...
    receipt = {
        ...
        "metadata": {
            ...
            "description_exact": True,
            "notes_exact": True,
            "scope": "SGA6 source-audit crops idx612-617",
            "selected_opened_crops": 4,
            "routine_page_bands_metadata_only": 30,
            "generated_unread_tight_crops_metadata_only": 2,
            "stable_audit_entries": "1364-1369",
        },
        "zip_archive_count": len(zip_summaries),
        "zip_file_member_count": len(zip_members),
        "zip_directory_entry_count": directory_entries,
