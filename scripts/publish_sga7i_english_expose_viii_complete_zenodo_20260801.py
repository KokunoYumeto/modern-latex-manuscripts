#!/usr/bin/env python3
"""Publish and read back SGA7 I English through complete Expose VIII."""
...
PREDECESSOR_RECORD = 21_728_674
PREDECESSOR_DOI = "10.5281/zenodo.21728674"
PREDECESSOR_FILES = 83
PREDECESSOR_BYTES = 677_586_021
FINAL_FILES = 83
DEFAULT_PREVIEW = "00a_SGA1_English_Reader.pdf"
OLD_CONTROLS_NAME = "10z_SGA_Current_Release_Controls_20260731.zip"
CONTROLS_NAME = "10z_SGA_Current_Release_Controls_20260801.zip"
VERSION = "2026-08-01 SGA7 I English through complete Expose VIII"

REPO_ROOT = Path(__file__).resolve().parents[1]
PACKAGE_REL = Path(
    "sources/sga/"
    "sga7i-english-source-first-working-through-expose-viii-complete-20260801"
)
PACKAGE_ROOT = REPO_ROOT / PACKAGE_REL
TEMP_ROOT = REPO_ROOT / "tmp/zenodo/sga7i-english-viii-complete-20260801"
...
RECEIPT_TAG = "20260801_sga7i_english_through_expose_viii_complete"
PREDECESSOR_RECEIPT = RECEIPT_ROOT / (
    "20260801_sga7i_english_through_expose_viii_3_7_"
    "record_21728674_public_readback.json"
)
GITHUB_PACKAGE_COMMIT = "f627b23010e44f75877152efc99388d3c82821fa"

OLD_PDF_NAME = (
    "00i_SGA7I_English_Working_Through_Expose_VIII_3_7_20260731.pdf"
)
OLD_TEX_NAME = (
    "02i_SGA7I_English_Working_Through_Expose_VIII_3_7_20260731.tex"
)
OLD_SOURCE_ZIP_NAME = (
    "10i_SGA7I_English_Working_Through_Expose_VIII_3_7_"
    "Reader_and_TeX_20260731.zip"
)
PDF_NAME = "00i_SGA7I_English_Working_Through_Expose_VIII_Complete_20260801.pdf"
TEX_NAME = "02i_SGA7I_English_Working_Through_Expose_VIII_Complete_20260801.tex"
SOURCE_ZIP_NAME = (
...
EXPECTED_PACKAGE_FILES = 137
EXPECTED_PACKAGE_BYTES = 3_692_531
EXPECTED_MANIFEST = (
    17_860,
    "E5614A6A964DDFEED690A2DD6F36DC1E102251C4830B177350BF4FA80BE71594",
)
EXPECTED_VALIDATION = (
    4_371,
    "EC085BBB22F147D8888E6A6759E31AAA5A9CABEC765DD5F703B6714D26FE7B5B",
)
EXPECTED_PDF = (
    1_433_416,
...
EXPECTED_PREDECESSOR_RECEIPT = (
    3_610,
    "83256192AF38FE4294E45CC77F043FA9B4241E2C5BC443998D7D44A9677CE2DE",
)
...
DESCRIPTION_ADDITION = (
    ...
    "and a 181-page SGA7 I English working reader containing complete Exposes "
    "I, II, VI, VII, and VIII. Its compact 131-member archive contains that "
    "reader and the exact 127-component buildable TeX closure. The next English "
    "cursor is Expose IX, Introduction 0.1, authority line 5, scan index 324, "
    "source folio 313.</p>"
...
def verify_package() -> dict[str, object]:
    ...
    if (
        ...
        raise RuntimeError("SGA7 I complete-Expose-VIII package boundary changed")
    ...
    if len(rows) != 136 or {row["path"] for row in rows} != set(represented):
    ...
    source_zip = PACKAGE_ROOT / (
        "SGA7I_English_Working_Through_Expose_VIII_Complete_"
        "Reader_and_TeX_20260801.zip"
    )
...
def local_uploads() -> dict[str, dict[str, object]]:
    ...
    paths = {
        ...
        / "reader/SGA7I_English_Working_Through_Expose_VIII_Complete_20260801.pdf",
        TEX_NAME: PACKAGE_ROOT
        / "source/SGA7I_English_Working_Through_Expose_VIII_Complete_20260801.tex",
...
def load_predecessor() -> dict[str, object]:
    ...
    if (
        ...
        or int(receipt.get("public_surface", {}).get("files", -1)) != PREDECESSOR_FILES
        or int(receipt.get("public_surface", {}).get("bytes", -1)) != PREDECESSOR_BYTES
    ...
    controls_row = receipt["new_files"].get(OLD_CONTROLS_NAME)
...
preview. This successor adds a bounded 181-page SGA7 I English reader
containing complete Exposes I, II, VI, VII, and VIII.

The exact continuation is Expose IX, Introduction 0.1, authority line 5,
zero-based scan index 324, source folio 313. Expose IX and later are absent.
...
""",
    ...
    base.save_json(
        CONTROLS_ROOT / "09b_RELEASE_VALIDATION.json",
        {
            ...
            "reader_pages": 181,
            "editable_tex_files": 128,
            "component_files": 127,
            "continuation": {
                "unit": "Expose IX, Introduction 0.1",
                "authority_line": 5,
                "scan_index_zero_based": 324,
                "source_folio": 313,
                "physical_pdf_page": 325,
            },
            "source_zip_members": 131,
...
def public_readback(
    ...
    if (
        ...
        or retained_errors
        or int(archives[SOURCE_ZIP_NAME]["members"]) != 131
    ):
    ...
    (RECEIPT_ROOT / f"{RECEIPT_TAG}_record_{record_id}.md").write_text(
        "\n".join(
            [
                "# SGA7 I English through complete Expose VIII publication receipt",
                "",
                ...
                f"- Reader/source ZIP: 131 members / `{EXPECTED_SOURCE_ZIP[1]}`",
                "- Scope: Exposes I, II, VI, VII, and VIII complete",
                "- Continuation: Expose IX Introduction 0.1 / authority line 5 / scan index 324 / source folio 313",
