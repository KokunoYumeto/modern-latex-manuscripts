#!/usr/bin/env python3
"""Freeze and validate the SGA7 I English complete-Expose-VIII checkpoint."""
...
WORKING_PDF = WORKING_ROOT / (
    "build/checkpoint_expose_VIII_complete_r1/"
...
EXPECTED_REBUILD = (
    1_433_416,
    "54A273C0D3A4A5BA7BD95D7B7D6156D14ED4D93BBFCAAEF73ED9D2CC24A0E92B",
)
EXPECTED_BUILD_LOG = (
    41_887,
    "469DBCBFFCEEC9F8E1CEEEE44EFBF8054D8E29FFC2FBA223B3C07494C6326683",
)
...
VISUAL_RENDER_SHA256 = {
    "seam-134.png": "6AD893B2D9AA9631FD5B7BB64AD2D0A66001050EDDD2100098D1FB361361FF20",
    "seam-135.png": "DB453A3211A4466DBB4F7D245D08DC66BBDDE7429B8998A0E29F6C9091BD83A9",
    "terminal-180.png": "16B62C75A3749AEDF97439345750E6538670CD961A924E892D6130637C72C1F7",
    "terminal-181.png": "D7E0E16D50361C61A348BB77E9BD6602180A64798AC7022254031C72FF3F6514",
}
PRIVACY_PATTERNS = {
...
def copy_frozen_source(frozen_root: Path) -> dict[str, object]:
    ...
    if inputs[-1] != "127_expose_VIII_proposition_7_4_corollary_7_5_and_bibliography":
        raise RuntimeError("Complete-Expose-VIII cursor changed")
    if "128_expose" in master_text or "expose_IX" in master_text:
        raise RuntimeError("Master crossed the frozen Expose IX cursor")
    ...
    if live_components != frozen_components:
        raise RuntimeError("Live components 1-127 diverged from the frozen snapshot")
...
def inspect_and_compare_pdfs(rebuild_pdf: Path) -> tuple[dict[str, object], str]:
    ...
    if len(public.pages) != 181 or len(rebuild.pages) != 181:
    ...
    for original, fresh in zip(public.pages, rebuild.pages, strict=True):
        ...
        if page_stream(original) == page_stream(fresh):
            streams += 1
    if (geometry, streams, texts, text_pages) != (181, 181, 181, 181):
...
def verify_visuals(render_root: Path) -> dict[str, object]:
    ...
    return {
        "render_dpi": 600,
        "directly_inspected_pages": [134, 135, 180, 181],
        "render_sha256": VISUAL_RENDER_SHA256,
        "checked_boundaries": [
            "Expose VII bibliography and end",
            "Expose VIII opening",
            "Expose VIII Proposition 7.4 through Corollary 7.5",
            "Expose VIII terminal bibliography and page number",
        ],
        "clipping_overlap_blank_or_malformed_content_errors": 0,
    }


README = """# SGA 7 I English working reader through complete Expose VIII

This current-progress reader contains complete English Exposes I, II, VI,
VII, and VIII. It covers source folios 1-312 of the 528-folio SGA 7 I scan.
The exact continuation is Expose IX, Introduction 0.1, authority
`source/expose_IX_body.tex` line 5, zero-based scan index 324 / source folio
313 / physical PDF page 325.
...
- `reader/SGA7I_English_Working_Through_Expose_VIII_Complete_20260801.pdf`
  is the 181-page A4 reader.
- `source/SGA7I_English_Working_Through_Expose_VIII_Complete_20260801.tex`
  is the exact frozen wrapper used for that reader.
- `source/components/` contains the exact 127 editable components used by the
  wrapper.
- `SGA7I_English_Working_Through_Expose_VIII_Complete_Reader_and_TeX_20260801.zip`
...
decoded page-content streams, and extracted text of all 181 pages exactly.
Pages 134-135 and 180-181 were rendered at 600 dpi and inspected directly;
the Expose VII/VIII join and terminal Expose VIII pages are clean.
"""
...
VII, and VIII are project English translations. Source-image review was used
...
formula, theorem statement, page map, or continuation cursor. The current
complete-Expose-VIII reader retains the corrected reading.
"""
...
def write_zip() -> dict[str, object]:
    ...
    with zipfile.ZipFile(zip_path) as archive:
        ...
        if len(names) != 131 or len(names) != len(set(names)):
...
def main() -> int:
    ...
    rebuild_pdf = rebuild_root / "SGA7I_English_Working_Through_Expose_VIII_Complete_20260801.pdf"
    reader, reader_text = inspect_and_compare_pdfs(rebuild_pdf)
    build = verify_build(
        rebuild_root / "SGA7I_English_Working_Through_Expose_VIII_Complete_20260801.log",
    ...
    validation = {
        ...
        "errors": [],
        "scope": {
            "included": "SGA 7 I Exposes I, II, VI, VII, and VIII complete",
            "excluded": "Expose IX and later",
            "source_folios_included": "1-312",
            "source_folios_total": 528,
            "source_folio_coverage_percent": 59.09,
            "continuation": {
