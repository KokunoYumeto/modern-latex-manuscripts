#!/usr/bin/env python3
"""Publish and read back the July 28 workflow methodology successor."""
...
PREDECESSOR_DOI = "10.5281/zenodo.21424987"
PUBLICATION_DATE = "2026-07-28"
VERSION = (
    "2026-07-28 source-image adjudication, disjoint sessions, "
...
GITHUB_COMMIT = "fb66709a0762df6f091bc145fea99704ec315e4b"
GITHUB_PACKAGE = (
...
GITHUB_RECEIPT = (
    ...
    / "manifests"
    / "published-github"
    / "20260728_workflow_methodology_commit_fb66709a_public_readback.json"
)
TOKEN_LOG = Path(
    os.environ.get(
        "ZENODO_TOKEN_LOG",
...
DRAFT_STATE = (
    RECEIPT_ROOT
    / "20260728_workflow_methodology_zenodo_draft_state.json"
)
READBACK_ROOT = Path(
    os.environ.get(
        ...
        / "Temp"
        / "workflow_methodology_zenodo_public_readback",
    )
)

PREDECESSOR_FILES = {
    "00_AI_Run_Modern_LaTeX_Manuscript_Workflow_Current_20260718.md": (
        14_047,
        "bfdf2d992268ff0758ffe9ded4e86337",
    ),
    "00_AI_Run_Modern_LaTeX_Manuscript_Workflow_Current_20260718.pdf": (
        209_004,
        "648f3953920965d4e675358980045182",
    ),
    RETAINED_ADDENDA: (
        209_743,
        "91cf57e6f0fc60116e5115cca91094e7",
    ),
    "02_Workflow_Current_Method_Gates_and_Publication_20260718.zip": (
        65_731,
        "238541330bbfec07367197934f6858a5",
    ),
    "02_Workflow_Method_Refresh_public_manifest_20260718.csv": (
        735,
        "b3a8714f4b14ed6c21468159dba8fab1",
    ),
    "02_Workflow_Method_Refresh_public_sha256_20260718.csv": (
        708,
        "f21ba2a74ec75887d218fe96c349ca3e",
    ),
    "99_Workflow_Public_Status_20260718.md": (
        735,
        "193ae699794ad316a44cf222ea6a9994",
    ),
}
REPLACED_FILES = set(PREDECESSOR_FILES) - {RETAINED_ADDENDA}

EXPECTED_FINAL_FILES = 9
EXPECTED_RETAINED_FILES = 1
EXPECTED_ZIP_ARCHIVES = 2
EXPECTED_ZIP_FILE_MEMBERS = 27
EXPECTED_ZIP_DIRECTORY_ENTRIES = 0
EXPECTED_ZIP_UNCOMPRESSED_BYTES = 829_112

DESCRIPTION_PARAGRAPHS = [
    (
        "This same-concept successor publishes the July 28 update to the "
        "AI-run Modern LaTeX Manuscript Workflow. The public surface keeps the "
        "current PDF and Markdown directly accessible, co-publishes the exact "
        "Claude cold-reverify method and the exact SGA resource-efficiency "
        "incident note, groups the current files into one source packet, and "
        "retains the July 6 addenda/scripts ZIP byte-identically."
    ),
    (
    ...
    (
        ...
        "for five overlapping 2400-dpi bands per page, targeted 6500-9000-dpi "
        "crops for real ambiguities, and node-by-node and edge-by-edge diagram "
        "review. Contact sheets are navigation aids, not PASS evidence."
    ),
    (
...
NOTES_HTML = (
    "<p>Compact nine-file workflow surface. The six-page A4 PDF is the default "
    "preview. The source packet has six exact members; the historical July 6 "
    "addenda/scripts ZIP is retained byte-identically. GitHub package commit: "
    f"{GITHUB_COMMIT}.</p>"
)
...
def expected_identities(predecessor: dict) -> tuple[dict, dict]:
    ...
    if len(rows) != 7:
        raise RuntimeError("Expected seven outer-manifest rows")
    expected = {}
    for row in rows:
        name = row["filename"]
        expected[name] = {
    ...
    if len(expected) != EXPECTED_FINAL_FILES:
        raise RuntimeError("Expected nine final workflow files")
...
def stage(
    ...
    save_json(
        RECEIPT_ROOT
        / f"20260728_workflow_methodology_record_{draft_id}_draft_files.json",
        result,
    )
    return result
...
def publish(
    ...
    save_json(
        RECEIPT_ROOT
        / f"20260728_workflow_methodology_record_{draft_id}_publish_response.json",
        result,
    )
    return result
...
def readback(record_id: int, expected: dict) -> tuple[dict, dict]:
    ...
    save_json(
        RECEIPT_ROOT
        / f"20260728_workflow_methodology_record_{record_id}_public_readback.json",
        public_receipt,
    )
    save_json(
        RECEIPT_ROOT
        / f"20260728_workflow_methodology_record_{record_id}_zip_member_readback.json",
        zip_receipt,
    )
    shutil.rmtree(READBACK_ROOT)
