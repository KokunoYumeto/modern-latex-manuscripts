#!/usr/bin/env python3
"""Validate and package the SGA7 II English reader through Expose XIX."""
...
PACKAGE_ROOT = REPO_ROOT / (
    "sources/sga/sga7ii-english-through-expose-xix-complete-20260801"
)
MASTER_REL = Path(
    "source/SGA7II_English_Through_Expose_XIX_Complete_20260801.tex"
)
PDF_REL = Path(
...
PUBLIC_PDF_NAME = (
    "00j_SGA7II_English_Through_Expose_XIX_Complete_"
...
def verify_source_closure() -> dict[str, object]:
    ...
    if inputs[-1] != "138_expose_XIX_section_5_and_bibliography":
        raise RuntimeError("Checkpoint continuation changed")
    if "139_expose" in text:
...
def build_and_compare() -> tuple[dict[str, object], str]:
    ...
    with tempfile.TemporaryDirectory(prefix="sga7ii-c138-") as directory:
        root = Path(directory)
        source = root / "source"
...
def main() -> int:
    ...
    validation = {
        ...
        "errors": [],
        "scope": {
            "included": "SGA 7 II Exposes X-XIX complete through the Expose XIX bibliography",
            "excluded": "Exposes XX-XXII",
            "continuation": "the opening of Expose XX",
