...
A July 2 Weber/SGA status refresh adds another public-archive lesson. Source-audit ledgers should report exact current cursors and the difference between a live local workpass and a promoted reader package. SGA5's ledger reached p391 while the compact promoted delta still stopped at p260-p265; Weber's local Phase 2 German workpass reached §§173-179 and 403 compiled pages while remaining non-English-synchronized and non-critical. Agent assistance was useful for locating severe failures, but Weber also showed hallucinated phantom-page fixes, so the safe pattern remains: locate, inspect the actual scan, apply only source-confirmed edits, compile, render-check, and publish the caveat with the status.
