...
Current local lesson, 2026-06-25/26: a Claude/Codex Lean side lane now contains small classical targets that build in the local Mathlib tree, including Jordan affine-line group cardinality, Jordan primitive-root count, a Steinitz perfect-field/Frobenius criterion, and a Weber cubic polynomial identity. These are useful Lean/mathlib-style candidate additions. They are not a check on the corresponding scans, translations, diagrams, page order, or full papers. The lane also exposed an inventory trap: Lean target selection should be anchored to the public repo catalog or a coherent mathematical motivation, not to loose local scan-folder searches.
