...
A June 25 Weber adaptation shows how that method changes when the starting TeX is not a faithful transcription. The corrected priority is German transcription first: audit for omitted lines, compressed prose, missing formulae, wrong symbols, and occasional over-added editorial completions before treating English translation as a production axis. The local source measurement helper found no native Weber scan at or above the 650dpi trust line; the best on-disk sources are about 500-560dpi, so hard formula checks need tight crops and explicit uncertainty flags. The initial Weber method packet is mirrored in `workflow/AI_Run_Workflow_Weber_FindVerifyFix_Audit_Method_20260625.zip`, and a newer live method snapshot is mirrored in `workflow/AI_Run_Workflow_Weber_LiveAudit_Method_p100_p527_20260625.zip`. The live log now records a German Vol. I audit cursor through p527 and explicitly held ranges including section 69, section 138 numbering/layout, section 141 reducibility paraphrase, p466, sections 148-149, section 153, section 154, section 155, section 156, and section 158. It also adds method lessons on whole-section holds, fabrication detection, over-expansion detection, line-wrap macro corruption, phantom no-scan hallucinations, printed-erratum discipline, and page-count compile gates as evidence of content restoration or swallowing. A later B143 package adds a sharper guardrail: a held or no-compile range must not suppress rendering/scanning for the next range. An anomalously fast "all clean" result after a held batch should be treated as a likely missing-scan/tooling failure until page images, crops, output PDF renders, and compile logs prove otherwise. It deliberately excludes bulky source chunks; those should be generated on demand from the page manifest.
...
## Lean Formalization / Library Notes

A Lean/Lake toolchain is useful as a selective formalization and library-growth lane, not as a bulk transcription verifier and not as certification of a scanned edition. The practical path is to create small Lean packets for stable theorem statements, definitions, examples, algebraic identities, or calculation lemmas that are useful in their own right and may later become mathlib-adjacent additions.
