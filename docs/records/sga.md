...
Current caveat from the 2026-06-24 SGA audit-support updates, with 2026-06-26 hand-audit reconciliation: the latest published record is DOI `10.5281/zenodo.20822648`. SGA5 is explicitly not certified complete. The current compact local status package is `SGA5_FullAudit_Workpass_Status_p001_p220_20260626.zip`: it preserves the current French workpass PDF/TeX/log plus CERT_LOG, AGENT_SCORECARD, method note, goal snapshot, and file manifest. CERT_LOG/AGENT_SCORECARD are the freshest controlling status files; STATUS.md is stale. Parsed completed rows reach p220, while p221 is only the next cursor and is not promoted as complete. The included workpass compiles to a 307-page PDF. The p190, p194, p197, p199, p205, and p208 packages remain provenance only and are superseded as the current cursor. Substantial source reconciliation remains, English remains unsynchronized, and risk lanes remain functor order (`R^! f` versus `R f^!`), underlines/object operators, O versus underlined O, Hom variants, `D^-`, `c\ell`/`cl`, guillemets, diagram index drift, and per-expose notation shifts. Expose III B is locally hand-audited through its bibliography in this workpass; Expose V opens at p204; p209-p220 cover Proposition 2.2.2 through Expose V section 3.1 Proposition 3.1.3, including the copied-source repairs at p209/p214, diagram checks D113-D131, Expose V section 2 closure at p219, and the p220 snake-lemma diagram; next work starts at p221. The p69 scope note clarifies that the source scan is the full 484-page LNM 589, but the local TeX is a curated 10-expose selection (I, III, IIIB, V, VI, VII, VIII, X, XII, XV); current p1-p220 evidence is opening-range workpass evidence, not global SGA5 certification. This is preservation/support evidence for ongoing repair, not a certified edition, not an independently packaged or promoted edition, not English synchronization, and not closure of diagram/formula/notation/typography queues. Treat SGA6 and SGA7 as substantial working drafts with explicit compression caveats unless a specific packet declares source-checked coverage. Older filenames containing words such as `Complete`, `Source-Checked`, `Strict`, or `High-Fidelity` are legacy package names, not current global certification. Witness-aid ZIPs are source-witness/anchor aids, not authority by themselves.
