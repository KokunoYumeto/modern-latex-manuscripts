...
Current caveat from the 2026-06-24 SGA audit-support updates, with 2026-06-25 local workpass reconciliation: the latest published record is DOI `10.5281/zenodo.20822648`. SGA5 is explicitly not complete and should be assumed to contain many remaining errors unless a later, narrower source-audit packet promotes a specific locus. The public surface carries `SGA5_repair032_audit_support_20260624.zip` plus later local workpass evidence. The live SGA5 status file reports 411 verified corrections, the careful p75-p484 main-body audit complete after manual p75-p101, workflow p102-p374, and the p375-p484 back-third pass, and a later workflow batch for p3-p32. Open areas still include p1-p2, p33-p74, deferred p344/p346-p347 diagram/prose-pass items and other deferred prose/notation items, current LaTeX error diagnostics in the latest workpass log despite a produced 306-page PDF, final compile/diff/FINDINGS refresh, remaining uncertain/type-B review as applicable, and English synchronization. This is preservation/support evidence for ongoing repair, not a certified edition, not a clean compiled edition, not English synchronization, and not closure of diagram/formula/notation/typography queues. Treat SGA6 and SGA7 as substantial working drafts with explicit compression caveats unless a specific packet declares source-checked coverage. Older filenames containing words such as `Complete`, `Source-Checked`, `Strict`, or `High-Fidelity` are legacy package names, not current global certification. Witness-aid ZIPs are source-witness/anchor aids, not authority by themselves.
