...
Current caveat from the 2026-06-24 SGA audit-support updates, with 2026-06-25 hand-certification reconciliation: the latest published record is DOI `10.5281/zenodo.20822648`. SGA5 is explicitly not certified complete. A newer manual certification log demotes agent/workflow passes to finder status rather than certification status: pp.1-90 are page-locally certified; pp.1-72 close Expose I (Verdier main text §§1-5, Bibliographie, and Illusie Appendix §§1-7; logged fixes include source-fidelity, twist-sign, underline, source-typo, shriek-functor, xref, and notation repairs; D001-D013 are hand-verified perfect; Type-B/source-or-editorial conventions, diagram-index page drift, and the open cross-document R^!f / R f^! notation-normalization item remain documented risks). Pages 73-90 cover Expose III through the end of section 2 and into section 3.2 (Corollary 2.6.1, the type-(3,4) case, Lemme 2.6.3, Proposition 2.5, the fundamental correspondence isomorphism, support correspondences, and the first trace example), with D014-D027 hand-verified perfect through the Expose III section 3.1 projection diagram; p91 is next. The broader local full-audit workpass remains repair evidence and reports 411 verified corrections, a p1-p484 page-by-page French source audit in the live status note, and a 307-page PDF with no fatal/LaTeX-error diagnostics in the latest observed log scan. Remaining publication/audit work includes final diff/FINDINGS refresh, compact packaging, independent validation, optional low-value prose-fidelity review where desired, continued page-by-page hand certification, and English synchronization. The p69 scope note clarifies that the source scan is the full 484-page LNM 589, but the local TeX is a curated 10-expose selection (I, III, IIIB, V, VI, VII, VIII, X, XII, XV); current pp.1-90 certification is opening-range evidence, not global SGA5 certification. This is preservation/support evidence for ongoing repair, not a certified edition, not an independently packaged or promoted edition, not English synchronization, and not closure of diagram/formula/notation/typography queues. Treat SGA6 and SGA7 as substantial working drafts with explicit compression caveats unless a specific packet declares source-checked coverage. Older filenames containing words such as `Complete`, `Source-Checked`, `Strict`, or `High-Fidelity` are legacy package names, not current global certification. Witness-aid ZIPs are source-witness/anchor aids, not authority by themselves.
