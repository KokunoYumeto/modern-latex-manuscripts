...
Current caveat from the 2026-06-24 SGA audit-support updates, with 2026-06-26 hand-audit reconciliation: the latest published record is DOI `10.5281/zenodo.20822648`. SGA5 is explicitly not certified complete. The current compact local status package is `SGA5_FullAudit_Workpass_Status_p001_p205_20260626.zip`: it preserves the current French workpass TeX/PDF/log plus CERT_LOG, AGENT_SCORECARD, FINDINGS, method/source/index files, README, and checksums. CERT_LOG/AGENT_SCORECARD are the freshest controlling status files; STATUS.md is stale. Parsed completed rows reach p205, while p206 is only the next cursor and is not promoted as complete. The included workpass compiles to a 307-page PDF. The p190, p194, p197, and p199 packages remain provenance only and are superseded as the current cursor. Substantial source reconciliation remains, English remains unsynchronized, and risk lanes remain functor order (`R^! f` versus `R f^!`), underlines/object operators, O versus underlined O, Hom variants, `D^-`, `c\ell`/`cl`, guillemets, diagram index drift, and per-expose notation shifts. Expose III B is locally hand-audited through its bibliography in this workpass, Expose V opens at p204, and p205 is checked; next work starts at p206 with the Mittag-Leffler-Artin-Rees/projective-system machinery. The p69 scope note clarifies that the source scan is the full 484-page LNM 589, but the local TeX is a curated 10-expose selection (I, III, IIIB, V, VI, VII, VIII, X, XII, XV); current p1-p205 evidence is opening-range workpass evidence, not global SGA5 certification. This is preservation/support evidence for ongoing repair, not a certified edition, not an independently packaged or promoted edition, not English synchronization, and not closure of diagram/formula/notation/typography queues. Treat SGA6 and SGA7 as substantial working drafts with explicit compression caveats unless a specific packet declares source-checked coverage. Older filenames containing words such as `Complete`, `Source-Checked`, `Strict`, or `High-Fidelity` are legacy package names, not current global certification. Witness-aid ZIPs are source-witness/anchor aids, not authority by themselves.
