...
Current caveat from the 2026-06-24 SGA audit-support updates, with 2026-06-26 hand-audit reconciliation: the latest published record is DOI `10.5281/zenodo.20822648`. SGA5 is explicitly not certified complete. The current compact local status package is `SGA5_FullAudit_Workpass_Status_p001_p197_20260626.zip`: it preserves the current French workpass TeX/PDF/log plus CERT_LOG, AGENT_SCORECARD, FINDINGS, STATUS, goal note, README, and checksums. CERT_LOG/AGENT_SCORECARD are the freshest controlling status files; STATUS.md is stale. Parsed completed rows in CERT_LOG reach source/page row p197, while p198 is only the next cursor and is not promoted as complete. The included workpass compiles to a 307-page PDF. The p190 and p194 packages remain provenance only and are superseded as the current cursor. More than two hundred pages still need reconciliation/certification, English remains unsynchronized, and risk lanes remain functor order (`R^! f` versus `R f^!`), underlines/object operators, O versus underlined O, Hom variants, `D^-`, `c\ell`/`cl`, guillemets, and diagram index drift. The next work should continue at p198, especially Proposition 6.19 and the section 6.18/6.19 norm/Tate criterion around D106, `N_G`, `d_1^*`/`d_2^!`, and `uRHom_{\\Lambda[G]}(\\Lambda,-)`. The p69 scope note clarifies that the source scan is the full 484-page LNM 589, but the local TeX is a curated 10-expose selection (I, III, IIIB, V, VI, VII, VIII, X, XII, XV); current p1-p197 evidence is opening-range workpass evidence, not global SGA5 certification. This is preservation/support evidence for ongoing repair, not a certified edition, not an independently packaged or promoted edition, not English synchronization, and not closure of diagram/formula/notation/typography queues. Treat SGA6 and SGA7 as substantial working drafts with explicit compression caveats unless a specific packet declares source-checked coverage. Older filenames containing words such as `Complete`, `Source-Checked`, `Strict`, or `High-Fidelity` are legacy package names, not current global certification. Witness-aid ZIPs are source-witness/anchor aids, not authority by themselves.
