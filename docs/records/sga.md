...
Current caveat from the 2026-06-24 SGA audit-support updates, with 2026-06-25 hand-certification reconciliation: the latest published record is DOI `10.5281/zenodo.20822648`. SGA5 is explicitly not certified complete. A newer manual certification log demotes agent/workflow passes to finder status rather than certification status: pp.1-45 certified so far (p1-p12, p14-p18, p20-p25, p27-p39, p41-p43 clean; p13 fixed; p19 fixed; p26 fixed with two substantive Tate-twist corrections; p37 fixed with one missed L-i_? occurrence; p40 fixed with one math-level source-typo correction; p44 fixed six systematic missed local-cohomology underlines (including retroactive p41 underlines); p45 fixed `SGA`→`SGAA` and `K_V=j^*K_X`→`j^!K_X`; D001-D005 verified perfect; p33 twist fix verified; p37 L-i_? fixes verified/applied; p38-p40 SGA→SGAA xref fixes verified; p41 (4.5.3)'' xbar paren-placement fix verified; p42 underlined local-cohomology functor fix and Remarque 4.5.5 `R_!f` duality notation verified; p43 closure-node fix `\ol{\{x\}}` verified; p15, p18, p23, p34, and p40 have documented Type-B/source-or-editorial/source-typo conventions; diagram-index page drift remains a known risk), with p46 next. The broader local full-audit workpass remains repair evidence and reports 411 verified corrections, a p1-p484 page-by-page French source audit in the live status note, and a 307-page PDF with no fatal/LaTeX-error diagnostics in the latest observed log scan. Remaining publication/audit work includes final diff/FINDINGS refresh, compact packaging, independent validation, optional low-value prose-fidelity review where desired, and English synchronization. This is preservation/support evidence for ongoing repair, not a certified edition, not an independently packaged or promoted edition, not English synchronization, and not closure of diagram/formula/notation/typography queues. Treat SGA6 and SGA7 as substantial working drafts with explicit compression caveats unless a specific packet declares source-checked coverage. Older filenames containing words such as `Complete`, `Source-Checked`, `Strict`, or `High-Fidelity` are legacy package names, not current global certification. Witness-aid ZIPs are source-witness/anchor aids, not authority by themselves.
