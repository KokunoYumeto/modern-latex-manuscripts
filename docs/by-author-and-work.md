...
|---|---|---|
| Grothendieck and collaborators, SGA | SGA 5/6 and further SGA working translation material, plus source/reference and audit packets. Latest compact SGA5 delta still promotes only p260-p265 as page-local French workpass/source-audit evidence, but later live ledger/provenance checkpoints now record the local SGA5 workpass through p406 in `CERT_LOG.md` / `AGENT_SCORECARD.md`. Exposé VIII is closed at p371; Exposé X is recorded as locally certified through its references at p406, including diagrams D178-D184, the Euler-Poincaré formula, local terms, and A/Λ transcription fixes. Treat p266-p406 as live provenance/workpass metadata only unless later compactly packaged, not SGA5 completion, not English synchronization, and not a critical edition. The next local task is locating Exposé XII in the scan because Exposé XI intervenes and is not in the current TeX branch. SGA6 and SGA7-I remain substantial working drafts with source-compression/detail caveats. Corrections are welcome via GitHub issues or pull requests. | [SGA](https://doi.org/10.5281/zenodo.20410947) |
