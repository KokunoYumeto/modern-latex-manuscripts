...
| Grothendieck and collaborators, SGA | SGA 5/6 and further SGA working translation material, plus source/reference and audit packets. Latest public update 2026-06-24 adds `SGA5_repair032_audit_support_20260624.zip` as repair/audit evidence only. SGA5 is not complete or certified: the later live workpass reports 411 verified corrections and a 307-page PDF with no fatal/LaTeX-error diagnostics in the latest observed log scan, but the newer manual certification log treats agents/workflows as finders and records pp.1-25 as certified so far (pp.1-12, pp.14-18, and pp.20-25 clean; p13 fixed with two source-fidelity corrections; p19 fixed with one source-fidelity correction (`constant` to `constante`); p15 and p18 have documented Type-B source typos handled in TeX; pp.16-18 expose the diagram-index page-drift risk, and D001 is verified perfect on p19), with p26 next and a flagged p26 twist check at line 778 (`R^!i(A_X)⥲(μ_n)^{⊗d}_Y[-2d]`, likely `⊗^{-d}` pending scan verification). Final diff/FINDINGS refresh, compact packaging, continued page-by-page hand certification or independent validation, optional low-value prose-fidelity review where desired, and English synchronization remain open. Diagrams/formulas/notation should be checked against source scans. | [SGA](https://doi.org/10.5281/zenodo.20410947) |
