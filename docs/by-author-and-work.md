...
| Grothendieck and collaborators, SGA | SGA 5/6 and further SGA working translation material, plus source/reference and audit packets. Latest public update 2026-06-24 adds `SGA5_repair032_audit_support_20260624.zip` as repair/audit evidence only. SGA5 is not complete or certified: the later live workpass reports 411 verified corrections and a 307-page PDF with no fatal/LaTeX-error diagnostics in the latest observed log scan, but the newer manual certification log treats agents/workflows as finders and records only pp.1-32 as certified so far (p1-p12, p14-p18, p20-p25, p27-p29, and p32 clean; p13 fixed with two source-fidelity corrections; p19 fixed with one source-fidelity correction; p26 fixed with two substantive Tate-twist corrections (`⊗d` to `⊗−d` for `R^!i(A_X)` and `T`); D001 is verified perfect on p19; D002 on p30 and D003 on p31 are verified perfect; p15 and p18 have documented Type-B source typos handled in TeX; diagram-index page drift remains a known risk), with p33 next. Final diff/FINDINGS refresh, compact packaging, continued page-by-page hand certification or independent validation, optional low-value prose-fidelity review where desired, and English synchronization remain open. Diagrams/formulas/notation should be checked against source scans. | [SGA](https://doi.org/10.5281/zenodo.20410947) |
