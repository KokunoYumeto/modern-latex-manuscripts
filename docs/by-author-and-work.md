...
| Grothendieck and collaborators, SGA | SGA 5/6 and further SGA working translation material, plus source/reference and audit packets. Latest local SGA5 control package is `SGA5_FullAudit_Workpass_Status_p001_p249_20260626.zip`, which preserves the current French workpass PDF/TeX/log plus CERT_LOG, AGENT_SCORECARD, FINDINGS, method/status notes, swarm status, manifest, and checksums. CERT_LOG/AGENT_SCORECARD are the controlling status files: completed rows reach p249, p250 is next, and substantial source reconciliation remains. Recent rows p246-p249 close Expose V Appendix A.2, open A.3, carry Theorem A.3 proof, and verify diagrams D141-D142; D140 keeps its earlier source-verified labels/content but simplified-layout caveat. This is opening-range workpass evidence for a curated SGA5 expose selection, not complete SGA5, not synchronized English, not a critical edition, and not global source-faithfulness. SGA6 and SGA7-I remain substantial working drafts with source-compression/detail caveats. | [SGA](https://doi.org/10.5281/zenodo.20410947) |
