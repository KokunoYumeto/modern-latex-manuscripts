...
- Caveat: do not upload this as a loose standalone public item. The Commons DjVu witnesses are higher-detail best-available page images at 3984 x 5592 px, but the package does not claim native PPI or 650+ PPI certification. Fold it into the next compact Noether source-support rollup only after deliberate pruning/replacement.
- Related local folder: `Noether_R124plus_P13_p255_end_CommonsDjVu_Audit_WebDrop_20260625`, 18 files / 12,450,066 bytes.
- Folder status: loose p255-end source/crop staging only. It has Commons DjVu page witnesses for printed pp.255-258, targeted crops, a copied/current candidate TeX, and a logbook snapshot, but no ZIP, README, confirmed-fixes ledger, source-quality ledger, visual/no-fix ledger, compiled output PDF, checksum manifest, or explicit statement that TeX changes were actually applied. Do not upload or front it until it is packaged, ledgered, compiled, and reviewed.
