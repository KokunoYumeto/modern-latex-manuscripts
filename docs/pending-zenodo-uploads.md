...
- Artifacts: 24 ZIPs, 775,902,608 bytes total. ZIP integrity tests passed.
...
- Important classifications: P17 has one p10 source fix and later no-patch coverage under GDZ600; P24 is a native400 sourcefix chain; P30 is best-available IA400 and the p56-61 package supersedes the p56-60 predecessor for closure; P35/P36 confirms the Cyrillic/Russian block is source-real Paper 35 Resume material; P39 is no-patch under ICM600; the R273 merge promotes only useful P40 pp.518-519 fixes into the newer LocalCodex base and explicitly does not make raw R273 the new base. The newest R273 merge flavor also adds two Paper 8 pp.97-98 source-shape fixes, restoring printed `wo` and a semicolon in `g(\lambda;xy)`.
