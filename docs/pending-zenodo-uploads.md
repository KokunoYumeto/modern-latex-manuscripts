...
- Contents: GDZ full-page source witness/map support for Noether Paper 15, `Die Endlichkeit des Systems der ganzzahligen Invarianten binÃ¤rer Formen`, GÃ¶ttinger Nachrichten 1919, printed pp.138-156. It contains 20 labelled 400-ppi GDZ full-page JPG witnesses, page-map/dimension CSVs, HTML acquisition trace, and source manifest.
...
- Contents: official GDZ source/provenance and page-map support for Paper 05, `Rationale FunktionenkÃ¶rper`, Jahresbericht der Deutschen Mathematiker-Vereinigung 22 (1913), pp.316-319. It includes the GDZ article PDF, IIIF manifest, printed-page/canvas map, raw full-page GDZ JPEGs, dimensions CSV, checksums, and inventory.
...
- Contents: compact GDZ/EuDML source-navigation witness for Noether Paper 34, `Hyperkomplexe GrÃ¶ÃŸen und Darstellungstheorie`, `Mathematische Zeitschrift` 30 (1929), printed pp.641-692. It includes GDZ/EuDML metadata, the GDZ IIIF manifest for volume `PPN266833020_0030`, a printed-page-to-canvas map for pp.641-692 / canvases 00000645-00000696, one full-resolution GDZ JPEG per printed page, a convenience source-witness PDF assembled from those images, image-dimension metadata, README, and checksums.
...
- Contents: targeted German cumulative source-text repair candidate for Noether Paper 33, `Hyperkomplexe GrÃ¶ÃŸen und Darstellungstheorie in arithmetischer Auffassung`, `Atti del Congresso Internazionale dei Matematici`, Bologna, Volume 2, printed/source pp.071-073. It includes cumulative TeX/PDF/log, RA74-to-RA75 diff, IA source/provenance material, original JP2/JPG page witnesses, JP2-derived source slice, rendered QA pages, labelled inspection crops, audit CSVs, method note, README, and checksums.
- Key fix: replaces the active Paper 33 block, which the package reports began with a later Math. Z. introduction, with the Bologna congress source text beginning `Ich mÃ¶chte Ihnen zeigen...`; restores the Bologna author/title block; resets Paper 33 footnotes; inserts the article boundary; and preserves/adjusts the two source matrix displays as editable TeX. XeLaTeX pass 2 produced a 433-page A4 PDF, and local spot render checks of the title/opening and matrix pages were visually sane.
...
- Contents: narrow German cumulative source-header and footnote-counter repair candidate for Noether Paper 31, `Der Diskriminantensatz fÃ¼r die Ordnungen eines algebraischen Zahl- oder FunktionenkÃ¶rpers`, `Journal fÃ¼r die reine und angewandte Mathematik` 157 (1927), pp.82-104. It includes cumulative TeX/PDF/log, RA72-to-RA73 diff, GDZ IIIF manifest and page/canvas map, raw full-resolution article-span JPG witnesses, labelled 1200dpi inspection crops, fix/disposition/source-crop CSVs, method note, README, and checksums.
- Key fix: restores the source author line `Von Emmy Noether in GÃ¶ttingen.` and inserts `\setcounter{footnote}{0}` before Paper 31 so the first source footnote renders as `1)`. No body prose or mathematical content changed. XeLaTeX compiled successfully twice.
...
- Contents: narrow German cumulative source-author-block/page-break repair candidate for Noether/Brauer Paper 32, `Ãœber minimale ZerfÃ¤llungskÃ¶rper irreduzibler Darstellungen`, `Sitzungsberichte der Preussischen Akademie der Wissenschaften` 1927, printed pp.221-228. It includes cumulative TeX/PDF/log, RA73-to-RA74 diff, IA/scandata source mapping, extracted JP2/JPG source leaves, JP2-derived article source PDF, labelled 1200dpi inspection crops, audit/provenance CSVs, method note, README, and checksums.
- Key fix: inserts `\clearpage` before Paper 32, restores the source author block `Von Privatdozent Dr. Richard Brauer` / `in KÃ¶nigsberg` / `und Prof. Dr. Emmy Noether` / `in GÃ¶ttingen.`, and keeps the existing Schur presentation note below the restored author block. The Paper 32 footnote reset already existed in RA73 and was not duplicated. No body prose or mathematical content changed. XeLaTeX compiled successfully twice.
...
- Contents: compact source/provenance witness for Noether/Brauer Paper 32, `Ãœber minimale ZerfÃ¤llungskÃ¶rper irreduzibler Darstellungen`, from `Sitzungsberichte der Preussischen Akademie der Wissenschaften`, 1927, printed/source pp.221-228. It includes the IA source PDF, IA scandata XML, two IA OCR/DJVU text witnesses, the pp.221-228 leaf map, and extracted JP2 plus JPG witnesses for IA leaves 0382-0389.
...
- Contents: narrow German cumulative source-header and footnote-counter repair candidate for Noether Paper 30, `Abstrakter Aufbau der Idealtheorie in algebraischen Zahl- und FunktionenkÃ¶rpern`, `Mathematische Annalen` 96 (1927), pp.26-61. It includes cumulative TeX/PDF/log, RA71-to-RA72 diff, GDZ official article PDF, IIIF manifest and page/canvas map, a raw first-page IIIF witness, labelled 1200dpi inspection crops, fix/disposition/source-crop CSVs, method note, README, and checksums.
- Key fix: restores the source author block `Von` / `Emmy Noether in GÃ¶ttingen.` and inserts `\setcounter{footnote}{0}` before Paper 30 so the first source footnote renders as `1)`. No body prose or mathematical content changed. XeLaTeX compiled successfully twice.
...
- Contents: narrow German cumulative source-header and footnote-counter repair candidate for Noether Paper 29, `Der Endlichkeitssatz der Invarianten endlicher linearer Gruppen der Charakteristik p`, `Nachr. v. d. Ges. d. Wiss. zu GÃ¶ttingen` 1926, printed pp.28-35. It includes cumulative TeX/PDF/log, RA70-to-RA71 diff, GDZ IIIF manifest, raw full-page JPG witnesses, assembled source PDF, labelled 1200dpi inspection crops, fix/disposition/source-crop CSVs, method note, README, and checksums.
- Key fix: restores the source author line `Von Emmy Noether in GÃ¶ttingen.`, keeps the Courant submission line in the source header block, and inserts `\setcounter{footnote}{0}` before Paper 29 so the first source footnote renders as `1)`. No body prose or mathematical content changed. XeLaTeX compiled successfully twice.
...
- Key fix: restores the missing source-visible footnote attached to the Section 9 heading, `Â§ 9. Formenreihen. ReduktionssÃ¤tze`.
...
- Contents: focused off-ledger repair for Weber, `Lehrbuch der Algebra`, Volume II English section 49. B139 fixes a merged display tag where equations (5) and (6) had been collapsed into one display tagged `5--6`; German section 49 and the source scan split them, so the English cumulative and direct Â§49 slice now split them into source-aligned equations (5) and (6). The package includes patched German/English Â§49 slices, cumulative Volume II TeX/PDF, section 49 source scan slice/renders for pp.189-190, render checks, page ledger, heuristic scan/logbook, manifest, and SHA256 sums.
...
## 2026-06-14: Maxwell Vol. I pp.060-079 source-witnessed continuation â€” published 2026-06-24
...
## 2026-06-14: Maxwell Vol. I pp.001-059 web/promo index closeout â€” published 2026-06-24
...
- Contents: German source-audit tranche for Paper 06, `Koerper und Systeme rationaler Funktionen`, Math. Ann. 76 source scan, printed pp.167-172. Package notes report corrected `k(x)` continuation in the Â§2 Hilfssatz, restored compact dotted argument notation, restored source-style left-numbered displays in the Â§Â§3-5 range, converted checked-range footnote markers to source-style page-local symbols, and rebuilt cumulative German TeX/PDF. Workspace status reports Paper 06 pp.161-172 checked, with p.173 onward, full Paper 06 symbolic footnote conversion, and non-German propagation still open.
