...
| [SGA](https://doi.org/10.5281/zenodo.20410947) | 100 | 74 | 25 | 3.23 GB | SGA 5/6 and further SGA working translation material, plus source/reference and audit packets. Latest local SGA5 control package is `SGA5_FullAudit_WebDrop_p001_p254_certified_staged_p255_p484_20260626.zip`, which preserves the active French workpass PDF/TeX/log plus CERT_LOG, AGENT_SCORECARD, method/status notes, source-witness crops/images, inventory, and checksums. The package wrapper is the public boundary: p254 is the current promoted cursor, p255 is next, and substantial source reconciliation remains. Recent rows p250-p254 close Expose V and open Expose VI; p254 closes §1.1.4 / opens §1.2 with Definition 1.2.1 and Z_l(1); diagrams remain tallied D001-D142; p253-p254 have no diagram; D140 keeps the earlier recorded layout simplification. p255-p484 source crops/images are included as continuation witnesses only and are not promoted as certified pages. Embedded p255 log text is not promoted until a later wrapper or deliberate review resolves that boundary. Local ledger terms such as certified/complete are interpreted as page-local workpass status only. This is opening-range workpass evidence for a curated SGA5 expose selection, not complete SGA5, not synchronized English, not a critical edition, and not global source-faithfulness. SGA6 and SGA7-I remain substantial working drafts with source-compression/detail caveats. Corrections are welcome via GitHub issues or pull requests. |
