...
| [SGA](https://doi.org/10.5281/zenodo.20410947) | 100 | 74 | 25 | 3.23 GB | SGA 5/6 and further SGA working translation material, plus source/reference and audit packets. Latest local SGA5 control package is `SGA5_FullAudit_Workpass_Status_p001_p225_20260626.zip`, which preserves the current French workpass PDF/TeX/log plus CERT_LOG, AGENT_SCORECARD, method note, goal snapshot, and file manifest. CERT_LOG/AGENT_SCORECARD are the controlling status files: completed rows reach p225, p226 is next, and substantial source reconciliation remains. This is opening-range workpass evidence for a curated SGA5 expose selection, not complete SGA5, not synchronized English, not a critical edition, and not global source-faithfulness. SGA6 and SGA7-I remain substantial working drafts with source-compression/detail caveats. |
