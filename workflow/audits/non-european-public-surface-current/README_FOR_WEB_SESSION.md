...
## Render/Typographic QC Addendum

A stricter rendered-page QC pass is included in `audit/NON_EU_STRICT_RENDER_QC_SHORTPATH.csv`. It was run from a short temporary path so external PDF tools did not fail on Windows path length. That pass found 60/60 current PDFs render as clean candidates. The remaining work is therefore mostly public-surface quality: source-page labels/process wording, rough al-Battani Arabic/OCR text, typography consistency, blankish pages, and page-edge overflow.

The typography audit is included as `audit/NON_EU_TYPOGRAPHY_AUDIT.csv`.
