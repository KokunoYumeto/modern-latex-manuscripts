...
| `prs_AF` Dari/Persian (Afghanistan) | 4 | 0 | 4 | Three rows remain manual/source-review; Iranian Persian evidence is comparator only. |
...
| `PRS-MOMAND-ALG` | `prs_AF` | eCampus Afghanistan Abdullah Momand algebra PDF, URL `https://ecampus-afghanistan.org/wp-content/uploads/2021/10/Algebra-Abdullah-Momand.pdf`, local `source_canon_witness_cache_20260704/prs_af_ecampus_algebra_momand.pdf`, SHA256 `5145F1EFA0AB4275AD3CBF03C0016A3362D5D7A6EDE3444FDE512719E813D8F4`; eCampus PDF, no open license found. |
...
| `term-fa-ir-0014` | simple | `SOURCE_CANON_SUFFICIENT_FOR_SCOPED_DRAFT_WORK` | ساده | `FA-IUT-ALG`; `FA-PNU-RM`; `FA-SHAHROOD-NC` | Broad common word; object sense must be explicit. | `مدول ساده`, `نمایش ساده`, `حلقه ساده`. | `simple -> ساده`; Dari same surface separately evidenced. | Ready row but context-sensitive. |
...
| `term-fa-ir-0020` | ring | `SOURCE_CANON_SUFFICIENT_FOR_SCOPED_DRAFT_WORK` | حلقه | `FA-PNU-RM`; `FA-SHAHROOD-NC`; `FA-KNTU-ALG` | Strongest fa_IR ring anchor. | `حلقه R`; add `واحددار` only when source requires unit. | `Ring/ring -> حلقه`; Dari same surface separately evidenced. | Ready row. |
...
| Row | Concept | Bucket | Draft rendering | Source witnesses | Alternatives/register notes | Formula-neighboring usage | Interlinear/scaffold | Open acquisition or review note |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `term-prs-af-0001` | algebra | `SOURCE_CANON_SUFFICIENT_FOR_SCOPED_DRAFT_WORK` | جبر | `PRS-MOMAND-ALG`; `PRS-MOHE-CURR` | Ready row; surface matches fa_IR but authority is Afghan source evidence. | `جبر A`; keep Afghan punctuation/spacing where source context requires. | `algebra -> جبر`; compare fa_IR only as a parallel, not authorization. | Ready context-note row. |
| `term-prs-af-0002` | field | `SOURCE_CANON_SUFFICIENT_FOR_SCOPED_DRAFT_WORK` | میدان | `PRS-MOMAND-ALG` | Manual row; do not borrow fa_IR Galois evidence. | `میدان K`; flag algebraic-field sense for Dari reviewer. | `field/Körper -> میدان`; Tajik `майдон` non-row. | Manual/source-review row; acquire stronger Dari field/Galois witness if found. |
| `term-prs-af-0003` | simple | `SOURCE_CANON_SUFFICIENT_FOR_SCOPED_DRAFT_WORK` | ساده | `PRS-MOMAND-ALG` | Low-count row; broad common word. | `مدول ساده` or `گروپ ساده` only if local source context supports object. | `simple -> ساده`; same surface as fa_IR but separately weakly evidenced. | Manual/source-review row; source acquisition should seek stronger Dari simple-module/group context. |
| `term-prs-af-0004` | ring | `SOURCE_CANON_SUFFICIENT_FOR_SCOPED_DRAFT_WORK` | حلقه | `PRS-MOMAND-ALG`; `PRS-MOHE-CURR`; `PRS-KNU-LA` as routing only | Source also has high `رينگ`; reviewer must choose `حلقه` vs `رینگ/رينگ`. | `حلقه R`; classroom register may prefer source-visible `رينگ`. | `ring/Ring -> حلقه / رينگ review`; fa_IR cannot settle this. | Manual/source-review row; acquire clearer Afghan ring-definition page if possible. |
