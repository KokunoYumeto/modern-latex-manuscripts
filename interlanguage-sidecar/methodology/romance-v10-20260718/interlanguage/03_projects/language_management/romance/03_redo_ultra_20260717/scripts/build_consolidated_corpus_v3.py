# Successor build: preserves v2 and adds the reviewed 2024 Rumantsch Grischun sources.
...
write_csv(ROOT/'corpus'/'ROMANCE_CONSOLIDATED_CORPUS_v3.csv',rows,fields)
write_csv(ROOT/'corpus'/'ROMANCE_CORPUS_REJECTED_OR_EXCLUDED_v3.csv',rejected,['source','locator','reason'])
...
write_csv(ROOT/'corpus'/'ROMANCE_CORPUS_LANGUAGE_DOMAIN_COVERAGE_v3.csv',coverage)

summary={
    'artifact':'ROMANCE_CONSOLIDATED_CORPUS_v3','record_count':len(rows),'primary_unique_count':sum(r['dedupe_status']=='primary_unique' for r in rows),
    ...
    'romansh_boundary':'Four downloaded automatic-search false hits remain quarantined. The three active Romansh records are individually verified official Rumantsch Grischun school-mathematics examination parts from 2021 and 2024; none is specialist algebra evidence.',
    'manifest_sha256':hash_file(ROOT/'corpus'/'ROMANCE_CONSOLIDATED_CORPUS_v3.csv'),
}
(ROOT/'corpus'/'ROMANCE_CONSOLIDATED_CORPUS_v3.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')

assert {'es','fr','pt','gl','ca','it','ro'}.issubset({r['language'] for r in rows})
assert sum(r['language']=='rm' and r['counting_eligible']=='true' for r in rows)==3
...
log=[f"PASS consolidated_records={len(rows)}",f"primary_unique={summary['primary_unique_count']}",f"excluded={len(rejected)}",f"coverage_rows={len(coverage)}",'romansh_active_bodies=3','romansh_specialist_algebra_bodies=0',f"manifest_sha256={summary['manifest_sha256']}"]
(ROOT/'qa'/'CORPUS_BUILD_v3.log').write_text('\n'.join(log)+'\n',encoding='utf-8')
print('\n'.join(log))
