...
$tex=Join-Path $tranche 'tex\R823_HG_T002_romance.tex'
...
$console=Join-Path $build 'R823_HG_T002_lualatex_console.log'
...
$pdf=Join-Path $build 'R823_HG_T002_romance.pdf'
& (Join-Path $poppler 'pdfinfo.exe') $pdf | Set-Content -Encoding utf8 (Join-Path $qa 'R823_HG_T002_pdfinfo.txt')
if($LASTEXITCODE -ne 0){throw 'pdfinfo failed'}
& pdftotext -layout $pdf (Join-Path $qa 'R823_HG_T002_extracted.txt')
if($LASTEXITCODE -ne 0){throw 'pdftotext failed'}
& (Join-Path $poppler 'pdftoppm.exe') -png -r 150 $pdf (Join-Path $render 'R823_HG_T002_page') | Out-Null
if($LASTEXITCODE -ne 0){throw 'pdftoppm failed'}
Copy-Item -LiteralPath $pdf -Destination (Join-Path $final 'R823_HG_T002_controlled_romance.pdf') -Force
