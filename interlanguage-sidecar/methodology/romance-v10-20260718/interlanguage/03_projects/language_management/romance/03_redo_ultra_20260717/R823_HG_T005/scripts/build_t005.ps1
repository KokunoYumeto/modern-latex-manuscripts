...
if ((Split-Path -Leaf $tranche) -ne 'R823_HG_T005') { throw "Unexpected tranche path: $tranche" }
$repo = (Resolve-Path (Join-Path $tranche '..\..\..\..\..')).Path
$tex = Join-Path $tranche 'tex\R823_HG_T005_romance.tex'
...
Get-ChildItem -LiteralPath $render -File -Filter 'R823_HG_T005_page-*.png' -ErrorAction SilentlyContinue | Remove-Item -Force

$console = Join-Path $build 'R823_HG_T005_lualatex_console.log'
$passone = Join-Path $build 'R823_HG_T005_lualatex_pass1.log'
...
$pdf = Join-Path $build 'R823_HG_T005_romance.pdf'
$texlog = Join-Path $build 'R823_HG_T005_romance.log'
...
& (Join-Path $poppler 'pdfinfo.exe') $pdf | Set-Content -Encoding utf8 (Join-Path $qa 'R823_HG_T005_pdfinfo.txt')
if ($LASTEXITCODE -ne 0) { throw 'pdfinfo failed' }
& $pdftotext -layout $pdf (Join-Path $qa 'R823_HG_T005_extracted.txt')
if ($LASTEXITCODE -ne 0) { throw 'pdftotext failed' }
& (Join-Path $poppler 'pdftoppm.exe') -png -r 150 $pdf (Join-Path $render 'R823_HG_T005_page') | Out-Null
if ($LASTEXITCODE -ne 0) { throw 'pdftoppm failed' }
Copy-Item -LiteralPath $pdf -Destination (Join-Path $final 'R823_HG_T005_controlled_romance.pdf') -Force
