# OTHER-PC SESSION MANAGER AND LOGBOOK DIRECTIVE

This is a duplicate visibility copy of:

`coordination/OTHER_PC_SESSION_MANAGER_AND_LOGBOOK_DIRECTIVE_20260705.md`

Other-PC / interlanguage / Interslavic sessions must read the coordination copy as governing. The core rules are:

1. keep a durable session logbook;
2. do not let sibling sessions sit idle without assigned tasks;
3. upload literal source bodies, not only indexes or ledgers;
4. apply Fable/ChatGPT-Pro branch-weighting and marginal-intelligibility findings;
5. create an acknowledgement file under `coordination/acks/`;
6. classify outputs conservatively.

If this duplicate and the coordination copy ever differ, the coordination copy controls.
