...
Eddington \index{Eddington, Arthur \yrange{1882}{1944}} ilustrirao riječima
\begin{quote}
...
\end{quote}
Ova karakterizacija teorije grupa kao "super-matematike" je možda malo pretjerana, jer
teorija grupa je čvrsti dio standardne matematike.
Isto tako, cilj teorije grupa nije zamagliti strukturu
tako da ni ne znamo što radimo, nego izdvojiti bitna svojstva koja su zajednička
mnogim različitim objektima matematike i fizike.
...
\begin{definicija}[Grupa]
    \label{def:grupa}  \index{Grupa!definicija}
Grupa $(G,\star)$ je skup objekata $G$ na kojem je definirana binarna operacija
 $\star$ tako da su zadovoljena sljedeća svojstva:
\begin{enumerate}
\item $g_1 \star g_2 \in G \quad \forall  g_1, g_2 \in G$  (zatvorenost) \index{Zatvorenost!kao aksiom grupe}
\item $ g_1 \star (g_2 \star g_3) = (g_1 \star  g_2)\star g_3 \quad \forall 
    g_1, g_2, g_3 \in G$  (asocijativnost) \index{Asocijativnost!kao aksiom grupe}
\item $\exists e \in G \td g\star e = e \star g = g \quad \forall g\in G$
    (egzistencija identiteta) \index{Identitet!kao aksiom grupe}
    \index{Element!jedinični|see{Identitet}}
\item $\forall g\in G \;\; \exists g^{-1} \td g\star g^{-1} = g^{-1}\star g =e$
    (egzistencija inverza za svaki element) \index{Inverz!kao aksiom grupe}
    \index{Element!suprotni|see{Inverz}}
\end{enumerate}
\end{definicija}
...
\begin{primjer}[$\mathbb{Z}$, +]
 Lako se uvjeriti da skup cijelih brojeva $\mathbb{Z}$ 
 \index{$\mathbb{Z}$!kao primjer grupe}
 \index{Skup cijelih brojeva|see{$\mathbb{Z}$}}
 čini grupu obzirom
...
\index{$\mathbb{Z}$!kao protuprimjer grupe}
...
Naime, element $m$ bi bio inverz od $n$ kada bi vrijedilo $mn=1$, ali takav $m=1/n$
ne pripada $\mathbb{Z}$ osim za specijalne slučajeve
...
\index{Opća linearna grupa|see{GL($n$)}}
\index{GL($n$)}
\end{primjer}

Grupu zovemo \emph{Abelova (abelovska, komutativna)} ako \index{Grupa!Abelova}
\index{Abel, Niels \yrange{1802}{1829}}
$\forall g_1, g_2 \in G$ vrijedi $g_1 \star g_2 = g_2 \star g_1$.
Na primjer ($\mathbb{Z}$, +) je Abelova dok grupa GL(2) to nije jer množenje
matrica općenito ne komutira.
Broj elemenata grupe nazivamo \emph{red grupe}. \index{Grupa!red}
Obzirom na njega razlikujemo:\index{Grupa!klasifikacija obzirom na red}
\begin{itemize}
...
\item \emph{Beskonačne diskretne grupe} koje imaju 
    prebrojivo\footnote{Beskonačan skup je \emph{prebrojiv} \index{Prebrojiv skup} ukoliko je moguće uspostaviti
...
\index{Liejeva grupa!beskonačna kontinuirana}
Primjeri su ($\mathbb{R}\backslash\{0\}$, $\cdot$) i GL(2).
\end{itemize}

\begin{primjer}[Grupa simetrija] \index{Grupa!simetrija}
...
\item zatvorenost: ako ni $g_1$ ni $g_2$ ne mijenjaju objekt, ne mijenja
    ga ni njihova kompozicija $g_2 \circ g_1$ \index{Zatvorenost!grupe simetrija}
\item asocijativnost: transformacije možemo interpretirati kao \index{Asocijativnost!grupe simetrija}
...
\item identiteta: transformaciju koja ne radi ništa tj. ne mijenja objekt
      također smatramo transformacijom \index{Identitet!grupe simetrija}
\item suprotna transformacija je isto simetrija (dobra vježba iz apstraktnog \index{Inverz!grupe simetrija}
...
značaja. To će nam kao prvo omogućiti elegantnu konstrukciju mnogih važnih
apstraktnih grupa, a kasnije ćemo često promatrati grupe simetrija konkretnih fizikalnih
objekata.
...
\begin{primjer}[Ciklička grupa C$_n$] \index{C$_n$}
    \index{Ciklička grupa|see{C$_n$}}
...
Ukoliko označimo "generirajuću" rotaciju grupe C$_n$ (onu za kut 2$\pi/n$)
\index{Grupa!generator (konačne) grupe}
simbolom $c$ onda je skup elemenata grupe
\begin{equation}
 \{c,c^2, \ldots , c^n=e \} \;,
\label{eq:cn}
\end{equation}
...
\index{Grupa!prezentacija grupe}
...
\index{Cayleyjeva tablica|see{Tablica množenja}}
\index{Cayley, Arthur \yrange{1821}{1895}}
...
množenja: \index{Tablica množenja (grupna)} \index{Grupa!tablica množenja}
\vspace{1ex}
\begin{equation}
\begin{array}{c|ccc}
...
kvadrata poznato i kao \index{Latinski kvadrat} \index{Teorem!o razmještaju}
...
\index{Asocijativnost!i grupna tablica množenja}
...
\index{Asocijativnost!Lightov test}
...
\index{Točkasta grupa|see{Kristal}}
\index{Kristal}
...
Može se pokazati da jedini elementi točkastih grupa mogu biti kompozicije
rotacija za 60$^\circ$ i 90$^\circ$ te inverzije $\vec{r}\to -\vec{r}$
(tzv. \emph{teorem kristalografske restrikcije}).
\index{Teorem!kristalografske restrikcije}
   

\begin{primjer}[Dihedralna grupa $\mathrm{D}_3$]
    \index{D$_3$}
    \index{Dihedralna grupa|see{D$_n$}}
...
dodatno rotacije $b_1, b_2$ i $b_3$ za kut od 180$^\circ$ oko triju osi
koje prolaze kroz središte i vrhove trokuta.
\end{primjer}
...
se iz slike: \index{Element!konjugirani}
...
$b_2 = c b c^{-1} = b c$ i pomnožimo zdesna s $c$:
\begin{equation}
c b = b c^2  \;.
\label{eq:d3com}
\end{equation}
...
\index{Kleinova četvorna grupa|see{D$_2$}} \index{D$_{2}$}
za koju nemamo odgovarajući pravilni poligon kao objekt grupe simetrija.


\section{Podgrupe}
\label{sec:podgrupe}

Za operacije s grupama i njihovu klasifikaciju važan je pojam \emph{podgrupe}:
\index{Podgrupa}
\begin{definicija}[Podgrupa]
\index{Podgrupa!definicija}
...
\index{Teorem!o razmještaju}
pa tako definiraju jednu particiju grupe G, prikazanu na slici \ref{fig:lagrange}.
\begin{figure}[htpb]
    \centering
    \input{figures/lagrange.tex}
    \caption{Particija grupe na lijeve susjedne klase po podgrupi $H$.}
    \label{fig:lagrange}
\end{figure}
...
Promatrajući sliku \ref{fig:lagrange} lako se uvjerimo u točnost Lagrangeovog
teorema: 

\begin{teorem}[Lagrange]
    \index{Teorem!Lagrangeov} \index{Grupa!red} \index{Lagrange, Joseph-Louis \yrange{1736}{1813}}
Red podgrupe dijeli red grupe.
\end{teorem}

Tako u Primjeru \ref{th:c2c3} redovi podgrupa C$_2$ i C$_3$, 2 i 3, dijele red
grupe D$_3$, 6. \index{C$_2$} \index{C$_3$}
Skup čiji su elementi predstavljeni kvadratičima na gornjoj slici
ima svoj naziv:

\begin{definicija}[Kvocijentni skup]
    \index{Kvocijentni skup}
...
\begin{definicija}[Normalna podgrupa]
    \index{Normalna podgrupa} \index{Invarijantna podgrupa|see{Normalna podgrupa}}
    \index{Podgrupa!normalna|see{Normalna podgrupa}}
    \index{Podgrupa!invarijantna|see{Normalna podgrupa}}
...
\index{Normalna podgrupa} \index{Kvocijentna grupa}
...
skupovi
\begin{align*}
    {\rm D}_{3}/{\rm C}_2 &= \{\{e,b\}, \{c, cb=bc^2\}, \{c^2, c^2b=bc\}\} \;\text{i} \\
...
Grupe koje nemaju normalnih podgrupa zovu se \emph{jednostavne}\footnote{Mi se
    ...
    da je \'{E}variste Galois \index{Galois, \'{E}variste \yrange{1811}{1832}}
    povezao njihovu općenitu rješivost (mogućnost
...
\index{Grupa!Abelova} \index{Grupa!jednostavna} \index{Grupa!polujednostavna}
a one koje nemaju normalnih Abelovih podgrupa zovu se \emph{polujednostavne}.
Kako posjedovanje normalne podgrupe znači mogućnost "dijeljenja" grupe na kvocijentnu
grupu, jednostavne grupe su u nekom smislu grupno-teorijski pandan prostim brojevima
u aritmetici.
...
Konjugacija je također relacija ekvivalencije:
    \index{Klasa ekvivalencije} \index{Klasa konjugacije}
\begin{enumerate}
...
\index{Identitet!kao klasa konjugacije}

Za Abelove grupe svaki element je klasa za sebe. \index{Grupa!Abelova}

Kako normalne podgrupe zadovoljavaju $gHg^{-1}=H$, $\forall g\in G$, slijedi
da su one sastavljenje od cijelih klasa konjugacije.

\begin{primjer}[Klase konjugacije grupe D$_3$] \label{pr:klaseD3}
Tri klase konjugacije  od D$_3$ su: \index{D$_3$}
\begin{enumerate}
\item $(e)$
\item $(c, c^{2})$
\item $(b, bc, bc^2)$
...
se naziva \emph{homomorfizam}. \index{Homomorfizam!definicija}
...
\begin{primjer}  \index{D$_3$} \index{C$_3$}
...
\begin{primjer}  \index{$\mathbb{Z}_n$} \index{$\mathbb{Z}_2$} \index{C$_n$}
...
\index{Homomorfizam!kernel}  \index{Kernel}
...
kako je ilustrirano na slici \ref{fig:homo1}.
\begin{figure}[ht]
    \centering
    \input{figures/homo1.tex}
    \caption{\label{fig:homo1}}
\end{figure}
\begin{figure}[ht]
    \centering
    \input{figures/homo2.tex}
    \caption{\label{fig:homo2}}
\end{figure}
...
kako je ilustrirano na slici \ref{fig:homo2}.
...
\emph{obje} podgrupe normalne, što u primjeru $\mathrm{D}_3$ nije slučaj.

\begin{definicija}[Direktni produkt grupa]\label{def:direktniproduktgrupa}
    \index{Direktni produkt grupa}
...
broj elemenata grupa.)\label{zad:Dn} \index{D$_n$}
...
\index{Centar grupe} \index{Grupa!centar}
\item Odredite klase konjugacije grupe D$_4$. \index{D$_4$}
...
\index{$\mathbb{Z}$} \index{C$_2$}
\item Promotrite opću linearnu grupu nesingularnih kvadratnih $n\times n$ 
    matrica $\GL{n}=\{M\}$. Pokažite da je $f(M)=\det M$ homomorfizam.
Koja grupa je slika tog homomorfizma?
    \index{GL($n$)}
\end{enumerate}
