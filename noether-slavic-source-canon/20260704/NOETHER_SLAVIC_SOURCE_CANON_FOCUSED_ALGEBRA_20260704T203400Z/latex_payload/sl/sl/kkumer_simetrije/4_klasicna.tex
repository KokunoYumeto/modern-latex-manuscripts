...
\index{Invarijantnost}
\index{Kovarijantnost}
...
U prvom redu smo iskoristili činjenicu da $R$ ne ovisi o vremenu,
a u zadnjem da se iznos vektora pri rotacijama ne mijenja.
...
sliku \ref{fig:aktivne}). U \emph{pasivnom} \index{Transformacija!aktivna i pasivna}
...
\begin{equation}
       m  \ddot{x}_{i} =  F_{i} \;.
\end{equation}
\index{Invarijantnost} \index{Kovarijantnost}
...
\index{Maxwellove jednadžbe} \index{Maxwell, James \yrange{1831}{1879}}
\begin{align}
...
\index{Vektor}
\begin{center}
    \emph{vektor} $\equiv$ {objekt koji se pri rotacijama transformira množenjem matricom}
    $R(\hat{\vec{n}}, \theta)$.
\end{center}
Ovo se poopćuje na veličine sa složenijim transformacijskim svojstvima,
\emph{tenzore}.  \index{Tenzor!definicija}
...
Kad se tip transformacije ne specificira obično se misli se na rotacije.
    \index{Tenzor!Lorentzov}

\begin{primjer}[Tenzor elektromagnetskog polja]
    \index{Tenzor!elektromagnetskog polja}
...
\begin{primjer}[Riemannov tenzor]
    \index{Tenzor!Riemannov} \index{Riemann, Bernhard \yrange{1826}{1866}}
...
\begin{primjer}[Vremenska inverzija]
    \index{Vrijeme!inverzija|see{Inverzija vremena}}  \index{Inverzija vremena}
...
\index{Vrijeme!strijela vremena}
...
\index{Lagrange, Joseph-Louis \yrange{1736}{1813}}
\index{Hamilton, William \yrange{1805}{1865}}
Naime, centralni objekt te formulacije je integral lagranžijana tj. akcija
\index{Akcija}
\begin{equation}
S = \int_{t_0}^{t_1} \rmd t\, L \,,
\label{eq:akcija}
\end{equation}
...
\index{Vrijeme!u teoriji relativnosti}
\index{Lagranžijan} 
...
\index{Euler-Lagrangeove jednadžbe}  \index{Euler, Leonhard \yrange{1707}{1783}}
\begin{equation}
...
grupu kontinuiranih transformacija onda postoji $n$ konstanti gibanja.
\index{Teorem!Noetherin}  \index{Noether, Emmy \yrange{1882}{1935}}
\index{Invarijantnost} \index{Zakon očuvanja}
\end{teorem}
...
Hilbertovom prostoru stanja\footnote{Druga mogućnost je da operatori
    ...
    uvijek uključuju inverziju vremena čime se nećemo baviti u ovoj
    knjizi.\index{Inverzija vremena}}.
...
\index{Teorem!Wignerov} \index{Hilbertov prostor}

\subsection{Kontinuirane prostorne translacije}
\label{sec:prostornetranslacije}
\index{Translacija!u prostoru}

Poznato je, vidi dodatak \ref{sec:qm}, da je konkretno kvantno stanje
...
\index{Transformacija!aktivna i pasivna}
...
\begin{equation}
 ...
 \label{eq:optrans}
\end{equation}
Sada, pretpostavljajući da je $\psi_{\alpha}(x,t)$ dovoljno glatka funkcija,
lijevu stranu jednadžbe (\ref{eq:optrans}) razvijemo u Taylorov red
i formalno zbrojimo članove
\begin{equation*}
\begin{split}
...
U_r(a)= \rme^{-a \pd / \pd  x} \,,
\end{equation}
a u trodimenzionalnom prostoru ćemo imati
\begin{equation}
U_r(\vec{a})= \rme^{-\vec{a}\cdot\nabla} \;.
\end{equation}
Poznavajući kvantnomehaničku korespondenciju $-\rmi\hbar\nabla \leftrightarrow
\vec{p}$ imamo konačno općeniti (neovisan o bazi) operator translacije
\begin{equation}
U_r(\vec{a})= \rme^{-(\rmi/\hbar)\vec{a}\cdot\vec{p}} \;.
\label{eq:Ur}
\end{equation}
...
napisati u obliku
\begin{equation}
    U_{r}(\vec{\epsilon}) = 1 - \frac{\rmi}{\hbar} \vec{p}\cdot\vec{\epsilon} + O(\epsilon^2) \;,
...
\begin{equation}
    \lim_{N\to\infty} \left(1 - \frac{\rmi}{\hbar} \frac{\vec{p}\cdot\vec{a}}{N} \right)^N
        = \rme^{-(\rmi/\hbar)\vec{a}\cdot\vec{p}} \;,
\end{equation}
što je upravo operator translacije $U_r(\vec{a})$ iz (\ref{eq:Ur}).
\index{Operator!translacije u prostoru} \index{Translacija!u prostoru}
...
Daljnja razmatranja trebaju pokazati
...
\index{Translacija!u vremenu} \index{Vrijeme!translacija}
...
{\pd t}+\ldots \\
&=\rme^{-\tau \pd / \pd  t} \psi_{\alpha}(x,t) \;,
\end{split}
\end{equation}
te kvantnomehanička korespondencija $\rmi\hbar\pd /\pd t \leftrightarrow H$
daje
\begin{equation}
U_t(\tau)= \rme^{(\rmi/\hbar)H\tau} \;.
\end{equation}
...
\index{Vrijeme!evolucija vs. translacija}
...
\begin{equation}
\psi_{\alpha}(x,t-\tau)=\rme^{(\rmi/\hbar)H\tau}\psi_{\alpha}(x,t)
\end{equation}
pa zamjenom $\tau\equiv t-t'$ imamo 
\begin{equation}
\psi_{\alpha}(x,t')=\rme^{-(\rmi/\hbar)H(t'-t)}\psi_{\alpha}(x,t)
\end{equation}
Vrijedi dakle\footnote{Razlikovanje operatora vremenske translacije i evolucije nije
...
\begin{equation}
  A\psi(x,t')= A \rme^{-(\rmi/\hbar)H(t'-t)} \psi(x,t) =
 \rme^{-(\rmi/\hbar)H(t'-t)} A \psi(x,t) = a \psi(x,t')
\end{equation}
...
\vec{r}, t) \\
 ...
 &= \psi_{\alpha}(\vec{r}, t)-\frac{\rmi}{\hbar}(\delta\phi\hat{\vec{n}}\times
  \vec{r})\cdot \vec{p}\psi_{\alpha}(\vec{r}, t) \\
 &= \left(1-\frac{\rmi}{\hbar}\vec{L}\cdot\hat{\vec{n}}\delta\phi \right)
  \psi_{\alpha}(\vec{r}, t) \,.
\end{split}
\end{equation}

Rotaciju za konačni kut dobijemo kompozicijom N rotacija za $\delta\phi = \phi / N$
gdje $N\to\infty$: \index{Operator!rotacije}

\begin{equation}
D(\hat{\vec{n}}, \phi)=\lim_{N\to\infty}
\left(1-\frac{\rmi}{\hbar}\vec{L}\cdot\hat{\vec{n}}\delta\phi \right)^N
= \rme^{-(\rmi/\hbar)\vec{L}\cdot\hat{\vec{n}}\phi} \,.
\label{eq:rotD}
\end{equation}
...
\begin{equation}
    [L_i, L_j] = \rmi\hbar \sum_{k} \epsilon_{ijk}  L_k \,,
    \label{eq:Lkomutacija}
\end{equation}
gdje smo upotrijebili Levi-Civitin ili totalno antisimetrični pseudotenzor\footnote{Za
...
\index{Tenzor!Levi-Civitin}  \index{Levi-Civitin tenzor|see{Tenzor!Levi-Civitin}}
\index{Levi-Civita, Tullio \yrange{1873}{1941}}
...
\delta\phi \hat{\vec{n}}\times\vec{\Psi}(\vec{r},t) + O(\delta\phi^2) \\
 &= \vec{\Psi}(\vec{r}, t)-\frac{\rmi}{\hbar}(\delta\phi\hat{\vec{n}}\cdot
...
definiranih pomoću Levi-Civitinog pseudotenzora (vidi zadatak \ref{zad:levicivita}):
\begin{equation}
(S_j)_{ik} = \rmi\hbar\epsilon_{ijk} \quad j=1,2,3 \;.
\end{equation}
Konkretno
\begin{align}
S_1& =\rmi\hbar
...
0 & 0 &-1 \\
0 & 1 & 0
\end{pmatrix} = \rmi\hbar X_1 \,,\\
S_2& = \rmi\hbar
\begin{pmatrix}
0 & 0 & 1 \\
0 & 0 & 0 \\
-1 & 0 & 0
\end{pmatrix} = \rmi\hbar X_2 \,, \\
S_3& = \rmi\hbar
\begin{pmatrix}
0 & -1 & 0 \\
1 & 0 & 0\\
0 & 0 & 0
\end{pmatrix} = \rmi\hbar X_3 \,.
\end{align}
...
&= -\frac{\rmi}{\hbar}\hat{n}_j (S_j)_{ik}\Psi_k\\
&= -\frac{\rmi}{\hbar}(\hat{\vec{n}}\cdot\vec{S})_{ik}\Psi_k \;,
\end{split}
\end{equation}
što znači
\begin{equation*}
D(\vec{\hat{n}},\phi)\vec{\Psi}(\vec{r}, t)=
 \left(1-\frac{\rmi}{\hbar}(\vec{L}+\vec{S})\cdot\hat{\vec{n}}\delta\phi \right)
  \Psi(\vec{r}, t) \,,
\end{equation*}
odnosno, operator konačne rotacije ovakvog kvantnog sustava jest
\begin{equation}
D(\hat{\vec{n}}, \phi)=
 \rme^{-(\rmi/\hbar)(\vec{L}+\vec{S})\cdot\hat{\vec{n}}\phi} \,.
 \label{eq:rotDJ}
\end{equation}
...
\index{Spin}  \index{Moment impulsa!orbitalni} 
\index{Moment impulsa!spinski|see{Spin}} \index{Zakon očuvanja}
...
vrlo specifičnu situaciju, tzv. česticu spina 1 (točnije, spina $\hbar$),
...
\section{Primjena: \emph{Blochov teorem}}
\index{Teorem!Blochov}
\index{Bloch, Felix \yrange{1905}{1983}}
...
jediničnu ćeliju kristalne rešetke. Prilikom izračuna valne funkcije
elektronskog
...
\end{equation}
\begin{equation}
U_{r}(\vec{a}_1) = \exp\Big\{-2\pi \rmi\: \frac{p_1 }{N_1}\Big\} \qquad 
...
\begin{align}
 ...
 &= \exp\Big\{ -2\pi \rmi \Big( \frac{n_1 p_1}{N_1} + \frac{n_2 p_2}{N_2} 
 + \frac{n_3 p_3}{N_3} \Big)\Big\} \;,
\label{blochirreps}
\end{align}
tj. s $N$ različitih operatora kako i očekujemo za grupu s $N$ ireducibilnih
reprezentacija.
Svaka od tih reprezentacija se onda može označiti ("labelirati") s trojkom brojeva $(p_1, p_2, p_3)$
gdje svaki od brojeva $p_j$ može poprimiti bilo koju vrijednost iz
skupa $\{0, 1, 2$, $\ldots$, $N_j -1\}$.
Operator translacije $U_{r}(\vec{t}_{\vec{n}})$ je u konkretnoj reprezentaciji
 $(p_1, p_2, p_3)$ reprezentiran operatorom/brojem (\ref{blochirreps}).
 \index{Operator!translacije u prostoru} \index{Translacija!u prostoru}

Umjesto trojke $(p_1, p_2, p_3)$ za označavanje ireducibilnih reprezentacija se obično
...
\label{defb}
\end{equation}
Sada vektor $\vec{k}$ koji odgovara reprezentaciji
...
operator translacije za $\vec{t}_{\vec{n}}$ u reprezentaciji $\vec{k}$
dan s
\begin{equation}
 U_{r}^{(\vec{k})} (\vec{t}_{\vec{n}}) = 
 \rme^{-\rmi \vec{k}\cdot \vec{t}_{\vec{n}}} \;,
\end{equation}
i odsad $N$ različitih $\vec{k}$-ova (\ref{defk}) labelira ireducibilne reprezentacije.
Vidimo da dimenzija $\vec{k}$ odgovara valnom broju, tj. impulsu
podijeljenom s Planckovom konstantom.

Djelovanje ovih operatora translacije na valne funkcije u datoj
reprezentaciji mora biti kao i kod svih operatora translacije (\ref{eq:optrans})
\begin{equation}
 \rme^{-\rmi \vec{k}\cdot \vec{t}_{\vec{n}}} \psi_{\vec{k}}(\vec{r}) =
...
$\vec{k}$ je Blochov valni vektor. \index{Blochove funkcije}
...
$\rme^{+\rmi \vec{k}\cdot \vec{r}} u_{\vec{k}}(\vec{r})$,
...
svojstvene funkcije. To onda povlači Blochov teorem.
\begin{teorem}[Bloch]
Valne funkcije periodičke rešetke mogu se izabrati kao Blochove funkcije.
\index{Teorem!Blochov}
\end{teorem}

Blochov teorem olakšava rješavanje Schr\"{o}dingerove jednadžbe
...
Teorem se obično izvodi i u udžbenicima fizike čvrstog
...
\item \label{zad:kronecker} Kroneckerov simbol $\delta_{ij}$, $i,j \in \{1, 2, 3\}$ je tenzor drugog ranga
    ...
    također i \emph{invarijantan}.\index{Kovarijantnost} \index{Invarijantnost}
\item Pokažite da je operator translacije kvantnomehaničkog stanja,
 $U_{r}(\vec{a})$ = $\exp (-\frac{\rmi}{\hbar} \vec{p}\cdot \vec{a} )$
...
jednadžbu ako je $[H, \vec{p}] = 0$

\item Koristeći fundamentalne kvantnomehaničke komutatore
\begin{equation}
 [x_i, p_j] = \rmi\hbar \delta_{ij}\;, \qquad
 [x_i, x_j] = [p_i, p_j] = 0 \;,
\end{equation}
te svojstva Levi-Civitinog tenzora, izračunajte komutacijske
...
\item \label{zad:levicivita} Uvjerite se da se pomoću Levi-Civitinog tenzora mogu elegantno zapisati
